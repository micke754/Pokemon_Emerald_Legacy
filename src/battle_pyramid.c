...
static bool8 TrySetPyramidObjectEventPositionAtCoords(bool8, u8, u8, u8 *, u8,
                                                      u8);
...
static const u16
    sPickupItemsLvl50[TOTAL_PYRAMID_ROUNDS][PICKUP_ITEMS_PER_ROUND] = {
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_APICOT_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_MAX_REVIVE, ITEM_BRIGHT_POWDER, ITEM_SHELL_BELL,
         ITEM_MAX_REVIVE, ITEM_SACRED_ASH},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_GANLON_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_REVIVE, ITEM_LEFTOVERS, ITEM_CHOICE_BAND,
         ITEM_FULL_RESTORE, ITEM_MAX_ELIXIR},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_PETAYA_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_MAX_REVIVE, ITEM_SCOPE_LENS, ITEM_FOCUS_BAND,
         ITEM_MAX_REVIVE, ITEM_SACRED_ASH},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_LIECHI_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_REVIVE, ITEM_QUICK_CLAW, ITEM_KINGS_ROCK,
         ITEM_FULL_RESTORE, ITEM_MAX_ELIXIR},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_SALAC_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_MAX_REVIVE, ITEM_BRIGHT_POWDER, ITEM_SHELL_BELL,
         ITEM_MAX_REVIVE, ITEM_SACRED_ASH},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_MAGO_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_REVIVE, ITEM_LEFTOVERS, ITEM_CHOICE_BAND,
         ITEM_FULL_RESTORE, ITEM_MAX_ELIXIR},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_IAPAPA_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_MAX_REVIVE, ITEM_SCOPE_LENS, ITEM_FOCUS_BAND,
         ITEM_MAX_REVIVE, ITEM_SACRED_ASH},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_WIKI_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_REVIVE, ITEM_QUICK_CLAW, ITEM_KINGS_ROCK,
         ITEM_FULL_RESTORE, ITEM_MAX_ELIXIR},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_AGUAV_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_MAX_REVIVE, ITEM_BRIGHT_POWDER, ITEM_SHELL_BELL,
         ITEM_MAX_REVIVE, ITEM_SACRED_ASH},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_FIGY_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_REVIVE, ITEM_LEFTOVERS, ITEM_CHOICE_BAND,
         ITEM_FULL_RESTORE, ITEM_MAX_ELIXIR},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_APICOT_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_MAX_REVIVE, ITEM_SCOPE_LENS, ITEM_FOCUS_BAND,
...
static const u16
    sPickupItemsLvlOpen[TOTAL_PYRAMID_ROUNDS][PICKUP_ITEMS_PER_ROUND] = {
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_APICOT_BERRY, ITEM_MAX_ETHER,
         ITEM_LUM_BERRY, ITEM_MAX_REVIVE, ITEM_BRIGHT_POWDER, ITEM_SHELL_BELL,
         ITEM_MAX_REVIVE, ITEM_SACRED_ASH},
        {ITEM_HYPER_POTION, ITEM_FLUFFY_TAIL, ITEM_GANLON_BERRY, ITEM_MAX_ETHER,
...
static const u8 sPickupItemOffsets[FRONTIER_STAGES_PER_CHALLENGE] = {
    0, 9, 18, 27, 36, 45, 54};

static const struct PyramidTrainerEncounterMusic
    sTrainerClassEncounterMusic[54] = {
...
static const u8 sHintTextTypes[] = {
    ...
    HINT_EXIT_MEDIUM_REMAINING_TRAINERS, HINT_EXIT_MEDIUM_REMAINING_ITEMS,
    HINT_EXIT_FAR_REMAINING_TRAINERS,    HINT_EXIT_FAR_REMAINING_ITEMS,
    HINT_EXIT_SHORT_REMAINING_TRAINERS,  HINT_EXIT_SHORT_REMAINING_ITEMS,
};
...
static const u16 sLongStreakRewardItems[] = {
    ITEM_BRIGHT_POWDER, ITEM_WHITE_HERB,  ITEM_QUICK_CLAW,
...
static void GetBattlePyramidData(void) {
  ...
  case PYRAMID_DATA_WIN_STREAK_50:
    gSpecialVar_Result =
        gSaveBlock2Ptr->frontier.pyramidWinStreaks[FRONTIER_LVL_50];
    break;
...
static void SetBattlePyramidPrize(void) {
  ...
  if (gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvlMode] > 41)
    ...
    gSaveBlock2Ptr->frontier.pyramidPrize =
        sShortStreakRewardItems[Random() %
                                ARRAY_COUNT(sShortStreakRewardItems)];
}
...
static void SeedPyramidFloor(void) {
  ...
  for (i = 0; i < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.pyramidRandoms);
       i++)
...
static void SetPickupItem(void) {
  ...
  u32 round = (gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvlMode] /
               FRONTIER_STAGES_PER_CHALLENGE) %
              TOTAL_PYRAMID_ROUNDS;
  ...
  itemIndex =
      (gSpecialVar_LastTalked - sPyramidFloorTemplates[id].numTrainers) - 1;
...
static void ShowPostBattleHintText(void) {
  ...
  struct ObjectEventTemplate *events = gSaveBlock1Ptr->objectEventTemplates;
  u16 trainerId =
      LocalIdToPyramidTrainerId(gObjectEvents[gSelectedObjectEvent].localId);

  for (i = 0; i < ARRAY_COUNT(sTrainerTextGroups); i++) {
    if (sTrainerTextGroups[i][0] ==
  ...
  while (!i) {
    ...
    case HINT_EXIT_DIRECTION:
      textIndex = GetPostBattleDirectionHintTextIndex(&hintType, 8,
                                                      HINT_EXIT_DIRECTION);
      i = 1;
      break;
    case HINT_REMAINING_ITEMS:
      for (i = 0; i < GetNumBattlePyramidObjectEvents(); i++) {
        if (events[i].graphicsId == OBJ_EVENT_GFX_ITEM_BALL &&
            events[i].x != SHRT_MAX && events[i].y != SHRT_MAX)
          textIndex++;
      }
      i = 1;
      break;
    ...
    case HINT_EXIT_SHORT_REMAINING_TRAINERS:
      GetPostBattleDirectionHintTextIndex(&hintType, 8,
    ...
    case HINT_EXIT_MEDIUM_REMAINING_TRAINERS:
      GetPostBattleDirectionHintTextIndex(&hintType, 16,
                                          HINT_REMAINING_TRAINERS);
      break;
    ...
    case HINT_EXIT_FAR_REMAINING_TRAINERS:
      GetPostBattleDirectionHintTextIndex(&hintType, 24,
                                          HINT_REMAINING_TRAINERS);
      break;
...
static void GetInBattlePyramid(void) { gSpecialVar_Result = InBattlePyramid(); }
...
static void Task_SetPyramidFloorPalette(u8 taskId) {
  if (gPaletteFade.active) {
    CpuCopy16(
        ...
        &gPlttBufferUnfaded[BG_PLTT_ID(6)], PLTT_SIZE_4BPP);
    DestroyTask(taskId);
  }
}

// Unused. Handled by BuildStartMenuActions
static void BattlePyramidStartMenu(void) { ShowBattlePyramidStartMenu(); }
...
static void RestorePyramidPlayerParty(void) {
  ...
  for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
    ...
    for (j = 0; j < FRONTIER_PARTY_SIZE; j++) {
      if (GetMonData(&gSaveBlock1Ptr->playerParty[partyIndex], MON_DATA_SPECIES,
                     NULL) ==
...
static u8 GetPostBattleDirectionHintTextIndex(int *hintType,
                                              u8 minDistanceForExitHint,
                                              u8 defaultHintType) {
  int x, y;
  u8 textIndex = 0;
...
static void MarkPyramidTrainerAsBattled(u16 trainerId) {
  ...
  gObjectEvents[gSelectedObjectEvent].movementType =
      MOVEMENT_TYPE_WANDER_AROUND;
  gSaveBlock1Ptr->objectEventTemplates[gSpecialVar_LastTalked - 1]
...
void GenerateBattlePyramidWildMon(void) {
  ...
  u16 round = (gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvl] /
               FRONTIER_STAGES_PER_CHALLENGE) %
              TOTAL_PYRAMID_ROUNDS;
...
// UB: Reading outside the array as lvl was used for mon level instead of
// frontier lvl mode.
#ifndef UBFIX
  if (gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvl] >= 140)
#else
  if (gSaveBlock2Ptr->frontier
          .pyramidWinStreaks[gSaveBlock2Ptr->frontier.lvlMode] >= 140)
#endif
  {
    id = (Random() % 17) + 15;
...
u8 GetTrainerEncounterMusicIdInBattlePyramid(u16 trainerId) {
  ...
  for (i = 0; i < ARRAY_COUNT(sTrainerClassEncounterMusic); i++) {
    if (sTrainerClassEncounterMusic[i].trainerClass ==
        gFacilityClassToTrainerClass[gFacilityTrainers[trainerId]
                                         .facilityClass])
...
static u16 GetUniqueTrainerId(u8 objectEventId) {
  ...
  u16 trainerId;
  u32 lvlMode = gSaveBlock2Ptr->frontier.lvlMode;
  u32 challengeNum = gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvlMode] /
                     FRONTIER_STAGES_PER_CHALLENGE;
  u32 floor = gSaveBlock2Ptr->frontier.curChallengeBattleNum;
...
void GenerateBattlePyramidFloorLayout(u16 *backupMapData,
  ...
  for (i = 0; i < NUM_PYRAMID_FLOOR_SQUARES; i++) {
    ...
    const struct MapLayout *mapLayout =
        gMapLayouts[floorLayoutOffsets[i] +
                    LAYOUT_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR];
    const u16 *layoutMap = mapLayout->map;

    gBackupMapLayout.map = backupMapData;
    gBackupMapLayout.width =
        mapLayout->width * PYRAMID_FLOOR_SQUARES_WIDE + MAP_OFFSET_W;
    gBackupMapLayout.height =
        mapLayout->height * PYRAMID_FLOOR_SQUARES_HIGH + MAP_OFFSET_H;
    map = backupMapData;
    yOffset =
        ((i / PYRAMID_FLOOR_SQUARES_WIDE * mapLayout->height) + MAP_OFFSET) *
        gBackupMapLayout.width;
    xOffset = (i % PYRAMID_FLOOR_SQUARES_WIDE * mapLayout->width) + MAP_OFFSET;
    map += yOffset + xOffset;
    for (y = 0; y < mapLayout->height; y++) {
      for (x = 0; x < mapLayout->width; x++) {
        if ((layoutMap[x] & MAPGRID_METATILE_ID_MASK) !=
            METATILE_BattlePyramid_Exit) {
          map[x] = layoutMap[x];
        } else if (i != exitSquareId) {
          if (i == entranceSquareId && setPlayerPosition == FALSE) {
            gSaveBlock1Ptr->pos.x =
                (mapLayout->width * (i % PYRAMID_FLOOR_SQUARES_WIDE)) + x;
            gSaveBlock1Ptr->pos.y =
                (mapLayout->height * (i / PYRAMID_FLOOR_SQUARES_WIDE)) + y;
          }
          map[x] = (layoutMap[x] &
                    (MAPGRID_ELEVATION_MASK | MAPGRID_COLLISION_MASK)) |
                   METATILE_BattlePyramid_Floor;
        } else {
          map[x] = layoutMap[x];
        }
      }
...
static void GetPyramidEntranceAndExitSquareIds(u8 *entranceSquareId,
                                               u8 *exitSquareId) {
  *entranceSquareId =
      gSaveBlock2Ptr->frontier.pyramidRandoms[3] % NUM_PYRAMID_FLOOR_SQUARES;
  *exitSquareId =
      gSaveBlock2Ptr->frontier.pyramidRandoms[0] % NUM_PYRAMID_FLOOR_SQUARES;

  if (*entranceSquareId == *exitSquareId) {
    *entranceSquareId = (gSaveBlock2Ptr->frontier.pyramidRandoms[3] + 1) %
                        NUM_PYRAMID_FLOOR_SQUARES;
    *exitSquareId = (gSaveBlock2Ptr->frontier.pyramidRandoms[0] +
                     NUM_PYRAMID_FLOOR_SQUARES - 1) %
                    NUM_PYRAMID_FLOOR_SQUARES;
  }
}
...
static void SetPyramidObjectPositionsUniformly(u8 objType) {
  ...
  GetPyramidFloorLayoutOffsets(floorLayoutOffsets);
  squareId =
      gSaveBlock2Ptr->frontier.pyramidRandoms[2] % NUM_PYRAMID_FLOOR_SQUARES;
  ...
  for (i = 0; i < numObjects; i++) {
    do {
      do {
        if (bits & 1) {
          if (!(gBitTable[squareId] &
                gSaveBlock2Ptr->frontier.pyramidRandoms[3]))
            bits |= 2;
        } else {
        ...
        if (squareId == gSaveBlock2Ptr->frontier.pyramidRandoms[2] %
                            NUM_PYRAMID_FLOOR_SQUARES) {
          if (bits & 1)
            bits |= 6;
          else
...
static bool8 SetPyramidObjectPositionsInAndNearSquare(u8 objType, u8 squareId) {
  ...
  for (i = 0; i < numObjects; i++) {
    ...
    if (r7 & 1) {
      if (TrySetPyramidObjectEventPositionInSquare(
              objType, floorLayoutOffsets,
              sBorderedSquareIds[squareId][borderedIndex],
              objectStartIndex + i)) {
        do {
          borderedIndex++;
          if (sBorderedSquareIds[squareId][borderedIndex] == 0xFF ||
              borderedIndex >= 4)
            borderedIndex = 0;
          r7 += 2;
        } while (r7 >> 1 != 4 &&
...
static bool8 SetPyramidObjectPositionsNearSquare(u8 objType, u8 squareId) {
  ...
  for (i = 0; i < numObjects; i++) {
    if (TrySetPyramidObjectEventPositionInSquare(
            objType, floorLayoutOffsets,
            sBorderedSquareIds[squareId][borderOffset], objectStartIndex + i)) {
      do {
        borderOffset++;
        if (sBorderedSquareIds[squareId][borderOffset] == 0xFF ||
            borderOffset >= 4)
          borderOffset = 0;
        r8++;
      } while (r8 != 4 && TrySetPyramidObjectEventPositionInSquare(
                              objType, floorLayoutOffsets,
...
static bool8 TrySetPyramidObjectEventPositionInSquare(u8 objType,
                                                      u8 *floorLayoutOffsets,
                                                      u8 squareId,
                                                      u8 objectEventId) {
  int x, y;
...
static bool8 TrySetPyramidObjectEventPositionAtCoords(u8 objType, u8 x, u8 y,
                                                      u8 *floorLayoutOffsets,
                                                      u8 squareId,
                                                      u8 objectEventId) {
  int i, j;
  ...
  mapHeader = Overworld_GetMapHeaderByGroupAndId(
      MAP_GROUP(BATTLE_PYRAMID_SQUARE01),
      floorLayoutOffsets[squareId] + MAP_NUM(BATTLE_PYRAMID_SQUARE01));
  for (i = 0; i < mapHeader->events->objectEventCount; i++) {
    if (mapHeader->events->objectEvents[i].x != x ||
        mapHeader->events->objectEvents[i].y != y)
      continue;

    if (objType != OBJ_TRAINERS ||
        mapHeader->events->objectEvents[i].graphicsId ==
    ...
    if (j == objectEventId) {
      ...
      if (floorEvents[objectEventId].graphicsId != OBJ_EVENT_GFX_ITEM_BALL) {
        ...
        floorEvents[objectEventId].graphicsId =
            GetBattleFacilityTrainerGfxId(i);
...
static void GetPyramidFloorLayoutOffsets(u8 *layoutOffsets) {
  ...
  for (i = 0; i < NUM_PYRAMID_FLOOR_SQUARES; i++) {
    layoutOffsets[i] =
        sPyramidFloorTemplates[id].layoutOffsets[MOD(rand, NUM_LAYOUT_OFFSETS)];
    rand >>= 3;
    if (i == 7) {
...
static u8 GetPyramidFloorTemplateId(void) {
  ...
  for (i = sFloorTemplateOffsets[floor];
       i < ARRAY_COUNT(sPyramidFloorTemplateOptions); i++) {
...
u16 GetBattlePyramidPickupItemId(void) {
  ...
  u32 i;
  u32 lvlMode = gSaveBlock2Ptr->frontier.lvlMode;
  int round = (gSaveBlock2Ptr->frontier.pyramidWinStreaks[lvlMode] /
               FRONTIER_STAGES_PER_CHALLENGE);
