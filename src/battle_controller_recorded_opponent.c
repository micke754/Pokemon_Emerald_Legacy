...
static void RecordedOpponentBufferRunCommand(void) {
  if (gBattleControllerExecFlags & gBitTable[gActiveBattler]) {
    if (gBattleBufferA[gActiveBattler][0] <
        ARRAY_COUNT(sRecordedOpponentBufferCommands))
...
static void RecordedOpponentHandleGetMonData(void) {
  ...
  if (gBattleBufferA[gActiveBattler][2] == 0) {
    size += CopyRecordedOpponentMonData(gBattlerPartyIndexes[gActiveBattler],
                                        monData);
  } else {
...
static void RecordedOpponentHandleDrawTrainerPic(void) {
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_MULTI) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_BATTLE_TOWER) {
      ...
      trainerPicId =
          PlayerGenderToFrontTrainerPicId(GetActiveBattlerLinkPlayerGender());
    }
  } else {
    xPos = 176;
    if (gTrainerBattleOpponent_A == TRAINER_UNION_ROOM) {
      ...
      trainerPicId = PlayerGenderToFrontTrainerPicId(
          gLinkPlayers[gRecordedBattleMultiplayerId ^ BIT_SIDE].gender);
    }
  }
