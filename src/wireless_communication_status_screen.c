...
static const u32 sBgTiles_Gfx[] =
    INCBIN_U32("graphics/wireless_status_screen/bg.4bpp.lz");
static const u32 sBgTiles_Tilemap[] =
    INCBIN_U32("graphics/wireless_status_screen/bg.bin.lz");
...
// 0 players means the number of players can change and should be counted
// dynamically GROUPTYPE_TOTAL have no unique group and are simply counted in
// the total of "people communicating"
...
// Cycle through palettes that relocate various shades of blue to create the
...
static void CyclePalette(s16 *counter, s16 *palIdx) {
  ...
  idx = *palIdx +
        2; // +2 skips over default.pal and the empty black palette after it
...
static void PrintHeaderTexts(void) {
  ...
  WCSS_AddTextPrinterParameterized(
      WIN_TITLE, FONT_NORMAL, sHeaderTexts[0],
      GetStringCenterAlignXOffset(FONT_NORMAL, sHeaderTexts[0], 0xC0), 6,
      COLORMODE_GREEN);

  // Print label for each group (excluding total)
  for (i = 0; i < NUM_GROUPTYPES - 1; i++)
    WCSS_AddTextPrinterParameterized(WIN_GROUP_NAMES, FONT_NORMAL,
                                     sHeaderTexts[i + 1], 0, 30 * i + 8,
                                     COLORMODE_WHITE_LGRAY);
...
static void Task_WirelessCommunicationScreen(u8 taskId) {
  ...
  case 3:
    if (UpdateCommunicationCounts(
            sStatusScreen->groupCounts, sStatusScreen->prevGroupCounts,
            sStatusScreen->activities, sStatusScreen->rfuTaskId)) {
      FillWindowPixelBuffer(WIN_GROUP_COUNTS, PIXEL_FILL(0));
      for (i = 0; i < NUM_GROUPTYPES; i++) {
        ConvertIntToDecimalStringN(gStringVar4, sStatusScreen->groupCounts[i],
                                   STR_CONV_MODE_RIGHT_ALIGN, 2);
        if (i != GROUPTYPE_TOTAL)
          WCSS_AddTextPrinterParameterized(WIN_GROUP_COUNTS, FONT_NORMAL,
                                           gStringVar4, 12, 30 * i + 8,
                                           COLORMODE_WHITE_LGRAY);
        else
          WCSS_AddTextPrinterParameterized(WIN_GROUP_COUNTS, FONT_NORMAL,
                                           gStringVar4, 12, 98, COLORMODE_RED);
      }
...
static void WCSS_AddTextPrinterParameterized(u8 windowId, u8 fontId,
...
static u32 CountPlayersInGroupAndGetActivity(struct RfuPlayer *player,
                                             u32 *groupCounts) {
  int i, j, k;
...
#ifdef UBFIX
    // GROUPTYPE_NONE is 0xFF, and shouldn't be used as an index into
    // groupCounts. In theory the only activity with this group type
    // (ACTIVITY_SEARCH) wouldn't satisfy the condition below, but not
    // necessarily.
    if (group_type(i) == GROUPTYPE_NONE)
      continue;
#endif
    if (activity == group_activity(i) &&
...
static bool32 UpdateCommunicationCounts(u32 *groupCounts, u32 *prevGroupCounts,
                                        u32 *activities, u8 taskId) {
  ...
  for (i = 0; i < NUM_TASK_DATA; i++) {
    u32 activity =
        CountPlayersInGroupAndGetActivity(&(*players)[i], groupCountBuffer);
