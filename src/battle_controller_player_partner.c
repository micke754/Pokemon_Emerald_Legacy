...
static const u8 sUnused[] = {
    0x83, 0x4d, 0xf3, 0x5f, 0x6f, 0x4f, 0xeb, 0x3e, 0x67, 0x2e, 0x10,
    0x46, 0x8c, 0x3d, 0x28, 0x35, 0xc5, 0x2c, 0x15, 0x7f, 0xb5, 0x56,
    0x9d, 0x53, 0x3b, 0x43, 0xda, 0x36, 0x79, 0x2a, 0x0e, 0x53,
};
...
static void PlayerPartnerBufferRunCommand(void) {
  if (gBattleControllerExecFlags & gBitTable[gActiveBattler]) {
    if (gBattleBufferA[gActiveBattler][0] <
        ARRAY_COUNT(sPlayerPartnerBufferCommands))
...
static void PlayerPartnerHandleGetMonData(void) {
  ...
  if (gBattleBufferA[gActiveBattler][2] == 0) {
    size +=
        CopyPlayerPartnerMonData(gBattlerPartyIndexes[gActiveBattler], monData);
  } else {
...
static void PlayerPartnerHandleLoadMonSprite(void) {
  ...
  species = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]],
                       MON_DATA_SPECIES);
...
// use the back pic as well as animate it
static void PlayerPartnerHandleDrawTrainerPic(void) {
  s16 xPos, yPos;
  u32 trainerPicId;
...
static void PlayerPartnerHandleChoosePokemon(void) {
  ...
  if (chosenMonId == PARTY_SIZE) // just switch to the next mon
  {
    ...
    for (chosenMonId = PARTY_SIZE / 2; chosenMonId < PARTY_SIZE;
         chosenMonId++) {
...
static void PlayerPartnerHandleExpUpdate(void) {
  ...
  if (GetMonData(&gPlayerParty[monId], MON_DATA_LEVEL) >= MAX_LEVEL ||
    ...
    expPointsToGive = gBattleBufferA[gActiveBattler][2] |
                      (gBattleBufferA[gActiveBattler][3] << 8);
...
static void PlayerPartnerHandleIntroTrainerBallThrow(void) {
  ...
  if (gPartnerTrainerId == TRAINER_STEVEN_PARTNER) {
    ...
    LoadCompressedPalette(gTrainerBackPicPaletteTable[spriteId].data,
                          OBJ_PLTT_ID(paletteNum), PLTT_SIZE_4BPP);
  } else {
    u8 spriteId = GetFrontierTrainerFrontSpriteId(gPartnerTrainerId);
    LoadCompressedPalette(gTrainerFrontPicPaletteTable[spriteId].data,
                          OBJ_PLTT_ID(paletteNum), PLTT_SIZE_4BPP);
  }
