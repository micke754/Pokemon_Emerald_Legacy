...
void SetTaskFuncWithFollowupFunc(u8 taskId, TaskFunc func,
  ...
  gTasks[taskId].data[followupFuncIndex + 1] =
      (s16)((u32)followupFunc >>
            16); // Store followupFunc as two half-words in the data array.
...
void SwitchTaskToFollowupFunc(u8 taskId) {
  ...
  gTasks[taskId].func =
      (TaskFunc)((u16)(gTasks[taskId].data[followupFuncIndex]) |
                 (gTasks[taskId].data[followupFuncIndex + 1] << 16));
}
...
u32 GetWordTaskArg(u8 taskId, u8 dataElem) {
  if (dataElem < NUM_TASK_DATA - 1)
    return (u16)gTasks[taskId].data[dataElem] |
           (gTasks[taskId].data[dataElem + 1] << 16);
  else
    return 0;
}
