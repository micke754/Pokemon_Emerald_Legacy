...
/*
NOTE: The data and functions in this file up until (but not including)
sSoundMovesTable are actually part of battle_main.c. They needed to be moved to
this file in order to match the ROM; this is also why sSoundMovesTable's
declaration is in the middle of functions instead of at the top of the file with
...
static const u8 sPkblToEscapeFactor[][3] = {{[B_MSG_MON_CURIOUS] = 0,
...
void HandleAction_UseMove(void) {
  ...
  gCurrMovePos = gChosenMovePos =
      *(gBattleStruct->chosenMovePositions + gBattlerAttacker);
  ...
  if (gProtectStructs[gBattlerAttacker].noValidMoves) {
    ...
    *(gBattleStruct->moveTarget + gBattlerAttacker) =
        GetMoveTarget(MOVE_STRUGGLE, NO_TARGET_OVERRIDE);
  ...
  // encore forces you to use the same move
    ...
    gCurrMovePos = gChosenMovePos =
        gDisableStructs[gBattlerAttacker].encoredMovePos;
    *(gBattleStruct->moveTarget + gBattlerAttacker) =
        GetMoveTarget(gCurrentMove, NO_TARGET_OVERRIDE);
  }
  ...
  if (gSideTimers[side].followmeTimer != 0 &&
      ...
      GetBattlerSide(gBattlerAttacker) !=
          GetBattlerSide(gSideTimers[side].followmeTarget) &&
    ...
    gBattlerTarget = gSideTimers[side].followmeTarget;
             ...
             gBattleMons[*(gBattleStruct->moveTarget + gBattlerAttacker)]
    ...
    if (var == 4) {
      ...
      if (gAbsentBattlerFlags & gBitTable[gBattlerTarget]) {
        if (GetBattlerSide(gBattlerAttacker) !=
          ...
          gBattlerTarget = GetBattlerAtPosition(
              BATTLE_OPPOSITE(GetBattlerPosition(gBattlerAttacker)));
          if (gAbsentBattlerFlags & gBitTable[gBattlerTarget])
            gBattlerTarget = GetBattlerAtPosition(
                BATTLE_PARTNER(GetBattlerPosition(gBattlerTarget)));
        }
...
void HandleAction_Switch(void) {
  ...
  PREPARE_MON_NICK_BUFFER(
      gBattleTextBuff1, gBattlerAttacker,
      *(gBattleStruct->battlerPartyIndexes + gBattlerAttacker))
...
void HandleAction_UseItem(void) {
  ...
  gLastUsedItem = gBattleBufferB[gBattlerAttacker][1] |
                  (gBattleBufferB[gBattlerAttacker][2] << 8);

  if (gLastUsedItem <= LAST_BALL) // is ball
  {
    gBattlescriptCurrInstr = gBattlescriptsForBallThrow[gLastUsedItem];
  } else if (gLastUsedItem == ITEM_POKE_DOLL ||
             gLastUsedItem == ITEM_FLUFFY_TAIL) {
    gBattlescriptCurrInstr =
        gBattlescriptsForRunningByItem[0]; // BattleScript_RunByUsingItem
  } else if (GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER) {
    gBattlescriptCurrInstr =
        gBattlescriptsForUsingItem[0]; // BattleScript_PlayerUsesItem
  } else {
    gBattleScripting.battler = gBattlerAttacker;
    ...
    case AI_ITEM_CURE_CONDITION:
      ...
      if (*(gBattleStruct->AI_itemFlags + gBattlerAttacker / 2) &
          (1 << AI_HEAL_CONFUSION)) {
        if (*(gBattleStruct->AI_itemFlags + gBattlerAttacker / 2) &
            ((1 << AI_HEAL_PARALYSIS) | (1 << AI_HEAL_FREEZE) |
             (1 << AI_HEAL_BURN) | (1 << AI_HEAL_POISON) |
             (1 << AI_HEAL_SLEEP)))
          gBattleCommunication[MULTISTRING_CHOOSER] = AI_HEAL_SLEEP;
      } else {
        // Check for other statuses, stopping at first (shouldn't be more than
        // one)
        while (!(*(gBattleStruct->AI_itemFlags + gBattlerAttacker / 2) & 1)) {
          *(gBattleStruct->AI_itemFlags + gBattlerAttacker / 2) >>= 1;
          gBattleCommunication[MULTISTRING_CHOOSER]++;
          // MULTISTRING_CHOOSER will be either AI_HEAL_PARALYSIS,
          // AI_HEAL_FREEZE, AI_HEAL_BURN, AI_HEAL_POISON, or AI_HEAL_SLEEP
        }
      }
      break;
    case AI_ITEM_X_STAT:
      gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_STAT_ROSE_ITEM;
      if (*(gBattleStruct->AI_itemFlags + (gBattlerAttacker >> 1)) &
          (1 << AI_DIRE_HIT)) {
    ...
    case AI_ITEM_GUARD_SPEC:
      ...
      // bounds for gMistUsedStringIds and instead prints "{mon} is getting
      // pumped" from the next table, gFocusEnergyUsedStringIds. In any case
      // this isn't an issue in the retail version, as no trainers are ever
      // given any Guard Spec to use.
#ifndef UBFIX
    ...
    gBattlescriptCurrInstr = gBattlescriptsForUsingItem[*(
        gBattleStruct->AI_itemType + gBattlerAttacker / 2)];
  }
  gCurrentActionFuncId = B_ACTION_EXEC_SCRIPT;
}
  ...
  if (holdEffect == HOLD_EFFECT_CAN_ALWAYS_RUN) {
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE)) {
      if (InBattlePyramid()) {
        ...
        if (speedVar > (Random() & 0xFF))
          effect++;
      } else if (gBattleMons[battler].speed <
                 gBattleMons[BATTLE_OPPOSITE(battler)].speed) {
...
void HandleAction_ThrowPokeblock(void) {
  ...
  gBattle_BG0_Y = 0;
  gBattleCommunication[MULTISTRING_CHOOSER] =
      gBattleBufferB[gBattlerAttacker][1] - 1;
...
// BUG: safariEscapeFactor can become 0 below. This causes the pokeblock throw
// glitch.
#ifdef BUGFIX
...
void HandleAction_NothingIsFainted(void) {
  ...
  gHitMarker &= ~(HITMARKER_DESTINYBOND | HITMARKER_IGNORE_SUBSTITUTE |
                  HITMARKER_ATTACKSTRING_PRINTED | HITMARKER_NO_PPDEDUCT |
                  HITMARKER_STATUS_ABILITY_EFFECT | HITMARKER_IGNORE_ON_AIR |
                  HITMARKER_IGNORE_UNDERGROUND | HITMARKER_IGNORE_UNDERWATER |
                  HITMARKER_PASSIVE_DAMAGE | HITMARKER_OBEYS |
                  HITMARKER_WAKE_UP_CLEAR | HITMARKER_SYNCHRONISE_EFFECT |
                  HITMARKER_CHARGING | HITMARKER_NEVER_SET);
}

void HandleAction_ActionFinished(void) {
  *(gBattleStruct->monToSwitchIntoId +
    gBattlerByTurnOrder[gCurrentTurnActionNumber]) = PARTY_SIZE;
  gCurrentTurnActionNumber++;
  gCurrentActionFuncId = gActionsByTurnOrder[gCurrentTurnActionNumber];
...
static const u16 sSoundMovesTable[] = {
    MOVE_GROWL,         MOVE_ROAR,        MOVE_SING,      MOVE_SUPERSONIC,
    MOVE_SCREECH,       MOVE_SNORE,       MOVE_UPROAR,    MOVE_METAL_SOUND,
    MOVE_GRASS_WHISTLE, MOVE_HYPER_VOICE, SOUND_MOVES_END};
...
void PressurePPLose(u8 target, u8 attacker, u16 move) {
  ...
  if (MOVE_IS_PERMANENT(attacker, moveIndex)) {
    ...
    BtlController_EmitSetMonData(BUFFER_A, REQUEST_PPMOVE1_BATTLE + moveIndex,
                                 0, 1,
...
void PressurePPLoseOnUsingImprison(u8 attacker) {
  ...
  for (i = 0; i < gBattlersCount; i++) {
    if (atkSide != GetBattlerSide(i) &&
        gBattleMons[i].ability == ABILITY_PRESSURE) {
  ...
  if (imprisonPos != MAX_MON_MOVES &&
      MOVE_IS_PERMANENT(attacker, imprisonPos)) {
    gActiveBattler = attacker;
    BtlController_EmitSetMonData(BUFFER_A, REQUEST_PPMOVE1_BATTLE + imprisonPos,
                                 0, 1,
...
void PressurePPLoseOnUsingPerishSong(u8 attacker) {
  ...
  if (perishSongPos != MAX_MON_MOVES &&
      MOVE_IS_PERMANENT(attacker, perishSongPos)) {
    gActiveBattler = attacker;
    BtlController_EmitSetMonData(
        BUFFER_A, REQUEST_PPMOVE1_BATTLE + perishSongPos, 0, 1,
...
void UpdateSentPokesToOpponentValue(u8 battler) {
  if (GetBattlerSide(battler) == B_SIDE_OPPONENT) {
    ...
    for (i = 1; i < gBattlersCount; i++)
      gSentPokesToOpponent[(i & BIT_FLANK) >> 1] |=
...
void BattleScriptPush(const u8 *bsPtr) {
  gBattleResources->battleScriptsStack
      ->ptr[gBattleResources->battleScriptsStack->size++] = bsPtr;
}

void BattleScriptPushCursor(void) {
  gBattleResources->battleScriptsStack
      ->ptr[gBattleResources->battleScriptsStack->size++] =
      gBattlescriptCurrInstr;
}
...
u8 TrySetCantSelectMoveBattleScript(void) {
  ...
  if (gDisableStructs[gActiveBattler].disabledMove == move &&
      move != MOVE_NONE) {
  ...
  if (holdEffect == HOLD_EFFECT_CHOICE_BAND && *choicedMove != MOVE_NONE &&
...
u8 CheckMoveLimitations(u8 battlerId, u8 unusableMoves, u8 check) {
  ...
  for (i = 0; i < MAX_MON_MOVES; i++) {
    ...
    if (gBattleMons[battlerId].moves[i] == MOVE_NONE &&
        check & MOVE_LIMITATION_ZEROMOVE)
    ...
    if (gBattleMons[battlerId].moves[i] ==
            gDisableStructs[battlerId].disabledMove &&
...
u8 GetImprisonedMovesCount(u8 battlerId, u16 move) {
  ...
  for (i = 0; i < gBattlersCount; i++) {
    if (battlerSide != GetBattlerSide(i) &&
        gStatuses3[i] & STATUS3_IMPRISONED_OTHERS) {
      s32 j;
      for (j = 0; j < MAX_MON_MOVES; j++) {
  ...
  do {
    ...
    case ENDTURN_LIGHT_SCREEN:
      while (gBattleStruct->turnSideTracker < 2) {
        ...
        gActiveBattler = gBattlerAttacker =
            gSideTimers[side].lightscreenBattlerId;
    ...
    case ENDTURN_SAFEGUARD:
      while (gBattleStruct->turnSideTracker < 2) {
        side = gBattleStruct->turnSideTracker;
        gActiveBattler = gBattlerAttacker =
            gSideTimers[side].safeguardBattlerId;
    ...
    if (gAbsentBattlerFlags & gBitTable[gActiveBattler]) {
      ...
      case ENDTURN_BAD_POISON: // toxic poison
        if ((gBattleMons[gActiveBattler].status1 & STATUS1_TOXIC_POISON) &&
          ...
          gBattleMoveDamage *=
              (gBattleMons[gActiveBattler].status1 & STATUS1_TOXIC_COUNTER) >>
              8;
          BattleScriptExecute(BattleScript_PoisonTurnDmg);
          effect++;
        }
      ...
      case ENDTURN_NIGHTMARES: // spooky nightmares
        if ((gBattleMons[gActiveBattler].status2 & STATUS2_NIGHTMARE) &&
            gBattleMons[gActiveBattler].hp != 0) {
          // R/S does not perform this sleep check, which causes the nightmare
          // effect to persist even after the affected Pokémon has been awakened
          // by Shed Skin.
        ...
        if (gBattleMons[gActiveBattler].status2 & STATUS2_UPROAR) {
          for (gBattlerAttacker = 0; gBattlerAttacker < gBattlersCount;
            ...
            if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP) &&
              ...
              gActiveBattler = gBattlerAttacker;
              BtlController_EmitSetMonData(
                  BUFFER_A, REQUEST_STATUS_BATTLE, 0, 4,
      ...
      case ENDTURN_THRASH: // thrash
        if (gBattleMons[gActiveBattler].status2 & STATUS2_LOCK_CONFUSE) {
          ...
          if (WasUnableToUseMove(gActiveBattler))
            CancelMultiTurnMoves(gActiveBattler);
          else if (!(gBattleMons[gActiveBattler].status2 &
      ...
      case ENDTURN_DISABLE: // disable
        if (gDisableStructs[gActiveBattler].disableTimer != 0) {
          ...
          if (i ==
              MAX_MON_MOVES) // Pokémon does not have the disabled move anymore
          {
        ...
        if (gDisableStructs[gActiveBattler].encoreTimer != 0) {
          if (gBattleMons[gActiveBattler]
              ...
              gDisableStructs[gActiveBattler]
                  .encoredMove) // Pokémon does not have the encored move
                                // anymore
          {
            ...
            gDisableStructs[gActiveBattler].encoreTimer = 0;
                     ...
                     gBattleMons[gActiveBattler]
                             .pp[gDisableStructs[gActiveBattler]
                                     .encoredMovePos] == 0) {
            gDisableStructs[gActiveBattler].encoredMove = MOVE_NONE;
            gDisableStructs[gActiveBattler].encoreTimer = 0;
      ...
      case ENDTURN_YAWN: // yawn
        if (gStatuses3[gActiveBattler] & STATUS3_YAWN) {
          ...
          if (!(gStatuses3[gActiveBattler] & STATUS3_YAWN) &&
            ...
            BtlController_EmitSetMonData(BUFFER_A, REQUEST_STATUS_BATTLE, 0, 4,
                                         &gBattleMons[gActiveBattler].status1);
            MarkBattlerForControllerExec(gActiveBattler);
...
bool8 HandleWishPerishSongOnTurnEnd(void) {
  ...
  case 0:
    while (gBattleStruct->wishPerishSongBattlerId < gBattlersCount) {
      ...
      if (gWishFutureKnock.futureSightCounter[gActiveBattler] != 0 &&
          ...
          gBattleMons[gActiveBattler].hp != 0) {
        if (gWishFutureKnock.futureSightMove[gActiveBattler] ==
            MOVE_FUTURE_SIGHT)
        ...
        PREPARE_MOVE_BUFFER(gBattleTextBuff1,
                            gWishFutureKnock.futureSightMove[gActiveBattler]);
        ...
        if (gWishFutureKnock.futureSightCounter[gActiveBattler] == 0 &&
            gWishFutureKnock
                    .futureSightCounter[BATTLE_PARTNER(gActiveBattler)] == 0) {
  ...
  case 2:
    if ((gBattleTypeFlags & BATTLE_TYPE_ARENA) &&
        gBattleStruct->arenaTurnCounter == 2 && gBattleMons[0].hp != 0 &&
        gBattleMons[1].hp != 0) {
      s32 i;
      ...
      do {
        ...
        if (gBattleMons[gBattleStruct->faintedActionsBattlerId].hp == 0 &&
            !(gBattleStruct->givenExpMons &
              gBitTable[gBattlerPartyIndexes[gBattleStruct
...
void TryClearRageStatuses(void) {
  s32 i;
  for (i = 0; i < gBattlersCount; i++) {
    if ((gBattleMons[i].status2 & STATUS2_RAGE) &&
        gChosenMoveByBattler[i] != MOVE_RAGE)
      ...
      if (gBattleMons[gBattlerAttacker].status1 & STATUS1_FREEZE) {
        if (Random() % 5) {
          if (gBattleMoves[gCurrentMove].effect !=
              EFFECT_THAW_HIT) // unfreezing via a move effect happens in case
                               // 13
          {
      ...
      if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION) {
        ...
        if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION) {
          if (Random() & 1) {
            ...
            // to BattleScript_MoveUsedIsConfused whether or not damage was
            // taken
          ...
          } else // confusion dmg
          {
            ...
            gBattleMoveDamage = CalculateBaseDamage(
                &gBattleMons[gBattlerAttacker], &gBattleMons[gBattlerAttacker],
        ...
        if (gBattleMons[gBattlerAttacker].status2 & STATUS2_BIDE) {
          ...
          if (gBideDmg[gBattlerAttacker]) {
            ...
            *bideDmg = gBideDmg[gBattlerAttacker] * 2;
            ...
            if (gAbsentBattlerFlags & gBitTable[gBattlerTarget])
              gBattlerTarget =
                  GetMoveTarget(MOVE_BIDE, MOVE_TARGET_SELECTED + 1);
...
bool8 HasNoMonsToSwitch(u8 battler, u8 partyIdBattlerOn1,
                        u8 partyIdBattlerOn2) {
  u8 playerId, flankId;
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER) {
    ...
    for (i = playerId * MULTI_PARTY_SIZE;
         i < playerId * MULTI_PARTY_SIZE + MULTI_PARTY_SIZE; i++) {
...
u8 CastformDataTypeChange(u8 battler) {
  ...
  if (!WEATHER_HAS_EFFECT)
    return 0; // No change
  if (!(gBattleWeather & (B_WEATHER_RAIN | B_WEATHER_SUN | B_WEATHER_HAIL)) &&
...
u8 AbilityBattleEffects(u8 caseID, u8 battler, u8 ability, u8 special,
                        u16 moveArg) {
  u8 effect = 0;
  ...
  if (!(gBattleTypeFlags &
        BATTLE_TYPE_SAFARI)) // Why isn't that check done at the beginning?
  {
    u8 moveType;
    s32 i;
    u16 move;
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
      ...
      case ABILITYEFFECT_SWITCH_IN_WEATHER:
        if (!(gBattleTypeFlags & BATTLE_TYPE_RECORDED)) {
          ...
          case WEATHER_DOWNPOUR:
            if (!(gBattleWeather & B_WEATHER_RAIN)) {
              gBattleWeather =
                  (B_WEATHER_RAIN_TEMPORARY | B_WEATHER_RAIN_PERMANENT);
      ...
      case ABILITY_DRIZZLE:
        if (!(gBattleWeather & B_WEATHER_RAIN_PERMANENT)) {
          gBattleWeather =
              (B_WEATHER_RAIN_PERMANENT | B_WEATHER_RAIN_TEMPORARY);
    ...
    case ABILITYEFFECT_ON_DAMAGE: // Contact abilities and Color Change
      ...
      case ABILITY_COLOR_CHANGE:
        if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) &&
            move != MOVE_STRUGGLE && gBattleMoves[move].power != 0 &&
            TARGET_TURN_DAMAGED && !IS_BATTLER_OF_TYPE(battler, moveType) &&
            gBattleMons[battler].hp != 0) {
      ...
      case ABILITY_EFFECT_SPORE:
        if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) &&
            ...
            (Random() % 10) == 0) {
          do {
            // Pick either MOVE_EFFECT_SLEEP, MOVE_EFFECT_POISON or
            // MOVE_EFFECT_BURN
          ...
          if (gBattleCommunication[MOVE_EFFECT_BYTE] == MOVE_EFFECT_BURN)
            gBattleCommunication[MOVE_EFFECT_BYTE] +=
                (MOVE_EFFECT_PARALYSIS - MOVE_EFFECT_BURN);
    ...
    case ABILITYEFFECT_TRACE: // 11
      for (i = 0; i < gBattlersCount; i++) {
        if (gBattleMons[i].ability == ABILITY_TRACE &&
            (gStatuses3[i] & STATUS3_TRACE)) {
          u8 target2;
          side = BATTLE_OPPOSITE(GetBattlerPosition(i)) &
                 BIT_SIDE; // side of the opposing Pokémon
          target1 = GetBattlerAtPosition(side);
          target2 = GetBattlerAtPosition(side + BIT_FLANK);
          if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE) {
            if (gBattleMons[target1].ability != ABILITY_NONE &&
                gBattleMons[target1].hp != 0 &&
                gBattleMons[target2].ability != ABILITY_NONE &&
                gBattleMons[target2].hp != 0) {
              gActiveBattler =
                  GetBattlerAtPosition(((Random() & 1) * 2) | side);
              gBattleMons[i].ability = gBattleMons[gActiveBattler].ability;
              gLastUsedAbility = gBattleMons[gActiveBattler].ability;
              effect++;
              RecordAbilityBattle(i, gBattleMons[gActiveBattler].ability);
            } else if (gBattleMons[target1].ability != ABILITY_NONE &&
                       gBattleMons[target1].hp != 0) {
              ...
              effect++;
              RecordAbilityBattle(i, gBattleMons[gActiveBattler].ability);
            } else if (gBattleMons[target2].ability != ABILITY_NONE &&
                       gBattleMons[target2].hp != 0) {
          ...
          if (effect != 0) {
            ...
            PREPARE_MON_NICK_WITH_PREFIX_BUFFER(
                gBattleTextBuff1, gActiveBattler,
    ...
    if (effect && caseID < ABILITYEFFECT_CHECK_OTHER_SIDE &&
        gLastUsedAbility != 0xFF)
...
#define TRY_EAT_CONFUSE_BERRY(flavor)                                          \
  if (gBattleMons[battlerId].hp <= gBattleMons[battlerId].maxHP / 2 &&         \
    ...
    gBattleMoveDamage *= -1;                                                   \
    if (GetFlavorRelationByPersonality(gBattleMons[battlerId].personality,     \
                                       flavor) < 0)                            \
    ...
    if (gBattleMons[battlerId].hp) {
      ...
      case HOLD_EFFECT_RESTORE_PP:
        if (!moveTurn) {
          ...
          if (i != MAX_MON_MOVES) {
            ...
            BtlController_EmitSetMonData(BUFFER_A, i + REQUEST_PPMOVE1_BATTLE,
                                         0, 1, &changedPP);
            MarkBattlerForControllerExec(gActiveBattler);
      ...
      case HOLD_EFFECT_ATTACK_UP:
        if (gBattleMons[battlerId].hp <=
          ...
          PREPARE_STAT_BUFFER(gBattleTextBuff1, STAT_ATK);
          PREPARE_STRING_BUFFER(
              gBattleTextBuff2,
              STRINGID_STATROSE); // Only the Attack stat-up berry has this
          gEffectBattler = battlerId;
  ...
  case ITEMEFFECT_KINGSROCK_SHELLBELL:
    if (gBattleMoveDamage) {
      ...
      case HOLD_EFFECT_FLINCH:
        if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) &&
            TARGET_TURN_DAMAGED && (Random() % 100) < atkHoldEffectParam &&
...
u8 GetMoveTarget(u16 move, u8 setTarget) {
  ...
  case MOVE_TARGET_SELECTED:
    ...
    if (gSideTimers[side].followmeTimer &&
      ...
      } while (targetBattler == gBattlerAttacker ||
  ...
  case MOVE_TARGET_OPPONENTS_FIELD:
    targetBattler = GetBattlerAtPosition(
        BATTLE_OPPOSITE(GET_BATTLER_SIDE(gBattlerAttacker)));
...
static bool32 IsBattlerModernFatefulEncounter(u8 battlerId) { return TRUE; }
...
u8 IsMonDisobedient(void) {
  ...
  if (IsBattlerModernFatefulEncounter(
          gBattlerAttacker)) // only false if illegal Mew or Deoxys
  {
    if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER &&
    ...
    if (!IsOtherTrainer(gBattleMons[gBattlerAttacker].otId,
                        gBattleMons[gBattlerAttacker].otName))
      return 0;
  ...
  calc = (gBattleMons[gBattlerAttacker].level + obedienceLevel) * rnd >> 8;
  if (calc < obedienceLevel) {
    calc = CheckMoveLimitations(gBattlerAttacker, gBitTable[gCurrMovePos],
                                MOVE_LIMITATIONS_ALL);
    if (calc == ALL_MOVES_MASK) // all moves cannot be used
    {
      // Randomly select, then print a disobedient string
      // B_MSG_LOAFING, B_MSG_WONT_OBEY, B_MSG_TURNED_AWAY, or
      // B_MSG_PRETEND_NOT_NOTICE
      gBattleCommunication[MULTISTRING_CHOOSER] =
          MOD(Random(), NUM_LOAF_STRINGS);
      gBattlescriptCurrInstr = BattleScript_MoveUsedLoafingAround;
      return 1;
