...
static void AnimTask_AnimateGustTornadoPalette_Step(u8 taskId) {
  ...
  if (gTasks[taskId].data[10]++ == gTasks[taskId].data[1]) {
    ...
    do {
      gPlttBufferFaded[base + OBJ_PLTT_OFFSET + 1 + i] =
          gPlttBufferFaded[base + OBJ_PLTT_OFFSET + i];
      i--;
    } while (i > 0);
...
static void AnimFallingFeather(struct Sprite *sprite) {
  ...
  sprite->x = GetBattlerSpriteCoord(battler, BATTLER_COORD_ATTR_HEIGHT) +
              gBattleAnimArgs[0];
...
// Launches a water droplet away from the specified battler. Used by Astonish
// and Dive
static void AnimSprayWaterDroplet(struct Sprite *sprite) {
  ...
  if (gBattleAnimArgs[1] == 0) {
    sprite->x = GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X);
    sprite->y =
        GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_Y) + 32;
  } else {
    sprite->x = GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_X);
...
void AnimSkyAttackBird_Step(struct Sprite *sprite) {
  ...
  if (sprite->x > DISPLAY_WIDTH + 45 || sprite->x < -45 || sprite->y > 157 ||
      sprite->y < -45)
