  ...
  case 3:
    ...
    if (gTasks[taskId].data[2] == 0) {
      ...
      SetGpuReg(REG_OFFSET_BG1CNT, BGCNT_PRIORITY(0) | BGCNT_CHARBASE(0) |
                                       BGCNT_16COLOR | BGCNT_SCREENBASE(28) |
                                       BGCNT_TXT256x512);
...
static void BattleIntroSlideLink(u8 taskId) {
  ...
  case 1:
    if (--gTasks[taskId].data[2] == 0) {
      gTasks[taskId].tState++;
      gSprites[gBattleStruct->linkBattleVsSpriteId_V].oam.objMode =
          ST_OAM_OBJ_WINDOW;
      gSprites[gBattleStruct->linkBattleVsSpriteId_V].callback =
          SpriteCB_VsLetterInit;
      gSprites[gBattleStruct->linkBattleVsSpriteId_S].oam.objMode =
          ST_OAM_OBJ_WINDOW;
      ...
      SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WINOBJ_BG_ALL | WINOUT_WINOBJ_OBJ |
                                       WINOUT_WINOBJ_CLR | WINOUT_WIN01_BG1 |
                                       WINOUT_WIN01_BG2);
    }
    break;
  case 2:
...
static void BattleIntroSlidePartner(u8 taskId) {
  ...
  case 1:
    if (--gTasks[taskId].data[2] == 0) {
      gTasks[taskId].tState++;
      SetGpuReg(REG_OFFSET_BG1CNT, BGCNT_PRIORITY(2) | BGCNT_CHARBASE(2) |
                                       BGCNT_16COLOR | BGCNT_SCREENBASE(28) |
                                       BGCNT_TXT512x256);
      SetGpuReg(REG_OFFSET_BG2CNT, BGCNT_PRIORITY(2) | BGCNT_CHARBASE(2) |
                                       BGCNT_16COLOR | BGCNT_SCREENBASE(30) |
                                       BGCNT_TXT512x256);
      SetGpuReg(REG_OFFSET_DISPCNT, GetGpuReg(REG_OFFSET_DISPCNT) |
                                        DISPCNT_OBJ_1D_MAP | DISPCNT_OBJ_ON |
                                        DISPCNT_WIN0_ON | DISPCNT_WIN1_ON |
                                        DISPCNT_OBJWIN_ON);
      SetGpuReg(REG_OFFSET_WININ, WININ_WIN1_BG1 | WININ_WIN1_BG2 |
                                      WININ_WIN1_BG3 | WININ_WIN1_OBJ |
                                      WININ_WIN1_CLR);
      SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WIN01_BG_ALL | WINOUT_WIN01_OBJ |
                                       WINOUT_WIN01_CLR | WINOUT_WINOBJ_BG_ALL |
                                       WINOUT_WINOBJ_OBJ | WINOUT_WINOBJ_CLR);
  ...
  case 4:
    ...
    if (!gBattle_BG0_Y) {
      ...
      SetGpuReg(REG_OFFSET_DISPCNT,
                GetGpuReg(REG_OFFSET_DISPCNT) & ~DISPCNT_WIN1_ON);
      SetBgAttribute(1, BG_ATTR_CHARBASEINDEX, 0);
      SetBgAttribute(2, BG_ATTR_CHARBASEINDEX, 0);
      SetGpuReg(REG_OFFSET_BG1CNT, BGCNT_PRIORITY(0) | BGCNT_CHARBASE(0) |
                                       BGCNT_16COLOR | BGCNT_SCREENBASE(28) |
...
void DrawBattlerOnBg(int bgId, u8 x, u8 y, u8 battlerPosition, u8 paletteId,
  ...
  CpuCopy16(gMonSpritesGfxPtr->sprites.ptr[battlerPosition] +
                BG_SCREEN_SIZE * gBattleMonForms[battler],
            tiles, BG_SCREEN_SIZE);
...
static void UNUSED DrawBattlerOnBgDMA(u8 x, u8 y, u8 battlerPosition, u8 arg3,
                                      u8 paletteId, u16 arg5, u8 arg6,
                                      u8 arg7) {
  int i, j, offset;

  DmaCopy16(3,
            gMonSpritesGfxPtr->sprites.ptr[battlerPosition] +
                BG_SCREEN_SIZE * arg3,
            (void *)BG_SCREEN_ADDR(0) + arg5, BG_SCREEN_SIZE);
  offset = (arg5 >> 5) - (arg7 << 9);
  for (i = y; i < y + 8; i++) {
    for (j = x; j < x + 8; j++) {
      *((u16 *)(BG_VRAM) + (i * 32) + (j + (arg6 << 10))) =
          offset | (paletteId << 12);
      offset++;
    }
  }
