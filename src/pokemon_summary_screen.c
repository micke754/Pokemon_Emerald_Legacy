...
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_RIGHT                            \
  11                                           // Sp. Attack, Sp. Defense, Speed
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_EXP 12 // EXP, Next Level
#define PSS_LABEL_WINDOW_POKEMON_SKILLS_STATUS 13

// Moves screen
#define PSS_LABEL_WINDOW_MOVES_POWER_ACC                                       \
  14 // Also contains the power and accuracy values
...
enum {
  ...
  SPRITE_ARR_ID_TYPE, // 2 for mon types, 5 for move types(4 moves and 1 to
                      // learn), used interchangeably, because mon types and
                      // move types aren't shown on the same screen
  SPRITE_ARR_ID_MOVE_SELECTOR1 =
      SPRITE_ARR_ID_TYPE +
      TYPE_ICON_SPRITE_COUNT, // 10 sprites that make up the selector
  SPRITE_ARR_ID_MOVE_SELECTOR2 =
      SPRITE_ARR_ID_MOVE_SELECTOR1 + MOVE_SELECTOR_SPRITES_COUNT,
  SPRITE_ARR_ID_COUNT =
      SPRITE_ARR_ID_MOVE_SELECTOR2 + MOVE_SELECTOR_SPRITES_COUNT
};
...
static EWRAM_DATA struct PokemonSummaryScreenData {
  ...
  u8 minPageIndex;
  u8 maxPageIndex;
  bool8 lockMonFlag; // This is used to prevent the player from changing Pokémon
                     // in the move deleter select, etc, but it is not needed
                     // because the input is handled differently there
  u16 newMove;
  u8 firstMoveIndex;
  u8 secondMoveIndex;
  bool8 lockMovesFlag; // This is used to prevent the player from changing
                       // position of moves in a battle or when trading.
  u8 bgDisplayOrder;   // Determines the order page backgrounds are loaded while
                       // scrolling between them
  u8 filler40CA;
  u8 windowIds[8];
  u8 spriteIds[SPRITE_ARR_ID_COUNT];
  bool8 handleDeoxys;
  s16 switchCounter; // Used for various switch statement cases that
                     // decompress/load graphics or Pokémon data
  u8 unk_filler4[6];
...
static const u16 sStatusTilemap[] =
    INCBIN_U16("graphics/summary_screen/status_tilemap.bin");
...
static const struct WindowTemplate
    sPageMovesTemplate[] = // This is used for both battle and contest moves
    {
...
static const u8 sMemoMiscTextColor[] =
    _("{COLOR WHITE}{SHADOW DARK_GRAY}"); // This is also affected by palettes,
                                          // apparently
static const u8 sStatsLeftColumnLayout[] =
    _("{DYNAMIC 0}/{DYNAMIC 1}\n{DYNAMIC 2}\n{DYNAMIC 3}");
static const u8 sStatsLeftColumnLayoutIVEV[] =
    _("{DYNAMIC 0}\n{DYNAMIC 1}\n{DYNAMIC 2}");
static const u8 sStatsRightColumnLayout[] =
    _("{DYNAMIC 0}\n{DYNAMIC 1}\n{DYNAMIC 2}");
...
static const union AnimCmd sSpriteAnim_TypeRock[] = {
    ANIMCMD_FRAME(TYPE_ROCK * 8, 0, FALSE, FALSE), ANIMCMD_END};
static const union AnimCmd sSpriteAnim_TypeBug[] = {
    ANIMCMD_FRAME(TYPE_BUG * 8, 0, FALSE, FALSE), ANIMCMD_END};
...
static const union AnimCmd sSpriteAnim_TypeFire[] = {
    ANIMCMD_FRAME(TYPE_FIRE * 8, 0, FALSE, FALSE), ANIMCMD_END};
...
static const union AnimCmd sSpriteAnim_TypeIce[] = {
    ANIMCMD_FRAME(TYPE_ICE * 8, 0, FALSE, FALSE), ANIMCMD_END};
static const union AnimCmd sSpriteAnim_TypeDragon[] = {
    ANIMCMD_FRAME(TYPE_DRAGON * 8, 0, FALSE, FALSE), ANIMCMD_END};
static const union AnimCmd sSpriteAnim_TypeDark[] = {
    ANIMCMD_FRAME(TYPE_DARK * 8, 0, FALSE, FALSE), ANIMCMD_END};
static const union AnimCmd sSpriteAnim_CategoryCool[] = {
    ANIMCMD_FRAME((CONTEST_CATEGORY_COOL + NUMBER_OF_MON_TYPES) * 8, 0, FALSE,
                  FALSE),
    ANIMCMD_END};
static const union AnimCmd sSpriteAnim_CategoryBeauty[] = {
    ANIMCMD_FRAME((CONTEST_CATEGORY_BEAUTY + NUMBER_OF_MON_TYPES) * 8, 0, FALSE,
                  FALSE),
    ANIMCMD_END};
static const union AnimCmd sSpriteAnim_CategoryCute[] = {
    ANIMCMD_FRAME((CONTEST_CATEGORY_CUTE + NUMBER_OF_MON_TYPES) * 8, 0, FALSE,
                  FALSE),
    ANIMCMD_END};
static const union AnimCmd sSpriteAnim_CategorySmart[] = {
    ANIMCMD_FRAME((CONTEST_CATEGORY_SMART + NUMBER_OF_MON_TYPES) * 8, 0, FALSE,
                  FALSE),
    ANIMCMD_END};
static const union AnimCmd sSpriteAnim_CategoryTough[] = {
    ANIMCMD_FRAME((CONTEST_CATEGORY_TOUGH + NUMBER_OF_MON_TYPES) * 8, 0, FALSE,
                  FALSE),
    ANIMCMD_END};
static const union AnimCmd *const
    sSpriteAnimTable_MoveTypes[NUMBER_OF_MON_TYPES + CONTEST_CATEGORIES_COUNT] =
        {
            sSpriteAnim_TypeNormal,    sSpriteAnim_TypeFighting,
            sSpriteAnim_TypeFlying,    sSpriteAnim_TypePoison,
            sSpriteAnim_TypeGround,    sSpriteAnim_TypeRock,
            sSpriteAnim_TypeBug,       sSpriteAnim_TypeGhost,
            sSpriteAnim_TypeSteel,     sSpriteAnim_TypeMystery,
            sSpriteAnim_TypeFire,      sSpriteAnim_TypeWater,
            sSpriteAnim_TypeGrass,     sSpriteAnim_TypeElectric,
            sSpriteAnim_TypePsychic,   sSpriteAnim_TypeIce,
            sSpriteAnim_TypeDragon,    sSpriteAnim_TypeDark,
            sSpriteAnim_CategoryCool,  sSpriteAnim_CategoryBeauty,
            sSpriteAnim_CategoryCute,  sSpriteAnim_CategorySmart,
...
static const union AnimCmd sSpriteAnim_MoveSelectorLeft[] = {
    ANIMCMD_FRAME(16, 0, FALSE, FALSE), ANIMCMD_END};
...
static const union AnimCmd sSpriteAnim_MoveSelectorMiddle[] = {
    ANIMCMD_FRAME(20, 0, FALSE, FALSE), ANIMCMD_END};
static const union AnimCmd sSpriteAnim_MoveSelector7[] = {
    ANIMCMD_FRAME(24, 0, FALSE, FALSE), ANIMCMD_END};
...
static const union AnimCmd *const sSpriteAnimTable_MoveSelector[] = {
    sSpriteAnim_MoveSelector0,      sSpriteAnim_MoveSelector1,
    sSpriteAnim_MoveSelector2,      sSpriteAnim_MoveSelector3,
    sSpriteAnim_MoveSelectorLeft,   sSpriteAnim_MoveSelectorRight,
    sSpriteAnim_MoveSelectorMiddle, sSpriteAnim_MoveSelector7,
    sSpriteAnim_MoveSelector8,      sSpriteAnim_MoveSelector9,
};
...
static const union AnimCmd *const sSpriteAnimTable_StatusCondition[] = {
    sSpriteAnim_StatusPoison, sSpriteAnim_StatusParalyzed,
    sSpriteAnim_StatusSleep,  sSpriteAnim_StatusFrozen,
    sSpriteAnim_StatusBurn,   sSpriteAnim_StatusPokerus,
...
static const struct CompressedSpritePalette sStatusIconsSpritePalette = {
    .data = gStatusPal_Icons, .tag = TAG_MON_STATUS};
static const struct SpriteTemplate sSpriteTemplate_StatusCondition = {
    ...
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCallbackDummy};
static const u16 sMarkings_Pal[] =
    INCBIN_U16("graphics/summary_screen/markings.gbapal");

// code
void ShowPokemonSummaryScreen(u8 mode, void *mons, u8 monIndex, u8 maxMonIndex,
                              void (*callback)(void)) {
...
void ShowSelectMovePokemonSummaryScreen(struct Pokemon *mons, u8 monIndex,
                                        u8 maxMonIndex, void (*callback)(void),
                                        u16 newMove) {
  ShowPokemonSummaryScreen(SUMMARY_MODE_SELECT_MOVE, mons, monIndex,
                           maxMonIndex, callback);
  sMonSummaryScreen->newMove = newMove;
}

void ShowPokemonSummaryScreenHandleDeoxys(u8 mode, struct BoxPokemon *mons,
                                          u8 monIndex, u8 maxMonIndex,
                                          void (*callback)(void)) {
...
static void CB2_InitSummaryScreen(void) {
  while (MenuHelpers_ShouldWaitForLinkRecv() != TRUE &&
         LoadGraphics() != TRUE && MenuHelpers_IsLinkActive() != TRUE)
    ;
}
...
static void InitBGs(void) {
  ...
  SetBgTilemapBuffer(
      1, sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_BATTLE_MOVES][0]);
  SetBgTilemapBuffer(2,
                     sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_SKILLS][0]);
...
static bool8 DecompressGraphics(void) {
  ...
  case 1:
    if (FreeTempTileDataBuffersIfPossible() != 1) {
      LZDecompressWram(gSummaryPage_Info_Tilemap,
                       sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][0]);
      sMonSummaryScreen->switchCounter++;
    }
    break;
  case 2:
    LZDecompressWram(gSummaryPage_InfoEgg_Tilemap,
                     sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][1]);
    sMonSummaryScreen->switchCounter++;
    break;
  case 3:
    LZDecompressWram(gSummaryPage_Skills_Tilemap,
                     sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_SKILLS][1]);
    sMonSummaryScreen->switchCounter++;
    break;
  case 4:
    LZDecompressWram(
        gSummaryPage_BattleMoves_Tilemap,
  ...
  case 5:
    ...
    break;
  case 6:
    LoadCompressedPalette(gSummaryScreen_Pal, BG_PLTT_ID(0),
                          8 * PLTT_SIZE_4BPP);
...
static void SetDefaultTilemaps(void) {
  if (sMonSummaryScreen->currPageIndex != PSS_PAGE_BATTLE_MOVES &&
    ...
    TilemapFiveMovesDisplay(
        sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_BATTLE_MOVES][0], 3,
        FALSE);
    TilemapFiveMovesDisplay(
        sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_CONTEST_MOVES][0], 1,
        FALSE);
    SetBgTilemapBuffer(
        1, sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_CONTEST_MOVES][0]);
    SetBgTilemapBuffer(
        2, sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_BATTLE_MOVES][0]);
...
static void ChangeSummaryState(s16 *taskData, u8 taskId) {
  FillBgTilemapBufferRect(1, STATS_BLANK_BLOCK, STATS_CORD_X + 3, STATS_CORD_Y,
                          1, 1, 2);
  FillBgTilemapBufferRect(1, STATS_BLANK_BLOCK, STATS_CORD_X + 4, STATS_CORD_Y,
                          1, 1, 2);
  FillBgTilemapBufferRect(1, STATS_BLANK_BLOCK, STATS_CORD_X + 5, STATS_CORD_Y,
                          1, 1, 2);
  switch (currentStat) {
  case STAT_STATS:
    FillBgTilemapBufferRect(1, IVS_STATS_BLOCK, STATS_CORD_X, STATS_CORD_Y, 1,
                            1, 2);
    FillBgTilemapBufferRect(1, IVS_STATS_BLOCK + 1, STATS_CORD_X + 1,
                            STATS_CORD_Y, 1, 1, 2);
    FillBgTilemapBufferRect(1, IVS_STATS_BLOCK + 2, STATS_CORD_X + 2,
                            STATS_CORD_Y, 1, 1, 2);
    taskData[3] = STAT_IVS;
    break;
  case STAT_IVS:
    FillBgTilemapBufferRect(1, EVS_STATS_BLOCK, STATS_CORD_X, STATS_CORD_Y, 1,
                            1, 2);
    FillBgTilemapBufferRect(1, EVS_STATS_BLOCK + 1, STATS_CORD_X + 1,
                            STATS_CORD_Y, 1, 1, 2);
    FillBgTilemapBufferRect(1, EVS_STATS_BLOCK + 2, STATS_CORD_X + 2,
                            STATS_CORD_Y, 1, 1, 2);
    taskData[3] = STAT_EVS;
    break;
  case STAT_EVS:
    FillBgTilemapBufferRect(1, STATS_STATS_BLOCK, STATS_CORD_X, STATS_CORD_Y, 1,
                            1, 2);
    FillBgTilemapBufferRect(1, STATS_STATS_BLOCK + 1, STATS_CORD_X + 1,
                            STATS_CORD_Y, 1, 1, 2);
    FillBgTilemapBufferRect(1, STATS_STATS_BLOCK + 2, STATS_CORD_X + 2,
                            STATS_CORD_Y, 1, 1, 2);
    FillBgTilemapBufferRect(1, STATS_STATS_BLOCK + 3, STATS_CORD_X + 3,
                            STATS_CORD_Y, 1, 1, 2);
...
static void ChangeSummaryPokemon(u8 taskId, s8 delta) {
  ...
  if (!sMonSummaryScreen->lockMonFlag) {
    if (sMonSummaryScreen->isBoxMon == TRUE) {
      ...
      monId = AdvanceStorageMonIndex(sMonSummaryScreen->monList.boxMons,
...
static void Task_ChangeSummaryMon(u8 taskId) {
  ...
  default:
    if (!MenuHelpers_ShouldWaitForLinkRecv() &&
        !FuncIsActiveTask(Task_ShowStatusWindow)) {
      data[0] = 0;
      gTasks[taskId].func = Task_HandleInput;
    }
    return;
  ...
  if (sMonSummaryScreen->currPageIndex == PSS_PAGE_INFO) {
    if (delta == -1 && sMonSummaryScreen->curMonIndex == 0)
      return -1;
             ...
             sMonSummaryScreen->curMonIndex >= sMonSummaryScreen->maxMonIndex)
      return -1;
    else
...
static bool8 IsValidToViewInMulti(struct Pokemon *mon) {
  if (GetMonData(mon, MON_DATA_SPECIES) == SPECIES_NONE)
    return FALSE;
  else if (sMonSummaryScreen->curMonIndex != 0 ||
...
static void PssScrollRight(u8 taskId) // Scroll right
{
  ...
  if (data[0] == 0) {
    ...
    SetBgTilemapBuffer(
        data[1], sMonSummaryScreen
                     ->bgTilemapBuffers[sMonSummaryScreen->currPageIndex][0]);
    ShowBg(1);
    ShowBg(2);
  }
...
static void PssScrollLeftEnd(u8 taskId) // display left
{
  ...
  if (sMonSummaryScreen->currPageIndex > 1) {
    SetBgTilemapBuffer(
        data[1],
        sMonSummaryScreen
            ->bgTilemapBuffers[sMonSummaryScreen->currPageIndex - 1][0]);
    ChangeBgX(data[1], 0x10000, BG_COORD_SET);
  }
  ShowBg(1);
  ShowBg(2);
...
static void CloseMoveSelectMode(u8 taskId) {
  ...
  PrintMoveDetails(0);
  TilemapFiveMovesDisplay(
      sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_BATTLE_MOVES][0], 3, TRUE);
  TilemapFiveMovesDisplay(
...
static void Task_HandleInput_MovePositionSwitch(u8 taskId) {
  ...
  if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE) {
    if (JOY_NEW(DPAD_UP)) {
      ...
      if (sMonSummaryScreen->firstMoveIndex ==
...
static void SwapMonMoves(struct Pokemon *mon, u8 moveIndex1, u8 moveIndex2) {
  ...
  ppBonuses &= ~ppUpMask2;
  ppBonuses |=
      (ppBonusMove1 << (moveIndex2 * 2)) + (ppBonusMove2 << (moveIndex1 * 2));
...
static void SwapBoxMonMoves(struct BoxPokemon *mon, u8 moveIndex1,
                            u8 moveIndex2) {
  struct PokeSummary *summary = &sMonSummaryScreen->summary;
...
static bool8 CanReplaceMove(void) {
  if (sMonSummaryScreen->firstMoveIndex == MAX_MON_MOVES ||
      sMonSummaryScreen->newMove == MOVE_NONE ||
      IsMoveHm(sMonSummaryScreen->summary
                   .moves[sMonSummaryScreen->firstMoveIndex]) != TRUE)
    return TRUE;
  else
    return FALSE;
}
...
// This redraws the power/accuracy window when the player scrolls out of the "HM
// Moves can't be forgotten" message
static void Task_HandleInputCantForgetHMsMoves(u8 taskId) {
  ...
  if (FuncIsActiveTask(Task_ShowPowerAccWindow) != 1) {
    if (JOY_NEW(DPAD_UP)) {
      ...
      if (sMonSummaryScreen->currPageIndex != PSS_PAGE_BATTLE_MOVES) {
        ClearWindowTilemap(PSS_LABEL_WINDOW_PORTRAIT_SPECIES);
        if (!gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_STATUS]]
                 .invisible)
          ClearWindowTilemap(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATUS);
        move =
            sMonSummaryScreen->summary.moves[sMonSummaryScreen->firstMoveIndex];
...
u8 GetMoveSlotToReplace(void) { return sMoveSlotToReplace; }

static void DrawPagination(
    void) // Updates the pagination dots at the top of the summary screen
{
  ...
  CopyToBgTilemapBufferRect_ChangePalette(3, tilemap, 11, 0, PSS_PAGE_COUNT * 2,
                                          2, 16);
  ScheduleBgCopyTilemapToVram(3);
  Free(tilemap);
}

static void ChangeTilemap(const struct TilemapCtrl *unkStruct, u16 *dest, u8 c,
                          bool8 d) {
  u16 i;
  u16 *alloced = Alloc(unkStruct->field_6 * 2 * unkStruct->field_7);
  CpuFill16(unkStruct->field_4, alloced,
            unkStruct->field_6 * 2 * unkStruct->field_7);
...
static void HandleAppealJamTilemap(u16 a, s16 b, u16 move) {
  ...
  if (b == 0 || b == sContestMoveTilemapCtrl.field_6) {
    ChangeTilemap(
        ...
        sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_CONTEST_MOVES][0], b,
        TRUE);
  } else {
...
static void Task_ShowAppealJamWindow(u8 taskId) {
  ...
  if (data[1] <= 0 || data[1] >= sContestMoveTilemapCtrl.field_6) {
    if (data[0] < 0) {
      if (sMonSummaryScreen->currPageIndex == PSS_PAGE_CONTEST_MOVES &&
          FuncIsActiveTask(PssScrollRight) == 0)
...
static void HandleStatusTilemap(u16 a, s16 b) {
  ...
  if (b == 0 || b == sStatusTilemapCtrl1.field_6) {
    ChangeTilemap(&sStatusTilemapCtrl1,
                  sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][0], b,
                  FALSE);
...
static void Task_ShowStatusWindow(u8 taskId) {
  ...
  ChangeTilemap(&sStatusTilemapCtrl1,
                sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][0], data[1],
                FALSE);
  ChangeTilemap(&sStatusTilemapCtrl2,
                sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_INFO][0], data[1],
                FALSE);
...
static void TilemapFiveMovesDisplay(u16 *dst, u16 palette, bool8 remove) {
  ...
  if (!remove) {
    for (i = 0; i < 20; i++) {
      ...
      dst[id + i + 0x40] =
          gSummaryScreen_MoveEffect_Cancel_Tilemap[i + 20] + palette;
    }
  } else // Remove
  {
    for (i = 0; i < 20; i++) {
      ...
      dst[id + i + 0x20] =
          gSummaryScreen_MoveEffect_Cancel_Tilemap[i + 40] + palette;
      dst[id + i + 0x40] =
          gSummaryScreen_MoveEffect_Cancel_Tilemap[i + 40] + palette;
    }
  }
...
static void DrawExperienceProgressBar(struct Pokemon *unused) {
  ...
  if (summary->level < MAX_LEVEL && !levelCappedNuzlocke(summary->level)) {
    u32 expBetweenLevels =
        gExperienceTables[gSpeciesInfo[summary->species].growthRate]
                         [summary->level + 1] -
        gExperienceTables[gSpeciesInfo[summary->species].growthRate]
                         [summary->level];
    ...
    // Calculate the number of 1-pixel "ticks" to illuminate in the experience
    // progress bar. There are 8 tiles that make up the bar, and each tile has 8
    // "ticks". Hence, the numerator is multiplied by 64.
...
static void LimitEggSummaryPageDisplay(
    void) // If the Pokémon is an egg, limit the number of pages displayed to 1
{
...
static void PrintTextOnWindow(u8 windowId, const u8 *string, u8 x, u8 y,
                              u8 lineSpacing, u8 colorId) {
  AddTextPrinterParameterized4(windowId, FONT_NORMAL, x, y, 0, lineSpacing,
                               sTextColors[colorId], 0, string);
}
...
static void PrintNotEggInfo(void) {
  ...
  if (dexNum != 0xFFFF) {
    ...
    ConvertIntToDecimalStringN(gStringVar2, dexNum, STR_CONV_MODE_LEADING_ZEROS,
                               3);
    StringAppend(gStringVar1, gStringVar2);
    if (!IsMonShiny(mon)) {
      PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_DEX_NUMBER, gStringVar1, 0, 1,
                        0, 1);
  ...
  ConvertIntToDecimalStringN(gStringVar2, summary->level,
                             STR_CONV_MODE_LEFT_ALIGN, 3);
  StringAppend(gStringVar1, gStringVar2);
  PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_SPECIES, gStringVar1, 24, 17, 0,
                    1);
  GetMonNickname(mon, gStringVar1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_NICKNAME, gStringVar1, 0, 1, 0,
                    1);
...
static void PrintGenderSymbol(struct Pokemon *mon, u16 species) {
  if (species != SPECIES_NIDORAN_M && species != SPECIES_NIDORAN_F) {
    ...
    case MON_MALE:
      PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_SPECIES, gText_MaleSymbol, 57,
                        17, 0, 3);
      break;
    case MON_FEMALE:
      PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_SPECIES, gText_FemaleSymbol,
                        57, 17, 0, 4);
      break;
    }
  }
...
static void PrintPageNamesAndStats(void) {
  ...
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_INFO_TITLE, gText_PkmnInfo, 2, 1,
                    0, 1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_TITLE, gText_PkmnSkills, 2,
                    1, 0, 1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_BATTLE_MOVES_TITLE, gText_BattleMoves, 2,
                    1, 0, 1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_CONTEST_MOVES_TITLE, gText_ContestMoves, 2,
                    1, 0, 1);
  ...
  PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_CANCEL, gText_Cancel2, stringXPos,
                    1, 0, 0);
  ...
  PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_INFO, gText_Info, stringXPos, 1, 0,
                    0);
  ...
  PrintTextOnWindow(PSS_LABEL_WINDOW_PROMPT_SWITCH, gText_Switch, stringXPos, 1,
                    0, 0);

  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_INFO_RENTAL, gText_RentalPkmn, 0,
                    1, 0, 1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_INFO_TYPE, gText_TypeSlash, 0, 1,
                    0, 0);
  statsXPos = 6 + GetStringCenterAlignXOffset(FONT_NORMAL, gText_HP4, 42);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_LEFT, gText_HP4,
                    statsXPos, 1, 0, 1);
  statsXPos = 6 + GetStringCenterAlignXOffset(FONT_NORMAL, gText_Attack3, 42);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_LEFT, gText_Attack3,
                    statsXPos, 17, 0, 1);
  statsXPos = 6 + GetStringCenterAlignXOffset(FONT_NORMAL, gText_Defense3, 42);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_LEFT, gText_Defense3,
                    statsXPos, 33, 0, 1);
  statsXPos = 2 + GetStringCenterAlignXOffset(FONT_NORMAL, gText_SpAtk4, 36);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_RIGHT, gText_SpAtk4,
                    statsXPos, 1, 0, 1);
  statsXPos = 2 + GetStringCenterAlignXOffset(FONT_NORMAL, gText_SpDef4, 36);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_RIGHT, gText_SpDef4,
                    statsXPos, 17, 0, 1);
  statsXPos = 2 + GetStringCenterAlignXOffset(FONT_NORMAL, gText_Speed2, 36);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATS_RIGHT, gText_Speed2,
                    statsXPos, 33, 0, 1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_EXP, gText_ExpPoints, 6, 1,
                    0, 1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_EXP, gText_NextLv, 6, 17, 0,
                    1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_POKEMON_SKILLS_STATUS, gText_Status, 2, 1,
                    0, 1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_POWER_ACC, gText_Power, 0, 1, 0, 1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_POWER_ACC, gText_Accuracy2, 0, 17, 0,
                    1);
  PrintTextOnWindow(PSS_LABEL_WINDOW_MOVES_APPEAL_JAM, gText_Appeal, 0, 1, 0,
                    1);
...
static u8 AddWindowFromTemplateList(const struct WindowTemplate *template,
                                    u8 templateId) {
...
static void PrintMonOTName(void) {
  int x, windowId;
  if (InBattleFactory() != TRUE && InSlateportBattleTent() != TRUE) {
    windowId = AddWindowFromTemplateList(sPageInfoTemplate,
                                         PSS_DATA_WINDOW_INFO_ORIGINAL_TRAINER);
    ...
    if (sMonSummaryScreen->summary.OTGender == 0)
      PrintTextOnWindow(windowId, sMonSummaryScreen->summary.OTName, x, 1, 0,
                        5);
    else
      PrintTextOnWindow(windowId, sMonSummaryScreen->summary.OTName, x, 1, 0,
                        6);
...
static void PrintMonOTID(void) {
  ...
  if (InBattleFactory() != TRUE && InSlateportBattleTent() != TRUE) {
    ...
    PrintTextOnWindow(
        AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_ID),
        gStringVar1, xPos, 1, 0, 1);
  }
}

static void PrintMonAbilityName(void) {
  u8 ability = GetAbilityBySpecies(sMonSummaryScreen->summary.species,
                                   sMonSummaryScreen->summary.abilityNum);
  PrintTextOnWindow(AddWindowFromTemplateList(sPageInfoTemplate,
                                              PSS_DATA_WINDOW_INFO_ABILITY),
...
static void BufferMonTrainerMemo(void) {
  ...
  if (InBattleFactory() == TRUE || InSlateportBattleTent() == TRUE ||
    ...
    if (DoesMonOTMatchOwner() == TRUE) {
      if (sum->metLevel == 0)
        text = (sum->metLocation >= MAPSEC_NONE)
...
static void PrintMonTrainerMemo(void) {
  PrintTextOnWindow(
      AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_MEMO),
      gStringVar4, 0, 1, 0, 0);
}
...
static void BufferNatureString(void) {
  ...
  DynamicPlaceholderTextUtil_SetPlaceholderPtr(
      2, gNatureNamePointers[sumStruct->summary.nature]);
...
static void PrintEggOTName(void) {
  u32 windowId = AddWindowFromTemplateList(
      sPageInfoTemplate, PSS_DATA_WINDOW_INFO_ORIGINAL_TRAINER);
...
static void PrintEggMemo(void) {
  ...
  if (sMonSummaryScreen->summary.sanity != 1) {
    ...
    else if (DidMonComeFromGBAGames() == FALSE ||
             DoesMonOTMatchOwner() == FALSE)
      text = gText_PeculiarEggTrade;
    else if (sum->metLocation == METLOC_SPECIAL_EGG)
      if (sum->species == SPECIES_TOGEPI)
        text = (DidMonComeFromRSE() == TRUE) ? gText_EggFromFortreeGym
                                             : gText_EggFromTraveler;
      else
        text = (DidMonComeFromRSE() == TRUE) ? gText_EggFromHotSprings
                                             : gText_EggFromTraveler;
    else
...
static void PrintHeldItemName(void) {
  ...
  PrintTextOnWindow(AddWindowFromTemplateList(sPageSkillsTemplate,
                                              PSS_DATA_WINDOW_SKILLS_HELD_ITEM),
                    text, x, 1, 0, 0);
}
...
static void PrintRibbonCount(void) {
  ...
  if (sMonSummaryScreen->summary.ribbonCount == 0) {
    ...
    ConvertIntToDecimalStringN(gStringVar1,
                               sMonSummaryScreen->summary.ribbonCount,
  ...
  PrintTextOnWindow(
      AddWindowFromTemplateList(sPageSkillsTemplate,
                                PSS_DATA_WINDOW_SKILLS_RIBBON_COUNT),
      text, x, 1, 0, 0);
}
...
static void BufferIvOrEvStats(u8 mode) {
  ...
  FillWindowPixelBuffer(
      sMonSummaryScreen->windowIds[PSS_DATA_WINDOW_SKILLS_STATS_LEFT], 0);
  FillWindowPixelBuffer(
      sMonSummaryScreen->windowIds[PSS_DATA_WINDOW_SKILLS_STATS_RIGHT], 0);
...
static void BufferLeftColumnStats(void) {
  ...
  ConvertIntToDecimalStringN(currentHPString,
                             sMonSummaryScreen->summary.currentHP,
                             STR_CONV_MODE_RIGHT_ALIGN, 3);
  ConvertIntToDecimalStringN(maxHPString, sMonSummaryScreen->summary.maxHP,
                             STR_CONV_MODE_RIGHT_ALIGN, 3);
  ConvertIntToDecimalStringN(attackString, sMonSummaryScreen->summary.atk,
                             STR_CONV_MODE_RIGHT_ALIGN, 7);
  ConvertIntToDecimalStringN(defenseString, sMonSummaryScreen->summary.def,
                             STR_CONV_MODE_RIGHT_ALIGN, 7);
  ...
  BufferStat(attackString, natureMod[STAT_ATK - 1],
             sMonSummaryScreen->summary.atk, 2, 7);
  BufferStat(defenseString, natureMod[STAT_DEF - 1],
             sMonSummaryScreen->summary.def, 3, 7);
  DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4,
                                                sStatsLeftColumnLayout);
...
static void PrintLeftColumnStats(void) {
  PrintTextOnWindow(AddWindowFromTemplateList(
                        sPageSkillsTemplate, PSS_DATA_WINDOW_SKILLS_STATS_LEFT),
                    gStringVar4, 4, 1, 0, 0);
}
...
static void BufferRightColumnStats(void) {
  ...
  DynamicPlaceholderTextUtil_Reset();
  BufferStat(gStringVar1, natureMod[STAT_SPATK - 1],
             sMonSummaryScreen->summary.spatk, 0, 3);
  BufferStat(gStringVar2, natureMod[STAT_SPDEF - 1],
             sMonSummaryScreen->summary.spdef, 1, 3);
  BufferStat(gStringVar3, natureMod[STAT_SPEED - 1],
             sMonSummaryScreen->summary.speed, 2, 3);
  DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4,
                                                sStatsRightColumnLayout);
}
...
static void PrintExpPointsNextLevel(void) {
  ...
  ConvertIntToDecimalStringN(gStringVar1, sum->exp, STR_CONV_MODE_RIGHT_ALIGN,
                             7);
  ...
  ConvertIntToDecimalStringN(gStringVar1, expToNextLevel,
                             STR_CONV_MODE_RIGHT_ALIGN, 6);
  x = GetStringRightAlignXOffset(FONT_NORMAL, gStringVar1, 42) + 2;
...
static void PrintMoveNameAndPP(u8 moveIndex) {
  ...
  struct PokeSummary *summary = &sMonSummaryScreen->summary;
  u8 moveNameWindowId =
      AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_NAMES);
  u8 ppValueWindowId =
      AddWindowFromTemplateList(sPageMovesTemplate, PSS_DATA_WINDOW_MOVE_PP);
  u16 move = summary->moves[moveIndex];

  if (move != 0) {
    pp = CalculatePPWithBonus(move, summary->ppBonuses, moveIndex);
    PrintTextOnWindow(moveNameWindowId, gMoveNames[move], 0, moveIndex * 16 + 1,
                      0, 1);
    ConvertIntToDecimalStringN(gStringVar1, summary->pp[moveIndex],
                               STR_CONV_MODE_RIGHT_ALIGN, 2);
    ...
    PrintTextOnWindow(moveNameWindowId, gText_OneDash, 0, moveIndex * 16 + 1, 0,
                      1);
...
static void PrintMovePowerAndAccuracy(u16 moveIndex) {
  const u8 *text;
  if (moveIndex != 0) {
    FillWindowPixelRect(PSS_LABEL_WINDOW_MOVES_POWER_ACC, PIXEL_FILL(0), 53, 0,
                        19, 32);

    if (gBattleMoves[moveIndex].power < 2) {
      text = gText_ThreeDashes;
    } else {
      ConvertIntToDecimalStringN(gStringVar1, gBattleMoves[moveIndex].power,
                                 STR_CONV_MODE_RIGHT_ALIGN, 3);
      text = gStringVar1;
    }
    ...
    if (gBattleMoves[moveIndex].accuracy == 0) {
      ...
      ConvertIntToDecimalStringN(gStringVar1, gBattleMoves[moveIndex].accuracy,
                                 STR_CONV_MODE_RIGHT_ALIGN, 3);
      text = gStringVar1;
    }
...
static void PrintContestMoveDescription(u8 moveSlot) {
  ...
  if (move != MOVE_NONE) {
    u8 windowId = AddWindowFromTemplateList(sPageMovesTemplate,
                                            PSS_DATA_WINDOW_MOVE_DESCRIPTION);
    PrintTextOnWindow(
...
static void PrintMoveDetails(u16 move) {
  ...
  if (move != MOVE_NONE) {
    if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES) {
      ...
      PrintTextOnWindow(windowId, gMoveDescriptionPointers[move - 1], 6, 1, 0,
                        0);
    } else {
      PrintTextOnWindow(
          windowId,
          gContestEffectDescriptionPointers[gContestMoves[move].effect], 6, 1,
          0, 0);
    }
...
static void PrintNewMoveDetailsOrCancelText(void) {
  ...
  if (sMonSummaryScreen->newMove == MOVE_NONE) {
    ...
    ConvertIntToDecimalStringN(gStringVar1, gBattleMoves[move].pp,
                               STR_CONV_MODE_RIGHT_ALIGN, 2);
    DynamicPlaceholderTextUtil_Reset();
    DynamicPlaceholderTextUtil_SetPlaceholderPtr(0, gStringVar1);
    DynamicPlaceholderTextUtil_SetPlaceholderPtr(1, gStringVar1);
    DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4, sMovesPPLayout);
    PrintTextOnWindow(windowId2, gStringVar4,
                      GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 44),
                      65, 0, 12);
  }
}
...
static void HidePageSpecificSprites(void) {
  ...
  for (i = SPRITE_ARR_ID_TYPE; i < ARRAY_COUNT(sMonSummaryScreen->spriteIds);
       i++) {
  ...
  for (i = SPRITE_ARR_ID_TYPE; i < SPRITE_ARR_ID_TYPE + TYPE_ICON_SPRITE_COUNT;
    ...
    if (sMonSummaryScreen->spriteIds[i] == SPRITE_NONE)
      sMonSummaryScreen->spriteIds[i] =
          CreateSprite(&sSpriteTemplate_MoveTypes, 0, 0, 2);
...
void SetTypeSpritePosAndPal(u8 typeId, u8 x, u8 y, u8 spriteArrayId) // HGSS_Ui
{
  struct Sprite *sprite =
      &gSprites[sMonSummaryScreen->spriteIds[spriteArrayId]];
...
static void SetMonTypeIcons(void) {
  ...
  if (summary->isEgg) {
    ...
    SetTypeSpritePosAndPal(gSpeciesInfo[summary->species].types[0], 120, 48,
                           SPRITE_ARR_ID_TYPE);
    if (gSpeciesInfo[summary->species].types[0] !=
        gSpeciesInfo[summary->species].types[1]) {
      SetTypeSpritePosAndPal(gSpeciesInfo[summary->species].types[1], 160, 48,
  ...
  if (!FlagGet(FLAG_SHOW_HIDDEN_POWER)) {
    for (i = 0; i < MAX_MON_MOVES; i++) {
      if (summary->moves[i] != MOVE_NONE)
        SetTypeSpritePosAndPal(gBattleMoves[summary->moves[i]].type, 85,
      ...
      if (summary->moves[i] != MOVE_NONE) {
        if (summary->moves[i] == MOVE_HIDDEN_POWER) {
          ...
          if (type >= TYPE_MYSTERY)
            type++;
          type |= 0xC0;
          SetTypeSpritePosAndPal(type & 0x3F, 85, 32 + (i * 16),
  ...
  for (i = 0; i < MAX_MON_MOVES; i++) {
    if (summary->moves[i] != MOVE_NONE)
      SetTypeSpritePosAndPal(
          NUMBER_OF_MON_TYPES +
              gContestMoves[summary->moves[i]].contestCategory,
...
static void SetNewMoveTypeIcon(void) {
  if (sMonSummaryScreen->newMove == MOVE_NONE) {
    ...
    if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
      SetTypeSpritePosAndPal(gBattleMoves[sMonSummaryScreen->newMove].type, 85,
                             96, SPRITE_ARR_ID_TYPE + 4);
    else
      SetTypeSpritePosAndPal(
          NUMBER_OF_MON_TYPES +
              gContestMoves[sMonSummaryScreen->newMove].contestCategory,
...
static u8 LoadMonGfxAndSprite(struct Pokemon *mon, s16 *state) {
  ...
  case 0:
    if (gMain.inBattle) {
      ...
      if (gMonSpritesGfxPtr != NULL) {
        ...
        if (sMonSummaryScreen->monList.mons == gPlayerParty ||
          ...
          HandleLoadSpecialPokePic_2(
              &gMonFrontPicTable[summary->species2],
              MonSpritesGfxManager_GetSpritePtr(MON_SPR_GFX_MANAGER_A,
    ...
    return 0xFF;
  case 1:
    pal = GetMonSpritePalStructFromOtIdPersonality(summary->species2,
                                                   summary->OTID, summary->pid);
...
// Normally destroys itself but it can be interrupted before the animation
// starts
void SummaryScreen_SetAnimDelayTaskId(u8 taskId) { sAnimDelayTaskId = taskId; }
...
static bool32 UNUSED IsMonAnimationFinished(void) {
  if (gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_MON]].callback ==
      SpriteCallbackDummy)
    return FALSE;
  else
    return TRUE;
...
static void
StopPokemonAnimations(void) // A subtle effect, this function stops Pokémon
                            // animations when leaving the PSS
{
  u16 i;
  u16 paletteIndex;

  gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_MON]].animPaused = TRUE;
  gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_MON]].callback =
      SpriteCallbackDummy;
  StopPokemonAnimationDelayTask();

  paletteIndex = OBJ_PLTT_ID(
      gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_MON]].oam.paletteNum);
...
static void CreateMonMarkingsSprite(struct Pokemon *mon) {
  struct Sprite *sprite = CreateMonMarkingAllCombosSprite(
      TAG_MON_MARKINGS, TAG_MON_MARKINGS, sMarkings_Pal);
...
static void CreateCaughtBallSprite(struct Pokemon *mon) {
  ...
  gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_BALL]].callback =
      SpriteCallbackDummy;
...
static void CreateMoveSelectorSprites(u8 idArrayStart) {
  ...
  if (sMonSummaryScreen->currPageIndex >= PSS_PAGE_BATTLE_MOVES) {
    ...
    for (i = 0; i < MOVE_SELECTOR_SPRITES_COUNT; i++) {
      spriteIds[i] = CreateSprite(&sMoveSelectorSpriteTemplate, i * 16 + 89, 40,
                                  subpriority);
      if (i == 0)
        StartSpriteAnim(&gSprites[spriteIds[i]], 4); // left
      else if (i == 9)
        StartSpriteAnim(&gSprites[spriteIds[i]],
                        5); // right, actually the same as left, but flipped
      else
