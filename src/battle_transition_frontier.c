...
/*
    ...
    2. Small squares with the logo on it are used
   (B_TRANSITION_FRONTIER_SQUARES_*)
    3. The balls that make up the logo come together to form the full logo
   (B_TRANSITION_FRONTIER_CIRCLES_*)

    This file handles category 3. Functions for the other two are handled in
   battle_transition.c
*/
...
static const u32 sLogoCenter_Tilemap[] =
    INCBIN_U32("graphics/battle_transitions/frontier_logo_center.bin.lz");
...
static const struct SpritePalette sSpritePalette_LogoCircles = {
    .data = sLogo_Pal, .tag = PALTAG_LOGO_CIRCLES};

static const union AnimCmd sAnim_LogoCircle_Top[] = {ANIMCMD_FRAME(0, 1),
                                                     ANIMCMD_END};

static const union AnimCmd sAnim_LogoCircle_Left[] = {ANIMCMD_FRAME(64, 1),
                                                      ANIMCMD_END};

static const union AnimCmd sAnim_LogoCircle_Right[] = {ANIMCMD_FRAME(128, 1),
                                                       ANIMCMD_END};
...
static const TransitionStateFunc sFrontierCirclesMeet_Funcs[] = {
    Circles_Init, CirclesMeet_CreateSprites, WaitForLogoCirclesAnim,
    FadeInCenterLogoCircle, CirclesMeet_End};

static const TransitionStateFunc sFrontierCirclesCross_Funcs[] = {
    Circles_Init, CirclesCross_CreateSprites, WaitForLogoCirclesAnim,
...
static const TransitionStateFunc sFrontierCirclesAsymmetricSpiral_Funcs[] = {
    ...
    FadeInCenterLogoCircle, CirclesAsymmetricSpiral_End};

static const TransitionStateFunc sFrontierCirclesSymmetricSpiral_Funcs[] = {
    Circles_Init, CirclesSymmetricSpiral_CreateSprites, WaitForLogoCirclesAnim,
    FadeInCenterLogoCircle, CirclesSymmetricSpiral_End};

static const TransitionStateFunc sFrontierCirclesMeetInSeq_Funcs[] = {
    Circles_Init, CirclesMeetInSeq_CreateSprites, WaitForLogoCirclesAnim,
    FadeInCenterLogoCircle, CirclesMeetInSeq_End};

static const TransitionStateFunc sFrontierCirclesCrossInSeq_Funcs[] = {
    Circles_Init, CirclesCrossInSeq_CreateSprites, WaitForLogoCirclesAnim,
    FadeInCenterLogoCircle, CirclesCrossInSeq_End};
...
static const TransitionStateFunc sFrontierCirclesSymmetricSpiralInSeq_Funcs[] =
    {Circles_Init, CirclesSymmetricSpiralInSeq_CreateSprites,
     WaitForLogoCirclesAnim, FadeInCenterLogoCircle,
...
static u8 CreateSlidingLogoCircleSprite(s16 x, s16 y, u8 delayX, u8 delayY,
                                        s8 speedX, s8 speedY,
                                        u8 spriteAnimNum) {
...
static u8 CreateSpiralingLogoCircleSprite(s16 x, s16 y, s16 angle,
                                          s16 rotateSpeed, s16 radiusStart,
                                          s16 radiusEnd, s16 radiusDelta,
                                          u8 spriteAnimNum) {
  u8 spriteId = CreateSprite(&sSpriteTemplate_LogoCircles, x, y, 0);
...
static bool8 CirclesMeet_CreateSprites(struct Task *task) {
  task->tCircle1SpriteId =
      CreateSlidingLogoCircleSprite(120, -51, 0, 0, 0, 2, 0);
  task->tCircle2SpriteId =
      CreateSlidingLogoCircleSprite(-7, 193, 0, 0, 2, -2, 1);
  task->tCircle3SpriteId =
      CreateSlidingLogoCircleSprite(247, 193, 0, 0, -2, -2, 2);
...
static bool8 CirclesCross_CreateSprites(struct Task *task) {
  task->tCircle1SpriteId =
      CreateSlidingLogoCircleSprite(120, 197, 0, 0, 0, -4, 0);
  task->tCircle2SpriteId =
      CreateSlidingLogoCircleSprite(241, 59, 0, 1, -4, 2, 1);
...
void Task_FrontierCirclesAsymmetricSpiral(u8 taskId) {
  while (sFrontierCirclesAsymmetricSpiral_Funcs[gTasks[taskId].tState](
      &gTasks[taskId]))
    ;
}
...
static bool8 CirclesAsymmetricSpiral_CreateSprites(struct Task *task) {
  task->tCircle1SpriteId =
      CreateSpiralingLogoCircleSprite(120, 45, 12, 4, 128, 0, -4, 0);
  task->tCircle2SpriteId =
      CreateSpiralingLogoCircleSprite(89, 97, 252, 4, 128, 0, -4, 1);
  task->tCircle3SpriteId =
      CreateSpiralingLogoCircleSprite(151, 97, 132, 4, 128, 0, -4, 2);
...
void Task_FrontierCirclesSymmetricSpiral(u8 taskId) {
  while (sFrontierCirclesSymmetricSpiral_Funcs[gTasks[taskId].tState](
      &gTasks[taskId]))
    ;
}
...
static bool8 CirclesSymmetricSpiral_CreateSprites(struct Task *task) {
  task->tCircle1SpriteId =
      CreateSpiralingLogoCircleSprite(120, 80, 284, 8, 131, 35, -3, 0);
  task->tCircle2SpriteId =
      CreateSpiralingLogoCircleSprite(120, 80, 44, 8, 131, 35, -3, 1);
  task->tCircle3SpriteId =
      CreateSpiralingLogoCircleSprite(121, 80, 164, 8, 131, 35, -3, 2);
...
void Task_FrontierCirclesMeetInSeq(u8 taskId) {
  while (
      sFrontierCirclesMeetInSeq_Funcs[gTasks[taskId].tState](&gTasks[taskId]))
    ;
}
...
static bool8 CirclesMeetInSeq_CreateSprites(struct Task *task) {
  if (task->tTimer == 0) {
    task->tCircle1SpriteId =
        CreateSlidingLogoCircleSprite(120, -51, 0, 0, 0, 4, 0);
  } else if (task->tTimer == 16) {
    task->tCircle2SpriteId =
        CreateSlidingLogoCircleSprite(-7, 193, 0, 0, 4, -4, 1);
  } else if (task->tTimer == 32) {
    task->tCircle3SpriteId =
        CreateSlidingLogoCircleSprite(247, 193, 0, 0, -4, -4, 2);
    task->tState++;
  }
...
void Task_FrontierCirclesCrossInSeq(u8 taskId) {
  while (
      sFrontierCirclesCrossInSeq_Funcs[gTasks[taskId].tState](&gTasks[taskId]))
    ;
}
...
static bool8 CirclesCrossInSeq_CreateSprites(struct Task *task) {
  if (task->tTimer == 0) {
    task->tCircle1SpriteId =
        CreateSlidingLogoCircleSprite(120, 197, 0, 0, 0, -8, 0);
  } else if (task->tTimer == 16) {
    task->tCircle2SpriteId =
        CreateSlidingLogoCircleSprite(241, 78, 0, 0, -8, 1, 1);
  } else if (task->tTimer == 32) {
    task->tCircle3SpriteId =
        CreateSlidingLogoCircleSprite(-1, 78, 0, 0, 8, 1, 2);
    task->tState++;
  }
...
void Task_FrontierCirclesAsymmetricSpiralInSeq(u8 taskId) {
  while (sFrontierCirclesAsymmetricSpiralInSeq_Funcs[gTasks[taskId].tState](
      &gTasks[taskId]))
    ;
}
...
void Task_FrontierCirclesSymmetricSpiralInSeq(u8 taskId) {
  while (sFrontierCirclesSymmetricSpiralInSeq_Funcs[gTasks[taskId].tState](
      &gTasks[taskId]))
    ;
}
