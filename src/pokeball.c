...
static void Task_DoPokeballSendOutAnim(u8 taskId) {
  ...
  if (GetBattlerSide(battlerId) != B_SIDE_PLAYER)
    itemId = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]],
                        MON_DATA_POKEBALL);
  else
    itemId = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                        MON_DATA_POKEBALL);
  ...
  case POKEBALL_OPPONENT_SENDOUT:
    gSprites[ballSpriteId].x =
        GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X);
    gSprites[ballSpriteId].y =
        GetBattlerSpriteCoord(battlerId, BATTLER_COORD_Y) + 24;
    gBattlerTarget = battlerId;
  ...
  gSprites[ballSpriteId].data[2] =
      GetBattlerSpriteCoord(gBattlerTarget, BATTLER_COORD_X);
  gSprites[ballSpriteId].data[4] =
      GetBattlerSpriteCoord(gBattlerTarget, BATTLER_COORD_Y) - 16;
...
static void SpriteCB_BallThrow_StartShrinkMon(struct Sprite *sprite) {
  if (++sprite->data[5] == 10) {
    ...
    StartSpriteAffineAnim(&gSprites[gBattlerSpriteIds[sprite->sBattler]],
...
static void SpriteCB_BallThrow_ShrinkMon(struct Sprite *sprite) {
  ...
  if (gSprites[gBattlerSpriteIds[sprite->sBattler]].affineAnimEnded) {
    ...
    gSprites[gBattlerSpriteIds[sprite->sBattler]].y2 =
...
static void Task_PlayCryWhenReleasedFromBall(u8 taskId) {
  ...
  struct Pokemon *mon = (void *)(u32)((gTasks[taskId].tCryTaskMonPtr1 << 16) |
...
static void SpriteCB_ReleaseMonFromBall(struct Sprite *sprite) {
  ...
  if (GetBattlerSide(sprite->sBattler) == B_SIDE_OPPONENT)
    gSprites[gBattlerSpriteIds[sprite->sBattler]].callback =
...
static void HandleBallAnimEnd(struct Sprite *sprite) {
  ...
  if (gSprites[gBattlerSpriteIds[battlerId]].affineAnimEnded) {
    StartSpriteAffineAnim(&gSprites[gBattlerSpriteIds[battlerId]],
                          BATTLER_AFFINE_NORMAL);
    affineAnimEnded = TRUE;
  } else {
    gSprites[gBattlerSpriteIds[battlerId]].data[1] -= 288;
    gSprites[gBattlerSpriteIds[battlerId]].y2 =
...
static void SpriteCB_PlayerMonSendOut_1(struct Sprite *sprite) {
  ...
  sprite->data[4] =
      GetBattlerSpriteCoord(sprite->sBattler, BATTLER_COORD_Y_PIC_OFFSET) + 24;
  sprite->data[5] = -30;
...
static u8 AnimateBallOpenParticlesForPokeball(u8 x, u8 y, u8 kindOfStars,
                                              u8 subpriority) {
  return AnimateBallOpenParticles(x, y, kindOfStars, subpriority, BALL_POKE);
}

static u8 LaunchBallFadeMonTaskForPokeball(bool8 unFadeLater, u8 spritePalNum,
                                           u32 selectedPalettes) {
  return LaunchBallFadeMonTask(unFadeLater, spritePalNum, selectedPalettes,
                               BALL_POKE);
}
...
void CreatePokeballSpriteToReleaseMon(u8 monSpriteId, u8 monPalNum, u8 x, u8 y,
                                      u8 oamPriority, u8 subpriority, u8 delay,
                                      u32 fadePalettes, u16 species) {
  u8 spriteId;
...
static void SpriteCB_PokeballReleaseMon(struct Sprite *sprite) {
  if (sprite->sDelay == 0) {
    ...
    u32 selectedPalettes =
        (u16)sprite->sFadePalsLo | ((u16)sprite->sFadePalsHi << 16);
    ...
    StartSpriteAnim(sprite, 1);
    AnimateBallOpenParticlesForPokeball(sprite->x, sprite->y - 5,
                                        sprite->oam.priority, subpriority);
    // sDelay re-used to store task id but never read
    sprite->sDelay =
        LaunchBallFadeMonTaskForPokeball(TRUE, monPalNum, selectedPalettes);
...
static void SpriteCB_ReleasedMonFlyOut(struct Sprite *sprite) {
  ...
  if (sprite->animEnded && emergeAnimFinished && atFinalPosition) {
    if (gSprites[monSpriteId].sSpecies == SPECIES_EGG)
      DoMonFrontSpriteAnimation(&gSprites[monSpriteId],
                                gSprites[monSpriteId].sSpecies, TRUE, 0);
    else
      DoMonFrontSpriteAnimation(&gSprites[monSpriteId],
                                gSprites[monSpriteId].sSpecies, FALSE, 0);
...
u8 CreateTradePokeballSprite(u8 monSpriteId, u8 monPalNum, u8 x, u8 y,
                             u8 oamPriority, u8 subPriority, u8 delay,
                             u32 fadePalettes) {
  u8 spriteId;
...
void StartHealthboxSlideIn(u8 battlerId) {
  ...
  gSprites[healthboxSprite->data[5]].callback(
...
static u16 GetBattlerPokeballItemId(u8 battlerId) {
  if (GetBattlerSide(battlerId) == B_SIDE_PLAYER)
    return GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                      MON_DATA_POKEBALL);
  else
    return GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]],
                      MON_DATA_POKEBALL);
}
