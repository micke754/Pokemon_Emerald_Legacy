...
// At any one time, the currently selected mon and its two adjacent neighbors
// can be loaded IDs to refer to one of these 3 are called "load id" in this
// file
...
// This struct is identical to PokenavMonListItem, the struct used for managing
// lists of Pokémon in the PokéNav Given that this screen is essentially
// duplicated in the poknav, this struct was probably the same one with a more
...
struct UsePokeblockMenuPokemon {
  u8 boxId; // Because this screen is never used for the PC this is always set
            // to TOTAL_BOXES_COUNT to refer to party
  u8 monId;
  u16 data; // never read
};
...
struct UsePokeblockMenu {
  ...
  u8 locationStrings[NUM_SELECTIONS_LOADED]
                    [24]; // Gets an "in party" or "in box #" string that never
                          // gets printed
...
static void CalculatePokeblockEffectiveness(struct Pokeblock *,
                                            struct Pokemon *);
...
// The below 3 are saved for returning to the screen after feeding a pokeblock
// to a mon so that the rest of the data can be freed
...
static const u32 sMonFrame_Pal[] =
    INCBIN_U32("graphics/pokeblock/use_screen/mon_frame_pal.bin");
static const u32 sMonFrame_Gfx[] =
    INCBIN_U32("graphics/pokeblock/use_screen/mon_frame.4bpp");
static const u32 sMonFrame_Tilemap[] =
    INCBIN_U32("graphics/pokeblock/use_screen/mon_frame.bin.lz");
static const u32 sGraphData_Tilemap[] =
    INCBIN_U32("graphics/pokeblock/use_screen/graph_data.bin.lz");

// The condition/flavors aren't listed in their normal order in this file,
// they're listed as shown on the graph going counter-clockwise Normally they
// would go Cool/Spicy, Beauty/Dry, Cute/Sweet, Smart/Bitter, Tough/Sour (also
// graph order, but clockwise)
...
static const u8 sNatureTextColors[] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_BLUE,
                                       TEXT_COLOR_WHITE};
...
static const struct SpriteSheet sSpriteSheet_UpDown = {gUsePokeblockUpDown_Gfx,
                                                       0x200, TAG_UP_DOWN};
...
static const union AnimCmd sAnim_Condition_0[] = {ANIMCMD_FRAME(0, 5),
                                                  ANIMCMD_END};

static const union AnimCmd sAnim_Condition_1[] = {ANIMCMD_FRAME(32, 5),
                                                  ANIMCMD_END};

static const union AnimCmd sAnim_Condition_2[] = {ANIMCMD_FRAME(64, 5),
                                                  ANIMCMD_END};
...
void ChooseMonToGivePokeblock(struct Pokeblock *pokeblock,
                              void (*callback)(void)) {
...
static void LoadUsePokeblockMenu(void) {
  ...
  case 10:
    ...
    break;
  case 11:
    ConditionGraph_CalcPositions(sMenu->graph.conditions[0],
                                 sMenu->graph.savedPositions[0]);
    ConditionGraph_InitResetScanline(&sMenu->graph);
    sInfo->mainState++;
    break;
  case 12:
    if (!ConditionGraph_ResetScanline(&sMenu->graph)) {
      ConditionGraph_SetNewPositions(&sMenu->graph,
                                     sMenu->graph.savedPositions[0],
                                     sMenu->graph.savedPositions[0]);
      sInfo->mainState++;
    }
  ...
  case 1:
    if (!gPaletteFade.active) {
      ...
      if (sMenu->info.curSelection != sMenu->info.numSelections - 1) {
        ...
        CreateConditionSparkleSprites(sMenu->sparkles, sMenu->curMonSpriteId,
                                      numSparkles);
      }
...
static void ShowPokeblockResults(void) {
  ...
  case 2:
    ...
    ConditionGraph_CalcPositions(
        sInfo->conditionsAfterBlock,
        sMenu->graph.savedPositions[CONDITION_GRAPH_LOAD_MAX - 1]);
    ConditionGraph_SetNewPositions(
        &sMenu->graph, sMenu->graph.savedPositions[sMenu->curLoadId],
        sMenu->graph.savedPositions[CONDITION_GRAPH_LOAD_MAX - 1]);
  ...
  case 3:
    if (!ConditionGraph_TryUpdate(&sMenu->graph)) {
      CalculateNumAdditionalSparkles(
          GetPartyIdFromSelectionId(sMenu->info.curSelection));
      if (sMenu->info.curSelection != sMenu->info.numSelections - 1) {
        u8 numSparkles = sMenu->numSparkles[sMenu->curLoadId];
        CreateConditionSparkleSprites(sMenu->sparkles, sMenu->curMonSpriteId,
                                      numSparkles);
...
static void AskUsePokeblock(void) {
  ...
  GetMonData(&gPlayerParty[GetPartyIdFromSelectionId(sMenu->info.curSelection)],
             MON_DATA_NICKNAME, stringBuffer);
  ...
  AddTextPrinterParameterized(WIN_TEXT, FONT_NORMAL, gStringVar4, 0, 1, 0,
                              NULL);
...
static void PrintFirstEnhancement(void) {
  ...
  for (sInfo->condition = 0; sInfo->condition < CONDITION_COUNT;
       sInfo->condition++) {
  ...
  if (sInfo->condition < CONDITION_COUNT)
    BufferEnhancedText(gStringVar4, sInfo->condition,
                       sInfo->enhancements[sInfo->condition]);
  else
...
static void PrintWontEatAnymore(void) {
  FillWindowPixelBuffer(WIN_TEXT, 17);
  DrawTextBorderOuter(WIN_TEXT, 151, 14);
  AddTextPrinterParameterized(WIN_TEXT, FONT_NORMAL, gText_WontEatAnymore, 0, 1,
                              0, NULL);
  PutWindowTilemap(WIN_TEXT);
  CopyWindowToVram(WIN_TEXT, COPYWIN_FULL);
}
...
static void AddPokeblockToConditions(struct Pokeblock *pokeblock,
                                     struct Pokemon *mon) {
  u16 i;
  s16 stat;
  u8 data;
...
static void CalculatePokeblockEffectiveness(struct Pokeblock *pokeblock,
                                            struct Pokemon *mon) {
...
// Eggs are not viewable on the condition screen, so count how many are skipped
// over to reach the party id
...
static void LoadAndCreateUpDownSprites(void) {
  ...
  for (i = 0; i < CONDITION_COUNT; i++) {
    if (sInfo->enhancements[i] != 0) {
      u16 spriteId =
          CreateSprite(&sSpriteTemplate_UpDown, sUpDownCoordsOnGraph[i][0],
                       sUpDownCoordsOnGraph[i][1], 0);
...
static void LoadMonInfo(s16 partyId, u8 loadId) {
  ...
  bool8 excludesCancel =
      FALSE; // whether or not numSelections excludes Cancel from the count

  GetConditionMenuMonNameAndLocString(
      sMenu->locationStrings[loadId], sMenu->monNameStrings[loadId], boxId,
      monId, partyId, numSelections, excludesCancel);
  GetConditionMenuMonConditions(&sMenu->graph, sMenu->numSparkles, boxId, monId,
                                partyId, loadId, numSelections, excludesCancel);
  GetConditionMenuMonGfx(sMenu->partySheets[loadId],
                         sMenu->partyPalettes[loadId], boxId, monId, partyId,
...
static void UpdateMonPic(u8 loadId) {
  ...
  if (sMenu->curMonSpriteId == SPRITE_NONE) {
    ...
    Dma3CopyLarge16_(sMenu->partySheets[loadId], sMenu->curMonTileStart,
                     MON_PIC_SIZE);
    LoadPalette(sMenu->partyPalettes[loadId], sMenu->curMonPalette,
                PLTT_SIZE_4BPP);
  }
}
...
static bool8 LoadUsePokeblockMenuGfx(void) {
  ...
  case 0:
    ...
    SetGpuReg(REG_OFFSET_BLDCNT,
              BLDCNT_TGT1_BG2 | BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_BG1);
...
static void UpdateMonInfoText(u16 loadId, bool8 firstPrint) {
  ...
  if (sMenu->info.curSelection != sMenu->info.numSelections - 1) {
    AddTextPrinterParameterized(WIN_NAME, FONT_NORMAL,
                                sMenu->monNameStrings[loadId], 0, 1, 0, NULL);
    ...
    AddTextPrinterParameterized3(WIN_NATURE, FONT_NORMAL, 2, 1,
                                 sNatureTextColors, 0, sMenu->info.natureText);
  }
...
static void UpdateSelection(bool8 up) {
  ...
  ConditionGraph_SetNewPositions(&sMenu->graph,
                                 sMenu->graph.savedPositions[sMenu->curLoadId],
                                 sMenu->graph.savedPositions[newLoadId]);
  ...
  if (up) {
    sMenu->prevLoadId = sMenu->nextLoadId; // temporarily store nextLoadId,
                                           // prevLoadId no longer needed
    sMenu->nextLoadId = sMenu->curLoadId;
    sMenu->curLoadId = newLoadId;
    sMenu->toLoadId = sMenu->prevLoadId; // next load will be the mon that's one
                                         // up from new selection
    ...
    sMenu->nextLoadId = sMenu->prevLoadId; // temporarily store prevLoadId,
                                           // nextLoadId no longer needed
    sMenu->prevLoadId = sMenu->curLoadId;
    sMenu->curLoadId = newLoadId;
    sMenu->toLoadId = sMenu->nextLoadId; // next load will be the mon that's one
                                         // down from new selection

    // Check for wrap to top of list
    sMenu->info.curSelection =
        (sMenu->info.curSelection < sMenu->info.numSelections - 1)
            ? sMenu->info.curSelection + 1
            : 0;
...
// Calculate the max id for sparkles/stars that appear around the Pokémon on the
// condition screen All Pokémon start with 1 sparkle (added by
// CreateConditionSparkleSprites), so the number here +1 is the total number of
...
// Literally the word "Condition", the title block that appears over the mon
// icon
