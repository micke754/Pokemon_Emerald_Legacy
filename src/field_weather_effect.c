...
const u16 gSandstormWeatherPalette[] =
    INCBIN_U16("graphics/weather/sandstorm.gbapal");
const u8 gWeatherFogDiagonalTiles[] =
    INCBIN_U8("graphics/weather/fog_diagonal.4bpp");
const u8 gWeatherFogHorizontalTiles[] =
    INCBIN_U8("graphics/weather/fog_horizontal.4bpp");
...
const u8 gWeatherSandstormTiles[] =
    INCBIN_U8("graphics/weather/sandstorm.4bpp");
...
void Sunny_InitAll(void) { Sunny_InitVars(); }

void Sunny_Main(void) {}

bool8 Sunny_Finish(void) { return FALSE; }
...
static void CreateCloudSprites(void) {
  ...
  for (i = 0; i < NUM_CLOUD_SPRITES; i++) {
    ...
    if (spriteId != MAX_SPRITES) {
      ...
      SetSpritePosToMapCoords(sCloudSpriteMapCoords[i].x + MAP_OFFSET,
                              sCloudSpriteMapCoords[i].y + MAP_OFFSET,
                              &sprite->x, &sprite->y);
...
bool8 Drought_Finish(void) { return FALSE; }

void StartDroughtWeatherBlend(void) { CreateTask(UpdateDroughtBlend, 80); }
...
static void UpdateDroughtBlend(u8 taskId) {
  ...
  case 0:
    ...
    SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_TGT1_BG1 | BLDCNT_TGT1_BG2 |
                                     BLDCNT_TGT1_BG3 | BLDCNT_TGT1_OBJ |
...
static const struct Coords16 sRainSpriteCoords[] = {
    {0, 0},    {0, 160},   {0, 64},   {144, 224}, {144, 128}, {32, 32},
    {32, 192}, {32, 96},   {72, 128}, {72, 32},   {72, 192},  {216, 96},
    {216, 0},  {104, 160}, {104, 64}, {104, 224}, {144, 0},   {144, 160},
    {144, 64}, {32, 224},  {32, 128}, {72, 32},   {72, 192},  {48, 96},
};
...
static void StartRainSpriteFall(struct Sprite *sprite) {
  ...
  sprite->tPosX = tileX * 8; // Useless assignment, leftover from before
                             // fixed-point values were used

  tileY = sprite->tRandom / 30;
  sprite->tPosY = tileY * 8; // Useless assignment, leftover from before
                             // fixed-point values were used

  sprite->tPosX = tileX;
  sprite->tPosX <<=
      7; // This is tileX * 8, using a fixed-point value with 4 decimal places
  ...
  sprite->tPosX -=
      sRainSpriteMovement[gWeatherPtr->isDownpour][0] * numFallingFrames;
  sprite->tPosY -=
      sRainSpriteMovement[gWeatherPtr->isDownpour][1] * numFallingFrames;
...
static void InitRainSpriteMovement(struct Sprite *sprite, u16 val) {
  u16 numFallingFrames =
      sRainSpriteFallingDurations[gWeatherPtr->isDownpour][0];
  u16 numAdvanceRng =
      val / (sRainSpriteFallingDurations[gWeatherPtr->isDownpour][1] +
             numFallingFrames);
  u16 frameVal =
      val % (sRainSpriteFallingDurations[gWeatherPtr->isDownpour][1] +
             numFallingFrames);
...
static void LoadRainSpriteSheet(void) { LoadSpriteSheet(&sRainSpriteSheet); }
...
static bool8 CreateRainSprite(void) {
  ...
  spriteId =
      CreateSpriteAtEnd(&sRainSpriteTemplate, sRainSpriteCoords[spriteIndex].x,
...
static bool8 UpdateVisibleRainSprites(void) {
  ...
  if (++gWeatherPtr->rainSpriteVisibleCounter >
      gWeatherPtr->rainSpriteVisibleDelay) {
    gWeatherPtr->rainSpriteVisibleCounter = 0;
    if (gWeatherPtr->curRainSpriteIndex < gWeatherPtr->targetRainSpriteCount) {
...
static bool8 UpdateVisibleSnowflakeSprites(void) {
  if (gWeatherPtr->snowflakeSpriteCount ==
      gWeatherPtr->targetSnowflakeSpriteCount)
    return FALSE;
  ...
  return gWeatherPtr->snowflakeSpriteCount !=
         gWeatherPtr->targetSnowflakeSpriteCount;
}
...
static bool8 CreateSnowflakeSprite(void) {
  ...
  gWeatherPtr->sprites.s1
      .snowflakeSprites[gWeatherPtr->snowflakeSpriteCount++] =
      &gSprites[spriteId];
  return TRUE;
}
...
static bool8 DestroySnowflakeSprite(void) {
  if (gWeatherPtr->snowflakeSpriteCount) {
    DestroySprite(gWeatherPtr->sprites.s1
                      .snowflakeSprites[--gWeatherPtr->snowflakeSpriteCount]);
    return TRUE;
  }
...
static const union AnimCmd *const sAnims_FogH[] = {
    sAnim_FogH_0, sAnim_FogH_1, sAnim_FogH_2,
    sAnim_FogH_3, sAnim_FogH_4, sAnim_FogH_5,
};
...
void FogHorizontal_Main(void) {
  gWeatherPtr->fogHScrollPosX =
      (gSpriteCoordOffsetX - gWeatherPtr->fogHScrollOffset) & 0xFF;
...
static void FogHorizontalSpriteCallback(struct Sprite *sprite) {
  ...
  if (sprite->x >= DISPLAY_WIDTH + 32) {
    sprite->x = (DISPLAY_WIDTH * 2) + gWeatherPtr->fogHScrollPosX -
                (4 - sprite->tSpriteColumn) * 64;
    sprite->x &= 0x1FF;
  }
}
...
void Ash_InitVars(void) {
  ...
  if (!gWeatherPtr->ashSpritesCreated) {
    ...
    SetGpuReg(REG_OFFSET_BLDALPHA,
              BLDALPHA_BLEND(64, 63)); // These aren't valid blend coefficients!
  }
}
...
static void LoadAshSpriteSheet(void) { LoadSpriteSheet(&sAshSpriteSheet); }
...
static void UpdateAshSprite(struct Sprite *sprite) {
  ...
  if (sprite->x >= DISPLAY_WIDTH + 32) {
    sprite->x = gWeatherPtr->ashBaseSpritesX + (DISPLAY_WIDTH * 2) -
                (4 - sprite->tSpriteColumn) * 64;
    sprite->x &= 0x1FF;
  }
}
...
static void UpdateFogDiagonalMovement(void) {
  ...
  gWeatherPtr->fogDBaseSpritesX =
      (gSpriteCoordOffsetX - gWeatherPtr->fogDXOffset) & 0xFF;
...
static void CreateFogDiagonalSprites(void) {
  ...
  if (!gWeatherPtr->fogDSpritesCreated) {
    ...
    for (i = 0; i < NUM_FOG_DIAGONAL_SPRITES; i++) {
      spriteId =
          CreateSpriteAtEnd(&sFogDiagonalSpriteTemplate, 0, (i / 5) * 64, 0xFF);
      if (spriteId != MAX_SPRITES) {
        sprite = &gSprites[spriteId];
        sprite->tSpriteColumn = i % 5;
...
static void UpdateFogDiagonalSprite(struct Sprite *sprite) {
  ...
  if (sprite->x >= DISPLAY_WIDTH + 32) {
    sprite->x = gWeatherPtr->fogDBaseSpritesX + (DISPLAY_WIDTH * 2) -
                (4 - sprite->tSpriteColumn) * 64;
    sprite->x &= 0x1FF;
  }
}
...
static void UpdateSandstormMovement(void) {
  gWeatherPtr->sandstormXOffset -=
      gSineTable[gWeatherPtr->sandstormWaveIndex] * 4;
  ...
  gWeatherPtr->sandstormPosY =
      gSpriteCoordOffsetY + (gWeatherPtr->sandstormYOffset >> 8);
}
...
static void CreateSandstormSprites(void) {
  ...
  if (!gWeatherPtr->sandstormSpritesCreated) {
    ...
    for (i = 0; i < NUM_SANDSTORM_SPRITES; i++) {
      spriteId =
          CreateSpriteAtEnd(&sSandstormSpriteTemplate, 0, (i / 5) * 64, 1);
      if (spriteId != MAX_SPRITES) {
...
static void CreateSwirlSandstormSprites(void) {
  ...
  if (!gWeatherPtr->sandstormSwirlSpritesCreated) {
    for (i = 0; i < NUM_SWIRL_SANDSTORM_SPRITES; i++) {
      spriteId =
          CreateSpriteAtEnd(&sSandstormSpriteTemplate, i * 48 + 24, 208, 1);
      if (spriteId != MAX_SPRITES) {
        ...
        gWeatherPtr->sprites.s2.sandstormSprites2[i]->data[4] =
            0x6730; // unused value
        ...
        CalcCenterToCornerVec(gWeatherPtr->sprites.s2.sandstormSprites2[i],
                              SPRITE_SHAPE(32x32), SPRITE_SIZE(32x32),
                              ST_OAM_AFFINE_OFF);
...
static void UpdateSandstormSprite(struct Sprite *sprite) {
  ...
  sprite->x =
      gWeatherPtr->sandstormBaseSpritesX + 32 + sprite->tSpriteColumn * 64;
  if (sprite->x >= DISPLAY_WIDTH + 32) {
    sprite->x = gWeatherPtr->sandstormBaseSpritesX + (DISPLAY_WIDTH * 2) -
                (4 - sprite->tSpriteColumn) * 64;
    sprite->x &= 0x1FF;
  }
}
...
void Shade_InitAll(void) { Shade_InitVars(); }

void Shade_Main(void) {}

bool8 Shade_Finish(void) { return FALSE; }
...
static const s16 sBubbleStartCoords[][2] = {
    {120, 160}, {376, 160}, {40, 140},  {296, 140}, {180, 130},
    {436, 130}, {60, 160},  {436, 160}, {220, 180}, {476, 180},
    {10, 90},   {266, 90},  {256, 160},
};
...
void Bubbles_Main(void) {
  FogHorizontal_Main();
  if (++gWeatherPtr->bubblesDelayCounter >
      sBubbleStartDelays[gWeatherPtr->bubblesDelayIndex]) {
...
u8 GetSavedWeather(void) { return gSaveBlock1Ptr->weather; }
