...
enum { PHRASE_CHANGED, PHRASE_NO_CHANGE, PHRASE_EMPTY };
...
void DoWaldaNamingScreen(void) {
  ...
  DoNamingScreen(NAMING_SCREEN_WALDA, gStringVar2, 0, 0, 0,
...
u16 TryGetWallpaperWithWaldaPhrase(void) {
  ...
  gSpecialVar_Result =
      TryCalculateWallpaper(&backgroundClr, &foregroundClr, &iconId, &patternId,
                            trainerId, GetWaldaPhrasePtr());
...
// Returns TRUE if successful and sets the wallpaper results to the given
// pointers. Returns FALSE if no wallpaper was generated (Walda "didn't like"
// the phrase). A 9-byte array is used to calculate the wallpaper's data. The
// elements of this array are defined below.
...
#define TO_BIT_OFFSET(i)                                                       \
  (3 + (8 * (i))) // Convert a position in the phrase to a bit number into the
                  // wallpaper data array
static bool32 TryCalculateWallpaper(u16 *backgroundClr, u16 *foregroundClr,
                                    u8 *iconId, u8 *patternId, u16 trainerId,
                                    u8 *phrase) {
  s32 i;
  ...
  // The data array is 9 bytes (72 bits) long, and each letter contributes to 5
  // bits of the array Because the phrase is 15 letters long there are 75 bits
  // from the phrase to distribute Therefore the last letter contributes to the
  // last 2 bits of the array, and the remaining 3 bits wrap around
  for (i = 0; i < WALDA_PHRASE_LENGTH - 1; i++)
    SetWallpaperDataFromLetter(data, charsByTableId, BITS_PER_LETTER * i,
                               TO_BIT_OFFSET(i), BITS_PER_LETTER);

  // Do first 2 bits of the last letter
  SetWallpaperDataFromLetter(data, charsByTableId,
                             BITS_PER_LETTER * (WALDA_PHRASE_LENGTH - 1),
                             TO_BIT_OFFSET(WALDA_PHRASE_LENGTH - 1), 2);

  // Check the first 3 bits of the data array against the remaining 3 bits of
  // the last letter Reject the phrase if they are not already the same
  if (GetWallpaperDataBits(data, 0, 3) !=
      GetWallpaperDataBits(charsByTableId,
                           TO_BIT_OFFSET(WALDA_PHRASE_LENGTH - 1) + 2, 3))
    return FALSE;

  // Perform some relatively arbitrary changes to the wallpaper data using the
  // last byte (KEY)
  ...
  // Reject the results of any phrase that are 'incompatible' with the player's
  // trainer id
  if (TID_CHECK_HI != (BG_COLOR_LO ^ FG_COLOR_LO ^ ICON_ID ^ (trainerId >> 8)))
    return FALSE;
  if (TID_CHECK_LO !=
      (BG_COLOR_HI ^ FG_COLOR_HI ^ PATTERN_ID ^ (trainerId & 0xFF)))
    return FALSE;
...
static void SetWallpaperDataFromLetter(u8 *data, u8 *letterTableIds,
                                       u32 setOffset, u32 getOffset,
                                       u32 numBits) {
  u32 i;
