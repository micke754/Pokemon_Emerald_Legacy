...
bool32 PokenavCallback_Init_MainMenuCursorOnMap(void) {
  struct Pokenav_Menu *menu = AllocSubstruct(
      POKENAV_SUBSTRUCT_MAIN_MENU_HANDLER, sizeof(struct Pokenav_Menu));
  if (!menu)
    return FALSE;
...
u32 GetMenuHandlerCallback(void) {
  struct Pokenav_Menu *menu =
      GetSubstructPtr(POKENAV_SUBSTRUCT_MAIN_MENU_HANDLER);
...
static u32 HandleMainMenuInput(struct Pokenav_Menu *menu) {
  ...
  if (JOY_NEW(A_BUTTON)) {
    ...
    case POKENAV_MENUITEM_MAP:
      menu->helpBarIndex = gSaveBlock2Ptr->regionMapZoom
...
// Force the player to select Match Call during the call Mr. Stone PokéNav
// tutorial
static u32 HandleMainMenuInputTutorial(struct Pokenav_Menu *menu) {
  ...
  if (JOY_NEW(A_BUTTON)) {
    if (sMenuItems[menu->menuType][menu->cursorPos] ==
        POKENAV_MENUITEM_MATCH_CALL) {
      menu->helpBarIndex = HELPBAR_MC_TRAINER_LIST;
      SetMenuIdAndCB(menu, POKENAV_MATCH_CALL);
      return POKENAV_MENU_FUNC_OPEN_FEATURE;
...
// After calling Mr. Stone during the PokéNav tutorial, force player to exit or
// use Match Call again
static u32 HandleMainMenuInputEndTutorial(struct Pokenav_Menu *menu) {
  ...
  if (JOY_NEW(A_BUTTON)) {
    ...
    if (menuItem != POKENAV_MENUITEM_MATCH_CALL &&
        menuItem != POKENAV_MENUITEM_SWITCH_OFF) {
      PlaySE(SE_FAILURE);
      return POKENAV_MENU_FUNC_NONE;
...
// Selecting it again just reprints the Ribbon description to replace the "No
// Ribbon winners" message
...
static u32 HandleConditionSearchMenuInput(struct Pokenav_Menu *menu) {
  ...
  if (JOY_NEW(A_BUTTON)) {
    ...
    if (menuItem != POKENAV_MENUITEM_CONDITION_SEARCH_CANCEL) {
      SetSelectedConditionSearch(menuItem -
                                 POKENAV_MENUITEM_CONDITION_SEARCH_COOL);
...
static u32 GetMenuId(struct Pokenav_Menu *menu) { return menu->menuId; }
