...
// originally for auto no$gba support, the string "no$gba" should be at this
// address, the user needs to read this string out as the memory viewer won't
// show it.
#define NOCASHGBAIDADDR 0x4FFFA00
#define NOCASHGBAPRINTADDR1                                                    \
  0x4FFFA10 // automatically adds a newline after the string has finished
#define NOCASHGBAPRINTADDR2                                                    \
  0x4FFFA14 // does not automatically add the newline. by default,
            // NOCASHGBAPRINTADDR2 is used. this is used to keep strings
            // consistent between no$gba and VBA-RR, but a user can choose to
            // forgo this.

// hardware extensions for LOG_HANDLER_MGBA_PRINT
#define REG_DEBUG_ENABLE                                                       \
  ((vu16 *)(0x4FFF780)) // handshake: (w)[0xC0DE] -> (r)[0x1DEA]
...
void AGBPrintInit(void) {
  volatile struct AGBPrintStruct *pPrint =
      (struct AGBPrintStruct *)AGB_PRINT_STRUCT_ADDR;
...
static void AGBPutcInternal(const char cChr) {
  ...
  *pProtect = 0x20;
  nData = (pPrint->m_nPut & 1) ? (nData & 0xFF) | (cChr << 8)
                               : (nData & 0xFF00) | cChr;
...
void AGBPrintFlush1Block(void) { AGBPrintTransferDataInternal(FALSE); }

void AGBPrintFlush(void) { AGBPrintTransferDataInternal(TRUE); }

void AGBAssert(const char *pFile, int nLine, const char *pExpression,
               int nStopProgram) {
  if (nStopProgram) {
...
void NoCashGBAAssert(const char *pFile, s32 nLine, const char *pExpression,
                     bool32 nStopProgram) {
  if (nStopProgram) {
    NoCashGBAPrintf("ASSERTION FAILED  FILE=[%s] LINE=[%d]  EXP=[%s]", pFile,
                    nLine, pExpression);
    asm(".hword 0xEFFF");
  } else {
    NoCashGBAPrintf("WARING FILE=[%s] LINE=[%d]  EXP=[%s]", pFile, nLine,
                    pExpression);
  }
}
#endif
...
void MgbaClose(void) { *REG_DEBUG_ENABLE = 0; }
...
void MgbaAssert(const char *pFile, s32 nLine, const char *pExpression,
                bool32 nStopProgram) {
  if (nStopProgram) {
    MgbaPrintf(MGBA_LOG_ERROR,
               "ASSERTION FAILED  FILE=[%s] LINE=[%d]  EXP=[%s]", pFile, nLine,
               pExpression);
    asm(".hword 0xEFFF");
  } else {
    MgbaPrintf(MGBA_LOG_WARN, "WARING FILE=[%s] LINE=[%d]  EXP=[%s]", pFile,
               nLine, pExpression);
  }
}
#endif
