...
#define TILE_HIGHLIGHT_NONE                                                    \
  0x0005 // Tile number for the bg of an unselected menu item
#define TILE_HIGHLIGHT_BLUE                                                    \
  0x1005 // Tile number for the bg of a selected menu item
#define TILE_HIGHLIGHT_RED                                                     \
  0x2005 // Tile number for the bg of a menu item to swap
...
struct PokeblockMenuStruct {
  ...
  u8 menuItemsStrings[POKEBLOCKS_COUNT + 1]
                     [32]; // + 1 because of STOW CASE item
...
static const struct MenuAction sPokeblockMenuActions[] = {
    ...
    [PKBL_USE_ON_FEEDER] = {gMenuText_Use,
                            {PokeblockAction_UseOnPokeblockFeeder}},
    [PKBL_GIVE_TO_LADY] = {gMenuText_Give2,
...
static const struct YesNoFuncTable sTossYesNoFuncTable = {
    TossedPokeblockMessage, CloseTossPokeblockWindow};

static const u8 sContestStatsMonData[] = {MON_DATA_COOL, MON_DATA_BEAUTY,
                                          MON_DATA_CUTE, MON_DATA_SMART,
                                          MON_DATA_TOUGH};
...
static const union AnimCmd sSpriteAnim_PokeblockCase[] = {ANIMCMD_FRAME(0, 0),
                                                          ANIMCMD_END};
...
static const union AffineAnimCmd sAffineAnim_PokeblockCaseShake[] = {
    AFFINEANIMCMD_FRAME(0, 0, -2, 2), AFFINEANIMCMD_FRAME(0, 0, 2, 4),
    AFFINEANIMCMD_FRAME(0, 0, -2, 4), AFFINEANIMCMD_FRAME(0, 0, 2, 2),
    AFFINEANIMCMD_END};
...
static const u8 sTextColor[3] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_DARK_GRAY,
...
static bool8 InitPokeblockMenu(void) {
  ...
  case 8:
    ...
    break;
  case 9:
    sPokeblockMenu->pokeblockCaseSpriteId =
        CreatePokeblockCaseSprite(56, 64, 0);
    gMain.state++;
    break;
  case 10:
    CreateSwapLineSprites(sPokeblockMenu->swapLineSpriteIds,
                          ARRAY_COUNT(sPokeblockMenu->swapLineSpriteIds));
    gMain.state++;
    break;
  case 11:
    DrawPokeblockMenuHighlight(sSavedPokeblockData.selectedRow,
                               TILE_HIGHLIGHT_BLUE);
    gMain.state++;
    break;
  case 12:
  ...
  case 15:
    ...
    gTasks[taskId].tListTaskId = ListMenuInit(&gMultiuseListMenuTemplate,
...
static void HandleInitBackgrounds(void) {
  ...
  InitBgsFromTemplates(0, sBgTemplatesForPokeblockMenu,
                       ARRAY_COUNT(sBgTemplatesForPokeblockMenu));
...
static bool8 LoadPokeblockMenuGfx(void) {
  ...
  case 1:
    ...
    break;
  case 2:
    LoadCompressedPalette(gMenuPokeblock_Pal, BG_PLTT_ID(0),
                          6 * PLTT_SIZE_4BPP);
    sPokeblockMenu->gfxState++;
    break;
  case 3:
...
static void PrintOnPokeblockWindow(u8 windowId, const u8 *string, s32 x) {
  AddTextPrinterParameterized4(windowId, FONT_NORMAL, x, 1, 0, 0, sTextColor, 0,
                               string);
}
...
static void DrawPokeblockMenuTitleText(void) {
  ...
  PrintOnPokeblockWindow(
      WIN_TITLE, itemName,
      GetStringCenterAlignXOffset(FONT_NORMAL, itemName, 0x48));
...
static void PutPokeblockListMenuString(u8 *dst, u16 pkblId) {
  ...
  *(txtPtr++) = CHAR_BLOCK_1;
  ...
  ConvertIntToDecimalStringN(gStringVar1,
                             GetHighestPokeblocksFlavorLevel(pkblock),
                             STR_CONV_MODE_LEFT_ALIGN, 3);
  StringExpandPlaceholders(txtPtr, gText_LvVar1);
}

static void MovePokeblockMenuCursor(s32 pkblId, bool8 onInit,
...
static void DrawPokeblockInfo(s32 pkblId) {
  ...
  if (pkblId != LIST_CANCEL) {
    ...
    for (i = 0; i < FLAVOR_COUNT; i++) {
      ...
      CopyToBgTilemapBufferRect(2, rectTilemapSrc, (i / 3 * 6) + 1,
                                (i % 3 * 2) + 13, 1, 2);
    }

    // Print the Pokéblock's feel
    ConvertIntToDecimalStringN(gStringVar1, GetPokeblocksFeel(pokeblock),
...
static void DrawPokeblockMenuHighlight(u16 cursorPos, u16 tileNum) {
  FillBgTilemapBufferRect_Palette0(2, tileNum, 0xF, (cursorPos * 2) + 1, 0xE,
                                   2);
  ScheduleBgCopyTilemapToVram(2);
}
...
static void LimitMenuScrollAndRow(void) {
  if (sSavedPokeblockData.scrollOffset != 0) {
    if (sSavedPokeblockData.scrollOffset + sPokeblockMenu->maxShowed >
        sPokeblockMenu->itemsNo)
      sSavedPokeblockData.scrollOffset =
          sPokeblockMenu->itemsNo - sPokeblockMenu->maxShowed;
  }

  if (sSavedPokeblockData.scrollOffset + sSavedPokeblockData.selectedRow >=
      sPokeblockMenu->itemsNo) {
...
static void SetInitialScroll(void) {
  if (sSavedPokeblockData.selectedRow > MENU_MIDPOINT) {
    ...
    for (i = 0; (i < sSavedPokeblockData.selectedRow - MENU_MIDPOINT) &&
                (sSavedPokeblockData.scrollOffset + sPokeblockMenu->maxShowed !=
                 sPokeblockMenu->itemsNo);
         sSavedPokeblockData.selectedRow--, sSavedPokeblockData.scrollOffset++,
        i++)
      ;
  }
}
...
static void CreateScrollArrows(void) {
  if (sPokeblockMenu->arrowTaskId == TASK_NONE) {
    sPokeblockMenu->arrowTaskId = AddScrollIndicatorArrowPairParameterized(
        SCROLL_ARROW_UP, 0xB0, 8, 0x98,
        sPokeblockMenu->itemsNo - sPokeblockMenu->maxShowed, TAG_SCROLL_ARROW,
        TAG_SCROLL_ARROW, &sSavedPokeblockData.scrollOffset);
  }
}
...
static void Task_FreeDataAndExitPokeblockCase(u8 taskId) {
  ...
  if (!gPaletteFade.active) {
    ...
    DestroyListMenuTask(tListTaskId, &sSavedPokeblockData.scrollOffset,
                        &sSavedPokeblockData.selectedRow);
    DestroyScrollArrows();
    ResetSpriteData();
    FreeAllSpritePalettes();
...
static void Task_HandlePokeblockMenuInput(u8 taskId) {
  ...
  if (!gPaletteFade.active && MenuHelpers_ShouldWaitForLinkRecv() != TRUE) {
    if (JOY_NEW(SELECT_BUTTON)) {
      ListMenuGetScrollAndRow(tListTaskId, &sSavedPokeblockData.scrollOffset,
                              &sSavedPokeblockData.selectedRow);
      if (sSavedPokeblockData.scrollOffset + sSavedPokeblockData.selectedRow !=
        ...
        DrawPokeblockMenuHighlight(sSavedPokeblockData.selectedRow,
                                   TILE_HIGHLIGHT_RED);
        tToSwapId =
            sSavedPokeblockData.scrollOffset + sSavedPokeblockData.selectedRow;
...
static void Task_HandlePokeblocksSwapInput(u8 taskId) {
  ...
  if (JOY_NEW(SELECT_BUTTON)) {
    ...
    if (i != sSavedPokeblockData.scrollOffset ||
        row != sSavedPokeblockData.selectedRow) {
    ...
    SetSwapLineSpritesInvisibility(
        sPokeblockMenu->swapLineSpriteIds,
        ARRAY_COUNT(sPokeblockMenu->swapLineSpriteIds), FALSE);
    UpdateSwapLineSpritesPos(sPokeblockMenu->swapLineSpriteIds,
                             ARRAY_COUNT(sPokeblockMenu->swapLineSpriteIds),
                             128, (sSavedPokeblockData.selectedRow * 16) + 8);
...
static void UpdatePokeblockSwapMenu(u8 taskId, bool8 noSwap) {
  ...
  tListTaskId =
      ListMenuInit(&gMultiuseListMenuTemplate, sSavedPokeblockData.scrollOffset,
                   sSavedPokeblockData.selectedRow);
  ScheduleBgCopyTilemapToVram(0);
  SetSwapLineSpritesInvisibility(sPokeblockMenu->swapLineSpriteIds,
                                 ARRAY_COUNT(sPokeblockMenu->swapLineSpriteIds),
                                 TRUE);
...
static void ShowPokeblockActionsWindow(u8 taskId) {
  ...
  PrintMenuActionTextsInUpperLeftCorner(tWindowId, sPokeblockMenu->numActions,
...
static void Task_HandlePokeblockActionsInput(u8 taskId) {
  ...
  if (itemId == MENU_NOTHING_CHOSEN) {
    ...
    PlaySE(SE_SELECT);
    sPokeblockMenuActions[sPokeblockMenu->pokeblockActionIds[itemId]]
        .func.void_u8(taskId);
  }
}
...
static void UsePokeblockOnField(void) {
  ChooseMonToGivePokeblock(&gSaveBlock1Ptr->pokeblocks[gSpecialVar_ItemId],
...
static void PokeblockAction_Toss(u8 taskId) {
  ...
  DisplayMessageAndContinueTask(taskId, WIN_TOSS_MSG, 10, 13, FONT_NORMAL,
                                GetPlayerTextSpeedDelay(), gStringVar4,
                                CreateTossPokeblockYesNoMenu);
}

static void CreateTossPokeblockYesNoMenu(u8 taskId) {
  CreateYesNoMenuWithCallbacks(taskId, &sTossPkblockWindowTemplate, 1, 0, 2, 1,
                               0xE, &sTossYesNoFuncTable);
}
...
static void PokeblockAction_UseInBattle(u8 taskId) {
  ...
  s16 gain =
      PokeblockGetGain(nature, &gSaveBlock1Ptr->pokeblocks[gSpecialVar_ItemId]);
  StringCopy(
      gBattleTextBuff1,
      gPokeblockNames[gSaveBlock1Ptr->pokeblocks[gSpecialVar_ItemId].color]);
  TryClearPokeblock(gSpecialVar_ItemId);

  gSpecialVar_ItemId = gSaveBlock1Ptr->pokeblocks[gSpecialVar_ItemId].color
                       << 8;
  if (gain == 0)
...
static void PokeblockAction_GiveToContestLady(u8 taskId) {
  gSpecialVar_0x8004 = GivePokeblockToContestLady(
      &gSaveBlock1Ptr->pokeblocks[gSpecialVar_ItemId]);
  gSpecialVar_Result = gSpecialVar_ItemId;
  TryClearPokeblock(gSpecialVar_ItemId);
  gSpecialVar_ItemId = 0;
  FadePaletteAndSetTaskToClosePokeblockCase(taskId);
...
s16 PokeblockGetGain(u8 nature, const struct Pokeblock *pokeblock) {
  ...
  for (flavor = 0; flavor < FLAVOR_COUNT; flavor++) {
    ...
    if (curGain > 0)
      totalGain +=
          curGain *
          gPokeblockFlavorCompatibilityTable[FLAVOR_COUNT * nature + flavor];
    if (MON_DATA_SHEEN > 100) {
      reduction_steps =
          (MON_DATA_SHEEN - 100) / 50; // Each step reduces gain by 10%
...
u8 GetPokeblocksFlavor(const struct Pokeblock *pokeblock) {
  ...
  for (i = 0; i < FLAVOR_COUNT; i++) {
    if (GetPokeblockData(pokeblock, bestFlavor + 1) <
        GetPokeblockData(pokeblock, i + 1))
      bestFlavor = i;
  }
