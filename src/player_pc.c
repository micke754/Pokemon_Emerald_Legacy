...
enum { MENU_ITEMSTORAGE, MENU_MAILBOX, MENU_DECORATION, MENU_TURNOFF };

// Item storage menu options
enum { MENU_WITHDRAW, MENU_DEPOSIT, MENU_TOSS, MENU_EXIT };
...
static const u8 sBedroomPC_OptionOrder[] = {MENU_ITEMSTORAGE, MENU_MAILBOX,
                                            MENU_DECORATION, MENU_TURNOFF};
#define NUM_BEDROOM_PC_OPTIONS ARRAY_COUNT(sBedroomPC_OptionOrder)

static const u8 sPlayerPC_OptionOrder[] = {MENU_ITEMSTORAGE, MENU_MAILBOX,
                                           MENU_TURNOFF};
...
static const u16 sNewGamePCItems[][2] = {{ITEM_POTION, 1}, {ITEM_NONE, 0}};
...
static const struct YesNoFuncTable ItemTossYesNoFuncs = {
    ItemStorage_TossItemYes, ItemStorage_TossItemNo};
...
static const struct WindowTemplate
    sWindowTemplates_ItemStorage[ITEMPC_WIN_COUNT] = {
        [ITEMPC_WIN_LIST] = {.bg = 0,
                             .tilemapLeft = 16,
                             .tilemapTop = 1,
                             .width = 13,
                             .height = 18,
...
static const u8 sSwapArrowTextColors[] = {
...
void BedroomPC(void) {
  ...
  DisplayItemMessageOnField(CreateTask(TaskDummy, 0), gText_WhatWouldYouLike,
                            InitPlayerPCMenu);
}
...
static void InitPlayerPCMenu(u8 taskId) {
  ...
  windowTemplate.width = GetMaxWidthInSubsetOfMenuTable(
      sPlayerPCMenuActions, sTopMenuOptionOrder, sTopMenuNumOptions);
  tWindowId = AddWindow(&windowTemplate);
  SetStandardWindowBorderStyle(tWindowId, FALSE);
  PrintMenuActionTextsInUpperLeftCorner(
      tWindowId, sTopMenuNumOptions, sPlayerPCMenuActions, sTopMenuOptionOrder);
...
static void PlayerPCProcessMenuInput(u8 taskId) {
  ...
  default:
    ...
    ScheduleBgCopyTilemapToVram(0);
    gTasks[taskId].func =
        sPlayerPCMenuActions[sTopMenuOptionOrder[inputOptionId]].func.void_u8;
    break;
  }
}
...
static void PlayerPC_TurnOff(u8 taskId) {
  if (sTopMenuNumOptions ==
      NUM_BEDROOM_PC_OPTIONS) // Flimsy way to determine if Bedroom PC is in use
  {
...
static void InitItemStorageMenu(u8 taskId, u8 var) {
  ...
  windowTemplate.width = GetMaxWidthInMenuTable(
      sItemStorage_MenuActions, ARRAY_COUNT(sItemStorage_MenuActions));
  tWindowId = AddWindow(&windowTemplate);
  SetStandardWindowBorderStyle(tWindowId, FALSE);
  PrintMenuTable(tWindowId, ARRAY_COUNT(sItemStorage_MenuActions),
                 sItemStorage_MenuActions);
  InitMenuInUpperLeftCornerNormal(tWindowId,
                                  ARRAY_COUNT(sItemStorage_MenuActions), var);
  ScheduleBgCopyTilemapToVram(0);
...
static void Mailbox_DrawMailboxMenu(u8 taskId) {
  ...
  AddTextPrinterParameterized(
      windowId, FONT_NORMAL, gText_Mailbox,
      GetStringCenterAlignXOffset(FONT_NORMAL, gText_Mailbox, 0x40), 1, 0,
      NULL);
  ScheduleBgCopyTilemapToVram(0);
...
static void Mailbox_ProcessInput(u8 taskId) {
  ...
  if (!gPaletteFade.active) {
    ...
    ListMenuGetScrollAndRow(tListTaskId, &gPlayerPCItemPageInfo.itemsAbove,
    ...
    case LIST_CANCEL:
      ...
      RemoveScrollIndicatorArrowPair(
          gPlayerPCItemPageInfo.scrollIndicatorTaskId);
    ...
    default:
      ...
      DestroyListMenuTask(tListTaskId, &gPlayerPCItemPageInfo.itemsAbove,
                          &gPlayerPCItemPageInfo.cursorPos);
      ScheduleBgCopyTilemapToVram(0);
      RemoveScrollIndicatorArrowPair(
          gPlayerPCItemPageInfo.scrollIndicatorTaskId);
...
static void Mailbox_PrintWhatToDoWithPlayerMailText(u8 taskId) {
  StringCopy(gStringVar1,
             gSaveBlock1Ptr
                 ->mail[gPlayerPCItemPageInfo.itemsAbove + PARTY_SIZE +
                        gPlayerPCItemPageInfo.cursorPos]
                 .playerName);
...
static void Mailbox_PrintMailOptions(u8 taskId) {
  ...
  PrintMenuTable(windowId, ARRAY_COUNT(gMailboxMailOptions),
                 gMailboxMailOptions);
  InitMenuInUpperLeftCornerNormal(windowId, ARRAY_COUNT(gMailboxMailOptions),
                                  0);
  ScheduleBgCopyTilemapToVram(0);
...
static void Mailbox_FadeAndReadMail(u8 taskId) {
  if (!gPaletteFade.active) {
    ...
    CleanupOverworldWindowsAndTilemaps();
    ReadMail(
        &gSaveBlock1Ptr->mail[gPlayerPCItemPageInfo.itemsAbove + PARTY_SIZE +
                              gPlayerPCItemPageInfo.cursorPos],
...
static void Mailbox_DoMailMoveToBag(u8 taskId) {
  ...
  if (!AddBagItem(mail->itemId, 1)) {
    ...
    DisplayItemMessageOnField(taskId, gText_MailToBagMessageErased,
                              Mailbox_Cancel);
    ClearMail(mail);
    Mailbox_CompactMailList();
    gPlayerPCItemPageInfo.count--;
    if (gPlayerPCItemPageInfo.count < (gPlayerPCItemPageInfo.pageItems +
...
static void Mailbox_CancelMoveToBag(u8 taskId) { Mailbox_Cancel(taskId); }
...
void ItemStorage_RefreshListMenu(void) {
  ...
  for (i = 0; i < gPlayerPCItemPageInfo.count - 1; i++) {
    CopyItemName_PlayerPC(&sItemStorageMenu->itemNames[i][0],
...
static void ItemStorage_MoveCursor(s32 id, bool8 onInit,
...
static void ItemStorage_PrintMenuItem(u8 windowId, u32 id, u8 yOffset) {
  if (id != LIST_CANCEL) {
    ...
    ConvertIntToDecimalStringN(gStringVar1,
                               gSaveBlock1Ptr->pcItems[id].quantity,
                               STR_CONV_MODE_RIGHT_ALIGN, 3);
    StringExpandPlaceholders(gStringVar4, gText_xVar1);
    AddTextPrinterParameterized(
        windowId, FONT_NARROW, gStringVar4,
        GetStringRightAlignXOffset(FONT_NARROW, gStringVar4, 104), yOffset,
        TEXT_SKIP_DRAW, NULL);
  }
}
...
static void ItemStorage_PrintDescription(s32 id) {
  ...
  if (id != LIST_CANCEL)
    description =
        (u8 *)ItemId_GetDescription(gSaveBlock1Ptr->pcItems[id].itemId);
  else
    description = ItemStorage_GetMessage(MSG_GO_BACK_TO_PREV);

  FillWindowPixelBuffer(windowId, PIXEL_FILL(1));
  AddTextPrinterParameterized(windowId, FONT_NORMAL, description, 0, 1, 0,
                              NULL);
}
...
static void ItemStorage_SetSwapArrow(u8 listTaskId, u8 b, u8 speed) {
  ItemStorage_DrawSwapArrow(ListMenuGetYCoordForPrintingArrowCursor(listTaskId),
                            b, speed);
}
...
static void ItemStorage_CompactList(void) {
  CompactPCItems();
  SetItemListPerPageCount(gSaveBlock1Ptr->pcItems, PC_ITEMS_COUNT,
                          &gPlayerPCItemPageInfo.pageItems,
                          &gPlayerPCItemPageInfo.count, 8);
}

static void ItemStorage_CompactCursor(void) {
  SetCursorWithinListBounds(
      &gPlayerPCItemPageInfo.itemsAbove, &gPlayerPCItemPageInfo.cursorPos,
...
static void ItemStorage_CreateListMenu(u8 taskId) {
  ...
  AddTextPrinterParameterized(sItemStorageMenu->windowIds[ITEMPC_WIN_TITLE],
                              FONT_NORMAL, text, x, 1, 0, NULL);
  ...
  tListTaskId =
      ListMenuInit(&gMultiuseListMenuTemplate, gPlayerPCItemPageInfo.itemsAbove,
                   gPlayerPCItemPageInfo.cursorPos);
...
static void ItemStorage_PrintMessage(const u8 *string) {
  ...
  AddTextPrinterParameterized(windowId, FONT_NORMAL, gStringVar4, 0, 1, 0,
                              NULL);
}
...
static void ItemStorage_ProcessInput(u8 taskId) {
  ...
  if (JOY_NEW(SELECT_BUTTON)) {
    ...
    ListMenuGetScrollAndRow(tListTaskId, &gPlayerPCItemPageInfo.itemsAbove,
                            &gPlayerPCItemPageInfo.cursorPos);
    if (gPlayerPCItemPageInfo.itemsAbove + gPlayerPCItemPageInfo.cursorPos !=
...
static void ItemStorage_StartItemSwap(u8 taskId) {
  ...
  CopyItemName(gSaveBlock1Ptr->pcItems[sItemStorageMenu->toSwapPos].itemId,
               gStringVar1);
...
static void ItemStorage_ProcessItemSwapInput(u8 taskId) {
  ...
  ListMenuGetScrollAndRow(tListTaskId, &gPlayerPCItemPageInfo.itemsAbove,
                          &gPlayerPCItemPageInfo.cursorPos);
  SetSwapLineSpritesInvisibility(sItemStorageMenu->swapLineSpriteIds,
                                 SWAP_LINE_LENGTH, FALSE);
...
static void ItemStorage_FinishItemSwap(u8 taskId, bool8 canceled) {
  ...
  if (!canceled && sItemStorageMenu->toSwapPos != newPos &&
      sItemStorageMenu->toSwapPos != newPos - 1) {
    MoveItemSlotInList(gSaveBlock1Ptr->pcItems, sItemStorageMenu->toSwapPos,
                       newPos);
    ItemStorage_RefreshListMenu();
  }
...
static void ItemStorage_PrintItemQuantity(u8 windowId, u16 value, u32 mode,
                                          u8 x, u8 y, u8 n) {
...
static void ItemStorage_DoItemAction(u8 taskId) {
  ...
  ItemStorage_PrintItemQuantity(ItemStorage_AddWindow(ITEMPC_WIN_QUANTITY),
                                tQuantity, STR_CONV_MODE_LEADING_ZEROS, 8, 1,
                                3);
...
static void ItemStorage_HandleQuantityRolling(u8 taskId) {
  ...
  if (AdjustQuantityAccordingToDPadInput(
          &tQuantity, gSaveBlock1Ptr->pcItems[pos].quantity) == TRUE) {
    ItemStorage_PrintItemQuantity(ItemStorage_AddWindow(ITEMPC_WIN_QUANTITY),
                                  tQuantity, STR_CONV_MODE_LEADING_ZEROS, 8, 1,
                                  3);
  } else {
    if (JOY_NEW(A_BUTTON)) {
      ...
      ItemStorage_PrintMessage(
          ItemStorage_GetMessage(gSaveBlock1Ptr->pcItems[pos].itemId));
...
static void ItemStorage_DoItemWithdraw(u8 taskId) {
  ...
  if (AddBagItem(gSaveBlock1Ptr->pcItems[pos].itemId, tQuantity) == TRUE) {
    // Item withdrawn
    CopyItemName(gSaveBlock1Ptr->pcItems[pos].itemId, gStringVar1);
    ConvertIntToDecimalStringN(gStringVar2, tQuantity, STR_CONV_MODE_LEFT_ALIGN,
                               3);
...
static void ItemStorage_DoItemToss(u8 taskId) {
  ...
  if (!ItemId_GetImportance(gSaveBlock1Ptr->pcItems[pos].itemId)) {
    ...
    CreateYesNoMenuWithCallbacks(
        taskId, &sWindowTemplates_ItemStorage[ITEMPC_WIN_YESNO], 1, 0, 1, 0x214,
        0xE, &ItemTossYesNoFuncs);
  } else {
...
static void ItemStorage_TossItemNo(u8 taskId) {
  ItemStorage_PrintMessage(
      ItemStorage_GetMessage(gSaveBlock1Ptr
...
static void ItemStorage_HandleRemoveItem(u8 taskId) {
  ...
  if (JOY_NEW(A_BUTTON | B_BUTTON)) {
    RemovePCItem(gPlayerPCItemPageInfo.cursorPos +
                     gPlayerPCItemPageInfo.itemsAbove,
                 tQuantity);
    DestroyListMenuTask(tListTaskId, &gPlayerPCItemPageInfo.itemsAbove,
                        &gPlayerPCItemPageInfo.cursorPos);
