...
static const u16 sDefaultBardSongLyrics[BARD_SONG_LENGTH] = {
    EC_WORD_SHAKE, EC_WORD_IT,   EC_WORD_DO,
    EC_WORD_THE,   EC_WORD_DIET, EC_WORD_DANCE};

static const u8 *const sGiddyAdjectives[] = {
    GiddyText_SoPretty,  GiddyText_SoDarling,   GiddyText_SoRelaxed,
    GiddyText_SoSunny,   GiddyText_SoDesirable, GiddyText_SoExciting,
    GiddyText_SoAmusing, GiddyText_SoMagical};
...
static void SetupStoryteller(void) { StorytellerSetup(); }
...
static void SetupTrader(void) { TraderSetup(); }

void SetMauvilleOldMan(void) {
  u16 trainerId = (gSaveBlock2Ptr->playerTrainerId[1] << 8) |
...
u8 GetCurrentMauvilleOldMan(void) { return gSaveBlock1Ptr->oldMan.common.id; }
...
static void PrepareSongText(void) {
  struct MauvilleManBard *bard = &gSaveBlock1Ptr->oldMan.bard;
  u16 *lyrics =
      gSpecialVar_0x8004 == 0 ? bard->songLyrics : bard->temporaryLyrics;
...
void GenerateGiddyLine(void) {
  ...
  if (giddy->randomWords[giddy->taleCounter] != EC_EMPTY_WORD) {
    ...
    stringPtr =
        CopyEasyChatWord(gStringVar4, giddy->randomWords[giddy->taleCounter]);
    ...
    StringCopy(gStringVar4,
               sGiddyQuestions[giddy->questionList[giddy->questionNum++]]);
  }
...
static void InitGiddyTaleList(void) {
  ...
  u16 wordGroupsAndCount[][2] = {
      {EC_GROUP_POKEMON, 0}, {EC_GROUP_LIFESTYLE, 0},
      {EC_GROUP_HOBBIES, 0}, {EC_GROUP_MOVE_1, 0},
      {EC_GROUP_MOVE_2, 0},  {EC_GROUP_POKEMON_NATIONAL, 0}};
  u16 i;
  u16 totalWords;
  u16 temp;
  u16 var; // re-used
  ...
  for (i = 0; i < ARRAY_COUNT(wordGroupsAndCount); i++) {
    wordGroupsAndCount[i][1] =
        EasyChat_GetNumWordsInGroup(wordGroupsAndCount[i][0]);
...
static void ResetTraderFlag(void) { Trader_ResetFlag(); }

static void ResetStorytellerFlag(void) { Storyteller_ResetFlag(); }
...
static void EnableTextPrinters(void) { gDisableTextPrinters = FALSE; }

static void DisableTextPrinters(struct TextPrinterTemplate *printer,
                                u16 renderCmd) {
...
static void DrawSongTextWindow(const u8 *str) {
  ...
  AddTextPrinterParameterized(0, FONT_NORMAL, str, 0, 1, 1,
                              DisableTextPrinters);
  gDisableTextPrinters = TRUE;
  ...
  case BARD_STATE_WAIT_WORD: {
    ...
    case 1:
      ...
      if (song->phonemeTimer == 0) {
        song->currPhoneme++;
        if (song->currPhoneme != 6 &&
            song->sound[song->currPhoneme].songLengthId != 0xFF)
          song->state = 0;
        else {
          song->state = 3;
          song->phonemeTimer = 2;
...
static void Task_BardSong(u8 taskId) {
  ...
  case BARD_STATE_GET_WORD: {
    ...
    while (*str != CHAR_SPACE && *str != CHAR_NEWLINE &&
           *str != EXT_CTRL_CODE_BEGIN && *str != EOS) {
      str++;
      wordLen++;
    }
...
static void UNUSED SetMauvilleOldManLanguage(union OldMan *oldMan,
                                             u32 language1, u32 language2,
                                             u32 language3) {
  s32 i;
...
void SanitizeReceivedEmeraldOldMan(union OldMan *oldMan, u32 version,
                                   u32 language) {
  u8 playerName[PLAYER_NAME_LENGTH + 1];
  s32 i;
  if (oldMan->common.id == MAUVILLE_MAN_STORYTELLER &&
...
void SanitizeReceivedRubyOldMan(union OldMan *oldMan, u32 version,
                                u32 language) {
...
static const struct Story sStorytellerStories[] = {
    ...
    {50, 1, MauvilleCity_PokemonCenter_1F_Text_SavedGameTitle,
...
static void StorytellerRecordNewStat(u32 player, u32 stat) {
  ...
  ConvertIntToDecimalStringN(gStringVar1, StorytellerGetGameStat(stat),
                             STR_CONV_MODE_LEFT_ALIGN, 10);
...
static void StorytellerDisplayStory(u32 player) {
  ...
  ConvertIntToDecimalStringN(gStringVar1,
                             StorytellerGetRecordedTrainerStat(player),
                             STR_CONV_MODE_LEFT_ALIGN, 10);
  StringCopy(gStringVar2, GetStoryActionByStat(stat));
...
static void PrintStoryList(void) {
  ...
  sStorytellerWindowId = CreateWindowFromRect(
      0, 0, ConvertPixelWidthToTileWidth(width), GetFreeStorySlot() * 2 + 2);
  SetStandardWindowBorderStyle(sStorytellerWindowId, FALSE);
  for (i = 0; i < NUM_STORYTELLER_TALES; i++) {
    u16 gameStatID = sStorytellerPtr->gameStatIDs[i];
    if (gameStatID == 0)
      break;
    AddTextPrinterParameterized(sStorytellerWindowId, FONT_NORMAL,
                                GetStoryTitleByStat(gameStatID), 8, 16 * i + 1,
                                TEXT_SKIP_DRAW, NULL);
  }
  AddTextPrinterParameterized(sStorytellerWindowId, FONT_NORMAL, gText_Exit, 8,
                              16 * i + 1, TEXT_SKIP_DRAW, NULL);
  InitMenuInUpperLeftCornerNormal(sStorytellerWindowId, GetFreeStorySlot() + 1,
                                  0);
...
void StorytellerStoryListMenu(void) { CreateTask(Task_StoryListMenu, 80); }
