...
static void sio32intr_clock_master(void) {
  ...
  if (gSTWIStatus->state == 0) // master send req
  {
    if (regSIODATA32 == 0x80000000) {
      if (gSTWIStatus->reqNext <= gSTWIStatus->reqLength) {
        REG_SIODATA32 =
            ((u32 *)
                 gSTWIStatus->txPacket->rfuPacket8.data)[gSTWIStatus->reqNext];
        gSTWIStatus->reqNext++;
      } else {
  ...
  if (gSTWIStatus->state == 3) // master done ack
  {
    if (gSTWIStatus->ackActiveCommand == (0x80 | ID_MS_CHANGE_REQ) ||
        ...
        gSTWIStatus->ackActiveCommand ==
            (0x80 | ID_RESUME_RETRANSMIT_AND_CHANGE_REQ)) {

      gSTWIStatus->msMode = AGB_CLK_SLAVE;
      REG_SIODATA32 = 0x80000000;
      REG_SIOCNT = SIO_INTR_ENABLE | SIO_32BIT_MODE | SIO_57600_BPS;
      REG_SIOCNT =
          SIO_INTR_ENABLE | SIO_32BIT_MODE | SIO_57600_BPS | SIO_ENABLE;
    ...
    if (gSTWIStatus->callbackM != NULL)
      Callback_Dummy_M(gSTWIStatus->reqActiveCommand, gSTWIStatus->error,
...
static void sio32intr_clock_slave(void) {
  ...
  if (gSTWIStatus->state == 5) // slave receive req init
  {
    ...
    if (reqLen == (r0 >> 16)) {
      ...
      // more consistent when both lines update the variables. Might have been a
      // macro?
      ...
      if (gSTWIStatus->reqLength == 0) {
        if (gSTWIStatus->reqActiveCommand == ID_MS_CHANGE_REQ ||
          ...
          ((u32 *)gSTWIStatus->txPacket)[0] =
              0x99660000 + gSTWIStatus->ackActiveCommand;
  ...
  if (gSTWIStatus->state == 8) // slave done ack
  {
    ...
    if (gSTWIStatus->error == ERR_REQ_CMD_ACK_REJECTION) {
      ...
      if (gSTWIStatus->callbackS != NULL) {
        Callback_Dummy_S((gSTWIStatus->reqLength << 8) |
...
NAKED
static void Callback_Dummy_ID(void (*callbackId)(void)) { asm("bx r0"); }
