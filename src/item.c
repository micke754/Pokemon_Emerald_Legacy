...
static u16 GetPCItemQuantity(u16 *quantity) { return *quantity; }
  ...
  for (pocket = 0; pocket < POCKETS_COUNT; pocket++) {
    for (item = 0; item < gBagPockets[pocket].capacity; item++)
      ApplyNewEncryptionKeyToHword(
          &(gBagPockets[pocket].itemSlots[item].quantity), newKey);
  }
}
...
void CopyItemNameHandlePlural(u16 itemId, u8 *dst, u32 quantity) {
  if (itemId == ITEM_POKE_BALL) {
    ...
    if (itemId >= FIRST_BERRY_INDEX && itemId <= LAST_BERRY_INDEX)
      GetBerryCountString(dst, gBerries[itemId - FIRST_BERRY_INDEX].name,
                          quantity);
    else
      StringCopy(dst, ItemId_GetName(itemId));
  }
...
bool8 CheckBagHasSpace(u16 itemId, u16 count) {
  ...
  for (i = 0; i < gBagPockets[pocket].capacity; i++) {
    if (gBagPockets[pocket].itemSlots[i].itemId == itemId) {
      ownedCount =
          GetBagItemQuantity(&gBagPockets[pocket].itemSlots[i].quantity);
...
bool8 AddBagItem(u16 itemId, u16 count) {
  ...
  if (InBattlePyramid() || FlagGet(FLAG_STORING_ITEMS_IN_PYRAMID_BAG) == TRUE) {
    ...
    memcpy(newItems, itemPocket->itemSlots,
    ...
    for (i = 0; i < itemPocket->capacity; i++) {
      if (newItems[i].itemId == itemId) {
        ...
        if (ownedCount + count <= slotCapacity) {
          ...
          memcpy(itemPocket->itemSlots, newItems,
                 itemPocket->capacity * sizeof(struct ItemSlot));
          Free(newItems);
          return TRUE;
        } else {
          ...
          if (pocket == TMHM_POCKET || pocket == BERRIES_POCKET) {
            ...
            // don't create another instance of the item if it's at max slot
            // capacity and count is equal to 0
            if (count == 0) {
              break;
            }
          }
    ...
    if (count > 0) {
      // either no existing item was found or we have to create another
      // instance, because the capacity was exceeded
      for (i = 0; i < itemPocket->capacity; i++) {
        if (newItems[i].itemId == ITEM_NONE) {
          newItems[i].itemId = itemId;
          if (count > slotCapacity) {
            // try creating a new slot with max capacity if duplicates are
            // possible
            if (pocket == TMHM_POCKET || pocket == BERRIES_POCKET) {
              Free(newItems);
              return FALSE;
...
bool8 RemoveBagItem(u16 itemId, u16 count) {
  ...
  if (InBattlePyramid() || FlagGet(FLAG_STORING_ITEMS_IN_PYRAMID_BAG) == TRUE) {
    ...
    if (itemPocket->capacity > var &&
      ...
      if (ownedCount >= count) {
        SetBagItemQuantity(&itemPocket->itemSlots[var].quantity,
                           ownedCount - count);
        count = 0;
      } else {
        count -= ownedCount;
    ...
    for (i = 0; i < itemPocket->capacity; i++) {
      if (itemPocket->itemSlots[i].itemId == itemId) {
        ...
        if (ownedCount >= count) {
          SetBagItemQuantity(&itemPocket->itemSlots[i].quantity,
                             ownedCount - count);
          count = 0;
        } else {
          count -= ownedCount;
...
bool8 CheckPCHasSpace(u16 itemId, u16 count) {
  ...
  if (ownedCount + count <=
      MAX_PC_ITEM_CAPACITY) // If there's room in already existing slot
  {
    return TRUE;
  }
  ...
  for (i = 0; i < PC_ITEMS_COUNT; i++) {
    if (newItems[i].itemId == itemId) {
      ...
      if (ownedCount + count <= MAX_PC_ITEM_CAPACITY) {
        ...
        memcpy(gSaveBlock1Ptr->pcItems, newItems,
               sizeof(gSaveBlock1Ptr->pcItems));
        Free(newItems);
        return TRUE;
      }
...
u16 BagGetQuantityByPocketPosition(u8 pocketId, u16 pocketPos) {
  return GetBagItemQuantity(
      &gBagPockets[pocketId - 1].itemSlots[pocketPos].quantity);
}
...
static bool8 CheckPyramidBagHasItem(u16 itemId, u16 count) {
  u8 i;
  u16 *items = gSaveBlock2Ptr->frontier.pyramidBag
...
bool8 AddPyramidBagItem(u16 itemId, u16 count) {
  ...
  memcpy(newQuantities, quantities,
  ...
  if (count == 0) {
    ...
    memcpy(quantities, newQuantities,
           PYRAMID_BAG_ITEMS_COUNT * sizeof(*quantities));
    Free(newItems);
    Free(newQuantities);
    return TRUE;
  } else {
