...
void static (*const sVerdanturfTentFuncs[])(void) = {
    ...
    [VERDANTURF_TENT_FUNC_GET_OPPONENT_INTRO] =
...
static void GenerateInitialRentalMons(void) {
  ...
  while (i != PARTY_SIZE) {
    ...
    for (j = firstMonId; j < i + firstMonId; j++) {
      if (heldItems[j] != 0 &&
          heldItems[j] ==
              gBattleFrontierHeldItems[gFacilityTrainerMons[monSetId]
                                           .itemTableId]) {
...
static void GenerateOpponentMons(void) {
  ...
  while (i != FRONTIER_PARTY_SIZE) {
    ...
    for (j = 0; j < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.rentalMons);
         j++) {
      if (gFacilityTrainerMons[sRandMonId].species ==
    ...
    for (k = 0; k < i; k++) {
      if (heldItems[k] != ITEM_NONE &&
          heldItems[k] ==
              gBattleFrontierHeldItems[gFacilityTrainerMons[sRandMonId]
                                           .itemTableId])
        break;
    }
    if (k != i)
    ...
    heldItems[i] =
        gBattleFrontierHeldItems[gFacilityTrainerMons[sRandMonId].itemTableId];
