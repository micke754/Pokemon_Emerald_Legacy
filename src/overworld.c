...
static u8 GetAdjustedInitialTransitionFlags(struct InitialPlayerAvatarState *,
                                            u16, u8);
static u8 GetAdjustedInitialDirection(struct InitialPlayerAvatarState *, u8,
                                      u16, u8);
...
EWRAM_DATA static struct InitialPlayerAvatarState sInitialPlayerAvatarState = {
    0};
...
static u8 MovementEventModeCB_Normal(struct LinkPlayerObjectEvent *,
                                     struct ObjectEvent *, u8);
static u8 MovementEventModeCB_Ignored(struct LinkPlayerObjectEvent *,
                                      struct ObjectEvent *, u8);
static u8 MovementEventModeCB_Scripted(struct LinkPlayerObjectEvent *,
                                       struct ObjectEvent *, u8);

static u8 (*const sLinkPlayerMovementModes[])(struct LinkPlayerObjectEvent *,
                                              struct ObjectEvent *, u8) = {
...
static u8 FacingHandler_DoNothing(struct LinkPlayerObjectEvent *,
                                  struct ObjectEvent *, u8);
static u8 FacingHandler_DpadMovement(struct LinkPlayerObjectEvent *,
                                     struct ObjectEvent *, u8);
static u8 FacingHandler_ForcedFacingChange(struct LinkPlayerObjectEvent *,
                                           struct ObjectEvent *, u8);

// These handlers return TRUE if the movement was scripted and successful, and
// FALSE otherwise.
static bool8 (*const sLinkPlayerFacingHandlers[])(
    struct LinkPlayerObjectEvent *, struct ObjectEvent *, u8) = {
    FacingHandler_DoNothing,          FacingHandler_DpadMovement,
    FacingHandler_DpadMovement,       FacingHandler_DpadMovement,
    FacingHandler_DpadMovement,       FacingHandler_DoNothing,
    FacingHandler_DoNothing,          FacingHandler_ForcedFacingChange,
    FacingHandler_ForcedFacingChange, FacingHandler_ForcedFacingChange,
    FacingHandler_ForcedFacingChange,
};

static void MovementStatusHandler_EnterFreeMode(struct LinkPlayerObjectEvent *,
                                                struct ObjectEvent *);
static void
MovementStatusHandler_TryAdvanceScript(struct LinkPlayerObjectEvent *,
                                       struct ObjectEvent *);

// These handlers are run after an attempted movement.
static void (*const sMovementStatusHandler[])(struct LinkPlayerObjectEvent *,
                                              struct ObjectEvent *) = {
    // FALSE:
...
void LoadObjEventTemplatesFromHeader(void) {
  ...
  CpuCopy32(
      gMapHeader.events->objectEvents, gSaveBlock1Ptr->objectEventTemplates,
...
static void SetWarpData(struct WarpData *warp, s8 mapGroup, s8 mapNum,
                        s8 warpId, s8 x, s8 y) {
...
struct MapHeader const *const GetDestinationWarpMapHeader(void) {
  return Overworld_GetMapHeaderByGroupAndId(sWarpDestination.mapGroup,
                                            sWarpDestination.mapNum);
}
...
static void SetPlayerCoordsFromWarp(void) {
  if (gSaveBlock1Ptr->location.warpId >= 0 &&
    ...
    gSaveBlock1Ptr->pos.x =
        gMapHeader.events->warps[gSaveBlock1Ptr->location.warpId].x;
    gSaveBlock1Ptr->pos.y =
        gMapHeader.events->warps[gSaveBlock1Ptr->location.warpId].y;
  } else if (gSaveBlock1Ptr->location.x >= 0 &&
             gSaveBlock1Ptr->location.y >= 0) {
...
void SetDynamicWarp(s32 unused, s8 mapGroup, s8 mapNum, s8 warpId) {
  SetWarpData(&gSaveBlock1Ptr->dynamicWarp, mapGroup, mapNum, warpId,
...
void SetWarpDestinationToHealLocation(u8 healLocationId) {
  ...
  if (healLocation)
    SetWarpDestination(healLocation->group, healLocation->map, WARP_ID_NONE,
                       healLocation->x, healLocation->y);
}
...
void SetLastHealLocationWarp(u8 healLocationId) {
  const struct HealLocation *healLocation = GetHealLocation(healLocationId);
  if (healLocation)
    SetWarpData(&gSaveBlock1Ptr->lastHealLocation, healLocation->group,
                healLocation->map, WARP_ID_NONE, healLocation->x,
                healLocation->y);
}

void UpdateEscapeWarp(s16 x, s16 y) {
  u8 currMapType = GetCurrentMapType();
  u8 destMapType = GetMapTypeByGroupAndId(sWarpDestination.mapGroup,
                                          sWarpDestination.mapNum);
  if (IsMapTypeOutdoors(currMapType) && IsMapTypeOutdoors(destMapType) != TRUE)
    SetEscapeWarp(gSaveBlock1Ptr->location.mapGroup,
                  gSaveBlock1Ptr->location.mapNum, WARP_ID_NONE, x - MAP_OFFSET,
                  y - MAP_OFFSET + 1);
}
...
void SetWarpDestinationToFixedHoleWarp(s16 x, s16 y) {
  if (IsDummyWarp(&sFixedHoleWarp) == TRUE)
    ...
    SetWarpDestination(sFixedHoleWarp.mapGroup, sFixedHoleWarp.mapNum,
                       WARP_ID_NONE, x, y);
}
...
void SetContinueGameWarp(s8 mapGroup, s8 mapNum, s8 warpId, s8 x, s8 y) {
  SetWarpData(&gSaveBlock1Ptr->continueGameWarp, mapGroup, mapNum, warpId, x,
              y);
}
...
void SetContinueGameWarpToHealLocation(u8 healLocationId) {
  const struct HealLocation *healLocation = GetHealLocation(healLocationId);
  if (healLocation)
    SetWarpData(&gSaveBlock1Ptr->continueGameWarp, healLocation->group,
                healLocation->map, WARP_ID_NONE, healLocation->x,
                healLocation->y);
}
...
static bool8 SetDiveWarp(u8 dir, u16 x, u16 y) {
  ...
  if (connection != NULL) {
    SetWarpDestination(connection->mapGroup, connection->mapNum, WARP_ID_NONE,
                       x, y);
  } else {
...
void LoadMapFromCameraTransition(u8 mapGroup, u8 mapNum) {
  ...
  for (paletteIndex = NUM_PALS_IN_PRIMARY; paletteIndex < NUM_PALS_TOTAL;
       paletteIndex++)
...
static void LoadMapFromWarp(bool32 a1) {
  ...
  ResetCyclingRoadChallengeData();
  RestartWildEncounterImmunitySteps();
  TryUpdateRandomTrainerRematches(gSaveBlock1Ptr->location.mapGroup,
...
static struct InitialPlayerAvatarState *GetInitialPlayerAvatarState(void) {
  ...
  u8 transitionFlags = GetAdjustedInitialTransitionFlags(
      &sInitialPlayerAvatarState, metatileBehavior, mapType);
...
static u8
GetAdjustedInitialTransitionFlags(struct InitialPlayerAvatarState *playerStruct,
...
static u8
GetAdjustedInitialDirection(struct InitialPlayerAvatarState *playerStruct,
                            u8 transitionFlags, u16 metatileBehavior,
                            u8 mapType) {
...
static u16 GetCenterScreenMetatileBehavior(void) {
  return MapGridGetMetatileBehaviorAt(gSaveBlock1Ptr->pos.x + MAP_OFFSET,
...
u8 GetFlashLevel(void) { return gSaveBlock1Ptr->flashLevel; }
...
void SetObjectEventLoadFlag(u8 flag) { sObjectEventLoadFlag = flag; }

// sObjectEventLoadFlag is read directly
static u8 UNUSED GetObjectEventLoadFlag(void) { return sObjectEventLoadFlag; }
...
u16 GetLocationMusic(struct WarpData *warp) {
  if (NoMusicInSotopolisWithLegendaries(warp) == TRUE)
    ...
    return Overworld_GetMapHeaderByGroupAndId(warp->mapGroup, warp->mapNum)
        ->music;
}
...
void Overworld_ResetMapMusic(void) { ResetMapMusic(); }
...
void Overworld_ClearSavedMusic(void) { gSaveBlock1Ptr->savedMusic = MUS_DUMMY; }
...
void TryFadeOutOldMapMusic(void) {
  ...
  if (FlagGet(FLAG_DONT_TRANSITION_MUSIC) != TRUE &&
    ...
    if (currentMusic == MUS_SURF && VarGet(VAR_SKY_PILLAR_STATE) == 2 &&
        ...
        sWarpDestination.x == 29 && sWarpDestination.y == 53)
      return;
    FadeOutMapMusic(GetMapMusicFadeoutSpeed());
  }
}

bool8 BGMusicStopped(void) { return IsNotWaitingForBGMStop(); }

void Overworld_FadeOutMapMusic(void) { FadeOutMapMusic(4); }
...
static void PlayAmbientCry(void) {
  ...
  pan = (Random() % 88) + 212;
  volume = (Random() % 30) + 50;
  PlayCry_NormalNoDucking(sAmbientCrySpecies, pan, volume,
...
void UpdateAmbientCry(s16 *state, u16 *delayCounter) {
  ...
  switch (*state) {
  case AMB_CRY_INIT:
    // This state will be revisited whenever ResetFieldTasksArgs is called
    // (which happens on map transition)
    ...
    break;
  case AMB_CRY_FIRST:
    // It takes between 1200-3599 frames (~20-60 seconds) to play the first
    // ambient cry after entering a map
  ...
  case AMB_CRY_RESET:
    ...
    // Ambient cries after the first one take between 1200-2399 frames (~20-40
    // seconds) If the player has a Pokémon with the ability Swarm in their
    // party, the time is halved to 600-1199 frames (~10-20 seconds)
...
bool8 IsMapTypeOutdoors(u8 mapType) {
  if (mapType == MAP_TYPE_ROUTE || mapType == MAP_TYPE_TOWN ||
      mapType == MAP_TYPE_UNDERWATER || mapType == MAP_TYPE_CITY ||
...
bool8 Overworld_MapTypeAllowsTeleportAndFly(u8 mapType) {
  if (mapType == MAP_TYPE_ROUTE || mapType == MAP_TYPE_TOWN ||
      mapType == MAP_TYPE_OCEAN_ROUTE || mapType == MAP_TYPE_CITY)
    return TRUE;
  else
    return FALSE;
}
...
u8 GetSavedWarpRegionMapSectionId(void) {
  return Overworld_GetMapHeaderByGroupAndId(
             gSaveBlock1Ptr->dynamicWarp.mapGroup,
             gSaveBlock1Ptr->dynamicWarp.mapNum)
      ->regionMapSectionId;
}

u8 GetCurrentRegionMapSectionId(void) {
  return Overworld_GetMapHeaderByGroupAndId(gSaveBlock1Ptr->location.mapGroup,
                                            gSaveBlock1Ptr->location.mapNum)
      ->regionMapSectionId;
}
...
static void InitOverworldBgs(void) {
  InitBgsFromTemplates(0, sOverworldBgTemplates,
                       ARRAY_COUNT(sOverworldBgTemplates));
...
static void ResetSafariZoneFlag_(void) { ResetSafariZoneFlag(); }
...
void CB2_OverworldBasic(void) { OverworldBasic(); }
...
void SetMainCallback1(MainCallback cb) { gMain.callback1 = cb; }

// This function is never called.
void SetUnusedCallback(void *func) { sUnusedOverworldCallback = func; }
...
static void SetFieldVBlankCallback(void) { SetVBlankCallback(VBlankCB_Field); }
...
static void InitOverworldGraphicsRegisters(void) {
  ...
  SetGpuReg(REG_OFFSET_BLDCNT, gOverworldBackgroundLayerFlags[1] |
  ...
  SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJ_ON | DISPCNT_WIN0_ON |
                                    DISPCNT_WIN1_ON | DISPCNT_OBJ_1D_MAP |
...
static void OffsetCameraFocusByLinkPlayerId(void) {
  ...
  // This is a hack of some kind; it's undone in SpawnLinkPlayers, which is
  // called soon after this function.
...
static void CB1_OverworldLink(void) {
  if (gWirelessCommType == 0 || !IsRfuRecvQueueEmpty() ||
      !IsSendingKeysToLink()) {
    ...
    // LINK_KEY_CODE_EMPTY, this is functionally equivalent to
    // `sPlayerKeyInterceptCallback(0)`. It is expecting the callback to be
    // KeyInterCB_SelfIdle, and that will completely ignore any input
    // parameters.
    //
    // UpdateHeldKeyCode performs a sanity check on its input; if
    // sPlayerKeyInterceptCallback echoes back the argument, which is selfId,
    // then it'll use LINK_KEY_CODE_EMPTY instead.
    //
    // Note 2: There are some key intercept callbacks that treat the key as a
    // player ID. It's so hacky.
...
// to SetKeyInterceptCallback), it does a bunch of sanity checks on the
// connection. I'm not sure if sRfuKeepAliveTimer is reset in the process,
// though; rfu stuff is still undocumented.
...
static void HandleLinkPlayerKeyInput(u32 playerId, u16 key,
...
static void UpdateHeldKeyCode(u16 key) {
  ...
  if (gWirelessCommType != 0 && GetLinkSendQueueLength() > 1 &&
      IsOverworldLinkActive() == TRUE && IsSendingKeysToLink() == TRUE) {
    switch (key) {
...
// Ignore the player's inputs as long as there is an event script being
// executed.
...
static u16 KeyInterCB_SendNothing(u32 key) { return LINK_KEY_CODE_EMPTY; }
...
static u16 KeyInterCB_InLinkActivity(u32 key) { return LINK_KEY_CODE_EMPTY; }

u32 GetCableClubPartnersReady(void) {
  if (IsAnyPlayerInLinkState(PLAYER_LINK_STATE_EXITING_ROOM) == TRUE)
    return CABLE_SEAT_FAILED;
  if (sPlayerKeyInterceptCallback == KeyInterCB_Ready &&
      sPlayerLinkStates[gLocalLinkPlayerId] != PLAYER_LINK_STATE_READY)
    return CABLE_SEAT_WAITING;
  if (sPlayerKeyInterceptCallback == KeyInterCB_ExitingSeat &&
...
static void LoadCableClubPlayer(s32 linkPlayerId, s32 myPlayerId,
                                struct CableClubPlayer *trainer) {
  s16 x, y;
...
static const u8 *TryInteractWithPlayer(struct CableClubPlayer *player) {
  ...
  return GetInteractedLinkPlayerScript(
      &otherPlayerPos, player->metatileBehavior, player->facing);
}
...
static void InitLinkPlayerQueueScript(void) { LockPlayerFieldControls(); }
...
static void
ZeroLinkPlayerObjectEvent(struct LinkPlayerObjectEvent *linkPlayerObjEvent) {
...
#define linkDirection(obj)                                                     \
  ((u8 *)obj)[offsetof(typeof(*obj), fieldEffectSpriteId) - 1] // -> rangeX

static void SpawnLinkPlayerObjectEvent(u8 linkPlayerId, s16 x, s16 y,
                                       u8 gender) {
  u8 objEventId = GetFirstInactiveObjectEventId();
  struct LinkPlayerObjectEvent *linkPlayerObjEvent =
      &gLinkPlayerObjectEvents[linkPlayerId];
...
static void InitLinkPlayerObjectEventPos(struct ObjectEvent *objEvent, s16 x,
                                         s16 y) {
  objEvent->currentCoords.x = x;
  objEvent->currentCoords.y = y;
  objEvent->previousCoords.x = x;
  objEvent->previousCoords.y = y;
  SetSpritePosToMapCoords(x, y, &objEvent->initialCoords.x,
                          &objEvent->initialCoords.y);
  ...
  for (i = 0; i < MAX_LINK_PLAYERS; i++) {
    if (gLinkPlayerObjectEvents[i].active &&
      ...
      struct ObjectEvent *objEvent =
          &gObjectEvents[gLinkPlayerObjectEvents[i].objEventId];
...
// This is a hack to split this code onto two separate lines, without declaring
// a local variable. C++ style inline variables would be nice here.
#define TEMP                                                                   \
  sLinkPlayerMovementModes[linkPlayerObjEvent->movementMode](                  \
      linkPlayerObjEvent, objEvent, facing)
...
static u8
MovementEventModeCB_Normal(struct LinkPlayerObjectEvent *linkPlayerObjEvent,
                           struct ObjectEvent *objEvent, u8 dir) {
  return sLinkPlayerFacingHandlers[dir](linkPlayerObjEvent, objEvent, dir);
}

static u8
MovementEventModeCB_Ignored(struct LinkPlayerObjectEvent *linkPlayerObjEvent,
                            struct ObjectEvent *objEvent, u8 dir) {
  return FACING_UP;
}
...
static u8
MovementEventModeCB_Scripted(struct LinkPlayerObjectEvent *linkPlayerObjEvent,
                             struct ObjectEvent *objEvent, u8 dir) {
  return sLinkPlayerFacingHandlers[dir](linkPlayerObjEvent, objEvent, dir);
}

static bool8
FacingHandler_DoNothing(struct LinkPlayerObjectEvent *linkPlayerObjEvent,
                        struct ObjectEvent *objEvent, u8 dir) {
  return FALSE;
}

static bool8
FacingHandler_DpadMovement(struct LinkPlayerObjectEvent *linkPlayerObjEvent,
                           struct ObjectEvent *objEvent, u8 dir) {
  s16 x, y;

  linkDirection(objEvent) =
      FlipVerticalAndClearForced(dir, linkDirection(objEvent));
  ObjectEventMoveDestCoords(objEvent, linkDirection(objEvent), &x, &y);

  if (LinkPlayerGetCollision(linkPlayerObjEvent->objEventId,
                             linkDirection(objEvent), x, y)) {
    return FALSE;
  } else {
...
// This is called every time a free movement happens. Most of the time it's a
// No-Op.
...
static void MovementStatusHandler_TryAdvanceScript(
  ...
  MoveCoords(linkDirection(objEvent), &objEvent->initialCoords.x,
             &objEvent->initialCoords.y);
...
// to oldFacing. Note that this clears also the "FORCED" part of the facing
// code, even for Left/Right codes.
...
static u8 LinkPlayerGetCollision(u8 selfObjEventId, u8 direction, s16 x,
                                 s16 y) {
  u8 i;
  ...
  if (linkPlayerObjEvent->active) {
    ...
    case VERSION_LEAF_GREEN:
      objEvent->spriteId = CreateObjectGraphicsSprite(
          GetFRLGAvatarGraphicsIdByGender(linkGender(objEvent)),
          SpriteCB_LinkPlayer, 0, 0, 0);
      break;
    case VERSION_RUBY:
    case VERSION_SAPPHIRE:
      objEvent->spriteId = CreateObjectGraphicsSprite(
          GetRSAvatarGraphicsIdByGender(linkGender(objEvent)),
          SpriteCB_LinkPlayer, 0, 0, 0);
      break;
...
static void SpriteCB_LinkPlayer(struct Sprite *sprite) {
  struct LinkPlayerObjectEvent *linkPlayerObjEvent =
      &gLinkPlayerObjectEvents[sprite->data[0]];
  ...
  if (linkPlayerObjEvent->movementMode == MOVEMENT_MODE_FREE)
    ...
    StartSpriteAnimIfDifferent(
        sprite, GetMoveDirectionAnimNum(linkDirection(objEvent)));
