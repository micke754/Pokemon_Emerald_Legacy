...
static const u16 sWirelessLinkDisplayPal[] =
    INCBIN_U16("graphics/link/wireless_display.gbapal");
static const u32 sWirelessLinkDisplayGfx[] =
    INCBIN_U32("graphics/link/wireless_display.4bpp.lz");
static const u32 sWirelessLinkDisplayTilemap[] =
    INCBIN_U32("graphics/link/wireless_display.bin.lz");
static const u16 sLinkTestDigitsPal[] =
    INCBIN_U16("graphics/link/test_digits.gbapal");
static const u16 sLinkTestDigitsGfx[] =
    INCBIN_U16("graphics/link/test_digits.4bpp");
static const u8 sUnusedTransparentWhite[] =
    _("{HIGHLIGHT TRANSPARENT}{COLOR WHITE}");
static const u16 sCommErrorBg_Gfx[] =
    INCBIN_U16("graphics/link/comm_error_bg.4bpp");
...
static const u8 sBGControlRegs[] = {REG_OFFSET_BG0CNT, REG_OFFSET_BG1CNT,
...
static const u8 sTextColors[] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_WHITE,
...
void Task_DestroySelf(u8 taskId) { DestroyTask(taskId); }

static void InitLinkTestBG(u8 paletteNum, u8 bgNum, u8 screenBaseBlock,
                           u8 charBaseBlock, u16 baseChar) {
  LoadPalette(sLinkTestDigitsPal, BG_PLTT_ID(paletteNum), PLTT_SIZE_4BPP);
  DmaCopy16(3, sLinkTestDigitsGfx,
            (u16 *)BG_CHAR_ADDR(charBaseBlock) + (16 * baseChar),
  ...
  switch (bgNum) {
  case 1:
    SetGpuReg(REG_OFFSET_BG1CNT, BGCNT_SCREENBASE(screenBaseBlock) |
                                     BGCNT_PRIORITY(1) |
                                     BGCNT_CHARBASE(charBaseBlock));
    break;
  case 2:
    SetGpuReg(REG_OFFSET_BG2CNT, BGCNT_SCREENBASE(screenBaseBlock) |
                                     BGCNT_PRIORITY(1) |
                                     BGCNT_CHARBASE(charBaseBlock));
    break;
  case 3:
    SetGpuReg(REG_OFFSET_BG3CNT, BGCNT_SCREENBASE(screenBaseBlock) |
                                     BGCNT_PRIORITY(1) |
                                     BGCNT_CHARBASE(charBaseBlock));
    break;
  }
...
static void UNUSED LoadLinkTestBgGfx(u8 paletteNum, u8 bgNum,
                                     u8 screenBaseBlock, u8 charBaseBlock) {
  LoadPalette(sLinkTestDigitsPal, BG_PLTT_ID(paletteNum), PLTT_SIZE_4BPP);
  DmaCopy16(3, sLinkTestDigitsGfx, (u16 *)BG_CHAR_ADDR(charBaseBlock),
            sizeof sLinkTestDigitsGfx);
  gLinkTestBGInfo.screenBaseBlock = screenBaseBlock;
  gLinkTestBGInfo.paletteNum = paletteNum;
  gLinkTestBGInfo.baseChar = 0;
  SetGpuReg(sBGControlRegs[bgNum],
            BGCNT_SCREENBASE(screenBaseBlock) | BGCNT_CHARBASE(charBaseBlock));
}
...
void SetLocalLinkPlayerId(u8 playerId) { gLocalLinkPlayer.id = playerId; }
...
static void LinkTestProcessKeyInput(void) {
  ...
  if (sLinkTestDebugValuesEnabled) {
    SetLinkDebugValues(gMain.vblankCounter2, gLinkCallback
                                                 ? gLinkVSyncDisabled
                                                 : gLinkVSyncDisabled | 0x10);
  }
}
  ...
  for (i = 0; i < MAX_LINK_PLAYERS; i++) {
    ...
    case LINKCMD_CONT_BLOCK: {
      if (sBlockRecv[i].size > BLOCK_BUFFER_SIZE) {
        ...
        for (j = 0; j < CMD_LENGTH - 1; j++) {
          gBlockRecvBuffer[i][(sBlockRecv[i].pos / 2) + j] =
              gRecvCmds[i][j + 1];
        }
      }
    ...
    case LINKCMD_SEND_BLOCK_REQ:
      SendBlock(0, sBlockRequests[gRecvCmds[i][1]].address,
...
u32 GetLinkPlayerTrainerId(u8 who) { return gLinkPlayers[who].trainerId; }
...
static void LinkCB_BlockSend(void) {
  ...
  for (i = 0; i < CMD_LENGTH - 1; i++) {
    gSendCmd[i + 1] =
        (src[sBlockSend.pos + i * 2 + 1] << 8) | src[sBlockSend.pos + i * 2];
  }
  sBlockSend.pos += 14;
...
static void LinkCB_BlockSendEnd(void) { gLinkCallback = NULL; }
...
static void LinkTest_PrintNumChar(char val, u8 x, u8 y) {
  ...
  vAddr[y * 32 + x] =
      (gLinkTestBGInfo.paletteNum << 12) | (val + 1 + gLinkTestBGInfo.baseChar);
}
...
static void LinkTest_PrintChar(char val, u8 x, u8 y) {
  ...
  vAddr = (u16 *)BG_SCREEN_ADDR(gLinkTestBGInfo.screenBaseBlock);
  vAddr[y * 32 + x] =
      (gLinkTestBGInfo.paletteNum << 12) | (val + gLinkTestBGInfo.baseChar);
}
...
// The number of players when trading began. This is frequently compared against
// the current number of connected players to check if anyone dropped out.
u8 GetSavedPlayerCount(void) { return gSavedLinkPlayerCount; }

static u8 UNUSED GetSavedMultiplayerId(void) { return gSavedMultiplayerId; }
...
u8 GetLinkPlayerCount_2(void) { return EXTRACT_PLAYER_COUNT(gLinkStatus); }
...
static u8 UNUSED GetDummy2(void) { return sDummy2; }
...
// Used instead of SetCloseLinkCallback when disconnecting from an attempt to
// link with a foreign game
...
void SetLinkErrorBuffer(u32 status, u8 lastSendQueueCount,
                        u8 lastRecvQueueCount, bool8 disconnected) {
...
void CB2_LinkError(void) {
  ...
  InitBgsFromTemplates(0, sLinkErrorBgTemplates,
                       ARRAY_COUNT(sLinkErrorBgTemplates));
...
static void ErrorMsg_MoveCloserToPartner(void) {
  ...
  LoadPalette(sWirelessLinkDisplayPal, BG_PLTT_ID(0),
              sizeof(sWirelessLinkDisplayPal));
  FillWindowPixelBuffer(WIN_LINK_ERROR_TOP, PIXEL_FILL(0));
  FillWindowPixelBuffer(WIN_LINK_ERROR_BOTTOM, PIXEL_FILL(0));
  AddTextPrinterParameterized3(WIN_LINK_ERROR_TOP, FONT_SHORT_COPY_1, 2, 6,
                               sTextColors, 0, gText_CommErrorEllipsis);
  AddTextPrinterParameterized3(WIN_LINK_ERROR_BOTTOM, FONT_SHORT_COPY_1, 2, 1,
                               sTextColors, 0, gText_MoveCloserToLinkPartner);
...
static void ErrorMsg_CheckConnections(void) {
  ...
  FillWindowPixelBuffer(WIN_LINK_ERROR_BOTTOM, PIXEL_FILL(0));
  AddTextPrinterParameterized3(WIN_LINK_ERROR_MID, FONT_SHORT_COPY_1, 2, 0,
                               sTextColors, 0, gText_CommErrorCheckConnections);
...
static void CB2_PrintErrorMessage(void) {
  ...
  case 130:
    if (gWirelessCommType == 2)
      AddTextPrinterParameterized3(WIN_LINK_ERROR_TOP, FONT_SHORT_COPY_1, 2, 20,
                                   sTextColors, 0, gText_ABtnTitleScreen);
...
bool8 GetSioMultiSI(void) { return (REG_SIOCNT & SIO_MULTI_SI) != 0; }
...
bool8 HasLinkErrorOccurred(void) { return gLinkErrorOccurred; }
...
// Unused
u8 GetWirelessCommType(void) { return gWirelessCommType; }

void ConvertLinkPlayerName(struct LinkPlayer *player) {
  player->progressFlagsCopy =
      player->progressFlags; // ? Perhaps relocating for a longer name field
...
u32 LinkMain1(u8 *shouldAdvanceLinkState, u16 *sendCmd,
              u16 (*recvCmds)[CMD_LENGTH]) {
  u32 retVal;
  u32 retVal2;
  ...
  *shouldAdvanceLinkState = 0;
  ...
  {
    u32 receivedNothing = gLink.receivedNothing
...
static bool8 DoHandshake(void) {
  ...
  *(u64 *)gLink.handshakeBuffer = REG_SIOMLT_RECV;
  ...
  for (i = 0; i < MAX_LINK_PLAYERS; i++) {
    if ((gLink.handshakeBuffer[i] & ~0x3) == SLAVE_HANDSHAKE ||
