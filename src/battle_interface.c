...
enum { // Corresponds to gHealthboxElementsGfxTable (and the tables after it) in
       // graphics.c These are indexes into the tables, which are filled with
       // 8x8 square pixel data.
...
static u8 *AddTextPrinterAndCreateWindowOnHealthbox(const u8 *, u32, u32, u32,
                                                    u32 *);
...
static const struct SpriteTemplate
    sHealthbarSpriteTemplates[MAX_BATTLERS_COUNT] = {
...
// This subsprite table has the same layout as above, but offset by 64 base
// tiles.
...
static const struct SubspriteTable sHealthBar_SubspriteTables[] = {
    [B_SIDE_PLAYER] = {ARRAY_COUNT(sHealthBar_Subsprites_Player),
                       sHealthBar_Subsprites_Player},
    [B_SIDE_OPPONENT] = {ARRAY_COUNT(sHealthBar_Subsprites_Opponent),
...
static const struct SubspriteTable sStatusSummaryBar_SubspriteTable_Enter[] = {
    {ARRAY_COUNT(sStatusSummaryBar_Subsprites_Enter),
     sStatusSummaryBar_Subsprites_Enter}};

static const struct SubspriteTable sStatusSummaryBar_SubspriteTable_Exit[] = {
    {ARRAY_COUNT(sStatusSummaryBar_Subsprites_Exit),
     sStatusSummaryBar_Subsprites_Exit}};

// unused unknown image
static const u8 sUnusedStatusSummary[] =
    INCBIN_U8("graphics/battle_interface/unused_status_summary.4bpp");
...
static const struct SpriteSheet sStatusSummaryBallsSpriteSheet = {
    &gHealthboxElementsGfxTable[HEALTHBOX_GFX_STATUS_BALL], 0x80,
...
static const u8 sEmptyWhiteText_GrayHighlight[] =
    __("{COLOR WHITE}{HIGHLIGHT DARK_GRAY}              ");
static const u8 sEmptyWhiteText_TransparentHighlight[] =
    __("{COLOR WHITE}{HIGHLIGHT TRANSPARENT}              ");
...
static const u16 sStatusIconColors[] = {
    [PAL_STATUS_PSN] = RGB(24, 12, 24), [PAL_STATUS_PAR] = RGB(23, 23, 3),
    [PAL_STATUS_SLP] = RGB(20, 20, 17), [PAL_STATUS_FRZ] = RGB(17, 22, 28),
    [PAL_STATUS_BRN] = RGB(28, 14, 10),
};

static const struct WindowTemplate sHealthboxWindowTemplate = {.bg = 0,
                                                               .tilemapLeft = 0,
                                                               .tilemapTop = 0,
                                                               .width = 8,
                                                               .height = 2,
...
// Because the healthbox is too large to fit into one sprite, it is divided into
// two sprites. healthboxLeft  or healthboxMain  is the left part that is used
// as the 'main' sprite. healthboxRight or healthboxOther is the right part of
// the healthbox. There's also the third sprite under name of healthbarSprite
// that refers to the healthbar visible on the healtbox.
...
u8 CreateBattlerHealthboxSprites(u8 battlerId) {
  ...
  if (!IsDoubleBattle()) {
    if (GetBattlerSide(battlerId) == B_SIDE_PLAYER) {
      healthboxLeftSpriteId = CreateSprite(&sHealthboxPlayerSpriteTemplates[0],
                                           DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
      healthboxRightSpriteId =
          CreateSpriteAtEnd(&sHealthboxPlayerSpriteTemplates[0], DISPLAY_WIDTH,
                            DISPLAY_HEIGHT, 1);
      ...
      healthboxLeftSpriteId =
          CreateSprite(&sHealthboxOpponentSpriteTemplates[0], DISPLAY_WIDTH,
                       DISPLAY_HEIGHT, 1);
      healthboxRightSpriteId =
          CreateSpriteAtEnd(&sHealthboxOpponentSpriteTemplates[0],
                            DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
    ...
    gSprites[healthboxRightSpriteId].hOther_HealthBoxSpriteId =
        healthboxLeftSpriteId;
    gSprites[healthboxRightSpriteId].callback = SpriteCB_HealthBoxOther;
  } else {
    if (GetBattlerSide(battlerId) == B_SIDE_PLAYER) {
      healthboxLeftSpriteId = CreateSprite(
          &sHealthboxPlayerSpriteTemplates[GetBattlerPosition(battlerId) / 2],
          DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
      ...
      healthboxLeftSpriteId = CreateSprite(
          &sHealthboxOpponentSpriteTemplates[GetBattlerPosition(battlerId) / 2],
          DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
      healthboxRightSpriteId = CreateSpriteAtEnd(
          &sHealthboxOpponentSpriteTemplates[GetBattlerPosition(battlerId) / 2],
          DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
  ...
  healthbarSpriteId = CreateSpriteAtEnd(
      &sHealthbarSpriteTemplates[gBattlerPositions[battlerId]], 140, 60, 0);
  healthBarSpritePtr = &gSprites[healthbarSpriteId];
  SetSubspriteTables(healthBarSpritePtr,
                     &sHealthBar_SubspriteTables[GetBattlerSide(battlerId)]);
  healthBarSpritePtr->subspriteMode = SUBSPRITES_IGNORE_PRIORITY;
  healthBarSpritePtr->oam.priority = 1;

  CpuCopy32(
      GetHealthboxElementGfxPtr(HEALTHBOX_GFX_1),
...
u8 CreateSafariPlayerHealthboxSprites(void) {
  ...
  healthboxLeftSpriteId = CreateSprite(&sHealthboxSafariSpriteTemplate,
                                       DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
  healthboxRightSpriteId = CreateSpriteAtEnd(&sHealthboxSafariSpriteTemplate,
                                             DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
...
void SetBattleBarStruct(u8 battlerId, u8 healthboxSpriteId, s32 maxVal,
                        s32 oldVal, s32 receivedValue) {
  gBattleSpritesDataPtr->battleBars[battlerId].healthboxSpriteId =
      healthboxSpriteId;
...
void SetHealthboxSpriteInvisible(u8 healthboxSpriteId) {
  ...
  gSprites[gSprites[healthboxSpriteId].hMain_HealthBarSpriteId].invisible =
      TRUE;
...
void DummyBattleInterfaceFunc(u8 healthboxSpriteId,
                              bool8 isDoubleBattleBattlerOnly) {}
...
static void UpdateLvlInHealthbox(u8 healthboxSpriteId, u8 lvl) {
  ...
  windowTileData =
      AddTextPrinterAndCreateWindowOnHealthbox(text, xPos, 3, 2, &windowId);
  spriteTileNum = gSprites[healthboxSpriteId].oam.tileNum * TILE_SIZE_4BPP;

  if (GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler) ==
      B_SIDE_PLAYER) {
...
void UpdateHpTextInHealthbox(u8 healthboxSpriteId, s16 value, u8 maxOrCurrent) {
  ...
  if (GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler) ==
    ...
    if (maxOrCurrent != HP_CURRENT) // singles, max
    {
      ...
      windowTileData =
          AddTextPrinterAndCreateWindowOnHealthbox(text, 0, 5, 2, &windowId);
      objVram = (void *)(OBJ_VRAM0);
    ...
    } else // singles, current
    {
      ...
      text[4] = EOS;
      windowTileData =
          AddTextPrinterAndCreateWindowOnHealthbox(text, 4, 5, 2, &windowId);
      objVram = (void *)(OBJ_VRAM0);
    ...
    memcpy(text, sEmptyWhiteText_GrayHighlight,
           sizeof(sEmptyWhiteText_GrayHighlight));
    ...
    if (IsDoubleBattle() == TRUE ||
      ...
      if (GetBattlerSide(gSprites[healthboxSpriteId].data[6]) ==
          B_SIDE_PLAYER) {
...
static void UpdateHpTextInHealthboxInDoubles(u8 healthboxSpriteId, s16 value,
                                             u8 maxOrCurrent) {
  u32 windowId, spriteTileNum;
  u8 *windowTileData;
  u8 text[32];
  ...
  if (GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler) ==
    ...
    {
      spriteTileNum =
          gSprites[gSprites[healthboxSpriteId].data[5]].oam.tileNum *
          TILE_SIZE_4BPP;
      ...
      if (maxOrCurrent != HP_CURRENT) // doubles, max hp
      {
        ConvertIntToDecimalStringN(text, value, STR_CONV_MODE_RIGHT_ALIGN, 3);
        windowTileData =
            AddTextPrinterAndCreateWindowOnHealthbox(text, 0, 5, 0, &windowId);
        HpTextIntoHealthboxObject((void *)(OBJ_VRAM0) + spriteTileNum + 0xC0,
                                  windowTileData, 2);
        RemoveWindowOnHealthbox(windowId);
        CpuCopy32(
            GetHealthboxElementGfxPtr(HEALTHBOX_GFX_FRAME_END),
        ...
        text[4] = EOS;
        windowTileData =
            AddTextPrinterAndCreateWindowOnHealthbox(text, 4, 5, 0, &windowId);
        FillHealthboxObject(objVram, 0, 3); // Erases HP bar leftover.
        HpTextIntoHealthboxObject((void *)(OBJ_VRAM0 + 0x60) + spriteTileNum,
                                  windowTileData, 3);
        RemoveWindowOnHealthbox(windowId);
      }
    }
    ...
    memcpy(text, sEmptyWhiteText_TransparentHighlight,
           sizeof(sEmptyWhiteText_TransparentHighlight));
            ...
            .hpNumbersNoBars) // don't print text if only bars are visible
    {
      ...
      txtPtr = ConvertIntToDecimalStringN(text + 6, value,
      ...
      if (maxOrCurrent == HP_CURRENT) {
        CpuCopy32(&gMonSpritesGfxPtr->barFontGfx[224],
                  (void *)((OBJ_VRAM0) +
                           ((gSprites[r7].oam.tileNum + 4) * TILE_SIZE_4BPP)),
                  0x20);
        CpuFill32(
            0,
            (void *)((OBJ_VRAM0) + (gSprites[r7].oam.tileNum * TILE_SIZE_4BPP)),
            0x20);
      } else {
        if (GetBattlerSide(battlerId) ==
            B_SIDE_PLAYER) // Impossible to reach part, because the battlerId is
                           // from the opponent's side.
        {
          CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_FRAME_END),
                    (void *)(OBJ_VRAM0) +
                        ((gSprites[healthboxSpriteId].oam.tileNum + 52) *
                         TILE_SIZE_4BPP),
                    0x20);
        }
      }
...
// Prints mon's nature, catch and flee rate. Probably used to test
// pokeblock-related features.
static void PrintSafariMonInfo(u8 healthboxSpriteId, struct Pokemon *mon) {
  ...
  barFontGfx =
      &gMonSpritesGfxPtr
           ->barFontGfx[0x520 + (GetBattlerPosition(gSprites[healthboxSpriteId]
                                                        .hMain_Battler) *
                                 384)];
  var = 5;
  ...
  for (j = 6, i = 0; i < var; i++, j++) {
    ...
    if ((text[j] >= 55 && text[j] <= 74) || (text[j] >= 135 && text[j] <= 154))
      elementId = 44;
    else if ((text[j] >= 75 && text[j] <= 79) ||
             (text[j] >= 155 && text[j] <= 159))
      elementId = 45;
    else
      elementId = 43;

    CpuCopy32(GetHealthboxElementGfxPtr(elementId), barFontGfx + (i * 64),
              0x20);
  }
  ...
  for (j = 1; j < var + 1; j++) {
    ...
    spriteTileNum = (8 + gSprites[healthboxSpriteId].oam.tileNum +
                     (j - (j / 8 * 8)) + (j / 8 * 64)) *
                    TILE_SIZE_4BPP;
    CpuCopy32(barFontGfx, (void *)(OBJ_VRAM0) + (spriteTileNum), 0x20);
    barFontGfx += 0x20;
  ...
  ConvertIntToDecimalStringN(&text[6], gBattleStruct->safariCatchFactor,
                             STR_CONV_MODE_RIGHT_ALIGN, 2);
  ConvertIntToDecimalStringN(&text[9], gBattleStruct->safariEscapeFactor,
                             STR_CONV_MODE_RIGHT_ALIGN, 2);
  text[5] = CHAR_SPACE;
  text[8] = CHAR_SLASH;
  RenderTextHandleBold(gMonSpritesGfxPtr->barFontGfx, FONT_BOLD, text);
  ...
  for (j = 0; j < 5; j++) {
    if (j <= 1) {
      CpuCopy32(&gMonSpritesGfxPtr->barFontGfx[0x40 * j + 0x20],
                (void *)(OBJ_VRAM0) +
                    (gSprites[healthBarSpriteId].oam.tileNum + 2 + j) *
                        TILE_SIZE_4BPP,
                32);
    } else {
      CpuCopy32(&gMonSpritesGfxPtr->barFontGfx[0x40 * j + 0x20],
                (void *)(OBJ_VRAM0 + 0xC0) +
                    (j + gSprites[healthBarSpriteId].oam.tileNum) *
                        TILE_SIZE_4BPP,
                32);
    }
  }
    ...
    if (gSprites[gHealthboxSpriteIds[i]].callback == SpriteCallbackDummy &&
      ...
      if (GetBattlerSide(i) == B_SIDE_PLAYER) {
        ...
        if (noBars == TRUE) // bars to text
        {
          ...
          CpuFill32(
              0,
              (void *)(OBJ_VRAM0 + gSprites[healthBarSpriteId].oam.tileNum *
                                       TILE_SIZE_4BPP),
              0x100);
          ...
          UpdateHpTextInHealthboxInDoubles(
              gHealthboxSpriteIds[i],
              GetMonData(&gPlayerParty[gBattlerPartyIndexes[i]],
                         MON_DATA_MAX_HP),
              HP_MAX);
        } else // text to bars
        {
          ...
          CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_FRAME_END_BAR),
                    (void *)(OBJ_VRAM0 + 0x680 +
                             gSprites[gHealthboxSpriteIds[i]].oam.tileNum *
                                 TILE_SIZE_4BPP),
                    32);
        }
      } else {
        if (noBars == TRUE) // bars to text
        {
          if (gBattleTypeFlags & BATTLE_TYPE_SAFARI) {
            ...
            PrintSafariMonInfo(gHealthboxSpriteIds[i],
            ...
            CpuFill32(0,
                      (void *)(OBJ_VRAM0 +
                               gSprites[healthBarSpriteId].oam.tileNum * 32),
                      0x100);
            UpdateHpTextInHealthboxInDoubles(
                gHealthboxSpriteIds[i],
            ...
            UpdateHpTextInHealthboxInDoubles(
                gHealthboxSpriteIds[i],
                GetMonData(&gEnemyParty[gBattlerPartyIndexes[i]],
                           MON_DATA_MAX_HP),
                HP_MAX);
          }
        } else // text to bars
...
u8 CreatePartyStatusSummarySprites(u8 battlerId, struct HpAndStatus *partyInfo,
                                   bool8 skipPlayer, bool8 isBattleStart) {
  bool8 isOpponent;
  ...
  if (!skipPlayer ||
      GetBattlerPosition(battlerId) != B_POSITION_OPPONENT_RIGHT) {
    if (GetBattlerSide(battlerId) == B_SIDE_PLAYER) {
      isOpponent = FALSE;
      bar_X = 136, bar_Y = 96;
      bar_pos2_X = 100;
  ...
  summaryBarSpriteId = CreateSprite(
      &sStatusSummaryBarSpriteTemplates[isOpponent], bar_X, bar_Y, 10);
  SetSubspriteTables(&gSprites[summaryBarSpriteId],
  ...
  for (i = 0; i < PARTY_SIZE; i++) {
    ballIconSpritesIds[i] = CreateSpriteAtEnd(
        &sStatusSummaryBallsSpriteTemplates[isOpponent], bar_X, bar_Y - 4, 9);
      ...
      for (var = 0, i = 0, j = 0; j < PARTY_SIZE; j++) {
        if (partyInfo[j].hp == HP_EMPTY_SLOT) {
          ...
          gSprites[ballIconSpritesIds[PARTY_SIZE - 1 - var]].oam.tileNum += 3;
                   ...
                   gBattleStruct->arenaLostOpponentMons & gBitTable[j]) {
          // fainted arena mon
          gSprites[ballIconSpritesIds[PARTY_SIZE - 1 - var]].oam.tileNum += 3;
        } else if (partyInfo[j].status != 0) {
          // mon with primary status
  ...
  if (isBattleStart) {
    for (i = 0; i < PARTY_SIZE; i++) {
      if (GetBattlerSide(battlerId) != B_SIDE_PLAYER) {
        ...
        gSprites[ballIconSpriteIds[PARTY_SIZE - 1 - i]].callback =
        ...
        gSprites[ballIconSpriteIds[i]].callback =
            SpriteCB_StatusSummaryBalls_Exit;
      }
    }
...
static void Task_HidePartyStatusSummary_BattleStart_1(u8 taskId) {
  if ((gTasks[taskId].data[11]++ % 2) == 0) {
    ...
    SetGpuReg(REG_OFFSET_BLDALPHA, BLDALPHA_BLEND(gTasks[taskId].tBlend,
                                                  16 - gTasks[taskId].tBlend));
  }
  if (gTasks[taskId].tBlend == 0)
...
static void Task_HidePartyStatusSummary_BattleStart_2(u8 taskId) {
  ...
  if (--gTasks[taskId].tBlend == -1) {
    ...
    gBattleSpritesDataPtr->healthBoxesData[battlerId].partyStatusSummaryShown =
        0;
...
static void UpdateNickInHealthbox(u8 healthboxSpriteId, struct Pokemon *mon) {
  ...
  // AddTextPrinterAndCreateWindowOnHealthbox's arguments are the same in all 3
  // cases. It's possible they may have been different in early development
  // phases.
  switch (gender) {
  default:
  ...
  if (GetBattlerSide(gSprites[healthboxSpriteId].data[6]) == B_SIDE_PLAYER) {
    TextIntoHealthboxObject((void *)(OBJ_VRAM0 + 0x40 + spriteTileNum),
                            windowTileData, 6);
    ptr = (void *)(OBJ_VRAM0);
    if (!IsDoubleBattle())
    ...
    TextIntoHealthboxObject((void *)(OBJ_VRAM0 + 0x20 + spriteTileNum),
                            windowTileData, 7);
  }
...
static void TryAddPokeballIconToHealthbox(u8 healthboxSpriteId,
  ...
  if (!GetSetPokedexFlag(
          SpeciesToNationalPokedexNum(GetMonData(
  ...
  if (noStatus)
    CpuCopy32(
        GetHealthboxElementGfxPtr(HEALTHBOX_GFX_STATUS_BALL_CAUGHT),
        (void *)(OBJ_VRAM0 + (gSprites[healthBarSpriteId].oam.tileNum + 8) *
                                 TILE_SIZE_4BPP),
        32);
  else
    CpuFill32(
...
static void UpdateStatusIconInHealthbox(u8 healthboxSpriteId) {
  ...
  healthBarSpriteId = gSprites[healthboxSpriteId].hMain_HealthBarSpriteId;
  if (GetBattlerSide(battlerId) == B_SIDE_PLAYER) {
    status = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                        MON_DATA_STATUS);
    if (!IsDoubleBattle())
      tileNumAdder = 0x1A;
    else
    ...
    status = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]],
                        MON_DATA_STATUS);
    tileNumAdder = 0x11;
  }
  ...
  if (status & STATUS1_SLEEP) {
    statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(
        HEALTHBOX_GFX_STATUS_SLP_BATTLER0, battlerId));
    statusPalId = PAL_STATUS_SLP;
  } else if (status & STATUS1_PSN_ANY) {
    statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(
        HEALTHBOX_GFX_STATUS_PSN_BATTLER0, battlerId));
    statusPalId = PAL_STATUS_PSN;
  } else if (status & STATUS1_BURN) {
    statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(
        HEALTHBOX_GFX_STATUS_BRN_BATTLER0, battlerId));
    statusPalId = PAL_STATUS_BRN;
  } else if (status & STATUS1_FREEZE) {
    statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(
        HEALTHBOX_GFX_STATUS_FRZ_BATTLER0, battlerId));
    statusPalId = PAL_STATUS_FRZ;
  } else if (status & STATUS1_PARALYSIS) {
    statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(
        HEALTHBOX_GFX_STATUS_PRZ_BATTLER0, battlerId));
    ...
    for (i = 0; i < 3; i++)
      CpuCopy32(statusGfxPtr,
                (void *)(OBJ_VRAM0 + (gSprites[healthboxSpriteId].oam.tileNum +
                                      tileNumAdder + i) *
                                         TILE_SIZE_4BPP),
                32);
  ...
  FillPalette(sStatusIconColors[statusPalId], OBJ_PLTT_OFFSET + pltAdder,
              PLTT_SIZEOF(1));
  CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_OFFSET + pltAdder],
            (u16 *)OBJ_PLTT + pltAdder, PLTT_SIZEOF(1));
  CpuCopy32(statusGfxPtr,
            (void *)(OBJ_VRAM0 +
                     (gSprites[healthboxSpriteId].oam.tileNum + tileNumAdder) *
                         TILE_SIZE_4BPP),
            96);
  if (IsDoubleBattle() == TRUE ||
    ...
    if (!gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars) {
      CpuCopy32(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_0),
                ...
                32);
      CpuCopy32(
          GetHealthboxElementGfxPtr(HEALTHBOX_GFX_65),
          (void *)(OBJ_VRAM0 + (gSprites[healthBarSpriteId].oam.tileNum + 1) *
                                   TILE_SIZE_4BPP),
          32);
    }
  }
...
static void UpdateSafariBallsTextOnHealthbox(u8 healthboxSpriteId) {
  ...
  windowTileData = AddTextPrinterAndCreateWindowOnHealthbox(gText_SafariBalls,
                                                            0, 3, 2, &windowId);
  spriteTileNum = gSprites[healthboxSpriteId].oam.tileNum * TILE_SIZE_4BPP;
  TextIntoHealthboxObject((void *)(OBJ_VRAM0 + 0x40) + spriteTileNum,
                          windowTileData, 6);
  TextIntoHealthboxObject((void *)(OBJ_VRAM0 + 0x800) + spriteTileNum,
                          windowTileData + 0xC0, 2);
  RemoveWindowOnHealthbox(windowId);
}
...
static void UpdateLeftNoOfBallsTextOnHealthbox(u8 healthboxSpriteId) {
  ...
  ConvertIntToDecimalStringN(txtPtr, gNumSafariBalls, STR_CONV_MODE_LEFT_ALIGN,
                             2);

  windowTileData = AddTextPrinterAndCreateWindowOnHealthbox(
      text, GetStringRightAlignXOffset(FONT_SMALL, text, 0x2F), 3, 2,
      &windowId);
  spriteTileNum = gSprites[healthboxSpriteId].oam.tileNum * TILE_SIZE_4BPP;
  SafariTextIntoHealthboxObject((void *)(OBJ_VRAM0 + 0x2C0) + spriteTileNum,
                                windowTileData, 2);
  SafariTextIntoHealthboxObject((void *)(OBJ_VRAM0 + 0xA00) + spriteTileNum,
                                windowTileData + 0x40, 4);
  RemoveWindowOnHealthbox(windowId);
}

void UpdateHealthboxAttribute(u8 healthboxSpriteId, struct Pokemon *mon,
                              u8 elementId) {
  s32 maxHp, currHp;
  ...
  if (GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler) ==
    ...
    if (elementId == HEALTHBOX_CURRENT_HP || elementId == HEALTHBOX_ALL)
      UpdateHpTextInHealthbox(healthboxSpriteId, GetMonData(mon, MON_DATA_HP),
                              HP_CURRENT);
    if (elementId == HEALTHBOX_MAX_HP || elementId == HEALTHBOX_ALL)
      UpdateHpTextInHealthbox(healthboxSpriteId,
                              GetMonData(mon, MON_DATA_MAX_HP), HP_MAX);
    ...
    if (!isDoubles &&
        (elementId == HEALTHBOX_EXP_BAR || elementId == HEALTHBOX_ALL)) {
      u16 species;
      ...
      maxExpBarValue =
          ...
          currLevelExp;
      SetBattleBarStruct(battlerId, healthboxSpriteId, maxExpBarValue,
                         currExpBarValue, isDoubles);
...
s32 MoveBattleBar(u8 battlerId, u8 healthboxSpriteId, u8 whichBar, u8 unused) {
  ...
  } else // exp bar
  {
    ...
    if (expFraction == 0)
      expFraction = 1;
    expFraction =
        abs(gBattleSpritesDataPtr->battleBars[battlerId].receivedValue /
            expFraction);
...
static void MoveBattleBarGraphically(u8 battlerId, u8 whichBar) {
  ...
  case HEALTH_BAR:
    ...
    if (filledPixelsCount >
        (B_HEALTHBAR_PIXELS * 50 / 100)) // more than 50 % hp
      barElementId = HEALTHBOX_GFX_HP_BAR_GREEN;
    else if (filledPixelsCount >
             (B_HEALTHBAR_PIXELS * 20 / 100)) // more than 20% hp
    ...
    for (i = 0; i < 6; i++) {
      ...
      if (i < 2)
        ...
        CpuCopy32(GetHealthboxElementGfxPtr(barElementId) + array[i] * 32,
                  (void *)(OBJ_VRAM0 + 64 +
                           (i + gSprites[healthbarSpriteId].oam.tileNum) *
                               TILE_SIZE_4BPP),
                  32);
    }
    break;
  case EXP_BAR:
    CalcBarFilledPixels(
        ...
        &gBattleSpritesDataPtr->battleBars[battlerId].currValue, array,
        B_EXPBAR_PIXELS / 8);
    level = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                       MON_DATA_LEVEL);
...
static s32 CalcNewBarValue(s32 maxValue, s32 oldValue, s32 receivedValue,
                           s32 *currValue, u8 scale, u16 toAdd) {
  s32 ret, newValue;
  scale *= 8;
  ...
  if (maxValue <
      scale) // handle cases of max var having less pixels than the whole bar
  {
...
static u8 CalcBarFilledPixels(s32 maxValue, s32 oldValue, s32 receivedValue,
                              s32 *currValue, u8 *pixelsArray, u8 scale) {
...
static s16 UNUSED Debug_TestHealthBar(struct TestingBar *barInfo,
                                      s32 *currValue, u16 *dest, s32 unused) {
  s16 ret, var;

  ret = CalcNewBarValue(barInfo->maxValue, barInfo->oldValue,
                        barInfo->receivedValue, currValue,
                        B_HEALTHBAR_PIXELS / 8, 1);
...
static void Debug_TestHealthBar_Helper(struct TestingBar *barInfo,
                                       s32 *currValue, u16 *dest) {
  u8 pixels[6];
  u16 src[6];
  u8 i;

  CalcBarFilledPixels(barInfo->maxValue, barInfo->oldValue,
                      barInfo->receivedValue, currValue, pixels,
                      B_HEALTHBAR_PIXELS / 8);
...
static u8 GetScaledExpFraction(s32 oldValue, s32 receivedValue, s32 maxValue,
                               u8 scale) {
  s32 newVal, result;
...
static u8 *AddTextPrinterAndCreateWindowOnHealthbox(const u8 *str, u32 x, u32 y,
                                                    u32 bgColor,
                                                    u32 *windowId) {
  u16 winId;
  u8 color[3];
  ...
  AddTextPrinterParameterized4(winId, FONT_SMALL, x, y, 0, 0, color,
                               TEXT_SKIP_DRAW, str);
...
static void RemoveWindowOnHealthbox(u32 windowId) { RemoveWindow(windowId); }
...
static void HpTextIntoHealthboxObject(void *dest, u8 *windowTileData,
                                      u32 windowWidth) {
  CpuCopy32(windowTileData + 256, dest, windowWidth * TILE_SIZE_4BPP);
}

static void TextIntoHealthboxObject(void *dest, u8 *windowTileData,
                                    s32 windowWidth) {
  CpuCopy32(windowTileData + 256, dest + 256, windowWidth * TILE_SIZE_4BPP);
  // + 256 as that prevents the top 4 blank rows of sHealthboxWindowTemplate
  // from being copied
  if (windowWidth > 0) {
    do {
...
static void SafariTextIntoHealthboxObject(void *dest, u8 *windowTileData,
                                          u32 windowWidth) {
