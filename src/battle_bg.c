...
void InitBattleBgsVideo(void) {
  ...
  SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJWIN_ON | DISPCNT_WIN0_ON |
...
void LoadBattleMenuWindowGfx(void) {
  ...
  LoadCompressedPalette(gBattleWindowTextPalette, BG_PLTT_ID(5),
                        PLTT_SIZE_4BPP);
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_ARENA) {
    ...
    CpuCopy16(&gPlttBufferUnfaded[BG_PLTT_ID(7) + 6],
              &gPlttBufferFaded[BG_PLTT_ID(7) + 6], PLTT_SIZEOF(1));
  }
}
...
void DrawMainBattleBackground(void) {
  if (gBattleTypeFlags &
    ...
    LZDecompressVram(gBattleTerrainTilemap_Building,
    ...
    LoadCompressedPalette(gBattleTerrainPalette_Groudon, BG_PLTT_ID(2),
                          3 * PLTT_SIZE_4BPP);
    ...
    LoadCompressedPalette(gBattleTerrainPalette_Kyogre, BG_PLTT_ID(2),
                          3 * PLTT_SIZE_4BPP);
  } else if (gBattleTypeFlags & BATTLE_TYPE_RAYQUAZA) {
    LZDecompressVram(gBattleTerrainTiles_Rayquaza, (void *)(BG_CHAR_ADDR(2)));
    LZDecompressVram(gBattleTerrainTilemap_Rayquaza,
                     (void *)(BG_SCREEN_ADDR(26)));
    LoadCompressedPalette(gBattleTerrainPalette_Rayquaza, BG_PLTT_ID(2),
                          3 * PLTT_SIZE_4BPP);
  } else {
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER) {
      u8 trainerClass = gTrainers[gTrainerBattleOpponent_A].trainerClass;
      if (trainerClass == TRAINER_CLASS_LEADER) {
        LZDecompressVram(gBattleTerrainTiles_Building,
                         (void *)(BG_CHAR_ADDR(2)));
        LZDecompressVram(gBattleTerrainTilemap_Building,
                         (void *)(BG_SCREEN_ADDR(26)));
        LoadCompressedPalette(gBattleTerrainPalette_BuildingLeader,
                              BG_PLTT_ID(2), 3 * PLTT_SIZE_4BPP);
        return;
      } else if (trainerClass == TRAINER_CLASS_CHAMPION) {
        LZDecompressVram(gBattleTerrainTiles_Stadium,
                         (void *)(BG_CHAR_ADDR(2)));
        LZDecompressVram(gBattleTerrainTilemap_Stadium,
                         (void *)(BG_SCREEN_ADDR(26)));
        LoadCompressedPalette(gBattleTerrainPalette_StadiumWallace,
                              BG_PLTT_ID(2), 3 * PLTT_SIZE_4BPP);
        return;
    ...
    case MAP_BATTLE_SCENE_NORMAL:
      LZDecompressVram(sBattleTerrainTable[gBattleTerrain].tileset,
                       (void *)(BG_CHAR_ADDR(2)));
      LZDecompressVram(sBattleTerrainTable[gBattleTerrain].tilemap,
                       (void *)(BG_SCREEN_ADDR(26)));
      LoadCompressedPalette(sBattleTerrainTable[gBattleTerrain].palette,
                            BG_PLTT_ID(2), 3 * PLTT_SIZE_4BPP);
      break;
    case MAP_BATTLE_SCENE_GYM:
      ...
      LZDecompressVram(gBattleTerrainTilemap_Building,
                       (void *)(BG_SCREEN_ADDR(26)));
      LoadCompressedPalette(gBattleTerrainPalette_BuildingGym, BG_PLTT_ID(2),
                            3 * PLTT_SIZE_4BPP);
      break;
    case MAP_BATTLE_SCENE_MAGMA:
      ...
      LZDecompressVram(gBattleTerrainTilemap_Stadium,
                       (void *)(BG_SCREEN_ADDR(26)));
      LoadCompressedPalette(gBattleTerrainPalette_StadiumMagma, BG_PLTT_ID(2),
                            3 * PLTT_SIZE_4BPP);
      break;
    case MAP_BATTLE_SCENE_AQUA:
      ...
      LZDecompressVram(gBattleTerrainTilemap_Stadium,
                       (void *)(BG_SCREEN_ADDR(26)));
      LoadCompressedPalette(gBattleTerrainPalette_StadiumAqua, BG_PLTT_ID(2),
                            3 * PLTT_SIZE_4BPP);
      break;
    case MAP_BATTLE_SCENE_SIDNEY:
      ...
      LZDecompressVram(gBattleTerrainTilemap_Stadium,
                       (void *)(BG_SCREEN_ADDR(26)));
      LoadCompressedPalette(gBattleTerrainPalette_StadiumSidney, BG_PLTT_ID(2),
                            3 * PLTT_SIZE_4BPP);
      break;
    case MAP_BATTLE_SCENE_PHOEBE:
      ...
      LZDecompressVram(gBattleTerrainTilemap_Stadium,
                       (void *)(BG_SCREEN_ADDR(26)));
      LoadCompressedPalette(gBattleTerrainPalette_StadiumPhoebe, BG_PLTT_ID(2),
                            3 * PLTT_SIZE_4BPP);
      break;
    case MAP_BATTLE_SCENE_GLACIA:
      ...
      LZDecompressVram(gBattleTerrainTilemap_Stadium,
                       (void *)(BG_SCREEN_ADDR(26)));
      LoadCompressedPalette(gBattleTerrainPalette_StadiumGlacia, BG_PLTT_ID(2),
                            3 * PLTT_SIZE_4BPP);
      break;
    case MAP_BATTLE_SCENE_DRAKE:
      ...
      LZDecompressVram(gBattleTerrainTilemap_Stadium,
                       (void *)(BG_SCREEN_ADDR(26)));
      LoadCompressedPalette(gBattleTerrainPalette_StadiumDrake, BG_PLTT_ID(2),
                            3 * PLTT_SIZE_4BPP);
      break;
...
static void DrawLinkBattleParticipantPokeballs(u8 taskId, u8 multiplayerId,
                                               u8 bgId, u8 destX, u8 destY) {
  s32 i;
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_MULTI) {
    ...
    CopyToBgTilemapBufferRect_ChangePalette(bgId, tiles, destX, destY, 3, 1,
                                            0x11);
    ...
    CopyToBgTilemapBufferRect_ChangePalette(bgId, tiles, destX, destY, 6, 1,
                                            0x11);
    CopyBgTilemapBufferToVram(bgId);
  }
}
...
void InitLinkBattleVsScreen(u8 taskId) {
  ...
  case 1:
    ...
    gPlttBufferUnfaded[OBJ_PLTT_ID(palId) + 15] =
        gPlttBufferFaded[OBJ_PLTT_ID(palId) + 15] = RGB_WHITE;
...
void DrawBattleEntryBackground(void) {
  if (gBattleTypeFlags & BATTLE_TYPE_LINK) {
    ...
    SetGpuReg(REG_OFFSET_WININ, WININ_WIN0_BG1 | WININ_WIN0_BG2 |
    ...
    LoadCompressedSpriteSheetUsingHeap(&sVsLettersSpriteSheet);
             ...
             (BATTLE_TYPE_FRONTIER | BATTLE_TYPE_LINK |
              BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_EREADER_TRAINER)) {
    if (!(gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER) ||
        gPartnerTrainerId == TRAINER_STEVEN_PARTNER) {
      LZDecompressVram(gBattleTerrainAnimTiles_Building,
                       (void *)(BG_CHAR_ADDR(1)));
      LZDecompressVram(gBattleTerrainAnimTilemap_Building,
                       (void *)(BG_SCREEN_ADDR(28)));
    } else {
      // Set up bg for the multi battle intro where both teams slide in facing
      // the screen. Note Steven's multi battle (which has a dedicated back pic)
      // is excluded above.
    ...
    LZDecompressVram(gBattleTerrainAnimTilemap_Cave,
                     (void *)(BG_SCREEN_ADDR(28)));
  } else if (gBattleTypeFlags & BATTLE_TYPE_KYOGRE) {
    LZDecompressVram(gBattleTerrainAnimTiles_Underwater,
                     (void *)(BG_CHAR_ADDR(1)));
    LZDecompressVram(gBattleTerrainAnimTilemap_Underwater,
                     (void *)(BG_SCREEN_ADDR(28)));
  } else if (gBattleTypeFlags & BATTLE_TYPE_RAYQUAZA) {
    LZDecompressVram(gBattleTerrainAnimTiles_Rayquaza,
                     (void *)(BG_CHAR_ADDR(1)));
    LZDecompressVram(gBattleTerrainAnimTilemap_Rayquaza,
                     (void *)(BG_SCREEN_ADDR(28)));
  } else {
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER) {
      u8 trainerClass = gTrainers[gTrainerBattleOpponent_A].trainerClass;
    ...
    if (GetCurrentMapBattleScene() == MAP_BATTLE_SCENE_NORMAL) {
      LZDecompressVram(sBattleTerrainTable[gBattleTerrain].entryTileset,
                       (void *)(BG_CHAR_ADDR(1)));
      LZDecompressVram(sBattleTerrainTable[gBattleTerrain].entryTilemap,
                       (void *)(BG_SCREEN_ADDR(28)));
    } else {
      LZDecompressVram(gBattleTerrainAnimTiles_Building,
                       (void *)(BG_CHAR_ADDR(1)));
  ...
  case 4:
    if (gBattleTypeFlags &
      ...
      if (gGameVersion == VERSION_RUBY)
        LZDecompressVram(gBattleTerrainTilemap_Cave,
                         (void *)(BG_SCREEN_ADDR(26)));
      else
        LZDecompressVram(gBattleTerrainTilemap_Water,
                         (void *)(BG_SCREEN_ADDR(26)));
    } else {
      if (gBattleTypeFlags & BATTLE_TYPE_TRAINER) {
        u8 trainerClass = gTrainers[gTrainerBattleOpponent_A].trainerClass;
  ...
  case 5:
    if (gBattleTypeFlags &
      ...
      case MAP_BATTLE_SCENE_AQUA:
        ...
        break;
      case MAP_BATTLE_SCENE_SIDNEY:
        LoadCompressedPalette(gBattleTerrainPalette_StadiumSidney,
                              BG_PLTT_ID(2), 3 * PLTT_SIZE_4BPP);
        break;
      case MAP_BATTLE_SCENE_PHOEBE:
        LoadCompressedPalette(gBattleTerrainPalette_StadiumPhoebe,
                              BG_PLTT_ID(2), 3 * PLTT_SIZE_4BPP);
        break;
      case MAP_BATTLE_SCENE_GLACIA:
        LoadCompressedPalette(gBattleTerrainPalette_StadiumGlacia,
                              BG_PLTT_ID(2), 3 * PLTT_SIZE_4BPP);
        break;
      case MAP_BATTLE_SCENE_DRAKE:
        LoadCompressedPalette(gBattleTerrainPalette_StadiumDrake, BG_PLTT_ID(2),
