...
// This allows them to have idle animations. Cursors prior to this are simply
// printed text.
...
static u8 ListMenuInitInternal(struct ListMenuTemplate *listMenuTemplate,
                               u16 scrollOffset, u16 selectedRow);
static bool8 ListMenuChangeSelection(struct ListMenu *list,
                                     bool8 updateCursorAndCallCallback,
                                     u8 count, bool8 movingDown);
static void ListMenuPrintEntries(struct ListMenu *list, u16 startIndex,
                                 u16 yOffset, u16 count);
static void ListMenuDrawCursor(struct ListMenu *list);
static void ListMenuCallSelectionChangedCallback(struct ListMenu *list,
                                                 u8 onInit);
...
static u8 ListMenuAddCursorObjectInternal(struct CursorStruct *cursor,
                                          u32 cursorObjId);
static void ListMenuUpdateCursorObject(u8 taskId, u16 x, u16 y,
                                       u32 cursorObjId);
...
static const union AnimCmd *const sSpriteAnimTable_ScrollArrowIndicator[] = {
    sSpriteAnim_ScrollArrowIndicator0, sSpriteAnim_ScrollArrowIndicator1,
    sSpriteAnim_ScrollArrowIndicator2, sSpriteAnim_ScrollArrowIndicator3};
...
static const union AnimCmd sSpriteAnim_RedArrowCursor[] = {ANIMCMD_FRAME(0, 30),
                                                           ANIMCMD_END};
...
static const u16 sRedInterface_Pal[] = INCBIN_U16(
    "graphics/interface/red.gbapal"); // Shared by all of the below gfx
static const u32 sScrollIndicator_Gfx[] =
    INCBIN_U32("graphics/interface/scroll_indicator.4bpp.lz");
static const u32 sOutlineCursor_Gfx[] =
    INCBIN_U32("graphics/interface/outline_cursor.4bpp.lz");
static const u32 sArrowCursor_Gfx[] =
    INCBIN_U32("graphics/interface/arrow_cursor.4bpp.lz");
...
s32 DoMysteryGiftListMenu(const struct WindowTemplate *windowTemplate,
                          ...
                          u8 drawMode, u16 tileNum, u16 palOffset) {
  ...
  default:
    ...
    switch (drawMode) {
    case 2:
      LoadUserWindowBorderGfx(sMysteryGiftLinkMenu.windowId, tileNum,
                              palOffset);
    case 1:
      DrawTextBorderOuter(sMysteryGiftLinkMenu.windowId, tileNum,
                          palOffset / 16);
      break;
    }
...
u8 ListMenuInit(struct ListMenuTemplate *listMenuTemplate, u16 scrollOffset,
                u16 selectedRow) {
...
u8 ListMenuInitInRect(struct ListMenuTemplate *listMenuTemplate,
                      struct ListMenuWindowRect *rect, u16 scrollOffset,
                      u16 selectedRow) {
  s32 i;

  u8 taskId = ListMenuInitInternal(listMenuTemplate, scrollOffset, selectedRow);
  for (i = 0; rect[i].palNum != 0xFF; i++) {
    PutWindowRectTilemapOverridePalette(listMenuTemplate->windowId, rect[i].x,
                                        rect[i].y, rect[i].width,
...
void DestroyListMenuTask(u8 listTaskId, u16 *scrollOffset, u16 *selectedRow) {
  ...
  if (list->taskId != TASK_NONE)
    ListMenuRemoveCursorObject(list->taskId,
                               list->template.cursorKind - CURSOR_OBJECT_START);
...
void RedrawListMenu(u8 listTaskId) {
  ...
  FillWindowPixelBuffer(list->template.windowId,
...
// unused
void ChangeListMenuPals(u8 listTaskId, u8 cursorPal, u8 fillValue,
                        u8 cursorShadowPal) {
  struct ListMenu *list = (void *)gTasks[listTaskId].data;
...
// unused
s32 ListMenuTestInput(struct ListMenuTemplate *template, u32 scrollOffset,
                      u32 selectedRow, u16 keys, u16 *newScrollOffset,
                      u16 *newSelectedRow) {
...
void ListMenuGetScrollAndRow(u8 listTaskId, u16 *scrollOffset,
                             u16 *selectedRow) {
  struct ListMenu *list = (void *)gTasks[listTaskId].data;
...
static void ListMenuPrint(struct ListMenu *list, const u8 *str, u8 x, u8 y) {
  ...
  if (gListMenuOverride.enabled) {
    ...
    AddTextPrinterParameterized4(
        list->template.windowId, gListMenuOverride.fontId, x, y,
        gListMenuOverride.lettersSpacing, 0, colors, TEXT_SKIP_DRAW, str);
    ...
    AddTextPrinterParameterized4(list->template.windowId, list->template.fontId,
                                 x, y, list->template.lettersSpacing, 0, colors,
                                 TEXT_SKIP_DRAW, str);
  }
}
...
static void ListMenuPrintEntries(struct ListMenu *list, u16 startIndex,
  ...
  for (i = 0; i < count; i++) {
    ...
    if (list->template.itemPrintFunc != NULL)
      list->template.itemPrintFunc(list->template.windowId,
                                   list->template.items[startIndex].id, y);
...
static void ListMenuDrawCursor(struct ListMenu *list) {
  ...
  case CURSOR_RED_OUTLINE:
    if (list->taskId == TASK_NONE)
      list->taskId = ListMenuAddCursorObject(list, CURSOR_RED_OUTLINE -
                                                       CURSOR_OBJECT_START);
    ListMenuUpdateCursorObject(
        list->taskId,
        GetWindowAttribute(list->template.windowId, WINDOW_TILEMAP_LEFT) * 8 -
            1,
        GetWindowAttribute(list->template.windowId, WINDOW_TILEMAP_TOP) * 8 +
            y - 1,
  ...
  case CURSOR_RED_ARROW:
    if (list->taskId == TASK_NONE)
      list->taskId =
          ListMenuAddCursorObject(list, CURSOR_RED_ARROW - CURSOR_OBJECT_START);
    ListMenuUpdateCursorObject(
        list->taskId,
        GetWindowAttribute(list->template.windowId, WINDOW_TILEMAP_LEFT) * 8 +
            x,
...
static u8 ListMenuAddCursorObject(struct ListMenu *list, u32 cursorObjId) {
  ...
  cursor.rowWidth =
      GetWindowAttribute(list->template.windowId, WINDOW_WIDTH) * 8 + 2;
  cursor.rowHeight =
      GetFontAttribute(list->template.fontId, FONTATTR_MAX_LETTER_HEIGHT) + 2;
...
static void ListMenuErasePrintedCursor(struct ListMenu *list, u16 selectedRow) {
  ...
  if (cursorKind == CURSOR_BLACK_ARROW) {
    ...
    FillWindowPixelRect(
        list->template.windowId, PIXEL_FILL(list->template.fillValue),
        list->template.cursor_X,
        selectedRow * yMultiplier + list->template.upText_Y, width, height);
  }
}

static u8 ListMenuUpdateSelectedRowIndexAndScrollOffset(struct ListMenu *list,
                                                        bool8 movingDown) {
  ...
  if (!movingDown) {
    if (list->template.maxShowed == 1)
      ...
      newRow =
          list->template.maxShowed -
          ((list->template.maxShowed / 2) + (list->template.maxShowed % 2)) - 1;

    if (scrollOffset == 0) {
      while (selectedRow != 0) {
        selectedRow--;
        if (list->template.items[scrollOffset + selectedRow].id !=
            LIST_HEADER) {
          list->selectedRow = selectedRow;
          return 1;
        }
    ...
    if (list->template.maxShowed == 1)
      ...
      newRow =
          ((list->template.maxShowed / 2) + (list->template.maxShowed % 2));
...
static void ListMenuScroll(struct ListMenu *list, u8 count, bool8 movingDown) {
  if (count >= list->template.maxShowed) {
    ...
    if (!movingDown) {
      ...
      ScrollWindow(list->template.windowId, 1, count * yMultiplier,
                   PIXEL_FILL(list->template.fillValue));
      ...
      height =
          (GetWindowAttribute(list->template.windowId, WINDOW_HEIGHT) * 8) - y;
      FillWindowPixelRect(list->template.windowId,
                          PIXEL_FILL(list->template.fillValue), 0, y, width,
                          height);
    } else {
      u16 width;

      ScrollWindow(list->template.windowId, 0, count * yMultiplier,
                   PIXEL_FILL(list->template.fillValue));
      ListMenuPrintEntries(
          list, list->scrollOffset + (list->template.maxShowed - count),
          list->template.maxShowed - count, count);

      width = GetWindowAttribute(list->template.windowId, WINDOW_WIDTH) * 8;
      FillWindowPixelRect(list->template.windowId,
                          PIXEL_FILL(list->template.fillValue), 0, 0, width,
  ...
  for (i = 0; i < count; i++) {
    do {
      ...
      if (ret != 2)
        break;
      cursorCount++;
    } while (list->template.items[list->scrollOffset + list->selectedRow].id ==
             LIST_HEADER);
  }
...
static void ListMenuCallSelectionChangedCallback(struct ListMenu *list,
  ...
  if (list->template.moveCursorFunc != NULL)
    list->template.moveCursorFunc(
        list->template.items[list->scrollOffset + list->selectedRow].id, onInit,
        list);
}
...
void ListMenuDefaultCursorMoveFunc(s32 itemIndex, bool8 onInit,
                                   struct ListMenu *list) {
  if (!onInit)
    PlaySE(SE_SELECT);
}
...
static u8 AddScrollIndicatorArrowObject(u8 arrowDir, u8 x, u8 y, u16 tileTag,
                                        u16 palTag) {
  u8 spriteId;
  ...
  gSprites[spriteId].tMultiplier =
      sScrollIndicatorTemplates[arrowDir].multiplier;
...
u8 AddScrollIndicatorArrowPair(const struct ScrollArrowsTemplate *arrowInfo,
                               u16 *scrollOffset) {
  ...
  if (arrowInfo->palTag == TAG_NONE) {
    LoadPalette(sRedInterface_Pal, OBJ_PLTT_ID(arrowInfo->palNum),
                PLTT_SIZE_4BPP);
  } else {
  ...
  data->topSpriteId = AddScrollIndicatorArrowObject(
      arrowInfo->firstArrowType, arrowInfo->firstX, arrowInfo->firstY,
      arrowInfo->tileTag, arrowInfo->palTag);
  data->bottomSpriteId = AddScrollIndicatorArrowObject(
      arrowInfo->secondArrowType, arrowInfo->secondX, arrowInfo->secondY,
      arrowInfo->tileTag, arrowInfo->palTag);
...
u8 AddScrollIndicatorArrowPairParameterized(u32 arrowType, s32 commonPos,
                                            s32 firstPos, s32 secondPos,
                                            s32 fullyDownThreshold, s32 tileTag,
                                            s32 palTag, u16 *scrollOffset) {
...
void ListMenuSetUpRedOutlineCursorSpriteOamTable(u16 rowWidth, u16 rowHeight,
...
static u8 ListMenuAddRedOutlineCursorObject(struct CursorStruct *cursor) {
  ...
  data->subspriteTable.subspriteCount = ListMenuGetRedOutlineCursorSpriteCount(
      cursor->rowWidth, cursor->rowHeight);
  ...
  data->spriteId =
      CreateSprite(&spriteTemplate, cursor->left + 120, cursor->top + 120, 0);
