...
u8 rfu_LMAN_initializeManager(void (*LMAN_callback_p)(u8, u8),
                              void (*MSC_callback_p)(u16)) {
...
void rfu_LMAN_powerDownRFU(void) { lman.state = LMAN_STATE_STOP_MODE; }

u8 rfu_LMAN_establishConnection(u8 parent_child, u16 connect_period,
                                u16 name_accept_period,
...
u8 rfu_LMAN_CHILD_connectParent(u16 parentId, u16 connect_period) {
  ...
  if (gRfuLinkStatus->findParentCount == 0 ||
      i == gRfuLinkStatus->findParentCount) {
    lman.param[0] = 3;
    rfu_LMAN_occureCallback(LMAN_MSG_LMAN_API_ERROR_RETURN, 1);
...
static void UNUSED
rfu_LMAN_PARENT_stopWaitLinkRecoveryAndDisconnect(u8 bm_targetSlot) {
  u8 i;
...
static bool8 rfu_LMAN_linkWatcher(u16 REQ_commandID) {
  ...
  rfu_REQBN_watchLink(REQ_commandID, &bm_linkLossSlot, &reason,
                      &bm_linkRecoverySlot);
  if (bm_linkLossSlot) {
  ...
  if (gRfuLinkStatus->parentChild == MODE_PARENT) {
    if (bm_linkRecoverySlot) {
      for (i = 0; i < RFU_CHILD_MAX; i++) {
        if ((lman.linkRecoveryTimer.active >> i) & 1 &&
            (bm_linkRecoverySlot >> i) & 1) {
          lman.linkRecoveryTimer.count[i] = 0;
        }
      }
...
void rfu_LMAN_manager_entity(u32 rand) {
  ...
  while (1) {
    if (lman.state != LMAN_STATE_READY) {
      ...
      case LMAN_STATE_CONFIG_GAME_DATA:
        rfu_REQ_configGameData(
            lman.init_param->mboot_flag, lman.init_param->serialNo,
    ...
    if (lman.state == LMAN_STATE_END_LINK_RECOVERY ||
...
static void rfu_LMAN_REQ_callback(u16 reqCommandId, u16 reqResult) {
  ...
  if (lman.active != 0) {
    ...
    case ID_CP_POLL_REQ:
      if (reqResult == 0 &&
          !rfu_getConnectParentStatus(&status, &lman.child_slot) && !status) {
        lman.state = LMAN_STATE_END_CONNECT_PARENT;
      }
      if (lman.connect_period && --lman.connect_period == 0) {
        lman.state = LMAN_STATE_END_CONNECT_PARENT;
    ...
    case ID_CP_END_REQ:
      if (reqResult == 0 &&
          !rfu_getConnectParentStatus(&status, &lman.child_slot)) {
        if (!status) {
          lman.state = LMAN_STATE_MS_CHANGE;
    ...
    case ID_CPR_START_REQ:
      if (reqResult == 0) {
        ...
        for (lman.child_slot = 0; lman.child_slot < RFU_CHILD_MAX;
             lman.child_slot++) {
      ...
      if (reqResult == 0) {
        if (lman.next_state == LMAN_STATE_BACK_STATE) {
          ...
          if (status) {
            lman.state = lman.next_state = LMAN_STATE_READY;
            rfu_LMAN_managerChangeAgbClockMaster();
            rfu_LMAN_disconnect(gRfuLinkStatus->connSlotFlag |
  ...
  if (lman.state == LMAN_STATE_START_SEARCH_CHILD ||
    ...
    if (lman.reserveDisconnectSlot_flag) {
      ...
      if (gRfuLinkStatus->sendSlotUNIFlag) {
        if (((lman.parentAck_flag & lman.acceptSlot_flag) !=
             lman.acceptSlot_flag)) {
          flags = 0;
        }
      }
      if (flags) {
    ...
    if (lman.nameAcceptTimer.active == 0 &&
        lman.state == LMAN_STATE_WAIT_RECV_CHILD_NAME) {
      if (lman.pcswitch_flag == 0) {
        lman.state = lman.next_state = LMAN_STATE_READY;
...
static void rfu_LMAN_CHILD_checkSendChildName(void) {
  ...
  if (lman.state == LMAN_STATE_SEND_CHILD_NAME) {
    if (--lman.nameAcceptTimer.count[lman.child_slot] == 0 ||
        gRfuSlotStatusNI[lman.child_slot]->send.state ==
  ...
  if (lman.state == LMAN_STATE_WAIT_CHANGE_CLOCK_MASTER) {
    ...
    if (lman.childClockSlave_flag == RFU_CHILD_CLOCK_SLAVE_OFF) {
      ...
      rfu_LMAN_disconnect(gRfuLinkStatus->connSlotFlag |
                          gRfuLinkStatus->linkLossSlotFlag);
      lman.param[0] = 0;
      rfu_LMAN_occureCallback(LMAN_MSG_CHILD_NAME_SEND_FAILED_AND_DISCONNECTED,
                              1);
    }
  }
}
...
static u8 rfu_LMAN_CHILD_checkEnableParentCandidate(void) {
  ...
  for (i = 0; i < gRfuLinkStatus->findParentCount; i++) {
    for (serialNo = lman.acceptable_serialNo_list; *serialNo != 0xFFFF;
         serialNo++) {
  ...
  if (gRfuLinkStatus->sendSlotNIFlag) {
    for (i = 0; i < RFU_CHILD_MAX; i++) {
      if (gRfuSlotStatusNI[i]->send.state & SLOT_BUSY_FLAG &&
        ...
        rfu_changeSendTarget(TYPE_NI, i,
                             gRfuSlotStatusNI[i]->send.bmSlot &
                                 ~bm_disconnectedSlot);
      }
    }
  }
        ...
        if (gRfuSlotStatusNI[i]->send.state & SLOT_BUSY_FLAG) {
          ...
          for (j = 0; j < RFU_CHILD_MAX; j++) {
            if ((gRfuSlotStatusNI[i]->send.bmSlot >> j) & 1 &&
                gRfuSlotStatusNI[j]->send.failCounter >
                    lman.NI_failCounter_limit) {
              flags |= (1 << j);
            }
            if (flags) {
              rfu_changeSendTarget(TYPE_NI, i,
...
u8 rfu_LMAN_setLinkRecovery(u8 enable_flag, u16 recovery_period) {
  u16 imeBak;
  if (lman.linkRecovery_enable && enable_flag == 0 &&
      lman.linkRecoveryTimer.active) {
