...
void PickLotteryCornerTicket(void) {
  ...
  for (i = 0; i < PARTY_SIZE; i++) {
    ...
    } else // Pokémon are always arranged from populated spots first to
           // unpopulated, so the moment a NONE species is found, that's the end
           // of the list.
      break;
  }
  ...
  for (i = 0; i < TOTAL_BOXES_COUNT; i++) {
    for (j = 0; j < IN_BOX_COUNT; j++) {
      if (GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], MON_DATA_SPECIES) !=
        ...
        u32 otId =
            GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], MON_DATA_OT_ID);
        u8 numMatchingDigits = GetMatchingDigits(gSpecialVar_Result, otId);
  ...
  if (gSpecialVar_0x8004 != 0) {
    ...
    if (box == TOTAL_BOXES_COUNT) {
      ...
      GetBoxMonData(&gPokemonStoragePtr->boxes[box][slot], MON_DATA_NICKNAME,
                    gStringVar1);
    }
...
// lottery numbers go from 0 to 99999, not 65535 (0xFFFF). interestingly enough,
// the function that calls GetLotteryNumber shifts to u16, so it cant be
// anything above 65535 anyway.
...
// interestingly, this may have been the original lottery number set function,
// but GF tried to change it to 32-bit later but didnt finish changing all calls
// as one GetLotteryNumber still shifts to u16.
void SetLotteryNumber16_Unused(u16 lotteryNum) { SetLotteryNumber(lotteryNum); }
