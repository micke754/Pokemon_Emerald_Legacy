...
static u8 EReader_Transfer(struct EReaderData *eReader) {
  ...
  eReader->status =
      EReaderHandleTransfer(TRUE, eReader->size, eReader->data, NULL);

  if ((eReader->status & EREADER_XFER_MASK) == 0 &&
      eReader->status & EREADER_CHECKSUM_OK_MASK)
...
static u32 TryReceiveCard(u8 *state, u16 *timer) {
  if (*state >= RECV_STATE_EXCHANGE && *state <= RECV_STATE_WAIT_DISCONNECT &&
...
static void ResetTimer(u16 *timer) { *timer = 0; }
...
static void Task_EReader(u8 taskId) {
  ...
  case ER_STATE_CONNECTING:
    ...
    // XXX: This (u32 *) cast is discarding the const qualifier from
    // gMultiBootProgram_EReader_Start
    EReader_Load(&gEReaderData,
  ...
  case ER_STATE_VALIDATE_CARD:
    data->status = ValidateTrainerHillData(
        (struct EReaderTrainerHillSet *)gDecompressionBuffer);
