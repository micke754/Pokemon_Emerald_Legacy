...
// There are two types of indicators for the area screen to show where a Pokémon
// can occur:
// - Area glows, which highlight any of the maps in MAP_GROUP_TOWNS_AND_ROUTES
// that have the species.
//   These are a tilemap with colored rectangular areas that blends in and out.
//   The positions of the rectangles is determined by the positions of the
//   matching MAPSEC values on the region map layout.
// - Area markers, which highlight any of the maps in MAP_GROUP_DUNGEONS or
// MAP_GROUP_SPECIAL_AREA that
//   have the species. These are circular sprites that flash twice. The
//   positions of the sprites is determined by the data for the corresponding
//   MAPSEC in gRegionMapEntries.

// Only maps in the following map groups have their encounters considered for
// the area screen
...
static const u32 sAreaGlow_Pal[] =
    INCBIN_U32("graphics/pokedex/area_glow.gbapal");
static const u32 sAreaGlow_Gfx[] =
    INCBIN_U32("graphics/pokedex/area_glow.4bpp.lz");
...
static const u16 sFeebasData[][3] = {
    {SPECIES_FEEBAS, MAP_GROUP(ROUTE119), MAP_NUM(ROUTE119)}, {NUM_SPECIES}};
...
static const u16 sAreaMarkerPalette[] =
    INCBIN_U16("graphics/pokedex/area_marker.gbapal");
static const u8 sAreaMarkerTiles[] =
    INCBIN_U8("graphics/pokedex/area_marker.4bpp");
...
static bool8 DrawAreaGlow(void) {
  ...
  case 2:
    ...
    LoadBgTilemap(2, sPokedexAreaScreen->areaGlowTilemap,
                  sizeof(sPokedexAreaScreen->areaGlowTilemap), 0);
    break;
  case 3:
    if (!FreeTempTileDataBuffersIfPossible()) {
      CpuCopy32(sAreaGlow_Pal, &gPlttBufferUnfaded[BG_PLTT_ID(GLOW_PALETTE)],
                sizeof(sAreaGlow_Pal));
  ...
  if (species != roamer->species) {
    ...
    for (i = 0; gWildMonHeaders[i].mapGroup != MAP_GROUP(UNDEFINED); i++) {
      if (MapHasSpecies(&gWildMonHeaders[i], species)) {
        ...
        case MAP_GROUP_DUNGEONS:
        case MAP_GROUP_SPECIAL_AREA:
          SetSpecialMapHasMon(gWildMonHeaders[i].mapGroup,
...
static void SetAreaHasMon(u16 mapGroup, u16 mapNum) {
  if (sPokedexAreaScreen->numOverworldAreas < MAX_AREA_HIGHLIGHTS) {
    sPokedexAreaScreen
        ->overworldAreasWithMons[sPokedexAreaScreen->numOverworldAreas]
        .mapGroup = mapGroup;
    sPokedexAreaScreen
        ->overworldAreasWithMons[sPokedexAreaScreen->numOverworldAreas]
        .mapNum = mapNum;
...
static void SetSpecialMapHasMon(u16 mapGroup, u16 mapNum) {
  ...
  if (sPokedexAreaScreen->numSpecialAreas < MAX_AREA_MARKERS) {
    ...
    if (regionMapSectionId < MAPSEC_NONE) {
      ...
      // Don't highlight the area if it's an undiscovered landmark (e.g. Sky
      // Pillar)
      for (i = 0; sLandmarkData[i][0] != MAPSEC_NONE; i++) {
        if (regionMapSectionId == sLandmarkData[i][0] &&
            !FlagGet(sLandmarkData[i][1]))
          return;
      }

      // Check if this special area is already being tracked
      for (i = 0; i < sPokedexAreaScreen->numSpecialAreas; i++) {
        if (sPokedexAreaScreen->specialAreaRegionMapSectionIds[i] ==
            regionMapSectionId)
          break;
      }
      ...
      if (i == sPokedexAreaScreen->numSpecialAreas) {
        // New special area
        sPokedexAreaScreen->specialAreaRegionMapSectionIds[i] =
            regionMapSectionId;
...
static u16 GetRegionMapSectionId(u8 mapGroup, u8 mapNum) {
  return Overworld_GetMapHeaderByGroupAndId(mapGroup, mapNum)
      ->regionMapSectionId;
}

static bool8 MapHasSpecies(const struct WildPokemonHeader *info, u16 species) {
  // If this is a header for Altering Cave, skip it if it's not the current
  // Altering Cave encounter set
  if (GetRegionMapSectionId(info->mapGroup, info->mapNum) ==
      MAPSEC_ALTERING_CAVE) {
    sPokedexAreaScreen->alteringCaveCounter++;
    if (sPokedexAreaScreen->alteringCaveCounter !=
        sPokedexAreaScreen->alteringCaveId + 1)
      return FALSE;
  }
...
static void BuildAreaGlowTilemap(void) {
  ...
  // For each area with this species, scan the region map layout and find any
  // locations that have a matching mapsec. Add a "full glow" indicator for
  // these matching spaces.
  ...
  // Scan the tilemap. For every "full glow" indicator added above, fill in its
  // edges and corners.
  j = 0;
  for (y = 0; y < AREA_SCREEN_HEIGHT; y++) {
    for (x = 0; x < AREA_SCREEN_WIDTH; x++) {
      if (sPokedexAreaScreen->areaGlowTilemap[j] == GLOW_FULL) {
        // The "tile != GLOW_FULL" check is pointless in all of these
        // conditionals, since there's no harm in OR'ing 0xFFFF with anything
        // else.
        ...
        if (x != AREA_SCREEN_WIDTH - 1 &&
            sPokedexAreaScreen->areaGlowTilemap[j + 1] != GLOW_FULL)
          sPokedexAreaScreen->areaGlowTilemap[j + 1] |= GLOW_EDGE_R;
        if (y != 0 &&
            sPokedexAreaScreen->areaGlowTilemap[j - AREA_SCREEN_WIDTH] !=
                GLOW_FULL)
          sPokedexAreaScreen->areaGlowTilemap[j - AREA_SCREEN_WIDTH] |=
              GLOW_EDGE_T;
  ...
  // Scan the tilemap again. Replace the "full tile" indicators with the actual
  // tile id, and remove corner flags when they're overlapped by an edge.
  for (i = 0; i < ARRAY_COUNT(sPokedexAreaScreen->areaGlowTilemap); i++) {
...
static void StartAreaGlow(void) {
  if (sPokedexAreaScreen->numSpecialAreas &&
      sPokedexAreaScreen->numOverworldAreas == 0)
...
static void Task_ShowPokedexAreaScreen(u8 taskId) {
  ...
  case 10:
    SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_TGT1_BG0 | BLDCNT_EFFECT_BLEND |
                                     BLDCNT_TGT2_BG0 | BLDCNT_TGT2_ALL);
    StartAreaGlow();
    ShowBg(2);
  ...
  case 1:
    if (JOY_NEW(B_BUTTON)) {
      ...
      if (!GetSetPokedexFlag(
              SpeciesToNationalPokedexNum(sPokedexAreaScreen->species),
              FLAG_GET_CAUGHT)) {
        PlaySE(SE_FAILURE);
        return;
      }
...
// Creates the circular sprites to highlight special areas (like caves) where a
...
static void LoadAreaUnknownGraphics(void) {
  ...
  LZ77UnCompWram(gPokedexAreaScreenAreaUnknown_Gfx,
                 sPokedexAreaScreen->areaUnknownGraphicsBuffer);
...
static void CreateAreaUnknownSprites(void) {
  ...
  if (sPokedexAreaScreen->numOverworldAreas ||
      sPokedexAreaScreen->numSpecialAreas) {
    // The current species is present on the map, don't create any "Area
    // Unknown" sprites
    for (i = 0; i < ARRAY_COUNT(sPokedexAreaScreen->areaUnknownSprites); i++)
      sPokedexAreaScreen->areaUnknownSprites[i] = NULL;
  } else {
    // The current species is absent on the map, try to create "Area Unknown"
    // sprites
    for (i = 0; i < ARRAY_COUNT(sPokedexAreaScreen->areaUnknownSprites); i++) {
      u8 spriteId =
          CreateSprite(&sAreaUnknownSpriteTemplate, i * 32 + 160, 140, 0);
      if (spriteId != MAX_SPRITES) {
