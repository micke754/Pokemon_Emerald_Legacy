...
static void Task_FreezeSelectedObjectAndPlayer(u8 taskId) {
  ...
  if (!task->tObjectFrozen &&
      !gObjectEvents[gSelectedObjectEvent].singleMovementActive) {
...
static void Task_FreezeObjectAndPlayer(u8 taskId) {
  ...
  if (!task->tObjectFrozen &&
      !gObjectEvents[objectEventId].singleMovementActive) {
...
// Freeze all objects immediately except the player and the approaching
// trainers. The approaching trainers and player are frozen once their movement
// is finished
