...
static void LinkOpponentBufferRunCommand(void) {
  if (gBattleControllerExecFlags & gBitTable[gActiveBattler]) {
    if (gBattleBufferA[gActiveBattler][0] <
        ARRAY_COUNT(sLinkOpponentBufferCommands))
...
static void Intro_TryShinyAnimShowHealthbox(void) {
  ...
  if (bgmRestored) {
    if (gSprites[gBattleControllerData[gActiveBattler]].callback ==
      ...
      gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].bgmRestored =
          FALSE;
      gBattleSpritesDataPtr->healthBoxesData[gActiveBattler]
...
static void LinkOpponentHandleGetMonData(void) {
  ...
  if (gBattleBufferA[gActiveBattler][2] == 0) {
    size +=
        CopyLinkOpponentMonData(gBattlerPartyIndexes[gActiveBattler], monData);
  } else {
...
static void LinkOpponentHandleDrawTrainerPic(void) {
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_MULTI) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_BATTLE_TOWER) {
      ...
      if ((gLinkPlayers[GetBattlerMultiplayerId(gActiveBattler)].version &
           0xFF) == VERSION_FIRE_RED ||
          (gLinkPlayers[GetBattlerMultiplayerId(gActiveBattler)].version &
           0xFF) == VERSION_LEAF_GREEN) {
      ...
      } else if ((gLinkPlayers[GetBattlerMultiplayerId(gActiveBattler)]
                      .version &
                  0xFF) == VERSION_RUBY ||
                 (gLinkPlayers[GetBattlerMultiplayerId(gActiveBattler)]
                      .version &
                  0xFF) == VERSION_SAPPHIRE) {
        if (gLinkPlayers[GetBattlerMultiplayerId(gActiveBattler)].gender !=
            MALE)
    ...
    if (gTrainerBattleOpponent_A == TRAINER_UNION_ROOM) {
    ...
    } else if ((gLinkPlayers[GetMultiplayerId() ^ BIT_SIDE].version & 0xFF) ==
                   VERSION_FIRE_RED ||
      ...
      trainerPicId = PlayerGenderToFrontTrainerPicId(
          gLinkPlayers[GetMultiplayerId() ^ BIT_SIDE].gender);
    }
  }
...
static void LinkOpponentHandlePause(void) { LinkOpponentBufferExecCompleted(); }
...
static void LinkOpponentHandleCmd23(void) { LinkOpponentBufferExecCompleted(); }
...
static void LinkOpponentHandleCmd32(void) { LinkOpponentBufferExecCompleted(); }
