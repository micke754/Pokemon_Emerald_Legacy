...
#define GIFT_RIBBON_ROW                                                        \
  (1 + (FIRST_GIFT_RIBBON / RIBBONS_PER_ROW)) // Gift ribbons start on a new row
                                              // after the normal ribbons.
...
struct {
  ...
  u8 numRibbons; // Never read. The contest ribbons have 4 (1 for each rank),
                 // the rest are just 1 ribbon
  u8 ribbonId;
  bool8 isGiftRibbon;
} static const sRibbonData[] = {{1, 1, CHAMPION_RIBBON, FALSE},
...
static const u16 sRibbonIcons1_Pal[] =
    INCBIN_U16("graphics/pokenav/ribbons/icons1.gbapal");
static const u16 sRibbonIcons2_Pal[] =
    INCBIN_U16("graphics/pokenav/ribbons/icons2.gbapal");
static const u16 sRibbonIcons3_Pal[] =
    INCBIN_U16("graphics/pokenav/ribbons/icons3.gbapal");
static const u16 sRibbonIcons4_Pal[] =
    INCBIN_U16("graphics/pokenav/ribbons/icons4.gbapal");
static const u16 sRibbonIcons5_Pal[] =
    INCBIN_U16("graphics/pokenav/ribbons/icons5.gbapal");
static const u16 sMonInfo_Pal[] = INCBIN_U16(
    "graphics/pokenav/ribbons/mon_info.gbapal"); // palette for Pokémon's
                                                 // name/gender/level text
static const u32 sRibbonIconsSmall_Gfx[] =
    INCBIN_U32("graphics/pokenav/ribbons/icons.4bpp.lz");
static const u32 sRibbonIconsBig_Gfx[] =
    INCBIN_U32("graphics/pokenav/ribbons/icons_big.4bpp.lz");

static const struct BgTemplate sBgTemplates[] = {{.bg = 1,
                                                  .charBaseIndex = 3,
                                                  .mapBaseIndex = 0x07,
                                                  .screenSize = 0,
                                                  .paletteMode = 0,
...
bool32 PokenavCallback_Init_RibbonsSummaryMenu(void) {
  struct Pokenav_RibbonsSummaryList *list =
      AllocSubstruct(POKENAV_SUBSTRUCT_RIBBONS_SUMMARY_LIST,
...
u32 GetRibbonsSummaryMenuCallback(void) {
  struct Pokenav_RibbonsSummaryList *list =
      GetSubstructPtr(POKENAV_SUBSTRUCT_RIBBONS_SUMMARY_LIST);
...
static u32 RibbonsSummaryHandleInput(struct Pokenav_RibbonsSummaryList *list) {
  ...
  if (JOY_REPEAT(DPAD_DOWN) &&
      list->monList->currIndex < list->monList->listCount - 1) {
    list->monList->currIndex++;
    list->selectedPos = 0;
    GetMonRibbons(list);
    return RIBBONS_SUMMARY_FUNC_SWITCH_MONS;
...
static u32
ReturnToRibbonsListFromSummary(struct Pokenav_RibbonsSummaryList *list) {
...
static bool32 TrySelectRibbonDown(struct Pokenav_RibbonsSummaryList *list) {
  ...
  if (list->numGiftRibbons != 0) {
    // In/beyond last of row of normal ribbons and gift ribbons present, move
    // down to gift ribbon row
...
static void GetMonSpeciesPersonalityOtId(u16 *species, u32 *personality,
                                         u32 *otId) {
  struct Pokenav_RibbonsSummaryList *list =
      GetSubstructPtr(POKENAV_SUBSTRUCT_RIBBONS_SUMMARY_LIST);
  struct PokenavMonList *mons = list->monList;
...
static u32 GetCurrMonRibbonCount(void) {
  ...
  if (monInfo->boxId == TOTAL_BOXES_COUNT)
    ...
    return GetBoxMonDataAt(monInfo->boxId, monInfo->monId,
...
static void GetMonRibbons(struct Pokenav_RibbonsSummaryList *list) {
  ...
  if (monInfo->boxId == TOTAL_BOXES_COUNT)
    ...
    ribbonFlags =
        GetBoxMonDataAt(monInfo->boxId, monInfo->monId, MON_DATA_RIBBONS);

  list->numNormalRibbons = 0;
  list->numGiftRibbons = 0;
  for (i = 0; i < ARRAY_COUNT(sRibbonData); i++) {
    // For all non-contest ribbons, numRibbons will be 1 if they have it, 0 if
    // they don't For contest ribbons, numRibbons will be 0-4
    ...
    if (!sRibbonData[i].isGiftRibbon) {
      ...
      for (j = 0; j < numRibbons; j++)
        list->giftRibbonIds[list->numGiftRibbons++] =
            sRibbonData[i].ribbonId + j;
    }
  ...
  if (list->numNormalRibbons != 0) {
    list->normalRibbonLastRowStart =
        ((list->numNormalRibbons - 1) / RIBBONS_PER_ROW) * RIBBONS_PER_ROW;
    list->selectedPos = 0;
  } else {
...
bool32 OpenRibbonsSummaryMenu(void) {
  struct Pokenav_RibbonsSummaryMenu *menu =
      AllocSubstruct(POKENAV_SUBSTRUCT_RIBBONS_SUMMARY_MENU,
...
void CreateRibbonsSummaryLoopedTask(s32 id) {
  struct Pokenav_RibbonsSummaryMenu *menu =
      GetSubstructPtr(POKENAV_SUBSTRUCT_RIBBONS_SUMMARY_MENU);
  menu->loopedTaskId =
      CreateLoopedTask(sRibbonsSummaryMenuLoopTaskFuncs[id], 1);
  menu->callback = GetCurrentLoopedTaskActive;
}
...
static u32 LoopedTask_OpenRibbonsSummaryMenu(s32 state) {
  ...
  case 0:
    ...
    CopyPaletteIntoBufferUnfaded(gPokenavRibbonsSummaryBg_Pal, BG_PLTT_ID(1),
                                 PLTT_SIZE_4BPP);
  ...
  case 1:
    if (!FreeTempTileDataBuffersIfPossible()) {
      ...
      CopyPaletteIntoBufferUnfaded(sRibbonIcons1_Pal, BG_PLTT_ID(2),
                                   5 * PLTT_SIZE_4BPP);
      CopyPaletteIntoBufferUnfaded(sMonInfo_Pal, BG_PLTT_ID(10),
                                   sizeof(sMonInfo_Pal));
...
static void
PrintCurrentMonRibbonCount(struct Pokenav_RibbonsSummaryMenu *menu) {
  u8 color[] = {TEXT_COLOR_RED, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_LIGHT_GRAY};

  ConvertIntToDecimalStringN(gStringVar1, GetCurrMonRibbonCount(),
  ...
  AddTextPrinterParameterized3(menu->ribbonCountWindowId, FONT_NORMAL, 0, 1,
                               color, TEXT_SKIP_DRAW, gStringVar4);
  CopyWindowToVram(menu->ribbonCountWindowId, COPYWIN_GFX);
}

static void
PrintRibbonNameAndDescription(struct Pokenav_RibbonsSummaryMenu *menu) {
  s32 i;
  ...
  if (ribbonId < FIRST_GIFT_RIBBON) {
    ...
    for (i = 0; i < 2; i++)
      AddTextPrinterParameterized3(menu->ribbonCountWindowId, FONT_NORMAL, 0,
                                   (i * 16) + 1, color, TEXT_SKIP_DRAW,
...
static void
AddRibbonSummaryMonNameWindow(struct Pokenav_RibbonsSummaryMenu *menu) {
...
static const u8 sMaleIconString[] =
    _("{COLOR_HIGHLIGHT_SHADOW}{LIGHT_RED}{WHITE}{GREEN}♂{COLOR_HIGHLIGHT_"
      "SHADOW}{DARK_GRAY}{WHITE}{LIGHT_GRAY}");
static const u8 sFemaleIconString[] =
    _("{COLOR_HIGHLIGHT_SHADOW}{LIGHT_GREEN}{WHITE}{BLUE}♀{COLOR_HIGHLIGHT_"
      "SHADOW}{DARK_GRAY}{WHITE}{LIGHT_GRAY}");
static const u8 sGenderlessIconString[] = _("{UNK_SPACER}");

static void
PrintRibbbonsSummaryMonInfo(struct Pokenav_RibbonsSummaryMenu *menu) {
  const u8 *genderTxt;
  u8 *txtPtr;
  u8 level, gender;
  ...
  AddTextPrinterParameterized(windowId, FONT_NORMAL, gStringVar3, 0, 1,
                              TEXT_SKIP_DRAW, NULL);
  switch (gender) {
  case MON_MALE:
  ...
  *(txtPtr++) = CHAR_LV_2;
  ...
  AddTextPrinterParameterized(windowId, FONT_NORMAL, gStringVar1, 60, 1,
                              TEXT_SKIP_DRAW, NULL);
...
static void PrintRibbonsMonListIndex(struct Pokenav_RibbonsSummaryMenu *menu) {
  ...
  *(txtPtr++) = CHAR_SLASH;
  ...
  AddTextPrinterParameterized(menu->listIdxWindowId, FONT_NORMAL, gStringVar1,
                              x, 1, TEXT_SKIP_DRAW, NULL);
  CopyWindowToVram(menu->listIdxWindowId, COPYWIN_GFX);
}

static void
ResetSpritesAndDrawMonFrontPic(struct Pokenav_RibbonsSummaryMenu *menu) {
  u16 species;
...
// x is given as either MON_SPRITE_X_ON or MON_SPRITE_X_OFF (but ignored and
// MON_SPRITE_X_ON is used)
static u16 DrawRibbonsMonFrontPic(s32 x, s32 y) {
  ...
  spriteId = CreateMonPicSprite_HandleDeoxys(species, otId, personality, TRUE,
                                             MON_SPRITE_X_ON, MON_SPRITE_Y, 15,
                                             TAG_NONE);
...
static void SlideMonSpriteOff(struct Pokenav_RibbonsSummaryMenu *menu) {
  StartMonSpriteSlide(&gSprites[menu->monSpriteId], MON_SPRITE_X_ON,
                      MON_SPRITE_X_OFF, 6);
}
...
static void SlideMonSpriteOn(struct Pokenav_RibbonsSummaryMenu *menu) {
  ...
  // Slide on
  StartMonSpriteSlide(&gSprites[menu->monSpriteId], MON_SPRITE_X_OFF,
                      MON_SPRITE_X_ON, 6);
}
...
static void StartMonSpriteSlide(struct Sprite *sprite, s32 startX, s32 destX,
                                s32 time) {
...
static void DrawAllRibbonsSmall(struct Pokenav_RibbonsSummaryMenu *menu) {
  ...
  for (sRibbonDraw_Current = 0; sRibbonDraw_Current < sRibbonDraw_Total;
       sRibbonDraw_Current++)
  ...
  for (sRibbonDraw_Current = 0; sRibbonDraw_Current < sRibbonDraw_Total;
       sRibbonDraw_Current++)
    DrawRibbonSmall(sRibbonDraw_Current + GIFT_RIBBON_START_POS,
                    *(ribbonIds++));
...
// Below correspond to a ribbon icon in ribbons/icons.png and
// ribbons/icons_big.png; 0 at top, 11 at bottom
enum {
  RIBBONGFX_CHAMPION,
...
struct {
  ...
  u16 palNumOffset;
    ...
    [CHAMPION_RIBBON] = {RIBBONGFX_CHAMPION,
                         TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_1)},
    [COOL_RIBBON_NORMAL] = {RIBBONGFX_CONTEST_NORMAL,
                            TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_1)},
    [COOL_RIBBON_SUPER] = {RIBBONGFX_CONTEST_SUPER,
                           TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_1)},
    [COOL_RIBBON_HYPER] = {RIBBONGFX_CONTEST_HYPER,
                           TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_1)},
    [COOL_RIBBON_MASTER] = {RIBBONGFX_CONTEST_MASTER,
                            TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_1)},
    [BEAUTY_RIBBON_NORMAL] = {RIBBONGFX_CONTEST_NORMAL,
                              TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_2)},
    [BEAUTY_RIBBON_SUPER] = {RIBBONGFX_CONTEST_SUPER,
                             TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_2)},
    [BEAUTY_RIBBON_HYPER] = {RIBBONGFX_CONTEST_HYPER,
                             TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_2)},
    [BEAUTY_RIBBON_MASTER] = {RIBBONGFX_CONTEST_MASTER,
                              TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_2)},
    [CUTE_RIBBON_NORMAL] = {RIBBONGFX_CONTEST_NORMAL,
                            TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_3)},
    [CUTE_RIBBON_SUPER] = {RIBBONGFX_CONTEST_SUPER,
                           TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_3)},
    [CUTE_RIBBON_HYPER] = {RIBBONGFX_CONTEST_HYPER,
                           TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_3)},
    [CUTE_RIBBON_MASTER] = {RIBBONGFX_CONTEST_MASTER,
                            TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_3)},
    [SMART_RIBBON_NORMAL] = {RIBBONGFX_CONTEST_NORMAL,
                             TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_4)},
    [SMART_RIBBON_SUPER] = {RIBBONGFX_CONTEST_SUPER,
                            TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_4)},
    [SMART_RIBBON_HYPER] = {RIBBONGFX_CONTEST_HYPER,
                            TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_4)},
    [SMART_RIBBON_MASTER] = {RIBBONGFX_CONTEST_MASTER,
                             TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_4)},
    [TOUGH_RIBBON_NORMAL] = {RIBBONGFX_CONTEST_NORMAL,
                             TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_5)},
    [TOUGH_RIBBON_SUPER] = {RIBBONGFX_CONTEST_SUPER,
                            TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_5)},
    [TOUGH_RIBBON_HYPER] = {RIBBONGFX_CONTEST_HYPER,
                            TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_5)},
    [TOUGH_RIBBON_MASTER] = {RIBBONGFX_CONTEST_MASTER,
                             TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_5)},
    [WINNING_RIBBON] = {RIBBONGFX_WINNING,
                        TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_1)},
    [VICTORY_RIBBON] = {RIBBONGFX_VICTORY,
                        TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_1)},
    ...
    [NATIONAL_RIBBON] = {RIBBONGFX_GIFT_2,
                         TO_PAL_OFFSET(PALTAG_RIBBON_ICONS_5)},
...
static const union AffineAnimCmd sAffineAnim_RibbonIconBig_ZoomIn[] = {
    AFFINEANIMCMD_FRAME(128, 128, 0, 0), AFFINEANIMCMD_FRAME(32, 32, 0, 4),
    AFFINEANIMCMD_END};

static const union AffineAnimCmd sAffineAnim_RibbonIconBig_ZoomOut[] = {
    AFFINEANIMCMD_FRAME(256, 256, 0, 0), AFFINEANIMCMD_FRAME(-32, -32, 0, 4),
    AFFINEANIMCMD_END};
...
// Create dummy sprite to be used for the zoomed in version of the selected
// ribbon
...
static void
UpdateAndZoomInSelectedRibbon(struct Pokenav_RibbonsSummaryMenu *menu) {
  u32 ribbonId;
  ...
  menu->bigRibbonSprite->oam.tileNum =
      (sRibbonGfxData[ribbonId].tileNumOffset * 16) +
      GetSpriteTileStartByTag(GFXTAG_RIBBON_ICONS_BIG);
  menu->bigRibbonSprite->oam.paletteNum = IndexOfSpritePaletteTag(
