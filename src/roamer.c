...
// Note: There are two potential softlocks that can occur with this table if its
// maps are
//       changed in particular ways. They can be avoided by ensuring the
//       following:
//       - There must be at least 2 location sets that start with a different
//       map,
//         i.e. every location set cannot start with the same map. This is
//         because of the while loop in RoamerMoveToOtherLocationSet.
//       - Each location set must have at least 3 unique maps. This is because
//       of
//         the while loop in RoamerMove. In this loop the first map in the set
//         is ignored, and an additional map is ignored if the roamer was there
//         recently.
//       - Additionally, while not a softlock, it's worth noting that if for any
//         map in the location table there is not a location set that starts
//         with that map then the roamer will be significantly less likely to
//         move away from that map when it lands there.
static const u8 sRoamerLocations[][6] = {
    {MAP_NUM(ROUTE110), MAP_NUM(ROUTE111), MAP_NUM(ROUTE117), MAP_NUM(ROUTE118),
     MAP_NUM(ROUTE134), ___},
    {MAP_NUM(ROUTE111), MAP_NUM(ROUTE110), MAP_NUM(ROUTE117), MAP_NUM(ROUTE118),
     ___, ___},
    {MAP_NUM(ROUTE117), MAP_NUM(ROUTE111), MAP_NUM(ROUTE110), MAP_NUM(ROUTE118),
     ___, ___},
    {MAP_NUM(ROUTE118), MAP_NUM(ROUTE117), MAP_NUM(ROUTE110), MAP_NUM(ROUTE111),
     MAP_NUM(ROUTE119), MAP_NUM(ROUTE123)},
    ...
    {MAP_NUM(ROUTE121), MAP_NUM(ROUTE120), MAP_NUM(ROUTE122), MAP_NUM(ROUTE123),
     ___, ___},
    ...
    {MAP_NUM(ROUTE124), MAP_NUM(ROUTE121), MAP_NUM(ROUTE125), MAP_NUM(ROUTE126),
     ___, ___},
    ...
    {MAP_NUM(ROUTE127), MAP_NUM(ROUTE125), MAP_NUM(ROUTE126), MAP_NUM(ROUTE128),
     ___, ___},
...
static void CreateInitialRoamerMon(bool16 createLatios) {
  ...
  CreateMon(&gEnemyParty[0], ROAMER->species, 40, USE_RANDOM_IVS, FALSE, 0,
            OT_ID_PLAYER_ID, 0);
  ROAMER->level = 40;
  ROAMER->status = 0;
  ROAMER->active = TRUE;
...
// gSpecialVar_0x8004 here corresponds to the options in the multichoice
// MULTI_TV_LATI (0 for 'Red', 1 for 'Blue')
    ...
    while (locSet < NUM_LOCATION_SETS) {
      ...
      if (sRoamerLocation[MAP_NUM] == sRoamerLocations[locSet][0]) {
        ...
        while (1) {
          ...
          mapNum =
              sRoamerLocations[locSet]
                              [(Random() % (NUM_LOCATIONS_PER_SET - 1)) + 1];
...
void CreateRoamerMonInstance(void) {
  ...
  ZeroEnemyPartyMons();
  CreateMonWithIVsPersonality(mon, ROAMER->species, ROAMER->level, ROAMER->ivs,
                              ROAMER->personality);
...
bool8 TryStartRoamerEncounter(void) {
  if (IsRoamerAt(gSaveBlock1Ptr->location.mapGroup,
                 gSaveBlock1Ptr->location.mapNum) == TRUE &&
      (Random() % 4) == 0) {
...
void SetRoamerInactive(void) { ROAMER->active = FALSE; }
