 ...
 *
 * There are two save slots for saving the player's game data. We alternate
 * between them each time the game is saved, so that if the current save slot is
 * corrupt, we can load the previous one. We also rotate the sectors in each
 * save slot so that the same data is not always being written to the same
 * sector. This might be done to reduce wear on the flash memory, but I'm not
 * sure, since all 14 sectors get written anyway.
 *
...
#define SAVEBLOCK_CHUNK(structure, chunkNum)                                   \
  {chunkNum * SECTOR_DATA_SIZE,                                                \
   sizeof(structure) >= chunkNum * SECTOR_DATA_SIZE                            \
       ? min(sizeof(structure) - chunkNum * SECTOR_DATA_SIZE,                  \
             SECTOR_DATA_SIZE)                                                 \
       : 0}
...
STATIC_ASSERT(sizeof(struct SaveBlock1) <=
                  SECTOR_DATA_SIZE * (SECTOR_ID_SAVEBLOCK1_END -
                                      SECTOR_ID_SAVEBLOCK1_START + 1),
              SaveBlock1FreeSpace);
STATIC_ASSERT(sizeof(struct PokemonStorage) <=
                  SECTOR_DATA_SIZE * (SECTOR_ID_PKMN_STORAGE_END -
                                      SECTOR_ID_PKMN_STORAGE_START + 1),
...
u32 gSaveCounter;
struct SaveSector
    *gReadWriteSector; // Pointer to a buffer for reading/writing a sector
...
EWRAM_DATA struct SaveSector gSaveDataBuffer = {
    0}; // Buffer used for reading/writing sectors
...
static u8 WriteSaveSectorOrSlot(u16 sectorId,
  ...
  if (sectorId != FULL_SAVE_SLOT) {
    ...
    gLastKnownGoodSector =
        gLastWrittenSector; // backup the current written sector before
                            // attempting to write.
...
static u8 HandleWriteSector(u16 sectorId,
                            const struct SaveSectorLocation *locations) {
  u16 i;
  u16 sector;
  u8 *data;
...
static u8 HandleWriteSectorNBytes(u8 sectorId, u8 *data, u16 size) {
  ...
  sector->id = CalculateChecksum(
      data, size); // though this appears to be incorrect, it might be some
                   // sector checksum instead of a whole save checksum and only
                   // appears to be relevent to HOF data, if used.
...
static u32
RestoreSaveBackupVarsAndIncrement(const struct SaveSectorLocation *locations) {
  gReadWriteSector = &gSaveDataBuffer;
...
static u8
HandleWriteIncrementalSector(u16 numSectors,
                             const struct SaveSectorLocation *locations) {
  u8 status;
...
static u8
HandleReplaceSectorAndVerify(u16 sectorId,
                             const struct SaveSectorLocation *locations) {
...
// Similar to HandleWriteSector, but fully erases the sector first, and skips
// writing the first signature byte
static u8 HandleReplaceSector(u16 sectorId,
                              const struct SaveSectorLocation *locations) {
  u16 i;
  u16 sector;
  u8 *data;
  ...
  if (status == SAVE_STATUS_ERROR) {
    ...
    // The byte of signature that is skipped is instead written by
    // WriteSectorSignatureByte or WriteSectorSignatureByte_NoOffset
    for (i = 0; i < SECTOR_SIZE - (SECTOR_SIGNATURE_OFFSET + 1); i++) {
      if (ProgramFlashByte(
              sector, SECTOR_SIGNATURE_OFFSET + 1 + i,
              ((u8 *)gReadWriteSector)[SECTOR_SIGNATURE_OFFSET + 1 + i])) {
        status = SAVE_STATUS_ERROR;
        break;
      }
    }
...
static u8
WriteSectorSignatureByte_NoOffset(u16 sectorId,
                                  const struct SaveSectorLocation *locations) {
  // Adjust sector id for current save slot
  // This first line lacking -1 is the only difference from
  // WriteSectorSignatureByte
  u16 sector = sectorId + gLastWrittenSector;
  sector %= NUM_SECTORS_PER_SLOT;
  sector += NUM_SECTORS_PER_SLOT * (gSaveCounter % NUM_SAVE_SLOTS);

  // Write just the first byte of the signature field, which was skipped by
  // HandleReplaceSector
  if (ProgramFlashByte(sector, SECTOR_SIGNATURE_OFFSET,
                       SECTOR_SIGNATURE & 0xFF)) {
    // Sector is damaged, so enable the bit in gDamagedSaveSectors and restore
    // the last written sector and save counter.
    SetDamagedSectorBits(ENABLE, sector);
    gLastWrittenSector = gLastKnownGoodSector;
    gSaveCounter = gLastSaveCounter;
    return SAVE_STATUS_ERROR;
...
static u8 CopySectorSignatureByte(u16 sectorId,
  ...
  if (ProgramFlashByte(sector, SECTOR_SIGNATURE_OFFSET,
                       ((u8 *)gReadWriteSector)[SECTOR_SIGNATURE_OFFSET])) {
    // Sector is damaged, so enable the bit in gDamagedSaveSectors and restore
    // the last written sector and save counter.
    SetDamagedSectorBits(ENABLE, sector);
    gLastWrittenSector = gLastKnownGoodSector;
...
static u8 CopySaveSlotData(u16 sectorId, struct SaveSectorLocation *locations) {
  ...
  for (i = 0; i < NUM_SECTORS_PER_SLOT; i++) {
    ...
    // Only copy data for sectors whose signature and checksum fields are
    // correct
...
static u8 GetSaveValidStatus(const struct SaveSectorLocation *locations) {
  ...
  for (i = 0; i < NUM_SECTORS_PER_SLOT; i++) {
    ...
    if (gReadWriteSector->signature == SECTOR_SIGNATURE) {
      signatureValid = TRUE;
      checksum = CalculateChecksum(gReadWriteSector->data,
...
static void UpdateSaveAddresses(void) {
  ...
  gRamSaveSectorLocations[i].data =
      (void *)(gSaveBlock2Ptr) + sSaveSlotLayout[i].offset;
  gRamSaveSectorLocations[i].size = sSaveSlotLayout[i].size;

  for (i = SECTOR_ID_SAVEBLOCK1_START; i <= SECTOR_ID_SAVEBLOCK1_END; i++) {
    gRamSaveSectorLocations[i].data =
        (void *)(gSaveBlock1Ptr) + sSaveSlotLayout[i].offset;
    gRamSaveSectorLocations[i].size = sSaveSlotLayout[i].size;
  }

  for (; i <= SECTOR_ID_PKMN_STORAGE_END;
       i++) // setting i to SECTOR_ID_PKMN_STORAGE_START does not match
  {
    gRamSaveSectorLocations[i].data =
        (void *)(gPokemonStoragePtr) + sSaveSlotLayout[i].offset;
    gRamSaveSectorLocations[i].size = sSaveSlotLayout[i].size;
  }
}
...
u8 HandleSavingData(u8 saveType) {
  ...
  case SAVE_HALL_OF_FAME_ERASE_BEFORE:
    // Unused. Erases the special save sectors (HOF, Trainer Hill, Recorded
    // Battle) before overwriting HOF.
  ...
  case SAVE_HALL_OF_FAME:
    ...
    HandleWriteSectorNBytes(SECTOR_ID_HOF_2, tempAddr + SECTOR_DATA_SIZE,
                            SECTOR_DATA_SIZE);
    break;
  case SAVE_NORMAL:
  default:
...
bool8 LinkFullSave_WriteSector(void) {
  u8 status = HandleWriteIncrementalSector(NUM_SECTORS_PER_SLOT,
                                           gRamSaveSectorLocations);
  if (gDamagedSaveSectors)
    DoSaveFailedScreen(SAVE_NORMAL);

  // In this case "error" either means that an actual error was encountered
  // or that the given max sector has been reached (meaning it has finished
  // successfully). If there was an actual error the save failed screen above
  // will also be shown.
  if (status == SAVE_STATUS_ERROR)
    return TRUE;
  else
...
u8 WriteSaveBlock2(void) {
  ...
  // gIncrementalSectorId will always be 0 below, so this function only saves
  // the first sector (SECTOR_ID_SAVEBLOCK2)
  HandleReplaceSectorAndVerify(gIncrementalSectorId + 1,
                               gRamSaveSectorLocations);
  return FALSE;
}

// Used in conjunction with WriteSaveBlock2 to write both for certain link
// saves. This will be called repeatedly in a task, writing each sector of
// SaveBlock1 incrementally. It returns TRUE when finished.
bool8 WriteSaveBlock1Sector(void) {
  u8 finished = FALSE;
  u16 sectorId =
      ++gIncrementalSectorId; // Because WriteSaveBlock2 will have been called
                              // prior, this will be SECTOR_ID_SAVEBLOCK1_START
...
u8 LoadGameSave(u8 saveType) {
  ...
  default:
    ...
    break;
  case SAVE_HALL_OF_FAME:
    status = TryLoadSaveSector(SECTOR_ID_HOF_1, gDecompressionBuffer,
                               SECTOR_DATA_SIZE);
    if (status == SAVE_STATUS_OK)
      status = TryLoadSaveSector(SECTOR_ID_HOF_2,
                                 &gDecompressionBuffer[SECTOR_DATA_SIZE],
                                 SECTOR_DATA_SIZE);
    break;
  }
...
u32 TryReadSpecialSaveSector(u8 sector, u8 *dst) {
  ...
  savData =
      &gSaveDataBuffer.data[4]; // data[4] to skip past SPECIAL_SECTOR_SENTINEL
  for (; i <= size; i++)
    dst[i] = savData[i];
  return SAVE_STATUS_OK;
}
