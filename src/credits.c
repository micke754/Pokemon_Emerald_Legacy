...
#define tTaskId_BgScenery                                                      \
  data[0]                          // ID for Task_BicycleBgAnimation (created by
...
struct CreditsData {
  u16 monToShow[NUM_MON_SLIDES]; // List of Pokémon species ids that will show
                                 // during the credits
  ...
  u16 numMonToShow; // number of Pokémon to show, always NUM_MON_SLIDES after
                    // determine function
  u16 caughtMonIds[NATIONAL_DEX_COUNT]; // temporary location to hold a
                                        // condensed array of all caught Pokémon
...
static void PrintCreditsText(const u8 *string, u8 y, bool8 isTitle) {
  ...
  x = GetStringCenterAlignXOffsetWithLetterSpacing(FONT_NORMAL, string,
                                                   DISPLAY_WIDTH, 1);
  AddTextPrinterParameterized4(0, FONT_NORMAL, x, y, 1, 0, color,
...
static void Task_LoadShowMons(u8 taskId) {
  ...
  case 0: {
    ...
    LoadPalette(gBirchBagGrass_Pal + 1, BG_PLTT_ID(0) + 1,
                PLTT_SIZEOF(2 * 16 - 1));
  ...
  case 1:
    ...
    gTasks[gTasks[taskId].tTaskId_ShowMons].data[2] =
        gTasks[taskId].tSceneNum; // data[2] never read
...
static void Task_CreditsTheEnd3(u8 taskId) {
  ...
  gTasks[taskId].tDelay = 235; // set this to 215 to actually show "THE END" in
                               // time to the last song beat
...
static void Task_UpdatePage(u8 taskId) {
      ...
      if (gTasks[taskId].tCurrentPage < PAGE_COUNT) {
        ...
        for (i = 0; i < ENTRIES_PER_PAGE; i++)
          PrintCreditsText(
              ...
              5 + i * 16,
              sCreditsEntryPointerTable[gTasks[taskId].tCurrentPage][i]
                  ->isTitle);
  ...
  case 4:
    ...
    if (CheckChangeScene((u8)gTasks[taskId].tCurrentPage,
                         (u8)gTasks[taskId].tMainTaskId)) {
      gTasks[taskId].tState++;
      return;
    }
    gTasks[taskId].tState++;
...
#define PAGE_INTERVAL                                                          \
  (PAGE_COUNT / 9) // 9 scenes (5 bike scenes, 4 Pokémon interludes)
...
static void Task_BikeScene(u8 taskId) {
  ...
  case 0:
    gIntroCredits_MovingSceneryVOffset =
        Sin((gTasks[taskId].tSinIdx >> 1) & 0x7F, 12);
  ...
  case 2:
    if (gTasks[taskId].tSinIdx < 64) {
      gTasks[taskId].tSinIdx++;
      gIntroCredits_MovingSceneryVOffset =
          Sin(gTasks[taskId].tSinIdx & 0x7F, 20);
    } else {
      gTasks[taskId].tState++;
    }
    break;
...
static void Task_CycleSceneryPalette(u8 taskId) {
  ...
  case SCENE_OCEAN_MORNING:
    if (gTasks[taskId].tTimer != TIMER_STOP) {
      if (gTasks[gTasks[gTasks[taskId].tMainTaskId].tTaskId_UpdatePage]
              .tCurrentPage == 2) {
        gTasks[gTasks[gTasks[taskId].tMainTaskId].tTaskId_BikeScene].tState =
            20;
...
static void SetBikeScene(u8 scene, u8 taskId) {
  ...
  case SCENE_OCEAN_MORNING:
    ...
    gTasks[taskId].tTaskId_BgScenery =
        CreateBicycleBgAnimationTask(0, 0x2000, 0x20, 8);
    break;
  case SCENE_OCEAN_SUNSET:
    gSprites[gTasks[taskId].tPlayerSpriteId].invisible = FALSE;
    gSprites[gTasks[taskId].tRivalSpriteId].invisible = FALSE;
  ...
  case SCENE_FOREST_RIVAL_ARRIVE:
    ...
    gSprites[gTasks[taskId].tRivalSpriteId].data[0] = 0;
    gTasks[taskId].tTaskId_BgScenery =
        CreateBicycleBgAnimationTask(1, 0x2000, 0x200, 8);
    break;
  case SCENE_FOREST_CATCH_RIVAL:
    gSprites[gTasks[taskId].tPlayerSpriteId].invisible = FALSE;
    gSprites[gTasks[taskId].tRivalSpriteId].invisible = FALSE;
  ...
  case SCENE_CITY_NIGHT:
    ...
    gSprites[gTasks[taskId].tRivalSpriteId].data[0] = 0;
    gTasks[taskId].tTaskId_BgScenery =
        CreateBicycleBgAnimationTask(2, 0x2000, 0x200, 8);
    break;
  }
  ...
  gTasks[gTasks[taskId].tTaskId_BikeScene].data[1] =
      taskId; // data[1] is never read
  gTasks[gTasks[taskId].tTaskId_BikeScene].tPlayer =
      gTasks[taskId].tPlayerSpriteId;
  gTasks[gTasks[taskId].tTaskId_BikeScene].tRival =
      gTasks[taskId].tRivalSpriteId;
...
static void LoadTheEndScreen(u16 tileOffsetLoad, u16 tileOffsetWrite,
                             u16 palOffset) {
  u16 baseTile;
  u16 i;
...
static void DrawLetterMapTiles(const u8 baseTiles[], u8 baseX, u8 baseY,
                               u16 offset, u16 palette) {
  u8 y, x;
  const u16 tileOffset = (palette / 16) << 12;

  for (y = 0; y < 5; y++) {
    for (x = 0; x < 3; x++)
      ((u16 *)(VRAM + offset + (baseY + y) * 64))[baseX + x] =
          tileOffset + GetLetterMapTile(baseTiles[y * 3 + x]);
  }
}
...
static void SpriteCB_CreditsMon(struct Sprite *sprite) {
  ...
  default:
    ...
    sprite->data[2] = 16;
    SetOamMatrix(sprite->sPosition, 0x10000 / sprite->data[2], 0, 0,
                 0x10000 / sprite->data[2]);
...
static u8 CreateCreditsMonSprite(u16 nationalDexNum, s16 x, s16 y,
                                 u16 position) {
  u8 monSpriteId;
  u8 bgSpriteId;

  monSpriteId =
      CreateMonSpriteFromNationalDexNumber(nationalDexNum, x, y, position);
  ...
  bgSpriteId =
      CreateSprite(&sSpriteTemplate_CreditsMonBg, gSprites[monSpriteId].x,
                   gSprites[monSpriteId].y, 1);
...
static void DeterminePokemonToShow(void) {
  u16 starter =
      SpeciesToNationalPokedexNum(GetStarterPokemon(VarGet(VAR_STARTER_MON)));
  u16 page;
  u16 dexNum;
  u16 j;

  // Go through the Pokédex, and anything that has gotten caught we put into our
  // massive array. This basically packs all of the caught Pokémon into the
  // front of the array
  ...
  // Cap the number of Pokémon we care about to NUM_MON_SLIDES, the max we show
  // in the credits scene (-1 for the starter)
  ...
  // Loop through our list of caught Pokémon and select randomly from it to fill
  // the images to show
  j = 0;
  do {
    ...
    if (page != sCreditsData->numCaughtMon) {
      // Instead of looping through and moving everything down, just take from
      // the end. Order doesn't matter after all.
  ...
  // If we don't have enough Pokémon in the dex to fill everything, copy the
  // selected mon into the end of the array, so it loops
  if (sCreditsData->numMonToShow < NUM_MON_SLIDES) {
    ...
    // Check to see if our starter has already appeared in this list, break if
    // it has
    for (dexNum = 0;
         sCreditsData->monToShow[dexNum] != starter && dexNum < NUM_MON_SLIDES;
         dexNum++)
      ;

    // If it has, swap it with the last Pokémon, to ensure our starter is the
    // last image
    if (dexNum < sCreditsData->numMonToShow - 1) {
      sCreditsData->monToShow[dexNum] =
          sCreditsData->monToShow[NUM_MON_SLIDES - 1];
      sCreditsData->monToShow[NUM_MON_SLIDES - 1] = starter;
    } else {
      // Ensure the last Pokémon is our starter
      sCreditsData->monToShow[NUM_MON_SLIDES - 1] = starter;
