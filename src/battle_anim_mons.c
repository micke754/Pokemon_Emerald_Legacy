...
static void
AnimTranslateLinear_WithFollowup_SetCornerVecX(struct Sprite *sprite);
...
// Placeholders for Pokémon sprites to be created for a move animation effect
// (e.g. Role Play / Snatch)
...
static const struct SpriteSheet sSpriteSheets_MoveEffectMons[] = {
    {
        gMiscBlank_Gfx,
        MON_PIC_SIZE,
        TAG_MOVE_EFFECT_MON_1,
    },
    {
        gMiscBlank_Gfx,
        MON_PIC_SIZE,
        TAG_MOVE_EFFECT_MON_2,
    },
};
...
u8 GetBattlerSpriteCoord(u8 battlerId, u8 coordType) {
  ...
  case BATTLER_COORD_X_2:
    retVal =
        sBattlerCoords[IS_DOUBLE_BATTLE()][GetBattlerPosition(battlerId)].x;
    break;
  case BATTLER_COORD_Y:
    retVal =
        sBattlerCoords[IS_DOUBLE_BATTLE()][GetBattlerPosition(battlerId)].y;
    break;
...
u8 GetBattlerYDelta(u8 battlerId, u16 species) {
  ...
  if (GetBattlerSide(battlerId) == B_SIDE_PLAYER || IsContest()) {
    ...
    if (species == SPECIES_UNOWN) {
      spriteInfo = gBattleSpritesDataPtr->battlerData;
      if (!spriteInfo[battlerId].transformSpecies)
        personality = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]],
                                 MON_DATA_PERSONALITY);
      else
        personality = gTransformedPersonalities[battlerId];
      letter = GET_UNOWN_LETTER(personality);
...
u8 GetBattlerSpriteCoord2(u8 battlerId, u8 coordType) {
  ...
  if (coordType == BATTLER_COORD_Y_PIC_OFFSET ||
      coordType == BATTLER_COORD_Y_PIC_OFFSET_DEFAULT) {
    if (IsContest()) {
      if (gContestResources->moveAnim->hasTargetAnim)
        species = gContestResources->moveAnim->targetSpecies;
      else
...
void StoreSpriteCallbackInData6(struct Sprite *sprite,
...
void TranslateSpriteInGrowingCircle(struct Sprite *sprite) {
  if (sprite->sDuration) {
    sprite->x2 = Sin(sprite->sCirclePos,
                     (sprite->sAmplitudeChange >> 8) + sprite->sAmplitude);
    sprite->y2 = Cos(sprite->sCirclePos,
                     (sprite->sAmplitudeChange >> 8) + sprite->sAmplitude);
    sprite->sCirclePos += sprite->sCircleSpeed;
...
// Exact shape depends on arguments. Can move in a figure-8-like pattern, or
// circular, etc.
...
// Sets the initial x offset of the anim sprite depending on the horizontal
// orientation of the two involved mons.
...
void InitSpritePosToAnimTarget(struct Sprite *sprite,
                               bool8 respectMonPicOffsets) {
  // Battle anim sprites are automatically created at the anim target's center,
  // which is why there is no else clause for the "respectMonPicOffsets" check.
...
void InitSpritePosToAnimAttacker(struct Sprite *sprite,
                                 bool8 respectMonPicOffsets) {
  if (!respectMonPicOffsets) {
    ...
    sprite->y =
        GetBattlerSpriteCoord2(gBattleAnimAttacker, BATTLER_COORD_Y_PIC_OFFSET);
  }
  SetAnimSpriteInitialXOffset(sprite, gBattleAnimArgs[0]);
  sprite->y += gBattleAnimArgs[1];
}

u8 GetBattlerSide(u8 battlerId) { return GET_BATTLER_SIDE2(battlerId); }

u8 GetBattlerPosition(u8 battlerId) { return gBattlerPositions[battlerId]; }
...
bool8 IsBattlerSpritePresent(u8 battlerId) {
  if (IsContest()) {
    ...
    if (gBattlerPositions[battlerId] == 0xff) {
      ...
      if (GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]],
                     MON_DATA_HP) != 0)
        return TRUE;
    } else {
      if (GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                     MON_DATA_HP) != 0)
        return TRUE;
    }
  }
...
bool8 IsDoubleBattle(void) { return IS_DOUBLE_BATTLE(); }
...
void AnimLoadCompressedBgTilemapHandleContest(struct BattleAnimBgData *data,
                                              const void *src,
                                              bool32 largeScreen) {
...
static void UNUSED
StartAnimLinearTranslation_SetCornerVecX(struct Sprite *sprite) {
  sprite->data[1] = sprite->x;
  sprite->data[3] = sprite->y;
  InitAnimLinearTranslation(sprite);
...
static bool8 ShouldRotScaleSpeciesBeFlipped(void) {
  if (IsContest()) {
    if (gSprites[GetAnimBattlerSpriteId(ANIM_ATTACKER)].data[2] ==
        SPECIES_UNOWN)
      return FALSE;
    else
      return TRUE;
...
void PrepareBattlerSpriteForRotScale(u8 spriteId, u8 objMode) {
  ...
  CalcCenterToCornerVec(&gSprites[spriteId], gSprites[spriteId].oam.shape,
...
void SetBattlerSpriteYOffsetFromRotation(u8 spriteId) {
  ...
  // The "c" component of the battler sprite matrix contains the sine of the
  // rotation angle divided by some scale amount.
...
void TrySetSpriteRotScale(struct Sprite *sprite, bool8 recalcCenterVector,
                          s16 xScale, s16 yScale, u16 rotation) {
  int i;
  struct ObjAffineSrcData src;
  struct OamMatrix matrix;
...
static u16 ArcTan2_(s16 x, s16 y) { return ArcTan2(x, y); }
...
void SetGrayscaleOrOriginalPalette(u16 paletteNum, bool8 restoreOriginalColor) {
  ...
  if (!restoreOriginalColor) {
    ...
    CpuCopy32(&gPlttBufferUnfaded[paletteOffset],
              &gPlttBufferFaded[paletteOffset], PLTT_SIZE_4BPP);
  }
}

u32 GetBattlePalettesMask(bool8 battleBackground, bool8 attacker, bool8 target,
                          bool8 attackerPartner, bool8 targetPartner,
                          bool8 anim1, bool8 anim2) {
...
u32 GetBattleMonSpritePalettesMask(u8 playerLeft, u8 playerRight,
                                   u8 opponentLeft, u8 opponentRight) {
  u32 selectedPalettes = 0;
  u32 shift;
  ...
  if (IsContest()) {
    ...
    if (playerLeft) {
      if (IsBattlerSpriteVisible(
        ...
        selectedPalettes |=
            1 << (GetBattlerAtPosition(B_POSITION_PLAYER_LEFT) + 16);
      }
    }
    if (playerRight) {
      if (IsBattlerSpriteVisible(
...
u8 GetSpritePalIdxByBattler(u8 battler) { return battler; }
...
// arg 4: duration
// arg 5: lower 8 bits = location on attacking mon, upper 8 bits = location on
// target mon pick to target
void TranslateAnimSpriteToTargetMonLocation(struct Sprite *sprite) {
  ...
  sprite->data[2] =
      ...
      gBattleAnimArgs[2];
  sprite->data[4] =
      GetBattlerSpriteCoord(gBattleAnimTarget, coordType) + gBattleAnimArgs[3];
...
void AnimTravelDiagonally(struct Sprite *sprite) {
  ...
  sprite->data[0] = gBattleAnimArgs[4];
  sprite->data[2] =
      GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X_2) + gBattleAnimArgs[2];
  sprite->data[4] =
      GetBattlerSpriteCoord(battlerId, coordType) + gBattleAnimArgs[3];
  sprite->callback = StartAnimLinearTranslation;
  StoreSpriteCallbackInData6(sprite, DestroyAnimSprite);
}
...
void AnimTask_AlphaFadeIn(u8 taskId) {
  ...
  SetGpuReg(REG_OFFSET_BLDALPHA,
            BLDALPHA_BLEND(gBattleAnimArgs[0], gBattleAnimArgs[1]));
...
void PrepareAffineAnimInTaskData(struct Task *task, u8 spriteId,
                                 const union AffineAnimCmd *affineAnimCmds) {
  task->data[7] = 0;
  task->data[8] = 0;
  task->data[9] = 0;
...
bool8 RunAffineAnimFromTaskData(struct Task *task) {
  sAnimTaskAffineAnim = &((union AffineAnimCmd *)LoadPointerFromVars(
      task->data[13], task->data[14]))[task->data[7]];
  ...
  default:
    ...
    task->data[11] += sAnimTaskAffineAnim->frame.yScale;
    task->data[12] += sAnimTaskAffineAnim->frame.rotation;
    SetSpriteRotScale(task->data[15], task->data[10], task->data[11],
                      task->data[12]);
...
// Sets the sprite's y offset equal to the y displacement caused by another
// sprite matrix's scale in the y dimension.
    ...
    if (gBattlerSpriteIds[i] == spriteId) {
      if (IsContest()) {
        ...
        if (GetBattlerSide(i) == B_SIDE_PLAYER) {
          spriteInfo = gBattleSpritesDataPtr->battlerData;
          if (!spriteInfo[battlerId].transformSpecies)
            species = GetMonData(&gPlayerParty[gBattlerPartyIndexes[i]],
                                 MON_DATA_SPECIES);
          else
            species = spriteInfo[battlerId].transformSpecies;
          ...
          spriteInfo = gBattleSpritesDataPtr->battlerData;
          if (!spriteInfo[battlerId].transformSpecies)
            species = GetMonData(&gEnemyParty[gBattlerPartyIndexes[i]],
                                 MON_DATA_SPECIES);
          else
            species = spriteInfo[battlerId].transformSpecies;
...
void PrepareEruptAnimTaskData(struct Task *task, u8 spriteId, s16 xScaleStart,
                              s16 yScaleStart, s16 xScaleEnd, s16 yScaleEnd,
                              u16 duration) {
...
static void UNUSED SetPriorityForVisibleBattlers(u8 priority) {
  ...
  if (IsBattlerSpriteVisible(BATTLE_PARTNER(gBattleAnimTarget)))
    gSprites[gBattlerSpriteIds[BATTLE_PARTNER(gBattleAnimTarget)]]
        .oam.priority = priority;
  if (IsBattlerSpriteVisible(BATTLE_PARTNER(gBattleAnimAttacker)))
    gSprites[gBattlerSpriteIds[BATTLE_PARTNER(gBattleAnimAttacker)]]
        .oam.priority = priority;
}
...
void InitPrioritiesForVisibleBattlers(void) {
  ...
  for (i = 0; i < gBattlersCount; i++) {
    if (IsBattlerSpriteVisible(i)) {
      gSprites[gBattlerSpriteIds[i]].subpriority =
          GetBattlerSpriteSubpriority(i);
...
// / Snatch)
u8 CreateAdditionalMonSpriteForMoveAnim(u16 species, bool8 isBackpic, u8 id,
                                        s16 x, s16 y, u8 subpriority,
                                        u32 personality, u32 trainerId,
                                        u32 battlerId,
                                        bool32 ignoreDeoxysForm) {
  u8 spriteId;
  u16 sheet = LoadSpriteSheet(&sSpriteSheets_MoveEffectMons[id]);
  u16 palette =
      AllocSpritePalette(sSpriteTemplates_MoveEffectMons[id].paletteTag);
  ...
  if (!isBackpic) {
    LoadCompressedPalette(GetMonSpritePalFromSpeciesAndPersonality(
                              species, trainerId, personality),
                          OBJ_PLTT_ID(palette), PLTT_SIZE_4BPP);
    if (ignoreDeoxysForm == TRUE ||
        ShouldIgnoreDeoxysForm(5, battlerId) == TRUE ||
      ...
      LoadSpecialPokePic_2(&gMonFrontPicTable[species],
                           gMonSpritesGfxPtr->buffer, species, personality,
                           TRUE);
  } else {
    LoadCompressedPalette(GetMonSpritePalFromSpeciesAndPersonality(
                              species, trainerId, personality),
  ...
  RequestDma3Copy(gMonSpritesGfxPtr->buffer,
                  (void *)(OBJ_VRAM0 + (sheet * 0x20)), MON_PIC_SIZE, 1);
  FREE_AND_SET_NULL(gMonSpritesGfxPtr->buffer);

  if (!isBackpic)
    spriteId =
        CreateSprite(&sSpriteTemplates_MoveEffectMons[id], x,
                     y + gMonFrontPicCoords[species].y_offset, subpriority);
  else
    spriteId =
        CreateSprite(&sSpriteTemplates_MoveEffectMons[id], x,
                     y + gMonBackPicCoords[species].y_offset, subpriority);
...
s16 GetBattlerSpriteCoordAttr(u8 battlerId, u8 attr) {
  ...
  if (IsContest()) {
    ...
    if (GetBattlerSide(battlerId) == B_SIDE_PLAYER) {
      ...
      if (!spriteInfo[battlerId].transformSpecies) {
        species = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                             MON_DATA_SPECIES);
        personality = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                                 MON_DATA_PERSONALITY);
      } else {
        species = spriteInfo[battlerId].transformSpecies;
        personality = gTransformedPersonalities[battlerId];
  ...
  case BATTLER_COORD_ATTR_LEFT:
    return GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X_2) -
           (GET_MON_COORDS_WIDTH(coords->size) / 2);
  ...
  case BATTLER_COORD_ATTR_TOP:
    return GetBattlerSpriteCoord(battlerId, BATTLER_COORD_Y_PIC_OFFSET) -
           (GET_MON_COORDS_HEIGHT(coords->size) / 2);
...
void SetAverageBattlerPositions(u8 battlerId, bool8 respectMonPicOffsets,
                                s16 *x, s16 *y) {
...
void AnimTask_AttackerPunchWithTrace(u8 taskId) {
  ...
  task->tMoveSpeed =
      (GetBattlerSide(gBattleAnimAttacker) != B_SIDE_PLAYER) ? -8 : 8;
  task->tState = 0;
  task->tCounter = 0;
