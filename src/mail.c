...
struct MailRead {
  ...
  /*0x0224*/ u8 *(*parserMultiple)(u8 *dest, const u16 *src, u16 length1,
                                   u16 length2);
...
static const u8 sTextColors[] = {TEXT_COLOR_TRANSPARENT, TEXT_DYNAMIC_COLOR_1,
...
static bool8 MailReadBuildGraphics(void) {
  ...
  case 7:
    ...
    break;
  case 8:
    DecompressAndCopyTileDataToVram(1, sMailGraphics[sMailRead->mailType].tiles,
                                    0, 0, 0);
    break;
  case 9:
  ...
  case 10:
    FillBgTilemapBufferRect_Palette0(0, 0, 0, 0, DISPLAY_TILE_WIDTH,
                                     DISPLAY_TILE_HEIGHT);
    FillBgTilemapBufferRect_Palette0(2, 1, 0, 0, DISPLAY_TILE_WIDTH,
                                     DISPLAY_TILE_HEIGHT);
  ...
  case 11:
    ...
    break;
  case 12:
    LoadPalette(GetOverworldTextboxPalettePtr(), BG_PLTT_ID(15),
                PLTT_SIZE_4BPP);
    gPlttBufferUnfaded[BG_PLTT_ID(15) + 10] =
        sMailGraphics[sMailRead->mailType].textColor;
    gPlttBufferFaded[BG_PLTT_ID(15) + 10] =
        sMailGraphics[sMailRead->mailType].textColor;
    gPlttBufferUnfaded[BG_PLTT_ID(15) + 11] =
        sMailGraphics[sMailRead->mailType].textShadow;
    gPlttBufferFaded[BG_PLTT_ID(15) + 11] =
        sMailGraphics[sMailRead->mailType].textShadow;

    LoadPalette(sMailGraphics[sMailRead->mailType].palette, BG_PLTT_ID(0),
                PLTT_SIZE_4BPP);
    gPlttBufferUnfaded[BG_PLTT_ID(0) + 10] =
        sBgColors[gSaveBlock2Ptr->playerGender][0];
    gPlttBufferFaded[BG_PLTT_ID(0) + 10] =
        sBgColors[gSaveBlock2Ptr->playerGender][0];
    gPlttBufferUnfaded[BG_PLTT_ID(0) + 11] =
        sBgColors[gSaveBlock2Ptr->playerGender][1];
    gPlttBufferFaded[BG_PLTT_ID(0) + 11] =
        sBgColors[gSaveBlock2Ptr->playerGender][1];
    break;
  case 13:
  ...
  case 17:
    ...
    case ICON_TYPE_BEAD:
      ...
      sMailRead->monIconSpriteId = CreateMonIconNoPersonality(
          icon, SpriteCallbackDummy, 96, 128, 0, FALSE);
      break;
    case ICON_TYPE_DREAM:
      LoadMonIconPalette(icon);
      sMailRead->monIconSpriteId = CreateMonIconNoPersonality(
          icon, SpriteCallbackDummy, 40, 128, 0, FALSE);
      break;
    }
    break;
  case 18:
...
static void BufferMailText(void) {
  ...
  // Convert the easy chat words to strings line by line and buffer them to
  // message
  numWords = 0;
  for (i = 0; i < sMailRead->layout->numLines; i++) {
    ConvertEasyChatWordsToString(
        sMailRead->message[i], &sMailRead->mail->words[numWords],
  ...
  if (!sMailRead->international) {
    // Never reached
    StringCopy(ptr, gText_FromSpace); // Odd, "From" text is already printed in
                                      // PrintMailText
    sMailRead->signatureWidth = sMailRead->layout->signatureWidth -
                                (StringLength(sMailRead->playerName) * 8 - 96);
  } else {
...
static void PrintMailText(void) {
  ...
  for (i = 0; i < sMailRead->layout->numLines; i++) {
    if (sMailRead->message[i][0] == EOS ||
        sMailRead->message[i][0] == CHAR_SPACE)
      continue;

    AddTextPrinterParameterized3(0, FONT_NORMAL,
                                 sMailRead->layout->lines[i].xOffset +
                                     sMailRead->layout->wordsXPos,
                                 y + sMailRead->layout->wordsYPos, sTextColors,
                                 0, sMailRead->message[i]);
    y += sMailRead->layout->lines[i].height;
  }
  bufptr = StringCopy(signature, gText_FromSpace);
  StringCopy(bufptr, sMailRead->playerName);
  box_x = GetStringCenterAlignXOffset(FONT_NORMAL, signature,
                                      sMailRead->signatureWidth) +
          104;
  box_y = sMailRead->layout->signatureYPos + 88;
  AddTextPrinterParameterized3(0, FONT_NORMAL, box_x, box_y, sTextColors, 0,
                               signature);
