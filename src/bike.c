...
static void (*const sMachBikeTransitions[])(u8) = {
    MachBikeTransition_FaceDirection, // Face vs Turn: Face has no anim while
                                      // Turn does. Turn checks for collision
                                      // because if you turn right as opposed to
                                      // face right, if there is a wall there,
                                      // turn will make a bonk sound effect
                                      // while face will not.
...
static u8 (*const sAcroBikeInputHandlers[])(u8 *, u16, u16) = {
    AcroBikeHandleInputNormal,          AcroBikeHandleInputTurning,
    AcroBikeHandleInputWheelieStanding, AcroBikeHandleInputBunnyHop,
    AcroBikeHandleInputWheelieMoving,   AcroBikeHandleInputSidewaysJump,
    AcroBikeHandleInputTurnJump,
};

// used with bikeFrameCounter from mach bike
static const u16 sMachBikeSpeeds[] = {PLAYER_SPEED_NORMAL, PLAYER_SPEED_FAST,
                                      PLAYER_SPEED_FASTEST};

// this is a list of timers to compare against later, terminated with 0. the
// only timer being compared against is 4 frames in this list.
static const u8 sAcroBikeJumpTimerList[] = {4, 0};

// this is a list of history inputs to do in order to do the check to retrieve a
// jump direction for acro bike. it seems to be an extensible list, so its
// possible that Game Freak may have intended for the Acro Bike to have more
// complex tricks at some point. The final list only has the acro jump.
static const struct BikeHistoryInputInfo sAcroBikeTricksList[] = {
    // the 0xF is a mask performed with each byte of the array in order to
    // perform the check on only the last entry of the history list, otherwise
    // the check wouldn't work as there can be 0xF0 as opposed to 0x0F.
    {DIR_SOUTH, B_BUTTON, 0xF, 0xF, sAcroBikeJumpTimerList,
     sAcroBikeJumpTimerList, DIR_SOUTH},
    {DIR_NORTH, B_BUTTON, 0xF, 0xF, sAcroBikeJumpTimerList,
     sAcroBikeJumpTimerList, DIR_NORTH},
    {DIR_WEST, B_BUTTON, 0xF, 0xF, sAcroBikeJumpTimerList,
     sAcroBikeJumpTimerList, DIR_WEST},
    {DIR_EAST, B_BUTTON, 0xF, 0xF, sAcroBikeJumpTimerList,
     sAcroBikeJumpTimerList, DIR_EAST},
};
...
static u8 GetMachBikeTransition(u8 *dirTraveling) {
  // if the dir updated before this function, get the relevent new direction to
  // check later.
  u8 direction = GetPlayerMovementDirection();

  // is the player standing still?
  if (*dirTraveling == 0) {
    *dirTraveling = direction; // update the direction, since below we either
                               // faced a direction or we started moving.
  ...
  // we need to check if the last traveled direction changed from the new
  // direction as well as ensuring that we dont update the state while the
  // player is moving: see the else check.
  if (*dirTraveling != direction && gPlayerAvatar.runningState != MOVING) {
    ...
    // if you didnt start moving but your dir was different, do a turn direction
    // instead.
    gPlayerAvatar.runningState = TURN_DIRECTION;
    return MACH_TRANS_TURN_DIRECTION;
  } else // the player is either going in the current direction and hasnt
         // changed or their state is currently moving.
  {
    gPlayerAvatar.runningState = MOVING;
...
// the difference between face direction and turn direction is that one changes
// direction while the other does the animation of turning as well as changing
// direction.
...
static void MachBikeTransition_TurnDirection(u8 direction) {
  ...
  if (CanBikeFaceDirOnMetatile(direction,
                               playerObjEvent->currentMetatileBehavior)) {
...
static void MachBikeTransition_TrySpeedUp(u8 direction) {
  ...
  if (CanBikeFaceDirOnMetatile(
          direction, playerObjEvent->currentMetatileBehavior) == FALSE) {
    // we cannot go forward, so either slow down or, if we are stopped, idle
    // face direction.
    ...
    if (collision > 0 && collision < COLLISION_VERTICAL_RAIL) {
      ...
      // we did not hit anything that can slow us down, so perform the
      // advancement callback depending on the bikeFrameCounter and try to
      // increase the mach bike's speed.
...
// the acro bike requires the input handler to be executed before the transition
// can.
static void MovePlayerOnAcroBike(u8 newDirection, u16 newKeys, u16 heldKeys) {
  sAcroBikeTransitions[CheckMovementInputAcroBike(&newDirection, newKeys,
                                                  heldKeys)](newDirection);
}

static u8 CheckMovementInputAcroBike(u8 *newDirection, u16 newKeys,
                                     u16 heldKeys) {
  return sAcroBikeInputHandlers[gPlayerAvatar.acroBikeState](newDirection,
                                                             newKeys, heldKeys);
}

static u8 AcroBikeHandleInputNormal(u8 *newDirection, u16 newKeys,
                                    u16 heldKeys) {
  u8 direction = GetPlayerMovementDirection();
...
static u8 AcroBikeHandleInputTurning(u8 *newDirection, u16 newKeys,
                                     u16 heldKeys) {
  u8 direction;
  ...
  *newDirection = gPlayerAvatar.newDirBackup;
  ...
  if (gPlayerAvatar.bikeFrameCounter >
      6) // ... because it takes 6 frames to advance 1 tile.
  {
    gPlayerAvatar.runningState = TURN_DIRECTION;
  ...
  if (*newDirection == AcroBike_GetJumpDirection()) {
    Bike_SetBikeStill(); // Bike_SetBikeStill sets speed to standing, but the
                         // next line immediately overrides it. could have just
                         // reset acroBikeState to 0 here instead of wasting a
                         // jump.
    ...
    if (*newDirection == GetOppositeDirection(direction)) {
      ...
      gPlayerAvatar.runningState =
          MOVING; // we need to move, set state to moving.
...
static u8 AcroBikeHandleInputWheelieStanding(u8 *newDirection, u16 newKeys,
                                             u16 heldKeys) {
  u8 direction;
  struct ObjectEvent *playerObjEvent;
...
static u8 AcroBikeHandleInputBunnyHop(u8 *newDirection, u16 newKeys,
                                      u16 heldKeys) {
  u8 direction;
  struct ObjectEvent *playerObjEvent;
...
static u8 AcroBikeHandleInputWheelieMoving(u8 *newDirection, u16 newKeys,
                                           u16 heldKeys) {
  u8 direction;
  struct ObjectEvent *playerObjEvent;
  ...
  playerObjEvent = &gObjectEvents[gPlayerAvatar.objectEventId];
  if (!(heldKeys & B_BUTTON)) {
    // we were moving on a wheelie, but we let go while moving. reset bike still
    // status
    Bike_SetBikeStill();
    if (!MetatileBehavior_IsBumpySlope(
            playerObjEvent->currentMetatileBehavior)) {
      // we let go of B and arent on a bumpy slope, set state to normal because
      // now we need to handle this
      gPlayerAvatar.acroBikeState = ACRO_STATE_NORMAL;
      if (*newDirection == DIR_NONE) {
        // we stopped moving but are turning, still try to lower the wheelie in
        // place.
        *newDirection = direction;
        gPlayerAvatar.runningState = NOT_MOVING;
        return ACRO_TRANS_WHEELIE_TO_NORMAL;
      ...
      // if we are moving while lowering wheelie, put the acro into a lowering
      // state while moving.
      gPlayerAvatar.runningState = MOVING;
...
static void AcroBikeTransition_TurnDirection(u8 direction) {
  ...
  if (CanBikeFaceDirOnMetatile(direction,
                               playerObjEvent->currentMetatileBehavior) == 0)
...
static void AcroBikeTransition_WheelieHoppingMoving(u8 direction) {
  ...
  if (CanBikeFaceDirOnMetatile(direction,
                               playerObjEvent->currentMetatileBehavior) == 0) {
    AcroBikeTransition_WheelieHoppingStanding(
        playerObjEvent->movementDirection);
    return;
  }
  collision = GetBikeCollision(direction);
...
static void AcroBike_TryHistoryUpdate(u16 newKeys,
                                      u16 heldKeys) // newKeys is unused
{
  ...
  direction =
      heldKeys & (A_BUTTON | B_BUTTON | SELECT_BUTTON |
...
static bool8
HasPlayerInputTakenLongerThanList(const u8 *dirTimerList,
                                  const u8 *abStartSelectTimerList) {
  u8 i;
...
static u8 AcroBike_GetJumpDirection(void) {
  ...
  for (i = 0; i < ARRAY_COUNT(sAcroBikeTricksList); i++) {
    const struct BikeHistoryInputInfo *historyInputInfo =
        &sAcroBikeTricksList[i];
...
static void Bike_UpdateDirTimerHistory(u8 dir) {
  ...
  gPlayerAvatar.directionHistory =
      (gPlayerAvatar.directionHistory << 4) | (dir & 0xF);
...
static void Bike_UpdateABStartSelectHistory(u8 input) {
  ...
  gPlayerAvatar.abStartSelectHistory =
      (gPlayerAvatar.abStartSelectHistory << 4) | (input & 0xF);

  for (i = ARRAY_COUNT(gPlayerAvatar.abStartSelectTimerHistory) - 1; i != 0;
       i--)
    gPlayerAvatar.abStartSelectTimerHistory[i] =
        gPlayerAvatar.abStartSelectTimerHistory[i - 1];
...
static u8 GetBikeCollisionAt(struct ObjectEvent *objectEvent, s16 x, s16 y,
                             u8 direction, u8 metatileBehavior) {
  u8 collision = CheckForObjectEventCollision(objectEvent, x, y, direction,
                                              metatileBehavior);
  ...
  if (collision == COLLISION_NONE &&
      IsRunningDisallowedByMetatile(metatileBehavior))
...
s16 GetPlayerSpeed(void) {
  // because the player pressed a direction, it won't ever return a speed of 0
  // since this function returns the player's current speed.
  s16 machSpeeds[3];
  ...
  else if (gPlayerAvatar.flags &
           (PLAYER_AVATAR_FLAG_SURFING | PLAYER_AVATAR_FLAG_DASH))
