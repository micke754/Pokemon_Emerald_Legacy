...
output_burst:
  ...
  case 1:
    ...
    mp->probe_count = 2;
    return MultiBootSend(mp, (MULTIBOOT_MASTER_START_PROBE << 8) |
                                 mp->probe_target_bit);
  ...
  case 0xd0:
    ...
    if (k == 0) {
      return MultiBootSend(mp, (MULTIBOOT_MASTER_REQUEST_DLREADY << 8) |
                                   mp->palette_data);
    }
  ...
  default:
    for (i = MULTIBOOT_NCHILD; i != 0; i--) {
      if (mp->probe_target_bit & (1 << i)) {
        j = REG_SIOMULTI(i);
        if ((j >> 8) !=
                (MULTIBOOT_MASTER_START_PROBE + 1 - (mp->probe_count >> 1)) ||
            ((j & 0xff) != (1 << i))) {
          mp->probe_target_bit ^= 1 << i;
        }
      }
