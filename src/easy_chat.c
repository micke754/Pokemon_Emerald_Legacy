...
static void PrintEasyChatTextWithColors(u8, u8, const u8 *, u8, u8, u8, u8, u8,
                                        u8);
...
enum { FOOTER_NORMAL, FOOTER_QUIZ, FOOTER_ANSWER, NUM_FOOTER_TYPES };
...
// Types of animations for the lower window (keyboard/word select), given to
// InitLowerWindowAnim
enum {
...
static const u16 sBerryMasterWifePhrases[][2] = {
    ...
    [PHRASE_OVERWHELMING_LATIAS - 1] = {EC_WORD_OVERWHELMING,
                                        EC_POKEMON(LATIAS)},
    [PHRASE_COOL_LATIOS - 1] = {EC_WORD_COOL, EC_POKEMON(LATIOS)},
    [PHRASE_SUPER_HUSTLE - 1] = {EC_WORD_SUPER, EC_WORD_HUSTLE},
};

static const u16 sTriangleCursor_Pal[] =
    INCBIN_U16("graphics/easy_chat/triangle_cursor.gbapal");
static const u32 sTriangleCursor_Gfx[] =
    INCBIN_U32("graphics/easy_chat/triangle_cursor.4bpp");
static const u32 sScrollIndicator_Gfx[] =
    INCBIN_U32("graphics/easy_chat/scroll_indicator.4bpp");
static const u32 sStartSelectButtons_Gfx[] =
    INCBIN_U32("graphics/easy_chat/start_select_buttons.4bpp");
// In Ruby/Sapphire Easy Chat screens had a black background, and when the
// player & interviewer were present on screen the interview_frame gfx was shown
// behind them. In Emerald all Easy Chat screens have a filled background, so
// these gfx go unused
static const u16 sRSInterviewFrame_Pal[] =
    INCBIN_U16("graphics/easy_chat/interview_frame.gbapal");
static const u32 sRSInterviewFrame_Gfx[] =
    INCBIN_U32("graphics/easy_chat/interview_frame.4bpp.lz");
static const u16 sTextInputFrameOrange_Pal[] =
    INCBIN_U16("graphics/easy_chat/text_input_frame_orange.gbapal");
static const u16 sTextInputFrameGreen_Pal[] =
    INCBIN_U16("graphics/easy_chat/text_input_frame_green.gbapal");
static const u32 sTextInputFrame_Gfx[] =
    INCBIN_U32("graphics/easy_chat/text_input_frame.4bpp.lz");
static const u16 sTitleText_Pal[] =
    INCBIN_U16("graphics/easy_chat/title_text.gbapal");
...
static const struct SpritePalette sSpritePalettes[] = {
    ...
    },
    {
        ...
        .tag = PALTAG_MISC_UI, // The palette is generated from the button
                               // window but used for various parts of the UI
    },
    {
...
// Each anim changes the width of the rectangle cursor to fit what it should be
// selecting
enum {
...
// Frame0 is Start button, Frame1 is Select button, both are identical for the
// scroll indicators
...
static const u16 sDefaultBattleStartWords[EASY_CHAT_BATTLE_WORDS_COUNT] = {
    EC_WORD_ARE,  EC_WORD_YOU,         EC_WORD_READY,
    EC_WORD_QUES, EC_WORD_HERE_I_COME, EC_WORD_EXCL,
};
...
void DoEasyChatScreen(u8 type, u16 *words, MainCallback exitCallback,
...
static void Task_EasyChatScreen(u8 taskId) {
  ...
  case MAINSTATE_EXIT:
    if (!gPaletteFade.active)
      ExitEasyChatScreen(
          (MainCallback)GetWordTaskArg(taskId, TASKIDX_EXIT_CALLBACK));
    break;
...
static bool8 InitEasyChatScreen(u8 taskId) {
  ...
  case 2:
    if (!InitEasyChatScreenStruct(
            tType, (u16 *)GetWordTaskArg(taskId, TASKIDX_WORDS), tPersonType)) {
      // Alloc failed, exit
      ExitEasyChatScreen(
          (MainCallback)GetWordTaskArg(taskId, TASKIDX_EXIT_CALLBACK));
    }
...
void ShowEasyChatScreen(void) {
  ...
  case EASY_CHAT_TYPE_CONTEST_INTERVIEW:
    words = &gSaveBlock1Ptr->tvShows[gSpecialVar_0x8005]
                 .bravoTrainer.words[gSpecialVar_0x8006];
    displayedPersonType = EASY_CHAT_PERSON_REPORTER_MALE;
    break;
...
void QuizLadyShowQuizQuestion(void) { SetMainCallback2(CB2_QuizLadyQuestion); }
...
static void DoQuizQuestionEasyChatScreen(void) {
  DoEasyChatScreen(
      EASY_CHAT_TYPE_QUIZ_QUESTION, gSaveBlock1Ptr->lilycoveLady.quiz.question,
...
static bool8 InitEasyChatScreenStruct(u8 type, u16 *words,
  ...
  sEasyChatScreen->maxWords =
      sEasyChatScreen->numColumns * sEasyChatScreen->numRows;
  ...
  if (words != NULL) {
    ...
    CpuCopy16(words, sEasyChatScreen->currentPhrase,
              sEasyChatScreen->maxWords * sizeof(u16));
  } else {
  ...
  sEasyChatScreen->keyboardLastRow =
      (GetNumUnlockedEasyChatGroups() - 1) / 2 + 1;
  return TRUE;
}
...
static u16 HandleEasyChatInput_Phrase(void) {
  ...
  // All 2x5 phrases are only 9 words long, exclude the bottom right (10th)
  // position
  if (IsCurrentFrame2x5() && sEasyChatScreen->mainCursorColumn == 1 &&
...
// Handles main screen input while cursor is below the phrase on one of the
// buttons, e.g. Del. All or Cancel
static u16 HandleEasyChatInput_MainScreenButtons(void) {
  do {
    if (JOY_NEW(A_BUTTON)) {
...
// Odd, could have been skipped. Just passes to
...
// 'Completed' if the phrase was changed, or in the case of making a quiz, the
// question and answer were filled out
static bool32 GetEasyChatCompleted(void) {
  if (sEasyChatScreen->type == EASY_CHAT_TYPE_QUIZ_SET_QUESTION ||
      sEasyChatScreen->type == EASY_CHAT_TYPE_QUIZ_SET_ANSWER) {
    if (IsQuizQuestionEmpty())
...
static int MoveKeyboardCursor_GroupNames(u32 input) {
  ...
  case INPUT_DOWN:
    if (sEasyChatScreen->keyboardRow + sEasyChatScreen->keyboardScrollOffset <
        sEasyChatScreen->keyboardLastRow - 1) {
      int funcId;
...
static void SetKeyboardCursorToLastColumn(void) {
  if (!sEasyChatScreen->inAlphabetMode) {
    ...
    sEasyChatScreen->keyboardColumn =
        GetLastAlphabetColumn(sEasyChatScreen->keyboardRow);
  }
}
...
static u16 MoveWordSelectCursor(u32 input) {
  ...
  switch (input) {
  case INPUT_UP:
    if (sEasyChatScreen->wordSelectRow +
  ...
  case INPUT_SELECT:
    // Page scroll down
    if (sEasyChatScreen->wordSelectScrollOffset <=
        sEasyChatScreen->wordSelectLastRow - NUM_WORD_SELECT_ROWS) {
      sEasyChatScreen->wordSelectScrollOffset += NUM_WORD_SELECT_ROWS;
      if (sEasyChatScreen->wordSelectScrollOffset >
...
static u16 GetSelectedGroupIndex(void) {
  return NUM_GROUP_NAME_COLUMNS * (sEasyChatScreen->keyboardRow +
                                   sEasyChatScreen->keyboardScrollOffset) +
         sEasyChatScreen->keyboardColumn;
}

static int GetSelectedAlphabetGroupId(void) {
  int column = (u8)sEasyChatScreen->keyboardColumn < NUM_ALPHABET_COLUMNS
                   ? sEasyChatScreen->keyboardColumn
                   : 0;
  int row = (u8)sEasyChatScreen->keyboardRow < NUM_ALPHABET_ROWS
                ? sEasyChatScreen->keyboardRow
                : 0;
...
static u16 GetSelectedWordIndex(void) {
  return NUM_WORD_SELECT_COLUMNS * (sEasyChatScreen->wordSelectRow +
                                    sEasyChatScreen->wordSelectScrollOffset) +
         sEasyChatScreen->wordSelectColumn;
}

// Get the index of the last column in the alphabet keyboard, depending on
// current row
static u8 GetLastAlphabetColumn(u8 row) {
  ...
  case 1:
    return NUM_ALPHABET_COLUMNS -
           2; // At 6 letters, only the 2nd row (index 1) has less than the max
              // columns The 3rd and 4th row have 7 letters, the 1st row has 6
              // letters and 'Others'
  }
}
...
static bool8 IsSelectedKeyboardIndexInvalid(void) {
  if (!sEasyChatScreen->inAlphabetMode)
    return GetSelectedGroupIndex() >= GetNumUnlockedEasyChatGroups() ? TRUE
                                                                     : FALSE;
  else
    return sEasyChatScreen->keyboardColumn >
                   GetLastAlphabetColumn(sEasyChatScreen->keyboardRow)
               ? TRUE
               : FALSE;
}
...
static int FooterHasFourOptions(void) {
  return sEasyChatScreenTemplates[sEasyChatScreen->templateId]
      .fourFooterOptions;
}

static u8 GetEasyChatScreenType(void) { return sEasyChatScreen->type; }

static u8 GetEasyChatScreenFrameId(void) {
  return sEasyChatScreenTemplates[sEasyChatScreen->templateId].frameId;
}

const u8 *GetTitleText(void) { return sEasyChatScreen->titleText; }

static u16 *GetCurrentPhrase(void) { return sEasyChatScreen->currentPhrase; }

static u8 GetNumRows(void) { return sEasyChatScreen->numRows; }

static u8 GetNumColumns(void) { return sEasyChatScreen->numColumns; }

static u8 GetMainCursorColumn(void) {
  return sEasyChatScreen->mainCursorColumn;
}

static u8 GetMainCursorRow(void) { return sEasyChatScreen->mainCursorRow; }

static void GetEasyChatInstructionsText(const u8 **str1, const u8 **str2) {
  *str1 =
      sEasyChatScreenTemplates[sEasyChatScreen->templateId].instructionsText1;
  *str2 =
      sEasyChatScreenTemplates[sEasyChatScreen->templateId].instructionsText2;
}
...
static bool8 GetInAlphabetMode(void) { return sEasyChatScreen->inAlphabetMode; }
...
static u8 UNUSED UnusedDummy(void) { return FALSE; }
...
static bool32 CanScrollDown(void) {
  ...
  case INPUTSTATE_KEYBOARD:
    if (!sEasyChatScreen->inAlphabetMode &&
        sEasyChatScreen->keyboardScrollOffset + NUM_GROUP_NAME_ROWS <=
            sEasyChatScreen->keyboardLastRow - 1)
      return TRUE;
    break;
  case INPUTSTATE_WORD_SELECT:
...
static int FooterHasFourOptions_(void) { return FooterHasFourOptions(); }

static bool8 IsPhraseDifferentThanPlayerInput(const u16 *phrase,
                                              u8 phraseLength) {
  u8 i;
...
static void GetQuizTitle(u8 *dst) {
  ...
  if (StringLength(saveBlock1->lilycoveLady.quiz.playerName) != 0) {
    TVShowConvertInternationalString(name,
                                     saveBlock1->lilycoveLady.quiz.playerName,
...
static int DidPlayerInputMysteryGiftPhrase(void) {
  return !IsPhraseDifferentThanPlayerInput(sMysteryGiftPhrase,
                                           ARRAY_COUNT(sMysteryGiftPhrase));
}
...
static void ClearUnusedField(void) { sEasyChatScreen->unused = 0; }

static bool32 DummyWordCheck(int easyChatWord) { return FALSE; }
...
static bool8 LoadEasyChatScreen(void) {
  ...
  case 0:
    ...
    InitBgsFromTemplates(0, sEasyChatBgTemplates,
                         ARRAY_COUNT(sEasyChatBgTemplates));
...
static bool8 WordSelectPageScrollDown(void) {
  ...
  case 1:
    if (!IsDma3ManagerBusyWithBgCopy()) {
      s16 scrollChange =
          GetWordSelectScrollOffset() - GetLowerWindowScrollOffset();
...
static void LoadEasyChatPalettes(void) {
  ...
  LoadPalette(sTextInputFrameOrange_Pal, BG_PLTT_ID(1),
              sizeof(sTextInputFrameOrange_Pal));
  LoadPalette(sTextInputFrameGreen_Pal, BG_PLTT_ID(4),
              sizeof(sTextInputFrameGreen_Pal));
...
static void PrintTitle(void) {
  ...
  PrintEasyChatTextWithColors(WIN_TITLE, FONT_NORMAL, titleText, xOffset, 1,
                              TEXT_SKIP_DRAW, TEXT_COLOR_TRANSPARENT,
...
static void
PrintEasyChatText(u8 windowId, u8 fontId, const u8 *str, u8 x, u8 y, u8 speed,
                  void (*callback)(struct TextPrinterTemplate *, u16)) {
  AddTextPrinterParameterized(windowId, fontId, str, x, y, speed, callback);
}

static void PrintEasyChatTextWithColors(u8 windowId, u8 fontId, const u8 *str,
                                        u8 left, u8 top, u8 speed, u8 bg, u8 fg,
                                        u8 shadow) {
  u8 color[3];
  color[0] = bg;
  color[1] = fg;
...
static void PrintCurrentPhrase(void) {
  ...
  for (i = 0; i < numRows; i++) {
    ...
    for (j = 0; j < numColumns; j++) {
      ...
      if (frameId == FRAMEID_MAIL || frameId == FRAMEID_QUIZ_QUESTION ||
          frameId == FRAMEID_QUIZ_SET_QUESTION) {
    ...
    *str = EOS;
    PrintEasyChatText(sScreenControl->windowId, FONT_NORMAL,
                      sScreenControl->phrasePrintBuffer, 0, i * 16 + 1,
                      TEXT_SKIP_DRAW, 0);
  }
...
static void BufferFrameTilemap(u16 *tilemap) {
  ...
  if (frameId == FRAMEID_MAIL || frameId == FRAMEID_QUIZ_SET_QUESTION) {
    ...
    right = sPhraseFrameDimensions[frameId].left +
            sPhraseFrameDimensions[frameId].width;
    bottom = sPhraseFrameDimensions[frameId].top +
             sPhraseFrameDimensions[frameId].height;
...
static void PrintKeyboardGroupNames(void) {
  ...
  while (1) {
    for (x = 0; x < 2; x++) {
      ...
      PrintEasyChatText(WIN_INPUT_SELECT, FONT_NORMAL,
                        GetEasyChatWordGroupName(groupId), x * 84 + 10, y,
                        TEXT_SKIP_DRAW, NULL);
    }
...
static void PrintKeyboardAlphabet(void) {
  ...
  for (i = 0; i < ARRAY_COUNT(sEasyChatKeyboardAlphabet); i++)
    PrintEasyChatText(WIN_INPUT_SELECT, FONT_NORMAL,
                      sEasyChatKeyboardAlphabet[i], 10, 97 + i * 16,
                      TEXT_SKIP_DRAW, NULL);
}
  ...
  for (i = 0; i < numRows; i++) {
    for (j = 0; j < 2; j++) {
      ...
      if (easyChatWord != EC_EMPTY_WORD) {
        CopyEasyChatWordPadded(sScreenControl->wordSelectPrintBuffer,
                               easyChatWord, 0);
        if (!DummyWordCheck(easyChatWord))
          PrintEasyChatText(WIN_INPUT_SELECT, FONT_NORMAL,
                            sScreenControl->wordSelectPrintBuffer,
                            (j * 13 + 3) * 8, y, TEXT_SKIP_DRAW, NULL);
        else // Never reached
          PrintEasyChatTextWithColors(WIN_INPUT_SELECT, FONT_NORMAL,
                                      sScreenControl->wordSelectPrintBuffer,
                                      (j * 13 + 3) * 8, y, TEXT_SKIP_DRAW,
...
static void InitLowerWindowAnim(int winAnimType) {
  ...
  sScreenControl->windowAnimStateDir =
      sScreenControl->curWindowAnimState < sScreenControl->destWindowAnimState
          ? 1
          : -1;
}
...
// to draw the lower window and create the appearance that it's
// opening/closing/animating. See InitLowerWindowAnim
...
static void BufferLowerWindowFrame(int left, int top, int width, int height) {
  ...
  SetWindowDimensions((left + 1) * 8, (top + 1) * 8, (width - 2) * 8,
                      (height - 2) * 8);
}
...
static void SetRectangleCursorPos_GroupMode(s8 column, s8 row) {
  if (column != -1) {
    ...
    StartSpriteAnim(sScreenControl->rectangleCursorSpriteRight,
                    RECTCURSOR_ANIM_ON_GROUP);
    sScreenControl->rectangleCursorSpriteRight->x = column * 84 + 58;
    sScreenControl->rectangleCursorSpriteRight->y = row * 16 + 96;

    StartSpriteAnim(sScreenControl->rectangleCursorSpriteLeft,
                    RECTCURSOR_ANIM_ON_GROUP);
...
static void SetRectangleCursorPos_AlphabetMode(s8 column, s8 row) {
  ...
  if (column != -1) {
    ...
    if (column == NUM_ALPHABET_COLUMNS - 1 && row == 0) {
      ...
      x += sAlphabetKeyboardColumnOffsets[(u8)column < NUM_ALPHABET_COLUMNS
                                              ? column
                                              : 0];
...
static bool8 ShowSideWindow(void) {
  ...
  case 0:
    ...
    if (sScreenControl->buttonWindowSprite->x2 >= 0) {
      ...
      if (!GetInAlphabetMode())
        StartSpriteAnim(sScreenControl->modeWindowSprite,
                        MODEWINDOW_ANIM_TO_GROUP);
      else
        StartSpriteAnim(sScreenControl->modeWindowSprite,
                        MODEWINDOW_ANIM_TO_ALPHABET);
...
static void TryAddInterviewObjectEvents(void) {
  ...
  spriteId =
      CreateObjectGraphicsSprite(graphicsId, SpriteCallbackDummy, 76, 40, 0);
  if (spriteId != MAX_SPRITES) {
  ...
  spriteId = CreateObjectGraphicsSprite(gSaveBlock2Ptr->playerGender == MALE
...
u8 *ConvertEasyChatWordsToString(u8 *dest, const u16 *src, u16 columns,
                                 u16 rows) {
  u16 i, j;
...
static u8 UNUSED *UnusedConvertEasyChatWordsToString(u8 *dest, const u16 *src,
...
static bool8 CanPhraseFitInXRowsYCols(const u16 *easyChatWords, u8 numRows,
                                      u8 numColumns, u16 maxLength) {
  u8 i, j;
...
void ShowEasyChatProfile(void) {
  ...
  case 1:
    ...
    if (CanPhraseFitInXRowsYCols(gSaveBlock1Ptr->easyChatBattleStart, 3, 2,
                                 18)) {
      columns = 2;
      rows = 3;
    } else {
...
// The phrase that a man in Dewford Hall suggests has a "deep link" to the
...
/*
    ...
    This is a group of easy chat words (EC_GROUP_TRENDY_SAYING) that are
   normally inaccessible. They can be unlocked either through Mystery Event
   (where they're referred to as "rare" words) or from the "Hipster" variety of
   the Mauville Old Man. The Hipster can unlock one word each time he is
   received via record mixing (and once if he is the player's default Old Man).

    Which words have been unlocked is saved in the unlockedTrendySayings
   bitfield in SaveBlock1

    Unlocked trendy saying words are only accessible if the flag
   FLAG_UNLOCKED_TRENDY_SAYINGS is set. It's set any time the player talks to
   the Hipster, but is not apparently set by Mystery Event, meaning trendy
   saying words unlocked via Mystery Event may not be available until the player
   has talked to the Hipster.
*/
...
u16 UnlockRandomTrendySaying(void) {
  ...
  for (i = 0; i < NUM_TRENDY_SAYINGS; i++) {
    if (!IsTrendySayingUnlocked(i)) {
      if (numToSkip) {
        // Skip the first n locked words, as determined by the Random call
        // above.
        numToSkip--;
      } else {
  ...
  // Would only be reached if there are no new words to teach, which is handled
  // at the start.
  return EC_EMPTY_WORD;
}
...
static void SetUnlockedEasyChatGroups(void) {
  ...
  if (GetNationalPokedexCount(FLAG_GET_SEEN))
    sWordData->unlockedGroupIds[sWordData->numUnlockedGroups++] =
        EC_GROUP_POKEMON;
...
static u8 UNUSED *BufferEasyChatWordGroupName(u8 *dest, u8 groupId,
                                              u16 totalChars) {
  u16 i;
...
static void SetSelectedWordGroup(bool32 inAlphabetMode, u16 groupId) {
  if (!inAlphabetMode)
    ...
    sWordData->numSelectedGroupWords =
        SetSelectedWordGroup_AlphabetMode(groupId);
}
  ...
  if (groupId == EC_GROUP_POKEMON || groupId == EC_GROUP_POKEMON_NATIONAL ||
    ...
    for (i = 0, totalWords = 0; i < numWords; i++) {
      ...
      if (IsEasyChatIndexAndGroupUnlocked(alphabeticalOrder, groupId))
        sWordData->selectedGroupWords[totalWords++] =
            EC_WORD(groupId, alphabeticalOrder);
    }
...
static u16 SetSelectedWordGroup_AlphabetMode(u16 groupId) {
  ...
  for (i = 0, totalWords = 0; i < sWordData->numUnlockedAlphabetWords[groupId];
       i++)
    sWordData->selectedGroupWords[totalWords++] =
...
static bool8 IsEasyChatIndexAndGroupUnlocked(u16 wordIndex, u8 groupId) {
  switch (groupId) {
  case EC_GROUP_POKEMON:
    return GetSetPokedexFlag(SpeciesToNationalPokedexNum(wordIndex),
                             FLAG_GET_SEEN);
  case EC_GROUP_POKEMON_NATIONAL:
...
// Pokémon words in EC_GROUP_POKEMON_NATIONAL are always allowed (assuming the
// group is unlocked) unless they are in this group. If they are in this group
// (just Deoxys), they must also have been seen.
