...
void STWI_set_MS_mode(u8 mode) { gSTWIStatus->msMode = mode; }
...
void STWI_init_Callback_M(void) { STWI_set_Callback_M(NULL); }

void STWI_init_Callback_S(void) { STWI_set_Callback_S(NULL); }
...
void STWI_set_Callback_ID(void (*func)(
    void)) // name in SDK, but is actually setting a function pointer
{
...
void STWI_send_DataTxREQ(const void *in, u8 size) {
  if (!STWI_init(ID_DATA_TX_REQ)) {
    ...
    CpuCopy32(in, gSTWIStatus->txPacket->rfuPacket32.data,
              gSTWIStatus->reqLength * sizeof(u32));
    STWI_start_Command();
  }
}
...
static s32 STWI_start_Command(void) {
  ...
  *(u32 *)gSTWIStatus->txPacket->rfuPacket8.data =
      0x99660000 | (gSTWIStatus->reqLength << 8) |
  ...
  REG_IME = imeTemp;
  REG_SIOCNT =
      SIO_INTR_ENABLE | SIO_32BIT_MODE | SIO_MULTI_BUSY | SIO_115200_BPS;
  return 0;
}
...
static s32 STWI_restart_Command(void) {
  if (gSTWIStatus->recoveryCount < 2) {
    ...
    if (gSTWIStatus->reqActiveCommand == ID_MS_CHANGE_REQ ||
      ...
      gSTWIStatus->sending = 0;
      if (gSTWIStatus->callbackM != NULL)
        gSTWIStatus->callbackM(gSTWIStatus->reqActiveCommand,
                               gSTWIStatus->error);
    } else {
      gSTWIStatus->error = ERR_REQ_CMD_CLOCK_DRIFT;
      gSTWIStatus->sending = 0;
