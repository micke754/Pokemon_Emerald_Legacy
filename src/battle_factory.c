...
// Number of moves needed on the team to be considered using a certain battle
// style
static const u8 sRequiredMoveCounts[FACTORY_NUM_STYLES - 1] = {
    [FACTORY_STYLE_PREPARATION - 1] = 3, [FACTORY_STYLE_SLOW_STEADY - 1] = 3,
    [FACTORY_STYLE_ENDURANCE - 1] = 3,   [FACTORY_STYLE_HIGH_RISK - 1] = 2,
    [FACTORY_STYLE_WEAKENING - 1] = 2,   [FACTORY_STYLE_UNPREDICTABLE - 1] = 2,
    [FACTORY_STYLE_WEATHER - 1] = 2};

static const u16 sMoves_TotalPreparation[] = {
    MOVE_SWORDS_DANCE, MOVE_GROWTH,       MOVE_MEDITATE,     MOVE_AGILITY,
    MOVE_DOUBLE_TEAM,  MOVE_HARDEN,       MOVE_MINIMIZE,     MOVE_WITHDRAW,
    MOVE_DEFENSE_CURL, MOVE_BARRIER,      MOVE_FOCUS_ENERGY, MOVE_AMNESIA,
    MOVE_ACID_ARMOR,   MOVE_SHARPEN,      MOVE_CONVERSION,   MOVE_CONVERSION_2,
    MOVE_BELLY_DRUM,   MOVE_PSYCH_UP,     MOVE_CHARGE,       MOVE_SNATCH,
    MOVE_TAIL_GLOW,    MOVE_COSMIC_POWER, MOVE_IRON_DEFENSE, MOVE_HOWL,
    MOVE_BULK_UP,      MOVE_CALM_MIND,    MOVE_DRAGON_DANCE, MOVE_NONE};

static const u16 sMoves_ImpossibleToPredict[] = {
    MOVE_MIMIC,      MOVE_METRONOME,  MOVE_MIRROR_MOVE, MOVE_TRANSFORM,
...
static const u16 sMoves_WeakeningTheFoe[] = {
    MOVE_SAND_ATTACK, MOVE_TAIL_WHIP,    MOVE_LEER,        MOVE_GROWL,
...
static const u16 sMoves_HighRiskHighReturn[] = {
    MOVE_GUILLOTINE,   MOVE_HORN_DRILL,   MOVE_DOUBLE_EDGE, MOVE_HYPER_BEAM,
    MOVE_COUNTER,      MOVE_FISSURE,      MOVE_BIDE,        MOVE_SELF_DESTRUCT,
    MOVE_SKY_ATTACK,   MOVE_EXPLOSION,    MOVE_FLAIL,       MOVE_REVERSAL,
    MOVE_DESTINY_BOND, MOVE_PERISH_SONG,  MOVE_PAIN_SPLIT,  MOVE_MIRROR_COAT,
    MOVE_MEMENTO,      MOVE_GRUDGE,       MOVE_FACADE,      MOVE_FOCUS_PUNCH,
    MOVE_BLAST_BURN,   MOVE_HYDRO_CANNON, MOVE_OVERHEAT,    MOVE_FRENZY_PLANT,
    MOVE_PSYCHO_BOOST, MOVE_VOLT_TACKLE,  MOVE_NONE};

static const u16 sMoves_Endurance[] = {
    MOVE_MIST,       MOVE_RECOVER,      MOVE_LIGHT_SCREEN, MOVE_HAZE,
...
static const u16 sMoves_SlowAndSteady[] = {
    MOVE_SING,          MOVE_SUPERSONIC,    MOVE_DISABLE,
    MOVE_LEECH_SEED,    MOVE_POISON_POWDER, MOVE_STUN_SPORE,
    MOVE_SLEEP_POWDER,  MOVE_THUNDER_WAVE,  MOVE_TOXIC,
    MOVE_HYPNOSIS,      MOVE_CONFUSE_RAY,   MOVE_GLARE,
    MOVE_POISON_GAS,    MOVE_LOVELY_KISS,   MOVE_SPORE,
    MOVE_SPIDER_WEB,    MOVE_SWEET_KISS,    MOVE_SPIKES,
    MOVE_SWAGGER,       MOVE_MEAN_LOOK,     MOVE_ATTRACT,
    MOVE_ENCORE,        MOVE_TORMENT,       MOVE_FLATTER,
    MOVE_WILL_O_WISP,   MOVE_TAUNT,         MOVE_YAWN,
    MOVE_IMPRISON,      MOVE_SNATCH,        MOVE_TEETER_DANCE,
    MOVE_GRASS_WHISTLE, MOVE_BLOCK,         MOVE_NONE};

static const u16 sMoves_DependsOnTheBattlesFlow[] = {
    MOVE_SANDSTORM, MOVE_RAIN_DANCE,   MOVE_SUNNY_DAY,
...
static void SwapRentalMons(void) { DoBattleFactorySwapScreen(); }

static void SetPerformedRentalSwap(void) { sPerformedRentalSwap = TRUE; }
...
static void GenerateOpponentMons(void) {
  ...
  if (gSaveBlock2Ptr->frontier.curChallengeBattleNum <
    ...
    gSaveBlock2Ptr->frontier
        .trainerIds[gSaveBlock2Ptr->frontier.curChallengeBattleNum] = trainerId;
  ...
  while (i != FRONTIER_PARTY_SIZE) {
    ...
    // Ensure none of the opponent's Pokémon are the same as the potential
    // rental Pokémon for the player
    for (j = 0; j < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.rentalMons);
         j++) {
      if (gFacilityTrainerMons[monId].species ==
          gFacilityTrainerMons[gSaveBlock2Ptr->frontier.rentalMons[j].monId]
              .species)
        break;
    }
...
static void SetRentalsToOpponentParty(void) {
  ...
  for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
    gSaveBlock2Ptr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].monId =
        gFrontierTempParty[i];
    gSaveBlock2Ptr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].ivs =
    ...
    gSaveBlock2Ptr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE].abilityNum =
        GetBoxMonData(&gEnemyParty[i].box, MON_DATA_ABILITY_NUM, NULL);
    SetMonData(
        &gEnemyParty[i], MON_DATA_HELD_ITEM,
        &gBattleFrontierHeldItems[gFacilityTrainerMons[gFrontierTempParty[i]]
                                      .itemTableId]);
  }
}
...
static void SetPlayerAndOpponentParties(void) {
  ...
  if (gSpecialVar_0x8005 < 2) {
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
      ...
      CreateMon(&gPlayerParty[i], gFacilityTrainerMons[monId].species, monLevel,
                ivs, TRUE, gSaveBlock2Ptr->frontier.rentalMons[i].personality,
                OT_ID_PLAYER_ID, 0);
      ...
      for (k = 0; k < MAX_MON_MOVES; k++)
        SetMonMoveAvoidReturn(&gPlayerParty[i],
                              gFacilityTrainerMons[monId].moves[k], k);
      SetMonData(&gPlayerParty[i], MON_DATA_FRIENDSHIP, &friendship);
      SetMonData(
          &gPlayerParty[i], MON_DATA_HELD_ITEM,
          &gBattleFrontierHeldItems[gFacilityTrainerMons[monId].itemTableId]);
      SetMonData(&gPlayerParty[i], MON_DATA_ABILITY_NUM,
  ...
  case 2:
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
      ...
      CreateMon(&gEnemyParty[i], gFacilityTrainerMons[monId].species, monLevel,
                ivs, TRUE,
                gSaveBlock2Ptr->frontier.rentalMons[i + FRONTIER_PARTY_SIZE]
                    .personality,
                OT_ID_PLAYER_ID, 0);
      ...
      for (k = 0; k < MAX_MON_MOVES; k++)
        SetMonMoveAvoidReturn(&gEnemyParty[i],
                              gFacilityTrainerMons[monId].moves[k], k);
      SetMonData(
          &gEnemyParty[i], MON_DATA_HELD_ITEM,
          &gBattleFrontierHeldItems[gFacilityTrainerMons[monId].itemTableId]);
...
static void GenerateInitialRentalMons(void) {
  ...
  while (i != PARTY_SIZE) {
    if (i <
        rentalRank) // The more times the player has rented, the more initial
                    // rentals are generated from a better set of Pokémon
    ...
    for (j = firstMonId; j < firstMonId + i; j++) {
      if (heldItems[j] != ITEM_NONE &&
          heldItems[j] == gBattleFrontierHeldItems[gFacilityTrainerMons[monId]
                                                       .itemTableId]) {
  ...
  for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
    SetMonData(
        ...
        &gBattleFrontierHeldItems
            [gFacilityTrainerMons[gSaveBlock2Ptr->frontier.rentalMons[i].monId]
                 .itemTableId]);
  }
}
...
u8 GetFactoryMonFixedIV(u8 challengeNum, bool8 isLastBattle) {
  ...
  // The Factory has an out-of-bounds access when generating the rental draft
  // for round 9 (challengeNum==8), or the "elevated" rentals from round 8
  // (challengeNum+1==8) This happens to land on a number higher than 31, which
  // is interpreted as "random IVs"
...
void FillFactoryBrainParty(void) {
  ...
  while (i != FRONTIER_PARTY_SIZE) {
    ...
    CreateMonWithEVSpreadNatureOTID(
        &gEnemyParty[i], gFacilityTrainerMons[monId].species, monLevel,
  ...
  if (lvlMode == FRONTIER_LVL_TENT) {
    ...
    if (gTrainerBattleOpponent_A == TRAINER_FRONTIER_BRAIN)
      return AI_SCRIPT_CHECK_BAD_MOVE | AI_SCRIPT_TRY_TO_FAINT |
             AI_SCRIPT_CHECK_VIABILITY | AI_SCRIPT_RISKY;
