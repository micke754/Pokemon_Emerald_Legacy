...
static u8 CreateTask_ListenForCompatiblePartners(struct RfuIncomingPlayerList *,
                                                 u32);
static u8 CreateTask_ListenForWonderDistributor(struct RfuIncomingPlayerList *,
                                                u32);
static bool8 PrintOnTextbox(u8 *, const u8 *);
static bool8 Leader_SetStateIfMemberListChanged(struct WirelessLink_Leader *,
                                                u32, u32);
...
static u32 GetNewIncomingPlayerId(struct RfuPlayer *,
                                  struct RfuIncomingPlayer *);
static u8 TryAddIncomingPlayerToList(struct RfuPlayer *,
                                     struct RfuIncomingPlayer *, u8);
static u8 GetNewLeaderCandidate(void);
static u32 IsTryingToTradeAcrossVersionTooSoon(struct WirelessLink_Group *,
                                               s32);
...
static bool32 ArePlayerDataDifferent(struct RfuPlayerData *,
...
static u8 CreateTask_SearchForChildOrParent(struct RfuIncomingPlayerList *,
...
static s32 UnionRoomGetPlayerInteractionResponse(struct RfuPlayerList *, u8, u8,
                                                 u32);
static void HandleCancelActivity(bool32);
static s32 ListMenuHandler_AllItemsAvailable(u8 *, u8 *, u8 *,
                                             const struct WindowTemplate *,
                                             const struct ListMenuTemplate *);
static s32 TradeBoardMenuHandler(u8 *, u8 *, u8 *, u8 *,
                                 const struct WindowTemplate *,
                                 const struct ListMenuTemplate *,
...
static void ViewURoomPartnerTrainerCard(u8 *, struct WirelessLink_URoom *,
                                        bool8);
...
static s32 GetChatLeaderActionRequestMessage(u8 *, u32, u16 *,
                                             struct WirelessLink_URoom *);
static void Task_InitUnionRoom(u8 taskId);
static bool8 ArePlayersDifferent(struct RfuPlayerData *,
                                 const struct RfuPlayerData *);
...
static void PrintNumPlayersWaitingForMsg(u8 windowId, u8 capacityCode,
                                         u8 stringId) {
  FillWindowPixelBuffer(windowId, PIXEL_FILL(1));
  switch (capacityCode << 8) {
  case LINK_GROUP_CAPACITY(0, 2):
    PrintUnionRoomText(windowId, FONT_NORMAL,
                       sPlayersNeededOrModeTexts[0][stringId - 1], 0, 1,
                       UR_COLOR_DEFAULT);
    break;
  case LINK_GROUP_CAPACITY(0, 4):
    PrintUnionRoomText(windowId, FONT_NORMAL,
                       sPlayersNeededOrModeTexts[1][stringId - 1], 0, 1,
                       UR_COLOR_DEFAULT);
    break;
  case LINK_GROUP_CAPACITY(2, 5):
    PrintUnionRoomText(windowId, FONT_NORMAL,
                       sPlayersNeededOrModeTexts[2][stringId - 1], 0, 1,
                       UR_COLOR_DEFAULT);
    break;
  case LINK_GROUP_CAPACITY(3, 5):
    PrintUnionRoomText(windowId, FONT_NORMAL,
                       sPlayersNeededOrModeTexts[3][stringId - 1], 0, 1,
                       UR_COLOR_DEFAULT);
    break;
  case LINK_GROUP_CAPACITY(2, 4):
    PrintUnionRoomText(windowId, FONT_NORMAL,
                       sPlayersNeededOrModeTexts[4][stringId - 1], 0, 1,
                       UR_COLOR_DEFAULT);
    break;
  }
...
static void PrintPlayerNameAndIdOnWindow(u8 windowId) {
  ...
  PrintUnionRoomText(windowId, FONT_NORMAL, gSaveBlock2Ptr->playerName, 0, 1,
                     UR_COLOR_DEFAULT);
  txtPtr = StringCopy(text, sText_ID);
  ConvertIntToDecimalStringN(txtPtr, ReadAsU16(gSaveBlock2Ptr->playerTrainerId),
...
static void GetAwaitingCommunicationText(u8 *dst, u8 activity) {
  ...
  case ACTIVITY_CONTEST_TOUGH:
    ...
    // not an issue in practice since Gamefreak never used any other arguments
    // here besides gStringVar4
#ifndef BUGFIX
...
static void Task_TryBecomeLinkLeader(u8 taskId) {
  ...
  case LL_STATE_INIT:
    ...
    sPlayerActivityGroupSize =
        sLinkGroupToActivityAndCapacity[gSpecialVar_0x8004] >> 8;
  ...
  case LL_STATE_INIT2:
    data->incomingPlayerList =
        AllocZeroed(RFU_CHILD_MAX * sizeof(struct RfuIncomingPlayer));
    data->playerList = AllocZeroed(MAX_RFU_PLAYERS * sizeof(struct RfuPlayer));
    data->playerListBackup =
        AllocZeroed(MAX_RFU_PLAYERS * sizeof(struct RfuPlayer));
    ClearIncomingPlayerList(data->incomingPlayerList, RFU_CHILD_MAX);
    ClearRfuPlayerList(data->playerList->players, MAX_RFU_PLAYERS);
    CopyHostRfuGameDataAndUsername(&data->playerList->players[0].rfu.data,
                                   data->playerList->players[0].rfu.name);
    ...
    PrintUnionRoomText(data->bButtonCancelWindowId, FONT_SMALL,
                       sText_BButtonCancel, 8, 1, UR_COLOR_CANCEL);
  ...
  case LL_STATE_GET_AWAITING_PLAYERS_TEXT:
    ...
    PrintNumPlayersWaitingForMsg(data->nPlayerModeWindowId,
                                 sPlayerActivityGroupSize, data->playerCount);
  ...
  case LL_STATE_AWAIT_PLAYERS:
    Leader_SetStateIfMemberListChanged(data, LL_STATE_ACCEPT_NEW_MEMBER_PROMPT,
                                       LL_STATE_MEMBER_LEFT);
    if (JOY_NEW(B_BUTTON)) {
    ...
    if (GROUP_MIN(sPlayerActivityGroupSize) != 0 &&
        ...
        GROUP_MAX(sPlayerActivityGroupSize) != 0 &&
        IsRfuCommunicatingWithAllChildren() && JOY_NEW(START_BUTTON)) {
  ...
  case LL_STATE_ACCEPT_NEW_MEMBER_PROMPT_HANDLE_INPUT:
    switch (UnionRoomHandleYesNo(
        ...
        HasTrainerLeftPartnersList(
            ReadAsU16(data->playerList->players[data->playerCount]
                          .rfu.data.compatibility.playerTrainerId),
    ...
    if (val == 1) // Send complete
    {
        ...
        if (data->playerCount == GROUP_MAX(sPlayerActivityGroupSize)) {
          if (GROUP_MIN2(sPlayerActivityGroupSize) != 0 ||
            ...
            CopyAndTranslatePlayerName(
                gStringVar1, &data->playerList->players[data->playerCount - 1]);
      ...
      } else // Member disconnected
      {
        RequestDisconnectSlotByTrainerNameAndId(
            ...
            ReadAsU16(data->playerList->players[data->playerCount]
                          .rfu.data.compatibility.playerTrainerId));
        data->playerList->players[data->playerCount].groupScheduledAnim =
  ...
  case LL_STATE_CONFIRMED_MEMBERS:
    if (!Leader_SetStateIfMemberListChanged(data,
                                            LL_STATE_ACCEPT_NEW_MEMBER_PROMPT,
  ...
  case LL_STATE_CANCEL_WITH_MSG:
    if (PrintOnTextbox(&data->textState,
...
static bool8
Leader_SetStateIfMemberListChanged(struct WirelessLink_Leader *data,
                                   u32 joinedState, u32 droppedState) {
  switch (LeaderUpdateGroupMembership(data->playerList)) {
  case UNION_ROOM_SPAWN_IN:
    PlaySE(SE_PC_LOGIN);
    RedrawListMenu(data->listTaskId);
    CopyAndTranslatePlayerName(gStringVar2,
...
static void ItemPrintFunc_PossibleGroupMembers(u8 windowId, u32 id, u8 y) {
  ...
  PrintGroupCandidateOnWindow(windowId, 0, y, &data->playerList->players[id],
                              colorIdx, id);
}
...
static u8 LeaderUpdateGroupMembership(struct RfuPlayerList *list) {
  ...
  for (i = 1; i < MAX_RFU_PLAYERS; i++) {
    ...
    if (var == UNION_ROOM_SPAWN_IN) {
      id = GetNewIncomingPlayerId(&data->playerList->players[i],
                                  data->incomingPlayerList->players);
      if (id != 0xFF) {
        // New incoming player
  ...
  for (id = 0; id < RFU_CHILD_MAX; id++)
    TryAddIncomingPlayerToList(data->playerList->players,
                               &data->incomingPlayerList->players[id],
                               MAX_RFU_PLAYERS);
...
static u8 LeaderPrunePlayerList(struct RfuPlayerList *list) {
  ...
  copiedCount = 0;
  for (i = 0; i < MAX_RFU_PLAYERS; i++) {
    if (data->playerListBackup->players[i].groupScheduledAnim ==
        UNION_ROOM_SPAWN_IN) {
      data->playerList->players[copiedCount] =
          data->playerListBackup->players[i];
      copiedCount++;
    }
  }
  ...
  for (; copiedCount < MAX_RFU_PLAYERS; copiedCount++) {
    ...
    data->playerList->players[copiedCount].groupScheduledAnim =
        UNION_ROOM_SPAWN_NONE;
...
static void Task_TryJoinLinkGroup(u8 taskId) {
  ...
  case LG_STATE_INIT:
    ...
    data->incomingPlayerList =
        AllocZeroed(RFU_CHILD_MAX * sizeof(struct RfuIncomingPlayer));
    data->playerList =
        AllocZeroed(MAX_RFU_PLAYER_LIST_SIZE * sizeof(struct RfuPlayer));
  ...
  case LG_STATE_INIT_WINDOWS:
    ...
    data->listenTaskId = CreateTask_ListenForCompatiblePartners(
        data->incomingPlayerList, gSpecialVar_0x8004);
    data->bButtonCancelWindowId = AddWindow(&sWindowTemplate_BButtonCancel);
    ...
    FillWindowPixelBuffer(data->bButtonCancelWindowId, PIXEL_FILL(2));
    PrintUnionRoomText(data->bButtonCancelWindowId, FONT_SMALL,
                       sText_ChooseJoinCancel, 8, 1, UR_COLOR_CANCEL);
    PutWindowTilemap(data->bButtonCancelWindowId);
    CopyWindowToVram(data->bButtonCancelWindowId, COPYWIN_GFX);
    ...
    case 0:
      ...
      if (JOY_NEW(A_BUTTON) && id != LIST_NOTHING_CHOSEN) {
        ...
        if (data->playerList->players[id].groupScheduledAnim ==
                UNION_ROOM_SPAWN_IN &&
            !data->playerList->players[id].rfu.data.startedActivity) {
  ...
  case LG_STATE_MAIN:
    if (gReceivedRemoteLinkPlayers) {
      gPlayerCurrActivity =
          data->playerList->players[data->leaderId].rfu.data.activity;
      RfuSetStatus(RFU_STATUS_OK, 0);
...
static u32 IsTryingToTradeAcrossVersionTooSoon(struct WirelessLink_Group *data,
                                               s32 id) {
  struct RfuPlayer *partner = &data->playerList->players[id];

  if (gPlayerCurrActivity == ACTIVITY_TRADE &&
      partner->rfu.data.compatibility.version != VERSION_EMERALD) {
...
static void AskToJoinRfuGroup(struct WirelessLink_Group *data, s32 id) {
  ...
  CopyAndTranslatePlayerName(gStringVar1,
                             &data->playerList->players[data->leaderId]);
  UpdateGameData_SetActivity(sLinkGroupToURoomActivity[gSpecialVar_0x8004], 0,
                             TRUE);
  CreateTask_RfuReconnectWithParent(
      data->playerList->players[data->leaderId].rfu.name,
      ReadAsU16(data->playerList->players[data->leaderId]
                    .rfu.data.compatibility.playerTrainerId));
}
...
static void ListMenuItemPrintFunc_UnionRoomGroups(u8 windowId, u32 id, u8 y) {
  ...
  PrintGroupMemberOnWindow(windowId, 8, y, &data->playerList->players[id],
                           colorId, id);
}
...
static u8 GetNewLeaderCandidate(void) {
      ...
      if (id != 0xFF) {
        ...
        if (data->playerList->players[i].groupScheduledAnim !=
          ...
          if (data->playerList->players[i].timeoutCounter >= 300) {
            data->playerList->players[i].groupScheduledAnim =
                UNION_ROOM_SPAWN_OUT;
            ret = 2;
          }
        }
  ...
  for (id = 0; id < RFU_CHILD_MAX; id++) {
    if (TryAddIncomingPlayerToList(data->playerList->players,
                                   &data->incomingPlayerList->players[id],
                                   MAX_RFU_PLAYER_LIST_SIZE) != 0xFF)
      ret = 1;
  }
...
static void Task_StartUnionRoomTrade(u8 taskId) {
  u32 monId =
      GetPartyPositionOfRegisteredMon(&sUnionRoomTrade, GetMultiplayerId());
  ...
  case 1:
    if (GetBlockReceivedStatus() == 3) {
      gEnemyParty[0] =
          *(struct Pokemon *)(gBlockRecvBuffer[GetMultiplayerId() ^ 1]);
    ...
    break;
  case 2:
    memcpy(gBlockSendBuffer, gSaveBlock1Ptr->mail,
           sizeof(struct Mail) * PARTY_SIZE + 4);
    if (SendBlock(0, gBlockSendBuffer, sizeof(struct Mail) * PARTY_SIZE + 4))
      gTasks[taskId].data[0]++;
    break;
  case 3:
    if (GetBlockReceivedStatus() == 3) {
      memcpy(gTradeMail, gBlockRecvBuffer[GetMultiplayerId() ^ 1],
             sizeof(struct Mail) * PARTY_SIZE);
      ResetBlockReceivedFlags();
  ...
  case 1:
    if (GetBlockReceivedStatus() == GetLinkPlayerCountAsBitFlags()) {
      ...
      for (i = 0; i < GetLinkPlayerCount(); i++) {
        ...
        CopyTrainerCardData(&gTrainerCards[i], (struct TrainerCard *)recvBuff,
                            gLinkPlayers[i].version);
      }
      ...
      if (GetLinkPlayerCount() == 2) {
        ...
        MysteryGift_TryEnableStatsByFlagId(
            ((struct TrainerCard *)recvBuff)->hasAllFrontierSymbols);
      } else {
...
static void WarpForWirelessMinigame(u16 linkService, u16 x, u16 y) {
  ...
  SetDynamicWarpWithCoords(0, gSaveBlock1Ptr->location.mapGroup,
                           gSaveBlock1Ptr->location.mapNum, WARP_ID_NONE, x, y);
  WarpIntoMap();
}

static void WarpForCableClubActivity(s8 mapGroup, s8 mapNum, s32 x, s32 y,
                                     u16 linkService) {
...
static void Task_StartActivity(u8 taskId) {
  ...
  case ACTIVITY_BATTLE_SINGLE:
    ...
    WarpForCableClubActivity(MAP_GROUP(BATTLE_COLOSSEUM_2P),
                             MAP_NUM(BATTLE_COLOSSEUM_2P), 6, 8,
                             USING_SINGLE_BATTLE);
  ...
  case ACTIVITY_BATTLE_MULTI:
    ...
    WarpForCableClubActivity(MAP_GROUP(BATTLE_COLOSSEUM_4P),
                             MAP_NUM(BATTLE_COLOSSEUM_4P), 5, 8,
                             USING_MULTI_BATTLE);
    SetMainCallback2(CB2_TransitionToCableClub);
    break;
  case ACTIVITY_TRADE:
    CreateTrainerCardInBuffer(gBlockSendBuffer, TRUE);
    CleanupOverworldWindowsAndTilemaps();
    WarpForCableClubActivity(MAP_GROUP(TRADE_CENTER), MAP_NUM(TRADE_CENTER), 5,
                             8, USING_TRADE_CENTER);
    SetMainCallback2(CB2_TransitionToCableClub);
    break;
  case ACTIVITY_RECORD_CORNER:
    CreateTrainerCardInBuffer(gBlockSendBuffer, TRUE);
    CleanupOverworldWindowsAndTilemaps();
    WarpForCableClubActivity(MAP_GROUP(RECORD_CORNER), MAP_NUM(RECORD_CORNER),
                             8, 9, USING_RECORD_CORNER);
    SetMainCallback2(CB2_TransitionToCableClub);
    break;
  case ACTIVITY_TRADE | IN_UNION_ROOM:
    CleanupOverworldWindowsAndTilemaps();
...
static void Task_RunScriptAndFadeToActivity(u8 taskId) {
  ...
  case 0:
    ...
    case ACTIVITY_BATTLE_TOWER_OPEN:
      ...
      gLinkPlayers[0].id = 0;
      gLinkPlayers[1].id = 2;
      sendBuff[0] = GetMonData(&gPlayerParty[gSelectedOrderFromParty[0] - 1],
                               MON_DATA_SPECIES);
      sendBuff[1] = GetMonData(&gPlayerParty[gSelectedOrderFromParty[1] - 1],
  ...
  case 5:
    if (GetBlockReceivedStatus() == 3) {
      ResetBlockReceivedFlags();
      if (AreBattleTowerLinkSpeciesSame(gBlockRecvBuffer[0],
                                        gBlockRecvBuffer[1])) {
...
static void Task_SendMysteryGift(u8 taskId) {
  ...
  case 4:
    ...
    break;
  case 6:
    if (PrintMysteryGiftMenuMessage(&data->textState,
  ...
  case 5:
    ...
    break;
  case 7:
    switch (DoMysteryGiftYesNo(&data->textState, &data->yesNoWindowId, FALSE,
                               gStringVar4)) {
    case 0:
      LoadWirelessStatusIndicatorSpriteGfx();
      CreateWirelessStatusIndicatorSprite(0, 0);
...
static void Task_CardOrNewsWithFriend(u8 taskId) {
  ...
  case 2:
    ...
    ClearRfuPlayerList(data->playerList->players, MAX_RFU_PLAYER_LIST_SIZE);
    data->listenTaskId = CreateTask_ListenForCompatiblePartners(
        data->incomingPlayerList, data->isWonderNews + LINK_GROUP_WONDER_CARD);
    ...
    case 0:
      ...
      if (JOY_NEW(A_BUTTON) && id != LIST_NOTHING_CHOSEN) {
        ...
        if (data->playerList->players[id].groupScheduledAnim ==
          ...
          RedrawListMenu(data->listTaskId);
          CopyAndTranslatePlayerName(
              gStringVar1, &data->playerList->players[data->leaderId]);
          CreateTask_RfuReconnectWithParent(
              data->playerList->players[data->leaderId].rfu.name,
              ReadAsU16(data->playerList->players[data->leaderId]
                            .rfu.data.compatibility.playerTrainerId));
    ...
    case 0:
      ...
      if (data->refreshTimer > 120) {
        if (data->playerList->players[0].groupScheduledAnim ==
                UNION_ROOM_SPAWN_IN &&
            !data->playerList->players[0].rfu.data.startedActivity) {
          if (HasWonderCardOrNewsByLinkGroup(
                  &data->playerList->players[0].rfu.data,
            ...
            CreateTask_RfuReconnectWithParent(
                data->playerList->players[0].rfu.name,
                ReadAsU16(data->playerList->players[0]
                              .rfu.data.compatibility.playerTrainerId));
            PlaySE(SE_POKENAV_ON);
            data->state = 4;
          } else {
...
static u16 ReadAsU16(const u8 *ptr) { return (ptr[1] << 8) | (ptr[0]); }

static void ScheduleFieldMessageWithFollowupState(u32 nextState,
                                                  const u8 *src) {
...
static void CopyPlayerListToBuffer(struct WirelessLink_URoom *uroom) {
  memcpy(&gDecompressionBuffer[sizeof(gDecompressionBuffer) -
                               (MAX_UNION_ROOM_LEADERS *
                                sizeof(struct RfuPlayer))],
         uroom->playerList, MAX_UNION_ROOM_LEADERS * sizeof(struct RfuPlayer));
}

static void CopyPlayerListFromBuffer(struct WirelessLink_URoom *uroom) {
  memcpy(uroom->playerList,
         &gDecompressionBuffer[sizeof(gDecompressionBuffer) -
                               (MAX_UNION_ROOM_LEADERS *
                                sizeof(struct RfuPlayer))],
...
static void Task_RunUnionRoom(u8 taskId) {
  ...
  case UR_STATE_INIT:
    ...
    uroom->incomingParentList =
        AllocZeroed(RFU_CHILD_MAX * sizeof(struct RfuIncomingPlayer));
    uroom->playerList =
        AllocZeroed(MAX_UNION_ROOM_LEADERS * sizeof(struct RfuPlayer));
    ...
    uroom->searchTaskId = CreateTask_SearchForChildOrParent(
        uroom->incomingParentList, uroom->incomingChildList,
  ...
  case UR_STATE_INIT_LINK:
    ...
    SetTradeBoardRegisteredMonInfo(sUnionRoomTrade.type,
                                   sUnionRoomTrade.playerSpecies,
  ...
  case UR_STATE_CHECK_SELECTING_MON:
    if ((GetPartyMenuType() == PARTY_MENU_TYPE_UNION_ROOM_REGISTER ||
      ...
      case URTRADE_STATE_REGISTERING:
        ...
        } else if (!RegisterTradeMonAndGetIsEgg(GetCursorSelectionMonId(),
                                                &sUnionRoomTrade)) {
      ...
      case URTRADE_STATE_OFFERING:
        ...
        if (id >= PARTY_SIZE) {
          ...
          UpdateGameData_SetActivity(ACTIVITY_PLYRTALK | IN_UNION_ROOM, 0,
                                     TRUE);
  ...
  case UR_STATE_MAIN:
    if (gSpecialVar_Result != 0) {
      ...
      if (JOY_NEW(A_BUTTON)) {
        if (TryInteractWithUnionRoomMember(uroom->playerList, &taskData[0],
                                           &taskData[1], uroom->spriteIds)) {
          PlaySE(SE_SELECT);
      ...
      case PLIST_CONTACTED:
        ...
        UpdateGameData_SetActivity(ACTIVITY_NPCTALK | IN_UNION_ROOM,
                                   GetActivePartnersInfo(uroom), FALSE);
        break;
      }
  ...
  case UR_STATE_INTERACT_WITH_PLAYER:
    ...
    switch (UnionRoomGetPlayerInteractionResponse(
        uroom->playerList, taskData[0], taskData[1], playerGender)) {
    case 0: // Player is or was just doing an activity
      uroom->state = UR_STATE_PRINT_AND_EXIT;
      break;
    case 1: // Link communicating
      TryConnectToUnionRoomParent(
          uroom->playerList->players[taskData[1]].rfu.name,
          &uroom->playerList->players[taskData[1]].rfu.data,
          gPlayerCurrActivity);
      uroom->unreadPlayerId =
          id; // Should be just 0, but won't match any other way.
      uroom->state = UR_STATE_TRY_COMMUNICATING;
      break;
    case 2: // Ask to join chat
      ScheduleFieldMessageWithFollowupState(UR_STATE_RECV_JOIN_CHAT_REQUEST,
                                            gStringVar4);
      break;
    }
    break;
  ...
  case UR_STATE_DO_SOMETHING_PROMPT:
    id = ConvPartnerUnameAndGetWhetherMetAlready(
        &uroom->playerList->players[taskData[1]]);
    playerGender = GetUnionRoomPlayerGender(taskData[1], uroom->playerList);
    ScheduleFieldMessageWithFollowupState(
        UR_STATE_HANDLE_DO_SOMETHING_PROMPT_INPUT,
  ...
  case UR_STATE_HANDLE_DO_SOMETHING_PROMPT_INPUT:
    input = ListMenuHandler_AllItemsAvailable(
        &uroom->textState, &uroom->topListMenuWindowId, &uroom->topListMenuId,
        &sWindowTemplate_InviteToActivity, &sListMenuTemplate_InviteToActivity);
    if (input != LIST_NOTHING_CHOSEN) {
      if (!gReceivedRemoteLinkPlayers) {
        ...
        if (input == LIST_CANCEL || input == IN_UNION_ROOM) {
          ...
          sPlayerActivityGroupSize =
              (u32)input >>
              8; // Extract capacity from sInviteToActivityMenuItems
  ...
  case UR_STATE_SEND_ACTIVITY_REQUEST:
    ...
    playerGender = GetUnionRoomPlayerGender(taskData[1], uroom->playerList);
    id =
        GetResponseIdx_InviteToURoomActivity(uroom->playerSendBuffer[0] & 0x3F);
    if (PrintOnTextbox(&uroom->textState,
  ...
  case UR_STATE_WAIT_FOR_RESPONSE_TO_REQUEST:
    if (!gReceivedRemoteLinkPlayers) {
      StringCopy(gStringVar4,
                 sText_TrainerBattleBusy); // Redundant, will be copied again in
                                           // next state
      uroom->state = UR_STATE_TRAINER_APPEARS_BUSY;
    } else {
      PollPartnerYesNoResponse(uroom);
      ...
      } else if (uroom->partnerYesNoResponse ==
                 (ACTIVITY_DECLINE | IN_UNION_ROOM)) {
        uroom->state = UR_STATE_REQUEST_DECLINED;
        GetURoomActivityRejectMsg(gStringVar4,
                                  gPlayerCurrActivity | IN_UNION_ROOM,
                                  gLinkPlayers[0].gender);
        gPlayerCurrActivity = ACTIVITY_NONE;
      }
    }
    break;

  case UR_STATE_DO_SOMETHING_PROMPT_2: // Identical to
                                       // UR_STATE_DO_SOMETHING_PROMPT
    id = ConvPartnerUnameAndGetWhetherMetAlready(
        &uroom->playerList->players[taskData[1]]);
    playerGender = GetUnionRoomPlayerGender(taskData[1], uroom->playerList);
  ...
  case UR_STATE_WAIT_FINISH_READING_CARD:
    if (IsLinkTaskFinished()) {
      if (GetMultiplayerId() == 0) {
        ...
        id = PlayerHasMetTrainerBefore(gLinkPlayers[1].trainerId,
                                       gLinkPlayers[1].name);
  ...
  case UR_STATE_RECV_ACTIVITY_REQUEST:
    ScheduleFieldMessageWithFollowupState(UR_STATE_HANDLE_ACTIVITY_REQUEST,
                                          gStringVar4);
    break;
  case UR_STATE_HANDLE_ACTIVITY_REQUEST:
    ...
    case 0: // ACCEPT
      ...
      if (gPlayerCurrActivity == (ACTIVITY_CHAT | IN_UNION_ROOM))
        UpdateGameData_SetActivity(gPlayerCurrActivity | IN_UNION_ROOM,
                                   GetLinkPlayerInfoFlags(1), FALSE);
      else
        UpdateGameData_SetActivity(gPlayerCurrActivity | IN_UNION_ROOM,
                                   GetLinkPlayerInfoFlags(1), TRUE);
  ...
  case UR_STATE_PRINT_START_ACTIVITY_MSG:
    ...
    ScheduleFieldMessageWithFollowupState(UR_STATE_START_ACTIVITY_LINK,
                                          gStringVar4);
    break;
    ...
    if (GetHostRfuGameData()->tradeSpecies == SPECIES_NONE) {
      ...
      if (GetHostRfuGameData()->tradeSpecies == SPECIES_EGG) {
        ...
        ConvertIntToDecimalStringN(gStringVar2,
                                   GetHostRfuGameData()->tradeLevel,
                                   STR_CONV_MODE_LEFT_ALIGN, 3);
        StringExpandPlaceholders(gStringVar4, sText_CancelRegistrationOfMon);
      }
      ScheduleFieldMessageWithFollowupState(UR_STATE_CANCEL_REGISTRATION_PROMPT,
                                            gStringVar4);
    }
    break;
  ...
  case UR_STATE_REGISTER_PROMPT_HANDLE_INPUT:
    input = ListMenuHandler_AllItemsAvailable(
        &uroom->textState, &uroom->tradeBoardMainWindowId,
        &uroom->tradeBoardHeaderWindowId, &sWindowTemplate_RegisterForTrade,
  ...
  case UR_STATE_REGISTER_SELECT_MON:
    if (!gPaletteFade.active) {
      ...
      ChooseMonForTradingBoard(PARTY_MENU_TYPE_UNION_ROOM_REGISTER,
                               CB2_ReturnToField);
    }
    break;
  ...
  case UR_STATE_TRADING_BOARD_HANDLE_INPUT:
    input = TradeBoardMenuHandler(
        &uroom->textState, &uroom->tradeBoardMainWindowId,
        &uroom->tradeBoardListMenuId, &uroom->tradeBoardHeaderWindowId,
        &sWindowTemplate_TradingBoardMain, &sTradeBoardListMenuTemplate,
        uroom->playerList);
    if (input != LIST_NOTHING_CHOSEN) {
      switch (input) {
      case LIST_CANCEL:
      ...
      default:
        UR_ClearBg0();
        switch (IsRequestedTradeInPlayerParty(
            uroom->playerList->players[input].rfu.data.tradeType,
            uroom->playerList->players[input].rfu.data.tradeSpecies)) {
        case UR_TRADE_MATCH:
          CopyAndTranslatePlayerName(gStringVar1,
        ...
        case UR_TRADE_NOTYPE:
          ...
          StringCopy(
              gStringVar2,
              gTypeNames[uroom->playerList->players[input].rfu.data.tradeType]);
  ...
  case UR_STATE_TRADE_SELECT_MON:
    if (PrintOnTextbox(&uroom->textState, sText_WhichMonWillYouOffer)) {
      ...
      memcpy(&gRfuPartnerCompatibilityData,
             &uroom->playerList->players[taskData[1]].rfu.data.compatibility,
             sizeof(gRfuPartnerCompatibilityData));
      gUnionRoomRequestedMonType =
          uroom->playerList->players[taskData[1]].rfu.data.tradeType;
      gUnionRoomOfferedSpecies =
          uroom->playerList->players[taskData[1]].rfu.data.tradeSpecies;
      gFieldCallback = FieldCB_ContinueScriptUnionRoom;
      ChooseMonForTradingBoard(PARTY_MENU_TYPE_UNION_ROOM_TRADE,
                               CB2_ReturnToField);
  ...
  case UR_STATE_TRADE_OFFER_MON:
    ...
    TryConnectToUnionRoomParent(
        uroom->playerList->players[taskData[1]].rfu.name,
        &uroom->playerList->players[taskData[1]].rfu.data, gPlayerCurrActivity);
    CopyAndTranslatePlayerName(gStringVar1,
                               &uroom->playerList->players[taskData[1]]);
...
static void ReceiveUnionRoomActivityPacket(struct WirelessLink_URoom *data) {
  if (gRecvCmds[1][1] != 0 &&
      (gRecvCmds[1][0] & RFUCMD_MASK) == RFUCMD_SEND_PACKET) {
...
static bool32 HandleContactFromOtherPlayer(struct WirelessLink_URoom *uroom) {
  if (uroom->recvActivityRequest[0] != 0) {
    s32 id = GetChatLeaderActionRequestMessage(
        gStringVar4, gLinkPlayers[1].gender, &uroom->recvActivityRequest[0],
        uroom);
    if (id == 0) // Error
    {
      return TRUE;
...
void InitUnionRoom(void) {
  ...
  sWirelessLinkMain.uRoom = data =
      AllocZeroed(sizeof(struct WirelessLink_URoom));
        ...
        for (i = 0; i < MAX_UNION_ROOM_LEADERS; i++) {
          if (data->playerList->players[i].groupScheduledAnim ==
            ...
            if (PlayerHasMetTrainerBefore(
                    ReadAsU16(data->playerList->players[i]
                                  .rfu.data.compatibility.playerTrainerId),
                    text)) {
...
static u8 HandlePlayerListUpdate(void) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; i++) {
    if (ArePlayersDifferent(&data->incomingParentList->players[i].rfu,
                            &sUnionRoomPlayer_DummyRfu) == TRUE) {
      data->spawnPlayer->players[0].rfu =
          data->incomingParentList->players[i].rfu;
  ...
  for (j = 0; j < MAX_UNION_ROOM_LEADERS; j++) {
    if (data->playerList->players[j].groupScheduledAnim !=
        UNION_ROOM_SPAWN_NONE) {
      i = GetNewIncomingPlayerId(&data->playerList->players[j],
                                 &data->incomingChildList->players[0]);
      if (i != 0xFF) {
        if (data->playerList->players[j].groupScheduledAnim ==
            UNION_ROOM_SPAWN_IN) {
          if (ArePlayerDataDifferent(
      ...
      } else if (data->playerList->players[j].groupScheduledAnim !=
                 UNION_ROOM_SPAWN_OUT) {
        data->playerList->players[j].timeoutCounter++;
        if (data->playerList->players[j].timeoutCounter >= 600) {
          data->playerList->players[j].groupScheduledAnim =
              UNION_ROOM_SPAWN_OUT;
          retVal = PLIST_RECENT_UPDATE;
        }
      } else if (data->playerList->players[j].groupScheduledAnim ==
  ...
  for (i = 0; i < RFU_CHILD_MAX; i++)
    if (TryAddIncomingPlayerToList(&data->playerList->players[0],
                                   &data->incomingChildList->players[i],
                                   MAX_UNION_ROOM_LEADERS) != 0xFF)
      retVal = PLIST_NEW_PLAYER;
...
static void Task_SearchForChildOrParent(u8 taskId) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; i++) {
    ...
    if (!isParent) {
      ...
      list[1]->players[i].active = ArePlayersDifferent(
          &list[1]->players[i].rfu, &sUnionRoomPlayer_DummyRfu);
    } else {
      list[0]->players[i].rfu = rfu;
      list[0]->players[i].active = ArePlayersDifferent(
...
static u8
CreateTask_SearchForChildOrParent(struct RfuIncomingPlayerList *parentList,
                                  struct RfuIncomingPlayerList *childList,
                                  u32 linkGroup) {
...
static void Task_ListenForCompatiblePartners(u8 taskId) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; i++) {
    Rfu_GetCompatiblePlayerData(&list[0]->players[i].rfu.data,
                                list[0]->players[i].rfu.name, i);
    if (!IsPartnerActivityAcceptable(list[0]->players[i].rfu.data.activity,
                                     gTasks[taskId].data[2])) {
      list[0]->players[i].rfu = sUnionRoomPlayer_DummyRfu;
    }
    for (j = 0; j < i; j++) {
      if (!ArePlayersDifferent(&list[0]->players[j].rfu,
                               &list[0]->players[i].rfu))
        list[0]->players[i].rfu = sUnionRoomPlayer_DummyRfu;
    }
    list[0]->players[i].active = ArePlayersDifferent(
        &list[0]->players[i].rfu, &sUnionRoomPlayer_DummyRfu);
...
static bool32 HasWonderCardOrNewsByLinkGroup(struct RfuGameData *data,
                                             s16 linkGroup) {
...
static void Task_ListenForWonderDistributor(u8 taskId) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; i++) {
    if (Rfu_GetWonderDistributorPlayerData(&list[0]->players[i].rfu.data,
                                           list[0]->players[i].rfu.name, i))
      HasWonderCardOrNewsByLinkGroup(&list[0]->players[i].rfu.data,
                                     gTasks[taskId].data[2]);
...
static u8
CreateTask_ListenForCompatiblePartners(struct RfuIncomingPlayerList *list,
                                       u32 linkGroup) {
...
static u8
CreateTask_ListenForWonderDistributor(struct RfuIncomingPlayerList *list,
                                      u32 linkGroup) {
...
static u8 CreateTradeBoardWindow(const struct WindowTemplate *template) {
  ...
  PrintUnionRoomText(windowId, FONT_NORMAL, sText_NameWantedOfferLv, 8, 1,
                     UR_COLOR_TRADE_BOARD_OTHER);
  CopyWindowToVram(windowId, COPYWIN_GFX);
...
static void DeleteTradeBoardWindow(u8 windowId) { RemoveWindow(windowId); }

static s32
ListMenuHandler_AllItemsAvailable(u8 *state, u8 *windowId, u8 *listMenuId,
  ...
  case 0:
    ...
    if (winTemplateCopy.tilemapLeft + winTemplateCopy.width >=
        DISPLAY_TILE_WIDTH)
      winTemplateCopy.tilemapLeft =
          max(DISPLAY_TILE_WIDTH - 1 - winTemplateCopy.width, 0);
...
static s32 TradeBoardMenuHandler(u8 *state, u8 *mainWindowId, u8 *listMenuId,
                                 u8 *headerWindowId,
                                 const struct WindowTemplate *winTemplate,
                                 const struct ListMenuTemplate *menuTemplate,
  ...
  case 0:
    *headerWindowId =
        CreateTradeBoardWindow(&sWindowTemplate_TradingBoardHeader);
...
static void JoinGroup_EnableScriptContexts(void) { ScriptContext_Enable(); }

static void PrintUnionRoomText(u8 windowId, u8 fontId, const u8 *str, u8 x,
                               u8 y, u8 colorIdx) {
...
static bool8 ArePlayersDifferent(struct RfuPlayerData *player1,
                                 const struct RfuPlayerData *player2) {
  s32 i;
...
static bool32 ArePlayerDataDifferent(struct RfuPlayerData *player1,
                                     struct RfuPlayerData *player2) {
  s32 i;
...
static u32 GetNewIncomingPlayerId(struct RfuPlayer *player,
                                  struct RfuIncomingPlayer *incomingPlayer) {
  u8 result = 0xFF; // None
  s32 i;

  for (i = 0; i < RFU_CHILD_MAX; i++) {
    if (incomingPlayer[i].active &&
        !ArePlayersDifferent(&player->rfu, &incomingPlayer[i].rfu)) {
      result = i;
...
static u8 TryAddIncomingPlayerToList(struct RfuPlayer *players,
                                     struct RfuIncomingPlayer *incomingPlayer,
                                     u8 max) {
  s32 i;
...
static void PrintGroupMemberOnWindow(u8 windowId, u8 x, u8 y,
                                     struct RfuPlayer *player, u8 colorIdx,
                                     u8 id) {
  u8 activity;
  u8 trainerId[6];

  ConvertIntToDecimalStringN(gStringVar4, id + 1, STR_CONV_MODE_LEADING_ZEROS,
                             2);
  StringAppend(gStringVar4, sText_Colon);
  PrintUnionRoomText(windowId, FONT_NORMAL, gStringVar4, x, y,
                     UR_COLOR_DEFAULT);
  x += 18;
  activity = player->rfu.data.activity;
  if (player->groupScheduledAnim == UNION_ROOM_SPAWN_IN &&
      !(activity & IN_UNION_ROOM)) {
    CopyAndTranslatePlayerName(gStringVar4, player);
    PrintUnionRoomText(windowId, FONT_NORMAL, gStringVar4, x, y, colorIdx);
    ConvertIntToDecimalStringN(
        trainerId,
        player->rfu.data.compatibility.playerTrainerId[0] |
            (player->rfu.data.compatibility.playerTrainerId[1] << 8),
        STR_CONV_MODE_LEADING_ZEROS, 5);
    StringCopy(gStringVar4, sText_ID);
    StringAppend(gStringVar4, trainerId);
    PrintUnionRoomText(
        windowId, FONT_NORMAL, gStringVar4,
        GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 0x88), y,
        colorIdx);
  }
}

static void PrintGroupCandidateOnWindow(u8 windowId, u8 x, u8 y,
                                        struct RfuPlayer *player, u8 colorIdx,
                                        u8 id) {
  u8 trainerId[6];
  ...
  if (player->groupScheduledAnim == UNION_ROOM_SPAWN_IN) {
    ...
    PrintUnionRoomText(
        windowId, FONT_NORMAL, gStringVar4,
        GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 0x68), y,
        colorIdx);
  }
}
...
static u32 ConvPartnerUnameAndGetWhetherMetAlready(struct RfuPlayer *player) {
  ...
  return PlayerHasMetTrainerBefore(
      ReadAsU16(player->rfu.data.compatibility.playerTrainerId), name);
}

static s32 UnionRoomGetPlayerInteractionResponse(struct RfuPlayerList *list,
                                                 bool8 overrideGender,
                                                 u8 playerIdx,
                                                 u32 playerGender) {
  bool32 metBefore;
  ...
  if (!player->rfu.data.startedActivity && !overrideGender) {
    ...
    metBefore = PlayerHasMetTrainerBefore(
        ReadAsU16(player->rfu.data.compatibility.playerTrainerId), gStringVar1);
    ...
    if (overrideGender) {
      playerGender =
          (player->rfu.data.compatibility.playerTrainerId[overrideGender + 1] >>
           3) &
          1;
    }
    ...
    case ACTIVITY_BATTLE_SINGLE:
      StringExpandPlaceholders(
          ...
          sBattleReactionTexts[playerGender]
                              [Random() %
                               ARRAY_COUNT(sBattleReactionTexts[0])]);
      break;
    case ACTIVITY_TRADE:
      StringExpandPlaceholders(gStringVar4,
                               sTradeReactionTexts[playerGender][Random() % 2]);
      break;
    case ACTIVITY_CHAT:
      StringExpandPlaceholders(
          gStringVar4,
...
static void TradeBoardPrintItemInfo(u8 windowId, u8 y, struct RfuGameData *data,
                                    const u8 *playerName, u8 colorIdx) {
  u8 levelStr[4];
  ...
  if (species == SPECIES_EGG) {
    ...
    PrintUnionRoomText(windowId, FONT_NORMAL, gSpeciesNames[species], 118, y,
                       colorIdx);
...
static void TradeBoardListMenuItemPrintFunc(u8 windowId, u32 itemId, u8 y) {
  ...
  if (itemId == LIST_HEADER && y == sTradeBoardListMenuTemplate.upText_Y) {
    ...
    if (gameData->tradeSpecies != SPECIES_NONE)
      TradeBoardPrintItemInfo(windowId, y, gameData, gSaveBlock2Ptr->playerName,
                              UR_COLOR_TRADE_BOARD_SELF);
  } else {
    j = 0;
    for (i = 0; i < MAX_UNION_ROOM_LEADERS; i++) {
      if (leader->playerList->players[i].groupScheduledAnim ==
              UNION_ROOM_SPAWN_IN &&
      ...
      if (j == itemId + 1) {
        ...
        TradeBoardPrintItemInfo(windowId, y,
                                &leader->playerList->players[i].rfu.data,
                                playerName, UR_COLOR_TRADE_BOARD_OTHER);
        break;
      }
    }
  }
...
static s32 GetChatLeaderActionRequestMessage(u8 *dst, u32 gender,
                                             u16 *activityData,
                                             struct WirelessLink_URoom *uroom) {
  s32 result = 0;
  ...
  case ACTIVITY_TRADE | IN_UNION_ROOM:
    ConvertIntToDecimalStringN(uroom->activityRequestStrbufs[0],
                               sUnionRoomTrade.playerLevel,
                               STR_CONV_MODE_LEFT_ALIGN, 3);
    StringCopy(uroom->activityRequestStrbufs[1],
    ...
    for (i = 0; i < RFU_CHILD_MAX; i++) {
      if (gRfuLinkStatus->partner[i].serialNo == RFU_SERIAL_GAME) {
        ConvertIntToDecimalStringN(uroom->activityRequestStrbufs[2],
                                   activityData[2], STR_CONV_MODE_LEFT_ALIGN,
                                   3);
        StringCopy(uroom->activityRequestStrbufs[3],
                   gSpeciesNames[activityData[1]]);
...
void Script_ResetUnionRoomTrade(void) { ResetUnionRoomTrade(&sUnionRoomTrade); }

static bool32 RegisterTradeMonAndGetIsEgg(u32 monId,
                                          struct UnionRoomTrade *trade) {
  trade->playerSpecies =
      GetMonData(&gPlayerParty[monId], MON_DATA_SPECIES_OR_EGG);
  trade->playerLevel = GetMonData(&gPlayerParty[monId], MON_DATA_LEVEL);
  trade->playerPersonality =
      GetMonData(&gPlayerParty[monId], MON_DATA_PERSONALITY);
...
static u32 GetPartyPositionOfRegisteredMon(struct UnionRoomTrade *trade,
                                           u8 multiplayerId) {
  u16 response = 0;
  u16 species;
  u32 personality;
...
static u8 GetActivePartnersInfo(struct WirelessLink_URoom *data) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; i++) {
    if (data->incomingParentList->players[i].active) {
      retVal |= data->incomingParentList->players[i].rfu.data.playerGender
                << PINFO_GENDER_SHIFT;
      retVal |= data->incomingParentList->players[i]
                    .rfu.data.compatibility.playerTrainerId[0] &
                PINFO_TID_MASK;
      break;
    }
  }
...
static void ViewURoomPartnerTrainerCard(u8 *unused,
                                        struct WirelessLink_URoom *data,
                                        bool8 isParent) {
  ...
  StringCopy(data->trainerCardStrBuffer[0],
             gTrainerClassNames[GetUnionRoomTrainerClass()]);
  DynamicPlaceholderTextUtil_SetPlaceholderPtr(0,
                                               data->trainerCardStrBuffer[0]);

  DynamicPlaceholderTextUtil_SetPlaceholderPtr(1, trainerCard->playerName);

  StringCopy(data->trainerCardColorStrBuffer,
             sCardColorTexts[trainerCard->stars]);
  DynamicPlaceholderTextUtil_SetPlaceholderPtr(2,
                                               data->trainerCardColorStrBuffer);

  ConvertIntToDecimalStringN(data->trainerCardStrBuffer[2],
                             trainerCard->caughtMonsCount,
                             STR_CONV_MODE_LEFT_ALIGN, 3);
  DynamicPlaceholderTextUtil_SetPlaceholderPtr(3,
                                               data->trainerCardStrBuffer[2]);

  ConvertIntToDecimalStringN(data->trainerCardStrBuffer[3],
  ...
  DynamicPlaceholderTextUtil_SetPlaceholderPtr(4,
                                               data->trainerCardStrBuffer[3]);
  DynamicPlaceholderTextUtil_SetPlaceholderPtr(5,
                                               data->trainerCardStrBuffer[4]);
  ...
  if (n > 9999)
    n = 9999;
  ConvertIntToDecimalStringN(data->trainerCardStrBuffer[0], n,
  ...
  if (n > 9999)
    n = 9999;
  ConvertIntToDecimalStringN(data->trainerCardStrBuffer[1], n,
                             STR_CONV_MODE_LEFT_ALIGN, 4);
  DynamicPlaceholderTextUtil_SetPlaceholderPtr(2,
                                               data->trainerCardStrBuffer[1]);
  ...
  for (i = 0; i < TRAINER_CARD_PROFILE_LENGTH; i++) {
    CopyEasyChatWord(data->trainerCardStrBuffer[i + 3],
                     trainerCard->easyChatProfile[i]);
    DynamicPlaceholderTextUtil_SetPlaceholderPtr(
        i + 4, data->trainerCardStrBuffer[i + 3]);
  }
