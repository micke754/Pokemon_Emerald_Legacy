...
struct FrontierPassData {
  ...
  u8 areaToShow : 3;
  u8 trainerStars : 4;
  u8 facilitySymbols[NUM_FRONTIER_FACILITIES]; // 0: no symbol, 1: silver, 2:
                                               // gold
};
...
static const u16 sMaleHead_Pal[] =
    INCBIN_U16("graphics/frontier_pass/map_heads.gbapal");
static const u16 sFemaleHead_Pal[] =
    INCBIN_U16("graphics/frontier_pass/map_heads_female.gbapal");
static const u32 sMapScreen_Gfx[] =
    INCBIN_U32("graphics/frontier_pass/map_screen.4bpp.lz");
static const u32 sCursor_Gfx[] =
    INCBIN_U32("graphics/frontier_pass/cursor.4bpp.lz");
static const u32 sHeads_Gfx[] =
    INCBIN_U32("graphics/frontier_pass/map_heads.4bpp.lz");
static const u32 sMapCursor_Gfx[] =
    INCBIN_U32("graphics/frontier_pass/map_cursor.4bpp.lz");
static const u32 sMapScreen_Tilemap[] =
    INCBIN_U32("graphics/frontier_pass/map_screen.bin.lz");
static const u32 sMapAndCard_ZoomedOut_Tilemap[] =
    INCBIN_U32("graphics/frontier_pass/small_map_and_card.bin.lz");
static const u32 sCardBall_Filled_Tilemap[] =
    INCBIN_U32("graphics/frontier_pass/card_ball_filled.bin"); // Unused
static const u32 sBattleRecord_Tilemap[] =
    INCBIN_U32("graphics/frontier_pass/record_frame.bin.lz");
static const u32 sMapAndCard_Zooming_Tilemap[] =
    INCBIN_U32("graphics/frontier_pass/small_map_and_card_affine.bin.lz");

static const s16 sBgAffineCoords[][2] = {[CURSOR_AREA_MAP - 1] = {216, 32},
...
static const union AnimCmd sAnim_Frame1_Unused[] = {ANIMCMD_FRAME(0, 0),
                                                    ANIMCMD_END};

static const union AnimCmd sAnim_Frame1[] = {ANIMCMD_FRAME(0, 0), ANIMCMD_END};

static const union AnimCmd sAnim_Frame2[] = {ANIMCMD_FRAME(4, 0), ANIMCMD_END};

static const union AnimCmd sAnim_Frame3[] = {ANIMCMD_FRAME(8, 0), ANIMCMD_END};

static const union AnimCmd sAnim_Frame4[] = {ANIMCMD_FRAME(12, 0), ANIMCMD_END};

static const union AnimCmd sAnim_Frame5[] = {ANIMCMD_FRAME(16, 0), ANIMCMD_END};

static const union AnimCmd sAnim_Frame6[] = {ANIMCMD_FRAME(20, 0), ANIMCMD_END};

static const union AnimCmd sAnim_Frame7[] = {ANIMCMD_FRAME(24, 0), ANIMCMD_END};
...
static const union AnimCmd *const sAnims_TwoFrame[] = {sAnim_Frame1,
                                                       sAnim_Frame2};
...
static const u8 *const sPassAreaDescriptions[CURSOR_AREA_COUNT + 1] = {
    [CURSOR_AREA_NOTHING] =
        gText_ThereIsNoBattleRecord, // NOTHING is re-used for
                                     // CURSOR_AREA_RECORD when no Record is
                                     // present
...
struct {
  ...
  u8 animNum;
    ...
    [FRONTIER_FACILITY_TOWER] = {gText_BattleTower3, gText_BattleTowerDesc, 89,
                                 40, MAP_INDICATOR_SQUARE},
    [FRONTIER_FACILITY_DOME] = {gText_BattleDome2, gText_BattleDomeDesc, 33, 42,
                                MAP_INDICATOR_SQUARE},
    [FRONTIER_FACILITY_PALACE] = {gText_BattlePalace2, gText_BattlePalaceDesc,
                                  120, 86, MAP_INDICATOR_RECTANGLE},
    [FRONTIER_FACILITY_ARENA] = {gText_BattleArena2, gText_BattleArenaDesc, 114,
                                 59, MAP_INDICATOR_RECTANGLE},
    [FRONTIER_FACILITY_FACTORY] = {gText_BattleFactory2,
                                   gText_BattleFactoryDesc, 25, 67,
                                   MAP_INDICATOR_RECTANGLE},
    [FRONTIER_FACILITY_PIKE] = {gText_BattlePike2, gText_BattlePikeDesc, 57, 57,
                                MAP_INDICATOR_SQUARE},
    [FRONTIER_FACILITY_PYRAMID] = {gText_BattlePyramid2,
                                   gText_BattlePyramidDesc, 134, 41,
                                   MAP_INDICATOR_SQUARE},
};
...
static u32 FreeFrontierPassData(void) {
  ...
  memset(
      sPassData, 0,
      sizeof(*sPassData)); // Why clear data, if it's going to be freed anyway?
...
static u32 FreeFrontierPassGfx(void) {
  ...
  memset(
      sPassGfx, 0,
      sizeof(*sPassGfx)); // Why clear data, if it's going to be freed anyway?
...
static void VBlankCB_FrontierPass(void) {
  if (sPassGfx->zooming) {
    SetBgAffine(2, sBgAffineCoords[sPassData->areaToShow - 1][0] << 8,
                sBgAffineCoords[sPassData->areaToShow - 1][1] << 8,
                sBgAffineCoords[sPassData->areaToShow - 1][0],
                sBgAffineCoords[sPassData->areaToShow - 1][1], sPassGfx->scaleX,
                sPassGfx->scaleY, 0);
  }
  LoadOam();
...
static bool32 InitFrontierPass(void) {
  ...
  case 5:
    ...
    break;
  case 6:
    sPassGfx->mapAndCardZoomTilemap =
        malloc_and_decompress(sMapAndCard_Zooming_Tilemap, &sizeOut);
    sPassGfx->mapAndCardTilemap =
        malloc_and_decompress(sMapAndCard_ZoomedOut_Tilemap, &sizeOut);
    sPassGfx->battleRecordTilemap =
        malloc_and_decompress(sBattleRecord_Tilemap, &sizeOut);
  ...
  case 7:
    ...
    FillBgTilemapBufferRect_Palette0(0, 0, 0, 0, DISPLAY_TILE_WIDTH,
                                     DISPLAY_TILE_HEIGHT);
    FillBgTilemapBufferRect_Palette0(1, 0, 0, 0, DISPLAY_TILE_WIDTH,
                                     DISPLAY_TILE_HEIGHT);
    FillBgTilemapBufferRect_Palette0(2, 0, 0, 0, DISPLAY_TILE_WIDTH,
                                     DISPLAY_TILE_HEIGHT);
  ...
  case 8:
    ...
    if (sPassData->trainerStars == 5) {
      ...
      LoadPalette(gFrontierPassBg_Pal[1 + sPassData->trainerStars],
...
static u8 GetCursorAreaFromCoords(s16 x, s16 y) {
  ...
  // Minus/Plus 1, because the table doesn't take CURSOR_AREA_NOTHING into
  // account.
...
static void Task_HandleFrontierPassInput(u8 taskId) {
  u8 var = FALSE; // Reused, first informs whether the cursor moves, then used
                  // as the new cursor area.
  ...
  if (!var) // Cursor did not change.
  {
    ...
    var = GetCursorAreaFromCoords(sPassGfx->cursorSprite->x - 5,
...
static void ShowAndPrintWindows(void) {
  ...
  AddTextPrinterParameterized3(WINDOW_EARNED_SYMBOLS, FONT_NORMAL, x, 5,
                               sTextColors[0], 0, gText_SymbolsEarned);

  x = GetStringCenterAlignXOffset(FONT_NORMAL, gText_BattleRecord, 96);
  AddTextPrinterParameterized3(WINDOW_BATTLE_RECORD, FONT_NORMAL, x, 5,
                               sTextColors[0], 0, gText_BattleRecord);

  AddTextPrinterParameterized3(WINDOW_BATTLE_POINTS, FONT_SMALL_NARROW, 5, 4,
                               sTextColors[0], 0, gText_BattlePoints);
  ConvertIntToDecimalStringN(gStringVar4, sPassData->battlePoints,
                             STR_CONV_MODE_LEFT_ALIGN, 5);
  x = GetStringRightAlignXOffset(FONT_SMALL_NARROW, gStringVar4, 91);
  AddTextPrinterParameterized3(WINDOW_BATTLE_POINTS, FONT_SMALL_NARROW, x, 16,
                               sTextColors[0], 0, gStringVar4);
...
static void PrintAreaDescription(u8 cursorArea) {
  ...
  if (cursorArea == CURSOR_AREA_RECORD && !sPassData->hasBattleRecord)
    AddTextPrinterParameterized3(WINDOW_DESCRIPTION, FONT_NORMAL, 2, 0,
                                 sTextColors[1], 0,
                                 sPassAreaDescriptions[CURSOR_AREA_NOTHING]);
...
static void ShowHideZoomingArea(bool8 show, bool8 zoomedIn) {
  ...
  case CURSOR_AREA_MAP:
    if (show)
      CopyToBgTilemapBufferRect_ChangePalette(
          2, sPassGfx->mapAndCardZoomTilemap, 16, 3, 12, 7, 16);
    else
  ...
  case CURSOR_AREA_CARD:
    if (show)
      CopyToBgTilemapBufferRect_ChangePalette(
          2, sPassGfx->mapAndCardZoomTilemap + 84, 16, 10, 12, 7, 16);
    else
...
#define NON_HIGHLIGHT_AREA(area)                                               \
  ((area) == CURSOR_AREA_NOTHING || (area) > CURSOR_AREA_CANCEL)

  // If moving off highlightable area, unhighlight it
  switch (previousCursorArea) {
  case CURSOR_AREA_MAP:
    CopyToBgTilemapBufferRect_ChangePalette(1, sPassGfx->mapAndCardTilemap, 16,
                                            3, 12, 7, 17);
    break;
  case CURSOR_AREA_CARD:
    CopyToBgTilemapBufferRect_ChangePalette(
        1, sPassGfx->mapAndCardTilemap + 336, 16, 10, 12, 7, 17);
    break;
  case CURSOR_AREA_RECORD:
    if (sPassData->hasBattleRecord)
      CopyToBgTilemapBufferRect_ChangePalette(1, sPassGfx->battleRecordTilemap,
                                              2, 10, 12, 3, 17);
  ...
  case CURSOR_AREA_CANCEL:
    CopyToBgTilemapBufferRect_ChangePalette(
        1, gFrontierPassCancelButton_Tilemap, 21, 0, 9, 2, 17);
    break;
  default:
  ...
  case CURSOR_AREA_MAP:
    CopyToBgTilemapBufferRect_ChangePalette(
        1, sPassGfx->mapAndCardTilemap + 168, 16, 3, 12, 7, 17);
    break;
  case CURSOR_AREA_CARD:
    CopyToBgTilemapBufferRect_ChangePalette(
        1, sPassGfx->mapAndCardTilemap + 504, 16, 10, 12, 7, 17);
    break;
  case CURSOR_AREA_RECORD:
    if (sPassData->hasBattleRecord)
      CopyToBgTilemapBufferRect_ChangePalette(
          1, sPassGfx->battleRecordTilemap + 72, 2, 10, 12, 3, 17);
    else
      return;
    break;
  case CURSOR_AREA_CANCEL:
    CopyToBgTilemapBufferRect_ChangePalette(
        1, gFrontierPassCancelButtonHighlighted_Tilemap, 21, 0, 9, 2, 17);
    break;
  default:
...
static void DrawFrontierPassBg(void) {
  ...
  UpdateAreaHighlight(sPassData->cursorArea, sPassData->previousCursorArea);
  ShowHideZoomingArea(
      TRUE, sPassData->areaToShow); // If returning to frontier pass from
                                    // map/card (areaToShow will be != 0)
  ShowAndPrintWindows();
  CopyBgTilemapBufferToVram(1);
}
...
static void LoadCursorAndSymbolSprites(void) {
  ...
  spriteId = CreateSprite(&sSpriteTemplates_Cursors[0], sPassData->cursorX,
                          sPassData->cursorY, 0);
  ...
  for (i = 0; i < NUM_FRONTIER_FACILITIES; i++) {
    if (sPassData->facilitySymbols[i] != 0) {
      ...
      sprite.paletteTag += sPassData->facilitySymbols[i] -
                           1; // Adds 1 if gold for TAG_MEDAL_GOLD
      spriteId = CreateSprite(
          &sprite, sPassAreasLayout[i + CURSOR_AREA_SYMBOL - 1].xStart + 8,
...
static void ShowFrontierMap(void (*callback)(void)) {
  if (sMapData != NULL)
    SetMainCallback2(callback); // This line doesn't make sense at all, since it
...
static bool32 InitFrontierMap(void) {
  ...
  case 5:
    if (FreeTempTileDataBuffersIfPossible())
      return FALSE;
    LoadPalette(gFrontierPassBg_Pal, BG_PLTT_ID(0),
                NUM_BG_PAL_SLOTS * PLTT_SIZE_4BPP);
    LoadPalette(GetTextWindowPalette(0), BG_PLTT_ID(15), PLTT_SIZE_4BPP);
...
static u8 MapNumToFrontierFacilityId(
    u16 mapNum) // id + 1, zero means not a frontier map number
{
  // In Battle Tower
...
static void InitFrontierMapSprites(void) {
  ...
  LoadCompressedSpriteSheet(&sCursorSpriteSheets[0]);
  spriteId = CreateSprite(&sSpriteTemplates_Cursors[0], 155,
                          (sMapData->cursorPos * 16) + 8, 2);
  ...
  StartSpriteAnim(sMapData->mapIndicatorSprite,
                  sMapLandmarks[sMapData->cursorPos].animNum);

  // Create player indicator head sprite only if it's in vicinity of battle
  // frontier.
  ...
  if (id == MAPSEC_BATTLE_FRONTIER || id == MAPSEC_ARTISAN_CAVE) {
    ...
    if (mapNum == MAP_NUM(BATTLE_FRONTIER_OUTSIDE_WEST) ||
      ...
      if (id != 0) {
        ...
        // Handle Artisan Cave.
        if (gSaveBlock1Ptr->escapeWarp.mapNum ==
    ...
    sprite.paletteTag = gSaveBlock2Ptr->playerGender +
                        TAG_HEAD_MALE; // TAG_HEAD_FEMALE if gender is FEMALE
    if (id != 0) {
...
static void PrintOnFrontierMap(void) {
  ...
  for (i = 0; i < NUM_FRONTIER_FACILITIES; i++) {
    if (i == sMapData->cursorPos)
      AddTextPrinterParameterized3(MAP_WINDOW_NAME, FONT_NARROW, 4,
                                   (i * 16) + 1, sTextColors[2], 0,
                                   sMapLandmarks[i].name);
    else
      AddTextPrinterParameterized3(MAP_WINDOW_NAME, FONT_NARROW, 4,
                                   (i * 16) + 1, sTextColors[1], 0,
                                   sMapLandmarks[i].name);
  }

  AddTextPrinterParameterized3(MAP_WINDOW_DESCRIPTION, FONT_NORMAL, 4, 0,
                               sTextColors[0], 0,
...
static void HandleFrontierMapCursorMove(u8 direction) {
  ...
  AddTextPrinterParameterized3(MAP_WINDOW_NAME, FONT_NARROW, 4,
                               (oldCursorPos * 16) + 1, sTextColors[1], 0,
                               sMapLandmarks[oldCursorPos].name);
  AddTextPrinterParameterized3(MAP_WINDOW_NAME, FONT_NARROW, 4,
                               (sMapData->cursorPos * 16) + 1, sTextColors[2],
                               0, sMapLandmarks[sMapData->cursorPos].name);
