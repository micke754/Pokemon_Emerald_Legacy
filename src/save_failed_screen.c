...
static const struct OamData sClockOamData = {.y = DISPLAY_HEIGHT,
...
static const struct WindowTemplate sDummyWindowTemplate[] = {
    DUMMY_WIN_TEMPLATE};
...
static const u8 sSaveFailedClockPal[] =
    INCBIN_U8("graphics/misc/clock_small.gbapal");
static const u32 sSaveFailedClockGfx[] =
    INCBIN_U32("graphics/misc/clock_small.4bpp.lz");
...
static void SaveFailedScreenTextPrint(const u8 *text, u8 x, u8 y) {
  ...
  AddTextPrinterParameterized4(sWindowIds[TEXT_WIN_ID], FONT_NORMAL, x * 8,
                               y * 8 + 1, 0, 0, color, 0, text);
}
...
static void CB2_SaveFailedScreen(void) {
  ...
  default:
    ...
    SetWindowAttribute(sWindowIds[TEXT_WIN_ID], 7,
                       (u32)&gDecompressionBuffer[0x2800]);
    sWindowIds[CLOCK_WIN_ID] = AddWindowWithoutTileMap(sWindowTemplate_Clock);
    SetWindowAttribute(sWindowIds[CLOCK_WIN_ID], 7,
                       (u32)&gDecompressionBuffer[0x3D00]);
    ...
    DrawStdFrameWithCustomTileAndPalette(sWindowIds[TEXT_WIN_ID], FALSE, 0x214,
                                         0xE);
    DrawStdFrameWithCustomTileAndPalette(sWindowIds[CLOCK_WIN_ID], FALSE, 0x214,
                                         0xE);
    FillWindowPixelBuffer(sWindowIds[CLOCK_WIN_ID],
                          PIXEL_FILL(1)); // backwards?
...
static void CB2_ReturnToTitleScreen(void) {
  if (!UpdatePaletteFade()) {
    if (gGameContinueCallback ==
        NULL) // no callback exists, so do a soft reset.
    {
      DoSoftReset();
    } else {
...
static void VBlankCB_UpdateClockGraphics(void) {
  ...
  if (sClockInfo[CLOCK_RUNNING]) {
    ...
    gMain.oamBuffer[0].matrixNum =
        (sClockFrames[n][2] << 4) | (sClockFrames[n][1] << 3);
  } else {
