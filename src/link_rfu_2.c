...
static const u32 sAllBlocksReceived[] = {
    BLOCK_MASK(0),  BLOCK_MASK(1),  BLOCK_MASK(2),  BLOCK_MASK(3),
    BLOCK_MASK(4),  BLOCK_MASK(5),  BLOCK_MASK(6),  BLOCK_MASK(7),
    BLOCK_MASK(8),  BLOCK_MASK(9),  BLOCK_MASK(10), BLOCK_MASK(11),
    BLOCK_MASK(12), BLOCK_MASK(13), BLOCK_MASK(14), BLOCK_MASK(15),
    BLOCK_MASK(16), BLOCK_MASK(17), BLOCK_MASK(18), BLOCK_MASK(19),
    BLOCK_MASK(20), BLOCK_MASK(21), BLOCK_MASK(22), BLOCK_MASK(23),
    BLOCK_MASK(24),
};
#undef BLOCK_MASK

static const u8 sSlotToLinkPlayerTableId[] = {0, 0, 1, 1, 2, 2, 2, 2, 3};
...
ALIGNED(4)
static const char sASCII_30Spaces[] = {"                              "};
...
static const char sASCII_ChildParentSearch[][8] = {"CHILD ", "PARENT",
                                                   "SEARCH"};
...
void InitRFUAPI(void) {
  if (!rfu_initializeAPI((void *)gRfuAPIBuffer, sizeof(gRfuAPIBuffer),
                         &gIntrTable[1], TRUE)) {
    gLinkType = 0;
...
static void Task_ParentSearchForChildren(u8 taskId) {
  ...
  case RFUSTATE_PARENT_CONNECT:
    rfu_LMAN_establishConnection(gRfu.parentChild, 0, 240,
                                 (u16 *)sAcceptedSerialNos);
...
static void SetLinkPlayerIdsFromSlots(s32 baseSlots, s32 addSlots) {
  ...
  if (addSlots == -1) {
    ...
    for (addSlots &= ~baseSlots, i = 0; i < RFU_CHILD_MAX;
         addSlots >>= 1, i++) {
      if (addSlots & 1)
...
static void Task_ChildSearchForParent(u8 taskId) {
  ...
  case RFUSTATE_CHILD_JOINED: {
    ...
    rfu_setRecvBuffer(TYPE_UNI, gRfu.childSlot, gRfu.childRecvQueue,
                      sizeof(gRfu.childRecvQueue));
    rfu_UNI_setSendData(bmChildSlot, gRfu.childSendBuffer,
                        sizeof(gRfu.childSendBuffer));
    gTasks[taskId].data[1] = 8;
    DestroyTask(taskId);
...
static void InitChildRecvBuffers(void) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; i++) {
    if (acceptSlot & 1) {
      rfu_setRecvBuffer(TYPE_UNI, i, gRfu.childRecvBuffer[i],
                        sizeof(gRfu.childRecvBuffer[0]));
...
static void Task_UnionRoomListen(u8 taskId) {
  if (GetHostRfuGameData()->activity == (ACTIVITY_PLYRTALK | IN_UNION_ROOM) &&
  ...
  case RFUSTATE_UR_CONNECT:
    rfu_LMAN_establishConnection(MODE_P_C_SWITCH, 0, 240,
                                 (u16 *)sAcceptedSerialNos);
  ...
  case RFUSTATE_UR_PLAYER_EXCHANGE:
    if (rfu_UNI_setSendData(1 << gRfu.childSlot, gRfu.childSendBuffer,
                            sizeof(gRfu.childSendBuffer)) == 0) {
...
// If no parent ID (or if child connection not ready) can't reconnect with
// parent yet
...
static bool32 TryReconnectParent(void) {
  if (gRfu.state == RFUSTATE_CHILD_CONNECT_END &&
      !rfu_LMAN_CHILD_connectParent(
          gRfuLinkStatus->partner[gRfu.reconnectParentId].id, 240)) {
...
void StopUnionRoomLinkManager(void) { gRfu.state = RFUSTATE_UR_STOP_MANAGER; }
...
static void UpdateBackupQueue(void) {
  if (gRfu.linkRecovered) {
    bool8 backupEmpty =
        RfuBackupQueue_Dequeue(&gRfu.backupQueue, gRfu.childSendBuffer);
...
static bool32 RfuMain1_Parent(void) {
  if (gRfu.state < RFUSTATE_FINALIZED) {
    ...
    gRfu.parentFinished = FALSE;
    if ((gRfu.parentSlots & gRfuLinkStatus->connSlotFlag) == gRfu.parentSlots &&
        (gRfu.parentSlots & gRfuLinkStatus->connSlotFlag)) {
  ...
  if (gRfu.state >= RFUSTATE_FINALIZED && gRfu.runParentMain2 == TRUE) {
    ...
    if ((lman.parentAck_flag & gRfu.parentSlots) == gRfu.parentSlots) {
        ...
        if (flags & 1) {
          if (gRfu.childRecvBuffer[i][1]) {
            if (gRfu.childRecvIds[i] != 0xFF &&
                (gRfu.childRecvBuffer[i][0] >> 5) !=
                    ((gRfu.childRecvIds[i] + 1) & 7)) {
              ...
              for (j = 0; j < CMD_LENGTH - 1; j++) {
                gRecvCmds[r0][j] =
                    (gRfu.childRecvBuffer[i][(j << 1) + 1] << 8) |
                    gRfu.childRecvBuffer[i][(j << 1) + 0];
      ...
      if (gRfu.nextChildBits && !gRfu.stopNewConnections) {
        ...
        SetLinkPlayerIdsFromSlots(gRfu.parentSlots,
                                  gRfu.parentSlots | gRfu.nextChildBits);
        gRfu.incomingChild = gRfu.nextChildBits;
        gRfu.parentSlots |= gRfu.nextChildBits;
        gRfu.nextChildBits = 0;
        rfu_UNI_setSendData(gRfu.parentSlots, gRfu.recvCmds,
                            sizeof(gRfu.recvCmds));
...
static bool32 RfuMain1_Child(void) {
  ...
  if (lman.childClockSlave_flag == 0 &&
    ...
    rfu_REQ_disconnect(gRfuLinkStatus->connSlotFlag |
                       gRfuLinkStatus->linkLossSlotFlag);
    rfu_waitREQComplete();
    status = RfuGetStatus();
    if (status != RFU_STATUS_FATAL_ERROR &&
        status != RFU_STATUS_JOIN_GROUP_NO && status != RFU_STATUS_LEAVE_GROUP)
  ...
  for (i = 0; i < gRfu.sendBlock.count; i++) {
    if (!(flags & 1)) {
      ...
      for (j = 0; j < CMD_LENGTH - 1; j++) {
        temp = j * 2;
        sResendBlock16[j + 1] =
            (payload[(COMM_SLOT_LENGTH - 2) * i + temp + 1] << 8) |
...
bool32 IsSendingKeysToRfu(void) { return gRfu.callback == SendKeysToRfu; }

void StartSendingKeysToRfu(void) { gRfu.callback = SendKeysToRfu; }

void ClearLinkRfuCallback(void) { gRfu.callback = NULL; }
  ...
  for (i = 0; i < MAX_RFU_PLAYERS; i++) {
    ...
    case RFUCMD_SEND_BLOCK:
      if (gRfu.recvBlock[i].receiving == RECV_STATE_RECEIVING) {
        ...
        for (j = 0; j < 6; j++)
          gBlockRecvBuffer[i][gRfu.recvBlock[i].next * 6 + j] =
              gRecvCmds[i][j + 1];
        if (gRfu.recvBlock[i].receivedFlags ==
          ...
          if (GetHostRfuGameData()->activity ==
                  (ACTIVITY_CHAT | IN_UNION_ROOM) &&
              gReceivedRemoteLinkPlayers && gRfu.parentChild == MODE_CHILD)
...
static bool8 AreAllPlayersFinishedReceiving(void) {
  ...
  for (i = 0; i < gRfu.playerCount; i++) {
    if (gRfu.recvBlock[i].receiving != RECV_STATE_FINISHED ||
        gRfu.blockReceived[i] != TRUE)
      return FALSE;
  }
  return TRUE;
...
static void HandleBlockSend(void) {
  if (gSendCmd[0] == 0) {
    ...
    if (gRfu.parentChild == MODE_PARENT) {
      ...
      if ((gRecvCmds[GetMultiplayerId()][0] & RFUCMD_MASK) ==
...
static void SendNextBlock(void) {
  ...
  for (i = 0; i < CMD_LENGTH - 1; i++)
    gSendCmd[i + 1] = (src[(i << 1) + gRfu.sendBlock.next * 12 + 1] << 8) |
                      src[(i << 1) + gRfu.sendBlock.next * 12 + 0];
  gRfu.sendBlock.next++;
...
static void SendLastBlock(void) {
  ...
  if (gRfu.parentChild == MODE_CHILD) {
    ...
    for (i = 0; i < CMD_LENGTH - 1; i++)
      gSendCmd[i + 1] =
          (src[(i << 1) + (gRfu.sendBlock.count - 1) * 12 + 1] << 8) |
          src[(i << 1) + (gRfu.sendBlock.count - 1) * 12 + 0];
    if ((u8)gRecvCmds[mpId][0] == gRfu.sendBlock.count - 1) {
      if (gRfu.recvBlock[mpId].receivedFlags !=
          sAllBlocksReceived[gRfu.recvBlock[mpId].count]) {
...
u8 Rfu_GetLinkPlayerCount(void) { return gRfu.playerCount; }
...
static bool8 CheckForLeavingGroupMembers(void) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; i++) {
    if (gRfu.partnerSendStatuses[i] < RFU_STATUS_JOIN_GROUP_OK ||
      ...
      } else if (gRfuSlotStatusNI[gRfu.childSlot]->recv.state ==
...
static s32 GetJoinGroupStatus(void) {
  ...
  if (gRfu.leaveGroupStatus == RFU_STATUS_LEAVE_GROUP_NOTICE) {
    if (gRfuSlotStatusNI[gRfu.childSlot]->send.state ==
  ...
  if (gRfuSlotStatusNI[gRfu.childSlot]->recv.state == SLOT_STATE_RECV_SUCCESS ||
      gRfuSlotStatusNI[gRfu.childSlot]->recv.state ==
          SLOT_STATE_RECV_SUCCESS_AND_SENDSIDE_UNKNOWN) {
...
static void Task_PlayerExchange(u8 taskId) {
  ...
  if (gRfu.status == RFU_STATUS_FATAL_ERROR ||
      gRfu.status == RFU_STATUS_CONNECTION_ERROR) {
...
// Equivalent to Task_PlayerExchange, but for when new children arrive after the
// first exchange
static void Task_PlayerExchangeUpdate(u8 taskId) {
  ...
  struct SioInfo *sio;
  u8 playerId =
      gRfu.linkPlayerIdx[sSlotToLinkPlayerTableId[gRfu.incomingChild]];
  if (gRfu.status == RFU_STATUS_FATAL_ERROR ||
      gRfu.status == RFU_STATUS_CONNECTION_ERROR) {
    gRfu.playerExchangeActive = FALSE;
    DestroyTask(taskId);
...
static void RfuCheckErrorStatus(void) {
  if (gRfu.errorState == RFU_ERROR_STATE_OCCURRED &&
    ...
    SetLinkErrorBuffer((gRfu.errorInfo << 16) | (gRfu.errorParam0 << 8) |
                           gRfu.errorParam1,
                       gRfu.recvQueue.count, gRfu.sendQueue.count,
    ...
    if (lman.childClockSlave_flag)
      rfu_LMAN_requestChangeAgbClockMaster();
    RfuSetStatus(RFU_STATUS_FATAL_ERROR,
                 F_RFU_ERROR_5 | F_RFU_ERROR_6 | F_RFU_ERROR_7);
...
void SetHostRfuGameData(u8 activity, u32 partnerInfo, bool32 startedActivity) {
  InitHostRfuGameData(&gHostRfuGameData, activity, startedActivity,
                      partnerInfo);
}
...
void UpdateGameData_GroupLockedIn(bool8 startedActivity) {
  ...
  rfu_REQ_configGameData(0, RFU_SERIAL_GAME, (void *)&gHostRfuGameData,
                         gHostRfuUsername);
}

void UpdateGameData_SetActivity(u8 activity, u32 partnerInfo,
                                bool32 startedActivity) {
...
void SetUnionRoomChatPlayerData(u32 numPlayers) {
  ...
  if (GetHostRfuGameData()->activity == (ACTIVITY_CHAT | IN_UNION_ROOM)) {
    ...
    for (i = 0; i < RFU_CHILD_MAX; i++) {
      if ((slots >> i) & 1) {
        // Only trainerId is shifted by the number of children, so the active
        // flag and gender are only ever set for the first child
        partnerInfo |=
            ((PINFO_ACTIVE_FLAG |
              ((gLinkPlayers[gRfu.linkPlayerIdx[i]].gender & 1)
               << PINFO_GENDER_SHIFT) |
              (gLinkPlayers[gRfu.linkPlayerIdx[i]].trainerId & PINFO_TID_MASK))
    ...
    UpdateGameData_SetActivity(ACTIVITY_CHAT | IN_UNION_ROOM, partnerInfo,
                               FALSE);
  }
}
    ...
    for (i = 0; i < RFU_CHILD_MAX; i++) {
      if ((lman.param[0] >> i) & 1) {
        ...
        if (data->activity == GetHostRfuGameData()->activity) {
          ...
          gRfu.partnerRecvStatuses[i] = RFU_STATUS_OK;
          rfu_setRecvBuffer(TYPE_NI, i, &gRfu.partnerRecvStatuses[i],
                            sizeof(gRfu.partnerRecvStatuses[0]));
        } else {
          disconnectFlag |= (1 << i);
        }
      }
...
static void LinkManagerCB_Child(u8 msg, u8 unused1) {
  ...
  case LMAN_MSG_CHILD_NAME_SEND_COMPLETED:
    ...
    rfu_setRecvBuffer(TYPE_NI, gRfu.childSlot, &gRfu.childRecvStatus,
                      sizeof(gRfu.childRecvStatus));
    rfu_setRecvBuffer(TYPE_UNI, gRfu.childSlot, gRfu.childRecvQueue,
                      sizeof(gRfu.childRecvQueue));
    break;
...
static void LinkManagerCB_UnionRoom(u8 msg, u8 paramCount) {
  ...
  case LMAN_MSG_NEW_CHILD_CONNECT_ACCEPTED:
    ...
    } else if (GetHostRfuGameData()->activity ==
  ...
  case LMAN_MSG_LINK_RECOVERY_FAILED_AND_DISCONNECTED:
    ...
    if (gRfuLinkStatus->parentChild == MODE_NEUTRAL && !lman.pcswitch_flag &&
...
u8 RfuGetStatus(void) { return gRfu.status; }
...
bool32 Rfu_IsPlayerExchangeActive(void) { return gRfu.playerExchangeActive; }

bool8 Rfu_IsMaster(void) { return gRfu.parentChild; }

void RfuVSync(void) { rfu_LMAN_syncVBlank(); }

void ClearRecvCommands(void) { CpuFill32(0, gRecvCmds, sizeof(gRecvCmds)); }
...
static void UNUSED Debug_RfuIdle(void) {
  ...
  if (IsWirelessAdapterConnected()) {
    ...
    SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJ_ON | DISPCNT_BG0_ON |
                                      DISPCNT_BG2_ON | DISPCNT_OBJ_1D_MAP);
    RunTasks();
    AnimateSprites();
    BuildOamBuffer();
    UpdatePaletteFade();
...
static u8 GetPartnerIndexByNameAndTrainerID(const u8 *name, u16 id) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; i++) {
    u16 trainerId =
        ReadU16(((struct RfuGameData *)gRfuLinkStatus->partner[i].gname)
                    ->compatibility.playerTrainerId);
  ...
  if (CanTryReconnectParent()) {
    ...
    if (id != 0xFF) {
      if (gRfuLinkStatus->partner[id].slot != 0xFF) {
        ...
        RfuSetStatus(RFU_STATUS_CONNECTION_ERROR,
                     F_RFU_ERROR_5 | F_RFU_ERROR_6 | F_RFU_ERROR_7);
        DestroyTask(taskId);
      }
    } else {
...
static bool32 IsPartnerActivityIncompatible(s16 activity,
                                            struct RfuGameData *partner) {
  if (GetHostRfuGameData()->activity == (ACTIVITY_CHAT | IN_UNION_ROOM)) {
  ...
  if (gRfu.parentId != 0 && lman.parent_child == MODE_CHILD) {
    ...
    if (id != 0xFF) {
      ...
      if (!IsPartnerActivityIncompatible(
              gTasks[taskId].tActivity,
              (void *)gRfuLinkStatus->partner[id].gname)) {
        if (gRfuLinkStatus->partner[id].slot != 0xFF &&
            !rfu_LMAN_CHILD_connectParent(gRfuLinkStatus->partner[id].id, 90)) {
...
void TryConnectToUnionRoomParent(const u8 *name, struct RfuGameData *parent,
                                 u8 activity) {
...
static void UNUSED Debug_PrintStatus(void) {
  ...
  if (gRfu.parentChild == MODE_PARENT) {
  ...
  } else if (gRfuLinkStatus->connSlotFlag != 0 &&
    ...
    Debug_PrintString((void *)gRfuLinkStatus->partner[gRfu.childSlot].gname, 6,
                      3);
...
static u32 UNUSED GetRfuSendQueueLength(void) { return gRfu.sendQueue.count; }

u32 GetRfuRecvQueueLength(void) { return gRfu.recvQueue.count; }
