...
static const u8 sUnusedWater_Gfx[] =
    INCBIN_U8("graphics/battle_anims/unused/water_gfx.4bpp");
static const u8 sUnusedWater[] =
    INCBIN_U8("graphics/battle_anims/unused/water.bin");

static const union AnimCmd sAnim_RainDrop[] = {
    ANIMCMD_FRAME(0, 2),  ANIMCMD_FRAME(8, 2),  ANIMCMD_FRAME(16, 2),
    ANIMCMD_FRAME(24, 6), ANIMCMD_FRAME(32, 2), ANIMCMD_FRAME(40, 2),
...
static const union AnimCmd sAnim_WaterMudOrb[] = {
    ANIMCMD_FRAME(0, 1),  ANIMCMD_FRAME(4, 1), ANIMCMD_FRAME(8, 1),
    ANIMCMD_FRAME(12, 1), ANIMCMD_JUMP(0),
};
...
static void AnimAuroraBeamRings(struct Sprite *sprite) {
  ...
  sprite->data[1] = sprite->x;
  sprite->data[2] =
      GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_X_2) + unkArg;
  sprite->data[3] = sprite->y;
  sprite->data[4] =
      GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y_PIC_OFFSET) +
      gBattleAnimArgs[3];
...
// Updates the palette on the rainbow rings used in Aurora Beam to make them
// appear to be rotating counterclockwise
void AnimTask_RotateAuroraRingColors(u8 taskId) {
  gTasks[taskId].data[0] = gBattleAnimArgs[0];
  gTasks[taskId].data[2] =
      OBJ_PLTT_ID(IndexOfSpritePaletteTag(ANIM_TAG_RAINBOW_RINGS));
...
// For animating undulating beam attacks (e.g. Flamethrower, Hydro Pump, Signal
// Beam)
...
// Flashing blue orbs grow in size near the attacker. First stage of Hydro
// Cannon
...
// Flashing blue orbs move from the attacker to the target. Second stage of
// Hydro Cannon
...
void AnimTask_CreateSurfWave(u8 taskId) {
  ...
  if (!IsContest()) {
    ...
    if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_OPPONENT)
      AnimLoadCompressedBgTilemap(animBg.bgId,
                                  gBattleAnimBgTilemap_SurfOpponent);
    else
      AnimLoadCompressedBgTilemap(animBg.bgId, gBattleAnimBgTilemap_SurfPlayer);
  } else {
    AnimLoadCompressedBgTilemapHandleContest(
        &animBg, gBattleAnimBgTilemap_SurfContest, TRUE);
  }
  AnimLoadCompressedBgGfx(animBg.bgId, gBattleAnimBgImage_Surf,
                          animBg.tilesOffset);
  if (gBattleAnimArgs[0] == 0)
    LoadCompressedPalette(gBattleAnimBgPalette_Surf,
                          BG_PLTT_ID(animBg.paletteId), PLTT_SIZE_4BPP);
  else
    LoadCompressedPalette(gBattleAnimBackgroundImageMuddyWater_Pal,
                          BG_PLTT_ID(animBg.paletteId), PLTT_SIZE_4BPP);
  taskId2 =
...
static void AnimTask_CreateSurfWave_Step1(u8 taskId) {
  ...
  *BGptrY += gTasks[taskId].data[1];
  ...
  if (++gTasks[taskId].data[5] == 4) {
    ...
    for (i = 6; i != 0; i--) {
      gPlttBufferFaded[BG_PLTT_ID(animBg.paletteId) + 1 + i] =
          gPlttBufferFaded[BG_PLTT_ID(animBg.paletteId) + 1 + i -
                           1]; // 1 + i - 1 is needed to match for some bizarre
                               // reason
    }
  ...
  if (++gTasks[taskId].data[6] > 1) {
    ...
    if (++gTasks[taskId].data[3] <= 13) {
      gTasks[gTasks[taskId].data[15]].data[1] =
          (s16)((gTasks[taskId].data[3]) |
                ((16 - gTasks[taskId].data[3]) << 8));
      gTasks[taskId].data[4]++;
    }
    if (gTasks[taskId].data[3] > 54) {
      gTasks[taskId].data[4]--;
      gTasks[gTasks[taskId].data[15]].data[1] =
          (s16)((gTasks[taskId].data[4]) |
                ((16 - gTasks[taskId].data[4]) << 8));
    }
  }
...
static void AnimTask_SurfWaveScanlineEffect(u8 taskId) {
  ...
  case 0:
    for (i = 0; i < task->data[4]; i++)
      gScanlineEffectRegBuffers[0][i] = gScanlineEffectRegBuffers[1][i] =
          task->data[2];
...
static void AnimTask_WaterSpoutLaunch_Step(u8 taskId) {
  ...
  switch (task->data[0]) {
  case 0:
    PrepareEruptAnimTaskData(task, task->data[15], 0x100, 0x100, 0xE0, 0x200,
                             32);
    task->data[0]++;
  case 1:
    if (++task->data[3] > 1) {
  ...
  case 2:
    if (++task->data[3] > 4) {
      PrepareEruptAnimTaskData(task, task->data[15], 0xE0, 0x200, 0x180, 0xE0,
                               8);
      task->data[3] = 0;
      task->data[0]++;
    }
  ...
  case 5:
    if (++task->data[3] > 1) {
      ...
      if (++task->data[4] & 1)
        ...
        gSprites[task->data[15]].y2 -= 2;
      if (task->data[4] == 10) {
        PrepareEruptAnimTaskData(task, task->data[15], 0x180, 0xE0, 0x100,
                                 0x100, 8);
        task->data[3] = 0;
        task->data[4] = 0;
        task->data[0]++;
...
// A higher number results in more water sprites during the Water Spout
// animation
static u8 GetWaterSpoutPowerForAnim(void) {
  u8 i;
  u16 hp;
...
static void AnimSmallWaterOrb(struct Sprite *sprite) {
  ...
  case 1:
    ...
    if (sprite->x < -8 || sprite->x > 248 || sprite->y < -8 ||
        sprite->y > 120) {
...
static void CreateWaterSpoutRainDroplet(struct Task *task, u8 taskId) {
  ...
  u8 spriteId =
      CreateSprite(&gSmallWaterOrbSpriteTemplate, task->data[7], 0, 0);
  ...
  task->data[7] =
      (ISO_RANDOMIZE2(task->data[7]) % task->data[5]) + task->data[4];
}
...
static void AnimWaterSpoutRain(struct Sprite *sprite) {
  if (sprite->data[0] == 0) {
    ...
    if (sprite->y >= sprite->data[5]) {
      ...
      sprite->data[1] =
          CreateSprite(&gWaterHitSplatSpriteTemplate, sprite->x, sprite->y, 1);
...
void AnimTask_WaterSport(u8 taskId) {
  ...
  task->data[4] =
      GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_Y_PIC_OFFSET);
  task->data[7] =
      (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_PLAYER) ? 1 : -1;
  if (IsContest())
    task->data[7] *= -1;
...
static void CreateWaterSportDroplet(struct Task *task) {
  ...
  if (++task->data[2] > 1) {
    task->data[2] = 0;
    spriteId = CreateSprite(&gSmallWaterOrbSpriteTemplate, task->data[3],
                            task->data[4], 10);
    if (spriteId != MAX_SPRITES) {
...
static void CreateWaterPulseRingBubbles(struct Sprite *sprite, int xDiff,
                                        int yDiff) {
  s16 combinedX;
  s16 combinedY;
  s16 i;
  ...
  for (i = 0; i <= 0; i++) {
    spriteId = CreateSprite(&gWaterPulseRingBubbleSpriteTemplate, combinedX,
                            combinedY + something, 130);
