...
static const u16 GymLeaderRematches_AfterNewMauville[] = {
    REMATCH_ROXANNE, REMATCH_BRAWLY, REMATCH_WATTSON,       REMATCH_FLANNERY,
    REMATCH_NORMAN,  REMATCH_WINONA, REMATCH_TATE_AND_LIZA, REMATCH_JUAN};
  ...
  for (i = 0; i < ARRAY_COUNT(GymLeaderRematches_AfterNewMauville); i++) {
    ...
    if (tableId != -1 && tableId < MAX_REMATCH_ENTRIES) {
      if (rematchIndex == 1) // 2nd fight
      {
        ...
        if (trainerIdx == REMATCH_ROXANNE &&
            FlagGet(FLAG_REMATCH_READY_ROXANNE) &&
            !HasTrainerBeenFought(TRAINER_ROXANNE_2))
          gSaveBlock1Ptr->trainerRematches[tableId] =
              1; // Set to 1 or any positive value to enable rematch
        else if (trainerIdx == REMATCH_BRAWLY &&
                 FlagGet(FLAG_REMATCH_READY_BRAWLY) &&
                 !HasTrainerBeenFought(TRAINER_BRAWLY_2) &&
                 HasTrainerBeenFought(TRAINER_BRAWLY_1))
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
        else if (trainerIdx == REMATCH_WATTSON &&
                 FlagGet(FLAG_WATTSON_REMATCH_AVAILABLE) &&
                 !HasTrainerBeenFought(TRAINER_WATTSON_2))
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
        else if (trainerIdx == REMATCH_FLANNERY &&
                 FlagGet(FLAG_REMATCH_READY_FLANNERY) &&
                 !HasTrainerBeenFought(TRAINER_FLANNERY_2))
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
        else if (trainerIdx == REMATCH_NORMAN &&
                 FlagGet(FLAG_REMATCH_READY_NORMAN) &&
                 !HasTrainerBeenFought(TRAINER_NORMAN_2))
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
        else if (trainerIdx == REMATCH_WINONA &&
                 FlagGet(FLAG_REMATCH_READY_WINONA) &&
                 !HasTrainerBeenFought(TRAINER_WINONA_2))
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
        else if (trainerIdx == REMATCH_TATE_AND_LIZA &&
                 FlagGet(FLAG_REMATCH_READY_TATEANDLIZA) &&
                 !HasTrainerBeenFought(TRAINER_TATE_AND_LIZA_2))
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
        // else if (trainerIdx == REMATCH_JUAN &&
        // FlagGet(FLAG_REMATCH_READY_JUAN) &&
        // !HasTrainerBeenFought(TRAINER_JUAN_2)) Juan 2 isn't accessible
        //     gSaveBlock1Ptr->trainerRematches[tableId] = 1;
      } else if ((rematchIndex == 2) &&
                 FlagGet(FLAG_SYS_GAME_CLEAR)) // 3rd fight
      {
        if (trainerIdx == REMATCH_ROXANNE &&
            !HasTrainerBeenFought(TRAINER_ROXANNE_3) &&
            HasTrainerBeenFought(TRAINER_ROXANNE_2))
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
        else if (trainerIdx == REMATCH_BRAWLY &&
          ...
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
                 ...
                 HasTrainerBeenFought(TRAINER_JUAN_2))
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
      } else if ((rematchIndex == 3) &&
                 FlagGet(FLAG_COLLECTED_ALL_SILVER_SYMBOLS)) // 4th fight
      {
        if (trainerIdx == REMATCH_ROXANNE &&
            !HasTrainerBeenFought(TRAINER_ROXANNE_4) &&
            HasTrainerBeenFought(TRAINER_ROXANNE_3))
          gSaveBlock1Ptr->trainerRematches[tableId] = 1;
        else if (trainerIdx == REMATCH_BRAWLY &&
