...
static const union AnimCmd *const sAnims_BasicRock[] = {
    sAnim_Rock_Biggest, sAnim_Rock_Bigger,  sAnim_Rock_Big,
    sAnim_Rock_Small,   sAnim_Rock_Smaller, sAnim_Rock_Smallest,
};
...
// Animates the rock particles that are shown on the impact for Rock Blast /
// Rock Smash
...
void AnimTask_LoadSandstormBackground(u8 taskId) {
  ...
  AnimLoadCompressedBgGfx(animBg.bgId, gBattleAnimBgImage_Sandstorm,
                          animBg.tilesOffset);
  AnimLoadCompressedBgTilemapHandleContest(
      &animBg, gBattleAnimBgTilemap_Sandstorm, FALSE);
  LoadCompressedPalette(gBattleAnimSpritePal_FlyingDirt,
...
static void CreateRolloutDirtSprite(struct Task *task) {
  ...
  if (spriteId != MAX_SPRITES) {
    ...
    gSprites[spriteId].data[2] =
        ((task->data[12] * 20) + x) + (task->data[1] * 3);
...
static u8 GetRolloutCounter(void) {
  u8 retVal = gAnimDisableStructPtr->rolloutTimerStartValue -
