...
// These belong in battle_intro.c, but putting them there causes 2 bytes of
// alignment padding between the two .rodata segments. Perhaps battle_intro.c
// actually belongs in this file, too.
const u8 gBattleAnimBgCntSet[] = {REG_OFFSET_BG0CNT, REG_OFFSET_BG1CNT,
                                  REG_OFFSET_BG2CNT, REG_OFFSET_BG3CNT};
const u8 gBattleAnimBgCntGet[] = {REG_OFFSET_BG0CNT, REG_OFFSET_BG1CNT,
                                  REG_OFFSET_BG2CNT, REG_OFFSET_BG3CNT};
...
void AnimTask_BlendBattleAnimPal(u8 taskId) {
  ...
  selectedPalettes |= GetBattleMonSpritePalettesMask(
      (gBattleAnimArgs[0] >> 7) & 1, (gBattleAnimArgs[0] >> 8) & 1,
  ...
  if (gTasks[taskId].data[9] == gTasks[taskId].data[2]) {
    ...
    while (selectedPalettes != 0) {
      if (selectedPalettes & 1)
        BlendPalette(singlePaletteOffset, 16, gTasks[taskId].data[10],
...
void AnimTask_HardwarePaletteFade(u8 taskId) {
  BeginHardwarePaletteFade(gBattleAnimArgs[0], gBattleAnimArgs[1],
                           gBattleAnimArgs[2], gBattleAnimArgs[3],
                           gBattleAnimArgs[4]);
...
// Used to leave blended traces of a mon, usually to imply speed as in Agility
// or Aerial Ace
...
void AnimTask_DrawFallingWhiteLinesOnAttacker(u8 taskId) {
  ...
  SetGpuReg(REG_OFFSET_BLDCNT,
            BLDCNT_TGT1_BG1 | BLDCNT_TGT2_ALL | BLDCNT_EFFECT_BLEND);
  ...
  newSpriteId =
      CreateInvisibleSpriteCopy(gBattleAnimAttacker, spriteId, species);
  GetBattleAnimBg1Data(&animBgData);
  AnimLoadCompressedBgTilemapHandleContest(&animBgData,
                                           gBattleAnimMaskTilemap_Curse, FALSE);
  AnimLoadCompressedBgGfx(animBgData.bgId, gBattleAnimMaskImage_Curse,
                          animBgData.tilesOffset);
  LoadPalette(sCurseLinesPalette, BG_PLTT_ID(animBgData.paletteId) + 1,
              PLTT_SIZEOF(1));
...
#define aMultipleBattlers                                                      \
  data[3] // Always false. Changes for multiple battlers are instead run
          // sequentially.
...
static void StatsChangeAnimation_Step1(u8 taskId) {
  ...
  sAnimStatsChangeData->battler2 =
      BATTLE_PARTNER(sAnimStatsChangeData->battler1);
  if (IsContest() || (sAnimStatsChangeData->aMultipleBattlers &&
  ...
  SetGpuReg(REG_OFFSET_WINOUT,
            (WINOUT_WIN01_ALL & ~WINOUT_WIN01_BG1) | WINOUT_WINOBJ_ALL);
  SetGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_OBJWIN_ON);
  SetGpuReg(REG_OFFSET_BLDCNT,
            BLDCNT_TGT1_BG1 | BLDCNT_TGT2_ALL | BLDCNT_EFFECT_BLEND);
  ...
  if (IsDoubleBattle() && !sAnimStatsChangeData->aMultipleBattlers) {
    if (GetBattlerPosition(sAnimStatsChangeData->battler1) ==
            B_POSITION_OPPONENT_RIGHT ||
        GetBattlerPosition(sAnimStatsChangeData->battler1) ==
            B_POSITION_PLAYER_LEFT) {
      if (IsBattlerSpriteVisible(sAnimStatsChangeData->battler2) == TRUE) {
        // Push the battler not being animated back so it doesn't receive the
        // animation.
  ...
  if (IsContest()) {
    ...
    if (GetBattlerSide(sAnimStatsChangeData->battler1) != B_SIDE_PLAYER)
      sAnimStatsChangeData->species = GetMonData(
          &gEnemyParty[gBattlerPartyIndexes[sAnimStatsChangeData->battler1]],
          MON_DATA_SPECIES);
    else
      sAnimStatsChangeData->species = GetMonData(
          &gPlayerParty[gBattlerPartyIndexes[sAnimStatsChangeData->battler1]],
          MON_DATA_SPECIES);
  }
...
static void StatsChangeAnimation_Step2(u8 taskId) {
  ...
  spriteId =
      CreateInvisibleSpriteCopy(sAnimStatsChangeData->battler1, battlerSpriteId,
  ...
  if (sAnimStatsChangeData->aMultipleBattlers) {
    ...
    spriteId2 = CreateInvisibleSpriteCopy(sAnimStatsChangeData->battler2,
                                          battlerSpriteId,
                                          sAnimStatsChangeData->species);
  }
  ...
  if (!sAnimStatsChangeData->aDecrease)
    AnimLoadCompressedBgTilemapHandleContest(&animBgData,
                                             gStatAnim_Increase_Tilemap, FALSE);
  else
    AnimLoadCompressedBgTilemapHandleContest(&animBgData,
                                             gStatAnim_Decrease_Tilemap, FALSE);

  AnimLoadCompressedBgGfx(animBgData.bgId, gStatAnim_Gfx,
                          animBgData.tilesOffset);
  switch (sAnimStatsChangeData->aAnimStatId) {
  case STAT_ANIM_PAL_ATK:
    LoadCompressedPalette(gStatAnim_Attack_Pal,
                          BG_PLTT_ID(animBgData.paletteId), PLTT_SIZE_4BPP);
    break;
  case STAT_ANIM_PAL_DEF:
    LoadCompressedPalette(gStatAnim_Defense_Pal,
                          BG_PLTT_ID(animBgData.paletteId), PLTT_SIZE_4BPP);
    break;
  case STAT_ANIM_PAL_ACC:
    LoadCompressedPalette(gStatAnim_Accuracy_Pal,
                          BG_PLTT_ID(animBgData.paletteId), PLTT_SIZE_4BPP);
    break;
  case STAT_ANIM_PAL_SPEED:
    LoadCompressedPalette(gStatAnim_Speed_Pal, BG_PLTT_ID(animBgData.paletteId),
                          PLTT_SIZE_4BPP);
    break;
  case STAT_ANIM_PAL_EVASION:
    LoadCompressedPalette(gStatAnim_Evasion_Pal,
                          BG_PLTT_ID(animBgData.paletteId), PLTT_SIZE_4BPP);
    break;
  case STAT_ANIM_PAL_SPATK:
    LoadCompressedPalette(gStatAnim_SpAttack_Pal,
                          BG_PLTT_ID(animBgData.paletteId), PLTT_SIZE_4BPP);
    break;
  case STAT_ANIM_PAL_SPDEF:
    LoadCompressedPalette(gStatAnim_SpDefense_Pal,
                          BG_PLTT_ID(animBgData.paletteId), PLTT_SIZE_4BPP);
    break;
  default:
    // case STAT_ANIM_PAL_MULTIPLE:
    LoadCompressedPalette(gStatAnim_Multiple_Pal,
                          BG_PLTT_ID(animBgData.paletteId), PLTT_SIZE_4BPP);
    break;
  }
  ...
  gTasks[taskId].tBattler2SpriteId =
      gBattlerSpriteIds[sAnimStatsChangeData->battler2];
  gTasks[taskId].func = StatsChangeAnimation_Step3;

  if (!sAnimStatsChangeData->aDecrease)
    PlaySE12WithPanning(SE_M_STAT_INCREASE,
                        BattleAnimAdjustPanning2(SOUND_PAN_ATTACKER));
  else
    PlaySE12WithPanning(SE_M_STAT_DECREASE,
                        BattleAnimAdjustPanning2(SOUND_PAN_ATTACKER));
}
...
static void StatsChangeAnimation_Step3(u8 taskId) {
  ...
  case 0:
    ...
    if (gTasks[taskId].tFadeTimer++ > 0) {
      ...
      SetGpuReg(
          REG_OFFSET_BLDALPHA,
          BLDALPHA_BLEND(gTasks[taskId].tBlend, 16 - gTasks[taskId].tBlend));
...
void AnimTask_GetTargetIsAttackerPartner(u8 taskId) {
  gBattleAnimArgs[ARG_RET_ID] =
      BATTLE_PARTNER(gBattleAnimAttacker) == gBattleAnimTarget;
  DestroyAnimVisualTask(taskId);
}
...
void StartMonScrollingBgMask(u8 taskId, int UNUSED unused, u16 scrollSpeed,
                             u8 battler, bool8 includePartner, u8 numFadeSteps,
                             u8 fadeStepDelay, u8 duration, const u32 *gfx,
  ...
  if (IsContest()) {
    ...
    if (GetBattlerSide(battler) != B_SIDE_PLAYER)
      species = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]],
                           MON_DATA_SPECIES);
    else
      species = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]],
                           MON_DATA_SPECIES);
  }

  spriteId =
      CreateInvisibleSpriteCopy(battler, gBattlerSpriteIds[battler], species);
  if (includePartner)
    spriteId2 = CreateInvisibleSpriteCopy(battler2, gBattlerSpriteIds[battler2],
                                          species);
  ...
  LoadCompressedPalette(palette, BG_PLTT_ID(animBgData.paletteId),
                        PLTT_SIZE_4BPP);
...
static void UpdateMonScrollingBgMask(u8 taskId) {
  gTasks[taskId].data[13] += gTasks[taskId].data[1] < 0
...
void AnimTask_CopyPalUnfadedToBackup(u8 taskId) {
  ...
  memcpy(&gMonSpritesGfxPtr->buffer[gBattleAnimArgs[1] * 16],
         &gPlttBufferUnfaded[PLTT_ID(paletteIndex)], PLTT_SIZE_4BPP);
  DestroyAnimVisualTask(taskId);
}
...
void AnimTask_CopyPalUnfadedFromBackup(u8 taskId) {
  ...
  memcpy(&gPlttBufferUnfaded[PLTT_ID(paletteIndex)],
         &gMonSpritesGfxPtr->buffer[gBattleAnimArgs[1] * 16], PLTT_SIZE_4BPP);
  DestroyAnimVisualTask(taskId);
}
...
void AnimTask_CopyPalFadedToUnfaded(u8 taskId) {
  ...
  memcpy(&gPlttBufferUnfaded[PLTT_ID(paletteIndex)],
         &gPlttBufferFaded[PLTT_ID(paletteIndex)], PLTT_SIZE_4BPP);
  DestroyAnimVisualTask(taskId);
}
