...
// TODO: These should also be combined into a single image, not matching for
// some reason
static const u8 sSandPillar0_Gfx[] =
    INCBIN_U8("graphics/field_effects/pics/sand_pillar/0.4bpp");
static const u8 sSandPillar1_Gfx[] =
    INCBIN_U8("graphics/field_effects/pics/sand_pillar/1.4bpp");
static const u8 sSandPillar2_Gfx[] =
    INCBIN_U8("graphics/field_effects/pics/sand_pillar/2.4bpp");
...
static const union AnimCmd sAnim_VineDropRight[] = {
    ANIMCMD_FRAME(0, 8, .hFlip = TRUE), ANIMCMD_FRAME(1, 8, .hFlip = TRUE),
    ANIMCMD_FRAME(2, 8, .hFlip = TRUE), ANIMCMD_FRAME(3, 8, .hFlip = TRUE),
    ANIMCMD_FRAME(4, 8, .hFlip = TRUE), ANIMCMD_END,
};

static const union AnimCmd sAnim_VineRiseRight[] = {
    ANIMCMD_FRAME(4, 8, .hFlip = TRUE), ANIMCMD_FRAME(3, 8, .hFlip = TRUE),
    ANIMCMD_FRAME(2, 8, .hFlip = TRUE), ANIMCMD_FRAME(1, 8, .hFlip = TRUE),
    ANIMCMD_FRAME(0, 8, .hFlip = TRUE), ANIMCMD_END,
};
...
static const struct SpriteFrameImage sPicTable_SecretPowerTree[] = {
    ...
    // 6th frame exists but isnt accessed, the tree vine metatile is used
    // instead
};
...
const struct SpritePalette gSpritePalette_SecretPower_Cave = {
    sSecretPowerCave_Pal, FLDEFF_PAL_TAG_SECRET_POWER_TREE};
const struct SpritePalette gSpritePalette_SecretPower_Plant = {
    sSecretPowerPlant_Pal, FLDEFF_PAL_TAG_SECRET_POWER_PLANT};
...
const struct SpritePalette gSpritePalette_SandPillar = {
    gTilesetPalettes_SecretBase[5], FLDEFF_PAL_TAG_SAND_PILLAR};
...
static const struct SpritePalette sSpritePalette_RecordMixLights = {
    sRecordMixLights_Pal, 0x1000};
...
void ComputerScreenOpenEffect(u16 increment, u16 unused, u8 priority) {
  CreateComputerScreenEffectTask(Task_ComputerScreenOpenEffect, increment,
                                 unused, priority);
}

void ComputerScreenCloseEffect(u16 increment, u16 unused, u8 priority) {
  CreateComputerScreenEffectTask(Task_ComputerScreenCloseEffect, increment,
                                 unused, priority);
}
...
static void CreateComputerScreenEffectTask(void (*taskfunc)(u8), u16 increment,
                                           u16 unused, u8 priority) {
...
bool8 SetUpFieldMove_SecretPower(void) {
  ...
  GetXYCoordsOneStepInFrontOfPlayer(&gPlayerFacingPosition.x,
                                    &gPlayerFacingPosition.y);
  mb = MapGridGetMetatileBehaviorAt(gPlayerFacingPosition.x,
                                    gPlayerFacingPosition.y);
...
bool8 FldEff_SecretPowerTree(void) {
  s16 mb = MapGridGetMetatileBehaviorAt(gPlayerFacingPosition.x,
                                        gPlayerFacingPosition.y) &
           0xFFF;
...
void DoSecretBasePCTurnOffEffect(void) {
  ...
  if (!VarGet(VAR_CURRENT_SECRET_BASE))
    MapGridSetMetatileIdAt(x, y,
                           METATILE_SecretBase_PC | MAPGRID_COLLISION_MASK);
  else
    MapGridSetMetatileIdAt(
        x, y, METATILE_SecretBase_RegisterPC | MAPGRID_COLLISION_MASK);
...
bool8 FldEff_Nop47(void) { return FALSE; }

bool8 FldEff_Nop48(void) { return FALSE; }
...
static void Task_ShatterSecretBaseBreakableDoor(u8 taskId) {
  if (gTasks[taskId].data[0] == 7) {
    DoSecretBaseBreakableDoorEffect(gTasks[taskId].data[1],
...
void DoSecretBaseGlitterMatSparkle(void) {
  ...
  spriteId = CreateSpriteAtEnd(
      gFieldEffectObjectTemplatePointers[FLDEFFOBJ_SPARKLE], x, y, 0);
...
static void SpriteCB_SandPillar_BreakTop(struct Sprite *sprite) {
  ...
  if (MapGridGetMetatileIdAt(gFieldEffectArguments[5],
                             gFieldEffectArguments[6] - 1) ==
      METATILE_SecretBase_SandOrnament_TopWall)
    MapGridSetMetatileIdAt(
        gFieldEffectArguments[5], gFieldEffectArguments[6] - 1,
        METATILE_SecretBase_Wall_TopMid | MAPGRID_COLLISION_MASK);
  else
  ...
  CurrentMapDrawMetatileAt(gFieldEffectArguments[5],
                           gFieldEffectArguments[6] - 1);
...
static void Task_WateringBerryTreeAnim_Start(u8 taskId) {
  ...
  if (!ObjectEventIsMovementOverridden(playerObjEvent) ||
    ...
    ObjectEventSetHeldMovement(
        playerObjEvent,
        GetWalkInPlaceNormalMovementAction(GetPlayerFacingDirection()));
