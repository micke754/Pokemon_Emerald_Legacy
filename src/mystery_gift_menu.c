...
static const u16 sTextboxBorder_Pal[] =
    INCBIN_U16("graphics/interface/mystery_gift_textbox_border.gbapal");
static const u32 sTextboxBorder_Gfx[] =
    INCBIN_U32("graphics/interface/mystery_gift_textbox_border.4bpp.lz");
...
static const struct WindowTemplate sWindowTemplate_YesNoMsg = {.bg = 0,
                                                               .tilemapLeft = 1,
                                                               .tilemapTop = 15,
                                                               .width = 20,
                                                               .height = 4,
...
static const struct ListMenuItem sListMenuItems_CardsOrNews[] = {
    {gText_WonderCards, 0}, {gText_WonderNews, 1}, {gText_Exit3, LIST_CANCEL}};
...
static const struct ListMenuItem sListMenuItems_ReceiveToss[] = {
    {gText_Receive, 0}, {gText_Toss, 2}, {gText_Cancel2, LIST_CANCEL}};

static const struct ListMenuItem sListMenuItems_ReceiveSend[] = {
    {gText_Receive, 0}, {gText_Send, 1}, {gText_Cancel2, LIST_CANCEL}};

static const struct ListMenuItem sListMenuItems_Receive[] = {
    {gText_Receive, 0}, {gText_Cancel2, LIST_CANCEL}};
...
static const u8 *const sUnusedMenuTexts[] = {
    gText_VarietyOfEventsImportedWireless, gText_WonderCardsInPossession,
    gText_ReadNewsThatArrived, gText_ReturnToTitle};

ALIGNED(2)
static const u8 sTextColors_Header[] = {TEXT_COLOR_TRANSPARENT,
                                        TEXT_COLOR_WHITE, TEXT_COLOR_DARK_GRAY};
ALIGNED(2)
static const u8 sTextColors_Header_Copy[] = {
    TEXT_COLOR_TRANSPARENT, TEXT_COLOR_WHITE, TEXT_COLOR_DARK_GRAY};
ALIGNED(2)
static const u8 sMG_Ereader_TextColor_2[] = {
...
// Print the text window at the top of the screen with the title and control
// instructions
void PrintMysteryGiftOrEReaderHeader(bool8 isEReader, bool32 useCancel) {
  ...
  AddTextPrinterParameterized4(WIN_HEADER, FONT_NORMAL, 4, 1, 0, 0,
                               sTextColors_Header, TEXT_SKIP_DRAW, title);
  AddTextPrinterParameterized4(
      WIN_HEADER, FONT_SMALL,
      GetStringRightAlignXOffset(FONT_SMALL, options, 0xDE), 1, 0, 0,
      sTextColors_Header, TEXT_SKIP_DRAW, options);
...
void MG_AddMessageTextPrinter(const u8 *str) {
  ...
  AddTextPrinterParameterized4(WIN_MSG, FONT_NORMAL, 0, 1, 0, 0,
                               sMG_Ereader_TextColor_2, 0, gStringVar4);
...
bool32 PrintMysteryGiftMenuMessage(u8 *textState, const u8 *str) {
  ...
  case 0:
    ...
    break;
  case 1:
    DrawDownArrow(WIN_MSG, DOWN_ARROW_X, DOWN_ARROW_Y, 1, FALSE,
    ...
    break;
  case 2:
    DrawDownArrow(WIN_MSG, DOWN_ARROW_X, DOWN_ARROW_Y, 1, TRUE,
                  &sDownArrowCounterAndYCoordIdx[0],
                  &sDownArrowCounterAndYCoordIdx[1]);
    *textState = 0;
    ClearMessage();
...
static u32 MysteryGift_HandleThreeOptionMenu(u8 *unused0, u16 *unused1,
                                             u8 whichMenu) {
  ...
  response = DoMysteryGiftListMenu(&windowTemplate, &listMenuTemplate, 1,
                                   LIST_MENU_TILE_NUM, LIST_MENU_PAL_NUM);
...
s8 DoMysteryGiftYesNo(u8 *textState, u16 *windowId, bool8 yesNoBoxPlacement,
  ...
  case 0:
    ...
    AddTextPrinterParameterized4(*windowId, FONT_NORMAL, 0, 1, 0, 0,
                                 sMG_Ereader_TextColor_2, 0, gStringVar4);
...
// Handle the "Receive/Send/Toss" menu that appears when selecting Wonder
// Card/News
static s32 HandleGiftSelectMenu(u8 *textState, u16 *windowId, bool32 cannotToss,
                                bool32 cannotSend) {
  ...
  case 1:
    ...
    if (cannotSend) {
      if (!cannotToss)
        input = DoMysteryGiftListMenu(&sWindowTemplate_GiftSelect_2Options,
                                      &sListMenu_ReceiveToss, 1,
                                      LIST_MENU_TILE_NUM, LIST_MENU_PAL_NUM);
      else
        input = DoMysteryGiftListMenu(&sWindowTemplate_GiftSelect_1Option,
                                      &sListMenu_Receive, 1, LIST_MENU_TILE_NUM,
                                      LIST_MENU_PAL_NUM);
    } else {
      if (!cannotToss)
        input = DoMysteryGiftListMenu(&sWindowTemplate_GiftSelect_3Options,
                                      &sListMenu_ReceiveSendToss, 1,
                                      LIST_MENU_TILE_NUM, LIST_MENU_PAL_NUM);
      else
        input = DoMysteryGiftListMenu(&sWindowTemplate_GiftSelect_2Options,
                                      &sListMenu_ReceiveSend, 1,
                                      LIST_MENU_TILE_NUM, LIST_MENU_PAL_NUM);
    }
...
static s32 AskDiscardGift(u8 *textState, u16 *windowId, bool32 isWonderNews) {
  if (!isWonderNews)
    return DoMysteryGiftYesNo(textState, windowId, TRUE,
...
static const u8 *GetClientResultMessage(bool32 *successMsg, bool8 isWonderNews,
                                        bool8 sourceIsFriend, u32 msgId) {
  const u8 *msg = NULL;
  *successMsg = FALSE;
  ...
  case CLI_MSG_CARD_RECEIVED:
    *successMsg = TRUE;
    msg = !sourceIsFriend ? gText_WonderCardReceived
                          : gText_WonderCardReceivedFrom;
    break;
  case CLI_MSG_NEWS_RECEIVED:
    *successMsg = TRUE;
    msg = !sourceIsFriend ? gText_WonderNewsReceived
  ...
  case CLI_MSG_CANT_ACCEPT:
    *successMsg = FALSE;
    msg = !isWonderNews ? gText_CantAcceptCardFromTrainer
...
static const u8 *GetServerResultMessage(bool32 *wonderSuccess,
                                        bool8 sourceIsFriend, u32 msgId) {
...
static bool32 PrintServerResultMessage(u8 *state, u16 *timer,
                                       bool8 sourceIsFriend, u32 msgId) {
...
// CLIENT states are for when the player is receiving a gift, and use
// mystery_gift_client.c link functions. SERVER states are for when the player
// is sending a gift, and use mystery_gift_server.c link functions. Other states
// handle the general Mystery Gift menu usage.
enum {
...
static void Task_MysteryGift(u8 taskId) {
  ...
  case MG_STATE_MAIN_MENU:
    ...
    switch (MysteryGift_HandleThreeOptionMenu(&data->textState, &data->var,
                                              FALSE)) {
    case 0: // "Wonder Cards"
  ...
  case MG_STATE_SOURCE_PROMPT_INPUT:
    ...
    switch (
        MysteryGift_HandleThreeOptionMenu(&data->textState, &data->var, TRUE)) {
  ...
  case MG_STATE_CLIENT_YES_NO:
    input = DoMysteryGiftYesNo(&data->textState, &data->var, FALSE,
  ...
  case MG_STATE_CLIENT_ASK_TOSS:
    // Player is receiving a new Wonder Card/News but needs to toss an existing
    // one to make room. Ask for confirmation.
    input = DoMysteryGiftYesNo(&data->textState, &data->var, FALSE,
  ...
  case MG_STATE_CLIENT_ASK_TOSS_UNRECEIVED:
    // Player has selected to toss a Wonder Card that they haven't received the
    // gift for. Ask for confirmation again.
    input = DoMysteryGiftYesNo(&data->textState, &data->var, FALSE,
  ...
  case MG_STATE_CLIENT_COMM_COMPLETED:
    if (PrintStringAndWait2Seconds(&data->textState,
  ...
  case MG_STATE_CLIENT_RESULT_MSG:
    msg = GetClientResultMessage(&successMsg, data->isWonderNews,
                                 data->sourceIsFriend, data->msgId);
    if (msg == NULL)
      msg = data->clientMsg;
    if (successMsg)
  ...
  case MG_STATE_HANDLE_GIFT_SELECT: {
    ...
    if (!data->isWonderNews) {
      if (IsSendingSavedWonderCardAllowed())
        result = HandleGiftSelectMenu(&data->textState, &data->var,
                                      data->isWonderNews, FALSE);
      else
        result = HandleGiftSelectMenu(&data->textState, &data->var,
                                      data->isWonderNews, TRUE);
  ...
  case MG_STATE_ASK_TOSS_UNRECEIVED:
    // Player has selected to toss a Wonder Card that they haven't received the
    // gift for. Ask for confirmation again.
    switch ((u32)DoMysteryGiftYesNo(&data->textState, &data->var, TRUE,
  ...
  case MG_STATE_SERVER_RESULT_MSG:
    if (PrintServerResultMessage(&data->textState, &data->var,
                                 data->sourceIsFriend, data->msgId)) {
...
u16 GetMysteryGiftBaseBlock(void) { return 0x1A9; }
