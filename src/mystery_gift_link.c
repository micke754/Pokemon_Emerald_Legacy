...
/*
    Handles the link connection functions used by the Mystery Gift
   client/server. Note: MysteryGiftLink is shortened to MGL for internal
   functions.
*/
...
void MysteryGiftLink_Init(struct MysteryGiftLink *link, u32 sendPlayerId,
                          u32 recvPlayerId) {
...
void MysteryGiftLink_InitSend(struct MysteryGiftLink *link, u32 ident,
...
void MysteryGiftLink_InitRecv(struct MysteryGiftLink *link, u32 ident,
                              void *dest) {
  link->state = 0;
  ...
  case 1:
    if (MGL_HasReceived(link->recvPlayerId)) {
      ...
      if (link->recvSize - blocksize <= 252) {
        MGL_ReceiveBlock(link->recvPlayerId, link->recvBuffer + blocksize,
                         link->recvSize - blocksize);
        link->recvCounter++;
        link->state++;
      } else {
    ...
    if (IsLinkTaskFinished()) {
      if (MGL_HasReceived(link->sendPlayerId)) {
        ...
        if (link->sendSize - blocksize <= 252) {
          SendBlock(0, link->sendBuffer + blocksize,
                    link->sendSize - blocksize);
          link->sendCounter++;
          link->state++;
        } else {
