...
static u8 AttacksThisTurn(
    u8 battlerId,
    u16 move); // Note: returns 1 if it's a charging turn, otherwise 2.
...
static const struct WindowTemplate sUnusedWinTemplate = {.bg = 0,
                                                         .tilemapLeft = 1,
                                                         .tilemapTop = 3,
                                                         .width = 7,
                                                         .height = 15,
...
static const u16 sLevelUpBanner_Pal[] =
    INCBIN_U16("graphics/battle_interface/level_up_banner.gbapal");
static const u32 sLevelUpBanner_Gfx[] =
    INCBIN_U32("graphics/battle_interface/level_up_banner.4bpp.lz");

// unused
static const u8 sRubyLevelUpStatBoxStats[] = {MON_DATA_MAX_HP, MON_DATA_SPATK,
                                              MON_DATA_ATK,    MON_DATA_SPDEF,
                                              MON_DATA_DEF,    MON_DATA_SPEED};
...
static const u16 sProtectSuccessRates[] = {USHRT_MAX, USHRT_MAX / 2,
                                           USHRT_MAX / 4, USHRT_MAX / 8};
...
static const u16 sMovesForbiddenToCopy[] = {
    MOVE_METRONOME,      MOVE_STRUGGLE,
    MOVE_SKETCH,         MOVE_MIMIC,
    MIMIC_FORBIDDEN_END, MOVE_COUNTER,
    MOVE_MIRROR_COAT,    MOVE_PROTECT,
    MOVE_DETECT,         MOVE_ENDURE,
    MOVE_DESTINY_BOND,   MOVE_SLEEP_TALK,
    MOVE_THIEF,          MOVE_FOLLOW_ME,
    MOVE_SNATCH,         MOVE_HELPING_HAND,
    MOVE_COVET,          MOVE_TRICK,
    MOVE_FOCUS_PUNCH,    METRONOME_FORBIDDEN_END};

static const u8 sFlailHpScaleToPowerTable[] = {1,  200, 4,  150, 9,  100,
                                               16, 80,  32, 40,  48, 20};
...
static const u16 sWeightToDamageTable[] = {100,  20, 250,  40,  500,    60,
                                           1000, 80, 2000, 100, 0xFFFF, 0xFFFF};

static const u16 sPickupItems[] = {
    ITEM_POTION,     ITEM_SUPER_POTION, ITEM_GREAT_BALL, ITEM_SUPER_REPEL,
    ITEM_ETHER,      ITEM_FULL_HEAL,    ITEM_ULTRA_BALL, ITEM_HYPER_POTION,
    ITEM_RARE_CANDY, ITEM_NUGGET,       ITEM_PP_UP,      ITEM_HP_UP,
    ITEM_PROTEIN,    ITEM_IRON,         ITEM_CARBOS,     ITEM_ZINC,
    ITEM_MAX_REVIVE, ITEM_MAX_ELIXIR,
};

static const u16 sRarePickupItems[] = {
    ITEM_FULL_RESTORE,    ITEM_NUGGET,        ITEM_KINGS_ROCK,
    ITEM_LEFTOVERS,       ITEM_BRIGHT_POWDER, ITEM_WHITE_HERB,
    ITEM_TM_FLAMETHROWER, ITEM_TM_ICE_BEAM,   ITEM_TM_THUNDERBOLT,
    ITEM_TM_EARTHQUAKE,   ITEM_RARE_CANDY,
};
...
static const u8 sBallCatchBonuses[] = {[ITEM_ULTRA_BALL - ITEM_ULTRA_BALL] = 20,
                                       ...
                                       [ITEM_SAFARI_BALL - ITEM_ULTRA_BALL] =
                                           15};

// In Battle Palace, moves are chosen based on the pokemons nature rather than
// by the player Moves are grouped into "Attack", "Defense", or "Support" (see
// PALACE_MOVE_GROUP_*) Each nature has a certain percent chance of selecting a
// move from a particular group and a separate percent chance for each group
// when at or below 50% HP The table below doesn't list percentages for Support
// because you can subtract the other two Support percentages are listed in
// comments off to the side instead
...
const ALIGNED(4) u8 gBattlePalaceNatureToMoveGroupLikelihood[NUM_NATURES][4] = {
    [NATURE_HARDY] = PALACE_STYLE(
        60, 10, 70, 10), // 32% support >= 50% HP, 32% support < 50% HP
...
static void JumpIfMoveFailed(u8 adder, u16 move) {
  ...
  if (gMoveResultFlags & MOVE_RESULT_NO_EFFECT) {
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_ABSORBING, gBattlerTarget, 0, 0,
                             move))
      return;
  }
...
static bool8 AccuracyCalcHelper(u16 move) {
  ...
  if ((WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_RAIN) &&
      ...
      (WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_HAIL) &&
       gBattleMoves[move].effect == EFFECT_BLIZZARD) ||
...
static void Cmd_accuracycheck(void) {
  ...
  if (move == NO_ACC_CALC || move == NO_ACC_CALC_CHECK_LOCK_ON) {
    ...
    if (holdEffect == HOLD_EFFECT_EVASION_UP ||
        gBattleMons[gBattlerTarget].ability == ABILITY_STENCH)
...
static void Cmd_ppreduce(void) {
  ...
  if (!gSpecialStatuses[gBattlerAttacker].ppNotAffectedByPressure) {
    ...
    case MOVE_TARGET_FOES_AND_ALLY:
      ppToDeduct +=
          AbilityBattleEffects(ABILITYEFFECT_COUNT_ON_FIELD, gBattlerAttacker,
                               ABILITY_PRESSURE, 0, 0);
      break;
    case MOVE_TARGET_BOTH:
    case MOVE_TARGET_OPPONENTS_FIELD:
      ppToDeduct +=
          AbilityBattleEffects(ABILITYEFFECT_COUNT_OTHER_SIDE, gBattlerAttacker,
                               ABILITY_PRESSURE, 0, 0);
      break;
    default:
  ...
  if (!(gHitMarker & (HITMARKER_NO_PPDEDUCT | HITMARKER_NO_ATTACKSTRING)) &&
    ...
    if (MOVE_IS_PERMANENT(gBattlerAttacker, gCurrMovePos)) {
      ...
      BtlController_EmitSetMonData(
          BUFFER_A, REQUEST_PPMOVE1_BATTLE + gCurrMovePos, 0,
...
static void Cmd_critcalc(void) {
  ...
  critChance = 2 * ((gBattleMons[gBattlerAttacker].status2 &
                     STATUS2_FOCUS_ENERGY) != 0) +
...
void AI_CalcDmg(u8 attacker, u8 defender) {
  ...
  gBattleMoveDamage = CalculateBaseDamage(
      &gBattleMons[attacker], &gBattleMons[defender], gCurrentMove, sideStatus,
...
u8 TypeCalc(u16 move, u8 attacker, u8 defender) {
  ...
  if (gBattleMons[defender].ability == ABILITY_WONDER_GUARD &&
      !(flags & MOVE_RESULT_MISSED) && AttacksThisTurn(attacker, move) == 2 &&
      (!(flags & MOVE_RESULT_SUPER_EFFECTIVE) ||
       ((flags &
         (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE)) ==
        (MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_NOT_VERY_EFFECTIVE))) &&
...
// The same as adjustnormaldamage except it doesn't check for false swipe move
// effect.
static void Cmd_adjustnormaldamage2(void) {
  u8 holdEffect, param;
  ...
  if ((gHitMarker & HITMARKER_NO_ANIMATIONS) &&
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)) {
      ...
      BtlController_EmitMoveAnimation(
          BUFFER_A, gCurrentMove, gBattleScripting.animTurn, gBattleMovePower,
...
static void Cmd_healthbarupdate(void) {
  ...
  if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)) {
    ...
    if (gBattleMons[gActiveBattler].status2 & STATUS2_SUBSTITUTE &&
      ...
      s32 maxPossibleDmgValue = 10000; // not present in R/S, ensures that huge
                                       // damage values don't change sign
...
// Update the active battler's HP and various HP trackers (Substitute, Bide,
// etc.)
static void Cmd_datahpupdate(void) {
  u32 moveType;
  ...
  // moveType will be used later to record for Counter/Mirror Coat whether this
  // was physical or special damage. For moves with a dynamic type that have
  // F_DYNAMIC_TYPE_IGNORE_PHYSICALITY set (in vanilla, just Hidden Power) this
  // will ignore the dynamic type and use the move's base type instead, meaning
  // (as a Normal type) Hidden Power will only ever trigger Counter. It also
  // means that Hidden Power Fire is unable to defrost targets.
  ...
  if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)) {
    ...
    if (gBattleMons[gActiveBattler].status2 & STATUS2_SUBSTITUTE &&
      ...
      if (gBattleMoveDamage < 0) {
        ...
        // Note: While physicalDmg/specialDmg below are only distinguished
        // between for Counter/Mirror Coat, they are
        //       used in combination as general damage trackers for other
        //       purposes. specialDmg is additionally used to help determine if
        //       a fire move should defrost the target.
        if (IS_TYPE_PHYSICAL(moveType) &&
          ...
          if (gBattlescriptCurrInstr[1] == BS_TARGET) {
            gProtectStructs[gActiveBattler].physicalBattlerId =
                gBattlerAttacker;
            gSpecialStatuses[gActiveBattler].physicalBattlerId =
                gBattlerAttacker;
          } else {
          ...
          if (gBattlescriptCurrInstr[1] == BS_TARGET) {
            ...
            gSpecialStatuses[gActiveBattler].specialBattlerId =
                gBattlerAttacker;
          } else {
      ...
      // Send updated HP
      BtlController_EmitSetMonData(BUFFER_A, REQUEST_HP_BATTLE, 0,
...
static void Cmd_printselectionstring(void) {
  ...
  BtlController_EmitPrintSelectionString(
      BUFFER_A, T2_READ_16(gBattlescriptCurrInstr + 1));
  MarkBattlerForControllerExec(gActiveBattler);
...
void SetMoveEffect(bool8 primary, u8 certain) {
  ...
  if (gBattleMons[gEffectBattler].ability == ABILITY_SHIELD_DUST &&
      !(gHitMarker & HITMARKER_STATUS_ABILITY_EFFECT) && !primary &&
  ...
  if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT) {
    ...
    if (statusChanged == TRUE) {
      ...
      if (sStatusFlagsForMoveEffects[gBattleCommunication[MOVE_EFFECT_BYTE]] ==
          STATUS1_SLEEP)
      ...
      BtlController_EmitSetMonData(BUFFER_A, REQUEST_STATUS_BATTLE, 0,
                                   sizeof(gBattleMons[gEffectBattler].status1),
                                   &gBattleMons[gEffectBattler].status1);
      MarkBattlerForControllerExec(gActiveBattler);
    ...
    if (gBattleMons[gEffectBattler].status2 &
      ...
      case MOVE_EFFECT_FLINCH:
        if (gBattleMons[gEffectBattler].ability == ABILITY_INNER_FOCUS) {
          ...
          if (GetBattlerTurnOrderNum(gEffectBattler) > gCurrentTurnActionNumber)
            gBattleMons[gEffectBattler].status2 |= sStatusFlagsForMoveEffects
      ...
      case MOVE_EFFECT_WRAP:
        if (gBattleMons[gEffectBattler].status2 & STATUS2_WRAPPED) {
          ...
          *(gBattleStruct->wrappedMove + gEffectBattler * 2 + 1) =
              gCurrentMove >> 8;
      ...
      case MOVE_EFFECT_STEAL_ITEM: {
        ...
        if (GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT &&
          ...
          gActiveBattler = gBattlerAttacker;
          BtlController_EmitSetMonData(BUFFER_A, REQUEST_HELDITEM_BATTLE, 0,
                                       sizeof(gLastUsedItem), &gLastUsedItem);
          MarkBattlerForControllerExec(gBattlerAttacker);

          gActiveBattler = gBattlerTarget;
          BtlController_EmitSetMonData(BUFFER_A, REQUEST_HELDITEM_BATTLE, 0,
                                       sizeof(gBattleMons[gBattlerTarget].item),
      ...
      case MOVE_EFFECT_PREVENT_ESCAPE:
        ...
        gDisableStructs[gBattlerTarget].battlerPreventingEscape =
            gBattlerAttacker;
        gBattlescriptCurrInstr++;
        break;
      ...
      case MOVE_EFFECT_REMOVE_PARALYSIS: // Smelling salts
        if (!(gBattleMons[gBattlerTarget].status1 & STATUS1_PARALYSIS)) {
          ...
          gActiveBattler = gBattlerTarget;
          BtlController_EmitSetMonData(
              BUFFER_A, REQUEST_STATUS_BATTLE, 0,
      ...
      case MOVE_EFFECT_THRASH:
        if (gBattleMons[gEffectBattler].status2 & STATUS2_LOCK_CONFUSE) {
          ...
          gBattleMons[gEffectBattler].status2 |= STATUS2_LOCK_CONFUSE_TURN(
              (Random() & 1) + 2); // thrash for 2-3 turns
        }
        break;
...
static void Cmd_seteffectprimary(void) { SetMoveEffect(TRUE, 0); }

static void Cmd_seteffectsecondary(void) { SetMoveEffect(FALSE, 0); }
    ...
    if (!(gAbsentBattlerFlags & gBitTable[gActiveBattler]) &&
      ...
      if (GetBattlerSide(gActiveBattler) == B_SIDE_PLAYER) {
        ...
        gBattleResults.lastOpponentSpecies =
            GetMonData(&gEnemyParty[gBattlerPartyIndexes[gActiveBattler]],
      ...
      if ((gStatuses3[gBattlerTarget] & STATUS3_GRUDGE) &&
        ...
        gActiveBattler = gBattlerAttacker;
        BtlController_EmitSetMonData(
            BUFFER_A, moveIndex + REQUEST_PPMOVE1_BATTLE, 0,
            sizeof(gBattleMons[gActiveBattler].pp[moveIndex]),
            &gBattleMons[gActiveBattler].pp[moveIndex]);
        MarkBattlerForControllerExec(gActiveBattler);

        PREPARE_MOVE_BUFFER(gBattleTextBuff1,
                            gBattleMons[gBattlerAttacker].moves[moveIndex])
      }
    } else {
      gBattlescriptCurrInstr += 7;
    }
...
static void Cmd_jumpifability(void) {
  ...
  if (gBattlescriptCurrInstr[1] == BS_ATTACKER_SIDE) {
    battlerId = AbilityBattleEffects(ABILITYEFFECT_CHECK_BATTLER_SIDE,
                                     gBattlerAttacker, ability, 0, 0);
    if (battlerId) {
    ...
    battlerId = AbilityBattleEffects(ABILITYEFFECT_CHECK_OTHER_SIDE,
                                     gBattlerAttacker, ability, 0, 0);
    if (battlerId) {
      gLastUsedAbility = ability;
      gBattlescriptCurrInstr = jumpPtr;
...
static void Cmd_getexp(void) {
  ...
  case 0: // check if should receive exp at all
    if (GetBattlerSide(gBattlerFainted) != B_SIDE_OPPONENT ||
        (gBattleTypeFlags &
         (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK |
          BATTLE_TYPE_TRAINER_HILL | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_SAFARI |
          BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_EREADER_TRAINER))) {
      gBattleScripting.getexpState = 6; // goto last case
    } else {
      gBattleScripting.getexpState++;
      gBattleStruct->givenExpMons |=
          gBitTable[gBattlerPartyIndexes[gBattlerFainted]];
    }
    break;
  ...
  case 2: // set exp value to the poke in expgetter_id and print message
    if (gBattleControllerExecFlags == 0) {
      item = GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId],
                        MON_DATA_HELD_ITEM);
      ...
      if (holdEffect != HOLD_EFFECT_EXP_SHARE &&
      ...
      } else if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId],
        ...
        *(&gBattleStruct->sentInPokes) >>= 1;
        ...
        if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId],
                       MON_DATA_HP)) {
          ...
          PrepareStringBattle(STRINGID_PKMNGAINEDEXP,
                              gBattleStruct->expGetterBattlerId);
  ...
  case 3: // Set stats and give exp
    if (gBattleControllerExecFlags == 0) {
      ...
      if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId],
          ...
          GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId],
                     MON_DATA_LEVEL) != MAX_LEVEL &&
          !levelCappedNuzlocke(GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL))) {
        gBattleResources->beforeLvlUp->stats[STAT_HP] = GetMonData(
            &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_MAX_HP);
        gBattleResources->beforeLvlUp->stats[STAT_ATK] = GetMonData(
            &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_ATK);
        gBattleResources->beforeLvlUp->stats[STAT_DEF] = GetMonData(
            &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_DEF);
        gBattleResources->beforeLvlUp->stats[STAT_SPEED] = GetMonData(
            &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPEED);
        gBattleResources->beforeLvlUp->stats[STAT_SPATK] = GetMonData(
            &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPATK);
        gBattleResources->beforeLvlUp->stats[STAT_SPDEF] = GetMonData(
            &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPDEF);

        gActiveBattler = gBattleStruct->expGetterBattlerId;
        BtlController_EmitExpUpdate(BUFFER_A, gBattleStruct->expGetterMonId,
                                    gBattleMoveDamage);
        MarkBattlerForControllerExec(gActiveBattler);
      }
      gBattleScripting.getexpState++;
  ...
  case 4: // lvl up if necessary
    if (gBattleControllerExecFlags == 0) {
      ...
      if (gBattleBufferB[gActiveBattler][0] == CONTROLLER_TWORETURNVALUES &&
        ...
        AdjustFriendship(&gPlayerParty[gBattleStruct->expGetterMonId],
        ...
        if (gBattlerPartyIndexes[0] == gBattleStruct->expGetterMonId &&
          ...
          gBattleMons[0].level = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL);
          gBattleMons[0].hp = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP);
          gBattleMons[0].maxHP = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_MAX_HP);
          gBattleMons[0].attack = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_ATK);
          gBattleMons[0].defense = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_DEF);
        ...
        if (gBattlerPartyIndexes[2] == gBattleStruct->expGetterMonId &&
          ...
          gBattleMons[2].level = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL);
          gBattleMons[2].hp = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP);
          gBattleMons[2].maxHP = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_MAX_HP);
          gBattleMons[2].attack = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_ATK);
          gBattleMons[2].defense = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_DEF);
          gBattleMons[2].speed = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPEED);
          gBattleMons[2].spDefense = GetMonData(
              &gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPDEF);
...
// For battles that aren't BATTLE_TYPE_LINK or BATTLE_TYPE_RECORDED_LINK, the
// only thing this command does is check whether the player has won/lost by
// totaling each team's HP. It then sets gBattleOutcome accordingly, if
// necessary.
static void Cmd_checkteamslost(void) {
  u16 HP_count = 0;
  s32 i;
  ...
  if (gBattleOutcome == 0 &&
    ...
    for (i = 0; i < gBattlersCount; i += 2) {
      if ((gHitMarker & HITMARKER_FAINTED2(i)) &&
          (!gSpecialStatuses[i].faintedHasReplacement))
        emptyPlayerSpots++;
    }
...
static void Cmd_jumpifabilitypresent(void) {
  if (AbilityBattleEffects(ABILITYEFFECT_CHECK_ON_FIELD, 0,
                           gBattlescriptCurrInstr[1], 0, 0))
...
static void Cmd_playanimation(void) {
  ...
  if (gBattlescriptCurrInstr[2] == B_ANIM_STATS_CHANGE ||
    ...
    BtlController_EmitBattleAnimation(BUFFER_A, gBattlescriptCurrInstr[2],
                                      *argumentPtr);
    MarkBattlerForControllerExec(gActiveBattler);
    gBattlescriptCurrInstr += 7;
...
// Same as playanimation, except it takes a pointer to some animation id,
// instead of taking the value directly
static void Cmd_playanimation_var(void) {
  const u16 *argumentPtr;
...
static void Cmd_setgraphicalstatchangevalues(void) {
  ...
  gBattleScripting.animArg1 =
      GET_STAT_BUFF_ID(gBattleScripting.statChanger) + value - 1;
  gBattleScripting.animArg2 = 0;
  gBattlescriptCurrInstr++;
}
...
static void Cmd_playstatchangeanimation(void) {
    ...
    while (statsToCheck != 0) {
      if (statsToCheck & 1) {
        if (gBattlescriptCurrInstr[3] & STAT_CHANGE_CANT_PREVENT) {
          if (gBattleMons[gActiveBattler].statStages[currStat] >
              MIN_STAT_STAGE) {
  ...
  if (gBattlescriptCurrInstr[3] & STAT_CHANGE_MULTIPLE_STATS &&
      changeableStatsCount < 2) {
    gBattlescriptCurrInstr += 4;
  } else if (changeableStatsCount != 0 && !gBattleScripting.statAnimPlayed) {
    BtlController_EmitBattleAnimation(BUFFER_A, B_ANIM_STATS_CHANGE,
                                      statAnimId);
    MarkBattlerForControllerExec(gActiveBattler);
    if (gBattlescriptCurrInstr[3] & STAT_CHANGE_MULTIPLE_STATS &&
...
static void Cmd_moveend(void) {
  ...
  do {
    ...
    case MOVEEND_SYNCHRONIZE_TARGET: // target synchronize
      ...
      gBattleScripting.moveendState++;
      break;
    case MOVEEND_ON_DAMAGE_ABILITIES: // Such as abilities activating on contact
                                      // (Effect Spore, Rough Skin, etc.).
      if (AbilityBattleEffects(ABILITYEFFECT_ON_DAMAGE, gBattlerTarget, 0, 0,
                               0))
        effect = TRUE;
      gBattleScripting.moveendState++;
      break;
    case MOVEEND_IMMUNITY_ABILITIES: // status immunities
      if (AbilityBattleEffects(ABILITYEFFECT_IMMUNITY, 0, 0, 0, 0))
        effect = TRUE; // it loops through all battlers, so we increment after
                       // its done with all battlers
      else
        gBattleScripting.moveendState++;
      break;
    case MOVEEND_SYNCHRONIZE_ATTACKER: // attacker synchronize
      if (AbilityBattleEffects(ABILITYEFFECT_ATK_SYNCHRONIZE, gBattlerAttacker,
                               0, 0, 0))
        effect = TRUE;
      gBattleScripting.moveendState++;
      break;
    case MOVEEND_CHOICE_MOVE: // update choice band move
      if (gHitMarker & HITMARKER_OBEYS &&
           ...
           *choicedMoveAtk == MOVE_UNAVAILABLE)) {
    ...
    case MOVEEND_MIRROR_MOVE: // mirror move
      if (!(gAbsentBattlerFlags & gBitTable[gBattlerAttacker]) &&
        ...
        *(gBattleStruct->lastTakenMove + gBattlerTarget * 2 + 1) =
            gChosenMove >> 8;

        target = gBattlerTarget;
        attacker = gBattlerAttacker;
        *(attacker * 2 + target * 8 + (u8 *)(gBattleStruct->lastTakenMoveFrom) +
          0) = gChosenMove;
    ...
    case MOVEEND_NEXT_TARGET: // For moves hitting two opposing Pokémon.
      if (!(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE) &&
          ...
          !(gHitMarker & HITMARKER_NO_ATTACKSTRING)) {
        u8 battlerId = GetBattlerAtPosition(
            BATTLE_PARTNER(GetBattlerPosition(gBattlerTarget)));
  ...
  if (gBattleMons[gBattlerTarget].ability == ABILITY_LEVITATE &&
    ...
    while (TYPE_EFFECT_ATK_TYPE(i) != TYPE_ENDTABLE) {
      ...
      if (TYPE_EFFECT_ATK_TYPE(i) == moveType) {
        ...
        // check type2
        if (TYPE_EFFECT_DEF_TYPE(i) == gBattleMons[gBattlerTarget].type2) {
          if (gBattleMons[gBattlerTarget].type1 !=
                  gBattleMons[gBattlerTarget].type2 &&
              TYPE_EFFECT_MULTIPLIER(i) == TYPE_MUL_NO_EFFECT) {
            gMoveResultFlags |= MOVE_RESULT_DOESNT_AFFECT_FOE;
            break;
...
static void Cmd_getswitchedmondata(void) {
  ...
  BtlController_EmitGetMonData(BUFFER_A, REQUEST_ALL_BATTLE,
...
static void Cmd_switchindataupdate(void) {
  ...
  gBattleMons[gActiveBattler].ability =
      GetAbilityBySpecies(gBattleMons[gActiveBattler].species,
                          gBattleMons[gActiveBattler].abilityNum);

  // check knocked off item
  i = GetBattlerSide(gActiveBattler);
  if (gWishFutureKnock.knockedOffMons[i] &
      gBitTable[gBattlerPartyIndexes[gActiveBattler]]) {
  ...
  PREPARE_MON_NICK_BUFFER(gBattleTextBuff1, gActiveBattler,
                          gBattlerPartyIndexes[gActiveBattler]);
...
static void Cmd_switchinanim(void) {
  ...
  if (GetBattlerSide(gActiveBattler) == B_SIDE_OPPONENT &&
      !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_EREADER_TRAINER |
  ...
  BtlController_EmitSwitchInAnim(BUFFER_A, gBattlerPartyIndexes[gActiveBattler],
                                 gBattlescriptCurrInstr[2]);
  MarkBattlerForControllerExec(gActiveBattler);
...
// Note that this is not used by the Switch action, only replacing fainted
// Pokémon or Baton Pass
static void ChooseMonToSendOut(u8 slotId) {
  ...
  *(gBattleStruct->monToSwitchIntoId + gActiveBattler) = PARTY_SIZE;
  ...
  BtlController_EmitChoosePokemon(
      BUFFER_A, PARTY_ACTION_SEND_OUT, slotId, ABILITY_NONE,
...
static void Cmd_openpartyscreen(void) {
  ...
  if (gBattlescriptCurrInstr[1] == BS_FAINTED_LINK_MULTIPLE_1) {
    if ((gBattleTypeFlags & BATTLE_TYPE_MULTI) ||
      ...
      bool8 hasReplacement_0, hasReplacement_1, hasReplacement_2,
          hasReplacement_3;
      ...
      if (gBitTable[2] & hitmarkerFaintBits &&
          !(gBitTable[0] & hitmarkerFaintBits)) {
        gActiveBattler = 2;
        if (HasNoMonsToSwitch(gActiveBattler, PARTY_SIZE, PARTY_SIZE)) {
          gAbsentBattlerFlags |= gBitTable[gActiveBattler];
          gHitMarker &= ~HITMARKER_FAINTED(gActiveBattler);
      ...
      if (gBitTable[3] & hitmarkerFaintBits &&
          !(gBitTable[1] & hitmarkerFaintBits)) {
        gActiveBattler = 3;
        if (HasNoMonsToSwitch(gActiveBattler, PARTY_SIZE, PARTY_SIZE)) {
          gAbsentBattlerFlags |= gBitTable[gActiveBattler];
          gHitMarker &= ~HITMARKER_FAINTED(gActiveBattler);
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_MULTI)) {
      if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE) {
        ...
        if (gBitTable[2] & hitmarkerFaintBits &&
            gBitTable[0] & hitmarkerFaintBits) {
          gActiveBattler = 2;
          if (HasNoMonsToSwitch(gActiveBattler, gBattleBufferB[0][1],
                                PARTY_SIZE)) {
            gAbsentBattlerFlags |= gBitTable[gActiveBattler];
            gHitMarker &= ~HITMARKER_FAINTED(gActiveBattler);
            BtlController_EmitCantSwitch(BUFFER_A);
        ...
        if (gBitTable[3] & hitmarkerFaintBits &&
            hitmarkerFaintBits & gBitTable[1]) {
          gActiveBattler = 3;
          if (HasNoMonsToSwitch(gActiveBattler, gBattleBufferB[1][1],
                                PARTY_SIZE)) {
            gAbsentBattlerFlags |= gBitTable[gActiveBattler];
            gHitMarker &= ~HITMARKER_FAINTED(gActiveBattler);
            BtlController_EmitCantSwitch(BUFFER_A);
    ...
    if (gBattlescriptCurrInstr[1] & PARTY_SCREEN_OPTIONAL)
      hitmarkerFaintBits =
          PARTY_ACTION_CHOOSE_MON; // Used here as the caseId for the EmitChoose
                                   // function.
    else
    ...
    if (gSpecialStatuses[battlerId].faintedHasReplacement) {
      ...
      *(gBattleStruct->monToSwitchIntoId + gActiveBattler) = PARTY_SIZE;
      ...
      BtlController_EmitChoosePokemon(
          BUFFER_A, hitmarkerFaintBits,
          *(gBattleStruct->monToSwitchIntoId + BATTLE_PARTNER(gActiveBattler)),
          ABILITY_NONE, gBattleStruct->battlerPartyOrders[gActiveBattler]);
      MarkBattlerForControllerExec(gActiveBattler);
      ...
      if (gBattleTypeFlags & BATTLE_TYPE_MULTI) {
        ...
        gActiveBattler = GetBattlerAtPosition(
            BATTLE_OPPOSITE(GetBattlerPosition(battlerId)));
        if (gAbsentBattlerFlags & gBitTable[gActiveBattler])
  ...
  case 3:
    ...
    *(gBattleStruct->monToSwitchIntoId + gActiveBattler) =
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK &&
      ...
      *(gActiveBattler * 3 + (u8 *)(gBattleStruct->battlerPartyOrders) + 0) &=
          0xF;
      *(gActiveBattler * 3 + (u8 *)(gBattleStruct->battlerPartyOrders) + 0) |=
          (gBattleBufferB[gActiveBattler][2] & 0xF0);
      *(gActiveBattler * 3 + (u8 *)(gBattleStruct->battlerPartyOrders) + 1) =
          gBattleBufferB[gActiveBattler][3];

      *((BATTLE_PARTNER(gActiveBattler)) * 3 +
        (u8 *)(gBattleStruct->battlerPartyOrders) + 0) &= (0xF0);
      *((BATTLE_PARTNER(gActiveBattler)) * 3 +
        (u8 *)(gBattleStruct->battlerPartyOrders) + 0) |=
          (gBattleBufferB[gActiveBattler][2] & 0xF0) >> 4;
      *((BATTLE_PARTNER(gActiveBattler)) * 3 +
        (u8 *)(gBattleStruct->battlerPartyOrders) + 2) =
          gBattleBufferB[gActiveBattler][3];
    } else if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER) {
    ...
    PREPARE_SPECIES_BUFFER(gBattleTextBuff1,
                           gBattleMons[gBattlerAttacker].species)
    PREPARE_MON_NICK_BUFFER(gBattleTextBuff2, gActiveBattler,
                            gBattleBufferB[gActiveBattler][1])
...
static void Cmd_switchineffects(void) {
  ...
  if (!(gSideStatuses[GetBattlerSide(gActiveBattler)] &
    ...
    spikesDmg =
        (5 - gSideTimers[GetBattlerSide(gActiveBattler)].spikesAmount) * 2;
    ...
    // There is a hack here to ensure the truant counter will be 0 when the
    // battler's next turn starts. The truant counter is not updated in the case
    // where a mon switches in after a lost judgment in the battle arena.
...
static void Cmd_fanfare(void) {
  ...
  BtlController_EmitPlayFanfareOrBGM(
      BUFFER_A, T2_READ_16(gBattlescriptCurrInstr + 1), FALSE);
  MarkBattlerForControllerExec(gActiveBattler);
...
static void Cmd_handlelearnnewmove(void) {
  ...
  while (learnMove == MON_ALREADY_KNOWS_MOVE)
    learnMove = MonTryLearningNewMove(
        &gPlayerParty[gBattleStruct->expGetterMonId], FALSE);
  ...
  if (learnMove == MOVE_NONE) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE) {
      ...
      if (gBattlerPartyIndexes[gActiveBattler] ==
              gBattleStruct->expGetterMonId &&
          !(gBattleMons[gActiveBattler].status2 & STATUS2_TRANSFORMED)) {
        GiveMoveToBattleMon(&gBattleMons[gActiveBattler], learnMove);
      }
  ...
  case 2:
    if (!gPaletteFade.active) {
      ...
      ShowSelectMovePokemonSummaryScreen(
          gPlayerParty, gBattleStruct->expGetterMonId, gPlayerPartyCount - 1,
  ...
  case 4:
    if (!gPaletteFade.active && gMain.callback2 == BattleMainCB2) {
      ...
      if (movePosition == MAX_MON_MOVES) {
        ...
        RemoveMonPPBonus(&gPlayerParty[gBattleStruct->expGetterMonId],
                         movePosition);
        SetMonMoveSlot(&gPlayerParty[gBattleStruct->expGetterMonId],
                       gMoveToLearn, movePosition);

        if (gBattlerPartyIndexes[0] == gBattleStruct->expGetterMonId &&
            MOVE_IS_PERMANENT(0, movePosition)) {
...
static u32 GetTrainerMoneyToGive(u16 trainerId) {
  ...
  if (trainerId == TRAINER_SECRET_BASE) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS)
      ...
      moneyReward = 4 * lastMonLevel * gBattleStruct->moneyMultiplier * 2 *
                    gTrainerMoneyTable[i].value;
    else
      moneyReward = 4 * lastMonLevel * gBattleStruct->moneyMultiplier *
                    gTrainerMoneyTable[i].value;
...
static void Cmd_statusanimation(void) {
  if (gBattleControllerExecFlags == 0) {
    ...
    if (!(gStatuses3[gActiveBattler] & STATUS3_SEMI_INVULNERABLE) &&
        gDisableStructs[gActiveBattler].substituteHP == 0 &&
        !(gHitMarker & HITMARKER_NO_ANIMATIONS)) {
      BtlController_EmitStatusAnimation(BUFFER_A, FALSE,
                                        gBattleMons[gActiveBattler].status1);
      MarkBattlerForControllerExec(gActiveBattler);
    }
    gBattlescriptCurrInstr += 2;
...
static void Cmd_status2animation(void) {
  ...
  if (gBattleControllerExecFlags == 0) {
    ...
    if (!(gStatuses3[gActiveBattler] & STATUS3_SEMI_INVULNERABLE) &&
        gDisableStructs[gActiveBattler].substituteHP == 0 &&
        !(gHitMarker & HITMARKER_NO_ANIMATIONS)) {
      BtlController_EmitStatusAnimation(BUFFER_A, TRUE,
                                        gBattleMons[gActiveBattler].status2 &
                                            wantedToAnimate);
      MarkBattlerForControllerExec(gActiveBattler);
    }
...
static void Cmd_chosenstatusanimation(void) {
  ...
  if (gBattleControllerExecFlags == 0) {
    ...
    if (!(gStatuses3[gActiveBattler] & STATUS3_SEMI_INVULNERABLE) &&
        gDisableStructs[gActiveBattler].substituteHP == 0 &&
        !(gHitMarker & HITMARKER_NO_ANIMATIONS)) {
      BtlController_EmitStatusAnimation(BUFFER_A, gBattlescriptCurrInstr[2],
                                        wantedStatus);
      MarkBattlerForControllerExec(gActiveBattler);
    }
    gBattlescriptCurrInstr += 7;
...
static void Cmd_removeitem(void) {
  ...
  *usedHeldItem = gBattleMons[gActiveBattler].item;
  ...
  BtlController_EmitSetMonData(BUFFER_A, REQUEST_HELDITEM_BATTLE, 0,
                               sizeof(gBattleMons[gActiveBattler].item),
                               &gBattleMons[gActiveBattler].item);
  MarkBattlerForControllerExec(gActiveBattler);
...
static void Cmd_atknameinbuff1(void) {
  PREPARE_MON_NICK_BUFFER(gBattleTextBuff1, gBattlerAttacker,
...
static void DrawLevelUpWindow1(void) {
  ...
  GetMonLevelUpWindowStats(&gPlayerParty[gBattleStruct->expGetterMonId],
                           currStats);
  DrawLevelUpWindowPg1(B_WIN_LEVEL_UP_BOX, gBattleResources->beforeLvlUp->stats,
                       currStats, TEXT_DYNAMIC_COLOR_5, TEXT_DYNAMIC_COLOR_4,
...
static void DrawLevelUpWindow2(void) {
  ...
  GetMonLevelUpWindowStats(&gPlayerParty[gBattleStruct->expGetterMonId],
                           currStats);
  DrawLevelUpWindowPg2(B_WIN_LEVEL_UP_BOX, currStats, TEXT_DYNAMIC_COLOR_5,
...
static void DrawLevelUpBannerText(void) {
  ...
  *(txtPtr)++ = CHAR_LV_2;
  ...
  var = (u32)(txtPtr);
  txtPtr =
      ConvertIntToDecimalStringN(txtPtr, monLevel, STR_CONV_MODE_LEFT_ALIGN, 3);
  var = (u32)(txtPtr)-var;
...
static void PutMonIconOnLvlUpBanner(void) {
  ...
  u16 species = GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId],
                           MON_DATA_SPECIES);
  u32 personality = GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId],
...
static void Cmd_hpthresholds(void) {
  ...
  if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE)) {
    ...
    result = gBattleMons[opposingBattler].hp * 100 /
             gBattleMons[opposingBattler].maxHP;
    if (result == 0)
      result = 1;
...
static void Cmd_hpthresholds2(void) {
  ...
  if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE)) {
    ...
    opposingBattler = BATTLE_OPPOSITE(gActiveBattler);
    hpSwitchout =
        *(gBattleStruct->hpOnSwitchout + GetBattlerSide(opposingBattler));
    result =
        (hpSwitchout - gBattleMons[opposingBattler].hp) * 100 / hpSwitchout;
...
static void Cmd_useitemonopponent(void) {
  ...
  PokemonUseItemEffects(&gEnemyParty[gBattlerPartyIndexes[gBattlerAttacker]],
                        gLastUsedItem, gBattlerPartyIndexes[gBattlerAttacker],
                        0, TRUE);
  gBattlescriptCurrInstr++;
}
...
static void Cmd_various(void) {
  ...
  case VARIOUS_PALACE_FLAVOR_TEXT:
    // Try and print end-of-turn Battle Palace flavor text (e.g. "A glint
    // appears in mon's eyes")
    ...
    if (!(gBattleStruct->palaceFlags & gBitTable[gActiveBattler]) &&
        gBattleMons[gActiveBattler].maxHP / 2 >=
            gBattleMons[gActiveBattler].hp &&
        gBattleMons[gActiveBattler].hp != 0 &&
        !(gBattleMons[gActiveBattler].status1 & STATUS1_SLEEP)) {
      gBattleStruct->palaceFlags |= gBitTable[gActiveBattler];
  ...
  case VARIOUS_ARENA_JUDGMENT_WINDOW:
    ...
    // Return without advancing the current instruction so that it will be
    // called again.
...
static void Cmd_setprotectlike(void) {
  ...
  if (lastMove != MOVE_PROTECT && lastMove != MOVE_DETECT &&
...
static void Cmd_tryexplosion(void) {
  ...
  if (gBattlerTarget == gBattlersCount) {
    ...
    // Find first target
    for (gBattlerTarget = 0; gBattlerTarget < gBattlersCount;
         gBattlerTarget++) {
...
static void Cmd_trymirrormove(void) {
  ...
  for (validMovesCount = 0, i = 0; i < gBattlersCount; i++) {
    if (i != gBattlerAttacker) {
      move = T1_READ_16(i * 2 + gBattlerAttacker * 8 +
                        gBattleStruct->lastTakenMoveFrom);
...
static void Cmd_setreflect(void) {
  if (gSideStatuses[GET_BATTLER_SIDE(gBattlerAttacker)] & SIDE_STATUS_REFLECT) {
    ...
    gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].reflectBattlerId =
        gBattlerAttacker;
...
static void Cmd_nop(void) { gBattlescriptCurrInstr++; }
    ...
    if (gBattleCommunication[MISS_TYPE] != B_MSG_PROTECTED) {
      gBattleMoveDamage =
          CalculateBaseDamage(&gBattleMons[gBattlerAttacker],
                              &gBattleMons[gBattlerTarget], gCurrentMove,
...
static void Cmd_stockpiletohpheal(void) {
  ...
  } else if (gBattleMons[gBattlerAttacker].maxHP ==
...
static u8 ChangeStatBuffs(s8 statValue, u8 statId, u8 flags, const u8 *BS_ptr) {
  ...
  if (statValue <= -1) // Stat decrease.
  {
    if (gSideTimers[GET_BATTLER_SIDE(gActiveBattler)].mistTimer && !certain &&
      ...
      return STAT_CHANGE_DIDNT_WORK;
    } else if (gBattleMons[gActiveBattler].ability == ABILITY_SHIELD_DUST &&
               flags == 0) {
      return STAT_CHANGE_DIDNT_WORK;
    } else // try to decrease
    {
      ...
      if (gBattleMons[gActiveBattler].statStages[statId] == MIN_STAT_STAGE)
        ...
        gBattleCommunication[MULTISTRING_CHOOSER] =
            (gBattlerTarget == gActiveBattler); // B_MSG_ATTACKER_STAT_FELL or
  ...
  } else // stat increase
  {
    ...
    if (gBattleMons[gActiveBattler].statStages[statId] == MAX_STAT_STAGE)
      ...
      gBattleCommunication[MULTISTRING_CHOOSER] =
          (gBattlerTarget == gActiveBattler); // B_MSG_ATTACKER_STAT_ROSE or
...
static void Cmd_statbuffchange(void) {
  ...
  if (ChangeStatBuffs(gBattleScripting.statChanger & 0xF0,
                      ...
                      gBattlescriptCurrInstr[1], jumpPtr) == STAT_CHANGE_WORKED)
    gBattlescriptCurrInstr += 6;
}
...
static bool8 TryDoForceSwitchOut(void) {
  if (gBattleMons[gBattlerAttacker].level >=
      gBattleMons[gBattlerTarget].level) {
    *(gBattleStruct->battlerPartyIndexes + gBattlerTarget) =
  ...
  if ((gBattleTypeFlags & BATTLE_TYPE_TRAINER)) {
    ...
    if ((gBattleTypeFlags & BATTLE_TYPE_BATTLE_TOWER &&
      ...
      battler1PartyId = gBattlerPartyIndexes[BATTLE_PARTNER(gBattlerTarget)];
               ...
               (gBattleTypeFlags & BATTLE_TYPE_MULTI &&
                gBattleTypeFlags & BATTLE_TYPE_RECORDED_LINK)) {
      if (GetLinkTrainerFlankId(GetBattlerMultiplayerId(gBattlerTarget)) ==
          B_FLANK_RIGHT) {
        firstMonId = PARTY_SIZE / 2;
        lastMonId = PARTY_SIZE;
      } else {
      ...
      if (GetBattlerSide(gBattlerTarget) == B_SIDE_PLAYER) {
        ...
        minNeeded =
            2; // since there are two opponents, it has to be a double battle
      } else {
        if ((gBattlerTarget & BIT_FLANK) != B_FLANK_LEFT) {
          firstMonId = PARTY_SIZE / 2;
          lastMonId = PARTY_SIZE;
      ...
      minNeeded = 1;
      battler2PartyId =
          gBattlerPartyIndexes[gBattlerTarget]; // there is only one Pokémon out
                                                // in single battles
          ...
          do {
            ...
            i += firstMonId;
                 ...
                 GetMonData(&party[i], MON_DATA_HP) ==
                     0); // should be one while loop, but that doesn't match.
      }
...
static void Cmd_tryconversiontypechange(void) {
  ...
  for (moveChecked = 0; moveChecked < validMoves; moveChecked++) {
    moveType =
        gBattleMoves[gBattleMons[gBattlerAttacker].moves[moveChecked]].type;
...
static void Cmd_setlightscreen(void) {
  if (gSideStatuses[GET_BATTLER_SIDE(gBattlerAttacker)] &
      SIDE_STATUS_LIGHTSCREEN) {
    gMoveResultFlags |= MOVE_RESULT_MISSED;
    gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SIDE_STATUS_FAILED;
  } else {
    gSideStatuses[GET_BATTLER_SIDE(gBattlerAttacker)] |=
        SIDE_STATUS_LIGHTSCREEN;
    gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].lightscreenTimer = 5;
    gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].lightscreenBattlerId =
        gBattlerAttacker;
    ...
    if (!(gStatuses3[gBattlerTarget] & STATUS3_ALWAYS_HITS)) {
      chance = gBattleMoves[gCurrentMove].accuracy +
               (gBattleMons[gBattlerAttacker].level -
                gBattleMons[gBattlerTarget].level);
      if (Random() % 100 + 1 < chance && gBattleMons[gBattlerAttacker].level >=
                                             gBattleMons[gBattlerTarget].level)
        chance = TRUE;
      else
        chance = FALSE;
    } else if (gDisableStructs[gBattlerTarget].battlerWithSureHit ==
                   gBattlerAttacker &&
               gBattleMons[gBattlerAttacker].level >=
                   gBattleMons[gBattlerTarget].level) {
      chance = TRUE;
...
static void Cmd_tryinfatuating(void) {
  ...
  if (gBattleMons[gBattlerTarget].ability == ABILITY_OBLIVIOUS) {
    ...
    if (GetGenderFromSpeciesAndPersonality(speciesAttacker,
                                           personalityAttacker) ==
            GetGenderFromSpeciesAndPersonality(speciesTarget,
                                               personalityTarget) ||
        gBattleMons[gBattlerTarget].status2 & STATUS2_INFATUATION ||
        GetGenderFromSpeciesAndPersonality(
            speciesAttacker, personalityAttacker) == MON_GENDERLESS ||
        GetGenderFromSpeciesAndPersonality(speciesTarget, personalityTarget) ==
            MON_GENDERLESS) {
      gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    } else {
      gBattleMons[gBattlerTarget].status2 |=
          STATUS2_INFATUATED_WITH(gBattlerAttacker);
      gBattlescriptCurrInstr += 5;
    }
  }
}
...
static void Cmd_updatestatusicon(void) {
  ...
  if (gBattlescriptCurrInstr[1] != BS_ATTACKER_WITH_PARTNER) {
    ...
    if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE)) {
      gActiveBattler = GetBattlerAtPosition(
          BATTLE_PARTNER(GetBattlerPosition(gBattlerAttacker)));
      if (!(gAbsentBattlerFlags & gBitTable[gActiveBattler])) {
        BtlController_EmitStatusIconUpdate(BUFFER_A,
                                           gBattleMons[gActiveBattler].status1,
                                           gBattleMons[gActiveBattler].status2);
...
static void Cmd_setmist(void) {
  if (gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].mistTimer) {
    ...
    gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].mistBattlerId =
        gBattlerAttacker;
...
static void Cmd_setsubstitute(void) {
  ...
  if (gBattleMons[gBattlerAttacker].hp <= hp) {
    ...
    gBattleMoveDamage =
        gBattleMons[gBattlerAttacker].maxHP /
        4; // one bit value will only work for Pokémon which max hp can go to
...
static void Cmd_mimicattackcopy(void) {
  ...
  if (IsMoveUncopyableByMimic(gLastMoves[gBattlerTarget]) ||
    ...
    if (i == MAX_MON_MOVES) {
      gBattleMons[gBattlerAttacker].moves[gCurrMovePos] =
      ...
      gDisableStructs[gBattlerAttacker].mimickedMoves |=
          gBitTable[gCurrMovePos];
      gBattlescriptCurrInstr += 5;
    } else {
      gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    }
...
static void Cmd_psywavedamageeffect(void) {
  ...
  randDamage *= 10;
  gBattleMoveDamage =
      gBattleMons[gBattlerAttacker].level * (randDamage + 50) / 100;
  gBattlescriptCurrInstr++;
}
...
static void Cmd_counterdamagecalculator(void) {
  ...
  u8 sideTarget =
      GetBattlerSide(gProtectStructs[gBattlerAttacker].physicalBattlerId);
...
static void Cmd_mirrorcoatdamagecalculator(void) {
  u8 sideAttacker = GetBattlerSide(gBattlerAttacker);
  u8 sideTarget =
      GetBattlerSide(gProtectStructs[gBattlerAttacker].specialBattlerId);
...
static void Cmd_disablelastusedattack(void) {
  ...
  if (gDisableStructs[gBattlerTarget].disabledMove == MOVE_NONE &&
    ...
    gDisableStructs[gBattlerTarget].disabledMove =
        gBattleMons[gBattlerTarget].moves[i];
...
static void Cmd_trysetencore(void) {
  ...
  if (gDisableStructs[gBattlerTarget].encoredMove == MOVE_NONE &&
      i != MAX_MON_MOVES && gBattleMons[gBattlerTarget].pp[i] != 0) {
    gDisableStructs[gBattlerTarget].encoredMove =
        gBattleMons[gBattlerTarget].moves[i];
...
static void Cmd_painsplitdmgcalc(void) {
  if (!(gBattleMons[gBattlerTarget].status2 & STATUS2_SUBSTITUTE)) {
    s32 hpDiff =
        (gBattleMons[gBattlerAttacker].hp + gBattleMons[gBattlerTarget].hp) / 2;
    s32 painSplitHp = gBattleMoveDamage =
        gBattleMons[gBattlerTarget].hp - hpDiff;
...
static void Cmd_settypetorandomresistance(void) {
  if (gLastLandedMoves[gBattlerAttacker] == MOVE_NONE ||
    ...
    gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
             ...
             gBattleMons[gLastHitBy[gBattlerAttacker]].status2 &
                 STATUS2_MULTIPLETURNS) {
    gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
  } else {
    s32 i, j, rands;
    ...
    for (j = 0, rands = 0; rands < sizeof(gTypeEffectiveness);
         j += 3, rands += 3) {
...
static void Cmd_copymovepermanently(void) {
  ...
  if (!(gBattleMons[gBattlerAttacker].status2 & STATUS2_TRANSFORMED) &&
    ...
    } else // sketch worked
    {
      ...
      BtlController_EmitSetMonData(BUFFER_A, REQUEST_MOVES_PP_BATTLE, 0,
                                   sizeof(movePpData), &movePpData);
      MarkBattlerForControllerExec(gActiveBattler);
...
static void TrySetDestinyBondToHappen(void) {
  ...
  if (gBattleMons[gBattlerTarget].status2 & STATUS2_DESTINY_BOND &&
      sideAttacker != sideTarget && !(gHitMarker & HITMARKER_GRUDGE)) {
    gHitMarker |= HITMARKER_DESTINYBOND;
  }
}
...
static void Cmd_remaininghptopower(void) {
  s32 i;
  s32 hpFraction = GetScaledHPFraction(gBattleMons[gBattlerAttacker].hp,
                                       gBattleMons[gBattlerAttacker].maxHP, 48);
...
static void Cmd_tryspiteppreduce(void) {
  if (gLastMoves[gBattlerTarget] != MOVE_NONE &&
    ...
    if (i != MAX_MON_MOVES && gBattleMons[gBattlerTarget].pp[i] > 1) {
      ...
      ConvertIntToDecimalStringN(gBattleTextBuff2, ppToDeduct,
      ...
      if (!(gDisableStructs[gActiveBattler].mimickedMoves & gBitTable[i]) &&
          !(gBattleMons[gActiveBattler].status2 & STATUS2_TRANSFORMED)) {
        BtlController_EmitSetMonData(BUFFER_A, REQUEST_PPMOVE1_BATTLE + i, 0,
                                     sizeof(gBattleMons[gActiveBattler].pp[i]),
...
static void Cmd_healpartystatus(void) {
  ...
  if (gCurrentMove == MOVE_HEAL_BELL) {
    ...
    if (gBattleMons[gBattlerAttacker].ability != ABILITY_SOUNDPROOF) {
      ...
      RecordAbilityBattle(gBattlerAttacker,
                          gBattleMons[gBattlerAttacker].ability);
      gBattleCommunication[MULTISTRING_CHOOSER] |=
          B_MSG_BELL_SOUNDPROOF_ATTACKER;
    }

    gActiveBattler = gBattleScripting.battler = GetBattlerAtPosition(
        BATTLE_PARTNER(GetBattlerPosition(gBattlerAttacker)));
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE &&
      ...
      if (gBattleMons[gActiveBattler].ability != ABILITY_SOUNDPROOF) {
        ...
        RecordAbilityBattle(gActiveBattler,
                            gBattleMons[gActiveBattler].ability);
        gBattleCommunication[MULTISTRING_CHOOSER] |=
    ...
    // Because the above MULTISTRING_CHOOSER are ORd, if both are set then it
    // will be B_MSG_BELL_BOTH_SOUNDPROOF
...
static void Cmd_rolloutdamagecalculation(void) {
  if (gMoveResultFlags & MOVE_RESULT_NO_EFFECT) {
    ...
    if (!(gBattleMons[gBattlerAttacker].status2 &
          STATUS2_MULTIPLETURNS)) // first hit
    {
...
static void Cmd_jumpifconfusedandstatmaxed(void) {
  if (gBattleMons[gBattlerTarget].status2 & STATUS2_CONFUSION &&
      gBattleMons[gBattlerTarget].statStages[gBattlescriptCurrInstr[1]] ==
          MAX_STAT_STAGE)
    gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 2);
  else
    gBattlescriptCurrInstr += 6;
...
static void Cmd_friendshiptodamagecalculation(void) {
  if (gBattleMoves[gCurrentMove].effect == EFFECT_RETURN)
    ...
    gDynamicBasePower =
        10 * (MAX_FRIENDSHIP - gBattleMons[gBattlerAttacker].friendship) / 25;
...
static void Cmd_setsafeguard(void) {
  if (gSideStatuses[GET_BATTLER_SIDE(gBattlerAttacker)] &
    ...
    gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].safeguardBattlerId =
        gBattlerAttacker;
...
static void Cmd_magnitudedamagecalculation(void) {
  ...
  for (gBattlerTarget = 0; gBattlerTarget < gBattlersCount; gBattlerTarget++) {
    ...
    if (!(gAbsentBattlerFlags &
          gBitTable[gBattlerTarget])) // a valid target was found
      break;
  }
...
static void Cmd_jumpifnopursuitswitchdmg(void) {
  ...
  if (gChosenActionByBattler[gBattlerTarget] == B_ACTION_USE_MOVE &&
    ...
    gCurrMovePos = gChosenMovePos =
        *(gBattleStruct->chosenMovePositions + gBattlerTarget);
    gBattlescriptCurrInstr += 5;
...
static void Cmd_copyfoestats(void) {
  ...
  gBattlescriptCurrInstr +=
      5; // Has an unused jump ptr(possibly for a failed attempt) parameter.
}
...
static void Cmd_rapidspinfree(void) {
  if (gBattleMons[gBattlerAttacker].status2 & STATUS2_WRAPPED) {
    ...
    gBattleTextBuff1[2] =
        *(gBattleStruct->wrappedMove + gBattlerAttacker * 2 + 0);
    gBattleTextBuff1[3] =
        *(gBattleStruct->wrappedMove + gBattlerAttacker * 2 + 1);
  ...
  } else if (gSideStatuses[GetBattlerSide(gBattlerAttacker)] &
             SIDE_STATUS_SPIKES) {
...
static void Cmd_hiddenpowercalc(void) {
  ...
  // Subtract 3 instead of 1 below because 2 types are excluded (TYPE_NORMAL and
  // TYPE_MYSTERY) The final + 1 skips past Normal, and the following
  ...
  if (gBattleMons[gBattlerTarget].hp == 0) {
    ...
    for (; gBattleCommunication[0] < PARTY_SIZE; gBattleCommunication[0]++) {
      if (GetMonData(&party[gBattleCommunication[0]], MON_DATA_HP) &&
          GetMonData(&party[gBattleCommunication[0]],
                     MON_DATA_SPECIES_OR_EGG) != SPECIES_NONE &&
          GetMonData(&party[gBattleCommunication[0]],
                     MON_DATA_SPECIES_OR_EGG) != SPECIES_EGG &&
          !GetMonData(&party[gBattleCommunication[0]], MON_DATA_STATUS))
        break;
    }
    if (gBattleCommunication[0] < PARTY_SIZE) {
      PREPARE_MON_NICK_WITH_PREFIX_BUFFER(gBattleTextBuff1, gBattlerAttacker,
                                          gBattleCommunication[0])

      gBattlescriptCurrInstr += 9;

      gBattleMoveDamage =
          gSpeciesInfo[GetMonData(&party[gBattleCommunication[0]],
                                  MON_DATA_SPECIES)]
              .baseAttack;
      gBattleMoveDamage *= gBattleMoves[gCurrentMove].power;
      gBattleMoveDamage *=
          (GetMonData(&party[gBattleCommunication[0]], MON_DATA_LEVEL) * 2 / 5 +
           2);
      gBattleMoveDamage /=
          gSpeciesInfo[gBattleMons[gBattlerTarget].species].baseDefense;
...
static void Cmd_setforcedtarget(void) {
  ...
  gSideTimers[GetBattlerSide(gBattlerAttacker)].followmeTarget =
      gBattlerAttacker;
  gBattlescriptCurrInstr++;
}
...
static void Cmd_cureifburnedparalysedorpoisoned(void) {
  if (gBattleMons[gBattlerAttacker].status1 &
      (STATUS1_POISON | STATUS1_BURN | STATUS1_PARALYSIS |
...
static void Cmd_tryswapitems(void) {
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_TRAINER_HILL ||
      (GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT &&
       !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_EREADER_TRAINER |
                             BATTLE_TYPE_FRONTIER | BATTLE_TYPE_SECRET_BASE |
                             BATTLE_TYPE_RECORDED_LINK)))) {
    gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
  } else {
    u8 sideAttacker = GetBattlerSide(gBattlerAttacker);
    ...
    if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_EREADER_TRAINER |
                              BATTLE_TYPE_FRONTIER | BATTLE_TYPE_SECRET_BASE |
                              BATTLE_TYPE_RECORDED_LINK)) &&
    ...
    // took a while, but all checks passed and items can be safely swapped
      ...
      *(u8 *)((u8 *)(&gBattleStruct->choicedMove[gBattlerAttacker]) + 1) = 0;
      ...
      if (oldItemAtk != ITEM_NONE && *newItemAtk != ITEM_NONE)
        gBattleCommunication[MULTISTRING_CHOOSER] =
            B_MSG_ITEM_SWAP_BOTH; // attacker's item -> <- target's item
      else if (oldItemAtk == ITEM_NONE && *newItemAtk != ITEM_NONE)
        gBattleCommunication[MULTISTRING_CHOOSER] =
            B_MSG_ITEM_SWAP_TAKEN; // nothing -> <- target's item
      else
        gBattleCommunication[MULTISTRING_CHOOSER] =
            B_MSG_ITEM_SWAP_GIVEN; // attacker's item -> <- nothing
    }
  }
}
...
static void Cmd_trywish(void) {
  ...
  case 1: // heal effect
    PREPARE_MON_NICK_WITH_PREFIX_BUFFER(
        gBattleTextBuff1, gBattlerTarget,
...
static void Cmd_setdamagetohealthdifference(void) {
  if (gBattleMons[gBattlerTarget].hp <= gBattleMons[gBattlerAttacker].hp) {
    ...
    gBattleMoveDamage =
        gBattleMons[gBattlerTarget].hp - gBattleMons[gBattlerAttacker].hp;
    gBattlescriptCurrInstr += 5;
  }
}
...
static void Cmd_scaledamagebyhealthratio(void) {
  if (gDynamicBasePower == 0) {
    ...
    gDynamicBasePower = gBattleMons[gBattlerAttacker].hp * power /
                        gBattleMons[gBattlerAttacker].maxHP;
    if (gDynamicBasePower == 0)
      gDynamicBasePower = 1;
  }
  ...
  if ((gStatuses3[gBattlerAttacker] & STATUS3_IMPRISONED_OTHERS)) {
    ...
    for (battlerId = 0; battlerId < gBattlersCount; battlerId++) {
      if (sideAttacker != GetBattlerSide(battlerId)) {
        s32 attackerMoveId;
        for (attackerMoveId = 0; attackerMoveId < MAX_MON_MOVES;
             attackerMoveId++) {
          s32 i;
          for (i = 0; i < MAX_MON_MOVES; i++) {
    ...
    if (battlerId ==
        gBattlersCount) // In Generation 3 games, Imprison fails if the user
...
static void Cmd_trygetintimidatetarget(void) {
  ...
  PREPARE_ABILITY_BUFFER(gBattleTextBuff1,
                         gBattleMons[gBattleScripting.battler].ability)
...
static void Cmd_switchoutabilities(void) {
  ...
  case ABILITY_NATURAL_CURE:
    gBattleMons[gActiveBattler].status1 = 0;
    BtlController_EmitSetMonData(
        BUFFER_A, REQUEST_STATUS_BATTLE,
...
static void Cmd_pickup(void) {
  ...
  if (InBattlePike()) {
    ...
    for (i = 0; i < PARTY_SIZE; i++) {
      ...
      if (ability == ABILITY_PICKUP && species != SPECIES_NONE &&
...
static void Cmd_docastformchangeanimation(void) {
  ...
  BtlController_EmitBattleAnimation(BUFFER_A, B_ANIM_CASTFORM_CHANGE,
                                    gBattleStruct->formToChangeInto);
  MarkBattlerForControllerExec(gActiveBattler);
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_TRAINER) {
    ...
    if (gLastUsedItem > ITEM_SAFARI_BALL) {
      ...
      case ITEM_REPEAT_BALL:
        if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(
                                  gBattleMons[gBattlerTarget].species),
                              FLAG_GET_CAUGHT))
          ballMultiplier = 30;
        else
    ...
    odds = (catchRate * ballMultiplier / 10) *
           (gBattleMons[gBattlerTarget].maxHP * 3 -
    ...
    if (odds > 254) // mon caught
    {
      ...
      SetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattlerTarget]],
                 MON_DATA_POKEBALL, &gLastUsedItem);
    ...
    } else // mon may be caught, calculate shakes
    {
      ...
      for (shakes = 0; shakes < BALL_3_SHAKES_SUCCESS && Random() < odds;
           shakes++)
        ;

      if (gLastUsedItem == ITEM_MASTER_BALL)
        shakes = BALL_3_SHAKES_SUCCESS; // why calculate the shakes before that
                                        // check?
...
static void Cmd_givecaughtmon(void) {
  if (GiveMonToPlayer(&gEnemyParty[gBattlerPartyIndexes[BATTLE_OPPOSITE(
          gBattlerAttacker)]]) != MON_GIVEN_TO_PARTY) {
    if (!ShouldShowBoxWasFullMessage()) {
      ...
      StringCopy(gStringVar1,
                 GetBoxNamePtr(VarGet(
                     VAR_PC_BOX_TO_SEND_MON))); // box the mon was sent to
      GetMonData(
          &gEnemyParty[gBattlerPartyIndexes[BATTLE_OPPOSITE(gBattlerAttacker)]],
          MON_DATA_NICKNAME, gStringVar2);
      StringCopy(
          gStringVar3,
          GetBoxNamePtr(
              GetPCBoxToSendMon())); // box the mon was going to be sent to
  ...
  GetMonData(
      &gEnemyParty[gBattlerPartyIndexes[BATTLE_OPPOSITE(gBattlerAttacker)]],
      MON_DATA_NICKNAME, gBattleResults.caughtMonNick);
  gBattleResults.caughtMonBall = GetMonData(
      &gEnemyParty[gBattlerPartyIndexes[BATTLE_OPPOSITE(gBattlerAttacker)]],
      MON_DATA_POKEBALL, NULL);
...
static void Cmd_trysetcaughtmondexflags(void) {
  ...
  if (species == SPECIES_UNOWN) // Check unown forms
  {
    ...
    if (letter == 0) {
      ...
      FlagSet(FLAG_CAUGHT_UNOWN_A); // Check unown A seperately since there's no
                                    // dex flag for it
    } else {
    ...
    if (CheckCaughtAllUnown()) {
      FlagSet(
          FLAG_SHOW_HIDDEN_POWER); // Unlock hidden power type in the party menu
    }
  }
}
...
static bool8 CheckCaughtAllUnown(void) {
  ...
  u16 species;
  if (!FlagGet(FLAG_CAUGHT_UNOWN_A)) // Check unown A seperately since there's
                                     // no dex flag for it
  {
    return FALSE;
  }
  for (species = startUnown; species <= endUnown; species++) {
    if (!GetSetPokedexFlag(SpeciesToNationalPokedexNum(species),
                           FLAG_GET_CAUGHT)) // Loop through all unown forms
    {
      return FALSE;
    }
  }
...
void HandleBattleWindow(u8 xStart, u8 yStart, u8 xEnd, u8 yEnd, u8 flags) {
  ...
  for (destY = yStart; destY <= yEnd; destY++) {
    for (destX = xStart; destX <= xEnd; destX++) {
      ...
      if (flags & WINDOW_BG1)
        CopyToBgTilemapBufferRect_ChangePalette(1, &var, destX, destY, 1, 1,
                                                0x11);
      else
        CopyToBgTilemapBufferRect_ChangePalette(0, &var, destX, destY, 1, 1,
                                                0x11);
    }
  }
}
...
void BattleCreateYesNoCursorAt(u8 cursorPosition) {
  ...
  CopyToBgTilemapBufferRect_ChangePalette(0, src, 0x19,
                                          9 + (2 * cursorPosition), 1, 2, 0x11);
  ...
  case 2:
    if (!gPaletteFade.active) {
      ...
      DoNamingScreen(
          ...
          GetMonData(&gEnemyParty[gBattlerPartyIndexes[BATTLE_OPPOSITE(
                         gBattlerAttacker)]],
                     MON_DATA_SPECIES),
          GetMonGender(&gEnemyParty[gBattlerPartyIndexes[BATTLE_OPPOSITE(
              gBattlerAttacker)]]),
          GetMonData(&gEnemyParty[gBattlerPartyIndexes[BATTLE_OPPOSITE(
                         gBattlerAttacker)]],
