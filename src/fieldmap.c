...
EWRAM_DATA static u32 UNUSED sFiller =
    0; // without this, the next file won't align properly
...
static void FillSouthConnection(struct MapHeader const *mapHeader,
                                struct MapHeader const *connectedMapHeader,
                                s32 offset);
static void FillNorthConnection(struct MapHeader const *mapHeader,
                                struct MapHeader const *connectedMapHeader,
                                s32 offset);
static void FillWestConnection(struct MapHeader const *mapHeader,
                               struct MapHeader const *connectedMapHeader,
                               s32 offset);
static void FillEastConnection(struct MapHeader const *mapHeader,
                               struct MapHeader const *connectedMapHeader,
                               s32 offset);
static void InitBackupMapLayoutConnections(struct MapHeader *mapHeader);
static void LoadSavedMapView(void);
static bool8 SkipCopyingMetatileFromSavedMap(u16 *mapBlock, u16 mapWidth,
                                             u8 yMode);
static const struct MapConnection *GetIncomingConnection(u8 direction, int x,
                                                         int y);
static bool8
IsPosInIncomingConnectingMap(u8 direction, int x, int y,
                             const struct MapConnection *connection);
static bool8 IsCoordInIncomingConnectingMap(int coord, int srcMax, int destMax,
                                            int offset);
...
#define GetBorderBlockAt(x, y)                                                 \
  ({                                                                           \
    ...
    int i;                                                                     \
    const u16 *border =                                                        \
        gMapHeader.mapLayout->border; /* Unused, they read it again below */   \
                                                                               \
    i = (x + 1) & 1;                                                           \
    i += ((y + 1) & 1) * 2;                                                    \
                                                                               \
...
#define AreCoordsWithinMapGridBounds(x, y)                                     \
  (x >= 0 && x < gBackupMapLayout.width && y >= 0 &&                           \
   y < gBackupMapLayout.height)
...
GetMapHeaderFromConnection(const struct MapConnection *connection) {
  return Overworld_GetMapHeaderByGroupAndId(connection->mapGroup,
                                            connection->mapNum);
}
...
static void InitMapLayoutData(struct MapHeader *mapHeader) {
  ...
  if (width * height <= MAX_MAP_DATA_SIZE) {
    InitBackupMapLayoutData(mapLayout->map, mapLayout->width,
                            mapLayout->height);
...
static void FillConnection(int x, int y,
                           struct MapHeader const *connectedMapHeader, int x2,
...
static void FillSouthConnection(struct MapHeader const *mapHeader,
  ...
  if (connectedMapHeader) {
    ...
    FillConnection(x, y, connectedMapHeader, x2, /*y2*/ 0, width,
                   /*height*/ MAP_OFFSET);
  }
}
...
static void FillNorthConnection(struct MapHeader const *mapHeader,
  ...
  if (connectedMapHeader) {
    ...
    FillConnection(x, /*y*/ 0, connectedMapHeader, x2, y2, width,
                   /*height*/ MAP_OFFSET);
  }
}
...
static void FillEastConnection(struct MapHeader const *mapHeader,
  ...
  if (connectedMapHeader) {
    ...
    FillConnection(x, y, connectedMapHeader,
                   /*x2*/ 0, y2,
...
u8 MapGridGetMetatileLayerTypeAt(int x, int y) {
  u16 metatile = MapGridGetMetatileIdAt(x, y);
  return (GetMetatileAttributesById(metatile) & METATILE_ATTR_LAYER_MASK) >>
...
void MapGridSetMetatileIdAt(int x, int y, u16 metatile) {
  ...
  if (AreCoordsWithinMapGridBounds(x, y)) {
    ...
    gBackupMapLayout.map[i] =
        (gBackupMapLayout.map[i] & MAPGRID_ELEVATION_MASK) |
...
#ifndef UBFIX
  // BUG: This loop extends past the bounds of the mapView array. Its size is
  // only 0x100.
  for (i = 0; i < 0x200; i++)
...
static void LoadSavedMapView(void) {
  ...
  if (!SavedMapViewIsEmpty()) {
    ...
    for (i = y; i < y + MAP_OFFSET_H; i++) {
      if (i == y && i != 0)
        yMode = 0;
      else if (i == y + MAP_OFFSET_H - 1 &&
               i != gMapHeader.mapLayout->height - 1)
        yMode = 1;
      else
        yMode = 0xFF;

      for (j = x; j < x + MAP_OFFSET_W; j++) {
        if (!SkipCopyingMetatileFromSavedMap(&sBackupMapData[j + width * i],
                                             width, yMode))
...
int GetPostCameraMoveMapBorderId(int x, int y) {
  return GetMapBorderIdAt(gSaveBlock1Ptr->pos.x + MAP_OFFSET + x,
...
static void SetPositionFromConnection(const struct MapConnection *connection,
...
bool8 CameraMove(int x, int y) {
  ...
  if (direction == CONNECTION_NONE || direction == CONNECTION_INVALID) {
    ...
    connection = GetIncomingConnection(direction, gSaveBlock1Ptr->pos.x,
...
IsPosInIncomingConnectingMap(u8 direction, int x, int y,
  ...
  case CONNECTION_SOUTH:
  case CONNECTION_NORTH:
    return IsCoordInIncomingConnectingMap(x, gMapHeader.mapLayout->width,
                                          mapHeader->mapLayout->width,
                                          connection->offset);
  case CONNECTION_WEST:
  case CONNECTION_EAST:
    return IsCoordInIncomingConnectingMap(y, gMapHeader.mapLayout->height,
...
static int IsPosInConnectingMap(const struct MapConnection *connection, int x,
                                int y) {
  struct MapHeader const *mapHeader;
  mapHeader = GetMapHeaderFromConnection(connection);
  switch (connection->direction) {
  case CONNECTION_SOUTH:
  case CONNECTION_NORTH:
    return IsCoordInConnectingMap(x - connection->offset,
                                  mapHeader->mapLayout->width);
  case CONNECTION_WEST:
  case CONNECTION_EAST:
    return IsCoordInConnectingMap(y - connection->offset,
                                  mapHeader->mapLayout->height);
  }
  return FALSE;
}
...
static void CopyTilesetToVram(struct Tileset const *tileset, u16 numTiles,
                              u16 offset) {
  if (tileset) {
    if (!tileset->isCompressed)
      LoadBgTiles(2, tileset->tiles, numTiles * 32, offset);
    else
      DecompressAndCopyTileDataToVram(2, tileset->tiles, numTiles * 32, offset,
                                      0);
  }
}

static void CopyTilesetToVramUsingHeap(struct Tileset const *tileset,
                                       u16 numTiles, u16 offset) {
  if (tileset) {
    if (!tileset->isCompressed)
      LoadBgTiles(2, tileset->tiles, numTiles * 32, offset);
    else
      DecompressAndLoadBgGfxUsingHeap(2, tileset->tiles, numTiles * 32, offset,
                                      0);
  }
}

// Below two are dummied functions from FRLG, used to tint the overworld
// palettes for the Quest Log
...
static void LoadTilesetPalette(struct Tileset const *tileset, u16 destOffset,
                               u16 size) {
  u16 black = RGB_BLACK;
  ...
  if (tileset) {
    if (tileset->isSecondary == FALSE) {
      ...
      LoadPalette(tileset->palettes[0] + 1, destOffset + 1,
                  size - PLTT_SIZEOF(1));
      ApplyGlobalTintToPaletteEntries(destOffset + 1,
                                      (size - PLTT_SIZEOF(1)) >> 1);
...
void CopySecondaryTilesetToVram(struct MapLayout const *mapLayout) {
  CopyTilesetToVram(mapLayout->secondaryTileset,
...
static void LoadPrimaryTilesetPalette(struct MapLayout const *mapLayout) {
  LoadTilesetPalette(mapLayout->primaryTileset, BG_PLTT_ID(0),
...
void CopyMapTilesetsToVram(struct MapLayout const *mapLayout) {
  if (mapLayout) {
    CopyTilesetToVramUsingHeap(mapLayout->primaryTileset, NUM_TILES_IN_PRIMARY,
                               0);
    CopyTilesetToVramUsingHeap(mapLayout->secondaryTileset,
                               NUM_TILES_TOTAL - NUM_TILES_IN_PRIMARY,
                               NUM_TILES_IN_PRIMARY);
