...
static const u16 sSpinningPokenav_Pal[] =
    INCBIN_U16("graphics/pokenav/nav_icon.gbapal");
static const u32 sSpinningPokenav_Gfx[] =
    INCBIN_U32("graphics/pokenav/nav_icon.4bpp.lz");
static const u32 sBlueLightCopy[] = INCBIN_U32(
    "graphics/pokenav/blue_light.4bpp.lz"); // Unused copy of
...
static const u8 sHelpBarTextColors[3] = {TEXT_COLOR_RED, TEXT_COLOR_WHITE,
...
static const struct CompressedSpriteSheet sMenuLeftHeaderSpriteSheet = {
    .data = gPokenavLeftHeaderHoennMap_Gfx, // Hoenn map is the first of the
                                            // headers listed
    .size = 0xC00,
    .tag = 2};

static const struct CompressedSpriteSheet sMenuLeftHeaderSpriteSheets[] = {
    [POKENAV_GFX_MAIN_MENU] = {.data = gPokenavLeftHeaderMainMenu_Gfx,
                               .size = 0x20,
                               .tag = 3},
    [POKENAV_GFX_CONDITION_MENU] = {.data = gPokenavLeftHeaderCondition_Gfx,
                                    .size = 0x20,
                                    .tag = 1},
    [POKENAV_GFX_RIBBONS_MENU] = {.data = gPokenavLeftHeaderRibbons_Gfx,
                                  .size = 0x20,
                                  .tag = 2},
    ...
    [POKENAV_GFX_MAP_MENU_ZOOMED_IN] = {
        .data = gPokenavLeftHeaderHoennMap_Gfx, .size = 0x40, .tag = 0}};

static const struct CompressedSpriteSheetNoSize
    sPokenavSubMenuLeftHeaderSpriteSheets[] = {
        [POKENAV_GFX_PARTY_MENU -
         POKENAV_GFX_SUBMENUS_START] = {.data = gPokenavLeftHeaderParty_Gfx,
                                        .tag = 1},
        [POKENAV_GFX_SEARCH_MENU -
            POKENAV_GFX_SUBMENUS_START] = {.data = gPokenavLeftHeaderSearch_Gfx,
                                           .tag = 1},
        [POKENAV_GFX_COOL_MENU -
            POKENAV_GFX_SUBMENUS_START] = {.data = gPokenavLeftHeaderCool_Gfx,
                                           .tag = 4},
        [POKENAV_GFX_BEAUTY_MENU -
            POKENAV_GFX_SUBMENUS_START] = {.data = gPokenavLeftHeaderBeauty_Gfx,
                                           .tag = 1},
        [POKENAV_GFX_CUTE_MENU -
            POKENAV_GFX_SUBMENUS_START] = {.data = gPokenavLeftHeaderCute_Gfx,
                                           .tag = 2},
        [POKENAV_GFX_SMART_MENU -
            POKENAV_GFX_SUBMENUS_START] = {.data = gPokenavLeftHeaderSmart_Gfx,
                                           .tag = 0},
        [POKENAV_GFX_TOUGH_MENU - POKENAV_GFX_SUBMENUS_START] = {
            .data = gPokenavLeftHeaderTough_Gfx, .tag = 0}};
...
static const union AnimCmd sSpinningPokenavAnims[] = {
    ...
    ANIMCMD_FRAME(48, 8), ANIMCMD_FRAME(64, 8),  ANIMCMD_FRAME(80, 8),
    ANIMCMD_FRAME(96, 8), ANIMCMD_FRAME(112, 8), ANIMCMD_JUMP(0)};
...
bool32 InitPokenavMainMenu(void) {
  ...
  menu = AllocSubstruct(POKENAV_SUBSTRUCT_MAIN_MENU,
                        sizeof(struct Pokenav_MainMenu));
  if (menu == NULL)
    return FALSE;
...
static u32 LoopedTask_InitPokenavMenu(s32 state) {
  ...
  case 0:
    ...
    InitBgsFromTemplates(0, gPokenavMainMenuBgTemplates,
                         ARRAY_COUNT(gPokenavMainMenuBgTemplates));
    ResetBgPositions();
  ...
  case 1:
    ...
    CopyPaletteIntoBufferUnfaded(gPokenavHeader_Pal, BG_PLTT_ID(0),
                                 PLTT_SIZE_4BPP);
...
void SetActiveMenuLoopTasks(void *createLoopTask,
                            void *isLoopTaskActive) // Fix types later.
{
  struct Pokenav_MainMenu *menu = GetSubstructPtr(POKENAV_SUBSTRUCT_MAIN_MENU);
...
void CopyPaletteIntoBufferUnfaded(const u16 *palette, u32 bufferOffset,
                                  u32 size) {
...
void PokenavFillPalette(u32 palIndex, u16 fillValue) {
  CpuFill16(fillValue, &gPlttBufferFaded[OBJ_PLTT_ID(palIndex)],
            PLTT_SIZE_4BPP);
}

void PokenavCopyPalette(const u16 *src, const u16 *dest, int size, int a3,
                        int a4, u16 *palette) {
  if (a4 == 0) {
...
bool32 IsPaletteFadeActive(void) { return gPaletteFade.active; }
...
void PrintHelpBarText(u32 textId) {
  ...
  DrawHelpBar(menu->helpBarWindowId);
  AddTextPrinterParameterized3(menu->helpBarWindowId, FONT_NORMAL, 0, 1,
                               sHelpBarTextColors, 0, sHelpBarTexts[textId]);
}

bool32 WaitForHelpBar(void) { return IsDma3ManagerBusyWithBgCopy(); }
...
static void LoadLeftHeaderGfxForMenu(u32 menuGfxId) {
  ...
  LoadPalette(&gPokenavLeftHeader_Pal[tag * 16],
              OBJ_PLTT_ID(IndexOfSpritePaletteTag(1)), PLTT_SIZE_4BPP);
  LZ77UnCompWram(sMenuLeftHeaderSpriteSheets[menuGfxId].data,
                 gDecompressionBuffer);
  RequestDma3Copy(gDecompressionBuffer,
                  (void *)OBJ_VRAM0 + (GetSpriteTileStartByTag(2) * 32), size,
                  1);
  menu->leftHeaderSprites[1]->oam.tileNum =
      GetSpriteTileStartByTag(2) + sMenuLeftHeaderSpriteSheets[menuGfxId].size;
...
static void LoadLeftHeaderGfxForSubMenu(u32 menuGfxId) {
  ...
  LoadPalette(&gPokenavLeftHeader_Pal[tag * 16],
              OBJ_PLTT_ID(IndexOfSpritePaletteTag(2)), PLTT_SIZE_4BPP);
  LZ77UnCompWram(sPokenavSubMenuLeftHeaderSpriteSheets[menuGfxId].data,
                 &gDecompressionBuffer[0x1000]);
  RequestDma3Copy(&gDecompressionBuffer[0x1000],
                  (void *)OBJ_VRAM0 + 0x800 + (GetSpriteTileStartByTag(2) * 32),
                  size, 1);
}
...
bool32 AreLeftHeaderSpritesMoving(void) {
  ...
  if (menu->leftHeaderSprites[0]->callback == SpriteCallbackDummy &&
      menu->submenuLeftHeaderSprites[0]->callback == SpriteCallbackDummy)
    return FALSE;
  else
    return TRUE;
}
...
static void MoveLeftHeader(struct Sprite *sprite, s32 startX, s32 endX,
                           s32 duration) {
  sprite->x = startX;
