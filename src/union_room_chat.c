...
// Excludes UNION_ROOM_KB_PAGE_REGISTER, the text for which is chosen by the
// player
static const u8 *const sUnionRoomKeyboardText[UNION_ROOM_KB_PAGE_COUNT -
                                              1][UNION_ROOM_KB_ROW_COUNT] = {
...
static const u16 sUnusedPalette[] =
    INCBIN_U16("graphics/union_room_chat/unused.gbapal"); // Loaded but never
                                                          // apparently used
static const u16 sChatMessagesWindow_Pal[] =
    INCBIN_U16("graphics/union_room_chat/chat_messages_window.gbapal");
...
static const struct MenuAction
    sKeyboardPageTitleTexts[UNION_ROOM_KB_PAGE_COUNT + 1] = {
...
static const u32 sRButtonGfxTiles[] =
    INCBIN_U32("graphics/union_room_chat/r_button.4bpp.lz");

static const struct CompressedSpriteSheet sSpriteSheets[] = {
    {.data = sKeyboardCursorTiles,
     .size = 0x1000,
     .tag = GFXTAG_KEYBOARD_CURSOR},
    {.data = sTextEntryArrowTiles,
     .size = 0x0040,
     .tag = GFXTAG_TEXT_ENTRY_ARROW},
    {.data = sTextEntryCursorTiles,
     .size = 0x0040,
     .tag = GFXTAG_TEXT_ENTRY_CURSOR},
    {.data = sRButtonGfxTiles, .size = 0x0080, .tag = GFXTAG_RBUTTON_ICON},
    {.data = gUnionRoomChat_RButtonLabels,
     .size = 0x0400,
     .tag = GFXTAG_RBUTTON_LABELS}};
...
static const union AnimCmd sAnim_KeyboardCursor_Open[] = {
    ANIMCMD_FRAME(0x00, 30), ANIMCMD_END};

static const union AnimCmd sAnim_KeyboardCursor_Closed[] = {
    ANIMCMD_FRAME(0x20, 30), ANIMCMD_END};

static const union AnimCmd sAnim_KeyboardCursorWide_Open[] = {
    ANIMCMD_FRAME(0x40, 30), ANIMCMD_END};

static const union AnimCmd sAnim_KeyboardCursorWide_Closed[] = {
    ANIMCMD_FRAME(0x60, 30), ANIMCMD_END};

static const union AnimCmd *const sAnims_KeyboardCursor[] = {
    sAnim_KeyboardCursor_Open, sAnim_KeyboardCursor_Closed,
    sAnim_KeyboardCursorWide_Open, sAnim_KeyboardCursorWide_Closed};
...
static const union AnimCmd sAnim_ToggleCaseIcon[] = {ANIMCMD_FRAME(0x00, 2),
                                                     ANIMCMD_END};

static const union AnimCmd sAnim_ToggleCaseIcon_Duplicate1[] = {
    ANIMCMD_FRAME(0x08, 2), ANIMCMD_END};

static const union AnimCmd sAnim_ToggleCaseIcon_Duplicate2[] = {
    ANIMCMD_FRAME(0x10, 2), ANIMCMD_END};

static const union AnimCmd sAnim_RegisterIcon[] = {ANIMCMD_FRAME(0x18, 2),
                                                   ANIMCMD_END};

static const union AnimCmd *const sAnims_RButtonLabels[] = {
    sAnim_ToggleCaseIcon, sAnim_ToggleCaseIcon_Duplicate1,
...
static void CB2_LoadInterface(void) {
  ...
  case 2:
    ...
    if (!gPaletteFade.active) {
      ...
      CreateWirelessStatusIndicatorSprite(DISPLAY_WIDTH - 8,
                                          DISPLAY_HEIGHT - 10);
    }
    break;
  }
...
static void Chat_Join(void) {
  ...
  case 1:
    if (IsLinkTaskFinished() && !Rfu_IsPlayerExchangeActive()) {
      if (SendBlock(0, sChat->sendMessageBuffer,
                    sizeof(sChat->sendMessageBuffer)))
        sChat->funcState++;
    }
    break;
  case 2:
...
static void Chat_AskQuitChatting(void) {
  ...
  case 4:
    if (IsLinkTaskFinished() && !Rfu_IsPlayerExchangeActive() &&
        SendBlock(0, sChat->sendMessageBuffer,
                  sizeof(sChat->sendMessageBuffer))) {
...
static void Chat_Drop(void) {
  ...
  case 0:
    ...
    break;
  case 1:
    if (!IsDisplaySubtaskActive(0) && IsLinkTaskFinished() &&
...
static void Chat_Disbanded(void) {
  ...
  case 1:
    ...
    break;
  case 2:
    if (IsDisplaySubtaskActive(0) != TRUE && IsLinkTaskFinished() &&
        !Rfu_IsPlayerExchangeActive()) {
      SetCloseLinkCallback();
      sChat->exitDelayTimer = 0;
      sChat->funcState++;
...
static void Chat_SendMessage(void) {
  ...
  case 0:
    ...
    // fall through
  case 1:
    if (IsLinkTaskFinished() == TRUE && !Rfu_IsPlayerExchangeActive() &&
        SendBlock(0, sChat->sendMessageBuffer,
                  sizeof(sChat->sendMessageBuffer)))
      sChat->funcState++;
    break;
...
static u8 GetCurrentKeyboardPage(void) { return sChat->currentPage; }
...
static u8 *GetMessageEntryBuffer(void) { return sChat->messageEntryBuffer; }
...
static u8 *GetLastReceivedMessage(void) { return sChat->receivedMessage; }

static u8 GetReceivedPlayerIndex(void) { return sChat->receivedPlayerIndex; }

static int GetTextEntryCursorPosition(void) { return sChat->bufferCursorPos; }

static int GetShouldShowCaseToggleIcon(void) {
  u8 *str = GetLastCharOfMessagePtr();
  u32 character = *str;
  if (character > EOS || sCaseToggleTable[character] == character ||
...
static u8 *GetChatHostName(void) { return sChat->hostName; }
...
static void Task_ReceiveChatMessage(u8 taskId) {
  ...
  case 1:
    ...
    // fall through
  case 3:
    for (; tI < MAX_RFU_PLAYERS && ((tBlockReceivedStatus >> tI) & 1) == 0;
         tI++)
      ;
    ...
    if (ProcessReceivedChatMessage(sChat->receivedMessage,
                                   (u8 *)gBlockRecvBuffer[tI])) {
...
static bool32 Display_Dummy(u8 *state) { return FALSE; }
...
static void AddYesNoMenuAt(u8 left, u8 top, u8 initialCursorPos) {
  ...
  if (sDisplay->yesNoMenuWindowId != WINDOW_NONE) {
    ...
    AddTextPrinterParameterized(sDisplay->yesNoMenuWindowId, FONT_NORMAL,
                                gText_Yes, 8, 1, TEXT_SKIP_DRAW, NULL);
    AddTextPrinterParameterized(sDisplay->yesNoMenuWindowId, FONT_NORMAL,
                                gText_No, 8, 17, TEXT_SKIP_DRAW, NULL);
    DrawTextBorderOuter(sDisplay->yesNoMenuWindowId, 1, 13);
    InitMenuInUpperLeftCornerNormal(sDisplay->yesNoMenuWindowId, 2,
                                    initialCursorPos);
  }
}
...
static s8 ProcessMenuInput(void) { return Menu_ProcessInput(); }
...
static void AddStdMessageWindow(int msgId, u16 bg0vofs) {
  ...
  if (sDisplayStdMessages[msgId].boxType == 1) {
    ...
    AddTextPrinterParameterized5(
        windowId, FONT_NORMAL, str, sDisplayStdMessages[msgId].x + 8,
        sDisplayStdMessages[msgId].y + 8, TEXT_SKIP_DRAW, NULL,
        sDisplayStdMessages[msgId].letterSpacing,
        sDisplayStdMessages[msgId].lineSpacing);
  } else {
    DrawTextBorderOuter(windowId, 0xA, 2);
    AddTextPrinterParameterized5(windowId, FONT_NORMAL, str,
                                 sDisplayStdMessages[msgId].x,
                                 sDisplayStdMessages[msgId].y, TEXT_SKIP_DRAW,
                                 NULL, sDisplayStdMessages[msgId].letterSpacing,
                                 sDisplayStdMessages[msgId].lineSpacing);
  }
...
static void DrawTextEntryMessage(u16 x, u8 *str, u8 bgColor, u8 fgColor,
                                 u8 shadowColor) {
  u8 color[3];
  u8 strBuffer[35];
  ...
  AddTextPrinterParameterized3(WIN_TEXT_ENTRY, FONT_SHORT, x * 8, 1, color,
                               TEXT_SKIP_DRAW, strBuffer);
}
...
static void PrintCurrentKeyboardPage(void) {
  ...
  if (page != UNION_ROOM_KB_PAGE_REGISTER) {
    ...
    for (i = 0, top = 0; i < UNION_ROOM_KB_ROW_COUNT; i++, top += 12) {
      ...
      AddTextPrinterParameterized3(WIN_KEYBOARD, FONT_SMALL, left, top, color,
                                   TEXT_SKIP_DRAW, str);
    }
  } else {
    left = 4;
    for (i = 0, top = 0; i < UNION_ROOM_KB_ROW_COUNT; i++, top += 12) {
      ...
      if (GetStringWidth(FONT_SMALL, str2, 0) <= 40) {
        ...
        AddTextPrinterParameterized3(WIN_KEYBOARD, FONT_SMALL, left, top, color,
                                     TEXT_SKIP_DRAW, str);
        AddTextPrinterParameterized3(WIN_KEYBOARD, FONT_SMALL, left + 35, top,
                                     color, TEXT_SKIP_DRAW, sText_Ellipsis);
      }
    }
  }
}
...
static void ShowKeyboardSwapMenu(void) {
  ...
  PrintMenuActionTextsAtPos(WIN_SWAP_MENU, FONT_SHORT, 8, 1, 14,
                            ARRAY_COUNT(sKeyboardPageTitleTexts),
                            sKeyboardPageTitleTexts);
  InitMenuNormal(WIN_SWAP_MENU, FONT_SHORT, 0, 1, 14, 5,
...
static void PrintChatMessage(u16 row, u8 *str, u8 colorIdx) {
  ...
  AddTextPrinterParameterized3(WIN_CHAT_HISTORY, FONT_SHORT, 0, row * 15 + 1,
                               color, TEXT_SKIP_DRAW, str);
}
...
static void ResetGpuBgState(void) {
  ...
  ClearGpuRegBits(REG_OFFSET_DISPCNT,
...
static void LoadChatWindowGfx(void) {
  ...
  ptr = DecompressAndCopyTileDataToVram(2, gUnionRoomChat_Background_Gfx, 0, 0,
                                        0);
  if (ptr) {
    // The below is nonsense. Tiles 0x11 and 0x21 of the background tileset are
    // the second half of "OK" and the "T" in "START" in the instructions
    // header. They're later blitted onto the text entry window, then
    // immediately cleared. The window has a different palette as well, so the
    // tiles would appear mostly black anyway.
    CpuFastCopy(&ptr[0x11 * TILE_SIZE_4BPP],
                &sDisplay->textEntryTiles[TILE_SIZE_4BPP * 0], TILE_SIZE_4BPP);
    CpuFastCopy(&ptr[0x21 * TILE_SIZE_4BPP],
                &sDisplay->textEntryTiles[TILE_SIZE_4BPP * 1], TILE_SIZE_4BPP);
  }
...
static void LoadChatUnkPalette(void) {
  ...
  RequestDma3Fill(0, (void *)BG_CHAR_ADDR(1) + TILE_SIZE_4BPP, TILE_SIZE_4BPP,
                  1);
}

static void LoadChatMessagesWindow(void) {
  LoadPalette(sChatMessagesWindow_Pal, BG_PLTT_ID(15),
              sizeof(sChatMessagesWindow_Pal));
...
static void LoadTextEntryWindow(void) {
  ...
  // Pointless, cleared below. The tiles are nonsense anyway, see
  // LoadChatWindowGfx.
  for (i = 0; i < MAX_MESSAGE_LENGTH; i++)
    BlitBitmapToWindow(WIN_TEXT_ENTRY, sDisplay->textEntryTiles, i * 8, 0, 8,
                       16);
...
static void UpdateSlidingKeyboard(s16 bg1hofs) {
  CpuFill16(bg1hofs, gScanlineEffectRegBuffers[gScanlineEffect.srcBuffer],
            0x120);
  CpuFill16(0, gScanlineEffectRegBuffers[gScanlineEffect.srcBuffer] + 0x90,
            0x20);
}
