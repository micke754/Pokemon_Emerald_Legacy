...
void m4aSoundInit(void) {
  ...
  memcpy(&gPokemonCrySong, &gPokemonCrySongTemplate,
         sizeof(struct PokemonCrySong));
...
void SoundInit(struct SoundInfo *soundInfo) {
  ...
  if (REG_DMA1CNT & (DMA_REPEAT << 16))
    REG_DMA1CNT =
        ((DMA_ENABLE | DMA_START_NOW | DMA_32BIT | DMA_SRC_INC | DMA_DEST_FIXED)
         << 16) |
        4;
...
void m4aSoundVSyncOff(void) {
  ...
  if (soundInfo->ident >= ID_NUMBER && soundInfo->ident <= ID_NUMBER + 1) {
    ...
    if (REG_DMA1CNT & (DMA_REPEAT << 16))
      REG_DMA1CNT = ((DMA_ENABLE | DMA_START_NOW | DMA_32BIT | DMA_SRC_INC |
                      DMA_DEST_FIXED)
                     << 16) |
                    4;
...
void MPlayOpen(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *tracks, u8 trackCount) {
  struct SoundInfo *soundInfo;
...
void MPlayStart(struct MusicPlayerInfo *mplayInfo,
...
void FadeOutBody(struct MusicPlayerInfo *mplayInfo) {
  ...
  if (mplayInfo->fadeOV & FADE_IN) {
    if ((u16)(mplayInfo->fadeOV += (4 << FADE_VOL_SHIFT)) >=
        (64 << FADE_VOL_SHIFT)) {
      mplayInfo->fadeOV = (64 << FADE_VOL_SHIFT);
      mplayInfo->fadeOI = 0;
    }
...
void TrkVolPitSet(struct MusicPlayerInfo *mplayInfo,
                  struct MusicPlayerTrack *track) {
  ...
  if (track->flags & MPT_FLG_PITSET) {
    ...
    s32 x = (track->tune + bend) * 4 + (track->keyShift << 8) +
      ...
      if (channels->envelopeCounter == 0) {
        ...
        if ((channels->statusFlags & SOUND_CHANNEL_SF_ENV) ==
          ...
          envelope_pseudoecho_start:
            channels->envelopeVolume =
                ((channels->envelopeGoal * channels->pseudoEchoVolume) +
                 0xFF) >>
                8;
    ...
    if (channels->modify & CGB_CHANNEL_MO_VOL) {
      ...
      if (ch == 3) {
        *nrx2ptr = gCgb3Vol[channels->envelopeVolume];
        ...
        u32 envMask = 0xF;
        *nrx2ptr = (envelopeStepTimeAndDir & envMask) +
...
void m4aMPlayModDepthSet(struct MusicPlayerInfo *mplayInfo, u16 trackBits,
                         u8 modDepth) {
  s32 i;
  u32 bit;
  struct MusicPlayerTrack *track;
...
void m4aMPlayLFOSpeedSet(struct MusicPlayerInfo *mplayInfo, u16 trackBits,
                         u8 lfoSpeed) {
  s32 i;
  u32 bit;
  struct MusicPlayerTrack *track;
...
void ply_memacc(struct MusicPlayerInfo *mplayInfo,
                struct MusicPlayerTrack *track) {
  u32 op;
  u8 *addr;
  u8 data;
...
void ply_xcmd(struct MusicPlayerInfo *mplayInfo,
              struct MusicPlayerTrack *track) {
  u32 n = *track->cmdPtr;
  track->cmdPtr++;
...
void ply_xxx(struct MusicPlayerInfo *mplayInfo,
             struct MusicPlayerTrack *track) {
...
void ply_xwave(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  u32 wav;
...
void ply_xtype(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  track->tone.type = *track->cmdPtr;
  track->cmdPtr++;
}

void ply_xatta(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  track->tone.attack = *track->cmdPtr;
  track->cmdPtr++;
}

void ply_xdeca(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  track->tone.decay = *track->cmdPtr;
  track->cmdPtr++;
}

void ply_xsust(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  track->tone.sustain = *track->cmdPtr;
  track->cmdPtr++;
}

void ply_xrele(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  track->tone.release = *track->cmdPtr;
  track->cmdPtr++;
}

void ply_xiecv(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  track->pseudoEchoVolume = *track->cmdPtr;
  track->cmdPtr++;
}

void ply_xiecl(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  track->pseudoEchoLength = *track->cmdPtr;
  track->cmdPtr++;
}

void ply_xleng(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  track->tone.length = *track->cmdPtr;
  track->cmdPtr++;
}

void ply_xswee(struct MusicPlayerInfo *mplayInfo,
               struct MusicPlayerTrack *track) {
  track->tone.pan_sweep = *track->cmdPtr;
  track->cmdPtr++;
}

void ply_xcmd_0C(struct MusicPlayerInfo *mplayInfo,
                 struct MusicPlayerTrack *track) {
  u32 unk;
...
void ply_xcmd_0D(struct MusicPlayerInfo *mplayInfo,
                 struct MusicPlayerTrack *track) {
  u32 unk;
...
void SetPokemonCryVolume(u8 val) { gPokemonCrySong.volumeValue = val & 0x7F; }
...
void SetPokemonCryLength(u16 val) { gPokemonCrySong.unkCmd0CParam = val; }

void SetPokemonCryRelease(u8 val) { gPokemonCrySong.releaseValue = val; }

void SetPokemonCryProgress(u32 val) { gPokemonCrySong.unkCmd0DParam = val; }
...
void SetPokemonCryStereo(u32 val) {
  ...
  if (val) {
    ...
    REG_SOUNDCNT_H = SOUND_B_TIMER_0 | SOUND_B_LEFT_OUTPUT |
                     SOUND_B_RIGHT_OUTPUT | SOUND_A_TIMER_0 |
                     SOUND_A_LEFT_OUTPUT | SOUND_A_RIGHT_OUTPUT |
