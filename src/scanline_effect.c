...
void ScanlineEffect_InitHBlankDmaTransfer(void) {
  if (gScanlineEffect.state == 0) {
    ...
    DmaSet(0, gScanlineEffect.dmaSrcBuffers[gScanlineEffect.srcBuffer],
           gScanlineEffect.dmaDest, gScanlineEffect.dmaControl);
...
// Initializes a background "wave" effect that affects scanlines startLine
// (inclusive) to endLine (exclusive). 'frequency' and 'amplitude' control the
// frequency and amplitude of the wave. 'delayInterval' controls how fast the
// wave travels up the screen. The wave will shift upwards one scanline every
// 'delayInterval'+1 frames. 'regOffset' is the offset of the video register to
// modify.
u8 ScanlineEffect_InitWave(u8 startLine, u8 endLine, u8 frequency, u8 amplitude,
