...
// In CursorCb_FieldMove, field moves <= FIELD_MOVE_WATERFALL are assumed to
// line up with the badge flags. Badge flag names are commented here for people
// searching for references to remove the badge requirement.
enum {
...
enum {
  // Window ids 0-5 are implicitly assigned to each party Pokémon in
  // InitPartyMenuBoxes
  WIN_MSG = PARTY_SIZE,
};
...
struct PartyMenuInternal {
  ...
  u32 chooseHalf : 1;
  u32 lastSelectedSlot : 3; // Used to return to same slot when going left/right
                            // bewtween columns
  ...
  u8 actions[8];
  u8 numActions;
  // In vanilla Emerald, only the first 0xB0 hwords (0x160 bytes) are actually
  // used. However, a full 0x100 hwords (0x200 bytes) are allocated. It is
  // likely that the 0x160 value used below is a constant defined by bin2c, the
  // utility used to encode the compressed palette data.
...
EWRAM_DATA u8 gBattlePartyCurrentOrder[PARTY_SIZE / 2] = {
    0}; // bits 0-3 are the current pos of Slot 1, 4-7 are Slot 2, and so on
...
static void DisplayPartyPokemonNickname(struct Pokemon *, struct PartyMenuBox *,
                                        u8);
static void DisplayPartyPokemonLevelCheck(struct Pokemon *,
                                          struct PartyMenuBox *, u8);
static void DisplayPartyPokemonGenderNidoranCheck(struct Pokemon *,
                                                  struct PartyMenuBox *, u8);
static void DisplayPartyPokemonHPCheck(struct Pokemon *, struct PartyMenuBox *,
                                       u8);
static void DisplayPartyPokemonMaxHPCheck(struct Pokemon *,
                                          struct PartyMenuBox *, u8);
static void DisplayPartyPokemonHPBarCheck(struct Pokemon *,
                                          struct PartyMenuBox *);
...
static void
CreatePartyMonIconSpriteParameterized(u16, u32, struct PartyMenuBox *, u8, u32);
static void CreatePartyMonHeldItemSpriteParameterized(u16, u16,
                                                      struct PartyMenuBox *);
static void CreatePartyMonPokeballSpriteParameterized(u16,
                                                      struct PartyMenuBox *);
static void CreatePartyMonStatusSpriteParameterized(u16, u8,
                                                    struct PartyMenuBox *);
// These next 4 functions are essentially redundant with the above 4
// The only difference is that rather than receive the data directly they
// retrieve it from the mon struct
static void CreatePartyMonHeldItemSprite(struct Pokemon *,
                                         struct PartyMenuBox *);
static void CreatePartyMonPokeballSprite(struct Pokemon *,
                                         struct PartyMenuBox *);
...
static void UpdatePartyMonHeldItemSprite(struct Pokemon *,
                                         struct PartyMenuBox *);
...
// code
static void InitPartyMenu(u8 menuType, u8 layout, u8 partyAction,
                          bool8 keepCursorPos, u8 messageId, TaskFunc task,
...
static void CB2_InitPartyMenu(void) {
  while (TRUE) {
    if (MenuHelpers_ShouldWaitForLinkRecv() == TRUE ||
        ShowPartyMenu() == TRUE || MenuHelpers_IsLinkActive() == TRUE)
      break;
  }
}
...
static bool8 AllocPartyMenuBg(void) {
  ...
  InitBgsFromTemplates(0, sPartyMenuBgTemplates,
                       ARRAY_COUNT(sPartyMenuBgTemplates));
...
static bool8 AllocPartyMenuBgGfx(void) {
  ...
  case 2:
    ...
    CpuCopy16(gPlttBufferUnfaded, sPartyMenuInternal->palBuffer,
              11 * PLTT_SIZE_4BPP);
    sPartyMenuInternal->data[0]++;
    break;
  case 3:
...
static void PartyPaletteBufferCopy(u8 palNum) {
  ...
  CpuCopy16(&gPlttBufferUnfaded[BG_PLTT_ID(3)], &gPlttBufferUnfaded[offset],
            PLTT_SIZE_4BPP);
  CpuCopy16(&gPlttBufferUnfaded[BG_PLTT_ID(3)], &gPlttBufferFaded[offset],
            PLTT_SIZE_4BPP);
}
...
static void DisplayPartyPokemonData(u8 slot) {
  if (GetMonData(&gPlayerParty[slot], MON_DATA_IS_EGG)) {
    sPartyMenuBoxes[slot].infoRects->blitFunc(sPartyMenuBoxes[slot].windowId, 0,
                                              0, 0, 0, TRUE);
    ...
    sPartyMenuBoxes[slot].infoRects->blitFunc(sPartyMenuBoxes[slot].windowId, 0,
                                              0, 0, 0, FALSE);
    DisplayPartyPokemonNickname(&gPlayerParty[slot], &sPartyMenuBoxes[slot], 0);
    DisplayPartyPokemonLevelCheck(&gPlayerParty[slot], &sPartyMenuBoxes[slot],
                                  0);
    DisplayPartyPokemonGenderNidoranCheck(&gPlayerParty[slot],
                                          &sPartyMenuBoxes[slot], 0);
    DisplayPartyPokemonHPCheck(&gPlayerParty[slot], &sPartyMenuBoxes[slot], 0);
    DisplayPartyPokemonMaxHPCheck(&gPlayerParty[slot], &sPartyMenuBoxes[slot],
                                  0);
...
// Returns TRUE if teaching move or cant evolve with item (i.e. description data
  ...
  if (gPartyMenu.action == PARTY_ACTION_MOVE_TUTOR) {
    ...
    case 2: // Evolution stone
      if (!GetMonData(currentPokemon, MON_DATA_IS_EGG) &&
          GetEvolutionTargetSpecies(currentPokemon, EVO_MODE_ITEM_CHECK,
                                    item) != SPECIES_NONE)
        return FALSE;
...
static void DisplayPartyPokemonDataForMultiBattle(u8 slot) {
  ...
  if (gMultiPartnerParty[actualSlot].species == SPECIES_NONE) {
    ...
    DisplayPartyPokemonBarDetail(menuBox->windowId, gStringVar1, 0,
    ...
    DisplayPartyPokemonHPBar(gMultiPartnerParty[actualSlot].hp,
                             gMultiPartnerParty[actualSlot].maxhp, menuBox);
  }
}
  ...
  if (gPartyMenu.menuType == PARTY_MENU_TYPE_MULTI_SHOWCASE &&
    ...
    if (gMultiPartnerParty[actualSlot].species != SPECIES_NONE) {
      ...
      CreatePartyMonStatusSpriteParameterized(
          gMultiPartnerParty[actualSlot].species, status,
...
static void CreateCancelConfirmPokeballSprites(void) {
  if (gPartyMenu.menuType == PARTY_MENU_TYPE_MULTI_SHOWCASE) {
    ...
    if (sPartyMenuInternal->chooseHalf) {
      ...
      sPartyMenuInternal->spriteIdCancelPokeball =
          CreatePokeballButtonSprite(198, 148);
    }
...
void AnimatePartySlot(u8 slot, u8 animNum) {
  ...
  default:
    if (GetMonData(&gPlayerParty[slot], MON_DATA_SPECIES) != SPECIES_NONE) {
      LoadPartyBoxPalette(&sPartyMenuBoxes[slot],
...
static bool8 PartyBoxPal_ParnterOrDisqualifiedInArena(u8 slot) {
  ...
  if (slot < MULTI_PARTY_SIZE && (gBattleTypeFlags & BATTLE_TYPE_ARENA) &&
      gMain.inBattle &&
      (gBattleStruct->arenaLostPlayerMons >> GetPartyIdFromBattleSlot(slot) &
       1))
    return TRUE;
...
static void DrawCancelConfirmButtons(void) {
  CopyToBgTilemapBufferRect_ChangePalette(1, sConfirmButton_Tilemap, 23, 16, 7,
                                          2, 17);
  CopyToBgTilemapBufferRect_ChangePalette(1, sCancelButton_Tilemap, 23, 18, 7,
                                          2, 17);
  ScheduleBgCopyTilemapToVram(1);
}
...
static void UpdatePartySelectionDoubleLayout(s8 *slotPtr, s8 movementDir) {
  // PARTY_SIZE + 1 is Cancel, PARTY_SIZE is Confirm
  // newSlot is used temporarily as a movement direction during its later
  // assignment
...
static void DisplayGaveHeldItemMessage(struct Pokemon *mon, u16 item,
                                       bool8 keepOpen, u8 unused) {
...
static void DisplayTookHeldItemMessage(struct Pokemon *mon, u16 item,
                                       bool8 keepOpen) {
  GetMonNickname(mon, gStringVar1);
  CopyItemName(item, gStringVar2);
...
static void DisplayAlreadyHoldingItemSwitchMessage(struct Pokemon *mon,
                                                   u16 item, bool8 keepOpen) {
  GetMonNickname(mon, gStringVar1);
  CopyItemName(item, gStringVar2);
...
static void DisplaySwitchedHeldItemMessage(u16 item, u16 item2,
                                           bool8 keepOpen) {
...
static void Task_PartyMenuModifyHP(u8 taskId) {
  ...
  DisplayPartyPokemonHPCheck(&gPlayerParty[tPartyId],
                             &sPartyMenuBoxes[tPartyId], 1);
  DisplayPartyPokemonHPBarCheck(&gPlayerParty[tPartyId],
                                &sPartyMenuBoxes[tPartyId]);
  if (tHPToAdd == 0 || tHP == 0 || tHP == tMaxHP) {
    // If HP was recovered, buffer the amount recovered
    if (tHP > tStartHP)
      ConvertIntToDecimalStringN(gStringVar2, tHP - tStartHP,
...
void PartyMenuModifyHP(u8 taskId, u8 slot, s8 hpIncrement, s16 hpDifference,
                       TaskFunc task) {
  struct Pokemon *mon = &gPlayerParty[slot];
  s16 *data = gTasks[taskId].data;
...
static void ResetHPTaskData(u8 taskId, u8 caseId, u32 hp) {
  ...
  case 4:
    ...
    break;
  case 5:
    SetTaskFuncWithFollowupFunc(taskId, Task_PartyMenuModifyHP,
                                (TaskFunc)hp); // >casting hp as a taskfunc
    break;
  }
}
...
static u16 GetTutorMove(u8 tutor) { return gTutorMoves[tutor]; }
...
static void CreateCancelConfirmWindows(bool8 chooseHalf) {
  ...
  if (gPartyMenu.menuType != PARTY_MENU_TYPE_MULTI_SHOWCASE) {
    if (chooseHalf == TRUE) {
      ...
      mainOffset =
          GetStringCenterAlignXOffset(FONT_SMALL, gMenuText_Confirm, 48);
      AddTextPrinterParameterized4(confirmWindowId, FONT_SMALL, mainOffset, 1,
                                   0, 0, sFontColorTable[0], TEXT_SKIP_DRAW,
                                   gMenuText_Confirm);
    ...
    // Branches are functionally identical. Second branch is never reached, Spin
    // Trade wasnt fully implemented
    if (gPartyMenu.menuType != PARTY_MENU_TYPE_SPIN_TRADE) {
      mainOffset = GetStringCenterAlignXOffset(FONT_SMALL, gText_Cancel, 48);
      AddTextPrinterParameterized3(cancelWindowId, FONT_SMALL,
                                   mainOffset + offset, 1, sFontColorTable[0],
                                   TEXT_SKIP_DRAW, gText_Cancel);
    } else {
      mainOffset = GetStringCenterAlignXOffset(FONT_SMALL, gText_Cancel2, 48);
      AddTextPrinterParameterized3(cancelWindowId, FONT_SMALL,
                                   mainOffset + offset, 1, sFontColorTable[0],
                                   TEXT_SKIP_DRAW, gText_Cancel2);
    }
...
static void BlitBitmapToPartyWindow(u8 windowId, const u8 *b, u8 c, u8 x, u8 y,
                                    u8 width, u8 height) {
...
static void BlitBitmapToPartyWindow_LeftColumn(u8 windowId, u8 x, u8 y,
                                               u8 width, u8 height,
                                               bool8 hideHP) {
  if (width == 0 && height == 0) {
    width = 10;
  ...
  if (hideHP == FALSE)
    BlitBitmapToPartyWindow(windowId, sSlotTilemap_Main, 10, x, y, width,
                            height);
  else
    BlitBitmapToPartyWindow(windowId, sSlotTilemap_MainNoHP, 10, x, y, width,
                            height);
}

static void BlitBitmapToPartyWindow_RightColumn(u8 windowId, u8 x, u8 y,
                                                u8 width, u8 height,
                                                bool8 hideHP) {
  if (width == 0 && height == 0) {
    width = 18;
  ...
  if (hideHP == FALSE)
    BlitBitmapToPartyWindow(windowId, sSlotTilemap_Wide, 18, x, y, width,
                            height);
  else
    BlitBitmapToPartyWindow(windowId, sSlotTilemap_WideNoHP, 18, x, y, width,
                            height);
}
...
#define LOAD_PARTY_BOX_PAL(paletteIds, paletteOffsets)                         \
  {                                                                            \
    LoadPalette(GetPartyMenuPalBufferPtr(paletteIds[0]),                       \
                paletteOffsets[0] + palOffset, PLTT_SIZEOF(1));                \
    LoadPalette(GetPartyMenuPalBufferPtr(paletteIds[1]),                       \
                paletteOffsets[1] + palOffset, PLTT_SIZEOF(1));                \
    LoadPalette(GetPartyMenuPalBufferPtr(paletteIds[2]),                       \
                paletteOffsets[2] + palOffset, PLTT_SIZEOF(1));                \
  }

static void LoadPartyBoxPalette(struct PartyMenuBox *menuBox, u8 palFlags) {
  u8 palOffset =
      BG_PLTT_ID(GetWindowAttribute(menuBox->windowId, WINDOW_PALETTE_NUM));
  ...
  if (palFlags & PARTY_PAL_NO_MON) {
    ...
    if (palFlags & PARTY_PAL_SELECTED) {
      LOAD_PARTY_BOX_PAL(sPartyBoxSelectedForActionPalIds1,
                         sPartyBoxPalOffsets1);
      LOAD_PARTY_BOX_PAL(sPartyBoxCurrSelectionPalIds2, sPartyBoxPalOffsets2);
    } else {
      LOAD_PARTY_BOX_PAL(sPartyBoxSelectedForActionPalIds1,
                         sPartyBoxPalOffsets1);
      LOAD_PARTY_BOX_PAL(sPartyBoxSelectedForActionPalIds2,
    ...
    if (palFlags & PARTY_PAL_SELECTED) {
      LOAD_PARTY_BOX_PAL(sPartyBoxCurrSelectionFaintedPalIds,
                         sPartyBoxPalOffsets1);
      LOAD_PARTY_BOX_PAL(sPartyBoxCurrSelectionPalIds2, sPartyBoxPalOffsets2);
    } else {
    ...
    if (palFlags & PARTY_PAL_SELECTED) {
      LOAD_PARTY_BOX_PAL(sPartyBoxCurrSelectionMultiPalIds,
                         sPartyBoxPalOffsets1);
      LOAD_PARTY_BOX_PAL(sPartyBoxCurrSelectionPalIds2, sPartyBoxPalOffsets2);
    } else {
...
static void DisplayPartyPokemonBarDetail(u8 windowId, const u8 *str, u8 color,
                                         const u8 *align) {
  AddTextPrinterParameterized3(windowId, FONT_SMALL, align[0], align[1],
                               sFontColorTable[color], 0, str);
}

static void DisplayPartyPokemonNickname(struct Pokemon *mon,
                                        struct PartyMenuBox *menuBox, u8 c) {
  u8 nickname[POKEMON_NAME_LENGTH + 1];

  if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) {
    if (c == 1)
      menuBox->infoRects->blitFunc(
          menuBox->windowId, menuBox->infoRects->dimensions[0] >> 3,
          menuBox->infoRects->dimensions[1] >> 3,
          menuBox->infoRects->dimensions[2] >> 3,
          menuBox->infoRects->dimensions[3] >> 3, FALSE);
    GetMonNickname(mon, nickname);
    DisplayPartyPokemonBarDetail(menuBox->windowId, nickname, 0,
                                 menuBox->infoRects->dimensions);
  }
}

static void DisplayPartyPokemonLevelCheck(struct Pokemon *mon,
                                          struct PartyMenuBox *menuBox, u8 c) {
  if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) {
    ...
    if (ailment == AILMENT_NONE || ailment == AILMENT_PKRS) {
      if (c != 0)
        menuBox->infoRects->blitFunc(
            menuBox->windowId, menuBox->infoRects->dimensions[4] >> 3,
...
static void DisplayPartyPokemonLevel(u8 level, struct PartyMenuBox *menuBox) {
  ...
  DisplayPartyPokemonBarDetail(menuBox->windowId, gStringVar1, 0,
                               &menuBox->infoRects->dimensions[4]);
}

static void DisplayPartyPokemonGenderNidoranCheck(struct Pokemon *mon,
  ...
  if (c == 1)
    menuBox->infoRects->blitFunc(
        menuBox->windowId, menuBox->infoRects->dimensions[8] >> 3,
        (menuBox->infoRects->dimensions[9] >> 3) + 1,
        menuBox->infoRects->dimensions[10] >> 3,
        menuBox->infoRects->dimensions[11] >> 3, FALSE);
  GetMonNickname(mon, nickname);
  DisplayPartyPokemonGender(
      GetMonGender(mon), GetMonData(mon, MON_DATA_SPECIES), nickname, menuBox);
}

static void DisplayPartyPokemonGender(u8 gender, u16 species, u8 *nickname,
                                      struct PartyMenuBox *menuBox) {
  u8 palOffset =
      BG_PLTT_ID(GetWindowAttribute(menuBox->windowId, WINDOW_PALETTE_NUM));
  ...
  switch (gender) {
  case MON_MALE:
    LoadPalette(GetPartyMenuPalBufferPtr(sGenderMalePalIds[0]),
                sGenderPalOffsets[0] + palOffset, PLTT_SIZEOF(1));
    LoadPalette(GetPartyMenuPalBufferPtr(sGenderMalePalIds[1]),
                sGenderPalOffsets[1] + palOffset, PLTT_SIZEOF(1));
    DisplayPartyPokemonBarDetail(menuBox->windowId, gText_MaleSymbol, 2,
                                 &menuBox->infoRects->dimensions[8]);
    break;
  case MON_FEMALE:
    LoadPalette(GetPartyMenuPalBufferPtr(sGenderFemalePalIds[0]),
                sGenderPalOffsets[0] + palOffset, PLTT_SIZEOF(1));
    LoadPalette(GetPartyMenuPalBufferPtr(sGenderFemalePalIds[1]),
                sGenderPalOffsets[1] + palOffset, PLTT_SIZEOF(1));
    DisplayPartyPokemonBarDetail(menuBox->windowId, gText_FemaleSymbol, 2,
                                 &menuBox->infoRects->dimensions[8]);
    break;
  }
}

static void DisplayPartyPokemonHPCheck(struct Pokemon *mon,
                                       struct PartyMenuBox *menuBox, u8 c) {
  if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) {
    if (c != 0)
      menuBox->infoRects->blitFunc(
          menuBox->windowId, menuBox->infoRects->dimensions[12] >> 3,
...
static void DisplayPartyPokemonHP(u16 hp, struct PartyMenuBox *menuBox) {
  ...
  DisplayPartyPokemonBarDetail(menuBox->windowId, gStringVar1, 0,
                               &menuBox->infoRects->dimensions[12]);
}

static void DisplayPartyPokemonMaxHPCheck(struct Pokemon *mon,
                                          struct PartyMenuBox *menuBox, u8 c) {
  if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) {
    if (c != 0)
      menuBox->infoRects->blitFunc(
          menuBox->windowId, (menuBox->infoRects->dimensions[16] >> 3) + 1,
...
static void DisplayPartyPokemonMaxHP(u16 maxhp, struct PartyMenuBox *menuBox) {
  ...
  StringAppend(gStringVar1, gStringVar2);
  DisplayPartyPokemonBarDetail(menuBox->windowId, gStringVar1, 0,
                               &menuBox->infoRects->dimensions[16]);
}

static void DisplayPartyPokemonHPBarCheck(struct Pokemon *mon,
                                          struct PartyMenuBox *menuBox) {
  if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
    DisplayPartyPokemonHPBar(GetMonData(mon, MON_DATA_HP),
                             GetMonData(mon, MON_DATA_MAX_HP), menuBox);
}
...
static void DisplayPartyPokemonHPBar(u16 hp, u16 maxhp,
  ...
  case HP_BAR_GREEN:
  case HP_BAR_FULL:
    LoadPalette(GetPartyMenuPalBufferPtr(sHPBarGreenPalIds[0]),
                sHPBarPalOffsets[0] + palOffset, PLTT_SIZEOF(1));
    LoadPalette(GetPartyMenuPalBufferPtr(sHPBarGreenPalIds[1]),
                sHPBarPalOffsets[1] + palOffset, PLTT_SIZEOF(1));
    break;
  case HP_BAR_YELLOW:
    LoadPalette(GetPartyMenuPalBufferPtr(sHPBarYellowPalIds[0]),
                sHPBarPalOffsets[0] + palOffset, PLTT_SIZEOF(1));
    LoadPalette(GetPartyMenuPalBufferPtr(sHPBarYellowPalIds[1]),
                sHPBarPalOffsets[1] + palOffset, PLTT_SIZEOF(1));
    break;
  default:
    LoadPalette(GetPartyMenuPalBufferPtr(sHPBarRedPalIds[0]),
                sHPBarPalOffsets[0] + palOffset, PLTT_SIZEOF(1));
    LoadPalette(GetPartyMenuPalBufferPtr(sHPBarRedPalIds[1]),
                sHPBarPalOffsets[1] + palOffset, PLTT_SIZEOF(1));
    break;
  }

  hpFraction =
      GetScaledHPFraction(hp, maxhp, menuBox->infoRects->dimensions[22]);
  FillWindowPixelRect(menuBox->windowId, sHPBarPalOffsets[1],
                      menuBox->infoRects->dimensions[20],
                      menuBox->infoRects->dimensions[21], hpFraction, 1);
  FillWindowPixelRect(menuBox->windowId, sHPBarPalOffsets[0],
                      menuBox->infoRects->dimensions[20],
  ...
  if (hpFraction != menuBox->infoRects->dimensions[22]) {
    ...
    FillWindowPixelRect(menuBox->windowId, 0x0D,
                        menuBox->infoRects->dimensions[20] + hpFraction,
                        menuBox->infoRects->dimensions[21],
                        menuBox->infoRects->dimensions[22] - hpFraction, 1);
    FillWindowPixelRect(menuBox->windowId, 0x02,
                        menuBox->infoRects->dimensions[20] + hpFraction,
                        menuBox->infoRects->dimensions[21] + 1,
                        menuBox->infoRects->dimensions[22] - hpFraction, 2);
  }
  CopyWindowToVram(menuBox->windowId, COPYWIN_GFX);
}

static void DisplayPartyPokemonDescriptionText(u8 stringID,
                                               struct PartyMenuBox *menuBox,
                                               u8 c) {
  if (c) {
    int width = ((menuBox->infoRects->descTextLeft % 8) +
                 menuBox->infoRects->descTextWidth + 7) /
                8;
    int height = ((menuBox->infoRects->descTextTop % 8) +
                  menuBox->infoRects->descTextHeight + 7) /
                 8;
    menuBox->infoRects->blitFunc(
        menuBox->windowId, menuBox->infoRects->descTextLeft >> 3,
        menuBox->infoRects->descTextTop >> 3, width, height, TRUE);
  }
  if (c != 2)
    AddTextPrinterParameterized3(
        menuBox->windowId, FONT_NORMAL, menuBox->infoRects->descTextLeft,
...
void DisplayPartyMenuStdMessage(u32 stringId) {
  ...
  if (stringId != PARTY_MSG_NONE) {
    ...
    AddTextPrinterParameterized(*windowPtr, FONT_NORMAL, gStringVar4, 0, 1, 0,
                                0);
    ScheduleBgCopyTilemapToVram(2);
  }
}
...
static u8 DisplaySelectionWindow(u8 windowType) {
  ...
  case SELECTWINDOW_ACTIONS:
    SetWindowTemplateFields(&window, 2, 19,
                            19 - (sPartyMenuInternal->numActions * 2), 10,
  ...
  DrawStdFrameWithCustomTileAndPalette(sPartyMenuInternal->windowId[0], FALSE,
                                       0x4F, 13);
  ...
  for (i = 0; i < sPartyMenuInternal->numActions; i++) {
    u8 fontColorsId =
        (sPartyMenuInternal->actions[i] >= MENU_FIELD_MOVES) ? 4 : 3;
    AddTextPrinterParameterized4(
        sPartyMenuInternal->windowId[0], FONT_NORMAL, cursorDimension,
        (i * 16) + 1, letterSpacing, 0, sFontColorTable[fontColorsId], 0,
...
static void PrintMessage(const u8 *text) {
  ...
  AddTextPrinterParameterized2(
      WIN_MSG, FONT_NORMAL, text, GetPlayerTextSpeedDelay(), 0,
...
static void SetPartyMonSelectionActions(struct Pokemon *mons, u8 slotId,
                                        u8 action) {
  u8 i;
    ...
    for (j = 0; sFieldMoves[j] != FIELD_MOVES_COUNT; j++) {
      if (GetMonData(&mons[slotId], i + MON_DATA_MOVE1) == sFieldMoves[j]) {
        AppendToList(sPartyMenuInternal->actions,
                     &sPartyMenuInternal->numActions, j + MENU_FIELD_MOVES);
        break;
      }
    }
  }
...
static u8 GetPartyMenuActionsType(struct Pokemon *mon) {
  ...
  case PARTY_MENU_TYPE_FIELD:
    if (InMultiPartnerRoom() == TRUE || GetMonData(mon, MON_DATA_IS_EGG))
      ...
      actionType =
          ACTIONS_NONE; // actions populated by SetPartyMonFieldSelectionActions
    break;
  ...
  case PARTY_MENU_TYPE_DAYCARE:
    actionType = (GetMonData(mon, MON_DATA_IS_EGG)) ? ACTIONS_SUMMARY_ONLY
                                                    : ACTIONS_STORE;
    break;
  ...
  case PARTY_MENU_TYPE_STORE_PYRAMID_HELD_ITEMS:
    ...
    break;
  // The following have no selection actions (i.e. they exit immediately upon
  // selection) PARTY_MENU_TYPE_CONTEST PARTY_MENU_TYPE_CHOOSE_MON
...
static bool8 CreateSelectionWindow(u8 taskId) {
  ...
  if (gPartyMenu.menuType != PARTY_MENU_TYPE_STORE_PYRAMID_HELD_ITEMS) {
    SetPartyMonSelectionActions(gPlayerParty, gPartyMenu.slotId,
...
static void Task_HandleSelectionMenuInput(u8 taskId) {
  if (!gPaletteFade.active && MenuHelpers_ShouldWaitForLinkRecv() != TRUE) {
    ...
    case MENU_B_PRESSED:
      ...
      sCursorOptions[sPartyMenuInternal
                         ->actions[sPartyMenuInternal->numActions - 1]]
          .func(taskId);
      break;
    default:
      PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[2]);
...
static void CB2_ShowPokemonSummaryScreen(void) {
  if (gPartyMenu.menuType == PARTY_MENU_TYPE_IN_BATTLE) {
    ...
    ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, gPlayerParty,
...
static void SwitchSelectedMons(u8 taskId) {
  ...
  if (gPartyMenu.slotId2 == gPartyMenu.slotId) {
    ...
    CopyToBufferFromBgTilemap(0, sSlot1TilemapBuffer, tSlot1Left, tSlot1Top,
                              tSlot1Width, tSlot1Height);
    CopyToBufferFromBgTilemap(0, sSlot2TilemapBuffer, tSlot2Left, tSlot2Top,
...
static bool8 TryMovePartySlot(s16 x, s16 width, u8 *leftMove, u8 *newX,
                              u8 *newWidth) {
  if (x + width < 0)
    return FALSE;
  if (x >= 32)
...
static void MoveAndBufferPartySlot(const void *rectSrc, s16 x, s16 y, s16 width,
                                   s16 height, s16 dir) {
  ...
  if (TryMovePartySlot(x, width, &srcX, &newX, &newWidth)) {
    ...
    if (TryMovePartySlot(x + dir, width, &srcX, &newX, &newWidth))
      CopyRectToBgTilemapBufferRect(0, rectSrc, srcX, 0, width, height, newX, y,
                                    newWidth, height, 17, 0, 0);
  }
}
...
static void SlidePartyMenuBoxSpritesOneStep(u8 taskId) {
  ...
  if (tSlot1SlideDir != 0)
    MovePartyMenuBoxSprites(&sPartyMenuBoxes[gPartyMenu.slotId],
                            tSlot1SlideDir);
  if (tSlot2SlideDir != 0)
    MovePartyMenuBoxSprites(&sPartyMenuBoxes[gPartyMenu.slotId2],
                            tSlot2SlideDir);
}
...
static void SlidePartyMenuBoxOneStep(u8 taskId) {
  ...
  if (tSlot1SlideDir != 0)
    MoveAndBufferPartySlot(sSlot1TilemapBuffer, tSlot1Left + tSlot1Offset,
                           tSlot1Top, tSlot1Width, tSlot1Height,
                           tSlot1SlideDir);
  if (tSlot2SlideDir != 0)
    MoveAndBufferPartySlot(sSlot2TilemapBuffer, tSlot2Left + tSlot2Offset,
...
static void SwitchPartyMon(void) {
  ...
  *mon2 = *monBuffer;
  Free(monBuffer);
  SwitchMenuBoxSprites(&menuBoxes[0]->pokeballSpriteId,
                       &menuBoxes[1]->pokeballSpriteId);
  SwitchMenuBoxSprites(&menuBoxes[0]->itemSpriteId,
                       &menuBoxes[1]->itemSpriteId);
  SwitchMenuBoxSprites(&menuBoxes[0]->monSpriteId, &menuBoxes[1]->monSpriteId);
  SwitchMenuBoxSprites(&menuBoxes[0]->statusSpriteId,
                       &menuBoxes[1]->statusSpriteId);
}
...
static void CB2_GiveHoldItem(void) {
  if (gSpecialVar_ItemId == ITEM_NONE) {
    InitPartyMenu(gPartyMenu.menuType, KEEP_PARTY_LAYOUT, gPartyMenu.action,
                  TRUE, PARTY_MSG_NONE, Task_TryCreateSelectionWindow,
                  gPartyMenu.exitCallback);
  } else {
    sPartyMenuItemId =
        GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_HELD_ITEM);

    // Already holding item
    if (sPartyMenuItemId != ITEM_NONE) {
      InitPartyMenu(gPartyMenu.menuType, KEEP_PARTY_LAYOUT, gPartyMenu.action,
                    TRUE, PARTY_MSG_NONE, Task_SwitchHoldItemsPrompt,
                    gPartyMenu.exitCallback);
    }
    // Give mail
    ...
    // Give item
      ...
      InitPartyMenu(gPartyMenu.menuType, KEEP_PARTY_LAYOUT, gPartyMenu.action,
                    TRUE, PARTY_MSG_NONE, Task_GiveHoldItem,
                    gPartyMenu.exitCallback);
    }
  }
}
...
static void Task_SwitchHoldItemsPrompt(u8 taskId) {
  if (!gPaletteFade.active) {
    DisplayAlreadyHoldingItemSwitchMessage(&gPlayerParty[gPartyMenu.slotId],
                                           sPartyMenuItemId, TRUE);
...
static void Task_HandleSwitchItemsYesNoInput(u8 taskId) {
  ...
  case 0: // Yes, switch items
    ...
    // Giving item
      ...
      GiveItemToMon(&gPlayerParty[gPartyMenu.slotId], gSpecialVar_ItemId);
      DisplaySwitchedHeldItemMessage(gSpecialVar_ItemId, sPartyMenuItemId,
                                     TRUE);
      gTasks[taskId].func = Task_UpdateHeldItemSprite;
    }
    break;
...
static void CB2_WriteMailToGiveMon(void) {
  ...
  DoEasyChatScreen(EASY_CHAT_TYPE_MAIL, gSaveBlock1Ptr->mail[mail].words,
...
static void CB2_ReturnToPartyMenuFromWritingMail(void) {
  ...
  if (gSpecialVar_Result == FALSE) {
    ...
    AddBagItem(item, 1);
    InitPartyMenu(gPartyMenu.menuType, KEEP_PARTY_LAYOUT, gPartyMenu.action,
                  TRUE, PARTY_MSG_CHOOSE_MON, Task_TryCreateSelectionWindow,
                  gPartyMenu.exitCallback);
  }
  // Wrote mail
  else {
    InitPartyMenu(gPartyMenu.menuType, KEEP_PARTY_LAYOUT, gPartyMenu.action,
                  TRUE, PARTY_MSG_NONE, Task_DisplayGaveMailFromPartyMessage,
                  gPartyMenu.exitCallback);
  }
}
...
static void Task_DisplayGaveMailFromPartyMessage(u8 taskId) {
  if (!gPaletteFade.active) {
    if (sPartyMenuItemId == ITEM_NONE)
      DisplayGaveHeldItemMessage(&gPlayerParty[gPartyMenu.slotId],
                                 gSpecialVar_ItemId, FALSE, 0);
    else
      DisplaySwitchedHeldItemMessage(gSpecialVar_ItemId, sPartyMenuItemId,
                                     FALSE);
    gTasks[taskId].func = Task_UpdateHeldItemSprite;
...
static void Task_TossHeldItem(u8 taskId) {
  ...
  if (IsPartyMenuTextPrinterActive() != TRUE) {
    ...
    SetMonData(mon, MON_DATA_HELD_ITEM, &item);
    UpdatePartyMonHeldItemSprite(mon, &sPartyMenuBoxes[gPartyMenu.slotId]);
    DisplayPartyPokemonDescriptionText(PARTYBOX_DESC_DONT_HAVE,
                                       &sPartyMenuBoxes[gPartyMenu.slotId], 1);
    gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
  }
}
...
static void CB2_ReadHeldMail(void) {
  ReadMail(&gSaveBlock1Ptr->mail[GetMonData(&gPlayerParty[gPartyMenu.slotId],
                                            MON_DATA_MAIL)],
           CB2_ReturnToPartyMenuFromReadingMail, TRUE);
}
...
static void CursorCb_Enter(u8 taskId) {
  ...
  ConvertIntToDecimalStringN(gStringVar1, maxBattlers, STR_CONV_MODE_LEFT_ALIGN,
                             1);
...
static void CursorCb_NoEntry(u8 taskId) {
  ...
  DisplayPartyPokemonDescriptionText(PARTYBOX_DESC_ABLE_3,
                                     &sPartyMenuBoxes[gPartyMenu.slotId], 1);
...
static void CursorCb_Register(u8 taskId) {
  u16 species2 =
      GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_SPECIES_OR_EGG);
  u16 species = GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_SPECIES);
  u8 isModernFatefulEncounter = GetMonData(&gPlayerParty[gPartyMenu.slotId],
                                           MON_DATA_MODERN_FATEFUL_ENCOUNTER);

  switch (CanRegisterMonForTradingBoard(
      *(struct RfuGameCompatibilityData *)GetHostRfuGameData(), species2,
      species, isModernFatefulEncounter)) {
...
static void CursorCb_Trade1(u8 taskId) {
  ...
  u32 stringId = GetUnionRoomTradeMessageId(
      *(struct RfuGameCompatibilityData *)GetHostRfuGameData(),
      gRfuPartnerCompatibilityData, species2, gUnionRoomOfferedSpecies,
      gUnionRoomRequestedMonType, species, isModernFatefulEncounter);
...
// Not fully implemented, and normally unreachable because
// PARTY_MENU_TYPE_SPIN_TRADE is never used
...
// See comment on CursorCb_Trade2. Because no callback is set, selecting YES (0)
// to spin trade just closes the party menu
...
static void CursorCb_FieldMove(u8 taskId) {
  u8 fieldMove =
      sPartyMenuInternal->actions[Menu_GetCursorPos()] - MENU_FIELD_MOVES;
  ...
  if (MenuHelpers_IsLinkActive() == TRUE || InUnionRoom() == TRUE) {
    if (fieldMove == FIELD_MOVE_MILK_DRINK ||
    ...
    if (fieldMove <= FIELD_MOVE_WATERFALL &&
      ...
      case FIELD_MOVE_DIG:
        mapHeader = Overworld_GetMapHeaderByGroupAndId(
            gSaveBlock1Ptr->escapeWarp.mapGroup,
...
void CB2_ReturnToPartyMenuFromFlyMap(void) {
  InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE,
                PARTY_ACTION_CHOOSE_MON, TRUE, PARTY_MSG_CHOOSE_MON,
...
static void CreatePartyMonIconSprite(struct Pokemon *mon,
                                     struct PartyMenuBox *menuBox, u32 slot) {
  ...
  if (IsMultiBattle() == TRUE && gMain.inBattle)
    handleDeoxys =
        (sMultiBattlePartnersPartyMask[slot] ^ handleDeoxys) ? TRUE : FALSE;

  species2 = GetMonData(mon, MON_DATA_SPECIES_OR_EGG);
  CreatePartyMonIconSpriteParameterized(species2,
                                        GetMonData(mon, MON_DATA_PERSONALITY),
                                        menuBox, 1, handleDeoxys);
  UpdatePartyMonHPBar(menuBox->monSpriteId, mon);
}

static void CreatePartyMonIconSpriteParameterized(u16 species, u32 pid,
                                                  struct PartyMenuBox *menuBox,
                                                  u8 priority,
                                                  bool32 handleDeoxys) {
  if (species != SPECIES_NONE) {
    menuBox->monSpriteId =
        CreateMonIcon(species, SpriteCB_MonIcon, menuBox->spriteCoords[0],
...
static void UpdatePartyMonHPBar(u8 spriteId, struct Pokemon *mon) {
  UpdateHPBar(spriteId, GetMonData(mon, MON_DATA_HP),
...
static void CreatePartyMonHeldItemSprite(struct Pokemon *mon,
                                         struct PartyMenuBox *menuBox) {
  if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) {
    menuBox->itemSpriteId =
        CreateSprite(&sSpriteTemplate_HeldItem, menuBox->spriteCoords[2],
                     menuBox->spriteCoords[3], 0);
    UpdatePartyMonHeldItemSprite(mon, menuBox);
  }
}

static void
CreatePartyMonHeldItemSpriteParameterized(u16 species, u16 item,
                                          struct PartyMenuBox *menuBox) {
  if (species != SPECIES_NONE) {
    menuBox->itemSpriteId =
        CreateSprite(&sSpriteTemplate_HeldItem, menuBox->spriteCoords[2],
...
static void UpdatePartyMonHeldItemSprite(struct Pokemon *mon,
                                         struct PartyMenuBox *menuBox) {
...
void DrawHeldItemIconsForTrade(u8 *partyCounts, u8 *partySpriteIds,
                               u8 whichParty) {
  u16 i;
  u16 item;
...
static void CreateHeldItemSpriteForTrade(u8 spriteId, bool8 isMail) {
  ...
  u8 newSpriteId =
      CreateSprite(&sSpriteTemplate_HeldItem, 250, 170, subpriority - 1);
...
static void CreatePartyMonPokeballSprite(struct Pokemon *mon,
                                         struct PartyMenuBox *menuBox) {
  if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
    menuBox->pokeballSpriteId =
        CreateSprite(&sSpriteTemplate_MenuPokeball, menuBox->spriteCoords[6],
                     menuBox->spriteCoords[7], 8);
}

static void
CreatePartyMonPokeballSpriteParameterized(u16 species,
                                          struct PartyMenuBox *menuBox) {
  if (species != SPECIES_NONE) {
    menuBox->pokeballSpriteId =
        CreateSprite(&sSpriteTemplate_MenuPokeball, menuBox->spriteCoords[6],
...
// At the very least this is how the unused anim cmds for
// sSpriteAnimTable_MenuPokeballSmall were meant to be accessed
static void UNUSED SpriteCB_BounceConfirmCancelButton(u8 spriteId, u8 spriteId2,
                                                      u8 animNum) {
  if (animNum == 0) {
...
static void CreatePartyMonStatusSprite(struct Pokemon *mon,
                                       struct PartyMenuBox *menuBox) {
  if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE) {
    menuBox->statusSpriteId =
        CreateSprite(&sSpriteTemplate_StatusIcons, menuBox->spriteCoords[4],
                     menuBox->spriteCoords[5], 0);
    SetPartyMonAilmentGfx(mon, menuBox);
  }
}

static void
CreatePartyMonStatusSpriteParameterized(u16 species, u8 status,
                                        struct PartyMenuBox *menuBox) {
  if (species != SPECIES_NONE) {
    menuBox->statusSpriteId =
        CreateSprite(&sSpriteTemplate_StatusIcons, menuBox->spriteCoords[4],
...
static void SetPartyMonAilmentGfx(struct Pokemon *mon,
                                  struct PartyMenuBox *menuBox) {
...
void CB2_ShowPartyMenuForItemUse(void) {
  ...
  InitPartyMenu(menuType, partyLayout, PARTY_ACTION_USE_ITEM, TRUE, msgId, task,
                callback);
}
...
static void CB2_ReturnToBagMenu(void) {
  if (InBattlePyramid() == FALSE)
    ...
    GoToBattlePyramidBagMenu(PYRAMIDBAG_LOC_PREV,
                             gPyramidBagMenuState.exitCallback);
}
...
static void Task_SetSacredAshCB(u8 taskId) {
  if (!gPaletteFade.active) {
    ...
    gItemUseCB(
        taskId,
        Task_ClosePartyMenuAfterText); // ItemUseCB_SacredAsh in this case
  }
}
...
static bool8 IsItemFlute(u16 item) {
  if (item == ITEM_BLUE_FLUTE || item == ITEM_RED_FLUTE ||
      item == ITEM_YELLOW_FLUTE)
    return TRUE;
  return FALSE;
}

static bool8 ExecuteTableBasedItemEffect_(u8 partyMonIndex, u16 item,
                                          u8 monMoveIndex) {
  if (gMain.inBattle)
    return ExecuteTableBasedItemEffect(&gPlayerParty[partyMonIndex], item,
                                       GetPartyIdFromBattleSlot(partyMonIndex),
                                       monMoveIndex);
  else
    return ExecuteTableBasedItemEffect(&gPlayerParty[partyMonIndex], item,
                                       partyMonIndex, monMoveIndex);
}
...
void ItemUseCB_Medicine(u8 taskId, TaskFunc task) {
  ...
  if (NotUsingHPEVItemOnShedinja(mon, item) == FALSE) {
    cannotUse = TRUE;
             ...
             GetMonData(mon,
                        MON_DATA_DEAD)) // Don't allow healing or reviving for
                                        // fainted mons on hardcore mode
  {
    canHeal = FALSE;
    cannotUse = TRUE;
  } else {
  ...
  if (cannotUse != FALSE) {
    ...
    if (gSprites[sPartyMenuBoxes[gPartyMenu.slotId].statusSpriteId].invisible)
      DisplayPartyPokemonLevelCheck(mon, &sPartyMenuBoxes[gPartyMenu.slotId],
                                    1);
    if (canHeal == TRUE) {
      if (hp == 0)
        AnimatePartySlot(gPartyMenu.slotId, 1);
      PartyMenuModifyHP(taskId, gPartyMenu.slotId, 1,
...
static void ShowMoveSelectWindow(u8 slot) {
  ...
  for (i = 0; i < MAX_MON_MOVES; i++) {
    ...
    AddTextPrinterParameterized(windowId, fontId, gMoveNames[move], 8,
                                (i * 16) + 1, TEXT_SKIP_DRAW, NULL);
    if (move != MOVE_NONE)
      moveCount++;
  }
...
// move[1] doesn't use constants cause I don't know if it's actually a move ID
// storage
...
static void CB2_ShowSummaryScreenToForgetMove(void) {
  ShowSelectMovePokemonSummaryScreen(
      gPlayerParty, gPartyMenu.slotId, gPlayerPartyCount - 1,
...
static void CB2_ReturnToPartyMenuWhileLearningMove(void) {
  if (gSpecialVar_ItemId == ITEM_RARE_CANDY &&
      ...
      CheckBagHasItem(gSpecialVar_ItemId, 1))
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE,
                  PARTY_ACTION_USE_ITEM, TRUE, PARTY_MSG_NONE,
                  Task_ReturnToPartyMenuWhileLearningMove,
                  gPartyMenu.exitCallback);
  else
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE,
                  PARTY_ACTION_CHOOSE_MON, TRUE, PARTY_MSG_NONE,
                  Task_ReturnToPartyMenuWhileLearningMove,
                  gPartyMenu.exitCallback);
}
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task) {
  ...
  if ((GetMonData(mon, MON_DATA_LEVEL) != MAX_LEVEL &&
    ...
    cannotUseEffect =
        ExecuteTableBasedItemEffect_(gPartyMenu.slotId, *itemPtr, 0);
  ...
  if (cannotUseEffect) {
    ...
    RemoveBagItem(gSpecialVar_ItemId, 1);
    GetMonNickname(mon, gStringVar1);
    ConvertIntToDecimalStringN(gStringVar2, GetMonData(mon, MON_DATA_LEVEL),
                               STR_CONV_MODE_LEFT_ALIGN, 3);
...
static void DisplayLevelUpStatsPg1(u8 taskId) {
  ...
  DrawLevelUpWindowPg1(arrayPtr[12], arrayPtr, &arrayPtr[6], TEXT_COLOR_WHITE,
                       TEXT_COLOR_DARK_GRAY, TEXT_COLOR_LIGHT_GRAY);
...
static void DisplayLevelUpStatsPg2(u8 taskId) {
  ...
  DrawLevelUpWindowPg2(arrayPtr[12], &arrayPtr[6], TEXT_COLOR_WHITE,
                       TEXT_COLOR_DARK_GRAY, TEXT_COLOR_LIGHT_GRAY);
  CopyWindowToVram(arrayPtr[12], COPYWIN_GFX);
  ScheduleBgCopyTilemapToVram(2);
}
...
static void PartyMenuTryEvolution(u8 taskId) {
  ...
  u16 targetSpecies =
      GetEvolutionTargetSpecies(mon, EVO_MODE_NORMAL, ITEM_NONE, NULL);
#else
  u16 targetSpecies =
      GetEvolutionTargetSpecies(mon, EVO_MODE_NORMAL, ITEM_NONE);
#endif
...
u8 GetItemEffectType(u16 item) {
  ...
  if ((itemEffect[0] & (ITEM0_DIRE_HIT | ITEM0_X_ATTACK)) || itemEffect[1] ||
      itemEffect[2] || (itemEffect[3] & ITEM3_GUARD_SPEC))
...
// As opposted to by selecting Give in the party menu, which is handled by
// CursorCb_Give
void CB2_ChooseMonToGiveItem(void) {
  MainCallback callback = (InBattlePyramid() == FALSE)
                              ? CB2_ReturnToBagMenu
                              : CB2_ReturnToPyramidBagMenu;
  InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE,
                PARTY_ACTION_GIVE_ITEM, FALSE, PARTY_MSG_GIVE_TO_WHICH_MON,
...
static void CB2_ReturnToPartyOrBagMenuFromWritingMail(void) {
  ...
  // Wrote mail
    ...
    InitPartyMenu(gPartyMenu.menuType, KEEP_PARTY_LAYOUT, gPartyMenu.action,
                  TRUE, PARTY_MSG_NONE, Task_DisplayGaveMailFromBagMessage,
                  gPartyMenu.exitCallback);
  }
}
...
static void Task_DisplayGaveMailFromBagMessage(u8 taskId) {
  if (!gPaletteFade.active) {
    if (sPartyMenuItemId != ITEM_NONE)
      DisplaySwitchedHeldItemMessage(gPartyMenu.bagItem, sPartyMenuItemId,
                                     FALSE);
    else
      DisplayGaveHeldItemMessage(&gPlayerParty[gPartyMenu.slotId],
                                 gPartyMenu.bagItem, FALSE, 1);
    gTasks[taskId].func = Task_UpdateHeldItemSpriteAndClosePartyMenu;
  }
}
...
void ChooseMonToGiveMailFromMailbox(void) {
  InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE,
                PARTY_ACTION_GIVE_MAILBOX_MAIL, FALSE,
...
static void TryGiveMailToSelectedMon(u8 taskId) {
  ...
  gPartyMenuUseExitCallback = FALSE;
  mail = &gSaveBlock1Ptr->mail[gPlayerPCItemPageInfo.itemsAbove + PARTY_SIZE +
...
void InitChooseHalfPartyForBattle(u8 unused) {
  ClearSelectedPartyOrder();
  InitPartyMenu(PARTY_MENU_TYPE_CHOOSE_HALF, PARTY_LAYOUT_SINGLE,
                PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON,
                Task_HandleChooseMonInput, gMain.savedCallback);
...
static bool8 GetBattleEntryEligibility(struct Pokemon *mon) {
  ...
  if (GetMonData(mon, MON_DATA_IS_EGG) ||
      ...
      (gSaveBlock1Ptr->location.mapGroup ==
           MAP_GROUP(BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY) &&
...
static u8 CheckBattleEntriesAndGetMessage(void) {
  ...
  if (order[minBattlers - 1] == 0) {
    ...
    ConvertIntToDecimalStringN(gStringVar1, minBattlers,
  ...
  for (i = 0; i < maxBattlers - 1; i++) {
    ...
    for (j = i + 1; j < maxBattlers; j++) {
      ...
      if (item != ITEM_NONE &&
          item == GetMonData(&party[order[j] - 1], MON_DATA_HELD_ITEM))
...
static const u8 *GetFacilityCancelString(void) {
  ...
  if (!(facilityNum != FACILITY_UNION_ROOM &&
        facilityNum != FACILITY_MULTI_OR_EREADER))
...
void ChooseMonForTradingBoard(u8 menuType, MainCallback callback) {
  InitPartyMenu(menuType, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, FALSE,
                PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, callback);
}

void ChooseMonForMoveTutor(void) {
  InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE,
                PARTY_ACTION_MOVE_TUTOR, FALSE, PARTY_MSG_TEACH_WHICH_MON,
...
void ChooseMonForWirelessMinigame(void) {
  InitPartyMenu(PARTY_MENU_TYPE_MINIGAME, PARTY_LAYOUT_SINGLE,
                PARTY_ACTION_MINIGAME, FALSE, PARTY_MSG_CHOOSE_MON_OR_CANCEL,
                Task_HandleChooseMonInput,
                CB2_ReturnToFieldContinueScriptPlayMapMusic);
}
...
void OpenPartyMenuInBattle(u8 partyAction) {
  InitPartyMenu(PARTY_MENU_TYPE_IN_BATTLE, GetPartyLayoutFromBattleType(),
                partyAction, FALSE, PARTY_MSG_CHOOSE_MON,
                Task_HandleChooseMonInput,
...
static bool8 TrySwitchInPokemon(void) {
  ...
  if (gPartyMenu.action == PARTY_ACTION_CANT_SWITCH) {
    ...
    GetMonNickname(&gPlayerParty[GetPartyIdFromBattlePartyId(
                       gBattlerPartyIndexes[currBattler])],
                   gStringVar1);
...
static void BufferBattlePartyOrder(u8 *partyBattleOrder, u8 flankId) {
  ...
  if (IsMultiBattle() == TRUE) {
    ...
    // i.e. the party id order below would be 0, 3, 5, 4, 2, 1, and the two
    // parties would be 0,5,4 and 3,2,1
    if (flankId != 0) {
...
void BufferBattlePartyCurrentOrderBySide(u8 battlerId, u8 flankId) {
  BufferBattlePartyOrderBySide(gBattleStruct->battlerPartyOrders[battlerId],
                               flankId, battlerId);
}

// when GetBattlerSide(battlerId) == B_SIDE_PLAYER, this function is identical
// the one above
static void BufferBattlePartyOrderBySide(u8 *partyBattleOrder, u8 flankId,
                                         u8 battlerId) {
  ...
  for (i = 0; i < 3; i++)
    partyBattleOrder[i] =
        (partyIndexes[0 + (i * 2)] << 4) | partyIndexes[1 + (i * 2)];
}
...
static void SetPartyIdAtBattleSlot(u8 slot, u8 setVal) {
  ...
  slot /= 2;
  if (modResult != 0)
    gBattlePartyCurrentOrder[slot] =
        (gBattlePartyCurrentOrder[slot] & 0xF0) | setVal;
  else
    gBattlePartyCurrentOrder[slot] =
        (gBattlePartyCurrentOrder[slot] & 0xF) | (setVal << 4);
}
...
static void UpdatePartyToBattleOrder(void) {
  ...
  for (i = 0; i < PARTY_SIZE; i++)
    memcpy(&gPlayerParty[GetPartyIdFromBattlePartyId(i)], &partyBuffer[i],
...
static void UpdatePartyToFieldOrder(void) {
  ...
  memcpy(partyBuffer, gPlayerParty, sizeof(gPlayerParty));
  for (i = 0; i < PARTY_SIZE; i++)
    memcpy(&gPlayerParty[GetPartyIdFromBattleSlot(i)], &partyBuffer[i],
           sizeof(struct Pokemon));
  Free(partyBuffer);
}
...
void ShowPartyMenuToShowcaseMultiBattleParty(void) {
  InitPartyMenu(PARTY_MENU_TYPE_MULTI_SHOWCASE, PARTY_LAYOUT_MULTI_SHOWCASE,
                PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_NONE,
                Task_InitMultiPartnerPartySlideIn, gMain.savedCallback);
}
...
static void Task_MultiPartnerPartySlideIn(u8 taskId) {
  ...
  if (!gPaletteFade.active) {
    ...
    if (tXPos == 0) {
      ...
      PlaySE(SE_M_HARDEN); // The Harden SE plays once the partners party mons
                           // have slid on screen
...
static void SlideMultiPartyMenuBoxSpritesOneStep(u8 taskId) {
  ...
  for (i = MULTI_PARTY_SIZE; i < PARTY_SIZE; i++) {
    if (gMultiPartnerParty[i - MULTI_PARTY_SIZE].species != SPECIES_NONE) {
      ...
      MoveMultiPartyMenuBoxSprite(sPartyMenuBoxes[i].pokeballSpriteId,
                                  tXPos - 8);
...
void ChooseMonForDaycare(void) {
  InitPartyMenu(PARTY_MENU_TYPE_DAYCARE, PARTY_LAYOUT_SINGLE,
                PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON_2,
                Task_HandleChooseMonInput, BufferMonSelection);
}

static void UNUSED ChoosePartyMonByMenuType(u8 menuType) {
  gFieldCallback2 = CB2_FadeFromPartyMenu;
  InitPartyMenu(menuType, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_AND_CLOSE,
                FALSE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput,
                CB2_ReturnToField);
}
...
static void Task_ChooseContestMon(u8 taskId) {
  if (!gPaletteFade.active) {
    ...
    InitPartyMenu(PARTY_MENU_TYPE_CONTEST, PARTY_LAYOUT_SINGLE,
                  PARTY_ACTION_CHOOSE_AND_CLOSE, FALSE, PARTY_MSG_CHOOSE_MON,
                  Task_HandleChooseMonInput, CB2_ChooseContestMon);
    DestroyTask(taskId);
  }
}
...
// Used as a script special for showing a party mon to various npcs (e.g.
// in-game trades, move deleter)
...
// Can be called if the Battle Pyramid Bag is full on exiting and at least one
// party mon still has held items The player can then select to toss items from
// the bag or take/toss held items from the party
...
static void Task_BattlePyramidChooseMonHeldItems(u8 taskId) {
  if (!gPaletteFade.active) {
    CleanupOverworldWindowsAndTilemaps();
    InitPartyMenu(PARTY_MENU_TYPE_STORE_PYRAMID_HELD_ITEMS, PARTY_LAYOUT_SINGLE,
                  PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON,
                  Task_HandleChooseMonInput, BufferMonSelection);
    DestroyTask(taskId);
  }
...
void MoveDeleterForgetMove(void) {
  ...
  SetMonMoveSlot(&gPlayerParty[gSpecialVar_0x8004], MOVE_NONE,
                 gSpecialVar_0x8005);
...
static void ShiftMoveSlot(struct Pokemon *mon, u8 slotTo, u8 slotFrom) {
  ...
  ppBonuses |=
      (ppBonusMove1 << (slotFrom * 2)) + (ppBonusMove2 << (slotTo * 2));
...
void IsLastMonThatKnowsSurf(void) {
  ...
  gSpecialVar_Result = FALSE;
  move = GetMonData(&gPlayerParty[gSpecialVar_0x8004],
