...
EWRAM_DATA struct UnusedControllerStruct gUnusedControllerStruct =
    {}; // Debug? Unused code that writes to it, never read
...
void SetUpBattleVarsAndBirchZigzagoon(void) {
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_FIRST_BATTLE) {
    ZeroEnemyPartyMons();
    CreateMon(&gEnemyParty[0], SPECIES_ZIGZAGOON, 2, USE_RANDOM_IVS, 0, 0,
              OT_ID_PLAYER_ID, 0);
    i = 0;
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_RECORDED) {
      if (gBattleTypeFlags & BATTLE_TYPE_MULTI &&
        ...
        for (multiplayerId = gRecordedBattleMultiplayerId, i = 0;
             i < MAX_BATTLERS_COUNT; i++) {
          ...
          } else if ((!(gLinkPlayers[i].id & 1) &&
                      !(gLinkPlayers[multiplayerId].id & 1)) ||
                     ((gLinkPlayers[i].id & 1) &&
                      (gLinkPlayers[multiplayerId].id & 1))) {
            gBattlerControllerFuncs[gLinkPlayers[i].id] =
                SetControllerToRecordedPlayer;
        ...
        if (i < 2) {
          if (GET_BATTLER_SIDE2(i) == B_SIDE_PLAYER) {
            if (GetMonData(&gPlayerParty[j], MON_DATA_HP) != 0 &&
                GetMonData(&gPlayerParty[j], MON_DATA_SPECIES_OR_EGG) !=
                    SPECIES_NONE &&
                GetMonData(&gPlayerParty[j], MON_DATA_SPECIES_OR_EGG) !=
                    SPECIES_EGG &&
            ...
            if (GetMonData(&gEnemyParty[j], MON_DATA_HP) != 0 &&
                GetMonData(&gEnemyParty[j], MON_DATA_SPECIES_OR_EGG) !=
                    SPECIES_NONE &&
                GetMonData(&gEnemyParty[j], MON_DATA_SPECIES_OR_EGG) !=
                    SPECIES_EGG &&
...
void PrepareBufferDataTransferLink(u8 bufferId, u16 size, u8 *data) {
  ...
  gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] +
                        LINK_BUFF_BUFFER_ID] = bufferId;
  gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] +
                        LINK_BUFF_ACTIVE_BATTLER] = gActiveBattler;
  gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] + LINK_BUFF_ATTACKER] =
      gBattlerAttacker;
  gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] + LINK_BUFF_TARGET] =
      gBattlerTarget;
  gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] + LINK_BUFF_SIZE_LO] =
      alignedSize;
  ...
  gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] +
                        LINK_BUFF_EFFECT_BATTLER] = gEffectBattler;

  for (i = 0; i < size; i++)
    gLinkBattleSendBuffer[gTasks[sLinkSendTaskId].data[14] + LINK_BUFF_DATA +
                          i] = data[i];
    ...
    if (gTasks[taskId].data[15] != gTasks[taskId].data[14]) {
      if (gTasks[taskId].data[13] == 0) {
        ...
        SendBlock(BitmaskAllOtherLinkPlayers(),
                  &gLinkBattleSendBuffer[gTasks[taskId].data[15]], blockSize);
        gTasks[taskId].data[11]++;
      } else {
  ...
  case 4:
    if (IsLinkTaskFinished()) {
      ...
      gTasks[taskId].data[15] =
          gTasks[taskId].data[15] + blockSize + LINK_BUFF_DATA;
      ...
      if (GetBlockReceivedStatus() & gBitTable[i]) {
        ...
        recvBuffer = (u8 *)gBlockRecvBuffer[i];
        {
          ...
          if (gTasks[sLinkReceiveTaskId].data[14] + 9 + dataSize > 0x1000) {
            gTasks[sLinkReceiveTaskId].data[12] =
...
static void Task_HandleCopyReceivedLinkBuffersData(u8 taskId) {
  ...
  if (gTasks[taskId].data[15] != gTasks[taskId].data[14]) {
    ...
    battlerId = gLinkBattleRecvBuffer[gTasks[taskId].data[15] +
                                      LINK_BUFF_ACTIVE_BATTLER];
    blockSize =
        gLinkBattleRecvBuffer[gTasks[taskId].data[15] + LINK_BUFF_SIZE_LO] |
        (gLinkBattleRecvBuffer[gTasks[taskId].data[15] + LINK_BUFF_SIZE_HI]
         << 8);
    ...
    case 0:
      ...
      if (!(gBattleTypeFlags & BATTLE_TYPE_IS_MASTER)) {
        ...
        gAbsentBattlerFlags =
            gLinkBattleRecvBuffer[gTasks[taskId].data[15] +
                                  LINK_BUFF_ABSENT_BATTLER_FLAGS];
        gEffectBattler = gLinkBattleRecvBuffer[gTasks[taskId].data[15] +
                                               LINK_BUFF_EFFECT_BATTLER];
      }
      break;
    case 1:
...
static void UNUSED BtlController_EmitGetRawMonData(u8 bufferId, u8 monId,
                                                   u8 bytes) {
...
void BtlController_EmitSetMonData(u8 bufferId, u8 requestId, u8 monToCheck,
                                  u8 bytes, void *data) {
  s32 i;
...
static void UNUSED BtlController_EmitSetRawMonData(u8 bufferId, u8 monId,
                                                   u8 bytes, void *data) {
  s32 i;
...
void BtlController_EmitSwitchInAnim(u8 bufferId, u8 partyId,
                                    bool8 dontClearSubstituteBit) {
...
void BtlController_EmitMoveAnimation(u8 bufferId, u16 move, u8 turnOfMove,
                                     u16 movePower, s32 dmg, u8 friendship,
  ...
  memcpy(&sBattleBuffersTransferData[16], disableStructPtr,
         sizeof(struct DisableStruct));
  PrepareBufferDataTransfer(bufferId, sBattleBuffersTransferData,
                            16 + sizeof(struct DisableStruct));
}
...
void BtlController_EmitChooseMove(u8 bufferId, bool8 isDoubleBattle,
                                  bool8 NoPpNumber,
...
void BtlController_EmitChoosePokemon(u8 bufferId, u8 caseId, u8 slotId,
                                     u8 abilityId, u8 *data) {
  s32 i;
...
static void UNUSED BtlController_EmitDMA3Transfer(u8 bufferId, void *dst,
                                                  u16 size, void *data) {
  s32 i;
...
static void UNUSED BtlController_EmitPlayBGM(u8 bufferId, u16 songId,
                                             void *data) {
  s32 i;
...
void BtlController_EmitChosenMonReturnValue(u8 bufferId, u8 partyId,
                                            u8 *battlePartyOrder) {
  s32 i;
...
void BtlController_EmitPlayFanfareOrBGM(u8 bufferId, u16 songId,
                                        bool8 playBGM) {
...
void BtlController_EmitDrawPartyStatusSummary(u8 bufferId,
                                              struct HpAndStatus *hpAndStatus,
                                              u8 flags) {
  s32 i;

  sBattleBuffersTransferData[0] = CONTROLLER_DRAWPARTYSTATUSSUMMARY;
  sBattleBuffersTransferData[1] =
      flags & ~PARTY_SUMM_SKIP_DRAW_DELAY; // If true, skip player side
  sBattleBuffersTransferData[2] =
      (flags & PARTY_SUMM_SKIP_DRAW_DELAY) >>
      7; // If true, skip delay after drawing. True during intro
  sBattleBuffersTransferData[3] = CONTROLLER_DRAWPARTYSTATUSSUMMARY;
  for (i = 0; i < (s32)(sizeof(struct HpAndStatus) * PARTY_SIZE); i++)
    sBattleBuffersTransferData[4 + i] = *(i + (u8 *)(hpAndStatus));
  PrepareBufferDataTransfer(bufferId, sBattleBuffersTransferData,
                            sizeof(struct HpAndStatus) * PARTY_SIZE + 4);
}
...
void BtlController_EmitBattleAnimation(u8 bufferId, u8 animationId,
                                       u16 argument) {
...
void BtlController_EmitLinkStandbyMsg(u8 bufferId, u8 mode, bool32 record) {
  ...
  if (record_)
    sBattleBuffersTransferData[3] = sBattleBuffersTransferData[2] =
        RecordedBattle_BufferNewBattlerData(&sBattleBuffersTransferData[4]);
  else
    sBattleBuffersTransferData[3] = sBattleBuffersTransferData[2] = 0;

  PrepareBufferDataTransfer(bufferId, sBattleBuffersTransferData,
                            sBattleBuffersTransferData[2] + 4);
}
...
void BtlController_EmitEndLinkBattle(u8 bufferId, u8 battleOutcome) {
  ...
  sBattleBuffersTransferData[5] = sBattleBuffersTransferData[4] =
      RecordedBattle_BufferNewBattlerData(&sBattleBuffersTransferData[6]);
  PrepareBufferDataTransfer(bufferId, sBattleBuffersTransferData,
                            sBattleBuffersTransferData[4] + 6);
}
