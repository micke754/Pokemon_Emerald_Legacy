...
static void _InitContestMonPixels(u8 *spriteGfx, u16 *palette,
                                  u16 (*destPixels)[64][64]);
...
static const u16 sPictureFramePalettes[] =
    INCBIN_U16("graphics/picture_frame/bg.gbapal");
static const u8 sPictureFrameTiles_Cool[] =
    INCBIN_U8("graphics/picture_frame/cool.4bpp.rl");
static const u8 sPictureFrameTiles_Beauty[] =
    INCBIN_U8("graphics/picture_frame/beauty.4bpp.rl");
static const u8 sPictureFrameTiles_Cute[] =
    INCBIN_U8("graphics/picture_frame/cute.4bpp.rl");
static const u8 sPictureFrameTiles_Smart[] =
    INCBIN_U8("graphics/picture_frame/smart.4bpp.rl");
static const u8 sPictureFrameTiles_Tough[] =
    INCBIN_U8("graphics/picture_frame/tough.4bpp.rl");
static const u8 sPictureFrameTiles_HallLobby[] =
    INCBIN_U8("graphics/picture_frame/lobby.4bpp.rl");
static const u8 sPictureFrameTilemap_Cool[] =
    INCBIN_U8("graphics/picture_frame/cool_map.bin.rl");
static const u8 sPictureFrameTilemap_Beauty[] =
    INCBIN_U8("graphics/picture_frame/beauty_map.bin.rl");
static const u8 sPictureFrameTilemap_Cute[] =
    INCBIN_U8("graphics/picture_frame/cute_map.bin.rl");
static const u8 sPictureFrameTilemap_Smart[] =
    INCBIN_U8("graphics/picture_frame/smart_map.bin.rl");
static const u8 sPictureFrameTilemap_Tough[] =
    INCBIN_U8("graphics/picture_frame/tough_map.bin.rl");
static const u8 sPictureFrameTilemap_HallLobby[] =
    INCBIN_U8("graphics/picture_frame/lobby_map.bin.rl");
...
static const u8 *const sMuseumCaptions[NUM_PAINTING_CAPTIONS *
    ...
    [0 + NUM_PAINTING_CAPTIONS * CONTEST_CATEGORY_BEAUTY] =
        gContestPaintingBeauty1,
    [1 + NUM_PAINTING_CAPTIONS * CONTEST_CATEGORY_BEAUTY] =
        gContestPaintingBeauty2,
    [2 + NUM_PAINTING_CAPTIONS * CONTEST_CATEGORY_BEAUTY] =
    ...
    [0 + NUM_PAINTING_CAPTIONS * CONTEST_CATEGORY_SMART] =
        gContestPaintingSmart1,
    [1 + NUM_PAINTING_CAPTIONS * CONTEST_CATEGORY_SMART] =
        gContestPaintingSmart2,
    [2 + NUM_PAINTING_CAPTIONS * CONTEST_CATEGORY_SMART] =
        gContestPaintingSmart3,
    [0 + NUM_PAINTING_CAPTIONS * CONTEST_CATEGORY_TOUGH] =
        gContestPaintingTough1,
    [1 + NUM_PAINTING_CAPTIONS * CONTEST_CATEGORY_TOUGH] =
        gContestPaintingTough2,
    [2 + NUM_PAINTING_CAPTIONS * CONTEST_CATEGORY_TOUGH] =
...
void CB2_ContestPainting(void) { ShowContestPainting(); }
...
static void ShowContestPainting(void) {
  ...
  case 2:
    ...
    break;
  case 3:
    CreateContestPaintingPicture(gCurContestWinnerSaveIdx,
                                 gCurContestWinnerIsForArtist);
    gMain.state++;
    break;
  case 4:
    PrintContestPaintingCaption(gCurContestWinnerSaveIdx,
                                gCurContestWinnerIsForArtist);
...
static void InitContestPaintingBg(void) {
  ...
  SetGpuReg(REG_OFFSET_BG0CNT, BGCNT_PRIORITY(2) | BGCNT_CHARBASE(0) |
                                   BGCNT_SCREENBASE(12) | BGCNT_MOSAIC |
                                   BGCNT_16COLOR | BGCNT_TXT256x256);
  SetGpuReg(REG_OFFSET_BG1CNT, BGCNT_PRIORITY(1) | BGCNT_CHARBASE(1) |
                                   BGCNT_SCREENBASE(10) | BGCNT_MOSAIC |
                                   BGCNT_16COLOR | BGCNT_TXT256x256);
...
static void UpdateContestPaintingMosaicEffect(void) {
  if (!sVarsInitialized) {
    ...
    SetGpuReg(REG_OFFSET_MOSAIC, (sMosaicVal << 12) | (sMosaicVal << 8) |
                                     (sMosaicVal << 4) | (sMosaicVal << 0));
  }
}
...
static void InitContestMonPixels(u16 species, bool8 backPic) {
  const void *pal = GetMonSpritePalFromSpeciesAndPersonality(
      species, gContestPaintingWinner->trainerId,
  ...
  if (!backPic) {
    ...
    _InitContestMonPixels(
        ...
        gContestPaintingMonPalette, (void *)gContestMonPixels);
  } else {
    HandleLoadSpecialPokePic_DontHandleDeoxys(
    ...
    for (tileX = 0; tileX < 8; tileX++) {
      for (pixelY = 0; pixelY < 8; pixelY++) {
        for (pixelX = 0; pixelX < 8; pixelX++) {
          colorIndex = spriteGfx[32 * (tileY * 8 + tileX) + (pixelY << 2) +
                                 (pixelX >> 1)];
          if (pixelX & 1)
            colorIndex >>= 4;
          else
          ...
          if (colorIndex == 0) // transparent pixel
            ...
            (*destPixels)[8 * tileY + pixelY][tileX * 8 + pixelX] =
                palette[colorIndex];
        }
      }
    }
...
static u8 GetImageEffectForContestWinner(u8 contestWinnerId) {
  ...
  if (contestWinnerId < MUSEUM_CONTEST_WINNERS_START)
    ...
    contestCategory =
        gContestPaintingWinner->contestCategory / NUM_PAINTING_CAPTIONS;
...
static void DoContestPaintingImageProcessing(u8 imageEffect) {
  ...
  gImageProcessingContext.personality =
      gContestPaintingWinner->personality % 256;
  ...
  default:
    gImageProcessingContext.quantizeEffect =
