...
void AllocateBattleResources(void) {
  ...
  gBattleResources->secretBase =
      AllocZeroed(sizeof(*gBattleResources->secretBase));
  ...
  gBattleResources->AI_ScriptsStack =
      AllocZeroed(sizeof(*gBattleResources->AI_ScriptsStack));
...
void AdjustFriendshipOnBattleFaint(u8 battlerId) {
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE) {
    ...
    if (gBattleMons[opposingBattlerId2].level >
        gBattleMons[opposingBattlerId].level)
  ...
  if (gBattleMons[opposingBattlerId].level > gBattleMons[battlerId].level) {
    if (gBattleMons[opposingBattlerId].level - gBattleMons[battlerId].level >
        29)
      AdjustFriendship(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
  ...
  if (GetBattlerSide(battlerId) != B_SIDE_OPPONENT) {
    ...
    for (i = 0; i < (int)ARRAY_COUNT(gBattlePartyCurrentOrder); i++)
      gBattlePartyCurrentOrder[i] =
          *(0 * 3 + i + (u8 *)(gBattleStruct->battlerPartyOrders));
...
// Check if it was because they are frozen/asleep, and if so try to cure the
// status.
u32 BattlePalace_TryEscapeStatus(u8 battlerId) {
  ...
  do {
    ...
    // Loop until reaching the final state, or stop early if Pokémon was
    // Asleep/Frozen
