...
#define MAX_POCKET_ITEMS                                                       \
  ((max(BAG_TMHM_COUNT,                                                        \
        max(BAG_BERRIES_COUNT,                                                 \
            max(BAG_ITEMS_COUNT,                                               \
                max(BAG_KEYITEMS_COUNT, BAG_POKEBALLS_COUNT))))) +             \
   1)
...
static void MergeSort(struct ItemSlot *array, u32 low, u32 high,
                      s8 (*comparator)(struct ItemSlot *, struct ItemSlot *));
static void Merge(struct ItemSlot *array, u32 low, u32 mid, u32 high,
                  s8 (*comparator)(struct ItemSlot *, struct ItemSlot *));
static s8 CompareItemsAlphabetically(struct ItemSlot *itemSlot1,
                                     struct ItemSlot *itemSlot2);
static s8 CompareItemsByMost(struct ItemSlot *itemSlot1,
                             struct ItemSlot *itemSlot2);
static s8 CompareItemsByType(struct ItemSlot *itemSlot1,
                             struct ItemSlot *itemSlot2);
...
static const struct MenuAction sItemMenuActions[] = {
    ...
    [ACTION_CONFIRM_QUIZ_LADY] = {gMenuText_Confirm,
...
static const u8 sContextMenuItems_ItemsPocket[] = {ACTION_USE, ACTION_GIVE,
                                                   ACTION_TOSS, ACTION_CANCEL};

static const u8 sContextMenuItems_KeyItemsPocket[] = {
    ACTION_USE, ACTION_REGISTER, ACTION_DUMMY, ACTION_CANCEL};

static const u8 sContextMenuItems_BallsPocket[] = {ACTION_GIVE, ACTION_DUMMY,
                                                   ACTION_TOSS, ACTION_CANCEL};

static const u8 sContextMenuItems_TmHmPocket[] = {ACTION_USE, ACTION_GIVE,
                                                  ACTION_DUMMY, ACTION_CANCEL};

static const u8 sContextMenuItems_BerriesPocket[] = {
    ACTION_USE,  ACTION_DUMMY, ACTION_CHECK_TAG,
    ACTION_GIVE, ACTION_TOSS,  ACTION_CANCEL};

static const u8 sContextMenuItems_BattleUse[] = {ACTION_BATTLE_USE,
                                                 ACTION_CANCEL};
...
static const u8 sContextMenuItems_BerryBlenderCrush[] = {
    ACTION_CONFIRM, ACTION_CHECK_TAG, ACTION_DUMMY, ACTION_CANCEL};

static const u8 sContextMenuItems_Apprentice[] = {ACTION_SHOW, ACTION_CANCEL};

static const u8 sContextMenuItems_FavorLady[] = {ACTION_GIVE_FAVOR_LADY,
                                                 ACTION_CANCEL};

static const u8 sContextMenuItems_QuizLady[] = {ACTION_CONFIRM_QUIZ_LADY,
                                                ACTION_CANCEL};
...
static const struct YesNoFuncTable sYesNoTossFunctions = {ConfirmToss,
                                                          CancelToss};

static const struct YesNoFuncTable sYesNoSellItemFunctions = {ConfirmSell,
                                                              CancelSell};
...
static const u8 sRegisteredSelect_Gfx[] =
    INCBIN_U8("graphics/bag/select_button.4bpp");
...
static const u8 sFontColorTable[][3] = {
    ...
    [COLORID_NORMAL] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_WHITE,
                        TEXT_COLOR_LIGHT_GRAY},
    [COLORID_POCKET_NAME] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_WHITE,
                             TEXT_COLOR_RED},
    [COLORID_GRAY_CURSOR] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_LIGHT_GRAY,
                             TEXT_COLOR_GREEN},
    [COLORID_UNUSED] = {TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE,
                        TEXT_COLOR_LIGHT_GRAY},
    [COLORID_TMHM_INFO] = {TEXT_COLOR_TRANSPARENT, TEXT_DYNAMIC_COLOR_5,
...
void CB2_BagMenuFromStartMenu(void) {
  GoToBagMenu(ITEMMENULOCATION_FIELD, POCKETS_COUNT,
              CB2_ReturnToFieldWithOpenMenu);
}

void CB2_BagMenuFromBattle(void) {
  if (!InBattlePyramid())
    GoToBagMenu(ITEMMENULOCATION_BATTLE, POCKETS_COUNT,
                CB2_SetUpReshowBattleScreenAfterMenu2);
  else
    GoToBattlePyramidBagMenu(PYRAMIDBAG_LOC_BATTLE,
                             CB2_SetUpReshowBattleScreenAfterMenu2);
}

// Choosing berry to plant
void CB2_ChooseBerry(void) {
  GoToBagMenu(ITEMMENULOCATION_BERRY_TREE, BERRIES_POCKET,
              CB2_ReturnToFieldContinueScript);
}

// Choosing berry for Berry Blender or Berry Crush
void ChooseBerryForMachine(void (*exitCallback)(void)) {
  GoToBagMenu(ITEMMENULOCATION_BERRY_BLENDER_CRUSH, BERRIES_POCKET,
              exitCallback);
}
...
void ApprenticeOpenBagMenu(void) {
  GoToBagMenu(ITEMMENULOCATION_APPRENTICE, POCKETS_COUNT,
...
void FavorLadyOpenBagMenu(void) {
  GoToBagMenu(ITEMMENULOCATION_FAVOR_LADY, POCKETS_COUNT,
              CB2_FavorLadyExitBagMenu);
  gSpecialVar_Result = FALSE;
}

void QuizLadyOpenBagMenu(void) {
  GoToBagMenu(ITEMMENULOCATION_QUIZ_LADY, POCKETS_COUNT,
...
static void CB2_Bag(void) {
  while (MenuHelpers_ShouldWaitForLinkRecv() != TRUE &&
         SetupBagMenu() != TRUE && MenuHelpers_IsLinkActive() != TRUE) {
  };
}
...
static void BagMenu_InitBGs(void) {
  ...
  InitBgsFromTemplates(0, sBgTemplates_ItemMenu,
                       ARRAY_COUNT(sBgTemplates_ItemMenu));
...
static bool8 LoadBagMenu_Graphics(void) {
  ...
  case 2:
    if (!IsWallysBag() && gSaveBlock2Ptr->playerGender != MALE)
      LoadCompressedPalette(gBagScreenFemale_Pal, BG_PLTT_ID(0),
                            2 * PLTT_SIZE_4BPP);
    else
      LoadCompressedPalette(gBagScreenMale_Pal, BG_PLTT_ID(0),
                            2 * PLTT_SIZE_4BPP);
    gBagMenu->graphicsLoadState++;
    break;
  case 3:
...
static void GetItemName(u8 *dest, u16 itemId) {
  ...
  case TMHM_POCKET:
    ...
    if (itemId >= ITEM_HM01) {
      // Get HM number
      ConvertIntToDecimalStringN(gStringVar1, itemId - ITEM_HM01 + 1,
                                 STR_CONV_MODE_LEADING_ZEROS, 1);
      StringExpandPlaceholders(dest, gText_NumberItem_HM);
    } else {
      // Get TM number
      ConvertIntToDecimalStringN(gStringVar1, itemId - ITEM_TM01 + 1,
                                 STR_CONV_MODE_LEADING_ZEROS, 2);
      StringExpandPlaceholders(dest, gText_NumberItem_TMBerry);
    }
    break;
  case BERRIES_POCKET:
    ConvertIntToDecimalStringN(gStringVar1, itemId - FIRST_BERRY_INDEX + 1,
                               STR_CONV_MODE_LEADING_ZEROS, 2);
  ...
  if (gBagMenu->toSwapPos == NOT_SWAPPING) {
    ...
    if (itemIndex != LIST_CANCEL)
      AddBagItemIconSprite(
          BagGetItemIdByPocketPosition(gBagPosition.pocket + 1, itemIndex),
...
static void BagMenu_ItemPrintCallback(u8 windowId, u32 itemIndex, u8 y) {
  ...
  if (itemIndex != LIST_CANCEL) {
    ...
    itemQuantity =
        BagGetQuantityByPocketPosition(gBagPosition.pocket + 1, itemIndex);
...
static void PrintItemDescription(int itemIndex) {
  ...
  BagMenu_Print(WIN_DESCRIPTION, FONT_NORMAL, str, 3, 1, 0, 0, 0,
                COLORID_NORMAL);
}

static void BagMenu_PrintCursor(u8 listTaskId, u8 colorIndex) {
  BagMenu_PrintCursorAtPos(ListMenuGetYCoordForPrintingArrowCursor(listTaskId),
                           colorIndex);
}
...
static void BagMenu_PrintCursorAtPos(u8 y, u8 colorIndex) {
  if (colorIndex == COLORID_NONE)
    ...
    BagMenu_Print(WIN_ITEM_LIST, FONT_NORMAL, gText_SelectorArrow2, 0, y, 0, 0,
                  0, colorIndex);
}
...
static void CreatePocketScrollArrowPair(void) {
  if (gBagMenu->pocketScrollArrowsTask == TASK_NONE)
    gBagMenu->pocketScrollArrowsTask = AddScrollIndicatorArrowPairParameterized(
        ...
        gBagMenu->numItemStacks[gBagPosition.pocket] -
            gBagMenu->numShownItems[gBagPosition.pocket],
...
static void Task_CloseBagMenu(u8 taskId) {
  ...
  if (!gPaletteFade.active) {
    ...
    // If ready for a new screen (e.g. party menu for giving an item) go to that
    // screen Otherwise exit the bag and use callback set up when the bag was
    // first opened
...
void UpdatePocketListPosition(u8 pocketId) {
  SetCursorWithinListBounds(&gBagPosition.scrollPosition[pocketId],
...
static void InitPocketScrollPositions(void) {
  ...
  for (i = 0; i < POCKETS_COUNT; i++)
    SetCursorScrollWithinListBounds(
        &gBagPosition.scrollPosition[i], &gBagPosition.cursorPosition[i],
        gBagMenu->numShownItems[i], gBagMenu->numItemStacks[i],
        MAX_ITEMS_SHOWN);
}
...
void DisplayItemMessage(u8 taskId, u8 fontId, const u8 *str,
                        void (*callback)(u8 taskId)) {
  s16 *data = gTasks[taskId].data;

  tMsgWindowId = AddItemMessageWindow(ITEMWIN_MESSAGE);
  FillWindowPixelBuffer(tMsgWindowId, PIXEL_FILL(1));
  DisplayMessageAndContinueTask(taskId, tMsgWindowId, 10, 13, fontId,
                                GetPlayerTextSpeedDelay(), str, callback);
...
void CloseItemMessage(u8 taskId) {
  ...
  LoadBagItemListBuffers(gBagPosition.pocket);
  tListTaskId =
      ListMenuInit(&gMultiuseListMenuTemplate, *scrollPos, *cursorPos);
...
static void PrintItemQuantity(u8 windowId, s16 quantity) {
  u8 numDigits = (gBagPosition.pocket == BERRIES_POCKET)
                     ? BERRY_CAPACITY_DIGITS
                     : BAG_ITEM_CAPACITY_DIGITS;
  ConvertIntToDecimalStringN(gStringVar1, quantity, STR_CONV_MODE_LEADING_ZEROS,
                             numDigits);
  StringExpandPlaceholders(gStringVar4, gText_xVar1);
...
static void PrintItemSoldAmount(int windowId, int numSold, int moneyEarned) {
  u8 numDigits = (gBagPosition.pocket == BERRIES_POCKET)
                     ? BERRY_CAPACITY_DIGITS
                     : BAG_ITEM_CAPACITY_DIGITS;
  ConvertIntToDecimalStringN(gStringVar1, numSold, STR_CONV_MODE_LEADING_ZEROS,
                             numDigits);
  StringExpandPlaceholders(gStringVar4, gText_xVar1);
  AddTextPrinterParameterized(windowId, FONT_NORMAL, gStringVar4, 0, 1,
                              TEXT_SKIP_DRAW, 0);
...
static void Task_BagMenu_HandleInput(u8 taskId) {
  ...
  if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active) {
      ...
      if (JOY_NEW(SELECT_BUTTON)) {
        if (CanSwapItems() == TRUE) {
          ...
          if ((*scrollPos + *cursorPos) !=
              gBagMenu->numItemStacks[gBagPosition.pocket] - 1) {
            PlaySE(SE_SELECT);
        ...
        if ((gBagMenu->numItemStacks[gBagPosition.pocket] - 1) <=
            1) // can't sort with 0 or 1 item in bag
        {
          static const u8 sText_NothingToSort[] = _("There's nothing to sort!");
          PlaySE(SE_FAILURE);
          DisplayItemMessage(taskId, 1, sText_NothingToSort,
                             HandleErrorMessage);
          break;
        }
        ...
        data[2] =
            BagGetQuantityByPocketPosition(gBagPosition.pocket + 1, data[1]);
        if (gBagPosition.cursorPosition[gBagPosition.pocket] ==
            gBagMenu->numItemStacks[gBagPosition.pocket] - 1)
          break;
        else
          gSpecialVar_ItemId =
              BagGetItemIdByPocketPosition(gBagPosition.pocket + 1, data[1]);
    ...
    default: // A_BUTTON
      ...
      tQuantity =
          BagGetQuantityByPocketPosition(gBagPosition.pocket + 1, listPosition);
      gSpecialVar_ItemId =
          BagGetItemIdByPocketPosition(gBagPosition.pocket + 1, listPosition);
...
static void SwitchBagPocket(u8 taskId, s16 deltaBagPocketId,
                            bool16 skipEraseList) {
  s16 *data = gTasks[taskId].data;
  u8 newPocket;
  ...
  if (!skipEraseList) {
    ...
    DestroyListMenuTask(tListTaskId,
                        ...
                        &gBagPosition.cursorPosition[gBagPosition.pocket]);
    ScheduleBgCopyTilemapToVram(0);
    gSprites[gBagMenu->spriteIds[ITEMMENUSPRITE_ITEM +
                                 (gBagMenu->itemIconSlot ^ 1)]]
        .invisible = TRUE;
    BagDestroyPocketScrollArrowPair();
  }
  ...
  if (deltaBagPocketId == MENU_CURSOR_DELTA_RIGHT) {
    PrintPocketNames(gPocketNamesStringsTable[gBagPosition.pocket],
                     gPocketNamesStringsTable[newPocket]);
    CopyPocketNameToWindow(0);
  } else {
    PrintPocketNames(gPocketNamesStringsTable[newPocket],
  ...
  SetTaskFuncWithFollowupFunc(taskId, Task_SwitchBagPocket,
                              gTasks[taskId].func);
}
...
static void StartItemSwap(u8 taskId) {
  ...
  tListPosition = gBagPosition.scrollPosition[gBagPosition.pocket] +
                  gBagPosition.cursorPosition[gBagPosition.pocket];
  gBagMenu->toSwapPos = tListPosition;
  CopyItemName(
      BagGetItemIdByPocketPosition(gBagPosition.pocket + 1, tListPosition),
      gStringVar1);
  StringExpandPlaceholders(gStringVar4, gText_MoveVar1Where);
  FillWindowPixelBuffer(WIN_DESCRIPTION, PIXEL_FILL(0));
  BagMenu_Print(WIN_DESCRIPTION, FONT_NORMAL, gStringVar4, 3, 1, 0, 0, 0,
                COLORID_NORMAL);
  ...
  if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE) {
    if (JOY_NEW(SELECT_BUTTON)) {
      ...
      ListMenuGetScrollAndRow(
          tListTaskId, &gBagPosition.scrollPosition[gBagPosition.pocket],
          &gBagPosition.cursorPosition[gBagPosition.pocket]);
      DoItemSwap(taskId);
    } else {
...
static void DoItemSwap(u8 taskId) {
  ...
  if (tListPosition == realPos || tListPosition == realPos - 1) {
    ...
    MoveItemSlotInList(gBagPockets[gBagPosition.pocket].itemSlots,
                       tListPosition, realPos);
    gBagMenu->toSwapPos = NOT_SWAPPING;
    DestroyListMenuTask(tListTaskId, scrollPos, cursorPos);
...
static void OpenContextMenu(u8 taskId) {
  ...
  case ITEMMENULOCATION_WALLY:
    if (ItemId_GetBattleUsage(gSpecialVar_ItemId) &&
        (!FlagGet(FLAG_HARD) || gBagPosition.pocket == BALLS_POCKET ||
         gBagPosition.location != ITEMMENULOCATION_BATTLE)) {
  ...
  case ITEMMENULOCATION_BERRY_BLENDER_CRUSH:
    ...
    gBagMenu->contextMenuNumItems =
        ARRAY_COUNT(sContextMenuItems_BerryBlenderCrush);
    break;
  ...
  default:
    if (MenuHelpers_IsLinkActive() == TRUE || InUnionRoom() == TRUE) {
      if (gBagPosition.pocket == KEYITEMS_POCKET ||
          !IsHoldingItemAllowed(gSpecialVar_ItemId)) {
        gBagMenu->contextMenuItemsPtr = sContextMenuItems_Cancel;
        gBagMenu->contextMenuNumItems = ARRAY_COUNT(sContextMenuItems_Cancel);
      } else {
      ...
      case ITEMS_POCKET:
        ...
        gBagMenu->contextMenuNumItems =
            ARRAY_COUNT(sContextMenuItems_ItemsPocket);
      ...
      case KEYITEMS_POCKET:
        ...
        if (ItemId_GetFieldFunc(gSpecialVar_ItemId) ==
          ...
          gBagMenu->contextMenuNumItems = ARRAY_COUNT(sContextMenuItems_Cancel);
          memcpy(&gBagMenu->contextMenuItemsBuffer, &sContextMenuItems_Cancel,
                 sizeof(sContextMenuItems_Cancel));
        } else {
          gBagMenu->contextMenuNumItems =
      ...
      case BALLS_POCKET:
        ...
        gBagMenu->contextMenuNumItems =
            ARRAY_COUNT(sContextMenuItems_BallsPocket);
        break;
      case TMHM_POCKET:
        gBagMenu->contextMenuItemsPtr = sContextMenuItems_TmHmPocket;
        gBagMenu->contextMenuNumItems =
            ARRAY_COUNT(sContextMenuItems_TmHmPocket);
        break;
      case BERRIES_POCKET:
...
static void PrintContextMenuItems(u8 windowId) {
  PrintMenuActionTexts(windowId, FONT_NARROW, 8, 1, 0, 16,
                       gBagMenu->contextMenuNumItems, sItemMenuActions,
...
static void Task_ItemContext_SingleRow(u8 taskId) {
  if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE) {
    ...
    default:
      PlaySE(SE_SELECT);
      sItemMenuActions[gBagMenu->contextMenuItemsPtr[selection]].func.void_u8(
          taskId);
      break;
    }
  }
    ...
    if (JOY_NEW(DPAD_UP)) {
      if (cursorPos > 0 && IsValidContextMenuPos(cursorPos - 2)) {
        ...
        ChangeMenuGridCursorPosition(MENU_CURSOR_DELTA_NONE,
                                     MENU_CURSOR_DELTA_UP);
      }
    } else if (JOY_NEW(DPAD_DOWN)) {
      if (cursorPos < (gBagMenu->contextMenuNumItems - 2) &&
          IsValidContextMenuPos(cursorPos + 2)) {
        PlaySE(SE_SELECT);
        ChangeMenuGridCursorPosition(MENU_CURSOR_DELTA_NONE,
      ...
      PlaySE(SE_SELECT);
      sItemMenuActions[gBagMenu->contextMenuItemsPtr[cursorPos]].func.void_u8(
          taskId);
...
static void ItemMenu_UseOutOfBattle(u8 taskId) {
  if (ItemId_GetFieldFunc(gSpecialVar_ItemId)) {
    ...
    if (CalculatePlayerPartyCount() == 0 &&
        ItemId_GetType(gSpecialVar_ItemId) == ITEM_USE_PARTY_MENU) {
...
static void AskTossItems(u8 taskId) {
  ...
  CopyItemName(gSpecialVar_ItemId, gStringVar1);
  ConvertIntToDecimalStringN(gStringVar2, tItemCount, STR_CONV_MODE_LEFT_ALIGN,
                             MAX_ITEM_DIGITS);
...
static void ItemMenu_Give(u8 taskId) {
  ...
  if (!IsWritingMailAllowed(gSpecialVar_ItemId)) {
    DisplayItemMessage(taskId, FONT_NORMAL, gText_CantWriteMail,
                       HandleErrorMessage);
...
bool8 UseRegisteredKeyItemOnField(void) {
  ...
  if (gSaveBlock1Ptr->registeredItem != ITEM_NONE) {
    if (CheckBagHasItem(gSaveBlock1Ptr->registeredItem, 1) == TRUE) {
      ...
      taskId =
          CreateTask(ItemId_GetFieldFunc(gSaveBlock1Ptr->registeredItem), 8);
...
static void DisplaySellItemPriceAndConfirm(u8 taskId) {
  ...
  ConvertIntToDecimalStringN(
      gStringVar1, (ItemId_GetPrice(gSpecialVar_ItemId) / 2) * tItemCount,
...
static void InitSellHowManyInput(u8 taskId) {
  ...
  PrintItemSoldAmount(windowId, 1,
...
static void Task_ChooseHowManyToSell(u8 taskId) {
  ...
  if (AdjustQuantityAccordingToDPadInput(&tItemCount, tQuantity) == TRUE) {
    PrintItemSoldAmount(gBagMenu->windowIds[ITEMWIN_QUANTITY_WIDE], tItemCount,
                        (ItemId_GetPrice(gSpecialVar_ItemId) / 2) * tItemCount);
  } else if (JOY_NEW(A_BUTTON)) {
    PlaySE(SE_SELECT);
...
static void SellItem(u8 taskId) {
  ...
  tListTaskId =
      ListMenuInit(&gMultiuseListMenuTemplate, *scrollPos, *cursorPos);
  BagMenu_PrintCursor(tListTaskId, COLORID_GRAY_CURSOR);
  PrintMoneyAmountInMoneyBox(gBagMenu->windowIds[ITEMWIN_MONEY],
                             GetMoney(&gSaveBlock1Ptr->money), 0);
...
static void TryDepositItem(u8 taskId) {
  ...
  if (ItemId_GetImportance(gSpecialVar_ItemId)) {
    ...
    BagMenu_Print(WIN_DESCRIPTION, FONT_NORMAL, gText_CantStoreImportantItems,
                  3, 1, 0, 0, 0, COLORID_NORMAL);
    ...
    // No room to deposit
    BagMenu_Print(WIN_DESCRIPTION, FONT_NORMAL, gText_NoRoomForItems, 3, 1, 0,
                  0, 0, COLORID_NORMAL);
    gTasks[taskId].func = WaitDepositErrorMessage;
  }
}
...
static void PrepareBagForWallyTutorial(void) {
  ...
  memcpy(sTempWallyBag->bagPocket_Items, gSaveBlock1Ptr->bagPocket_Items,
...
static void RestoreBagAfterWallyTutorial(void) {
  ...
  memcpy(gSaveBlock1Ptr->bagPocket_Items, sTempWallyBag->bagPocket_Items,
...
void DoWallyTutorialBagMenu(void) {
  ...
  GoToBagMenu(ITEMMENULOCATION_WALLY, ITEMS_POCKET,
              CB2_SetUpReshowBattleScreenAfterMenu2);
}

#define tTimer data[8]
#define WALLY_BAG_DELAY                                                        \
  102 // The number of frames between each action Wally takes in the bag
...
static void PrintPocketNames(const u8 *pocketName1, const u8 *pocketName2) {
  ...
  BagMenu_Print(windowId, FONT_NORMAL, pocketName1, offset, 1, 0, 0,
                TEXT_SKIP_DRAW, COLORID_POCKET_NAME);
  if (pocketName2) {
    offset = GetStringCenterAlignXOffset(FONT_NORMAL, pocketName2, 0x40);
    BagMenu_Print(windowId, FONT_NORMAL, pocketName2, offset + 0x40, 1, 0, 0,
                  TEXT_SKIP_DRAW, COLORID_POCKET_NAME);
  }
  CpuCopy32((u8 *)GetWindowAttribute(windowId, WINDOW_TILE_DATA),
            gBagMenu->pocketNameBuffer, sizeof(gBagMenu->pocketNameBuffer));
...
static void CopyPocketNameToWindow(u32 a) {
  ...
  CpuCopy32(&tileDataBuffer[0][a], windowTileData,
            0x100); // Top half of pocket name
  b = a + 16;
  CpuCopy32(&tileDataBuffer[0][b], windowTileData + 0x100,
            0x100); // Bottom half of pocket name
...
static void BagMenu_Print(u8 windowId, u8 fontId, const u8 *str, u8 left,
                          u8 top, u8 letterSpacing, u8 lineSpacing, u8 speed,
                          u8 colorIndex) {
  AddTextPrinterParameterized4(windowId, fontId, left, top, letterSpacing,
                               lineSpacing, sFontColorTable[colorIndex], speed,
                               str);
}
...
static void RemoveItemMessageWindow(u8 windowType) {
  ...
  if (*windowId != WINDOW_NONE) {
    ...
    // This ClearWindowTilemap call is redundant, since
    // ClearDialogWindowAndFrameToTransparent already calls it.
    ClearWindowTilemap(*windowId);
    RemoveWindow(*windowId);
    ScheduleBgCopyTilemapToVram(1);
    *windowId = WINDOW_NONE;
...
void BagMenu_YesNo(u8 taskId, u8 windowType,
                   const struct YesNoFuncTable *funcTable) {
  CreateYesNoMenuWithCallbacks(taskId, &sContextMenuWindowTemplates[windowType],
                               1, 0, 2, 1, 14, funcTable);
}

static void DisplayCurrentMoneyWindow(void) {
  u8 windowId = BagMenu_AddWindow(ITEMWIN_MONEY);
  PrintMoneyAmountInMoneyBoxWithBorder(windowId, 1, 14,
...
static void PrintTMHMMoveData(u16 itemId) {
  ...
  if (itemId == ITEM_NONE) {
    for (i = 0; i < 4; i++)
      BagMenu_Print(WIN_TMHM_INFO, FONT_NORMAL, gText_ThreeDashes, 7, i * 12, 0,
                    0, TEXT_SKIP_DRAW, COLORID_TMHM_INFO);
    ...
    if (gBattleMoves[moveId].power <= 1) {
      ...
      ConvertIntToDecimalStringN(gStringVar1, gBattleMoves[moveId].power,
                                 STR_CONV_MODE_RIGHT_ALIGN, 3);
      text = gStringVar1;
    }
    BagMenu_Print(WIN_TMHM_INFO, FONT_NORMAL, text, 7, 12, 0, 0, TEXT_SKIP_DRAW,
                  COLORID_TMHM_INFO);
    ...
    if (gBattleMoves[moveId].accuracy == 0) {
      ...
      ConvertIntToDecimalStringN(gStringVar1, gBattleMoves[moveId].accuracy,
                                 STR_CONV_MODE_RIGHT_ALIGN, 3);
      text = gStringVar1;
    }
    BagMenu_Print(WIN_TMHM_INFO, FONT_NORMAL, text, 7, 24, 0, 0, TEXT_SKIP_DRAW,
                  COLORID_TMHM_INFO);

    // Print TMHM pp
    ConvertIntToDecimalStringN(gStringVar1, gBattleMoves[moveId].pp,
                               STR_CONV_MODE_RIGHT_ALIGN, 3);
    BagMenu_Print(WIN_TMHM_INFO, FONT_NORMAL, gStringVar1, 7, 36, 0, 0,
                  TEXT_SKIP_DRAW, COLORID_TMHM_INFO);
...
static void AddBagSortSubMenu(void) {
  ...
  case POCKET_KEY_ITEMS:
    ...
    memcpy(&gBagMenu->contextMenuItemsBuffer, &sBagMenuSortKeyItems,
  ...
  case POCKET_TM_HM:
    ...
    memcpy(&gBagMenu->contextMenuItemsBuffer, &sBagMenuSortPokeBallsBerries,
           NELEMS(sBagMenuSortPokeBallsBerries));
    gBagMenu->contextMenuNumItems = NELEMS(sBagMenuSortPokeBallsBerries);
    break;
  default:
    gBagMenu->contextMenuItemsPtr = sBagMenuSortItems;
    memcpy(&gBagMenu->contextMenuItemsBuffer, &sBagMenuSortItems,
           NELEMS(sBagMenuSortItems));
...
static s8 CompareItemsByMost(struct ItemSlot *itemSlot1,
  ...
  return CompareItemsAlphabetically(
      itemSlot1, itemSlot2); // Items have same quantity so sort alphabetically
}
...
static s8 CompareItemsByType(struct ItemSlot *itemSlot1,
  ...
  // Null items go last
  u8 type1 =
      (itemSlot1->itemId == ITEM_NONE) ? 0xFF : sItemsByType[itemSlot1->itemId];
  u8 type2 =
      (itemSlot2->itemId == ITEM_NONE) ? 0xFF : sItemsByType[itemSlot2->itemId];
