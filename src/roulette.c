...
// "Board" is used in this file to refer to both the wheel and the bet selection
// grid
...
#define GET_ROW(selectionId)                                                   \
  ((selectionId) / (NUM_BOARD_POKES + 1) * (NUM_BOARD_POKES + 1))
...
// Flags for the above selections, used to set which spaces have been hit or bet
// on
...
// 2 different Roulette tables with 2 different rates (normal vs service day
// special) & 1 gets which table, >> 7 gets if ROULETTE_SPECIAL_RATE is set
#define GET_MIN_BET_ID(var) (((var) & 1) + (((var) >> 7) * 2))

// Having Shroomish or Taillow in the party can make rolls more consistent in
// length It also increases the likelihood that, if they appear to unstick a
// ball, they'll move it to a slot the player bet on
...
// Roulette uses a large amount of sprites, and stores ids for these in a single
// array Many are looped over rather than referenced directly
enum {
  SPR_WHEEL_BALL_1,
  SPR_WHEEL_BALL_2,
  SPR_WHEEL_BALL_3,
...
static EWRAM_DATA struct Roulette {
  ...
  u8 minBet;
  u8 curBallNum : 4; // Never actually gets incremented, tracked with tBallNum
                     // instead
  u8 unk1 : 4;       // Never read
  u8 betSelection[BALLS_PER_ROUND]; // Because curBallNum is used as the only
                                    // index, only the first element is ever
                                    // used (prev bet selections are never
                                    // needed)
  u8 wheelDelayTimer;
  u8 wheelSpeed;
  u8 wheelDelay;
...
static const u32 sWheel_Tilemap[] =
    INCBIN_U32("graphics/roulette/wheel.bin.lz");
        ...
        [COL_WYNAUT] =
            {
                ...
                .tilemapOffset = 0,
                .flag = F_WYNAUT_COL,
                .inSelectionFlags = F_WYNAUT_COL | F_ORANGE_WYNAUT |
                                    F_GREEN_WYNAUT | F_PURPLE_WYNAUT,
        ...
        [COL_AZURILL] =
            {
                ...
                .tilemapOffset = 0,
                .flag = F_AZURILL_COL,
                .inSelectionFlags = F_AZURILL_COL | F_ORANGE_AZURILL |
                                    F_GREEN_AZURILL | F_PURPLE_AZURILL,
                .flashFlags = F_FLASH_COLUMN,
            },
        [COL_SKITTY] =
            {
                ...
                .tilemapOffset = 0,
                .flag = F_SKITTY_COL,
                .inSelectionFlags = F_SKITTY_COL | F_ORANGE_SKITTY |
                                    F_GREEN_SKITTY | F_PURPLE_SKITTY,
                .flashFlags = F_FLASH_COLUMN,
            },
        [COL_MAKUHITA] =
            {
                ...
                .tilemapOffset = 0,
                .flag = F_MAKUHITA_COL,
                .inSelectionFlags = F_MAKUHITA_COL | F_ORANGE_MAKUHITA |
                                    F_GREEN_MAKUHITA | F_PURPLE_MAKUHITA,
                .flashFlags = F_FLASH_COLUMN,
            },
        [ROW_ORANGE] =
            {
                ...
                .tilemapOffset = 12,
                .flag = F_ORANGE_ROW,
                .inSelectionFlags = F_ORANGE_ROW | F_ORANGE_WYNAUT |
                                    F_ORANGE_AZURILL | F_ORANGE_SKITTY |
                                    F_ORANGE_MAKUHITA,
                .flashFlags = F_FLASH_COLOR_O_WYNAUT | F_FLASH_COLOR_O_AZURILL |
                              F_FLASH_COLOR_O_SKITTY | F_FLASH_COLOR_O_MAKUHITA,
            },
        [SQU_ORANGE_WYNAUT] =
            {
                .spriteIdOffset = 0,
        ...
        [ROW_GREEN] =
            {
                ...
                .tilemapOffset = 15,
                .flag = F_GREEN_ROW,
                .inSelectionFlags = F_GREEN_ROW | F_GREEN_WYNAUT |
                                    F_GREEN_AZURILL | F_GREEN_SKITTY |
                                    F_GREEN_MAKUHITA,
                .flashFlags = F_FLASH_COLOR_G_WYNAUT | F_FLASH_COLOR_G_AZURILL |
                              F_FLASH_COLOR_G_SKITTY | F_FLASH_COLOR_G_MAKUHITA,
            },
        [SQU_GREEN_WYNAUT] =
            {
                .spriteIdOffset = 4,
        ...
        [ROW_PURPLE] =
            {
                ...
                .tilemapOffset = 18,
                .flag = F_PURPLE_ROW,
                .inSelectionFlags = F_PURPLE_ROW | F_PURPLE_WYNAUT |
                                    F_PURPLE_AZURILL | F_PURPLE_SKITTY |
                                    F_PURPLE_MAKUHITA,
...
static const struct RouletteFlashSettings
    sFlashData_Colors[NUM_ROULETTE_SLOTS + 1] = {
        {
...
// Data to flash any Pokémon icon (F_FLASH_ICON) on the roulette wheel. One
// entry for each color row Each poke icon flashes with the tint of the row
// color it belongs to, so the Pokémon itself is irrelevant
static const struct RouletteFlashSettings
    sFlashData_PokeIcons[NUM_BOARD_COLORS] = {
...
static const struct YesNoFuncTable sYesNoTable_AcceptMinBet = {
    Task_AcceptMinBet, Task_DeclineMinBet};

static const struct YesNoFuncTable sYesNoTable_KeepPlaying = {
    Task_ContinuePlaying, Task_StopPlaying};
...
static void VBlankCB_Roulette(void) {
  ...
  if (sRoulette->updateGridHighlight) {
    DmaCopy16(3, &sRoulette->tilemapBuffers[2][0xE0],
              (void *)BG_SCREEN_ADDR(4) + 0x1C0, 0x340);
  ...
  case SELECT_STATE_DRAW:
    ...
    ShowBg(0);
    DmaCopy16(3, &sRoulette->tilemapBuffers[0][0xE0],
              (void *)BG_SCREEN_ADDR(31) + 0x1C0, 0x340);
...
static void InitRouletteTableData(void) {
  u8 i;
  u16 bgColors[3] = {RGB(24, 4, 10), RGB(10, 19, 6),
                     RGB(24, 4, 10)}; // 3rd is never used, same as 1st
  ...
  sRoulette->minBet =
      sTableMinBets[sRoulette->tableId + sRoulette->isSpecialRate * 2];
  sRoulette->unk1 = 1;

  // Left table (with min bet of 1) has red background, other table has green
  if (sRoulette->minBet == 1)
    gPlttBufferUnfaded[BG_PLTT_ID(0)] = gPlttBufferUnfaded[BG_PLTT_ID(5) + 1] =
        gPlttBufferFaded[BG_PLTT_ID(0)] = gPlttBufferFaded[BG_PLTT_ID(5) + 1] =
            bgColors[0];
  else
    gPlttBufferUnfaded[BG_PLTT_ID(0)] = gPlttBufferUnfaded[BG_PLTT_ID(5) + 1] =
        gPlttBufferFaded[BG_PLTT_ID(0)] = gPlttBufferFaded[BG_PLTT_ID(5) + 1] =
...
#define tTotalBallNum                                                          \
  data[8] // Same as tBallNum but isn't cleared every 6 balls
...
static void CB2_LoadRoulette(void) {
  ...
  case 1:
    ...
    SetGpuReg(REG_OFFSET_BLDCNT,
              BLDCNT_EFFECT_NONE | BLDCNT_TGT2_BG2 | BLDCNT_TGT2_BD);
  ...
  case 6:
    ...
    AddTextPrinterParameterized(sTextWindowId, FONT_NORMAL,
                                Roulette_Text_ControlsInstruction, 0, 1,
                                TEXT_SKIP_DRAW, NULL);
...
static void Task_StartPlaying(u8 taskId) {
  if (UpdatePaletteFade() == 0) {
    ...
    DrawGridBackground(SELECTION_NONE);
    SetBallCounterNumLeft(BALLS_PER_ROUND);
    StartTaskAfterDelayOrInput(taskId, Task_ContinuePlaying, NO_DELAY,
                               A_BUTTON | B_BUTTON);
  }
}
...
static void Task_AskKeepPlaying(u8 taskId) {
  ...
  DrawStdWindowFrame(sTextWindowId, FALSE);
  AddTextPrinterParameterized(sTextWindowId, FONT_NORMAL,
                              Roulette_Text_KeepPlaying, 0, 1, TEXT_SKIP_DRAW,
                              0);
  CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
...
static void UpdateGridSelectionRect(u8 selectionId) {
  ...
  case COL_MAKUHITA:
    ...
    FillTilemapRect(&sRoulette->tilemapBuffers[0][0], 0, 14, 7, 16, 13);
    SetTilemapRect(&sRoulette->tilemapBuffers[0][0],
                   &sRoulette->gridTilemap[281], temp0, 7, 3, 13);
    break;
  case ROW_ORANGE:
  case ROW_GREEN:
  case ROW_PURPLE:
    ...
    FillTilemapRect(&sRoulette->tilemapBuffers[0][0], 0, 14, 7, 16, 13);
    SetTilemapRect(&sRoulette->tilemapBuffers[0][0],
                   &sRoulette->gridTilemap[320], 14, temp1, 16, 3);
    break;
  // Individual square
  default:
    ...
    FillTilemapRect(&sRoulette->tilemapBuffers[0][0], 0, 14, 7, 16, 13);
    SetTilemapRect(&sRoulette->tilemapBuffers[0][0],
                   &sRoulette->gridTilemap[272], temp0, temp1, 3, 3);
    break;
  }
}
...
static void ProcessBetGridInput(u8 taskId) {
  ...
  if ((!(JOY_NEW(DPAD_UP)) ||
       ((dirPressed = TRUE) &&
        CanMoveSelectionInDir(&gTasks[taskId].tSelectionId, 0))) &&
      (!(JOY_NEW(DPAD_DOWN)) ||
       ((dirPressed = TRUE) &&
        CanMoveSelectionInDir(&gTasks[taskId].tSelectionId, 1))) &&
      (!(JOY_NEW(DPAD_LEFT)) ||
       ((dirPressed = TRUE) &&
        CanMoveSelectionInDir(&gTasks[taskId].tSelectionId, 2))) &&
      (!(JOY_NEW(DPAD_RIGHT)) ||
       ((dirPressed = TRUE) &&
        CanMoveSelectionInDir(&gTasks[taskId].tSelectionId, 3))) &&
      (dirPressed)) {
    u8 i;
    DrawGridBackground(gTasks[taskId].tSelectionId);
    ...
    // If the current selection is a column with at least 1 unhit space, fill in
    // the header
    if ((u16)(gTasks[taskId].tSelectionId - 1) < COL_MAKUHITA &&
        !(sRoulette->hitFlags &
          sGridSelections[gTasks[taskId].tSelectionId].flag)) {
      headerOffset = gTasks[taskId].tSelectionId - 1;
      gSprites[sRoulette->spriteIds[headerOffset + SPR_POKE_HEADERS]]
          .oam.tileNum =
          gSprites[sRoulette->spriteIds[headerOffset + SPR_POKE_HEADERS]]
              .sheetTileStart +
          (*gSprites[sRoulette->spriteIds[headerOffset + SPR_POKE_HEADERS]]
...
static void Task_PlaceBet(u8 taskId) {
  ...
  gTasks[taskId].tMultiplier =
      GetMultiplier(sRoulette->betSelection[sRoulette->curBallNum]);
...
static void Task_SlideGridOffscreen(u8 taskId) {
  if (gTasks[taskId].data[1]-- > 0) {
    ...
    if ((sRoulette->gridX += 4) == 104)
      gSprites[sRoulette->spriteIds[SPR_MULTIPLIER]].callback =
          &SpriteCallbackDummy;
  } else {
...
// Each table has a set base distance used to determine how far the ball will
// travel Each roll a random value is generated to add onto this distance Half
// the value returned by this function is the max distance that can be added on
// per roll i.e. the lower this value is, the closer the roll will be to a
// consistent distance Odds of a lower value increase as play continues, if the
// player has Shroomish and/or Taillow in the party, and dependent on the time
...
static void Task_InitBallRoll(u8 taskId) {
  ...
  randTravelMod =
      GetRandomForBallTravelDistance(gTasks[taskId].tTotalBallNum, rand);
  ...
  sRoulette->ballAngleSpeed =
      S16TOPOSFLOAT(sRouletteTables[sRoulette->tableId].ballSpeed);
  sRoulette->ballAngleAccel =
      ((sRoulette->ballAngleSpeed * 0.5f) - sRoulette->ballAngleSpeed) /
  ...
  sRoulette->ballFallSpeed =
      -(8.0f / S16TOPOSFLOAT(sRoulette->ballTravelDistFast));
...
static void Task_RecordBallHit(u8 taskId) {
  ...
  if (sRoulette->ballState != BALL_STATE_ROLLING) {
    ...
    if (sRoulette->ballStuck) {
      ...
      if (gTasks[taskId].data[1] == 0) {
        bool8 won =
            IsHitInBetSelection(RecordHit(taskId, sRoulette->hitSlot),
...
static void Task_TryIncrementWins(u8 taskId) {
  ...
  case 2: // never happens
    if (IsFanfareTaskInactive()) {
      ...
      StartTaskAfterDelayOrInput(taskId, Task_PrintPayout, NO_DELAY,
                                 A_BUTTON | B_BUTTON);
    }
    break;
  case FALSE:
  default:
    if (!IsSEPlaying()) {
      ...
      StartTaskAfterDelayOrInput(taskId, Task_EndTurn, NO_DELAY,
                                 A_BUTTON | B_BUTTON);
    }
    break;
  }
...
static void Task_PrintSpinResult(u8 taskId) {
  ...
  case 2: // never happens
    if (gTasks[taskId].tMultiplier == MAX_MULTIPLIER) {
      ...
      DrawStdWindowFrame(sTextWindowId, FALSE);
      AddTextPrinterParameterized(sTextWindowId, FONT_NORMAL,
                                  Roulette_Text_Jackpot, 0, 1, TEXT_SKIP_DRAW,
                                  NULL);
      CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
    } else {
      ...
      DrawStdWindowFrame(sTextWindowId, FALSE);
      AddTextPrinterParameterized(sTextWindowId, FONT_NORMAL,
                                  Roulette_Text_ItsAHit, 0, 1, TEXT_SKIP_DRAW,
                                  NULL);
      CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
    }
    break;
  ...
  default:
    ...
    DrawStdWindowFrame(sTextWindowId, FALSE);
    AddTextPrinterParameterized(sTextWindowId, FONT_NORMAL,
                                Roulette_Text_NothingDoing, 0, 1,
                                TEXT_SKIP_DRAW, NULL);
    CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
    break;
  }
...
static void Task_PrintPayout(u8 taskId) {
  ConvertIntToDecimalStringN(gStringVar1,
                             (sRoulette->minBet * gTasks[taskId].tMultiplier),
                             STR_CONV_MODE_LEFT_ALIGN, 2);
  StringExpandPlaceholders(gStringVar4, Roulette_Text_YouveWonXCoins);
  DrawStdWindowFrame(sTextWindowId, FALSE);
  AddTextPrinterParameterized(sTextWindowId, FONT_NORMAL, gStringVar4, 0, 1,
                              TEXT_SKIP_DRAW, NULL);
  CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
...
static void Task_EndTurn(u8 taskId) {
  ...
  gSprites[sRoulette->spriteIds[SPR_WHEEL_ICONS +
                                sGridSelections[gTasks[taskId].tWinningSquare]
                                    .spriteIdOffset]]
      .invisible = TRUE;
...
static void Task_TryPrintEndTurnMsg(u8 taskId) {
  ...
  if (gTasks[taskId].tCoins >= sRoulette->minBet) {
    if (gTasks[taskId].tBallNum == BALLS_PER_ROUND) {
      ...
      DrawStdWindowFrame(sTextWindowId, FALSE);
      AddTextPrinterParameterized(sTextWindowId, FONT_NORMAL,
                                  Roulette_Text_BoardWillBeCleared, 0, 1,
                                  TEXT_SKIP_DRAW, NULL);
      CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
      StartTaskAfterDelayOrInput(taskId, Task_ClearBoard, NO_DELAY,
                                 A_BUTTON | B_BUTTON);
    } else if (gTasks[taskId].tCoins == MAX_COINS) {
      // Player maxed out coins
      DrawStdWindowFrame(sTextWindowId, FALSE);
      AddTextPrinterParameterized(sTextWindowId, FONT_NORMAL,
                                  Roulette_Text_CoinCaseIsFull, 0, 1,
                                  TEXT_SKIP_DRAW, NULL);
      CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
      StartTaskAfterDelayOrInput(taskId, Task_AskKeepPlaying, NO_DELAY,
                                 A_BUTTON | B_BUTTON);
    } else {
    ...
    DrawStdWindowFrame(sTextWindowId, FALSE);
    AddTextPrinterParameterized(sTextWindowId, FONT_NORMAL,
                                Roulette_Text_NoCoinsLeft, 0, 1, TEXT_SKIP_DRAW,
                                NULL);
    CopyWindowToVram(sTextWindowId, COPYWIN_FULL);
    StartTaskAfterDelayOrInput(taskId, Task_StopPlaying, 60,
                               A_BUTTON | B_BUTTON);
  }
}
...
static void StartTaskAfterDelayOrInput(u8 taskId, TaskFunc task, u16 delay,
                                       u16 key) {
...
static u8 RecordHit(u8 taskId, u8 slotId) {
  ...
  u32 columnFlags[NUM_BOARD_POKES] = {
      ...
      F_MAKUHITA_COL | F_ORANGE_MAKUHITA | F_GREEN_MAKUHITA |
          F_PURPLE_MAKUHITA};
  u32 rowFlags[NUM_BOARD_COLORS] = {
      F_ORANGE_ROW | F_ORANGE_WYNAUT | F_ORANGE_AZURILL | F_ORANGE_SKITTY |
          F_ORANGE_MAKUHITA,
      F_GREEN_ROW | F_GREEN_WYNAUT | F_GREEN_AZURILL | F_GREEN_SKITTY |
          F_GREEN_MAKUHITA,
      F_PURPLE_ROW | F_PURPLE_WYNAUT | F_PURPLE_AZURILL | F_PURPLE_SKITTY |
          F_PURPLE_MAKUHITA};

  if (slotId >= NUM_ROULETTE_SLOTS)
    return 0;

  sRoulette->hitSquares[gTasks[taskId].tBallNum - 1] =
...
static void FlashSelectionOnWheel(u8 selectionId) {
  ...
  default: {
    ...
    // The specific color of the poke it references doesn't matter, because the
    // icons of a poke share a palette So it just uses the first sprite ID of
    // each
    case COL_WYNAUT:
      palOffset =
          PLTT_ID(gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_ORANGE_WYNAUT]]
                      .oam.paletteNum);
      break;
    case COL_AZURILL:
      palOffset =
          PLTT_ID(gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_GREEN_AZURILL]]
                      .oam.paletteNum);
      break;
    case COL_SKITTY:
      palOffset =
          PLTT_ID(gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_PURPLE_SKITTY]]
                      .oam.paletteNum);
      break;
    case COL_MAKUHITA:
      palOffset =
          PLTT_ID(gSprites[sRoulette->spriteIds[SPR_WHEEL_ICON_ORANGE_MAKUHITA]]
                      .oam.paletteNum);
      break;
    }
    if (numSelected == 1) {
      ...
      // Selection is full column, add entry in flash util for each unhit
      // space's icon If there is only 1 unhit space, also add its flags so its
      // color will flash as well
      for (i = 0; i < NUM_BOARD_COLORS; i++) {
      ...
      // If there was more than 1 space in the column, reset flags; only icons
      // will flash
      if (numSelected != 2)
        flashFlags = 0;
    }
    // Do flash
    RouletteFlash_Enable(&sRoulette->flashUtil,
...
static void DrawGridBackground(u8 selectionId) {
  ...
  SetTilemapRect(sRoulette->tilemapBuffers[2], sRoulette->gridTilemap, 14, 7,
                 16, 13);
  ...
  case COL_MAKUHITA:
    numSquares =
        NUM_BOARD_COLORS + 1; // For each poke column, 3 colors and a header
  ...
  case ROW_PURPLE:
    numSquares =
        NUM_BOARD_POKES + 1; // For each color row, 4 pokes and a header
    for (i = 0; i < numSquares; i++)
  ...
  for (i = 0; i < numSquares; i++) {
    ...
    // Grid square highlight is drawn in 9 segments, starting from the top left
    // of the square Each iteration of this loop draws 3 segments to form a
...
static const u16 sBallCounter_Pal[] =
    INCBIN_U16("graphics/roulette/ball_counter.gbapal");
...
static const u16 sShroomish_Pal[] =
    INCBIN_U16("graphics/roulette/shroomish.gbapal");
static const u16 sTaillow_Pal[] = INCBIN_U16("graphics/roulette/tailow.gbapal");
static const u16 sGridIcons_Pal[] =
    INCBIN_U16("graphics/roulette/grid_icons.gbapal");
static const u16 sWynaut_Pal[] = INCBIN_U16("graphics/roulette/wynaut.gbapal");
static const u16 sAzurill_Pal[] =
    INCBIN_U16("graphics/roulette/azurill.gbapal");
static const u16 sSkitty_Pal[] = INCBIN_U16("graphics/roulette/skitty.gbapal");
static const u16 sMakuhita_Pal[] =
    INCBIN_U16("graphics/roulette/makuhita.gbapal");
static const u16 sUnused1_Pal[] =
    INCBIN_U16("graphics/roulette/unused_1.gbapal");
static const u16 sUnused2_Pal[] =
    INCBIN_U16("graphics/roulette/unused_2.gbapal");
static const u16 sUnused3_Pal[] =
    INCBIN_U16("graphics/roulette/unused_3.gbapal");
static const u16 sUnused4_Pal[] =
    INCBIN_U16("graphics/roulette/unused_4.gbapal");
static const u32 sBall_Gfx[] = INCBIN_U32("graphics/roulette/ball.4bpp.lz");
static const u32 sBallCounter_Gfx[] =
    INCBIN_U32("graphics/roulette/ball_counter.4bpp.lz");
static const u32 sShroomishTaillow_Gfx[] =
    INCBIN_U32("graphics/roulette/roulette_tilt.4bpp.lz");
static const u32 sGridIcons_Gfx[] =
    INCBIN_U32("graphics/roulette/grid_icons.4bpp.lz");
static const u32 sWheelIcons_Gfx[] =
    INCBIN_U32("graphics/roulette/wheel_icons.4bpp.lz");
...
static const union AnimCmd sAffineAnim_Unused1[] = {ANIMCMD_FRAME(0, 0),
                                                    ANIMCMD_END};
...
static const union AnimCmd sAnim_WheelIcons[] = {
    ANIMCMD_FRAME(0, 0),  ANIMCMD_FRAME(32, 0), ANIMCMD_FRAME(64, 0),
...
static const union AnimCmd sAnim_Headers[] = {
    ANIMCMD_FRAME(0, 0),   ANIMCMD_FRAME(16, 0),  ANIMCMD_FRAME(32, 0),
...
static const union AnimCmd sAnim_GridIcons[] = {
    ANIMCMD_FRAME(0, 0), ANIMCMD_FRAME(4, 0), ANIMCMD_FRAME(8, 0),
...
static const struct SpriteTemplate
    sSpriteTemplates_PokeHeaders[NUM_BOARD_POKES] = {
...
static const struct SpriteTemplate
    sSpriteTemplates_ColorHeaders[NUM_BOARD_COLORS] = {
        {.tileTag = GFXTAG_HEADERS,
         .paletteTag = PALTAG_GRID_ICONS,
         .oam = &sOam_GridHeader,
...
// Wheel icons are listed clockwise starting from 1 oclock on the roulette wheel
// (with Poké Ball upside right) They go Wynaut -> Azurill -> Skitty ->
// Makuhita, and Orange -> Green -> Purple
static const struct SpriteTemplate
    sSpriteTemplates_WheelIcons[NUM_ROULETTE_SLOTS] = {
...
static const union AnimCmd sAnim_CreditDigit[] = {ANIMCMD_FRAME(0, 0),  // 0
                                                  ANIMCMD_FRAME(2, 0),  // 1
                                                  ANIMCMD_FRAME(4, 0),  // 2
                                                  ANIMCMD_FRAME(6, 0),  // 3
                                                  ANIMCMD_FRAME(8, 0),  // 4
...
static const union AnimCmd sAnim_Multiplier[] = {
    ANIMCMD_FRAME(0, 0),  ANIMCMD_FRAME(8, 0),  ANIMCMD_FRAME(16, 0),
...
static const union AnimCmd sAnim_BallCounter[] = {
    ANIMCMD_FRAME(0, 0), ANIMCMD_FRAME(2, 0), ANIMCMD_FRAME(4, 0),
...
static const union AnimCmd sAnim_Ball_RollFast[] = {
    ANIMCMD_FRAME(0, 5), ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(8, 5),
    ANIMCMD_FRAME(4, 5), ANIMCMD_JUMP(0)};

static const union AnimCmd sAnim_Ball_RollMedium[] = {
    ANIMCMD_FRAME(0, 10), ANIMCMD_FRAME(4, 10), ANIMCMD_FRAME(8, 10),
    ANIMCMD_FRAME(4, 10), ANIMCMD_JUMP(0)};

static const union AnimCmd sAnim_Ball_RollSlow[] = {
    ANIMCMD_FRAME(0, 15), ANIMCMD_FRAME(4, 15), ANIMCMD_FRAME(8, 15),
    ANIMCMD_FRAME(4, 15), ANIMCMD_JUMP(0)};

static const union AnimCmd sAnim_Ball_StopOnFrame1[] = {
    ANIMCMD_FRAME(4, 2), ANIMCMD_FRAME(8, 5), ANIMCMD_FRAME(4, 5),
    ANIMCMD_FRAME(12, 5), ANIMCMD_END};

static const union AnimCmd sAnim_Ball_StopOnFrame3[] = {
    ANIMCMD_FRAME(4, 2), ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(4, 4),
    ANIMCMD_FRAME(8, 4), ANIMCMD_FRAME(12, 4), ANIMCMD_END};

static const union AnimCmd sAnim_Ball_StopOnFrame4[] = {
    ANIMCMD_FRAME(0, 2), ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(8, 5),
    ANIMCMD_FRAME(12, 5), ANIMCMD_END};

static const union AnimCmd sAnim_Ball_Still[] = {ANIMCMD_FRAME(12, 0),
                                                 ANIMCMD_END};

static const union AnimCmd sAnim_Ball_StopOnFrame2[] = {
    ANIMCMD_FRAME(8, 2), ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(0, 5),
    ANIMCMD_FRAME(12, 5), ANIMCMD_END};

static const union AnimCmd *const sAnims_Ball[] = {
    sAnim_Ball_RollFast,     sAnim_Ball_RollMedium,   sAnim_Ball_RollSlow,
    sAnim_Ball_StopOnFrame1, sAnim_Ball_StopOnFrame2, sAnim_Ball_StopOnFrame3,
    sAnim_Ball_StopOnFrame4, sAnim_Ball_StopOnFrame4, sAnim_Ball_Still};
...
static const union AnimCmd sAnim_Taillow_FlapSlow_Right[] = {
    ANIMCMD_FRAME(80, 20, .hFlip = TRUE), ANIMCMD_FRAME(96, 20, .hFlip = TRUE),
    ANIMCMD_JUMP(0)};
...
static const union AnimCmd sAnim_Taillow_FlapFast_Right[] = {
    ANIMCMD_FRAME(80, 10, .hFlip = TRUE), ANIMCMD_FRAME(96, 10, .hFlip = TRUE),
    ANIMCMD_JUMP(0)};
...
static const union AffineAnimCmd sAffineAnim_Unused3[] = {
    AFFINEANIMCMD_FRAME(0x80, 0x80, 0, 0), AFFINEANIMCMD_FRAME(2, 2, 0, 60),
    AFFINEANIMCMD_END};

static const union AffineAnimCmd sAffineAnim_TaillowShadow[] = {
    AFFINEANIMCMD_FRAME(0x100, 0x100, 0, 0),
    AFFINEANIMCMD_FRAME(-2, 0x0, 0, 15), AFFINEANIMCMD_FRAME(-1, -2, 0, 15),
...
static const union AnimCmd sAnim_ShroomishBallShadow[] = {ANIMCMD_FRAME(0, 0),
                                                          ANIMCMD_END};

static const union AnimCmd sAnim_UnstickMonShadow[] = {ANIMCMD_FRAME(4, 0),
                                                       ANIMCMD_END};
...
static void Task_PrintMinBet(u8 taskId) {
  if (JOY_NEW(A_BUTTON | B_BUTTON)) {
    ...
    ConvertIntToDecimalStringN(gStringVar1, minBet, STR_CONV_MODE_LEADING_ZEROS,
                               1);
    StringExpandPlaceholders(gStringVar4, Roulette_Text_PlayMinimumWagerIsX);
    DrawStdWindowFrame(0, FALSE);
    AddTextPrinterParameterized(0, FONT_NORMAL, gStringVar4, 0, 1,
                                TEXT_SKIP_DRAW, NULL);
...
static void Task_PrintRouletteEntryMsg(u8 taskId) {
  ...
  if (gTasks[taskId].tCoins >= minBet) {
    if ((gSpecialVar_0x8004 & ROULETTE_SPECIAL_RATE) &&
      ...
      DrawStdWindowFrame(0, FALSE);
      AddTextPrinterParameterized(0, FONT_NORMAL,
                                  Roulette_Text_SpecialRateTable, 0, 1,
                                  TEXT_SKIP_DRAW, NULL);
      CopyWindowToVram(0, COPYWIN_FULL);
...
static u8 CreateWheelIconSprite(const struct SpriteTemplate *template, u8 r1,
                                u16 *angle) {
  u16 temp;
...
static void CreateGridSprites(void) {
  ...
  for (i = 0; i < NUM_BOARD_COLORS; i++) {
    ...
    for (j = 0; j < NUM_BOARD_POKES; j++) {
      spriteId =
          sRoulette->spriteIds[(i * NUM_BOARD_POKES) + SPR_GRID_ICONS + j] =
              CreateSprite(&sSpriteTemplates_GridIcons[j], (j * 24) + 148,
                           y + 92, 30);
      gSprites[spriteId].animPaused = TRUE;
      y += 24;
      if (y >= 72)
  ...
  if (hideAll) {
    ...
    for (; i < BALLS_PER_ROUND; i++) {
      if (!sRoulette->hitSquares[i] || i == hideBallId) {
        ...
        gSprites[sRoulette->spriteIds[i + SPR_GRID_BALLS]].x =
            (sGridSelections[sRoulette->hitSquares[i]].x + 1) * 8 + 4;
        gSprites[sRoulette->spriteIds[i + SPR_GRID_BALLS]].y =
...
static void ShowHideWinSlotCursor(u8 selectionId) {
  if (selectionId == 0) {
    ...
    gSprites[sRoulette->spriteIds[SPR_WIN_SLOT_CURSOR]].x =
        (sGridSelections[selectionId].x + 2) * 8;
    gSprites[sRoulette->spriteIds[SPR_WIN_SLOT_CURSOR]].y =
...
static void CreateWheelIconSprites(void) {
  ...
  for (i = 0; i < NUM_BOARD_COLORS; i++) {
    for (j = 0; j < NUM_BOARD_POKES; j++) {
      u8 spriteId;
      spriteId =
          sRoulette->spriteIds[(i * NUM_BOARD_POKES) + SPR_WHEEL_ICONS + j] =
              CreateWheelIconSprite(
                  &sSpriteTemplates_WheelIcons[i * NUM_BOARD_POKES + j], 40,
                  &angle);
      gSprites[spriteId].animPaused = TRUE;
      gSprites[spriteId].affineAnimPaused = TRUE;
    }
...
static void CreateInterfaceSprites(void) {
  ...
  sRoulette->spriteIds[SPR_CREDIT] =
      CreateSprite(&sSpriteTemplate_Credit, 208, 16, 4);
  ...
  sRoulette->spriteIds[SPR_MULTIPLIER] =
      CreateSprite(&sSpriteTemplate_Multiplier, 120, 68, 4);
  ...
  sRoulette->spriteIds[SPR_WIN_SLOT_CURSOR] =
      CreateSprite(&sSpriteTemplate_Cursor, 152, 96, 9);
  ...
  for (i = 0; i < MAX_COIN_DIGITS; i++) {
    ...
    if (digit > 0 || printZero || i == MAX_COIN_DIGITS - 1) {
      ...
      gSprites[sRoulette->spriteIds[i + SPR_CREDIT_DIGITS]].oam.tileNum =
          ...
          (*gSprites[sRoulette->spriteIds[i + SPR_CREDIT_DIGITS]].anims + digit)
              ->type;
      printZero = TRUE;
    }
    num = num % d;
...
static void SetMultiplierSprite(u8 selectionId) {
  ...
  sprite->oam.tileNum =
      sprite->sheetTileStart + (*sprite->anims + sprite->animCmdIndex)->type;
}
...
static void SetBallCounterNumLeft(u8 numBalls) {
  ...
  case 5:
    gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_3]].oam.tileNum =
        ...
        (*gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_3]].anims + t + 1)
            ->type;
    break;
  case 4:
    gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_3]].oam.tileNum =
        gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_3]].sheetTileStart +
        (*gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_3]].anims + t + 2)
            ->type;
    break;
  case 3:
    gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_2]].oam.tileNum =
        ...
        (*gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_2]].anims + t + 1)
            ->type;
    break;
  case 2:
    gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_2]].oam.tileNum =
        gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_2]].sheetTileStart +
        (*gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_2]].anims + t + 2)
            ->type;
    break;
  case 1:
    gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_1]].oam.tileNum =
        ...
        (*gSprites[sRoulette->spriteIds[SPR_BALL_COUNTER_1]].anims + t + 1)
            ->type;
    break;
  case 0:
  default:
    for (i = 0; i < BALLS_PER_ROUND / 2; i++) {
      gSprites[sRoulette->spriteIds[i + SPR_BALL_COUNTER]].oam.tileNum =
          gSprites[sRoulette->spriteIds[i + SPR_BALL_COUNTER]].sheetTileStart +
          (*gSprites[sRoulette->spriteIds[i + SPR_BALL_COUNTER]].anims + t + 2)
              ->type;
    }
  }
}
...
static void CreateWheelBallSprites(void) {
  u8 i;
  for (i = 0; i < BALLS_PER_ROUND; i++) {
    sRoulette->spriteIds[i] =
        CreateSprite(&sSpriteTemplate_Ball, 116, 80, 57 - i);
...
#define sStuckOnWheelLeft                                                      \
  data[0] // if true, ball got stuck in left half of wheel, else got stuck in
          // right half
...
// "wheelAngle" and "sBallAngle" are relative to the screen (e.g. 180 degrees
// for either is always screen bottom) "sBallWheelAngle" is the ball's angle
// relative to the wheel
//   e.g. if the ball is screen right (90), but wheel is upside down (180),
//   sBallWheelAngle is 270 (because the ball is wheel left)
...
static u8 UpdateSlotBelowBall(struct Sprite *sprite) {
  sRoulette->hitSlot =
      UpdateBallRelativeWheelAngle(sprite) / (f32)DEGREES_PER_SLOT;
...
static void SpriteCB_UnstickBall_Shroomish(struct Sprite *sprite) {
  ...
  case 0:
    if (sprite->sStuckOnWheelLeft != TRUE) {
      ...
      ballFallDist =
          (slotOffset * sRouletteTables[sRoulette->tableId].randDistanceHigh +
           (sRouletteTables[sRoulette->tableId].randDistanceLow - 1));
      ballFallSpeed =
          (slotOffset /
           sRouletteTables[sRoulette->tableId].shroomish.fallSlowdown);
    } else {
      return;
    }
    break;
  ...
  sRoulette->ballFallAccel = -((ballFallSpeed * 2.0f) / ballFallDist +
                               (2.0f / (ballFallDist * ballFallDist)));
...
// The below SpriteCB_UnstickBall_* callbacks handle the ball while its being
// cleared by Shroomish/Taillow For what Shroomish/Taillow do during this
// sequence, see SpriteCB_Shroomish / SpriteCB_Taillow
...
static void SpriteCB_RollBall_TryLand(struct Sprite *sprite) {
  ...
  if (!(sRouletteSlots[sRoulette->hitSlot].flag & sRoulette->hitFlags)) {
    ...
    if (fallRight) {
      sRoulette->ballAngleSpeed = 0.0f;
      sRoulette->stuckHitSlot = slotId =
          (sRoulette->hitSlot + 1) % NUM_ROULETTE_SLOTS;
    } else // fall left
    {
      f32 temp;
      sRoulette->ballAngleSpeed =
          (temp = sRouletteTables[sRoulette->tableId].var1C) * 2.0f;
      slotId =
          (sRoulette->hitSlot + NUM_ROULETTE_SLOTS - 1) % NUM_ROULETTE_SLOTS;
...
static void SpriteCB_RollBall_Slow(struct Sprite *sprite) {
  ...
  if (GetBallDistanceToSlotMidpoint(sprite) == 0) {
    ...
    sRoulette->ballAngleSpeed -=
        (f32)(sRouletteTables[sRoulette->tableId].wheelSpeed) /
...
static void SpriteCB_RollBall_Medium(struct Sprite *sprite) {
  ...
  sRoulette->ballAngleAccel =
      -(sRoulette->ballAngleSpeed / (f32)(sRoulette->ballTravelDistSlow));
  sprite->animNum = 2;
  sprite->animBeginning = TRUE;
  sprite->animEnded = FALSE;
  sprite->sState = 3;
...
static void SpriteCB_RollBall_Fast(struct Sprite *sprite) {
  ...
  sRoulette->ballAngleAccel = ((1.0f - sRoulette->ballAngleSpeed) /
                               (f32)(sRoulette->ballTravelDistMed));
  sprite->animNum = 1;
  sprite->animBeginning = TRUE;
  sprite->animEnded = FALSE;
  sprite->sState = 2;
...
static void CreateShroomishSprite(struct Sprite *ball) {
  ...
  sRoulette->spriteIds[SPR_CLEAR_MON] =
      CreateSprite(&sSpriteTemplate_Shroomish, 36, -12, 50);
  sRoulette->spriteIds[SPR_CLEAR_MON_SHADOW_1] = CreateSprite(
      &sSpriteTemplate_ShroomishShadow[0], coords[ball->sStuckOnWheelLeft][0],
  ...
  gSprites[sRoulette->spriteIds[SPR_CLEAR_MON_SHADOW_2]].oam.objMode =
      ST_OAM_OBJ_BLEND;
  for (i = 0; i < 3; i++) {
    gSprites[sRoulette->spriteIds[i + SPR_CLEAR_MON]].coordOffsetEnabled =
        FALSE;
  ...
  gSprites[sRoulette->spriteIds[SPR_CLEAR_MON_SHADOW_1]].coordOffsetEnabled =
      TRUE;
  sRoulette->ball = ball;
}
...
static void CreateTaillowSprite(struct Sprite *ball) {
  ...
  t = ball->data[7] - 2;
  sRoulette->spriteIds[SPR_CLEAR_MON] =
      CreateSprite(&sSpriteTemplate_Taillow, coords[ball->sStuckOnWheelLeft][0],
                   coords[ball->sStuckOnWheelLeft][1], 50);
  StartSpriteAnim(&gSprites[sRoulette->spriteIds[SPR_CLEAR_MON]],
                  ball->sStuckOnWheelLeft);
  sRoulette->spriteIds[SPR_CLEAR_MON_SHADOW_1] = CreateSprite(
      &sSpriteTemplate_TaillowShadow, coords[ball->sStuckOnWheelLeft][0],
      coords[ball->sStuckOnWheelLeft][1], 51);
  gSprites[sRoulette->spriteIds[SPR_CLEAR_MON_SHADOW_1]].affineAnimPaused =
      TRUE;
  gSprites[sRoulette->spriteIds[SPR_CLEAR_MON_SHADOW_1]].animPaused = TRUE;
  ball->data[7] =
      (t * sRouletteTables[sRoulette->tableId].randDistanceHigh) +
  ...
  for (; i < 2; i++) {
    ...
    gSprites[sRoulette->spriteIds[SPR_CLEAR_MON + i]].data[3] =
        ball->data[7] - 45;
  }
  sRoulette->ball = ball;
}
...
static void SetBallStuck(struct Sprite *sprite) {
  ...
  u8 slotCandidates[NUM_ROULETTE_SLOTS - 2] =
      {}; // - 2 because we know at least 2 are already occupied
  u16 rand = Random();
  ...
  angle = (sRoulette->tableId * DEGREES_PER_SLOT + 33) +
          (1 - sRoulette->useTaillow) * 15;

  // Determine which quadrant the ball got stuck in
  // Use either Shroomish or Taillow to clear the ball depending on where it's
  // stuck
  for (i = 0; i < 4; i++) {
  ...
  if (sRoulette->useTaillow == TRUE && sRoulette->tableId == 1)
    ...
    maxSlotToCheck +=
        slotsToSkip; // Check enough slots to guarantee an empty will be found
  ...
  if ((sRoulette->useTaillow + 1) & sRoulette->partySpeciesFlags) {
    // If the player has the corresponding Pokémon in their party (HAS_SHROOMISH
    // or HAS_TAILLOW), there's a 75% chance that the ball will be moved to a
    // spot they bet on assuming it was one of the slots identified as a
    // candidate
...
// Handles both the screen shake and ball shadow effect for when Shroomish
// unsticks the ball
static void SpriteCB_ShroomishShakeScreen(struct Sprite *sprite) {
  ...
  if (sprite->data[1]++ < sprite->data[3]) {
    if (sprite->data[1] & 1) {
      // Shake screen
      gSpriteCoordOffsetY =
          screenShakeOffsets[sprite->data[2] / 2][sprite->data[7]];
...
static void SpriteCB_ShroomishFall(struct Sprite *sprite) {
  ...
  if (sRoulette->shroomishShadowTimer <
      ARRAY_COUNT(sShroomishShadowAlphas) * 2 - 1)
  ...
  if (sprite->data[1] > 60) {
    ...
    gSprites[sprite->sBallShadowSpriteId].callback =
...
static void SpriteCB_Shroomish(struct Sprite *sprite) {
  if (sprite->data[7] == 0) {
    // Wait for the ball to be a specific angle (or its 180 degree opposite)
    // specified by the table Once it is, reveal the shadow for Shroomish
    // falling in
    ...
    // Wait for the ball to be a specific angle (or its 180 degree opposite)
    // specified by the table Once it is, have Shroomish begin to fall in On
    // both tables this angle is 15 degrees off the "start" angle
    if (!sRoulette->ball->sStuckOnWheelLeft) {
      if (sRoulette->ball->sBallAngle !=
...
static void SpriteCB_Taillow_FlyAway(struct Sprite *sprite) {
  if (sprite->y > -16) {
    ...
    DestroySprite(sprite);
    FreeOamMatrix(
        gSprites[sRoulette->spriteIds[SPR_CLEAR_MON_SHADOW_1]].oam.matrixNum);
...
static void SpriteCB_Taillow_FlyIn(struct Sprite *sprite) {
  ...
  if (sprite->data[1]-- > 7) {
    ...
    if (sprite->data[1] >= 0) {
      sprite->x += xMoveOffsets[sRoulette->ball->sStuckOnWheelLeft] *
