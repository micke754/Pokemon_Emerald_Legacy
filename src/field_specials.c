...
static EWRAM_DATA struct ListMenuItem *sScrollableMultichoice_ListMenuItem =
    NULL;
...
static void DetermineCyclingRoadResults(u32 numFrames, u8 numBikeCollisions) {
  ...
  if (numBikeCollisions < 100) {
    ConvertIntToDecimalStringN(gStringVar1, numBikeCollisions,
  ...
  if (numFrames < 3600) {
    ConvertIntToDecimalStringN(gStringVar2, numFrames / 60,
                               STR_CONV_MODE_RIGHT_ALIGN, 2);
    gStringVar2[2] = CHAR_DEC_SEPARATOR;
    ConvertIntToDecimalStringN(&gStringVar2[3], ((numFrames % 60) * 100) / 60,
...
u16 GetRecordedCyclingRoadResults(void) {
  ...
  DetermineCyclingRoadResults(framesRecord,
                              VarGet(VAR_CYCLING_ROAD_RECORD_COLLISIONS));
  return TRUE;
}

void UpdateCyclingRoadState(void) {
  if (gLastUsedWarp.mapNum ==
          MAP_NUM(ROUTE110_SEASIDE_CYCLING_ROAD_SOUTH_ENTRANCE) &&
      gLastUsedWarp.mapGroup ==
          MAP_GROUP(ROUTE110_SEASIDE_CYCLING_ROAD_SOUTH_ENTRANCE))
    return;

  if (VarGet(VAR_CYCLING_CHALLENGE_STATE) == 2 ||
      VarGet(VAR_CYCLING_CHALLENGE_STATE) == 3) {
...
void SpawnLinkPartnerObjectEvent(void) {
  ...
  s16 x = 0;
  s16 y = 0;
  u8 movementTypes[] = {MOVEMENT_TYPE_FACE_UP, MOVEMENT_TYPE_FACE_LEFT,
                        MOVEMENT_TYPE_FACE_DOWN, MOVEMENT_TYPE_FACE_RIGHT};
  ...
  for (i = 0; i < gSpecialVar_0x8004; i++) {
    if (myLinkPlayerNumber != i) {
      ...
      SpawnSpecialObjectEventParameterized(
          linkSpriteId, movementTypes[j], 240 - i,
          coordOffsets[j][0] + x + MAP_OFFSET,
          coordOffsets[j][1] + y + MAP_OFFSET, 0);
...
static void LoadLinkPartnerObjectEventSpritePalette(u16 graphicsId,
  ...
  if (graphicsId == OBJ_EVENT_GFX_LINK_RS_BRENDAN ||
    ...
    u8 obj = GetObjectEventIdByLocalIdAndMap(localEventId,
    ...
    if (obj != OBJECT_EVENTS_COUNT) {
      ...
      case OBJ_EVENT_GFX_LINK_RS_BRENDAN:
        LoadPalette(gObjectEventPal_RubySapphireBrendan,
                    OBJ_PLTT_ID(adjustedPaletteNum), PLTT_SIZE_4BPP);
        break;
      case OBJ_EVENT_GFX_LINK_RS_MAY:
        LoadPalette(gObjectEventPal_RubySapphireMay,
                    OBJ_PLTT_ID(adjustedPaletteNum), PLTT_SIZE_4BPP);
        break;
      case OBJ_EVENT_GFX_RIVAL_BRENDAN_NORMAL:
        LoadPalette(gObjectEventPal_Brendan, OBJ_PLTT_ID(adjustedPaletteNum),
                    PLTT_SIZE_4BPP);
        break;
      case OBJ_EVENT_GFX_RIVAL_MAY_NORMAL:
        LoadPalette(gObjectEventPal_May, OBJ_PLTT_ID(adjustedPaletteNum),
                    PLTT_SIZE_4BPP);
        break;
      }
    }
...
// Sets the gym barriers back to the default state; their alt state is handled
// by MauvilleCity_Gym_EventScript_SetAltBarriers
    ...
    for (x = 0 + MAP_OFFSET; x < 9 + MAP_OFFSET; x++) {
      ...
      case METATILE_MauvilleGym_GreenBeamH3_Off:
        MapGridSetMetatileIdAt(
            x, y, METATILE_MauvilleGym_GreenBeamH3_On | MAPGRID_COLLISION_MASK);
        break;
      case METATILE_MauvilleGym_GreenBeamH4_Off:
        MapGridSetMetatileIdAt(
            x, y, METATILE_MauvilleGym_GreenBeamH4_On | MAPGRID_COLLISION_MASK);
        break;
      ...
      case METATILE_MauvilleGym_RedBeamH3_Off:
        MapGridSetMetatileIdAt(
            x, y, METATILE_MauvilleGym_RedBeamH3_On | MAPGRID_COLLISION_MASK);
        break;
      case METATILE_MauvilleGym_RedBeamH4_Off:
        MapGridSetMetatileIdAt(
            x, y, METATILE_MauvilleGym_RedBeamH4_On | MAPGRID_COLLISION_MASK);
        break;
      case METATILE_MauvilleGym_GreenBeamV1_On:
        MapGridSetMetatileIdAt(
            x, y, METATILE_MauvilleGym_PoleBottom_On | MAPGRID_COLLISION_MASK);
        break;
      ...
      case METATILE_MauvilleGym_RedBeamV1_On:
        MapGridSetMetatileIdAt(
            x, y, METATILE_MauvilleGym_PoleBottom_Off | MAPGRID_COLLISION_MASK);
        break;
      ...
      case METATILE_MauvilleGym_PoleBottom_On:
        MapGridSetMetatileIdAt(
            x, y, METATILE_MauvilleGym_GreenBeamV1_On | MAPGRID_COLLISION_MASK);
        break;
      case METATILE_MauvilleGym_FloorTile:
        if (MapGridGetMetatileIdAt(x, y - 1) ==
          ...
          MapGridSetMetatileIdAt(
              x, y, METATILE_MauvilleGym_RedBeamV2_On | MAPGRID_COLLISION_MASK);
        break;
      case METATILE_MauvilleGym_PoleBottom_Off:
        MapGridSetMetatileIdAt(
            x, y, METATILE_MauvilleGym_RedBeamV1_On | MAPGRID_COLLISION_MASK);
        break;
      case METATILE_MauvilleGym_PoleTop_Off:
        MapGridSetMetatileIdAt(
            x, y, METATILE_MauvilleGym_PoleTop_On | MAPGRID_COLLISION_MASK);
        break;
...
void MauvilleGymDeactivatePuzzle(void) {
  ...
  for (i = ARRAY_COUNT(sMauvilleGymSwitchCoords) - 1; i >= 0; i--) {
    MapGridSetMetatileIdAt(switchCoords->x, switchCoords->y,
                           METATILE_MauvilleGym_PressedSwitch);
    switchCoords++;
  }
  for (y = 5 + MAP_OFFSET; y < 17 + MAP_OFFSET; y++) {
...
static void Task_PetalburgGymSlideOpenRoomDoors(u8 taskId) {
  if (sSlidingDoorNextFrameDelay[sSlidingDoorFrame] ==
      sSlidingDoorNextFrameCounter) {
    PetalburgGymSetDoorMetatiles(
        gSpecialVar_0x8004,
        sPetalburgGymSlidingDoorMetatiles[sSlidingDoorFrame]);
    sSlidingDoorNextFrameCounter = 0;
    if ((++sSlidingDoorFrame) ==
        ARRAY_COUNT(sPetalburgGymSlidingDoorMetatiles)) {
      DestroyTask(taskId);
...
static void PetalburgGymSetDoorMetatiles(u8 roomNumber, u16 metatileId) {
  ...
  for (i = 0; i < nDoors; i++) {
    MapGridSetMetatileIdAt(doorCoordsX[i] + MAP_OFFSET,
                           doorCoordsY[i] + MAP_OFFSET,
                           metatileId | MAPGRID_COLLISION_MASK);
    MapGridSetMetatileIdAt(
        doorCoordsX[i] + MAP_OFFSET, doorCoordsY[i] + MAP_OFFSET + 1,
        (metatileId + METATILE_ROW_WIDTH) | MAPGRID_COLLISION_MASK);
  }
  DrawWholeMapView();
}

void PetalburgGymUnlockRoomDoors(void) {
  PetalburgGymSetDoorMetatiles(gSpecialVar_0x8004,
...
u8 GetPlayerTrainerIdOnesDigit(void) {
  return (u16)((gSaveBlock2Ptr->playerTrainerId[1] << 8) |
...
void CableCarWarp(void) {
  if (gSpecialVar_0x8004 != 0)
    SetWarpDestination(MAP_GROUP(ROUTE112_CABLE_CAR_STATION),
                       MAP_NUM(ROUTE112_CABLE_CAR_STATION), WARP_ID_NONE, 6, 4);
  else
    SetWarpDestination(MAP_GROUP(MT_CHIMNEY_CABLE_CAR_STATION),
                       MAP_NUM(MT_CHIMNEY_CABLE_CAR_STATION), WARP_ID_NONE, 6,
                       4);
}
...
// For this special, gSpecialVar_0x8004 is expected to be some PC_LOCATION_*
// value.
...
static void PCTurnOnEffect_SetMetatile(s16 isScreenOn, s8 dx, s8 dy) {
  ...
  MapGridSetMetatileIdAt(gSaveBlock1Ptr->pos.x + dx + MAP_OFFSET,
...
static void LotteryCornerComputerEffect(struct Task *task) {
  if (task->tTimer == 6) {
    ...
    if (task->tIsScreenOn) {
      // Screen is on, set it off
      MapGridSetMetatileIdAt(11 + MAP_OFFSET, 1 + MAP_OFFSET,
                             METATILE_Shop_Laptop1_Normal |
                                 MAPGRID_COLLISION_MASK);
      MapGridSetMetatileIdAt(11 + MAP_OFFSET, 2 + MAP_OFFSET,
                             METATILE_Shop_Laptop2_Normal |
                                 MAPGRID_COLLISION_MASK);
    } else {
      // Screen is off, set it on
      MapGridSetMetatileIdAt(11 + MAP_OFFSET, 1 + MAP_OFFSET,
                             METATILE_Shop_Laptop1_Flash |
                                 MAPGRID_COLLISION_MASK);
      MapGridSetMetatileIdAt(11 + MAP_OFFSET, 2 + MAP_OFFSET,
                             METATILE_Shop_Laptop2_Flash |
                                 MAPGRID_COLLISION_MASK);
    }
    DrawWholeMapView();
...
void EndLotteryCornerComputerEffect(void) {
  MapGridSetMetatileIdAt(11 + MAP_OFFSET, 1 + MAP_OFFSET,
                         METATILE_Shop_Laptop1_Normal | MAPGRID_COLLISION_MASK);
  MapGridSetMetatileIdAt(11 + MAP_OFFSET, 2 + MAP_OFFSET,
                         METATILE_Shop_Laptop2_Normal | MAPGRID_COLLISION_MASK);
  DrawWholeMapView();
}
...
void SpawnCameraObject(void) {
  u8 obj = SpawnSpecialObjectEventParameterized(
      OBJ_EVENT_GFX_BOY_1, MOVEMENT_TYPE_FACE_DOWN, OBJ_EVENT_ID_CAMERA,
...
void RemoveCameraObject(void) {
  ...
  RemoveObjectEventByLocalIdAndMap(OBJ_EVENT_ID_CAMERA,
                                   gSaveBlock1Ptr->location.mapNum,
                                   gSaveBlock1Ptr->location.mapGroup);
}
...
u8 GetPokeblockNameByMonNature(void) {
  return CopyMonFavoritePokeblockName(
      GetNature(&gPlayerParty[GetLeadMonIndex()]), gStringVar1);
}
...
u16 GetSlotMachineId(void) {
  static const u8 sSlotMachineRandomSeeds[SLOT_MACHINE_COUNT] = {
      12, 2, 4, 5, 1, 8, 7, 11, 3, 10, 9, 6};
  static const u8 sSlotMachineIds[SLOT_MACHINE_COUNT] = {
      SLOT_MACHINE_UNLUCKIEST, SLOT_MACHINE_UNLUCKIER, SLOT_MACHINE_UNLUCKIER,
      SLOT_MACHINE_UNLUCKY,    SLOT_MACHINE_UNLUCKY,   SLOT_MACHINE_UNLUCKY,
      SLOT_MACHINE_LUCKY,      SLOT_MACHINE_LUCKY,     SLOT_MACHINE_LUCKY,
      SLOT_MACHINE_LUCKIER,    SLOT_MACHINE_LUCKIER,   SLOT_MACHINE_LUCKIEST};
  static const u8 sSlotMachineServiceDayIds[SLOT_MACHINE_COUNT] = {
      SLOT_MACHINE_LUCKY,   SLOT_MACHINE_LUCKY,    SLOT_MACHINE_LUCKY,
      SLOT_MACHINE_LUCKY,   SLOT_MACHINE_LUCKY,    SLOT_MACHINE_LUCKY,
      SLOT_MACHINE_LUCKIER, SLOT_MACHINE_LUCKIER,  SLOT_MACHINE_LUCKIER,
      SLOT_MACHINE_LUCKIER, SLOT_MACHINE_LUCKIEST, SLOT_MACHINE_LUCKIEST};

  u32 rnd = gSaveBlock1Ptr->dewfordTrends[0].trendiness +
            gSaveBlock1Ptr->dewfordTrends[0].rand +
...
void LoadWallyZigzagoon(void) {
  u16 monData;
  CreateMon(&gPlayerParty[0], SPECIES_ZIGZAGOON, 7, USE_RANDOM_IVS, FALSE, 0,
            OT_ID_PLAYER_ID, 0);
  monData = TRUE;
...
u8 GetLeadMonIndex(void) {
  ...
  u8 partyCount = CalculatePlayerPartyCount();
  for (i = 0; i < partyCount; i++) {
    if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES_OR_EGG, NULL) !=
            SPECIES_EGG &&
...
bool8 MonOTNameNotPlayer(void) {
  if (GetMonData(&gPlayerParty[gSpecialVar_0x8004], MON_DATA_LANGUAGE) !=
      GAME_LANGUAGE)
    return TRUE;
...
void BufferLottoTicketNumber(void) {
  if (gSpecialVar_Result >= 10000) {
    ...
    ConvertIntToDecimalStringN(gStringVar1 + 1, gSpecialVar_Result,
                               STR_CONV_MODE_LEFT_ALIGN,
                               CountDigits(gSpecialVar_Result));
  } else if (gSpecialVar_Result >= 100) {
    gStringVar1[0] = CHAR_0;
    gStringVar1[1] = CHAR_0;
    ConvertIntToDecimalStringN(gStringVar1 + 2, gSpecialVar_Result,
                               STR_CONV_MODE_LEFT_ALIGN,
                               CountDigits(gSpecialVar_Result));
    ...
    ConvertIntToDecimalStringN(gStringVar1 + 3, gSpecialVar_Result,
                               STR_CONV_MODE_LEFT_ALIGN,
                               CountDigits(gSpecialVar_Result));
  } else {
    gStringVar1[0] = CHAR_0;
    ...
    ConvertIntToDecimalStringN(gStringVar1 + 4, gSpecialVar_Result,
                               STR_CONV_MODE_LEFT_ALIGN,
                               CountDigits(gSpecialVar_Result));
  }
}
...
static const u16 sElevatorWindowTiles_Ascending
    [ELEVATOR_WINDOW_HEIGHT][ELEVATOR_LIGHT_STAGES] = {
...
static const u16 sElevatorWindowTiles_Descending
    [ELEVATOR_WINDOW_HEIGHT][ELEVATOR_LIGHT_STAGES] = {
        {METATILE_BattleFrontier_Elevator_Top0,
         METATILE_BattleFrontier_Elevator_Top2,
         METATILE_BattleFrontier_Elevator_Top1},
...
u16 GetDeptStoreDefaultFloorChoice(void) {
  ...
  if (gSaveBlock1Ptr->dynamicWarp.mapGroup ==
      MAP_GROUP(LILYCOVE_CITY_DEPARTMENT_STORE_1F)) {
    switch (gSaveBlock1Ptr->dynamicWarp.mapNum) {
    case MAP_NUM(LILYCOVE_CITY_DEPARTMENT_STORE_5F):
      sLilycoveDeptStore_NeverRead = 0;
      sLilycoveDeptStore_DefaultFloorChoice = 0;
...
void MoveElevator(void) {
  static const u8 sElevatorTripLength[MAX_ELEVATOR_TRIP] = {8,  16, 24, 32, 38,
                                                            46, 52, 56, 57};
...
void ShowDeptStoreElevatorFloorSelect(void) {
  ...
  AddTextPrinterParameterized(sTutorMoveAndElevatorWindowId, FONT_NORMAL,
                              gText_ElevatorNowOn, xPos, 1, TEXT_SKIP_DRAW,
                              NULL);

  xPos = GetStringCenterAlignXOffset(
      FONT_NORMAL, sDeptStoreFloorNames[gSpecialVar_0x8005], 64);
  AddTextPrinterParameterized(sTutorMoveAndElevatorWindowId, FONT_NORMAL,
                              sDeptStoreFloorNames[gSpecialVar_0x8005], xPos,
                              17, TEXT_SKIP_DRAW, NULL);
...
static void MoveElevatorWindowLights(u16 floorDelta, bool8 descending) {
  static const u8 sElevatorLightCycles[MAX_ELEVATOR_TRIP] = {3,  6,  9,  12, 15,
                                                             18, 21, 24, 27};
    ...
    if (!tDescending) {
      ...
      for (y = 0; y < ELEVATOR_WINDOW_HEIGHT; y++) {
        for (x = 0; x < ELEVATOR_WINDOW_WIDTH; x++)
          MapGridSetMetatileIdAt(
              x + MAP_OFFSET + 1, y + MAP_OFFSET,
              sElevatorWindowTiles_Ascending[y][tMoveCounter %
                                                ELEVATOR_LIGHT_STAGES] |
                  MAPGRID_COLLISION_MASK);
      }
    } else {
      // Descending
      for (y = 0; y < ELEVATOR_WINDOW_HEIGHT; y++) {
        for (x = 0; x < ELEVATOR_WINDOW_WIDTH; x++)
          MapGridSetMetatileIdAt(
              x + MAP_OFFSET + 1, y + MAP_OFFSET,
              sElevatorWindowTiles_Descending[y][tMoveCounter %
                                                 ELEVATOR_LIGHT_STAGES] |
                  MAPGRID_COLLISION_MASK);
      }
    }
...
void ShowFrontierManiacMessage(void) {
  static const u8
      *const sFrontierManiacMessages[][FRONTIER_MANIAC_MESSAGE_COUNT] = {
  ...
  static const u8
      sFrontierManiacStreakThresholds[][FRONTIER_MANIAC_MESSAGE_COUNT - 1] = {
          ...
          [FRONTIER_MANIAC_PIKE] =
              {13, 112}, // BUG: 112 (0x70) is probably a mistake; the Pike
                         // Queen is battled twice well before that
  ...
  case FRONTIER_MANIAC_TOWER_LINK:
    if (gSaveBlock2Ptr->frontier.towerWinStreaks[facility][FRONTIER_LVL_50] >=
      ...
      winStreak =
          gSaveBlock2Ptr->frontier.towerWinStreaks[facility][FRONTIER_LVL_50];
    else
      winStreak =
          gSaveBlock2Ptr->frontier.towerWinStreaks[facility][FRONTIER_LVL_OPEN];
    break;
  case FRONTIER_MANIAC_DOME:
    if (gSaveBlock2Ptr->frontier
            .domeWinStreaks[FRONTIER_MODE_SINGLES][FRONTIER_LVL_50] >=
...
void BufferBattleTowerElevatorFloors(void) {
  static const u16 sBattleTowerStreakThresholds[] = {7,  14, 21, 28, 35,
                                                     49, 63, 77, 91, 0};
...
static const u8
    *const sScrollableMultichoiceOptions[][MAX_SCROLL_MULTI_LENGTH] = {
        [SCROLL_MULTI_NONE] = {gText_Exit},
        [SCROLL_MULTI_GLASS_WORKSHOP_VENDOR] =
            {gText_BlueFlute, gText_YellowFlute, gText_RedFlute,
             gText_WhiteFlute, gText_BlackFlute, gText_PrettyChair,
             gText_PrettyDesk, gText_Exit},
        [SCROLL_MULTI_POKEMON_FAN_CLUB_RATER] =
            {gText_0Pts, gText_10Pts, gText_20Pts, gText_30Pts, gText_40Pts,
        ...
        [SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1] =
            {gText_KissPoster16BP, gText_KissCushion32BP,
             gText_SmoochumDoll32BP, gText_TogepiDoll48BP, gText_MeowthDoll48BP,
             gText_ClefairyDoll48BP, gText_DittoDoll48BP,
             gText_CyndaquilDoll80BP, gText_ChikoritaDoll80BP,
             gText_TotodileDoll80BP, gText_Exit},
        [SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2] =
            {gText_LaprasDoll128BP, gText_SnorlaxDoll128BP,
        ...
        [SCROLL_MULTI_BF_EXCHANGE_CORNER_VITAMIN_VENDOR] =
            {gText_Protein1BP, gText_Calcium1BP, gText_Iron1BP, gText_Zinc1BP,
             gText_Carbos1BP, gText_HpUp1BP, gText_RareCandy25BP, gText_Exit},
        [SCROLL_MULTI_BF_EXCHANGE_CORNER_HOLD_ITEM_VENDOR] =
            {gText_Leftovers48BP, gText_WhiteHerb48BP, gText_QuickClaw48BP,
             gText_MentalHerb48BP, gText_BrightPowder64BP, gText_ChoiceBand64BP,
             gText_KingsRock64BP, gText_FocusBand64BP, gText_ScopeLens64BP,
             gText_MetalCoat64BP, gText_Exit},
        [SCROLL_MULTI_BF_EXCHANGE_CORNER_BERRY_VENDOR] =
            {gText_Liechi48BP, gText_Ganlon48BP, gText_Salac48BP,
             gText_Petaya48BP, gText_Apicot48BP, gText_Pomeg3BP,
             gText_Kelpsy3BP, gText_Qualot3BP, gText_Hondew3BP, gText_Grepa3BP,
             gText_Tamato3BP, gText_Exit},
        [SCROLL_MULTI_BERRY_POWDER_VENDOR] =
            {gText_EnergyPowder50, gText_EnergyRoot80, gText_HealPowder50,
             gText_RevivalHerb300, gText_Protein1000, gText_Iron1000,
             gText_Carbos1000, gText_Calcium1000, gText_Zinc1000,
             gText_HPUp1000, gText_PPUp3000, gText_Exit},
        [SCROLL_MULTI_BF_RECEPTIONIST] = {gText_BattleTower2, gText_BattleDome,
                                          gText_BattlePalace, gText_BattleArena,
                                          gText_BattleFactory, gText_BattlePike,
                                          gText_BattlePyramid,
                                          gText_RankingHall,
                                          gText_ExchangeService, gText_Exit},
        [SCROLL_MULTI_BF_MOVE_TUTOR_1] =
            {gText_Softboiled16BP, gText_SeismicToss24BP, gText_DreamEater24BP,
             gText_MegaPunch24BP, gText_MegaKick48BP, gText_BodySlam48BP,
             gText_RockSlide48BP, gText_Counter48BP, gText_ThunderWave48BP,
             gText_SwordsDance48BP, gText_Exit},
        [SCROLL_MULTI_BF_MOVE_TUTOR_2] = {gText_DefenseCurl16BP,
                                          gText_Snore24BP, gText_MudSlap24BP,
                                          gText_Swift24BP, gText_IcyWind24BP,
                                          gText_Endure48BP, gText_PsychUp48BP,
                                          gText_IcePunch48BP,
                                          gText_ThunderPunch48BP,
                                          gText_FirePunch48BP, gText_Exit},
        ...
        [SCROLL_MULTI_BATTLE_TENT_RULES] = {
            gText_BattleTrainers, gText_BattleBasics, gText_PokemonNature,
            gText_PokemonMoves, gText_Underpowered, gText_WhenInDanger,
            gText_Exit}};
...
static void Task_ShowScrollableMultichoice(u8 taskId) {
  ...
  template = CreateWindowTemplate(0, task->tLeft, task->tTop, task->tWidth,
                                  task->tHeight, 0xF, 0x64);
  ...
  task->tListTaskId = ListMenuInit(&gScrollableMultichoice_ListMenuTemplate,
                                   task->tScrollOffset, task->tSelectedRow);
  ScheduleBgCopyTilemapToVram(0);
  gTasks[taskId].func = ScrollableMultichoice_ProcessInput;
}

static void InitScrollableMultichoice(void) {
  gScrollableMultichoice_ListMenuTemplate.items =
      sScrollableMultichoice_ListMenuItem;
  gScrollableMultichoice_ListMenuTemplate.moveCursorFunc =
  ...
  gScrollableMultichoice_ListMenuTemplate.scrollMultiple =
...
static void ScrollableMultichoice_MoveCursor(s32 itemIndex, bool8 onInit,
  ...
  if (taskId != TASK_NONE) {
    ...
    FillFrontierExchangeCornerWindowAndItemIcon(task->tScrollMultiId,
                                                selection);
...
static void ScrollableMultichoice_UpdateScrollArrows(u8 taskId) {
  static const struct ScrollArrowsTemplate
      sScrollableMultichoice_ScrollArrowsTemplate = {
  ...
  if (task->tMaxItemsOnScreen != task->tNumItems) {
    ...
    task->tScrollArrowId = AddScrollIndicatorArrowPair(
...
void ShowFrontierGamblerGoMessage(void) {
  ...
  ShowFieldMessage(
      sFrontierGamblerGoMessages[VarGet(VAR_FRONTIER_GAMBLER_SET_CHALLENGE)]);
}
...
void FrontierGamblerSetWonOrLost(bool8 won) {
  ...
  if (VarGet(VAR_FRONTIER_GAMBLER_STATE) == FRONTIER_GAMBLER_PLACED_BET) {
    if (sFrontierChallenges[challenge] ==
        FRONTIER_CHALLENGE(frontierFacilityId, battleMode)) {
      if (won)
...
void UpdateBattlePointsWindow(void) {
  u8 string[32];
  u32 x;
  StringCopy(ConvertIntToDecimalStringN(string,
                                        gSaveBlock2Ptr->frontier.battlePoints,
                                        STR_CONV_MODE_RIGHT_ALIGN, 4),
             gText_BP);
  x = GetStringRightAlignXOffset(FONT_NORMAL, string, 48);
  AddTextPrinterParameterized(sBattlePointsWindowId, FONT_NORMAL, string, x, 1,
                              0, NULL);
}
...
void ShowFrontierExchangeCornerItemIconWindow(void) {
  static const struct WindowTemplate
      sFrontierExchangeCorner_ItemIconWindowTemplate = {
          .bg = 0,
          .tilemapLeft = 2,
          .tilemapTop = 9,
          .width = 4,
...
static void FillFrontierExchangeCornerWindowAndItemIcon(u16 menu,
                                                        u16 selection) {
#include "data/battle_frontier/battle_frontier_exchange_corner.h"

  if ((menu >= SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1 &&
       menu <= SCROLL_MULTI_BF_EXCHANGE_CORNER_HOLD_ITEM_VENDOR) ||
      menu == SCROLL_MULTI_BF_EXCHANGE_CORNER_BERRY_VENDOR) {
    FillWindowPixelRect(0, PIXEL_FILL(1), 0, 0, 216, 32);
    switch (menu) {
    case SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1:
      AddTextPrinterParameterized2(
          0, FONT_NORMAL, sFrontierExchangeCorner_Decor1Descriptions[selection],
          0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE,
          TEXT_COLOR_LIGHT_GRAY);
      if (sFrontierExchangeCorner_Decor1[selection] == ITEM_LIST_END) {
        ShowFrontierExchangeCornerItemIcon(
            sFrontierExchangeCorner_Decor1[selection]);
      } else {
        FreeSpriteTilesByTag(TAG_ITEM_ICON);
        FreeSpritePaletteByTag(TAG_ITEM_ICON);
        sScrollableMultichoice_ItemSpriteId =
            AddDecorationIconObject(sFrontierExchangeCorner_Decor1[selection],
                                    33, 88, 0, TAG_ITEM_ICON, TAG_ITEM_ICON);
      }
      break;
    case SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2:
      AddTextPrinterParameterized2(
          0, FONT_NORMAL, sFrontierExchangeCorner_Decor2Descriptions[selection],
          0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE,
          TEXT_COLOR_LIGHT_GRAY);
      if (sFrontierExchangeCorner_Decor2[selection] == ITEM_LIST_END) {
        ShowFrontierExchangeCornerItemIcon(
            sFrontierExchangeCorner_Decor2[selection]);
      } else {
        FreeSpriteTilesByTag(TAG_ITEM_ICON);
        FreeSpritePaletteByTag(TAG_ITEM_ICON);
        sScrollableMultichoice_ItemSpriteId =
            AddDecorationIconObject(sFrontierExchangeCorner_Decor2[selection],
                                    33, 88, 0, TAG_ITEM_ICON, TAG_ITEM_ICON);
      }
      break;
    case SCROLL_MULTI_BF_EXCHANGE_CORNER_VITAMIN_VENDOR:
      AddTextPrinterParameterized2(
          0, FONT_NORMAL,
          sFrontierExchangeCorner_VitaminsDescriptions[selection], 0, NULL,
          TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
      ShowFrontierExchangeCornerItemIcon(
          sFrontierExchangeCorner_Vitamins[selection]);
      break;
    case SCROLL_MULTI_BF_EXCHANGE_CORNER_HOLD_ITEM_VENDOR:
      AddTextPrinterParameterized2(
          0, FONT_NORMAL,
          sFrontierExchangeCorner_HoldItemsDescriptions[selection], 0, NULL,
          TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
      ShowFrontierExchangeCornerItemIcon(
          sFrontierExchangeCorner_HoldItems[selection]);
      break;
    case SCROLL_MULTI_BF_EXCHANGE_CORNER_BERRY_VENDOR:
      AddTextPrinterParameterized2(
          0, FONT_NORMAL, sFrontierExchangeCorner_BerryDescriptions[selection],
          0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE,
          TEXT_COLOR_LIGHT_GRAY);
      ShowFrontierExchangeCornerItemIcon(
          sFrontierExchangeCorner_Berries[selection]);
      break;
    }
  }
}
...
static void ShowFrontierExchangeCornerItemIcon(u16 item) {
  ...
  FreeSpritePaletteByTag(TAG_ITEM_ICON);
  sScrollableMultichoice_ItemSpriteId =
      AddItemIconSprite(TAG_ITEM_ICON, TAG_ITEM_ICON, item);
...
static const u16 sBattleFrontier_TutorMoves1[] = {
    MOVE_SOFT_BOILED,  MOVE_SEISMIC_TOSS, MOVE_DREAM_EATER, MOVE_MEGA_PUNCH,
    MOVE_MEGA_KICK,    MOVE_BODY_SLAM,    MOVE_ROCK_SLIDE,  MOVE_COUNTER,
    MOVE_THUNDER_WAVE, MOVE_SWORDS_DANCE};

static const u16 sBattleFrontier_TutorMoves2[] = {
    MOVE_DEFENSE_CURL,  MOVE_SNORE,     MOVE_MUD_SLAP, MOVE_SWIFT,
  ...
  if (menu == SCROLL_MULTI_BF_MOVE_TUTOR_1 ||
    ...
    if (gSpecialVar_0x8006 == 0) {
      sTutorMoveAndElevatorWindowId =
          AddWindow(&sBattleFrontierTutor_WindowTemplate);
      SetStandardWindowBorderStyle(sTutorMoveAndElevatorWindowId, FALSE);
    }
...
static void ShowBattleFrontierTutorMoveDescription(u8 menu, u16 selection) {
  ...
  if (menu == SCROLL_MULTI_BF_MOVE_TUTOR_1 ||
      menu == SCROLL_MULTI_BF_MOVE_TUTOR_2) {
    FillWindowPixelRect(sTutorMoveAndElevatorWindowId, PIXEL_FILL(1), 0, 0, 96,
                        48);
    if (menu == SCROLL_MULTI_BF_MOVE_TUTOR_2)
      AddTextPrinterParameterized(
          sTutorMoveAndElevatorWindowId, FONT_NORMAL,
  ...
  if (taskId != TASK_NONE) {
    ...
    for (i = 0; i < MAX_SCROLL_MULTI_ON_SCREEN; i++)
      AddTextPrinterParameterized5(
          task->tWindowId, FONT_NORMAL,
          sScrollableMultichoiceOptions[gSpecialVar_0x8004][scrollOffset + i],
          10, i * 16, TEXT_SKIP_DRAW, NULL, 0, 0);

    AddTextPrinterParameterized(task->tWindowId, FONT_NORMAL,
                                gText_SelectorArrow, 0, selectedRow * 16,
                                TEXT_SKIP_DRAW, NULL);
...
static void Task_DeoxysRockInteraction(u8 taskId) {
  static const u8 sStoneMaxStepCounts[DEOXYS_ROCK_LEVELS - 1] = {4, 8, 8, 8, 4,
                                                                 4, 4, 6, 3, 3};
...
static void ChangeDeoxysRockLevel(u8 rockLevel) {
  u8 objectEventId;
  LoadPalette(&sDeoxysRockPalettes[rockLevel], OBJ_PLTT_ID(ROCK_PAL_ID),
              PLTT_SIZEOF(4));
  TryGetObjectEventIdByLocalIdAndMap(
      LOCALID_BIRTH_ISLAND_EXTERIOR_ROCK, gSaveBlock1Ptr->location.mapNum,
  ...
  SetObjEventTemplateCoords(LOCALID_BIRTH_ISLAND_EXTERIOR_ROCK,
                            sDeoxysRockCoords[rockLevel][0],
                            sDeoxysRockCoords[rockLevel][1]);
}
...
void SetDeoxysRockPalette(void) {
  LoadPalette(&sDeoxysRockPalettes[(u8)VarGet(VAR_DEOXYS_ROCK_LEVEL)],
              OBJ_PLTT_ID(ROCK_PAL_ID), PLTT_SIZEOF(4));
...
bool8 IsDestinationBoxFull(void) {
  ...
  do {
    for (i = 0; i < IN_BOX_COUNT; i++) {
      if (GetBoxMonData(GetBoxedMonPtr(box, i), MON_DATA_SPECIES, 0) ==
          SPECIES_NONE) {
...
// Saves the map name for the current abnormal weather location in gStringVar1,
// then returns TRUE if the weather is for Kyogre, and FALSE if it's for
// Groudon.
bool32 GetAbnormalWeatherMapNameAndType(void) {
  static const u8 sAbnormalWeatherMapNumbers[] = {
      MAP_NUM(ROUTE114), MAP_NUM(ROUTE114), MAP_NUM(ROUTE115),
...
bool8 AbnormalWeatherHasExpired(void) {
  ...
  if (++steps > 999) {
    VarSet(VAR_ABNORMAL_WEATHER_STEP_COUNTER, 0);
    if (gSaveBlock1Ptr->location.mapGroup ==
        MAP_GROUP(UNDERWATER_MARINE_CAVE)) {
    ...
    if (gSaveBlock1Ptr->location.mapNum ==
            sAbnormalWeatherMapNumbers[abnormalWeather - 1] &&
...
u32 GetMartEmployeeObjectEventId(void) {
  static const u8 sPokeMarts[][3] = {
      {MAP_GROUP(OLDALE_TOWN_MART), MAP_NUM(OLDALE_TOWN_MART),
       LOCALID_OLDALE_MART_CLERK},
      {MAP_GROUP(LAVARIDGE_TOWN_MART), MAP_NUM(LAVARIDGE_TOWN_MART),
       LOCALID_LAVARIDGE_MART_CLERK},
      {MAP_GROUP(FALLARBOR_TOWN_MART), MAP_NUM(FALLARBOR_TOWN_MART),
       LOCALID_FALLARBOR_MART_CLERK},
      {MAP_GROUP(VERDANTURF_TOWN_MART), MAP_NUM(VERDANTURF_TOWN_MART),
       LOCALID_VERDANTURF_MART_CLERK},
      {MAP_GROUP(PETALBURG_CITY_MART), MAP_NUM(PETALBURG_CITY_MART),
       LOCALID_PETALBURG_MART_CLERK},
      {MAP_GROUP(SLATEPORT_CITY_MART), MAP_NUM(SLATEPORT_CITY_MART),
       LOCALID_SLATEPORT_MART_CLERK},
      {MAP_GROUP(MAUVILLE_CITY_MART), MAP_NUM(MAUVILLE_CITY_MART),
       LOCALID_MAUVILLE_MART_CLERK},
      {MAP_GROUP(RUSTBORO_CITY_MART), MAP_NUM(RUSTBORO_CITY_MART),
       LOCALID_RUSTBORO_MART_CLERK},
      {MAP_GROUP(FORTREE_CITY_MART), MAP_NUM(FORTREE_CITY_MART),
       LOCALID_FORTREE_MART_CLERK},
      {MAP_GROUP(MOSSDEEP_CITY_MART), MAP_NUM(MOSSDEEP_CITY_MART),
       LOCALID_MOSSDEEP_MART_CLERK},
      {MAP_GROUP(SOOTOPOLIS_CITY_MART), MAP_NUM(SOOTOPOLIS_CITY_MART),
       LOCALID_SOOTOPOLIS_MART_CLERK},
      {MAP_GROUP(BATTLE_FRONTIER_MART), MAP_NUM(BATTLE_FRONTIER_MART),
       LOCALID_BATTLE_FRONTIER_MART_CLERK}};
  ...
  case 1:
    if (IsLinkTaskFinished() == TRUE) {
      if (GetMultiplayerId() == 0) {
        ...
        SendBlock(BitmaskAllOtherLinkPlayers(), &gSpecialVar_0x8004,
                  sizeof(gSpecialVar_0x8004));
        gTasks[taskId].tState++;
      }
    }
    break;
  ...
  case 3:
    if (IsLinkTaskFinished() == TRUE) {
      if (GetMultiplayerId() != 0) {
        ...
        SendBlock(BitmaskAllOtherLinkPlayers(), &gSpecialVar_Result,
                  sizeof(gSpecialVar_Result));
        gTasks[taskId].tState++;
      }
    }
    break;
  ...
  if (tFrameTimer[tCurrentFrame] == 0) {
    for (y = 0; y < CURTAIN_HEIGHT; y++) {
      for (x = 0; x < CURTAIN_WIDTH; x++) {
        MapGridSetMetatileIdAt(
            gSaveBlock1Ptr->pos.x + x + MAP_OFFSET - 1,
            gSaveBlock1Ptr->pos.y + y + MAP_OFFSET - 3,
            (x + METATILE_BattlePike_CurtainFrames_Start) +
                (y * METATILE_ROW_WIDTH) +
                (tCurrentFrame * CURTAIN_HEIGHT * METATILE_ROW_WIDTH));
      }
    }
    DrawWholeMapView();
    tCurrentFrame++;
...
void GetBattlePyramidHint(void) {
  // gSpecialVar_0x8004 here is expected to be the current Battle Pyramid win
  // streak.
  gSpecialVar_Result = gSpecialVar_0x8004 / FRONTIER_STAGES_PER_CHALLENGE;
  gSpecialVar_Result -=
      (gSpecialVar_Result / TOTAL_PYRAMID_ROUNDS) * TOTAL_PYRAMID_ROUNDS;
}

// Used to avoid a potential softlock if the player respawns on Dewford with no
// way off
...
bool8 InPokemonCenter(void) {
  ...
  int i;
  u16 map = (gSaveBlock1Ptr->location.mapGroup << 8) +
            gSaveBlock1Ptr->location.mapNum;
...
/*  Summary of the Lilycove Trainer Fan Club, because it's a little messy
    ...
    There are initially 4 members of the Fan Club (+ an interviewer), none of
   whom are fans of the player After becoming the champion there will be 8
   members of the Fan Club, 3 of whom are automatically fans of the player After
   this point, if a club member is a fan of the player they will sit at the
   front table and comment on the player If they are not fans of the player,
   they will sit at the far table and can make comments about a different
   trainer (see BufferFanClubTrainerName)

    ## Gaining/losing fans
    After every link battle the player will gain a fan if they won, or lose a
   fan if they lost If the player has at least 3 fans, this is the only way to
   gain fans If the player has fewer than 3 fans, they may also gain fans by
   completing certain tasks enough times (see TryGainNewFanFromCounter) If the
   player has at least 5 fans, they can lose a fan every 12 real-time hours, or
   more often if the timer variable is reset (see TryLoseFansFromPlayTime) If
   the player has only 1 fan left it cannot be lost
    ...
    VAR_FANCLUB_FAN_COUNTER, a bitfield for tracking the fans
      ...
      Bits 9-16: Flags for each of the 8 club members, set to 1 when theyre a
   fan of the player and 0 when theyre not

    VAR_FANCLUB_LOSE_FAN_TIMER, a timer for when to lose fans
      Compared against playTimeHours. When theyre equal, a fan is ready to be
   lost For every fan thats lost this way 12 hours are added to the timer
...
#define FANCLUB_BITFIELD                                                       \
  (gSaveBlock1Ptr->vars[VAR_FANCLUB_FAN_COUNTER - VARS_START])
...
#define SET_TRAINER_FAN_CLUB_COUNTER(count)                                    \
  (FANCLUB_BITFIELD = (FANCLUB_BITFIELD & ~FANCLUB_COUNTER) | (count))
...
// Participating at Battle Tower or in a Secret Base battle increments the
// counter by 1
...
// Loop through the fan club members, and if theyre not a fan of the player
// there is a 50% chance for them to become a fan Stops when a fan is gained If
// no new fan was gained while looping, the last non-fan in the list becomes a
// fan If all the members are already fans of the player then this redundantly
// sets the first fan in the list to be a fan
static u16 PlayerGainRandomTrainerFan(void) {
  static const u8 sFanClubMemberIds[NUM_TRAINER_FAN_CLUB_MEMBERS] = {
      FANCLUB_MEMBER1, FANCLUB_MEMBER2, FANCLUB_MEMBER3, FANCLUB_MEMBER4,
      FANCLUB_MEMBER5, FANCLUB_MEMBER6, FANCLUB_MEMBER7, FANCLUB_MEMBER8};
...
// Loops through the fan club members, and if theyre a fan of the player there
// is a 50% chance for them to stop being a fan Stops if a fan is removed, or if
// the player has only one fan left If no fan was lost while looping, the last
// current fan in the list will stop being a fan
static u16 PlayerLoseRandomTrainerFan(void) {
  static const u8 sFanClubMemberIds[NUM_TRAINER_FAN_CLUB_MEMBERS] = {
      FANCLUB_MEMBER1, FANCLUB_MEMBER6, FANCLUB_MEMBER7, FANCLUB_MEMBER4,
      FANCLUB_MEMBER3, FANCLUB_MEMBER5, FANCLUB_MEMBER8, FANCLUB_MEMBER2};
...
// If the player has > 5 fans in the Trainer Fan Club, then lose 1 fan for every
// 12 hours since the last fan loss / timer reset
void TryLoseFansFromPlayTime(void) {
  u8 i = 0;
    ...
    while (TRUE) {
      if (GetNumFansOfPlayerInTrainerFanClub() < 5) {
        ...
        break;
                     ...
                     gSaveBlock1Ptr
                         ->vars[VAR_FANCLUB_LOSE_FAN_TIMER - VARS_START] <
                 12) {
        return;
      }
...
void BufferFanClubTrainerName(void) {
  ...
  BufferFanClubTrainerName_(&gSaveBlock1Ptr->linkBattleRecords,
                            whichLinkTrainer, whichNPCTrainer);
}

static void BufferFanClubTrainerName_(struct LinkBattleRecords *linkRecords,
                                      u8 whichLinkTrainer, u8 whichNPCTrainer) {
...
void TryChangeDeoxysForm(void) {
  u16 baseSpecies =
      GetMonData(&gPlayerParty[gSpecialVar_0x8004], MON_DATA_SPECIES);
  u16 targetSpecies;
  ...
  SetMonData(&gPlayerParty[gSpecialVar_0x8004], MON_DATA_SPECIES,
             &targetSpecies);
...
void CheckGotAllKantoStarters(void) {
  if (FlagGet(FLAG_RIVAL_GOT_BULBASAUR) && FlagGet(FLAG_RIVAL_GOT_CHARMANDER) &&
