...
static void LinkPartnerBufferRunCommand(void) {
  if (gBattleControllerExecFlags & gBitTable[gActiveBattler]) {
    if (gBattleBufferA[gActiveBattler][0] <
        ARRAY_COUNT(sLinkPartnerBufferCommands))
...
static void LinkPartnerHandleGetMonData(void) {
  ...
  if (gBattleBufferA[gActiveBattler][2] == 0) {
    size +=
        CopyLinkPartnerMonData(gBattlerPartyIndexes[gActiveBattler], monData);
  } else {
...
static void LinkPartnerHandleDrawTrainerPic(void) {
  ...
  } else if ((gLinkPlayers[GetBattlerMultiplayerId(gActiveBattler)].version &
              0xFF) == VERSION_RUBY ||
...
static void LinkPartnerHandlePause(void) { LinkPartnerBufferExecCompleted(); }
...
static void LinkPartnerHandleCmd23(void) { LinkPartnerBufferExecCompleted(); }
...
static void LinkPartnerHandlePlayBGM(void) { LinkPartnerBufferExecCompleted(); }

static void LinkPartnerHandleCmd32(void) { LinkPartnerBufferExecCompleted(); }
...
static void LinkPartnerHandleIntroTrainerBallThrow(void) {
  ...
  LoadCompressedPalette(gTrainerBackPicPaletteTable[trainerPicId].data,
