...
// desired, ensure that the name string is limited in size when it's copied from
// the saveblock to any Rfu-related fields (e.g. in SetHostRfuUsername). If
// wireless link functionality is not desired ignore or delete this warning.
#if RFU_USER_NAME_LENGTH < (PLAYER_NAME_LENGTH + 1)
#warning                                                                       \
    "The Wireless Adapter hardware expects a username of no more than 8 bytes."
#endif
...
static void rfu_STC_NI_receive_Receiver(u8, const struct RfuLocalStruct *,
                                        const u8 *);
...
u16 rfu_initializeAPI(u32 *APIBuffer, u16 buffByteSize,
  ...
  gRfuSlotStatusUNI[0] =
      (void *)APIBuffer +
      0x37c; // + sizeof(*gRfuSlotStatusNI[0]) * RFU_CHILD_MAX
  ...
  STWI_init_all((struct RfuIntrStruct *)&gRfuSlotStatusUNI[3][1],
                sioIntrTable_p, copyInterruptToRam);
  ...
  buffByteSizeMax =
      ((void *)rfu_REQ_changeMasterSlave - (void *)rfu_STC_fastCopy) /
      sizeof(u16);
  while (buffByteSizeMax-- != 0)
    *dst++ = *src++;
#else
  COPY((uintptr_t)&rfu_STC_fastCopy & ~1, gRfuFixed->fastCopyBuffer,
...
u8 *rfu_getSTWIRecvBuffer(void) { return (u8 *)gRfuFixed->STWIBuffer; }
...
void rfu_REQ_configSystem(u16 availSlotFlag, u8 maxMFrame, u8 mcTimer) {
  STWI_set_Callback_M(rfu_STC_REQ_callback);
  STWI_send_SystemConfigREQ((availSlotFlag & AVAIL_SLOT1) | 0x3C, maxMFrame,
                            mcTimer);
  if (mcTimer == 0) {
...
static void rfu_STC_readChildList(void) {
  ...
  for (data_p = &gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[4];
    ...
    if (bm_slot_id < RFU_CHILD_MAX &&
        !((gRfuLinkStatus->connSlotFlag >> bm_slot_id) & 1) &&
  ...
  if (reqResult == 0) {
    ...
    if (gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[7] == 0) {
      ...
      if (!(bm_slot_flag & gRfuLinkStatus->connSlotFlag)) {
        ...
        for (i = 0; i < RFU_CHILD_MAX; ++i) {
          if (gRfuLinkStatus->partner[i].id == gRfuStatic->tryPid) {
            if (gRfuLinkStatus->findParentCount != 0) {
              ...
              CpuCopy16(&gRfuLinkStatus->partner[i], &target_local,
                        sizeof(struct RfuTgtData));
              CpuFill16(0, gRfuLinkStatus->partner,
                        sizeof(gRfuLinkStatus->partner));
              gRfuLinkStatus->findParentCount = 0;
            } else {
        ...
        if (i < RFU_CHILD_MAX) {
          CpuCopy16(target_p, &gRfuLinkStatus->partner[slot],
                    sizeof(struct RfuTgtData));
...
void rfu_REQ_endConnectParent(void) {
  ...
  if (gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[6] < 4)
    gRfuStatic->linkEmergencyFlag[gRfuFixed->STWIBuffer->rxPacketAlloc
                                      .rfuPacket8.data[6]] = 0;
}
...
u16 rfu_REQBN_watchLink(u16 reqCommandId, u8 *bmLinkLossSlot,
  ...
  u8 *packet_p;
  u8 to_req_disconnect, newLinkLossFlag, num_packets, connSlotFlag,
      to_disconnect;
      ...
      if (reasonMaybe == 1 &&
        ...
        if (gRfuLinkStatus->strength[i] == 0) {
          if (gRfuLinkStatus->parentChild == MODE_PARENT) {
            ...
            STWI_send_SystemStatusREQ();
            if (STWI_poll_CommandEnd() == 0) {
              if (gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[7] ==
                  0) {
                *bmLinkLossSlot |= newLinkLossFlag;
                *linkLossReason = REASON_LINK_LOSS;
              } else {
      ...
      if (gRfuLinkStatus->parentChild == MODE_PARENT &&
        ...
        if (newLinkLossFlag & gRfuLinkStatus->linkLossSlotFlag) {
          ...
          if (!((gRfuLinkStatus->connSlotFlag |
            ...
            for (packet_p += 8; num_packets != 0;
                 packet_p += 4, --num_packets) {
...
static void rfu_STC_removeLinkData(u8 bmConnectedPartnerId, u8 bmDisconnect) {
...
#endif
  if ((gRfuLinkStatus->connSlotFlag & bmLinkLossFlag) &&
      gRfuLinkStatus->connCount != 0)
    --gRfuLinkStatus->connCount;
  ...
  if (gRfuLinkStatus->parentChild == MODE_CHILD &&
    ...
    gRfuLinkStatus->parentChild = MODE_NEUTRAL;
  if (bmDisconnect) {
    CpuFill16(0, &gRfuLinkStatus->partner[bmConnectedPartnerId],
              sizeof(struct RfuTgtData));
...
static void rfu_CB_disconnect(u8 reqCommand, u16 reqResult) {
  ...
  gRfuStatic->recoveryBmSlot &=
      gRfuLinkStatus->connSlotFlag | gRfuLinkStatus->linkLossSlotFlag;
  gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[8] =
      gRfuStatic->recoveryBmSlot;
  if (reqResult == 0) {
    for (i = 0; i < RFU_CHILD_MAX; ++i) {
      bm_slot_flag = 1 << i;
...
void rfu_REQ_CHILD_startConnectRecovery(u8 bmRecoverySlot) {
  ...
  STWI_send_CPR_StartREQ(gRfuLinkStatus->partner[i].id, gRfuLinkStatus->my.id,
                         bmRecoverySlot);
}
...
static void rfu_CB_CHILD_pollConnectRecovery(u8 reqCommand, u16 reqResult) {
  ...
  if (reqResult == 0 &&
    ...
    for (i = 0; i < RFU_CHILD_MAX; ++i) {
      ...
      if (gRfuStatic->recoveryBmSlot & bm_slot_flag &
          rfuLinkStatus->linkLossSlotFlag) {
        gRfuLinkStatus->connSlotFlag |= bm_slot_flag;
        gRfuLinkStatus->linkLossSlotFlag &= ~bm_slot_flag;
        ++gRfuLinkStatus->connCount;
        gRfuStatic->linkEmergencyFlag[i] = 0;
...
void rfu_clearAllSlot(void) {
  ...
  for (i = 0; i < RFU_CHILD_MAX; ++i) {
    ...
    CpuFill16(0, gRfuSlotStatusUNI[i],
              sizeof(struct UNISend) + sizeof(struct UNIRecv));
...
static void rfu_STC_releaseFrame(u8 bm_slot_id, u8 send_recv,
  ...
  if (!(gRfuStatic->flags & 0x80)) {
    ...
    if (send_recv == 0)
      gRfuLinkStatus->remainLLFrameSizeChild[bm_slot_id] +=
          NI_comm->payloadSize;
...
u16 rfu_clearSlot(u8 connTypeFlag, u8 slotStatusIndex) {
  ...
  if (slotStatusIndex >= RFU_CHILD_MAX)
    return ERR_SLOT_NO;
  if (!(connTypeFlag &
        (TYPE_UNI_SEND | TYPE_UNI_RECV | TYPE_NI_SEND | TYPE_NI_RECV)))
    ...
    if (slotStatusUNI->send.state & SLOT_BUSY_FLAG) {
      if (!(gRfuStatic->flags & 0x80))
        gRfuLinkStatus->remainLLFrameSizeParent +=
            3 + (u8)slotStatusUNI->send.payloadSize;
      else
  ...
  if (connTypeFlag & TYPE_UNI_RECV) {
    CpuFill16(0, &gRfuSlotStatusUNI[slotStatusIndex]->recv,
              sizeof(struct UNIRecv));
  }
  REG_IME = imeBak;
  return 0;
}
...
u16 rfu_NI_CHILD_setSendGameName(u8 slotNo, u8 subFrameSize) {
  return rfu_STC_setSendData_org(64, 1 << slotNo, subFrameSize,
                                 &gRfuLinkStatus->my.serialNo, 26);
}

static u16 rfu_STC_setSendData_org(u8 ni_or_uni, u8 bmSendSlot, u8 subFrameSize,
                                   const void *src, u32 dataSize) {
  ...
  for (bm_slot_id = 0;
       bm_slot_id < RFU_CHILD_MAX && !((bmSendSlot >> bm_slot_id) & 1);
       ++bm_slot_id)
    ;
  if (gRfuLinkStatus->parentChild == MODE_PARENT)
...
#ifdef UBFIX
  if ((llFrameSize_p && subFrameSize > *llFrameSize_p) ||
      subFrameSize <= frameSize)
#else
  ...
  if (connType == 0x20) {
    ...
    if (connType == 16) {
      ...
      gRfuLinkStatus->sendSlotUNIFlag &=
          ~gRfuSlotStatusUNI[slotStatusIndex]->send.bmSlot;
...
u16 rfu_UNI_changeAndReadySendData(u8 slotStatusIndex, const void *src,
  ...
  if (gRfuLinkStatus->parentChild == MODE_PARENT) {
    ...
    frameEnd =
        gRfuLinkStatus->remainLLFrameSizeParent + (u8)UNI_send->payloadSize;
  } else {
  ...
  if (gRfuLinkStatus->parentChild != MODE_NEUTRAL) {
    if (gRfuLinkStatus->parentChild == MODE_PARENT &&
      ...
      if (gRfuLinkStatus->LLFReadyFlag) {
        ...
        if (clockChangeFlag) {
          STWI_send_DataTxAndChangeREQ(gRfuFixed->LLFBuffer,
                                       gRfuStatic->totalPacketSize + 4);
          return;
        }
        STWI_send_DataTxREQ(gRfuFixed->LLFBuffer,
                            gRfuStatic->totalPacketSize + 4);
      }
    }
    if (clockChangeFlag) {
  ...
  if (gRfuLinkStatus->parentChild != MODE_NEUTRAL &&
    ...
    for (i = 0; i < RFU_CHILD_MAX; ++i) {
      ...
      if (gRfuSlotStatusNI[i]->send.state & SLOT_BUSY_FLAG)
        currSize =
            rfu_STC_NI_constructLLSF(i, &llf_p, &gRfuSlotStatusNI[i]->send);
      if (gRfuSlotStatusNI[i]->recv.state & SLOT_BUSY_FLAG)
        currSize +=
            rfu_STC_NI_constructLLSF(i, &llf_p, &gRfuSlotStatusNI[i]->recv);
...
static u16 rfu_STC_NI_constructLLSF(u8 bm_slot_id, u8 **dest_pp,
                                    struct NIComm *NI_comm) {
  u16 size;
  u32 frame;
  u8 i;
  ...
  if (NI_comm->state & SLOT_RECV_FLAG) {
    ...
    if (NI_comm->now_p[NI_comm->phase] + NI_comm->payloadSize >
        (const u8 *)NI_comm->src + NI_comm->dataSize)
      size = (const u8 *)NI_comm->src + NI_comm->dataSize -
             NI_comm->now_p[NI_comm->phase];
    else
  ...
  frame = (NI_comm->state & 0xF) << llsf->slotStateShift |
          NI_comm->ack << llsf->ackShift | NI_comm->phase << llsf->phaseShift |
          NI_comm->n[NI_comm->phase] << llsf->nShift | size;
  if (gRfuLinkStatus->parentChild == MODE_PARENT)
...
static u16 rfu_STC_UNI_constructLLSF(u8 bm_slot_id, u8 **dest_p) {
  ...
  frame =
      (UNI_send->state & 0xF) << llsf->slotStateShift | UNI_send->payloadSize;
  if (gRfuLinkStatus->parentChild == MODE_PARENT)
...
static void rfu_CB_recvData(u8 reqCommand, u16 reqResult) {
  ...
  if (reqResult == 0 &&
      gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[1]) {
...
static void rfu_STC_PARENT_analyzeRecvPacket(void) {
  ...
  for (bm_slot_id = 0; bm_slot_id < RFU_CHILD_MAX; ++bm_slot_id) {
    if (frame_counts[bm_slot_id]) {
      ...
      do {
        u8 analyzed_frames =
            rfu_STC_analyzeLLSF(bm_slot_id, packet_p, *frames_p);
...
static void rfu_STC_CHILD_analyzeRecvPacket(void) {
  ...
  frames_remaining =
      *(u16 *)&gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[4] & 0x7F;
  packet_p = &gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[8];
...
static u16 rfu_STC_analyzeLLSF(u8 slot_id, const u8 *src, u16 last_frame) {
  ...
  llsf_p =
      &llsf_struct[~gRfuLinkStatus->parentChild & (MODE_NEUTRAL & MODE_PARENT)];
  ...
  for (i = 0; i < llsf_p->frameSize; ++i)
    frames |= *src++ << 8 * i;
  llsf_NI.recvFirst =
      (frames >> llsf_p->recvFirstShift) & llsf_p->recvFirstMask;
  llsf_NI.connSlotFlag =
      (frames >> llsf_p->connSlotFlagShift) & llsf_p->connSlotFlagMask;
  llsf_NI.slotState =
      (frames >> llsf_p->slotStateShift) & llsf_p->slotStateMask;
...
static void rfu_STC_UNI_receive(u8 bm_slot_id,
                                const struct RfuLocalStruct *llsf_NI,
                                const u8 *src) {
  u8 *dest;
  u32 size;
...
static void rfu_STC_NI_receive_Sender(u8 NI_slot, u8 bm_flag,
                                      const struct RfuLocalStruct *llsf_NI,
                                      const u8 *data_p) {
  ...
  if ((NI_comm->recvAckFlag[llsf_NI->phase] & NI_comm->bmSlot) ==
      NI_comm->bmSlot) {
...
static void rfu_STC_NI_receive_Receiver(u8 bm_slot_id,
                                        const struct RfuLocalStruct *llsf_NI,
                                        const u8 *data_p) {
  u16 imeBak;
  u32 state_check = 0;
  ...
  if (state_check != 0) {
    if (llsf_NI->n == ((recvSlot->n[llsf_NI->phase] + 1) & 3)) {
      gRfuFixed->fastCopyPtr(&data_p, (u8 **)&recvSlot->now_p[llsf_NI->phase],
                             llsf_NI->frame);
      if (recvSlot->state == SLOT_STATE_RECEIVING)
...
static void rfu_STC_NI_initSlot_asRecvControllData(u8 bm_slot_id,
                                                   struct NIComm *NI_comm) {
  u8 *llFrameSize_p;
  u32 llFrameSize;
  u8 bm_slot_flag;
...
static void rfu_STC_NI_initSlot_asRecvDataEntity(u8 bm_slot_id,
                                                 struct NIComm *NI_comm) {
