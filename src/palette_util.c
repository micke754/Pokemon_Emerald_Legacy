...
// this file handles fading the palettes for the color/icon selections on the
// Roulette wheel but it also handles the "pulse blend" effect of Mirage Tower
...
u8 RouletteFlash_Add(struct RouletteFlashUtil *flash, u8 id,
                     const struct RouletteFlashSettings *settings) {
...
void InitPulseBlend(struct PulseBlend *pulseBlend) {
  ...
  memset(&pulseBlend->pulseBlendPalettes, 0,
         sizeof(pulseBlend->pulseBlendPalettes));
  for (; i < 16; i++)
    pulseBlend->pulseBlendPalettes[i].paletteSelector = i;
}

int InitPulseBlendPaletteSettings(struct PulseBlend *pulseBlend,
                                  const struct PulseBlendSettings *settings) {
  u8 i = 0;
...
static void
ClearPulseBlendPalettesSettings(struct PulseBlendPalette *pulseBlendPalette) {
  u16 i;
  ...
  memset(&pulseBlendPalette->pulseBlendSettings, 0,
         sizeof(pulseBlendPalette->pulseBlendSettings));
  pulseBlendPalette->blendCoeff = 0;
  pulseBlendPalette->fadeDirection = 0;
...
void UnloadUsedPulseBlendPalettes(struct PulseBlend *pulseBlend,
                                  u16 pulseBlendPaletteSelector,
                                  u8 multiSelection) {
  u16 i = 0;
  ...
  if (!multiSelection) {
    ...
    for (i = 0; i < 16; i++) {
      if ((pulseBlendPaletteSelector & 1) &&
...
void MarkUsedPulseBlendPalettes(struct PulseBlend *pulseBlend,
                                u16 pulseBlendPaletteSelector,
                                u8 multiSelection) {
  u8 i = 0;
...
void UnmarkUsedPulseBlendPalettes(struct PulseBlend *pulseBlend,
                                  u16 pulseBlendPaletteSelector,
                                  u8 multiSelection) {
  u16 i;
...
void UpdatePulseBlend(struct PulseBlend *pulseBlend) {
  ...
  if (pulseBlend->usedPulseBlendPalettes) {
    for (i = 0; i < 16; i++) {
      ...
      if ((!pulseBlendPalette->available && pulseBlendPalette->inUse) &&
          (!gPaletteFade.active ||
           !pulseBlendPalette->pulseBlendSettings.unk7_7)) {
        if (--pulseBlendPalette->delayCounter == 0xFF) {
          ...
          case 0: // Fade all the way to the max blend amount, then wrap around
            // BUG: This comparison will never be true for maxBlendCoeff values
            // that are >= 8. This is because maxBlendCoeff is a signed 4-bit
            // field, but blendCoeff is an unsigned 4-bit field. This code is
            // never reached, anyway, so the bug is not observable in vanilla
            // gameplay.
          ...
          case 1: // Fade in and out
            if (pulseBlendPalette->fadeDirection) {
              ...
              u8 max =
                  (pulseBlendPalette->pulseBlendSettings.maxBlendCoeff - 1) &
                  0xF;
            ...
            break;
                    ...
                    if (pulseBlendPalette->pulseBlendSettings.numFadeCycles !=
                            0xFF &&
                        pulseBlendPalette->fadeCycleCounter ==
                            pulseBlendPalette->pulseBlendSettings.numFadeCycles)
                      UnmarkUsedPulseBlendPalettes(
                          pulseBlend, pulseBlendPalette->paletteSelector,
                          FALSE);
        }
      }
    }
...
void FillTilemapRect(u16 *dest, u16 value, u8 left, u8 top, u8 width,
                     u8 height) {
  u16 *_dest;
  u8 i;
  u8 j;
...
static void UNUSED FillTilemapRect_Unused(void *dest, u16 value, u8 left,
                                          u8 top, u8 width, u8 height) {
  u8 i, j;
  u8 x, y;
...
static void UNUSED SetTilemapRect_Unused(void *dest, const u16 *src, u8 left,
                                         u8 top, u8 width, u8 height) {
  u8 i, j;
  u8 x, y;
  const u16 *_src;
