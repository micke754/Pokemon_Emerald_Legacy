...
STATIC_ASSERT(sizeof(struct HallofFameTeam) * HALL_OF_FAME_MAX_TEAMS <=
                  SECTOR_DATA_SIZE * NUM_HOF_SECTORS,
              HallOfFameFreeSpace);
...
static void HallOfFame_PrintMonInfo(struct HallofFameMon *currMon, u8 unused1,
                                    u8 unused2);
...
static const struct WindowTemplate sHof_WindowTemplate = {.bg = 0,
                                                          .tilemapLeft = 2,
                                                          .tilemapTop = 2,
                                                          .width = 14,
                                                          .height = 8,
...
static const u8 sUnusedTextColors[4] = {TEXT_COLOR_RED, TEXT_COLOR_LIGHT_RED,
...
static const union AnimCmd sAnim_PinkConfettiA[] = {ANIMCMD_FRAME(0, 30),
                                                    ANIMCMD_END};

static const union AnimCmd sAnim_RedConfettiA[] = {ANIMCMD_FRAME(1, 30),
                                                   ANIMCMD_END};

static const union AnimCmd sAnim_BlueConfettiA[] = {ANIMCMD_FRAME(2, 30),
                                                    ANIMCMD_END};

static const union AnimCmd sAnim_RedConfettiB[] = {ANIMCMD_FRAME(3, 30),
                                                   ANIMCMD_END};

static const union AnimCmd sAnim_BlueConfettiB[] = {ANIMCMD_FRAME(4, 30),
                                                    ANIMCMD_END};

static const union AnimCmd sAnim_YellowConfettiA[] = {ANIMCMD_FRAME(5, 30),
                                                      ANIMCMD_END};

static const union AnimCmd sAnim_WhiteConfettiA[] = {ANIMCMD_FRAME(6, 30),
                                                     ANIMCMD_END};

static const union AnimCmd sAnim_GreenConfettiA[] = {ANIMCMD_FRAME(7, 30),
                                                     ANIMCMD_END};

static const union AnimCmd sAnim_PinkConfettiB[] = {ANIMCMD_FRAME(8, 30),
                                                    ANIMCMD_END};

static const union AnimCmd sAnim_BlueConfettiC[] = {ANIMCMD_FRAME(9, 30),
                                                    ANIMCMD_END};

static const union AnimCmd sAnim_YellowConfettiB[] = {ANIMCMD_FRAME(10, 30),
                                                      ANIMCMD_END};

static const union AnimCmd sAnim_WhiteConfettiB[] = {ANIMCMD_FRAME(11, 30),
                                                     ANIMCMD_END};

static const union AnimCmd sAnim_GreenConfettiB[] = {ANIMCMD_FRAME(12, 30),
                                                     ANIMCMD_END};

static const union AnimCmd sAnim_PinkConfettiC[] = {ANIMCMD_FRAME(13, 30),
                                                    ANIMCMD_END};

static const union AnimCmd sAnim_RedConfettiC[] = {ANIMCMD_FRAME(14, 30),
                                                   ANIMCMD_END};

static const union AnimCmd sAnim_YellowConfettiC[] = {ANIMCMD_FRAME(15, 30),
                                                      ANIMCMD_END};

static const union AnimCmd sAnim_WhiteConfettiC[] = {ANIMCMD_FRAME(16, 30),
                                                     ANIMCMD_END};

static const union AnimCmd *const sAnims_Confetti[] = {
    sAnim_PinkConfettiA,   sAnim_RedConfettiA,    sAnim_BlueConfettiA,
    sAnim_RedConfettiB,    sAnim_BlueConfettiB,   sAnim_YellowConfettiA,
    sAnim_WhiteConfettiA,  sAnim_GreenConfettiA,  sAnim_PinkConfettiB,
    sAnim_BlueConfettiC,   sAnim_YellowConfettiB, sAnim_WhiteConfettiB,
    sAnim_GreenConfettiB,  sAnim_PinkConfettiC,   sAnim_RedConfettiC,
...
static const u16 sHallOfFame_Pal[] =
    INCBIN_U16("graphics/misc/japanese_hof.gbapal");

static const u32 sHallOfFame_Gfx[] =
    INCBIN_U32("graphics/misc/japanese_hof.4bpp.lz");

static const struct HallofFameMon sDummyFameMon = {.tid = 0x3EA03EA,
                                                   .personality = 0,
...
static void Task_Hof_InitTeamSaveData(u8 taskId) {
  ...
  struct HallofFameTeam *lastSavedTeam =
      (struct HallofFameTeam *)(gDecompressionBuffer);
  ...
  if (i >= HALL_OF_FAME_MAX_TEAMS) {
    struct HallofFameTeam *afterTeam =
        (struct HallofFameTeam *)(gDecompressionBuffer);
    struct HallofFameTeam *beforeTeam =
        (struct HallofFameTeam *)(gDecompressionBuffer);
    afterTeam++;
    for (i = 0; i < HALL_OF_FAME_MAX_TEAMS - 1;
         i++, beforeTeam++, afterTeam++) {
  ...
  *lastSavedTeam = *sHofMonPtr;
  ...
  AddTextPrinterParameterized2(0, FONT_NORMAL, gText_SavingDontTurnOffPower, 0,
                               NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE,
...
static void Task_Hof_TrySaveData(u8 taskId) {
  ...
  if (TrySavingData(SAVE_HALL_OF_FAME) == SAVE_STATUS_ERROR &&
...
static void Task_Hof_DisplayMon(u8 taskId) {
  ...
  spriteId = CreateMonPicSprite_Affine(
      currMon->species, currMon->tid, currMon->personality,
      MON_PIC_AFFINE_FRONT, startX, startY, currMonId, TAG_NONE);
...
static void Task_Hof_TryDisplayAnotherMon(u8 taskId) {
  ...
  if (gTasks[taskId].tFrameCount != 0) {
    ...
    sHofFadePalettes |=
        (0x10000
         << gSprites[gTasks[taskId].tMonSpriteId(currPokeID)].oam.paletteNum);
    if (gTasks[taskId].tDisplayedMonId < PARTY_SIZE - 1 &&
        currMon[1].species !=
            SPECIES_NONE) // there is another Pokémon to display
    {
...
static void Task_Hof_DoConfetti(u8 taskId) {
  if (gTasks[taskId].tFrameCount != 0) {
    ...
    if ((gTasks[taskId].tFrameCount & 3) == 0 &&
        gTasks[taskId].tFrameCount > 110)
...
static void Task_Hof_DisplayPlayer(u8 taskId) {
  ...
  gTasks[taskId].tPlayerSpriteID = CreateTrainerPicSprite(
      ...
      TRUE, 120, 72, 6, TAG_NONE);
  AddWindow(&sHof_WindowTemplate);
  LoadWindowGfx(1, gSaveBlock2Ptr->optionsWindowFrameType, 0x21D,
                BG_PLTT_ID(13));
...
static void Task_Hof_WaitAndPrintPlayerInfo(u8 taskId) {
  if (gTasks[taskId].tFrameCount != 0) {
    ...
    DrawDialogueFrame(0, FALSE);
    AddTextPrinterParameterized2(0, FONT_NORMAL, gText_LeagueChamp, 0, NULL,
...
static void Task_HofPC_DrawSpritesPrintText(u8 taskId) {
  ...
  for (i = 0; i < PARTY_SIZE; i++, currMon++) {
    if (currMon->species != 0) {
      ...
      spriteId = CreateMonPicSprite_HandleDeoxys(currMon->species, currMon->tid,
                                                 currMon->personality, TRUE,
                                                 posX, posY, i, TAG_NONE);
  ...
  ConvertIntToDecimalStringN(gStringVar1, gTasks[taskId].tCurrPageNo,
...
static void Task_HofPC_PrintMonInfo(u8 taskId) {
  ...
  sHofFadePalettes =
      (0x10000 << gSprites[currMonID].oam.paletteNum) ^ PALETTES_OBJECTS;
...
static void Task_HofPC_HandleInput(u8 taskId) {
  ...
  } else if (JOY_NEW(B_BUTTON)) // turn off hall of fame PC
  {
    ...
    gTasks[taskId].func = Task_HofPC_HandlePaletteOnExit;
             ...
             gTasks[taskId].tCurrMonId != 0) // change mon -1
  {
    gTasks[taskId].tCurrMonId--;
    gTasks[taskId].func = Task_HofPC_PrintMonInfo;
  } else if (JOY_NEW(DPAD_DOWN) &&
             gTasks[taskId].tCurrMonId <
                 gTasks[taskId].tMonNo - 1) // change mon +1
  {
    gTasks[taskId].tCurrMonId++;
    gTasks[taskId].func = Task_HofPC_PrintMonInfo;
  }
...
static void Task_HofPC_PrintDataIsCorrupted(u8 taskId) {
  ...
  DrawDialogueFrame(0, FALSE);
  AddTextPrinterParameterized2(0, FONT_NORMAL, gText_HOFCorrupted, 0, NULL,
                               TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE,
                               TEXT_COLOR_LIGHT_GRAY);
  CopyWindowToVram(0, COPYWIN_FULL);
...
static void HallOfFame_PrintWelcomeText(u8 unusedPossiblyWindowId, u8 unused2) {
  ...
  AddTextPrinterParameterized3(
      0, FONT_NORMAL,
      GetStringCenterAlignXOffset(FONT_NORMAL, gText_WelcomeToHOF, 0xD0), 1,
      sMonInfoTextColors, 0, gText_WelcomeToHOF);
  CopyWindowToVram(0, COPYWIN_FULL);
}
...
static void HallOfFame_PrintMonInfo(struct HallofFameMon *currMon, u8 unused1,
  ...
  if (currMon->species != SPECIES_EGG) {
    ...
    stringPtr[0] = EOS;
    AddTextPrinterParameterized3(0, FONT_NORMAL, 0x10, 1, sMonInfoTextColors,
                                 TEXT_SKIP_DRAW, text);
  }
  ...
  if (currMon->species == SPECIES_EGG) {
    ...
    AddTextPrinterParameterized3(0, FONT_NORMAL, width, 1, sMonInfoTextColors,
                                 TEXT_SKIP_DRAW, text);
    CopyWindowToVram(0, COPYWIN_FULL);
  } else {
    ...
    if (currMon->species != SPECIES_NIDORAN_M &&
        currMon->species != SPECIES_NIDORAN_F) {
      switch (GetGenderFromSpeciesAndPersonality(currMon->species,
                                                 currMon->personality)) {
      case MON_MALE:
    ...
    stringPtr[0] = EOS;
    AddTextPrinterParameterized3(0, FONT_NORMAL, 0x80, 1, sMonInfoTextColors,
                                 TEXT_SKIP_DRAW, text);

    stringPtr = StringCopy(text, gText_Level);
    ConvertIntToDecimalStringN(stringPtr, currMon->lvl,
                               STR_CONV_MODE_LEFT_ALIGN, 3);
    AddTextPrinterParameterized3(0, FONT_NORMAL, 0x24, 0x11, sMonInfoTextColors,
                                 TEXT_SKIP_DRAW, text);

    stringPtr = StringCopy(text, gText_IDNumber);
    ConvertIntToDecimalStringN(stringPtr, (u16)(currMon->tid),
                               STR_CONV_MODE_LEADING_ZEROS, 5);
    AddTextPrinterParameterized3(0, FONT_NORMAL, 0x68, 0x11, sMonInfoTextColors,
                                 TEXT_SKIP_DRAW, text);
...
static void HallOfFame_PrintPlayerInfo(u8 unused1, u8 unused2) {
  ...
  if (FlagGet(FLAG_NUZLOCKE)) {
    AddTextPrinterParameterized3(1, FONT_NORMAL, 12, 1, sPlayerInfoTextColors,
                                 TEXT_SKIP_DRAW, gText_LegacyHardcore);
  } else if (FlagGet(FLAG_HARD)) {
    AddTextPrinterParameterized3(1, FONT_NORMAL, 24, 1, sPlayerInfoTextColors,
                                 TEXT_SKIP_DRAW, gText_LegacyHard);
  } else {
    AddTextPrinterParameterized3(1, FONT_NORMAL, 15, 1, sPlayerInfoTextColors,
                                 TEXT_SKIP_DRAW, gText_EmeraldLegacy);
  }

  AddTextPrinterParameterized3(1, FONT_NORMAL, 0, 0x11, sPlayerInfoTextColors,
                               TEXT_SKIP_DRAW, gText_Name);

  width =
      GetStringRightAlignXOffset(FONT_NORMAL, gSaveBlock2Ptr->playerName, 0x70);
  AddTextPrinterParameterized3(1, FONT_NORMAL, width, 0x11,
                               sPlayerInfoTextColors, TEXT_SKIP_DRAW,
                               gSaveBlock2Ptr->playerName);

  trainerId = (gSaveBlock2Ptr->playerTrainerId[0]) |
              (gSaveBlock2Ptr->playerTrainerId[1] << 8);
  AddTextPrinterParameterized3(1, FONT_NORMAL, 0, 0x21, sPlayerInfoTextColors,
                               0, gText_IDNumber);
  ...
  AddTextPrinterParameterized3(1, FONT_NORMAL, width, 0x21,
                               sPlayerInfoTextColors, TEXT_SKIP_DRAW, text);

  AddTextPrinterParameterized3(1, FONT_NORMAL, 0, 0x31, sPlayerInfoTextColors,
                               TEXT_SKIP_DRAW, gText_Time);
  ...
  width = GetStringRightAlignXOffset(FONT_NORMAL, text, 0x70);
  AddTextPrinterParameterized3(1, FONT_NORMAL, width, 0x31,
                               sPlayerInfoTextColors, TEXT_SKIP_DRAW, text);
...
// The end result is essentially the same, just a very different way of handling
// it
  ...
  case 1:
    if (tTimer != 0 && tTimer % 3 == 0) {
      ...
      id = ConfettiUtil_AddNew(&sOamData_Confetti, TAG_CONFETTI, TAG_CONFETTI,
                               Random() % DISPLAY_WIDTH, -(Random() % 8),
                               Random() % ARRAY_COUNT(sAnims_Confetti), id);
      if (id != 0xFF) {
