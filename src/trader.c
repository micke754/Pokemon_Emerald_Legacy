...
void CreateAvailableDecorationsMenu(u8 taskId) {
  ...
  struct MauvilleOldManTrader *trader = &gSaveBlock1Ptr->oldMan.trader;
  struct WindowTemplate windowTemplate = {.bg = 0,
                                          .tilemapLeft = 1,
                                          .tilemapTop = 1,
                                          .width = 10,
                                          .height = 10,
  ...
  for (i = 0; i < NUM_TRADER_ITEMS; i++) {
    ...
    if (trader->decorations[i] > NUM_DECORATIONS)
      ...
      curWidth = GetStringWidth(FONT_NORMAL,
                                gDecorations[trader->decorations[i]].name, 0);
  ...
  for (i = 0; i < NUM_TRADER_ITEMS; i++) {
    if (trader->decorations[i] > NUM_DECORATIONS)
      AddTextPrinterParameterized(tWindowId, FONT_NORMAL, gText_FiveMarks, 8,
                                  16 * i + 1, TEXT_SKIP_DRAW, NULL);
    else
      AddTextPrinterParameterized(tWindowId, FONT_NORMAL,
                                  gDecorations[trader->decorations[i]].name, 8,
                                  16 * i + 1, TEXT_SKIP_DRAW, NULL);
  }
  AddTextPrinterParameterized(tWindowId, FONT_NORMAL, gText_Exit, 8, 16 * i + 1,
                              TEXT_SKIP_DRAW, NULL);
...
void IsDecorationCategoryFull(void) {
  gSpecialVar_Result = FALSE;
  if (gDecorations[gSpecialVar_0x8004].category !=
          gDecorations[gSpecialVar_0x8006].category &&
      GetFirstEmptyDecorSlot(gDecorations[gSpecialVar_0x8004].category) == -1) {
    CopyDecorationCategoryName(gStringVar2,
                               gDecorations[gSpecialVar_0x8004].category);
    gSpecialVar_Result = TRUE;
  }
}
...
void TraderDoDecorationTrade(void) {
  ...
  StringCopy(trader->playerNames[gSpecialVar_0x8005],
