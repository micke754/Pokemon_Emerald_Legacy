...
#define TILE_FLAG_UNUSED                                                       \
  (1 << 2) // Roughly all of the traversable metatiles. Set but never read
...
bool8 MetatileBehavior_IsATile(u8 metatileBehavior) { return TRUE; }
...
bool8 MetatileBehavior_IsForcedMovementTile(u8 metatileBehavior) {
  if ((metatileBehavior >= MB_WALK_EAST &&
       metatileBehavior <= MB_TRICK_HOUSE_PUZZLE_8_FLOOR) ||
...
bool8 MetatileBehavior_IsPlayerFacingTVScreen(u8 metatileBehavior,
                                              u8 playerDir) {
...
// Used by the rock/grass floor spaces that the secret base trainer is not
// standing on
...
bool8 MetatileBehavior_IsUnableToEmerge(u8 metatileBehavior) {
  ...
  // Also the narrower underwater door in the underwater tileset has the wrong
  // metatile behavior. This causes the dive glitch. To fix change the metatile
  // behavior of the narrower water door with porymap's tileset editor.
...
bool8 MetatileBehavior_IsPlayerFacingWirelessBoxResults(u8 tile, u8 playerDir) {
  if (playerDir !=
      CONNECTION_NORTH) // if the player isn't facing north, forget about it.
    return FALSE;
  else if (tile == MB_WIRELESS_BOX_RESULTS) // is the player's north tile the
                                            // monitor with results?
    return TRUE;
  else
    return FALSE;
}
...
bool8 MetatileBehavior_IsCableBoxResults2(u8 tile, u8 playerDir) {
  if (playerDir !=
      CONNECTION_NORTH) // if the player isn't facing north, forget about it.
    return FALSE;
  else if (tile == MB_CABLE_BOX_RESULTS_2) // is the player's north tile the
                                           // monitor with results?
    return TRUE;
  else
    return FALSE;
