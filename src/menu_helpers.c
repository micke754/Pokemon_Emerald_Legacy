...
static const struct OamData sOamData_SwapLine = {.y = 0,
                                                 .affineMode =
                                                     ST_OAM_AFFINE_OFF,
...
static const union AnimCmd sAnim_SwapLine_RightArrow[] = {ANIMCMD_FRAME(0, 0),
                                                          ANIMCMD_END};

static const union AnimCmd sAnim_SwapLine_Line[] = {ANIMCMD_FRAME(4, 0),
                                                    ANIMCMD_END};

static const union AnimCmd sAnim_SwapLine_LeftArrow[] = {
    ANIMCMD_FRAME(0, 0, .hFlip = TRUE), ANIMCMD_END};

static const union AnimCmd *const sAnims_SwapLine[] = {
    sAnim_SwapLine_RightArrow, sAnim_SwapLine_Line, sAnim_SwapLine_LeftArrow};
...
void DisplayMessageAndContinueTask(u8 taskId, u8 windowId, u16 tileNum,
                                   u8 paletteNum, u8 fontId, u8 textSpeed,
                                   const u8 *string, void *taskFunc) {
...
void CreateYesNoMenuWithCallbacks(u8 taskId,
                                  ...
                                  u8 unused1, u8 unused2, u8 unused3,
...
bool8 MenuHelpers_ShouldWaitForLinkRecv(void) {
  if (IsActiveOverworldLinkBusy() == TRUE ||
      IsLinkRecvQueueAtOverworldMax() == TRUE)
    return TRUE;
  else
    return FALSE;
}

void SetItemListPerPageCount(struct ItemSlot *slots, u8 slotsCount,
                             u8 *pageItems, u8 *totalItems, u8 maxPerPage) {
  u16 i;
...
void SetCursorWithinListBounds(u16 *scrollOffset, u16 *cursorPos,
                               u8 maxShownItems, u8 totalItems) {
...
void SetCursorScrollWithinListBounds(u16 *scrollOffset, u16 *cursorPos,
                                     u8 shownItems, u8 totalItems,
                                     u8 maxShownItems) {
  u8 i;
