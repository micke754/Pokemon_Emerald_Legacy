 ...
 *   - Whenever the selected move changes (and once on init), the
 * MoveRelearnerCursorCallback is called (see sMoveRelearnerMovesListTemplate).
 * That callback will reload the contest display and battle display windows for
 * the new move. Both are always loaded in memory, but only the currently active
 * one is copied to VRAM. The exception to this is the appeal and jam hearts,
 * which are sprites. MoveRelearnerShowHideHearts is called while reloading the
 * contest display to control them. DoMoveRelearnerMain:
 * MENU_STATE_FADE_TO_BLACK DoMoveRelearnerMain: MENU_STATE_WAIT_FOR_FADE
 ...
 *   - If the player selected a move (pressed A), go to
 * MENU_STATE_PRINT_TEACH_MOVE_PROMPT.
 *   - If the player cancelled (pressed B), go to
 * MENU_STATE_PRINT_GIVE_UP_PROMPT.
 *   - If the player pressed left or right, swap the move display window to
 * contest mode, and go to MENU_STATE_SETUP_CONTEST_MODE.
 *
 ...
 *   - If the player pressed left or right, swap the move display window to
 * battle mode, and go to MENU_STATE_SETUP_BATTLE_MODE.
 *
 ...
 *   - If cancelled, go to either MENU_STATE_SETUP_BATTLE_MODE or
 * MENU_STATE_SETUP_CONTEST_MODE.
 *   - If confirmed and the pokemon had an empty move slot, set VAR_0x8004 to
 * TRUE and go to MENU_STATE_PRINT_TEXT_THEN_FANFARE.
 ...
 *   - If the player cancels, go back to
 * MENU_STATE_PRINT_TRYING_TO_LEARN_PROMPT.
 *
 ...
 *   - If the player confirms, go to MENU_STATE_FADE_AND_RETURN, and set
 * VAR_0x8004 to FALSE.
 ...
 *   - If any of the pokemon's existing moves were chosen, overwrite the move
 * and go to MENU_STATE_DOUBLE_FANFARE_FORGOT_MOVE and set VAR_0x8004 to TRUE.
 *   - If the chosen move is the one the player selected before the summary
 * screen, go to MENU_STATE_PRINT_STOP_TEACHING.
 *
 */
...
static const u16 sUI_Pal[] =
    INCBIN_U16("graphics/interface/ui_learn_move.gbapal");

// The arrow sprites in this spritesheet aren't used. The scroll-arrow system
// provides its own arrow sprites.
static const u8 sUI_Tiles[] =
...
static const struct SpriteSheet sMoveRelearnerSpriteSheet = {
    .data = sUI_Tiles, .size = sizeof(sUI_Tiles), .tag = GFXTAG_UI};

static const struct SpritePalette sMoveRelearnerPalette = {.data = sUI_Pal,
                                                           .tag = PALTAG_UI};
...
static const union AnimCmd sHeartSprite_AppealEmptyFrame[] = {
    ANIMCMD_FRAME(8, 5, FALSE, FALSE), ANIMCMD_END};
...
static void CB2_InitLearnMove(void) {
  ...
  sMoveRelearnerStruct->moveListMenuTask = ListMenuInit(
      &gMultiuseListMenuTemplate, sMoveRelearnerMenuSate.listOffset,
...
static void InitMoveRelearnerBackgroundLayers(void) {
  ...
  InitBgsFromTemplates(0, sMoveRelearnerMenuBackgroundTemplates,
...
static void DoMoveRelearnerMain(void) {
  ...
  case MENU_STATE_CONFIRM_STOP_TEACHING: {
    ...
    if (selection == 0) {
      ...
      // What's the point? It gets set to
      // MENU_STATE_PRINT_TRYING_TO_LEARN_PROMPT, anyway.
      if (sMoveRelearnerMenuSate.showContestInfo == FALSE) {
        sMoveRelearnerStruct->state = MENU_STATE_SETUP_BATTLE_MODE;
      } else if (sMoveRelearnerMenuSate.showContestInfo == TRUE) {
        sMoveRelearnerStruct->state = MENU_STATE_SETUP_CONTEST_MODE;
  ...
  case MENU_STATE_SHOW_MOVE_SUMMARY_SCREEN:
    if (!gPaletteFade.active) {
      ShowSelectMovePokemonSummaryScreen(
          gPlayerParty, sMoveRelearnerStruct->partyMon, gPlayerPartyCount - 1,
  ...
  case MENU_STATE_TRY_OVERWRITE_MOVE:
    if (!gPaletteFade.active) {
      if (sMoveRelearnerStruct->moveSlot == MAX_MON_MOVES) {
        ...
        u16 moveId =
            GetMonData(&gPlayerParty[sMoveRelearnerStruct->partyMon],
...
static void HideHeartSpritesAndShowTeachMoveText(bool8 onlyHideSprites) {
  ...
  if (!onlyHideSprites) {
    ...
    FillWindowPixelBuffer(RELEARNERWIN_MSG, 0x11);
    AddTextPrinterParameterized(RELEARNERWIN_MSG, FONT_NORMAL, gStringVar4, 0,
                                1, 0, NULL);
  }
}
...
static s32 GetCurrentSelectedMove(void) {
  return sMoveRelearnerStruct
      ->menuItems[sMoveRelearnerMenuSate.listRow +
                  sMoveRelearnerMenuSate.listOffset]
      .id;
}
...
static void CreateUISprites(void) {
  ...
  for (i = 0; i < 8; i++)
    sMoveRelearnerStruct->heartSpriteIds[i] =
        CreateSprite(&sConstestMoveHeartSprite, (i - (i / 4) * 4) * 8 + 104,
                     (i / 4) * 8 + 36, 0);
...
static void CreateLearnableMovesList(void) {
  ...
  if (FlagGet(FLAG_EGG_MOVES_TUTOR))
    sMoveRelearnerStruct->numMenuChoices =
        GetEggMoves(&gPlayerParty[sMoveRelearnerStruct->partyMon],
  ...
  for (i = 0; i < sMoveRelearnerStruct->numMenuChoices; i++) {
    sMoveRelearnerStruct->menuItems[i].name =
        gMoveNames[sMoveRelearnerStruct->movesToLearn[i]];
    sMoveRelearnerStruct->menuItems[i].id =
        sMoveRelearnerStruct->movesToLearn[i];
  }

  GetMonData(&gPlayerParty[sMoveRelearnerStruct->partyMon], MON_DATA_NICKNAME,
             nickname);
  StringCopy_Nickname(gStringVar1, nickname);
  sMoveRelearnerStruct->menuItems[sMoveRelearnerStruct->numMenuChoices].name =
      gText_Cancel;
  sMoveRelearnerStruct->menuItems[sMoveRelearnerStruct->numMenuChoices].id =
      LIST_CANCEL;
  sMoveRelearnerStruct->numMenuChoices++;
...
void MoveRelearnerShowHideHearts(s32 moveId) {
  ...
  if (!sMoveRelearnerMenuSate.showContestInfo || moveId == LIST_CANCEL) {
    ...
    for (i = 0; i < 8; i++) {
      if (i < numHearts)
        StartSpriteAnim(&gSprites[sMoveRelearnerStruct->heartSpriteIds[i + 8]],
                        3);
      else
        StartSpriteAnim(&gSprites[sMoveRelearnerStruct->heartSpriteIds[i + 8]],
                        2);
