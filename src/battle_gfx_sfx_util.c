...
static const struct CompressedSpriteSheet
    sSpriteSheets_HealthBar[MAX_BATTLERS_COUNT] = {
...
void AllocateBattleSpritesData(void) {
  ...
  gBattleSpritesDataPtr->animationData =
      AllocZeroed(sizeof(struct BattleAnimationInfo));
...
u16 ChooseMoveAndTargetInBattlePalace(void) {
  ...
  u8 unusableMovesBits =
      CheckMoveLimitations(gActiveBattler, 0, MOVE_LIMITATIONS_ALL);
...
#define randSelectGroup var2
  ...
  // If battler is < 50% HP and not asleep, use second set of move group
  // likelihoods otherwise use first set
  i = (gBattleStruct->palaceFlags & gBitTable[gActiveBattler]) ? 2 : 0;
  minGroupNum = i;

  maxGroupNum =
      i + 2; // + 2 because there are two percentages per set of likelihoods

  // Each nature has a different percent chance to select a move from one of 3
  // move groups If percent is less than 1st check, use move from "Attack" group
  ...
  if (selectedMoves != 0) {
    ...
    // Clear the rest of palaceFlags, then set the selected moves in the upper 4
    // bits.
  ...
  // If no moves matched the selected group, pick a new move from groups the
  // Pokémon has In this case the AI is not checked again, so the choice may be
  // worse If a move is chosen this way, there's a 50% chance that it will be
  ...
  if (chosenMoveId == -1) {
    if (unusableMovesBits != ALL_MOVES_MASK) {
      ...
      for (i = 0; i < MAX_MON_MOVES; i++) {
        ...
        // The totals will be stored separately in 3 groups of 4 bits each in
        // numMovesPerGroup.
        if (GetBattlePalaceMoveGroup(moveInfo->moves[i]) ==
                PALACE_MOVE_GROUP_ATTACK &&
        ...
        if (GetBattlePalaceMoveGroup(moveInfo->moves[i]) ==
                PALACE_MOVE_GROUP_SUPPORT &&
            !(gBitTable[i] & unusableMovesBits))
          numMovesPerGroup += (1 << 8);
      }

      // Count the number of move groups for which the battler has at least 2
      // usable moves. This is a roundabout way to determine if there is a move
      // group that should be preferred, because it has multiple move options
      // and the others do not. The condition intended to check the total for
      // the Support group is accidentally checking the Defense total, and is
      // never true. As a result the preferences for random move selection here
      // will skew away from the Support move group.
      ...
      // By this point we already know the battler only has usable moves from at
      // most 2 of the 3 move groups, because they had no usable moves from the
      // move group that was selected based on Nature.
      //
      // The below condition is effectively 'numMultipleMoveGroups != 1'.
      // There is no stand-out group with multiple moves to choose from, so we
      // pick randomly. Note that because of the bug above the battler may
      // actually have any number of Support moves.
      if (numMultipleMoveGroups > 1 || numMultipleMoveGroups == 0) {
        ...
        // The battler has just 1 move group with multiple move options to
        // choose from. Choose a move randomly from this group.
        if ((numMovesPerGroup & 0xF) >= 2)
        ...
        do {
          i = Random() % MAX_MON_MOVES;
          if (!(gBitTable[i] & unusableMovesBits) &&
              randSelectGroup == GetBattlePalaceMoveGroup(moveInfo->moves[i]))
            chosenMoveId = i;
        } while (chosenMoveId == -1);
      }
    } else {
  ...
  if (moveTarget & MOVE_TARGET_USER)
    ...
    chosenMoveId |=
        (GetBattlerAtPosition(BATTLE_OPPOSITE(GET_BATTLER_SIDE(gActiveBattler)))
         << 8);
...
static void UNUSED UnusedDoBattleSpriteAffineAnim(struct Sprite *sprite,
                                                  bool8 pointless) {
...
void InitAndLaunchChosenStatusAnimation(bool8 isStatus2, u32 status) {
  ...
  if (!isStatus2) {
    if (status == STATUS1_FREEZE)
      ...
      gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].statusAnimActive =
          0;
  } else {
    if (status & STATUS2_INFATUATION)
      ...
      LaunchStatusAnimation(
          gActiveBattler,
          B_ANIM_STATUS_WRAPPED); // this animation doesn't actually exist
    else                          // no animation
      gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].statusAnimActive =
          0;
  }
...
bool8 TryHandleLaunchBattleTableAnimation(u8 activeBattler, u8 atkBattler,
                                          u8 defBattler, u8 tableId,
                                          u16 argument) {
  u8 taskId;
  ...
  if (gBattleSpritesDataPtr->battlerData[activeBattler].behindSubstitute &&
    ...
    LoadBattleMonGfxAndAnimate(activeBattler, TRUE,
                               gBattlerSpriteIds[activeBattler]);
  ...
  gBattleSpritesDataPtr->healthBoxesData[gTasks[taskId].tBattlerId]
      .animFromTableActive = 1;
...
void InitAndLaunchSpecialAnimation(u8 activeBattler, u8 atkBattler,
                                   u8 defBattler, u8 tableId) {
  u8 taskId;
...
bool8 IsMoveWithoutAnimation(u16 moveId, u8 animationTurn) { return FALSE; }
...
void BattleLoadOpponentMonSpriteGfx(struct Pokemon *mon, u8 battlerId) {
  ...
  HandleLoadSpecialPokePic_DontHandleDeoxys(
      &gMonFrontPicTable[species], gMonSpritesGfxPtr->sprites.ptr[position],
  ...
  if (gBattleSpritesDataPtr->battlerData[battlerId].transformSpecies ==
    ...
    lzPaletteData = GetMonSpritePalFromSpeciesAndPersonality(species, otId,
                                                             monsPersonality);

  LZDecompressWram(lzPaletteData, gDecompressionBuffer);
  LoadPalette(gDecompressionBuffer, paletteOffset, PLTT_SIZE_4BPP);
  LoadPalette(gDecompressionBuffer, BG_PLTT_ID(8) + BG_PLTT_ID(battlerId),
              PLTT_SIZE_4BPP);

  if (species == SPECIES_CASTFORM) {
    paletteOffset = OBJ_PLTT_ID(battlerId);
    LZDecompressWram(lzPaletteData, gBattleStruct->castformPalette);
    LoadPalette(gBattleStruct->castformPalette[gBattleMonForms[battlerId]],
                paletteOffset, PLTT_SIZE_4BPP);
  }
  ...
  if (gBattleSpritesDataPtr->battlerData[battlerId].transformSpecies !=
    ...
    CpuCopy32(&gPlttBufferFaded[paletteOffset],
              &gPlttBufferUnfaded[paletteOffset], PLTT_SIZEOF(16));
  }
}
...
void BattleLoadPlayerMonSpriteGfx(struct Pokemon *mon, u8 battlerId) {
  ...
  if (ShouldIgnoreDeoxysForm(1, battlerId) == TRUE ||
      gBattleSpritesDataPtr->battlerData[battlerId].transformSpecies !=
          SPECIES_NONE) {
    HandleLoadSpecialPokePic_DontHandleDeoxys(
        &gMonBackPicTable[species], gMonSpritesGfxPtr->sprites.ptr[position],
        species, currentPersonality);
  } else {
    HandleLoadSpecialPokePic(&gMonBackPicTable[species],
                             gMonSpritesGfxPtr->sprites.ptr[position], species,
                             currentPersonality);
  }
    ...
    if (state == 1) {
      ...
      if (state == 2) {
        ...
        LoadCompressedSpriteSheet(
            &sSpriteSheets_HealthBar[gBattlerPositions[0]]);
      else if (state == 5)
        LoadCompressedSpriteSheet(
            &sSpriteSheets_HealthBar[gBattlerPositions[1]]);
      else
        retVal = TRUE;
    } else {
      if (state == 2)
        ...
        LoadCompressedSpriteSheet(
            &sSpriteSheets_HealthBar[gBattlerPositions[2]]);
      else if (state == 9)
        LoadCompressedSpriteSheet(
            &sSpriteSheets_HealthBar[gBattlerPositions[3]]);
      else
        retVal = TRUE;
    }
  }
...
void LoadBattleBarGfx(u8 unused) {
  LZDecompressWram(gBattleInterfaceGfx_BattleBar,
...
bool8 BattleInitAllSprites(u8 *state1, u8 *battlerId) {
  ...
  case 3:
    if ((gBattleTypeFlags & BATTLE_TYPE_SAFARI) && *battlerId == 0)
      ...
      gHealthboxSpriteIds[*battlerId] =
          CreateBattlerHealthboxSprites(*battlerId);
...
void ClearSpritesHealthboxAnimData(void) {
  memset(gBattleSpritesDataPtr->healthBoxesData, 0,
         sizeof(struct BattleHealthboxInfo) * MAX_BATTLERS_COUNT);
  memset(gBattleSpritesDataPtr->animationData, 0,
         sizeof(struct BattleAnimationInfo));
}

static void ClearSpritesBattlerHealthboxAnimData(void) {
  ClearSpritesHealthboxAnimData();
  memset(gBattleSpritesDataPtr->battlerData, 0,
...
void CopyAllBattleSpritesInvisibilities(void) {
  ...
  for (i = 0; i < gBattlersCount; i++)
    gBattleSpritesDataPtr->battlerData[i].invisible =
        gSprites[gBattlerSpriteIds[i]].invisible;
}
...
void HandleSpeciesGfxDataChange(u8 battlerAtk, u8 battlerDef, bool8 castform) {
  ...
  if (castform) {
    ...
    LoadPalette(
        gBattleStruct
            ->castformPalette[gBattleSpritesDataPtr->animationData->animArg],
        paletteOffset, PLTT_SIZE_4BPP);
    gBattleMonForms[battlerAtk] = gBattleSpritesDataPtr->animationData->animArg;
    if (gBattleSpritesDataPtr->battlerData[battlerAtk].transformSpecies !=
        SPECIES_NONE) {
      BlendPalette(paletteOffset, 16, 6, RGB_WHITE);
      CpuCopy32(&gPlttBufferFaded[paletteOffset],
                &gPlttBufferUnfaded[paletteOffset], PLTT_SIZEOF(16));
    }
    gSprites[gBattlerSpriteIds[battlerAtk]].y =
        GetBattlerSpriteDefault_Y(battlerAtk);
  } else {
    const void *src;
    void *dst;
    u16 targetSpecies;
    ...
    if (IsContest()) {
      ...
      HandleLoadSpecialPokePic_DontHandleDeoxys(
          ...
          gMonSpritesGfxPtr->sprites.ptr[position], targetSpecies,
      ...
      if (GetBattlerSide(battlerDef) == B_SIDE_OPPONENT)
        targetSpecies = GetMonData(
            &gEnemyParty[gBattlerPartyIndexes[battlerDef]], MON_DATA_SPECIES);
      else
        targetSpecies = GetMonData(
            &gPlayerParty[gBattlerPartyIndexes[battlerDef]], MON_DATA_SPECIES);

      if (GetBattlerSide(battlerAtk) == B_SIDE_PLAYER) {
        personalityValue =
            GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerAtk]],
                       MON_DATA_PERSONALITY);
        otId = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerAtk]],
                          MON_DATA_OT_ID);
        ...
        personalityValue =
            GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerAtk]],
                       MON_DATA_PERSONALITY);
        otId = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerAtk]],
                          MON_DATA_OT_ID);
    ...
    dst = (void *)(OBJ_VRAM0 +
                   gSprites[gBattlerSpriteIds[battlerAtk]].oam.tileNum * 32);
    DmaCopy32(3, src, dst, MON_PIC_SIZE);
    paletteOffset = OBJ_PLTT_ID(battlerAtk);
    lzPaletteData = GetMonSpritePalFromSpeciesAndPersonality(
        targetSpecies, otId, personalityValue);
    LZDecompressWram(lzPaletteData, gDecompressionBuffer);
    LoadPalette(gDecompressionBuffer, paletteOffset, PLTT_SIZE_4BPP);

    if (targetSpecies == SPECIES_CASTFORM) {
      gSprites[gBattlerSpriteIds[battlerAtk]].anims =
          gMonFrontAnimsPtrTable[targetSpecies];
      LZDecompressWram(lzPaletteData, gBattleStruct->castformPalette);
      LoadPalette(gBattleStruct->castformPalette[gBattleMonForms[battlerDef]],
                  paletteOffset, PLTT_SIZE_4BPP);
    }
    ...
    if (!IsContest()) {
      gBattleSpritesDataPtr->battlerData[battlerAtk].transformSpecies =
          targetSpecies;
...
void BattleLoadSubstituteOrMonSpriteGfx(u8 battlerId, bool8 loadMonSprite) {
  ...
  if (!loadMonSprite) {
    ...
    if (IsContest())
      LZDecompressVram(gSubstituteDollBackGfx,
                       gMonSpritesGfxPtr->sprites.ptr[position]);
    else if (GetBattlerSide(battlerId) != B_SIDE_PLAYER)
      LZDecompressVram(gSubstituteDollFrontGfx,
                       gMonSpritesGfxPtr->sprites.ptr[position]);
    else
      LZDecompressVram(gSubstituteDollBackGfx,
                       gMonSpritesGfxPtr->sprites.ptr[position]);
    ...
    for (i = 1; i < 4; i++) {
      Dma3CopyLarge32_(
          gMonSpritesGfxPtr->sprites.ptr[position],
          &gMonSpritesGfxPtr->sprites.byte[position][MON_PIC_SIZE * i],
          MON_PIC_SIZE);
    }
...
void LoadBattleMonGfxAndAnimate(u8 battlerId, bool8 loadMonSprite,
                                u8 spriteId) {
...
void HandleLowHpMusicChange(struct Pokemon *mon, u8 battlerId) {
  ...
  if (GetHPBarLevel(hp, maxHP) == HP_BAR_RED) {
    if (!gBattleSpritesDataPtr->battlerData[battlerId].lowHpSong) {
      if (!gBattleSpritesDataPtr->battlerData[BATTLE_PARTNER(battlerId)]
               .lowHpSong)
    ...
    if (IsDoubleBattle() &&
        !gBattleSpritesDataPtr->battlerData[BATTLE_PARTNER(battlerId)]
             .lowHpSong) {
      m4aSongNumStop(SE_LOW_HEALTH);
      return;
    }
...
void BattleStopLowHpSound(void) {
  ...
  if (IsDoubleBattle())
    gBattleSpritesDataPtr->battlerData[BATTLE_PARTNER(playerBattler)]
        .lowHpSong = 0;
...
void HandleBattleLowHpMusicChange(void) {
  if (gMain.inBattle) {
    ...
    u8 battler1PartyId =
        GetPartyIdFromBattlePartyId(gBattlerPartyIndexes[playerBattler1]);
    u8 battler2PartyId =
        GetPartyIdFromBattlePartyId(gBattlerPartyIndexes[playerBattler2]);

    if (GetMonData(&gPlayerParty[battler1PartyId], MON_DATA_HP) != 0)
      HandleLowHpMusicChange(&gPlayerParty[battler1PartyId], playerBattler1);
    if (IsDoubleBattle() &&
        GetMonData(&gPlayerParty[battler2PartyId], MON_DATA_HP) != 0)
...
void LoadAndCreateEnemyShadowSprites(void) {
  ...
  gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId =
      CreateSprite(&gSpriteTemplate_EnemyShadow,
                   ...
                   0xC8);
  gSprites[gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId]
      .data[0] = battlerId;
...
void SpriteCB_EnemyShadow(struct Sprite *shadowSprite) {
  ...
  if (gAnimScriptActive || battlerSprite->invisible)
    invisible = TRUE;
               ...
               SPECIES_NONE &&
           gEnemyMonElevation[gBattleSpritesDataPtr->battlerData[battlerId]
                                  .transformSpecies] == 0)
    invisible = TRUE;
...
void AllocateMonSpritesGfx(void) {
  ...
  gMonSpritesGfxPtr->firstDecompressed =
      AllocZeroed(MON_PIC_SIZE * 4 * MAX_BATTLERS_COUNT);

  for (i = 0; i < MAX_BATTLERS_COUNT; i++) {
    gMonSpritesGfxPtr->sprites.ptr[i] =
        gMonSpritesGfxPtr->firstDecompressed + (i * MON_PIC_SIZE * 4);
    *(gMonSpritesGfxPtr->templates + i) = gBattlerSpriteTemplates[i];

    for (j = 0; j < 4; j++) {
      gMonSpritesGfxPtr->frameImages[i][j].data =
          gMonSpritesGfxPtr->sprites.ptr[i] + (j * MON_PIC_SIZE);
