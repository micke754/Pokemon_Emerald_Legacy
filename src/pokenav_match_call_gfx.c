...
static const u16 sMatchCallUI_Pal[] =
    INCBIN_U16("graphics/pokenav/match_call/ui.gbapal");
static const u32 sMatchCallUI_Gfx[] =
    INCBIN_U32("graphics/pokenav/match_call/ui.4bpp.lz");
static const u32 sMatchCallUI_Tilemap[] =
    INCBIN_U32("graphics/pokenav/match_call/ui.bin.lz");
static const u16 sOptionsCursor_Pal[] =
    INCBIN_U16("graphics/pokenav/match_call/options_cursor.gbapal");
static const u32 sOptionsCursor_Gfx[] =
    INCBIN_U32("graphics/pokenav/match_call/options_cursor.4bpp.lz");
static const u16 sCallWindow_Pal[] =
    INCBIN_U16("graphics/pokenav/match_call/call_window.gbapal");
static const u16 sListWindow_Pal[] =
    INCBIN_U16("graphics/pokenav/match_call/list_window.gbapal");
static const u16 sPokeball_Pal[] =
    INCBIN_U16("graphics/pokenav/match_call/pokeball.gbapal");
static const u32 sPokeball_Gfx[] =
    INCBIN_U32("graphics/pokenav/match_call/pokeball.4bpp.lz");
...
static const u8 sText_CallingDots[] =
    _("·{PAUSE 4}·{PAUSE 4}·{PAUSE 4}·{PAUSE 4}·\p");
...
bool32 OpenMatchCall(void) {
  struct Pokenav_MatchCallGfx *gfx = AllocSubstruct(
      POKENAV_SUBSTRUCT_MATCH_CALL_OPEN, sizeof(struct Pokenav_MatchCallGfx));
  if (!gfx)
    return FALSE;
...
void CreateMatchCallLoopedTask(s32 index) {
  struct Pokenav_MatchCallGfx *gfx =
      GetSubstructPtr(POKENAV_SUBSTRUCT_MATCH_CALL_OPEN);
...
static u32 LoopedTask_OpenMatchCall(s32 state) {
  ...
  case 0:
    ...
    CopyPaletteIntoBufferUnfaded(sMatchCallUI_Pal, BG_PLTT_ID(2),
                                 sizeof(sMatchCallUI_Pal));
    CopyBgTilemapBufferToVram(2);
  ...
  case 1:
    ...
    CopyPaletteIntoBufferUnfaded(sCallWindow_Pal, BG_PLTT_ID(1),
                                 sizeof(sCallWindow_Pal));
  ...
  case 2:
    ...
    CopyPaletteIntoBufferUnfaded(sListWindow_Pal, BG_PLTT_ID(3),
                                 sizeof(sListWindow_Pal));
...
static void CreateMatchCallList(void) {
  ...
  template.bufferItemFunc =
      (PokenavListBufferItemFunc)BufferMatchCallNameAndDesc;
...
static void Task_FlashPokeballIcons(u8 taskId) {
  ...
  if (tActive) {
    ...
    PokenavCopyPalette(sPokeball_Pal, &sPokeball_Pal[0x10], 0x10, 0x10, tSinVal,
                       &gPlttBufferUnfaded[BG_PLTT_ID(5)]);
    if (!gPaletteFade.active)
      CpuCopy32(&gPlttBufferUnfaded[BG_PLTT_ID(5)],
                &gPlttBufferFaded[BG_PLTT_ID(5)], PLTT_SIZE_4BPP);
  }
}
...
static void TryDrawRematchPokeballIcon(u16 windowId, u32 rematchId,
                                       u32 tileOffset) {
...
static void PrintNumberRegistered(u16 windowId) {
  u8 str[3];
  ConvertIntToDecimalStringN(str, GetNumberRegistered(),
...
static void PrintNumberOfBattles(u16 windowId) {
  ...
  ConvertIntToDecimalStringN(str, numTrainerBattles, STR_CONV_MODE_LEFT_ALIGN,
                             5);
...
static void PrintMatchCallInfoLabel(u16 windowId, const u8 *str, int top) {
  int y = top * 16 + 1;
  AddTextPrinterParameterized(windowId, FONT_NARROW, str, 2, y, TEXT_SKIP_DRAW,
                              NULL);
}
...
static void PrintMatchCallInfoNumber(u16 windowId, const u8 *str, int top) {
  ...
  int y = top * 16 + 1;
  AddTextPrinterParameterized(windowId, FONT_NARROW, str, x, y, TEXT_SKIP_DRAW,
                              NULL);
}

static void PrintMatchCallLocation(struct Pokenav_MatchCallGfx *gfx,
                                   int delta) {
  u8 mapName[32];
  int x;
  ...
  FillWindowPixelBuffer(gfx->locWindowId, PIXEL_FILL(1));
  AddTextPrinterParameterized(gfx->locWindowId, FONT_NARROW, mapName, x, 1, 0,
                              NULL);
}
...
static void PrintMatchCallSelectionOptions(struct Pokenav_MatchCallGfx *gfx) {
  ...
  for (i = 0; i < MATCH_CALL_OPTION_COUNT; i++) {
    ...
    AddTextPrinterParameterized(gfx->infoBoxWindowId, FONT_NARROW,
                                sMatchCallOptionTexts[optionText], 16,
...
static void PrintCallingDots(struct Pokenav_MatchCallGfx *gfx) {
  AddTextPrinterParameterized(gfx->msgBoxWindowId, FONT_NORMAL,
                              sText_CallingDots, 32, 1, 1, NULL);
}
...
static void PrintTrainerIsCloseBy(struct Pokenav_MatchCallGfx *gfx) {
  AddTextPrinterParameterized(gfx->msgBoxWindowId, FONT_NORMAL,
                              gText_TrainerCloseBy, 0, 1, 1, NULL);
}
...
static void PrintMatchCallMessage(struct Pokenav_MatchCallGfx *gfx) {
  ...
  AddTextPrinterParameterized(gfx->msgBoxWindowId, FONT_NORMAL, str, 32, 1,
                              speed, NULL);
}
...
static void AllocMatchCallSprites(void) {
  ...
  gfx->trainerPicGfxPtr =
      (u8 *)OBJ_VRAM0 + LoadSpriteSheet(&spriteSheet) * 0x20;
...
static void CreateOptionsCursorSprite(struct Pokenav_MatchCallGfx *gfx,
                                      int top) {
  if (!gfx->optionsCursorSprite) {
...
static void
CloseMatchCallSelectOptionsWindow(struct Pokenav_MatchCallGfx *gfx) {
  DestroySprite(gfx->optionsCursorSprite);
  gfx->optionsCursorSprite = NULL;
}
...
static void LoadCheckPageTrainerPic(struct Pokenav_MatchCallGfx *gfx) {
  ...
  if (trainerPic >= 0) {
    DecompressPicFromTable(&gTrainerFrontPicTable[trainerPic],
                           gfx->trainerPicGfx, SPECIES_NONE);
    LZ77UnCompWram(gTrainerFrontPicPaletteTable[trainerPic].data,
                   gfx->trainerPicPal);
    cursor = RequestDma3Copy(gfx->trainerPicGfx, gfx->trainerPicGfxPtr,
                             sizeof(gfx->trainerPicGfx), 1);
    LoadPalette(gfx->trainerPicPal, gfx->trainerPicPalOffset,
                sizeof(gfx->trainerPicPal));
