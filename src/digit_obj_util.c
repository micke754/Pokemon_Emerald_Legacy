...
static void DrawNumObjsLeadingZeros(struct DigitPrinter *objWork, s32 num,
                                    bool32 sign);
static void DrawNumObjsMinusInFront(struct DigitPrinter *objWork, s32 num,
                                    bool32 sign);
static void DrawNumObjsMinusInBack(struct DigitPrinter *objWork, s32 num,
                                   bool32 sign);
...
bool32 DigitObjUtil_CreatePrinter(u32 id, s32 num,
                                  const struct DigitObjUtilTemplate *template) {
  u32 i;
  ...
  sOamWork->array[id].tileStart =
      GetSpriteTileStartByTag(template->spriteSheet->tag);
  if (sOamWork->array[id].tileStart == 0xFFFF) {
    if (template->spriteSheet->size != 0) {
      ...
      compSpriteSheet =
          *(struct CompressedSpriteSheet *)(template->spriteSheet);
      compSpriteSheet.size =
          GetDecompressedDataSize(template->spriteSheet->data);
      sOamWork->array[id].tileStart =
  ...
  sOamWork->array[id].palTagIndex =
      IndexOfSpritePaletteTag(template->spritePal->tag);
  ...
  sOamWork->array[id].tilesPerImage =
      GetTilesPerImage(template->shape, template->size);
...
static void CopyWorkToOam(struct DigitPrinter *objWork) {
  ...
  gMain.oamBuffer[oamId].tileNum =
      objWork->tileStart + (objWork->tilesPerImage * 10);
}
...
static void DrawNumObjsLeadingZeros(struct DigitPrinter *objWork, s32 num,
  ...
  while (pow10 != 0) {
    ...
    gMain.oamBuffer[oamId].tileNum =
        (digit * objWork->tilesPerImage) + objWork->tileStart;
    oamId++;
  }
...
static void DrawNumObjsMinusInFront(struct DigitPrinter *objWork, s32 num,
  ...
  if (sign) {
    gMain.oamBuffer[oamId].affineMode = ST_OAM_AFFINE_OFF;
    gMain.oamBuffer[oamId].x =
        objWork->x + ((firstDigit - 1) * objWork->xDelta);
  } else {
    gMain.oamBuffer[oamId].affineMode = ST_OAM_AFFINE_ERASE;
  }
}
...
static u8 GetFirstOamId(u8 oamCount) {
  ...
  for (i = 0; i < sOamWork->count; i++) {
    if (!sOamWork->array[i].isActive) {
      if (sOamWork->array[i].firstOamId != 0xFF &&
          sOamWork->array[i].oamCount <= oamCount)
...
u8 GetTilesPerImage(u32 shape, u32 size) { return sTilesPerImage[shape][size]; }
