...
const union AnimCmd gFangAnimCmds[] = {
    ANIMCMD_FRAME(0, 8),  ANIMCMD_FRAME(16, 16), ANIMCMD_FRAME(32, 4),
    ANIMCMD_FRAME(48, 4), ANIMCMD_END,
};
...
const union AffineAnimCmd gSwallowDeformMonAffineAnimCmds[] = {
    AFFINEANIMCMD_FRAME(0, 6, 0, 20),  AFFINEANIMCMD_FRAME(0, 0, 0, 20),
    AFFINEANIMCMD_FRAME(7, -30, 0, 6), AFFINEANIMCMD_FRAME(0, 0, 0, 20),
    AFFINEANIMCMD_FRAME(-2, 3, 0, 20), AFFINEANIMCMD_END,
};
...
static const u16 sUnusedPalette[] =
    INCBIN_U16("graphics/battle_anims/unused/unknown.gbapal");
...
const union AffineAnimCmd gDeepInhaleAffineAnimCmds[] = {
    AFFINEANIMCMD_FRAME(16, 0, 0, 4),  AFFINEANIMCMD_FRAME(0, -3, 0, 16),
    AFFINEANIMCMD_FRAME(4, 0, 0, 4),   AFFINEANIMCMD_FRAME(0, 0, 0, 24),
    AFFINEANIMCMD_FRAME(-5, 3, 0, 16), AFFINEANIMCMD_END,
};
...
const u16 gFacadeBlendColors[] = {
    RGB(28, 25, 1),  RGB(28, 21, 5), RGB(27, 18, 8), RGB(27, 14, 11),
    RGB(26, 10, 15), RGB(26, 7, 18), RGB(25, 3, 21), RGB(25, 0, 25),
    RGB(25, 0, 23),  RGB(25, 0, 20), RGB(25, 0, 16), RGB(25, 0, 13),
    RGB(26, 0, 10),  RGB(26, 0, 6),  RGB(26, 0, 3),  RGB(27, 0, 0),
    RGB(27, 1, 0),   RGB(27, 5, 0),  RGB(27, 9, 0),  RGB(27, 12, 0),
    RGB(28, 16, 0),  RGB(28, 19, 0), RGB(28, 23, 0), RGB(29, 27, 0),
};
...
const union AffineAnimCmd gSlackOffSquishAffineAnimCmds[] = {
    AFFINEANIMCMD_FRAME(0, 16, 0, 4),  AFFINEANIMCMD_FRAME(-2, 0, 0, 8),
    AFFINEANIMCMD_FRAME(0, 4, 0, 4),   AFFINEANIMCMD_FRAME(0, 0, 0, 24),
    AFFINEANIMCMD_FRAME(1, -5, 0, 16), AFFINEANIMCMD_END,
};
...
static void AnimWhiteHalo(struct Sprite *sprite) {
  ...
  SetGpuReg(REG_OFFSET_BLDALPHA,
            BLDALPHA_BLEND(sprite->data[1], 16 - sprite->data[1]));
}
...
static void AnimMeanLookEye_Step1(struct Sprite *sprite) {
  SetGpuReg(REG_OFFSET_BLDALPHA,
            BLDALPHA_BLEND(sprite->data[0], 16 - sprite->data[0]));
...
static void SetPsychicBackground_Step(u8 taskId) {
  ...
  if (++gTasks[taskId].data[5] == 4) {
    ...
    for (i = 10; i > 0; i--)
      gPlttBufferFaded[BG_PLTT_ID(paletteIndex) + i + 1] =
          gPlttBufferFaded[BG_PLTT_ID(paletteIndex) + i];
...
static void FadeScreenToWhite_Step(u8 taskId) {
  ...
  if (++gTasks[taskId].data[5] == 4) {
    ...
    for (i = 10; i > 0; i--)
      gPlttBufferUnfaded[BG_PLTT_ID(paletteIndex) + i + 1] =
          gPlttBufferUnfaded[BG_PLTT_ID(paletteIndex) + i];
...
static void AnimSpotlight(struct Sprite *sprite) {
  SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WIN01_BG_ALL | WINOUT_WIN01_OBJ |
                                   WINOUT_WIN01_CLR | WINOUT_WINOBJ_BG_ALL |
                                   WINOUT_WINOBJ_OBJ);
...
static void AnimClappingHand_Step(struct Sprite *sprite) {
  if (sprite->data[2] == 0) {
    ...
    if (sprite->x2 == 0) {
      sprite->data[2]++;
      if (sprite->data[3] == 0) {
        PlaySE1WithPanning(SE_M_ENCORE,
...
static void TormentAttacker_Step(u8 taskId) {
  ...
  case 0:
    ...
    spriteId =
        CreateSprite(&gThoughtBubbleSpriteTemplate, x, y, 6 - task->data[1]);
    PlaySE12WithPanning(SE_M_METRONOME,
                        BattleAnimAdjustPanning(SOUND_PAN_ATTACKER));
...
void AnimTask_TransformMon(u8 taskId) {
  ...
  case 1:
    ...
    break;
  case 2:
    HandleSpeciesGfxDataChange(gBattleAnimAttacker, gBattleAnimTarget,
    ...
    src = gMonSpritesGfxPtr->sprites.ptr[position] +
          (gBattleMonForms[gBattleAnimAttacker] << 11);
    dest = animBg.bgTiles;
    ...
    if (IsContest()) {
      ...
      StartSpriteAffineAnim(&gSprites[gBattlerSpriteIds[gBattleAnimAttacker]],
...
void AnimTask_IsMonInvisible(u8 taskId) {
  gBattleAnimArgs[ARG_RET_ID] =
      gSprites[gBattlerSpriteIds[gBattleAnimAttacker]].invisible;
  DestroyAnimVisualTask(taskId);
}
...
void AnimTask_MorningSunLightBeam(u8 taskId) {
  ...
  case 0:
    SetGpuReg(REG_OFFSET_BLDCNT,
              BLDCNT_TGT2_ALL | BLDCNT_EFFECT_BLEND | BLDCNT_TGT1_BG1);
    SetGpuReg(REG_OFFSET_BLDALPHA, BLDALPHA_BLEND(0, 16));
    ...
    AnimLoadCompressedBgTilemapHandleContest(
        &animBg, &gBattleAnimMaskTilemap_LightBeam, FALSE);
    if (IsContest()) {
      gBattle_BG1_X = -56;
      gBattle_BG1_Y = 0;
    } else {
    ...
    AnimLoadCompressedBgGfx(animBg.bgId, gBattleAnimMaskImage_LightBeam,
                            animBg.tilesOffset);
    LoadCompressedPalette(gBattleAnimMaskPalette_LightBeam,
                          BG_PLTT_ID(animBg.paletteId), PLTT_SIZE_4BPP);

    gTasks[taskId].data[10] = gBattle_BG1_X;
    gTasks[taskId].data[11] = gBattle_BG1_Y;

    gTasks[taskId].data[0]++;
    PlaySE12WithPanning(SE_M_MORNING_SUN,
                        BattleAnimAdjustPanning(SOUND_PAN_ATTACKER));
    break;
  case 1:
...
static void AnimGreenStar(struct Sprite *sprite) {
  ...
  sprite->x =
      GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X) + xOffset;
  sprite->y = GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_Y) + 32;
  sprite->data[1] = gBattleAnimArgs[0];
  sprite->data[2] = gBattleAnimArgs[1];

  spriteId1 = CreateSprite(&gGreenStarSpriteTemplate, sprite->x, sprite->y,
                           sprite->subpriority + 1);
  spriteId2 = CreateSprite(&gGreenStarSpriteTemplate, sprite->x, sprite->y,
                           sprite->subpriority + 1);
...
void AnimTask_DoomDesireLightBeam(u8 taskId) {
  ...
  case 3:
    if (++gTasks[taskId].data[3] >
        gDoomDesireLightBeamDelayTable[gTasks[taskId].data[2]])
      gTasks[taskId].data[0]++;
    break;
  case 4:
...
// Rocks the mon back and forth. This is done on a pivot so it is done via
// rotation. arg 0: which battler arg 1: number of rocks arg 2: rotation speed
// increase
...
static void AnimPainSplitProjectile(struct Sprite *sprite) {
  if (!sprite->data[0]) {
    if (gBattleAnimArgs[2] == ANIM_ATTACKER) {
      sprite->x = GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X_2);
      sprite->y = GetBattlerSpriteCoord(gBattleAnimAttacker,
...
void AnimTask_PainSplitMovement(u8 taskId) {
  ...
  if (gTasks[taskId].data[0] == 0) {
    ...
    case 1:
      ...
      SetBattlerSpriteYOffsetFromYScale(spriteId);
      if (IsContest() ||
          GetBattlerSide(gTasks[taskId].data[11]) == B_SIDE_PLAYER)
...
// Uses a spotlight sprite as a light mask to illuminate the target mon. The
// spotlight grows and shrinks. arg 0: initial x pixel offset arg 1: initial y
// pixel offset arg 2: duration of fully-opened spotlight
...
// Spins an orb around the attacking mon, while its path radius grows and
// shrinks. arg 0: duration arg 1: initial wave offset
...
// Copies the target mon's sprite, and makes a white silhouette that shrinks
// away.
void AnimTask_RolePlaySilhouette(u8 taskId) {
  ...
  if (IsContest()) {
    ...
    if (GetBattlerSide(gBattleAnimAttacker) != B_SIDE_PLAYER) {
      isBackPic = FALSE;
      personality =
          GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattleAnimTarget]],
                     MON_DATA_PERSONALITY);
      otId = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattleAnimTarget]],
                        MON_DATA_OT_ID);
      if (gBattleSpritesDataPtr->battlerData[gBattleAnimTarget]
              .transformSpecies == SPECIES_NONE) {
        if (GetBattlerSide(gBattleAnimTarget) == B_SIDE_PLAYER)
          ...
          species =
              GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattleAnimTarget]],
                         MON_DATA_SPECIES);
      } else {
        species = gBattleSpritesDataPtr->battlerData[gBattleAnimTarget]
                      .transformSpecies;
      }
      ...
      personality =
          GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattleAnimTarget]],
                     MON_DATA_PERSONALITY);
      otId = GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattleAnimTarget]],
                        MON_DATA_OT_ID);
      if (gBattleSpritesDataPtr->battlerData[gBattleAnimTarget]
              .transformSpecies == SPECIES_NONE) {
        if (GetBattlerSide(gBattleAnimTarget) == B_SIDE_PLAYER)
  ...
  spriteId = CreateAdditionalMonSpriteForMoveAnim(
      species, isBackPic, 0, coord1 + xOffset, coord2, 5, personality, otId,
      gBattleAnimTarget, TRUE);

  gSprites[spriteId].oam.priority = priority;
  gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
  FillPalette(RGB_WHITE, OBJ_PLTT_ID(gSprites[spriteId].oam.paletteNum),
              PLTT_SIZE_4BPP);
  gSprites[spriteId].oam.priority = priority;
...
static void AnimTask_RolePlaySilhouette_Step2(u8 taskId) {
  ...
  TrySetSpriteRotScale(&gSprites[spriteId], TRUE, gTasks[taskId].data[10],
...
void AnimTask_AcidArmor(u8 taskId) {
  ...
  if (GetBattlerSpriteBGPriorityRank(battler) == 1) {
    ...
    scanlineParams.dmaDest = &REG_BG2HOFS;
    SetGpuReg(REG_OFFSET_BLDCNT,
              BLDCNT_TGT2_ALL | BLDCNT_EFFECT_BLEND | BLDCNT_TGT1_BG2);
    bgX = gBattle_BG2_X;
    bgY = gBattle_BG2_Y;
  }
...
static void AnimTask_AcidArmor_Step(u8 taskId) {
  ...
  case 0:
    ...
    while (var0 > task->data[13]) {
      gScanlineEffectRegBuffers[gScanlineEffect.srcBuffer][offset + 1] =
          (i - var2) + bgY;
    ...
    if (++task->data[6] > 63) {
      ...
      SetGpuReg(REG_OFFSET_BLDALPHA,
                BLDALPHA_BLEND(task->data[3], task->data[4]));
...
void AnimTask_DeepInhale(u8 taskId) {
  ...
  task->data[0] = 0;
  task->data[15] = GetAnimBattlerSpriteId(gBattleAnimArgs[0]);
  PrepareAffineAnimInTaskData(&gTasks[taskId], task->data[15],
...
static void InitYawnCloudPosition(struct Sprite *sprite, s16 startX, s16 startY,
                                  s16 destX, s16 destY, u16 duration) {
  sprite->x = startX;
  sprite->y = startY;
...
static void AnimTask_SlideMonForFocusBand_Step2(u8 taskId) {
  ...
  if (gTasks[taskId].data[3] & 0x8000)
    gSprites[gTasks[taskId].data[15]].y2 =
        gTasks[taskId].data[10] - (var1 >> 8);
  else
    gSprites[gTasks[taskId].data[15]].y2 =
        gTasks[taskId].data[10] + (var1 >> 8);
...
#define IDX_ACTIVE_SPRITES                                                     \
  2 // Used by the sprite callback to modify the number of active sprites
...
void AnimTask_SquishAndSweatDroplets(u8 taskId) {
  ...
  PrepareAffineAnimInTaskData(task, task->tBattlerSpriteId,
...
static void CreateSweatDroplets(u8 taskId, bool8 lowerDroplets) {
  ...
  for (i = 0; i < 4; i++) {
    u8 spriteId = CreateSprite(&gFacadeSweatDropSpriteTemplate, xCoords[i],
                               yCoords[i & 1], task->tSubpriority - 5);
    if (spriteId != MAX_SPRITES) {
      gSprites[spriteId].sTimer = 0;
      gSprites[spriteId].sVelocX =
          i < 2 ? -2 : 2; // First two travel left, remaining travel right
...
static void AnimTask_FacadeColorBlend_Step(u8 taskId) {
  if (gTasks[taskId].data[1]) {
    BlendPalette(gTasks[taskId].data[2], 16, 8,
...
void AnimTask_StatusClearedEffect(u8 taskId) {
  StartMonScrollingBgMask(taskId, 0, 0x1A0, gBattleAnimAttacker,
                          gBattleAnimArgs[0], 10, 2, 30, gCureBubblesGfx,
...
#define IDX_ACTIVE_SPRITES                                                     \
  10 // Used by the sprite callback to modify the number of active sprites
...
void AnimTask_GlareEyeDots(u8 taskId) {
  ...
  if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_PLAYER)
    task->tStartX =
        ...
        GetBattlerSpriteCoordAttr(gBattleAnimAttacker,
...
static void AnimTask_GlareEyeDots_Step(u8 taskId) {
  ...
  case 0:
    ...
    if (++task->tTimer > 3) {
      task->tTimer = 0;
      GetGlareEyeDotCoords(task->tStartX, task->tStartY, task->tEndX,
                           task->tEndY, task->tPairMax, task->tPairNum, &x, &y);
...
static void GetGlareEyeDotCoords(s16 startX, s16 startY, s16 endX, s16 endY,
                                 u8 pairMax, u8 pairNum, s16 *x, s16 *y) {
  int x2;
  int y2;
...
void AnimTask_BarrageBall(u8 taskId) {
  ...
  task->data[14] =
      GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y_PIC_OFFSET) +
      GetBattlerSpriteCoordAttr(gBattleAnimTarget, BATTLER_COORD_ATTR_HEIGHT) /
          4;
  task->data[15] =
      CreateSprite(&gBarrageBallSpriteTemplate, task->data[11], task->data[12],
...
static void AnimSmellingSaltsHand(struct Sprite *sprite) {
  ...
  if (gBattleAnimArgs[1] == 0) {
    ...
    sprite->x =
        GetBattlerSpriteCoordAttr(battler, BATTLER_COORD_ATTR_RIGHT) + 8;
  }
...
void AnimTask_SmellingSaltsSquish(u8 taskId) {
  if (gBattleAnimArgs[0] == ANIM_ATTACKER) {
    ...
    PrepareAffineAnimInTaskData(&gTasks[taskId], gTasks[taskId].data[15],
...
static void AnimSmellingSaltExclamation(struct Sprite *sprite) {
  if (gBattleAnimArgs[0] == ANIM_ATTACKER) {
    sprite->x = GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X_2);
    sprite->y =
        GetBattlerSpriteCoordAttr(gBattleAnimAttacker, BATTLER_COORD_ATTR_TOP);
  } else {
    sprite->x = GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_X_2);
    sprite->y =
        GetBattlerSpriteCoordAttr(gBattleAnimTarget, BATTLER_COORD_ATTR_TOP);
  }
...
void AnimTask_HelpingHandAttackerMovement(u8 taskId) {
  ...
  if (!IsContest()) {
    if (IsDoubleBattle() == TRUE) {
      ...
      int partnerX = GetBattlerSpriteCoord(BATTLE_PARTNER(gBattleAnimAttacker),
                                           BATTLER_COORD_X);
...
static void AnimForesightMagnifyingGlass_Step(struct Sprite *sprite) {
  ...
  case 0:
    ...
    case 0:
    case 4:
      x = GetBattlerSpriteCoordAttr(sprite->data[7], BATTLER_COORD_ATTR_RIGHT) -
          4;
      y = GetBattlerSpriteCoordAttr(sprite->data[7],
    ...
    case 1:
      x = GetBattlerSpriteCoordAttr(sprite->data[7], BATTLER_COORD_ATTR_RIGHT) -
          4;
      y = GetBattlerSpriteCoordAttr(sprite->data[7], BATTLER_COORD_ATTR_TOP) +
          4;
      break;
    case 2:
      x = GetBattlerSpriteCoordAttr(sprite->data[7], BATTLER_COORD_ATTR_LEFT) +
          4;
      y = GetBattlerSpriteCoordAttr(sprite->data[7],
                                    BATTLER_COORD_ATTR_BOTTOM) -
          4;
  ...
  case 3:
    ...
    SetGpuReg(REG_OFFSET_BLDALPHA,
              BLDALPHA_BLEND(sprite->data[1], sprite->data[2]));
    if (++sprite->data[0] == 32) {
      sprite->invisible = TRUE;
      sprite->data[5]++;
    }
...
static void AnimMeteorMashStar_Step(struct Sprite *sprite) {
  sprite->x2 =
      ((sprite->data[2] - sprite->data[0]) * sprite->data[5]) / sprite->data[4];
  sprite->y2 =
      ((sprite->data[3] - sprite->data[1]) * sprite->data[5]) / sprite->data[4];
  if (!(sprite->data[5] & 1)) {
    CreateSprite(&gMiniTwinklingStarSpriteTemplate, sprite->x + sprite->x2,
...
// Moves a shooting star across the screen that leaves little twinkling stars
// behind its path. arg 0: initial x pixel offset arg 1: initial y pixel offset
...
void AnimTask_MonToSubstitute(u8 taskId) {
  ...
  if (gTasks[taskId].data[0] == 0) {
    ...
    SetSpriteRotScale(spriteId, gTasks[taskId].data[1], gTasks[taskId].data[2],
                      0);
...
static void AnimTask_MonToSubstituteDoll(u8 taskId) {
  ...
  case 1:
    ...
    if (gSprites[spriteId].y2 == 0) {
      PlaySE12WithPanning(SE_M_BUBBLE2,
                          BattleAnimAdjustPanning(SOUND_PAN_ATTACKER));
...
static void AnimBlockX_Step(struct Sprite *sprite) {
  ...
  case 0:
    sprite->y2 += 10;
    if (sprite->y2 >= 0) {
      PlaySE12WithPanning(SE_M_SKETCH,
                          BattleAnimAdjustPanning(SOUND_PAN_TARGET));
      sprite->y2 = 0;
      sprite->data[0]++;
    }
    break;
...
// Makes the mon run out of screen, run past the opposing mon, and return to its
// original position. No args.
void AnimTask_SnatchOpposingMonMove(u8 taskId) {
  ...
  case 1:
    if (IsContest()) {
      ...
      if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_PLAYER) {
        ...
        otId =
            GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattleAnimAttacker]],
                       MON_DATA_OT_ID);
        if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker]
                .transformSpecies == SPECIES_NONE)
          species = GetMonData(
              &gPlayerParty[gBattlerPartyIndexes[gBattleAnimAttacker]],
              MON_DATA_SPECIES);
        else
        ...
        if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker]
                .transformSpecies == SPECIES_NONE)
          species = GetMonData(
              &gEnemyParty[gBattlerPartyIndexes[gBattleAnimAttacker]],
              MON_DATA_SPECIES);
        else
          species = gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker]
                        .transformSpecies;
    ...
    spriteId2 = CreateAdditionalMonSpriteForMoveAnim(
        species, isBackPic, 0, x,
        GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y), subpriority,
        personality, otId, gBattleAnimAttacker, FALSE);
    if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker]
            .transformSpecies != SPECIES_NONE)
      BlendPalette(OBJ_PLTT_ID(gSprites[spriteId2].oam.paletteNum), 16, 6,
                   RGB_WHITE);
  ...
  case 4:
    ...
    if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_PLAYER) {
      gSprites[spriteId].x2 += (gTasks[taskId].data[1] >> 8);
      if (gSprites[spriteId].x2 + gSprites[spriteId].x >=
          GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X))
        gSprites[spriteId].x2 = 0;
    } else {
      gSprites[spriteId].x2 -= (gTasks[taskId].data[1] >> 8);
...
static void AnimUnusedItemBagSteal(struct Sprite *sprite) {
  ...
  case 0:
    if (gBattleAnimArgs[7] == -1) {
      PlaySE12WithPanning(SE_M_VITAL_THROW,
                          BattleAnimAdjustPanning(SOUND_PAN_TARGET));
      sprite->y =
          GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y) + 16;
      sprite->data[0] = -32;
      sprite->data[7]++;
      sprite->invisible = FALSE;
      if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_OPPONENT &&
...
void AnimTask_SnatchPartnerMove(u8 taskId) {
  ...
  case 1:
    ...
    if (gTasks[taskId].data[0] > 0) {
      if (gSprites[spriteId].x + gSprites[spriteId].x2 >=
...
static void AnimRecycle(struct Sprite *sprite) {
  ...
  SetGpuReg(REG_OFFSET_BLDALPHA,
            BLDALPHA_BLEND(sprite->data[6], sprite->data[7]));
}
...
void AnimTask_SlackOffSquish(u8 taskId) {
  ...
  task->data[0] = 0;
  task->data[15] = GetAnimBattlerSpriteId(gBattleAnimArgs[0]);
  PrepareAffineAnimInTaskData(task, task->data[15],
