...
static u8 CheckForObjectEventStaticCollision(struct ObjectEvent *, s16, s16, u8,
                                             u8);
...
static bool8 PushBoulder_Start(struct Task *, struct ObjectEvent *,
                               struct ObjectEvent *);
static bool8 PushBoulder_Move(struct Task *, struct ObjectEvent *,
                              struct ObjectEvent *);
static bool8 PushBoulder_End(struct Task *, struct ObjectEvent *,
                             struct ObjectEvent *);
...
static bool8 PlayerAvatar_SecretBaseMatSpinStep0(struct Task *,
                                                 struct ObjectEvent *);
static bool8 PlayerAvatar_SecretBaseMatSpinStep1(struct Task *,
                                                 struct ObjectEvent *);
static bool8 PlayerAvatar_SecretBaseMatSpinStep2(struct Task *,
                                                 struct ObjectEvent *);
static bool8 PlayerAvatar_SecretBaseMatSpinStep3(struct Task *,
                                                 struct ObjectEvent *);
...
static const u16 sRivalAvatarGfxIds[][2] = {
    [PLAYER_AVATAR_STATE_NORMAL] = {OBJ_EVENT_GFX_RIVAL_BRENDAN_NORMAL,
                                    OBJ_EVENT_GFX_RIVAL_MAY_NORMAL},
    [PLAYER_AVATAR_STATE_MACH_BIKE] = {OBJ_EVENT_GFX_RIVAL_BRENDAN_MACH_BIKE,
                                       OBJ_EVENT_GFX_RIVAL_MAY_MACH_BIKE},
    [PLAYER_AVATAR_STATE_ACRO_BIKE] = {OBJ_EVENT_GFX_RIVAL_BRENDAN_ACRO_BIKE,
                                       OBJ_EVENT_GFX_RIVAL_MAY_ACRO_BIKE},
    [PLAYER_AVATAR_STATE_SURFING] = {OBJ_EVENT_GFX_RIVAL_BRENDAN_SURFING,
    ...
    [PLAYER_AVATAR_STATE_FIELD_MOVE] = {OBJ_EVENT_GFX_RIVAL_BRENDAN_FIELD_MOVE,
...
static const u16 sFRLGAvatarGfxIds[GENDER_COUNT] = {
    [MALE] = OBJ_EVENT_GFX_RED, [FEMALE] = OBJ_EVENT_GFX_LEAF};
...
static bool8 (*const sPushBoulderFuncs[])(struct Task *, struct ObjectEvent *,
                                          struct ObjectEvent *) = {
    PushBoulder_Start,
    PushBoulder_Move,
    PushBoulder_End,
...
static bool8 (*const sPlayerAvatarSecretBaseMatJump[])(struct Task *,
                                                       struct ObjectEvent *) = {
    PlayerAvatar_DoSecretBaseMatJump,
};

static bool8 (*const sPlayerAvatarSecretBaseMatSpin[])(struct Task *,
                                                       struct ObjectEvent *) = {
...
static u8 ObjectEventCB2_NoMovement2(void) { return 0; }
...
static bool8
TryInterruptObjectEventSpecialAnim(struct ObjectEvent *playerObjEvent,
                                   u8 direction) {
  if (ObjectEventIsMovementOverridden(playerObjEvent) &&
      !ObjectEventClearHeldMovementIfFinished(playerObjEvent)) {
    u8 heldMovementActionId =
        ObjectEventGetHeldMovementActionId(playerObjEvent);
...
static void MovePlayerAvatarUsingKeypadInput(u8 direction, u16 newKeys,
                                             u16 heldKeys) {
...
static void MovePlayerNotOnBike(u8 direction, u16 heldKeys) {
  sPlayerNotOnBikeFuncs[CheckMovementInputNotOnBike(direction)](direction,
                                                                heldKeys);
}
...
static u8 CheckMovementInputNotOnBike(u8 direction) {
  ...
  else if (direction != GetPlayerMovementDirection() &&
...
static u8 CheckForPlayerAvatarStaticCollision(u8 direction) {
  ...
  y = playerObjEvent->currentCoords.y;
  MoveCoords(direction, &x, &y);
  return CheckForObjectEventStaticCollision(playerObjEvent, x, y, direction,
                                            MapGridGetMetatileBehaviorAt(x, y));
}

u8 CheckForObjectEventCollision(struct ObjectEvent *objectEvent, s16 x, s16 y,
                                u8 direction, u8 metatileBehavior) {
  u8 collision = GetCollisionAtCoords(objectEvent, x, y, direction);
  if (collision == COLLISION_ELEVATION_MISMATCH &&
      CanStopSurfing(x, y, direction))
...
static u8 CheckForObjectEventStaticCollision(struct ObjectEvent *objectEvent,
...
static bool8 TryPushBoulder(s16 x, s16 y, u8 direction) {
  if (FlagGet(FLAG_SYS_USE_STRENGTH)) {
    ...
    if (objectEventId != OBJECT_EVENTS_COUNT &&
      ...
      MoveCoords(direction, &x, &y);
      if (GetCollisionAtCoords(&gObjectEvents[objectEventId], x, y,
...
static void CheckAcroBikeCollision(s16 x, s16 y, u8 metatileBehavior,
                                   u8 *collision) {
  u8 i;
...
static void DoPlayerAvatarTransition(void) {
  ...
  if (flags != 0) {
    for (i = 0; i < ARRAY_COUNT(sPlayerAvatarTransitionFuncs);
         i++, flags >>= 1) {
      if (flags & 1)
        sPlayerAvatarTransitionFuncs[i](
            &gObjectEvents[gPlayerAvatar.objectEventId]);
    }
...
static void PlayerAvatarTransition_MachBike(struct ObjectEvent *objEvent) {
  ObjectEventSetGraphicsId(objEvent, GetPlayerAvatarGraphicsIdByStateId(
...
static void PlayerAvatarTransition_Underwater(struct ObjectEvent *objEvent) {
  ...
  objEvent->fieldEffectSpriteId =
      StartUnderwaterSurfBlobBobbing(objEvent->spriteId);
}
...
static bool8 PlayerAnimIsMultiFrameStationary(void) {
  u8 movementActionId =
      gObjectEvents[gPlayerAvatar.objectEventId].movementActionId;
...
void PlayerOnBikeCollide(u8 direction) {
  ...
  PlayerSetAnimId(GetWalkInPlaceNormalMovementAction(direction),
                  COPY_MOVE_WALK);
}
...
// Stop player on current facing direction once they're done moving and if
// they're not currently Acro Biking on bumpy slope
...
void PlayerIdleWheelie(u8 direction) {
  PlayerSetAnimId(GetAcroWheelieFaceDirectionMovementAction(direction),
                  COPY_MOVE_FACE);
}

// normal to wheelie
void PlayerStartWheelie(u8 direction) {
  PlayerSetAnimId(GetAcroPopWheelieFaceDirectionMovementAction(direction),
                  COPY_MOVE_FACE);
}

// wheelie to normal
void PlayerEndWheelie(u8 direction) {
  PlayerSetAnimId(GetAcroEndWheelieFaceDirectionMovementAction(direction),
                  COPY_MOVE_FACE);
}
...
void PlayerStandingHoppingWheelie(u8 direction) {
  PlaySE(SE_BIKE_HOP);
  PlayerSetAnimId(GetAcroWheelieHopFaceDirectionMovementAction(direction),
                  COPY_MOVE_FACE);
}
...
void PlayerMovingHoppingWheelie(u8 direction) {
  PlaySE(SE_BIKE_HOP);
  PlayerSetAnimId(GetAcroWheelieHopDirectionMovementAction(direction),
                  COPY_MOVE_WALK);
}
...
void PlayerLedgeHoppingWheelie(u8 direction) {
  PlaySE(SE_BIKE_HOP);
  PlayerSetAnimId(GetAcroWheelieJumpDirectionMovementAction(direction),
                  COPY_MOVE_JUMP2);
}
...
void PlayerAcroTurnJump(u8 direction) {
  PlaySE(SE_BIKE_HOP);
  PlayerSetAnimId(GetJumpInPlaceTurnAroundMovementAction(direction),
                  COPY_MOVE_FACE);
}

void PlayerWheelieInPlace(u8 direction) {
  PlaySE(SE_WALL_HIT);
  PlayerSetAnimId(GetAcroWheelieInPlaceDirectionMovementAction(direction),
                  COPY_MOVE_WALK);
}

void PlayerPopWheelieWhileMoving(u8 direction) {
  PlayerSetAnimId(GetAcroPopWheelieMoveDirectionMovementAction(direction),
                  COPY_MOVE_WALK);
}

void PlayerWheelieMove(u8 direction) {
  PlayerSetAnimId(GetAcroWheelieMoveDirectionMovementAction(direction),
                  COPY_MOVE_WALK);
}

void PlayerEndWheelieWhileMoving(u8 direction) {
  PlayerSetAnimId(GetAcroEndWheelieMoveDirectionMovementAction(direction),
                  COPY_MOVE_WALK);
}
...
u8 TestPlayerAvatarFlags(u8 flag) { return gPlayerAvatar.flags & flag; }

u8 GetPlayerAvatarFlags(void) { return gPlayerAvatar.flags; }

u8 GetPlayerAvatarSpriteId(void) { return gPlayerAvatar.spriteId; }

void CancelPlayerForcedMovement(void) { ForcedMovement_None(); }
...
u16 GetRSAvatarGraphicsIdByGender(u8 gender) { return sRSAvatarGfxIds[gender]; }
...
bool8 IsPlayerFacingSurfableFishableWater(void) {
  ...
  if (GetCollisionAtCoords(playerObjEvent, x, y,
...
void SetPlayerAvatarStateMask(u8 flags) {
  gPlayerAvatar.flags &=
      (PLAYER_AVATAR_FLAG_DASH | PLAYER_AVATAR_FLAG_FORCED_MOVE |
...
void SetPlayerAvatarExtraStateTransition(u16 graphicsId, u8 transitionFlag) {
  u8 stateFlag = GetPlayerAvatarStateTransitionByGraphicsId(
      graphicsId, gPlayerAvatar.gender);
...
void InitPlayerAvatar(s16 x, s16 y, u8 direction, u8 gender) {
  ...
  SetPlayerAvatarStateMask(PLAYER_AVATAR_FLAG_CONTROLLABLE |
...
static void Task_PushBoulder(u8 taskId) {
  while (sPushBoulderFuncs[gTasks[taskId].tState](
      &gTasks[taskId], &gObjectEvents[gPlayerAvatar.objectEventId],
      &gObjectEvents[gTasks[taskId].tBoulderObjId]))
    ;
}

static bool8 PushBoulder_Start(struct Task *task, struct ObjectEvent *player,
...
static bool8 PushBoulder_Move(struct Task *task, struct ObjectEvent *player,
                              struct ObjectEvent *boulder) {
  ...
  if (!ObjectEventIsMovementOverridden(player) &&
    ...
    ObjectEventSetHeldMovement(
        player, GetWalkInPlaceNormalMovementAction((u8)task->tDirection));
    ObjectEventSetHeldMovement(boulder,
                               GetWalkSlowMovementAction((u8)task->tDirection));
...
static bool8 PushBoulder_End(struct Task *task, struct ObjectEvent *player,
                             struct ObjectEvent *boulder) {
...
static void DoPlayerMatJump(void) {
  DoPlayerAvatarSecretBaseMatJump(
      CreateTask(DoPlayerAvatarSecretBaseMatJump, 0xFF));
}
...
// because data[0] is used to call this, it can be inferred that there may have
// been multiple mat jump functions at one point, so the name for these groups
// of functions is appropriate in assuming the sole use of mat jump.
static u8 PlayerAvatar_DoSecretBaseMatJump(struct Task *task,
                                           struct ObjectEvent *objectEvent) {
  gPlayerAvatar.preventStep = TRUE;
  if (ObjectEventClearHeldMovementIfFinished(objectEvent)) {
    PlaySE(SE_LEDGE);
    ObjectEventSetHeldMovement(objectEvent, GetJumpInPlaceMovementAction(
...
static bool8
PlayerAvatar_SecretBaseMatSpinStep0(struct Task *task,
                                    struct ObjectEvent *objectEvent) {
  task->data[0]++;
...
static bool8
PlayerAvatar_SecretBaseMatSpinStep1(struct Task *task,
                                    struct ObjectEvent *objectEvent) {
...
static bool8
PlayerAvatar_SecretBaseMatSpinStep2(struct Task *task,
                                    struct ObjectEvent *objectEvent) {
  const u8 actions[] = {
...
static bool8
PlayerAvatar_SecretBaseMatSpinStep3(struct Task *task,
                                    struct ObjectEvent *objectEvent) {
  if (ObjectEventClearHeldMovementIfFinished(objectEvent)) {
    ObjectEventSetHeldMovement(
        objectEvent,
        GetWalkSlowMovementAction(GetOppositeDirection(task->data[1])));
    UnlockPlayerFieldControls();
    gPlayerAvatar.preventStep = FALSE;
...
static void Task_WaitStopSurfing(u8 taskId) {
  ...
  if (ObjectEventClearHeldMovementIfFinished(playerObjEvent)) {
    ObjectEventSetGraphicsId(playerObjEvent, GetPlayerAvatarGraphicsIdByStateId(
                                                 PLAYER_AVATAR_STATE_NORMAL));
    ObjectEventSetHeldMovement(
        playerObjEvent,
        GetFaceDirectionMovementAction(playerObjEvent->facingDirection));
    gPlayerAvatar.preventStep = FALSE;
    UnlockPlayerFieldControls();
...
static bool8 Fishing_GetRodOut(struct Task *task) {
  ...
  task->tMinRoundsRequired = minRounds1[task->tFishingRod] +
                             (Random() % minRounds2[task->tFishingRod]);
  ...
  if (JOY_NEW(A_BUTTON)) {
    ...
    if (task->tFrameCounter >= 20) {
      ...
      if (task->tNumDots >= task->tDotsRequired) {
        ...
        AddTextPrinterParameterized(0, FONT_NORMAL, dot, task->tNumDots * 8, 1,
                                    0, NULL);
        task->tNumDots++;
      }
    }
...
static bool8 Fishing_WaitForA(struct Task *task) {
  const s16 reelTimeouts[3] = {
      [OLD_ROD] = 36, [GOOD_ROD] = 33, [SUPER_ROD] = 30};
...
static bool8 Fishing_MonOnHook(struct Task *task) {
  ...
  AddTextPrinterParameterized2(0, FONT_NORMAL, gText_PokemonOnHook, 1, 0,
...
static bool8 Fishing_NotEvenNibble(struct Task *task) {
  ...
  FillWindowPixelBuffer(0, PIXEL_FILL(1));
  AddTextPrinterParameterized2(0, FONT_NORMAL, gText_NotEvenANibble, 1, 0,
                               TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE,
                               TEXT_COLOR_LIGHT_GRAY);
...
static bool8 Fishing_GotAway(struct Task *task) {
  ...
  StartSpriteAnim(
      ...
      GetFishingNoCatchDirectionAnimNum(GetPlayerFacingDirection()));
  FillWindowPixelBuffer(0, PIXEL_FILL(1));
  AddTextPrinterParameterized2(0, FONT_NORMAL, gText_ItGotAway, 1, 0,
                               TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE,
                               TEXT_COLOR_LIGHT_GRAY);
  task->tStep++;
  return TRUE;
...
static bool8 Fishing_PutRodAway(struct Task *task) {
  ...
  if (gSprites[gPlayerAvatar.spriteId].animEnded) {
    ...
    if (gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_SURFING)
      SetSurfBlob_PlayerOffset(
          gObjectEvents[gPlayerAvatar.objectEventId].fieldEffectSpriteId, FALSE,
          0);
    gSprites[gPlayerAvatar.spriteId].x2 = 0;
    gSprites[gPlayerAvatar.spriteId].y2 = 0;
    task->tStep++;
...
static void AlignFishingAnimationFrames(void) {
  ...
  if (gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_SURFING)
    SetSurfBlob_PlayerOffset(
        gObjectEvents[gPlayerAvatar.objectEventId].fieldEffectSpriteId, TRUE,
        playerSprite->y2);
}

void SetSpinStartFacingDir(u8 direction) { sSpinStartFacingDir = direction; }
...
static const u8 sSpinDirections[] = {DIR_SOUTH, DIR_WEST, DIR_EAST, DIR_NORTH,
                                     DIR_SOUTH};
...
static void Task_DoPlayerSpinEntrance(u8 taskId) {
  ...
  case 0:
    ...
    ObjectEventForceSetHeldMovement(
        object, GetFaceDirectionMovementAction(sSpinDirections[tStartDir]));
    tSpinDelayTimer = 0;
    tSpeed = 116;
    tDestY = sprite->y;
...
static u8 TrySpinPlayerForWarp(struct ObjectEvent *object, s16 *delayTimer) {
  ...
  ObjectEventForceSetHeldMovement(
      object,
      GetFaceDirectionMovementAction(sSpinDirections[object->facingDirection]));
  *delayTimer = 0;
