...
static void AnimSimplePaletteBlend(struct Sprite *sprite) {
  ...
  BeginNormalPaletteFade(selectedPalettes, gBattleAnimArgs[1],
...
// Bits 0-6 of the selector parameter result in the following palettes being
// selected:
...
u32 UnpackSelectedBattlePalettes(s16 selector) {
  ...
  return GetBattlePalettesMask(battleBackground, attacker, target,
                               attackerPartner, targetPartner, anim1, anim2);
}
...
// Many uses of this task only set a tNumBlends of 2, which has the effect of
// blending to a color and back once
...
static void BlendColorCycle(u8 taskId, u8 startBlendAmount,
                            u8 targetBlendAmount) {
  u32 selectedPalettes =
      UnpackSelectedBattlePalettes(gTasks[taskId].tPalSelector);
  BeginNormalPaletteFade(selectedPalettes, gTasks[taskId].tDelay,
                         startBlendAmount, targetBlendAmount,
...
static void BlendColorCycleExclude(u8 taskId, u8 startBlendAmount,
                                   u8 targetBlendAmount) {
  u32 selectedPalettes = ((u16)gTasks[taskId].tPalSelectorHi << 16) |
                         (u16)gTasks[taskId].tPalSelectorLo;
  BeginNormalPaletteFade(selectedPalettes, gTasks[taskId].tDelay,
                         startBlendAmount, targetBlendAmount,
                         gTasks[taskId].tBlendColor);
...
static void BlendColorCycleByTag(u8 taskId, u8 startBlendAmount,
                                 u8 targetBlendAmount) {
  u8 paletteIndex = IndexOfSpritePaletteTag(gTasks[taskId].tPalTag);
  BeginNormalPaletteFade(1 << (paletteIndex + 16), gTasks[taskId].tDelay,
                         startBlendAmount, targetBlendAmount,
                         gTasks[taskId].tBlendColor);
...
// Flashes the specified anim tag with given color. Used e.g. to flash the
// particles red in Hyper Beam
void AnimTask_FlashAnimTagWithColor(u8 taskId) {
  ...
  BeginNormalPaletteFade(1 << (paletteIndex + 16), 0, gBattleAnimArgs[4],
                         gBattleAnimArgs[4], gBattleAnimArgs[3]);
...
static void AnimTask_FlashAnimTagWithColor_Step1(u8 taskId) {
  ...
  selectedPalettes =
      1 << (IndexOfSpritePaletteTag(gTasks[taskId].data[7]) + 16);
  if (gTasks[taskId].data[1] & 0x100) {
    BeginNormalPaletteFade(selectedPalettes, 0, gTasks[taskId].data[4],
                           gTasks[taskId].data[4], gTasks[taskId].data[3]);
  } else {
    BeginNormalPaletteFade(selectedPalettes, 0, gTasks[taskId].data[6],
...
void AnimTask_TintPalettes(u8 taskId) {
  ...
  TintPlttBuffer(selectedPalettes, gTasks[taskId].tColorR,
                 gTasks[taskId].tColorG, gTasks[taskId].tColorB);
...
static void AnimShakeMonOrBattleTerrain_UpdateCoordOffsetEnabled(void) {
  ...
  if (gBattleAnimArgs[4] == 2) {
    ...
    if (gBattleAnimArgs[4] == 0)
      gSprites[gBattlerSpriteIds[gBattleAnimAttacker]].coordOffsetEnabled =
          TRUE;
    else
      gSprites[gBattlerSpriteIds[gBattleAnimTarget]].coordOffsetEnabled = TRUE;
  }
...
// Can shake battle terrain back and forth on the X or down and back to original
// pos on Y (cant shake up from orig pos) arg0: x offset of shake arg1: y offset
// of shake arg2: number of shakes arg3: time between shakes
...
// For paired hit splats whose position is inverted when used by the opponent on
// the player. Used by Twineedle and Spike Cannon
