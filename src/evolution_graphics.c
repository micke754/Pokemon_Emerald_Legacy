...
static const u16 sEvoSparkle_Pal[] =
    INCBIN_U16("graphics/misc/evo_sparkle.gbapal");
static const u32 sEvoSparkle_Gfx[] =
    INCBIN_U32("graphics/misc/evo_sparkle.4bpp.lz");

static const struct CompressedSpriteSheet sEvoSparkleSpriteSheets[] = {
    {sEvoSparkle_Gfx, 0x20, TAG_SPARKLE}, {NULL, 0, 0}};

static const struct SpritePalette sEvoSparkleSpritePals[] = {
    {sEvoSparkle_Pal, TAG_SPARKLE}, {NULL, 0}};
...
static const union AnimCmd sSpriteAnim_EvoSparkle[] = {ANIMCMD_FRAME(0, 8),
                                                       ANIMCMD_END};
...
static const u16 sEvoSparkleMatrices[] = {0x3C0, 0x380, 0x340, 0x300,
                                          0x2C0, 0x280, 0x240, 0x200,
                                          0x1C0, 0x180, 0x140, 0x100};

static const s16 sUnused[] = {-4, 0x10, -3, 0x30, -2, 0x50, -1, 0x70,
...
static void CreateSparkle_SpiralUpward(u8 trigIdx) {
  u8 spriteId =
      CreateSprite(&sEvoSparkleSpriteTemplate, DISPLAY_WIDTH / 2, 88, 0);
...
static void CreateSparkle_ArcDown(u8 trigIdx) {
  u8 spriteId =
      CreateSprite(&sEvoSparkleSpriteTemplate, DISPLAY_WIDTH / 2, 8, 0);
  if (spriteId != MAX_SPRITES) {
...
static void CreateSparkle_CircleInward(u8 trigIdx, u8 speed) {
  u8 spriteId =
      CreateSprite(&sEvoSparkleSpriteTemplate, DISPLAY_WIDTH / 2, 56, 0);
  if (spriteId != MAX_SPRITES) {
...
static void Task_Sparkles_SpiralUpward_Init(u8 taskId) {
  ...
  PlaySE(SE_M_MEGA_KICK); // 'Charging up' sound for the sparkles as they spiral
                          // upwards
}
...
static void Task_Sparkles_ArcDown_End(u8 taskId) { DestroyTask(taskId); }
...
static void Task_Sparkles_CircleInward_End(u8 taskId) { DestroyTask(taskId); }
...
static void Task_Sparkles_SprayAndFlash_Init(u8 taskId) {
  ...
  CpuCopy16(&gPlttBufferFaded[BG_PLTT_ID(2)],
            &gPlttBufferUnfaded[BG_PLTT_ID(2)], 3 * PLTT_SIZE_4BPP);
  BeginNormalPaletteFade(0xFFF9041C, 0, 0, 0x10,
                         RGB_WHITE); // was 0xFFF9001C in R/S
...
static void Task_Sparkles_SprayAndFlash(u8 taskId) {
  if (gTasks[taskId].tTimer < 128) {
    ...
    case 0:
      ...
      break;
    case 32:
      BeginNormalPaletteFade(0xFFFF041C, 0x10, 0x10, 0,
                             RGB_WHITE); // was 0xFFF9001C in R/S
      break;
    }
    gTasks[taskId].tTimer++;
...
static void Task_Sparkles_SprayAndFlashTrade_Init(u8 taskId) {
  ...
  CpuCopy16(&gPlttBufferFaded[BG_PLTT_ID(2)],
            &gPlttBufferUnfaded[BG_PLTT_ID(2)], 3 * PLTT_SIZE_4BPP);
  BeginNormalPaletteFade(0xFFF90400, 0, 0, 0x10,
                         RGB_WHITE); // was 0xFFFF0001 in R/S
...
static void Task_Sparkles_SprayAndFlashTrade(u8 taskId) {
  if (gTasks[taskId].tTimer < 128) {
    ...
    case 0:
      ...
      break;
    case 32:
      BeginNormalPaletteFade(0xFFFF0400, 0x10, 0x10, 0,
                             RGB_WHITE); // was 0xFFFF0001 in R/S
      break;
    }
    gTasks[taskId].tTimer++;
...
u8 CycleEvolutionMonSprite(u8 preEvoSpriteId, u8 postEvoSpriteId) {
  ...
  SetOamMatrix(MATRIX_POST_EVO, toDiv / gTasks[taskId].tPostEvoScale, 0, 0,
               toDiv / gTasks[taskId].tPostEvoScale);
  ...
  CpuSet(
      monPalette,
      &gPlttBufferFaded[OBJ_PLTT_ID(gSprites[preEvoSpriteId].oam.paletteNum)],
      16);
  ...
  CpuSet(
      monPalette,
      &gPlttBufferFaded[OBJ_PLTT_ID(gSprites[postEvoSpriteId].oam.paletteNum)],
      16);
...
// If evo hasn't stopped or growth/shrink speed hasn't been maxed out, start
// another cycle
static void Task_CycleEvolutionMonSprite_TryEnd(u8 taskId) {
...
static void Task_CycleEvolutionMonSprite_UpdateSize(u8 taskId) {
  if (gTasks[taskId].tEvoStopped)
    ...
    if (!gTasks[taskId].tShowingPostEvo) {
      ...
      if (gTasks[taskId].tPreEvoScale <
          MON_MAX_SCALE - gTasks[taskId].tScaleSpeed)
      ...
      if (gTasks[taskId].tPostEvoScale >
          MON_MIN_SCALE + gTasks[taskId].tScaleSpeed)
