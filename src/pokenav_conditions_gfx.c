...
static const struct WindowTemplate sListIndexWindowTemplate = {.bg = 1,
                                                               .tilemapLeft = 1,
                                                               .tilemapTop = 6,
                                                               .width = 7,
                                                               .height = 2,
...
static const struct WindowTemplate sUnusedWindowTemplate1 = {.bg = 1,
                                                             .tilemapLeft = 1,
                                                             .tilemapTop = 0x1C,
                                                             .width = 5,
                                                             .height = 2,
...
static const struct WindowTemplate sUnusedWindowTemplate2 = {.bg = 1,
                                                             .tilemapLeft = 13,
                                                             .tilemapTop = 0x1C,
                                                             .width = 3,
                                                             .height = 2,
...
extern s8 GetConditionGraphMenuCurrentLoadIndex(
    void); // This function's declaration here is s8 vs. u8 in
...
bool32 OpenConditionGraphMenu(void) {
  struct Pokenav_ConditionMenuGfx *menu =
      AllocSubstruct(POKENAV_SUBSTRUCT_CONDITION_GRAPH_MENU_GFX,
                     sizeof(struct Pokenav_ConditionMenuGfx));
...
static u32 LoopedTask_OpenConditionGraphMenu(s32 state) {
  ...
  case 1:
    ...
    SetGpuReg(REG_OFFSET_BLDCNT,
              BLDCNT_TGT1_BG2 | BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_BG3);
  ...
  case 3:
    ...
    CopyPaletteIntoBufferUnfaded(gPokenavCondition_Pal, BG_PLTT_ID(1),
                                 PLTT_SIZE_4BPP);
    CopyPaletteIntoBufferUnfaded(gConditionText_Pal, BG_PLTT_ID(15),
                                 PLTT_SIZE_4BPP);
  ...
  case 4:
    ...
    CopyPaletteIntoBufferUnfaded(gConditionGraphData_Pal, BG_PLTT_ID(3),
                                 PLTT_SIZE_4BPP);
  ...
  case 9:
    ...
    return LT_INC_AND_PAUSE;
  case 10:
    UpdateConditionGraphMenuWindows(0, GetConditionGraphMenuCurrentLoadIndex(),
                                    TRUE);
    return LT_INC_AND_PAUSE;
  case 11:
    UpdateConditionGraphMenuWindows(1, GetConditionGraphMenuCurrentLoadIndex(),
                                    TRUE);
    return LT_INC_AND_PAUSE;
  case 12:
    UpdateConditionGraphMenuWindows(2, GetConditionGraphMenuCurrentLoadIndex(),
                                    TRUE);
    return LT_INC_AND_PAUSE;
  case 13:
    if (UpdateConditionGraphMenuWindows(
            3, GetConditionGraphMenuCurrentLoadIndex(), TRUE) != TRUE)
      return LT_PAUSE;
  ...
  case 19:
    ...
    return LT_INC_AND_PAUSE;
  case 20:
    if (!ConditionMenu_UpdateMonEnter(GetConditionGraphPtr(),
                                      &menu->monTransitionX)) {
      ResetConditionSparkleSprites(menu->conditionSparkleSprites);
      if (IsConditionMenuSearchMode() == TRUE ||
          GetConditionGraphCurrentListIndex() != GetMonListCount())
        CreateConditionSparkleSprites(menu->conditionSparkleSprites,
                                      menu->monPicSpriteId,
...
static u32 LoopedTask_ExitConditionGraphMenu(s32 state) {
  ...
  case 1:
    if (ConditionMenu_UpdateMonExit(GetConditionGraphPtr(),
                                    &menu->monTransitionX))
      return 2;
...
static u32 LoopedTask_TransitionMons(s32 state) {
  ...
  case 8:
    if (UpdateConditionGraphMenuWindows(
            3, GetConditionGraphMenuCurrentLoadIndex(), FALSE) == TRUE)
      return LT_INC_AND_CONTINUE;
    return LT_PAUSE;
  case 9:
...
static u8 UNUSED *UnusedPrintNumberString(u8 *dst, u16 num) {
  u8 *txtPtr =
      ConvertIntToDecimalStringN(dst, num, STR_CONV_MODE_RIGHT_ALIGN, 4);
  txtPtr = StringCopy(txtPtr, gText_Number2);

  return txtPtr;
}

static bool32 UpdateConditionGraphMenuWindows(u8 mode, u16 bufferIndex,
                                              bool8 winMode) {
  u8 text[32];
  const u8 *str;
  struct Pokenav_ConditionMenuGfx *menu =
  ...
  case 1:
    if (GetConditionGraphCurrentListIndex() != GetMonListCount() - 1 ||
        IsConditionMenuSearchMode() == TRUE) {
      str = GetConditionMonNameText(bufferIndex);
      AddTextPrinterParameterized(menu->nameGenderWindowId, FONT_NORMAL, str, 0,
                                  1, 0, NULL);
    }
    break;
  case 2:
    if (IsConditionMenuSearchMode() == TRUE) {
      ...
      AddTextPrinterParameterized(menu->listIndexWindowId, FONT_NORMAL, text, 4,
                                  1, 0, NULL);
      ConvertIntToDecimalStringN(&text[5], GetConditionMonDataBuffer(),
                                 STR_CONV_MODE_RIGHT_ALIGN, 4);
      AddTextPrinterParameterized(menu->listIndexWindowId, FONT_NORMAL, text,
                                  28, 1, 0, NULL);
    }
    break;
  case 3:
...
void FreeConditionGraphMenuSubstruct2(void) {
  ...
  SetGpuReg(REG_OFFSET_DISPCNT,
            DISPCNT_OBJ_ON | DISPCNT_BG0_ON | DISPCNT_OBJ_1D_MAP);
...
static void CreateConditionMonPic(u8 id) {
  ...
  if (menu->monPicSpriteId == SPRITE_NONE) {
    ...
    if (spriteId == MAX_SPRITES) {
      ...
      menu->monGfxPtr =
          (void *)VRAM + BG_VRAM_SIZE + (menu->monGfxTileStart * 32);
      menu->monPalIndex = OBJ_PLTT_ID(menu->monPalIndex);
    }
  } else {
    DmaCopy16Defvars(3, GetConditionMonPicGfx(id), menu->monGfxPtr,
                     MON_PIC_SIZE);
...
static void SetExitVBlank(void) { SetPokenavVBlankCallback(); }
...
static void DoConditionGraphEnterTransition(void) {
  ...
  ConditionGraph_SetNewPositions(
      graph, graph->savedPositions[CONDITION_GRAPH_LOAD_MAX - 1],
...
static void DoConditionGraphExitTransition(void) {
  ...
  if (IsConditionMenuSearchMode() ||
      GetConditionGraphCurrentListIndex() != GetMonListCount() - 1)
    ConditionGraph_SetNewPositions(
        graph, graph->savedPositions[GetConditionGraphMenuCurrentLoadIndex()],
        graph->savedPositions[CONDITION_GRAPH_LOAD_MAX - 1]);
}
