...
u8 GiveMailToMonByItemId(struct Pokemon *mon, u16 itemId) {
  ...
  for (id = 0; id < PARTY_SIZE; id++) {
    if (gSaveBlock1Ptr->mail[id].itemId == ITEM_NONE) {
      ...
      for (i = 0; i < TRAINER_ID_LENGTH; i++)
        gSaveBlock1Ptr->mail[id].trainerId[i] =
      ...
      gSaveBlock1Ptr->mail[id].species =
          SpeciesToMailSpecies(species, personality);
...
static bool32 UNUSED DummyMailFunc(void) { return FALSE; }
