...
#define BATTLE_HISTORY                                                         \
  ((struct BattleHistory *)(gBattleResources->battleHistory))
...
in order to read the next command correctly. refer to battle_ai_scripts.s for
the AI scripts.
*/
...
static const u16 sIgnoredPowerfulMoveEffects[] = {
    EFFECT_EXPLOSION,  EFFECT_DREAM_EATER, EFFECT_RAZOR_WIND,
    EFFECT_SKY_ATTACK, EFFECT_RECHARGE,    EFFECT_SKULL_BASH,
    EFFECT_SOLAR_BEAM, EFFECT_SPIT_UP,     EFFECT_FOCUS_PUNCH,
    EFFECT_SUPERPOWER, EFFECT_ERUPTION,    EFFECT_OVERHEAT,
    IGNORED_MOVES_END};
  ...
  if ((gBattleTypeFlags & BATTLE_TYPE_TRAINER) &&
      !(gBattleTypeFlags &
        (BATTLE_TYPE_LINK | BATTLE_TYPE_SAFARI | BATTLE_TYPE_BATTLE_TOWER |
         ...
         BATTLE_TYPE_FRONTIER | BATTLE_TYPE_INGAME_PARTNER |
...
void BattleAI_SetupAIData(u8 defaultScoreMoves) {
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE) {
    gBattlerTarget = (Random() & BIT_FLANK) +
                     BATTLE_OPPOSITE(GetBattlerSide(gActiveBattler));
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
    AI_THINKING_STRUCT->aiFlags |=
        AI_SCRIPT_DOUBLE_BATTLE; // act smart in doubles and don't attack your
                                 // partner
}
  ...
  for (i = 0; i < MAX_BATTLERS_COUNT; i++) {
    if (i == sBattler_AI || gBattleMons[i].hp == 0) {
      ...
      if (AI_THINKING_STRUCT->aiAction & AI_ACTION_FLEE) {
        ...
        actionOrMoveIndex[i] =
            mostViableMovesIndices[Random() % mostViableMovesNo];
        bestMovePointsForTarget[i] = mostViableMovesScores[0];

        // Don't use a move against ally if it has less than 100 points.
        if (i == BATTLE_PARTNER(sBattler_AI) &&
            bestMovePointsForTarget[i] < 100) {
          bestMovePointsForTarget[i] = -1;
          mostViableMovesScores[0] =
              mostViableMovesScores[0]; // Needed to match.
        }
      }
    }
  }
...
static void BattleAI_DoAIProcessing(void) {
  while (AI_THINKING_STRUCT->aiState != AIState_FinishedProcessing) {
    ...
    case AIState_SettingUp:
      gAIScriptPtr =
          gBattleAI_ScriptsTable[AI_THINKING_STRUCT->aiLogicId]; // set AI ptr
                                                                 // to logic ID.
...
static void Cmd_score(void) {
  AI_THINKING_STRUCT->score[AI_THINKING_STRUCT->movesetIndex] +=
      gAIScriptPtr[1]; // Add the result to the array of the move consider's
                       // score.
...
static void Cmd_if_hp_less_than(void) {
  ...
  if ((u32)(100 * gBattleMons[battlerId].hp / gBattleMons[battlerId].maxHP) <
      gAIScriptPtr[2])
...
static void Cmd_get_type(void) {
  ...
  case AI_TYPE_MOVE: // type of move being pointed to
    AI_THINKING_STRUCT->funcResult =
        gBattleMoves[AI_THINKING_STRUCT->moveConsidered].type;
    break;
  }
  gAIScriptPtr += 2;
}
...
static void Cmd_get_considered_move_power(void) {
  AI_THINKING_STRUCT->funcResult =
      gBattleMoves[AI_THINKING_STRUCT->moveConsidered].power;
  gAIScriptPtr += 1;
}
...
static void Cmd_get_considered_move_effect(void) {
  AI_THINKING_STRUCT->funcResult =
      gBattleMoves[AI_THINKING_STRUCT->moveConsidered].effect;
  gAIScriptPtr += 1;
}
...
static void Cmd_get_ability(void) {
  ...
  if (gActiveBattler != battlerId) {
    ...
    if (gSpeciesInfo[gBattleMons[battlerId].species].abilities[0] !=
        ABILITY_NONE) {
      if (gSpeciesInfo[gBattleMons[battlerId].species].abilities[1] !=
          ABILITY_NONE) {
        // AI has no knowledge of opponent, so it guesses which ability.
        if (Random() & 1)
          AI_THINKING_STRUCT->funcResult =
              gSpeciesInfo[gBattleMons[battlerId].species].abilities[0];
        else
          AI_THINKING_STRUCT->funcResult =
              gSpeciesInfo[gBattleMons[battlerId].species].abilities[1];
      } else {
        AI_THINKING_STRUCT->funcResult =
            gSpeciesInfo[gBattleMons[battlerId].species]
                .abilities[0]; // It's definitely ability 1.
      }
    } else {
      AI_THINKING_STRUCT->funcResult =
          gSpeciesInfo[gBattleMons[battlerId].species]
              .abilities[1]; // AI can't actually reach this part since no
                             // Pokémon has ability 2 and no ability 1.
    }
  } else {
...
static void Cmd_check_ability(void) {
  ...
  if (gAIScriptPtr[1] == AI_TARGET || gAIScriptPtr[1] == AI_TARGET_PARTNER) {
    ...
    // Abilities that prevent fleeing.
    ...
    } else if (gSpeciesInfo[gBattleMons[battlerId].species].abilities[0] !=
               ABILITY_NONE) {
      if (gSpeciesInfo[gBattleMons[battlerId].species].abilities[1] !=
          ABILITY_NONE) {
      ...
      ability = gSpeciesInfo[gBattleMons[battlerId].species]
                    .abilities[1]; // AI can't actually reach this part since no
                                   // Pokémon has ability 2 and no ability 1.
    }
  } else {
  ...
  if (ability == 0)
    ...
    AI_THINKING_STRUCT->funcResult =
        1; // Pokémon has the ability we wanted to check.
  else
    AI_THINKING_STRUCT->funcResult =
        0; // Pokémon doesn't have the ability we wanted to check.
...
static void Cmd_if_status_in_party(void) {
  ...
  party =
      (GetBattlerSide(battlerId) == B_SIDE_PLAYER) ? gPlayerParty : gEnemyParty;
...
static void Cmd_if_has_move_with_effect(void) {
  ...
  case AI_USER_PARTNER:
    for (i = 0; i < MAX_MON_MOVES; i++) {
      if (gBattleMons[sBattler_AI].moves[i] != 0 &&
          gBattleMoves[gBattleMons[sBattler_AI].moves[i]].effect ==
              gAIScriptPtr[2])
        break;
    }
    if (i == MAX_MON_MOVES)
  ...
  case AI_TARGET_PARTNER:
    for (i = 0; i < MAX_MON_MOVES; i++) {
      if (gBattleMons[gBattlerTarget].moves[i] != 0 &&
          gBattleMoves[BATTLE_HISTORY->usedMoves[gBattlerTarget].moves[i]]
                  .effect == gAIScriptPtr[2])
        break;
    }
    if (i == MAX_MON_MOVES)
      gAIScriptPtr += 7;
...
static void Cmd_flee(void) {
  AI_THINKING_STRUCT->aiAction |=
      (AI_ACTION_DONE | AI_ACTION_FLEE | AI_ACTION_DO_NOT_ATTACK);
}

static void Cmd_if_random_safari_flee(void) {
  u8 safariFleeRate =
      gBattleStruct->safariEscapeFactor * 5; // Safari flee rate, from 0-20.
...
static void Cmd_get_hold_effect(void) {
  ...
  if (gActiveBattler != battlerId)
    ...
    AI_THINKING_STRUCT->funcResult =
        ItemId_GetHoldEffect(gBattleMons[battlerId].item);
...
static void Cmd_get_used_held_item(void) {
  ...
  AI_THINKING_STRUCT->funcResult =
      *(u8 *)&gBattleStruct->usedHeldItems[battlerId];
...
static void Cmd_get_move_type_from_result(void) {
  AI_THINKING_STRUCT->funcResult =
      gBattleMoves[AI_THINKING_STRUCT->funcResult].type;
...
static void Cmd_get_move_power_from_result(void) {
  AI_THINKING_STRUCT->funcResult =
      gBattleMoves[AI_THINKING_STRUCT->funcResult].power;
...
static void Cmd_get_move_effect_from_result(void) {
  AI_THINKING_STRUCT->funcResult =
      gBattleMoves[AI_THINKING_STRUCT->funcResult].effect;
...
static void Cmd_goto(void) { gAIScriptPtr = T1_READ_PTR(gAIScriptPtr + 1); }
...
static void AIStackPushVar(const u8 *var) {
  gBattleResources->AI_ScriptsStack
      ->ptr[gBattleResources->AI_ScriptsStack->size++] = var;
}

static void UNUSED AIStackPushVar_cursor(void) {
  gBattleResources->AI_ScriptsStack
      ->ptr[gBattleResources->AI_ScriptsStack->size++] = gAIScriptPtr;
}

static bool8 AIStackPop(void) {
  if (gBattleResources->AI_ScriptsStack->size != 0) {
    --gBattleResources->AI_ScriptsStack->size;
    gAIScriptPtr = gBattleResources->AI_ScriptsStack
                       ->ptr[gBattleResources->AI_ScriptsStack->size];
    return TRUE;
  } else {
    return FALSE;
  }
