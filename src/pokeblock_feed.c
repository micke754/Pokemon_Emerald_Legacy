...
// The animation the Pokémon does during the feeding scene depends on their
// nature. The below values are offsets into sMonPokeblockAnims of the animation
// data for that nature.
#define ANIM_HARDY 0
...
// Data for the animation the Pokémon does while readying to jump for the
// Pokéblock Each nature can have up to 8 anim 'stages' it progresses through,
// and each stage has its own array of data. The elements in each array
// correspond in order to the following:
...
// - ANIMDATA_APPR_TIME    : The time over which the target position should be
// approached
// - ANIMDATA_IS_LAST      : TRUE if it's the last anim stage for this nature,
// FALSE otherwise
//
static const s16 sMonPokeblockAnims[][NUM_ANIMDATA] = {
    [ANIM_HARDY] = {0, 4, 0, 8, 24, 0, 0, 0, 12, FALSE},
    {0, 4, 0, 16, 24, 0, 0, 0, 12, FALSE},
    {0, 4, 0, 32, 32, 0, 0, 0, 16, TRUE},

    [ANIM_LONELY] = {0, 3, 6, 0, 48, 0, 0, 0, 24, TRUE},

    [ANIM_BRAVE] = {64, 16, -24, 0, 32, 0, 0, 0, 0, TRUE},

    [ANIM_ADAMANT] = {0, 4, 8, 0, 16, 0, -8, 0, 0, FALSE},
    {0, 0, 0, 0, 16, 0, 0, 0, 0, FALSE},
    {0, 4, 8, 0, 16, 0, -8, 0, 0, FALSE},
    {0, 0, 0, 0, 16, 0, 0, 0, 0, FALSE},
    {0, 4, -16, 0, 4, 0, 16, 0, 0, TRUE},

    [ANIM_NAUGHTY] = {0, 3, 6, 0, 12, 0, 0, 0, 6, FALSE},
    {0, 3, -6, 0, 12, 0, 0, 0, 6, FALSE},
    {0, 16, 16, 0, 45, 1, 0, 0, 0, TRUE},

    [ANIM_BOLD] = {0, 16, 0, 24, 32, 0, 0, 0, 16, FALSE},
    {0, 16, 0, 23, 32, 0, 0, 0, 16, TRUE},

    [ANIM_DOCILE] = {0, 0, 0, 0, 80, 0, 0, 0, 0, TRUE},

    [ANIM_RELAXED] = {0, 2, 8, 0, 32, 0, 0, 0, 0, FALSE},
    {0, 2, -8, 0, 32, 0, 0, 0, 0, TRUE},

    [ANIM_IMPISH] = {0, 32, 2, 1, 48, 1, 0, 0, 24, TRUE},

    [ANIM_LAX] = {0, 2, 16, 16, 128, 0, 0, 0, 0, TRUE},

    [ANIM_TIMID] = {0, 2, -8, 0, 48, 0, -24, 0, 0, FALSE},
    {0, 0, 0, 0, 8, 0, 0, 0, 0, FALSE},
    {64, 32, 2, 0, 36, 0, 0, 0, 0, FALSE},
    {0, 0, 0, 0, 8, 0, 0, 0, 0, FALSE},
    {0, 2, 8, 0, 48, 0, 24, 0, 0, TRUE},

    [ANIM_HASTY] = {64, 24, 16, 0, 32, 0, 0, 0, 0, FALSE},
    {0, 28, 2, 1, 32, 1, 0, 0, 16, TRUE},

    [ANIM_SERIOUS] = {0, 0, 0, 0, 32, 0, 0, 0, 0, TRUE},

    [ANIM_JOLLY] = {64, 16, -16, 2, 48, 0, 0, 0, 32, TRUE},

    [ANIM_NAIVE] = {0, 12, -8, 4, 24, 0, 8, 0, 12, FALSE},
    {0, 12, 8, 8, 24, 0, -16, 0, 12, FALSE},
    {0, 12, -8, 16, 24, 0, 16, 0, 12, FALSE},
    {0, 12, 8, 28, 24, 0, -8, 0, 12, TRUE},

    [ANIM_MODEST] = {0, 0, 0, 0, 8, 0, 0, 0, 0, FALSE},
    {64, 16, -4, 0, 32, 0, 0, 0, 0, FALSE},
    {0, 0, 0, 0, 8, 0, 0, 0, 0, TRUE},

    [ANIM_MILD] = {128, 4, 0, 8, 64, 0, 0, 0, 0, TRUE},

    [ANIM_QUIET] = {0, 2, 16, 0, 48, 0, 0, 0, 0, FALSE},
    {128, 2, 16, 0, 48, 0, 0, 0, 0, TRUE},

    [ANIM_BASHFUL] = {0, 2, -4, 0, 48, 0, -48, 0, 0, FALSE},
    {0, 0, 0, 0, 80, 0, 0, 0, 0, FALSE},
    {0, 2, 8, 0, 24, 0, 48, 0, 0, TRUE},

    [ANIM_RASH] = {64, 4, 64, 58, 52, 0, -88, 0, 0, FALSE},
    {0, 0, 0, 0, 80, 0, 0, 0, 0, FALSE},
    {0, 24, 80, 0, 32, 0, 88, 0, 0, TRUE},

    [ANIM_CALM] = {0, 2, 16, 4, 64, 0, 0, 0, 0, TRUE},

    [ANIM_GENTLE] = {0, 0, 0, 0, 32, 0, 0, 0, 0, TRUE},

    [ANIM_SASSY] = {0, 0, 0, 0, 42, 0, 0, 0, 0, TRUE},

    [ANIM_CAREFUL] = {0, 4, 0, 8, 24, 0, 0, 0, 12, FALSE},
    {0, 0, 0, 0, 12, 0, 0, 0, 0, FALSE},
    {0, 4, 0, 12, 24, 0, 0, 0, 12, FALSE},
    {0, 0, 0, 0, 12, 0, 0, 0, 0, FALSE},
    {0, 4, 0, 4, 24, 0, 0, 0, 12, TRUE},

    [ANIM_QUIRKY] = {0, 4, 16, 12, 64, 0, 0, 0, 0, FALSE},
...
static const union AffineAnimCmd sAffineAnim_Mon_TurnUp[] = {
    AFFINEANIMCMD_FRAME(0, 0, 12, 1), AFFINEANIMCMD_FRAME(0, 0, 0, 30),
    AFFINEANIMCMD_FRAME(0, 0, -12, 1), AFFINEANIMCMD_END};

static const union AffineAnimCmd sAffineAnim_Mon_TurnUp_Flipped[] = {
    AFFINEANIMCMD_FRAME(-0x100, 0x100, 0, 0), AFFINEANIMCMD_FRAME(0, 0, 12, 1),
    AFFINEANIMCMD_FRAME(0, 0, 0, 28), AFFINEANIMCMD_FRAME(0, 0, -4, 3),
    AFFINEANIMCMD_END};

static const union AffineAnimCmd sAffineAnim_Mon_TurnUpAndDown[] = {
    AFFINEANIMCMD_FRAME(0x0, 0x0, 1, 16), AFFINEANIMCMD_FRAME(0x0, 0x0, -1, 32),
    AFFINEANIMCMD_FRAME(0x0, 0x0, 1, 16), AFFINEANIMCMD_END};
...
static const union AffineAnimCmd sAffineAnim_Mon_TurnDown[] = {
    AFFINEANIMCMD_FRAME(0x0, 0x0, -1, 8), AFFINEANIMCMD_FRAME(0x0, 0x0, 0, 16),
    AFFINEANIMCMD_FRAME(0x0, 0x0, 1, 8), AFFINEANIMCMD_END};
...
static const union AffineAnimCmd sAffineAnim_Mon_TurnDownSlow[] = {
    AFFINEANIMCMD_FRAME(0x0, 0x0, -1, 8), AFFINEANIMCMD_FRAME(0x0, 0x0, 0, 32),
    AFFINEANIMCMD_FRAME(0x0, 0x0, 1, 8), AFFINEANIMCMD_END};
...
static const union AffineAnimCmd sAffineAnim_Mon_TurnDownSlight[] = {
    AFFINEANIMCMD_FRAME(0x0, 0x0, -1, 4), AFFINEANIMCMD_FRAME(0x0, 0x0, 0, 24),
    AFFINEANIMCMD_FRAME(0x0, 0x0, 1, 4), AFFINEANIMCMD_END};
...
static const union AffineAnimCmd sAffineAnim_Mon_TurnUpHigh[] = {
    AFFINEANIMCMD_FRAME(0x0, 0x0, 1, 24), AFFINEANIMCMD_FRAME(0x0, 0x0, 0, 16),
    AFFINEANIMCMD_FRAME(0x0, 0x0, -12, 2), AFFINEANIMCMD_END};
...
static const union AffineAnimCmd *const sAffineAnims_Mon[] = {
    ...
    [AFFINE_TURN_UP_AND_DOWN + NUM_MON_AFFINES] =
    ...
    [AFFINE_TURN_DOWN_SLIGHT + NUM_MON_AFFINES] =
        sAffineAnim_Mon_TurnDownSlight_Flipped,
    [AFFINE_TURN_UP_HIGH + NUM_MON_AFFINES] =
...
static const union AffineAnimCmd
    *const sAffineAnims_PokeblockCase_ThrowFromVertical[] = {
        sAffineAnim_PokeblockCase_ThrowFromVertical};

static const union AffineAnimCmd
    *const sAffineAnims_PokeblockCase_ThrowFromHorizontal[] = {
        sAffineAnim_PokeblockCase_ThrowFromHorizontal};
...
static const union AnimCmd sAnim_Pokeblock[] = {ANIMCMD_FRAME(0, 0),
                                                ANIMCMD_END};
...
static const union AffineAnimCmd sAffineAnim_Pokeblock[] = {
    AFFINEANIMCMD_FRAME(0x100, 0x100, 0, 0), AFFINEANIMCMD_FRAME(-8, -8, 0, 1),
...
static bool8 LoadPokeblockFeedScene(void) {
  ...
  case 9:
    sPokeblockFeed->monSpriteId =
        CreateMonSprite(&gPlayerParty[gPokeblockMonId]);
    gMain.state++;
    break;
  case 10:
...
static void HandleInitBackgrounds(void) {
  ...
  InitBgsFromTemplates(0, sBackgroundTemplates,
                       ARRAY_COUNT(sBackgroundTemplates));
...
static bool8 LoadMonAndSceneGfx(struct Pokemon *mon) {
  ...
  case 1:
    ...
    palette = GetMonSpritePalStructFromOtIdPersonality(species, trainerId,
                                                       personality);
  ...
  case 7:
    ...
    break;
  case 8:
    LoadCompressedPalette(gBattleTerrainPalette_Frontier, BG_PLTT_ID(2),
                          3 * PLTT_SIZE_4BPP);
...
#define STATE_START_THROW                                                      \
  255 // If the length of the Pokémon's animation exceeds 255 the throw may
      // happen twice
...
static void Task_HandlePokeblockFeed(u8 taskId) {
  if (!gPaletteFade.active) {
    ...
    case STATE_START_THROW:
      DoPokeblockCaseThrowEffect(sPokeblockFeed->pokeblockCaseSpriteId,
                                 gTasks[taskId].tHorizontalThrow);
      break;
...
static void DoPokeblockCaseThrowEffect(u8 spriteId, bool8 horizontalThrow) {
  ...
  if (!horizontalThrow)
    gSprites[spriteId].affineAnims =
        sAffineAnims_PokeblockCase_ThrowFromVertical; // Never used,
                                                      // horizontalThrow always
                                                      // TRUE
  else
    gSprites[spriteId].affineAnims =
...
static void UpdateMonAnim(void) {
  ...
  case 50:
    if (sNatureToMonPokeblockAnim[pokeblockFeed->nature][1] != AFFINE_NONE) {
      // Start affine anim
      if (!pokeblockFeed
               ->noMonFlip) // double negation, so mon's sprite is flipped
...
static bool8 InitMonAnimStage(void) {
  ...
  if (pokeblockFeed->animData[ANIMDATA_TIME] == 0) {
    ...
    pokeblockFeed->monInitX =
        Sin(pokeblockFeed->animData[ANIMDATA_ROT_IDX],
            pokeblockFeed->animData[ANIMDATA_SIN_AMPLITUDE]);
    pokeblockFeed->monInitY =
        Cos(pokeblockFeed->animData[ANIMDATA_ROT_IDX],
            pokeblockFeed->animData[ANIMDATA_COS_AMPLITUDE]);
    ...
    pokeblockFeed->animData[ANIMDATA_TIME] =
        pokeblockFeed->maxAnimStageTime; // Redundant
    return FALSE;
  }
}
...
static bool8 DoMonAnimStep(void) {
  ...
  u16 time = sPokeblockFeed->maxAnimStageTime -
             sPokeblockFeed->animData[ANIMDATA_TIME];
...
static void CalculateMonAnimMovement(void) {
  ...
  while (1) {
    ...
    time = pokeblockFeed->maxAnimStageTime -
           pokeblockFeed->animData[ANIMDATA_TIME];
    ...
    if (!negative) {
      pokeblockFeed->monAnimX[time] =
          Sin(pokeblockFeed->animData[ANIMDATA_ROT_IDX],
              pokeblockFeed->animData[ANIMDATA_SIN_AMPLITUDE] +
                  amplitude / 0x100) +
          x;
      pokeblockFeed->monAnimY[time] =
          Cos(pokeblockFeed->animData[ANIMDATA_ROT_IDX],
              pokeblockFeed->animData[ANIMDATA_COS_AMPLITUDE] +
                  amplitude / 0x100) +
          y;
    ...
    pokeblockFeed->animData[ANIMDATA_ROT_IDX] +=
        pokeblockFeed->animData[ANIMDATA_ROT_SPEED];
