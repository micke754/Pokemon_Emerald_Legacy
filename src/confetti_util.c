  ...
  for (i = 0; i < sWork->count; i++) {
    if (sWork->array[i].active && sWork->array[i].allowUpdates) {
      ...
      if (sWork->array[i].dummied) {
        memcpy(&gMain.oamBuffer[i + 64], &gDummyOamData,
        ...
        memcpy(&gMain.oamBuffer[i + 64], &sWork->array[i],
               sizeof(struct OamData));
      }
    }
  }
...
static bool32 SetAnimAndTileNum(struct ConfettiUtil *structPtr, u8 animNum) {
  ...
  structPtr->tileNum =
      (GetTilesPerImage(structPtr->oam.shape, structPtr->oam.size) * animNum) +
      tileStart;
  return TRUE;
}
...
u8 ConfettiUtil_AddNew(const struct OamData *oam, u16 tileTag, u16 palTag,
                       s16 x, s16 y, u8 animNum, u8 priority) {
