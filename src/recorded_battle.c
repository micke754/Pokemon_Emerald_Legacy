...
STATIC_ASSERT(sizeof(struct RecordedBattleSave) <= SECTOR_COUNTER_OFFSET,
              RecordedBattleSaveFreeSpace);

EWRAM_DATA u32 gRecordedBattleRngSeed = 0;
EWRAM_DATA u32 gBattlePalaceMoveSelectionRngValue = 0;
EWRAM_DATA static u8 sBattleRecords[MAX_BATTLERS_COUNT][BATTLER_RECORD_SIZE] = {
    0};
...
EWRAM_DATA static u16 sPlayerMonMoves[MAX_BATTLERS_COUNT / 2][MAX_MON_MOVES] = {
    0};
...
void RecordedBattle_SetBattlerAction(u8 battlerId, u8 action) {
  if (sBattlerRecordSizes[battlerId] < BATTLER_RECORD_SIZE &&
...
static u8 UNUSED GetRecordedBattleMode(void) { return sRecordMode; }
  ...
  if (!(gBattleTypeFlags & BATTLE_TYPE_IS_MASTER)) {
    for (size = *src; size != 0;) {
      ...
      for (i = 0; i < numActions; i++)
        sBattleRecords[battlerId][sBattlerSavedRecordSizes[battlerId]++] =
            GetNextRecordedDataByte(src, &idx, &size);
    }
  }
}
...
bool32 CanCopyRecordedBattleSaveData(void) {
  struct RecordedBattleSave *dst =
      AllocZeroed(sizeof(struct RecordedBattleSave));
...
static bool32 RecordedBattleToSave(struct RecordedBattleSave *battleSave,
                                   struct RecordedBattleSave *saveSector) {
  memset(saveSector, 0, SECTOR_SIZE);
  memcpy(saveSector, battleSave, sizeof(*battleSave));

  saveSector->checksum =
      CalcByteArraySum((void *)(saveSector), sizeof(*saveSector) - 4);

  if (TryWriteSpecialSaveSector(SECTOR_ID_RECORDED_BATTLE,
                                (void *)(saveSector)) != SAVE_STATUS_OK)
    return FALSE;
  else
    return TRUE;
}
...
bool32 MoveRecordedBattleToSaveData(void) {
  ...
  if (sBattleFlags & BATTLE_TYPE_LINK) {
    battleSave->battleFlags =
        (sBattleFlags & ~(BATTLE_TYPE_LINK | BATTLE_TYPE_LINK_IN_BATTLE)) |
  ...
  if (gTrainerBattleOpponent_A >= TRAINER_RECORD_MIXING_FRIEND &&
    ...
    for (i = 0; i < PLAYER_NAME_LENGTH + 1; i++)
      battleSave->recordMixFriendName[i] =
          gSaveBlock2Ptr->frontier
              .towerRecords[gTrainerBattleOpponent_A -
    ...
    for (i = 0; i < PLAYER_NAME_LENGTH + 1; i++)
      battleSave->recordMixFriendName[i] =
          gSaveBlock2Ptr->frontier
              .towerRecords[gTrainerBattleOpponent_B -
                            TRAINER_RECORD_MIXING_FRIEND]
              .name[i];
    battleSave->recordMixFriendClass =
        gSaveBlock2Ptr->frontier
  ...
  if (gTrainerBattleOpponent_A >= TRAINER_RECORD_MIXING_APPRENTICE) {
    battleSave->apprenticeId =
        gSaveBlock2Ptr
            ->apprentices[gTrainerBattleOpponent_A -
    ...
    battleSave->apprenticeId =
        gSaveBlock2Ptr
            ->apprentices[gTrainerBattleOpponent_B -
                          TRAINER_RECORD_MIXING_APPRENTICE]
            .id;
    for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
      battleSave->easyChatSpeech[i] =
    ...
    battleSave->apprenticeId =
        gSaveBlock2Ptr
            ->apprentices[gPartnerTrainerId - TRAINER_RECORD_MIXING_APPRENTICE]
            .id;
...
static bool32 TryCopyRecordedBattleSaveData(struct RecordedBattleSave *dst,
                                            struct SaveSector *saveBuffer) {
  if (TryReadSpecialSaveSector(SECTOR_ID_RECORDED_BATTLE,
                               (void *)(saveBuffer)) != SAVE_STATUS_OK)
    return FALSE;
...
static void SetVariablesForRecordedBattle(struct RecordedBattleSave *src) {
  ...
  for (i = 0; i < MAX_BATTLERS_COUNT; i++) {
    ...
    if (var)
      ConvertInternationalString(gLinkPlayers[i].name,
                                 gLinkPlayers[i].language);
  }
...
void PlayRecordedBattle(void (*CB2_After)(void)) {
  struct RecordedBattleSave *battleSave =
      AllocZeroed(sizeof(struct RecordedBattleSave));
...
u8 GetRecordedBattleFrontierFacility(void) { return sFrontierFacility; }

u8 GetRecordedBattleFronterBrainSymbol(void) { return sFrontierBrainSymbol; }
...
void RecordedBattle_ClearFrontierPassFlag(void) { sFrontierPassFlag = 0; }
...
u8 RecordedBattle_GetFrontierPassFlag(void) { return sFrontierPassFlag; }

u8 GetBattleSceneInRecordedBattle(void) { return sBattleScene; }

u8 GetTextSpeedInRecordedBattle(void) { return sTextSpeed; }
...
void RecordedBattle_CopyBattlerMoves(void) {
  ...
  for (i = 0; i < MAX_MON_MOVES; i++)
    sPlayerMonMoves[gActiveBattler / 2][i] =
        gBattleMons[gActiveBattler].moves[i];
}

// This is a special battle action only used by this function
// It shares a value with B_ACTION_SAFARI_POKEBLOCK, which can never occur in a
// recorded battle.
...
void RecordedBattle_CheckMovesetChanges(u8 mode) {
  ...
  for (battlerId = 0; battlerId < gBattlersCount; battlerId++) {
    ...
    if (GetBattlerSide(battlerId) != B_SIDE_OPPONENT) {
      ...
      {
        if (sBattleRecords[battlerId][sBattlerRecordSizes[battlerId]] ==
            ACTION_MOVE_CHANGE) {
          ...
          for (j = 0; j < MAX_MON_MOVES; j++)
            ppBonuses[j] =
                ((gBattleMons[battlerId].ppBonuses & (3 << (j << 1))) >>
                 (j << 1));
          ...
          for (j = 0; j < MAX_MON_MOVES; j++) {
            ...
            mimickedMoveSlots[j] =
                (gDisableStructs[battlerId].mimickedMoves & gBitTable[j]) >> j;
          }
          for (j = 0; j < MAX_MON_MOVES; j++) {
          ...
          for (j = 0; j < MAX_MON_MOVES; j++) {
            ...
            gDisableStructs[battlerId].mimickedMoves |= mimickedMoveSlots[j]
                                                        << j;
          }
          ...
          if (!(gBattleMons[battlerId].status2 & STATUS2_TRANSFORMED)) {
            ...
            for (j = 0; j < MAX_MON_MOVES; j++) {
              movePp.moves[j] =
                  GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                             MON_DATA_MOVE1 + moveSlots[j], NULL);
              movePp.currentPp[j] =
                  GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                             MON_DATA_PP1 + moveSlots[j], NULL);
              movePp.maxPp[j] = ppBonuses[moveSlots[j]];
            }
            for (j = 0; j < MAX_MON_MOVES; j++) {
              ...
              SetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                         MON_DATA_PP1 + j, &movePp.currentPp[j]);
            }
            ppBonusSet = 0;
            for (j = 0; j < MAX_MON_MOVES; j++)
            ...
            SetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]],
                       MON_DATA_PP_BONUSES, &ppBonusSet);
          }
...
u32 GetAiScriptsInRecordedBattle(void) { return sAI_Scripts; }

// Used to determine when the player is allowed to press B to end a recorded
// battle's playback
void RecordedBattle_SetPlaybackFinished(void) { sIsPlaybackFinished = TRUE; }
...
u8 GetRecordedBattleRecordMixFriendClass(void) { return sRecordMixFriendClass; }

u8 GetRecordedBattleApprenticeId(void) { return sApprenticeId; }

u8 GetRecordedBattleRecordMixFriendLanguage(void) {
  return sRecordMixFriendLanguage;
}

u8 GetRecordedBattleApprenticeLanguage(void) { return sApprenticeLanguage; }

void RecordedBattle_SaveBattleOutcome(void) { sBattleOutcome = gBattleOutcome; }

u16 *GetRecordedBattleEasyChatSpeech(void) { return sEasyChatSpeech; }
