...
static const union AnimCmd sAnim_UnusedCirclingShock[] = {
    ...
    ANIMCMD_FRAME(48, 5), ANIMCMD_FRAME(64, 5), ANIMCMD_FRAME(80, 5),
    ANIMCMD_JUMP(0),
};
...
static const s8 sElectricChargingParticleCoordOffsets[][2] = {
    {58, -60}, {-56, -36}, {8, -56},  {-16, 56},  {58, -10}, {-58, 10},
    {48, -18}, {-8, 56},   {16, -56}, {-58, -42}, {58, 30},  {-48, 40},
    {12, -48}, {48, -12},  {-56, 18}, {48, 48},
};

static const union AnimCmd sAnim_ElectricChargingParticles_0[] = {
    ANIMCMD_FRAME(3, 1), ANIMCMD_FRAME(2, 1), ANIMCMD_FRAME(1, 1),
...
static void AnimSparkElectricity(struct Sprite *sprite) {
  ...
  gOamMatrices[matrixNum].a = gOamMatrices[matrixNum].d =
      gSineTable[gBattleAnimArgs[2] + 64];
...
static void AnimSparkElectricityFlashing(struct Sprite *sprite) {
  ...
  sprite->x =
      GetBattlerSpriteCoord(battler, BATTLER_COORD_X_2) + gBattleAnimArgs[0];
  sprite->y = GetBattlerSpriteCoord(battler, BATTLER_COORD_Y_PIC_OFFSET) +
              gBattleAnimArgs[1];
...
// Electricity arcs around the target. Used for Paralysis and various electric
// move hits
...
static void AnimThunderWave(struct Sprite *sprite) {
  ...
  sprite->x += gBattleAnimArgs[0];
  sprite->y += gBattleAnimArgs[1];
  spriteId = CreateSprite(&gThunderWaveSpriteTemplate, sprite->x + 32,
                          sprite->y, sprite->subpriority);
...
// Animates small electric orbs moving from around the battler inward. For
// Charge/Shock Wave
...
static void AnimTask_ElectricChargingParticles_Step(u8 taskId) {
  ...
  if (task->data[6]) {
    if (++task->data[12] > task->data[13]) {
      u8 spriteId;
      task->data[12] = 0;
      spriteId = CreateSprite(&gElectricChargingParticlesSpriteTemplate,
...
// Creates an orb of electricity that grows then slides off-screen. The attacker
// slides with it
...
void AnimTask_VoltTackleBolt(u8 taskId) {
  ...
  case 0:
    ...
    case 0:
      task->data[3] =
          GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X_2);
      task->data[5] = GetBattlerSpriteCoord(gBattleAnimAttacker,
  ...
  case 1:
    if (++task->data[2] > 0) {
      task->data[2] = 0;
      if (CreateVoltTackleBolt(task, taskId) ||
          CreateVoltTackleBolt(task, taskId))
        task->data[0]++;
    }
    break;
  case 2:
...
static bool8 CreateVoltTackleBolt(struct Task *task, u8 taskId) {
  u8 spriteId = CreateSprite(&gVoltTackleBoltSpriteTemplate, task->data[3],
                             task->data[5], 35);
  if (spriteId != MAX_SPRITES) {
...
// Thin bolt of electricity that moves up and down toward the target. Used by
// Shock Wave
...
static bool8 CreateShockWaveBoltSprite(struct Task *task, u8 taskId) {
  u8 spriteId = CreateSprite(&gShockWaveProgressingBoltSpriteTemplate,
                             task->data[6], task->data[7], 35);
  if (spriteId != MAX_SPRITES) {
...
void AnimTask_ShockWaveLightning(u8 taskId) {
  ...
  case 0:
    task->data[15] =
        GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y) + 32;
    task->data[14] = task->data[15];
...
static bool8 CreateShockWaveLightningSprite(struct Task *task, u8 taskId) {
  u8 spriteId = CreateSprite(&gLightningSpriteTemplate, task->data[13],
                             task->data[14], task->data[12]);
