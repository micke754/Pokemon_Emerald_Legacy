...
static void PrintNumPlayersInLink(u16 windowId, u32 numPlayers) {
  ...
  ConvertIntToDecimalStringN(gStringVar1, numPlayers, STR_CONV_MODE_LEFT_ALIGN,
                             1);
  ...
  AddTextPrinterParameterized(windowId, FONT_NORMAL, gStringVar4, xPos, 1,
                              TEXT_SKIP_DRAW, NULL);
  CopyWindowToVram(windowId, COPYWIN_FULL);
}

static void ClearLinkPlayerCountWindow(u16 windowId) {
  // Following this call with a copy-to-vram with mode COPYWIN_FULL is identical
  // to calling ClearStdWindowAndFrame(windowId, TRUE).
...
static u32 ExchangeDataAndGetLinkupStatus(u8 minPlayers, u8 maxPlayers) {
  ...
  case EXCHANGE_WRONG_NUM_PLAYERS:
    ConvertIntToDecimalStringN(gStringVar1, GetLinkPlayerCount_2(),
...
static void Task_LinkupAwaitConnection(u8 taskId) {
  ...
  if (CheckLinkCanceledBeforeConnection(taskId) == TRUE ||
      CheckLinkCanceled(taskId) == TRUE || playerCount < 2)
    return;
...
static void Task_LinkupConfirmWhenReady(u8 taskId) {
  if (CheckLinkCanceledBeforeConnection(taskId) == TRUE ||
      CheckSioErrored(taskId) == TRUE || CheckLinkErrored(taskId) == TRUE)
    return;
...
static void Task_LinkupAwaitConfirmation(u8 taskId) {
  ...
  ClearLinkPlayerCountWindow(tWindowId);
  ConvertIntToDecimalStringN(gStringVar1, linkPlayerCount,
                             STR_CONV_MODE_LEFT_ALIGN, 1);
...
static void Task_LinkupExchangeDataWithLeader(u8 taskId) {
  ...
  if (gSpecialVar_Result == LINKUP_DIFF_SELECTIONS ||
    ...
    card->monSpecies[0] = GetMonData(
        &gPlayerParty[gSelectedOrderFromParty[0] - 1], MON_DATA_SPECIES, NULL);
    card->monSpecies[1] = GetMonData(
...
static void FinishLinkup(u16 *linkupStatus, u32 taskId) {
  ...
  if (*linkupStatus == LINKUP_SUCCESS) {
    if (gLinkType == LINKTYPE_BATTLE_TOWER_50 ||
        gLinkType == LINKTYPE_BATTLE_TOWER_OPEN) {
      if (AreBattleTowerLinkSpeciesSame(trainerCards[0].monSpecies,
                                        trainerCards[1].monSpecies)) {
...
static void Task_LinkupAwaitTrainerCardData(u8 taskId) {
  ...
  for (index = 0; index < GetLinkPlayerCount(); index++) {
    CopyTrainerCardData(&gTrainerCards[index],
                        (struct TrainerCard *)gBlockRecvBuffer[index],
                        gLinkPlayers[index].version);
  }
...
// Unused
void CableClubSaveGame(void) { SaveGame(); }
...
static void SetLinkBattleTypeFlags(int linkService) {
  ...
  case USING_DOUBLE_BATTLE:
    gBattleTypeFlags =
        BATTLE_TYPE_DOUBLE | BATTLE_TYPE_LINK | BATTLE_TYPE_TRAINER;
    break;
  case USING_MULTI_BATTLE:
    ...
    break;
  case USING_BATTLE_TOWER:
    gBattleTypeFlags = BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_DOUBLE |
                       BATTLE_TYPE_LINK | BATTLE_TYPE_TRAINER |
                       BATTLE_TYPE_MULTI;
    break;
  }
}
    ...
    if (gWirelessCommType) {
      ...
      case B_OUTCOME_WON:
        MysteryGift_TryIncrementStat(
            ...
            gLinkPlayers[GetMultiplayerId() ^ 1].trainerId);
        break;
      case B_OUTCOME_LOST:
        MysteryGift_TryIncrementStat(
...
void ExitLinkRoom(void) { QueueExitLinkRoomKey(); }
...
void Script_ShowLinkTrainerCard(void) {
  ShowTrainerCardInLink(gSpecialVar_0x8006,
                        CB2_ReturnToFieldContinueScriptPlayMapMusic);
}

// Returns FALSE if the player has no stars. Returns TRUE otherwise, and puts
// the name of the color into gStringVar2.
