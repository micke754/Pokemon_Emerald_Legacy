...
static const union AnimCmd sAnim_LargeFlame[] = {
    ...
    ANIMCMD_FRAME(48, 3), ANIMCMD_FRAME(64, 3),  ANIMCMD_FRAME(80, 3),
    ANIMCMD_FRAME(96, 3), ANIMCMD_FRAME(112, 3), ANIMCMD_JUMP(0),
};
...
static const union AnimCmd sAnim_WillOWispOrb_0[] = {
    ...
    ANIMCMD_FRAME(12, 5), ANIMCMD_JUMP(0),
};
...
static const union AnimCmd sAnim_WillOWispFire[] = {
    ANIMCMD_FRAME(0, 5),  ANIMCMD_FRAME(16, 5), ANIMCMD_FRAME(32, 5),
    ANIMCMD_FRAME(48, 5), ANIMCMD_JUMP(0),
};
...
// arg 4: duration
// arg 5: ? (todo: something related to which mon the pixel offsets are based
// on) arg 6: ? (todo: something related to which mon the pixel offsets are
// based on)
...
#define IDX_ACTIVE_SPRITES                                                     \
  6 // Used by the sprite callback to modify the number of active sprites
...
// Animates first stage of Eruption where the attacker squishes and launches
// rocks away from themself
...
static void AnimTask_EruptionLaunchRocks_Step(u8 taskId) {
  ...
  switch (task->tState) {
  case 0:
    PrepareEruptAnimTaskData(task, task->tAttackerSpriteId, 0x100, 0x100, 0xE0,
                             0x200, 32);
    task->tState++;
  case 1:
    if (++task->tTimer1 > 1) {
  ...
  case 2:
    if (++task->tTimer1 > 4) {
      if (task->tAttackerSide != B_SIDE_PLAYER)
        PrepareEruptAnimTaskData(task, task->tAttackerSpriteId, 0xE0, 0x200,
                                 0x180, 0xF0, 6);
      else
        PrepareEruptAnimTaskData(task, task->tAttackerSpriteId, 0xE0, 0x200,
                                 0x180, 0xC0, 6);
  ...
  case 3:
    if (!UpdateEruptAnimTask(task)) {
      CreateEruptionLaunchRocks(task->tAttackerSpriteId, taskId,
                                IDX_ACTIVE_SPRITES);
      task->tState++;
    }
    break;
  case 4:
    ...
    if (++task->tTimer3 > 24) {
      if (task->tAttackerSide != B_SIDE_PLAYER)
        PrepareEruptAnimTaskData(task, task->tAttackerSpriteId, 0x180, 0xF0,
                                 0x100, 0x100, 8);
      else
        PrepareEruptAnimTaskData(task, task->tAttackerSpriteId, 0x180, 0xC0,
                                 0x100, 0x100, 8);
...
static void CreateEruptionLaunchRocks(u8 spriteId, u8 taskId,
                                      u8 activeSpritesIdx) {
  u16 i, j;
  s8 sign;
...
static u16 GetEruptionLaunchRockInitialYPos(u8 spriteId) {
  s16 y = gSprites[spriteId].y + gSprites[spriteId].y2 +
...
static void InitEruptionLaunchRockCoordData(struct Sprite *sprite, s16 speedX,
                                            s16 speedY) {
...
static void UpdateEruptionLaunchRockPos(struct Sprite *sprite) {
  ...
  if (sprite->x < -8 || sprite->x > DISPLAY_WIDTH + 8 || sprite->y < -8 ||
      sprite->y > 120)
    sprite->invisible = TRUE;
}
...
static void AnimWillOWispOrb_Step(struct Sprite *sprite) {
  ...
  if (!AnimTranslateLinear(sprite)) {
    ...
    if ((initialData5 == 0 || initialData5 > 196) && newData5 > 0 &&
        sprite->data[7] == 0) {
...
void AnimTask_MoveHeatWaveTargets(u8 taskId) {
  ...
  task->data[13] =
      IsBattlerSpriteVisible(BATTLE_PARTNER(gBattleAnimTarget)) + 1;
...
static void AnimTask_MoveHeatWaveTargets_Step(u8 taskId) {
  ...
  case 0:
    ...
    for (task->data[3] = 0; task->data[3] < task->data[13]; task->data[3]++) {
      gSprites[task->data[task->data[3] + 14]].x2 =
...
void AnimTask_BlendBackground(u8 taskId) {
  ...
  BlendPalette(BG_PLTT_ID(animBg.paletteId), 16, gBattleAnimArgs[0],
               gBattleAnimArgs[1]);
  DestroyAnimVisualTask(taskId);
}
...
#define tShakeOffset                                                           \
  data[2] // Never read, gBattleAnimArgs[1] is used directly instead
#define tVertical data[3]
#define tPatternId data[4]

// Shakes target horizontally or vertically tMaxShakes times, following a set
...
void AnimTask_ShakeTargetInPattern(u8 taskId) {
  ...
  if (gTasks[taskId].tVertical == TRUE)
    gSprites[spriteId].y2 = gBattleAnimArgs[1] * dir < 0
                                ? -(gBattleAnimArgs[1] * dir)
                                : gBattleAnimArgs[1] * dir;
  else
