...
static bool8 ScriptMovement_TryAddNewMovement(u8 taskId, u8 objEventId,
...
static void ScriptMovement_AddNewMovement(u8 taskId, u8 moveScrId,
                                          u8 objEventId,
                                          const u8 *movementScript);
static void ScriptMovement_UnfreezeActiveObjects(u8 taskId);
static void ScriptMovement_MoveObjects(u8 taskId);
static void ScriptMovement_TakeStep(u8 taskId, u8 moveScrId, u8 objEventId,
                                    const u8 *movementScript);

static EWRAM_DATA const u8 *sMovementScripts[OBJECT_EVENTS_COUNT] = {0};

bool8 ScriptMovement_StartObjectMovementScript(u8 localId, u8 mapNum,
                                               u8 mapGroup,
                                               const u8 *movementScript) {
  u8 objEventId;
  ...
  return ScriptMovement_TryAddNewMovement(GetMoveObjectsTaskId(), objEventId,
                                          movementScript);
}

bool8 ScriptMovement_IsObjectMovementFinished(u8 localId, u8 mapNum,
                                              u8 mapGroup) {
  u8 objEventId;
  u8 taskId;
  u8 moveScrId;
...
static bool8 ScriptMovement_TryAddNewMovement(u8 taskId, u8 objEventId,
  ...
  if (moveScrId != OBJECT_EVENTS_COUNT) {
    if (IsMovementScriptFinished(taskId, moveScrId) == 0) {
      ...
      ScriptMovement_AddNewMovement(taskId, moveScrId, objEventId,
                                    movementScript);
      return FALSE;
    }
  }
...
static void LoadObjectEventIdPtrFromMovementScript(u8 taskId, u8 moveScrId,
                                                   u8 **pObjEventId) {
  u8 i;
...
static void SetObjectEventIdAtMovementScript(u8 taskId, u8 moveScrId,
                                             u8 objEventId) {
  u8 *ptr;
...
static void LoadObjectEventIdFromMovementScript(u8 taskId, u8 moveScrId,
                                                u8 *objEventId) {
  u8 *ptr;
...
static void ScriptMovement_TakeStep(u8 taskId, u8 moveScrId, u8 objEventId,
  ...
  if (nextMoveActionId == MOVEMENT_ACTION_STEP_END) {
    ...
    if (!ObjectEventSetHeldMovement(&gObjectEvents[objEventId],
                                    nextMoveActionId)) {
      movementScript++;
      SetMovementScript(moveScrId, movementScript);
    }
