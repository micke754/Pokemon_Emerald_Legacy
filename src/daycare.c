...
static void SetInitialEggData(struct Pokemon *mon, u16 species,
                              struct DayCare *daycare);
static u8 GetDaycareCompatibilityScore(struct DayCare *daycare);
static void DaycarePrintMonInfo(u8 windowId, u32 daycareSlotId, u8 y);

// RAM buffers used to assist with BuildEggMoveset()
EWRAM_DATA static u16 sHatchedEggLevelUpMoves[EGG_LVL_UP_MOVES_ARRAY_COUNT] = {
    0};
...
void InitDaycareMailRecordMixing(struct DayCare *daycare,
...
static void StorePokemonInDaycare(struct Pokemon *mon,
...
static void StorePokemonInEmptyDaycareSlot(struct Pokemon *mon,
                                           struct DayCare *daycare) {
...
void StoreSelectedPokemonInDaycare(void) {
  ...
  StorePokemonInEmptyDaycareSlot(&gPlayerParty[monId],
                                 &gSaveBlock1Ptr->daycare);
}

// Shifts the second daycare Pokémon slot into the first slot.
static void ShiftDaycareSlots(struct DayCare *daycare) {
  // This condition is only satisfied when the player takes out the first
  // Pokémon from the daycare.
...
static void ApplyDaycareExperience(struct Pokemon *mon) {
  ...
  for (i = 0; i < MAX_LEVEL; i++) {
    // Add the mon's gained daycare experience level by level until it can't
    // level up anymore.
...
static u16 TakeSelectedPokemonFromDaycare(struct DaycareMon *daycareMon) {
  ...
  if (GetMonData(&pokemon, MON_DATA_LEVEL) != MAX_LEVEL &&
    ...
    if (experience <=
        gExperienceTables[gSpeciesInfo[species].growthRate][levelCap]) {
...
static u16 TakeSelectedPokemonMonFromDaycareShiftSlots(struct DayCare *daycare,
                                                       u8 slotId) {
...
u16 TakePokemonFromDaycare(void) {
  return TakeSelectedPokemonMonFromDaycareShiftSlots(&gSaveBlock1Ptr->daycare,
                                                     gSpecialVar_0x8004);
}
...
static u8 GetNumLevelsGainedForDaycareMon(struct DaycareMon *daycareMon) {
  ...
  ConvertIntToDecimalStringN(gStringVar2, numLevelsGained,
...
void GetDaycareCost(void) {
  gSpecialVar_0x8005 =
      GetDaycareCostForMon(&gSaveBlock1Ptr->daycare, gSpecialVar_0x8004);
}
...
u8 GetNumLevelsGainedFromDaycare(void) {
  if (GetBoxMonData(&gSaveBlock1Ptr->daycare.mons[gSpecialVar_0x8004].mon,
                    MON_DATA_SPECIES) != 0)
    return GetNumLevelsGainedForDaycareMon(
        &gSaveBlock1Ptr->daycare.mons[gSpecialVar_0x8004]);
...
static s32 GetParentToInheritNature(struct DayCare *daycare) {
  ...
  for (i = 0; i < DAYCARE_MON_COUNT; i++) {
    if (GetBoxMonData(&daycare->mons[i].mon, MON_DATA_HELD_ITEM) ==
        ITEM_EVERSTONE)
...
static void InheritIVs(struct Pokemon *egg, struct DayCare *daycare) {
  ...
  for (i = 0; i < INHERITED_IV_COUNT; i++) {
    // Randomly pick an IV from the available list and stop from being chosen
    // again.
  ...
  for (i = 0; i < INHERITED_IV_COUNT; i++) {
    ...
    case 3:
      iv =
          GetBoxMonData(&daycare->mons[whichParents[i]].mon, MON_DATA_SPEED_IV);
      SetMonData(egg, MON_DATA_SPEED_IV, &iv);
      break;
    case 4:
      iv =
          GetBoxMonData(&daycare->mons[whichParents[i]].mon, MON_DATA_SPATK_IV);
      SetMonData(egg, MON_DATA_SPATK_IV, &iv);
      break;
    case 5:
      iv =
          GetBoxMonData(&daycare->mons[whichParents[i]].mon, MON_DATA_SPDEF_IV);
...
u8 GetEggMovesSpecies(u16 species, u16 *eggMoves) {
  ...
  for (i = 0; i < EGG_MOVES_ARRAY_COUNT; i++) {
    if (gEggMoves[eggMoveIdx + i] > EGG_MOVES_SPECIES_OFFSET) {
      // TODO: the curly braces around this if statement are required for a
      // matching build.
      break;
    }
...
bool8 SpeciesCanLearnEggMove(u16 species,
...
static void BuildEggMoveset(struct Pokemon *egg, struct BoxPokemon *father,
                            struct BoxPokemon *mother) {
  ...
  numLevelUpMoves = GetLevelUpMovesBySpecies(GetMonData(egg, MON_DATA_SPECIES),
    ...
    if (sHatchedEggFatherMoves[i] != MOVE_NONE) {
      for (j = 0; j < numEggMoves; j++) {
        if (sHatchedEggFatherMoves[i] == sHatchedEggEggMoves[j]) {
          if (GiveMoveToMon(egg, sHatchedEggFatherMoves[i]) ==
              MON_HAS_MAX_MOVES)
  ...
  for (i = 0; i < MAX_MON_MOVES; i++) {
    ...
    for (j = 0; j < MAX_MON_MOVES; j++) {
      if (sHatchedEggFatherMoves[i] == sHatchedEggMotherMoves[j] &&
          sHatchedEggFatherMoves[i] != MOVE_NONE)
        sHatchedEggFinalMoves[numSharedParentMoves++] =
...
static void AlterEggSpeciesWithIncenseItem(u16 *species,
                                           struct DayCare *daycare) {
...
static void GiveVoltTackleIfLightBall(struct Pokemon *mon,
                                      struct DayCare *daycare) {
...
static u16 DetermineEggSpeciesAndParentSlots(struct DayCare *daycare,
                                             u8 *parentSlots) {
  u16 i;
...
static void _GiveEggFromDaycare(struct DayCare *daycare) {
  ...
  BuildEggMoveset(&egg, &daycare->mons[parentSlots[1]].mon,
                  &daycare->mons[parentSlots[0]].mon);
...
void CreateEgg(struct Pokemon *mon, u16 species, bool8 setHotSpringsLocation) {
  ...
  CreateMon(mon, species, EGG_HATCH_LEVEL, USE_RANDOM_IVS, FALSE, 0,
            OT_ID_PLAYER_ID, 0);
  metLevel = 0;
  ball = ITEM_POKE_BALL;
...
static void SetInitialEggData(struct Pokemon *mon, u16 species,
  ...
  CreateMon(mon, species, EGG_HATCH_LEVEL, USE_RANDOM_IVS, TRUE, personality,
            OT_ID_PLAYER_ID, 0);
  metLevel = 0;
  ball = ITEM_POKE_BALL;
  language = LANGUAGE_JAPANESE;
...
u16 GetSelectedMonNicknameAndSpecies(void) {
  ...
  return GetBoxMonData(&gPlayerParty[GetCursorSelectionMonId()].box,
                       MON_DATA_SPECIES);
}
...
static u8 GetDaycareCompatibilityScore(struct DayCare *daycare) {
  ...
  // one parent is Ditto
  if (eggGroups[0][0] == EGG_GROUP_DITTO ||
      eggGroups[1][0] == EGG_GROUP_DITTO) {
  ...
  // neither parent is Ditto
    ...
    if (species[0] == species[1]) {
      ...
      if (trainerIds[0] != trainerIds[1])
        return PARENTS_MED_COMPATIBILITY; // different species, different
                                          // trainers
...
bool8 NameHasGenderSymbol(const u8 *name, u8 genderRatio) {
  ...
  if (genderRatio == MON_MALE && symbolsCount[MALE] != 0 &&
      symbolsCount[FEMALE] == 0)
    return TRUE;
  if (genderRatio == MON_FEMALE && symbolsCount[FEMALE] != 0 &&
      symbolsCount[MALE] == 0)
    return TRUE;
...
static void UNUSED GetDaycareLevelMenuLevelText(struct DayCare *daycare,
                                                u8 *dest) {
  u8 i;
  u8 level;
  u8 text[20];
  ...
  *dest = EOS;
  for (i = 0; i < DAYCARE_MON_COUNT; i++) {
    ...
    level = GetLevelAfterDaycareSteps(&daycare->mons[i].mon,
...
static void DaycarePrintMonNickname(struct DayCare *daycare, u8 windowId,
                                    u32 daycareSlotId, u32 y) {
  u8 nickname[POKEMON_NAME_BUFFER_SIZE];
...
static void DaycarePrintMonLvl(struct DayCare *daycare, u8 windowId,
                               u32 daycareSlotId, u32 y) {
  u8 level;
  u32 x;
  u8 lvlText[12];
  ...
  level = GetLevelAfterDaycareSteps(&daycare->mons[daycareSlotId].mon,
...
static void DaycarePrintMonInfo(u8 windowId, u32 daycareSlotId, u8 y) {
  if (daycareSlotId < (unsigned)DAYCARE_MON_COUNT) {
    DaycarePrintMonNickname(&gSaveBlock1Ptr->daycare, windowId, daycareSlotId,
                            y);
