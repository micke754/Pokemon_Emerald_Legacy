...
bool32 PokenavCallback_Init_MatchCall(void) {
  struct Pokenav_MatchCallMenu *state = AllocSubstruct(
      POKENAV_SUBSTRUCT_MATCH_CALL_MAIN, sizeof(struct Pokenav_MatchCallMenu));
  if (!state)
    return FALSE;
...
u32 GetMatchCallCallback(void) {
  struct Pokenav_MatchCallMenu *state =
      GetSubstructPtr(POKENAV_SUBSTRUCT_MATCH_CALL_MAIN);
...
static u32
CB2_HandleMatchCallOptionsInput(struct Pokenav_MatchCallMenu *state) {
  ...
  case 2:
    ...
    for (i = 0, j = state->headerId; i < 30; i++, j++) {
      if (!MatchCall_HasRematchId(state->headerId) &&
        ...
        state->matchCallEntries[state->numRegistered].headerId =
            state->headerId;
...
bool32 ShouldDrawRematchPokeballIcon(int index) {
  ...
  if (!state->matchCallEntries[index].isSpecialTrainer)
    ...
    index =
        MatchCall_GetRematchTableIdx(state->matchCallEntries[index].headerId);
  ...
  *newRematchRequest = FALSE;
  ...
  if (!state->matchCallEntries[index].isSpecialTrainer)
    *newRematchRequest = SelectMatchCallMessage(
        GetTrainerIdxByRematchIdx(state->matchCallEntries[index].headerId),
        gStringVar4);
  else
...
void BufferMatchCallNameAndDesc(struct PokenavMatchCallEntry *matchCallEntry,
  ...
  if (!matchCallEntry->isSpecialTrainer) {
    ...
    MatchCall_GetNameAndDesc(matchCallEntry->headerId, &className,
                             &trainerName);
  }
...
static bool32 ShouldDoNearbyMessage(void) {
  ...
  if (!state->matchCallEntries[selection].isSpecialTrainer) {
    if (GetMatchCallMapSec(selection) == gMapHeader.regionMapSectionId) {
      if (!gSaveBlock1Ptr
               ->trainerRematches[state->matchCallEntries[selection].headerId])
        return TRUE;
    }
  } else {
