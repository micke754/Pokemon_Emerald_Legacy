...
// Select_ refers to the first Pokémon selection screen where you choose your
// initial 3 rental Pokémon. Swap_   refers to the subsequent selection screens
// where you can swap a Pokémon with one from the beaten trainer

// Note that, generally, "Action" will refer to the immediate actions that can
// be taken on each screen, i.e. selecting a Pokémon or selecting the Cancel
// button The "Options menu" will refer to the popup menu that shows when some
// actions have been selected

#define SWAP_PLAYER_SCREEN                                                     \
  0 // The screen where the player selects which of their Pokémon to swap away
#define SWAP_ENEMY_SCREEN                                                      \
  1 // The screen where the player selects which new Pokémon from the defeated
    // party to swap for
...
enum { FADESTATE_INIT, FADESTATE_RUN, FADESTATE_DELAY };
...
struct FactorySelectableMon {
  u16 monId;
  u16 ballSpriteId;
  u8 selectedId; // 0 - not selected, 1 - first Pokémon, 2 - second Pokémon, 3 -
                 // third Pokémon
...
struct FactorySelectScreen {
  ...
  struct FactoryMonPic monPics[FRONTIER_PARTY_SIZE]; // Array so all chosen mons
...
struct FactorySwapScreen {
  ...
  u8 pkmnForSwapButtonSpriteIds[2][3]; // For this and sprite ID array below,
                                       // [0][i] is the button background,
                                       // [1][i] is the button highlight
...
static const u16 sPokeballGray_Pal[] =
    INCBIN_U16("graphics/battle_frontier/factory_screen/pokeball_gray.gbapal");
static const u16 sPokeballSelected_Pal[] = INCBIN_U16(
    "graphics/battle_frontier/factory_screen/pokeball_selected.gbapal");
static const u16 sInterface_Pal[] = INCBIN_U16(
    "graphics/battle_frontier/factory_screen/interface.gbapal"); // Arrow,
                                                                 // menu/action
                                                                 // highlights,
                                                                 // action box,
                                                                 // etc
static const u8 sPokeball_Gfx[] = INCBIN_U8(
    "graphics/battle_frontier/factory_screen/pokeball.4bpp"); // Unused,
                                                              // gPokeballSelection_Gfx
                                                              // used instead
static const u8 sArrow_Gfx[] =
    INCBIN_U8("graphics/battle_frontier/factory_screen/arrow.4bpp");
static const u8 sMenuHighlightLeft_Gfx[] = INCBIN_U8(
    "graphics/battle_frontier/factory_screen/menu_highlight_left.4bpp");
static const u8 sMenuHighlightRight_Gfx[] = INCBIN_U8(
    "graphics/battle_frontier/factory_screen/menu_highlight_right.4bpp");
static const u8 sActionBoxLeft_Gfx[] =
    INCBIN_U8("graphics/battle_frontier/factory_screen/action_box_left.4bpp");
static const u8 sActionBoxRight_Gfx[] =
    INCBIN_U8("graphics/battle_frontier/factory_screen/action_box_right.4bpp");
static const u8 sActionHighlightLeft_Gfx[] = INCBIN_U8(
    "graphics/battle_frontier/factory_screen/action_highlight_left.4bpp");
static const u8 sActionHighlightMiddle_Gfx[] = INCBIN_U8(
    "graphics/battle_frontier/factory_screen/action_highlight_middle.4bpp");
static const u8 sActionHighlightRight_Gfx[] = INCBIN_U8(
    "graphics/battle_frontier/factory_screen/action_highlight_right.4bpp");
static const u8 sMonPicBgAnim_Gfx[] =
    INCBIN_U8("graphics/battle_frontier/factory_screen/mon_pic_bg_anim.4bpp");
static const u8 sMonPicBg_Tilemap[] =
    INCBIN_U8("graphics/battle_frontier/factory_screen/mon_pic_bg.bin");
static const u16 sMonPicBg_Gfx[] =
    INCBIN_U16("graphics/battle_frontier/factory_screen/mon_pic_bg.4bpp");
static const u16 sMonPicBg_Pal[] =
    INCBIN_U16("graphics/battle_frontier/factory_screen/mon_pic_bg.gbapal");

static const struct SpriteSheet sSelect_SpriteSheets[] = {
    {sArrow_Gfx, sizeof(sArrow_Gfx), GFXTAG_ARROW},
    {sMenuHighlightLeft_Gfx, sizeof(sMenuHighlightLeft_Gfx),
     GFXTAG_MENU_HIGHLIGHT_LEFT},
    {sMenuHighlightRight_Gfx, sizeof(sMenuHighlightRight_Gfx),
     GFXTAG_MENU_HIGHLIGHT_RIGHT},
...
u8 static (*const sSelect_MenuOptionFuncs[])(void) = {
    Select_OptionSummary, Select_OptionRentDeselect, Select_OptionOthers};
...
static const u16 sSelectText_Pal[] =
    INCBIN_U16("graphics/battle_frontier/factory_screen/text.gbapal");
...
static const union AnimCmd sAnim_Select_Pokeball_Moving[] = {
    ANIMCMD_FRAME(16, 4), ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(32, 4),
    ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(16, 4), ANIMCMD_FRAME(0, 4),
    ANIMCMD_FRAME(32, 4), ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(0, 32),
    ANIMCMD_FRAME(16, 8), ANIMCMD_FRAME(0, 8),  ANIMCMD_FRAME(32, 8),
    ANIMCMD_FRAME(0, 8),  ANIMCMD_FRAME(16, 8), ANIMCMD_FRAME(0, 8),
...
static const union AffineAnimCmd sAffineAnim_Select_MonPicBg_Opening[] = {
    AFFINEANIMCMD_FRAME(5, 5, 0, 0),   AFFINEANIMCMD_FRAME(0, 0, 0, 1),
    ...
    AFFINEANIMCMD_FRAME(256, 5, 0, 0), AFFINEANIMCMD_END,
};
...
static const struct SpriteSheet sSwap_SpriteSheets[] = {
    ...
    {sActionHighlightLeft_Gfx, sizeof(sActionHighlightLeft_Gfx),
     GFXTAG_ACTION_HIGHLIGHT_LEFT},
    {sActionHighlightMiddle_Gfx, sizeof(sActionHighlightMiddle_Gfx),
     GFXTAG_ACTION_HIGHLIGHT_MIDDLE},
...
enum {
  ...
  SWAP_WIN_OPTIONS,
  SWAP_WIN_YES_NO,
  SWAP_WIN_ACTION_FADE, // Used for action text fading out during screen
                        // transition
  SWAP_WIN_UNUSED,
  SWAP_WIN_SPECIES_AT_FADE, // Used to print species name stopped at current
                            // fade level
...
static const u16 sSwapText_Pal[] = INCBIN_U16(
    "graphics/battle_frontier/factory_screen/text.gbapal"); // Identical to
                                                            // sSelectText_Pal
...
// Main select states
// States for the main tasks of the Select_ functions after initialization,
// including: Select_Task_OpenSummaryScreen, Select_Task_HandleYesNo,
// Select_Task_HandleMenu, and Select_Task_HandleChooseMons Unnecessarily
// complicated, could easily have just kept states numbered in each task always
// starting at 0 There's only one instance (Select_Task_HandleChooseMons) where
// a non-initial case is used Select_Task_Exit has its own straightforward
// states
...
static void CB2_InitSelectScreen(void) {
  ...
  case 0:
    ...
    InitBgsFromTemplates(0, sSelect_BgTemplates,
                         ARRAY_COUNT(sSelect_BgTemplates));
  ...
  case 2:
    ...
    CpuCopy16(gFrontierFactoryMenu_Gfx, sSelectMenuTilesetBuffer,
              sizeof(gFrontierFactoryMenu_Gfx));
    CpuCopy16(sMonPicBg_Gfx, sSelectMonPicBgTilesetBuffer,
              sizeof(sMonPicBg_Gfx));
    LoadBgTiles(1, sSelectMenuTilesetBuffer, sizeof(gFrontierFactoryMenu_Gfx),
                0);
    LoadBgTiles(3, sSelectMonPicBgTilesetBuffer, sizeof(sMonPicBg_Gfx), 0);
    CpuCopy16(gFrontierFactoryMenu_Tilemap, sSelectMenuTilemapBuffer,
              BG_SCREEN_SIZE);
...
#endif
      gPlttBufferUnfaded[BG_PLTT_ID(PALNUM_FADE_TEXT) + 4] =
          sFactorySelectScreen->speciesNameColorBackup;
  ...
  case 4:
    ...
    SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJ_ON | DISPCNT_BG0_ON |
                                      DISPCNT_BG1_ON | DISPCNT_OBJ_1D_MAP);
#ifdef UBFIX
    if (sFactorySelectScreen && sFactorySelectScreen->fromSummaryScreen)
#else
    if (sFactorySelectScreen->fromSummaryScreen == TRUE)
#endif
    {
      ...
      ShowBg(3);
      SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_TGT1_BG3 | BLDCNT_EFFECT_BLEND |
                                       BLDCNT_TGT2_BG1 | BLDCNT_TGT2_OBJ);
  ...
  case 10:
    sFactorySelectScreen->fadeSpeciesNameTaskId =
        CreateTask(Select_Task_FadeSpeciesName, 0);
    if (!sFactorySelectScreen->fromSummaryScreen) {
      gTasks[sFactorySelectScreen->fadeSpeciesNameTaskId].tState =
          FADESTATE_INIT;
...
static void Select_InitAllSprites(void) {
  ...
  for (i = 0; i < SELECTABLE_MONS_COUNT; i++) {
    sFactorySelectScreen->mons[i].ballSpriteId =
        CreateSprite(&sSpriteTemplate_Select_Pokeball, (35 * i) + 32, 64, 1);
  ...
  sFactorySelectScreen->cursorSpriteId =
      CreateSprite(&sSpriteTemplate_Select_Arrow, x, 88, 0);
  sFactorySelectScreen->menuCursor1SpriteId =
      CreateSprite(&sSpriteTemplate_Select_MenuHighlightLeft, 176, 112, 0);
  sFactorySelectScreen->menuCursor2SpriteId =
      CreateSprite(&sSpriteTemplate_Select_MenuHighlightRight, 176, 144, 0);
...
static void Select_UpdateMenuCursorPosition(s8 direction) {
  ...
  {
    if (sFactorySelectScreen->menuCursorPos !=
        ARRAY_COUNT(sSelect_MenuOptionFuncs) - 1)
...
static void Select_HandleMonSelectionChange(void) {
  ...
  gSprites[sFactorySelectScreen->mons[cursorPos].ballSpriteId].oam.paletteNum =
      paletteNum;
}
...
static void Select_Task_OpenSummaryScreen(u8 taskId) {
  ...
  case STATE_SUMMARY_FADE:
    gPlttBufferUnfaded[BG_PLTT_ID(PALNUM_FADE_TEXT) + 4] =
        gPlttBufferFaded[BG_PLTT_ID(PALNUM_FADE_TEXT) + 4];
  ...
  case STATE_SUMMARY_CLEAN:
    if (!gPaletteFade.active) {
      ...
      HideMonPic(sFactorySelectScreen->monPics[1],
                 &sFactorySelectScreen->monPicAnimating);
  ...
  case STATE_SUMMARY_SHOW:
    ...
    sFactorySelectMons =
        AllocZeroed(sizeof(struct Pokemon) * SELECTABLE_MONS_COUNT);
    for (i = 0; i < SELECTABLE_MONS_COUNT; i++)
      sFactorySelectMons[i] = sFactorySelectScreen->mons[i].monData;
    ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, sFactorySelectMons,
                             currMonId, SELECTABLE_MONS_COUNT - 1,
...
static void Select_Task_HandleMenu(u8 taskId) {
  ...
  case STATE_MENU_INIT:
    if (!sFactorySelectScreen->fromSummaryScreen)
      OpenMonPic(&sFactorySelectScreen->monPics[1].bgSpriteId,
  ...
  case STATE_MENU_HANDLE_INPUT:
    if (JOY_NEW(A_BUTTON)) {
      ...
      PlaySE(SE_SELECT);
      CloseMonPic(sFactorySelectScreen->monPics[1],
                  &sFactorySelectScreen->monPicAnimating, FALSE);
    ...
    if (!gPaletteFade.active) {
      if (sFactorySelectScreen->fromSummaryScreen == TRUE) {
        gPlttBufferFaded[BG_PLTT_ID(PALNUM_FADE_TEXT) + 4] =
            sFactorySelectScreen->speciesNameColorBackup;
        gPlttBufferUnfaded[BG_PLTT_ID(PALNUM_FADE_TEXT) + 4] =
            gPlttBufferUnfaded[BG_PLTT_ID(PALNUM_TEXT) + 4];
      }
      sFactorySelectScreen->fromSummaryScreen = FALSE;
      gTasks[taskId].tState = STATE_MENU_HANDLE_INPUT;
    }
...
static void CreateFrontierFactorySelectableMons(u8 firstMonId) {
  ...
  for (i = 0; i < SELECTABLE_MONS_COUNT; i++) {
    ...
    CreateMonWithEVSpreadNatureOTID(
        ...
        gFacilityTrainerMons[monId].species, level,
        gFacilityTrainerMons[monId].nature, ivs,
    ...
    SetMonData(&sFactorySelectScreen->mons[i + firstMonId].monData,
               MON_DATA_FRIENDSHIP, &friendship);
    SetMonData(
        &sFactorySelectScreen->mons[i + firstMonId].monData, MON_DATA_HELD_ITEM,
...
static void Select_PrintRentalPkmnString(void) {
  ...
  AddTextPrinterParameterized(SELECT_WIN_TITLE, FONT_NORMAL, gText_RentalPkmn2,
                              2, 1, 0, NULL);
...
static void Select_PrintMonSpecies(void) {
  ...
  species = GetMonData(&sFactorySelectScreen->mons[monId].monData,
                       MON_DATA_SPECIES, NULL);
  StringCopy(gStringVar4, gSpeciesNames[species]);
  x = GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 86);
  AddTextPrinterParameterized3(SELECT_WIN_SPECIES, FONT_NORMAL, x, 1,
                               sSpeciesNameTextColors, 0, gStringVar4);
...
static void Select_PrintCantSelectSameMon(void) {
  FillWindowPixelBuffer(SELECT_WIN_INFO, PIXEL_FILL(0));
  AddTextPrinterParameterized(SELECT_WIN_INFO, FONT_NORMAL,
                              gText_CantSelectSamePkmn, 2, 5, 0, NULL);
  CopyWindowToVram(SELECT_WIN_INFO, COPYWIN_GFX);
}
...
static void Select_PrintMenuOptions(void) {
  ...
  AddTextPrinterParameterized3(SELECT_WIN_OPTIONS, FONT_NORMAL, 7, 1,
                               sMenuOptionTextColors, 0, gText_Summary);
  if (selectedId != 0)
    AddTextPrinterParameterized3(SELECT_WIN_OPTIONS, FONT_NORMAL, 7, 17,
                                 sMenuOptionTextColors, 0, gText_Deselect);
  else
    AddTextPrinterParameterized3(SELECT_WIN_OPTIONS, FONT_NORMAL, 7, 17,
                                 sMenuOptionTextColors, 0, gText_Rent);

  AddTextPrinterParameterized3(SELECT_WIN_OPTIONS, FONT_NORMAL, 7, 33,
                               sMenuOptionTextColors, 0, gText_Others2);
...
static void Select_PrintYesNoOptions(void) {
  ...
  AddTextPrinterParameterized3(SELECT_WIN_YES_NO, FONT_NORMAL, 7, 1,
                               sMenuOptionTextColors, 0, gText_Yes2);
  AddTextPrinterParameterized3(SELECT_WIN_YES_NO, FONT_NORMAL, 7, 17,
                               sMenuOptionTextColors, 0, gText_No2);
...
static u8 Select_OptionSummary(void) { return SELECT_SUMMARY; }
...
static void Select_PrintMonCategory(void) {
  ...
  if (monId < SELECTABLE_MONS_COUNT) {
    ...
    AddTextPrinterParameterized(SELECT_WIN_MON_CATEGORY, FONT_NORMAL, text, x,
                                1, 0, NULL);
...
static void Select_CreateMonSprite(void) {
  ...
  sFactorySelectScreen->monPics[1].monSpriteId =
      CreateMonPicSprite_HandleDeoxys(species, otId, personality, TRUE, 88, 32,
                                      15, TAG_NONE);
...
static void Select_ReshowMonSprite(void) {
  ...
  sFactorySelectScreen->monPics[1].bgSpriteId =
      CreateSprite(&sSpriteTemplate_Select_MonPicBgAnim, 120, 64, 1);
  StartSpriteAffineAnim(&gSprites[sFactorySelectScreen->monPics[1].bgSpriteId],
                        2);
  ...
  for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
    for (j = 0; j < SELECTABLE_MONS_COUNT; j++) {
      if (sFactorySelectScreen->mons[j].selectedId == i + 1) {
        ...
        sFactorySelectScreen->monPics[i].monSpriteId =
            CreateMonPicSprite_HandleDeoxys(species, otId, personality, TRUE,
                                            (i * 72) + 16, 32, i + 13,
                                            TAG_NONE);
        gSprites[sFactorySelectScreen->monPics[i].monSpriteId]
...
static void SpriteCB_CloseChosenMonPics(struct Sprite *sprite) {
  ...
  if (sprite->affineAnimEnded &&
    ...
    FreeOamMatrix(
        gSprites[sFactorySelectScreen->monPics[0].bgSpriteId].oam.matrixNum);
    FreeOamMatrix(
        gSprites[sFactorySelectScreen->monPics[2].bgSpriteId].oam.matrixNum);
...
// Task data for Select_Task_OpenChosenMonPics, Select_Task_CloseChosenMonPics,
// Task_CloseMonPic, and Task_OpenMonPic
#define tWinLeft data[3]
#ifndef UBFIX
#define tWinRight                                                              \
  data[24] // UB: Typo? Likely meant data[4], 24 is out of bounds
#else
...
static void Select_Task_CloseChosenMonPics(u8 taskId) {
  ...
  default:
    ...
    StartSpriteAffineAnim(
        &gSprites[sFactorySelectScreen->monPics[1].bgSpriteId], 1);
    StartSpriteAffineAnim(
        &gSprites[sFactorySelectScreen->monPics[0].bgSpriteId], 1);
    StartSpriteAffineAnim(
        &gSprites[sFactorySelectScreen->monPics[2].bgSpriteId], 1);
    ClearGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_WIN0_ON);
    DestroyTask(taskId);
    break;
  }
...
static void Select_ShowChosenMons(void) {
  sFactorySelectScreen->monPics[1].bgSpriteId =
      CreateSprite(&sSpriteTemplate_Select_MonPicBgAnim, 120, 64, 1);
  sFactorySelectScreen->monPics[0].bgSpriteId =
      CreateSprite(&sSpriteTemplate_Select_MonPicBgAnim, 44, 64, 1);
  sFactorySelectScreen->monPics[2].bgSpriteId =
      CreateSprite(&sSpriteTemplate_Select_MonPicBgAnim, 196, 64, 1);
  ...
  for (i = 1; i < selectState; i++) {
    for (j = 0; j < SELECTABLE_MONS_COUNT; j++) {
      if (sFactorySelectScreen->mons[j].selectedId == i) {
        if (gFacilityTrainerMons[sFactorySelectScreen->mons[j].monId].species ==
            species)
          return FALSE;
  ...
  case FADESTATE_RUN:
    if (sFactorySelectScreen->fadeSpeciesNameActive) {
      if (sFactorySelectScreen->faceSpeciesNameDelay) {
        ...
        BlendPalettes(1 << PALNUM_FADE_TEXT,
                      sFactorySelectScreen->fadeSpeciesNameCoeff, 0);
...
static void CopySwappedMonData(void) {
  ...
  gPlayerParty[sFactorySwapScreen->playerMonId] =
      gEnemyParty[sFactorySwapScreen->enemyMonId];
  friendship = 0;
  SetMonData(&gPlayerParty[sFactorySwapScreen->playerMonId],
             MON_DATA_FRIENDSHIP, &friendship);
  gSaveBlock2Ptr->frontier.rentalMons[sFactorySwapScreen->playerMonId].monId =
      gSaveBlock2Ptr->frontier
          .rentalMons[sFactorySwapScreen->enemyMonId + FRONTIER_PARTY_SIZE]
          .monId;
  gSaveBlock2Ptr->frontier.rentalMons[sFactorySwapScreen->playerMonId].ivs =
      gSaveBlock2Ptr->frontier
          .rentalMons[sFactorySwapScreen->enemyMonId + FRONTIER_PARTY_SIZE]
          .ivs;
  gSaveBlock2Ptr->frontier.rentalMons[sFactorySwapScreen->playerMonId]
      .personality = GetMonData(&gEnemyParty[sFactorySwapScreen->enemyMonId],
                                MON_DATA_PERSONALITY, NULL);
  gSaveBlock2Ptr->frontier.rentalMons[sFactorySwapScreen->playerMonId]
      .abilityNum =
      GetBoxMonData(&gEnemyParty[sFactorySwapScreen->enemyMonId].box,
                    MON_DATA_ABILITY_NUM, NULL);
}

// Main swap states
// States for the main tasks of the Swap_ functions after initialization,
// including: Swap_Task_OpenSummaryScreen, Swap_Task_HandleYesNo,
// Swap_Task_HandleMenu, and Swap_Task_HandleChooseMons Tasks sharing states was
// unnecessary, see "Main select states"
#define STATE_CHOOSE_MONS_INIT 0
#define STATE_CHOOSE_MONS_HANDLE_INPUT 1
#define STATE_MENU_INIT 2
#define STATE_MENU_HANDLE_INPUT 3
...
static void Swap_Task_OpenSummaryScreen(u8 taskId) {
  ...
  case STATE_SUMMARY_CLEAN:
    if (!gPaletteFade.active) {
      ...
      HideMonPic(sFactorySwapScreen->monPic,
                 &sFactorySwapScreen->monPicAnimating);
...
static void Swap_AskQuitSwapping(u8 taskId) {
  if (gTasks[taskId].tState == 0) {
    ...
    gTasks[taskId].tFollowUpTaskPtrHi =
        (u32)(Swap_HandleQuitSwappingResponse) >> 16;
...
static void Swap_HandleAcceptMonResponse(u8 taskId) {
  CloseMonPic(sFactorySwapScreen->monPic, &sFactorySwapScreen->monPicAnimating,
              TRUE);
  if (gTasks[taskId].tSaidYes == TRUE) {
    gTasks[taskId].tState = 0;
    gTasks[taskId].func = Swap_Task_Exit;
...
static void Swap_AskAcceptMon(u8 taskId) {
  if (gTasks[taskId].tState == 0) {
    OpenMonPic(&sFactorySwapScreen->monPic.bgSpriteId,
               &sFactorySwapScreen->monPicAnimating, TRUE);
    Swap_PrintOnInfoWindow(gText_AcceptThisPkmn);
    sFactorySwapScreen->monSwapped = TRUE;
    gTasks[taskId].tState = STATE_YESNO_SHOW;
    gTasks[taskId].tFollowUpTaskPtrHi =
        (u32)(Swap_HandleAcceptMonResponse) >> 16;
  ...
  case STATE_MENU_HANDLE_INPUT:
    if (sFactorySwapScreen->monPicAnimating != TRUE) {
      if (JOY_NEW(A_BUTTON)) {
        ...
        PlaySE(SE_SELECT);
        CloseMonPic(sFactorySwapScreen->monPic,
                    &sFactorySwapScreen->monPicAnimating, TRUE);
        Swap_ErasePopupMenu(SWAP_WIN_OPTIONS);
        gTasks[taskId].tState = 0;
        gTasks[taskId].tFollowUpTaskPtrHi =
            (u32)(Swap_Task_HandleChooseMons) >> 16;
        gTasks[taskId].tFollowUpTaskPtrLo = (u32)(Swap_Task_HandleChooseMons);
        gTasks[taskId].tFollowUpTaskState = STATE_CHOOSE_MONS_HANDLE_INPUT;
        gTasks[taskId].func = Swap_Task_ScreenInfoTransitionIn;
      } else if (JOY_REPEAT(DPAD_UP)) {
...
// Handles input on the two main swap screens (choosing a current pokeon to get
// rid of, and choosing a new Pokémon to receive)
static void Swap_Task_HandleChooseMons(u8 taskId) {
  ...
  case STATE_CHOOSE_MONS_HANDLE_INPUT:
    if (JOY_NEW(A_BUTTON)) {
      // Run whatever action is currently selected (a Poké Ball, the Cancel
      // button, etc.)
      PlaySE(SE_SELECT);
...
static void Swap_Task_FadeOutSpeciesName(u8 taskId) {
  ...
  case 0:
    ...
    break;
  case 1:
    LoadPalette(&gPlttBufferUnfaded[BG_PLTT_ID(PALNUM_TEXT)],
                BG_PLTT_ID(PALNUM_FADE_TEXT), PLTT_SIZEOF(5));
    gTasks[taskId].tState++;
    break;
  case 2:
...
// Slide current pokeballs offscreen to the right and new pokeballs onscreen
// from the left during transition between player's/enemy's party screens
    ...
    for (i = FRONTIER_PARTY_SIZE - 1; i >= 0; i--) {
      if (i != FRONTIER_PARTY_SIZE - 1) {
        ...
        if (posX == 16 || gTasks[taskId].tBallCycled(i + 1) == TRUE) {
          ...
          gSprites[sFactorySwapScreen->ballSpriteIds[i]].x =
              gSprites[sFactorySwapScreen->ballSpriteIds[i + 1]].x - 48;
        }
      } else {
        lastX = gSprites[sFactorySwapScreen->ballSpriteIds[i]].x;
        gSprites[sFactorySwapScreen->ballSpriteIds[i]].x += 10;
      ...
      if (gTasks[taskId].tBallCycled(i) == TRUE) {
        ...
        if (gSprites[sFactorySwapScreen->ballSpriteIds[i]].x > (i * 48) + 72) {
          ...
          finished = TRUE;
        } else if (gSprites[sFactorySwapScreen->ballSpriteIds[i]].x ==
                   (i * 48) + 72) {
          finished = TRUE;
        } else {
          finished = FALSE;
      ...
      if (gSprites[sFactorySwapScreen->ballSpriteIds[i]].x - 16 >
          DISPLAY_WIDTH) {
        // Ball is offscreen right, cycle its palette and move to left side of
        // screen
        lastX = gSprites[sFactorySwapScreen->ballSpriteIds[i]].x;
        ...
        if (sFactorySwapScreen->inEnemyScreen == TRUE)
          ...
          gSprites[sFactorySwapScreen->ballSpriteIds[i]].oam.paletteNum =
              IndexOfSpritePaletteTag(PALTAG_BALL_GRAY);
...
static void Swap_Task_SlideButtonOnOffScreen(u8 taskId) {
  ...
  case SLIDE_BUTTON_PKMN:
    ...
    if (sliding == TRUE) {
      // Do movement
      for (i = 0;
           i < ARRAY_COUNT(sFactorySwapScreen->pkmnForSwapButtonSpriteIds[0]);
           i++) {
        for (j = 0;
             j < ARRAY_COUNT(sFactorySwapScreen->pkmnForSwapButtonSpriteIds);
             j++)
  ...
  case SLIDE_BUTTON_CANCEL:
    ...
    if (sliding == TRUE) {
      // Do movement
      for (i = 0; i < ARRAY_COUNT(sFactorySwapScreen->cancelButtonSpriteIds);
           i++) {
        for (j = 0;
             j < ARRAY_COUNT(sFactorySwapScreen->cancelButtonSpriteIds[0]); j++)
          gSprites[sFactorySwapScreen->cancelButtonSpriteIds[j][i]].x += deltaX;
      }
    } else {
      // Set final position
      for (j = 0; j < ARRAY_COUNT(sFactorySwapScreen->cancelButtonSpriteIds);
           j++) {
...
static void Swap_Task_ScreenInfoTransitionOut(u8 taskId) {
  ...
  switch (gTasks[taskId].tState) {
  case 0:
    LoadPalette(sSwapText_Pal, BG_PLTT_ID(PALNUM_FADE_TEXT),
                sizeof(sSwapText_Pal));
  ...
  case 1:
    ...
    break;
  case 2:
    BeginNormalPaletteFade(1 << PALNUM_FADE_TEXT, 0, 0, 16,
                           sPokeballGray_Pal[37]);
    gTasks[taskId].tState++;
    break;
  case 3:
    if (!gPaletteFade.active) {
...
static void Swap_Task_ScreenInfoTransitionIn(u8 taskId) {
  ...
  case 2:
    if (gTasks[taskId].tSlideFinishedPkmn == TRUE &&
        gTasks[taskId].tSlideFinishedCancel == TRUE) {
      gPlttBufferFaded[BG_PLTT_ID(PALNUM_FADE_TEXT) + 2] =
          sPokeballGray_Pal[37];
      Swap_PrintActionStrings();
      PutWindowTilemap(SWAP_WIN_ACTION_FADE);
      gTasks[taskId].tState++;
    ...
    break;
  case 3:
    BeginNormalPaletteFade(1 << PALNUM_FADE_TEXT, 0, 16, 0,
                           sPokeballGray_Pal[37]);
    gTasks[taskId].tState++;
    break;
  case 4:
  ...
  case 3:
    if (!FuncIsActiveTask(Swap_Task_SlideCycleBalls) &&
      ...
      gSprites[sFactorySwapScreen->cursorSpriteId].x =
          gSprites[sFactorySwapScreen
                       ->ballSpriteIds[sFactorySwapScreen->cursorPos]]
              .x;
      gTasks[sFactorySwapScreen->fadeSpeciesNameTaskId].func =
...
static void CB2_InitSwapScreen(void) {
  ...
  case 2:
    ...
    FreeAllSpritePalettes();
    ResetAllPicSprites();
    CpuCopy16(gFrontierFactoryMenu_Gfx, sSwapMenuTilesetBuffer,
              sizeof(gFrontierFactoryMenu_Gfx));
    ...
    CpuCopy16(gFrontierFactoryMenu_Tilemap, sSwapMenuTilemapBuffer,
              BG_SCREEN_SIZE);
  ...
  case 15:
    sFactorySwapScreen->fadeSpeciesNameTaskId =
        CreateTask(Swap_Task_FadeSpeciesName, 0);
    if (!sFactorySwapScreen->fromSummaryScreen) {
...
static void Swap_InitAllSprites(void) {
  ...
  for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
    sFactorySwapScreen->ballSpriteIds[i] =
        CreateSprite(&spriteTemplate, (48 * i) + 72, 64, 1);
  ...
  sFactorySwapScreen->cursorSpriteId = CreateSprite(
      ...
      gSprites[sFactorySwapScreen->ballSpriteIds[sFactorySwapScreen->cursorPos]]
          .x,
      88, 0);
  sFactorySwapScreen->menuCursor1SpriteId =
      CreateSprite(&sSpriteTemplate_Swap_MenuHighlightLeft, 176, 112, 0);
  sFactorySwapScreen->menuCursor2SpriteId =
      CreateSprite(&sSpriteTemplate_Swap_MenuHighlightRight, 176, 144, 0);
  ...
  sFactorySwapScreen->pkmnForSwapButtonSpriteIds[0][0] =
      CreateSprite(&spriteTemplate, DISPLAY_WIDTH, 120, 10);
  ...
  sFactorySwapScreen->pkmnForSwapButtonSpriteIds[0][1] =
      CreateSprite(&spriteTemplate, DISPLAY_WIDTH + 16, 120, 10);
  sFactorySwapScreen->pkmnForSwapButtonSpriteIds[0][2] =
      CreateSprite(&spriteTemplate, DISPLAY_WIDTH + 48, 120, 10);

  spriteTemplate = sSpriteTemplate_Swap_Arrow;
  spriteTemplate.tileTag = GFXTAG_ACTION_HIGHLIGHT_LEFT;
  sFactorySwapScreen->pkmnForSwapButtonSpriteIds[1][0] =
      CreateSprite(&spriteTemplate, DISPLAY_WIDTH, 120, 1);

  spriteTemplate = sSpriteTemplate_Swap_MenuHighlightLeft;
  spriteTemplate.tileTag = GFXTAG_ACTION_HIGHLIGHT_MIDDLE;
  sFactorySwapScreen->pkmnForSwapButtonSpriteIds[1][1] =
      CreateSprite(&spriteTemplate, DISPLAY_WIDTH + 16, 120, 1);
  spriteTemplate.tileTag = GFXTAG_ACTION_HIGHLIGHT_RIGHT;
  sFactorySwapScreen->pkmnForSwapButtonSpriteIds[1][2] =
      CreateSprite(&spriteTemplate, DISPLAY_WIDTH + 48, 120, 1);

  spriteTemplate = sSpriteTemplate_Swap_Arrow;
  spriteTemplate.tileTag = GFXTAG_ACTION_BOX_LEFT;
  sFactorySwapScreen->cancelButtonSpriteIds[0][0] =
      CreateSprite(&spriteTemplate, x, 144, 10);

  spriteTemplate = sSpriteTemplate_Swap_MenuHighlightLeft;
  spriteTemplate.tileTag = GFXTAG_ACTION_BOX_RIGHT;
  sFactorySwapScreen->cancelButtonSpriteIds[0][1] =
      CreateSprite(&spriteTemplate, x + 16, 144, 10);

  spriteTemplate = sSpriteTemplate_Swap_Arrow;
  spriteTemplate.tileTag = GFXTAG_ACTION_HIGHLIGHT_LEFT;
  sFactorySwapScreen->cancelButtonSpriteIds[1][0] =
      CreateSprite(&spriteTemplate, x, 144, 1);

  spriteTemplate = sSpriteTemplate_Swap_MenuHighlightLeft;
  spriteTemplate.tileTag = GFXTAG_ACTION_HIGHLIGHT_RIGHT;
  sFactorySwapScreen->cancelButtonSpriteIds[1][1] =
      CreateSprite(&spriteTemplate, x + 16, 144, 1);

  for (i = 0; i < 2; i++) {
    gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][0]]
        .centerToCornerVecX = 0;
    gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][0]]
        .centerToCornerVecY = 0;
    gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][1]]
        .centerToCornerVecX = 0;
    gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][1]]
        .centerToCornerVecY = 0;
    gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][2]]
        .centerToCornerVecX = 0;
    gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][2]]
        .centerToCornerVecY = 0;
    gSprites[sFactorySwapScreen->cancelButtonSpriteIds[i][0]]
        .centerToCornerVecX = 0;
    gSprites[sFactorySwapScreen->cancelButtonSpriteIds[i][0]]
        .centerToCornerVecY = 0;
    gSprites[sFactorySwapScreen->cancelButtonSpriteIds[i][1]]
        .centerToCornerVecX = 0;
    gSprites[sFactorySwapScreen->cancelButtonSpriteIds[i][1]]
        .centerToCornerVecY = 0;

    gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][0]].invisible =
        TRUE;
    gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][1]].invisible =
        TRUE;
    gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][2]].invisible =
        TRUE;
  ...
  gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[0][0]].invisible =
      FALSE;
  gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[0][1]].invisible =
      FALSE;
  gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[0][2]].invisible =
      FALSE;
}
...
static void Swap_DestroyAllSprites(void) {
  ...
  for (i = 0; i < ARRAY_COUNT(sFactorySwapScreen->pkmnForSwapButtonSpriteIds);
       i++) {
    for (j = 0;
         j < ARRAY_COUNT(sFactorySwapScreen->pkmnForSwapButtonSpriteIds[0]);
         j++)
      DestroySprite(
          &gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[i][j]]);
  }
  for (i = 0; i < ARRAY_COUNT(sFactorySwapScreen->cancelButtonSpriteIds); i++) {
    for (j = 0; j < ARRAY_COUNT(sFactorySwapScreen->cancelButtonSpriteIds[0]);
         j++)
...
static void Swap_UpdateActionCursorPosition(s8 direction) {
  ...
  if (direction > 0) // Move cursor down.
  {
    ...
    else if (sFactorySwapScreen->cursorPos + 1 !=
             sFactorySwapScreen->actionsCount)
      sFactorySwapScreen->cursorPos++;
    else
      sFactorySwapScreen->cursorPos = 0;
  } else // Move cursor up.
...
static void Swap_UpdateMenuCursorPosition(s8 direction) {
  ...
  if (direction > 0) // Move cursor down.
  {
    if (sFactorySwapScreen->menuCursorPos !=
        ARRAY_COUNT(sSwap_MenuOptionFuncs) - 1)
...
static void Swap_HighlightActionButton(u8 actionId) {
  ...
  for (i = 0;
    ...
    if (actionId == SWAPACTION_PKMN_FOR_SWAP) {
      ...
      gSprites[sFactorySwapScreen->pkmnForSwapButtonSpriteIds[1][i]].invisible =
          FALSE;

      // Hide button highlight on Cancel
      if (i < ARRAY_COUNT(sFactorySwapScreen->cancelButtonSpriteIds[0]))
        gSprites[sFactorySwapScreen->cancelButtonSpriteIds[1][i]].invisible =
            TRUE;
...
static void Swap_PrintPkmnSwap(void) {
  ...
  AddTextPrinterParameterized(SWAP_WIN_TITLE, FONT_NORMAL, gText_PkmnSwap, 2, 1,
                              0, NULL);
...
static void Swap_PrintMonSpecies(void) {
  ...
  if (sFactorySwapScreen->cursorPos >= FRONTIER_PARTY_SIZE) {
    ...
    StringCopy(gStringVar4, gSpeciesNames[species]);
    x = GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 86);
    AddTextPrinterParameterized3(SWAP_WIN_SPECIES, FONT_NORMAL, x, 1,
                                 sSwapSpeciesNameTextColors, 0, gStringVar4);
...
static void Swap_PrintMenuOptions(void) {
  ...
  AddTextPrinterParameterized3(SWAP_WIN_OPTIONS, FONT_NORMAL, 15, 1,
                               sSwapMenuOptionsTextColors, 0, gText_Summary2);
  AddTextPrinterParameterized3(SWAP_WIN_OPTIONS, FONT_NORMAL, 15, 17,
                               sSwapMenuOptionsTextColors, 0, gText_Swap);
  AddTextPrinterParameterized3(SWAP_WIN_OPTIONS, FONT_NORMAL, 15, 33,
                               sSwapMenuOptionsTextColors, 0, gText_Rechoose);
...
static void Swap_PrintYesNoOptions(void) {
  ...
  AddTextPrinterParameterized3(SWAP_WIN_YES_NO, FONT_NORMAL, 7, 1,
                               sSwapMenuOptionsTextColors, 0, gText_Yes3);
  AddTextPrinterParameterized3(SWAP_WIN_YES_NO, FONT_NORMAL, 7, 17,
                               sSwapMenuOptionsTextColors, 0, gText_No3);
...
static void Swap_PrintActionString(const u8 *str, u32 y, u32 windowId) {
  ...
  AddTextPrinterParameterized3(windowId, FONT_SMALL, x, y,
                               sSwapMenuOptionsTextColors, 0, str);
}
...
// For printing the species name once its selected. Keep the current fade but
// don't keep fading in and out
static void Swap_PrintMonSpeciesAtFade(void) {
  u16 species;
  u8 x;
  u16 pal[5];
  ...
  if (sFactorySwapScreen->cursorPos >= FRONTIER_PARTY_SIZE) {
    ...
    StringCopy(gStringVar4, gSpeciesNames[species]);
    x = GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 86);
    AddTextPrinterParameterized3(SWAP_WIN_SPECIES_AT_FADE, FONT_NORMAL, x, 1,
                                 sSwapSpeciesNameTextColors, 0, gStringVar4);
    CopyWindowToVram(SWAP_WIN_SPECIES_AT_FADE, COPYWIN_FULL);
  }
}
...
static void Swap_PrintMonSpeciesForTransition(void) {
  ...
  LoadPalette(sSwapText_Pal, BG_PLTT_ID(PALNUM_FADE_TEXT),
              sizeof(sSwapText_Pal));
  CpuCopy16(&gPlttBufferUnfaded[BG_PLTT_ID(PALNUM_TEXT)],
            &gPlttBufferFaded[BG_PLTT_ID(PALNUM_FADE_TEXT)], PLTT_SIZEOF(5));
...
static void Swap_PrintMonCategory(void) {
  ...
  if (monId >= FRONTIER_PARTY_SIZE) {
    ...
    CopyMonCategoryText(SpeciesToNationalPokedexNum(species), text);
    x = GetStringRightAlignXOffset(FONT_NORMAL, text, 118);
    AddTextPrinterParameterized(SWAP_WIN_MON_CATEGORY, FONT_NORMAL, text, x, 1,
                                0, NULL);
    CopyWindowToVram(SWAP_WIN_MON_CATEGORY, COPYWIN_GFX);
  }
}
...
static void Swap_RunMenuOptionFunc(u8 taskId) {
  sSwap_CurrentOptionFunc =
      sSwap_MenuOptionFuncs[sFactorySwapScreen->menuCursorPos];
...
static void Swap_RunActionFunc(u8 taskId) {
  sSwap_CurrentOptionFunc =
      sFactorySwapScreen->actionsData[sFactorySwapScreen->cursorPos].func;
  sSwap_CurrentOptionFunc(taskId);
}
...
static void Swap_ShowSummaryMonSprite(void) {
  ...
  sFactorySwapScreen->monPic.bgSpriteId =
      CreateSprite(&sSpriteTemplate_Swap_MonPicBgAnim, 120, 64, 1);
...
#ifdef BUGFIX
  sFactorySwapScreen->monPic.monSpriteId = CreateMonPicSprite_HandleDeoxys(
      species, otId, personality, TRUE, 88, 32, 15, TAG_NONE);
#else
  sFactorySwapScreen->monPic.monSpriteId = CreateMonPicSprite_HandleDeoxys(
      species, personality, otId, TRUE, 88, 32, 15, TAG_NONE);
#endif
...
static void CloseMonPic(struct FactoryMonPic pic, bool8 *animating,
                        bool8 swapScreen) {
  u8 taskId;
...
static void Task_OpenMonPic(u8 taskId) {
  ...
  default:
    DestroyTask(taskId);
    // Accessing data of destroyed task. Task data isn't reset until a new task
    // needs that task id.
