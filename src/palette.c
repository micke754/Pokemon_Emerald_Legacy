...
static EWRAM_DATA struct PaletteStruct sPaletteStructs[NUM_PALETTE_STRUCTS] = {
    0};
...
static const u8 sRoundedDownGrayscaleMap[] = {
    0,  0,  0,  0,  0,  5,  5,  5,  5,  5,  11, 11, 11, 11, 11, 16,
...
static bool8 UNUSED BeginPlttFade(u32 selectedPalettes, u8 delay, u8 startY,
                                  u8 targetY, u16 blendColor) {
  ReadPlttIntoBuffers();
  return BeginNormalPaletteFade(selectedPalettes, delay, startY, targetY,
                                blendColor);
}
  ...
  if (!palStruct->template->pst_field_8_0) {
    while (i < palStruct->template->size) {
      gPlttBufferUnfaded[palStruct->destOffset] =
          palStruct->template->src[srcOffset];
...
static void PaletteStruct_Blend(struct PaletteStruct *palStruct,
                                u32 *unkFlags) {
  if (gPaletteFade.active && ((1 << (palStruct->baseDestOffset >> 4)) &
                              gPaletteFade_selectedPalettes)) {
    if (!palStruct->template->pst_field_8_0) {
      if (gPaletteFade.delayCounter != gPaletteFade_delay) {
        BlendPalette(palStruct->baseDestOffset, palStruct->template->size,
                     gPaletteFade.y, gPaletteFade.blendColor);
      }
    } else {
      if (!gPaletteFade.delayCounter) {
        if (palStruct->countdown1 != palStruct->template->time1) {
          ...
          for (i = 0; i < palStruct->template->size; i++)
            gPlttBufferFaded[palStruct->baseDestOffset + i] =
                palStruct->template->src[srcOffset + i];
        }
      }
    }
  }
...
static u8 UpdateNormalPaletteFade(void) {
  ...
  if (IsSoftwarePaletteFadeFinishing()) {
    return gPaletteFade.active ? PALETTE_FADE_STATUS_ACTIVE
    ...
    while (selectedPalettes) {
      if (selectedPalettes & 1)
        BlendPalette(paletteOffset, 16, gPaletteFade.y,
    ...
    // gPaletteFade.active cannot change since the last time it was checked. So
    // this is equivalent to `return PALETTE_FADE_STATUS_ACTIVE;`
    return gPaletteFade.active ? PALETTE_FADE_STATUS_ACTIVE
                               : PALETTE_FADE_STATUS_DONE;
  }
}
...
void InvertPlttBuffer(u32 selectedPalettes) {
  ...
  while (selectedPalettes) {
    if (selectedPalettes & 1) {
      u8 i;
      for (i = 0; i < 16; i++)
        gPlttBufferFaded[paletteOffset + i] =
            ~gPlttBufferFaded[paletteOffset + i];
    }
    selectedPalettes >>= 1;
    paletteOffset += 16;
    ...
    if (selectedPalettes & 1) {
      ...
      for (i = 0; i < 16; i++) {
        struct PlttData *data =
            (struct PlttData *)&gPlttBufferFaded[paletteOffset + i];
        data->r += r;
        data->g += g;
        data->b += b;
      }
...
void UnfadePlttBuffer(u32 selectedPalettes) {
  ...
  while (selectedPalettes) {
    if (selectedPalettes & 1) {
      ...
      for (i = 0; i < 16; i++)
        gPlttBufferFaded[paletteOffset + i] =
            gPlttBufferUnfaded[paletteOffset + i];
    }
    selectedPalettes >>= 1;
    paletteOffset += 16;
  }
...
void BeginHardwarePaletteFade(u8 blendCnt, u8 delay, u8 y, u8 targetY,
...
void TintPalette_CustomTone(u16 *palette, u16 count, u16 rTone, u16 gTone,
                            u16 bTone) {
  s32 r, g, b, i;
  u32 gray;
...
// Blend the selected palettes in a series of steps toward or away from the
// color. Only used by the Groudon/Kyogre fight scene to flash the screen for
// lightning. One call is used to fade the bg from white, while another fades
// the duo from black
void BlendPalettesGradually(u32 selectedPalettes, s8 delay, u8 coeff,
                            u8 coeffTarget, u16 color, u8 priority, u8 id) {
  u8 taskId;
