...
#define GET_CONTEST_WINNER_ID(i)                                               \
  {                                                                            \
    for ((i) = 0; (i) < CONTESTANT_COUNT && gContestFinalStandings[(i)] != 0;  \
         (i)++)                                                                \
      ;                                                                        \
  }
...
// These tags are used by the spritesheets, and implicitly in the loop in
...
#define BAR_SEGMENT_LENGTH                                                     \
  8 // Each segment of the results bar is a single tile, so 8 pixels long
...
static const u16 sResultsTextWindow_Pal[] =
    INCBIN_U16("graphics/contest/results_screen/text_window.gbapal");
static const u8 sResultsTextWindow_Gfx[] =
    INCBIN_U8("graphics/contest/results_screen/text_window.4bpp");
static const u16 sMiscBlank_Pal[] =
    INCBIN_U16("graphics/interface/blank.gbapal");
...
static const struct SpriteSheet sSpriteSheets_ResultsTextWindow[] = {
    {.data = gMiscBlank_Gfx,
     .size = 0x400,
     .tag = TAG_RESULTS_TEXT_WINDOW_LEFT},
    {.data = gMiscBlank_Gfx,
     .size = 0x400,
     .tag = TAG_RESULTS_TEXT_WINDOW_MIDLEFT},
    {.data = gMiscBlank_Gfx,
     .size = 0x400,
     .tag = TAG_RESULTS_TEXT_WINDOW_MIDRIGHT},
    {.data = gMiscBlank_Gfx,
     .size = 0x400,
     .tag = TAG_RESULTS_TEXT_WINDOW_RIGHT},
    {.data = gMiscBlank_Gfx, .size = 0x400, .tag = TAG_LINK_TEXT_WINDOW_LEFT},
    {.data = gMiscBlank_Gfx,
     .size = 0x400,
     .tag = TAG_LINK_TEXT_WINDOW_MIDLEFT},
    {.data = gMiscBlank_Gfx,
     .size = 0x400,
     .tag = TAG_LINK_TEXT_WINDOW_MIDRIGHT},
...
static void InitContestResultsDisplay(void) {
  ...
  SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WIN01_BG1 | WINOUT_WIN01_BG2 |
                                   WINOUT_WIN01_BG3 | WINOUT_WIN01_CLR |
  ...
  SetGpuRegBits(REG_OFFSET_DISPCNT, DISPCNT_BG_ALL_ON | DISPCNT_OBJ_ON |
...
static void LoadContestResultsBgGfx(void) {
  ...
  LoadPalette(sResultsTextWindow_Pal, BG_PLTT_ID(15),
              sizeof(sResultsTextWindow_Pal));
...
static void CB2_StartShowContestResults(void) {
  ...
  sContestResults->data->showResultsTaskId =
      CreateTask(Task_ShowContestResults, 5);
  ...
  if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK) {
    ...
    case 0:
      ...
      gCurContestWinnerSaveIdx =
          GetContestWinnerSaveIdx(CONTEST_SAVE_FOR_ARTIST, FALSE);
...
static void Task_AnnouncePreliminaryResults(u8 taskId) {
  ...
  if (gTasks[taskId].tState == 0) {
    ...
    x = DrawResultsTextWindow(gText_AnnouncingResults,
    ...
    x = DrawResultsTextWindow(gText_PreliminaryResults,
                              sContestResults->data->slidingTextBoxSpriteId);
...
static void Task_AnnounceRound2Results(u8 taskId) {
  ...
  if (sContestResults->data->slidingTextBoxState == SLIDING_TEXT_OFFSCREEN) {
    if (++gTasks[taskId].tTimer == 21) {
      gTasks[taskId].tTimer = 0;
      x = DrawResultsTextWindow(gText_Round2Results,
                                sContestResults->data->slidingTextBoxSpriteId);
      StartTextBoxSlideIn(x, TEXT_BOX_Y, -1, 1088);
    }
  } else if (sContestResults->data->slidingTextBoxState ==
             SLIDING_TEXT_ARRIVED) {
...
static void Task_AnnounceWinner(u8 taskId) {
  ...
  case 4:
    if (++gTasks[taskId].tTimer == 21) {
      ...
      x = DrawResultsTextWindow(winnerTextBuffer,
                                sContestResults->data->slidingTextBoxSpriteId);
      StartTextBoxSlideIn(x, TEXT_BOX_Y, -1, 1088);
      gTasks[taskId].tState++;
    }
...
static void Task_ShowWinnerMonBanner(u8 taskId) {
  ...
  case 0:
    ...
    spriteId = CreateSprite(&gMultiuseSpriteTemplate, DISPLAY_WIDTH + 32,
                            DISPLAY_HEIGHT / 2, 10);
  ...
  case 1:
    if (++gTasks[taskId].data[3] == 1) {
      ...
      gBattle_WIN0V =
          WIN_RANGE(DISPLAY_HEIGHT / 2 - counter, DISPLAY_HEIGHT / 2 + counter);
      if (counter == 32)
        gTasks[taskId].tState++;
    }
    break;
...
static void LoadContestMonIcon(u16 species, u8 monIndex, u8 srcOffset,
                               u8 useDmaNow, u32 personality) {
  const u8 *iconPtr;
  ...
  if (useDmaNow) {
    RequestDma3Copy(iconPtr, (void *)BG_CHAR_ADDR(1) + monIndex * 0x200, 0x180,
                    1);
    var0 = ((monIndex + 10) << 12);
    var1 = (monIndex * 0x10 + 0x200);
    WriteSequenceToBgTilemapBuffer(1, var1 | var0, 3, monIndex * 3 + 4, 4, 3,
                                   17, 1);
  } else {
    RequestDma3Copy(iconPtr, (void *)BG_CHAR_ADDR(1) + monIndex * 0x200, 0x180,
                    1);
...
static void LoadAllContestMonIcons(u8 srcOffset, bool8 useDmaNow) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++)
    LoadContestMonIcon(gContestMons[i].species, i, srcOffset, useDmaNow,
...
static void LoadAllContestMonIconPalettes(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    species = gContestMons[i].species;
    LoadPalette(
        gMonIconPalettes[gMonIconPaletteIndices[GetIconSpecies(species, 0)]],
        BG_PLTT_ID(10 + i), PLTT_SIZE_4BPP);
  }
}
...
static void TryCreateWirelessSprites(void) {
  ...
  if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_WIRELESS) {
    ...
    RequestDma3Fill(0xFFFFFFFF, (void *)BG_CHAR_ADDR(4) + sheet * 0x20, 0x80,
                    1);
...
static s32 DrawResultsTextWindow(const u8 *text, u8 spriteId) {
  ...
  AddTextPrinterParameterized3(windowId, FONT_NORMAL,
                               (tileWidth * 8 - strWidth) / 2, 1,
                               sContestLinkTextColors, TEXT_SKIP_DRAW, text);
  {
    s32 i;
    ...
    for (i = 1; i < (int)ARRAY_COUNT(spriteTilePtrs); i++)
      spriteTilePtrs[i] =
          (void *)(gSprites[sprite->data[i - 1]].oam.tileNum * 32 + OBJ_VRAM0);
...
static void StartTextBoxSlideIn(s16 x, u16 y, u16 slideOutTimer,
                                u16 slideIncrement) {
  struct Sprite *sprite =
      &gSprites[sContestResults->data->slidingTextBoxSpriteId];
  sprite->x = TEXT_BOX_X;
  sprite->y = y;
  sprite->x2 = 0;
  sprite->y2 = 0;
...
static void ShowLinkResultsTextBox(const u8 *text) {
  ...
  SetGpuReg(REG_OFFSET_WININ, WININ_WIN1_BG_ALL | WININ_WIN1_OBJ |
                                  WININ_WIN1_CLR | WININ_WIN0_BG1 |
                                  WININ_WIN0_BG2 | WININ_WIN0_BG3 |
...
static void LoadContestResultsTitleBarTilemaps(void) {
  ...
  if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK) {
    ...
    CopyToBgTilemapBufferRect(2, gContestResultsTitle_Normal_Tilemap, 5, 1, 10,
                              2);
    x = 15;
  } else if (gSpecialVar_ContestRank == CONTEST_RANK_SUPER) {
    CopyToBgTilemapBufferRect(2, gContestResultsTitle_Super_Tilemap, 5, 1, 10,
                              2);
    x = 15;
  } else if (gSpecialVar_ContestRank == CONTEST_RANK_HYPER) {
    CopyToBgTilemapBufferRect(2, gContestResultsTitle_Hyper_Tilemap, 5, 1, 10,
                              2);
    x = 15;
  } else // CONTEST_RANK_MASTER
  {
    CopyToBgTilemapBufferRect(2, gContestResultsTitle_Master_Tilemap, 5, 1, 10,
                              2);
    x = 15;
  }
  ...
  if (gSpecialVar_ContestCategory == CONTEST_CATEGORY_COOL) {
    ...
    palette = 1;
    CopyToBgTilemapBufferRect(2, gContestResultsTitle_Beauty_Tilemap, x, y, 5,
                              2);
    ...
    palette = 3;
    CopyToBgTilemapBufferRect(2, gContestResultsTitle_Smart_Tilemap, x, y, 5,
                              2);
  } else // CONTEST_CATEGORY_TOUGH
  {
    palette = 4;
    CopyToBgTilemapBufferRect(2, gContestResultsTitle_Tough_Tilemap, x, y, 5,
                              2);
  }

  x += 5;
  CopyToBgTilemapBufferRect(2, gContestResultsTitle_Tilemap, x, y, 6, 2);
  CopyToBgTilemapBufferRect_ChangePalette(2, sContestResults->tilemapBuffers[2],
                                          0, 0, 32, 4, palette);
}
...
static void Task_DrawFinalStandingNumber(u8 taskId) {
  ...
  if (gTasks[taskId].tState == 0) {
    ...
    if (--gTasks[taskId].data[11] == -1) {
      ...
      WriteSequenceToBgTilemapBuffer(2, firstTileNum + 0x10, 1,
                                     gTasks[taskId].tMonIndex * 3 + 6, 2, 1, 17,
                                     1);
...
static void Task_StartHighlightWinnersBox(u8 taskId) {
  ...
  CopyToBgTilemapBufferRect_ChangePalette(
      2, i * 0xC0 + 0x100 + sContestResults->tilemapBuffers[2], 0, i * 3 + 4,
      32, 3, 9);
...
static void Task_HighlightWinnersBox(u8 taskId) {
  if (++gTasks[taskId].data[11] == 1) {
    ...
    BlendPalette(BG_PLTT_ID(9) + 1, 1, gTasks[taskId].data[12],
                 RGB(13, 28, 27));
...
static void Task_CreateConfetti(u8 taskId) {
  if (++gTasks[taskId].data[0] == 5) {
    ...
    if (sContestResults->data->confettiCount < 40) {
      u8 spriteId = CreateSprite(&sSpriteTemplate_Confetti,
                                 (Random() % DISPLAY_WIDTH) - 20, 44, 5);
...
static void Task_BounceMonIconInBox(u8 taskId) {
  ...
  if (gTasks[taskId].tTimer++ == gTasks[taskId].tNumFrames) {
    gTasks[taskId].tTimer = 0;
    LoadContestMonIcon(gTasks[taskId].tSpecies, monIndex,
                       gTasks[taskId].tBounced, FALSE,
...
static void CalculateContestantsResultData(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    ...
    if (relativePoints % 10 > 4)
      relativePoints += 10;
    (*sContestResults->monResults)[i].relativePreliminaryPoints =
        relativePoints / 10;

    relativePoints =
        (abs(gContestMonRound2Points[i]) * 1000) / abs(highestPoints);
    if (relativePoints % 10 > 4)
      relativePoints += 10;
    (*sContestResults->monResults)[i].relativeRound2Points =
        relativePoints / 10;
    ...
    barLength =
        ((*sContestResults->monResults)[i].relativePreliminaryPoints * 0x5800) /
        100;
...
static void Task_UpdateContestResultBar(u8 taskId) {
  ...
  if (!minMaxReached && !targetReached) {
    ...
    for (i = 0; i < NUM_BAR_SEGMENTS; i++) {
      if (sContestResults->data->barLength[monId] >=
          (i + 1) * BAR_SEGMENT_LENGTH)
        tileOffset = 8; // Bar segment is full
      else if (sContestResults->data->barLength[monId] >=
               i * BAR_SEGMENT_LENGTH)
        tileOffset = sContestResults->data->barLength[monId] %
                     8; // Bar segment is between full and empty
      else
...
static void AllocContestResults(void) {
  ...
  sContestResults->monResults =
      AllocZeroed(sizeof(*sContestResults->monResults));
...
void TryEnterContestMon(void) {
  u8 eligibility =
      GetContestEntryEligibility(&gPlayerParty[gContestMonPartyIndex]);
...
void GiveMonContestRibbon(void) {
  ...
  case CONTEST_CATEGORY_COOL:
    ...
    if (ribbonData <= gSpecialVar_ContestRank &&
        ribbonData <= CONTEST_RANK_MASTER) {
      ribbonData++;
      SetMonData(&gPlayerParty[gContestMonPartyIndex], MON_DATA_COOL_RIBBON,
                 &ribbonData);
      if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) >
          NUM_CUTIES_RIBBONS)
        TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex],
                                 MON_DATA_COOL_RIBBON);
    }
    break;
  case CONTEST_CATEGORY_BEAUTY:
    ribbonData = GetMonData(&gPlayerParty[gContestMonPartyIndex],
                            MON_DATA_BEAUTY_RIBBON);
    if (ribbonData <= gSpecialVar_ContestRank &&
        ribbonData <= CONTEST_RANK_MASTER) {
      ribbonData++;
      SetMonData(&gPlayerParty[gContestMonPartyIndex], MON_DATA_BEAUTY_RIBBON,
                 &ribbonData);
      if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) >
          NUM_CUTIES_RIBBONS)
        TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex],
  ...
  case CONTEST_CATEGORY_CUTE:
    ...
    if (ribbonData <= gSpecialVar_ContestRank &&
        ribbonData <= CONTEST_RANK_MASTER) {
      ribbonData++;
      SetMonData(&gPlayerParty[gContestMonPartyIndex], MON_DATA_CUTE_RIBBON,
                 &ribbonData);
      if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) >
          NUM_CUTIES_RIBBONS)
        TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex],
  ...
  case CONTEST_CATEGORY_SMART:
    ...
    if (ribbonData <= gSpecialVar_ContestRank &&
        ribbonData <= CONTEST_RANK_MASTER) {
      ribbonData++;
      SetMonData(&gPlayerParty[gContestMonPartyIndex], MON_DATA_SMART_RIBBON,
                 &ribbonData);
      if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) >
          NUM_CUTIES_RIBBONS)
        TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex],
  ...
  case CONTEST_CATEGORY_TOUGH:
    ...
    if (ribbonData <= gSpecialVar_ContestRank &&
        ribbonData <= CONTEST_RANK_MASTER) {
      ribbonData++;
      SetMonData(&gPlayerParty[gContestMonPartyIndex], MON_DATA_TOUGH_RIBBON,
                 &ribbonData);
      if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) >
          NUM_CUTIES_RIBBONS)
        TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex],
...
void GetContestMonConditionRanking(void) {
  ...
  for (i = 0, rank = 0; i < CONTESTANT_COUNT; i++) {
    if (gContestMonRound1Points[gSpecialVar_0x8006] <
...
void CB2_SetStartContestCallback(void) { SetMainCallback2(CB2_StartContest); }
...
void GetContestPlayerId(void) { gSpecialVar_0x8004 = gContestPlayerMonIndex; }

void ContestLinkTransfer(u8 category) {
  u8 newTaskId;
  LockPlayerFieldControls();
  newTaskId = CreateTask(Task_LinkContest_Init, 0);
  SetTaskFuncWithFollowupFunc(newTaskId, Task_LinkContest_Init,
...
static void Task_StartCommunication(u8 taskId) {
  if (gLinkContestFlags & LINK_CONTEST_FLAG_HAS_RS_PLAYER) {
    ...
    SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateMonsRS,
...
static void Task_StartCommunicateRngRS(u8 taskId) {
  SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateRngRS,
                              Task_StartCommunicateLeaderIdsRS);
}

static void Task_StartCommunicateLeaderIdsRS(u8 taskId) {
  SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateLeaderIdsRS,
                              Task_StartCommunicateCategoryRS);
}

static void Task_StartCommunicateCategoryRS(u8 taskId) {
  SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateCategoryRS,
...
static void Task_LinkContest_CalculateTurnOrderRS(u8 taskId) {
  ...
  SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateTurnOrder,
...
void SetContestTrainerGfxIds(void) {
  gSaveBlock1Ptr->vars[VAR_OBJ_GFX_ID_0 - VARS_START] =
      gContestMons[0].trainerGfxId;
  gSaveBlock1Ptr->vars[VAR_OBJ_GFX_ID_1 - VARS_START] =
      gContestMons[1].trainerGfxId;
  gSaveBlock1Ptr->vars[VAR_OBJ_GFX_ID_2 - VARS_START] =
      gContestMons[2].trainerGfxId;
}
...
u8 CountPlayerMuseumPaintings(void) {
  ...
  for (i = 0; i < NUM_CONTEST_WINNERS - MUSEUM_CONTEST_WINNERS_START; i++) {
    if (gSaveBlock1Ptr->contestWinners[MUSEUM_CONTEST_WINNERS_START + i]
            .species)
      count++;
  }
  ...
  if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK) {
    for (i = 0; i < gNumLinkContestPlayers; i++) {
      objectEventId = GetObjectEventIdByLocalIdAndMap(
          sContestantLocalIds[i], gSaveBlock1Ptr->location.mapNum,
      ...
      if (version == VERSION_RUBY || version == VERSION_SAPPHIRE) {
        if (gLinkPlayers[i].gender == MALE)
          LoadPalette(gObjectEventPal_RubySapphireBrendan, OBJ_PLTT_ID(6 + i),
                      PLTT_SIZE_4BPP);
        else
          LoadPalette(gObjectEventPal_RubySapphireMay, OBJ_PLTT_ID(6 + i),
                      PLTT_SIZE_4BPP);
      } else {
        if (gLinkPlayers[i].gender == MALE)
          LoadPalette(gObjectEventPal_Brendan, OBJ_PLTT_ID(6 + i),
                      PLTT_SIZE_4BPP);
        else
...
bool8 GiveMonArtistRibbon(void) {
  ...
  hasArtistRibbon =
      GetMonData(&gPlayerParty[gContestMonPartyIndex], MON_DATA_ARTIST_RIBBON);
  if (!hasArtistRibbon && gContestFinalStandings[gContestPlayerMonIndex] == 0 &&
      gSpecialVar_ContestRank == CONTEST_RANK_MASTER &&
      gContestMonTotalPoints[gContestPlayerMonIndex] >= 800) {
    hasArtistRibbon = 1;
    SetMonData(&gPlayerParty[gContestMonPartyIndex], MON_DATA_ARTIST_RIBBON,
               &hasArtistRibbon);
    if (GetRibbonCount(&gPlayerParty[gContestMonPartyIndex]) >
        NUM_CUTIES_RIBBONS)
      TryPutSpotTheCutiesOnAir(&gPlayerParty[gContestMonPartyIndex],
...
void ShowContestEntryMonPic(void) {
  ...
  if (FindTaskIdByFunc(Task_ShowContestEntryMonPic) == TASK_NONE) {
    ...
    spriteId = CreateSprite(&gMultiuseSpriteTemplate, (left + 1) * 8 + 32,
                            (top * 8) + 40, 0);
...
void ClearLinkContestFlags(void) { gLinkContestFlags = 0; }
