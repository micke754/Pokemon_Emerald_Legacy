...
static void PrintMysteryMenuText(u8 windowId, const u8 *text, u8 x, u8 y,
                                 s32 speed);
...
static void CB2_MysteryEventMenu(void) {
  ...
  case 3:
    if ((gLinkStatus & LINK_STAT_MASTER) &&
        (gLinkStatus & LINK_STAT_PLAYER_COUNT) > 4) {
      PlaySE(SE_PIN);
  ...
  case 6:
    if (IsLinkConnectionEstablished()) {
      if (gReceivedRemoteLinkPlayers) {
        if (GetLinkPlayerDataExchangeStatusTimed(2, 2) ==
...
static void PrintMysteryMenuText(u8 windowId, const u8 *text, u8 x, u8 y,
  ...
  AddTextPrinterParameterized4(windowId, FONT_NORMAL, x, y, letterSpacing,
                               lineSpacing, textColor, speed, text);
}
