...
void FadeInBGM(u8 speed) { m4aMPlayFadeIn(&gMPlayInfo_BGM, speed); }

void FadeOutBGM(u8 speed) { m4aMPlayFadeOut(&gMPlayInfo_BGM, speed); }
...
void PlayCry_Normal(u16 species, s8 pan) {
  ...
  PlayCryInternal(species, pan, CRY_VOLUME, CRY_PRIORITY_NORMAL,
                  CRY_MODE_NORMAL);
...
// Assuming it's not CRY_MODE_DOUBLES, this is equivalent to PlayCry_Normal
// except it allows other modes.
...
#define GET_CRY(speciesIndex, tableId, reversed)                               \
  ((reversed) ? &gCryTable_Reverse[(128 * (tableId)) + (speciesIndex)]         \
              : &gCryTable[(128 * (tableId)) + (speciesIndex)])
...
void StopCry(void) { m4aMPlayStop(gMPlay_PokemonCry); }
...
void PlaySE(u16 songNum) { m4aSongNumStart(songNum); }
