...
enum {
  ...
  WIN_TURN_OFF_TITLE, // "Ruby/Sapphire" at the top of the "turn off the power"
                      // screen
};
...
static const u16
    ALIGNED(4) sText_Pal[] = INCBIN_U16("graphics/berry_fix/text.gbapal");
...
static const struct {
  ...
  const u16 *palette;
    ...
    [SCENE_ENSURE_CONNECT] = {gBerryFixGbaConnect_Gfx,
                              gBerryFixGbaConnect_Tilemap,
                              gBerryFixGbaConnect_Pal},
    [SCENE_TURN_OFF_POWER] = {gBerryFixGameboyLogo_Gfx,
                              gBerryFixGameboyLogo_Tilemap,
                              gBerryFixGameboyLogo_Pal},
    [SCENE_TRANSMITTING] = {gBerryFixGbaTransfer_Gfx,
                            gBerryFixGbaTransfer_Tilemap,
                            gBerryFixGbaTransfer_Pal},
    [SCENE_FOLLOW_INSTRUCT] = {gBerryFixGbaTransferHighlight_Gfx,
                               gBerryFixGbaTransferHighlight_Tilemap,
                               gBerryFixGbaTransferHighlight_Pal},
    [SCENE_TRANSMIT_FAILED] = {gBerryFixGbaTransferError_Gfx,
                               gBerryFixGbaTransferError_Tilemap,
                               gBerryFixGbaTransferError_Pal},
    [SCENE_BEGIN] = {gBerryFixWindow_Gfx, gBerryFixWindow_Tilemap,
                     gBerryFixWindow_Pal},
};
...
static void BerryFix_Main(void) {
  ...
  case MAINSTATE_MULTIBOOT:
    ...
    if (sBerryFix->mb.probe_count != 0 || (!(sBerryFix->mb.response_bit & 2) ||
                                           !(sBerryFix->mb.client_bit & 2))) {
      sBerryFix->timer = 0;
...
static void BerryFix_GpuSet(void) {
  ...
  InitBgsFromTemplates(0, sBerryFixBgTemplates,
                       ARRAY_COUNT(sBerryFixBgTemplates));
  ...
  left = (120 - width) / 2;
  AddTextPrinterParameterized3(WIN_GAME_NAMES, FONT_SMALL, left, 3,
                               sGameTitleTextColors, TEXT_SKIP_DRAW,
                               sText_Emerald);
  ...
  width = GetStringWidth(FONT_SMALL, sText_RubySapphire, 0);
  left = (112 - width) / 2;
  AddTextPrinterParameterized3(WIN_TURN_OFF_TITLE, FONT_SMALL, left, 0,
                               sGameTitleTextColors, TEXT_SKIP_DRAW,
                               sText_RubySapphire);

  width = GetStringWidth(FONT_NORMAL, sText_BerryProgramUpdate, 0);
  left = (208 - width) / 2;
  AddTextPrinterParameterized3(WIN_TITLE, FONT_NORMAL, left, 2,
                               sBerryProgramTextColors, TEXT_SKIP_DRAW,
...
static void BerryFix_SetScene(int scene) {
  ...
  AddTextPrinterParameterized3(WIN_MSG_BODY, FONT_NORMAL, 0, 0,
                               sBerryProgramTextColors, TEXT_SKIP_DRAW,
  ...
  // These palettes range in size from 32-48 colors, so the below is
  // interpreting whatever follows the palette (by default, the corresponding
  // tiles) as the remaining 80-96.
  CpuCopy32(sBerryFixGraphics[scene].palette, (void *)BG_PLTT,
            PLTT_SIZEOF(128));
  ShowBg(0);
  ShowBg(1);
}
