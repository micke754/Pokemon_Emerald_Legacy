...
void InitScriptContext(struct ScriptContext *ctx, void *cmdTable,
                       void *cmdTableEnd) {
  s32 i;
...
void LockPlayerFieldControls(void) { sLockFieldControls = TRUE; }

void UnlockPlayerFieldControls(void) { sLockFieldControls = FALSE; }

bool8 ArePlayerFieldControlsLocked(void) { return sLockFieldControls; }
...
// Puts the script into waiting mode; usually called from a wait* script
// command.
void ScriptContext_Stop(void) { sGlobalScriptContextStatus = CONTEXT_WAITING; }
...
void RunOnLoadMapScript(void) { MapHeaderRunScriptType(MAP_SCRIPT_ON_LOAD); }
...
u32 CalculateRamScriptChecksum(void) {
  return CalcCRC16WithTable((u8 *)(&gSaveBlock1Ptr->ramScript.data),
                            sizeof(gSaveBlock1Ptr->ramScript.data));
}
...
bool8 InitRamScript(const u8 *script, u16 scriptSize, u8 mapGroup, u8 mapNum,
                    u8 objectId) {
...
void InitRamScript_NoObjectEvent(u8 *script, u16 scriptSize) {
  ...
  InitRamScript(script, scriptSize, MAP_GROUP(UNDEFINED), MAP_NUM(UNDEFINED),
                NO_OBJECT);
}
