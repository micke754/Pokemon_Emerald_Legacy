...
static void AnimDirtScatter(struct Sprite *sprite) {
  ...
  targetYPos =
      GetBattlerSpriteCoord2(gBattleAnimTarget, BATTLER_COORD_Y_PIC_OFFSET);
...
static void AnimTask_DigBounceMovement(u8 taskId) {
  ...
  case 2:
    ...
    if (task->data[5] > 63) {
      ...
      gSprites[task->data[10]].x2 =
          DISPLAY_WIDTH + 32 - gSprites[task->data[10]].x;
      task->data[0]++;
    }
    break;
  case 3:
...
static void AnimDigDirtMound(struct Sprite *sprite) {
  ...
  sprite->x = GetBattlerSpriteCoord(battler, BATTLER_COORD_X) - 16 +
...
// Shakes battler(s) or the battle terrain back and forth horizontally. Used by
// e.g. Earthquake, Eruption arg0: What to shake. 0-3 for any specific battler,
// MAX_BATTLERS_COUNT for all battlers, MAX_BATTLERS_COUNT + 1 for the terrain
// arg1: Shake intensity, used to calculate horizontal pixel offset (if 0, use
// move power instead) arg2: Length of time to shake for
...
void AnimTask_PositionFissureBgOnBattler(u8 taskId) {
  struct Task *newTask;
  u8 battler = (gBattleAnimArgs[0] & ANIM_TARGET) ? gBattleAnimTarget
                                                  : gBattleAnimAttacker;
  ...
  newTask->data[1] =
      (32 - GetBattlerSpriteCoord(battler, BATTLER_COORD_X_2)) & 0x1FF;
  newTask->data[2] =
      (64 - GetBattlerSpriteCoord(battler, BATTLER_COORD_Y_PIC_OFFSET)) & 0xFF;
