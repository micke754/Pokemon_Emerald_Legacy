...
// Each section is an area of the route between the y coordinates in
// sRoute119WaterTileData
#define NUM_FISHING_SPOTS_1 131
#define NUM_FISHING_SPOTS_2 167
#define NUM_FISHING_SPOTS_3 149
#define NUM_FISHING_SPOTS                                                      \
  (NUM_FISHING_SPOTS_1 + NUM_FISHING_SPOTS_2 + NUM_FISHING_SPOTS_3)
...
static bool8
TryGetAbilityInfluencedWildMonIndex(const struct WildPokemon *wildMon, u8 type,
                                    u8 ability, u8 *monIndex, u32 size);
...
// Each fishing spot on Route 119 is given a number between 1 and
// NUM_FISHING_SPOTS inclusive. The number is determined by counting the valid
// fishing spots left to right top to bottom. The map is divided into three
// sections, with each section having a pre-counted number of fishing spots to
// start from to avoid counting a large number of spots at the bottom of the
// map. Note that a spot is considered valid if it is surfable and not a
// waterfall. To exclude all of the inaccessible water metatiles (so that they
// can't be selected as a Feebas spot) they use a different metatile that isn't
// actually surfable because it has MB_NORMAL instead. This function is given
// the coordinates and section of a fishing spot and returns which number it is.
...
static bool8 CheckFeebas(void) {
  ...
  if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(ROUTE119) &&
    ...
    if (y >= sRoute119WaterTileData[3 * 0 + 0] &&
        y <= sRoute119WaterTileData[3 * 0 + 1])
      route119Section = 0;
    if (y >= sRoute119WaterTileData[3 * 1 + 0] &&
        y <= sRoute119WaterTileData[3 * 1 + 1])
      route119Section = 1;
    if (y >= sRoute119WaterTileData[3 * 2 + 0] &&
        y <= sRoute119WaterTileData[3 * 2 + 1])
      route119Section = 2;
...
static u8 ChooseWildMonIndex_Land(void) {
  ...
  if (rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_0)
    return 0;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_0 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_1)
    return 1;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_1 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_2)
    return 2;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_2 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_3)
    return 3;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_3 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_4)
    return 4;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_4 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_5)
    return 5;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_5 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_6)
    return 6;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_6 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_7)
    return 7;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_7 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_8)
    return 8;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_8 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_9)
    return 9;
  else if (rand >= ENCOUNTER_CHANCE_LAND_MONS_SLOT_9 &&
           rand < ENCOUNTER_CHANCE_LAND_MONS_SLOT_10)
    return 10;
  else
    return 11;
}
  ...
  for (i = 0;; i++) {
    ...
    if (gWildMonHeaders[i].mapGroup == gSaveBlock1Ptr->location.mapGroup &&
      ...
      if (gSaveBlock1Ptr->location.mapGroup ==
              MAP_GROUP(SHOAL_CAVE_LOW_TIDE_ICE_ROOM) &&
          gSaveBlock1Ptr->location.mapNum ==
              MAP_NUM(SHOAL_CAVE_LOW_TIDE_ICE_ROOM)) {
      ...
      if (gSaveBlock1Ptr->location.mapGroup ==
              MAP_GROUP(SHOAL_CAVE_LOW_TIDE_INNER_ROOM) &&
          gSaveBlock1Ptr->location.mapNum ==
              MAP_NUM(SHOAL_CAVE_LOW_TIDE_INNER_ROOM)) {
...
static void CreateWildMon(u16 species, u8 level) {
  ...
  if (checkCuteCharm && !GetMonData(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG) &&
    ...
    u8 gender = GetGenderFromSpeciesAndPersonality(leadingMonSpecies,
                                                   leadingMonPersonality);

    // misses mon is genderless check, although no genderless mon can have cute
    // charm as ability
    ...
    CreateMonWithGenderNatureLetter(&gEnemyParty[0], species, level,
                                    USE_RANDOM_IVS, gender, PickWildMonNature(),
                                    0, OT_ID_PLAYER_ID);
    return;
  }

  CreateMonWithNature(&gEnemyParty[0], species, level, USE_RANDOM_IVS,
                      PickWildMonNature());
}
#define TRY_GET_ABILITY_INFLUENCED_WILD_MON_INDEX(wildPokemon, type, ability,  \
                                                  ptr, count)                  \
  TryGetAbilityInfluencedWildMonIndex(wildPokemon, type, ability, ptr, count)

static bool8 TryGenerateWildMon(const struct WildPokemonInfo *wildMonInfo,
                                u8 area, u8 flags) {
  u8 wildMonIndex = 0;
  u8 level;
  ...
  switch (area) {
  case WILD_AREA_LAND:
    if (TRY_GET_ABILITY_INFLUENCED_WILD_MON_INDEX(
            wildMonInfo->wildPokemon, TYPE_STEEL, ABILITY_MAGNET_PULL,
            &wildMonIndex, LAND_WILD_COUNT))
      break;
    if (TRY_GET_ABILITY_INFLUENCED_WILD_MON_INDEX(
            wildMonInfo->wildPokemon, TYPE_ELECTRIC, ABILITY_STATIC,
            &wildMonIndex, LAND_WILD_COUNT))
      break;
...
static bool8 SetUpMassOutbreakEncounter(u8 flags) {
  ...
  CreateWildMon(gSaveBlock1Ptr->outbreakPokemonSpecies,
...
static bool8 WildEncounterCheck(u32 encounterRate, bool8 ignoreAbility) {
  ...
  if (!ignoreAbility && !GetMonData(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG)) {
    ...
    if (ability == ABILITY_STENCH &&
      ...
      encounterRate *= 2;
    else if (ability == ABILITY_SAND_VEIL &&
  ...
  if (headerId == HEADER_NONE) {
    if (gMapHeader.mapLayoutId ==
      ...
      if (prevMetatileBehavior != curMetatileBehavior &&
        ...
        return FALSE;
      else if (WildEncounterCheck(gBattlePikeWildMonHeaders[headerId]
                                      .landMonsInfo->encounterRate,
                                  FALSE) != TRUE)
        return FALSE;
      else if (TryGenerateWildMon(
                   gBattlePikeWildMonHeaders[headerId].landMonsInfo,
                   WILD_AREA_LAND, WILD_CHECK_KEEN_EYE) != TRUE)
        return FALSE;
    ...
    if (gMapHeader.mapLayoutId == LAYOUT_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR) {
      ...
      if (prevMetatileBehavior != curMetatileBehavior &&
          !AllowWildCheckOnNewMetatile())
        return FALSE;
      else if (WildEncounterCheck(gBattlePyramidWildMonHeaders[headerId]
                                      .landMonsInfo->encounterRate,
                                  FALSE) != TRUE)
        return FALSE;
      else if (TryGenerateWildMon(
    ...
    if (MetatileBehavior_IsLandWildEncounter(curMetatileBehavior) == TRUE) {
      if (gWildMonHeaders[headerId].landMonsInfo == NULL)
        ...
        return FALSE;
                   ...
                   gWildMonHeaders[headerId].landMonsInfo->encounterRate,
                   FALSE) != TRUE)
        return FALSE;
      ...
      if (TryStartRoamerEncounter() == TRUE) {
        ...
        if (DoMassOutbreakEncounterTest() == TRUE &&
            SetUpMassOutbreakEncounter(WILD_CHECK_REPEL |
        ...
        if (TryGenerateWildMon(
                gWildMonHeaders[headerId].landMonsInfo, WILD_AREA_LAND,
    ...
    } else if (MetatileBehavior_IsWaterWildEncounter(curMetatileBehavior) ==
                   TRUE ||
               (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING) &&
                MetatileBehavior_IsBridgeOverWater(curMetatileBehavior) ==
                    TRUE)) {
      if (AreLegendariesInSootopolisPreventingEncounters() == TRUE)
        ...
        return FALSE;
                   ...
                   gWildMonHeaders[headerId].waterMonsInfo->encounterRate,
                   FALSE) != TRUE)
        return FALSE;
      ...
      {
        if (TryGenerateWildMon(
                gWildMonHeaders[headerId].waterMonsInfo, WILD_AREA_WATER,
                WILD_CHECK_REPEL | WILD_CHECK_KEEN_EYE) == TRUE) {
          BattleSetup_StartWildBattle();
          return TRUE;
        }
  ...
  if (headerId != HEADER_NONE) {
    ...
    if (wildPokemonInfo == NULL) {
      gSpecialVar_Result = FALSE;
                   ...
                   TRUE &&
               TryGenerateWildMon(wildPokemonInfo, WILD_AREA_ROCKS,
...
bool8 SweetScentWildEncounter(void) {
  ...
  if (headerId == HEADER_NONE) {
    if (gMapHeader.mapLayoutId ==
        LAYOUT_BATTLE_FRONTIER_BATTLE_PIKE_ROOM_WILD_MONS) {
      headerId = GetBattlePikeWildMonHeaderId();
      if (TryGenerateWildMon(gBattlePikeWildMonHeaders[headerId].landMonsInfo,
                             WILD_AREA_LAND, 0) != TRUE)
        return FALSE;
    ...
    if (MetatileBehavior_IsLandWildEncounter(
      ...
      if (DoMassOutbreakEncounterTest() == TRUE)
        ...
        TryGenerateWildMon(gWildMonHeaders[headerId].landMonsInfo,
                           WILD_AREA_LAND, 0);

      BattleSetup_StartWildBattle();
      return TRUE;
    } else if (MetatileBehavior_IsWaterWildEncounter(
                   MapGridGetMetatileBehaviorAt(x, y)) == TRUE) {
      if (AreLegendariesInSootopolisPreventingEncounters() == TRUE)
        return FALSE;
      ...
      TryGenerateWildMon(gWildMonHeaders[headerId].waterMonsInfo,
                         WILD_AREA_WATER, 0);
      BattleSetup_StartWildBattle();
      return TRUE;
    }
...
static bool8 TryGetRandomWildMonIndexByType(const struct WildPokemon *wildMon,
                                            u8 type, u8 numMon, u8 *monIndex) {
  ...
  for (validMonCount = 0, i = 0; i < numMon; i++) {
    if (gSpeciesInfo[wildMon[i].species].types[0] == type ||
        gSpeciesInfo[wildMon[i].species].types[1] == type)
