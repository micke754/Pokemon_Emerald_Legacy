...
static const u8 sTextColors[] = {TEXT_COLOR_WHITE, TEXT_COLOR_DARK_GRAY,
...
static void AddTextPrinterForUnionRoomBattle(u8 windowId, const u8 *str, u8 x,
                                             u8 y, s32 speed) {
  s32 letterSpacing = 0;
  s32 lineSpacing = 1;
  ...
  AddTextPrinterParameterized4(windowId, FONT_NORMAL, x, y, letterSpacing,
                               lineSpacing, sTextColors, speed, str);
}

static bool32 PrintUnionRoomBattleMessage(s16 *state, const u8 *str,
                                          s32 speed) {
  switch (*state) {
  case 0:
...
void CB2_UnionRoomBattle(void) {
  ...
  case 0:
    ...
    break;
  case 1:
    if (PrintUnionRoomBattleMessage(&sBattle->textState,
                                    gText_CommStandbyAwaitingOtherPlayer, 0)) {
      gMain.state++;
    }
    break;
  case 2:
  ...
  case 6:
    ...
    break;
  case 7:
    if (PrintUnionRoomBattleMessage(&sBattle->textState, gText_RefusedBattle,
                                    1)) {
  ...
  case 8:
    ...
    break;
  case 9:
    if (PrintUnionRoomBattleMessage(&sBattle->textState, gText_BattleWasRefused,
                                    1)) {
      SetMainCallback2(CB2_ReturnToField);
    }
    break;
