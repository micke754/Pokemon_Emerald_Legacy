...
static const u16 sMonMarkings_Pal[] =
    INCBIN_U16("graphics/interface/mon_markings.gbapal");
static const u8 sMonMarkings_Gfx[] =
    INCBIN_U8("graphics/interface/mon_markings.4bpp");
...
static const union AnimCmd *const sAnims_MenuSprite[] = {
    sAnim_Marking_CircleOff,      sAnim_Marking_CircleOn,
    sAnim_Marking_SquareOff,      sAnim_Marking_SquareOn,
    sAnim_Marking_TriangleOff,    sAnim_Marking_TriangleOn,
    sAnim_Marking_HeartOff,       sAnim_Marking_HeartOn,
    [ANIM_CURSOR] = sAnim_Cursor, [ANIM_TEXT] = sAnim_OKCancelText,
};
...
void InitMonMarkingsMenu(struct MonMarkingsMenu *ptr) { sMenu = ptr; }
  ...
  case 0:
    ...
    for (i = 0; i < 6; i++)
      CpuFastCopy(sMenu->frameTiles + TILE_SIZE_4BPP,
                  dest + TILE_SIZE_4BPP * (i + 1), TILE_SIZE_4BPP);

    CpuFastCopy(sMenu->frameTiles + TILE_SIZE_4BPP * 2,
                dest + TILE_SIZE_4BPP * 7, TILE_SIZE_4BPP);
    sMenu->tileLoadState++;
    break;
  default:
    ...
    CpuFastCopy(sMenu->frameTiles + TILE_SIZE_4BPP * 5,
                dest + TILE_SIZE_4BPP * 7, TILE_SIZE_4BPP);
    sMenu->tileLoadState++;
    break;
  case 13:
    ...
    CpuFastCopy(sMenu->frameTiles + TILE_SIZE_4BPP * 8,
                dest + TILE_SIZE_4BPP * 7, TILE_SIZE_4BPP);
    sMenu->tileLoadState++;
    return FALSE;
  case 14:
...
static void CreateMonMarkingsMenuSprites(s16 x, s16 y, u16 baseTileTag,
                                         u16 basePaletteTag) {
  u16 i;
  u8 spriteId;
  ...
  if (spriteId != MAX_SPRITES) {
    ...
    CalcCenterToCornerVec(sMenu->textSprite, SPRITE_SHAPE(32x16),
                          SPRITE_SIZE(32x16), ST_OAM_AFFINE_OFF);
  } else {
    sMenu->textSprite = NULL;
  }
...
// Creates a mon marking combination sprite with a spritesheet that holds every
// possible combination, used by the summary screen / Pokénav
struct Sprite *CreateMonMarkingAllCombosSprite(u16 tileTag, u16 paletteTag,
                                               const u16 *palette) {
  if (!palette)
    palette = sMonMarkings_Pal;
  return CreateMarkingComboSprite(tileTag, paletteTag, palette,
                                  1 << NUM_MON_MARKINGS);
}

// Creates a mon marking combination sprite with a spritesheet that holds only
// one combination, used for the currently selected PC mon
struct Sprite *CreateMonMarkingComboSprite(u16 tileTag, u16 paletteTag,
                                           const u16 *palette) {
  if (!palette)
    palette = sMonMarkings_Pal;
...
static struct Sprite *CreateMarkingComboSprite(u16 tileTag, u16 paletteTag,
                                               const u16 *palette, u16 size) {
  u8 spriteId;
  struct SpriteTemplate template;
...
// Update what combination is shown, used for sprites created with
