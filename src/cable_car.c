...
static const u16 sGround_Tilemap[] =
    INCBIN_U16("graphics/cable_car/ground.bin.lz");
static const u16 sTrees_Tilemap[] =
    INCBIN_U16("graphics/cable_car/trees.bin.lz");
static const u16 sBgMountains_Tilemap[] =
    INCBIN_U16("graphics/cable_car/bg_mountains.bin.lz");
static const u16 sPylonTop_Tilemap[] =
    INCBIN_U16("graphics/cable_car/pylon_top.bin");
static const u16 sPylonPole_Tilemap[] =
    INCBIN_U16("graphics/cable_car/pylon_pole.bin.lz");
...
static void CB2_LoadCableCar(void) {
  ...
  case 2:
    ...
    sCableCar->bgMountainsTilemap =
        malloc_and_decompress(sBgMountains_Tilemap, &sizeOut);
    sCableCar->pylonPoleTilemap =
        malloc_and_decompress(sPylonPole_Tilemap, &sizeOut);
  ...
  case 5:
    ...
    break;
  case 6:
    CopyToBgTilemapBufferRect_ChangePalette(1, sCableCar->treesTilemap, 0, 17,
                                            32, 15, 17);
    CopyToBgTilemapBufferRect_ChangePalette(2, sCableCar->bgMountainsTilemap, 0,
                                            0, 30, 20, 17);
    CopyToBgTilemapBufferRect_ChangePalette(3, sCableCar->pylonTopTilemap, 0, 0,
                                            5, 2, 17);
    CopyToBgTilemapBufferRect_ChangePalette(3, sCableCar->pylonPoleTilemap, 0,
                                            2, 2, 20, 17);
    gMain.state++;
    break;
  case 7:
    InitGroundTilemapData(GOING_DOWN);
    CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->groundTilemap + 0x48,
                                            0, 14, 12, 3, 17);
    CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->groundTilemap + 0x6C,
                                            12, 17, 12, 3, 17);
    CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->groundTilemap + 0x90,
                                            24, 20, 12, 3, 17);
    CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->groundTilemap + 0x0,
                                            0, 17, 12, 3, 17);
    CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->groundTilemap + 0x24,
                                            0, 20, 12, 3, 17);
    CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->groundTilemap + 0x0,
                                            12, 20, 12, 3, 17);
    CopyToBgTilemapBufferRect_ChangePalette(0, sCableCar->groundTilemap + 0x24,
...
static void Task_AnimateBgGoingUp(u8 taskId) {
  if (sCableCar->state != STATE_END) {
    ...
    case 32:
      ...
      break;
    case 16:
      CopyToBgTilemapBufferRect_ChangePalette(3, sCableCar->pylonTopTilemap, 0,
                                              0, 5, 2, 17);
      CopyToBgTilemapBufferRect_ChangePalette(3, sCableCar->pylonPoleTilemap, 0,
                                              2, 2, 30, 17);
...
static void SpriteCB_CableCar(struct Sprite *sprite) {
  if (sCableCar->state != STATE_END) {
    if (!GOING_DOWN) {
      ...
      sprite->y =
          sprite->sYPos - (u8)(0.067f * S16TOPOSFLOAT(sCableCar->timer));
    } else {
      sprite->x = sprite->sXPos + (u8)(0.14f * S16TOPOSFLOAT(sCableCar->timer));
      sprite->y =
          sprite->sYPos + (u8)(0.067f * S16TOPOSFLOAT(sCableCar->timer));
...
#define sSameDir                                                               \
  data[1] // Whether or not the hiker is headed the same dir as the Cable Car
...
static void CreateCableCarSprites(void) {
  ...
  u16 playerGraphicsIds[2] = {[MALE] = OBJ_EVENT_GFX_RIVAL_BRENDAN_NORMAL,
                              [FEMALE] = OBJ_EVENT_GFX_RIVAL_MAY_NORMAL};
  u16 rval = Random();
  u16 hikerGraphicsIds[4] = {OBJ_EVENT_GFX_HIKER, OBJ_EVENT_GFX_CAMPER,
  ...
  void (*hikerCallbacks[2])(struct Sprite *) = {SpriteCB_HikerGoingUp,
  ...
  default:
    ...
    spriteId = CreateObjectGraphicsSprite(
        playerGraphicsIds[gSaveBlock2Ptr->playerGender], SpriteCB_Player, 200,
        73, 102);
  ...
  case TRUE:
    ...
    // Create player sprite
    spriteId = CreateObjectGraphicsSprite(
        playerGraphicsIds[gSaveBlock2Ptr->playerGender], SpriteCB_Player, 128,
        39, 102);
    if (spriteId != MAX_SPRITES) {
      gSprites[spriteId].oam.priority = 2;
      gSprites[spriteId].x2 = 8;
  ...
  for (i = 0; i < 9; i++) {
    spriteId =
        CreateSprite(&sSpriteTemplate_Cable, 16 * i + 96, 8 * i - 8, 0x68);
    gSprites[spriteId].x2 = 8;
  ...
  if ((rval % 64) == 0) {
    rval >>= 7;
    spriteId = CreateObjectGraphicsSprite(
        hikerGraphicsIds[rval % (ARRAY_COUNT(hikerGraphicsIds))],
        hikerCallbacks[GOING_DOWN], hikerCoords[GOING_DOWN][0],
        hikerCoords[GOING_DOWN][1], 106);
    if (spriteId != MAX_SPRITES) {
      gSprites[spriteId].oam.priority = 2;
      ...
      gSprites[spriteId].sDelay =
          hikerMovementDelayTable[rval % ARRAY_COUNT(hikerMovementDelayTable)];
    }
  }
}
...
static void BufferNextGroundSegment(void) {
  ...
  for (i = 0, k = 0, offset = 0x24 * (sCableCar->groundTilemapOffset + 2);
       i < 3; i++) {
    for (j = 0; j < ARRAY_COUNT(sCableCar->groundTileBuffer[0]); j++) {
      ...
      sCableCar->groundTileBuffer[i + 6][j] =
          (sCableCar->groundTilemap + 0x24)[k];
      k++;
    }
  }
...
static void AnimateGroundGoingUp(void) {
  ...
  sCableCar->bg0HorizontalOffset =
      sCableCar->groundXBase - sCableCar->groundXOffset;
  sCableCar->bg0VerticalOffset =
      sCableCar->groundYBase - sCableCar->groundYOffset;
...
static void DrawNextGroundSegmentGoingUp(void) {
  ...
  for (i = 0; i < ARRAY_COUNT(sCableCar->groundTileBuffer); i++) {
    ...
    FillBgTilemapBufferRect(
        0, sCableCar->groundTileBuffer[i][sCableCar->groundTileIdx],
        sGroundX_Up, sGroundY_Up, 1, 1, 17);
    sGroundX_Up = (sGroundX_Up + 1) % 32;
    FillBgTilemapBufferRect(
        0, sCableCar->groundTileBuffer[i][sCableCar->groundTileIdx + 1],
        sGroundX_Up, sGroundY_Up, 1, 1, 17);
  }
...
static void DrawNextGroundSegmentGoingDown(void) {
  ...
  for (i = 0; i < ARRAY_COUNT(sCableCar->groundTileBuffer); i++) {
    ...
    FillBgTilemapBufferRect(
        0, sCableCar->groundTileBuffer[i][sCableCar->groundTileIdx],
        sGroundX_Down, sGroundY_Down, 1, 1, 17);
  ...
  FillBgTilemapBufferRect(0, 0, sCableCar->groundSegmentXStart, sGroundY_Down,
                          2, 9, 17);
