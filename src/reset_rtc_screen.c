...
// Task data for the Task_ResetRtc_* series of tasks, when setting the time on
// the clock Data from these tasks is also used by the cursors and the main task
// (Task_ResetRtcScreen)
enum {
  DATAIDX_DAYS = 3,
  DATAIDX_HOURS,
...
static const struct WindowTemplate sInputTimeWindow = {.bg = 0,
                                                       .tilemapLeft = 4,
                                                       .tilemapTop = 9,
                                                       .width = 21,
                                                       .height = 2,
...
static const u8 sArrowDown_Gfx[] =
    INCBIN_U8("graphics/reset_rtc_screen/arrow_down.4bpp");
static const u8 sArrowRight_Gfx[] =
    INCBIN_U8("graphics/reset_rtc_screen/arrow_right.4bpp");
static const u16 sArrow_Pal[] =
    INCBIN_U16("graphics/reset_rtc_screen/arrow.gbapal");

static const struct SpriteFrameImage sPicTable_Arrow[] = {
    obj_frame_tiles(sArrowDown_Gfx), obj_frame_tiles(sArrowRight_Gfx)};

static const struct SpritePalette sSpritePalette_Arrow = {sArrow_Pal,
                                                          PALTAG_ARROW};
...
static void PrintTime(u8 windowId, u8 x, u8 y, u16 days, u8 hours, u8 minutes,
                      u8 seconds) {
  u8 *dest = gStringVar4;
  ...
  // Print minutes
  ConvertIntToDecimalStringN(gStringVar1, minutes, STR_CONV_MODE_LEADING_ZEROS,
                             2);
  dest = StringCopy(dest, gStringVar1);
  dest = StringCopy(dest, gText_Colon3);

  // Print seconds
  ConvertIntToDecimalStringN(gStringVar1, seconds, STR_CONV_MODE_LEADING_ZEROS,
                             2);
  dest = StringCopy(dest, gStringVar1);

  AddTextPrinterParameterized(windowId, FONT_NORMAL, gStringVar4, x, y,
                              TEXT_SKIP_DRAW, NULL);
}

static void ShowChooseTimeWindow(u8 windowId, u16 days, u8 hours, u8 minutes,
                                 u8 seconds) {
  DrawStdFrameWithCustomTileAndPalette(windowId, FALSE, 0x214, 0xE);
  PrintTime(windowId, 0, 1, days, hours, minutes, seconds);
  AddTextPrinterParameterized(windowId, FONT_NORMAL, gText_Confirm2, 126, 1, 0,
                              NULL);
  ScheduleBgCopyTilemapToVram(0);
}
  ...
  if (selection == SELECTION_CONFIRM) {
    if (JOY_NEW(A_BUTTON)) {
      ...
      tSelection = SELECTION_NONE;
                            ...
                            selectionInfo->minVal, selectionInfo->maxVal,
                            JOY_REPEAT(DPAD_UP | DPAD_DOWN))) {
    PlaySE(SE_SELECT);
...
static void Task_ShowResetRtcPrompt(u8 taskId) {
  ...
  case 0:
    ...
    AddTextPrinterParameterized(WIN_TIME, FONT_NORMAL, gText_PresentTime, 0, 1,
                                TEXT_SKIP_DRAW, 0);
    PrintTime(WIN_TIME, 0, 17, gLocalTime.days, gLocalTime.hours,
              gLocalTime.minutes, gLocalTime.seconds);

    AddTextPrinterParameterized(WIN_TIME, FONT_NORMAL, gText_PreviousTime, 0,
                                33, TEXT_SKIP_DRAW, 0);
    PrintTime(WIN_TIME, 0, 49, gSaveBlock2Ptr->lastBerryTreeUpdate.days,
  ...
  case MAINSTATE_WAIT_SET_TIME:
    if (gTasks[tSubTaskId].tFinished) {
      if (!gTasks[tSubTaskId].tSetTime) {
        ...
        DestroyTask(tSubTaskId);
        RtcReset();
        RtcCalcLocalTimeOffset(gLocalTime.days, gLocalTime.hours,
                               gLocalTime.minutes, gLocalTime.seconds);
