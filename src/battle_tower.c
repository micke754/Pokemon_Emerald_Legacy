...
static void
SetEReaderTrainerChecksum(struct BattleTowerEReaderTrainer *ereaderTrainer);
...
static const u8 sRubyFacilityClassToEmerald[RS_FACILITY_CLASSES_COUNT -
                                            2][2] = {
...
static const u8 *const *const sPartnerApprenticeTextTables[NUM_APPRENTICES] = {
    sPartnerApprenticeTexts1,  sPartnerApprenticeTexts2,
    sPartnerApprenticeTexts3,  sPartnerApprenticeTexts4,
    sPartnerApprenticeTexts5,  sPartnerApprenticeTexts6,
    sPartnerApprenticeTexts7,  sPartnerApprenticeTexts8,
    sPartnerApprenticeTexts9,  sPartnerApprenticeTexts10,
    sPartnerApprenticeTexts11, sPartnerApprenticeTexts12,
    sPartnerApprenticeTexts13, sPartnerApprenticeTexts14,
    sPartnerApprenticeTexts15, sPartnerApprenticeTexts16};
...
struct {
  ...
  u16 moves[MAX_MON_MOVES];
    ...
    {.species = SPECIES_AGGRON,
     ...
     .moves = {MOVE_IRON_DEFENSE, MOVE_IRON_TAIL, MOVE_SOLAR_BEAM,
               MOVE_THUNDERBOLT}},
    {.species = SPECIES_METAGROSS,
     ...
     .evs = {0, 0, 0, 0, 0, 0},
     .moves = {MOVE_REFLECT, MOVE_THUNDER_PUNCH, MOVE_PSYCHIC,
               MOVE_METAL_CLAW}}};
...
// The challenge number at which an Apprentice can appear, depending on how many
...
// Trainer ID ranges for possible frontier trainers to encounter on particular
...
static const u16 sRecordTrainerSpeechWon[] = {
    EC_WORD_YAY,  EC_WORD_YAY, EC_WORD_EXCL_EXCL,
    EC_WORD_I_VE, EC_WORD_WON, EC_WORD_EXCL_EXCL};

static const u16 sRecordTrainerSpeechLost[] = {
    EC_WORD_TOO, EC_WORD_BAD,  EC_WORD_ELLIPSIS,
    EC_WORD_WE,  EC_WORD_LOST, EC_WORD_ELLIPSIS};

// code
void CallBattleTowerFunc(void) { sBattleTowerFuncs[gSpecialVar_0x8004](); }
  ...
  if (battleMode == FRONTIER_MODE_SINGLES) {
    ...
    for (i = 0; i < APPRENTICE_COUNT; i++) {
      if (gSaveBlock2Ptr->apprentices[i].lvlMode != 0 &&
          sApprenticeChallengeThreshold[gSaveBlock2Ptr->apprentices[i]
                                            .numQuestions] == winStreak &&
  ...
  if (lvlMode == FRONTIER_LVL_TENT) {
    ...
    if (battleMode == FRONTIER_MODE_MULTIS ||
      ...
      gTrainerBattleOpponent_B =
          gSaveBlock2Ptr->frontier.trainerIds[id * 2 + 1];
...
u16 GetRandomScaledFrontierTrainerId(u8 challengeNum, u8 battleNum) {
  ...
  if (challengeNum <= 7) {
    if (battleNum == FRONTIER_STAGES_PER_CHALLENGE - 1) {
      // The last battle in each challenge has a jump in difficulty, pulls from
      // a table with higher ranges
      ...
      trainerId = (sFrontierTrainerIdRanges[challengeNum][1] -
                  ...
                  1;
      trainerId =
          sFrontierTrainerIdRanges[challengeNum][0] + (Random() % trainerId);
    }
  } else {
    // After challenge 7, trainer IDs always come from the last, hardest range,
    // which is the same for both trainer ID tables
    trainerId =
        (sFrontierTrainerIdRanges[7][1] - sFrontierTrainerIdRanges[7][0]) + 1;
...
static void UNUSED GetRandomScaledFrontierTrainerIdRange(u8 challengeNum,
                                                         u8 battleNum,
                                                         u16 *trainerIdPtr,
                                                         u8 *rangePtr) {
  u16 trainerId, range;
...
void PutNewBattleTowerRecord(struct EmeraldBattleTowerRecord *newRecordEm) {
  ...
  for (i = 0; i < BATTLE_TOWER_RECORD_COUNT; i++) {
    ...
    for (j = 0; j < TRAINER_ID_LENGTH; j++) {
      if (gSaveBlock2Ptr->frontier.towerRecords[i].trainerId[j] !=
          newRecord->trainerId[j])
        break;
    }
    if (j == TRAINER_ID_LENGTH) {
      for (k = 0; k < PLAYER_NAME_LENGTH; k++) {
#define INDEX k
        if (gSaveBlock2Ptr->frontier.towerRecords[i].name[INDEX] !=
            newRecord->name[INDEX])
          break;
  ...
  for (i = 1; i < BATTLE_TOWER_RECORD_COUNT; i++) {
    for (j = 0; j < slotsCount; j++) {
      if (gSaveBlock2Ptr->frontier.towerRecords[i].winStreak < slotValues[j]) {
        ...
        slotIds[0] = i;
        break;
      } else if (gSaveBlock2Ptr->frontier.towerRecords[i].winStreak >
                 slotValues[j]) {
        break;
      }
    }
    ...
    if (j == slotsCount) {
      slotValues[slotsCount] =
          gSaveBlock2Ptr->frontier.towerRecords[i].winStreak;
...
u8 GetFrontierTrainerFrontSpriteId(u16 trainerId) {
  ...
  if (trainerId == TRAINER_EREADER) {
    return gFacilityClassToPicIndex[gSaveBlock2Ptr->frontier.ereaderTrainer
                                        .facilityClass];
  } else if (trainerId == TRAINER_FRONTIER_BRAIN) {
...
u8 GetFrontierOpponentClass(u16 trainerId) {
  ...
  if (trainerId == TRAINER_EREADER) {
    trainerClass =
        gFacilityClassToTrainerClass[gSaveBlock2Ptr->frontier.ereaderTrainer
                                         .facilityClass];
  } else if (trainerId == TRAINER_FRONTIER_BRAIN) {
    ...
    trainerClass = gFacilityClassToTrainerClass[gFacilityTrainers[trainerId]
                                                    .facilityClass];
  } else if (trainerId < TRAINER_RECORD_MIXING_APPRENTICE) {
    if (gBattleTypeFlags & BATTLE_TYPE_RECORDED) {
      trainerClass =
          gFacilityClassToTrainerClass[GetRecordedBattleRecordMixFriendClass()];
    } else {
...
void GetFrontierTrainerName(u8 *dst, u16 trainerId) {
  ...
  if (trainerId == TRAINER_EREADER) {
    ...
    TVShowConvertInternationalString(
        dst, GetApprenticeNameInLanguage(id, language), language);
    return;
  }
...
static void FillTrainerParty(u16 trainerId, u8 firstMonId, u8 monCount) {
  ...
  if (trainerId < FRONTIER_TRAINERS_COUNT) {
    ...
    for (i = firstMonId; i < firstMonId + FRONTIER_PARTY_SIZE; i++)
      CreateBattleTowerMon(
          &gEnemyParty[i],
          &gSaveBlock2Ptr->frontier.ereaderTrainer.party[i - firstMonId]);
    return;
  } else if (trainerId == TRAINER_FRONTIER_BRAIN) {
  ...
  // successfully chosen. The trainer's party may not have duplicate Pokémon
  ...
  while (i != monCount) {
    ...
    // Ensure this exact Pokémon index isn't a duplicate. This check doesn't
    // seem necessary because the species and held items were already checked
    // directly above.
    for (j = 0; j < i; j++) {
    ...
    for (j = 0; j < MAX_MON_MOVES; j++) {
      SetMonMoveSlot(&gEnemyParty[i + firstMonId],
                     gFacilityTrainerMons[monId].moves[j], j);
      if (gFacilityTrainerMons[monId].moves[j] == MOVE_FRUSTRATION)
        friendship = 0; // Frustration is more powerful the lower the pokemon's
                        // friendship is.
    }

    SetMonData(&gEnemyParty[i + firstMonId], MON_DATA_FRIENDSHIP, &friendship);
    SetMonData(
        &gEnemyParty[i + firstMonId], MON_DATA_HELD_ITEM,
        &gBattleFrontierHeldItems[gFacilityTrainerMons[monId].itemTableId]);

    // The Pokémon was successfully added to the trainer's party, so it's safe
    // to move on to the next party slot.
    i++;
  }
}
...
static void UNUSED Unused_CreateApprenticeMons(u16 trainerId, u8 firstMonId) {
  ...
  for (i = 0; i != FRONTIER_PARTY_SIZE; i++) {
    CreateMonWithEVSpread(&gEnemyParty[firstMonId + i],
                          apprentice->party[i].species, level, fixedIV, 8);
    friendship = MAX_FRIENDSHIP;
    for (j = 0; j < MAX_MON_MOVES; j++) {
...
u16 GetRandomFrontierMonFromSet(u16 trainerId) {
  ...
  } while ((level == FRONTIER_MAX_LEVEL_50 || level == 20) &&
           monId > FRONTIER_MONS_HIGH_TIER);
...
static void FillFactoryFrontierTrainerParty(u16 trainerId, u8 firstMonId) {
  ...
  if (trainerId < FRONTIER_TRAINERS_COUNT) {
    ...
    if (gSaveBlock2Ptr->frontier.curChallengeBattleNum <
      ...
      fixedIV = GetFactoryMonFixedIV(
          challengeNum, TRUE); // Last trainer in challenge uses higher IVs
  } else if (trainerId == TRAINER_EREADER) {
    for (i = firstMonId; i < firstMonId + FRONTIER_PARTY_SIZE; i++)
      CreateBattleTowerMon(
          &gEnemyParty[i],
  ...
  for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
    ...
    friendship = 0;
    for (j = 0; j < MAX_MON_MOVES; j++)
      SetMonMoveAvoidReturn(&gEnemyParty[firstMonId + i],
                            gFacilityTrainerMons[monId].moves[j], j);

    SetMonData(&gEnemyParty[firstMonId + i], MON_DATA_FRIENDSHIP, &friendship);
    SetMonData(
        &gEnemyParty[firstMonId + i], MON_DATA_HELD_ITEM,
        &gBattleFrontierHeldItems[gFacilityTrainerMons[monId].itemTableId]);
  }
}
...
static void HandleSpecialTrainerBattleEnd(void) {
  ...
  case SPECIAL_BATTLE_SECRET_BASE:
    for (i = 0; i < PARTY_SIZE; i++) {
      u16 itemBefore =
          GetMonData(&gSaveBlock1Ptr->playerParty[i], MON_DATA_HELD_ITEM);
...
void DoSpecialTrainerBattle(void) {
  ...
  case SPECIAL_BATTLE_TOWER:
    ...
    case FRONTIER_MODE_LINK_MULTIS:
      gBattleTypeFlags |= BATTLE_TYPE_DOUBLE | BATTLE_TYPE_LINK |
                          BATTLE_TYPE_MULTI | BATTLE_TYPE_TOWER_LINK_MULTI;
      FillFrontierTrainersParties(FRONTIER_MULTI_PARTY_SIZE);
      break;
    }
    ...
    BattleTransition_StartOnField(
        GetSpecialBattleTransition(B_TRANSITION_GROUP_B_TOWER));
    break;
  case SPECIAL_BATTLE_SECRET_BASE:
    for (i = 0; i < PARTY_SIZE; i++) {
      u16 itemBefore = GetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM);
      SetMonData(&gSaveBlock1Ptr->playerParty[i], MON_DATA_HELD_ITEM,
                 &itemBefore);
    }
    CreateTask(Task_StartBattleAfterTransition, 1);
    PlayMapChosenOrBattleBGM(0);
  ...
  case SPECIAL_BATTLE_EREADER:
    ZeroEnemyPartyMons();
    for (i = 0;
         i < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.ereaderTrainer.party);
         i++)
      CreateBattleTowerMon(&gEnemyParty[i],
                           &gSaveBlock2Ptr->frontier.ereaderTrainer.party[i]);
  ...
  case SPECIAL_BATTLE_DOME:
    ...
    BattleTransition_StartOnField(
        GetSpecialBattleTransition(B_TRANSITION_GROUP_B_DOME));
    break;
  case SPECIAL_BATTLE_PALACE:
    ...
    PlayMapChosenOrBattleBGM(0);
    BattleTransition_StartOnField(
        GetSpecialBattleTransition(B_TRANSITION_GROUP_B_PALACE));
    break;
  case SPECIAL_BATTLE_ARENA:
  ...
  case SPECIAL_BATTLE_FACTORY:
    ...
    PlayMapChosenOrBattleBGM(0);
    BattleTransition_StartOnField(
        GetSpecialBattleTransition(B_TRANSITION_GROUP_B_FACTORY));
    break;
  case SPECIAL_BATTLE_PIKE_SINGLE:
    gBattleTypeFlags = BATTLE_TYPE_TRAINER | BATTLE_TYPE_BATTLE_TOWER;
    FillFrontierTrainerParty(FRONTIER_PARTY_SIZE);
    ...
    PlayMapChosenOrBattleBGM(0);
    BattleTransition_StartOnField(
        GetSpecialBattleTransition(B_TRANSITION_GROUP_B_PIKE));
    break;
  case SPECIAL_BATTLE_PYRAMID:
    ...
    PlayMapChosenOrBattleBGM(0);
    BattleTransition_StartOnField(
        GetSpecialBattleTransition(B_TRANSITION_GROUP_B_PYRAMID));
    break;
  case SPECIAL_BATTLE_PIKE_DOUBLE:
    gBattleTypeFlags = BATTLE_TYPE_TRAINER | BATTLE_TYPE_BATTLE_TOWER |
                       BATTLE_TYPE_DOUBLE | BATTLE_TYPE_TWO_OPPONENTS;
  ...
  case SPECIAL_BATTLE_ARCHIE_AND_MATT:
    ...
    gApproachingTrainerId = 0;
    BattleSetup_ConfigureTrainerBattle(MtPyre_Summit_EventScript_ArchieTrainer +
                                       1);
    gApproachingTrainerId = 1;
    BattleSetup_ConfigureTrainerBattle(MtPyre_Summit_EventScript_MattTrainer +
                                       1);
    CreateTask(Task_StartBattleAfterTransition, 1);
    PlayMapChosenOrBattleBGM(0);
...
static void SaveBattleTowerRecord(void) {
  ...
  struct EmeraldBattleTowerRecord *playerRecord =
      &gSaveBlock2Ptr->frontier.towerPlayer;
  ...
  if (gSaveBlock2Ptr->playerGender != MALE) {
    class =
        gTowerFemaleFacilityClasses[(gSaveBlock2Ptr->playerTrainerId[0] +
...
static void SaveTowerChallenge(void) {
  ...
  s32 challengeNum =
      (signed)(gSaveBlock2Ptr->frontier.towerWinStreaks[battleMode][lvlMode] /
               FRONTIER_STAGES_PER_CHALLENGE);
...
static void LoadMultiPartnerCandidatesData(void) {
    ...
    do {
      ...
      for (i = 0; i < j; i++) {
        if (gSaveBlock2Ptr->frontier.trainerIds[i] == trainerId)
          break;
        if (gFacilityTrainers[gSaveBlock2Ptr->frontier.trainerIds[i]]
                .facilityClass == gFacilityTrainers[trainerId].facilityClass)
          break;
      }
    } while (i != j);
  ...
  for (i = 0; i < 6; i++) {
    ...
    objEventTemplates[i + 1].graphicsId =
        GetBattleFacilityTrainerGfxId(trainerId);
    for (j = 0; j < 2; j++) {
      while (1) {
        ...
        for (k = 8; k < r10; k++) {
          if (gFacilityTrainerMons[gSaveBlock2Ptr->frontier.trainerIds[k]]
...
static void GetPotentialPartnerMoveAndSpecies(u16 trainerId, u16 monId) {
  ...
  if (trainerId != TRAINER_EREADER) {
    if (trainerId < FRONTIER_TRAINERS_COUNT) {
      ...
      move = gSaveBlock2Ptr->frontier
                 .towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND]
                 .party[gFrontierTempParty[gSpecialVar_0x8005 + 1]]
                 .moves[0];
      species = gSaveBlock2Ptr->frontier
                    .towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND]
                    .party[gFrontierTempParty[gSpecialVar_0x8005 + 1]]
                    .species;
    } else {
...
// For multi battles in the Battle Tower, the player may choose a partner by
// talking to them These partners can be an NPC or a former/record-mixed
// Apprentice When talked to, their response consists of: PARTNER_MSGID_INTRO -
// A greeting PARTNER_MSGID_MON1 - Naming one Pokémon on their team, and a move
// it has PARTNER_MSGID_MON2_ASK - Naming a second Pokémon on their team, a move
// it has, and asking if they'd like to be their partner PARTNER_MSGID_ACCEPT -
// If the player agrees to be their partner PARTNER_MSGID_REJECT - If the player
// declines to be their partner
static void ShowPartnerCandidateMessage(void) {
  s32 i, j, partnerId;
  s32 monId;
  ...
  for (partnerId = 0; partnerId < ARRAY_COUNT(sPartnerTrainerTextTables);
       partnerId++) {
  ...
  if (trainerId < FRONTIER_TRAINERS_COUNT) {
    ShowFieldMessage(
        sPartnerTrainerTextTables[partnerId].strings[gSpecialVar_0x8005]);
  } else if (trainerId < TRAINER_RECORD_MIXING_APPRENTICE) {
    ShowFieldMessage(
        sPartnerTrainerTextTables[partnerId].strings[gSpecialVar_0x8005]);
  }
  // Trainer is a former/record-mixed Apprentice, do Apprentice message
    ...
    u8 apprenticeId =
        gSaveBlock2Ptr
            ...
            .id;
    ShowFieldMessage(
        sPartnerApprenticeTextTables[apprenticeId][gSpecialVar_0x8005]);
  }
}
  ...
  case 0:
    if (battleMode == FRONTIER_MODE_LINK_MULTIS) {
      ...
      if (IsLinkTaskFinished()) {
        SendBlock(BitmaskAllOtherLinkPlayers(), &challengeNum,
                  sizeof(challengeNum));
  ...
  case 3:
    if ((GetBlockReceivedStatus() & 3) == 3) {
      ...
      memcpy(&gSaveBlock2Ptr->frontier.trainerIds, gBlockRecvBuffer,
             sizeof(gSaveBlock2Ptr->frontier.trainerIds));
      gTrainerBattleOpponent_A =
          gSaveBlock2Ptr->frontier.trainerIds[battleNum * 2];
      gTrainerBattleOpponent_B =
          gSaveBlock2Ptr->frontier.trainerIds[battleNum * 2 + 1];
      SetBattleFacilityTrainerGfxId(gTrainerBattleOpponent_A, 0);
      SetBattleFacilityTrainerGfxId(gTrainerBattleOpponent_B, 1);
...
static void SetTowerInterviewData(void) {
  ...
  gSaveBlock2Ptr->frontier.towerInterview.playerSpecies = GetMonData(
      &gPlayerParty[gBattlerPartyIndexes[0]], MON_DATA_SPECIES, NULL);
  for (i = 0; i < POKEMON_NAME_LENGTH + 1; i++)
    gSaveBlock2Ptr->frontier.towerInterview.opponentMonNickname[i] =
        gBattleMons[0].nickname[i];
  ...
  if (GetCurrentBattleTowerWinStreak(lvlMode, battleMode) > 55) {
    for (i = 0; i < monCount; i++) {
      ...
      if (!GetMonData(&gSaveBlock1Ptr->playerParty[partyIndex], ribbonType)) {
        ...
        SetMonData(&gSaveBlock1Ptr->playerParty[partyIndex], ribbonType,
                   &gSpecialVar_Result);
        ribbons[i].count =
            GetRibbonCount(&gSaveBlock1Ptr->playerParty[partyIndex]);
      }
    }
  }
...
static void UNUSED FillEReaderTrainerWithPlayerData(void) {
  struct BattleTowerEReaderTrainer *ereaderTrainer =
      &gSaveBlock2Ptr->frontier.ereaderTrainer;
  s32 i, j;
  ...
  if (gSaveBlock2Ptr->playerGender != MALE) {
    ...
    ereaderTrainer->facilityClass =
        gTowerMaleFacilityClasses[(gSaveBlock2Ptr->playerTrainerId[0] +
                                   gSaveBlock2Ptr->playerTrainerId[1] +
                                   gSaveBlock2Ptr->playerTrainerId[2] +
                                   gSaveBlock2Ptr->playerTrainerId[3]) %
                                  ARRAY_COUNT(gTowerMaleFacilityClasses)];
  ...
  for (i = 0; i < (int)ARRAY_COUNT(ereaderTrainer->party); i++)
    ConvertPokemonToBattleTowerPokemon(&gPlayerParty[i],
                                       &ereaderTrainer->party[i]);
...
u8 GetEreaderTrainerClassId(void) {
  return gFacilityClassToTrainerClass[gSaveBlock2Ptr->frontier.ereaderTrainer
                                          .facilityClass];
}
...
static void FillPartnerParty(u16 trainerId) {
  ...
  if (trainerId == TRAINER_STEVEN_PARTNER) {
    for (i = 0; i < MULTI_PARTY_SIZE; i++) {
      ...
      CreateMon(&gPlayerParty[MULTI_PARTY_SIZE + i], sStevenMons[i].species,
                sStevenMons[i].level, sStevenMons[i].fixedIV, TRUE, j,
                OT_ID_PRESET, STEVEN_OTID);
      for (j = 0; j < PARTY_SIZE; j++)
        SetMonData(&gPlayerParty[MULTI_PARTY_SIZE + i], MON_DATA_HP_EV + j,
                   &sStevenMons[i].evs[j]);
      for (j = 0; j < MAX_MON_MOVES; j++)
        SetMonMoveSlot(&gPlayerParty[MULTI_PARTY_SIZE + i],
                       sStevenMons[i].moves[j], j);
      SetMonData(&gPlayerParty[MULTI_PARTY_SIZE + i], MON_DATA_OT_NAME,
                 gTrainers[TRAINER_STEVEN_2].trainerName);
      j = MALE;
    ...
    const struct TrainerMon *partyData =
        gTrainers[trainerId - TRAINER_CUSTOM_PARTNER].party.TrainerMon;
    otID = Random32();
    ...
    for (i = 0;
         i < 3 && i < gTrainers[trainerId - TRAINER_CUSTOM_PARTNER].partySize;
         i++) {
      const struct TrainerMon *partyData =
          gTrainers[trainerId - TRAINER_CUSTOM_PARTNER].party.TrainerMon;
      ...
      for (j = 0;
           gTrainers[trainerId - TRAINER_CUSTOM_PARTNER].trainerName[j] != EOS;
           j++)
        nameHash +=
            gTrainers[trainerId - TRAINER_CUSTOM_PARTNER].trainerName[j];

      gender = MON_MALE; // defaults to 0

      if (gTrainers[trainerId - TRAINER_CUSTOM_PARTNER].doubleBattle == TRUE)
        personalityValue = 0x80;
      else if (gTrainers[trainerId - TRAINER_CUSTOM_PARTNER]
      ...
      if ((partyData[i].nature > 0) || (partyData[i].gender > 0))
        CreateMonWithGenderNatureLetter(
            &gPlayerParty[i + 3], partyData[i].species, partyData[i].lvl,
            fixedIV, gender, partyData[i].nature, 0,
            partyData[i].shiny ? OT_ID_SHINY : OT_ID_RANDOM_NO_SHINY);
      else {
        CreateMon(&gPlayerParty[i + 3], partyData[i].species, partyData[i].lvl,
                  fixedIV, TRUE, personalityValue,
      ...
      if (partyData[i].nickname[0] != '\0')
        SetMonData(&gPlayerParty[i + 3], MON_DATA_NICKNAME,
                   &partyData[i].nickname);

      // Should take only constants of ABILITY_SLOT_1, ABILITY_SLOT_2, or
      // ABILITY_HIDDEN. If desired, implement else where undefined abilities
      // are chosen between slots 1 and 2, a la gen IV.
      ...
      if (partyData[i].heldItem > 0)
        SetMonData(&gPlayerParty[i + 3], MON_DATA_HELD_ITEM,
                   &partyData[i].heldItem);

      // Check if moves were defined.
      if (partyData[i].moves[0] != 0) {
        for (j = 0; j < MAX_MON_MOVES; j++) {
          SetMonData(&gPlayerParty[i + 3], MON_DATA_MOVE1 + j,
                     &partyData[i].moves[j]);
      ...
      if (partyData[i].iv == 0) {
        for (j = 0; j < NUM_STATS; j++) {
          SetMonData(&gPlayerParty[i + 3], MON_DATA_HP_IV + j,
                     &partyData[i].ivs[j]);
        }
      ...
      for (j = 0; j < NUM_STATS; j++) {
        SetMonData(&gPlayerParty[i + 3], MON_DATA_HP_EV + j,
                   &partyData[i].evs[j]);
      }
      StringCopy(trainerName,
                 gTrainers[trainerId - TRAINER_CUSTOM_PARTNER].trainerName);
    ...
    for (i = 0; i < FRONTIER_MULTI_PARTY_SIZE; i++) {
      ...
      CreateMonWithEVSpreadNatureOTID(&gPlayerParty[MULTI_PARTY_SIZE + i],
                                      gFacilityTrainerMons[monId].species,
                                      level, gFacilityTrainerMons[monId].nature,
                                      ivs, gFacilityTrainerMons[monId].evSpread,
                                      otID);
      friendship = MAX_FRIENDSHIP;
      for (j = 0; j < MAX_MON_MOVES; j++) {
        SetMonMoveSlot(&gPlayerParty[MULTI_PARTY_SIZE + i],
      ...
      SetMonData(&gPlayerParty[MULTI_PARTY_SIZE + i], MON_DATA_FRIENDSHIP,
                 &friendship);
      SetMonData(
          &gPlayerParty[MULTI_PARTY_SIZE + i], MON_DATA_HELD_ITEM,
          &gBattleFrontierHeldItems[gFacilityTrainerMons[monId].itemTableId]);
    ...
    for (i = 0; i < FRONTIER_MULTI_PARTY_SIZE; i++) {
      ...
      CreateBattleTowerMon_HandleLevel(&gPlayerParty[MULTI_PARTY_SIZE + i],
                                       &monData, TRUE);
      SetMonData(&gPlayerParty[MULTI_PARTY_SIZE + i], MON_DATA_OT_NAME,
                 trainerName);
...
bool32 RubyBattleTowerRecordToEmerald(struct RSBattleTowerRecord *src,
                                      struct EmeraldBattleTowerRecord *dst) {
  s32 i, validMons = 0;
...
// UB: Reading outside the array. sRubyFacilityClassToEmerald has less than
// FACILITY_CLASSES_COUNT entries.
    ...
    CpuFill32(0, &dst->party[FRONTIER_PARTY_SIZE],
              sizeof(dst->party[FRONTIER_PARTY_SIZE]));
    CalcEmeraldBattleTowerChecksum(dst);
    return TRUE;
  }
}

bool32 EmeraldBattleTowerRecordToRuby(struct EmeraldBattleTowerRecord *src,
                                      struct RSBattleTowerRecord *dst) {
  s32 i, validMons = 0;
...
// Frontier Trainer parties are roughly scaled in difficulty with higher trainer
// IDs, so scale IVs as well Duplicated in Battle Dome as GetDomeTrainerMonIvs
static u8 GetFrontierTrainerFixedIvs(u16 trainerId) {
  u8 fixedIv;
...
static u16 GetBattleTentTrainerId(void) {
  ...
  if (facility ==
      FRONTIER_FACILITY_PALACE) // Verdanturf Tent; uses Palace mechanics
    return Random() % NUM_BATTLE_TENT_TRAINERS;
  else if (facility ==
           FRONTIER_FACILITY_ARENA) // Fallarbor Tent; uses Arena mechanics
    return Random() % NUM_BATTLE_TENT_TRAINERS;
  else if (facility ==
           FRONTIER_FACILITY_FACTORY) // Slateport Tent; uses Factory mechanics
    return Random() % NUM_BATTLE_TENT_TRAINERS;
...
bool32 ValidateBattleTowerRecord(u8 recordId) // unused
{
  ...
  u32 hasData = 0;
  for (i = 0;
       i < offsetof(struct EmeraldBattleTowerRecord, checksum) / sizeof(u32);
       i++) {
    checksum += record[i];
    hasData |= record[i];
  }
  ...
  if (checksum == 0 && hasData == 0) {
    return FALSE;
  } else if (gSaveBlock2Ptr->frontier.towerRecords[recordId].checksum !=
             checksum) {
...
void TrySetLinkBattleTowerEnemyPartyLevel(void) {
  if (gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK)) {
    ...
    for (i = 0; i < PARTY_SIZE; i++) {
      ...
      if (species) {
        SetMonData(
            &gEnemyParty[i], MON_DATA_EXP,
            &gExperienceTables[gSpeciesInfo[species].growthRate][enemyLevel]);
