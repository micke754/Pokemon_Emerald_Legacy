...
int GetStringCenterAlignXOffset(int fontId, const u8 *str, int totalWidth) {
  return GetStringCenterAlignXOffsetWithLetterSpacing(fontId, str, totalWidth,
                                                      0);
}
...
int GetMaxWidthInSubsetOfMenuTable(const struct MenuAction *actions,
  ...
  for (maxWidth = 0, i = 0; i < numActions; i++) {
    int stringWidth =
        GetStringWidth(FONT_NORMAL, actions[actionIds[i]].text, 0);
    if (stringWidth > maxWidth)
      maxWidth = stringWidth;
  }
...
// It's impossible to distinguish between Latin languages just from a string
// alone, so the function defaults to LANGUAGE_ENGLISH. This is the case in all
...
// Used by Pokénav's Match Call to erase the previous trainer's flavor text when
// switching between their info pages.
void FillWindowTilesByRow(int windowId, int columnStart, int rowStart,
  ...
  windowTileData = window->tileData + (rowStart * windowRowSize) +
                   (columnStart * TILE_SIZE_4BPP);
  if (numRows > 0) {
    for (i = numRows; i != 0; i--) {
