...
static void InitMysteryEventScript(struct ScriptContext *ctx, u8 *script) {
  InitScriptContext(ctx, gMysteryEventScriptCmdTable,
...
static bool32 IsRecordMixingGiftValid(void) {
  ...
  if (data->unk0 == 0 || data->quantity == 0 || data->itemId == 0 ||
      checksum == 0 || checksum != gSaveBlock1Ptr->recordMixingGift.checksum)
    return FALSE;
  else
    return TRUE;
}
...
static void ClearRecordMixingGift(void) {
  CpuFill16(0, &gSaveBlock1Ptr->recordMixingGift,
            sizeof(gSaveBlock1Ptr->recordMixingGift));
}
...
u16 GetRecordMixingGift(void) {
  ...
  if (!IsRecordMixingGiftValid()) {
    ...
    if (data->quantity == 0)
      ...
      gSaveBlock1Ptr->recordMixingGift.checksum =
...
bool8 MEScrCmd_nop(struct ScriptContext *ctx) { return FALSE; }
...
bool8 MEScrCmd_setenigmaberry(struct ScriptContext *ctx) {
  ...
  StringCopyN(gStringVar1, gSaveBlock1Ptr->enigmaBerry.berry.name,
              BERRY_NAME_LENGTH + 1);
  SetEnigmaBerry(berry);
  StringCopyN(gStringVar2, gSaveBlock1Ptr->enigmaBerry.berry.name,
              BERRY_NAME_LENGTH + 1);
...
bool8 MEScrCmd_addtrainer(struct ScriptContext *ctx) {
  u32 data = ScriptReadWord(ctx) - ctx->mOffset + ctx->mScriptBase;
  memcpy(&gSaveBlock2Ptr->frontier.ereaderTrainer, (void *)data,
