...
static void InsertMonListItem(struct Pokenav_SearchResults *,
...
static const u32 sSearchMonDataIds[] = {MON_DATA_COOL, MON_DATA_BEAUTY,
                                        MON_DATA_CUTE, MON_DATA_SMART,
                                        MON_DATA_TOUGH};

static const LoopedTask sConditionSearchLoopedTaskFuncs[] = {
    BuildPartyMonSearchResults, InitBoxMonSearchResults,
    BuildBoxMonSearchResults, ConvertConditionsToListRanks};

static const u16 sConditionSearchResultFramePal[] =
    INCBIN_U16("graphics/pokenav/condition/search_results.gbapal");
static const u32 sConditionSearchResultTiles[] =
    INCBIN_U32("graphics/pokenav/condition/search_results.4bpp.lz");
static const u32 sConditionSearchResultTilemap[] =
    INCBIN_U32("graphics/pokenav/condition/search_results.bin.lz");
static const u16 sListBg_Pal[] =
    INCBIN_U16("graphics/pokenav/condition/search_results_list.gbapal");
...
bool32 PokenavCallback_Init_ConditionSearch(void) {
  struct Pokenav_SearchResults *menu =
      AllocSubstruct(POKENAV_SUBSTRUCT_CONDITION_SEARCH_RESULTS,
                     sizeof(struct Pokenav_SearchResults));
  if (menu == NULL)
    return FALSE;

  menu->monList =
      AllocSubstruct(POKENAV_SUBSTRUCT_MON_LIST, sizeof(struct PokenavMonList));
...
u32 GetConditionSearchResultsCallback(void) {
  struct Pokenav_SearchResults *menu =
      GetSubstructPtr(POKENAV_SUBSTRUCT_CONDITION_SEARCH_RESULTS);
...
static bool32
HandleConditionSearchInput_WaitSetup(struct Pokenav_SearchResults *menu) {
...
static u32
OpenConditionGraphFromSearchList(struct Pokenav_SearchResults *menu) {
...
// Data below is initially set by BuildPartyMonSearchResults /
// BuildBoxMonSearchResults, and is the Pokémon's condition value for the
// condition they are sorted by. The condition value in data is then overwritten
// with their ranking.
...
static void InsertMonListItem(struct Pokenav_SearchResults *menu,
                              struct PokenavMonListItem *item) {
  u32 left = 0;
...
bool32 OpenConditionSearchResults(void) {
  struct Pokenav_SearchResultsGfx *gfx =
      AllocSubstruct(POKENAV_SUBSTRUCT_CONDITION_SEARCH_RESULTS_GFX,
                     sizeof(struct Pokenav_SearchResultsGfx));
  if (gfx == NULL)
    return FALSE;
  gfx->loopedTaskId =
      CreateLoopedTask(LoopedTask_OpenConditionSearchResults, 1);
...
static u32 LoopedTask_OpenConditionSearchResults(s32 state) {
  ...
  switch (state) {
  case 0:
    InitBgTemplates(sConditionSearchResultBgTemplates,
                    ARRAY_COUNT(sConditionSearchResultBgTemplates));
    ...
    CopyPaletteIntoBufferUnfaded(sConditionSearchResultFramePal, BG_PLTT_ID(1),
                                 sizeof(sConditionSearchResultFramePal));
    CopyBgTilemapBufferToVram(1);
  ...
  case 2:
    if (FreeTempTileDataBuffersIfPossible())
      return LT_PAUSE;
    CopyPaletteIntoBufferUnfaded(sListBg_Pal, BG_PLTT_ID(2),
                                 sizeof(sListBg_Pal));
  ...
  case 4:
    ...
    if (!gfx->fromGraph) {
      u8 searchGfxId =
          GetSelectedConditionSearch() + POKENAV_MENUITEM_CONDITION_SEARCH_COOL;
...
static void
AddSearchResultListMenuWindow(struct Pokenav_SearchResultsGfx *gfx) {
...
static void
PrintSearchResultListMenuItems(struct Pokenav_SearchResultsGfx *gfx) {
  ...
  *gStringVar1 = EOS;
  ...
  AddTextPrinterParameterized(gfx->winid, FONT_NORMAL, gStringVar2, 4, 1,
                              TEXT_SKIP_DRAW, NULL);
  ConvertIntToDecimalStringN(gStringVar1, rank, STR_CONV_MODE_RIGHT_ALIGN, 3);
  AddTextPrinterParameterized(gfx->winid, FONT_NORMAL, gStringVar1, 34, 1,
                              TEXT_SKIP_DRAW, NULL);
