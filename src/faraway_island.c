...
static u8 GetMewObjectEventId(void) {
  ...
  TryGetObjectEventIdByLocalIdAndMap(
      LOCALID_FARAWAY_ISLAND_MEW, gSaveBlock1Ptr->location.mapNum,
      gSaveBlock1Ptr->location.mapGroup, &objectEventId);
  return objectEventId;
}

// When the player enters Faraway Island interior it begins a "hide and seek"
// minigame where Mew disappears into the grass This function returns the
// direction Mew will take a step, and is run every time the player takes a step
u32 GetMewMoveDirection(void) {
  ...
  // Below loop is for Mew to try to avoid getting trapped between the player
  // and a rock
      ...
      if (!mewSafeFromTrap) {
        if (sPlayerToMewDeltaX > 0) {
          if (mew->currentCoords.x + 1 ==
              gObjectEvents[gPlayerAvatar.objectEventId].previousCoords.x) {
            if (CanMewMoveToCoords(mew->currentCoords.x + 1,
                                   mew->currentCoords.y))
              return DIR_EAST;
          }
        ...
        if (mew->currentCoords.x ==
          ...
          if (sPlayerToMewDeltaY > 0) {
            if (CanMewMoveToCoords(mew->currentCoords.x,
                                   mew->currentCoords.y - 1))
              return DIR_NORTH;
          } else {
            if (CanMewMoveToCoords(mew->currentCoords.x,
                                   mew->currentCoords.y + 1))
  ...
  // If this point is reached, Mew cannot move without getting closer to the
  // player

  // Avoid player on same Y, try move North/South
  if (sPlayerToMewDeltaY == 0) {
    if (gObjectEvents[gPlayerAvatar.objectEventId].currentCoords.y >
        mew->currentCoords.y) {
  ...
  if (sPlayerToMewDeltaX == 0) {
    if (gObjectEvents[gPlayerAvatar.objectEventId].currentCoords.x >
        mew->currentCoords.x) {
...
static u8 GetValidMewMoveDirection(u8 ignoredDir) {
  ...
  if (count > 1)
    return sMewDirectionCandidates[VarGet(VAR_FARAWAY_ISLAND_STEP_COUNTER) %
                                   count];
  else
...
void SetMewAboveGrass(void) {
  ...
  if (gSpecialVar_0x8004 == 1) {
    // For after battle where Mew should still be present (e.g. if ran from
    // battle)
    mew->fixedPriority = 1;
    ...
    sGrassSpriteId = CreateSpriteAtEnd(
        gFieldEffectObjectTemplatePointers[FLDEFFOBJ_LONG_GRASS], x, y,
        gSprites[mew->spriteId].subpriority - 1);
