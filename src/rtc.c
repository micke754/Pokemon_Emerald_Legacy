...
static const s32 sNumDaysInMonths[MONTH_COUNT] = {
    [MONTH_JAN - 1] = 31, [MONTH_FEB - 1] = 28, [MONTH_MAR - 1] = 31,
    [MONTH_APR - 1] = 30, [MONTH_MAY - 1] = 31, [MONTH_JUN - 1] = 30,
    [MONTH_JUL - 1] = 31, [MONTH_AUG - 1] = 31, [MONTH_SEP - 1] = 30,
    [MONTH_OCT - 1] = 31, [MONTH_NOV - 1] = 30, [MONTH_DEC - 1] = 31,
};
...
void RtcRestoreInterrupts(void) { REG_IME = sSavedIme; }
...
u16 RtcGetErrorStatus(void) { return sErrorStatus; }
...
void FormatDecimalTime(u8 *dest, s32 hour, s32 minute, s32 second) {
  ...
  *dest++ = CHAR_COLON;
  dest =
      ConvertIntToDecimalStringN(dest, minute, STR_CONV_MODE_LEADING_ZEROS, 2);
  *dest++ = CHAR_COLON;
  dest =
      ConvertIntToDecimalStringN(dest, second, STR_CONV_MODE_LEADING_ZEROS, 2);
  *dest = EOS;
}
...
void FormatDecimalDate(u8 *dest, s32 year, s32 month, s32 day) {
  ...
  *dest++ = CHAR_HYPHEN;
  dest =
      ConvertIntToDecimalStringN(dest, month, STR_CONV_MODE_LEADING_ZEROS, 2);
  *dest++ = CHAR_HYPHEN;
...
void RtcCalcTimeDifference(struct SiiRtcInfo *rtc, struct Time *result,
                           struct Time *t) {
...
u32 RtcGetMinuteCount(void) {
  ...
  return (HOURS_PER_DAY * MINUTES_PER_HOUR) * RtcGetDayCount(&sRtc) +
         MINUTES_PER_HOUR * sRtc.hour + sRtc.minute;
}

u32 RtcGetLocalDayCount(void) { return RtcGetDayCount(&sRtc); }
