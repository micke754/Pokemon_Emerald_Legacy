 ...
 * pokdex_area_screen.c/pokedex_area_region_map.c For the fly map, and utility
 * functions all of the maps use, see region_map.c
 */
...
static void MCB2_InitRegionMapRegisters(void) {
  ...
  InitBgsFromTemplates(1, sFieldRegionMapBgTemplates,
                       ARRAY_COUNT(sFieldRegionMapBgTemplates));
...
static void FieldUpdateRegionMap(void) {
  ...
  case 1:
    ...
    AddTextPrinterParameterized(WIN_TITLE, FONT_NORMAL, gText_Hoenn, offset, 1,
                                0, NULL);
...
static void PrintRegionMapSecName(void) {
  if (sFieldRegionMapHandler->regionMap.mapSecType != MAPSECTYPE_NONE) {
    ...
    AddTextPrinterParameterized(WIN_MAPSEC_NAME, FONT_NORMAL,
                                sFieldRegionMapHandler->regionMap.mapSecName, 0,
                                1, 0, NULL);
