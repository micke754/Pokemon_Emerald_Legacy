...
const union AffineAnimCmd *const gAffineAnims_Bite[] = {
    sAffineAnim_Bite_0, sAffineAnim_Bite_1, sAffineAnim_Bite_2,
    sAffineAnim_Bite_3, sAffineAnim_Bite_4, sAffineAnim_Bite_5,
    sAffineAnim_Bite_6, sAffineAnim_Bite_7,
};
...
static const union AnimCmd sAnim_ClawSlash_1[] = {
    ...
    ANIMCMD_FRAME(32, 4, .hFlip = TRUE), ANIMCMD_FRAME(48, 4, .hFlip = TRUE),
    ANIMCMD_FRAME(64, 4, .hFlip = TRUE), ANIMCMD_END,
};
...
static void AnimTearDrop(struct Sprite *sprite) {
  ...
  case 1:
    sprite->x =
        GetBattlerSpriteCoordAttr(battler, BATTLER_COORD_ATTR_RIGHT) - 14;
  ...
  case 3:
    sprite->x =
        GetBattlerSpriteCoordAttr(battler, BATTLER_COORD_ATTR_LEFT) + 14;
    sprite->y = GetBattlerSpriteCoordAttr(battler, BATTLER_COORD_ATTR_TOP) + 16;
    StartSpriteAffineAnim(sprite, 1);
    xOffset = -20;
    break;
...
void AnimTask_MoveAttackerMementoShadow(u8 taskId) {
  ...
  task->data[7] =
      GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_Y) + 31;
  task->data[6] =
  ...
  case 0:
    if (++task->data[1] > 1) {
      ...
      SetGpuReg(REG_OFFSET_BLDALPHA,
                BLDALPHA_BLEND(task->data[11], task->data[12]));
...
void AnimTask_MoveTargetMementoShadow(u8 taskId) {
  ...
  case 0:
    if (IsContest() == TRUE) {
      ...
      SetGpuReg(REG_OFFSET_WININ, WININ_WIN0_BG_ALL | WININ_WIN0_OBJ |
                                      WININ_WIN0_CLR | WININ_WIN1_BG_ALL |
                                      WININ_WIN1_OBJ | WININ_WIN1_CLR);
      SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WINOBJ_BG_ALL | WINOUT_WINOBJ_OBJ |
                                       WINOUT_WINOBJ_CLR | WINOUT_WIN01_BG_ALL |
  ...
  case 2:
    task->data[7] =
        GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y) + 31;
    task->data[6] =
  ...
  case 4:
    if (task->data[3] == 1)
      SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WINOBJ_BG_ALL | WINOUT_WINOBJ_OBJ |
                                       WINOUT_WINOBJ_CLR | WINOUT_WIN01_BG0 |
                                       WINOUT_WIN01_BG2 | WINOUT_WIN01_BG3 |
                                       WINOUT_WIN01_OBJ | WINOUT_WIN01_CLR);
    else
      SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WINOBJ_BG_ALL | WINOUT_WINOBJ_OBJ |
                                       WINOUT_WINOBJ_CLR | WINOUT_WIN01_BG0 |
                                       WINOUT_WIN01_BG1 | WINOUT_WIN01_BG3 |
                                       WINOUT_WIN01_OBJ | WINOUT_WIN01_CLR);
...
void AnimTask_InitMementoShadow(u8 taskId) {
  ...
  if (IsBattlerSpriteVisible(BATTLE_PARTNER(gBattleAnimAttacker))) {
    ...
    gSprites[gBattlerSpriteIds[BATTLE_PARTNER(gBattleAnimAttacker)]].invisible =
        FALSE;
  }
...
void AnimTask_MementoHandleBg(u8 taskId) {
  bool8 toBG2 =
      GetBattlerSpriteBGPriorityRank(gBattleAnimAttacker) ^ 1 ? TRUE : FALSE;
...
// Animates a deep slash from a claw. Used by Metal Claw, Dragon Claw, and Crush
// Claw
...
// arg2: custom color
// Custom color argument is used in MOVE_POISON_TAIL to make the mon turn
// purplish/pinkish as if became cloaked in poison.
void AnimTask_MetallicShine(u8 taskId) {
  ...
  LoadCompressedPalette(gMetalShinePalette, BG_PLTT_ID(animBg.paletteId),
                        PLTT_SIZE_4BPP);
...
void AnimTask_SetGrayscaleOrOriginalPal(u8 taskId) {
  ...
  if (spriteId != SPRITE_NONE)
    SetGrayscaleOrOriginalPalette(gSprites[spriteId].oam.paletteNum + 16,
                                  gBattleAnimArgs[1]);
