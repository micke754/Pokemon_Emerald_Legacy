...
static const union AnimCmd *const sAnims_HandsAndFeet[] = {
    sAnim_Fist, sAnim_FootWide, sAnim_FootTall, sAnim_HandLeft, sAnim_HandRight,
};
...
static void AnimFistOrFootRandomPos(struct Sprite *sprite) {
  ...
  sprite->data[7] = CreateSprite(&gBasicHitSplatSpriteTemplate, sprite->x,
                                 sprite->y, sprite->subpriority + 1);
...
// reappears at full size. Used by moves such as MOVE_MEGA_PUNCH and
// MOVE_MEGA_KICK. arg 0: initial x pixel offset arg 1: initial y pixel offset
// arg 2: anim num
// arg 3: spin duration
...
// The wall that appears when Brick Break is going to shatter the target's
// defensive wall
...
// Piece of shattered defensive wall flies off. Used by Brick Break when the
// target has a defensive wall
...
static void AnimSuperpowerOrb(struct Sprite *sprite) {
  if (gBattleAnimArgs[0] == ANIM_ATTACKER) {
    ...
    sprite->y =
        GetBattlerSpriteCoord(gBattlerAttacker, BATTLER_COORD_Y_PIC_OFFSET);
...
static void AnimSuperpowerOrb_Step(struct Sprite *sprite) {
  if (++sprite->data[0] == 180) {
    ...
    sprite->data[3] = sprite->y;
    sprite->data[4] =
        GetBattlerSpriteCoord(sprite->data[7], BATTLER_COORD_Y_PIC_OFFSET);
...
static void AnimSuperpowerRock(struct Sprite *sprite) {
  ...
  sprite->data[0] = gBattleAnimArgs[3];
  StorePointerInVars(&sprite->data[4], &sprite->data[5],
                     (void *)(sprite->y << 8));
