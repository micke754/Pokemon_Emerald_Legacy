...
static const u16 sIntroDrops_Pal[] =
    INCBIN_U16("graphics/intro/scene_1/drops.gbapal");
static const u16 sIntroLogo_Pal[] =
    INCBIN_U16("graphics/intro/scene_1/logo.gbapal");
static const u32 sIntroDropsLogo_Gfx[] =
    INCBIN_U32("graphics/intro/scene_1/drops_logo.4bpp.lz");
static const u16 sIntro1Bg_Pal[] =
    INCBIN_U16("graphics/intro/scene_1/bg.gbapal"); // 16 x 16
static const u32 sIntro1Bg0_Tilemap[] =
    INCBIN_U32("graphics/intro/scene_1/bg0_map.bin.lz");
static const u32 sIntro1Bg1_Tilemap[] =
    INCBIN_U32("graphics/intro/scene_1/bg1_map.bin.lz");
static const u32 sIntro1Bg2_Tilemap[] =
    INCBIN_U32("graphics/intro/scene_1/bg2_map.bin.lz");
static const u32 sIntro1Bg3_Tilemap[] =
    INCBIN_U32("graphics/intro/scene_1/bg3_map.bin.lz");
static const u32 sIntro1Bg_Gfx[] =
    INCBIN_U32("graphics/intro/scene_1/bg.4bpp.lz");
static const u16 sIntroPokeball_Pal[] =
    INCBIN_U16("graphics/intro/scene_3/pokeball.gbapal");
static const u32 sIntroPokeball_Tilemap[] =
    INCBIN_U32("graphics/intro/scene_3/pokeball_map.bin.lz");
static const u32 sIntroPokeball_Gfx[] =
    INCBIN_U32("graphics/intro/scene_3/pokeball.8bpp.lz");
static const u16 sIntroStreaks_Pal[] =
    INCBIN_U16("graphics/intro/scene_3/streaks.gbapal"); // Unused
static const u32 sIntroStreaks_Gfx[] =
    INCBIN_U32("graphics/intro/scene_3/streaks.4bpp.lz"); // Unused
static const u32 sIntroStreaks_Tilemap[] =
    INCBIN_U32("graphics/intro/scene_3/streaks_map.bin.lz"); // Unused
static const u16 sIntroRayquzaOrb_Pal[] =
    INCBIN_U16("graphics/intro/scene_3/rayquaza_orb.gbapal");
static const u16 sIntroMisc_Pal[] =
    INCBIN_U16("graphics/intro/scene_3/misc.gbapal"); // Unused
static const u32 sIntroMisc_Gfx[] = INCBIN_U32(
    "graphics/intro/scene_3/misc.4bpp.lz"); // Rayquza orb, and misc unused gfx
static const u16 sIntroFlygonSilhouette_Pal[] =
    INCBIN_U16("graphics/intro/scene_1/flygon.gbapal");
static const u32 sIntroLati_Gfx[] =
    INCBIN_U32("graphics/intro/scene_1/lati.4bpp.lz"); // Unused
static const u8 sUnusedData[] = {
    0x02, 0x03, 0x04, 0x05, 0x01, 0x01, 0x01, 0x06, 0x07, 0x08, 0x09, 0x0A,
...
static const u8 sSparkleCoords[][2] = {
    ...
    {93, 40},  {148, 32}, {173, 41}, {94, 20}, {208, 38}, {},
};
...
static const union AnimCmd sAnim_Torchic_Walk[] = {
    ...
    ANIMCMD_FRAME(16, 5), ANIMCMD_JUMP(0),
};
...
static const union AnimCmd sAnim_Manectric[] = {
    ANIMCMD_FRAME(0, 4),   ANIMCMD_FRAME(64, 4), ANIMCMD_FRAME(128, 4),
    ANIMCMD_FRAME(192, 4), ANIMCMD_JUMP(0),
};
...
// Can be produced in two different sets depending on the function called to
// create the sprites
...
static const union AnimCmd sAnim_PlayerBicycle_Slow[] = {
    ANIMCMD_FRAME(0, 8),   ANIMCMD_FRAME(64, 8), ANIMCMD_FRAME(128, 8),
...
static const s16 sGameFreakLetterData[NUM_GF_LETTERS][2] = {
    // Letter,   x offset
    {GAMEFREAK_G, -72}, {GAMEFREAK_A, -56}, {GAMEFREAK_M, -40},
    {GAMEFREAK_E, -24}, {GAMEFREAK_F, 8},   {GAMEFREAK_R, 24},
    {GAMEFREAK_E, 40},  {GAMEFREAK_A, 56},  {GAMEFREAK_K, 72},
};
...
static const union AffineAnimCmd *const sAffineAnims_GameFreak[] = {
    ...
    sAffineAnim_GameFreak_GrowAndShrink, // For letters appearing. Logo does
                                         // this too, but while it's invisible
...
static void LoadCopyrightGraphics(u16 tilesetAddress, u16 tilemapAddress,
                                  u16 paletteOffset) {
  ...
  case 0:
    ...
    *(u16 *)PLTT = RGB_WHITE;
    ...
    SetGpuReg(REG_OFFSET_BG0CNT, BGCNT_PRIORITY(0) | BGCNT_CHARBASE(0) |
                                     BGCNT_SCREENBASE(7) | BGCNT_16COLOR |
                                     BGCNT_TXT256x256);
  ...
  case 141:
    ...
    if (gMultibootProgramStruct.gcmb_field_2 != 0) {
      if (gMultibootProgramStruct.gcmb_field_2 == 2) {
        // check the multiboot ROM header game code to see if we already did
        // this
...
static void Task_Scene1_Load(u8 taskId) {
  ...
  sIntroCharacterGender =
      gSaveBlock2Ptr
          ->playerGender; // Loads the gender in the save file to show in into.
                          // If no save file, defaults to male
  IntroResetGpuRegs();
  ...
  SetGpuReg(REG_OFFSET_BG3CNT, BGCNT_PRIORITY(3) | BGCNT_CHARBASE(0) |
                                   BGCNT_SCREENBASE(22) | BGCNT_16COLOR |
                                   BGCNT_TXT256x512);
  SetGpuReg(REG_OFFSET_BG2CNT, BGCNT_PRIORITY(2) | BGCNT_CHARBASE(0) |
                                   BGCNT_SCREENBASE(20) | BGCNT_16COLOR |
                                   BGCNT_TXT256x512);
  SetGpuReg(REG_OFFSET_BG1CNT, BGCNT_PRIORITY(1) | BGCNT_CHARBASE(0) |
                                   BGCNT_SCREENBASE(18) | BGCNT_16COLOR |
                                   BGCNT_TXT256x512);
  SetGpuReg(REG_OFFSET_BG0CNT, BGCNT_PRIORITY(0) | BGCNT_CHARBASE(0) |
                                   BGCNT_SCREENBASE(16) | BGCNT_16COLOR |
                                   BGCNT_TXT256x512);
  ...
  CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(0)],
            &gPlttBufferUnfaded[OBJ_PLTT_ID(15) + 0], PLTT_SIZEOF(16 - 0));
  CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(0)],
            &gPlttBufferUnfaded[OBJ_PLTT_ID(14) + 1],
            PLTT_SIZEOF(16 - 1) + 1); // Copying an extra half color?
  CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(0)],
            &gPlttBufferUnfaded[OBJ_PLTT_ID(13) + 2], PLTT_SIZEOF(16 - 2));
  CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(0)],
            &gPlttBufferUnfaded[OBJ_PLTT_ID(12) + 3], PLTT_SIZEOF(16 - 3));
  CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(0)],
            &gPlttBufferUnfaded[OBJ_PLTT_ID(11) + 4], PLTT_SIZEOF(16 - 4));
  CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(0)],
            &gPlttBufferUnfaded[OBJ_PLTT_ID(10) + 5], PLTT_SIZEOF(16 - 5));
  CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(0)],
            &gPlttBufferUnfaded[OBJ_PLTT_ID(9) + 6], PLTT_SIZEOF(16 - 6));
  CreateGameFreakLogoSprites(DISPLAY_WIDTH / 2, DISPLAY_HEIGHT / 2, 0);
  gTasks[taskId].sBigDropSpriteId =
      CreateWaterDrop(236, -14, 0x200, 1, 0x78, FALSE);
...
static void Task_Scene1_FadeIn(u8 taskId) {
  ...
  SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_0 | DISPCNT_OBJ_1D_MAP |
                                    DISPCNT_BG_ALL_ON | DISPCNT_OBJ_ON);
...
static void Task_CreateSparkles(u8 taskId) {
  ...
  switch (tState) {
  case 0:
    CreateSprite(&sSpriteTemplate_Sparkle, sSparkleCoords[tNumSparkles][0],
                 sSparkleCoords[tNumSparkles][1] + tTimerSteps, 0);
    tState++;
    tDelay = 12;
    tNumSparkles++;
    break;
...
static void Task_Scene1_PanUp(u8 taskId) {
  if (gIntroFrameCounter < TIMER_END_PAN_UP) {
    ...
    if (gIntroFrameCounter == TIMER_FLYGON_SILHOUETTE_APPEAR) {
      ...
      u8 spriteId = CreateSprite(&sSpriteTemplate_FlygonSilhouette, 120,
                                 DISPLAY_HEIGHT, 10);
...
static void Task_Scene2_CreateSprites(u8 taskId) {
  ...
  for (spriteId = 0; spriteId < ARRAY_COUNT(sSpriteSheet_RunningPokemon) - 1;
       spriteId++)
  ...
  gTasks[taskId].tBgAnimTaskId =
      CreateBicycleBgAnimationTask(1, 0x4000, 0x400, 0x10);
...
static void Task_Scene2_BikeRide(u8 taskId) {
  ...
  // These states are for SpriteCB_PlayerOnBicycle and SpriteCB_Flygon
  // respectively
...
static void Task_Scene3_Load(u8 taskId) {
  ...
  FreeAllSpritePalettes();
  BeginNormalPaletteFade(PALETTES_ALL, 0, 16, 0, RGB_WHITEALPHA);
  SetGpuReg(REG_OFFSET_BG2CNT, BGCNT_PRIORITY(3) | BGCNT_CHARBASE(0) |
                                   BGCNT_SCREENBASE(8) | BGCNT_256COLOR |
                                   BGCNT_AFF256x256);
...
static void Task_Scene3_SpinPokeball(u8 taskId) {
  ...
  PanFadeAndZoomScreen(DISPLAY_WIDTH / 2, DISPLAY_HEIGHT / 2,
                       SAFE_DIV(0x10000, gTasks[taskId].tZoomDiv),
                       gTasks[taskId].tAlpha);
...
static void Task_Scene3_InitGroudonBg(u8 taskId) {
  ...
  SetGpuReg(REG_OFFSET_BG2CNT, BGCNT_PRIORITY(0) | BGCNT_CHARBASE(0) |
                                   BGCNT_SCREENBASE(24) | BGCNT_256COLOR |
                                   BGCNT_WRAP | BGCNT_AFF512x512);
  SetGpuReg(REG_OFFSET_BG1CNT, BGCNT_PRIORITY(1) | BGCNT_CHARBASE(1) |
                                   BGCNT_SCREENBASE(28) | BGCNT_16COLOR |
                                   BGCNT_TXT256x256);
  SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_1 | DISPCNT_OBJ_1D_MAP |
  ...
  PanFadeAndZoomScreen(gTasks[taskId].tScreenX, gTasks[taskId].tScreenY,
                       gTasks[taskId].tZoom, 0);
...
static void Task_Scene3_NarrowWindow(u8 taskId) {
  if (gTasks[taskId].tWinPos != NARROW_HEIGHT) {
    ...
    SetGpuReg(REG_OFFSET_WIN0V, (gTasks[taskId].tWinPos * 256) -
                                    (gTasks[taskId].tWinPos - DISPLAY_HEIGHT));
  } else {
    SetGpuReg(REG_OFFSET_WIN0V,
              WIN_RANGE(NARROW_HEIGHT, DISPLAY_HEIGHT - NARROW_HEIGHT));
...
static void Task_Scene3_StartGroudon(u8 taskId) {
  ...
  ScanlineEffect_InitWave(0, DISPLAY_HEIGHT, 4, 4, 1,
                          SCANLINE_EFFECT_REG_BG1HOFS, FALSE);
}
...
static void Task_Scene3_Groudon(u8 taskId) {
  ...
  case 1:
    if (--tDelay == 0) {
      tDelay = 2;
      CpuCopy16(&gIntro3Bg_Pal[tPalIdx], &gPlttBufferFaded[BG_PLTT_ID(1) + 15],
                PLTT_SIZEOF(1));
      tPalIdx += 2;
      if (tPalIdx == 0x1EC)
        tState++;
  ...
  case 5:
    if (--tDelay == 0) {
      ...
      tScreenY = 41;
      tDelay = 16;
      PlayCryInternal(SPECIES_GROUDON, 0, 100, CRY_PRIORITY_NORMAL,
                      CRY_MODE_NORMAL);
      tState++;
    }
    break;
...
static void CreateGroudonRockSprites(u8 taskId) {
  ...
  for (i = 0; i < (int)ARRAY_COUNT(sGroudonRockData); i++) {
    spriteId = CreateSprite(gAncientPowerRockSpriteTemplate,
                            sGroudonRockData[i][0], DISPLAY_HEIGHT, i);
...
static void Task_Scene3_LoadKyogre(u8 taskId) {
  ...
  PanFadeAndZoomScreen(gTasks[taskId].tScreenX, gTasks[taskId].tScreenY,
                       gTasks[taskId].tZoom, 0);
  ScanlineEffect_InitWave(0, DISPLAY_HEIGHT, 4, 4, 1,
                          SCANLINE_EFFECT_REG_BG1VOFS, FALSE);
}
...
static void Task_Scene3_Kyogre(u8 taskId) {
  ...
  case 5:
    if (--tDelay == 0) {
      ...
      if (tPalIdx != 0) {
        ...
        tDelay = 1;
        tState++;
        PlayCryInternal(SPECIES_KYOGRE, 0, 120, CRY_PRIORITY_NORMAL,
                        CRY_MODE_NORMAL);
      }
    }
    break;
  ...
  case 7:
    if (--tDelay == 0) {
      tDelay = 4;
      CpuCopy16(&gIntro3Bg_Pal[tPalIdx], &gPlttBufferFaded[BG_PLTT_ID(2) + 15],
                PLTT_SIZEOF(1));
      tPalIdx -= 2;
      if (tPalIdx == 0x1E0)
        tState++;
...
static void CreateKyogreBubbleSprites_Body(u8 taskId) {
  ...
  for (i = 0; i < NUM_BUBBLES_IN_SET; i++) {
    spriteId = CreateSprite(&sSpriteTemplate_Bubbles, sKyogreBubbleData[i][0],
                            sKyogreBubbleData[i][1], i);
    gSprites[spriteId].invisible = TRUE;
    gSprites[spriteId].sTaskId = taskId;
...
static void CreateKyogreBubbleSprites_Fins(void) {
  ...
  for (i = 0; i < NUM_BUBBLES_IN_SET; i++) {
    spriteId = CreateSprite(&sSpriteTemplate_Bubbles,
                            ...
                            sKyogreBubbleData[i + NUM_BUBBLES_IN_SET][1], i);
    gSprites[spriteId].invisible = TRUE;
#ifdef BUGFIX
    gSprites[spriteId].sDelay = sKyogreBubbleData[i + NUM_BUBBLES_IN_SET][2];
#else
    gSprites[spriteId].sDelay =
        sKyogreBubbleData[i][2]; // Using the wrong set of delays here
#endif
    gSprites[spriteId].sUnk = 64;
  }
}
...
static void Task_Scene3_LoadClouds1(u8 taskId) {
  SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_TGT1_BG0 | BLDCNT_TGT1_BG1 |
                                   BLDCNT_TGT1_BG2 | BLDCNT_EFFECT_LIGHTEN);
  SetGpuReg(REG_OFFSET_BLDALPHA, BLDALPHA_BLEND(31, 31));
  SetGpuReg(REG_OFFSET_BLDY, 31);
  SetGpuReg(REG_OFFSET_BG0CNT, BGCNT_PRIORITY(0) | BGCNT_CHARBASE(0) |
                                   BGCNT_SCREENBASE(24) | BGCNT_16COLOR |
                                   BGCNT_TXT512x256);
  SetGpuReg(REG_OFFSET_BG1CNT, BGCNT_PRIORITY(0) | BGCNT_CHARBASE(1) |
                                   BGCNT_SCREENBASE(26) | BGCNT_16COLOR |
                                   BGCNT_TXT512x256);
...
static void SpriteCB_Lightning(struct Sprite *sprite) {
  ...
  case 0:
    ...
    sprite->sState++;
  case 1:
    CpuCopy16(&gIntro3Bg_Pal[sprite->sPalIdx],
              &gPlttBufferFaded[BG_PLTT_ID(5) + 13], PLTT_SIZEOF(1));
    sprite->sPalIdx += 2;
...
static void Task_RayquazaAttack(u8 taskId) {
  ...
  case 0:
    if ((data[2] & 1) != 0) {
      CpuCopy16(&gIntro3Bg_Pal[0x1A2 + data[1] * 2],
                &gPlttBufferFaded[BG_PLTT_ID(5) + 14], PLTT_SIZEOF(1));
      data[1]++;
    }
    if (data[1] == 6) {
      tState++;
  ...
  case 1:
    if (data[3] == 0) {
      if ((data[2] & 1) != 0) {
        CpuCopy16(&gIntro3Bg_Pal[0x1A2 + data[1] * 2],
                  &gPlttBufferFaded[BG_PLTT_ID(5) + 8], PLTT_SIZEOF(1));
        data[1]++;
      }
      if (data[1] == 6) {
        tState++;
  ...
  case 2:
    if (data[3] == 0) {
      if ((data[2] & 1) != 0) {
        CpuCopy16(&gIntro3Bg_Pal[0x182 + data[1] * 2],
  ...
  case 3:
    if ((data[2] & 1) != 0) {
      if (--data[3] != 0) {
        ...
        CpuCopy16(&gIntro3Bg_Pal[428], &gPlttBufferFaded[BG_PLTT_ID(5) + 14],
                  PLTT_SIZEOF(1));
        CpuCopy16(&gIntro3Bg_Pal[428], &gPlttBufferFaded[BG_PLTT_ID(5) + 8],
                  PLTT_SIZEOF(1));
        CpuCopy16(&gIntro3Bg_Pal[396], &gPlttBufferFaded[BG_PLTT_ID(5) + 12],
                  PLTT_SIZEOF(1));
      } else {
        tState++;
        data[3] = 53;
...
static void Task_BlendLogoIn(u8 taskId) {
  ...
  case 0:
  default:
    SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_BG0 |
...
static void SpriteCB_WaterDropHalf(struct Sprite *sprite) {
  if (gSprites[sprite->data[7]].data[7] != 0) {
    ...
    CalcCenterToCornerVec(sprite, SPRITE_SHAPE(64x32), SPRITE_SIZE(64x32),
                          ST_OAM_AFFINE_ERASE);
  } else {
...
static void SpriteCB_WaterDrop_ReachLeafEnd(struct Sprite *sprite) {
  SetOamMatrix(sprite->data[1], sprite->data[6] + 64, 0, 0,
               sprite->data[6] + 64);
  SetOamMatrix(sprite->data[1] + 1, sprite->data[6] + 64, 0, 0,
               sprite->data[6] + 64);
  SetOamMatrix(sprite->data[1] + 2, sprite->data[6] + 64, 0, 0,
               sprite->data[6] + 64);
...
static u8 CreateWaterDrop(s16 x, s16 y, u16 c, u16 d, u16 e,
                          u8 fallImmediately) {
  u8 spriteId;
  u8 oldSpriteId;
  ...
  CalcCenterToCornerVec(&gSprites[spriteId], SPRITE_SHAPE(32x32),
                        SPRITE_SIZE(32x32), ST_OAM_AFFINE_ERASE);
  StartSpriteAnim(&gSprites[spriteId], DROP_ANIM_REFLECTION);
  if (!fallImmediately)
    gSprites[spriteId].callback =
        SpriteCB_WaterDrop; // Do full anim, for 1st drop that slides along the
                            // leaf
  else
    gSprites[spriteId].callback =
        SpriteCB_WaterDropShort; // Skip to drop falling into the water, for 2nd
                                 // and 3rd drops
...
static void SpriteCB_LogoLetter(struct Sprite *sprite) {
  ...
  case 2:
    ...
    if (sprite->sColorDelay == 0) {
      sprite->sColorDelay = 2;
      if (sprite->sTimer != 0) {
        CpuCopy16(&gIntroGameFreakTextFade_Pal[sprite->sTimer],
                  &gPlttBufferFaded[OBJ_PLTT_ID(1) + 15], PLTT_SIZEOF(1));
        CpuCopy16(&gIntroGameFreakTextFade_Pal[sprite->sTimer + 16],
                  &gPlttBufferFaded[OBJ_PLTT_ID(1) + 4], PLTT_SIZEOF(1));
        CpuCopy16(&gIntroGameFreakTextFade_Pal[sprite->sTimer + 32],
                  &gPlttBufferFaded[OBJ_PLTT_ID(1) + 10], PLTT_SIZEOF(1));
        sprite->sTimer--;
      } else {
        CpuCopy16(&gIntroGameFreakTextFade_Pal[sprite->sTimer],
                  &gPlttBufferFaded[OBJ_PLTT_ID(1) + 15], PLTT_SIZEOF(1));
...
static u8 CreateGameFreakLogoSprites(s16 x, s16 y, s16 unused) {
  ...
  for (i = 0; i < NUM_GF_LETTERS; i++) {
    spriteId = CreateSprite(&sSpriteTemplate_GameFreakLetter,
                            sGameFreakLetterData[i][1] + x, y - 4, 0);
...
static void SpriteCB_RayquazaOrb(struct Sprite *sprite) {
  ...
  default:
    ...
    CalcCenterToCornerVec(sprite, SPRITE_SHAPE(64x64), SPRITE_SIZE(64x64),
                          ST_OAM_AFFINE_DOUBLE);
    sprite->data[1] = 0;
    sprite->sState = 1;
    // fall through
