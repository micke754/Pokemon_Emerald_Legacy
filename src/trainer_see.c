...
static u8 CheckPathBetweenTrainerAndPlayer(struct ObjectEvent *trainerObj,
                                           u8 approachDistance, u8 direction);
...
static void SetIconSpriteData(struct Sprite *sprite, u16 fldEffId,
                              u8 spriteAnimNum);

static u8 GetTrainerApproachDistanceSouth(struct ObjectEvent *trainerObj,
                                          s16 range, s16 x, s16 y);
static u8 GetTrainerApproachDistanceNorth(struct ObjectEvent *trainerObj,
                                          s16 range, s16 x, s16 y);
static u8 GetTrainerApproachDistanceWest(struct ObjectEvent *trainerObj,
                                         s16 range, s16 x, s16 y);
static u8 GetTrainerApproachDistanceEast(struct ObjectEvent *trainerObj,
                                         s16 range, s16 x, s16 y);

static bool8 TrainerSeeIdle(u8 taskId, struct Task *task,
                            struct ObjectEvent *trainerObj);
static bool8 TrainerExclamationMark(u8 taskId, struct Task *task,
                                    struct ObjectEvent *trainerObj);
static bool8 WaitTrainerExclamationMark(u8 taskId, struct Task *task,
                                        struct ObjectEvent *trainerObj);
static bool8 TrainerMoveToPlayer(u8 taskId, struct Task *task,
                                 struct ObjectEvent *trainerObj);
static bool8 PlayerFaceApproachingTrainer(u8 taskId, struct Task *task,
                                          struct ObjectEvent *trainerObj);
static bool8 WaitPlayerFaceApproachingTrainer(u8 taskId, struct Task *task,
                                              struct ObjectEvent *trainerObj);
static bool8 RevealDisguisedTrainer(u8 taskId, struct Task *task,
                                    struct ObjectEvent *trainerObj);
static bool8 WaitRevealDisguisedTrainer(u8 taskId, struct Task *task,
                                        struct ObjectEvent *trainerObj);
static bool8 RevealBuriedTrainer(u8 taskId, struct Task *task,
                                 struct ObjectEvent *trainerObj);
static bool8 PopOutOfAshBuriedTrainer(u8 taskId, struct Task *task,
                                      struct ObjectEvent *trainerObj);
static bool8 JumpInPlaceBuriedTrainer(u8 taskId, struct Task *task,
                                      struct ObjectEvent *trainerObj);
static bool8 WaitRevealBuriedTrainer(u8 taskId, struct Task *task,
                                     struct ObjectEvent *trainerObj);
...
static bool8 (*const sTrainerSeeFuncList[])(u8 taskId, struct Task *task,
                                            struct ObjectEvent *trainerObj) = {
...
static bool8 (*const sTrainerSeeFuncList2[])(u8 taskId, struct Task *task,
                                             struct ObjectEvent *trainerObj) = {
    RevealBuriedTrainer,
...
static const struct SpriteFrameImage sSpriteImageTable_HeartIcon[] = {
    {.data = sEmotion_HeartGfx, .size = sizeof(sEmotion_HeartGfx)}};

static const union AnimCmd sSpriteAnim_Icons1[] = {ANIMCMD_FRAME(0, 60),
                                                   ANIMCMD_END};

static const union AnimCmd sSpriteAnim_Icons2[] = {ANIMCMD_FRAME(1, 60),
                                                   ANIMCMD_END};
...
bool8 CheckForTrainersWantingBattle(void) {
  ...
  for (i = 0; i < OBJECT_EVENTS_COUNT; i++) {
    ...
    if (gNoOfApproachingTrainers > 1)
      break;
    if (GetMonsStateToDoubles_2() !=
        PLAYER_HAS_TWO_USABLE_MONS) // one trainer found and cant have a double
                                    // battle
      break;
  }
...
static u8 CheckTrainer(u8 objectEventId) {
  ...
  if (approachDistance != 0) {
    ...
    gApproachingTrainers[gNoOfApproachingTrainers].objectEventId =
        objectEventId;
    gApproachingTrainers[gNoOfApproachingTrainers].trainerScriptPtr = scriptPtr;
    gApproachingTrainers[gNoOfApproachingTrainers].radius = approachDistance;
    InitTrainerApproachTask(&gObjectEvents[objectEventId],
                            approachDistance - 1);
...
static u8 GetTrainerApproachDistance(struct ObjectEvent *trainerObj) {
  ...
  if (trainerObj->trainerType ==
      TRAINER_TYPE_NORMAL) // can only see in one direction
  {
    approachDistance =
        sDirectionalApproachDistanceFuncs[trainerObj->facingDirection - 1](
            trainerObj, trainerObj->trainerRange_berryTreeId, x, y);
    return CheckPathBetweenTrainerAndPlayer(trainerObj, approachDistance,
                                            trainerObj->facingDirection);
  } else // TRAINER_TYPE_SEE_ALL_DIRECTIONS, TRAINER_TYPE_BURIED
  {
    for (i = 0; i < ARRAY_COUNT(sDirectionalApproachDistanceFuncs); i++) {
      ...
      if (CheckPathBetweenTrainerAndPlayer(
              trainerObj, approachDistance,
              i + 1)) // directions are 1-4 instead of 0-3. south north west
                      // east
...
// Returns how far south the player is from trainer. 0 if out of trainer's
// sight.
static u8 GetTrainerApproachDistanceSouth(struct ObjectEvent *trainerObj,
                                          s16 range, s16 x, s16 y) {
...
// Returns how far north the player is from trainer. 0 if out of trainer's
// sight.
static u8 GetTrainerApproachDistanceNorth(struct ObjectEvent *trainerObj,
                                          s16 range, s16 x, s16 y) {
  if (trainerObj->currentCoords.x == x && y < trainerObj->currentCoords.y &&
...
static bool8 TrainerExclamationMark(u8 taskId, struct Task *task,
  ...
  ObjectEventGetLocalIdAndMap(trainerObj, &gFieldEffectArguments[0],
...
static bool8 PlayerFaceApproachingTrainer(u8 taskId, struct Task *task,
  ...
  SetTrainerMovementType(trainerObj, GetTrainerFacingDirectionMovementType(
                                         trainerObj->facingDirection));
  ...
  ObjectEventSetHeldMovement(
      ...
      GetFaceDirectionMovementAction(
          GetOppositeDirection(trainerObj->facingDirection)));
...
static void Task_SetBuriedTrainerMovement(u8 taskId) {
  ...
  if (task->tFuncId == ((int)ARRAY_COUNT(sTrainerSeeFuncList2) - 1) &&
      !FieldEffectActiveListContains(FLDEFF_ASH_PUFF)) {
    SetTrainerMovementType(objEvent, GetTrainerFacingDirectionMovementType(
...
// Called when a buried Trainer has the reveal_trainer movement applied, from
// direct interaction
void SetBuriedTrainerMovement(struct ObjectEvent *objEvent) {
  StoreWordInTwoHalfwords(
      (u16 *)&gTasks[CreateTask(Task_SetBuriedTrainerMovement, 0)].tObjEvent,
      (u32)objEvent);
}

void DoTrainerApproach(void) { StartTrainerApproach(Task_EndTrainerApproach); }
...
u8 FldEff_QuestionMarkIcon(void) {
  u8 spriteId =
      CreateSpriteAtEnd(&sSpriteTemplate_ExclamationQuestionMark, 0, 0, 0x52);
...
static void SpriteCB_TrainerIcons(struct Sprite *sprite) {
  ...
  if (TryGetObjectEventIdByLocalIdAndMap(sprite->sLocalId, sprite->sMapNum,
                                         sprite->sMapGroup, &objEventId) ||
      sprite->animEnded) {
    FieldEffectStop(sprite, sprite->sFldEffId);
  } else {
    struct Sprite *objEventSprite =
        &gSprites[gObjectEvents[objEventId].spriteId];
...
void PlayerFaceTrainerAfterBattle(void) {
  ...
  if (gTrainerApproachedPlayer == TRUE) {
    objEvent =
        &gObjectEvents[gApproachingTrainers[gWhichTrainerToFaceAfterBattle]
                           .objectEventId];
    gPostBattleMovementScript[0] = GetFaceDirectionMovementAction(
    ...
    ScriptMovement_StartObjectMovementScript(
        OBJ_EVENT_ID_PLAYER, gSaveBlock1Ptr->location.mapNum,
    ...
    gPostBattleMovementScript[0] =
        GetFaceDirectionMovementAction(objEvent->facingDirection);
    gPostBattleMovementScript[1] = MOVEMENT_ACTION_STEP_END;
    ScriptMovement_StartObjectMovementScript(
        OBJ_EVENT_ID_PLAYER, gSaveBlock1Ptr->location.mapNum,
        gSaveBlock1Ptr->location.mapGroup, gPostBattleMovementScript);
