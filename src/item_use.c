...
// UB here if an item with type ITEM_USE_MAIL or ITEM_USE_BAG_MENU uses
// SetUpItemUseCallback Never occurs in vanilla, but can occur with improperly
// created items
...
static const u8 sClockwiseDirections[] = {DIR_NORTH, DIR_EAST, DIR_SOUTH,
                                          DIR_WEST};
...
static void DisplayCannotUseItemMessage(u8 taskId,
                                        bool8 isUsingRegisteredKeyItemOnField,
                                        const u8 *str) {
...
static void
DisplayDadsAdviceCannotUseItemMessage(u8 taskId,
                                      bool8 isUsingRegisteredKeyItemOnField) {
  DisplayCannotUseItemMessage(taskId, isUsingRegisteredKeyItemOnField,
                              gText_DadsAdvice);
}

static void
DisplayCannotDismountBikeMessage(u8 taskId,
                                 bool8 isUsingRegisteredKeyItemOnField) {
  DisplayCannotUseItemMessage(taskId, isUsingRegisteredKeyItemOnField,
...
// Mail in the bag menu can't have a message but it can be checked (view the
// mail background, no message)
...
static void ItemUseOnFieldCB_Itemfinder(u8 taskId) {
  if (ItemfinderCheckForHiddenItems(gMapHeader.events, taskId) == TRUE)
    ...
    DisplayItemMessageOnField(taskId, gText_ItemFinderNothing,
...
#define tCounter                                                               \
  data[3] // Used to count delay between beeps and rotations during player spin
...
static void Task_UseItemfinder(u8 taskId) {
  ...
  if (tCounter == 0) {
    if (tItemfinderBeeps == 4) {
      playerDirToItem =
          GetDirectionToHiddenItem(tItemDistanceX, tItemDistanceY);
...
static bool8 ItemfinderCheckForHiddenItems(const struct MapEvents *events,
                                           u8 taskId) {
  int itemX, itemY;
  ...
  for (i = 0; i < events->bgEventCount; i++) {
    // Check if there are any hidden items on the current map that haven't been
    // picked up
    if (events->bgEvents[i].kind == BG_EVENT_HIDDEN_ITEM &&
        !FlagGet(events->bgEvents[i].bgUnion.hiddenItem.hiddenItemId +
      ...
      if (distanceX >= -7 && distanceX <= 7 && distanceY >= -5 &&
          distanceY <= 5)
...
static bool8 IsHiddenItemPresentAtCoords(const struct MapEvents *events, s16 x,
                                         s16 y) {
  ...
  for (i = 0; i < bgEventCount; i++) {
    if (bgEvent[i].kind == BG_EVENT_HIDDEN_ITEM && x == (u16)bgEvent[i].x &&
        y == (u16)bgEvent[i]
                 .y) // hidden item and coordinates matches x and y passed?
    {
      if (!FlagGet(bgEvent[i].bgUnion.hiddenItem.hiddenItemId +
                   FLAG_HIDDEN_ITEMS_START))
        return TRUE;
      else
        return FALSE;
...
static bool8
IsHiddenItemPresentInConnection(const struct MapConnection *connection, int x,
                                int y) {
  ...
  struct MapHeader const *const mapHeader =
      GetMapHeaderFromConnection(connection);
...
static void SetDistanceOfClosestHiddenItem(u8 taskId, s16 itemDistanceX,
                                           s16 itemDistanceY) {
  s16 *data = gTasks[taskId].data;
  ...
  if (tItemFound == FALSE) {
    ...
    if (oldItemAbsX + oldItemAbsY > newItemAbsX + newItemAbsY) {
      ...
      if (oldItemAbsX + oldItemAbsY == newItemAbsX + newItemAbsY &&
        ...
        // If items are equal distance, use whichever is closer on the Y axis or
        // further south
        tItemDistanceX = itemDistanceX;
        tItemDistanceY = itemDistanceY;
      }
...
static void PlayerFaceHiddenItem(u8 direction) {
  ObjectEventClearHeldMovementIfFinished(
      &gObjectEvents[GetObjectEventIdByLocalIdAndMap(OBJ_EVENT_ID_PLAYER, 0,
                                                     0)]);
  ObjectEventClearHeldMovement(&gObjectEvents[GetObjectEventIdByLocalIdAndMap(
      OBJ_EVENT_ID_PLAYER, 0, 0)]);
  UnfreezeObjectEvent(&gObjectEvents[GetObjectEventIdByLocalIdAndMap(
      OBJ_EVENT_ID_PLAYER, 0, 0)]);
...
static void Task_HiddenItemNearby(u8 taskId) {
  if (ObjectEventCheckHeldMovementStatus(
          &gObjectEvents[GetObjectEventIdByLocalIdAndMap(OBJ_EVENT_ID_PLAYER, 0,
                                                         0)]) == TRUE)
    DisplayItemMessageOnField(taskId, gText_ItemFinderNearby,
                              Task_CloseItemfinderMessage);
}
...
static void Task_StandingOnHiddenItem(u8 taskId) {
  ...
  if (ObjectEventCheckHeldMovementStatus(
    ...
    if (tCounter == 4)
      DisplayItemMessageOnField(taskId, gText_ItemFinderOnTop,
                                Task_CloseItemfinderMessage);
  }
}
...
void ItemUseOutOfBattle_CoinCase(u8 taskId) {
  ConvertIntToDecimalStringN(gStringVar1, GetCoins(), STR_CONV_MODE_LEFT_ALIGN,
                             4);
...
static bool8 TryToWaterSudowoodo(void) {
  ...
  if (objId == OBJECT_EVENTS_COUNT ||
      gObjectEvents[objId].graphicsId != OBJ_EVENT_GFX_SUDOWOODO)
    return FALSE;
  else
    return TRUE;
}
...
void ItemUseOutOfBattle_TMHM(u8 taskId) {
  if (gSpecialVar_ItemId >= ITEM_HM01)
    DisplayItemMessage(taskId, FONT_NORMAL, gText_BootedUpHM,
                       BootUpSoundTMHM); // HM
  else
    DisplayItemMessage(taskId, FONT_NORMAL, gText_BootedUpTM,
                       BootUpSoundTMHM); // TM
}
...
void ItemUseOutOfBattle_Repel(u8 taskId) {
  if (VarGet(VAR_REPEL_STEP_COUNT) == 0)
    ...
    DisplayItemMessage(taskId, FONT_NORMAL, gText_RepelEffectsLingered,
                       CloseItemMessage);
  else
    DisplayItemMessageInBattlePyramid(taskId, gText_RepelEffectsLingered,
                                      Task_CloseBattlePyramidBagMessage);
}
...
static void Task_UseStatIncreaseItem(u8 taskId) {
  if (++gTasks[taskId].data[8] > 7) {
    ...
    if (!InBattlePyramid())
      ...
      DisplayItemMessageInBattlePyramid(taskId,
                                        UseStatIncreaseItem(gSpecialVar_ItemId),
                                        Task_CloseStatIncreaseMessage);
  }
}
...
void ItemUseInBattle_StatIncrease(u8 taskId) {
  ...
  if (ExecuteTableBasedItemEffect(&gPlayerParty[partyId], gSpecialVar_ItemId,
                                  partyId, 0) != FALSE) {
    if (!InBattlePyramid())
      DisplayItemMessage(taskId, FONT_NORMAL, gText_WontHaveEffect,
                         CloseItemMessage);
    else
      DisplayItemMessageInBattlePyramid(taskId, gText_WontHaveEffect,
                                        Task_CloseBattlePyramidBagMessage);
  } else {
...
void ItemUseOutOfBattle_CannotUse(u8 taskId) {
  DisplayDadsAdviceCannotUseItemMessage(taskId,
