...
const u8 *GetPokedexRatingText(u16 count) {
  ...
  if (count == 208) {
    if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_JIRACHI),
        ...
        GetSetPokedexFlag(
            ...
            FLAG_GET_CAUGHT)) // Jirachi or Deoxys is not counted towards the
                              // dex completion. If either of these flags are
                              // enabled, it means the actual count is less than
                              // 200.
      return gBirchDexRatingText_LessThan200;
  ...
  if (count == HOENN_DEX_COUNT - 1) {
    if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_JIRACHI),
        ...
        GetSetPokedexFlag(
            SpeciesToNationalPokedexNum(SPECIES_DEOXYS),
            FLAG_GET_CAUGHT)) // If both of these flags are enabled, it means
                              // the actual count is less than 200.
      return gBirchDexRatingText_LessThan200;
    return gBirchDexRatingText_DexCompleted;
  }
