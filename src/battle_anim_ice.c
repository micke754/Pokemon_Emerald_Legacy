...
static bool8 GenerateHailParticle(u8 hailStructId, u8 affineAnimNum, u8 taskId,
                                  u8 c);
...
static const union AffineAnimCmd
    *const sAffineAnims_IceCrystalSpiralInwardLarge[] = {
...
static const union AnimCmd sAnim_IceGroundSpike[] = {
    ...
    ANIMCMD_FRAME(6, 5), ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(2, 5),
...
static const union AnimCmd sAnim_IceBallChunk_1[] = {
    ANIMCMD_FRAME(16, 4), ANIMCMD_FRAME(32, 4), ANIMCMD_FRAME(48, 4),
    ANIMCMD_FRAME(64, 4), ANIMCMD_END,
};
...
static const union AffineAnimCmd *const sAffineAnims_IceBallChunk[] = {
    sAffineAnim_IceBallChunk_0, sAffineAnim_IceBallChunk_1,
    sAffineAnim_IceBallChunk_2, sAffineAnim_IceBallChunk_3,
...
static void AnimUnusedIceCrystalThrow(struct Sprite *sprite) {
  ...
  for (; (targetX >= -32 && targetX <= DISPLAY_WIDTH + 32) &&
         (targetY >= -32 && targetY <= DISPLAY_HEIGHT + 32);
       targetX += sprite->data[1], targetY += sprite->data[2])
    ;

  sprite->data[1] = -sprite->data[1];
  sprite->data[2] = -sprite->data[2];
  for (; (attackerX >= -32 && attackerX <= DISPLAY_WIDTH + 32) &&
         (attackerY >= -32 && attackerY <= DISPLAY_HEIGHT + 32);
...
// Animates the small snowballs that swirl around the target in Blizzard and Icy
// Wind. arg 0: initial x pixel offset arg 1: initial y pixel offset arg 2:
// target x offset arg 3: target y offset arg 4: particle speed arg 5: multiple
// targets? (boolean)
...
static void AnimSwirlingSnowball_End(struct Sprite *sprite) {
  ...
  if (sprite->x + sprite->x2 > 256 || sprite->x + sprite->x2 < -16 ||
      sprite->y + sprite->y2 > 256 || sprite->y + sprite->y2 < -16)
    DestroyAnimSprite(sprite);
}
...
static void AnimWaveFromCenterOfTarget(struct Sprite *sprite) {
  if (sprite->data[0] == 0) {
    if (gBattleAnimArgs[2] == 0) {
      ...
      SetAverageBattlerPositions(gBattleAnimTarget, FALSE, &sprite->x,
                                 &sprite->y);
...
void AnimTask_HazeScrollingFog(u8 taskId) {
  ...
  LoadBgTiles(animBg.bgId, gWeatherFogHorizontalTiles, 0x800,
              animBg.tilesOffset);
  AnimLoadCompressedBgTilemapHandleContest(&animBg, gBattleAnimFogTilemap,
                                           FALSE);
...
static void InitPoisonGasCloudAnim(struct Sprite *sprite) {
  ...
  if (GET_BATTLER_SIDE2(gBattleAnimTarget) == B_SIDE_PLAYER) {
    ...
    if ((sprite->data[7] & 0x8000) &&
        GET_BATTLER_SIDE2(gBattleAnimAttacker) == B_SIDE_PLAYER)
      sprite->subpriority =
  ...
  if (unk != 2) {
    ...
    if (IsBattlerSpriteVisible(id)) {
      ...
      case 0:
        ...
        battlerY -=
            GetBattlerSpriteCoordAttr(id, BATTLER_COORD_ATTR_HEIGHT) / 6;
        break;
      case 1:
        battlerX += GetBattlerSpriteCoordAttr(id, BATTLER_COORD_ATTR_WIDTH) / 6;
        battlerY +=
...
static void AnimHailBegin(struct Sprite *sprite) {
  ...
  if (sprite->data[0] == 1 && sprite->data[5] == 0) {
    spriteId = CreateSprite(&gIceCrystalHitLargeSpriteTemplate, sprite->data[3],
                            sprite->data[4], sprite->subpriority);
...
static void InitIceBallAnim(struct Sprite *sprite) {
  u8 animNum = gAnimDisableStructPtr->rolloutTimerStartValue -
               gAnimDisableStructPtr->rolloutTimer - 1;
...
void AnimTask_GetIceBallCounter(u8 taskId) {
  ...
  gBattleAnimArgs[arg] = gAnimDisableStructPtr->rolloutTimerStartValue -
                         gAnimDisableStructPtr->rolloutTimer - 1;
  DestroyAnimVisualTask(taskId);
}
