...
static const u32 sRegionMapCityZoomTiles_Gfx[] =
...
u32 PokenavCallback_Init_RegionMap(void) {
  struct Pokenav_RegionMapMenu *state = AllocSubstruct(
...
u32 GetRegionMapCallback(void) {
  struct Pokenav_RegionMapMenu *state =
      GetSubstructPtr(POKENAV_SUBSTRUCT_REGION_MAP_STATE);
...
static u32
HandleRegionMapInputZoomDisabled(struct Pokenav_RegionMapMenu *state) {
  if (JOY_NEW(B_BUTTON)) {
    state->callback = GetExitRegionMapMenuId;
    return POKENAV_MAP_FUNC_EXIT;
  }
...
bool32 OpenPokenavRegionMap(void) {
  struct Pokenav_RegionMapGfx *state = AllocSubstruct(
      POKENAV_SUBSTRUCT_REGION_MAP_ZOOM, sizeof(struct Pokenav_RegionMapGfx));
  if (!state)
    return FALSE;
...
void CreateRegionMapLoopedTask(s32 index) {
  struct Pokenav_RegionMapGfx *state =
      GetSubstructPtr(POKENAV_SUBSTRUCT_REGION_MAP_ZOOM);
...
static u32 LoopedTask_OpenRegionMap(s32 taskState) {
  ...
  case 0:
    ...
    HideBg(3);
    SetBgMode(1);
    InitBgTemplates(sRegionMapBgTemplates,
                    ARRAY_COUNT(sRegionMapBgTemplates) - 1);
    regionMap = GetSubstructPtr(POKENAV_SUBSTRUCT_REGION_MAP);
    InitRegionMapData(regionMap, &sRegionMapBgTemplates[1],
...
static void LoadPokenavRegionMapGfx(struct Pokenav_RegionMapGfx *state) {
  ...
  CopyPaletteIntoBufferUnfaded(sMapSecInfoWindow_Pal, BG_PLTT_ID(1),
                               sizeof(sMapSecInfoWindow_Pal));
  CopyPaletteIntoBufferUnfaded(gRegionMapCityZoomTiles_Pal, BG_PLTT_ID(3),
                               PLTT_SIZE_4BPP);
  if (!IsRegionMapZoomed())
...
static void UpdateMapSecInfoWindow(struct Pokenav_RegionMapGfx *state) {
  ...
  case MAPSECTYPE_CITY_CANFLY:
    ...
    AddTextPrinterParameterized(state->infoWindowId, FONT_NARROW,
                                regionMap->mapSecName, 0, 1, TEXT_SKIP_DRAW,
                                NULL);
...
static u32 LoopedTask_DecompressCityMaps(s32 taskState) {
  ...
  if (taskState < NUM_CITY_MAPS) {
    LZ77UnCompWram(sPokenavCityMaps[taskState].tilemap,
                   state->cityZoomPics[taskState]);
    return LT_INC_AND_CONTINUE;
  }
...
static void DrawCityMap(struct Pokenav_RegionMapGfx *state, int mapSecId,
                        int pos) {
  int i;
  for (i = 0; i < NUM_CITY_MAPS && (sPokenavCityMaps[i].mapSecId != mapSecId ||
                                    sPokenavCityMaps[i].index != pos);
       i++)
    ;
...
static void PrintLandmarkNames(struct Pokenav_RegionMapGfx *state, int mapSecId,
                               int pos) {
  int i = 0;
  while (1) {
    ...
    AddTextPrinterParameterized(state->infoWindowId, FONT_NARROW, gStringVar1,
                                0, i * 16 + 17, TEXT_SKIP_DRAW, NULL);
    i++;
  }
}
...
static void CreateCityZoomTextSprites(void) {
  ...
  for (i = 0; i < (int)ARRAY_COUNT(state->cityZoomTextSprites); i++) {
    u8 spriteId =
        CreateSprite(&sCityZoomTextSpriteTemplate, 152 + i * 32, y, 8);
...
// Slide and cycle through the text key showing what the features on the zoomed
// city map are
static void SpriteCB_CityZoomText(struct Sprite *sprite) {
  if (sprite->data[3]) {
    sprite->data[3]--;
