...
static const u32 sTrainerHillWindowTileset[] =
    INCBIN_U32("graphics/trainer_hill/records_window.4bpp");
static const u16 sTrainerHillWindowPalette[] =
    INCBIN_U16("graphics/trainer_hill/records_window.gbapal");
static const u32 sTrainerHillWindowTilemap[] =
    INCBIN_U32("graphics/trainer_hill/records_window.bin");
...
static s32 FindLinkBattleRecord(struct LinkBattleRecord *records,
                                const u8 *name, u16 trainerId) {
  s32 i;
...
static void UpdateLinkBattleRecord(struct LinkBattleRecord *record,
                                   s32 battleOutcome) {
...
static void UpdateLinkBattleRecords(struct LinkBattleRecords *records,
                                    const u8 *name, u16 trainerId,
                                    s32 battleOutcome, u8 battlerId) {
  s32 index;
...
void UpdatePlayerLinkBattleRecords(s32 battlerId) {
  if (InUnionRoom() != TRUE) {
    ...
    UpdateLinkBattleRecords(
        &gSaveBlock1Ptr->linkBattleRecords, gTrainerCards[battlerId].playerName,
...
static void PrintLinkBattleWinsLossesDraws(struct LinkBattleRecord *records) {
  ...
  ConvertIntToDecimalStringN(gStringVar1,
                             GetGameStat(GAME_STAT_LINK_BATTLE_WINS),
                             STR_CONV_MODE_LEFT_ALIGN, 4);
  ConvertIntToDecimalStringN(gStringVar2,
                             GetGameStat(GAME_STAT_LINK_BATTLE_LOSSES),
                             STR_CONV_MODE_LEFT_ALIGN, 4);
  ConvertIntToDecimalStringN(gStringVar3,
                             GetGameStat(GAME_STAT_LINK_BATTLE_DRAWS),
                             STR_CONV_MODE_LEFT_ALIGN, 4);
  StringExpandPlaceholders(gStringVar4, gText_TotalRecordWLD);

  x = GetStringCenterAlignXOffset(FONT_NORMAL, gStringVar4, 0xD0);
  AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gStringVar4, x,
                              0x11, 0, NULL);
}
...
static void PrintLinkBattleRecord(struct LinkBattleRecord *record, u8 y,
  ...
  if (record->wins == 0 && record->losses == 0 && record->draws == 0) {
    ...
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL,
                                sText_DashesNoPlayer, 8, (y * 8) + 1, 0, NULL);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL,
                                sText_DashesNoScore, 80, (y * 8) + 1, 0, NULL);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL,
                                sText_DashesNoScore, 128, (y * 8) + 1, 0, NULL);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL,
                                sText_DashesNoScore, 176, (y * 8) + 1, 0, NULL);
  } else {
    ...
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gStringVar1, 8,
                                (y * 8) + 1, 0, NULL);

    ConvertIntToDecimalStringN(gStringVar1, record->wins,
                               STR_CONV_MODE_RIGHT_ALIGN, 4);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gStringVar1, 80,
                                (y * 8) + 1, 0, NULL);

    ConvertIntToDecimalStringN(gStringVar1, record->losses,
                               STR_CONV_MODE_RIGHT_ALIGN, 4);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gStringVar1, 128,
                                (y * 8) + 1, 0, NULL);

    ConvertIntToDecimalStringN(gStringVar1, record->draws,
                               STR_CONV_MODE_RIGHT_ALIGN, 4);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gStringVar1, 176,
                                (y * 8) + 1, 0, NULL);
  }
}
...
void ShowLinkBattleRecords(void) {
  ...
  AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gStringVar4, 0, 41,
                              0, NULL);
...
static void SetDispcntReg(void) {
  SetGpuReg(REG_OFFSET_DISPCNT,
            DISPCNT_BG0_ON | DISPCNT_BG3_ON | DISPCNT_OBJ_1D_MAP);
}

static void LoadTrainerHillRecordsWindowGfx(u8 bgId) {
  LoadBgTiles(bgId, sTrainerHillWindowTileset,
              sizeof(sTrainerHillWindowTileset), 0);
  CopyToBgTilemapBufferRect(bgId, sTrainerHillWindowTilemap, 0, 0, 0x20, 0x20);
  LoadPalette(sTrainerHillWindowPalette, BG_PLTT_ID(0),
              sizeof(sTrainerHillWindowPalette));
}
...
static void CB2_ShowTrainerHillRecords(void) {
  ...
  case 2:
    ...
    InitBgsFromTemplates(0, sTrainerHillRecordsBgTemplates,
                         ARRAY_COUNT(sTrainerHillRecordsBgTemplates));
