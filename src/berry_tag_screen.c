...
static const u16 sFontPalette[] =
    INCBIN_U16("graphics/bag/berry_tag_screen.gbapal");
...
static const u8 *const sBerryFirmnessStrings[] = {
    gBerryFirmnessString_VerySoft, gBerryFirmnessString_Soft,
...
static void PrintTextInBerryTagScreen(u8 windowId, const u8 *text, u8 x, u8 y,
                                      s32 speed, u8 colorStructId) {
  AddTextPrinterParameterized4(windowId, FONT_NORMAL, x, y, 0, 0,
                               sTextColors[colorStructId], speed, text);
}

static void AddBerryTagTextToBg0(void) {
  memcpy(GetBgTilemapBuffer(0), sBerryTag->tilemapBuffers[2],
         sizeof(sBerryTag->tilemapBuffers[2]));
  FillWindowPixelBuffer(WIN_BERRY_TAG, PIXEL_FILL(15));
  PrintTextInBerryTagScreen(
      WIN_BERRY_TAG, gText_BerryTag,
      GetStringCenterAlignXOffset(FONT_NORMAL, gText_BerryTag, 0x40), 1, 0, 1);
...
static void PrintBerryNumberAndName(void) {
  ...
  ConvertIntToDecimalStringN(gStringVar1, sBerryTag->berryId,
...
static void PrintBerrySize(void) {
  const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
  AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL, gText_SizeSlash, 0, 1,
                              TEXT_SKIP_DRAW, NULL);
  if (berry->size != 0) {
    u32 inches, fraction;
    ...
    ConvertIntToDecimalStringN(gStringVar1, inches, STR_CONV_MODE_LEFT_ALIGN,
                               2);
    ConvertIntToDecimalStringN(gStringVar2, fraction, STR_CONV_MODE_LEFT_ALIGN,
                               2);
    StringExpandPlaceholders(gStringVar4, gText_Var1DotVar2);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL, gStringVar4, 0x28,
                                1, 0, NULL);
  } else {
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL, gText_ThreeMarks,
                                0x28, 1, 0, NULL);
  }
}
...
static void PrintBerryFirmness(void) {
  const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
  AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL, gText_FirmSlash, 0,
                              0x11, TEXT_SKIP_DRAW, NULL);
  if (berry->firmness != 0)
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL,
                                sBerryFirmnessStrings[berry->firmness - 1],
                                0x28, 0x11, 0, NULL);
  else
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_NORMAL, gText_ThreeMarks,
                                0x28, 0x11, 0, NULL);
...
static void PrintBerryDescription1(void) {
  const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
  AddTextPrinterParameterized(WIN_DESC, FONT_NORMAL, berry->description1, 0, 1,
                              0, NULL);
}

static void PrintBerryDescription2(void) {
  const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
  AddTextPrinterParameterized(WIN_DESC, FONT_NORMAL, berry->description2, 0,
                              0x11, 0, NULL);
}

static void CreateBerrySprite(void) {
  sBerryTag->berrySpriteId =
      CreateBerryTagSprite(sBerryTag->berryId - 1, 56, 64);
}
...
static void CreateFlavorCircleSprites(void) {
  ...
  sBerryTag->flavorCircleIds[FLAVOR_BITTER] =
      CreateBerryFlavorCircleSprite(184);
...
static void TryChangeDisplayedBerry(u8 taskId, s8 toMove) {
  ...
  s16 currPocketPosition = gBagPosition.scrollPosition[BERRIES_POCKET] +
                           gBagPosition.cursorPosition[BERRIES_POCKET];
  u32 newPocketPosition = currPocketPosition + toMove;
  if (newPocketPosition < ITEM_TO_BERRY(MAX_BERRY_INDEX) &&
      BagGetItemIdByPocketPosition(POCKET_BERRIES, newPocketPosition) !=
          ITEM_NONE) {
    if (toMove < 0)
      tBgOp = BG_COORD_SUB;
    else
...
static void HandleBagCursorPositionChange(s8 toMove) {
  ...
  if (toMove > 0) {
    if (*cursorPos < 4 ||
        BagGetItemIdByPocketPosition(POCKET_BERRIES, *scrollPos + 8) == 0)
      *cursorPos += toMove;
    else
      *scrollPos += toMove;
  } else {
  ...
  sBerryTag->berryId = ItemIdToBerryType(
      BagGetItemIdByPocketPosition(POCKET_BERRIES, *scrollPos + *cursorPos));
}
