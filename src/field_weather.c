...
static void ApplyColorMapWithBlend(u8 startPalIndex, u8 numPalettes,
                                   s8 colorMapIndex, u8 blendCoeff,
                                   u16 blendColor);
static void ApplyDroughtColorMapWithBlend(s8 colorMapIndex, u8 blendCoeff,
                                          u16 blendColor);
...
// The drought weather effect uses a precalculated color lookup table.
// Presumably this is because the underlying color shift calculation is slow.
...
// This is a pointer to gWeather. All code in this file accesses gWeather
// directly, while code in other field weather files accesses gWeather through
// this pointer. This is likely the result of compiler optimization, since using
// the pointer in this file produces the same result as accessing gWeather
// directly.
struct Weather *const gWeatherPtr = &gWeather;

static const struct WeatherCallbacks sWeatherFuncs[] = {
    [WEATHER_NONE] = {None_Init, None_Main, None_Init, None_Finish},
    [WEATHER_SUNNY_CLOUDS] = {Clouds_InitVars, Clouds_Main, Clouds_InitAll,
                              Clouds_Finish},
    ...
    [WEATHER_RAIN_THUNDERSTORM] = {Thunderstorm_InitVars, Thunderstorm_Main,
                                   Thunderstorm_InitAll, Thunderstorm_Finish},
    [WEATHER_FOG_HORIZONTAL] = {FogHorizontal_InitVars, FogHorizontal_Main,
                                FogHorizontal_InitAll, FogHorizontal_Finish},
    [WEATHER_VOLCANIC_ASH] = {Ash_InitVars, Ash_Main, Ash_InitAll, Ash_Finish},
    [WEATHER_SANDSTORM] = {Sandstorm_InitVars, Sandstorm_Main,
                           Sandstorm_InitAll, Sandstorm_Finish},
    [WEATHER_FOG_DIAGONAL] = {FogDiagonal_InitVars, FogDiagonal_Main,
                              FogDiagonal_InitAll, FogDiagonal_Finish},
    [WEATHER_UNDERWATER] = {FogHorizontal_InitVars, FogHorizontal_Main,
                            FogHorizontal_InitAll, FogHorizontal_Finish},
    [WEATHER_SHADE] = {Shade_InitVars, Shade_Main, Shade_InitAll, Shade_Finish},
    [WEATHER_DROUGHT] = {Drought_InitVars, Drought_Main, Drought_InitAll,
                         Drought_Finish},
    [WEATHER_DOWNPOUR] = {Downpour_InitVars, Thunderstorm_Main,
                          Downpour_InitAll, Thunderstorm_Finish},
    [WEATHER_UNDERWATER_BUBBLES] = {Bubbles_InitVars, Bubbles_Main,
                                    Bubbles_InitAll, Bubbles_Finish},
};
...
void StartWeather(void) {
  if (!FuncIsActiveTask(Task_WeatherMain)) {
    ...
    CpuCopy32(gFogPalette, &gPlttBufferUnfaded[OBJ_PLTT_ID(index)],
              PLTT_SIZE_4BPP);
    BuildColorMaps();
    gWeatherPtr->contrastColorMapSpritePalIndex = index;
    gWeatherPtr->weatherPicSpritePalIndex =
        AllocSpritePalette(PALTAG_WEATHER_2);
...
void SetNextWeather(u8 weather) {
  ...
  if (gWeatherPtr->nextWeather != weather &&
...
static u8 None_Finish(void) { return 0; }
  ...
  for (i = 0; i < 2; i++) {
    ...
    for (colorVal = 0; colorVal < 32; colorVal++) {
      ...
      if (colorVal < 12) {
        ...
        // brightness modifiers, which are increased at a significantly lower
        // rate than the midtones and highlights (color values >= 12). This
        // creates a high contrast effect, used in the thunderstorm weather.
...
static void UpdateWeatherColorMap(void) {
  if (gWeatherPtr->palProcessingState != WEATHER_PAL_STATE_SCREEN_FADING_OUT) {
    if (gWeatherPtr->colorMapIndex == gWeatherPtr->targetColorMapIndex) {
      ...
      if (++gWeatherPtr->colorMapStepCounter >=
...
static bool8 FadeInScreen_RainShowShade(void) {
  ...
  ApplyColorMapWithBlend(0, 32, 3, 16 - gWeatherPtr->fadeScreenCounter,
...
static bool8 FadeInScreen_Drought(void) {
  ...
  ApplyDroughtColorMapWithBlend(-6, 16 - gWeatherPtr->fadeScreenCounter,
                                gWeatherPtr->fadeDestColor);
  return TRUE;
}
...
static bool8 FadeInScreen_FogHorizontal(void) {
  ...
  ApplyFogBlend(16 - gWeatherPtr->fadeScreenCounter,
                gWeatherPtr->fadeDestColor);
  return TRUE;
}
  ...
  if (colorMapIndex > 0) {
    ...
    while (curPalIndex < numPalettes) {
      if (sPaletteColorMapTypes[curPalIndex] == COLOR_MAP_NONE) {
        // No palette change.
        CpuFastCopy(&gPlttBufferUnfaded[palOffset],
                    &gPlttBufferFaded[palOffset], PLTT_SIZE_4BPP);
        palOffset += 16;
      } else {
        u8 r, g, b;

        if (sPaletteColorMapTypes[curPalIndex] == COLOR_MAP_CONTRAST ||
            curPalIndex - 16 == gWeatherPtr->contrastColorMapSpritePalIndex)
        ...
        for (i = 0; i < 16; i++) {
          ...
          struct RGBColor baseColor =
              *(struct RGBColor *)&gPlttBufferUnfaded[palOffset];
    ...
    // A negative gammIndex value means that the blending will come from the
    // special Drought weather's palette tables.
      ...
      if (sPaletteColorMapTypes[curPalIndex] == COLOR_MAP_NONE) {
        ...
        for (i = 0; i < 16; i++) {
          gPlttBufferFaded[palOffset] =
              sDroughtWeatherColors[colorMapIndex][DROUGHT_COLOR_INDEX(
                  gPlttBufferUnfaded[palOffset])];
          palOffset++;
        }
      }
    ...
    // No palette blending.
    CpuFastCopy(&gPlttBufferUnfaded[PLTT_ID(startPalIndex)],
                &gPlttBufferFaded[PLTT_ID(startPalIndex)],
                numPalettes * PLTT_SIZE_4BPP);
  }
}
  ...
  for (curPalIndex = 0; curPalIndex < 32; curPalIndex++) {
    if (sPaletteColorMapTypes[curPalIndex] == COLOR_MAP_NONE) {
      ...
      for (i = 0; i < 16; i++) {
        ...
        color2 =
            *(struct RGBColor *)&sDroughtWeatherColors[colorMapIndex][offset];
        r2 = color2.r;
        g2 = color2.g;
        b2 = color2.b;
...
static void MarkFogSpritePalToLighten(u8 paletteIndex) {
  if (gWeatherPtr->lightenedFogSpritePalsCount < 6) {
    gWeatherPtr
        ->lightenedFogSpritePals[gWeatherPtr->lightenedFogSpritePalsCount] =
        paletteIndex;
...
void ApplyWeatherColorMapIfIdle_Gradual(u8 colorMapIndex,
                                        u8 targetColorMapIndex,
...
void FadeScreen(u8 mode, s8 delay) {
  ...
  if (fadeOut) {
    ...
    Weather_SetBlendCoeffs(gWeatherPtr->currBlendEVA,
...
bool8 IsWeatherNotFadingIn(void) {
  return (gWeatherPtr->palProcessingState !=
...
void UpdateSpritePaletteWithWeather(u8 spritePaletteIndex) {
  ...
  case WEATHER_PAL_STATE_SCREEN_FADING_OUT:
    paletteIndex = PLTT_ID(paletteIndex);
    CpuFastCopy(&gPlttBufferFaded[paletteIndex],
                &gPlttBufferUnfaded[paletteIndex], PLTT_SIZE_4BPP);
...
void LoadCustomWeatherSpritePalette(const u16 *palette) {
  LoadPalette(palette, OBJ_PLTT_ID(gWeatherPtr->weatherPicSpritePalIndex),
              PLTT_SIZE_4BPP);
...
bool8 LoadDroughtWeatherPalettes(void) {
  if (gWeatherPtr->loadDroughtPalsIndex < 32) {
    LoadDroughtWeatherPalette(&gWeatherPtr->loadDroughtPalsIndex,
...
void DroughtStateRun(void) {
  ...
  case 1:
    ...
    gWeatherPtr->droughtBrightnessStage =
        ((gSineTable[gWeatherPtr->droughtTimer] - 1) >> 6) + 2;
...
bool8 Weather_UpdateBlend(void) {
  ...
  SetGpuReg(REG_OFFSET_BLDALPHA, BLDALPHA_BLEND(gWeatherPtr->currBlendEVA,
                                                gWeatherPtr->currBlendEVB));
...
u8 GetCurrentWeather(void) { return gWeatherPtr->currWeather; }
...
u8 IsWeatherChangeComplete(void) { return gWeatherPtr->weatherChangeComplete; }
