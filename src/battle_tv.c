...
static const u16 sVariableDmgMoves[] = {
    MOVE_COUNTER, MOVE_FISSURE, MOVE_BIDE, MOVE_MIRROR_COAT, MOVE_HORN_DRILL,
...
static const u16 sPoints_RainMoves[] = {
    MOVE_BUBBLE,       3,  MOVE_WHIRLPOOL,    3,
    MOVE_OCTAZOOKA,    3,  MOVE_CLAMP,        3,
    MOVE_WITHDRAW,     3,  MOVE_CRABHAMMER,   3,
    MOVE_WATER_SPOUT,  3,  MOVE_DIVE,         3,
    MOVE_WATERFALL,    3,  MOVE_MUDDY_WATER,  3,
    MOVE_SURF,         3,  MOVE_HYDRO_CANNON, 3,
    MOVE_HYDRO_PUMP,   3,  MOVE_BUBBLE_BEAM,  3,
    MOVE_WATER_SPORT,  0, // Unnecessary, unlisted moves are already given 0
                          // points
    MOVE_WATER_GUN,    3,  MOVE_WATER_PULSE,  3,
    MOVE_WEATHER_BALL, 3,  MOVE_THUNDER,      3,
    MOVE_SOLAR_BEAM,   -4, MOVE_OVERHEAT,     -4,
    MOVE_FLAME_WHEEL,  -4, MOVE_FLAMETHROWER, -4,
    MOVE_SACRED_FIRE,  -4, MOVE_FIRE_BLAST,   -4,
    MOVE_HEAT_WAVE,    -4, MOVE_EMBER,        -4,
    MOVE_BLAST_BURN,   -4, MOVE_BLAZE_KICK,   -4,
    MOVE_ERUPTION,     -4, MOVE_FIRE_SPIN,    -4,
    MOVE_FIRE_PUNCH,   -4, MOVE_SOLAR_BEAM,   -4, // Repeated
    TABLE_END,         0};
...
static const u16 sPoints_ElectricMoves[] = {
    ...
    MOVE_THUNDER_WAVE,  0, // Unnecessary, unlisted moves are already given 0
                           // points
    MOVE_THUNDER,       3, MOVE_VOLT_TACKLE,   3, TABLE_END,       0};
...
static const u16 sPoints_StatIncrease1[NUM_BATTLE_STATS - 1] = {
    [STAT_ATK - 1] = 2,    [STAT_DEF - 1] = 2,   [STAT_SPEED - 1] = 2,
    [STAT_SPATK - 1] = 2,  [STAT_SPDEF - 1] = 2, [STAT_ACC - 1] = 2,
    [STAT_EVASION - 1] = 2};
static const u16 sPoints_StatIncrease2[NUM_BATTLE_STATS - 1] = {
    [STAT_ATK - 1] = 4,    [STAT_DEF - 1] = 4,   [STAT_SPEED - 1] = 4,
    [STAT_SPATK - 1] = 4,  [STAT_SPDEF - 1] = 4, [STAT_ACC - 1] = 4,
    [STAT_EVASION - 1] = 4};
static const u16 sPoints_StatDecreaseSelf[NUM_BATTLE_STATS - 1] = {
    [STAT_ATK - 1] = -1,    [STAT_DEF - 1] = -1,   [STAT_SPEED - 1] = -1,
    [STAT_SPATK - 1] = -1,  [STAT_SPDEF - 1] = -1, [STAT_ACC - 1] = -1,
    [STAT_EVASION - 1] = -1};
...
static const u16 sSpecialBattleStrings[] = {
    STRINGID_PKMNPERISHCOUNTFELL,   STRINGID_PKMNWISHCAMETRUE,
    STRINGID_PKMNLOSTPPGRUDGE,      STRINGID_PKMNTOOKFOE,
    STRINGID_PKMNABSORBEDNUTRIENTS, STRINGID_PKMNANCHOREDITSELF,
    STRINGID_PKMNAFFLICTEDBYCURSE,  STRINGID_PKMNSAPPEDBYLEECHSEED,
    STRINGID_PKMNLOCKEDINNIGHTMARE, STRINGID_PKMNHURTBY,
    STRINGID_PKMNHURTBYBURN,        STRINGID_PKMNHURTBYPOISON,
    STRINGID_PKMNHURTBYSPIKES,      STRINGID_ATTACKERFAINTED,
    STRINGID_TARGETFAINTED,         STRINGID_PKMNHITWITHRECOIL,
    STRINGID_PKMNCRASHED,           TABLE_END};
...
void BattleTv_SetDataBasedOnString(u16 stringId) {
  ...
  moveSlot =
      GetBattlerMoveSlotId(gBattlerAttacker, gBattleMsgDataPtr->currentMove);

  if (moveSlot >= MAX_MON_MOVES && IsNotSpecialBattleString(stringId) &&
      stringId > BATTLESTRINGS_TABLE_START) {
  ...
  case STRINGID_PKMNWISHCAMETRUE:
    if (tvPtr->side[defSide].wishMonId != 0) {
      AddMovePoints(PTS_SET_UP, 3, defSide,
                    (tvPtr->side[defSide].wishMonId - 1) * 4 +
                        tvPtr->side[defSide].wishMoveSlot);
    }
    break;
  case STRINGID_PKMNWANTSGRUDGE:
    tvPtr->side[atkSide].grudgeMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
  ...
  case STRINGID_PKMNTRYINGTOTAKEFOE:
    tvPtr->side[atkSide].destinyBondMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
  ...
  case STRINGID_PKMNPLANTEDROOTS:
    tvPtr->pos[atkSide][atkFlank].ingrainMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
  ...
  case STRINGID_ATTACKERSSTATROSE:
    if (gBattleTextBuff1[2] != 0) {
      if (*statStringId == STRINGID_STATSHARPLY)
        AddMovePoints(PTS_STAT_INCREASE_2, moveSlot, gBattleTextBuff1[2] - 1,
                      0);
      else
        AddMovePoints(PTS_STAT_INCREASE_1, moveSlot, gBattleTextBuff1[2] - 1,
                      0);
    }
    break;
  case STRINGID_DEFENDERSSTATROSE:
    if (gBattleTextBuff1[2] != 0) {
      if (gBattlerAttacker == gBattlerTarget) {
        ...
        AddMovePoints(PTS_STAT_INCREASE_NOT_SELF, moveSlot,
                      gBattleTextBuff1[2] - 1, 0);
      }
    }
    break;
  case STRINGID_ATTACKERSSTATFELL:
    if (gBattleTextBuff1[2] != 0)
      AddMovePoints(PTS_STAT_DECREASE_SELF, moveSlot, gBattleTextBuff1[2] - 1,
                    0);
    break;
  case STRINGID_DEFENDERSSTATFELL:
    if (gBattleTextBuff1[2] != 0) {
      if (*statStringId == STRINGID_STATHARSHLY)
        AddMovePoints(PTS_STAT_DECREASE_2, moveSlot, gBattleTextBuff1[2] - 1,
                      0);
      else
        AddMovePoints(PTS_STAT_DECREASE_1, moveSlot, gBattleTextBuff1[2] - 1,
                      0);
    }
    break;
  case STRINGID_PKMNLAIDCURSE:
    tvPtr->pos[defSide][defFlank].curseMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
  ...
  case STRINGID_PKMNAFFLICTEDBYCURSE:
    if (GetMonData(atkMon, MON_DATA_HP, NULL) &&
      ...
      AddMovePoints(PTS_STATUS_DMG, 0,
  ...
  case STRINGID_PKMNSAPPEDBYLEECHSEED:
    if (tvPtr->pos[atkSide][atkFlank].leechSeedMonId != 0) {
      AddMovePoints(PTS_STATUS_DMG, 1,
  ...
  case STRINGID_PKMNLOCKEDINNIGHTMARE:
    if (GetMonData(atkMon, MON_DATA_HP, NULL) != 0 &&
      ...
      AddMovePoints(PTS_STATUS_DMG, 5,
  ...
  case STRINGID_PKMNTRAPPEDBYSANDTOMB:
    tvPtr->pos[defSide][defFlank].wrapMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
  ...
  case STRINGID_PKMNHURTBY:
    if (GetMonData(atkMon, MON_DATA_HP, NULL) != 0 &&
      ...
      AddMovePoints(PTS_STATUS_DMG, 6,
  ...
  case STRINGID_PKMNWASBURNED:
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].brnMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].brnMoveSlot =
        moveSlot;
    break;
  case STRINGID_PKMNHURTBYBURN:
    if (GetMonData(atkMon, MON_DATA_HP, NULL) != 0) {
      if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]]
              .brnMonId != 0)
        AddMovePoints(
            PTS_STATUS_DMG, 4,
      ...
      tvPtr->side[atkSide].faintCauseMonId =
  ...
  case STRINGID_PKMNWASPOISONED:
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].psnMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].psnMoveSlot =
        moveSlot;
    break;
  case STRINGID_PKMNBADLYPOISONED:
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].badPsnMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].badPsnMoveSlot =
        moveSlot;
    break;
  ...
  case STRINGID_PKMNFELLINLOVE:
    tvPtr->pos[defSide][defFlank].attractMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
  ...
  case STRINGID_PKMNWASPARALYZED:
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].prlzMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].prlzMoveSlot =
        moveSlot;
    break;
  case STRINGID_PKMNISPARALYZED:
    if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].prlzMonId !=
        0)
      AddMovePoints(PTS_STATUS, 2,
                    tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]]
                            .prlzMonId -
                        1,
                    tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]]
  ...
  case STRINGID_PKMNFELLASLEEP:
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].slpMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].slpMoveSlot =
        moveSlot;
    break;
  case STRINGID_PKMNFASTASLEEP:
    if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].slpMonId !=
            0 &&
  ...
  case STRINGID_PKMNWASFROZEN:
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].frzMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
    tvPtr->mon[effSide][gBattlerPartyIndexes[gEffectBattler]].frzMoveSlot =
        moveSlot;
    break;
  case STRINGID_PKMNISFROZEN:
    if (tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].frzMonId !=
        0)
      AddMovePoints(
          PTS_STATUS, 4,
          tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]].frzMonId -
              1,
          tvPtr->mon[atkSide][gBattlerPartyIndexes[gBattlerAttacker]]
              .frzMoveSlot);
    break;
  ...
  case STRINGID_PKMNPROTECTEDBYMIST:
    if (tvPtr->side[defSide].mistMonId != 0)
      AddMovePoints(PTS_MIST, 0, tvPtr->side[defSide].mistMonId - 1,
                    tvPtr->side[defSide].mistMoveSlot);
    break;
  case STRINGID_THEWALLSHATTERED:
    ...
    AddMovePoints(PTS_BREAK_WALL, 0, gBattlerPartyIndexes[gBattlerAttacker],
                  moveSlot);
    break;
  case STRINGID_PKMNFLINCHED:
    if (tvPtr->pos[atkSide][0].attackedByMonId != 0)
      AddMovePoints(PTS_FLINCHED, 0, tvPtr->pos[atkSide][0].attackedByMonId - 1,
                    tvPtr->pos[atkSide][0].attackedByMoveSlot);
    if (tvPtr->pos[atkSide][1].attackedByMonId != 0)
      AddMovePoints(PTS_FLINCHED, 0, tvPtr->pos[atkSide][1].attackedByMonId - 1,
...
void BattleTv_SetDataBasedOnMove(u16 move, u16 weatherFlags,
  ...
  tvPtr->pos[defSide][GetBattlerPosition(gBattlerAttacker) / 2]
      .attackedByMonId = gBattlerPartyIndexes[gBattlerAttacker] + 1;
  tvPtr->pos[defSide][GetBattlerPosition(gBattlerAttacker) / 2]
      .attackedByMoveSlot = moveSlot;
  ...
  if (move == MOVE_SELF_DESTRUCT || move == MOVE_EXPLOSION) {
    tvPtr->side[atkSide ^ BIT_SIDE].explosionMonId =
        gBattlerPartyIndexes[gBattlerAttacker] + 1;
  ...
  AddMovePoints(PTS_REFLECT, gBattleMoves[move].type, gBattleMoves[move].power,
                0);
  AddMovePoints(PTS_LIGHT_SCREEN, gBattleMoves[move].type,
...
void TryPutLinkBattleTvShowOnAir(void) {
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_MULTI) {
    if ((playerBestMonId < MULTI_PARTY_SIZE &&
         !GetLinkTrainerFlankId(gBattleScripting.multiplayerId)) ||
        (playerBestMonId >= MULTI_PARTY_SIZE &&
         GetLinkTrainerFlankId(gBattleScripting.multiplayerId))) {
      j = (opponentBestMonId < MULTI_PARTY_SIZE) ? FALSE : TRUE;
      PutBattleUpdateOnTheAir(
          GetOpposingLinkMultiBattlerId(j, gBattleScripting.multiplayerId),
          moveId, playerBestSpecies, opponentBestSpecies);
    }
  } else {
    PutBattleUpdateOnTheAir(gBattleScripting.multiplayerId ^ 1, moveId,
                            playerBestSpecies, opponentBestSpecies);
  }
}
...
static void AddMovePoints(u8 caseId, u16 arg1, u8 arg2, u8 arg3) {
  ...
  case PTS_STAT_INCREASE_NOT_SELF:
    movePoints
        ->points[atkSide][gBattlerPartyIndexes[gBattlerAttacker] * 4 + arg1] +=
...
#define move arg1
  ...
  case PTS_ELECTRIC:
    ...
    do {
      if (move == ptr[i]) {
        movePoints->points[atkSide][gBattlerPartyIndexes[gBattlerAttacker] * 4 +
                                    arg2] += ptr[i + 1];
        break;
      }
      i += 2;
...
#undef move
  ...
  case PTS_MIST:
  case PTS_FLINCHED:
    movePoints->points[atkSide ^ BIT_SIDE][arg2 * 4 + arg3] +=
        sPointsArray[caseId][arg1];
    break;
  case PTS_SPIKES:
...
#define power arg2
  ...
  case PTS_REFLECT:
    ...
    if (IS_TYPE_PHYSICAL(type) && power != 0 &&
        tvPtr->side[defSide].reflectMonId != 0) {
  ...
  case PTS_LIGHT_SCREEN:
    ...
    if (!IS_TYPE_PHYSICAL(type) && power != 0 &&
      ...
      movePoints
          ->points[defSide][id + tvPtr->side[defSide].lightScreenMoveSlot] +=
          sPointsArray[caseId][0];
    }
    break;
#undef type
  ...
  if (tvPtr->side[atkSide].faintCause != FNT_NONE) {
    ...
    case FNT_PERISH_SONG:
      ...
      if (tvPtr->side[atkSide ^ BIT_SIDE].perishSong) {
        AddMovePoints(PTS_FAINT, 0, atkSide ^ BIT_SIDE,
                      (tvPtr->side[atkSide ^ BIT_SIDE].perishSongMonId - 1) *
                              4 +
    ...
    case FNT_DESTINY_BOND:
      if (tvPtr->side[atkSide ^ BIT_SIDE].destinyBondMonId != 0) {
        AddMovePoints(PTS_FAINT, 0, atkSide ^ BIT_SIDE,
                      (tvPtr->side[atkSide ^ BIT_SIDE].destinyBondMonId - 1) *
                              4 +
...
static void TrySetBattleSeminarShow(void) {
  ...
  if (gBattleTypeFlags &
    ...
    return;
  else if (gBattleMons[gBattlerAttacker].statStages[STAT_ACC] <
           DEFAULT_STAT_STAGE)
    return;
  else if (gBattleMons[gBattlerTarget].statStages[STAT_EVASION] >
           DEFAULT_STAT_STAGE)
    return;
  else if (gCurrentMove == MOVE_HIDDEN_POWER ||
           gCurrentMove == MOVE_WEATHER_BALL)
    return;
  else if (gBattleTypeFlags &
           (BATTLE_TYPE_PALACE | BATTLE_TYPE_PIKE | BATTLE_TYPE_PYRAMID))
    return;
  ...
  i = 0;
  currMoveSaved = gBattleMons[gBattlerAttacker]
                      .moves[gMoveSelectionCursor[gBattlerAttacker]];
  do {
  ...
  for (i = 0; i < MAX_MON_MOVES; i++) {
    ...
    if (ShouldCalculateDamage(gCurrentMove, &dmgByMove[i], &powerOverride)) {
      ...
      gBattleMoveDamage = CalculateBaseDamage(
          ...
          gCurrentMove, sideStatus, powerOverride, 0, gBattlerAttacker,
          gBattlerTarget);
  ...
  for (i = 0; i < MAX_MON_MOVES; i++) {
    if (i != gMoveSelectionCursor[gBattlerAttacker] &&
      ...
      opponentSpecies =
          GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattlerTarget]],
                     MON_DATA_SPECIES, NULL);
      playerSpecies =
          GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattlerAttacker]],
                     MON_DATA_SPECIES, NULL);
      TryPutBattleSeminarOnAir(opponentSpecies, playerSpecies,
...
u8 GetBattlerMoveSlotId(u8 battlerId, u16 moveId) {
  ...
  while (1) {
    if (i >= MAX_MON_MOVES)
      break;
    if (GetMonData(&party[gBattlerPartyIndexes[battlerId]], MON_DATA_MOVE1 + i,
                   NULL) == moveId)
      break;
    i++;
  }
