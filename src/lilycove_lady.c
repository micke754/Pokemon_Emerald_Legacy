...
u8 GetLilycoveLadyId(void) { return gSaveBlock1Ptr->lilycoveLady.id; }
...
void InitLilycoveLady(void) {
  u16 id = ((gSaveBlock2Ptr->playerTrainerId[1] << 8) |
...
static void FavorLadyPickFavorAndBestItem(void) {
  ...
  numItems =
      GetNumAcceptedItems(sFavorLadyAcceptedItemLists[sFavorLadyPtr->favorId]);
  bestItem = Random() % numItems;
  sFavorLadyPtr->bestItem =
      sFavorLadyAcceptedItemLists[sFavorLadyPtr->favorId][bestItem];
}
...
static const u8 *GetFavorLadyRequest(u8 idx) { return sFavorLadyRequests[idx]; }
...
// Only used to determine if a record-mixed player had given her an item she
// liked
...
void Script_FavorLadyOpenBagMenu(void) { FavorLadyOpenBagMenu(); }
...
u8 GetQuizAuthor(void) {
  ...
  else if (authorNameId == QUIZ_AUTHOR_NAME_OTHER_PLAYER ||
...
static bool8 IsQuizTrainerIdNotPlayer(void) {
  ...
  notPlayer = FALSE;
  for (i = 0; i < TRAINER_ID_LENGTH; i++) {
    if (sQuizLadyPtr->playerTrainerId[i] !=
...
void QuizLadyGetPlayerAnswer(void) { ShowEasyChatScreen(); }
...
void Script_QuizLadyOpenBagMenu(void) { QuizLadyOpenBagMenu(); }
...
void QuizLadyClearQuestionForRecordMix(const LilycoveLady *lilycoveLady) {
  ...
  if (lilycoveLady->quiz.prevQuestionId < ARRAY_COUNT(sQuizLadyQuizQuestions) &&
    ...
    if (lilycoveLady->quiz.prevQuestionId == sQuizLadyPtr->questionId)
      sQuizLadyPtr->questionId = (sQuizLadyPtr->questionId + 1) %
                                 (int)ARRAY_COUNT(sQuizLadyQuizQuestions);
...
static void ContestLadySavePlayerNameIfHighSheen(u8 sheen) {
  ...
  if (sContestLadyPtr->maxSheen <= sheen) {
    ...
    memset(sContestLadyPtr->playerName, EOS,
           sizeof(sContestLadyPtr->playerName));
    memcpy(sContestLadyPtr->playerName, gSaveBlock2Ptr->playerName,
           sizeof(sContestLadyPtr->playerName));
    sContestLadyPtr->language = gGameLanguage;
  }
}
...
static void BufferContestLadyCategoryAndMonName(u8 *category, u8 *nickname) {
  ...
  StringCopy_Nickname(nickname,
                      sContestLadyMonNames[sContestLadyPtr->category]);
}
