...
/* Summary of Apprentice, because (as of writing at least) it's not very well
 * documented online
 *
 * ## Basic info
 * In the Battle Tower lobby there is an NPC which asks to be taught by the
 * player They can be any 1 of 16 NPC trainers, each with their own name, class,
 * and set of possible party species They ask the player a series of questions
 * once per day, and eventually depart the lobby to be replaced by a new
 * Apprentice
 *
 * ## Initial Questions
 * The first question they always ask is a request to be taught, which cannot be
 * rejected The second question (which follows immediately after) is whether
 * they should participate in Battle Tower Lv 50 or Open Lv After these opening
 * questions they always ask the player to choose between 2 mons, which they
 * repeat 3 times
 *
 * ## Random Questions
 * After choosing 3 mons for them, the Apprentice will randomly ask between 1
 * and 8 questions of 4 different types, as follows
 ...
 * - Asking what held item to give to a mon, which they will ask at most 3 times
 * (once for each mon)
 * - Asking what they should say when they win a battle, which will always be
 * their final question before departing
 *
 * ## After departing
 * After telling them what they should say when they win a battle they will
 * leave the lobby for a final time They will then be replaced by a new random
 * Apprentice (they can repeat) Up to 4 old Apprentices are saved and can be
 * encountered (or partnered with) during challenges of the mode they were told
 * to battle in They can also be record mixed to and from other Emerald games
 * Old/record mixed Apprentices are stored in struct Apprentice apprentices of
 * SaveBlock2 and the current Apprentice is stored in struct PlayersApprentice
 * playerApprentice of SaveBlock2
 */

#define PLAYER_APPRENTICE gSaveBlock2Ptr->playerApprentice
#define CURRENT_QUESTION_NUM                                                   \
  PLAYER_APPRENTICE.questionsAnswered - NUM_WHICH_MON_QUESTIONS
...
void BufferApprenticeChallengeText(u8 saveApprenticeId) {
  ...
  StringCopy_PlayerName(
      gStringVar1, gSaveBlock2Ptr->apprentices[saveApprenticeId].playerName);
  ConvertInternationalString(
      gStringVar1, gSaveBlock2Ptr->apprentices[saveApprenticeId].language);
  ConvertIntToDecimalStringN(
      gStringVar2, gSaveBlock2Ptr->apprentices[saveApprenticeId].number,
      STR_CONV_MODE_RIGHT_ALIGN, i);
  challengeText =
      sApprenticeChallengeTexts[gSaveBlock2Ptr->apprentices[saveApprenticeId]
                                    .id];
...
// Picking a move chooses a random mon, picking a held item is sequential (so
...
// Sets the random order and data for the remaining questions after the initial
// "choose mon" questions
    ...
    if (questionOrder[i] != QUESTION_ID_WHICH_FIRST) {
      ...
      if (questionOrder[i] == QUESTION_ID_WHICH_MOVE) {
        ...
        gApprenticePartyMovesData
            ->moveSlots[id][gApprenticePartyMovesData->moveCounter] = rand1;
...
#define APPRENTICE_SPECIES_ID(monId)                                           \
  ((monId < MULTI_PARTY_SIZE)                                                  \
       ? (PLAYER_APPRENTICE.speciesIds[monId] >>                               \
              (((PLAYER_APPRENTICE.party >> monId) & 1) << 2) &                \
          0xF)                                                                 \
       : 0)
...
// Unlike the first move choice, this can be either a level up move or a TM/HM
// move
static u16 GetRandomAlternateMove(u8 monId) {
  u8 i, j;
  u8 id;
  ...
  if (PLAYER_APPRENTICE.lvlMode == APPRENTICE_LVL_MODE_50)
    ...
    level = 60; // Despite being open level, level up moves are only read up to
                // level 60
  ...
  // i < 5 here is arbitrary, i isnt used and is only incremented when the
  // selected move isnt in sValidApprenticeMoves This while loop contains 3
  // potential infinite loops, though none of them would occur in the base game
  while (i < 5) {
    if (Random() % 2 == 0 || needTMs == TRUE) {
      // Get TM move
      // NOTE: Below is an infinite loop if a species that only learns TMs for
      // moves
      //       that are also in its level up learnset is assigned to an
      //       Apprentice
      do {
        // NOTE: Below is an infinite loop if a species which cannot learn TMs
        // is assigned to an Apprentice
        do {
        ...
        for (; j < numLearnsetMoves; j++) {
          // Keep looking for TMs until one not in the level up learnset is
          // found
      ...
      if (numLearnsetMoves <= MAX_MON_MOVES) {
        ...
        //       all the moves in the rest of its learnset is assigned to an
        //       Apprentice
        do {
          ...
          for (j = numLearnsetMoves - MAX_MON_MOVES; j < numLearnsetMoves;
               j++) {
...
static bool8 TrySetMove(u8 monId, u16 moveId) {
  ...
  gApprenticePartyMovesData
      ->moves[monId][gApprenticePartyMovesData->moveCounter] = moveId;
  return TRUE;
}
...
// Get the level up move or previously suggested move to be the first move
// choice Compare to GetRandomAlternateMove, which gets the move that will be
// the second choice
static u16 GetDefaultMove(u8 monId, u8 speciesArrayId, u8 moveSlot) {
  ...
  numQuestions = 0;
  for (i = 0;
       i < APPRENTICE_MAX_QUESTIONS &&
       PLAYER_APPRENTICE.questions[i].questionId != QUESTION_ID_WIN_SPEECH;
       i++)
    numQuestions++;

  GetLatestLearnedMoves(
      gApprentices[PLAYER_APPRENTICE.id].species[speciesArrayId], moves);
  for (i = 0; i < numQuestions && i < CURRENT_QUESTION_NUM; i++) {
    if (PLAYER_APPRENTICE.questions[i].questionId == QUESTION_ID_WHICH_MOVE &&
      ...
      moves[PLAYER_APPRENTICE.questions[i].moveSlot] =
          PLAYER_APPRENTICE.questions[i].data;
    }
  }
...
static void SaveApprenticeParty(u8 numQuestions) {
  ...
  for (i = 0; i < MULTI_PARTY_SIZE; i++) {
    ...
    apprenticeMons[i]->species =
        gApprentices[PLAYER_APPRENTICE.id].species[speciesTableId];
...
static void CreateApprenticeMenu(u8 menu) {
  ...
  for (i = 0; i < count; i++)
    AddTextPrinterParameterized(windowId, FONT_NORMAL, strings[i], 8,
...
// VAR_0x8005 is 1 + the selection value from the multichoice
// APPRENTICE_ASK_WHICH_LEVEL i.e. APPRENTICE_LVL_MODE_50 or
// APPRENTICE_LVL_MODE_OPEN
static void Script_SetApprenticeLvlMode(void) {
  SetPlayersApprenticeLvlMode(gSpecialVar_0x8005);
}

// Never called, APPRENTICE_FUNC_SET_ID is unused
static void Script_SetApprenticeId(void) { SetApprenticeId(); }

static void Script_SetRandomQuestionData(void) { SetRandomQuestionData(); }

static void IncrementQuestionsAnswered(void) {
  PLAYER_APPRENTICE.questionsAnswered++;
}

// The first 3 questions answered after meeting the Apprentice are always
// selecting party mons
...
static void IsFinalQuestion(void) {
  ...
  if (questionNum < 0) {
    ...
    if (PLAYER_APPRENTICE.questions[questionNum].questionId ==
...
static void InitQuestionData(void) {
  ...
  if (gSpecialVar_0x8005 == APPRENTICE_QUESTION_WHICH_MON) {
    if (PLAYER_APPRENTICE.questionsAnswered < NUM_WHICH_MON_QUESTIONS) {
      // For the first MULTI_PARTY_SIZE (3) questions, a mon is asked to be
      // selected for the Apprentice's party
      id1 = PLAYER_APPRENTICE.speciesIds[PLAYER_APPRENTICE.questionsAnswered] >>
            4;
      gApprenticeQuestionData->altSpeciesId =
          gApprentices[PLAYER_APPRENTICE.id].species[id1];

      id2 = PLAYER_APPRENTICE.speciesIds[PLAYER_APPRENTICE.questionsAnswered] &
            0xF;
      gApprenticeQuestionData->speciesId =
          gApprentices[PLAYER_APPRENTICE.id].species[id2];
    }
    ...
    if (PLAYER_APPRENTICE.questionsAnswered >= NUM_WHICH_MON_QUESTIONS &&
      ...
      gApprenticeQuestionData->moveId1 = GetDefaultMove(
          count, id1,
          PLAYER_APPRENTICE.questions[CURRENT_QUESTION_NUM].moveSlot);
...
static void ApprenticeBufferString(void) {
  ...
  case APPRENTICE_BUFF_ITEM:
    StringCopy(
        stringDst,
        ItemId_GetName(PLAYER_APPRENTICE.questions[CURRENT_QUESTION_NUM].data));
    break;
  case APPRENTICE_BUFF_NAME:
    TVShowConvertInternationalString(
        text, GetApprenticeNameInLanguage(PLAYER_APPRENTICE.id, GAME_LANGUAGE),
        GAME_LANGUAGE);
  ...
  case APPRENTICE_BUFF_LEAD_MON_SPECIES:
    ...
    StringCopy(stringDst, gSpeciesNames[gApprentices[PLAYER_APPRENTICE.id]
                                            .species[speciesArrayId]]);
    break;
  }
}
...
static void Script_ApprenticeOpenBagMenu(void) { ApprenticeOpenBagMenu(); }
...
// Apprentice is always saved in the first slot. Pre-existing Apprentices are
// moved by ShiftSavedApprentices
static void SaveApprentice(void) {
  u8 i;
  ...
  for (i = 0; i < TRAINER_ID_LENGTH; i++)
    gSaveBlock2Ptr->apprentices[0].playerId[i] =
        gSaveBlock2Ptr->playerTrainerId[i];

  StringCopy(gSaveBlock2Ptr->apprentices[0].playerName,
...
static void SetSavedApprenticeTrainerGfxId(void) {
  ...
  for (i = 0; i < ARRAY_COUNT(gTowerMaleFacilityClasses) &&
              gTowerMaleFacilityClasses[i] != class;
       i++)
    ;
  ...
  for (i = 0; i < ARRAY_COUNT(gTowerFemaleFacilityClasses) &&
              gTowerFemaleFacilityClasses[i] != class;
       i++)
    ;
...
// In all cases theres a conditional for VAR_0x8004 right after the call to
// these functions
static void GetShouldCheckApprenticeGone(void) { gSpecialVar_0x8004 = TRUE; }

static void GetShouldApprenticeLeave(void) { gSpecialVar_0x8004 = TRUE; }
...
static void Task_ExecuteFuncAfterButtonPress(u8 taskId) {
  if (JOY_NEW(A_BUTTON) || JOY_NEW(B_BUTTON)) {
    gApprenticeFunc = (void *)(u32)(((u16)gTasks[taskId].data[0] |
...
static void UNUSED ExecuteFollowupFuncAfterButtonPress(TaskFunc task) {
  ...
  SetTaskFuncWithFollowupFunc(taskId, Task_SwitchToFollowupFuncAfterButtonPress,
                              task);
}
