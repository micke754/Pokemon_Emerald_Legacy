...
static bool32 CopyPokenavListMenuTemplate(struct PokenavListSub *,
                                          const struct BgTemplate *,
                                          struct PokenavListTemplate *, s32);
static void InitPokenavListWindowState(struct PokenavListWindowState *,
...
static void CreateListArrowSprites(struct PokenavListWindowState *,
                                   struct PokenavListSub *);
static void LoadListArrowGfx(void);
static void PrintMatchCallFlavorText(struct PokenavListWindowState *,
                                     struct PokenavListSub *, u32);
static void PrintMatchCallFieldNames(struct PokenavListSub *, u32);
static void PrintMatchCallListTrainerName(struct PokenavListWindowState *,
                                          struct PokenavListSub *);
static void PrintCheckPageTrainerName(struct PokenavListWindowState *,
                                      struct PokenavListSub *);
...
static void InitListItems(struct PokenavListWindowState *,
                          struct PokenavListSub *);
...
static const u16 sListArrow_Pal[] =
    INCBIN_U16("graphics/pokenav/list_arrows.gbapal");
static const u32 sListArrow_Gfx[] =
    INCBIN_U32("graphics/pokenav/list_arrows.4bpp.lz");

static EWRAM_DATA u32 sMoveWindowDownIndex = 0; // Read, but pointlessly

bool32 CreatePokenavList(const struct BgTemplate *bgTemplate,
                         struct PokenavListTemplate *listTemplate,
                         s32 tileOffset) {
  struct PokenavList *list =
      AllocSubstruct(POKENAV_SUBSTRUCT_LIST, sizeof(struct PokenavList));
  if (list == NULL)
    return FALSE;

  InitPokenavListWindowState(&list->windowState, listTemplate);
  if (!CopyPokenavListMenuTemplate(&list->sub, bgTemplate, listTemplate,
                                   tileOffset))
    return FALSE;
...
static void InitPokenavListBg(struct PokenavList *list) {
  u16 tileNum =
      (list->sub.listWindow.fillValue << 12) | list->sub.listWindow.tileOffset;
  BgDmaFill(list->sub.listWindow.bg, PIXEL_FILL(1),
            list->sub.listWindow.tileOffset, 1);
  BgDmaFill(list->sub.listWindow.bg, PIXEL_FILL(4),
            list->sub.listWindow.tileOffset + 1, 1);
  SetBgTilemapBuffer(list->sub.listWindow.bg, list->tilemapBuffer);
  FillBgTilemapBufferRect_Palette0(list->sub.listWindow.bg, tileNum, 0, 0, 32,
                                   32);
  ChangeBgY(list->sub.listWindow.bg, 0, BG_COORD_SET);
  ChangeBgX(list->sub.listWindow.bg, 0, BG_COORD_SET);
  ChangeBgY(list->sub.listWindow.bg, list->sub.listWindow.y << 11,
            BG_COORD_SUB);
...
static void InitListItems(struct PokenavListWindowState *windowState,
  ...
  PrintListItems(windowState->listPtr, windowState->windowTopIndex, numToPrint,
                 windowState->listItemSize, 0, subPtr);
}

static void PrintListItems(void *listPtr, u32 topIndex, u32 numItems,
                           u32 itemSize, u32 printStart,
...
static u32 LoopedTask_PrintListItems(s32 state) {
  ...
  switch (state) {
  case 0:
    row = (listSub->listWindow.unkA + listSub->listWindow.numPrinted +
           listSub->printStart) &
          0xF;
    listSub->bufferItemFunc(listSub->listPtr, listSub->itemTextBuffer);
    if (listSub->iconDrawFunc != NULL)
      listSub->iconDrawFunc(listSub->listWindow.windowId, listSub->printIndex,
                            row);

    AddTextPrinterParameterized(
        listSub->listWindow.windowId, listSub->listWindow.fontId,
        listSub->itemTextBuffer, 8, (row << 4) + 1, TEXT_SKIP_DRAW, NULL);
...
static bool32 ShouldShowDownArrow(void) {
  ...
  return (windowState->windowTopIndex + windowState->entriesOnscreen <
          windowState->listLength);
}
...
static void MoveListWindow(s32 delta, bool32 printItems) {
  ...
  if (delta < 0) {
    ...
    if (printItems)
      PrintListItems(windowState->listPtr, windowState->windowTopIndex + delta,
                     delta * -1, windowState->listItemSize, delta, &list->sub);
  } else if (printItems) {
    s32 index = sMoveWindowDownIndex =
        windowState->windowTopIndex + windowState->entriesOnscreen;
    if (index + delta >= windowState->listLength)
      delta = windowState->listLength - index;

    PrintListItems(windowState->listPtr, index, delta,
                   windowState->listItemSize, windowState->entriesOnscreen,
                   &list->sub);
  }
...
static u32 LoopedTask_MoveListWindow(s32 state) {
  ...
  case 1:
    ...
    if (subPtr->bgMoveType == BG_COORD_SUB) {
      if ((oldY > subPtr->endBgY || oldY <= subPtr->startBgY) &&
          newY <= subPtr->endBgY)
        finished = TRUE;
    } else // BG_COORD_ADD
    {
    ...
    if (finished) {
      subPtr->listWindow.unkA =
          (subPtr->listWindow.unkA + subPtr->moveDelta) & 0xF;
...
int PokenavList_MoveCursorDown(void) {
  ...
  if (windowState->windowTopIndex + windowState->selectedIndexOffset >=
      windowState->listLength - 1)
    return 0;
...
void PokenavList_DrawCurrentItemIcon(void) {
  struct PokenavList *list = GetSubstructPtr(POKENAV_SUBSTRUCT_LIST);
  struct PokenavListWindowState *windowState = &list->windowState;
  list->sub.iconDrawFunc(
      list->sub.listWindow.windowId,
      windowState->windowTopIndex + windowState->selectedIndexOffset,
      (list->sub.listWindow.unkA + windowState->selectedIndexOffset) & 0xF);
  ...
  case 2:
    if (!IsDma3ManagerBusyWithBgCopy()) {
      ...
      if (list->windowState.selectedIndexOffset != 0)
        EraseListEntry(&list->sub.listWindow, list->eraseIndex,
                       list->windowState.selectedIndexOffset);
...
static u32 LoopedTask_PrintCheckPageInfo(s32 state) {
  ...
  case 1:
    ...
    break;
  case 2:
    PrintMatchCallFlavorText(&list->windowState, &list->sub,
                             CHECK_PAGE_STRATEGY);
    break;
  case 3:
...
static u32 LoopedTask_ReshowListFromCheckPage(s32 state) {
  ...
  case 1:
    ...
    *ptr = 0;
    if (windowState->listLength <= windowState->entriesOnscreen) {
      ...
      if (windowState->windowTopIndex + windowState->entriesOnscreen >
          windowState->listLength) {
        s32 r4 = windowState->windowTopIndex + windowState->entriesOnscreen -
                 windowState->listLength;
        r5 = -r4;
        EraseListEntry(&subPtr->listWindow, r5, r4);
        windowState->selectedIndexOffset = r4;
  ...
  case 3:
    ...
    return LT_PAUSE;
  case 4:
    PrintListItems(windowState->listPtr,
                   windowState->windowTopIndex + list->eraseIndex, 1,
                   windowState->listItemSize, list->eraseIndex, &list->sub);
    return LT_INC_AND_PAUSE;
  case 5:
    if (IsPrintListItemsTaskActive())
      return LT_PAUSE;
    if (++list->eraseIndex >= windowState->listLength ||
        list->eraseIndex >= windowState->entriesOnscreen)
      return LT_INC_AND_CONTINUE;
    return LT_SET_STATE(4);
  case 6:
...
static void EraseListEntry(struct PokenavListMenuWindow *listWindow, s32 offset,
                           s32 entries) {
  u8 *tileData =
      (u8 *)GetWindowAttribute(listWindow->windowId, WINDOW_TILE_DATA);
...
// Pointless
static void SetListMarginTile(struct PokenavListMenuWindow *listWindow,
                              bool32 draw) {
  u16 var;
  u16 *tilemapBuffer = (u16 *)GetBgTilemapBuffer(
      GetWindowAttribute(listWindow->windowId, WINDOW_BG));
...
static void PrintCheckPageTrainerName(struct PokenavListWindowState *state,
                                      struct PokenavListSub *list) {
  u8 colors[3] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_DARK_GRAY,
                  TEXT_COLOR_LIGHT_RED};

  list->bufferItemFunc(state->listPtr +
                           state->listItemSize * state->windowTopIndex,
                       list->itemTextBuffer);
  list->iconDrawFunc(list->listWindow.windowId, state->windowTopIndex,
                     list->listWindow.unkA);
  FillWindowPixelRect(list->listWindow.windowId, PIXEL_FILL(4), 0,
                      list->listWindow.unkA * 16, list->listWindow.width * 8,
                      16);
  AddTextPrinterParameterized3(list->listWindow.windowId,
                               list->listWindow.fontId, 8,
                               (list->listWindow.unkA * 16) + 1, colors,
                               TEXT_SKIP_DRAW, list->itemTextBuffer);
  SetListMarginTile(&list->listWindow, TRUE);
  CopyWindowRectToVram(list->listWindow.windowId, COPYWIN_FULL, 0,
                       list->listWindow.unkA * 2, list->listWindow.width, 2);
}

// Print the trainer's name and title for the list (to replace the check page
// name and title, which has a red background)
static void PrintMatchCallListTrainerName(struct PokenavListWindowState *state,
                                          struct PokenavListSub *list) {
  list->bufferItemFunc(state->listPtr +
                           state->listItemSize * state->windowTopIndex,
                       list->itemTextBuffer);
  FillWindowPixelRect(list->listWindow.windowId, PIXEL_FILL(1), 0,
                      list->listWindow.unkA * 16, list->listWindow.width * 8,
                      16);
  AddTextPrinterParameterized(
      list->listWindow.windowId, list->listWindow.fontId, list->itemTextBuffer,
      8, list->listWindow.unkA * 16 + 1, TEXT_SKIP_DRAW, NULL);
...
static void PrintMatchCallFieldNames(struct PokenavListSub *list, u32 fieldId) {
  ...
  FillWindowPixelRect(list->listWindow.windowId, PIXEL_FILL(1), 0, top << 4,
                      list->listWindow.width, 16);
  AddTextPrinterParameterized3(list->listWindow.windowId, FONT_NARROW, 2,
                               (top << 4) + 1, colors, TEXT_SKIP_DRAW,
                               fieldNames[fieldId]);
  CopyWindowRectToVram(list->listWindow.windowId, COPYWIN_GFX, 0, top << 1,
                       list->listWindow.width, 2);
}

static void PrintMatchCallFlavorText(struct PokenavListWindowState *windowState,
                                     struct PokenavListSub *list,
                                     u32 checkPageEntry) {
  ...
  const u8 *str =
      GetMatchCallFlavorText(windowState->windowTopIndex, checkPageEntry);

  if (str != NULL) {
    FillWindowTilesByRow(list->listWindow.windowId, 1, r6 * 2,
                         list->listWindow.width - 1, 2);
    AddTextPrinterParameterized(list->listWindow.windowId, FONT_NARROW, str, 2,
                                (r6 << 4) + 1, TEXT_SKIP_DRAW, NULL);
    CopyWindowRectToVram(list->listWindow.windowId, COPYWIN_GFX, 0, r6 * 2,
                         list->listWindow.width, 2);
  }
}
...
static const struct OamData sOamData_RightArrow = {.y = 0,
                                                   .affineMode =
                                                       ST_OAM_AFFINE_OFF,
...
static void LoadListArrowGfx(void) {
  ...
  for (i = 0, ptr = sListArrowSpriteSheets;
       i < ARRAY_COUNT(sListArrowSpriteSheets); ptr++, i++)
    LoadCompressedSpriteSheet(ptr);

  Pokenav_AllocAndLoadPalettes(sListArrowPalettes);
}

static void CreateListArrowSprites(struct PokenavListWindowState *windowState,
                                   struct PokenavListSub *list) {
  u32 spriteId;
  s16 x;

  spriteId =
      CreateSprite(&sSpriteTemplate_RightArrow, list->listWindow.x * 8 + 3,
                   (list->listWindow.y + 1) * 8, 7);
  list->rightArrow = &gSprites[spriteId];

  x = list->listWindow.x * 8 + (list->listWindow.width - 1) * 4;
  spriteId = CreateSprite(
      &sSpriteTemplate_UpDownArrow, x,
      list->listWindow.y * 8 + windowState->entriesOnscreen * 16, 7);
  list->downArrow = &gSprites[spriteId];
  list->downArrow->oam.tileNum += 2;
  list->downArrow->callback = SpriteCB_DownArrow;

  spriteId =
      CreateSprite(&sSpriteTemplate_UpDownArrow, x, list->listWindow.y * 8, 7);
...
static void InitPokenavListWindowState(struct PokenavListWindowState *dst,
...
static bool32 CopyPokenavListMenuTemplate(struct PokenavListSub *dest,
                                          const struct BgTemplate *bgTemplate,
                                          struct PokenavListTemplate *template,
                                          s32 tileOffset) {
