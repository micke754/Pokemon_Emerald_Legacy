...
void Task_LinkContest_StartCommunicationEm(u8 taskId) {
  ...
  case CONTEST_CATEGORY_BEAUTY:
    gHighestRibbonRank = GetMonData(&gPlayerParty[gContestMonPartyIndex],
  ...
  SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateMonsEm,
...
static void Task_LinkContest_StartCommunicateCategoryEm(u8 taskId) {
  SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateCategoryEm,
...
static void Task_LinkContest_SetUpContestEm(u8 taskId) {
  ...
  // If < 4 players and player is leader, set AI contestants based on rank and
  // game clear
  ...
  // Assign link leader. After initial communication all players will read data
  // only from them
  gContestLinkLeaderIndex = LinkContest_GetLeaderIndex(leaderIds);

  if (gNumLinkContestPlayers < CONTESTANT_COUNT)
    SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateAIMonsEm,
...
static void Task_LinkContest_CommunicateMonsEm(u8 taskId) {
  ...
  case 0:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(&gContestMons[gContestPlayerMonIndex],
