...
static const u8 sMenuActionIds_Field[] = {ACTION_USE_FIELD, ACTION_GIVE,
...
static const struct YesNoFuncTable sYesNoTossFuncions = {TossItem,
                                                         DontTossItem};
...
static const u8 sTextColors[][3] = {
    [COLORID_DARK_GRAY] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_DARK_GRAY,
                           TEXT_COLOR_LIGHT_GRAY},
    [COLORID_LIGHT_GRAY] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_LIGHT_GRAY,
                            TEXT_COLOR_WHITE},
    [COLORID_WHITE_BG] = {TEXT_COLOR_WHITE, TEXT_COLOR_DARK_GRAY,
                          TEXT_COLOR_LIGHT_GRAY}, // Unused
};
...
static const struct CompressedSpriteSheet sSpriteSheet_PyramidBag = {
    gBattlePyramidBag_Gfx, 0x0800, TAG_PYRAMID_BAG};
...
// If the player finishes a round at the Battle Pyramid with insufficient space
// in their Pyramid Bag to store the party's held items, they may choose items
// to toss in order to make room.
...
void GoToBattlePyramidBagMenu(u8 location, void (*exitCallback)(void)) {
  ...
  memset(gPyramidBagMenu->spriteIds, SPRITE_NONE,
         sizeof(gPyramidBagMenu->spriteIds));
  memset(gPyramidBagMenu->windowIds, WINDOW_NONE,
         sizeof(gPyramidBagMenu->windowIds));
...
static void CB2_LoadPyramidBagMenu(void) {
  while (MenuHelpers_ShouldWaitForLinkRecv() != TRUE &&
         LoadPyramidBagMenu() != TRUE && MenuHelpers_IsLinkActive() != TRUE)
    ;
}
...
static bool8 LoadPyramidBagGfx(void) {
  ...
  case 1:
    if (FreeTempTileDataBuffersIfPossible() != TRUE) {
      LZDecompressWram(gBattlePyramidBagTilemap,
                       gPyramidBagMenu->tilemapBuffer);
      gPyramidBagMenu->state++;
    }
    break;
  case 2:
    LoadCompressedPalette(gBattlePyramidBagInterface_Pal, BG_PLTT_ID(0),
                          PLTT_SIZE_4BPP);
    gPyramidBagMenu->state++;
    break;
  case 3:
...
static void SetBagItemsListTemplate(void) {
  u16 i;
  u16 *itemIds = gSaveBlock2Ptr->frontier.pyramidBag
                     .itemId[gSaveBlock2Ptr->frontier.lvlMode];
...
static void CopyBagItemName(u8 *dst, u16 itemId) {
  if (ItemId_GetPocket(itemId) == POCKET_BERRIES) {
    ConvertIntToDecimalStringN(gStringVar1, ITEM_TO_BERRY(itemId),
...
static void BagCursorMoved(s32 itemIndex, bool8 onInit, struct ListMenu *list) {
  ...
  if (gPyramidBagMenu->toSwapPos == POS_NONE) {
    ...
    if (itemIndex != LIST_CANCEL)
      ShowItemIcon(gSaveBlock2Ptr->frontier.pyramidBag
                       .itemId[gSaveBlock2Ptr->frontier.lvlMode][itemIndex],
                   gPyramidBagMenu->isAltIcon);
    else
      ShowItemIcon(ITEM_LIST_END,
                   gPyramidBagMenu->isAltIcon); // Show exit arrow if on Cancel
...
static void PrintItemQuantity(u8 windowId, u32 itemIndex, u8 y) {
  ...
  ConvertIntToDecimalStringN(
      gStringVar1,
      gSaveBlock2Ptr->frontier.pyramidBag
          .quantity[gSaveBlock2Ptr->frontier.lvlMode][itemIndex],
      STR_CONV_MODE_RIGHT_ALIGN, 2);
  StringExpandPlaceholders(gStringVar4, gText_xVar1);
  xAlign = GetStringRightAlignXOffset(FONT_NARROW, gStringVar4, 119);
  PyramidBagPrint_Quantity(windowId, gStringVar4, xAlign, y, 0, 0,
                           TEXT_SKIP_DRAW, COLORID_DARK_GRAY);
}
...
static void AddScrollArrows(void) {
  if (gPyramidBagMenu->scrollIndicatorsTaskId == TASK_NONE)
    gPyramidBagMenu->scrollIndicatorsTaskId =
...
static void SwapItems(u8 id1, u8 id2) {
  ...
  u16 *itemIds = gSaveBlock2Ptr->frontier.pyramidBag
                     .itemId[gSaveBlock2Ptr->frontier.lvlMode];
  u8 *quantities = gSaveBlock2Ptr->frontier.pyramidBag
                       .quantity[gSaveBlock2Ptr->frontier.lvlMode];
...
void UpdatePyramidBagCursorPos(void) {
  if (gPyramidBagMenuState.scrollPosition != 0 &&
      gPyramidBagMenuState.scrollPosition + gPyramidBagMenu->listMenuMaxShown >
          gPyramidBagMenu->listMenuCount)
    gPyramidBagMenuState.scrollPosition =
        gPyramidBagMenu->listMenuCount - gPyramidBagMenu->listMenuMaxShown;

  if (gPyramidBagMenuState.scrollPosition +
          gPyramidBagMenuState.cursorPosition >=
      gPyramidBagMenu->listMenuCount) {
...
static void InitPyramidBagScroll(void) {
  ...
  if (gPyramidBagMenuState.cursorPosition > 4) {
    for (i = 0; i <= gPyramidBagMenuState.cursorPosition - 4; i++) {
      if (gPyramidBagMenuState.scrollPosition +
              gPyramidBagMenu->listMenuMaxShown ==
          gPyramidBagMenu->listMenuCount)
        break;
...
static void PrintSelectorArrowAtPos(u8 y, u8 colorId) {
  if (colorId == COLORID_NONE) // If 'no color', erase arrow
    FillWindowPixelRect(WIN_LIST, PIXEL_FILL(0), 0, y,
...
static void Task_ClosePyramidBag(u8 taskId) {
  s16 *data = gTasks[taskId].data;
  if (!gPaletteFade.active) {
    DestroyListMenuTask(tListTaskId, &gPyramidBagMenuState.scrollPosition,
                        &gPyramidBagMenuState.cursorPosition);
...
static void Task_HandlePyramidBagInput(u8 taskId) {
  ...
  if (JOY_NEW(SELECT_BUTTON)) {
    if (gPyramidBagMenuState.location != PYRAMIDBAG_LOC_PARTY) {
      ListMenuGetScrollAndRow(tListTaskId, &gPyramidBagMenuState.scrollPosition,
                              &gPyramidBagMenuState.cursorPosition);
      if (gPyramidBagMenuState.scrollPosition +
              gPyramidBagMenuState.cursorPosition !=
    ...
    default:
      ...
      tListPos = listId;
      tQuantity = gSaveBlock2Ptr->frontier.pyramidBag
...
static void OpenContextMenu(u8 taskId) {
  ...
  case PYRAMIDBAG_LOC_BATTLE:
    if (ItemId_GetBattleUsage(gSpecialVar_ItemId)) {
      ...
      gPyramidBagMenu->menuActionsCount =
          ARRAY_COUNT(sMenuActionIds_BattleCannotUse);
    }
    break;
  ...
  if (gPyramidBagMenu->menuActionsCount ==
      2 * 2) // Assumes any non 2x2 menu is single-row
...
static void PrintMenuActionText_SingleRow(u8 windowId) {
  PrintMenuActionTexts(windowId, FONT_NARROW, 8, 1, 0, 0x10,
                       gPyramidBagMenu->menuActionsCount, sMenuActions,
                       gPyramidBagMenu->menuActionIds);
  InitMenuInUpperLeftCornerNormal(windowId, gPyramidBagMenu->menuActionsCount,
                                  0);
}

static void PrintMenuActionText_MultiRow(u8 windowId, u8 horizontalCount,
                                         u8 verticalCount) {
  PrintMenuActionGrid(windowId, FONT_NARROW, 8, 1, 56, horizontalCount,
                      verticalCount, sMenuActions,
                      gPyramidBagMenu->menuActionIds);
...
static void HandleMenuActionInput_2x2(u8 taskId) {
  if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE) {
    ...
    if (JOY_NEW(DPAD_UP)) {
      ...
      if (id < gPyramidBagMenu->menuActionsCount - 2 &&
          IsValidMenuAction(id + 2)) {
        PlaySE(SE_SELECT);
        ChangeMenuGridCursorPosition(MENU_CURSOR_DELTA_NONE,
...
static void BagAction_UseOnField(u8 taskId) {
  ...
  if (pocketId == POCKET_KEY_ITEMS || pocketId == POCKET_POKE_BALLS ||
      pocketId == POCKET_TM_HM || ItemIsMail(gSpecialVar_ItemId) == TRUE) {
    CloseMenuActionWindow();
    DisplayItemMessageInBattlePyramid(taskId, gText_DadsAdvice,
...
static void AskConfirmToss(u8 taskId) {
  ...
  CopyItemName(gSpecialVar_ItemId, gStringVar1);
  ConvertIntToDecimalStringN(gStringVar2, tNumToToss, STR_CONV_MODE_LEFT_ALIGN,
                             2);
...
static void ShowNumToToss(void) {
  ...
  AddTextPrinterParameterized(WIN_TOSS_NUM, FONT_NORMAL, gStringVar4, x, 2, 0,
                              NULL);
}
...
static void Task_TossItem(u8 taskId) {
  ...
  if (JOY_NEW(A_BUTTON | B_BUTTON)) {
    ...
    SetBagItemsListTemplate();
    tListTaskId =
        ListMenuInit(&gMultiuseListMenuTemplate, *scrollOffset, *selectedRow);
    ScheduleBgCopyTilemapToVram(0);
    SetTaskToMainPyramidBagInputHandler(taskId);
  }
}
...
static void BagAction_Give(u8 taskId) {
  ...
  if (ItemIsMail(gSpecialVar_ItemId) == TRUE) {
    DisplayItemMessageInBattlePyramid(taskId, gText_CantWriteMail,
...
static void ShowCantHoldMessage(u8 taskId) {
  ...
  DisplayItemMessageInBattlePyramid(taskId, gStringVar4,
                                    Task_WaitCloseErrorMessage);
}
...
static void Task_BeginItemSwap(u8 taskId) {
  ...
  tListPos =
      gPyramidBagMenuState.scrollPosition + gPyramidBagMenuState.cursorPosition;
  gPyramidBagMenu->toSwapPos = tListPos;
  ListMenuSetUnkIndicatorsStructField(tListTaskId, 0x10, 1);
  CopyItemName(gSaveBlock2Ptr->frontier.pyramidBag
                   .itemId[gSaveBlock2Ptr->frontier.lvlMode][tListPos],
               gStringVar1);
...
void TryStoreHeldItemsInPyramidBag(void) {
  ...
  memcpy(newItems,
         gSaveBlock2Ptr->frontier.pyramidBag
             .itemId[gSaveBlock2Ptr->frontier.lvlMode],
         PYRAMID_BAG_ITEMS_COUNT * sizeof(*newItems));
  memcpy(newQuantities,
         gSaveBlock2Ptr->frontier.pyramidBag
             .quantity[gSaveBlock2Ptr->frontier.lvlMode],
         PYRAMID_BAG_ITEMS_COUNT * sizeof(*newQuantities));
  for (i = 0; i < FRONTIER_PARTY_SIZE; i++) {
    ...
    if (heldItem != ITEM_NONE && !AddBagItem(heldItem, 1)) {
      ...
      memcpy(gSaveBlock2Ptr->frontier.pyramidBag
                 .itemId[gSaveBlock2Ptr->frontier.lvlMode],
             newItems, PYRAMID_BAG_ITEMS_COUNT * sizeof(*newItems));
      memcpy(gSaveBlock2Ptr->frontier.pyramidBag
                 .quantity[gSaveBlock2Ptr->frontier.lvlMode],
             newQuantities, PYRAMID_BAG_ITEMS_COUNT * sizeof(*newQuantities));
      Free(newItems);
      Free(newQuantities);
...
static void PyramidBagPrint(u8 windowId, const u8 *src, u8 x, u8 y,
                            u8 letterSpacing, u8 lineSpacing, u8 speed,
                            u8 colorTableId) {
  AddTextPrinterParameterized4(windowId, FONT_NORMAL, x, y, letterSpacing,
                               lineSpacing, sTextColors[colorTableId], speed,
                               src);
}

static void PyramidBagPrint_Quantity(u8 windowId, const u8 *src, u8 x, u8 y,
                                     u8 letterSpacing, u8 lineSpacing, u8 speed,
                                     u8 colorTableId) {
  AddTextPrinterParameterized4(windowId, FONT_NARROW, x, y, letterSpacing,
                               lineSpacing, sTextColors[colorTableId], speed,
                               src);
}
...
static void CreatePyramidBagYesNo(u8 taskId,
                                  const struct YesNoFuncTable *yesNoTable) {
  CreateYesNoMenuWithCallbacks(taskId,
                               &sWindowTemplates_MenuActions[MENU_WIN_YESNO], 1,
                               0, 2, 1, 0xE, yesNoTable);
}

void DisplayItemMessageInBattlePyramid(u8 taskId, const u8 *str,
                                       void (*callback)(u8 taskId)) {
  FillWindowPixelBuffer(WIN_MSG, PIXEL_FILL(1));
  DisplayMessageAndContinueTask(taskId, WIN_MSG, 0xA, 0xD, FONT_NORMAL,
...
static void FreeItemIconSprite(u8 spriteArrId) {
  ...
  if (*spriteId != SPRITE_NONE) {
    // spriteArrId is PBAG_SPRITE_ITEM_ICON / PBAG_SPRITE_ITEM_ICON_ALT here
    // (1-2) so tag will be TAG_ITEM_ICON / TAG_ITEM_ICON_ALT
...
static void ShakePyramidBag(void) {
  struct Sprite *sprite =
      &gSprites[gPyramidBagMenu->spriteIds[PBAG_SPRITE_BAG]];
...
static void ShowItemIcon(u16 itemId, bool8 isAlt) {
  ...
  if (*spriteId == SPRITE_NONE) {
    ...
    itemSpriteId =
        AddItemIconSprite(TAG_ITEM_ICON + isAlt, TAG_ITEM_ICON + isAlt, itemId);
...
static void CreateSwapLine(void) {
  CreateSwapLineSprites(
      &gPyramidBagMenu->spriteIds[PBAG_SPRITE_SWAP_LINE_START],
...
static void SetSwapLineInvisibility(bool8 invisible) {
  SetSwapLineSpritesInvisibility(
      &gPyramidBagMenu->spriteIds[PBAG_SPRITE_SWAP_LINE_START],
      NUM_SWAP_LINE_SPRITES, invisible);
}

static void UpdateSwapLinePos(u8 y) {
  UpdateSwapLineSpritesPos(
      &gPyramidBagMenu->spriteIds[PBAG_SPRITE_SWAP_LINE_START],
      NUM_SWAP_LINE_SPRITES | SWAP_LINE_HAS_MARGIN, 120, (y + 1) * 16);
}
