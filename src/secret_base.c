...
enum {
  UNREGISTERED,
  REGISTERED,
  NEW, // Temporary, so new secret bases mixed at the same time don't overwrite
       // each other
};
...
// mapNum, warpId, x, y
// x, y positions are for when the player warps in for the first time (in front
// of the computer)
static const u8 sSecretBaseEntrancePositions[NUM_SECRET_BASE_GROUPS * 4] = {
    [SECRET_BASE_RED_CAVE1] = MAP_NUM(SECRET_BASE_RED_CAVE1),
    0,
    1,
    3,
    [SECRET_BASE_RED_CAVE2] = MAP_NUM(SECRET_BASE_RED_CAVE2),
    0,
    5,
    9,
    [SECRET_BASE_RED_CAVE3] = MAP_NUM(SECRET_BASE_RED_CAVE3),
    0,
    1,
    3,
    [SECRET_BASE_RED_CAVE4] = MAP_NUM(SECRET_BASE_RED_CAVE4),
    0,
    7,
    13,
    [SECRET_BASE_BROWN_CAVE1] = MAP_NUM(SECRET_BASE_BROWN_CAVE1),
    0,
    2,
    3,
    [SECRET_BASE_BROWN_CAVE2] = MAP_NUM(SECRET_BASE_BROWN_CAVE2),
    0,
    9,
    2,
    [SECRET_BASE_BROWN_CAVE3] = MAP_NUM(SECRET_BASE_BROWN_CAVE3),
    0,
    13,
    4,
    [SECRET_BASE_BROWN_CAVE4] = MAP_NUM(SECRET_BASE_BROWN_CAVE4),
    0,
    1,
    2,
    [SECRET_BASE_BLUE_CAVE1] = MAP_NUM(SECRET_BASE_BLUE_CAVE1),
    0,
    1,
    3,
    [SECRET_BASE_BLUE_CAVE2] = MAP_NUM(SECRET_BASE_BLUE_CAVE2),
    0,
    1,
    2,
    [SECRET_BASE_BLUE_CAVE3] = MAP_NUM(SECRET_BASE_BLUE_CAVE3),
    0,
    3,
    15,
    [SECRET_BASE_BLUE_CAVE4] = MAP_NUM(SECRET_BASE_BLUE_CAVE4),
    0,
    3,
    14,
    [SECRET_BASE_YELLOW_CAVE1] = MAP_NUM(SECRET_BASE_YELLOW_CAVE1),
    0,
    9,
    3,
    [SECRET_BASE_YELLOW_CAVE2] = MAP_NUM(SECRET_BASE_YELLOW_CAVE2),
    0,
    8,
    7,
    [SECRET_BASE_YELLOW_CAVE3] = MAP_NUM(SECRET_BASE_YELLOW_CAVE3),
    0,
    3,
    6,
    [SECRET_BASE_YELLOW_CAVE4] = MAP_NUM(SECRET_BASE_YELLOW_CAVE4),
    0,
    5,
    9,
    [SECRET_BASE_TREE1] = MAP_NUM(SECRET_BASE_TREE1),
    0,
    2,
    3,
    [SECRET_BASE_TREE2] = MAP_NUM(SECRET_BASE_TREE2),
    0,
    5,
    6,
    [SECRET_BASE_TREE3] = MAP_NUM(SECRET_BASE_TREE3),
    0,
    15,
    3,
    [SECRET_BASE_TREE4] = MAP_NUM(SECRET_BASE_TREE4),
    0,
    4,
    10,
    [SECRET_BASE_SHRUB1] = MAP_NUM(SECRET_BASE_SHRUB1),
    0,
    3,
    3,
    [SECRET_BASE_SHRUB2] = MAP_NUM(SECRET_BASE_SHRUB2),
    0,
    1,
    2,
    [SECRET_BASE_SHRUB3] = MAP_NUM(SECRET_BASE_SHRUB3),
    0,
    7,
    8,
    [SECRET_BASE_SHRUB4] = MAP_NUM(SECRET_BASE_SHRUB4),
    0,
    9,
    6,
};
...
static void SetCurSecretBaseId(void) { sCurSecretBaseId = gSpecialVar_0x8004; }
...
static u8 GetSecretBaseTypeInFrontOfPlayer_(void) {
  ...
  if (behavior == MB_SECRET_BASE_SPOT_RED_CAVE ||
      behavior == MB_SECRET_BASE_SPOT_RED_CAVE_OPEN)
  ...
  if (behavior == MB_SECRET_BASE_SPOT_BLUE_CAVE ||
      behavior == MB_SECRET_BASE_SPOT_BLUE_CAVE_OPEN)
  ...
  if (behavior == MB_SECRET_BASE_SPOT_SHRUB ||
      behavior == MB_SECRET_BASE_SPOT_SHRUB_OPEN)
...
static void FindMetatileIdMapCoords(s16 *x, s16 *y, u16 metatileId) {
  ...
  for (j = 0; j < mapLayout->height; j++) {
    for (i = 0; i < mapLayout->width; i++) {
      if ((mapLayout->map[j * mapLayout->width + i] &
           MAPGRID_METATILE_ID_MASK) == metatileId) {
        *x = i;
        *y = j;
        return;
      }
...
void ToggleSecretBaseEntranceMetatile(void) {
  ...
  for (i = 0; i < ARRAY_COUNT(sSecretBaseEntranceMetatiles); i++) {
    if (sSecretBaseEntranceMetatiles[i].openMetatileId == metatileId) {
      MapGridSetMetatileIdAt(x, y,
                             sSecretBaseEntranceMetatiles[i].closedMetatileId |
                                 MAPGRID_COLLISION_MASK);
      CurrentMapDrawMetatileAt(x, y);
      return;
    }
...
void SetPlayerSecretBase(void) {
  ...
  for (i = 0; i < TRAINER_ID_LENGTH; i++)
    gSaveBlock1Ptr->secretBases[0].trainerId[i] =
        gSaveBlock2Ptr->playerTrainerId[i];
...
static void SetSecretBaseWarpDestination(void) {
  ...
  SetWarpDestinationToMapWarp(MAP_GROUP(SECRET_BASE_RED_CAVE1),
                              GET_BASE_MAP_NUM(secretBaseGroup),
                              GET_BASE_WARP_ID(secretBaseGroup));
}
  ...
  if (CurMapIsSecretBase()) {
    ...
    for (x = 0; x < DECOR_MAX_SECRET_BASE; x++) {
      if (decorations[x] > 0 && decorations[x] <= NUM_DECORATIONS &&
        ...
        ShowDecorationOnMap((decorPos[x] >> 4) + MAP_OFFSET,
                            (decorPos[x] & 0xF) + MAP_OFFSET, decorations[x]);
    }
...
void InitSecretBaseDecorationSprites(void) {
  ...
  for (i = 0; i < numDecorations; i++) {
    ...
    if (permission == DECORPERM_SPRITE) {
      for (objectEventId = 0;
           objectEventId < gMapHeader.events->objectEventCount;
           objectEventId++) {
      ...
      metatileBehavior = MapGridGetMetatileBehaviorAt(
          gSpecialVar_0x8006 + MAP_OFFSET, gSpecialVar_0x8007 + MAP_OFFSET);
...
void SetSecretBaseOwnerGfxId(void) {
  VarSet(VAR_OBJ_GFX_ID_F, sSecretBaseOwnerGfxIds[GetSecretBaseOwnerType(
                               VarGet(VAR_CURRENT_SECRET_BASE))]);
}
...
static u8 *GetSecretBaseName(u8 *dest, u8 secretBaseIdx) {
  *StringCopyN(
  ...
  ConvertInternationalString(
      dest, gSaveBlock1Ptr->secretBases[secretBaseIdx].language);
...
void CopyCurSecretBaseOwnerName_StrVar1(void) {
  ...
  *StringCopyN(gStringVar1, name, GetNameLength(name)) = EOS;
  ConvertInternationalString(
      gStringVar1, gSaveBlock1Ptr->secretBases[secretBaseIdx].language);
}
    ...
    for (i = 0; i < PARTY_SIZE; i++) {
      ...
      if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES) != SPECIES_NONE &&
        ...
        party->heldItems[partyId] =
            GetMonData(&gPlayerParty[i], MON_DATA_HELD_ITEM);
...
static void ClosePlayerSecretBaseEntrance(void) {
  ...
  for (i = 0; i < events->bgEventCount; i++) {
    if (events->bgEvents[i].kind == BG_EVENT_SECRET_BASE &&
      ...
      metatileId = MapGridGetMetatileIdAt(events->bgEvents[i].x + MAP_OFFSET,
      ...
      for (j = 0; j < ARRAY_COUNT(sSecretBaseEntranceMetatiles); j++) {
        if (sSecretBaseEntranceMetatiles[j].openMetatileId == metatileId) {
          MapGridSetMetatileIdAt(
              events->bgEvents[i].x + MAP_OFFSET,
              events->bgEvents[i].y + MAP_OFFSET,
              sSecretBaseEntranceMetatiles[j].closedMetatileId |
                  MAPGRID_COLLISION_MASK);
          break;
        }
      }
...
void ToggleCurSecretBaseRegistry(void) {
  gSaveBlock1Ptr->secretBases[VarGet(VAR_CURRENT_SECRET_BASE)].registryStatus ^=
      1;
...
static void Task_ShowSecretBaseRegistryMenu(u8 taskId) {
  ...
  if (tNumBases != 0) {
    ...
    DisplayItemMessageOnField(taskId, gText_NoRegistry,
...
static void RegistryMenu_OnCursorMove(s32 unused, bool8 flag,
...
static void FinalizeRegistryMenu(u8 taskId) {
  ...
  tListTaskId =
      ListMenuInit(&gMultiuseListMenuTemplate, tScrollOffset, tSelectedRow);
...
static void AddRegistryMenuScrollArrows(u8 taskId) {
  s16 *data = gTasks[taskId].data;
  tArrowTaskId = AddScrollIndicatorArrowPairParameterized(
      SCROLL_ARROW_UP, 188, 12, 148, tNumBases - tMaxShownItems,
      TAG_SCROLL_ARROW, TAG_SCROLL_ARROW, &tScrollOffset);
}
...
static void ShowRegistryMenuActions(u8 taskId) {
  ...
  PrintMenuTable(tActionWindowId, ARRAY_COUNT(sRegistryMenuActions),
                 sRegistryMenuActions);
  InitMenuInUpperLeftCornerNormal(tActionWindowId,
                                  ARRAY_COUNT(sRegistryMenuActions), 0);
  ScheduleBgCopyTilemapToVram(0);
...
void DeleteRegistry_Yes_Callback(u8 taskId) {
  ...
  BuildRegistryMenuItems(taskId);
  SetCursorWithinListBounds(&tScrollOffset, &tSelectedRow, tMaxShownItems,
                            tNumBases);
  FinalizeRegistryMenu(taskId);
  gTasks[taskId].func = HandleRegistryMenuInput;
}
...
static void DeleteRegistry_Yes(u8 taskId) {
  DisplayItemMessageOnField(taskId, gText_RegisteredDataDeleted,
...
void SetBattledOwnerFromResult(void) {
  gSaveBlock1Ptr->secretBases[VarGet(VAR_CURRENT_SECRET_BASE)]
      .battledOwnerToday = gSpecialVar_Result;
}
...
void GetSecretBaseOwnerAndState(void) {
  ...
  gSpecialVar_Result =
      gSaveBlock1Ptr->secretBases[secretBaseIdx].battledOwnerToday;
}
...
void SecretBasePerStepCallback(u8 taskId) {
  ...
  case 1:
    ...
    tPlayerX = x;
    tPlayerY = y;
    VarSet(VAR_SECRET_BASE_STEP_COUNTER,
    ...
    } else if (behavior == MB_SLIDE_SOUTH &&
               tileId == METATILE_SecretBase_Slide_SlideTop) {
      if (sInFriendSecretBase == TRUE) {
        VarSet(VAR_SECRET_BASE_HIGH_TV_FLAGS,
               VarGet(VAR_SECRET_BASE_HIGH_TV_FLAGS) | SECRET_BASE_USED_SLIDE);
        VarSet(VAR_SECRET_BASE_HIGH_TV_FLAGS,
...
static void SaveSecretBase(u8 secretBaseIdx, struct SecretBase *secretBase,
                           u32 version, u32 language) {
  int stringLength;
  u8 *name;
...
static bool8 SecretBasesHaveSameTrainerId(struct SecretBase *secretBase1,
                                          struct SecretBase *secretBase2) {
  u8 i;
  for (i = 0; i < TRAINER_ID_LENGTH; i++) {
...
static bool8 SecretBasesHaveSameTrainerName(struct SecretBase *sbr1,
                                            struct SecretBase *sbr2) {
  u8 i;
...
static bool8 SecretBasesBelongToSamePlayer(struct SecretBase *secretBase1,
                                           struct SecretBase *secretBase2) {
...
static u8 TrySaveFriendsSecretBase(struct SecretBase *secretBase, u32 version,
                                   u32 language) {
  s16 index;
...
static void SortSecretBasesByRegistryStatus(void) {
  ...
  for (i = 1; i < SECRET_BASES_COUNT - 1; i++) {
    for (j = i + 1; j < SECRET_BASES_COUNT; j++) {
      if ((secretBases[i].registryStatus == UNREGISTERED &&
           secretBases[j].registryStatus == REGISTERED) ||
          (secretBases[i].registryStatus == NEW &&
           secretBases[j].registryStatus != NEW)) {
...
static void TrySaveFriendsSecretBases(struct SecretBaseRecordMixer *mixer,
                                      u8 registryStatus) {
  u16 i;
  for (i = 1; i < SECRET_BASES_COUNT; i++) {
    if (mixer->secretBases[i].registryStatus == registryStatus)
      TrySaveFriendsSecretBase(&mixer->secretBases[i], mixer->version,
                               mixer->language);
  }
}
...
static bool8 SecretBaseBelongsToPlayer(struct SecretBase *secretBase) {
  ...
  if (secretBase->secretBaseId &&
      secretBase->gender != gSaveBlock2Ptr->playerGender)
    return FALSE;
  ...
  for (i = 0; i < PLAYER_NAME_LENGTH && (secretBase->trainerName[i] != EOS ||
...
static void DeleteFirstOldBaseFromPlayerInRecordMixingFriendsRecords(
    struct SecretBase *basesA, struct SecretBase *basesB,
...
static bool8 ClearDuplicateOwnedSecretBase(struct SecretBase *secretBase,
                                           struct SecretBase *secretBases,
                                           u8 idx) {
  u8 i;
    ...
    if (secretBases[i].secretBaseId != 0) {
      if (SecretBasesBelongToSamePlayer(secretBase, &secretBases[i]) == TRUE) {
        ...
        if (secretBase->numSecretBasesReceived >
            secretBases[i].numSecretBasesReceived) {
          ClearSecretBase(&secretBases[i]);
          return FALSE;
        }
...
static void ClearDuplicateOwnedSecretBases(struct SecretBase *playersBases,
                                           struct SecretBase *friendsBasesA,
                                           struct SecretBase *friendsBasesB,
                                           struct SecretBase *friendsBasesC) {
  u8 i;
  ...
  for (i = 1; i < SECRET_BASES_COUNT; i++) {
    if (playersBases[i].secretBaseId) {
      ...
      if (!ClearDuplicateOwnedSecretBase(&playersBases[i], friendsBasesA, i)) {
        if (!ClearDuplicateOwnedSecretBase(&playersBases[i], friendsBasesB,
                                           i)) {
...
static void TrySaveRegisteredDuplicate(struct SecretBase *base, u32 version,
                                       u32 language) {
...
static void TrySaveRegisteredDuplicates(struct SecretBaseRecordMixer *mixers) {
  ...
  for (i = 0; i < SECRET_BASES_COUNT; i++) {
    TrySaveRegisteredDuplicate(&mixers[0].secretBases[i], mixers[0].version,
                               mixers[0].language);
    TrySaveRegisteredDuplicate(&mixers[1].secretBases[i], mixers[1].version,
                               mixers[1].language);
    TrySaveRegisteredDuplicate(&mixers[2].secretBases[i], mixers[2].version,
                               mixers[2].language);
  }
}
...
static void SaveRecordMixBases(struct SecretBaseRecordMixer *mixers) {
  ...
  ClearDuplicateOwnedSecretBases(gSaveBlock1Ptr->secretBases,
                                 mixers[0].secretBases, mixers[1].secretBases,
  ...
  TrySaveFriendsSecretBase(mixers[0].secretBases, mixers[0].version,
                           mixers[0].language);
  TrySaveFriendsSecretBase(mixers[1].secretBases, mixers[1].version,
                           mixers[1].language);
  TrySaveFriendsSecretBase(mixers[2].secretBases, mixers[2].version,
                           mixers[2].language);
...
void ReceiveSecretBasesData(void *secretBases, size_t recordSize, u8 linkIdx) {
  ...
  if (FlagGet(FLAG_RECEIVED_SECRET_POWER)) {
    ...
    for (i = 1; i < SECRET_BASES_COUNT; i++) {
      // In the process of deleting duplicate bases, if a base the player has
      // registered is deleted it is flagged with the temporary toRegister flag,
      // so it can be re-registered after it has been newly saved
