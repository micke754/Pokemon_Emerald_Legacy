...
static void Task_LinkContest_InitFlags(u8 taskId) {
  ...
  if (gWirelessCommType == 1)
    gLinkContestFlags =
        LINK_CONTEST_FLAG_IS_LINK | LINK_CONTEST_FLAG_IS_WIRELESS;

  // Get number of players using Emerald/FRLG
  for (i = 0; i < gNumLinkContestPlayers &&
              (u32)(gLinkPlayers[i].version & 0xFF) - 1 > VERSION_RUBY - 1;
       i++)
    ;
...
void Task_LinkContest_CommunicateMonsRS(u8 taskId) {
  ...
  case 0:
    ...
    if (GetMultiplayerId() == 0) {
      if (IsLinkTaskFinished()) {
        memcpy(gBlockSendBuffer, &gContestMons[gContestPlayerMonIndex],
  ...
  case 1:
    ...
    if (LinkContest_GetBlockReceivedFromAllPlayers()) {
      for (i = 0; i < gNumLinkContestPlayers; i++) {
        memcpy(&gContestMons[i], gBlockRecvBuffer[i],
               sizeof(struct ContestPokemon));
        StripPlayerAndMonNamesForLinkContest(&gContestMons[i],
...
void Task_LinkContest_CommunicateMonIdxs(u8 taskId) {
  ...
  case 0:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(&gContestPlayerMonIndex,
                                sizeof(gContestPlayerMonIndex)) == TRUE)
        gTasks[taskId].tState++;
    }
    break;
  case 1:
  ...
  case 0:
    if (IsLinkTaskFinished()) {
      ...
      if (LinkContest_SendBlock(
              &eContestantStatus[gContestPlayerMonIndex].currMove,
...
void Task_LinkContest_CommunicateFinalStandings(u8 taskId) {
  ...
  case 0:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(gContestMonTotalPoints,
                                sizeof(gContestMonTotalPoints)) == 1)
        gTasks[taskId].tState++;
    }
    break;
  case 1:
  ...
  case 3:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(gContestMonAppealPointTotals,
                                sizeof(gContestMonAppealPointTotals)) == 1)
        gTasks[taskId].tState++;
    }
    break;
  case 4:
  ...
  case 6:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(gContestMonRound2Points,
                                sizeof(gContestMonRound2Points)) == 1)
        gTasks[taskId].tState++;
    }
    break;
  case 7:
  ...
  case 9:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(gContestFinalStandings,
                                sizeof(gContestFinalStandings)) == 1)
        gTasks[taskId].tState++;
    }
    break;
  case 10:
...
void Task_LinkContest_CommunicateAppealsState(u8 taskId) {
  ...
  case 0:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(eContestantStatus,
                                CONTESTANT_COUNT *
                                    sizeof(struct ContestantStatus)) == 1)
        gTasks[taskId].tState++;
    }
    break;
  case 1:
  ...
  case 3:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(gContestResources->appealResults,
                                sizeof(struct ContestAppealMoveResults)) == 1)
        gTasks[taskId].tState++;
    }
    break;
  case 4:
  ...
  case 6:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(gContestResources->excitement,
                                sizeof(struct ContestExcitement)) == 1)
        gTasks[taskId].tState++;
    }
    break;
  case 7:
  ...
  case 9:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(gContestantTurnOrder,
                                sizeof(gContestantTurnOrder)) == 1)
        gTasks[taskId].tState++;
    }
    break;
  case 10:
    if (LinkContest_GetBlockReceivedFromAllPlayers()) {
      memcpy(gContestantTurnOrder, gBlockRecvBuffer[gContestLinkLeaderIndex],
...
void Task_LinkContest_CommunicateRound1Points(u8 taskId) {
  ...
  case 0:
    if (IsLinkTaskFinished()) {
      if (LinkContest_SendBlock(gContestMonRound1Points,
                                sizeof(gContestMonRound1Points)) == 1)
        gTasks[taskId].tState++;
    }
    break;
  case 1:
