...
/*
    ...
    - rand:
        ...
        It's used in calculations for Feebas tiles, Slot Machines, and Match
   Call.
    - trendiness / maxTrendiness:
        ...
        "boring" (see below) it will lose trendiness over time until it reaches
   0, at which point it will stop being boring and gain trendiness until it
        reaches maxTrendiness (then it becomes boring again and the cycle
   repeats).
    - gainingTrendiness:
        This is a flag that determines whether a phrase should be gaining or
   losing trendiness. An NPC in Dewford Hall will comment on whether the current
   phrase is "boring" or not, and if it is gaining trendiness (or if it is still
   trendier than the last phrase) it is not boring. This field will always be
   TRUE for any new phrase submitted after the 1st submission.

    ## Saving trends ##
    Each time a potential trendy phrase is submitted, it is saved in
   gSaveBlock1Ptr->dewfordTrends[]. Up to SAVED_TRENDS_COUNT (5) trends may be
   saved at one time. The trends in this array are kept in sorted order from
   most trendy to least trendy. The current trendy phrase is always at
    gSaveBlock1Ptr->dewfordTrends[0]. If the player mixes records with another
   player, their own trends are replaced with their mixing partner's, unless the
   phrase is the same, in which case the version with a higher trendiness value
   is used (see ReceiveDewfordTrendData).

    ## TV Show ##
    If a submitted phrase is only trendier than 1 or none of the saved trends,
   it may trigger a TV Show called Trend Watcher (see TryPutTrendWatcherOnAir)
   that, ironically, spends the show talking about how the submitted phrase was
   not trendy.
...
static s16 GetSavedTrendIndex(struct DewfordTrend *, struct DewfordTrend *,
                              u16);
...
void InitDewfordTrend(void) {
  ...
  for (i = 0; i < SAVED_TRENDS_COUNT; i++) {
    gSaveBlock1Ptr->dewfordTrends[i].words[0] =
        GetRandomEasyChatWordFromGroup(EC_GROUP_CONDITIONS);

    if (Random() & 1)
      gSaveBlock1Ptr->dewfordTrends[i].words[1] =
  ...
  SortTrends(gSaveBlock1Ptr->dewfordTrends, SAVED_TRENDS_COUNT,
             SORT_MODE_NORMAL);
}
  ...
  if (days != 0) {
    ...
    for (i = 0; i < SAVED_TRENDS_COUNT; i++) {
      ...
      if ((u16)trendiness > trend->maxTrendiness) {
        ...
        // Trend has reached its max, becoming "boring" and start losing
        // trendiness
...
// Returns TRUE if the current trendy phrase was successfully changed to the
// given phrase Returns FALSE otherwise Regardless of whether or not the current
// trendy phrase was changed, the submitted phrase is always saved in
// gSaveBlock1Ptr->dewfordTrends
bool8 TrySetTrendyPhrase(u16 *phrase) {
  ...
  if (!IsPhraseInSavedTrends(phrase)) {
    ...
    for (i = 0; i < SAVED_TRENDS_COUNT; i++) {
      if (CompareTrends(&trend, &(gSaveBlock1Ptr->dewfordTrends[i]),
                        SORT_MODE_NORMAL)) {
        // New trend is "trendier" than dewfordTrend[i]
        // Shift other trends back to insert new trend
        u16 j = SAVED_TRENDS_COUNT - 1;
        while (j > i) {
          gSaveBlock1Ptr->dewfordTrends[j] =
              gSaveBlock1Ptr->dewfordTrends[j - 1];
          j--;
        }
...
#define BUFFER_SIZE                                                            \
  max(SAVED_TRENDS_SIZE *MAX_LINK_PLAYERS,                                     \
      0x100) // More space was allocated than needed

void ReceiveDewfordTrendData(struct DewfordTrend *linkedTrends, size_t size,
                             u8 unused) {
  u16 i, j, numTrends, players;
  struct DewfordTrend *linkedTrendsBuffer, *savedTrendsBuffer, *src, *dst,
      *temp;
  ...
  for (i = 0; i < players; i++)
    memcpy(&linkedTrendsBuffer[i * SAVED_TRENDS_COUNT],
           (u8 *)linkedTrends + i * size, SAVED_TRENDS_SIZE);
...
void BufferTrendyPhraseString(void) {
  struct DewfordTrend *trend =
      &gSaveBlock1Ptr->dewfordTrends[gSpecialVar_0x8004];
...
void IsTrendyPhraseBoring(void) {
  ...
  do {
    if (gSaveBlock1Ptr->dewfordTrends[0].trendiness -
            gSaveBlock1Ptr->dewfordTrends[1].trendiness >
        1)
      break;
...
void GetDewfordHallPaintingNameIndex(void) {
  gSpecialVar_Result = (gSaveBlock1Ptr->dewfordTrends[0].words[0] +
                        gSaveBlock1Ptr->dewfordTrends[0].words[1]) &
                       7;
}

// Returns TRUE if a > b (a is "trendier" than b), FALSE if a < b (b is
// "trendier" than a) How one trend is compared to the other depends on the mode
// In SORT_MODE_FULL if the trends are equal then TRUE is always returned,
// otherwise TRUE or FALSE is returned randomly
static bool8 CompareTrends(struct DewfordTrend *a, struct DewfordTrend *b,
                           u8 mode) {
  switch (mode) {
  ...
  // Invalid mode given, or trends are equal in SORT_MODE_NORMAL or
  // SORT_MODE_MAX_FIRST Randomly pick one of the phrases
  return Random() & 1;
}
...
static s16 GetSavedTrendIndex(struct DewfordTrend *savedTrends,
                              struct DewfordTrend *trend, u16 numSaved) {
  s16 i;
