...
static EWRAM_DATA u32 sAddressOffset =
    0; // For relative addressing in vgoto etc., used by saved scripts (e.g.
       // Mystery Event)
...
bool8 ScrCmd_nop(struct ScriptContext *ctx) { return FALSE; }

bool8 ScrCmd_nop1(struct ScriptContext *ctx) { return FALSE; }
...
// Note: addvar doesn't support adding from a variable in vanilla. If you were
// to add a VarGet() to the above, make sure you change the `addvar VAR_*, -1`
...
bool8 ScrCmd_warphole(struct ScriptContext *ctx) {
  ...
  if (mapGroup == MAP_GROUP(UNDEFINED) && mapNum == MAP_NUM(UNDEFINED))
    ...
    SetWarpDestination(mapGroup, mapNum, WARP_ID_NONE, x - MAP_OFFSET,
                       y - MAP_OFFSET);
  DoFallWarp();
  ResetInitialPlayerAvatarState();
  return TRUE;
...
static bool8 WaitForFanfareFinish(void) { return IsFanfareTaskInactive(); }
...
bool8 ScrCmd_applymovementat(struct ScriptContext *ctx) {
  ...
  ScriptMovement_StartObjectMovementScript(localId, mapNum, mapGroup,
                                           movementScript);
  sMovingNpcId = localId;
  return FALSE;
}
...
static bool8 WaitForMovementFinish(void) {
  return ScriptMovement_IsObjectMovementFinished(sMovingNpcId, sMovingNpcMapNum,
                                                 sMovingNpcMapGroup);
}
...
bool8 ScrCmd_removeobject(struct ScriptContext *ctx) {
  ...
  RemoveObjectEventByLocalIdAndMap(localId, gSaveBlock1Ptr->location.mapNum,
...
bool8 ScrCmd_addobject(struct ScriptContext *ctx) {
  ...
  TrySpawnObjectEvent(objectId, gSaveBlock1Ptr->location.mapNum,
                      gSaveBlock1Ptr->location.mapGroup);
  return FALSE;
}
...
bool8 ScrCmd_setobjectxy(struct ScriptContext *ctx) {
  ...
  TryMoveObjectEventToMapCoords(localId, gSaveBlock1Ptr->location.mapNum,
                                gSaveBlock1Ptr->location.mapGroup, x, y);
  return FALSE;
}
...
bool8 ScrCmd_copyobjectxytoperm(struct ScriptContext *ctx) {
  ...
  TryOverrideObjectEventTemplateCoords(localId, gSaveBlock1Ptr->location.mapNum,
                                       gSaveBlock1Ptr->location.mapGroup);
  return FALSE;
}
...
bool8 ScrCmd_turnobject(struct ScriptContext *ctx) {
  ...
  ObjectEventTurnByLocalIdAndMap(localId, gSaveBlock1Ptr->location.mapNum,
                                 gSaveBlock1Ptr->location.mapGroup, direction);
  return FALSE;
}
...
// lock freezes all object events except the player and the selected object
// immediately. The player and selected object are frozen after waiting for
// their current movement to finish.
...
bool8 ScrCmd_release(struct ScriptContext *ctx) {
  ...
  if (gObjectEvents[gSelectedObjectEvent].active)
    ObjectEventClearHeldMovementIfFinished(
        &gObjectEvents[gSelectedObjectEvent]);
  playerObjectId = GetObjectEventIdByLocalIdAndMap(OBJ_EVENT_ID_PLAYER, 0, 0);
  ObjectEventClearHeldMovementIfFinished(&gObjectEvents[playerObjectId]);
  ScriptMovement_UnfreezeObjectEvents();
  UnfreezeObjectEvents();
...
// Prints all at once. Skips waiting for player input. Only used by link
// contests
...
bool8 ScrCmd_multichoicedefault(struct ScriptContext *ctx) {
  ...
  if (ScriptMenu_MultichoiceWithDefault(left, top, multichoiceId, ignoreBPress,
                                        defaultChoice) == TRUE) {
    ScriptContext_Stop();
    return TRUE;
  } else {
...
bool8 ScrCmd_multichoicegrid(struct ScriptContext *ctx) {
  ...
  if (ScriptMenu_MultichoiceGrid(left, top, multichoiceId, ignoreBPress,
                                 numColumns) == TRUE) {
    ScriptContext_Stop();
    return TRUE;
  } else {
...
bool8 ScrCmd_braillemessage(struct ScriptContext *ctx) {
  ...
  AddTextPrinterParameterized(sBrailleWindowId, FONT_BRAILLE, gStringVar4,
                              xText, yText, TEXT_SKIP_DRAW, NULL);
...
bool8 ScrCmd_buffernumberstring(struct ScriptContext *ctx) {
  ...
  ConvertIntToDecimalStringN(sScriptStringVars[stringVarIndex], num,
                             STR_CONV_MODE_LEFT_ALIGN, numDigits);
  return FALSE;
}
...
// This command will set a Pokémon's modernFatefulEncounter bit; there is no
// similar command to clear it.
bool8 ScrCmd_setmodernfatefulencounter(struct ScriptContext *ctx) {
  bool8 isModernFatefulEncounter = TRUE;
  u16 partyIndex = VarGet(ScriptReadHalfword(ctx));

  SetMonData(&gPlayerParty[partyIndex], MON_DATA_MODERN_FATEFUL_ENCOUNTER,
...
bool8 ScrCmd_checkmodernfatefulencounter(struct ScriptContext *ctx) {
  ...
  gSpecialVar_Result = GetMonData(&gPlayerParty[partyIndex],
                                  MON_DATA_MODERN_FATEFUL_ENCOUNTER, NULL);
  return FALSE;
}
...
void SetMovingNpcId(u16 npcId) { sMovingNpcId = npcId; }
