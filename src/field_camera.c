...
static void AddCameraTileOffset(struct FieldCameraOffset *cameraOffset,
...
static void AddCameraPixelOffset(struct FieldCameraOffset *cameraOffset,
                                 u32 xOffset, u32 yOffset) {
  cameraOffset->xPixelOffset += xOffset;
  cameraOffset->yPixelOffset += yOffset;
}

void ResetFieldCamera(void) { ResetCameraOffset(&sFieldCameraOffset); }
...
static void DrawWholeMapViewInternal(int x, int y,
...
static void
RedrawMapSlicesForCameraUpdate(struct FieldCameraOffset *cameraOffset, int x,
                               int y) {
...
static void RedrawMapSliceNorth(struct FieldCameraOffset *cameraOffset,
  ...
  for (i = 0; i < 32; i += 2) {
    ...
    if (temp >= 32)
      temp -= 32;
    DrawMetatileAt(mapLayout, r7 + temp, gSaveBlock1Ptr->pos.x + i / 2,
                   gSaveBlock1Ptr->pos.y + 14);
  }
}
...
static void RedrawMapSliceEast(struct FieldCameraOffset *cameraOffset,
  ...
  for (i = 0; i < 32; i += 2) {
    ...
    if (temp >= 32)
      temp -= 32;
    DrawMetatileAt(mapLayout, temp * 32 + r6, gSaveBlock1Ptr->pos.x,
                   gSaveBlock1Ptr->pos.y + i / 2);
  }
}
...
static void RedrawMapSliceWest(struct FieldCameraOffset *cameraOffset,
  ...
  for (i = 0; i < 32; i += 2) {
    ...
    if (temp >= 32)
      temp -= 32;
    DrawMetatileAt(mapLayout, temp * 32 + r5, gSaveBlock1Ptr->pos.x + 14,
                   gSaveBlock1Ptr->pos.y + i / 2);
  }
}
...
static void DrawMetatileAt(const struct MapLayout *mapLayout, u16 offset, int x,
                           int y) {
  ...
  DrawMetatile(MapGridGetMetatileLayerTypeAt(x, y),
               metatiles + metatileId * NUM_TILES_PER_METATILE, offset);
}
...
static void DrawMetatile(s32 metatileLayerType, const u16 *tiles, u16 offset) {
  ...
  case METATILE_LAYER_TYPE_NORMAL:
    ...
    // Draw metatile's top layer to the top background layer, which covers
...
static s32 MapPosToBgTilemapOffset(struct FieldCameraOffset *cameraOffset,
                                   s32 x, s32 y) {
