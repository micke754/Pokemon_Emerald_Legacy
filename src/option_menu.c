...
static const u16 sOptionMenuText_Pal[] =
    INCBIN_U16("graphics/interface/option_menu_text.gbapal");
// note: this is only used in the Japanese release
static const u8 sEqualSignGfx[] =
    INCBIN_U8("graphics/interface/option_menu_equals_sign.4bpp");
...
void CB2_InitOptionMenu(void) {
  ...
  case 4:
    ...
    LoadPalette(
        ...
        BG_PLTT_ID(7), PLTT_SIZE_4BPP);
    gMain.state++;
    break;
  case 5:
    LoadPalette(sOptionMenuText_Pal, BG_PLTT_ID(1),
                sizeof(sOptionMenuText_Pal));
    gMain.state++;
    break;
  case 6:
...
static void Task_OptionMenuProcessInput(u8 taskId) {
  if (JOY_NEW(A_BUTTON)) {
    ...
    case MENUITEM_BATTLESCENE:
      ...
      gTasks[taskId].tBattleSceneOff =
          BattleScene_ProcessInput(gTasks[taskId].tBattleSceneOff);
    ...
    case MENUITEM_BATTLESTYLE:
      ...
      gTasks[taskId].tBattleStyle =
          BattleStyle_ProcessInput(gTasks[taskId].tBattleStyle);
    ...
    case MENUITEM_BUTTONMODE:
      ...
      gTasks[taskId].tButtonMode =
          ButtonMode_ProcessInput(gTasks[taskId].tButtonMode);
    ...
    case MENUITEM_FRAMETYPE:
      ...
      gTasks[taskId].tWindowFrameType =
          FrameType_ProcessInput(gTasks[taskId].tWindowFrameType);
...
static void DrawOptionMenuChoice(const u8 *text, u8 x, u8 y, u8 style) {
  ...
  dst[i] = EOS;
  AddTextPrinterParameterized(WIN_OPTIONS, FONT_NORMAL, dst, x, y + 1,
                              TEXT_SKIP_DRAW, NULL);
}
...
static void BattleStyle_DrawChoices(u8 selection) {
  ...
  if (FlagGet(FLAG_HARD)) {
    styles[1] = 1;
    DrawOptionMenuChoice(gText_BattleStyleSet, 104, YPOS_BATTLESTYLE,
                         styles[1]);
    return;
  }
  styles[selection] = 1;

  DrawOptionMenuChoice(gText_BattleStyleShift, 104, YPOS_BATTLESTYLE,
                       styles[0]);
  DrawOptionMenuChoice(
      gText_BattleStyleSet,
      GetStringRightAlignXOffset(FONT_NORMAL, gText_BattleStyleSet, 198),
...
static void Sound_DrawChoices(u8 selection) {
  ...
  DrawOptionMenuChoice(
      gText_SoundStereo,
      GetStringRightAlignXOffset(FONT_NORMAL, gText_SoundStereo, 198),
      YPOS_SOUND, styles[1]);
}
...
static u8 FrameType_ProcessInput(u8 selection) {
  if (JOY_NEW(DPAD_RIGHT)) {
    ...
    LoadPalette(GetWindowFrameTilesPal(selection)->pal, BG_PLTT_ID(7),
                PLTT_SIZE_4BPP);
    sArrowPressed = TRUE;
  }
  if (JOY_NEW(DPAD_LEFT)) {
...
static void ButtonMode_DrawChoices(u8 selection) {
  ...
  DrawOptionMenuChoice(
      ...
      GetStringRightAlignXOffset(FONT_NORMAL, gText_ButtonTypeLEqualsA, 198),
      YPOS_BUTTONMODE, styles[2]);
}

static void DrawHeaderText(void) {
  FillWindowPixelBuffer(WIN_HEADER, PIXEL_FILL(1));
  AddTextPrinterParameterized(WIN_HEADER, FONT_NORMAL, gText_Option, 8, 1,
                              TEXT_SKIP_DRAW, NULL);
...
static void DrawOptionMenuTexts(void) {
  ...
  for (i = 0; i < MENUITEM_COUNT; i++)
    AddTextPrinterParameterized(WIN_OPTIONS, FONT_NORMAL,
                                sOptionMenuItemsNames[i], 8, (i * 16) + 1,
                                TEXT_SKIP_DRAW, NULL);
  CopyWindowToVram(WIN_OPTIONS, COPYWIN_FULL);
}
