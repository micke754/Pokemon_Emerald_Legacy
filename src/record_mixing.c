...
struct RecordMixingHallRecords {
  struct RankingHall1P hallRecords1P[HALL_FACILITIES_COUNT]
                                    ...
                                    [HALL_RECORDS_COUNT * 2];
  struct RankingHall2P hallRecords2P[FRONTIER_LVL_MODE_COUNT]
                                    [HALL_RECORDS_COUNT * 2];
};
...
// When 3 players can swap mail 2 players are randomly selected and the 3rd is
// left out
...
// these variables were const in R/S, but had to become changeable because of
// saveblocks changing RAM position
...
static void PrepareExchangePacket(void) {
  ...
  if (Link_AnyPartnersPlayingRubyOrSapphire()) {
    ...
    memcpy(sSentRecord->emerald.secretBases, sSecretBasesSave,
           sizeof(sSentRecord->emerald.secretBases));
    memcpy(sSentRecord->emerald.tvShows, sTvShowsSave,
           sizeof(sSentRecord->emerald.tvShows));
    memcpy(sSentRecord->emerald.pokeNews, sPokeNewsSave,
           sizeof(sSentRecord->emerald.pokeNews));
    memcpy(&sSentRecord->emerald.oldMan, sOldManSave,
           sizeof(sSentRecord->emerald.oldMan));
    memcpy(&sSentRecord->emerald.lilycoveLady, sLilycoveLadySave,
           sizeof(sSentRecord->emerald.lilycoveLady));
    memcpy(sSentRecord->emerald.dewfordTrends, sDewfordTrendsSave,
           sizeof(sSentRecord->emerald.dewfordTrends));
...
static void ReceiveExchangePacket(u32 multiplayerId) {
  if (Link_AnyPartnersPlayingRubyOrSapphire()) {
    ...
    ReceiveSecretBasesData(sReceivedRecords->ruby.secretBases,
                           sizeof(sReceivedRecords->ruby), multiplayerId);
    ReceiveDaycareMailData(&sReceivedRecords->ruby.daycareMail,
                           sizeof(sReceivedRecords->ruby), multiplayerId,
                           sReceivedRecords->ruby.tvShows);
    ReceiveBattleTowerData(&sReceivedRecords->ruby.battleTowerRecord,
                           sizeof(sReceivedRecords->ruby), multiplayerId);
    ReceiveTvShowsData(sReceivedRecords->ruby.tvShows,
                       sizeof(sReceivedRecords->ruby), multiplayerId);
    ReceivePokeNewsData(sReceivedRecords->ruby.pokeNews,
                        sizeof(sReceivedRecords->ruby), multiplayerId);
    ReceiveOldManData(&sReceivedRecords->ruby.oldMan,
                      sizeof(sReceivedRecords->ruby), multiplayerId);
    ReceiveDewfordTrendData(sReceivedRecords->ruby.dewfordTrends,
                            sizeof(sReceivedRecords->ruby), multiplayerId);
    ...
    ReceiveSecretBasesData(sReceivedRecords->emerald.secretBases,
                           sizeof(sReceivedRecords->emerald), multiplayerId);
    ReceiveTvShowsData(sReceivedRecords->emerald.tvShows,
                       sizeof(sReceivedRecords->emerald), multiplayerId);
    ReceivePokeNewsData(sReceivedRecords->emerald.pokeNews,
                        sizeof(sReceivedRecords->emerald), multiplayerId);
    ReceiveOldManData(&sReceivedRecords->emerald.oldMan,
                      sizeof(sReceivedRecords->emerald), multiplayerId);
    ReceiveDewfordTrendData(sReceivedRecords->emerald.dewfordTrends,
                            sizeof(sReceivedRecords->emerald), multiplayerId);
    ...
    ReceiveLilycoveLadyData(&sReceivedRecords->emerald.lilycoveLady,
                            sizeof(sReceivedRecords->emerald), multiplayerId);
    ReceiveApprenticeData(sReceivedRecords->emerald.apprentices,
                          sizeof(sReceivedRecords->emerald), (u8)multiplayerId);
    ReceiveRankingHallRecords(&sReceivedRecords->emerald.hallRecords,
                              sizeof(sReceivedRecords->emerald),
                              (u8)multiplayerId);
  }
}
...
#define tNumChunksRecv(i)                                                      \
  data[1 + (i)] // Number of chunks of the record received per player
...
static void Task_MixingRecordsRecv(u8 taskId) {
  ...
  case 1: // wait for handshake
    if (gReceivedRemoteLinkPlayers) {
      ConvertIntToDecimalStringN(gStringVar1, GetMultiplayerId_(),
...
static void Task_SendPacket(u8 taskId) {
  ...
  {
    void *recordData = LoadPtrFromTaskData(&task->tSentRecord) +
                       task->tNumChunksSent * BUFFER_CHUNK_SIZE;
...
static void Task_CopyReceiveBuffer(u8 taskId) {
  ...
  if (status == GetLinkPlayerCountAsBitFlags()) {
    ...
    for (i = 0; i < GetLinkPlayerCount(); i++) {
      if ((status >> i) & 1) {
        void *dest = LoadPtrFromTaskData(&task->tRecvRecords) +
                     task->tNumChunksRecv(i) * BUFFER_CHUNK_SIZE +
                     sRecordStructSize * i;
        void *src = GetPlayerRecvBuffer(i);
        if ((task->tNumChunksRecv(i) + 1) * BUFFER_CHUNK_SIZE >
            sRecordStructSize)
          memcpy(dest, src,
                 sRecordStructSize -
                     task->tNumChunksRecv(i) * BUFFER_CHUNK_SIZE);
        ...
        if (task->tNumChunksRecv(i) ==
            sRecordStructSize / BUFFER_CHUNK_SIZE + 1)
          handledPlayers++;
      }
    }
...
static u8 GetMultiplayerId_(void) { return GetMultiplayerId(); }

static void *GetPlayerRecvBuffer(u8 id) { return gBlockRecvBuffer[id]; }
...
static void ShufflePlayerIndices(u32 *data) {
  ...
  case 3:
    linkTrainerId =
        GetLinkPlayerTrainerId(0) % ARRAY_COUNT(sPlayerIdxOrders_3Player);
    for (i = 0; i < ARRAY_COUNT(sPlayerIdxOrders_3Player[0]); i++)
      data[i] = sPlayerIdxOrders_3Player[linkTrainerId][i];
    break;
  case 4:
    linkTrainerId =
        GetLinkPlayerTrainerId(0) % ARRAY_COUNT(sPlayerIdxOrders_4Player);
...
static void ReceiveOldManData(OldMan *records, size_t recordSize,
                              u8 multiplayerId) {
  u8 version;
  u16 language;
  OldMan *oldMan;
  ...
  memcpy(sOldManSave, (void *)records + recordSize * mixIndices[multiplayerId],
         sizeof(OldMan));
  ResetMauvilleOldManFlag();
}

static void ReceiveBattleTowerData(void *records, size_t recordSize,
                                   u8 multiplayerId) {
  ...
  if (Link_AnyPartnersPlayingRubyOrSapphire()) {
    if (RubyBattleTowerRecordToEmerald(
            (void *)records + recordSize * mixIndices[multiplayerId],
            (void *)records + recordSize * multiplayerId) == TRUE) {
      battleTowerRecord = (void *)records + recordSize * multiplayerId;
      battleTowerRecord->language =
          gLinkPlayers[mixIndices[multiplayerId]].language;
      CalcEmeraldBattleTowerChecksum(battleTowerRecord);
    }
  } else {
    memcpy((void *)records + recordSize * multiplayerId,
           (void *)records + recordSize * mixIndices[multiplayerId],
    ...
    for (i = 0; i < MAX_FRONTIER_PARTY_SIZE; i++) {
      ...
      if (btPokemon->species != SPECIES_NONE &&
          IsStringJapanese(btPokemon->nickname))
...
static void ReceiveLilycoveLadyData(LilycoveLady *records, size_t recordSize,
                                    u8 multiplayerId) {
  LilycoveLady *lilycoveLady;
  u32 mixIndices[MAX_LINK_PLAYERS];

  ShufflePlayerIndices(mixIndices);
  memcpy((void *)records + recordSize * multiplayerId, sLilycoveLadySave,
         sizeof(LilycoveLady));
...
static void SwapDaycareMail(struct RecordMixingDaycareMail *records,
                            size_t recordSize, u8 (*idxs)[2], u8 playerSlot1,
                            u8 playerSlot2) {
  ...
  memcpy(&temp, &mixMail1->mail[idxs[playerSlot1][DAYCARE_SLOT]],
         sizeof(struct DaycareMail));

  // 2nd player's daycare mail --> 1st player's daycare mail
  mixMail2 = (void *)records + recordSize * idxs[playerSlot2][MULTIPLAYER_ID];
  memcpy(&mixMail1->mail[idxs[playerSlot1][DAYCARE_SLOT]],
         &mixMail2->mail[idxs[playerSlot2][DAYCARE_SLOT]],
         sizeof(struct DaycareMail));

  // temp --> 2nd player's daycare mail
  memcpy(&mixMail2->mail[idxs[playerSlot2][DAYCARE_SLOT]], &temp,
         sizeof(struct DaycareMail));
}

// This sum is used to determine which players will swap daycare mail if there
// are more than 2 players who can. The TV show data is used to calculate this
// sum.
...
static u8 GetDaycareMailRandSum(void) { return sDaycareMailRandSum; }

static void ReceiveDaycareMailData(struct RecordMixingDaycareMail *records,
                                   size_t recordSize, u8 multiplayerId,
                                   TVShow *shows) {
  u16 i, j;
  u8 linkPlayerCount;
  u8 tableId;
  ...
  memcpy(&gSaveBlock1Ptr->daycare.mons[0].mail, &mixMail->mail[0],
         sizeof(struct DaycareMail));
  memcpy(&gSaveBlock1Ptr->daycare.mons[1].mail, &mixMail->mail[1],
         sizeof(struct DaycareMail));
  SeedRng(oldSeed);
}

static void ReceiveGiftItem(u16 *item, u8 multiplayerId) {
  if (multiplayerId != 0 && *item != ITEM_NONE &&
      GetPocketByItemId(*item) == POCKET_KEY_ITEMS) {
...
static void GetSavedApprentices(struct Apprentice *dst,
                                struct Apprentice *src) {
  s32 i, id;
  ...
  for (i = 0; i < 2; i++) {
    id =
        (i + gSaveBlock2Ptr->playerApprentice.saveId) % (APPRENTICE_COUNT - 1) +
        1;
    if (src[id].playerName[0] != EOS) {
      if (GetTrainerId(src[id].playerId) !=
  ...
  // Prefer passing on other mixed Apprentices rather than old player's
  // Apprentices
...
void GetPlayerHallRecords(struct PlayerHallRecords *dst) {
  ...
  for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++) {
    ...
    CopyTrainerId(dst->twoPlayers[j].id2,
                  gSaveBlock2Ptr->frontier.opponentTrainerIds[j]);
    StringCopy(dst->twoPlayers[j].name1, gSaveBlock2Ptr->playerName);
    StringCopy(dst->twoPlayers[j].name2,
               gSaveBlock2Ptr->frontier.opponentNames[j]);
  }

  for (i = 0; i < FRONTIER_LVL_MODE_COUNT; i++) {
    dst->onePlayer[RANKING_HALL_TOWER_SINGLES][i].winStreak =
        gSaveBlock2Ptr->frontier
            .towerRecordWinStreaks[FRONTIER_MODE_SINGLES][i];
    dst->onePlayer[RANKING_HALL_TOWER_DOUBLES][i].winStreak =
        gSaveBlock2Ptr->frontier
            .towerRecordWinStreaks[FRONTIER_MODE_DOUBLES][i];
    dst->onePlayer[RANKING_HALL_TOWER_MULTIS][i].winStreak =
        gSaveBlock2Ptr->frontier.towerRecordWinStreaks[FRONTIER_MODE_MULTIS][i];
    dst->onePlayer[RANKING_HALL_DOME][i].winStreak =
        gSaveBlock2Ptr->frontier.domeRecordWinStreaks[FRONTIER_MODE_SINGLES][i];
    dst->onePlayer[RANKING_HALL_PALACE][i].winStreak =
        gSaveBlock2Ptr->frontier
            .palaceRecordWinStreaks[FRONTIER_MODE_SINGLES][i];
    dst->onePlayer[RANKING_HALL_ARENA][i].winStreak =
        gSaveBlock2Ptr->frontier.arenaRecordStreaks[i];
    dst->onePlayer[RANKING_HALL_FACTORY][i].winStreak =
        gSaveBlock2Ptr->frontier
            .factoryRecordWinStreaks[FRONTIER_MODE_SINGLES][i];
    dst->onePlayer[RANKING_HALL_PIKE][i].winStreak =
        gSaveBlock2Ptr->frontier.pikeRecordStreaks[i];
    dst->onePlayer[RANKING_HALL_PYRAMID][i].winStreak =
        gSaveBlock2Ptr->frontier.pyramidRecordStreaks[i];

    dst->twoPlayers[i].winStreak =
        gSaveBlock2Ptr->frontier
            .towerRecordWinStreaks[FRONTIER_MODE_LINK_MULTIS][i];
  }
}

static bool32 IsApprenticeAlreadySaved(struct Apprentice *mixApprentice,
                                       struct Apprentice *apprentices) {
  s32 i;

  for (i = 0; i < APPRENTICE_COUNT; i++) {
    if (GetTrainerId(mixApprentice->playerId) ==
            GetTrainerId(apprentices[i].playerId) &&
...
static void ReceiveApprenticeData(struct Apprentice *records, size_t recordSize,
                                  u32 multiplayerId) {
  ...
  apprenticeId = 0;
  for (i = 0; i < 2; i++) {
    if (mixApprentice[i].playerName[0] != EOS &&
        !IsApprenticeAlreadySaved(&mixApprentice[i],
                                  &gSaveBlock2Ptr->apprentices[0])) {
      numApprentices++;
      apprenticeId = i;
    }
  }
  ...
  case 2:
    for (i = 0; i < 2; i++) {
      apprenticeSaveId = ((i ^ 1) + gSaveBlock2Ptr->playerApprentice.saveId) %
                             (APPRENTICE_COUNT - 1) +
                         1;
...
static void GetNewHallRecords(struct RecordMixingHallRecords *dst,
                              void *records, size_t recordSize,
                              u32 multiplayerId, s32 linkPlayerCount) {
  s32 i, j, k, l;
  s32 repeatTrainers;
      ...
      for (k = 0; k < linkPlayerCount - 1; k++) {
        ...
        for (l = 0; l < HALL_RECORDS_COUNT; l++) {
          ...
          if (GetTrainerId(dst->hallRecords1P[i][j][l].id) ==
              GetTrainerId(sPartnerHallRecords[k]->onePlayer[i][j].id)) {
            repeatTrainers++;
  ...
  for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++) {
    ...
    for (k = 0; k < linkPlayerCount - 1; k++) {
      repeatTrainers = 0;
      for (l = 0; l < HALL_RECORDS_COUNT; l++) {
        // If the new trainer pair is already in the existing saved records,
        // only use the new pair if the win streak is better
        if (GetTrainerId(dst->hallRecords2P[j][l].id1) ==
                GetTrainerId(sPartnerHallRecords[k]->twoPlayers[j].id1) &&
            GetTrainerId(dst->hallRecords2P[j][l].id2) ==
                GetTrainerId(sPartnerHallRecords[k]->twoPlayers[j].id2)) {
          repeatTrainers++;
...
static void FillWinStreakRecords1P(struct RankingHall1P *playerRecords,
                                   struct RankingHall1P *mixRecords) {
  s32 i, j;

  // Fill the player's 1P records with the highest win streaks from the mixed
  // records
  for (i = 0; i < HALL_RECORDS_COUNT; i++) {
    ...
    // Save the win streak to the player's records, then clear it from the mixed
    // records
    if (highestId >= 0) {
...
static void FillWinStreakRecords2P(struct RankingHall2P *playerRecords,
                                   struct RankingHall2P *mixRecords) {
  s32 i, j;

  // Fill the player's 2P records with the highest win streaks from the mixed
  // records
  for (i = 0; i < HALL_RECORDS_COUNT; i++) {
    // Get the highest remaining win streak in the mixed hall records
    s32 highestWinStreak = 0;
...
static void
SaveHighestWinStreakRecords(struct RecordMixingHallRecords *mixHallRecords) {
  s32 i, j;

  for (i = 0; i < HALL_FACILITIES_COUNT; i++) {
    for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++)
      FillWinStreakRecords1P(gSaveBlock2Ptr->hallRecords1P[i][j],
                             mixHallRecords->hallRecords1P[i][j]);
  }
  for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++)
    FillWinStreakRecords2P(gSaveBlock2Ptr->hallRecords2P[j],
                           mixHallRecords->hallRecords2P[j]);
}

static void ReceiveRankingHallRecords(struct PlayerHallRecords *records,
                                      size_t recordSize, u32 multiplayerId) {
  u8 linkPlayerCount = GetLinkPlayerCount();
  struct RecordMixingHallRecords *mixHallRecords =
      AllocZeroed(sizeof(*mixHallRecords));

  GetNewHallRecords(mixHallRecords, records, recordSize, multiplayerId,
                    linkPlayerCount);
...
static void
SanitizeEmeraldBattleTowerRecord(struct EmeraldBattleTowerRecord *dst) {
  s32 i;
