 ...
 *  - Task_RunTimeBasedEvents: Periodically updates local time and RTC events.
 * Also triggers ambient cries.
 *  - Task_MuddySlope: Handles the metatile animation when the player steps on
 * muddy slopes.
 *  - Task_RunPerStepCallback: Calls one of the functions in sPerStepCallbacks,
 * listed below... . DummyPerStepCallback: Default, does nothing .
 * AshGrassPerStepCallback: Removes the ash from ash-covered grass that the
 * player steps on. . FortreeBridgePerStepCallback: Depresses Fortree log
 * bridges that the player steps on. . PacifidlogBridgePerStepCallback:
 * Submerges Pacifidlog log bridges that the player steps on. .
 * SootopolisGymIcePerStepCallback: Cracks/breaks ice in Sootopolis Gym that the
 * player steps on. . EndTruckSequence: Sets the moving truck boxes to their
 * final position when the truck sequence ends. . SecretBasePerStepCallback:
 * Records the decorations in a friend's secret base that the player steps on.
 *      . CrackedFloorPerStepCallback: Breaks cracked floors that the player
 * steps on.
 *
 *  NOTE: "PerStep" is perhaps misleading. One function in sPerStepCallbacks is
 * called every frame while in the overworld by Task_RunPerStepCallback
 * regardless of whether or not steps are being taken. However, nearly all of
 * the functions in the table check if the player has moved from their previous
...
// Each array has 4 pairs of data, each pair representing two metatiles of a log
// and their relative position. The 4 pairs are for: 0: If the player is
// standing on the top of a vertical log 1: If the player is standing on the
// bottom of a vertical log 2: If the player is standing on the left of a
// horizontal log 3: If the player is standing on the right of a horizontal log
...
static const struct PacifidlogMetatileOffsets
    sHalfSubmergedBridgeMetatileOffsets[] = {
        ...
        {0, -1, METATILE_Pacifidlog_HalfSubmergedLogs_VerticalTop},
        {0, 0, METATILE_Pacifidlog_HalfSubmergedLogs_VerticalBottom},
        {0, 0, METATILE_Pacifidlog_HalfSubmergedLogs_HorizontalLeft},
...
static const struct PacifidlogMetatileOffsets
    sFullySubmergedBridgeMetatileOffsets[] = {
        ...
        {0, 0, METATILE_Pacifidlog_SubmergedLogs_HorizontalLeft},
        {1, 0, METATILE_Pacifidlog_SubmergedLogs_HorizontalRight},
        {-1, 0, METATILE_Pacifidlog_SubmergedLogs_HorizontalLeft},
        {0, 0, METATILE_Pacifidlog_SubmergedLogs_HorizontalRight}};
...
static const struct PacifidlogMetatileOffsets sFloatingBridgeMetatileOffsets[] =
    {{0, 0, METATILE_Pacifidlog_FloatingLogs_VerticalTop},
     ...
     {0, 0, METATILE_Pacifidlog_FloatingLogs_VerticalBottom},
     {0, 0, METATILE_Pacifidlog_FloatingLogs_HorizontalLeft},
...
// The rows with ice each have a temp var used to track the ice steps. Each bit
// in the var represents whether ice at that x coordinate (starting from the
// left edge) has been visited. This method of tracking steps will break if the
// ice puzzle is more than 16 map spaces wide.
...
static void
TrySetPacifidlogBridgeMetatiles(const struct PacifidlogMetatileOffsets *offsets,
                                s16 x, s16 y, bool32 redrawMap) {
  offsets = GetPacifidlogBridgeMetatileOffsets(
      offsets, MapGridGetMetatileBehaviorAt(x, y));

  // If offsets is NULL, position is not a log (don't set it)
  if (offsets) {
    // Set both metatiles of the log
    MapGridSetMetatileIdAt(x + offsets[0].x, y + offsets[0].y,
                           offsets[0].metatileId);
    if (redrawMap)
      CurrentMapDrawMetatileAt(x + offsets[0].x, y + offsets[0].y);

    MapGridSetMetatileIdAt(x + offsets[1].x, y + offsets[1].y,
                           offsets[1].metatileId);
    if (redrawMap)
...
static void TrySetLogBridgeHalfSubmerged(s16 x, s16 y, bool32 redrawMap) {
  TrySetPacifidlogBridgeMetatiles(sHalfSubmergedBridgeMetatileOffsets, x, y,
                                  redrawMap);
}

static void TrySetLogBridgeFullySubmerged(s16 x, s16 y, bool32 redrawMap) {
  TrySetPacifidlogBridgeMetatiles(sFullySubmergedBridgeMetatileOffsets, x, y,
                                  redrawMap);
}

static void TrySetLogBridgeFloating(s16 x, s16 y, bool32 redrawMap) {
  TrySetPacifidlogBridgeMetatiles(sFloatingBridgeMetatileOffsets, x, y,
                                  redrawMap);
}

// Returns FALSE if player has moved from one end of a log to the other (log
// should remain submerged). Otherwise it returns TRUE.
static bool32 ShouldRaisePacifidlogLogs(s16 newX, s16 newY, s16 oldX,
                                        s16 oldY) {
...
// Returns FALSE if player has moved from one end of a log to the other (log
// should remain submerged). Otherwise it returns TRUE. This is the effectively
// the same as ShouldRaisePacifidlogLogs, as it swaps both the conditions and
// which position's behavior to check. In effect the previous function asks "was
// the player's previous position not the other end of a log they're standing
// on?" while this function asks "is the player's current position not the other
// end of a log they were previously standing on?" and with the same positions
// both questions always have the same answer.
...
static void PacifidlogBridgePerStepCallback(u8 taskId) {
  ...
  case 1:
    ...
    if (ShouldSinkPacifidlogLogs(x, y, tPrevX, tPrevY)) {
      ...
      // they were previously standing on, try and set it half-submerged
      // (sinking)
...
static void FortreeBridgePerStepCallback(u8 taskId) {
  ...
  case 0:
    ...
    // If player is already on bridge when callback is set then lower it
    // immediately.
  ...
  case 1:
    ...
    isFortreeBridgePrev = MetatileBehavior_IsFortreeBridge(
        MapGridGetMetatileBehaviorAt(prevX, prevY));
    ...
    if (onBridgeElevation &&
        (isFortreeBridgeCur == TRUE || isFortreeBridgePrev == TRUE))
...
// This function uses the constants for gTileset_Cave's metatile labels, but
// other tilesets with the CrackedFloorPerStepCallback callback use the same
// metatile numbers for the cracked floor and hole metatiles, such as
// gTileset_MirageTower.
static void SetCrackedFloorHoleMetatile(s16 x, s16 y) {
  u16 metatileId = MapGridGetMetatileIdAt(x, y) == METATILE_Cave_CrackedFloor
...
#define tPrevX data[2]
#define tPrevY data[3]
// data[4] - data[15] are data about up to 4 individual animating muddy slope
// metatiles They're divided into groups of 3, i.e data[4]-[6] track one
// metatile, data[7]-[9] another, and so on. For each data triplet, the 1sst is
// the animation time, and the 2nd/3rd are the x/y coordinates.
#define SLOPE_DATA_START 4
#define SLOPE_DATA_END                                                         \
  (3 * SLOPE_DATA_SIZE +                                                       \
   SLOPE_DATA_START) // 13, which is the last slope data start point
enum { SLOPE_TIME, SLOPE_X, SLOPE_Y, SLOPE_DATA_SIZE };
#define tSlopeAnimTime(i)                                                      \
  data[(i) * SLOPE_DATA_SIZE + SLOPE_DATA_START + SLOPE_TIME]
...
#define SLOPE_ANIM_STEP_TIME                                                   \
  (SLOPE_ANIM_TIME / (int)ARRAY_COUNT(sMuddySlopeMetatiles))
...
static void Task_MuddySlope(u8 taskId) {
  ...
  s16 *data = gTasks[taskId].data;
  PlayerGetDestCoords(&x, &y);
  mapId = (gSaveBlock1Ptr->location.mapGroup << 8) |
  ...
  for (i = SLOPE_DATA_START; i <= SLOPE_DATA_END; i += SLOPE_DATA_SIZE) {
    if (data[i + SLOPE_TIME]) {
      ...
      SetMuddySlopeMetatile(&data[i + SLOPE_TIME], data[i + SLOPE_X],
                            data[i + SLOPE_Y]);
    }
  }
}
