...
enum {
  ...
  GFXTAG_CHOOSE_PKMN_EMPTY_1, // 6 sprites to cover the full bottom bar, but
                              // only first 3 are needed
  GFXTAG_CHOOSE_PKMN_EMPTY_2,
  GFXTAG_CHOOSE_PKMN_EMPTY_3,
  NUM_MENU_TEXT_SPRITES
};
#define NUM_PLAYER_NAME_SPRITES                                                \
  (1 + GFXTAG_PLAYER_NAME_R - GFXTAG_PLAYER_NAME_L)
#define NUM_PARTNER_NAME_SPRITES                                               \
  (1 + GFXTAG_PARTNER_NAME_R - GFXTAG_PARTNER_NAME_L)
#define NUM_CHOOSE_PKMN_SPRITES                                                \
  (1 + GFXTAG_CHOOSE_PKMN_EMPTY_3 - GFXTAG_CHOOSE_PKMN_L)
...
static EWRAM_DATA struct {
  ...
  u16 neverRead_8C;
  u8 monSpriteIds[2];
  u8 connectionSpriteId1; // Multi-purpose sprite ids used during the transfer
                          // sequence
...
static void RequestLinkData(u8 type) { SendBlockRequest(type); }
...
static u32 _GetBlockReceivedStatus(void) { return GetBlockReceivedStatus(); }
...
static void SetTradeLinkStandbyCallback(u8 unused) { SetLinkStandbyCallback(); }

static bool32 _IsLinkTaskFinished(void) { return IsLinkTaskFinished(); }
...
static void InitTradeMenu(void) {
  ...
  if (InitWindows(sWindowTemplates)) {
    ...
    FillBgTilemapBufferRect(0, 0, 0, 0, DISPLAY_TILE_WIDTH, DISPLAY_TILE_HEIGHT,
                            15);
...
static void CB2_CreateTradeMenu(void) {
  ...
  case 1:
    ...
    for (i = 0; i < PARTY_SIZE; i++)
      CreateMon(&gEnemyParty[i], SPECIES_NONE, 0, USE_RANDOM_IVS, FALSE, 0,
                OT_ID_PLAYER_ID, 0);
  ...
  case 7:
    ...
    for (i = 0; i < sTradeMenu->partyCounts[TRADE_PLAYER]; i++) {
      ...
      sTradeMenu->partySpriteIds[TRADE_PLAYER][i] = CreateMonIcon(
          GetMonData(mon, MON_DATA_SPECIES_OR_EGG), SpriteCB_MonIcon,
          (sTradeMonSpriteCoords[i][0] * 8) + 14,
          (sTradeMonSpriteCoords[i][1] * 8) - 12, 1,
          GetMonData(mon, MON_DATA_PERSONALITY), TRUE);
    }

    for (i = 0; i < sTradeMenu->partyCounts[TRADE_PARTNER]; i++) {
      struct Pokemon *mon = &gEnemyParty[i];
      sTradeMenu->partySpriteIds[TRADE_PARTNER][i] = CreateMonIcon(
          GetMonData(mon, MON_DATA_SPECIES_OR_EGG, NULL), SpriteCB_MonIcon,
  ...
  case 8:
    LoadHeldItemIcons();
    DrawHeldItemIconsForTrade(&sTradeMenu->partyCounts[0],
                              sTradeMenu->partySpriteIds[0], TRADE_PLAYER);
    gMain.state++;
    break;
  case 9:
    DrawHeldItemIconsForTrade(&sTradeMenu->partyCounts[0],
                              sTradeMenu->partySpriteIds[0], TRADE_PARTNER);
    gMain.state++;
    break;
  case 10:
    DrawTextWindowAndBufferTiles(gSaveBlock2Ptr->playerName,
                                 sMenuTextTileBuffers[GFXTAG_PLAYER_NAME_L], 0,
                                 0, 3);
    id = GetMultiplayerId();
    DrawTextWindowAndBufferTiles(gLinkPlayers[id ^ 1].name,
                                 sMenuTextTileBuffers[GFXTAG_PARTNER_NAME_L], 0,
                                 0, 3);
    DrawTextWindowAndBufferTiles(sActionTexts[TEXT_CANCEL],
                                 sMenuTextTileBuffers[GFXTAG_CANCEL_L], 0, 0,
                                 2);
    DrawBottomRowText(sActionTexts[TEXT_CHOOSE_MON],
                      sMenuTextTileBuffers[GFXTAG_CHOOSE_PKMN_L], 24);
    gMain.state++;
    sTradeMenu->timer = 0;
    break;
  case 11:
  ...
  case 12:
    ...
    xPos = GetStringCenterAlignXOffset(FONT_NORMAL, gSaveBlock2Ptr->playerName,
                                       120);
    ...
    xPos = GetStringCenterAlignXOffset(
        FONT_NORMAL, gLinkPlayers[GetMultiplayerId() ^ 1].name, 120);
  ...
  case 13:
    ...
    // Create Choose a Pokémon text sprites (only 3 are needed, other 3 are
    // empty)
    ...
    sTradeMenu->cursorSpriteId = CreateSprite(
        &sSpriteTemplate_Cursor, sTradeMonSpriteCoords[0][0] * 8 + 32,
...
static void CB_WaitToStartTrade(void) {
  if (!gPaletteFade.active) {
    ...
    gSelectedTradeMonPositions[TRADE_PARTNER] =
...
static void LoadTradeBgGfx(u8 state) {
  ...
  case 0:
    ...
    CopyToBgTilemapBufferRect_ChangePalette(1, gTradeMenu_Tilemap, 0, 0, 32, 20,
                                            0);
...
static bool8 BufferTradeParties(void) {
  ...
  case 0:
    ...
    Trade_Memcpy(gBlockSendBuffer, &gPlayerParty[0],
                 2 * sizeof(struct Pokemon));
  ...
  case 4:
    if (_GetBlockReceivedStatus() == 3) {
      Trade_Memcpy(&gEnemyParty[0], gBlockRecvBuffer[id ^ 1],
                   2 * sizeof(struct Pokemon));
      TradeResetReceivedFlags();
      sTradeMenu->bufferPartyState++;
    }
    break;
  case 5:
    Trade_Memcpy(gBlockSendBuffer, &gPlayerParty[2],
                 2 * sizeof(struct Pokemon));
    sTradeMenu->bufferPartyState++;
    break;
  case 7:
  ...
  case 8:
    if (_GetBlockReceivedStatus() == 3) {
      Trade_Memcpy(&gEnemyParty[2], gBlockRecvBuffer[id ^ 1],
                   2 * sizeof(struct Pokemon));
      TradeResetReceivedFlags();
      sTradeMenu->bufferPartyState++;
    }
    break;
  case 9:
    Trade_Memcpy(gBlockSendBuffer, &gPlayerParty[4],
                 2 * sizeof(struct Pokemon));
    sTradeMenu->bufferPartyState++;
    break;
  case 11:
  ...
  case 12:
    if (_GetBlockReceivedStatus() == 3) {
      Trade_Memcpy(&gEnemyParty[4], gBlockRecvBuffer[id ^ 1],
                   2 * sizeof(struct Pokemon));
      TradeResetReceivedFlags();
      sTradeMenu->bufferPartyState++;
    }
    break;
  case 13:
    Trade_Memcpy(gBlockSendBuffer, gSaveBlock1Ptr->mail,
                 PARTY_SIZE * sizeof(struct Mail) + 4);
    sTradeMenu->bufferPartyState++;
    break;
  case 15:
    if (id == 0)
  ...
  case 16:
    if (_GetBlockReceivedStatus() == 3) {
      Trade_Memcpy(gTradeMail, gBlockRecvBuffer[id ^ 1],
                   PARTY_SIZE * sizeof(struct Mail));
      TradeResetReceivedFlags();
      sTradeMenu->bufferPartyState++;
    }
    break;
  case 17:
    Trade_Memcpy(gBlockSendBuffer, gSaveBlock1Ptr->giftRibbons,
                 sizeof(sTradeMenu->giftRibbons));
    sTradeMenu->bufferPartyState++;
    break;
  case 19:
    if (id == 0)
  ...
  case 20:
    if (_GetBlockReceivedStatus() == 3) {
      Trade_Memcpy(sTradeMenu->giftRibbons, gBlockRecvBuffer[id ^ 1],
                   sizeof(sTradeMenu->giftRibbons));
      TradeResetReceivedFlags();
      sTradeMenu->bufferPartyState++;
    }
...
static void PrintIsThisTradeOkay(void) {
  DrawBottomRowText(
      ...
      (void *)(OBJ_VRAM0 + (sTradeMenu->bottomTextTileStart * 32)), 24);
}
...
static void Leader_HandleCommunication(void) {
  if (sTradeMenu->playerSelectStatus != STATUS_NONE &&
    ...
    if (sTradeMenu->playerSelectStatus == STATUS_READY &&
      ...
      sTradeMenu->playerSelectStatus = sTradeMenu->partnerSelectStatus =
          STATUS_NONE;
      ...
      QueueAction(QUEUE_DELAY_DATA, QUEUE_SEND_DATA);
      sTradeMenu->playerConfirmStatus = sTradeMenu->partnerConfirmStatus =
          STATUS_NONE;
      sTradeMenu->playerSelectStatus = sTradeMenu->partnerSelectStatus =
          STATUS_NONE;
      sTradeMenu->callbackId = CB_HANDLE_TRADE_CANCELED;
...
static u8 GetNewCursorPosition(u8 oldPosition, u8 direction) {
  ...
  for (i = 0; i < PARTY_SIZE; i++) {
    if (sTradeMenu->optionsActive[sCursorMoveDestinations[oldPosition]
                                                         [direction][i]] ==
        TRUE) {
...
static void TradeMenuMoveCursor(u8 *cursorPosition, u8 direction) {
  ...
  {
    StartSpriteAnim(&gSprites[sTradeMenu->cursorSpriteId],
    ...
    gSprites[sTradeMenu->cursorSpriteId].y =
...
static void CB_ProcessMenuInput(void) {
  ...
  if (JOY_NEW(A_BUTTON)) {
    ...
    if (sTradeMenu->cursorPosition < PARTY_SIZE) {
      ...
      PrintMenuTable(1, ARRAY_COUNT(sSelectTradeMonActions),
                     sSelectTradeMonActions);
      InitMenuInUpperLeftCornerNormal(1, ARRAY_COUNT(sSelectTradeMonActions),
                                      0);
      PutWindowTilemap(1);
      ...
      DrawBottomRowText(
          sActionTexts[TEXT_CANCEL_TRADE],
          (void *)(OBJ_VRAM0 + sTradeMenu->bottomTextTileStart * 32), 24);
    }
  }
}
...
static void RedrawChooseAPokemonWindow(void) {
  ...
  DrawBottomRowText(sActionTexts[TEXT_CHOOSE_MON],
                    (void *)(OBJ_VRAM0 + sTradeMenu->bottomTextTileStart * 32),
                    24);
}
...
static void CB_ProcessSelectedMonInput(void) {
  ...
  case 0: // Summary
    ...
    break;
  case 1: // Trade
    switch (CanTradeSelectedMon(gPlayerParty, gPlayerPartyCount,
  ...
  if (!gPaletteFade.active) {
    if (sTradeMenu->cursorPosition < PARTY_SIZE)
      ShowPokemonSummaryScreen(
          SUMMARY_MODE_LOCK_MOVES, gPlayerParty, sTradeMenu->cursorPosition,
          sTradeMenu->partyCounts[TRADE_PLAYER] - 1, CB2_ReturnToTradeMenu);
    else
      ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, gEnemyParty,
                               sTradeMenu->cursorPosition - PARTY_SIZE,
                               sTradeMenu->partyCounts[TRADE_PARTNER] - 1,
                               CB2_ReturnToTradeMenu);
    FreeAllWindowBuffers();
  }
}

static u8 CheckValidityOfTradeMons(u8 *aliveMons, u8 playerPartyCount,
                                   u8 playerMonIdx, u8 partnerMonIdx) {
  int i;
  u16 partnerSpecies;
  u8 hasLiveMon = 0;
  ...
  if (partnerSpecies == SPECIES_DEOXYS || partnerSpecies == SPECIES_MEW) {
    if (!GetMonData(&gEnemyParty[partnerMonIdx],
...
static bool32 CheckMonsBeforeTrade(void) {
  ...
  switch (CheckValidityOfTradeMons(
      aliveMons, sTradeMenu->partyCounts[TRADE_PLAYER],
      sTradeMenu->cursorPosition, sTradeMenu->partnerCursorPosition)) {
...
static void DrawSelectedMonScreen(u8 whichParty) {
  ...
  default:
    // Idle while state is 0, and once it reaches the final state
    // (DRAW_SELECTED_FINISH)
    break;
  case 1:
    // Erase the rest of the party
    for (i = 0; i < sTradeMenu->partyCounts[whichParty]; i++)
      gSprites[sTradeMenu->partySpriteIds[selectedMonParty][i]].invisible =
          TRUE;
    for (i = 0; i < PARTY_SIZE; i++)
      ClearWindowTilemap(i + (whichParty * PARTY_SIZE + 2));

    // Re-display the selected Pokémon
    gSprites[sTradeMenu->partySpriteIds[selectedMonParty][partyIdx]].invisible =
        FALSE;

    // Move the selected Pokémon to the center
    gSprites[sTradeMenu->partySpriteIds[selectedMonParty][partyIdx]].data[0] =
        20;
    ...
    CopyToBgTilemapBufferRect_ChangePalette(1, sTradePartyBoxTilemap,
                                            whichParty * 15, 0, 15, 17, 0);
    CopyBgTilemapBufferToVram(1);
  ...
  case 2:
    ...
    if (gSprites[sTradeMenu->partySpriteIds[selectedMonParty][partyIdx]]
  ...
  case 3:
    ...
    AddTextPrinterParameterized3((whichParty * 2) + 14, FONT_SMALL,
                                 (80 - nameStringWidth) / 2, 4,
                                 sTradeTextColors, 0, nickname);
    BufferMovesString(movesString, selectedMonParty, partyIdx);
    AddTextPrinterParameterized4((whichParty * 2) + 15, FONT_NORMAL, 0, 0, 0, 0,
                                 sTradeTextColors, 0, movesString);
...
static u8 GetMonNicknameWidth(u8 *str, u8 whichParty, u8 partyIdx) {
  ...
  return GetStringWidth(FONT_SMALL, str,
                        GetFontAttribute(FONT_SMALL, FONTATTR_LETTER_SPACING));
}
...
static void BufferMovesString(u8 *str, u8 whichParty, u8 partyIdx) {
  ...
  if (!sTradeMenu->isEgg[whichParty][partyIdx]) {
    for (i = 0; i < MAX_MON_MOVES; i++) {
      if (whichParty == TRADE_PLAYER)
        moves[i] =
            GetMonData(&gPlayerParty[partyIdx], i + MON_DATA_MOVE1, NULL);
      else
...
static void PrintPartyMonNickname(u8 whichParty, u8 windowId, u8 *nickname) {
  ...
  AddTextPrinterParameterized3(windowId, FONT_SMALL, xPos, 4, sTradeTextColors,
                               0, nickname);
...
static void PrintPartyNicknames(u8 whichParty) {
  ...
  struct Pokemon *party =
      (whichParty == TRADE_PLAYER) ? gPlayerParty : gEnemyParty;
...
static void PrintLevelAndGender(u8 whichParty, u8 monIdx, u8 x, u8 y, u8 width,
                                u8 height) {
  u8 level;
  u32 symbolTile;
  u8 gender;
  ...
  CopyToBgTilemapBufferRect_ChangePalette(1, gTradeMenuMonBox_Tilemap, width,
                                          height, 6, 3, 0);
  CopyBgTilemapBufferToVram(1);
  ...
  if (!sTradeMenu->isEgg[whichParty][monIdx]) {
    ...
    sTradeMenu->tilemapBuffer[x + (y * 32) - 32] =
        sTradeMenu->tilemapBuffer[x + (y * 32) - 33];
    sTradeMenu->tilemapBuffer[x + (y * 32) - 31] =
        sTradeMenu->tilemapBuffer[x + (y * 32) - 36] | 0x400;
  }
...
static void PrintPartyLevelsAndGenders(u8 whichParty) {
  ...
  for (i = 0; i < sTradeMenu->partyCounts[whichParty]; i++) {
    ...
    PrintLevelAndGender(whichParty, i, sTradeMonLevelCoords[j][0],
                        sTradeMonLevelCoords[j][1], sTradeMonBoxCoords[j][0],
...
static void PrintTradeMessage(u8 messageId) {
  ...
  AddTextPrinterParameterized(0, FONT_NORMAL, sMessages[messageId], 0, 1,
                              TEXT_SKIP_DRAW, NULL);
...
static void SetTradePartyHPBarSprites(void) {
  ...
  for (i = 0; i < 2; i++) {
    for (j = 0; j < sTradeMenu->partyCounts[i]; j++)
      SetPartyHPBarSprite(&gSprites[sTradeMenu->partySpriteIds[i][j]],
...
static u32 CanTradeSelectedMon(struct Pokemon *playerParty, int partyCount,
                               int monIdx) {
  int i, numMonsLeft;
...
s32 GetGameProgressForLinkTrade(void) {
  ...
  // but it's the result of its usage in FRLG, where 0 is FRLG, 1 is RS, and 2
  // is Emerald.
  ...
  if (gReceivedRemoteLinkPlayers) {
    ...
    if (versionId > 0) {
      ...
      if (gLinkPlayers[GetMultiplayerId()].progressFlagsCopy & 0xF0) {
        if (versionId ==
            2) // Check is only relevant in FRLG, this will always be true
        {
...
static bool32 IsDeoxysOrMewUntradable(u16 species,
                                      bool8 isModernFatefulEncounter) {
...
int GetUnionRoomTradeMessageId(struct RfuGameCompatibilityData player,
                               struct RfuGameCompatibilityData partner,
                               u16 playerSpecies2, u16 partnerSpecies,
                               u8 requestedType, u16 playerSpecies,
                               bool8 isModernFatefulEncounter) {
  ...
  // If partner is not using Emerald, both players must have progressed the
  // story to a certain point (becoming champion in RSE, finishing the Sevii
  // islands in FRLG)
  ...
  // If the player is trading an Egg then the partner must also be trading an
  // Egg Odd that this wasn't checked earlier, as by this point we know either
  // the partner doesn't have an Egg or that both do.
  if (playerSpecies2 == SPECIES_EGG && playerSpecies2 != partnerSpecies)
    return UR_TRADE_MSG_MON_CANT_BE_TRADED_1;

  // If the player doesn't have the National Dex then Eggs and non-Hoenn Pokémon
  // can't be traded
  ...
  // If the partner doesn't have the National Dex then the player's offer has to
  // be a Hoenn Pokémon
...
int CanRegisterMonForTradingBoard(struct RfuGameCompatibilityData player,
                                  u16 species2, u16 species,
                                  bool8 isModernFatefulEncounter) {
...
// Spin Trade wasnt fully implemented, but this checks if a mon would be valid
// to Spin Trade Unlike later generations, this version of Spin Trade isnt only
// for Eggs
...
static void SpriteCB_LinkMonShadow(struct Sprite *sprite) {
  if (!sprite->data[1]) {
    ...
    LoadPalette(&sLinkMonShadow_Pal[sprite->data[0]],
                OBJ_PLTT_ID2(sprite->oam.paletteNum) + 4, PLTT_SIZEOF(1));
  }
}
...
static void SetTradeBGAffine(void) {
  ...
  DoBgAffineSet(&affine, sTradeAnim->texX * 0x100, sTradeAnim->texY * 0x100,
                sTradeAnim->scrX, sTradeAnim->scrY, sTradeAnim->sXY,
                sTradeAnim->sXY, sTradeAnim->alpha);
...
static void LoadTradeMonPic(u8 whichParty, u8 state) {
  ...
  case 0:
    ...
    if (whichParty == TRADE_PLAYER)
      HandleLoadSpecialPokePic_2(
          ...
          gMonSpritesGfxPtr->sprites.ptr[B_POSITION_OPPONENT_LEFT], species,
          personality);
    else
      HandleLoadSpecialPokePic_DontHandleDeoxys(
          &gMonFrontPicTable[species],
          gMonSpritesGfxPtr->sprites
              .ptr[whichParty * 2 + B_POSITION_OPPONENT_LEFT],
          species, personality);
  ...
  case 1:
    ...
    gSprites[sTradeAnim->monSpriteIds[whichParty]].callback =
        SpriteCallbackDummy;
    break;
  }
}
...
static void TradeAnimInit_LoadGfx(void) {
  SetGpuReg(REG_OFFSET_DISPCNT, 0);
  ResetBgsAndClearDma3BusyFlags(0);
  InitBgsFromTemplates(0, sTradeSequenceBgTemplates,
                       ARRAY_COUNT(sTradeSequenceBgTemplates));
...
static void HandleLinkDataSend(void) {
  ...
  case 1:
    if (IsLinkTaskFinished()) {
      SendBlock(BitmaskAllOtherLinkPlayers(), sTradeAnim->linkData,
                sizeof(sTradeAnim->linkData));
...
static void SetTradeSequenceBgGpuRegs(u8 state) {
  ...
  case 0:
    ...
    SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_0 | DISPCNT_OBJ_1D_MAP |
                                      DISPCNT_BG0_ON | DISPCNT_BG2_ON |
                                      DISPCNT_OBJ_ON);
    SetGpuReg(REG_OFFSET_BG2CNT, BGCNT_PRIORITY(2) | BGCNT_CHARBASE(1) |
                                     BGCNT_16COLOR | BGCNT_SCREENBASE(18) |
                                     BGCNT_TXT512x256);
    LoadPalette(gTradeGba2_Pal, BG_PLTT_ID(1), 3 * PLTT_SIZE_4BPP);
    DmaCopyLarge16(3, gTradeGba_Gfx, (void *)BG_CHAR_ADDR(1), 0x1420, 0x1000);
    DmaCopy16Defvars(3, gTradePlatform_Tilemap, (void *)BG_SCREEN_ADDR(18),
                     0x1000);
    break;
  case 1:
    ...
    SetGpuReg(REG_OFFSET_BG1CNT, BGCNT_PRIORITY(2) | BGCNT_CHARBASE(0) |
                                     BGCNT_16COLOR | BGCNT_SCREENBASE(5) |
                                     BGCNT_TXT256x512);
    SetGpuReg(REG_OFFSET_BG2CNT, BGCNT_PRIORITY(2) | BGCNT_CHARBASE(1) |
                                     BGCNT_16COLOR | BGCNT_SCREENBASE(18) |
                                     BGCNT_TXT256x512);
    ...
    SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_0 | DISPCNT_OBJ_1D_MAP |
                                      DISPCNT_BG1_ON | DISPCNT_OBJ_ON);
    break;
  case 2:
    sTradeAnim->bg1vofs = 0;
    sTradeAnim->bg1hofs = 0;
    if (!sTradeAnim->isCableTrade) {
      SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_1 | DISPCNT_OBJ_1D_MAP |
                                        DISPCNT_BG1_ON | DISPCNT_OBJ_ON);
  ...
  case 4:
    ...
    SetGpuReg(REG_OFFSET_BG2CNT, BGCNT_PRIORITY(3) | BGCNT_CHARBASE(1) |
                                     BGCNT_256COLOR | BGCNT_SCREENBASE(18) |
                                     BGCNT_AFF128x128);
    sTradeAnim->texX = 64;
    sTradeAnim->texY = 92;
    sTradeAnim->sXY = 32;
    ...
    if (sTradeAnim->isCableTrade) {
      DmaCopy16Defvars(3, sGbaAffineMapCable, (void *)BG_SCREEN_ADDR(18),
                       0x100);
    } else {
      DmaCopy16Defvars(3, sGbaAffineMapWireless, (void *)BG_SCREEN_ADDR(18),
                       0x100);
    }
    break;
  case 5:
...
static void BufferTradeSceneStrings(void) {
  ...
  if (sTradeAnim->isLinkTrade) {
    ...
    GetMonData(
        &gEnemyParty[gSelectedTradeMonPositions[TRADE_PARTNER] % PARTY_SIZE],
        MON_DATA_NICKNAME, name);
    StringCopy_Nickname(gStringVar3, name);
    GetMonData(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]],
               MON_DATA_NICKNAME, name);
...
// returns TRUE if it finished a link trade, FALSE if it finished an in-game
// trade or if sequence is still going
static bool8 DoTradeAnim(void) {
  if (sTradeAnim->isCableTrade)
...
// Below are the states for the main switch in DoTradeAnim_Cable and
// DoTradeAnim_Wireless When DoTradeAnim_Wireless has a unique version of a
// state used by DoTradeAnim_Cable, it adds the below modifier
...
static bool8 DoTradeAnim_Cable(void) {
  ...
  case STATE_START:
    ...
    gSprites[sTradeAnim->monSpriteIds[TRADE_PLAYER]].y2 =
        gMonFrontPicCoords[sTradeAnim->monSpecies[TRADE_PLAYER]].y_offset;
    sTradeAnim->state++;
  ...
  case STATE_BYE_BYE:
    if (++sTradeAnim->timer == 80) {
      sTradeAnim->releasePokeballSpriteId = CreateTradePokeballSprite(
          sTradeAnim->monSpriteIds[TRADE_PLAYER],
          gSprites[sTradeAnim->monSpriteIds[TRADE_PLAYER]].oam.paletteNum, 120,
          32, 2, 1, 0x14, 0xfffff);
      sTradeAnim->state++;
  ...
  case STATE_POKEBALL_DEPART:
    if (gSprites[sTradeAnim->releasePokeballSpriteId].callback ==
      ...
      sTradeAnim->bouncingPokeballSpriteId =
          CreateSprite(&sSpriteTemplate_Pokeball, 120, 32, 0);
  ...
  case STATE_GBA_FLASH_SEND:
    if (++sTradeAnim->timer > 20) {
      SetTradeBGAffine();
      sTradeAnim->connectionSpriteId2 =
          CreateSprite(&sSpriteTemplate_GbaScreenFlash_Long, 120, 80, 0);
      sTradeAnim->state++;
    }
    break;
  case STATE_GBA_STOP_FLASH_SEND:
    if (gSprites[sTradeAnim->connectionSpriteId2].animEnded) {
      ...
      SetGpuReg(REG_OFFSET_BLDCNT,
                BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_BG1 | BLDCNT_TGT2_BG2);
  ...
  case STATE_PAN_AWAY_GBA:
    ...
    if (sTradeAnim->bg1vofs == 328)
      sTradeAnim->cableEndSpriteId =
          CreateSprite(&sSpriteTemplate_CableEnd, 128, 65, 0);
    break;
  case STATE_CREATE_LINK_MON_LEAVING:
    sTradeAnim->connectionSpriteId1 =
        CreateSprite(&sSpriteTemplate_LinkMonGlow, 128, 80, 3);
    sTradeAnim->connectionSpriteId2 =
        CreateSprite(&sSpriteTemplate_LinkMonShadow, 128, 80, 0);
    StartSpriteAnim(&gSprites[sTradeAnim->connectionSpriteId2],
                    ANIM_LINKMON_SMALL);
    sTradeAnim->state++;
    break;
  ...
  case STATE_FADE_IN_TO_CROSSING:
    BeginNormalPaletteFade(PALETTES_ALL, -1, 16, 0, RGB_BLACK);
    sTradeAnim->connectionSpriteId1 =
        CreateSprite(&sSpriteTemplate_LinkMonShadow, 111, 170, 0);
    sTradeAnim->connectionSpriteId2 =
        CreateSprite(&sSpriteTemplate_LinkMonShadow, 129, -10, 0);
    sTradeAnim->state++;
    break;
  ...
  case STATE_CROSSING_CREATE_MON_PICS:
    if (!IsMonSpriteNotFlipped(sTradeAnim->monSpecies[TRADE_PLAYER])) {
      gSprites[sTradeAnim->monSpriteIds[TRADE_PLAYER]].affineAnims =
          sAffineAnims_CrossingMonPics;
      gSprites[sTradeAnim->monSpriteIds[TRADE_PLAYER]].oam.affineMode =
          ST_OAM_AFFINE_DOUBLE;
      CalcCenterToCornerVec(&gSprites[sTradeAnim->monSpriteIds[TRADE_PLAYER]],
                            SPRITE_SHAPE(64x64), SPRITE_SIZE(64x64),
                            ST_OAM_AFFINE_DOUBLE);
      StartSpriteAffineAnim(&gSprites[sTradeAnim->monSpriteIds[TRADE_PLAYER]],
                            0);
    } else {
      StartSpriteAffineAnim(&gSprites[sTradeAnim->monSpriteIds[TRADE_PLAYER]],
                            0);
    }
    StartSpriteAffineAnim(&gSprites[sTradeAnim->monSpriteIds[TRADE_PARTNER]],
                          0);
  ...
  case STATE_CREATE_LINK_MON_ARRIVING:
    if (!gPaletteFade.active) {
      ...
      sTradeAnim->connectionSpriteId1 =
          CreateSprite(&sSpriteTemplate_LinkMonGlow, 128, -20, 3);
      sTradeAnim->connectionSpriteId2 =
          CreateSprite(&sSpriteTemplate_LinkMonShadow, 128, -20, 0);
      StartSpriteAnim(&gSprites[sTradeAnim->connectionSpriteId2],
                      ANIM_LINKMON_SMALL);
    }
    break;
  ...
  case STATE_LINK_MON_TRAVEL_IN:
    gSprites[sTradeAnim->connectionSpriteId1].y2 += 3;
    gSprites[sTradeAnim->connectionSpriteId2].y2 += 3;
    if (gSprites[sTradeAnim->connectionSpriteId1].y2 +
            gSprites[sTradeAnim->connectionSpriteId1].y ==
        64)
      sTradeAnim->state++;
    break;
  ...
  case STATE_POKEBALL_ARRIVE:
    sTradeAnim->bouncingPokeballSpriteId =
        CreateSprite(&sSpriteTemplate_Pokeball, 120, -8, 0);
    ...
    BlendPalettes(
        1 << (16 +
              gSprites[sTradeAnim->bouncingPokeballSpriteId].oam.paletteNum),
        16, RGB_WHITEALPHA);
    sTradeAnim->state++;
    sTradeAnim->timer = 0;
    break;
  ...
  case STATE_POKEBALL_ARRIVE_WAIT:
    if (gSprites[sTradeAnim->bouncingPokeballSpriteId].callback ==
        SpriteCallbackDummy) {
      HandleLoadSpecialPokePic_2(
          &gMonFrontPicTable[sTradeAnim->monSpecies[TRADE_PARTNER]],
  ...
  case STATE_SHOW_NEW_MON:
    ...
    gSprites[sTradeAnim->monSpriteIds[TRADE_PARTNER]].y =
        gMonFrontPicCoords[sTradeAnim->monSpecies[TRADE_PARTNER]].y_offset + 60;
    ...
    CreatePokeballSpriteToReleaseMon(
        sTradeAnim->monSpriteIds[TRADE_PARTNER],
        gSprites[sTradeAnim->monSpriteIds[TRADE_PARTNER]].oam.paletteNum, 120,
        84, 2, 1, 20, PALETTES_BG | (0xF << 16),
        sTradeAnim->monSpecies[TRADE_PARTNER]);
  ...
  case STATE_END_LINK_TRADE:
    if (sTradeAnim->isLinkTrade)
      ...
      sTradeAnim->state++;
    break;
  case STATE_TRY_EVOLUTION: // Only if in-game trade, link trades use
                            // CB2_TryLinkTradeEvolution
    TradeMons(gSpecialVar_0x8005, 0);
    gCB2_AfterEvolution = CB2_InGameTrade;
    evoTarget = GetEvolutionTargetSpecies(
        &gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]], EVO_MODE_TRADE,
        ITEM_NONE);
    if (evoTarget != SPECIES_NONE)
      TradeEvolutionScene(
          &gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]], evoTarget,
          sTradeAnim->monSpriteIds[TRADE_PARTNER],
...
static bool8 DoTradeAnim_Wireless(void) {
  ...
  case STATE_GBA_FLASH_SEND_WIRELESS:
    if (++sTradeAnim->timer > 20) {
      ...
      sTradeAnim->connectionSpriteId2 =
          CreateSprite(&sSpriteTemplate_GbaScreenFlash_Short, 120, 80, 0);
      sTradeAnim->state++;
    }
    break;
  case STATE_GBA_STOP_FLASH_SEND_WIRELESS:
    if (gSprites[sTradeAnim->connectionSpriteId2].animEnded) {
      ...
      SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_TGT1_BG1 | BLDCNT_TGT1_OBJ |
                                       BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_BG2);
  ...
  case STATE_CREATE_LINK_MON_LEAVING:
    sTradeAnim->connectionSpriteId1 =
        CreateSprite(&sSpriteTemplate_LinkMonGlow, 120, 80, 3);
    gSprites[sTradeAnim->connectionSpriteId1].callback =
        SpriteCB_LinkMonGlowWireless;
    sTradeAnim->connectionSpriteId2 =
        CreateSprite(&sSpriteTemplate_LinkMonShadow, 120, 80, 0);
    StartSpriteAnim(&gSprites[sTradeAnim->connectionSpriteId2],
                    ANIM_LINKMON_SMALL);
    sTradeAnim->state++;
    break;
  ...
  case STATE_CREATE_LINK_MON_ARRIVING:
    if (!gPaletteFade.active) {
      ...
      sTradeAnim->connectionSpriteId1 =
          CreateSprite(&sSpriteTemplate_LinkMonGlow, 120, -20, 3);
      gSprites[sTradeAnim->connectionSpriteId1].callback =
          SpriteCB_LinkMonGlowWireless;
      sTradeAnim->connectionSpriteId2 =
          CreateSprite(&sSpriteTemplate_LinkMonShadow, 120, -20, 0);
      StartSpriteAnim(&gSprites[sTradeAnim->connectionSpriteId2],
                      ANIM_LINKMON_SMALL);
    }
    break;
...
// In-game trades resolve evolution during the trade sequence, in
// STATE_TRY_EVOLUTION
...
static void SpriteCB_BouncingPokeballDepart(struct Sprite *sprite) {
  ...
  if (++sprite->data[0] == 44) {
    ...
    sprite->data[0] = 0;
    BeginNormalPaletteFade(1 << (16 + sprite->oam.paletteNum), -1, 0, 16,
                           RGB_WHITEALPHA);
  }
}
...
static void SpriteCB_BouncingPokeballDepartEnd(struct Sprite *sprite) {
  ...
  if (++sprite->data[1] > 20) {
    ...
    if (++sprite->data[0] == 23) {
      DestroySprite(sprite);
      sTradeAnim->state =
          STATE_FADE_OUT_TO_GBA_SEND; // Resume the master trade animation
    }
  }
}
...
static void CreateInGameTradePokemonInternal(u8 whichPlayerMon,
                                             u8 whichInGameTrade) {
  ...
  CreateMon(pokemon, inGameTrade->species, level, USE_RANDOM_IVS, TRUE,
            inGameTrade->personality, OT_ID_PRESET, inGameTrade->otId);
...
static void GetInGameTradeMail(struct Mail *mail,
                               const struct InGameTrade *trade) {
  s32 i;
...
static void CB2_WaitTradeComplete(void) {
  ...
  if (IsWirelessTrade()) {
    ...
    HandleLinkDataReceive();
    if (mpId == 0 && sTradeAnim->playerFinishStatus == STATUS_READY &&
        sTradeAnim->partnerFinishStatus == STATUS_READY) {
  ...
  case 50:
    ...
    if (gWirelessCommType)
      MysteryGift_TryIncrementStat(
          CARD_STAT_NUM_TRADES, gLinkPlayers[GetMultiplayerId() ^ 1].trainerId);
...
void LoadTradeAnimGfx(void) { TradeAnimInit_LoadGfx(); }
...
void DrawTextOnTradeWindow(u8 windowId, const u8 *str, u8 speed) {
  ...
  AddTextPrinterParameterized4(windowId, FONT_NORMAL, 0, 2, 0, 0,
                               sTradeAnim->textColors, speed, str);
  CopyWindowToVram(windowId, COPYWIN_FULL);
}
...
static void Task_AnimateWirelessSignal(u8 taskId) {
  ...
  if (!tSignalComingBack) {
    if (paletteIdx == 256)
      ...
      LoadPalette(&sWirelessSignalSend_Pal[paletteIdx], BG_PLTT_ID(3),
                  PLTT_SIZE_4BPP);
  } else {
    if (paletteIdx == 256)
      LoadPalette(sWirelessSignalNone_Pal, BG_PLTT_ID(3), PLTT_SIZE_4BPP);
    else
      LoadPalette(&sWirelessSignalRecv_Pal[paletteIdx], BG_PLTT_ID(3),
                  PLTT_SIZE_4BPP);
  }
...
static void Task_OpenCenterWhiteColumn(u8 taskId) {
  ...
  if (data[0] == 0) {
    sTradeAnim->wirelessWinLeft = sTradeAnim->wirelessWinRight =
        DISPLAY_WIDTH / 2;
    ...
    SetGpuReg(REG_OFFSET_WININ,
              WININ_WIN0_BG0 | WININ_WIN0_BG1 | WININ_WIN0_OBJ);
  }

  SetGpuReg(REG_OFFSET_WIN0H, WIN_RANGE2(sTradeAnim->wirelessWinLeft,
                                         sTradeAnim->wirelessWinRight));
  SetGpuReg(REG_OFFSET_WIN0V, WIN_RANGE2(sTradeAnim->wirelessWinTop,
                                         sTradeAnim->wirelessWinBottom));
