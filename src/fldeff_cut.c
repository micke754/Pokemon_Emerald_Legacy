...
static const struct HyperCutterUnk sHyperCutStruct[] = {
    {-2, -2, {1}}, {-1, -2, {1}}, {0, -2, {2}}, {1, -2, {3}},
    {2, -2, {3}},  {-2, -1, {1}}, {2, -1, {3}}, {-2, 0, {4}},
    {2, 0, {6}},   {-2, 1, {7}},  {2, 1, {9}},  {-2, 2, {7}},
    ...
    if (userAbility != ABILITY_HYPER_CUTTER) {
      ...
      for (i = 0; i < 16; i++) {
        ...
        if (tileCuttable == TRUE) {
          if (MapGridGetElevationAt(x, y) == gPlayerFacingPosition.elevation) {
            u8 tileArrayId =
                ((sHyperCutStruct[i].y * 5) + 12) + (sHyperCutStruct[i].x);
            tileBehavior = MapGridGetMetatileBehaviorAt(x, y);
            if (MetatileBehavior_IsPokeGrass(tileBehavior) == TRUE ||
                MetatileBehavior_IsAshGrass(tileBehavior) == TRUE) {
              gFieldCallback2 = FieldCallback_PrepareFadeInFromMenu;
...
bool8 FldEff_CutGrass(void) {
  ...
  for (i = 0; i < CUT_SPRITE_ARRAY_COUNT; i++) {
    sCutGrassSpriteArrayPtr[i] = CreateSprite(
        &sSpriteTemplate_CutGrass, gSprites[gPlayerAvatar.spriteId].oam.x + 8,
  ...
  for (i = 0; i < sCutSquareSide; i++) {
    ...
    if (MapGridGetMetatileIdAt(currentX, y) == METATILE_General_LongGrass) {
      ...
      case LONG_GRASS_FIELD:
        MapGridSetMetatileIdAt(currentX, y + 1,
    ...
    if (MapGridGetMetatileIdAt(currentX, lowerY) == METATILE_General_Grass) {
      ...
      if (MapGridGetMetatileIdAt(currentX, lowerY + 1) ==
        ...
        MapGridSetMetatileIdAt(currentX, lowerY + 1,
...
static void HandleLongGrassOnHyper(u8 caseId, s16 x, s16 y) {
  ...
  if (arr[0] == TRUE) {
    ...
    if (MapGridGetMetatileIdAt(newX, y + 3) ==
        METATILE_Fortree_SecretBase_LongGrass_BottomLeft)
      MapGridSetMetatileIdAt(newX, y + 3,
                             METATILE_Fortree_SecretBase_LongGrass_TopLeft);
    if (MapGridGetMetatileIdAt(newX, y + 3) ==
        METATILE_Fortree_SecretBase_LongGrass_BottomMid)
      MapGridSetMetatileIdAt(newX, y + 3,
  ...
  if (arr[1] == TRUE) {
    ...
    if (MapGridGetMetatileIdAt(newX, y + 4) ==
        METATILE_Fortree_SecretBase_LongGrass_BottomLeft)
      MapGridSetMetatileIdAt(newX, y + 4,
                             METATILE_Fortree_SecretBase_LongGrass_TopLeft);
    if (MapGridGetMetatileIdAt(newX, y + 4) ==
        METATILE_Fortree_SecretBase_LongGrass_BottomMid)
      MapGridSetMetatileIdAt(newX, y + 4,
...
static void CutGrassSpriteCallback2(struct Sprite *sprite) {
  ...
  sprite->data[0] +=
      1 + (sprite->data[3] >> 2); // right shift by 2 is dividing by 4
  sprite->data[3]++;

  if (sprite->data[1] != 28)
    sprite->data[1]++;
  else
    sprite->callback = CutGrassSpriteCallbackEnd; // done rotating the grass,
