...
static void SetEscalatorMetatile(u8 taskId, const s16 *metatileIds,
                                 u16 metatileMasks);
...
static void SetEscalatorMetatile(u8 taskId, const s16 *metatileIds,
  ...
  // Check all the escalator sections and only progress the selected one to the
  // next stage
      ...
      for (j = 0; j < 3; j++) {
        ...
        if (metatileIds[transitionStage] == metatileId) {
          if (transitionStage != LAST_ESCALATOR_STAGE)
            ...
            MapGridSetMetatileIdAt(x + j, y + i,
                                   metatileMasks | metatileIds[0]);
        }
      }
    }
  } else {
      ...
      for (j = 0; j < 3; j++) {
        ...
        if (metatileIds[LAST_ESCALATOR_STAGE - transitionStage] == metatileId) {
          if (transitionStage != LAST_ESCALATOR_STAGE)
            ...
            MapGridSetMetatileIdAt(x + j, y + i,
                                   metatileMasks |
                                       metatileIds[LAST_ESCALATOR_STAGE]);
        }
      }
    }
  }
...
static void Task_DrawEscalator(u8 taskId) {
  ...
  // Set tile for each section of the escalator in sequence for current
  // transition stage
  switch (tState) {
  case 0:
  ...
  case 1:
    ...
    break;
  case 2:
    SetEscalatorMetatile(taskId, sEscalatorMetatiles_1F_2,
  ...
  case 3:
    ...
    break;
  case 4:
    SetEscalatorMetatile(taskId, sEscalatorMetatiles_2F_0,
                         MAPGRID_COLLISION_MASK);
    break;
  case 5:
  ...
  // If all metatiles of the escalator have been set, draw map and progress to
  // next stage
  if (tState == 0) {
    DrawWholeMapView();
...
void StopEscalator(void) { DestroyTask(sEscalatorAnim_TaskId); }
