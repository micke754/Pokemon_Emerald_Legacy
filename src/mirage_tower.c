...
#define MIRAGE_TOWER_GFX_LENGTH                                                \
  (sizeof(sBlankTile_Gfx) + sizeof(sMirageTower_Gfx))
...
static const u8 sMirageTower_Gfx[] =
    INCBIN_U8("graphics/misc/mirage_tower.4bpp");
static const u16 sMirageTowerTilemap[] =
    INCBIN_U16("graphics/misc/mirage_tower.bin");
static const u16 sFossil_Pal[] =
    INCBIN_U16("graphics/object_events/pics/misc/fossil.gbapal"); // Unused
static const u8 sFossil_Gfx[] = INCBIN_U8(
    "graphics/object_events/pics/misc/fossil.4bpp"); // Duplicate of
                                                     // gObjectEventPic_Fossil
static const u8 sMirageTowerCrumbles_Gfx[] =
    INCBIN_U8("graphics/misc/mirage_tower_crumbles.4bpp");
...
static const struct SpriteSheet sCeilingCrumbleSpriteSheets[] = {
    {sMirageTowerCrumbles_Gfx, sizeof(sMirageTowerCrumbles_Gfx),
     TAG_CEILING_CRUMBLE},
    {}};
...
// Holds data about the disintegration effect for Mirage Tower / the unchosen
// fossil. Never read, presumably for debugging
...
void ClearMirageTowerPulseBlend(void) { sMirageTowerPulseBlend = NULL; }
...
void TryStartMirageTowerPulseBlendEffect(void) {
  ...
  InitPulseBlendPaletteSettings(&sMirageTowerPulseBlend->pulseBlend,
                                &gMirageTowerPulseBlendSettings);
  MarkUsedPulseBlendPalettes(&sMirageTowerPulseBlend->pulseBlend, 0x1, TRUE);
  sMirageTowerPulseBlend->taskId =
      CreateTask(UpdateMirageTowerPulseBlend, 0xFF);
}
...
static void PlayerDescendMirageTower(u8 taskId) {
  ...
  TryGetObjectEventIdByLocalIdAndMap(
      LOCALID_ROUTE111_PLAYER_FALLING, gSaveBlock1Ptr->location.mapNum,
  ...
  if ((gSprites[fallingPlayer->spriteId].y +
       gSprites[fallingPlayer->spriteId].y2) >=
...
static void StartScreenShake(u8 yShakeOffset, u8 xShakeOffset, u8 numShakes,
                             u8 shakeDelay) {
...
static void WaitCeilingCrumble(u8 taskId) {
  ...
  data[1]++;
  // Either wait 1000 frames, or until all 16 crumble sprites and the one
  // screen-shake task are completed.
...
static void CreateCeilingCrumbleSprites(void) {
  ...
  for (i = 0; i < 8; i++) {
    spriteId = CreateSprite(&sSpriteTemplate_CeilingCrumbleLarge,
  ...
  for (i = 0; i < 8; i++) {
    spriteId = CreateSprite(&sSpriteTemplate_CeilingCrumbleSmall,
...
static void SpriteCB_CeilingCrumble(struct Sprite *sprite) {
  ...
  if (((sprite->y) + (sprite->y2)) >
      sCeilingCrumblePositions[sprite->data[0]][2]) {
...
void StartMirageTowerShake(void) { CreateTask(InitMirageTowerShake, 9); }
...
static void InitMirageTowerShake(u8 taskId) {
  ...
  case 3:
    ...
    CopyToBgTilemapBufferRect_ChangePalette(0, &sMirageTowerTilemap, 12, 29, 6,
                                            12, 17);
...
static void DoMirageTowerDisintegration(u8 taskId) {
  ...
  case 1:
    sFallingTower =
        AllocZeroed(OUTER_BUFFER_LENGTH * sizeof(struct FallAnim_Tower));
    break;
  case 3:
    if (gTasks[taskId].data[3] <= (OUTER_BUFFER_LENGTH - 1)) {
      if (gTasks[taskId].data[1] > 1) {
        ...
        for (i = 0; i <= (INNER_BUFFER_LENGTH - 1); i++) {
          ...
          SWAP(sFallingTower[index].disintegrateRand[rand2],
               sFallingTower[index].disintegrateRand[rand1], temp);
        }
        if (gTasks[taskId].data[3] <= (OUTER_BUFFER_LENGTH - 1))
      ...
      for (j = 0; j < 1; j++) {
        UpdateDisintegrationEffect(
            ...
            (OUTER_BUFFER_LENGTH - 1 - i) * INNER_BUFFER_LENGTH +
                sFallingTower[i]
                    .disintegrateRand[sFallingTower[i].disintegrateIdx++],
    ...
    LoadBgTiles(0, sMirageTowerGfxBuffer, MIRAGE_TOWER_GFX_LENGTH, 0);
    if (sFallingTower[OUTER_BUFFER_LENGTH - 1].disintegrateIdx >
        INNER_BUFFER_LENGTH - 1)
      break;
    return;
  case 4:
...
static void Task_FossilFallAndSink(u8 taskId) {
  ...
  case 1:
    ...
    sFallingFossil->frameImage =
        AllocZeroed(sizeof(*sFallingFossil->frameImage));
    sFallingFossil->disintegrateRand =
        AllocZeroed(FOSSIL_DISINTEGRATE_LENGTH * sizeof(u16));
  ...
  case 6:
    ...
    for (i = 0; i < FOSSIL_DISINTEGRATE_LENGTH * sizeof(u16); i++) {
      ...
      SWAP(sFallingFossil->disintegrateRand[rand2],
           sFallingFossil->disintegrateRand[rand1], temp);
    }
  ...
  if (sFallingFossil->disintegrateIdx >= FOSSIL_DISINTEGRATE_LENGTH) {
    ...
    for (i = 0; i < 2; i++)
      UpdateDisintegrationEffect(
          ...
          sFallingFossil->disintegrateRand[sFallingFossil->disintegrateIdx++],
          0, 16, 0);
...
static void UpdateDisintegrationEffect(u8 *tiles, u16 randId, u8 c, u8 size,
                                       u8 offset) {
