...
static void AnimMegahornHorn(struct Sprite *sprite) {
  ...
  sprite->x = GetBattlerSpriteCoord2(gBattleAnimTarget, BATTLER_COORD_X_2) +
              gBattleAnimArgs[0];
  sprite->y =
...
static void AnimTranslateStinger(struct Sprite *sprite) {
  ...
  if (!IsContest() && GetBattlerSide(gBattleAnimAttacker) ==
  ...
  lVarX = GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_X_2) +
          gBattleAnimArgs[2];
  lVarY = GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y_PIC_OFFSET) +
          gBattleAnimArgs[3];
...
// Rotates sprite and moves it in an arc, so that it appears like a missle or
// arrow traveling. arg 0: initial x pixel offset arg 1: initial y pixel offset
// arg 2: target x pixel offset
// arg 3: target y pixel offset
// arg 4: duration
...
static void AnimMissileArc_Step(struct Sprite *sprite) {
  ...
  if (TranslateAnimHorizontalArc(sprite)) {
    ...
    if (!TranslateAnimHorizontalArc(sprite)) {
      u16 rotation =
          ArcTan2Neg(sprite->x + sprite->x2 - x2, sprite->y + sprite->y2 - y2);
      rotation += 0xC000;
