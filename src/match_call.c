...
// In this file only the values normally associated with Battle Pike and Factory
// are swapped. Note that this is *not* a bug, because they are properly swapped
// consistently in this file. There would only be an issue if anything in this
// file interacted with something expecting the usual value order, or vice
// versa.
...
#define BATTLE_TEXT_IDS(msgId)                                                 \
  {TEXT_ID(B_TOPIC_WILD, (msgId)), TEXT_ID(B_TOPIC_NEGATIVE, (msgId)),         \
   TEXT_ID(B_TOPIC_POSITIVE, (msgId))}
...
static EWRAM_DATA struct BattleFrontierStreakInfo sBattleFrontierStreakInfo = {
    0};
...
static const struct MatchCallTrainerTextInfo sMatchCallTrainers[] = {
    ...
    },
    {
        ...
        .unused = 0,
        // Thalia and Sawyer are the only ones who use different msg ids for
        // their battle topics
        .battleTopicTextIds = {TEXT_ID(B_TOPIC_WILD, 8),
    ...
    },
    {
        ...
        // Thalia and Sawyer are the only ones who use different msg ids for
        // their battle topics
        .battleTopicTextIds = {TEXT_ID(B_TOPIC_WILD, 15),
...
static const struct MatchCallText sMatchCallPersonalizedTexts[] = {
    ...
    {.text = MatchCall_PersonalizedText13,
     .stringVarFuncIds = {STR_TRAINER_NAME, STR_SPECIES_IN_ROUTE, STR_NONE}},
    ...
    {.text = MatchCall_PersonalizedText18,
     .stringVarFuncIds = {STR_TRAINER_NAME, STR_SPECIES_IN_PARTY, STR_NONE}},
...
static bool32 UpdateMatchCallMinutesCounter(void) {
  ...
  if (sMatchCallState.minutes > curMinutes ||
      curMinutes - sMatchCallState.minutes > 9) {
...
static bool32 SelectMatchCallTrainer(void) {
  ...
  sMatchCallState.trainerId =
      GetActiveMatchCallTrainerId(Random() % numRegistered);
  ...
  if (!((TrainerIsEligibleForRematch(matchCallId) &&
         GetRematchTrainerLocation(matchCallId) ==
...
/*
    ...
    - Every 10th step
    - Every 10 minutes
    - 1/3 of the time (or 2/3 of the time, if the lead party Pokémon has
   Lightning Rod)
    - If in a valid outdoor map (not Safari Zone, not underwater, not Mt Chimney
   with Team Magma, not Sootopolis with legendaries)
    - If an eligible trainer to call the player is selected
*/
bool32 TryStartMatchCall(void) {
  if (FlagGet(FLAG_HAS_MATCH_CALL) && UpdateMatchCallStepCounter() &&
      UpdateMatchCallMinutesCounter() && CheckMatchCallChance() &&
      MapAllowsMatchCall() && SelectMatchCallTrainer()) {
    StartMatchCall();
    return TRUE;
  }
...
static const u16 sMatchCallWindow_Pal[] =
    INCBIN_U16("graphics/pokenav/match_call/window.gbapal");
static const u8 sMatchCallWindow_Gfx[] =
    INCBIN_U8("graphics/pokenav/match_call/window.4bpp");
static const u16 sPokenavIcon_Pal[] =
    INCBIN_U16("graphics/pokenav/match_call/nav_icon.gbapal");
static const u32 sPokenavIcon_Gfx[] =
    INCBIN_U32("graphics/pokenav/match_call/nav_icon.4bpp.lz");
...
static bool32 (*const sMatchCallTaskFuncs[])(u8) = {
    MatchCall_LoadGfx,        MatchCall_DrawWindow, MatchCall_ReadyIntro,
    MatchCall_SlideWindowIn,  MatchCall_PrintIntro, MatchCall_PrintMessage,
    MatchCall_SlideWindowOut, MatchCall_EndCall,
};
...
static bool32 MatchCall_LoadGfx(u8 taskId) {
  ...
  if (LoadBgTiles(0, sMatchCallWindow_Gfx, sizeof(sMatchCallWindow_Gfx),
                  TILE_MC_WINDOW) == 0xFFFF) {
    RemoveWindow(tWindowId);
    DestroyTask(taskId);
    return FALSE;
  }

  if (!DecompressAndCopyTileDataToVram(0, sPokenavIcon_Gfx, 0,
                                       TILE_POKENAV_ICON, 0)) {
    RemoveWindow(tWindowId);
    DestroyTask(taskId);
    return FALSE;
  ...
  LoadPalette(sMatchCallWindow_Pal, BG_PLTT_ID(14),
              sizeof(sMatchCallWindow_Pal));
...
static bool32 MatchCall_DrawWindow(u8 taskId) {
  ...
  WriteSequenceToBgTilemapBuffer(0, (0xF << 12) | TILE_POKENAV_ICON, 1, 15, 4,
                                 4, 17, 1);
...
static bool32 MatchCall_PrintMessage(u8 taskId) {
  s16 *data = gTasks[taskId].data;
  if (!RunMatchCallTextPrinter(tWindowId) && !IsSEPlaying() &&
...
static bool32 MatchCall_EndCall(u8 taskId) {
  ...
  if (!IsDma3ManagerBusyWithBgCopy() && !IsSEPlaying()) {
    ...
    if (!sMatchCallState.triggeredFromScript) {
      ...
      playerObjectId =
          GetObjectEventIdByLocalIdAndMap(OBJ_EVENT_ID_PLAYER, 0, 0);
...
static void DrawMatchCallTextBoxBorder_Internal(u32 windowId, u32 tileOffset,
                                                u32 paletteId) {
  ...
  FillBgTilemapBufferRect_Palette0(bg,
                                   ((paletteId << 12) & 0xF000) | (tileNum + 4),
                                   x + width, y, 1, height);
  FillBgTilemapBufferRect_Palette0(bg,
                                   ((paletteId << 12) & 0xF000) | (tileNum + 5),
                                   x - 1, y + height, 1, 1);
  FillBgTilemapBufferRect_Palette0(bg,
                                   ((paletteId << 12) & 0xF000) | (tileNum + 6),
                                   x, y + height, width, 1);
  FillBgTilemapBufferRect_Palette0(bg,
                                   ((paletteId << 12) & 0xF000) | (tileNum + 7),
...
// Look through the rematch table for trainers that have been defeated once
// before. Return the index into the rematch table of the nth defeated trainer,
...
static const struct MatchCallText *GetSameRouteMatchCallText(int matchCallId,
                                                             u8 *str) {
...
static const struct MatchCallText *
GetDifferentRouteMatchCallText(int matchCallId, u8 *str) {
...
static const struct MatchCallText *GetBattleMatchCallText(int matchCallId,
                                                          u8 *str) {
  int mask;
  u32 textId, topic, id;
...
static const struct MatchCallText *GetGeneralMatchCallText(int matchCallId,
                                                           u8 *str) {
  int i;
  int count;
  u32 topic, id;
  ...
  if (!(rand & 1)) {
    ...
    if (count) {
      ...
      id = sMatchCallTrainers[matchCallId].battleFrontierRecordStreakTextIndex -
           1;
...
static void BuildMatchCallString(int matchCallId,
                                 const struct MatchCallText *matchCallText,
                                 u8 *str) {
...
static u8 *const sMatchCallTextStringVars[] = {gStringVar1, gStringVar2,
                                               gStringVar3};

static void PopulateMatchCallStringVars(int matchCallId,
                                        const s8 *stringVarFuncIds) {
  int i;
  for (i = 0; i < NUM_STRVARS_IN_MSG; i++) {
    if (stringVarFuncIds[i] >= 0)
      PopulateMatchCallStringVar(matchCallId, stringVarFuncIds[i],
...
static void PopulateMatchCallStringVar(int matchCallId, int funcId,
                                       u8 *destStr) {
...
static void PopulateSpeciesFromTrainerLocation(int matchCallId, u8 *destStr) {
  ...
  if (gWildMonHeaders[i].mapGroup !=
      MAP_GROUP(UNDEFINED)) // ??? This check is nonsense.
  {
    ...
    if (gWildMonHeaders[i].mapGroup != MAP_GROUP(UNDEFINED)) {
      ...
      if (gWildMonHeaders[i].landMonsInfo) {
        ...
        species[numSpecies] =
            gWildMonHeaders[i].landMonsInfo->wildPokemon[slot].species;
        numSpecies++;
      }
      ...
      if (gWildMonHeaders[i].waterMonsInfo) {
        ...
        species[numSpecies] =
            gWildMonHeaders[i].waterMonsInfo->wildPokemon[slot].species;
        numSpecies++;
      }
...
static void PopulateSpeciesFromTrainerParty(int matchCallId, u8 *destStr) {
  ...
  trainerId =
      GetLastBeatenRematchTrainerId(sMatchCallTrainers[matchCallId].trainerId);
...
static void PopulateBattleFrontierStreak(int matchCallId, u8 *destStr) {
  ...
  ConvertIntToDecimalStringN(destStr, sBattleFrontierStreakInfo.streak,
                             STR_CONV_MODE_LEFT_ALIGN, i);
}
...
// Whether or not a trainer calling the player from a different route should
// request a battle
static bool32 ShouldTrainerRequestBattle(int matchCallId) {
  ...
  rand = ((dayCount ^ dewfordRand) +
          (dewfordRand ^ GetGameStat(GAME_STAT_TRAINER_BATTLES))) ^
         otId;
  n = rand % max;
...
static u16 GetFrontierStreakInfo(u16 facilityId, u32 *topicTextId) {
  ...
  case FRONTIER_FACILITY_DOME:
    for (i = 0;
         i < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.domeRecordWinStreaks);
         i++) {
  ...
  case FRONTIER_FACILITY_TOWER:
    for (i = 0;
         i < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.towerRecordWinStreaks);
         i++) {
      for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++) {
  ...
  case FRONTIER_FACILITY_PALACE:
    for (i = 0;
         i < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.palaceRecordWinStreaks);
         i++) {
      for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++) {
  ...
  case MATCH_CALL_FACTORY:
    for (i = 0;
         i < (int)ARRAY_COUNT(gSaveBlock2Ptr->frontier.factoryRecordWinStreaks);
         i++) {
      for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++) {
...
static u8 GetPokedexRatingLevel(u16 numSeen) {
  ...
  if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_DEOXYS),
                        FLAG_GET_CAUGHT))
    numSeen--;
  if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_JIRACHI),
                        FLAG_GET_CAUGHT))
    numSeen--;
...
static const u8 *const sBirchDexRatingTexts[] = {
    gBirchDexRatingText_LessThan10,   gBirchDexRatingText_LessThan20,
    gBirchDexRatingText_LessThan30,   gBirchDexRatingText_LessThan40,
    gBirchDexRatingText_LessThan50,   gBirchDexRatingText_LessThan60,
    gBirchDexRatingText_LessThan70,   gBirchDexRatingText_LessThan80,
    ...
    gBirchDexRatingText_LessThan150,  gBirchDexRatingText_LessThan160,
    gBirchDexRatingText_LessThan170,  gBirchDexRatingText_LessThan180,
    gBirchDexRatingText_LessThan190,  gBirchDexRatingText_LessThan200,
...
void BufferPokedexRatingForMatchCall(u8 *destStr) {
  ...
  ConvertIntToDecimalStringN(gStringVar2, numCaught, STR_CONV_MODE_LEFT_ALIGN,
                             3);
  ...
  *(str++) = CHAR_PROMPT_CLEAR;
  ...
  if (IsNationalPokedexEnabled()) {
    *(str++) = CHAR_PROMPT_CLEAR;
    ...
    ConvertIntToDecimalStringN(gStringVar1, numSeen, STR_CONV_MODE_LEFT_ALIGN,
                               3);
    ConvertIntToDecimalStringN(gStringVar2, numCaught, STR_CONV_MODE_LEFT_ALIGN,
                               3);
...
void LoadMatchCallWindowGfx(u32 windowId, u32 destOffset, u32 paletteId) {
  ...
  LoadPalette(sMatchCallWindow_Pal, BG_PLTT_ID(paletteId),
              sizeof(sMatchCallWindow_Pal));
}
