...
static const union AnimCmd sSpriteAnim_ItemIcon[] = {ANIMCMD_FRAME(0, 0),
                                                     ANIMCMD_END};
...
u8 AddItemIconSprite(u16 tilesTag, u16 paletteTag, u16 itemId) {
  if (!AllocItemIconTemporaryBuffers()) {
    ...
    LZDecompressWram(GetItemIconPicOrPalette(itemId, 0),
                     gItemIconDecompressionBuffer);
    CopyItemIconPicTo4x4Buffer(gItemIconDecompressionBuffer,
                               gItemIcon4x4Buffer);
    ...
    CpuCopy16(&gItemIconSpriteTemplate, spriteTemplate,
              sizeof(*spriteTemplate));
...
u8 AddCustomItemIconSprite(const struct SpriteTemplate *customSpriteTemplate,
                           u16 tilesTag, u16 paletteTag, u16 itemId) {
  if (!AllocItemIconTemporaryBuffers()) {
    return MAX_SPRITES;
  } else {
    u8 spriteId;
