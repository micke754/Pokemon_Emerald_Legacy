...
static void BuyMenuPrint(u8 windowId, const u8 *text, u8 x, u8 y, s8 speed,
                         u8 colorSet);
...
static void BuyMenuDrawMapMetatileLayer(u16 *dest, s16 offset1, s16 offset2,
                                        const u16 *src);
...
static void BuyMenuPrintItemDescriptionAndShowItemIcon(s32 item, bool8 onInit,
...
static u8 CreateShopMenu(u8 martType) {
  ...
  if (martType == MART_TYPE_NORMAL) {
    struct WindowTemplate winTemplate =
        sShopMenuWindowTemplates[WIN_BUY_SELL_QUIT];
    winTemplate.width =
        GetMaxWidthInMenuTable(sShopMenuActions_BuySellQuit,
    ...
    winTemplate.width = GetMaxWidthInMenuTable(
        sShopMenuActions_BuyQuit, ARRAY_COUNT(sShopMenuActions_BuyQuit));
    sMartInfo.windowId = AddWindow(&winTemplate);
...
static void Task_HandleShopMenuQuit(u8 taskId) {
  ClearStdWindowAndFrameToTransparent(
      sMartInfo.windowId, 2); // Incorrect use, making it not copy it to vram.
...
static void Task_ReturnToShopMenu(u8 taskId) {
  if (IsWeatherNotFadingIn() == TRUE) {
    if (sMartInfo.martType == MART_TYPE_DECOR2)
      DisplayItemMessageOnField(taskId, gText_CanIHelpWithAnythingElse,
                                ShowShopMenuAfterExitingBuyOrSellMenu);
    else
      DisplayItemMessageOnField(taskId, gText_AnythingElseICanHelp,
                                ShowShopMenuAfterExitingBuyOrSellMenu);
  }
}
...
static void BuyMenuBuildListMenuTemplate(void) {
  ...
  for (i = 0; i < sMartInfo.itemCount; i++)
    BuyMenuSetListEntry(&sListMenuItems[i], sMartInfo.itemList[i],
                        sItemNames[i]);
...
static void BuyMenuSetListEntry(struct ListMenuItem *menuItem, u16 item,
                                u8 *name) {
...
static void BuyMenuPrintPriceInList(u8 windowId, u32 itemId, u8 y) {
  ...
  if (itemId != LIST_CANCEL) {
    if (sMartInfo.martType == MART_TYPE_NORMAL) {
      ...
      ConvertIntToDecimalStringN(gStringVar1, gDecorations[itemId].price,
                                 STR_CONV_MODE_LEFT_ALIGN, 5);
    }

    StringExpandPlaceholders(gStringVar4, gText_PokedollarVar1);
    x = GetStringRightAlignXOffset(FONT_NARROW, gStringVar4, 120);
    AddTextPrinterParameterized4(windowId, FONT_NARROW, x, y, 0, 0,
                                 sShopBuyMenuTextColors[COLORID_ITEM_LIST],
...
static void BuyMenuAddScrollIndicatorArrows(void) {
  if (sShopData->scrollIndicatorsTaskId == TASK_NONE &&
      sMartInfo.itemCount + 1 > MAX_ITEMS_SHOWN) {
    sShopData->scrollIndicatorsTaskId =
        AddScrollIndicatorArrowPairParameterized(
            SCROLL_ARROW_UP, 172, 12, 148,
            sMartInfo.itemCount - (MAX_ITEMS_SHOWN - 1), TAG_SCROLL_ARROW,
...
static void BuyMenuAddItemIcon(u16 item, u8 iconSlot) {
  ...
  if (sMartInfo.martType == MART_TYPE_NORMAL || item == ITEM_LIST_END) {
    spriteId = AddItemIconSprite(iconSlot + TAG_ITEM_ICON_BASE,
                                 iconSlot + TAG_ITEM_ICON_BASE, item);
    ...
    spriteId =
        AddDecorationIconObject(item, 20, 84, 1, iconSlot + TAG_ITEM_ICON_BASE,
                                iconSlot + TAG_ITEM_ICON_BASE);
    if (spriteId != MAX_SPRITES)
      *spriteIdPtr = spriteId;
  }
}
...
static void BuyMenuInitBgs(void) {
  ...
  InitBgsFromTemplates(0, sShopBuyMenuBgTemplates,
                       ARRAY_COUNT(sShopBuyMenuBgTemplates));
  ...
  SetGpuReg(REG_OFFSET_DISPCNT,
            DISPCNT_MODE_0 | DISPCNT_OBJ_ON | DISPCNT_OBJ_1D_MAP);
  ShowBg(0);
  ShowBg(1);
  ShowBg(2);
  ShowBg(3);
...
static void BuyMenuPrint(u8 windowId, const u8 *text, u8 x, u8 y, s8 speed,
                         u8 colorSet) {
  AddTextPrinterParameterized4(windowId, FONT_NORMAL, x, y, 0, 0,
                               sShopBuyMenuTextColors[colorSet], speed, text);
}

static void BuyMenuDisplayMessage(u8 taskId, const u8 *text,
                                  TaskFunc callback) {
  DisplayMessageAndContinueTask(taskId, WIN_MESSAGE, 10, 14, FONT_NORMAL,
...
static void BuyMenuDrawGraphics(void) {
  ...
  PrintMoneyAmountInMoneyBoxWithBorder(WIN_MONEY, 1, 13,
    ...
    for (i = 0; i < 15; i++) {
      ...
      if (metatile < NUM_METATILES_IN_PRIMARY)
        BuyMenuDrawMapMetatile(i, j,
                               mapLayout->primaryTileset->metatiles +
                                   metatile * NUM_TILES_PER_METATILE,
                               metatileLayerType);
      else
        BuyMenuDrawMapMetatile(i, j,
                               mapLayout->secondaryTileset->metatiles +
...
static void BuyMenuDrawMapMetatile(s16 x, s16 y, const u16 *src,
  ...
  case METATILE_LAYER_TYPE_NORMAL:
    BuyMenuDrawMapMetatileLayer(sShopData->tilemapBuffers[3], offset1, offset2,
                                src);
    BuyMenuDrawMapMetatileLayer(sShopData->tilemapBuffers[1], offset1, offset2,
                                src + 4);
    break;
  case METATILE_LAYER_TYPE_COVERED:
    BuyMenuDrawMapMetatileLayer(sShopData->tilemapBuffers[2], offset1, offset2,
                                src);
    BuyMenuDrawMapMetatileLayer(sShopData->tilemapBuffers[3], offset1, offset2,
                                src + 4);
    break;
...
static bool8 BuyMenuCheckIfObjectEventOverlapsMenuBg(s16 *object) {
  if (!BuyMenuCheckForOverlapWithMenuBg(object[X_COORD], object[Y_COORD] + 2) &&
      object[LAYER_TYPE] != METATILE_LAYER_TYPE_COVERED)
    return TRUE;
  else
    return FALSE;
}
...
static void Task_BuyMenu(u8 taskId) {
  ...
  if (!gPaletteFade.active) {
    ...
    ListMenuGetScrollAndRow(tListTaskId, &sShopData->scrollOffset,
    ...
    default:
      ...
      if (!IsEnoughMoney(&gSaveBlock1Ptr->money, sShopData->totalCost)) {
        BuyMenuDisplayMessage(taskId, gText_YouDontHaveMoney,
        ...
        if (sMartInfo.martType == MART_TYPE_NORMAL) {
          ...
          if (ItemId_GetPocket(itemId) == POCKET_TM_HM) {
            ...
            BuyMenuDisplayMessage(taskId, gText_Var1CertainlyHowMany2,
                                  Task_BuyHowManyDialogueInit);
          } else {
            BuyMenuDisplayMessage(taskId, gText_Var1CertainlyHowMany,
                                  Task_BuyHowManyDialogueInit);
          }
        } else {
          StringCopy(gStringVar1, gDecorations[itemId].name);
          ConvertIntToDecimalStringN(gStringVar2, sShopData->totalCost,
...
static void Task_BuyHowManyDialogueInit(u8 taskId) {
  ...
  ConvertIntToDecimalStringN(gStringVar1, quantityInBag,
                             STR_CONV_MODE_RIGHT_ALIGN, MAX_ITEM_DIGITS + 1);
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId) {
  ...
  if (AdjustQuantityAccordingToDPadInput(&tItemCount, sShopData->maxQuantity) ==
      TRUE) {
    sShopData->totalCost =
        (ItemId_GetPrice(tItemId) >> IsPokeNewsActive(POKENEWS_SLATEPORT)) *
        tItemCount;
    BuyMenuPrintItemQuantityAndPrice(taskId);
  } else {
    if (JOY_NEW(A_BUTTON)) {
...
static void BuyMenuConfirmPurchase(u8 taskId) {
  CreateYesNoMenuWithCallbacks(taskId, &sShopBuyMenuYesNoWindowTemplates, 1, 0,
                               0, 1, 13, &sShopPurchaseYesNoFuncs);
}
...
static void BuyMenuTryMakePurchase(u8 taskId) {
  ...
  if (sMartInfo.martType == MART_TYPE_NORMAL) {
    if (AddBagItem(tItemId, tItemCount) == TRUE) {
      BuyMenuDisplayMessage(taskId, gText_HereYouGoThankYou,
                            BuyMenuSubtractMoney);
      RecordItemPurchase(taskId);
    } else {
      BuyMenuDisplayMessage(taskId, gText_NoMoreRoomForThis,
                            BuyMenuReturnToItemList);
    }
  } else {
    if (DecorationAdd(tItemId)) {
      if (sMartInfo.martType == MART_TYPE_DECOR)
        BuyMenuDisplayMessage(taskId, gText_ThankYouIllSendItHome,
                              BuyMenuSubtractMoney);
      else // MART_TYPE_DECOR2
        BuyMenuDisplayMessage(taskId, gText_ThanksIllSendItHome,
                              BuyMenuSubtractMoney);
    } else {
      BuyMenuDisplayMessage(taskId, gText_SpaceForVar1Full,
                            BuyMenuReturnToItemList);
    }
  }
}
...
static void Task_ReturnToItemListAfterItemPurchase(u8 taskId) {
  ...
  if (JOY_NEW(A_BUTTON | B_BUTTON)) {
    ...
    if (tItemId == ITEM_POKE_BALL && tItemCount >= 10 &&
        AddBagItem(ITEM_PREMIER_BALL, 1) == TRUE)
      BuyMenuDisplayMessage(taskId, gText_ThrowInPremierBall,
                            BuyMenuReturnToItemList);
    else
      BuyMenuReturnToItemList(taskId);
  }
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId) {
  ...
  PrintMoneyAmount(WIN_QUANTITY_PRICE, 38, 1, sShopData->totalCost,
                   TEXT_SKIP_DRAW);
...
static void RecordItemPurchase(u8 taskId) {
  ...
  for (i = 0; i < ARRAY_COUNT(gMartPurchaseHistory); i++) {
    if (gMartPurchaseHistory[i].itemId == tItemId &&
