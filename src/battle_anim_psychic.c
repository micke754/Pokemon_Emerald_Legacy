...
static const union AnimCmd sAnim_BentSpoon_1[] = {
    ANIMCMD_FRAME(8, 60), ANIMCMD_FRAME(16, 5), ANIMCMD_FRAME(8, 5),
    ANIMCMD_FRAME(0, 5),  ANIMCMD_FRAME(8, 22), ANIMCMD_LOOP(0),
    ANIMCMD_FRAME(16, 5), ANIMCMD_FRAME(8, 5),  ANIMCMD_FRAME(0, 5),
    ANIMCMD_FRAME(8, 5),  ANIMCMD_LOOP(1),      ANIMCMD_FRAME(8, 22),
    ANIMCMD_FRAME(24, 3), ANIMCMD_FRAME(32, 3), ANIMCMD_FRAME(40, 22),
    ANIMCMD_END,
};
...
static const union AnimCmd sAnim_QuestionMark[] = {
    ...
    ANIMCMD_FRAME(48, 6),  ANIMCMD_FRAME(64, 6), ANIMCMD_FRAME(80, 6),
    ANIMCMD_FRAME(96, 18), ANIMCMD_END,
};
...
static void AnimDefensiveWall_Step2(struct Sprite *sprite) {
  SetGpuReg(REG_OFFSET_BLDALPHA,
            BLDALPHA_BLEND(sprite->data[3], 16 - sprite->data[3]));
  if (sprite->data[3] == 13)
...
static void AnimQuestionMark(struct Sprite *sprite) {
  s16 x =
      GetBattlerSpriteCoordAttr(gBattleAnimAttacker, BATTLER_COORD_ATTR_WIDTH) /
      2;
  s16 y = GetBattlerSpriteCoordAttr(gBattleAnimAttacker,
...
void AnimTask_ImprisonOrbs(u8 taskId) {
  ...
  var0 =
      GetBattlerSpriteCoordAttr(gBattleAnimAttacker, BATTLER_COORD_ATTR_WIDTH) /
      3;
  var1 = GetBattlerSpriteCoordAttr(gBattleAnimAttacker,
                                   BATTLER_COORD_ATTR_HEIGHT) /
         3;
...
static void AnimTask_ImprisonOrbs_Step(u8 taskId) {
  ...
  case 0:
    if (++task->data[1] > 8) {
      task->data[1] = 0;
      spriteId = CreateSprite(&gImprisonOrbSpriteTemplate, task->data[13],
                              task->data[14], 0);
...
void AnimTask_SkillSwap(u8 taskId) {
  ...
  if (IsContest()) {
    if (gBattleAnimArgs[0] == ANIM_TARGET) {
      task->data[10] = -10;
      task->data[11] = GetBattlerSpriteCoordAttr(gBattleAnimTarget,
                                                 BATTLER_COORD_ATTR_RIGHT) -
                       8;
      task->data[12] =
          GetBattlerSpriteCoordAttr(gBattleAnimTarget, BATTLER_COORD_ATTR_TOP) +
          8;
      task->data[13] = GetBattlerSpriteCoordAttr(gBattleAnimAttacker,
                                                 BATTLER_COORD_ATTR_RIGHT) -
                       8;
      task->data[14] = GetBattlerSpriteCoordAttr(gBattleAnimAttacker,
                                                 BATTLER_COORD_ATTR_TOP) +
                       8;
    } else {
      task->data[10] = 10;
      task->data[11] = GetBattlerSpriteCoordAttr(gBattleAnimAttacker,
                                                 BATTLER_COORD_ATTR_LEFT) +
                       8;
      task->data[12] = GetBattlerSpriteCoordAttr(gBattleAnimAttacker,
                                                 BATTLER_COORD_ATTR_BOTTOM) -
                       8;
      task->data[13] = GetBattlerSpriteCoordAttr(gBattleAnimTarget,
                                                 BATTLER_COORD_ATTR_LEFT) +
                       8;
      task->data[14] = GetBattlerSpriteCoordAttr(gBattleAnimTarget,
                                                 BATTLER_COORD_ATTR_BOTTOM) -
                       8;
    }
  } else {
...
static void AnimTask_SkillSwap_Step(u8 taskId) {
  ...
  case 0:
    if (++task->data[1] > 6) {
      task->data[1] = 0;
      spriteId = CreateSprite(&gSkillSwapOrbSpriteTemplate, task->data[11],
                              task->data[12], 0);
      if (spriteId != MAX_SPRITES) {
...
// The scanline effect that distorts the target during Extrasensory by
// segmenting the mon vertically and shifting the slices arg0: Stage. Stage 0 is
// a slight right distortion, 1 is a medium left distortion, and 2 is a severe
// right distortion
...
// Creates a cloned transparent sprite of the battler that grows and then
// shrinks back to original size. Used by Extrasensory arg0: battler
...
static void AnimPsychoBoost(struct Sprite *sprite) {
  ...
  case 1:
    if (sprite->affineAnimEnded) {
      PlaySE12WithPanning(SE_M_TELEPORT,
