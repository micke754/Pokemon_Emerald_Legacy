...
enum {
  WIN_KB_PAGE_1, // Which of these two windows is in front is cycled as the
                 // player swaps
  WIN_KB_PAGE_2, // Initially WIN_KB_PAGE_1 is in front, with WIN_KB_PAGE_2 on
                 // deck
  WIN_TEXT_ENTRY,
  WIN_TEXT_ENTRY_BOX,
  WIN_BANNER,
...
// The constants for the pages are needlessly complicated because GF didn't keep
// the indexing order consistent This set is used for
// sNamingScreen->currentPage. It uses the order that the pages are cycled in
enum {
  KBPAGE_SYMBOLS,
...
// This set is used for initializing a page's keyboard text and getting its
// number of columns
enum {
...
static const u8 sPCIconOff_Gfx[] =
    INCBIN_U8("graphics/naming_screen/pc_icon_off.4bpp");
static const u8 sPCIconOn_Gfx[] =
    INCBIN_U8("graphics/naming_screen/pc_icon_on.4bpp");
static const u16 sKeyboard_Pal[] =
    INCBIN_U16("graphics/naming_screen/keyboard.gbapal");
static const u16 sRival_Pal[] = INCBIN_U16(
    "graphics/naming_screen/rival.gbapal"); // Unused, leftover from FRLG rival

static const u8 *const sTransferredToPCMessages[] = {
    gText_PkmnTransferredSomeonesPC, gText_PkmnTransferredLanettesPC,
...
// The keys shown on the keyboard are handled separately by
...
void DoNamingScreen(u8 templateNum, u8 *destBuffer, u16 monSpecies,
                    u16 monGender, u32 monPersonality,
                    MainCallback returnCallback) {
...
static void NamingScreen_Init(void) {
  ...
  sNamingScreen->inputCharBaseXPos =
      (DISPLAY_WIDTH - sNamingScreen->template->maxChars * 8) / 2 + 6;
...
static u8 PageToNextGfxId(u8 page) { return sPageToNextGfxId[page]; }
...
static bool8 MainState_FadeIn(void) {
  ...
  PrintKeyboardKeys(sNamingScreen->windows[WIN_KB_PAGE_2],
                    KEYBOARD_LETTERS_LOWER);
...
static bool8 MainState_HandleInput(void) { return HandleKeyboardEvent(); }
...
static void DisplaySentToPCMessage(void) {
  ...
  AddTextPrinterParameterized2(
      0, FONT_NORMAL, gStringVar4, GetPlayerTextSpeedDelay(), 0,
...
static bool8 PageSwapAnimState_1(struct Task *task) {
  u16 *const vOffsets[] = {&sNamingScreen->bg2vOffset,
                           &sNamingScreen->bg1vOffset};

  task->tFrameCount += 4;
  *vOffsets[sNamingScreen->bgToReveal] = Sin(task->tFrameCount, 40);
  *vOffsets[sNamingScreen->bgToHide] =
      Sin((task->tFrameCount + 128) & 0xFF, 40);
...
static void TryStartButtonFlash(u8 button, bool8 keepFlashing,
                                bool8 interruptCurFlash) {
...
static void Task_UpdateButtonFlash(u8 taskId) {
  ...
  MultiplyInvertedPaletteRGBComponents(GetButtonPalOffset(task->tButtonId),
                                       task->tColor, task->tColor,
                                       task->tColor);
...
static u16 GetButtonPalOffset(u8 button) {
  const u16 palOffsets[BUTTON_COUNT + 1] = {
      [BUTTON_PAGE] =
          OBJ_PLTT_ID(IndexOfSpritePaletteTag(PALTAG_PAGE_SWAP)) + 14,
      [BUTTON_BACK] =
          OBJ_PLTT_ID(IndexOfSpritePaletteTag(PALTAG_BACK_BUTTON)) + 14,
      [BUTTON_OK] = OBJ_PLTT_ID(IndexOfSpritePaletteTag(PALTAG_OK_BUTTON)) + 14,
      [BUTTON_COUNT] =
          OBJ_PLTT_ID(IndexOfSpritePaletteTag(PALTAG_OK_BUTTON)) + 1,
  };
...
static void SpriteCB_Cursor(struct Sprite *sprite) {
  ...
  if (sprite->invisible || !(sprite->sFlashing) ||
...
static const u8 sButtonKeyRoles[] = {KEY_ROLE_PAGE, KEY_ROLE_BACKSPACE,
                                     KEY_ROLE_OK};
...
// If the cursor's x is equal to the column count, cursor is in the button
// column
static u8 GetCurrentPageColumnCount(void) {
...
static bool8 PageSwapSprite_Init(struct Sprite *sprite) {
  ...
  SetPageSwapButtonGfx(PageToNextGfxId(sNamingScreen->currentPage), text,
                       button);
  sprite->sState++;
  return FALSE;
}

static bool8 PageSwapSprite_Idle(struct Sprite *sprite) { return FALSE; }
...
static bool8 PageSwapSprite_SlideOff(struct Sprite *sprite) {
  ...
  if (text->y2 > 7) {
    ...
    SetPageSwapButtonGfx(
        PageToNextGfxId(((u8)sprite->sPage + 1) % KBPAGE_COUNT), text, button);
  }
  return FALSE;
}
...
static void SetPageSwapButtonGfx(u8 page, struct Sprite *text,
                                 struct Sprite *button) {
...
static void NamingScreen_CreatePlayerIcon(void) {
  ...
  rivalGfxId = GetRivalAvatarGraphicsIdByStateIdAndGender(
      PLAYER_AVATAR_STATE_NORMAL, sNamingScreen->monSpecies);
  spriteId =
      CreateObjectGraphicsSprite(rivalGfxId, SpriteCallbackDummy, 56, 37, 0);
  gSprites[spriteId].oam.priority = 3;
...
static void NamingScreen_CreateMonIcon(void) {
  ...
  LoadMonIconPalettes();
  spriteId = CreateMonIcon(sNamingScreen->monSpecies, SpriteCallbackDummy, 56,
                           40, 0, sNamingScreen->monPersonality, 1);
  gSprites[spriteId].oam.priority = 3;
}
...
static void NamingScreen_CreateWaldaDadIcon(void) {
  ...
  spriteId = CreateObjectGraphicsSprite(OBJ_EVENT_GFX_MAN_1,
                                        SpriteCallbackDummy, 56, 37, 0);
  gSprites[spriteId].oam.priority = 3;
  StartSpriteAnim(&gSprites[spriteId], ANIM_STD_GO_SOUTH);
}
...
static void CreateInputHandlerTask(void) { CreateTask(Task_HandleInput, 1); }
...
static void DrawNormalTextEntryBox(void) {
  FillWindowPixelBuffer(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX],
                        PIXEL_FILL(1));
  AddTextPrinterParameterized(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX],
                              FONT_NORMAL, sNamingScreen->template->title, 8, 1,
                              0, 0);
...
static void DrawGenderIcon(void) {
  ...
  if (sNamingScreen->monGender != MON_GENDERLESS) {
    ...
    AddTextPrinterParameterized3(sNamingScreen->windows[WIN_TEXT_ENTRY],
                                 FONT_NORMAL, (POKEMON_NAME_LENGTH * 4) + 64, 1,
                                 sGenderColors[isFemale], TEXT_SKIP_DRAW, text);
  }
}
...
static void SaveInputText(void) {
  ...
  for (i = 0; i < sNamingScreen->template->maxChars; i++) {
    if (sNamingScreen->textBuffer[i] != CHAR_SPACE &&
        sNamingScreen->textBuffer[i] != EOS) {
      StringCopyN(sNamingScreen->destBuffer, sNamingScreen->textBuffer,
...
static void LoadGfx(void) {
  ...
  LoadBgTiles(1, sNamingScreen->tileBuffer, sizeof(sNamingScreen->tileBuffer),
              0);
  LoadBgTiles(2, sNamingScreen->tileBuffer, sizeof(sNamingScreen->tileBuffer),
              0);
  LoadBgTiles(3, sNamingScreen->tileBuffer, sizeof(sNamingScreen->tileBuffer),
              0);
...
static void LoadPalettes(void) {
  LoadPalette(gNamingScreenMenu_Pal, BG_PLTT_ID(0),
              sizeof(gNamingScreenMenu_Pal));
...
static void DrawTextEntry(void) {
  ...
  for (i = 0; i < maxChars; i++) {
    ...
    AddTextPrinterParameterized(sNamingScreen->windows[WIN_TEXT_ENTRY],
                                FONT_NORMAL, temp, i * 8 + x + extraWidth, 1,
                                TEXT_SKIP_DRAW, NULL);
  }
...
// There are always 2 keyboard pages drawn, the current page and the one that
// will shown next if the player swaps When the page swap is complete this
// function invisibly replaces the old page with the new next one
static void DrawKeyboardPageOnDeck(void) {
  u8 bg;
  u8 bg_;
  u8 windowId;
...
static void PrintControls(void) {
  const u8 color[3] = {TEXT_DYNAMIC_COLOR_6, TEXT_COLOR_WHITE,
                       TEXT_COLOR_DARK_GRAY};

  FillWindowPixelBuffer(sNamingScreen->windows[WIN_BANNER], PIXEL_FILL(15));
  AddTextPrinterParameterized3(sNamingScreen->windows[WIN_BANNER], FONT_SMALL,
                               2, 1, color, 0, gText_MoveOkBack);
...
static void SetVBlank(void) { SetVBlankCallback(VBlankCB_NamingScreen); }
...
static void UNUSED Debug_NamingScreenBox(void) {
  DoNamingScreen(NAMING_SCREEN_BOX, gSaveBlock2Ptr->playerName,
                 gSaveBlock2Ptr->playerGender, 0, 0,
                 CB2_ReturnToFieldWithOpenMenu);
}

static void UNUSED Debug_NamingScreenCaughtMon(void) {
  DoNamingScreen(NAMING_SCREEN_CAUGHT_MON, gSaveBlock2Ptr->playerName,
                 gSaveBlock2Ptr->playerGender, 0, 0,
                 CB2_ReturnToFieldWithOpenMenu);
}

static void UNUSED Debug_NamingScreenNickname(void) {
  DoNamingScreen(NAMING_SCREEN_NICKNAME, gSaveBlock2Ptr->playerName,
                 gSaveBlock2Ptr->playerGender, 0, 0,
                 CB2_ReturnToFieldWithOpenMenu);
}
...
// Initial pages below are pointless, they're overwritten with
// KBPAGE_LETTERS_UPPER in MainState_FadeIn()
...
static const union AnimCmd sAnim_Loop[] = {ANIMCMD_FRAME(0, 1),
                                           ANIMCMD_JUMP(0)};
...
static const union AnimCmd *const sAnims_Cursor[] = {sAnim_Loop,
                                                     sAnim_CursorSquish};
