...
static const u16 sEggPalette[] =
    INCBIN_U16("graphics/pokemon/egg/normal.gbapal");
...
static const union AnimCmd sSpriteAnim_Egg_Normal[] = {ANIMCMD_FRAME(0, 5),
                                                       ANIMCMD_END};

static const union AnimCmd sSpriteAnim_Egg_Cracked1[] = {ANIMCMD_FRAME(16, 5),
                                                         ANIMCMD_END};

static const union AnimCmd sSpriteAnim_Egg_Cracked2[] = {ANIMCMD_FRAME(32, 5),
                                                         ANIMCMD_END};

static const union AnimCmd sSpriteAnim_Egg_Cracked3[] = {ANIMCMD_FRAME(48, 5),
                                                         ANIMCMD_END};
...
static const struct SpritePalette sEgg_SpritePalette = {.data = sEggPalette,
                                                        .tag = PALTAG_EGG};
...
static const union AnimCmd sSpriteAnim_EggShard0[] = {ANIMCMD_FRAME(0, 5),
                                                      ANIMCMD_END};

static const union AnimCmd sSpriteAnim_EggShard1[] = {ANIMCMD_FRAME(1, 5),
                                                      ANIMCMD_END};

static const union AnimCmd sSpriteAnim_EggShard2[] = {ANIMCMD_FRAME(2, 5),
                                                      ANIMCMD_END};

static const union AnimCmd sSpriteAnim_EggShard3[] = {ANIMCMD_FRAME(3, 5),
                                                      ANIMCMD_END};
...
static void CreateHatchedMon(struct Pokemon *egg, struct Pokemon *temp) {
  ...
  CreateMon(temp, species, EGG_HATCH_LEVEL, USE_RANDOM_IVS, TRUE, personality,
            OT_ID_PLAYER_ID, 0);
  ...
  SetMonData(temp, MON_DATA_MODERN_FATEFUL_ENCOUNTER,
...
void ScriptHatchMon(void) { AddHatchedMonToParty(gSpecialVar_0x8004); }

static bool8 _CheckDaycareMonReceivedMail(struct DayCare *daycare,
                                          u8 daycareId) {
  ...
  if (daycareMon->mail.message.itemId != ITEM_NONE &&
      (StringCompareWithoutExtCtrlCodes(nickname, daycareMon->mail.monName) !=
           0 ||
       StringCompareWithoutExtCtrlCodes(gSaveBlock2Ptr->playerName,
                                        daycareMon->mail.otName) != 0)) {
    StringCopy(gStringVar1, nickname);
    TVShowConvertInternationalString(gStringVar2, daycareMon->mail.otName,
                                     daycareMon->mail.gameLanguage);
    TVShowConvertInternationalString(gStringVar3, daycareMon->mail.monName,
                                     daycareMon->mail.monLanguage);
    return TRUE;
  }
  return FALSE;
}
...
bool8 CheckDaycareMonReceivedMail(void) {
  return _CheckDaycareMonReceivedMail(&gSaveBlock1Ptr->daycare,
                                      gSpecialVar_0x8004);
}

static u8 EggHatchCreateMonSprite(u8 useAlt, u8 state, u8 partyId,
                                  u16 *speciesLoc) {
  u8 position = 0;
  u8 spriteId = 0;
    ...
    // Load mon sprite gfx
    {
      ...
      HandleLoadSpecialPokePic_DontHandleDeoxys(
          ...
          gMonSpritesGfxPtr->sprites
              .ptr[(useAlt * 2) + B_POSITION_OPPONENT_LEFT],
          species, pid);
  ...
  case 1:
    // Create mon sprite
    SetMultiuseSpriteTemplateToPokemon(GetMonSpritePalStruct(mon)->tag,
                                       position);
...
static void CB2_LoadEggHatch(void) {
  ...
  case 0:
    ...
    InitBgsFromTemplates(0, sBgTemplates_EggHatch,
                         ARRAY_COUNT(sBgTemplates_EggHatch));
  ...
  case 4:
    ...
    break;
  case 5:
    EggHatchCreateMonSprite(FALSE, 0, sEggHatchData->eggPartyId,
                            &sEggHatchData->species);
    gMain.state++;
    break;
  case 6:
    sEggHatchData->monSpriteId = EggHatchCreateMonSprite(
        FALSE, 1, sEggHatchData->eggPartyId, &sEggHatchData->species);
    gMain.state++;
    break;
  case 7:
...
static void CB2_EggHatch(void) {
  ...
  case 0:
    ...
    sEggHatchData->eggSpriteId =
        CreateSprite(&sSpriteTemplate_Egg, EGG_X, EGG_Y, 5);
    ShowBg(0);
    ShowBg(1);
    sEggHatchData->state++;
  ...
  case 3:
    ...
    if (gSprites[sEggHatchData->eggSpriteId].callback == SpriteCallbackDummy) {
      species = GetMonData(&gPlayerParty[sEggHatchData->eggPartyId],
                           MON_DATA_SPECIES);
      DoMonFrontSpriteAnimation(&gSprites[sEggHatchData->monSpriteId], species,
                                FALSE, 1);
      sEggHatchData->state++;
    }
    break;
  ...
  case 5:
    ...
    EggHatchPrintMessage(sEggHatchData->windowId, gStringVar4, 0, 3,
                         TEXT_SKIP_DRAW);
  ...
  case 10:
    ...
    case 0: // Yes
      ...
      personality = GetMonData(&gPlayerParty[sEggHatchData->eggPartyId],
                               MON_DATA_PERSONALITY, 0);
      DoNamingScreen(NAMING_SCREEN_NICKNAME, gStringVar3, species, gender,
                     personality, EggHatchSetMonNickname);
      break;
    case 1: // No
    case MENU_B_PRESSED:
      sEggHatchData->state++;
...
static void SpriteCB_Egg_Shake3(struct Sprite *sprite) {
  if (++sprite->sDelayTimer > 30) {
    if (++sprite->sTimer > 38) {
      ...
      if (sprite->sTimer == 15) {
        // Third egg crack
        // This ineffectually sets the animation to the frame it's already
        // using. They likely meant to use the 3rd and final cracked frame of
        // the egg, which goes unused as a result.
        PlaySE(SE_BALL);
...
static void SpriteCB_Egg_Reveal(struct Sprite *sprite) {
  if (sprite->sTimer == 0) {
    ...
    StartSpriteAffineAnim(&gSprites[sEggHatchData->monSpriteId],
...
static void CreateRandomEggShardSprite(void) {
  ...
  CreateEggShardSprite(EGG_X, EGG_Y - 15, velocityX, velocityY, 100,
                       spriteAnimIndex);
}

static void CreateEggShardSprite(u8 x, u8 y, s16 velocityX, s16 velocityY,
                                 s16 acceleration, u8 spriteAnimIndex) {
...
static void EggHatchPrintMessage(u8 windowId, u8 *string, u8 x, u8 y,
                                 u8 speed) {
  ...
  AddTextPrinterParameterized4(windowId, FONT_NORMAL, x, y, 0, 0,
                               sEggHatchData->textColor, speed, string);
}
