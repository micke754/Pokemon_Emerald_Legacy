...
static const union AnimCmd sAnim_SmallExplosion[] = {
    ANIMCMD_FRAME(0, 9),  ANIMCMD_FRAME(16, 3), ANIMCMD_FRAME(32, 3),
    ANIMCMD_FRAME(48, 3), ANIMCMD_END,
};
...
const union AnimCmd gBreathPuffAnimCmds2[] = {
    ANIMCMD_FRAME(0, 4),  ANIMCMD_FRAME(4, 40), ANIMCMD_FRAME(8, 4),
...
const union AnimCmd gMovementWavesAnimCmds1[] = {
    ANIMCMD_FRAME(0, 8),  ANIMCMD_FRAME(16, 8), ANIMCMD_FRAME(32, 8),
    ANIMCMD_FRAME(16, 8), ANIMCMD_END,
};
...
// Rotates the attacking mon sprite downwards and then back upwards to its
// original position. No args.
void AnimTask_Withdraw(u8 taskId) {
  PrepareBattlerSpriteForRotScale(gBattlerSpriteIds[gBattleAnimAttacker],
                                  ST_OAM_OBJ_NORMAL);
...
static void AnimTask_Withdraw_Step(u8 taskId) {
  ...
  if (gTasks[taskId].data[1] == 0) {
    ...
    // this y position update gets overwritten by
...
// Moves a projectile towards the target mon. The sprite is rotated to be
// pointing in the same direction it's moving. arg 0: initial x pixel offset arg
// 1: initial y pixel offset arg 2: target x pixel offset arg 3: target y pixel
// offset arg 4: duration
...
static void AnimAirWaveProjectile(struct Sprite *sprite) {
  ...
  if (sprite->data[0]-- <= 0) {
    ...
    sprite->data[5] = 0;
    sprite->data[3] =
        MathUtil_Mul16(MathUtil_Mul16(b, a), MathUtil_Inv16(Q_8_8(1.75)));
    sprite->data[4] =
        MathUtil_Mul16(MathUtil_Mul16(c, a), MathUtil_Inv16(Q_8_8(1.75)));
...
static void AirCutterProjectileStep1(u8 taskId) {
  if (gTasks[taskId].data[0]-- <= 0) {
    ...
    spriteId = CreateSprite(&gAirWaveProjectileSpriteTemplate,
                            gTasks[taskId].data[9], gTasks[taskId].data[10],
    ...
    gTasks[taskId].data[1]++;
    PlaySE12WithPanning(SE_M_BLIZZARD2,
                        BattleAnimAdjustPanning(SOUND_PAN_ATTACKER + 1));
    if (gTasks[taskId].data[1] > 2)
...
void AnimTask_AirCutterProjectile(u8 taskId) {
  ...
  if (gBattleAnimArgs[4] & 0x80) {
    ...
    if (gBattleAnimArgs[4] >= 64) {
      u16 var = GetBattlerSpriteSubpriority(gBattleAnimTarget) +
                (gBattleAnimArgs[4] - 64);
      gTasks[taskId].data[2] = var;
    } else {
      u16 var =
          GetBattlerSpriteSubpriority(gBattleAnimTarget) - gBattleAnimArgs[4];
      gTasks[taskId].data[2] = var;
    }
  } else {
    if (gBattleAnimArgs[4] >= 64) {
...
static void AnimVoidLines(struct Sprite *sprite) {
  InitSpritePosToAnimAttacker(sprite, FALSE);
  sprite->data[0] =
      OBJ_PLTT_ID(IndexOfSpritePaletteTag(sVoidLinesSpriteTemplate.paletteTag));
...
static void AnimCoinThrow(struct Sprite *sprite) {
  ...
  r7 = GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y_PIC_OFFSET) +
       gBattleAnimArgs[3];
  if (GetBattlerSide(gBattleAnimAttacker) != B_SIDE_PLAYER)
    gBattleAnimArgs[2] = -gBattleAnimArgs[2];
...
static void AnimBulletSeed_Step1(struct Sprite *sprite) {
  ...
  u16 rand;
  s16 *ptr;
  PlaySE12WithPanning(SE_M_HORN_ATTACK,
...
// Animates a single pincer line that extends towards the center of the target
// mon. arg 0: invert
static void AnimViceGripPincer(struct Sprite *sprite) {
  ...
  sprite->data[0] = 6;
  sprite->data[2] =
      GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_X_2) + endXOffset;
  sprite->data[4] =
      GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_Y_PIC_OFFSET) +
      endYOffset;
  sprite->callback = StartAnimLinearTranslation;
...
// Animates a single pincer line that extends towards the center of the target
// mon, and then back out. arg 0: animation id
...
// versions of the mon's sprite while it is shrinking. No args.
  ...
  case 3:
    if (!RunAffineAnimFromTaskData(task)) {
      if (--task->data[2] == 0) {
        ...
        PrepareAffineAnimInTaskData(task, task->data[0],
...
static void AnimBreathPuff(struct Sprite *sprite) {
  if (GetBattlerSide(gBattleAnimAttacker) == B_SIDE_PLAYER) {
    ...
    sprite->x =
        GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_X_2) + 32;
    sprite->data[1] = 64;
  } else {
    StartSpriteAnim(sprite, 1);
    sprite->x =
...
static void AnimAngerMark(struct Sprite *sprite) {
  ...
  sprite->x =
      GetBattlerSpriteCoord(battler, BATTLER_COORD_X_2) + gBattleAnimArgs[1];
...
static void AnimHyperVoiceRing(struct Sprite *sprite) {
  ...
  if (GetBattlerSide(battler1) != B_SIDE_PLAYER) {
    ...
    if (IsBattlerSpriteVisible(BATTLE_PARTNER(battler2)))
      ...
      sprite->subpriority =
          gSprites[gBattlerSpriteIds[battler2]].subpriority - 1;
  } else {
    startX = GetBattlerSpriteCoord(battler1, xCoordType) - gBattleAnimArgs[0];
    if (!IsContest() && IsBattlerSpriteVisible(BATTLE_PARTNER(battler1))) {
      if (gSprites[gBattlerSpriteIds[battler1]].x <
          gSprites[gBattlerSpriteIds[BATTLE_PARTNER(battler1)]].x)
        sprite->subpriority =
            gSprites[gBattlerSpriteIds[BATTLE_PARTNER(battler1)]].subpriority +
            1;
      else
        sprite->subpriority =
            gSprites[gBattlerSpriteIds[battler1]].subpriority - 1;
    } else {
      sprite->subpriority =
          gSprites[gBattlerSpriteIds[battler1]].subpriority - 1;
    }
...
static void AnimUproarRing(struct Sprite *sprite) {
  ...
  if (index != 0xFF) {
    BlendPalette((OBJ_PLTT_ID(index) + 1), 15, gBattleAnimArgs[5],
                 gBattleAnimArgs[4]);
  }
...
static void AnimTask_SpeedDust_Step(u8 taskId) {
  ...
  case 0:
    if (++task->data[1] > 4) {
      u8 spriteId;
      task->data[1] = 0;
      spriteId = CreateSprite(&gSpeedDustSpriteTemplate, task->data[14],
                              task->data[15], 0);
...
void AnimTask_LoadMusicNotesPals(u8 taskId) {
  ...
  for (i = 0; i < NUM_MUSIC_NOTE_PAL_TAGS; i++)
    LoadPalette(&gMonSpritesGfxPtr->buffer[i * 32],
                (u16)(OBJ_PLTT_ID(paletteNums[i])), PLTT_SIZE_4BPP);
...
static void SetMusicNotePalette(struct Sprite *sprite, u8 a, u8 b) {
  ...
  sprite->oam.paletteNum =
      IndexOfSpritePaletteTag(sMusicNotePaletteTagsTable[b >> 1]);
}
...
void AnimTask_FakeOut(u8 taskId) {
  ...
  SetGpuReg(REG_OFFSET_WININ,
            WININ_WIN0_BG_ALL | WININ_WIN0_OBJ | WININ_WIN1_ALL);
...
void AnimTask_HeartsBackground(u8 taskId) {
  ...
  SetGpuReg(REG_OFFSET_BLDCNT,
            BLDCNT_TGT2_ALL | BLDCNT_TGT1_BG1 | BLDCNT_EFFECT_BLEND);
  ...
  AnimLoadCompressedBgGfx(animBg.bgId, gBattleAnimBgImage_Attract,
                          animBg.tilesOffset);
  AnimLoadCompressedBgTilemapHandleContest(&animBg,
                                           gBattleAnimBgTilemap_Attract, FALSE);
  LoadCompressedPalette(gBattleAnimBgPalette_Attract,
...
static void AnimTask_HeartsBackground_Step(u8 taskId) {
  ...
  case 0:
    if (++gTasks[taskId].data[10] == 4) {
      ...
      SetGpuReg(REG_OFFSET_BLDALPHA,
                BLDALPHA_BLEND(gTasks[taskId].data[11],
                               16 - gTasks[taskId].data[11]));
...
void AnimTask_ScaryFace(u8 taskId) {
  ...
  if (IsContest())
    AnimLoadCompressedBgTilemapHandleContest(
        &animBg, &gBattleAnimBgTilemap_ScaryFaceContest, FALSE);
  else if (GetBattlerSide(gBattleAnimTarget) == B_SIDE_OPPONENT)
    AnimLoadCompressedBgTilemapHandleContest(
        &animBg, &gBattleAnimBgTilemap_ScaryFacePlayer, FALSE);
  else
    AnimLoadCompressedBgTilemapHandleContest(
        &animBg, &gBattleAnimBgTilemap_ScaryFaceOpponent, FALSE);

  AnimLoadCompressedBgGfx(animBg.bgId, gBattleAnimBgImage_ScaryFace,
                          animBg.tilesOffset);
  LoadCompressedPalette(gBattleAnimBgPalette_ScaryFace,
                        BG_PLTT_ID(animBg.paletteId), PLTT_SIZE_4BPP);
...
void AnimTask_UproarDistortion(u8 taskId) {
  ...
  PrepareAffineAnimInTaskData(&gTasks[taskId], spriteId,
...
static void AnimPerishSongMusicNote(struct Sprite *sprite) {
  ...
  if (!sprite->data[0]) {
    sprite->x = 120;
    sprite->y =
        (gBattleAnimArgs[0] + (((u16)gBattleAnimArgs[0]) >> 31)) / 2 - 15;
...
static void AnimGuardRing(struct Sprite *sprite) {
  if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) &&
    ...
    SetAverageBattlerPositions(gBattleAnimAttacker, FALSE, &sprite->x,
                               &sprite->y);
    sprite->y += 40;
    ...
    sprite->y =
        GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_Y) + 40;
  }
