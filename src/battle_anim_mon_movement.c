...
// Task to facilitate simple shaking of a pokemon's picture in battle.
// The shaking alternates between the positive and negative versions of the
// specified pixel offsets. arg 0: anim battler arg 1: x pixel offset arg 2: y
// pixel offset arg 3: num times to shake arg 4: frame delay
...
// Task to facilitate simple shaking of a pokemon's picture in battle.
// The shaking alternates between the positive and negative versions of the
// specified pixel offsets with respect to the current location of the mon's
// picture. arg 0: battler arg 1: x offset arg 2: y offset arg 3: num shakes arg
// 4: delay
...
// The sprite parameter is a dummy sprite used for facilitating the movement
// with its callback. arg 0: 1 = target or 0 = attacker arg 1: direction (0 =
// horizontal and vertical, 1 = horizontal only, 2 = vertical only) arg 2:
// duration
static void SlideMonToOriginalPos(struct Sprite *sprite) {
  u32 monSpriteId;
...
static void AnimTask_WindUpLunge_Step1(u8 taskId) {
  ...
  gSprites[spriteId].y2 =
      Sin((u8)(gTasks[taskId].data[10] >> 8), gTasks[taskId].data[2]);
...
static void AnimTask_WindUpLunge_Step2(u8 taskId) {
  ...
  if (gTasks[taskId].data[4] > 0) {
    ...
    gSprites[spriteId].x2 =
        (gTasks[taskId].data[12] >> 8) + (gTasks[taskId].data[11] >> 8);
...
static void AnimTask_SlideOffScreen_Step(u8 taskId) {
  ...
  if (gSprites[spriteId].x2 + gSprites[spriteId].x < -32 ||
      gSprites[spriteId].x2 + gSprites[spriteId].x > DISPLAY_WIDTH + 32) {
    DestroyAnimVisualTask(taskId);
    return;
  }
}
...
static void AnimTask_ScaleMonAndRestore_Step(u8 taskId) {
  ...
  gTasks[taskId].data[11] += gTasks[taskId].data[1];
  spriteId = gTasks[taskId].data[4];
  SetSpriteRotScale(spriteId, gTasks[taskId].data[10], gTasks[taskId].data[11],
                    0);
...
// Rotates mon to side and back to original position. For Peck and when a held
// item activates
...
static void AnimTask_RotateMonSpriteToSide_Step(u8 taskId) {
  gTasks[taskId].data[3] += gTasks[taskId].data[4];
  SetSpriteRotScale(gTasks[taskId].data[5], 0x100, 0x100,
                    gTasks[taskId].data[3]);
  if (gTasks[taskId].data[7]) {
...
void AnimTask_ShakeTargetBasedOnMovePowerOrDmg(u8 taskId) {
  ...
  gTasks[taskId].data[13] =
      gTasks[taskId].data[14] + (gTasks[taskId].data[15] & 1);
  gTasks[taskId].data[12] = 0;
