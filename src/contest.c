...
static void ContestBG_FillBoxWithIncrementingTile(u8, u16, u8, u8, u8, u8, u8,
                                                  s16);
...
EWRAM_DATA s16 gContestMonRound1Points[CONTESTANT_COUNT] = {
    0}; // "Round 1" points are based on condition
EWRAM_DATA s16 gContestMonTotalPoints[CONTESTANT_COUNT] = {
    0}; // Round 1 points + Round 2 points
EWRAM_DATA s16 gContestMonAppealPointTotals[CONTESTANT_COUNT] = {0};
EWRAM_DATA s16 gContestMonRound2Points[CONTESTANT_COUNT] = {
    0}; // "Round 2" points are just appeal points * 2
...
// The "Next Turn" sprites cover up the slider, so the y positions are the same
// as above
static const u8 sNextTurnSpriteYPositions[CONTESTANT_COUNT] = {36, 76, 116,
                                                               156};
...
static const union AffineAnimCmd sAffineAnim_SliderHeart_SpinDisappear[] = {
    AFFINEANIMCMD_FRAME(256, 256, 0, 0), AFFINEANIMCMD_FRAME(-10, -10, -20, 20),
    AFFINEANIMCMD_END};

static const union AffineAnimCmd sAffineAnim_SliderHeart_SpinAppear[] = {
    AFFINEANIMCMD_FRAME(56, 56, 0, 0), AFFINEANIMCMD_FRAME(10, 10, 20, 20),
    AFFINEANIMCMD_END};
...
static const struct CompressedSpriteSheet
    sSpriteSheet_NextTurn[CONTESTANT_COUNT] = {{.data = gContestNextTurnGfx,
                                                .size = 0x100,
...
static const struct SpritePalette sSpritePalette_NextTurn = {
    .data = gContestPal, .tag = TAG_NEXT_TURN_PAL};
...
static const struct SpritePalette sSpritePalette_ApplauseMeter = {
    .data = gContestPal, .tag = TAG_APPLAUSE_METER};
...
static const struct CompressedSpritePalette sSpritePalette_JudgeSymbols = {
    .data = gContestJudgeSymbolsPal, .tag = TAG_CONTEST_SYMBOLS_PAL};
...
static const struct SpritePalette
    sSpritePalettes_ContestantsTurnBlinkEffect[CONTESTANT_COUNT] = {
...
static const union AffineAnimCmd
    *const sAffineAnims_ContestantsTurnBlinkEffect[] = {
        sAffineAnim_ContestantsTurnBlinkEffect_0,
        sAffineAnim_ContestantsTurnBlinkEffect_1};

static const struct SpriteTemplate
    sSpriteTemplates_ContestantsTurnBlinkEffect[CONTESTANT_COUNT] = {
...
static const s8 sContestExcitementTable
    [CONTEST_CATEGORIES_COUNT][CONTEST_CATEGORIES_COUNT] = {
...
void ResetLinkContestBoolean(void) { gLinkContestFlags = 0; }
...
static void SetupContestGpuRegs(void) {
  ...
  SetGpuReg(REG_OFFSET_WININ,
            WININ_WIN0_BG0 | WININ_WIN0_BG1 | WININ_WIN0_BG2 | WININ_WIN0_BG3 |
                WININ_WIN0_OBJ | WININ_WIN0_CLR | WININ_WIN1_BG0 |
                WININ_WIN1_BG1 | WININ_WIN1_BG2 | WININ_WIN1_BG3 |
                WININ_WIN1_OBJ | WININ_WIN1_CLR);
  SetGpuReg(REG_OFFSET_WINOUT,
            WINOUT_WIN01_BG0 | WINOUT_WIN01_BG1 | WINOUT_WIN01_BG2 |
                WINOUT_WIN01_BG3 | WINOUT_WIN01_OBJ | WINOUT_WIN01_CLR |
                WINOUT_WINOBJ_BG0 | WINOUT_WINOBJ_BG1 | WINOUT_WINOBJ_BG2 |
...
void LoadContestBgAfterMoveAnim(void) {
  ...
  LoadCompressedPalette(gContestInterfaceAudiencePalette, BG_PLTT_OFFSET,
                        BG_PLTT_SIZE);
  LoadContestPalettes();
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    ...
    LoadPalette(
        eContestTempSave.cachedWindowPalettes[contestantWindowId],
        BG_PLTT_ID(5 + gContestantTurnOrder[i]),
        sizeof((eContestTempSave.cachedWindowPalettes[contestantWindowId])));
  }
}
...
static void InitContestInfoBgs(void) {
  ...
  InitBgsFromTemplates(0, sContestBgTemplates,
                       ARRAY_COUNT(sContestBgTemplates));
...
static void LoadContestPalettes(void) {
  ...
  for (i = 10; i < 14; i++)
    LoadPalette(&gPlttBufferUnfaded[BG_PLTT_ID(15) + 1], BG_PLTT_ID(15) + i,
                PLTT_SIZEOF(1));
...
static void InitContestResources(void) {
  ...
  *gContestResources->excitement = (struct ContestExcitement){};
  memset(eContestGfxState, 0,
         CONTESTANT_COUNT * sizeof(struct ContestGraphicsState));
  ...
  // will actually just reverse the turn order.
  ApplyNextTurnOrder();
  memset(gContestResources->tv, 0,
         sizeof(*gContestResources->tv) * CONTESTANT_COUNT);
}
...
static void AllocContestResources(void) {
  ...
  gContestResources->status =
      AllocZeroed(sizeof(struct ContestantStatus) * CONTESTANT_COUNT);
  gContestResources->appealResults =
      AllocZeroed(sizeof(struct ContestAppealMoveResults));
  ...
  gContestResources->tv =
      AllocZeroed(sizeof(struct ContestTV) * CONTESTANT_COUNT);
...
static void Task_CommunicateMonIdxs(u8 taskId) {
  SetTaskFuncWithFollowupFunc(taskId, Task_LinkContest_CommunicateMonIdxs,
...
static bool8 SetupContestGraphics(u8 *stateVar) {
  ...
  case 2:
    LZDecompressVram(gContestAudienceGfx, (void *)(BG_SCREEN_ADDR(4)));
    DmaCopyLarge32(3, (void *)(BG_SCREEN_ADDR(4)), eUnzippedContestAudience_Gfx,
                   0x2000, 0x1000);
    break;
  case 3:
    CopyToBgTilemapBuffer(3, gContestAudienceTilemap, 0, 0);
  ...
  case 4:
    ...
    DmaCopy32Defvars(3, gContestResources->contestBgTilemaps[2],
                     eContestTempSave.savedJunk,
                     sizeof(eContestTempSave.savedJunk));
    break;
  case 5:
    LoadCompressedPalette(gContestInterfaceAudiencePalette, BG_PLTT_OFFSET,
                          BG_PLTT_SIZE);
    CpuCopy32(&gPlttBufferUnfaded[BG_PLTT_ID(8)], tempPalette1, PLTT_SIZE_4BPP);
    CpuCopy32(&gPlttBufferUnfaded[BG_PLTT_ID(5 + gContestPlayerMonIndex)],
              tempPalette2, PLTT_SIZE_4BPP);
    CpuCopy32(tempPalette2, &gPlttBufferUnfaded[BG_PLTT_ID(8)], PLTT_SIZE_4BPP);
    CpuCopy32(tempPalette1,
              &gPlttBufferUnfaded[BG_PLTT_ID(5 + gContestPlayerMonIndex)],
              PLTT_SIZE_4BPP);
    DmaCopy32Defvars(3, gPlttBufferUnfaded,
                     eContestTempSave.cachedWindowPalettes,
                     sizeof(eContestTempSave.cachedWindowPalettes));
    LoadContestPalettes();
    break;
  case 6:
...
static void Task_DisplayAppealNumberText(u8 taskId) {
  if (gTasks[taskId].data[0] == 0) {
    ...
    ContestDebugDoPrint();
    DmaCopy32Defvars(3, gPlttBufferUnfaded,
                     eContestTempSave.cachedPlttBufferUnfaded, PLTT_SIZE);
    ConvertIntToDecimalStringN(gStringVar1, eContest.appealNumber + 1,
...
static void Task_ShowMoveSelectScreen(u8 taskId) {
  ...
  for (i = 0; i < MAX_MON_MOVES; i++) {
    ...
    if (eContestantStatus[gContestPlayerMonIndex].prevMove != MOVE_NONE &&
        ...
        AreMovesContestCombo(eContestantStatus[gContestPlayerMonIndex].prevMove,
                             move) &&
    ...
    Contest_PrintTextToBg0WindowAt(i + MOVE_WINDOWS_START, moveName, 5, 1,
                                   FONT_NARROW);
  }
...
static void Task_HandleMoveSelectInput(u8 taskId) {
  ...
  if (JOY_NEW(A_BUTTON)) {
    ...
    case DPAD_UP:
      ...
      DrawMoveSelectArrow(eContest.playerMoveChoice);
      PrintContestMoveDescription(gContestMons[gContestPlayerMonIndex]
                                      .moves[eContest.playerMoveChoice]);
      if (numMoves > 1)
        PlaySE(SE_SELECT);
      break;
    case DPAD_DOWN:
...
static void DrawMoveSelectArrow(s8 moveIndex) {
  ContestBG_FillBoxWithIncrementingTile(2, 55, 0, 31 + moveIndex * 2, 2, 2, 17,
                                        1);
}

static void EraseMoveSelectArrow(s8 moveIndex) {
  ContestBG_FillBoxWithIncrementingTile(2, 11, 0, 31 + moveIndex * 2, 2, 1, 17,
                                        1);
  ContestBG_FillBoxWithIncrementingTile(2, 11, 0, 32 + moveIndex * 2, 2, 1, 17,
                                        1);
}
...
static void Task_SelectedMove(u8 taskId) {
  if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK) {
    ...
    SetTaskFuncWithFollowupFunc(taskId2,
...
static void Task_HideMoveSelectScreen(u8 taskId) {
  ...
  DmaCopy32Defvars(3, gPlttBufferFaded, eContestTempSave.cachedPlttBufferFaded,
                   PLTT_SIZE);
...
static void Task_DoAppeals(u8 taskId) {
  ...
  case APPEALSTATE_SLIDE_MON_IN:
    for (i = 0; i < CONTESTANT_COUNT; i++)
      gBattleMonForms[i] = 0;
    memset(gContestResources->moveAnim, 0,
           sizeof(*gContestResources->moveAnim));
    ...
    BlinkContestantBox(
        CreateContestantBoxBlinkSprites(eContest.currentContestant), FALSE);
    ...
    if (eContestantStatus[contestant].nervous) {
      ...
      if (eContestantStatus[contestant].currMove < MOVES_COUNT)
        StringCopy(gStringVar2,
                   gMoveNames[eContestantStatus[contestant].currMove]);
      else
        StringCopy(gStringVar2,
                   sInvalidContestMoveNames[eContestantStatus[contestant]
                                                .moveCategory]);
  ...
  case APPEALSTATE_MOVE_ANIM: {
    u16 move =
        SanitizeMove(eContestantStatus[eContest.currentContestant].currMove);
  ...
  case APPEALSTATE_TRY_PRINT_MOVE_RESULT:
    ...
    if (eContestantStatus[contestant].effectStringId != CONTEST_STRING_NONE) {
      PrintAppealMoveResultText(contestant,
                                eContestantStatus[contestant].effectStringId);
      eContestantStatus[contestant].effectStringId = CONTEST_STRING_NONE;
      gTasks[taskId].tState = APPEALSTATE_WAIT_MOVE_RESULT_MSG;
    } else {
      if (eContestantStatus[contestant].effectStringId2 !=
          CONTEST_STRING_NONE) {
        for (i = 0; i < CONTESTANT_COUNT; i++) {
          if (i != contestant &&
              eContestantStatus[i].effectStringId != CONTEST_STRING_NONE)
  ...
  case APPEALSTATE_UPDATE_HEARTS_FROM_REPEAT:
    if (!Contest_RunTextPrinters()) {
      UpdateAppealHearts(eContestantStatus[contestant].appeal,
                         -eContestantStatus[contestant].repeatJam, contestant);
      eContestantStatus[contestant].appeal -=
  ...
  case APPEALSTATE_UPDATE_CROWD:
    if (eContestExcitement.frozen && contestant != eContestExcitement.freezer) {
      ...
      r3 = eContestExcitement
               .moveExcitement; // Can't get this to use local variable. Should
                                // be "moveExcitement"
      if (eContestantStatus[contestant].overrideCategoryExcitementMod) {
        ...
        StringCopy(gStringVar3,
                   sContestConditions
                       [gContestMoves[eContestantStatus[contestant].currMove]
                            .contestCategory]);
      }
...
static void Task_RestorePlttBufferUnfaded(u8 taskId) {
  DmaCopy32Defvars(3, eContestTempSave.cachedPlttBufferUnfaded,
                   gPlttBufferUnfaded, PLTT_SIZE);
  gTasks[taskId].data[0] = 0;
...
void CreateContestMonFromParty(u8 partyIndex) {
  ...
  memcpy(gContestMons[gContestPlayerMonIndex].trainerName, name,
         PLAYER_NAME_LENGTH + 1);
  if (gSaveBlock2Ptr->playerGender == MALE)
    gContestMons[gContestPlayerMonIndex].trainerGfxId =
  ...
  if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK) {
    StripMonNameForLinkContest(
        name, GetMonData(&gPlayerParty[partyIndex], MON_DATA_LANGUAGE));
  }
  memcpy(gContestMons[gContestPlayerMonIndex].nickname, name,
...
void SetContestants(u8 contestType, u8 rank) {
  ...
  for (i = 0; i < ARRAY_COUNT(gContestOpponents); i++) {
    if (rank == gContestOpponents[i].whichRank) {
      ...
      if (contestType == CONTEST_CATEGORY_COOL &&
          gContestOpponents[i].aiPool_Cool)
        opponents[opponentsCount++] = i;
      else if (contestType == CONTEST_CATEGORY_BEAUTY &&
               gContestOpponents[i].aiPool_Beauty)
        opponents[opponentsCount++] = i;
      else if (contestType == CONTEST_CATEGORY_CUTE &&
               gContestOpponents[i].aiPool_Cute)
        opponents[opponentsCount++] = i;
      else if (contestType == CONTEST_CATEGORY_SMART &&
               gContestOpponents[i].aiPool_Smart)
        opponents[opponentsCount++] = i;
      else if (contestType == CONTEST_CATEGORY_TOUGH &&
               gContestOpponents[i].aiPool_Tough)
        opponents[opponentsCount++] = i;
    }
  }
...
void SetLinkAIContestants(u8 contestType, u8 rank, bool32 isPostgame) {
  ...
  for (i = 0; i < ARRAY_COUNT(gContestOpponents); i++) {
    ...
    if ((contestType == CONTEST_CATEGORY_COOL &&
        ...
        (contestType == CONTEST_CATEGORY_BEAUTY &&
         gContestOpponents[i].aiPool_Beauty) ||
        (contestType == CONTEST_CATEGORY_CUTE &&
         gContestOpponents[i].aiPool_Cute) ||
        (contestType == CONTEST_CATEGORY_SMART &&
         gContestOpponents[i].aiPool_Smart) ||
        (contestType == CONTEST_CATEGORY_TOUGH &&
         gContestOpponents[i].aiPool_Tough))
      opponents[opponentsCount++] = i;
  }
  opponents[opponentsCount] = CONTESTANT_NONE;
  ...
  for (i = 0; i < CONTESTANT_COUNT - gNumLinkContestPlayers; i++) {
    ...
    gContestMons[gNumLinkContestPlayers + i] =
        gContestOpponents[opponents[rnd]];
    StripPlayerNameForLinkContest(
        gContestMons[gNumLinkContestPlayers + i].trainerName);
    StripMonNameForLinkContest(
...
static void PrintContestantTrainerName(u8 contestant) {
  PrintContestantTrainerNameWithColor(contestant,
                                      contestant + CONTESTANT_TEXT_COLOR_START);
}
...
static void PrintContestantTrainerNameWithColor(u8 contestant, u8 color) {
  ...
  offset =
      GetStringRightAlignXOffset(FONT_NARROW, gDisplayedStringBattle, 0x60);
  if (offset > 55)
    offset = 55;
  Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[contestant],
                                 gDisplayedStringBattle, offset, 1,
                                 FONT_NARROW);
}

static void PrintContestantMonName(u8 contestant) {
  PrintContestantMonNameWithColor(contestant,
                                  contestant + CONTESTANT_TEXT_COLOR_START);
}
...
static void PrintContestantMonNameWithColor(u8 contestant, u8 color) {
  ...
  Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[contestant],
                                 gDisplayedStringBattle, 5, 1, FONT_NARROW);
}
...
void CalculateRound1Points(u8 contestCategory) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++)
    gContestMonRound1Points[i] =
        CalculateContestantRound1Points(i, contestCategory);
}
...
static u8 CreateContestantSprite(u16 species, u32 otId, u32 personality,
                                 u32 index) {
  u8 spriteId;
  ...
  if (index == gContestPlayerMonIndex)
    HandleLoadSpecialPokePic_2(
        ...
        gMonSpritesGfxPtr->sprites.ptr[B_POSITION_PLAYER_LEFT], species,
        personality);
  else
  ...
  LoadCompressedPalette(
      ...
      OBJ_PLTT_ID(2), PLTT_SIZE_4BPP);
  SetMultiuseSpriteTemplateToPokemon(species, B_POSITION_PLAYER_LEFT);

  spriteId = CreateSprite(&gMultiuseSpriteTemplate, 0x70,
                          GetBattlerSpriteFinal_Y(2, species, FALSE), 30);
...
// The contestant info windows and general-purpose text box are drawn on one
// half, while the moves and move description windows are drawn on another
// screen. Only the first 32 * 20 tiles are actually drawn on screen.
static void SwapMoveDescAndContestTilemaps(void) {
  CpuCopy16(gContestResources->contestBgTilemaps[0],
            gContestResources->contestBgTilemaps[0] + 0x500, 32 * 20);
  CpuCopy16(gContestResources->contestBgTilemaps[2],
            gContestResources->contestBgTilemaps[2] + 0x500, 32 * 20);
}
...
static void PrintContestMoveDescription(u16 move) {
  ...
  ContestBG_FillBoxWithIncrementingTile(0, categoryTile, 0x0b, 0x1f, 0x05, 0x01,
                                        0x11, 0x01);
  ContestBG_FillBoxWithIncrementingTile(0, categoryTile + 0x10, 0x0b, 0x20,
                                        0x05, 0x01, 0x11, 0x01);
  ...
  ContestBG_FillBoxWithTile(0, TILE_EMPTY_APPEAL_HEART, 0x15, 0x1f,
                            MAX_CONTEST_MOVE_HEARTS, 0x01, 0x11);
  ContestBG_FillBoxWithTile(0, TILE_FILLED_APPEAL_HEART, 0x15, 0x1f, numHearts,
                            0x01, 0x11);
  ...
  if (numHearts > MAX_CONTEST_MOVE_HEARTS)
    numHearts = MAX_CONTEST_MOVE_HEARTS;
  ContestBG_FillBoxWithTile(0, TILE_EMPTY_JAM_HEART, 0x15, 0x20,
                            MAX_CONTEST_MOVE_HEARTS, 0x01, 0x11);
  ContestBG_FillBoxWithTile(0, TILE_FILLED_JAM_HEART, 0x15, 0x20, numHearts,
                            0x01, 0x11);
...
static void DrawMoveEffectSymbol(u16 move, u8 contestant) {
  ...
  if (!Contest_IsMonsTurnDisabled(contestant) && move != MOVE_NONE) {
    ...
    ContestBG_FillBoxWithIncrementingTile(0, tile, 20, contestantOffset, 2, 1,
                                          17, 1);
    ContestBG_FillBoxWithIncrementingTile(0, tile + 16, 20,
                                          contestantOffset + 1, 2, 1, 17, 1);
  } else {
...
static u16 GetStarTileOffset(void) { return 0x2034; }
...
static bool8 UpdateConditionStars(u8 contestantIdx, bool8 resetMod) {
  ...
  if (eContestantStatus[contestantIdx].conditionMod == CONDITION_GAIN) {
    ContestBG_FillBoxWithTile(0, GetStarTileOffset(), 19, contestantOffset, 1,
                              numStars, 17);
    if (resetMod) {
      PlaySE(SE_EXP_MAX);
  ...
  {
    ContestBG_FillBoxWithTile(0, 0, 19, contestantOffset + numStars, 1,
                              3 - numStars, 17);
    if (resetMod) {
...
static void DrawConditionStars(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    ...
    ContestBG_FillBoxWithTile(0, starOffset, 19, contestantOffset, 1, numStars,
                              17);
    ContestBG_FillBoxWithTile(0, 0, 19, contestantOffset + numStars, 1,
                              3 - numStars, 17);
  }
...
static bool8 DrawStatusSymbol(u8 contestant) {
  ...
  if (statused) {
    ContestBG_FillBoxWithIncrementingTile(0, symbolOffset, 20, contestantOffset,
                                          2, 1, 17, 1);
    ContestBG_FillBoxWithIncrementingTile(0, symbolOffset + 16, 20,
                                          contestantOffset + 1, 2, 1, 17, 1);
  } else {
    ContestBG_FillBoxWithTile(0, 0, 20, contestantOffset, 2, 2, 17);
  }
...
static void SetContestantStatusesForNextRound(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    ...
    eContest.moveHistory[eContest.appealNumber][i] =
        eContestantStatus[i].currMove;
...
void SaveLinkContestResults(void) {
  if ((gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK)) {
    gSaveBlock2Ptr
        ...
        ((gSaveBlock2Ptr->contestLinkResults
...
static bool8 DidContestantPlaceHigher(s32 a, s32 b,
                                      struct ContestFinalStandings *standings) {
  bool8 retVal;
...
static u8 UpdateAppealHearts(s16 startAppeal, s16 appealDelta, u8 contestant) {
  ...
  heartsDelta =
      GetNumHeartsFromAppealPoints(startAppeal + appealDelta) - startHearts;
...
static void UpdateSliderHeartSpriteYPositions(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++)
    gSprites[eContestGfxState[i].sliderHeartSpriteId].y =
        sSliderHeartYPositions[gContestantTurnOrder[i]];
}

// Used to hide (or subsequently reshow) the bottom two slider hearts that get
// hidden by text windows by moving them offscreen
...
static void CreateNextTurnSprites(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    ...
    eContestGfxState[i].nextTurnSpriteId =
        CreateSprite(&sSpriteTemplates_NextTurn[i], 204,
                     sNextTurnSpriteYPositions[gContestantTurnOrder[i]], 0);
    SetSubspriteTables(&gSprites[eContestGfxState[i].nextTurnSpriteId],
...
static void Task_StopFlashJudgeAttentionEye(u8 taskId) {
  ...
  if (gTasks[eContest.judgeAttentionTaskId].data[contestant * 4 + 0] == 0 ||
    ...
    gTasks[eContest.judgeAttentionTaskId].data[contestant * 4 + 0] = 0xFF;
    gTasks[eContest.judgeAttentionTaskId].data[contestant * 4 + 1] = 0;
    BlendPalette(BG_PLTT_ID(5 + eContest.prevTurnOrder[contestant]) + 6, 2, 0,
                 RGB(31, 31, 18));
    DestroyTask(taskId);
  }
}
...
static void Task_FlashJudgeAttentionEye(u8 taskId) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    ...
    if (gTasks[taskId].data[offset + 0] != 0xFF) {
      ...
      BlendPalette(BG_PLTT_ID(5 + eContest.prevTurnOrder[i]) + 6, 2,
                   gTasks[taskId].data[offset + 0], RGB(31, 31, 18));
    }
  }
}

// Note: While the below task is run for the entire Appeals portion of the
// contest,
//       because data[i * 4] is always 0xFF it never does anything
//       If turned on by setting that data between 0 and 16, it blends
//       an odd selection of palette colors (e.g. the text box, the appeal
//       hearts for only one contestant, the heart outlines in the move
//       selection box, etc) Given the similarities, it's possible this was an
//       incorrect attempt at something similar to what
//       CreateJudgeAttentionEyeTask does
static void CreateUnusedBlendTask(void) {
  s32 i;
...
static void UpdateBlendTaskContestantData(u8 contestant) {
  ...
  DmaCopy16Defvars(3, &gPlttBufferUnfaded[PLTT_ID(palOffset1) + 10],
                   &gPlttBufferFaded[PLTT_ID(palOffset1) + 10], PLTT_SIZEOF(1));
  palOffset2 = PLTT_ID(contestant + 5) + 12 + contestant;
  DmaCopy16Defvars(3, &gPlttBufferUnfaded[palOffset2],
                   &gPlttBufferFaded[palOffset2], PLTT_SIZEOF(1));
}
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    ...
    if (gTasks[taskId].data[idx] != 0xFF) {
      if (++gTasks[taskId].data[idx + 2] > 2) {
        ...
        BlendPalette(BG_PLTT_ID(5 + i) + 10, 1, gTasks[taskId].data[idx + 0],
                     RGB(31, 31, 18));
        BlendPalette(BG_PLTT_ID(5 + i) + 12 + i, 1,
                     gTasks[taskId].data[idx + 0], RGB(31, 31, 18));
      }
    }
  }
}
...
static u8 CreateContestantBoxBlinkSprites(u8 contestant) {
  ...
  LoadCompressedSpriteSheet(
      &sSpriteSheets_ContestantsTurnBlinkEffect[contestant]);
  LoadSpritePalette(&sSpritePalettes_ContestantsTurnBlinkEffect[contestant]);
  spriteId1 = CreateSprite(
      &sSpriteTemplates_ContestantsTurnBlinkEffect[contestant], 184, x, 29);
  spriteId2 = CreateSprite(
      &sSpriteTemplates_ContestantsTurnBlinkEffect[contestant], 248, x, 29);
  gSprites[spriteId2].oam.tileNum += 64;

  CopySpriteTiles(0, 3, (void *)VRAM,
                  (u16 *)(BG_SCREEN_ADDR(28) +
                          gContestantTurnOrder[contestant] * 5 * 64 + 0x26),
                  gContestResources->boxBlinkTiles1);

  CopySpriteTiles(0, 3, (void *)VRAM,
                  (u16 *)(BG_SCREEN_ADDR(28) +
                          gContestantTurnOrder[contestant] * 5 * 64 + 0x36),
...
static void ContestDebugDoPrint(void) {
  ...
  case CONTEST_DEBUG_MODE_PRINT_LOSER_FLAGS:
    ContestDebugPrintBitStrings();
    break;
  // The only other possible value is 1, which is only set by
  ...
  default:
    ...
    for (i = 0; i < CONTESTANT_COUNT; i++) {
      ...
      Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text, 55, 1,
                                     FONT_NARROW);
    }
    for (i = 0; i < CONTESTANT_COUNT; i++) {
      ...
      ConvertIntToDecimalStringN(txtPtr, value, STR_CONV_MODE_LEFT_ALIGN, 4);
      Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text, 5, 1,
                                     FONT_NARROW);
    }
    SwapMoveDescAndContestTilemaps();
    break;
...
void SortContestants(bool8 useRanking) {
  ...
  if (!useRanking) {
    ...
    for (i = 0; i < CONTESTANT_COUNT; i++) {
      ...
      // This is redundant.
      // Perhaps GF switched from true insertion sort to in-place insertion
      // sort, and forgot to remove this check?
      if (v3 == i)
        gContestantTurnOrder[i] = i;
    }

    // Invert gContestantTurnOrder; above, it was a list of contestant IDs. Now
    // it's a list of turn orderings.
    //
    // For example, if contestant 3 had the first turn, then
    // `gContestantTurnOrder[1] = 3`. The turn is the index, the contestant is
    // the data. After inverting the list, `gContestantTurnOrder[3] = 1`. The
    // contestant is the index, and the turn is the data.
    ...
    //
    // Note that ranking is calculated so that shared places still take up a
    // ranking space. A ranking like [1, 2, 2, 3] is not possible; it would be
    // [1, 2, 2, 4] instead.
    ...
    // Randomize the order of contestants with tied rankings using Selection
    // Sort.
    //
    // Look through the array for tied ranks, and use randomOrdering to break
    // the tie. This ensures that contestants with the same rank will be
    // randomly ordered. This uses an in-place slection sort, which involves a
    // lot of extra swapping.
    for (i = 0; i < CONTESTANT_COUNT - 1; i++) {
      for (v3 = CONTESTANT_COUNT - 1; v3 > i; v3--) {
        if (eContestantStatus[v3 - 1].ranking ==
                eContestantStatus[v3].ranking &&
...
static void DrawContestantWindows(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    s32 windowId = i + 5;
    LoadPalette(eContestTempSave.cachedWindowPalettes[windowId],
                BG_PLTT_ID(5 + gContestantTurnOrder[i]),
                sizeof(eContestTempSave.cachedWindowPalettes[0]));
  }
  DrawContestantWindowText();
}
...
static void CalculateAppealMoveImpact(u8 contestant) {
  ...
  if (eContestantStatus[contestant].currMove ==
          eContestantStatus[contestant].prevMove &&
  ...
  if (eContestantStatus[contestant].hasJudgesAttention &&
      !AreMovesContestCombo(eContestantStatus[contestant].prevMove,
  ...
  if (eContestantStatus[contestant].conditionMod == CONDITION_GAIN)
    eContestantStatus[contestant].appeal +=
        eContestantStatus[contestant].condition - 10;
  else if (eContestantStatus[contestant].appealTripleCondition)
    eContestantStatus[contestant].appeal +=
        eContestantStatus[contestant].condition * 3;
  else
    eContestantStatus[contestant].appeal +=
        eContestantStatus[contestant].condition;
  ...
  if (IsContestantAllowedToCombo(contestant)) {
    ...
    if (completedCombo && eContestantStatus[contestant].hasJudgesAttention) {
      ...
      if (gContestMoves[eContestantStatus[contestant].currMove]
              .comboStarterId != 0) {
...
static void PrintAppealMoveResultText(u8 contestant, u8 stringId) {
  StringCopy(gStringVar1, gContestMons[contestant].nickname);
  StringCopy(gStringVar2, gMoveNames[eContestantStatus[contestant].currMove]);
  if (gContestMoves[eContestantStatus[eContestAppealResults.contestant]
                        .currMove]
          .contestCategory == CONTEST_CATEGORY_COOL)
    StringCopy(gStringVar3, gText_Contest_Shyness);
  else if (gContestMoves[eContestantStatus[eContestAppealResults.contestant]
                             .currMove]
...
static void ApplyNextTurnOrder(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    ...
    if (j == CONTESTANT_COUNT) {
      // No contestant was assigned to this turn. Look for the unassigned
      // contestant with the highest turn order.
      //
      // First, look for the first unassigned contestant.
      for (j = 0; j < CONTESTANT_COUNT; j++) {
        if (!isContestantOrdered[j] &&
            eContestantStatus[j].nextTurnOrder == CONTESTANT_NONE) {
          nextContestant = j;
          j++;
          break;
        }
...
static void UpdateApplauseMeter(void) {
  ...
  for (i = 0; i < APPLAUSE_METER_SIZE; i++) {
    ...
    CpuCopy32(src,
              (void *)(OBJ_VRAM0 +
                       (gSprites[eContest.applauseMeterSpriteId].oam.tileNum +
                        17 + i) *
                           32),
              32);
    CpuCopy32(src + 32,
...
static void Task_ApplauseOverflowAnimation(u8 taskId) {
  ...
  if (++gTasks[taskId].data[0] == 1) {
    ...
    BlendPalette(OBJ_PLTT_ID(gTasks[taskId].data[2]) + 8, 1,
                 gTasks[taskId].data[4], RGB_WHITE);
...
static void Task_AnimateAudience(u8 taskId) {
  if (gTasks[taskId].tDelay++ > 6) {
    ...
    if (gTasks[taskId].tFrame == 0) {
      RequestDma3Copy(eContestAudienceFrame2_Gfx, (void *)(BG_SCREEN_ADDR(4)),
                      0x1000, 1);
    } else {
      RequestDma3Copy(eUnzippedContestAudience_Gfx, (void *)(BG_SCREEN_ADDR(4)),
                      0x1000, 1);
...
static void BlendAudienceBackground(s8 excitementDir, s8 blendDir) {
  ...
  // Because this isn't set to TRUE here, the main task doesn't wait for the
  // color blend Unclear if this was intentional or not (perhaps waiting added
  // too much delay). In any case it does nothing now
...
static void Task_BlendAudienceBackground(u8 taskId) {
  if (gTasks[taskId].tBlendDelay++ >= 0) {
    ...
    BlendPalette(BG_PLTT_ID(1) + 1, 1, gTasks[taskId].tBlendCoeff,
                 gTasks[taskId].tBlendColor);
    BlendPalette(BG_PLTT_ID(1) + 10, 1, gTasks[taskId].tBlendCoeff,
                 gTasks[taskId].tBlendColor);
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    if (eContestantStatus[i].turnOrderMod != 0 && show) {
      CpuCopy32(
          ...
          (void *)(OBJ_VRAM0 +
                   (gSprites[eContestGfxState[i].nextTurnSpriteId].oam.tileNum +
                    6) *
                       32),
          32);
      gSprites[eContestGfxState[i].nextTurnSpriteId].y =
...
static const u8 *GetTurnOrderNumberGfx(u8 contestant) {
  if (eContestantStatus[contestant].turnOrderMod != 1)
    ...
    return gContestNextTurnNumbersGfx +
           eContestantStatus[contestant].nextTurnOrder * 32;
}

static void DrawUnnervedSymbols(void) {
  s32 i = 0;
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    if (eContestAppealResults.unnervedPokes[i] != 0 &&
        !Contest_IsMonsTurnDisabled(i)) {
      u32 contestantOffset = gContestantTurnOrder[i] * 5 + 2;
      u16 symbolOffset = GetStatusSymbolTileOffset(STAT_SYMBOL_SWIRL);

      ContestBG_FillBoxWithIncrementingTile(0, symbolOffset, 20,
                                            contestantOffset, 2, 1, 17, 1);
      symbolOffset += 16;
      ContestBG_FillBoxWithIncrementingTile(0, symbolOffset, 20,
                                            contestantOffset + 1, 2, 1, 17, 1);
      PlaySE(SE_CONTEST_ICON_CHANGE);
...
bool8 IsContestantAllowedToCombo(u8 contestant) {
  if (eContestantStatus[contestant].repeatedMove ||
      eContestantStatus[contestant].nervous)
    return FALSE;
  else
    return TRUE;
}
...
static void AnimateSliderHearts(u8 animId) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    gSprites[eContestGfxState[i].sliderHeartSpriteId].oam.matrixNum =
        AllocOamMatrix();
    gSprites[eContestGfxState[i].sliderHeartSpriteId].oam.affineMode =
        ST_OAM_AFFINE_NORMAL;
    StartSpriteAffineAnim(&gSprites[eContestGfxState[i].sliderHeartSpriteId],
                          animId);
...
static void SetMoveAnimAttackerData(u8 contestant) {
  ...
  gContestResources->moveAnim->species =
      SanitizeSpecies(gContestMons[contestant].species);
  gContestResources->moveAnim->personality =
      gContestMons[contestant].personality;
...
static void SetBattleTargetSpritePosition(void) {
  struct Sprite *sprite =
      &gSprites[gBattlerSpriteIds[B_POSITION_OPPONENT_RIGHT]];
...
void Contest_PrintTextToBg0WindowAt(u32 windowId, u8 *currChar, s32 x, s32 y,
                                    s32 fontId) {
  struct TextPrinterTemplate printerTemplate;
...
static void ContestBG_FillBoxWithIncrementingTile(u8 bg, u16 firstTileNum, u8 x,
                                                  u8 y, u8 width, u8 height,
                                                  u8 paletteSlot,
                                                  s16 tileNumData) {
  WriteSequenceToBgTilemapBuffer(bg, firstTileNum, x, y, width, height,
                                 paletteSlot, tileNumData);
  Contest_SetBgCopyFlags(bg);
}

static void ContestBG_FillBoxWithTile(u8 bg, u16 firstTileNum, u8 x, u8 y,
                                      u8 width, u8 height, u8 paletteSlot) {
  ContestBG_FillBoxWithIncrementingTile(bg, firstTileNum, x, y, width, height,
                                        paletteSlot, 0);
}
...
bool8 SaveContestWinner(u8 rank) {
  ...
  if (rank != CONTEST_SAVE_FOR_ARTIST) {
    ...
    gSaveBlock1Ptr->contestWinners[id].personality =
    ...
    StringCopy(gSaveBlock1Ptr->contestWinners[id].monName,
               gContestMons[i].nickname);
    StringCopy(gSaveBlock1Ptr->contestWinners[id].trainerName,
    ...
    // Set the most recent winner so the artist can show the player their
    // painting
...
// Rank is either a regular contest rank (for saving winners to show in the
// Contest Hall) Or one of two special IDs listed below (for saving winners to
// show in Museum, or from the artist) If just retrieving the index where the
// winner *would* go, shift is FALSE If actually preparing to insert the winner
// into the saveblock, shift is TRUE
...
static void SetContestLiveUpdateFlags(u8 contestant) {
  ...
  if (!eContestExcitement.frozen && eContestExcitement.moveExcitement > 0 &&
    ...
    gContestResources->tv[contestant].winnerFlags |=
  ...
  if (eContestantStatus[contestant].repeatedMove)
    gContestResources->tv[contestant].loserFlags |=
...
static void CalculateContestLiveUpdateData(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    if (i != winner &&
        gContestMonTotalPoints[winner] - gContestMonTotalPoints[i] <= 50)
  ...
  gContestResources->tv[winner].move =
      moveCandidates[Random() % numMoveCandidates];
}
  ...
  for (; i < CONTESTANT_COUNT; i++) {
    if (i != winner) {
      if (loserFlag < gContestResources->tv[i].loserFlags) {
        // Losing player currently has the worst (highest) set of flags, only
        // candidate
        loserCandidates[0] = i;
...
static void ContestDebugPrintBitStrings(void) {
  ...
  if (eContestDebugMode == CONTEST_DEBUG_MODE_PRINT_WINNER_FLAGS) {
    for (i = 0; i < CONTESTANT_COUNT; i++) {
      ...
      Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text1, 5, 1,
                                     FONT_NARROW);
      bits = gContestResources->tv[i].winnerFlags;
      for (j = 7; j > -1; j--) // Weird loop.
      {
        txtPtr = ConvertIntToDecimalStringN(txtPtr, bits & 1,
                                            STR_CONV_MODE_LEFT_ALIGN, 1);
        bits >>= 1;
      }
      ...
      Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text1 + j, 55, 1,
                                     FONT_NARROW);
    }
  } else // CONTEST_DEBUG_MODE_PRINT_LOSER_FLAGS
  {
    for (i = 0; i < CONTESTANT_COUNT; i++) {
      ...
      text2[j] = EOS;
      Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text2, 5, 1,
                                     FONT_NARROW);
      Contest_PrintTextToBg0WindowAt(gContestantTurnOrder[i], text1 + j, 55, 1,
                                     FONT_NARROW);
    }
  ...
  if (StringLength(nickname) <= 5) {
    ...
    while (*nickname != EOS) {
      if ((*nickname >= CHAR_A && *nickname <= CHAR_z) ||
          ...
          *nickname == CHAR_SPACE || *nickname == CHAR_PERIOD ||
          *nickname == CHAR_COMMA || *nickname == CHAR_EXCL_MARK ||
          *nickname == CHAR_QUESTION_MARK || *nickname == CHAR_MALE ||
          *nickname == CHAR_FEMALE || *nickname == CHAR_SLASH ||
