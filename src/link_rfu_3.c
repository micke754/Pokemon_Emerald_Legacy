...
static const u16 sWirelessLinkIconPalette[] =
    INCBIN_U16("graphics/link/wireless_icon.gbapal");
static const u32 sWirelessLinkIconPic[] =
    INCBIN_U32("graphics/link/wireless_icon.4bpp.lz");
...
static const union AnimCmd sWirelessStatusIndicator_3Bars[] = {
    ANIMCMD_FRAME(4, 5),   ANIMCMD_FRAME(8, 5),  ANIMCMD_FRAME(12, 5),
    ANIMCMD_FRAME(16, 10), ANIMCMD_FRAME(12, 5), ANIMCMD_FRAME(8, 5),
    ANIMCMD_JUMP(0)};

static const union AnimCmd sWirelessStatusIndicator_2Bars[] = {
    ANIMCMD_FRAME(4, 5), ANIMCMD_FRAME(8, 5), ANIMCMD_FRAME(12, 10),
    ANIMCMD_FRAME(8, 5), ANIMCMD_JUMP(0)};
...
static void UNUSED RfuUnusedQueue_Enqueue(struct RfuUnusedQueue *queue,
                                          u8 *data) {
  s32 i;
...
static bool8 UNUSED RfuUnusedQueue_Dequeue(struct RfuUnusedQueue *queue,
                                           u8 *dest) {
  s32 i;
...
// Populates an array with a sequence of numbers (which numbers depends on the
// mode) and sets the final element to the total of the other elements
...
void InitHostRfuGameData(struct RfuGameData *data, u8 activity,
                         bool32 startedActivity, s32 partnerInfo) {
  s32 i;
...
bool8 Rfu_GetCompatiblePlayerData(struct RfuGameData *gameData, u8 *username,
                                  u8 idx) {
  bool8 retVal;
  ...
  if (lman.parent_child == MODE_PARENT) {
    ...
    if (IsRfuSerialNumberValid(gRfuLinkStatus->partner[idx].serialNo) &&
      ...
      memcpy(gameData, gRfuLinkStatus->partner[idx].gname,
             RFU_GAME_NAME_LENGTH);
      memcpy(username, gRfuLinkStatus->partner[idx].uname,
...
bool8 Rfu_GetWonderDistributorPlayerData(struct RfuGameData *gameData,
                                         u8 *username, u8 idx) {
  bool8 retVal = FALSE;
...
void CopyHostRfuGameDataAndUsername(struct RfuGameData *gameData,
                                    u8 *username) {
...
void CreateWirelessStatusIndicatorSprite(u8 x, u8 y) {
  ...
  if (gRfuLinkStatus->parentChild == MODE_PARENT) {
    ...
    gSprites[gWirelessStatusIndicatorSpriteId].sValidator =
...
void DestroyWirelessStatusIndicatorSprite(void) {
  if (gSprites[gWirelessStatusIndicatorSpriteId].sValidator ==
      STATUS_INDICATOR_ACTIVE) {
    ...
    CpuCopy16(&gDummyOamData, (struct OamData *)OAM + 125,
              sizeof(struct OamData));
  }
}

void LoadWirelessStatusIndicatorSpriteGfx(void) {
  if (GetSpriteTileStartByTag(sWirelessStatusIndicatorSpriteSheet.tag) ==
      0xFFFF)
...
void UpdateWirelessStatusIndicatorSprite(void) {
  if (gWirelessStatusIndicatorSpriteId != SPRITE_NONE &&
    ...
    CpuCopy16(&gMain.oamBuffer[125], (struct OamData *)OAM + 125,
              sizeof(struct OamData));
...
static void CopyTrainerRecord(struct TrainerNameRecord *dest, u32 trainerId,
                              const u8 *name) {
...
// Save the currently connected players into the trainer records, shifting all
// previous records down.
void SaveLinkTrainerNames(void) {
  if (gWirelessCommType != 0) {
    ...
    for (i = 0; i < GetLinkPlayerCount(); i++) {
      if (i != GetMultiplayerId() &&
        ...
        CopyTrainerRecord(&newRecords[nextSpace],
                          (u16)gLinkPlayers[i].trainerId, gLinkPlayers[i].name);

        // If we already had a record for this trainer, wipe it so that the next
        // step doesn't duplicate it.
        if (connectedTrainerRecordIndices[i] >= 0)
          memset(gSaveBlock1Ptr
                     ->trainerNameRecords[connectedTrainerRecordIndices[i]]
                     .trainerName,
                 0, PLAYER_NAME_LENGTH + 1);
        nextSpace++;
      }
    }

    // Copy all non-empty records to the new list, in the order they appear on
    // the old list. If the list is full, the last (oldest) records will be
    // dropped.
    ...
    memcpy(gSaveBlock1Ptr->trainerNameRecords, newRecords,
...
bool32 PlayerHasMetTrainerBefore(u16 id, u8 *name) {
  ...
  for (i = 0; i < (int)ARRAY_COUNT(gSaveBlock1Ptr->trainerNameRecords); i++) {
    if (StringCompare(gSaveBlock1Ptr->trainerNameRecords[i].trainerName,
                      name) == 0 &&
...
void WipeTrainerNameRecords(void) {
  ...
  for (i = 0; i < (int)ARRAY_COUNT(gSaveBlock1Ptr->trainerNameRecords); i++) {
    ...
    CpuFill16(0, gSaveBlock1Ptr->trainerNameRecords[i].trainerName,
