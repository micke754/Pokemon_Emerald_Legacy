...
void LaunchBattleAnimation(const u8 *const animsTable[], u16 tableId,
                           bool8 isMoveAnim) {
  s32 i;
  ...
  if (!IsContest()) {
    ...
    for (i = 0; i < MAX_BATTLERS_COUNT; i++) {
      if (GetBattlerSide(i) != B_SIDE_PLAYER)
        ...
        gAnimBattlerSpecies[i] = GetMonData(
            &gPlayerParty[gBattlerPartyIndexes[i]], MON_DATA_SPECIES);
    }
  } else {
...
static void Cmd_loadspritegfx(void) {
  ...
  LoadCompressedSpriteSheetUsingHeap(
      &gBattleAnimPicTable[GET_TRUE_SPRITE_INDEX(index)]);
  LoadCompressedSpritePaletteUsingHeap(
      &gBattleAnimPaletteTable[GET_TRUE_SPRITE_INDEX(index)]);
...
static void Cmd_createsprite(void) {
  ...
  if (argVar & ANIMSPRITE_IS_TARGET) {
    ...
    subpriority =
        GetBattlerSpriteSubpriority(gBattleAnimAttacker) + (s8)(argVar);
  }
  ...
  CreateSpriteAndAnimate(
      template, GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_X_2),
...
static void Cmd_end(void) {
  ...
  if (IsSEPlaying()) {
    if (++sSoundAnimFramesToWait <=
        90) // Wait 90 frames, then halt the sound effect.
    {
      sAnimFramesToWait = 1;
      return;
    } else {
...
static void Task_InitUpdateMonBg(u8 taskId) {
  ...
  gTasks[updateTaskId].t2_SpriteX =
      gSprites[battlerSpriteId].x + gSprites[battlerSpriteId].x2;
  gTasks[updateTaskId].t2_SpriteY =
...
static void Cmd_monbg(void) {
  ...
  if (IsBattlerSpriteVisible(battlerId)) {
    ...
    if (position == B_POSITION_OPPONENT_LEFT ||
        position == B_POSITION_PLAYER_RIGHT || IsContest())
      toBG_2 = FALSE;
    else
      toBG_2 = TRUE;
...
void MoveBattlerSpriteToBG(u8 battlerId, bool8 toBG_2,
  ...
  if (!toBG_2) {
    ...
    gBattle_BG1_X =
        -(gSprites[battlerSpriteId].x + gSprites[battlerSpriteId].x2) + 0x20;
    if (IsContest() && IsSpeciesNotUnown(gContestResources->moveAnim->species))
      gBattle_BG1_X--;

    gBattle_BG1_Y =
        -(gSprites[battlerSpriteId].y + gSprites[battlerSpriteId].y2) + 0x20;
    ...
    LoadPalette(&gPlttBufferUnfaded[OBJ_PLTT_ID(battlerId)],
                BG_PLTT_ID(animBg.paletteId), PLTT_SIZE_4BPP);
    CpuCopy32(&gPlttBufferUnfaded[OBJ_PLTT_ID(battlerId)],
              (void *)(BG_PLTT + PLTT_OFFSET_4BPP(animBg.paletteId)),
              PLTT_SIZE_4BPP);
    ...
    DrawBattlerOnBg(1, 0, 0, battlerPosition, animBg.paletteId, animBg.bgTiles,
    ...
    LoadPalette(&gPlttBufferUnfaded[OBJ_PLTT_ID(battlerId)], BG_PLTT_ID(9),
                PLTT_SIZE_4BPP);
    CpuCopy32(&gPlttBufferUnfaded[OBJ_PLTT_ID(battlerId)],
              (void *)(BG_PLTT + PLTT_OFFSET_4BPP(9)), PLTT_SIZE_4BPP);

    DrawBattlerOnBg(2, 0, 0, GetBattlerPosition(battlerId), animBg.paletteId,
                    animBg.bgTiles + 0x1000, animBg.bgTilemap + 0x400,
                    animBg.tilesOffset);
  }
}
...
void RelocateBattleBgPal(u16 paletteNum, u16 *dest, u32 offset,
                         bool8 largeScreen) {
  s32 i, j;
  s32 size;
...
static void Task_UpdateMonBg(u8 taskId) {
  ...
  GetBattleAnimBg1Data(&animBg);
  x = gTasks[taskId].t2_SpriteX -
      (gSprites[spriteId].x + gSprites[spriteId].x2);
  y = gTasks[taskId].t2_SpriteY -
      (gSprites[spriteId].y + gSprites[spriteId].y2);

  if (!gTasks[taskId].t2_InBg2) {
    gBattle_BG1_X = x + gTasks[taskId].t2_BgX;
    gBattle_BG1_Y = y + gTasks[taskId].t2_BgY;
    CpuCopy32(&gPlttBufferFaded[OBJ_PLTT_ID(battlerId)],
              &gPlttBufferFaded[BG_PLTT_ID(animBg.paletteId)], PLTT_SIZE_4BPP);
  } else {
    gBattle_BG2_X = x + gTasks[taskId].t2_BgX;
    gBattle_BG2_Y = y + gTasks[taskId].t2_BgY;
    CpuCopy32(&gPlttBufferFaded[OBJ_PLTT_ID(battlerId)],
              &gPlttBufferFaded[BG_PLTT_ID(9)], PLTT_SIZE_4BPP);
  }
}
...
// Equivalent to Cmd_monbg but never creates Task_InitUpdateMonBg /
// Task_UpdateMonBg
static void Cmd_monbg_static(void) {
  bool8 toBG_2;
  u8 battlerId;
...
static void Task_ClearMonBgStatic(u8 taskId) {
  ...
  if (gTasks[taskId].data[1] != 1) {
    ...
    if (gTasks[taskId].data[0] > 1 &&
        IsBattlerSpriteVisible(BATTLE_PARTNER(battlerId)))
...
// Uses of this function that rely on a TRUE return are expecting inBattle to
// not be ticked as defined in contest behavior. As a result, if misused, this
// function cannot reliably discern between field and contest status and could
...
static void LoadMoveBg(u16 bgId) {
  if (IsContest()) {
    ...
    RelocateBattleBgPal(GetBattleBgPaletteNum(), (void *)gDecompressionBuffer,
                        0x100, FALSE);
    dmaSrc = gDecompressionBuffer;
    dmaDest = (void *)BG_SCREEN_ADDR(26);
    DmaCopy32(3, dmaSrc, dmaDest, 0x800);
    LZDecompressVram(gBattleAnimBackgroundTable[bgId].image,
                     (void *)BG_SCREEN_ADDR(4));
    LoadCompressedPalette(gBattleAnimBackgroundTable[bgId].palette,
                          BG_PLTT_ID(GetBattleBgPaletteNum()), PLTT_SIZE_4BPP);
  } else {
    LZDecompressVram(gBattleAnimBackgroundTable[bgId].tilemap,
...
s8 BattleAnimAdjustPanning(s8 pan) {
  if (!IsContest() &&
      gBattleSpritesDataPtr->healthBoxesData[gBattleAnimAttacker]
          .statusAnimActive) {
...
void Task_PanFromInitialToTarget(u8 taskId) {
  ...
  if (gTasks[taskId].tFrameCounter++ >= gTasks[taskId].tFramesToWait) {
    ...
    if (incrementPan ==
        0) // If we're not incrementing, just cancel the task immediately.
    {
      destroyTask = TRUE;
...
static void Cmd_panse_adjustall(void) {
  u16 songId;
  s8 targetPanArg, incrementPanArg, currentPanArg, currentPan, targetPan,
      incrementPan;
  u8 framesToWait;
  u8 taskId;
...
static void Cmd_splitbgprio(void) {
  ...
  // Apply only if the given battler is the lead (on left from team's
  // perspective)
  battlerPosition = GetBattlerPosition(battlerId);
...
static void Cmd_splitbgprio_foes(void) {
  ...
  if (GetBattlerSide(gBattleAnimAttacker) !=
      GetBattlerSide(gBattleAnimTarget)) {
    if (wantedBattler != ANIM_ATTACKER)
      battlerId = gBattleAnimTarget;
    else
      battlerId = gBattleAnimAttacker;
...
static void Cmd_teamattack_moveback(void) {
  ...
  if (!IsContest() && IsDoubleBattle() &&
      GetBattlerSide(gBattleAnimAttacker) ==
          GetBattlerSide(gBattleAnimTarget)) {
    if (wantedBattler == ANIM_ATTACKER) {
