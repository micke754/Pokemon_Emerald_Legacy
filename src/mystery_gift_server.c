...
static void MysteryGiftServer_Init(struct MysteryGiftServer *, const void *,
                                   u32, u32);
...
void MysterGiftServer_CreateForNews(void) {
  ...
  MysteryGiftServer_Init(sServer, gMysteryGiftServerScript_SendWonderNews, 0,
                         1);
}

void MysterGiftServer_CreateForCard(void) {
  sServer = AllocZeroed(sizeof(*sServer));
  MysteryGiftServer_Init(sServer, gMysteryGiftServerScript_SendWonderCard, 0,
                         1);
}
...
static void MysteryGiftServer_Init(struct MysteryGiftServer *svr,
                                   const void *script, u32 sendPlayerId,
                                   u32 recvPlayerId) {
  svr->unused = 0;
...
static void MysteryGiftServer_InitSend(struct MysteryGiftServer *svr, u32 ident,
...
// If the command's pointer is not empty use that as the send data, otherwise
// use the default.
static const void *MysteryGiftServer_GetSendData(const void *dynamicData,
...
static u32 Server_Done(struct MysteryGiftServer *svr) { return SVR_RET_END; }
...
static u32 Server_Run(struct MysteryGiftServer *svr) {
  ...
  case SVR_GET_CARD_STAT:
    AGB_ASSERT(cmd->ptr == NULL);
    svr->param =
        MysteryGift_GetCardStatFromLinkData(svr->linkGameData, cmd->parameter);
    break;
  case SVR_CHECK_QUESTIONNAIRE:
    AGB_ASSERT(cmd->parameter == 0);
    svr->param =
        MysteryGift_DoesQuestionnaireMatch(svr->linkGameData, cmd->ptr);
    break;
  case SVR_COMPARE:
    AGB_ASSERT(cmd->parameter == 0);
  ...
  case SVR_LOAD_NEWS:
    ...
    MysteryGiftServer_InitSend(
        svr, MG_LINKID_NEWS, MysteryGiftServer_GetSendData(cmd->ptr, svr->news),
        sizeof(*svr->news));
    break;
  case SVR_LOAD_CARD:
    AGB_ASSERT(cmd->parameter == 0);
    MysteryGiftServer_InitSend(
        svr, MG_LINKID_CARD, MysteryGiftServer_GetSendData(cmd->ptr, svr->card),
        sizeof(*svr->card));
    break;
  case SVR_LOAD_STAMP:
    AGB_ASSERT(cmd->parameter == 0);
    MysteryGiftServer_InitSend(
        svr, MG_LINKID_STAMP,
        MysteryGiftServer_GetSendData(cmd->ptr, &svr->stamp),
        sizeof(svr->stamp));
    break;
  case SVR_LOAD_RAM_SCRIPT:
    if (cmd->ptr == NULL)
      MysteryGiftServer_InitSend(svr, MG_LINKID_RAM_SCRIPT, svr->ramScript,
                                 svr->ramScriptSize);
    else
      MysteryGiftServer_InitSend(svr, MG_LINKID_RAM_SCRIPT, cmd->ptr,
                                 cmd->parameter);
    break;
  case SVR_LOAD_CLIENT_SCRIPT:
    if (cmd->ptr == NULL)
      MysteryGiftServer_InitSend(svr, MG_LINKID_CLIENT_SCRIPT,
                                 svr->clientScript, svr->clientScriptSize);
    else
      MysteryGiftServer_InitSend(svr, MG_LINKID_CLIENT_SCRIPT, cmd->ptr,
                                 cmd->parameter);
    break;
  case SVR_LOAD_EREADER_TRAINER:
    AGB_ASSERT(cmd->parameter == 0);
    MysteryGiftServer_InitSend(svr, MG_LINKID_EREADER_TRAINER, cmd->ptr,
                               sizeof(struct BattleTowerEReaderTrainer));
    break;
  case SVR_LOAD_MSG:
    MysteryGiftServer_InitSend(svr, MG_LINKID_DYNAMIC_MSG, cmd->ptr,
                               cmd->parameter);
    break;
  case SVR_LOAD_UNK_2:
