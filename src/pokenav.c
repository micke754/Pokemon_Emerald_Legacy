  ...
  case 5:
    if (!WaitForPokenavShutdownFade()) {
      bool32 calledFromScript =
          (gPokenavResources->mode != POKENAV_MODE_NORMAL);
...
static bool32 SetActivePokenavMenu(u32 menuId) {
  ...
  SetActiveMenuLoopTasks(PokenavMenuCallbacks[index].createLoopTask,
...
static void InitKeys_(void) { InitKeys(); }

void SetVBlankCallback_(IntrCallback callback) { SetVBlankCallback(callback); }

void SetPokenavVBlankCallback(void) { SetVBlankCallback(VBlankCB_Pokenav); }
...
u32 GetPokenavMode(void) { return gPokenavResources->mode; }

void SetPokenavMode(u16 mode) { gPokenavResources->mode = mode; }

void SetSelectedConditionSearch(u32 cursorPos) {
  u32 searchId = cursorPos;

  if (searchId > POKENAV_MENUITEM_CONDITION_SEARCH_TOUGH -
...
bool32 CanViewRibbonsMenu(void) { return gPokenavResources->hasAnyRibbons; }
