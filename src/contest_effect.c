...
static void ContestEffect_UserMoreEasilyStartled(void) {
  ...
  SetContestantEffectStringID(eContestAppealResults.contestant,
...
static void ContestEffect_StartleFrontMon(void) {
  ...
  if (eContestAppealResults.turnOrder[a] != 0) {
    ...
    for (i = 0; i < CONTESTANT_COUNT; i++) {
      if (eContestAppealResults.turnOrder[a] - 1 ==
  ...
  if (idx == 0)
    SetContestantEffectStringID2(eContestAppealResults.contestant,
...
static void ContestEffect_StartlePrevMons(void) {
  ...
  if (eContestAppealResults.turnOrder[contestant] != 0) {
    ...
    for (i = 0, j = 0; i < CONTESTANT_COUNT; i++) {
      if (eContestAppealResults.turnOrder[contestant] >
          eContestAppealResults.turnOrder[i])
...
static void ContestEffect_MakeFollowingMonsNervous(void) {
  ...
  for (i = 0; i < CONTESTANT_COUNT; i++) {
    if (eContestantStatus[i].hasJudgesAttention &&
        IsContestantAllowedToCombo(i))
      oddsMod[i] =
          gComboStarterLookupTable[gContestMoves[eContestantStatus[i].prevMove]
                                       .comboStarterId] *
          10;
    else
      oddsMod[i] = 0;
  ...
  if (odds[0] != 0) {
    for (i = 0; contestantIds[i] != CONTESTANT_NONE; i++) {
      ...
      if (contestantUnnerved) {
        ...
        SetContestantEffectStringID(contestantIds[i],
...
static void ContestEffect_AppealAsGoodAsPrevOnes(void) {
  ...
  eContestantStatus[eContestAppealResults.contestant].appeal = RoundTowardsZero(
      eContestantStatus[eContestAppealResults.contestant].appeal);
}
...
static void ContestEffect_BetterWhenLater(void) {
  u8 whichTurn =
      eContestAppealResults.turnOrder[eContestAppealResults.contestant];
  if (whichTurn == 0)
...
static void ContestEffect_ImproveConditionPreventNervousness(void) {
  if (eContestantStatus[eContestAppealResults.contestant].condition < 30) {
    ...
    eContestantStatus[eContestAppealResults.contestant].conditionMod =
        CONDITION_GAIN;
    SetContestantEffectStringID(eContestAppealResults.contestant,
...
static void ContestEffect_ExciteAudienceInAnyContest(void) {
  if (gContestMoves[eContestantStatus[eContestAppealResults.contestant]
    ...
    eContestantStatus[eContestAppealResults.contestant]
        .overrideCategoryExcitementMod = TRUE;
  }
}
...
static bool8 CanUnnerveContestant(u8 i) {
  ...
  if (eContestantStatus[i].immune) {
    ...
    return FALSE;
  } else if (!eContestantStatus[i].noMoreTurns &&
             eContestantStatus[i].numTurnsSkipped == 0) {
    return TRUE;
  } else {
    return FALSE;
  }
  ...
  for (i = 0; eContestAppealResults.jamQueue[i] != CONTESTANT_NONE; i++) {
    ...
    if (CanUnnerveContestant(contestant)) {
      ...
      if (eContestantStatus[contestant].resistant) {
        ...
        SetContestantEffectStringID(contestant,
                                    CONTEST_STRING_LITTLE_DISTRACTED);
      } else {
        eContestAppealResults.jam2 -=
            eContestantStatus[contestant].jamReduction;
