...
EWRAM_DATA struct MultiPartnerMenuPokemon gMultiPartnerParty[MULTI_PARTY_SIZE] =
    {0};
EWRAM_DATA static struct MultiPartnerMenuPokemon *sMultiPartnerPartyBuffer =
    NULL;
...
EWRAM_DATA struct BattleHealthboxInfo *gBattleControllerOpponentHealthboxData =
    NULL; // Never read
EWRAM_DATA struct BattleHealthboxInfo
    *gBattleControllerOpponentFlankHealthboxData = NULL; // Never read
...
u8 gBattleControllerData[MAX_BATTLERS_COUNT]; // Used by the battle controllers
                                              // to store misc sprite/task IDs
                                              // for each battler
...
const u8 gTypeNames[NUMBER_OF_MON_TYPES][TYPE_NAME_LENGTH + 1] = {
    [TYPE_NORMAL] = _("NORMAL"),  [TYPE_FIGHTING] = _("FIGHT"),
    [TYPE_FLYING] = _("FLYING"),  [TYPE_POISON] = _("POISON"),
    [TYPE_GROUND] = _("GROUND"),  [TYPE_ROCK] = _("ROCK"),
    [TYPE_BUG] = _("BUG"),        [TYPE_GHOST] = _("GHOST"),
    [TYPE_STEEL] = _("STEEL"),    [TYPE_MYSTERY] = _("???"),
    [TYPE_FIRE] = _("FIRE"),      [TYPE_WATER] = _("WATER"),
    [TYPE_GRASS] = _("GRASS"),    [TYPE_ELECTRIC] = _("ELECTR"),
    [TYPE_PSYCHIC] = _("PSYCHC"), [TYPE_ICE] = _("ICE"),
    [TYPE_DRAGON] = _("DRAGON"),  [TYPE_DARK] = _("DARK"),
};
...
static void CB2_InitBattleInternal(void) {
  ...
  SetGpuReg(REG_OFFSET_WIN0V,
            WIN_RANGE(DISPLAY_HEIGHT / 2, DISPLAY_HEIGHT / 2 + 1));
  ...
  if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED))) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS)
      CreateNPCTrainerParty(&gEnemyParty[PARTY_SIZE / 2],
                            gTrainerBattleOpponent_B, FALSE);
    SetWildMonHeldItem();
  }
...
static void BufferPartyVsScreenHealth_AtStart(void) {
  ...
  *(&gBattleStruct->multiBuffer.linkBattlerHeader.vsScreenHealthFlagsHi) =
      flags >> 8;
  gBattleStruct->multiBuffer.linkBattlerHeader.vsScreenHealthFlagsHi |=
      FlagGet(FLAG_SYS_FRONTIER_PASS) << 7;
}
...
static void SetPlayerBerryDataInBattleStruct(void) {
  ...
  struct BattleEnigmaBerry *battleBerry =
      &battleStruct->multiBuffer.linkBattlerHeader.battleEnigmaBerry;
  ...
  if (IsEnigmaBerryValid() == TRUE) {
    ...
    const struct Berry *berryData =
        GetBerryInfo(ItemIdToBerryType(ITEM_ENIGMA_BERRY));
...
static void SetAllPlayersBerryData(void) {
  ...
  if (!(gBattleTypeFlags & BATTLE_TYPE_LINK)) {
    if (IsEnigmaBerryValid() == TRUE) {
      ...
      for (i = 0; i < BERRY_ITEM_EFFECT_COUNT; i++) {
        gEnigmaBerries[0].itemEffect[i] =
            gSaveBlock1Ptr->enigmaBerry.itemEffect[i];
        gEnigmaBerries[2].itemEffect[i] =
            gSaveBlock1Ptr->enigmaBerry.itemEffect[i];
      }

      gEnigmaBerries[0].holdEffect = gSaveBlock1Ptr->enigmaBerry.holdEffect;
      gEnigmaBerries[2].holdEffect = gSaveBlock1Ptr->enigmaBerry.holdEffect;
      gEnigmaBerries[0].holdEffectParam =
          gSaveBlock1Ptr->enigmaBerry.holdEffectParam;
      gEnigmaBerries[2].holdEffectParam =
          gSaveBlock1Ptr->enigmaBerry.holdEffectParam;
    } else {
      const struct Berry *berryData =
          GetBerryInfo(ItemIdToBerryType(ITEM_ENIGMA_BERRY));
...
static void CB2_HandleStartBattle(void) {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK) {
      if (gReceivedRemoteLinkPlayers) {
        if (IsLinkTaskFinished()) {
          // 0x300
          *(&gBattleStruct->multiBuffer.linkBattlerHeader.versionSignatureLo) =
              0;
          *(&gBattleStruct->multiBuffer.linkBattlerHeader.versionSignatureHi) =
              3;
  ...
  case 2:
    if ((GetBlockReceivedStatus() & 3) == 3) {
      ...
      gTasks[taskId].data[3] =
          gBattleStruct->multiBuffer.linkBattlerHeader.vsScreenHealthFlagsLo |
          (gBattleStruct->multiBuffer.linkBattlerHeader.vsScreenHealthFlagsHi
           << 8);
  ...
  case 3:
    ...
    if (IsLinkTaskFinished()) {
      // Send Pokémon 1-2
      SendBlock(BitmaskAllOtherLinkPlayers(), gPlayerParty,
  ...
  case 4:
    if ((GetBlockReceivedStatus() & 3) == 3) {
      // Recv Pokémon 1-2
      ResetBlockReceivedFlags();
      memcpy(gEnemyParty, gBlockRecvBuffer[enemyMultiplayerId],
             sizeof(struct Pokemon) * 2);
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 7:
    if (IsLinkTaskFinished()) {
      // Send Pokémon 3-4
      SendBlock(BitmaskAllOtherLinkPlayers(), &gPlayerParty[2],
                sizeof(struct Pokemon) * 2);
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 8:
    if ((GetBlockReceivedStatus() & 3) == 3) {
      // Recv Pokémon 3-4
      ResetBlockReceivedFlags();
      memcpy(&gEnemyParty[2], gBlockRecvBuffer[enemyMultiplayerId],
             sizeof(struct Pokemon) * 2);
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 11:
    if (IsLinkTaskFinished()) {
      // Send Pokémon 5-6
      SendBlock(BitmaskAllOtherLinkPlayers(), &gPlayerParty[4],
                sizeof(struct Pokemon) * 2);
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 12:
    if ((GetBlockReceivedStatus() & 3) == 3) {
      // Recv Pokémon 5-6
      ResetBlockReceivedFlags();
      memcpy(&gEnemyParty[4], gBlockRecvBuffer[enemyMultiplayerId],
             sizeof(struct Pokemon) * 2);
  ...
  case 15:
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK) {
      ...
      s32 i;
      for (i = 0; i < 2 && (gLinkPlayers[i].version & 0xFF) == VERSION_EMERALD;
           i++)
        ;
  ...
  case 16:
    ...
    if (IsLinkTaskFinished()) {
      SendBlock(BitmaskAllOtherLinkPlayers(), &gRecordedBattleRngSeed,
                sizeof(gRecordedBattleRngSeed));
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 17:
    // Receive rng seed for recorded battle (only read it if partner is the link
    // master)
    if ((GetBlockReceivedStatus() & 3) == 3) {
      ResetBlockReceivedFlags();
...
static void CB2_HandleStartMultiPartnerBattle(void) {
  ...
  case 4:
    if ((GetBlockReceivedStatus() & 3) == 3) {
      // Recv partner's Pokémon 1-2, and copy partner's and own Pokémon into
      // party positions
      ResetBlockReceivedFlags();
      if (gLinkPlayers[playerMultiplayerId].id != 0) {
        memcpy(gPlayerParty, gBlockRecvBuffer[partnerMultiplayerId],
               sizeof(struct Pokemon) * 2);
        memcpy(&gPlayerParty[MULTI_PARTY_SIZE],
               gBlockRecvBuffer[playerMultiplayerId],
               sizeof(struct Pokemon) * 2);
      } else {
        memcpy(gPlayerParty, gBlockRecvBuffer[playerMultiplayerId],
               sizeof(struct Pokemon) * 2);
        memcpy(&gPlayerParty[MULTI_PARTY_SIZE],
               gBlockRecvBuffer[partnerMultiplayerId],
               sizeof(struct Pokemon) * 2);
      }
      gBattleCommunication[MULTIUSE_STATE]++;
    }
  ...
  case 6:
    if ((GetBlockReceivedStatus() & 3) == 3) {
      // Recv partner's Pokémon 3, and copy partner's and own Pokémon into party
      // positions
      ResetBlockReceivedFlags();
      if (gLinkPlayers[playerMultiplayerId].id != 0) {
        memcpy(&gPlayerParty[2], gBlockRecvBuffer[partnerMultiplayerId],
               sizeof(struct Pokemon));
        memcpy(&gPlayerParty[2 + MULTI_PARTY_SIZE],
               gBlockRecvBuffer[playerMultiplayerId], sizeof(struct Pokemon));
      } else {
        memcpy(&gPlayerParty[2], gBlockRecvBuffer[playerMultiplayerId],
               sizeof(struct Pokemon));
        memcpy(&gPlayerParty[2 + MULTI_PARTY_SIZE],
               gBlockRecvBuffer[partnerMultiplayerId], sizeof(struct Pokemon));
      }
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 7:
    if (IsLinkTaskFinished()) {
      ...
      SendBlock(BitmaskAllOtherLinkPlayers(), gEnemyParty,
                sizeof(struct Pokemon) * 2);
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  ...
  case 9:
    if (IsLinkTaskFinished()) {
      ...
      SendBlock(BitmaskAllOtherLinkPlayers(), &gEnemyParty[2],
                sizeof(struct Pokemon) * 2);
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 10:
    if ((GetBlockReceivedStatus() & 3) == 3) {
      ...
      ResetBlockReceivedFlags();
      if (GetMultiplayerId() != 0)
        memcpy(&gEnemyParty[2], gBlockRecvBuffer[0],
               sizeof(struct Pokemon) * 2);
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 11:
    if (IsLinkTaskFinished()) {
      ...
      SendBlock(BitmaskAllOtherLinkPlayers(), &gEnemyParty[4],
                sizeof(struct Pokemon) * 2);
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 12:
    if ((GetBlockReceivedStatus() & 3) == 3) {
      ...
      ResetBlockReceivedFlags();
      if (GetMultiplayerId() != 0)
        memcpy(&gEnemyParty[4], gBlockRecvBuffer[0],
               sizeof(struct Pokemon) * 2);
  ...
  case 15:
    ...
    if ((GetBlockReceivedStatus() & 3) == 3) {
      ResetBlockReceivedFlags();
      if (!(gBattleTypeFlags & BATTLE_TYPE_IS_MASTER))
        memcpy(&gRecordedBattleRngSeed, gBlockRecvBuffer[partnerMultiplayerId],
               sizeof(gRecordedBattleRngSeed));
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
...
static void SetMultiPartnerMenuParty(u8 offset) {
  ...
  for (i = 0; i < MULTI_PARTY_SIZE; i++) {
    gMultiPartnerParty[i].species =
        GetMonData(&gPlayerParty[offset + i], MON_DATA_SPECIES);
    gMultiPartnerParty[i].heldItem =
        GetMonData(&gPlayerParty[offset + i], MON_DATA_HELD_ITEM);
    GetMonData(&gPlayerParty[offset + i], MON_DATA_NICKNAME,
               gMultiPartnerParty[i].nickname);
    gMultiPartnerParty[i].level =
        GetMonData(&gPlayerParty[offset + i], MON_DATA_LEVEL);
    gMultiPartnerParty[i].hp =
        GetMonData(&gPlayerParty[offset + i], MON_DATA_HP);
    gMultiPartnerParty[i].maxhp =
        GetMonData(&gPlayerParty[offset + i], MON_DATA_MAX_HP);
    gMultiPartnerParty[i].status =
        GetMonData(&gPlayerParty[offset + i], MON_DATA_STATUS);
    gMultiPartnerParty[i].personality =
        GetMonData(&gPlayerParty[offset + i], MON_DATA_PERSONALITY);
    gMultiPartnerParty[i].gender = GetMonGender(&gPlayerParty[offset + i]);
    StripExtCtrlCodes(gMultiPartnerParty[i].nickname);
    if (GetMonData(&gPlayerParty[offset + i], MON_DATA_LANGUAGE) !=
        LANGUAGE_JAPANESE)
      PadNameString(gMultiPartnerParty[i].nickname, CHAR_SPACE);
  }
  memcpy(sMultiPartnerPartyBuffer, gMultiPartnerParty,
         sizeof(gMultiPartnerParty));
}
...
static void CB2_PreInitMultiBattle(void) {
  ...
  case 0:
    if (gReceivedRemoteLinkPlayers && IsLinkTaskFinished()) {
      ...
      SendBlock(BitmaskAllOtherLinkPlayers(), sMultiPartnerPartyBuffer,
                sizeof(gMultiPartnerParty));
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
...
static void CB2_HandleStartMultiBattle(void) {
    ...
    if ((GetBlockReceivedStatus() & 0xF) == 0xF) {
      ...
      for (id = 0; id < MAX_LINK_PLAYERS; id++) {
        if (id == playerMultiplayerId) {
          ...
          case 0:
          case 3:
            memcpy(gPlayerParty, gBlockRecvBuffer[id],
                   sizeof(struct Pokemon) * 2);
            break;
          case 1:
          case 2:
          ...
          if ((!(gLinkPlayers[id].id & 1) &&
            ...
            case 0:
            case 3:
              memcpy(gEnemyParty, gBlockRecvBuffer[id],
                     sizeof(struct Pokemon) * 2);
              break;
            case 1:
            case 2:
    ...
    if ((GetBlockReceivedStatus() & 0xF) == 0xF) {
      ...
      for (id = 0; id < MAX_LINK_PLAYERS; id++) {
        if (id == playerMultiplayerId) {
          ...
          case 0:
          case 3:
            memcpy(gPlayerParty + 2, gBlockRecvBuffer[id],
                   sizeof(struct Pokemon));
            break;
          case 1:
          case 2:
            memcpy(gPlayerParty + 5, gBlockRecvBuffer[id],
                   sizeof(struct Pokemon));
            break;
          }
        } else {
          if ((!(gLinkPlayers[id].id & 1) &&
            ...
            case 0:
            case 3:
              memcpy(gEnemyParty + 2, gBlockRecvBuffer[id],
                     sizeof(struct Pokemon));
              break;
            case 1:
            case 2:
              memcpy(gEnemyParty + 5, gBlockRecvBuffer[id],
                     sizeof(struct Pokemon));
              break;
            }
          }
  ...
  case 7:
    ...
    gBattleCommunication[SPRITES_INIT_STATE2] = 0;
    if (gBattleTypeFlags & BATTLE_TYPE_LINK) {
      for (id = 0; id < MAX_LINK_PLAYERS &&
                   (gLinkPlayers[id].version & 0xFF) == VERSION_EMERALD;
           id++)
        ;
  ...
  case 8:
    if (IsLinkTaskFinished()) {
      ...
      SendBlock(BitmaskAllOtherLinkPlayers(), ptr,
                sizeof(gBattleStruct->multiBuffer.battleVideo));
      gBattleCommunication[MULTIUSE_STATE]++;
    }
    break;
  case 9:
    if ((GetBlockReceivedStatus() & 0xF) == 0xF) {
      ...
      for (var = 0; var < 4; var++) {
        ...
        if (blockValue & 4) {
          memcpy(&gRecordedBattleRngSeed, &gBlockRecvBuffer[var][2],
                 sizeof(gRecordedBattleRngSeed));
          break;
        }
      }
...
void BattleMainCB2(void) {
  ...
  if (JOY_HELD(B_BUTTON) && gBattleTypeFlags & BATTLE_TYPE_RECORDED &&
...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum,
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_TRAINER &&
      !(gBattleTypeFlags & (BATTLE_TYPE_FRONTIER | BATTLE_TYPE_EREADER_TRAINER |
    ...
    for (i = 0; i < monsCount; i++) {
      ...
      // Comment out the following line if you have changed .iv to go 0-31,
      // instead of 0-255 as in vanilla.
      ...
      if (partyData[i].nature > 0)
        CreateMonWithGenderNatureLetter(
            &party[i], partyData[i].species, partyData[i].lvl, fixedIV, gender,
            partyData[i].nature, 0,
            partyData[i].shiny ? OT_ID_SHINY : OT_ID_RANDOM_NO_SHINY);
      else {
        CreateMon(&party[i], partyData[i].species, partyData[i].lvl, fixedIV,
                  TRUE, personalityValue,
                  partyData[i].shiny ? OT_ID_SHINY : OT_ID_RANDOM_NO_SHINY, 0);
      }
      ...
      if (partyData[i].moves[0] != '\0') {
        for (j = 0; j < MAX_MON_MOVES; j++) {
          ...
          SetMonData(&party[i], MON_DATA_PP1 + j,
                     &gBattleMoves[partyData[i].moves[j]].pp);
          SetMonData(&party[i], MON_DATA_PP1 + j,
                     &gBattleMoves[partyData[i].moves[j]].pp);
        }
      }
...
void CB2_InitEndLinkBattle(void) {
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_FRONTIER) {
    ...
    InitBattleBgsVideo();
    LoadCompressedPalette(gBattleTextboxPalette, BG_PLTT_ID(0),
                          2 * PLTT_SIZE_4BPP);
    ...
    SetGpuReg(REG_OFFSET_WINOUT, WINOUT_WIN01_BG0 | WINOUT_WIN01_BG1 |
                                     WINOUT_WIN01_BG2 | WINOUT_WIN01_OBJ |
                                     WINOUT_WIN01_CLR);
    FreeAllSpritePalettes();
    gReservedSpritePaletteCount = MAX_BATTLERS_COUNT;
    SetVBlankCallback(VBlankCB_Battle);
...
static void EndLinkBattleInSteps(void) {
  ...
  case 2:
    if (!gPaletteFade.active) {
      ...
      gMain.anyLinkBattlerHasFrontierPass =
      ...
      for (i = 0; i < battlerCount &&
                  (gLinkPlayers[i].version & 0xFF) == VERSION_EMERALD;
           i++)
        ;
...
static void AskRecordBattle(void) {
  ...
  case STATE_RECORD_WAIT:
    if (IsLinkTaskFinished() == TRUE && !IsTextPrinterActive(B_WIN_MSG) &&
...
void SpriteCB_FaintOpponentMon(struct Sprite *sprite) {
  ...
  GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]],
             MON_DATA_PERSONALITY); // Unused return value.

  if (species == SPECIES_UNOWN) {
    u32 personalityValue = GetMonData(
        &gEnemyParty[gBattlerPartyIndexes[battler]], MON_DATA_PERSONALITY);
    ...
    if (unownForm == 0)
      ...
      unownSpecies =
          NUM_SPECIES + unownForm; // Use one of the other Unown letters.
...
static void SpriteCB_AnimFaintOpponent(struct Sprite *sprite) {
  ...
  if (--sprite->data[4] == 0) {
    ...
    if (--sprite->data[3] < 0) {
      FreeSpriteOamMatrix(sprite);
      DestroySprite(sprite);
    } else // Erase bottom part of the sprite to create a smooth illusion of mon
           // falling down.
    {
      u8 *dst = gMonSpritesGfxPtr->sprites
                    .byte[GetBattlerPosition(sprite->sBattler)] +
                (gBattleMonForms[sprite->sBattler] << 11) +
                (sprite->data[3] << 8);
...
// Used when selecting a move, which can hit multiple targets, in double
// battles.
...
void EndBounceEffect(u8 battler, u8 which) {
  ...
  if (which == BOUNCE_HEALTHBOX) {
    ...
    bouncerSpriteId = gSprites[gBattleSpritesDataPtr->healthBoxesData[battler]
                                   .healthboxBounceSpriteId]
                          .sBouncerSpriteId;
    DestroySprite(&gSprites[gBattleSpritesDataPtr->healthBoxesData[battler]
                                .healthboxBounceSpriteId]);
...
static void SpriteCB_BounceEffect(struct Sprite *sprite) {
  ...
  gSprites[bouncerSpriteId].y2 =
      Sin(index, sprite->sAmplitude) + sprite->sAmplitude;
...
static void BattleStartClearSetData(void) {
  ...
  gBattleScripting.battleStyle =
      gSaveBlock2Ptr->optionsBattleStyle || FlagGet(FLAG_HARD);
  ...
  *(&gBattleStruct->safariCatchFactor) =
      gSpeciesInfo[GetMonData(&gEnemyParty[0], MON_DATA_SPECIES)].catchRate *
      100 / 1275;
...
void SwitchInClearSetData(void) {
  ...
  if (gBattleMoves[gCurrentMove].effect == EFFECT_BATON_PASS) {
    gDisableStructs[gActiveBattler].substituteHP =
        disableStructCopy.substituteHP;
    gDisableStructs[gActiveBattler].battlerWithSureHit =
        disableStructCopy.battlerWithSureHit;
  ...
  gDisableStructs[gActiveBattler].truantSwitchInHack =
      disableStructCopy.truantSwitchInHack;
  ...
  *(0 * 2 + gActiveBattler * 8 + (u8 *)(gBattleStruct->lastTakenMoveFrom) + 0) =
      0;
  *(0 * 2 + gActiveBattler * 8 + (u8 *)(gBattleStruct->lastTakenMoveFrom) + 1) =
      0;
  *(1 * 2 + gActiveBattler * 8 + (u8 *)(gBattleStruct->lastTakenMoveFrom) + 0) =
      0;
  *(1 * 2 + gActiveBattler * 8 + (u8 *)(gBattleStruct->lastTakenMoveFrom) + 1) =
      0;
  *(2 * 2 + gActiveBattler * 8 + (u8 *)(gBattleStruct->lastTakenMoveFrom) + 0) =
      0;
  *(2 * 2 + gActiveBattler * 8 + (u8 *)(gBattleStruct->lastTakenMoveFrom) + 1) =
      0;
  *(3 * 2 + gActiveBattler * 8 + (u8 *)(gBattleStruct->lastTakenMoveFrom) + 0) =
      0;
  *(3 * 2 + gActiveBattler * 8 + (u8 *)(gBattleStruct->lastTakenMoveFrom) + 1) =
      0;
  ...
  for (i = 0; i < gBattlersCount; i++) {
    if (i != gActiveBattler &&
        GetBattlerSide(i) != GetBattlerSide(gActiveBattler)) {
      *(gBattleStruct->lastTakenMove + i * 2 + 0) = MOVE_NONE;
      *(gBattleStruct->lastTakenMove + i * 2 + 1) = MOVE_NONE;
    }
    *(i * 8 + gActiveBattler * 2 + (u8 *)(gBattleStruct->lastTakenMoveFrom) +
      0) = 0;
    *(i * 8 + gActiveBattler * 2 + (u8 *)(gBattleStruct->lastTakenMoveFrom) +
      1) = 0;
  }
...
static void BattleIntroDrawTrainersOrMonsSprites(void) {
  ...
  for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++) {
    if ((gBattleTypeFlags & BATTLE_TYPE_SAFARI) &&
      ...
      hpOnSwitchout =
          &gBattleStruct->hpOnSwitchout[GetBattlerSide(gActiveBattler)];
...
static void BattleIntroDrawPartySummaryScreens(void) {
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_TRAINER) {
    for (i = 0; i < PARTY_SIZE; i++) {
      if (GetMonData(&gEnemyParty[i], MON_DATA_SPECIES_OR_EGG) ==
              SPECIES_NONE ||
    ...
    BtlController_EmitDrawPartyStatusSummary(BUFFER_A, hpStatus,
                                             PARTY_SUMM_SKIP_DRAW_DELAY);
    MarkBattlerForControllerExec(gActiveBattler);

    for (i = 0; i < PARTY_SIZE; i++) {
      if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES_OR_EGG) ==
              SPECIES_NONE ||
...
static void BattleIntroPrintPlayerSendsOut(void) {
  if (gBattleControllerExecFlags == 0) {
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI))
      PrepareStringBattle(STRINGID_INTROSENDOUT,
    ...
    for (gActiveBattler = 0; gActiveBattler < gBattlersCount;
      ...
      if (GetBattlerSide(gActiveBattler) == B_SIDE_PLAYER) {
        BtlController_EmitSwitchInAnim(
            BUFFER_A, gBattlerPartyIndexes[gActiveBattler], FALSE);
        MarkBattlerForControllerExec(gActiveBattler);
      }
    }
...
static void TryDoEventsBeforeFirstTurn(void) {
  ...
  if (gBattleStruct->switchInAbilitiesCounter == 0) {
    ...
    for (i = 0; i < gBattlersCount - 1; i++) {
      for (j = i + 1; j < gBattlersCount; j++) {
        if (GetWhoStrikesFirst(gBattlerByTurnOrder[i], gBattlerByTurnOrder[j],
                               TRUE) != 0)
          SwapTurnOrder(i, j);
      }
    }
  ...
  while (gBattleStruct->switchInAbilitiesCounter < gBattlersCount) {
    if (AbilityBattleEffects(
            ...
            gBattlerByTurnOrder[gBattleStruct->switchInAbilitiesCounter], 0, 0,
            0) != 0)
      effect++;
  ...
  while (gBattleStruct->switchInItemsCounter < gBattlersCount) {
    if (ItemBattleEffects(
            ...
            gBattlerByTurnOrder[gBattleStruct->switchInItemsCounter], FALSE))
      effect++;
...
void SwitchPartyOrder(u8 battler) {
  ...
  for (i = 0; i < (int)ARRAY_COUNT(gBattlePartyCurrentOrder); i++)
    gBattlePartyCurrentOrder[i] =
        *(battler * 3 + i + (u8 *)(gBattleStruct->battlerPartyOrders));
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE) {
    for (i = 0; i < (int)ARRAY_COUNT(gBattlePartyCurrentOrder); i++) {
      *(battler * 3 + i + (u8 *)(gBattleStruct->battlerPartyOrders)) =
          gBattlePartyCurrentOrder[i];
      *(BATTLE_PARTNER(battler) * 3 + i +
        (u8 *)(gBattleStruct->battlerPartyOrders)) =
          gBattlePartyCurrentOrder[i];
    }
  } else {
    for (i = 0; i < (int)ARRAY_COUNT(gBattlePartyCurrentOrder); i++) {
...
static void HandleTurnActionSelectionState(void) {
  ...
  for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++) {
    ...
    case STATE_TURN_START_RECORD: // Recorded battle related action on start of
                                  // every turn.
      ...
      if (gBattleTypeFlags & BATTLE_TYPE_MULTI ||
        ...
        if (gBattleStruct->absentBattlerFlags & gBitTable[gActiveBattler]) {
          ...
          if (gBattleMons[gActiveBattler].status2 & STATUS2_MULTIPLETURNS ||
            ...
            BtlController_EmitChooseAction(BUFFER_A, gChosenActionByBattler[0],
                                           gBattleBufferB[0][1] |
                                               (gBattleBufferB[0][2] << 8));
            MarkBattlerForControllerExec(gActiveBattler);
            gBattleCommunication[gActiveBattler]++;
          }
        }
    ...
    case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
      if (!(gBattleControllerExecFlags &
            ((gBitTable[gActiveBattler]) | (0xF << 28) |
          ...
          if (AreAllMovesUnusable()) {
            ...
            *(gBattleStruct->chosenMovePositions + gActiveBattler) =
            ...
            BtlController_EmitChooseMove(
                BUFFER_A, (gBattleTypeFlags & BATTLE_TYPE_DOUBLE) != 0, FALSE,
                &moveInfo);
            MarkBattlerForControllerExec(gActiveBattler);
          }
          break;
          ...
          if (gBattleTypeFlags &
            ...
            *(gBattleStruct->stateIdAfterSelScript + gActiveBattler) =
            ...
            BtlController_EmitChooseItem(
                BUFFER_A, gBattleStruct->battlerPartyOrders[gActiveBattler]);
            MarkBattlerForControllerExec(gActiveBattler);
          }
          break;
          ...
          *(gBattleStruct->battlerPartyIndexes + gActiveBattler) =
          ...
          if (gBattleMons[gActiveBattler].status2 &
            ...
            BtlController_EmitChoosePokemon(
                BUFFER_A, PARTY_ACTION_CANT_SWITCH, PARTY_SIZE, ABILITY_NONE,
                gBattleStruct->battlerPartyOrders[gActiveBattler]);
          } else if ((i = ABILITY_ON_OPPOSING_FIELD(gActiveBattler,
                                                    ABILITY_SHADOW_TAG)) ||
                     ((i = ABILITY_ON_OPPOSING_FIELD(gActiveBattler,
                                                     ABILITY_ARENA_TRAP)) &&
                      !IS_BATTLER_OF_TYPE(gActiveBattler, TYPE_FLYING) &&
                      gBattleMons[gActiveBattler].ability !=
                          ABILITY_LEVITATE) ||
            ...
            BtlController_EmitChoosePokemon(
                BUFFER_A, ((i - 1) << 4) | PARTY_ACTION_ABILITY_PREVENTS,
            ...
            if (gActiveBattler == 2 &&
              ...
              BtlController_EmitChoosePokemon(
                  BUFFER_A, PARTY_ACTION_CHOOSE_MON,
                  *(gBattleStruct->monToSwitchIntoId + 0), ABILITY_NONE,
                  gBattleStruct->battlerPartyOrders[gActiveBattler]);
            else if (gActiveBattler == 3 &&
                     gChosenActionByBattler[1] == B_ACTION_SWITCH)
              BtlController_EmitChoosePokemon(
                  BUFFER_A, PARTY_ACTION_CHOOSE_MON,
                  *(gBattleStruct->monToSwitchIntoId + 1), ABILITY_NONE,
                  gBattleStruct->battlerPartyOrders[gActiveBattler]);
            else
              BtlController_EmitChoosePokemon(
                  BUFFER_A, PARTY_ACTION_CHOOSE_MON, PARTY_SIZE, ABILITY_NONE,
                  gBattleStruct->battlerPartyOrders[gActiveBattler]);
          }
          MarkBattlerForControllerExec(gActiveBattler);
          break;
        ...
        case B_ACTION_CANCEL_PARTNER:
          ...
          gBattleCommunication[GetBattlerAtPosition(
              ...
              STATE_BEFORE_ACTION_CHOSEN;
          RecordedBattle_ClearBattlerAction(gActiveBattler, 1);
          if (gBattleMons[GetBattlerAtPosition(BATTLE_PARTNER(
          ...
          default:
            ...
            if ((gBattleBufferB[gActiveBattler][2] |
              ...
              *(gBattleStruct->chosenMovePositions + gActiveBattler) =
              ...
              gChosenMoveByBattler[gActiveBattler] =
                  gBattleMons[gActiveBattler].moves[*(
                      gBattleStruct->chosenMovePositions + gActiveBattler)];
              *(gBattleStruct->moveTarget + gActiveBattler) =
                  gBattleBufferB[gActiveBattler][3];
              gBattleCommunication[gActiveBattler]++;
    ...
    case STATE_WAIT_ACTION_CONFIRMED_STANDBY:
      if (!(gBattleControllerExecFlags &
        ...
        if (((gBattleTypeFlags & BATTLE_TYPE_MULTI) ||
            ...
            (*(&gBattleStruct->absentBattlerFlags) &
             gBitTable[GetBattlerAtPosition(BATTLE_PARTNER(position))])) {
          BtlController_EmitLinkStandbyMsg(BUFFER_A,
                                           LINK_STANDBY_MSG_STOP_BOUNCE, i);
        } else {
          BtlController_EmitLinkStandbyMsg(BUFFER_A,
                                           LINK_STANDBY_STOP_BOUNCE_ONLY, i);
        }
        MarkBattlerForControllerExec(gActiveBattler);
        gBattleCommunication[gActiveBattler]++;
      }
...
u8 GetWhoStrikesFirst(u8 battler1, u8 battler2, bool8 ignoreChosenMoves) {
  ...
  speedBattler1 =
      (gBattleMons[battler1].speed * speedMultiplierBattler1) *
  ...
  speedBattler2 =
      (gBattleMons[battler2].speed * speedMultiplierBattler2) *
  ...
  if (ignoreChosenMoves) {
    ...
    if (gChosenActionByBattler[battler1] == B_ACTION_USE_MOVE) {
      if (gProtectStructs[battler1].noValidMoves)
        ...
        moveBattler1 =
            gBattleMons[battler1]
                .moves[*(gBattleStruct->chosenMovePositions + battler1)];
    } else
      moveBattler1 = MOVE_NONE;
    ...
    if (gChosenActionByBattler[battler2] == B_ACTION_USE_MOVE) {
      if (gProtectStructs[battler2].noValidMoves)
        ...
        moveBattler2 =
            gBattleMons[battler2]
                .moves[*(gBattleStruct->chosenMovePositions + battler2)];
    } else
      moveBattler2 = MOVE_NONE;
  }

  // both move priorities are different than 0
  if (gBattleMoves[moveBattler1].priority != 0 ||
      gBattleMoves[moveBattler2].priority != 0) {
    // both priorities are the same
    if (gBattleMoves[moveBattler1].priority ==
        gBattleMoves[moveBattler2].priority) {
    ...
    } else if (gBattleMoves[moveBattler1].priority <
               gBattleMoves[moveBattler2].priority)
    ...
    if (turnOrderId == 5) // One of battlers wants to run.
    {
      ...
      for (gActiveBattler = 0; gActiveBattler < gBattlersCount;
        ...
        if (gChosenActionByBattler[gActiveBattler] == B_ACTION_USE_ITEM ||
            gChosenActionByBattler[gActiveBattler] == B_ACTION_SWITCH) {
          gActionsByTurnOrder[turnOrderId] =
              gChosenActionByBattler[gActiveBattler];
          gBattlerByTurnOrder[turnOrderId] = gActiveBattler;
          turnOrderId++;
        }
...
static void RunTurnActionsFunctions(void) {
  ...
  *(&gBattleStruct->savedTurnActionNumber) = gCurrentTurnActionNumber;
  ...
  if (gCurrentTurnActionNumber >=
      gBattlersCount) // everyone did their actions, turn finished
  {
    gHitMarker &= ~HITMARKER_PASSIVE_DAMAGE;
    gBattleMainFunc = sEndTurnFuncsTable[gBattleOutcome & 0x7F];
  } else {
    if (gBattleStruct->savedTurnActionNumber !=
        gCurrentTurnActionNumber) // action turn has been done, clear hitmarker
                                  // bits for another battlerId
    {
      gHitMarker &= ~HITMARKER_NO_ATTACKSTRING;
      gHitMarker &= ~HITMARKER_UNABLE_TO_USE_MOVE;
    }
...
static void HandleEndTurn_FinishBattle(void) {
  if (gCurrentActionFuncId == B_ACTION_TRY_FINISH ||
      gCurrentActionFuncId == B_ACTION_FINISHED) {
    if (!(gBattleTypeFlags &
          (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK |
          ...
          if (gBattleResults.playerMon1Species == SPECIES_NONE) {
            gBattleResults.playerMon1Species =
                GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]],
                           MON_DATA_SPECIES, NULL);
            GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]],
                       MON_DATA_NICKNAME, gBattleResults.playerMon1Name);
          } else {
            gBattleResults.playerMon2Species =
                GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]],
                           MON_DATA_SPECIES, NULL);
            GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]],
                       MON_DATA_NICKNAME, gBattleResults.playerMon2Name);
          }
        }
      }
    ...
    if (!(gBattleTypeFlags &
          (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_TRAINER |
           BATTLE_TYPE_FIRST_BATTLE | BATTLE_TYPE_SAFARI |
  ...
  while (gLeveledUpInBattle != 0) {
    for (i = 0; i < PARTY_SIZE; i++) {
      if (gLeveledUpInBattle & gBitTable[i]) {
        ...
        species = GetEvolutionTargetSpecies(&gPlayerParty[i], EVO_MODE_NORMAL,
                                            levelUpBits);
        if (species != SPECIES_NONE) {
          FreeAllWindowBuffers();
