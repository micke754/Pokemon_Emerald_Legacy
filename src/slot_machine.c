...
#define BIAS_REGULAR                                                           \
  (BIAS_REPLAY | BIAS_CHERRY | BIAS_LOATAD | BIAS_AZURILL | BIAS_POWER)
...
// IDs for digital display "scenes", i.e. each of the screens it can show made
// up of sprites
enum {
...
static u8 CreateDigitalDisplaySprite(u8, void (*callback)(struct Sprite *), s16,
                                     s16, s16);
...
static EWRAM_DATA struct SpriteFrameImage *sImageTable_ReelTimeMachineAntennae =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_ReelTimeMachine = NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_BrokenReelTimeMachine =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Reel =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Time =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Insert =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Stop =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Win =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Lose =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Bonus =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Big =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Reg =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_AButton =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Smoke =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Number =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_Pokeball =
    NULL;
static EWRAM_DATA struct SpriteFrameImage *sImageTable_DigitalDisplay_DPad =
    NULL;
...
// IWRAM bss
static struct SpriteFrameImage
    *sImageTables_DigitalDisplay[NUM_DIG_DISPLAY_SPRITES];
...
static const u8 sColors_ReeltimeHelp[] = {
    TEXT_COLOR_LIGHT_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_DARK_GRAY};
...
// Just idles, digital display is handled by CreateDigitalDisplayScene and
// sprite callbacks
...
static void SlotMachine_InitFromTask(void) {
  ...
  LoadWordFromTwoHalfwords((u16 *)&task->tExitCallback,
                           (u32 *)&sSlotMachine->prevMainCb);
}
...
static void SlotMachineSetup_InitGpuRegs(void) {
  ...
  SetGpuReg(REG_OFFSET_BLDCNT,
            BLDCNT_TGT1_BG3 | BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_OBJ);
...
static bool8 SlotTask_HandleBetInput(struct Task *task) {
  ...
  // Try to bet the max amount
    ...
    if (sSlotMachine->bet >= MAX_BET ||
        (sSlotMachine->bet != 0 && JOY_NEW(A_BUTTON)))
      sSlotMachine->state = SLOTTASK_START_SPIN;
...
static bool8 SlotTask_PrintMsg_Need3Coins(struct Task *task) {
  ...
  AddTextPrinterParameterized(0, FONT_NORMAL, gText_YouDontHaveThreeCoins, 0, 1,
                              0, 0);
...
static bool8 SlotTask_CheckMatches(struct Task *task) {
  ...
  if (sSlotMachine->matches) {
    ...
    if (sSlotMachine->matches &
        ((1 << MATCH_MIXED_7) | (1 << MATCH_BLUE_7) | (1 << MATCH_RED_7))) {
      sSlotMachine->machineBias &= ~(BIAS_STRAIGHT_7 | BIAS_MIXED_7);
      if (sSlotMachine->matches & ((1 << MATCH_BLUE_7) | (1 << MATCH_RED_7))) {
    ...
    if (sSlotMachine->matches & (1 << MATCH_POWER) &&
        sSlotMachine->pikaPowerBolts < 16) {
...
static bool8 SlotTask_EndPayout(struct Task *task) {
  if (TryStopSlotMachineLights()) {
    ...
    if (sSlotMachine->reelTimeSpinsLeft &&
        sSlotMachine->matches & (1 << MATCH_REPLAY)) {
      CreateDigitalDisplayScene(DIG_DISPLAY_REEL_TIME);
...
static bool8 SlotTask_PrintMsg_MaxCoins(struct Task *task) {
  DrawDialogueFrame(0, FALSE);
  AddTextPrinterParameterized(0, FONT_NORMAL, gText_YouveGot9999Coins, 0, 1, 0,
                              0);
  CopyWindowToVram(0, COPYWIN_FULL);
...
static bool8 SlotTask_PrintMsg_NoMoreCoins(struct Task *task) {
  DrawDialogueFrame(0, FALSE);
  AddTextPrinterParameterized(0, FONT_NORMAL, gText_YouveRunOutOfCoins, 0, 1, 0,
                              0);
  CopyWindowToVram(0, COPYWIN_FULL);
...
static u8 TrySelectBias_Special(void) {
  ...
  for (whichBias = 0; whichBias < (int)ARRAY_COUNT(sBiasesSpecial);
       whichBias++) {
...
static u8 TrySelectBias_Regular(void) {
  ...
  for (whichBias = 0; whichBias < (int)ARRAY_COUNT(sBiasesRegular);
       whichBias++) {
    s16 rval = Random() & 0xff;
...
static u8 GetReelTimeSpinProbability(u8 spins) {
  if (sSlotMachine->luckyGame == FALSE)
    return sReelTimeProbabilities_NormalGame[spins]
                                            [sSlotMachine->pikaPowerBolts];
  else
    return sReelTimeProbabilities_LuckyGame[spins]
                                           [sSlotMachine->pikaPowerBolts];
}
...
static u16 ReelTimeSpeed(void) {
  ...
  rval = Random() % 100;
  value = sReelTimeSpeed_Probabilities[i][1] +
          sQuarterSpeed_ProbabilityBoost[sSlotMachine->reelTimeSpinsUsed];
  if (rval < value)
...
static void AwardPayout(void) { Task_Payout(CreateTask(Task_Payout, 4)); }
...
static u8 GetReelTimeSymbol(s16 offset) {
  s16 newPosition =
      (sSlotMachine->reeltimePosition + offset) % REELTIME_SYMBOLS;
  if (newPosition < 0)
...
static void AdvanceSlotReel(u8 reelIndex, s16 value) {
  ...
  sSlotMachine->reelPositions[reelIndex] =
      SYMBOLS_PER_REEL -
      sSlotMachine->reelPixelOffsets[reelIndex] / REEL_SYMBOL_HEIGHT;
}
...
static void AdvanceReeltimeReel(s16 value) {
  ...
  sSlotMachine->reeltimePosition =
      REELTIME_SYMBOLS -
      sSlotMachine->reeltimePixelOffset / REELTIME_SYMBOL_HEIGHT;
}
...
static bool8 ReelTask_StayStill(struct Task *task) { return FALSE; }
...
static bool8 ReelTask_DecideStop(struct Task *task) {
  ...
  if (sSlotMachine->reelTimeSpinsLeft == 0) {
    if (sSlotMachine->machineBias == 0 || !sSlotMachine->didNotFailBias ||
...
static bool8 ReelTask_MoveToStop(struct Task *task) {
  ...
  reelPixelPos =
      sSlotMachine->reelPixelOffsets[task->tReelId] % REEL_SYMBOL_HEIGHT;
  if (reelPixelPos != 0)
    reelPixelPos =
        AdvanceSlotReelToNextSymbol(task->tReelId, sSlotMachine->reelSpeed);
...
static bool8 BiasedTowardCherryOr7s(void) {
  if (sSlotMachine->machineBias &
      (BIAS_STRAIGHT_7 | BIAS_MIXED_7 | BIAS_CHERRY))
    return TRUE;
  else
    return FALSE;
}
...
//    necessary. If there are cherries in the two positions below the bias
//    symbol, it will skip over this option, even if those cherries would not
...
static void DecideStop_NoBias_Reel2_Bet1(void) {
  if (sSlotMachine->winnerRows[0] != 0 &&
    ...
    u8 reel1MiddleSym =
        GetSymbol(LEFT_REEL, 2 - sSlotMachine->reelExtraTurns[0]);
...
static void DecideStop_NoBias_Reel2_Bet2(void) {
  if (sSlotMachine->winnerRows[0] != 0 &&
      sSlotMachine->machineBias & BIAS_STRAIGHT_7) {
    u8 reel1BiasSym = GetSymbol(LEFT_REEL, sSlotMachine->winnerRows[0] -
                                               sSlotMachine->reelExtraTurns[0]);
    if (IfSymbol7_SwitchColor(&reel1BiasSym)) {
      s16 i;
      for (i = 0; i <= MAX_EXTRA_TURNS; i++) {
        if (reel1BiasSym ==
            GetSymbol(MIDDLE_REEL, sSlotMachine->winnerRows[0] - i)) {
...
static void DecideStop_NoBias_Reel2_Bet3(void) {
  ...
  if (sSlotMachine->winnerRows[0] != 0 &&
    ...
    reel1BiasSym = GetSymbol(LEFT_REEL, sSlotMachine->winnerRows[0] -
                                            sSlotMachine->reelExtraTurns[0]);
    if (IfSymbol7_SwitchColor(&reel1BiasSym)) {
      ...
      for (j = 1; j <= MAX_EXTRA_TURNS; j++) {
        if (reel1BiasSym ==
            GetSymbol(MIDDLE_REEL, sSlotMachine->winnerRows[0] - j)) {
...
static bool8 NeitherMatchNor7Mismatch(u8 sym1, u8 sym2, u8 sym3) {
  if ((sym1 == SYMBOL_7_RED && sym2 == SYMBOL_7_BLUE && sym3 == SYMBOL_7_RED) ||
      ...
      (sym1 == SYMBOL_7_BLUE && sym2 == SYMBOL_7_BLUE &&
       sym3 == SYMBOL_7_RED) ||
      (sym1 == sym2 && sym1 == sym3)) {
    return FALSE;
  }
...
static void DecideStop_NoBias_Reel3_Bet2(void) {
  ...
  if (sSlotMachine->winnerRows[1] != 0 &&
      ...
      sSlotMachine->machineBias & BIAS_STRAIGHT_7) {
    sym1 = GetSymbol(LEFT_REEL, sSlotMachine->winnerRows[0] -
                                    sSlotMachine->reelExtraTurns[0]);
    sym2 = GetSymbol(MIDDLE_REEL, sSlotMachine->winnerRows[1] -
                                      sSlotMachine->reelExtraTurns[1]);
  ...
  while (TRUE) {
    ...
    for (i = 1, numMatches = 0; i <= 3; i++) {
      ...
      if (!NeitherMatchNor7Mismatch(sym1, sym2, sym3) &&
          !(MismatchedSyms_777(sym1, sym2, sym3) &&
            (sSlotMachine->machineBias & BIAS_STRAIGHT_7))) {
        numMatches++;
        break;
      }
...
static void DecideStop_NoBias_Reel3_Bet3(void) {
  ...
  if (sSlotMachine->winnerRows[1] != 0 &&
    ...
    if (MismatchedSyms_77(sym1, sym2)) {
      ...
      for (i = 0; i <= MAX_EXTRA_TURNS; i++) {
        sym3 =
            GetSymbol(RIGHT_REEL, row - (sSlotMachine->reelExtraTurns[2] + i));
        if (sym1 == sym3) {
          sSlotMachine->reelExtraTurns[2] += i;
          break;
        }
...
static void StopReelButton_Press(struct Task *task, u8 taskId) {
  SetReelButtonTilemap(sReelButtonOffsets[task->data[15]], 0x62, 0x63, 0x72,
                       0x73);
  task->data[0]++;
}
...
static void StopReelButton_Unpress(struct Task *task, u8 taskId) {
  SetReelButtonTilemap(sReelButtonOffsets[task->data[15]], 0x42, 0x43, 0x52,
                       0x53);
  DestroyTask(taskId);
}

static void LightenMatchLine(u8 matchLineId) {
  LoadPalette(sLitMatchLinePalTable[matchLineId],
              sMatchLinePalOffsets[matchLineId], PLTT_SIZEOF(1));
}

static void DarkenMatchLine(u8 matchLineId) {
  LoadPalette(sDarkMatchLinePalTable[matchLineId],
              sMatchLinePalOffsets[matchLineId], PLTT_SIZEOF(1));
}
...
// Creates invisible sprites that flash the bet lines/numbers where a match
// occurs 5 are created, 1 for each possible match line (3 rows, 2 diagonals)
static void CreateInvisibleFlashMatchLineSprites(void) {
  u8 i;
...
static void FlashMatchLine(u8 matchLineId) {
  struct Sprite *sprite =
      &gSprites[sSlotMachine->flashMatchLineSpriteIds[matchLineId]];
...
// When payout is finished, stop lines flashing (but not if they're in the
// middle of a flash)
static bool8 TryStopMatchLinesFlashing(void) {
  u8 i;
  for (i = 0; i < ARRAY_COUNT(sSlotMachine->flashMatchLineSpriteIds); i++) {
...
static void Task_FlashSlotMachineLights(u8 taskId) {
  ...
  LoadPalette(sFlashingLightsPalTable[task->sFlashState], BG_PLTT_ID(1),
              PLTT_SIZE_4BPP);
}
...
// Once the anim is done, destroy the sprite and set the bolt in the tilemap
// instead
...
static void ReelTime_PikachuSpeedUp1(struct Task *task) {
  ...
  memcpy(pikachuAnimIds, sReelTimePikachuAnimIds,
         sizeof(sReelTimePikachuAnimIds));
  memcpy(reelTimeBoltDelays, sReelTimeBoltDelays, sizeof(sReelTimeBoltDelays));
  memcpy(pikachuAuraFlashDelays, sPikachuAuraFlashDelays,
         sizeof(sPikachuAuraFlashDelays));
  ...
  StartSpriteAnimIfDifferent(&gSprites[sSlotMachine->reelTimePikachuSpriteId],
                             pikachuAnimIds[i]);
  // once speed goes below 256, go to next ReelTime task and keep the speed
  // level
...
static void ReelTime_LandOnOutcome(struct Task *task) {
  ...
  if (reeltimePixelOffset) {
    reeltimePixelOffset =
        AdvanceReeltimeReelToNextSymbol(task->tRtReelSpeed >> 8);
    task->tRtReelSpeed = (u8)task->tRtReelSpeed + 0x40;
  ...
  if (reeltimePixelOffset == 0 &&
      GetReelTimeSymbol(1) == sSlotMachine->reelTimeDraw) {
  ...
  if (++task->tTimer2 >= 60) {
    ...
    if (sSlotMachine->reelTimeDraw == 0) {
      ...
      StartSpriteAnimIfDifferent(
          &gSprites[sSlotMachine->reelTimePikachuSpriteId], 5);
      PlayFanfare(MUS_TOO_BAD);
    } else {
      task->tTimer2 = 0xc0;
      StartSpriteAnimIfDifferent(
          &gSprites[sSlotMachine->reelTimePikachuSpriteId], 4);
...
static void LoadReelTimeWindowTilemap(s16 a0, s16 a1) {
  s16 i;
  for (i = 4; i < 15; i++)
    LoadBgTilemap(1, &sReelTimeWindow_Tilemap[a1 + (i - 4) * 20], 2,
                  32 * i + a0);
}
...
static void InfoBox_AddText(struct Task *task) {
  AddTextPrinterParameterized3(1, FONT_NORMAL, 2, 5, sColors_ReeltimeHelp, 0,
                               gText_ReelTimeHelp);
...
static void AddDigitalDisplaySprite(u8 templateIdx, SpriteCallback callback,
                                    s16 x, s16 y, s16 spriteId) {
  u8 i;
...
static void CreateReelSymbolSprites(void) {
  ...
  for (i = 0, x = 0x30; i < 3; i++, x += 0x28) {
    for (j = 0; j < 120; j += 24) {
      struct Sprite *sprite =
          gSprites + CreateSprite(&sSpriteTemplate_ReelSymbol, x, 0, 14);
...
static void SpriteCB_ReelSymbol(struct Sprite *sprite) {
  sprite->data[2] =
      sSlotMachine->reelPixelOffsets[sprite->data[0]] + sprite->data[1];
  sprite->data[2] %= 120;
  sprite->y =
      sSlotMachine->reelShockOffsets[sprite->data[0]] + 28 + sprite->data[2];
  sprite->sheetTileStart = GetSpriteTileStartByTag(
      GetSymbolAtRest(sprite->data[0], sprite->data[2] / 24));
...
#define sCurNum                                                                \
  data[3] // Only used to determine whether the sprite has already been updated
          // to show the correct digit

static void CreateCoinNumberSprite(s16 x, s16 y, bool8 isPayout,
                                   s16 digitMult) {
  struct Sprite *sprite =
      &gSprites[CreateSprite(&sSpriteTemplate_CoinNumber, x, y, 13)];
...
static void CreateReelTimePikachuSprite(void) {
  ...
  if (sImageTable_ReelTimePikachu == NULL)
    sImageTable_ReelTimePikachu =
        AllocZeroed(sizeof(struct SpriteFrameImage) * 5);
...
static void CreateReelTimeMachineSprites(void) {
  ...
  if (sImageTable_ReelTimeMachineAntennae == NULL)
    sImageTable_ReelTimeMachineAntennae =
        AllocZeroed(sizeof(struct SpriteFrameImage) * 1);
...
static void CreateReelTimeNumberSprites(void) {
  u8 i;
  s16 r5;
  for (i = 0, r5 = 0; i < ARRAY_COUNT(sSlotMachine->reelTimeNumberSpriteIds);
       i++, r5 += 20) {
...
// Creates a small black bar on the Reel Time machine to fill the gap between
// the numbers
static void CreateReelTimeNumberGapSprite(void) {
...
static void SpriteCB_ReelTimePikachuAura(struct Sprite *sprite) {
  ...
  if (sprite->sFlashPal && --sprite->sDelayTimer <= 0) {
    MultiplyInvertedPaletteRGBComponents(
        OBJ_PLTT_ID(IndexOfSpritePaletteTag(PALTAG_PIKA_AURA)) + 3,
...
static void DestroyReelTimePikachuAuraSprites(void) {
  u8 i;
  MultiplyInvertedPaletteRGBComponents(
      OBJ_PLTT_ID(IndexOfSpritePaletteTag(PALTAG_PIKA_AURA)) + 3, 0, 0, 0);
...
static void CreateReelTimeDuckSprites(void) {
  ...
  for (i = 0; i < ARRAY_COUNT(sSlotMachine->reelTimeDuckSpriteIds); i++) {
    u8 spriteId = CreateSprite(&sSpriteTemplate_ReelTimeDuck,
                               80 - gSpriteCoordOffsetX, 68, 0);
    struct Sprite *sprite = &gSprites[spriteId];
    sprite->oam.priority = 1;
    sprite->coordOffsetEnabled = TRUE;
    sprite->data[0] = sp[i];
...
static u8 CreateStdDigitalDisplaySprite(u8 templateIdx, u8 dispInfoId,
                                        s16 spriteId) {
  return CreateDigitalDisplaySprite(
      templateIdx, sDigitalDisplay_SpriteCallbacks[dispInfoId],
...
static u8 CreateDigitalDisplaySprite(u8 templateIdx, SpriteCallback callback,
                                     s16 x, s16 y, s16 internalSpriteId) {
  struct SpriteTemplate spriteTemplate;
  u8 spriteId;
  struct Sprite *sprite;
...
static void SpriteCB_DigitalDisplay_AButtonStop(struct Sprite *sprite) {
  ...
  case 0:
    ...
    if (++sprite->sCounter > 0x20) {
      ...
      SetGpuReg(REG_OFFSET_MOSAIC, ((sprite->sCounter << 4) | sprite->sCounter)
                                       << 8);
    }
    break;
  case 1:
...
static void SpriteCB_DigitalDisplay_PokeballShining(struct Sprite *sprite) {
  if (sprite->sCounter < 3) {
    LoadPalette(sPokeballShiningPalTable[sprite->sCounter],
                OBJ_PLTT_ID(IndexOfSpritePaletteTag(PALTAG_DIG_DISPLAY)),
                PLTT_SIZE_4BPP);
    if (++sprite->data[2] >= 4) {
      sprite->data[1]++;
      sprite->data[2] = 0;
...
static void SpriteCB_DigitalDisplay_AButtonStart(struct Sprite *sprite) {
  ...
  case 2:
    if (sSlotMachine->bet == 0)
      break;
    AddDigitalDisplaySprite(DIG_SPRITE_A_BUTTON, SpriteCallbackDummy, 208, 116,
                            0);
...
static void EndDigitalDisplayScene_Win(void) {
  LoadPalette(sDigitalDisplay_Pal,
              OBJ_PLTT_ID(IndexOfSpritePaletteTag(PALTAG_DIG_DISPLAY)),
              PLTT_SIZE_4BPP);
}
...
static void LoadSlotMachineGfx(void) {
  ...
  sSlotMachineSpritesheetsPtr = AllocZeroed(
      sizeof(struct SpriteSheet) * ARRAY_COUNT(sSlotMachineSpriteSheets));
  ...
  sSlotMachineSpritesheetsPtr[GFXTAG_STOP - 1].data =
      sDigitalDisplayGfxPtr + 0xA00;
  sSlotMachineSpritesheetsPtr[GFXTAG_BONUS - 1].data =
      sDigitalDisplayGfxPtr + 0x1400;
  sSlotMachineSpritesheetsPtr[GFXTAG_BIG - 1].data =
      sDigitalDisplayGfxPtr + 0x1600;
  sSlotMachineSpritesheetsPtr[GFXTAG_REG - 1].data =
      sDigitalDisplayGfxPtr + 0x1900;
...
// For (un)shading the gray button at the bottom of a reel when A is pressed.
// The button is colored in quadrants
static void SetReelButtonTilemap(s16 offset, u16 topLeft, u16 topRight,
                                 u16 bottomLeft, u16 bottomRight) {
  ...
  LoadBgTilemap(2, sReelButtonPress_Tilemap + 1, 2,
                15 * 32 + 1 + offset); // Top right
  LoadBgTilemap(2, sReelButtonPress_Tilemap + 2, 2,
                16 * 32 + offset); // Bottom left
  LoadBgTilemap(2, sReelButtonPress_Tilemap + 3, 2,
                16 * 32 + 1 + offset); // Bottom Right
}
...
static void SetDigitalDisplayImagePtrs(void) {
  sImageTables_DigitalDisplay[DIG_SPRITE_REEL] =
      sImageTable_DigitalDisplay_Reel;
  sImageTables_DigitalDisplay[DIG_SPRITE_TIME] =
      sImageTable_DigitalDisplay_Time;
  sImageTables_DigitalDisplay[DIG_SPRITE_INSERT] =
      sImageTable_DigitalDisplay_Insert;
  sImageTables_DigitalDisplay[DIG_SPRITE_WIN] = sImageTable_DigitalDisplay_Win;
  sImageTables_DigitalDisplay[DIG_SPRITE_LOSE] =
      sImageTable_DigitalDisplay_Lose;
  sImageTables_DigitalDisplay[DIG_SPRITE_A_BUTTON] =
      sImageTable_DigitalDisplay_AButton;
  sImageTables_DigitalDisplay[DIG_SPRITE_SMOKE] =
      sImageTable_DigitalDisplay_Smoke;
  sImageTables_DigitalDisplay[DIG_SPRITE_NUMBER] =
      sImageTable_DigitalDisplay_Number;
  sImageTables_DigitalDisplay[DIG_SPRITE_POKE_BALL] =
      sImageTable_DigitalDisplay_Pokeball;
  sImageTables_DigitalDisplay[DIG_SPRITE_D_PAD] =
      sImageTable_DigitalDisplay_DPad;
  sImageTables_DigitalDisplay[DIG_SPRITE_STOP_S] =
      sImageTable_DigitalDisplay_Stop;
  sImageTables_DigitalDisplay[DIG_SPRITE_STOP_T] =
      sImageTable_DigitalDisplay_Stop;
  sImageTables_DigitalDisplay[DIG_SPRITE_STOP_O] =
      sImageTable_DigitalDisplay_Stop;
  sImageTables_DigitalDisplay[DIG_SPRITE_STOP_P] =
      sImageTable_DigitalDisplay_Stop;
  sImageTables_DigitalDisplay[DIG_SPRITE_BONUS_B] =
      sImageTable_DigitalDisplay_Bonus;
  sImageTables_DigitalDisplay[DIG_SPRITE_BONUS_O] =
      sImageTable_DigitalDisplay_Bonus;
  sImageTables_DigitalDisplay[DIG_SPRITE_BONUS_N] =
      sImageTable_DigitalDisplay_Bonus;
  sImageTables_DigitalDisplay[DIG_SPRITE_BONUS_U] =
      sImageTable_DigitalDisplay_Bonus;
  sImageTables_DigitalDisplay[DIG_SPRITE_BONUS_S] =
      sImageTable_DigitalDisplay_Bonus;
  sImageTables_DigitalDisplay[DIG_SPRITE_BIG_B] =
      sImageTable_DigitalDisplay_Big;
  sImageTables_DigitalDisplay[DIG_SPRITE_BIG_I] =
      sImageTable_DigitalDisplay_Big;
  sImageTables_DigitalDisplay[DIG_SPRITE_BIG_G] =
      sImageTable_DigitalDisplay_Big;
  sImageTables_DigitalDisplay[DIG_SPRITE_REG_R] =
      sImageTable_DigitalDisplay_Reg;
  sImageTables_DigitalDisplay[DIG_SPRITE_REG_E] =
      sImageTable_DigitalDisplay_Reg;
  sImageTables_DigitalDisplay[DIG_SPRITE_REG_G] =
      sImageTable_DigitalDisplay_Reg;
...
static void AllocDigitalDisplayGfx(void) {
  ...
  sImageTable_DigitalDisplay_AButton =
      AllocZeroed(sizeof(struct SpriteFrameImage) * 2);
...
static const u8 sReelSymbols[NUM_REELS][SYMBOLS_PER_REEL] = {
    [LEFT_REEL] = {SYMBOL_7_RED,  SYMBOL_CHERRY,  SYMBOL_AZURILL,
                   SYMBOL_REPLAY, SYMBOL_POWER,   SYMBOL_LOTAD,
                   SYMBOL_7_BLUE, SYMBOL_LOTAD,   SYMBOL_CHERRY,
                   SYMBOL_POWER,  SYMBOL_REPLAY,  SYMBOL_AZURILL,
                   SYMBOL_7_RED,  SYMBOL_POWER,   SYMBOL_LOTAD,
                   SYMBOL_REPLAY, SYMBOL_AZURILL, SYMBOL_7_BLUE,
                   SYMBOL_POWER,  SYMBOL_LOTAD,   SYMBOL_REPLAY},
    [MIDDLE_REEL] = {SYMBOL_7_RED,  SYMBOL_CHERRY,  SYMBOL_REPLAY,
                     SYMBOL_LOTAD,  SYMBOL_AZURILL, SYMBOL_CHERRY,
                     SYMBOL_REPLAY, SYMBOL_POWER,   SYMBOL_POWER,
                     SYMBOL_LOTAD,  SYMBOL_7_BLUE,  SYMBOL_LOTAD,
                     SYMBOL_REPLAY, SYMBOL_CHERRY,  SYMBOL_AZURILL,
                     SYMBOL_LOTAD,  SYMBOL_REPLAY,  SYMBOL_CHERRY,
                     SYMBOL_LOTAD,  SYMBOL_REPLAY,  SYMBOL_CHERRY},
    [RIGHT_REEL] = {SYMBOL_7_RED,   SYMBOL_POWER,   SYMBOL_7_BLUE,
                    SYMBOL_REPLAY,  SYMBOL_LOTAD,   SYMBOL_AZURILL,
                    SYMBOL_REPLAY,  SYMBOL_LOTAD,   SYMBOL_POWER,
                    SYMBOL_AZURILL, SYMBOL_REPLAY,  SYMBOL_LOTAD,
                    SYMBOL_AZURILL, SYMBOL_POWER,   SYMBOL_REPLAY,
                    SYMBOL_LOTAD,   SYMBOL_AZURILL, SYMBOL_POWER,
                    SYMBOL_REPLAY,  SYMBOL_LOTAD,   SYMBOL_CHERRY},
};

static const u8 sReelTimeSymbols[] = {1, 0, 5, 4, 3, 2};
...
//       | 16            |  2%           | 31%         |
static const u8 sReelTimeProbabilities_NormalGame[][17] = {
    {243, 243, 243, 80, 80, 80, 80, 40, 40, 40, 40, 40, 40, 5, 5, 5,
     5}, // 0 spins
    {5, 5, 5, 150, 150, 150, 150, 130, 130, 130, 130, 130, 130, 100, 100, 100,
     5}, // 1 spin
    {4, 4, 4, 20, 20, 20, 20, 80, 80, 80, 80, 80, 80, 100, 100, 100,
     40},                                                     // 2 spins
...
//       | 16            |  2%           | 33%         |
static const u8 sReelTimeProbabilities_LuckyGame[][17] = {
    {243, 243, 243, 200, 200, 200, 200, 160, 160, 160, 160, 160, 160, 70, 70,
     70, 5},                                                 // 0 spins
    {5, 5, 5, 25, 25, 25, 25, 5, 5, 5, 5, 5, 5, 2, 2, 2, 6}, // 1 spin
    {4, 4, 4, 25, 25, 25, 25, 30, 30, 30, 30, 30, 30, 40, 40, 40,
     35},                                                             // 2 spins
...
static const u16 sQuarterSpeed_ProbabilityBoost[] = {0, 5, 10, 15, 20};
...
static const u16 sBiasesSpecial[] = {BIAS_STRAIGHT_7, BIAS_REELTIME,
                                     BIAS_MIXED_7};

static const u16 sBiasesRegular[] = {BIAS_POWER, BIAS_AZURILL, BIAS_LOTAD,
                                     BIAS_CHERRY, BIAS_REPLAY};

static const u8 sSymbolToMatch[] = {
    [SYMBOL_7_RED] = MATCH_RED_7,     [SYMBOL_7_BLUE] = MATCH_BLUE_7,
    [SYMBOL_AZURILL] = MATCH_AZURILL, [SYMBOL_LOTAD] = MATCH_LOTAD,
    [SYMBOL_CHERRY] = MATCH_CHERRY,   [SYMBOL_POWER] = MATCH_POWER,
    [SYMBOL_REPLAY] = MATCH_REPLAY};

static const u16 sSlotMatchFlags[] = {[MATCH_CHERRY] = 1 << MATCH_CHERRY,
                                      [MATCH_TOPBOT_CHERRY] =
                                          1 << MATCH_TOPBOT_CHERRY,
...
static const u16 sSlotPayouts[] = {
    [MATCH_CHERRY] = 2,   [MATCH_TOPBOT_CHERRY] = 4, [MATCH_REPLAY] = 0,
    [MATCH_LOTAD] = 6,    [MATCH_AZURILL] = 12,      [MATCH_POWER] = 3,
    [MATCH_MIXED_7] = 90, [MATCH_RED_7] = 300,       [MATCH_BLUE_7] = 300};
...
static const struct DigitalDisplaySprite sDigitalDisplay_InsertBet[] = {
    {DIG_SPRITE_EMPTY, DIG_DISPINFO_A_BUTTON_START,
...
static const struct SpriteFrameImage sImageTable_ReelTimeNumbers[] = {
    {gSlotMachineReelTimeNumber0, 0x80}, {gSlotMachineReelTimeNumber1, 0x80},
    {gSlotMachineReelTimeNumber2, 0x80}, {gSlotMachineReelTimeNumber3, 0x80},
    {gSlotMachineReelTimeNumber4, 0x80}, {gSlotMachineReelTimeNumber5, 0x80},
};

static const struct SpriteFrameImage sImageTable_ReelTimeShadow[] = {
    {gSlotMachineReelTimeShadow, 0x200}};
...
static const struct SpriteFrameImage sImageTable_ReelTimeDuck[] = {
    {gSlotMachineReelTimeDuck, 0x20}};
static const struct SpriteFrameImage sImageTable_ReelTimeSmoke[] = {
    {gSlotMachineReelTimeSmoke, 0x80}};
static const struct SpriteFrameImage sImageTable_PikaPowerBolt[] = {
    {gSlotMachinePikaPowerBolt, 0x20}};

static const union AnimCmd sAnim_SingleFrame[] = {ANIMCMD_FRAME(0, 1),
                                                  ANIMCMD_END};

static const union AnimCmd sAnim_ReelTimeDuck[] = {ANIMCMD_FRAME(0, 1),
                                                   ANIMCMD_JUMP(0)};
...
static const union AnimCmd sAnim_ReelTimePikachu_FellOver[] = {
    ANIMCMD_FRAME(4, 1), ANIMCMD_END};

static const union AnimCmd sAnim_ReelTimeNumber_0[] = {ANIMCMD_FRAME(0, 1),
                                                       ANIMCMD_END};

static const union AnimCmd sAnim_ReelTimeNumber_1[] = {ANIMCMD_FRAME(1, 1),
                                                       ANIMCMD_END};

static const union AnimCmd sAnim_ReelTimeNumber_2[] = {ANIMCMD_FRAME(2, 1),
                                                       ANIMCMD_END};

static const union AnimCmd sAnim_ReelTimeNumber_3[] = {ANIMCMD_FRAME(3, 1),
                                                       ANIMCMD_END};

static const union AnimCmd sAnim_ReelTimeNumber_4[] = {ANIMCMD_FRAME(4, 1),
                                                       ANIMCMD_END};

static const union AnimCmd sAnim_ReelTimeNumber_5[] = {ANIMCMD_FRAME(5, 1),
                                                       ANIMCMD_END};
...
static const union AnimCmd sAnim_DigitalDisplay_Pokeball_Rocking[] = {
    ANIMCMD_FRAME(0, 16), ANIMCMD_FRAME(1, 16), ANIMCMD_FRAME(0, 16),
    ANIMCMD_FRAME(1, 16, .hFlip = TRUE), ANIMCMD_JUMP(0)};
...
static const union AnimCmd sAnim_DigitalDisplay_Number_3[] = {
    ANIMCMD_FRAME(2, 1), ANIMCMD_END};

static const union AnimCmd sAnim_DigitalDisplay_Number_4[] = {
    ANIMCMD_FRAME(3, 1), ANIMCMD_END};
...
static const union AnimCmd *const sAnims_ReelTimePikachu[] = {
    ...
    sAnim_ReelTimePikachu_ChargingMedium, sAnim_ReelTimePikachu_ChargingFast,
    sAnim_ReelTimePikachu_Cheering,       sAnim_ReelTimePikachu_FellOver};

static const union AnimCmd *const sAnims_ReelTimeNumbers[] = {
    sAnim_ReelTimeNumber_0, sAnim_ReelTimeNumber_1, sAnim_ReelTimeNumber_2,
    sAnim_ReelTimeNumber_3, sAnim_ReelTimeNumber_4, sAnim_ReelTimeNumber_5};
...
static const union AnimCmd *const sAnims_DigitalDisplay_Number[] = {
    sAnim_DigitalDisplay_Number_1, sAnim_DigitalDisplay_Number_2,
    sAnim_DigitalDisplay_Number_3, sAnim_DigitalDisplay_Number_4,
    sAnim_DigitalDisplay_Number_5};

static const union AffineAnimCmd sAffineAnim_ReelTimeSmoke[] = {
    AFFINEANIMCMD_FRAME(16, 16, 0, 0), AFFINEANIMCMD_LOOP(0),
    AFFINEANIMCMD_FRAME(1, 1, 0, 1), AFFINEANIMCMD_LOOP(0xFF),
    AFFINEANIMCMD_END};
...
static const struct SubspriteTable sSubspriteTable_ReelTimeMachineAntennae[] = {
    {ARRAY_COUNT(sSubsprites_ReelTimeMachineAntennae),
...
static const struct SubspriteTable sSubspriteTable_BrokenReelTimeMachine[] = {
    {ARRAY_COUNT(sSubsprites_BrokenReelTimeMachine),
...
static const struct SubspriteTable sSubspriteTable_ReelTimeNumberGap[] = {
    {ARRAY_COUNT(sSubsprites_ReelTimeNumberGap),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_Reel[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_Reel),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_Time[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_Time),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_Insert[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_Insert),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_Unused1[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_Unused1),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_Win[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_Win),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_Smoke[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_SmokeBig),
     sSubsprites_DigitalDisplay_SmokeBig},
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_SmokeSmall),
     sSubsprites_DigitalDisplay_SmokeSmall}};
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_Pokeball[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_Pokeball),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_StopS[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_StopS),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_StopT[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_StopT),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_StopO[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_StopO),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_StopP[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_StopP),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_BonusO[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_BonusO),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_BonusN[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_BonusN),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_BonusU[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_BonusU),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_BonusS[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_BonusS),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_BigB[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_BigB),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_BigI[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_BigI),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_BigG[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_BigG),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_RegR[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_RegR),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_RegE[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_RegE),
...
static const struct SubspriteTable sSubspriteTable_DigitalDisplay_RegG[] = {
    {ARRAY_COUNT(sSubsprites_DigitalDisplay_RegG),
...
static const struct SpriteSheet sSlotMachineSpriteSheets[22] = {
    ...
    {.data = gSlotMachineReelSymbol3Tiles,
     .size = 0x200,
     .tag = GFXTAG_AZURILL},
...
static const u8 *const sReelBackground_Tilemap =
    gSlotMachineReelBackground_Tilemap;

static const u16 sUnusedColors[] = {
    RGB(27, 27, 27), RGB(8, 11, 26), RGB(11, 21, 13), RGB_WHITE,
    RGB(16, 26, 21), RGB(0, 22, 31), RGB(26, 21, 0),  RGB(26, 21, 0),
    RGB(29, 15, 0),  RGB(29, 15, 0),
};
...
static const u8 sMatchLinePalOffsets[NUM_MATCH_LINES] = {
    ...
    [MATCH_NWSE_DIAG] =
        BG_PLTT_ID(4) + 14, // Diag colors flipped for some reason
    [MATCH_NESW_DIAG] =
        BG_PLTT_ID(4) + 13 // Doesn't matter as both are identical
};
...
// Flashing lights at top of slot machine, brightest point inside light goes
// from toward center of machine, to middle, to toward edges
...
static const u32 sReelTimeGfx[] = INCBIN_U32(
    "graphics/slot_machine/reel_time_gfx.4bpp.lz"); // reel_time_machine and
                                                    // reel_time_pikachu
static const u16 sReelTimeWindow_Tilemap[] =
