...
static u32
HandleRibbonsMonListInput_WaitListInit(struct Pokenav_RibbonsMonList *);
...
static void InsertMonListItem(struct Pokenav_RibbonsMonList *,
...
static const LoopedTask sMonRibbonListLoopTaskFuncs[] = {
    BuildPartyMonRibbonList, InitBoxMonRibbonList, BuildBoxMonRibbonList};

static const u16 sMonRibbonListFramePal[] =
    INCBIN_U16("graphics/pokenav/ribbons/list_bg.gbapal");
static const u32 sMonRibbonListFrameTiles[] =
    INCBIN_U32("graphics/pokenav/ribbons/list_bg.4bpp.lz");
static const u32 sMonRibbonListFrameTilemap[] =
    INCBIN_U32("graphics/pokenav/ribbons/list_bg.bin.lz");
static const u16 sMonRibbonListUi_Pal[] =
    INCBIN_U16("graphics/pokenav/ribbons/list_ui.gbapal");
...
bool32 PokenavCallback_Init_MonRibbonList(void) {
  struct Pokenav_RibbonsMonList *list =
      AllocSubstruct(POKENAV_SUBSTRUCT_RIBBONS_MON_LIST,
                     sizeof(struct Pokenav_RibbonsMonList));
  if (list == NULL)
    return FALSE;
...
u32 GetRibbonsMonListCallback(void) {
  struct Pokenav_RibbonsMonList *list =
      GetSubstructPtr(POKENAV_SUBSTRUCT_RIBBONS_MON_LIST);
...
static u32
HandleRibbonsMonListInput_WaitListInit(struct Pokenav_RibbonsMonList *list) {
...
static u32
RibbonsMonMenu_ReturnToMainMenu(struct Pokenav_RibbonsMonList *list) {
...
static void InsertMonListItem(struct Pokenav_RibbonsMonList *list,
                              struct PokenavMonListItem *item) {
  u32 left = 0;
...
bool32 OpenRibbonsMonList(void) {
  struct Pokenav_RibbonsMonMenu *menu =
      AllocSubstruct(POKENAV_SUBSTRUCT_RIBBONS_MON_MENU,
                     sizeof(struct Pokenav_RibbonsMonMenu));
  if (menu == NULL)
    return FALSE;
...
void CreateRibbonsMonListLoopedTask(s32 idx) {
  struct Pokenav_RibbonsMonMenu *menu =
      GetSubstructPtr(POKENAV_SUBSTRUCT_RIBBONS_MON_MENU);
...
static u32 LoopedTask_OpenRibbonsMonList(s32 state) {
  ...
  switch (state) {
  case 0:
    InitBgTemplates(sMonRibbonListBgTemplates,
                    ARRAY_COUNT(sMonRibbonListBgTemplates));
    ...
    CopyPaletteIntoBufferUnfaded(sMonRibbonListFramePal, BG_PLTT_ID(1),
                                 sizeof(sMonRibbonListFramePal));
  ...
  case 2:
    if (FreeTempTileDataBuffersIfPossible())
      return LT_PAUSE;
    CopyPaletteIntoBufferUnfaded(sMonRibbonListUi_Pal, BG_PLTT_ID(2),
                                 sizeof(sMonRibbonListUi_Pal));
...
static void DrawListIndexNumber(s32 windowId, s32 index, s32 max) {
  ...
  *ptr++ = CHAR_SLASH;
  ...
  AddTextPrinterParameterized(windowId, FONT_NORMAL, strbuf, x, 1,
                              TEXT_SKIP_DRAW, NULL);
}
...
static void BufferRibbonMonInfoText(struct PokenavListItem *listItem,
                                    u8 *dest) {
  u8 gender;
  u8 level;
  u8 *s;
