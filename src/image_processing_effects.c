...
static void ApplyImageEffect_BlackOutline(void) {
  ...
  for (i = 0; i < gCanvasColumnEnd; i++) {
    ...
    *pixel = QuantizePixel_BlackOutline(pixel, pixel + gCanvasWidth);
    for (j = 1, pixel += gCanvasWidth; j < gCanvasRowEnd - 1;
...
static void ApplyImageEffect_Shimmer(void) {
  ...
  for (j = 0; j < MAX_DIMENSION; j++) {
    ...
    prevPixel = *pixel;
    *pixel = RGB_ALPHA;
    for (i = 1, pixel += MAX_DIMENSION; i < MAX_DIMENSION - 1;
         i++, pixel += MAX_DIMENSION) {
      if (!IS_ALPHA(*pixel)) {
        *pixel =
            QuantizePixel_BlurHard(&prevPixel, pixel, pixel + MAX_DIMENSION);
        prevPixel = *pixel;
      }
    }
...
static void AddPointillismPoints(u16 point) {
  ...
  for (i = 0; i < points[0].delta; i++) {
    u16 *pixel =
        &gCanvasPixels[points[i].row * MAX_DIMENSION] + points[i].column;
...
static u16 QuantizePixel_MotionBlur(u16 *prevPixel, u16 *curPixel) {
  ...
  if (pixelChannels[0][0] > 25 && pixelChannels[0][1] > 25 &&
      pixelChannels[0][2] > 25)
    return *curPixel;
  if (pixelChannels[1][0] > 25 && pixelChannels[1][1] > 25 &&
      pixelChannels[1][2] > 25)
    return *curPixel;
...
static u16 QuantizePixel_BlurHard(u16 *prevPixel, u16 *curPixel,
                                  u16 *nextPixel) {
  u16 red, green, blue;
  u16 prevAvg, curAvg, nextAvg;
  u16 prevDiff, nextDiff;
        ...
        *pixel = gCanvasPaletteStart;
        ...
        if (curIndex < maxIndex) {
          if (gCanvasPalette[curIndex] == RGB_BLACK) {
            ...
            // This if block seems pointless because the below while loop
            // handles this same logic.
...
// Quantizes the pixel's color channels to nearest multiple of 4, and clamps to
// [6, 30].
