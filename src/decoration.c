...
static void PrintDecorationCategoryMenuItem(u8 winid, u8 category, u8 x, u8 y,
                                            bool8 disabled, u8 speed);
...
static void DecorationItemsMenu_OnCursorMove(s32 itemIndex, bool8 flag,
                                             struct ListMenu *menu);
static void DecorationItemsMenu_PrintDecorationInUse(u8 windowId, u32 itemIndex,
                                                     u8 y);
...
static const u8 *const sSecretBasePCMenuItemDescriptions[] = {
    gText_PutOutSelectedDecorItem, gText_StoreChosenDecorInPC,
...
static const union AnimCmd *const sDecorSelectorAnimCmds[] = {
...
static const u16 sDecorShapeSizes[] = {
    [DECORSHAPE_1x1] = 4,  [DECORSHAPE_2x1] = 8,  [DECORSHAPE_3x1] = 16,
    [DECORSHAPE_4x2] = 32, [DECORSHAPE_2x2] = 16, [DECORSHAPE_1x2] = 8,
    [DECORSHAPE_1x3] = 16, [DECORSHAPE_2x4] = 32, [DECORSHAPE_3x3] = 64,
    [DECORSHAPE_3x2] = 32,
};

static const u16 sBrendanPalette[] =
    INCBIN_U16("graphics/decorations/brendan.gbapal");
...
static const u8 sDecorationPuttingAwayCursor[] =
    INCBIN_U8("graphics/decorations/put_away_cursor.4bpp");
...
static const union AnimCmd sPuttingAwayCursorAnimCmd0[] = {
    ANIMCMD_FRAME(0, 0, 0), ANIMCMD_END};
...
static void AddDecorationActionsWindow(void) {
  ...
  PrintMenuTable(windowId, ARRAY_COUNT(sDecorationMainMenuActions),
                 sDecorationMainMenuActions);
  InitMenuInUpperLeftCornerNormal(windowId,
                                  ARRAY_COUNT(sDecorationMainMenuActions),
...
static void HandleDecorationActionsMenuInput(u8 taskId) {
  if (!gPaletteFade.active) {
    ...
    default:
      PlaySE(SE_SELECT);
      sDecorationMainMenuActions[sDecorationActionsCursorPos].func.void_u8(
          taskId);
      break;
...
static void PrintCurMainMenuDescription(void) {
  ...
  AddTextPrinterParameterized2(
      0, FONT_NORMAL,
      sSecretBasePCMenuItemDescriptions[sDecorationActionsCursorPos], 0, 0,
...
static void DecorationMenuAction_Decorate(u8 taskId) {
  if (GetNumOwnedDecorations() == 0) {
    ...
    DisplayItemMessageOnField(taskId, gStringVar4,
...
static void InitDecorationCategoriesWindow(u8 taskId) {
  ...
  InitMenuInUpperLeftCornerNormal(windowId, DECORCAT_COUNT + 1,
...
static void ReinitDecorationCategoriesWindow(u8 taskId) {
  FillWindowPixelBuffer(sDecorMenuWindowIds[WINDOW_DECORATION_CATEGORIES],
                        PIXEL_FILL(1));
  PrintDecorationCategoryMenuItems(taskId);
  InitMenuInUpperLeftCornerNormal(
      sDecorMenuWindowIds[WINDOW_DECORATION_CATEGORIES], DECORCAT_COUNT + 1,
      sCurDecorationCategory);
  gTasks[taskId].func = HandleDecorationCategoriesMenuInput;
}
...
static void PrintDecorationCategoryMenuItems(u8 taskId) {
  ...
  for (i = 0; i < DECORCAT_COUNT; i++) {
    // Only DOLL and CUSHION decorations are enabled when decorating the
    // player's room.
    if (shouldDisable == TRUE && i != DECORCAT_DOLL && i != DECORCAT_CUSHION)
      PrintDecorationCategoryMenuItem(windowId, i, 8, i * 16, TRUE,
                                      TEXT_SKIP_DRAW);
    else
      PrintDecorationCategoryMenuItem(windowId, i, 8, i * 16, FALSE,
                                      TEXT_SKIP_DRAW);
  }

  AddTextPrinterParameterized(
      windowId, FONT_NORMAL,
      gTasks[taskId].tDecorationMenuCommand == DECOR_MENU_TRADE ? gText_Exit
                                                                : gText_Cancel,
      8, i * 16 + 1, 0, NULL);
  ScheduleBgCopyTilemapToVram(0);
}
...
static void PrintDecorationCategoryMenuItem(u8 winid, u8 category, u8 x, u8 y,
  ...
  AddTextPrinterParameterized(winid, FONT_NORMAL, gStringVar4, x, y, speed,
                              NULL);
  str = ConvertIntToDecimalStringN(str,
                                   GetNumOwnedDecorationsInCategory(category),
                                   STR_CONV_MODE_RIGHT_ALIGN, 2);
  *(str++) = CHAR_SLASH;
  ConvertIntToDecimalStringN(str, gDecorationInventories[category].size,
                             STR_CONV_MODE_RIGHT_ALIGN, 2);
...
static void SelectDecorationCategory(u8 taskId) {
  sNumOwnedDecorationsInCurCategory =
      GetNumOwnedDecorationsInCategory(sCurDecorationCategory);
...
static void InitDecorationItemsMenuScrollAndCursor(void) {
  SetCursorWithinListBounds(&sDecorationsScrollOffset, &sDecorationsCursorPos,
...
static void InitDecorationItemsMenuScrollAndCursor2(void) {
  SetCursorScrollWithinListBounds(&sDecorationsScrollOffset,
...
static void PrintDecorationItemMenuItems(u8 taskId) {
  ...
  if ((sCurDecorationCategory < DECORCAT_DOLL ||
  ...
  for (i = 0; i < sDecorationItemsMenu->numMenuItems - 1; i++) {
    CopyDecorationMenuItemName(sDecorationItemsMenu->names[i],
  ...
  gMultiuseListMenuTemplate.windowId =
  ...
  if (sDecorationItemsMenu->scrollIndicatorsTaskId == TASK_NONE) {
    sDecorationItemsMenu->scrollIndicatorsTaskId =
        AddScrollIndicatorArrowPairParameterized(
            SCROLL_ARROW_UP, 0x3c, 0x0c, 0x94,
            sDecorationItemsMenu->numMenuItems -
                sDecorationItemsMenu->maxShownItems,
...
static void InitDecorationItemsWindow(u8 taskId) {
  ...
  tMenuTaskId = ListMenuInit(&gMultiuseListMenuTemplate,
                             sDecorationsScrollOffset, sDecorationsCursorPos);
...
static void HandleDecorationItemsMenuInput(u8 taskId) {
  ...
  if (!gPaletteFade.active) {
    ...
    ListMenuGetScrollAndRow(tMenuTaskId, &sDecorationsScrollOffset,
                            &sDecorationsCursorPos);
    switch (input) {
    ...
    case LIST_CANCEL:
      PlaySE(SE_SELECT);
      sSecretBasePC_SelectedDecorationActions[tDecorationMenuCommand][1](
          taskId);
      break;
    default:
      PlaySE(SE_SELECT);
      ...
      DestroyListMenuTask(tMenuTaskId, &sDecorationsScrollOffset,
                          &sDecorationsCursorPos);
      RemoveDecorationWindow(WINDOW_DECORATION_CATEGORIES);
      RemoveDecorationItemsOtherWindows();
      Free(sDecorationItemsMenu);
      sSecretBasePC_SelectedDecorationActions[tDecorationMenuCommand][0](
          taskId);
      break;
    }
  }
...
static void ShowDecorationCategorySummaryWindow(u8 category) {
  PrintDecorationCategoryMenuItem(
      AddDecorationWindow(WINDOW_DECORATION_CATEGORY_SUMMARY), category, 0, 0,
      0, 0);
}
...
static void IdentifyOwnedDecorationsCurrentlyInUseInternal(u8 taskId) {
  ...
  count = 0;
  memset(sSecretBaseItemsIndicesBuffer, 0,
         sizeof(sSecretBaseItemsIndicesBuffer));
  memset(sPlayerRoomItemsIndicesBuffer, 0,
         sizeof(sPlayerRoomItemsIndicesBuffer));

  for (i = 0; i < ARRAY_COUNT(sSecretBaseItemsIndicesBuffer); i++) {
    if (gSaveBlock1Ptr->secretBases[0].decorations[i] != DECOR_NONE) {
      for (j = 0; j < gDecorationInventories[sCurDecorationCategory].size;
           j++) {
        if (gCurDecorationItems[j] ==
            gSaveBlock1Ptr->secretBases[0].decorations[i]) {
          for (k = 0; k < count && sSecretBaseItemsIndicesBuffer[k] != j + 1;
               k++)
            ;
  ...
  for (i = 0; i < ARRAY_COUNT(sPlayerRoomItemsIndicesBuffer); i++) {
    if (gSaveBlock1Ptr->playerRoomDecorations[i] != DECOR_NONE) {
      for (j = 0; j < gDecorationInventories[sCurDecorationCategory].size;
           j++) {
        if (gCurDecorationItems[j] ==
                gSaveBlock1Ptr->playerRoomDecorations[i] &&
            IsDecorationIndexInSecretBase(j + 1) != TRUE) {
          for (k = 0; k < count && sPlayerRoomItemsIndicesBuffer[k] != j + 1;
               k++)
            ;
          if (k == count) {
...
static void WarpToInitialPosition(u8 taskId) {
  ...
  SetWarpDestination(gSaveBlock1Ptr->location.mapGroup,
                     ...
                     gTasks[taskId].tInitialX, gTasks[taskId].tInitialY);
  WarpIntoMap();
}
...
static void ShowDecorationOnMap_(u16 mapX, u16 mapY, u8 decWidth, u8 decHeight,
                                 u16 decoration) {
  u16 i, j;
  s16 x, y;
  u16 attributes;
  ...
  for (j = 0; j < decHeight; j++) {
    ...
    for (i = 0; i < decWidth; i++) {
      ...
      attributes = GetMetatileAttributesById(
          ...
          gDecorations[decoration].tiles[j * decWidth + i]);
      if (MetatileBehavior_IsSecretBaseImpassable(
              attributes & METATILE_ATTR_BEHAVIOR_MASK) == TRUE ||
          (gDecorations[decoration].permission != DECORPERM_PASS_FLOOR &&
           (attributes >> METATILE_ATTR_LAYER_SHIFT) !=
               METATILE_LAYER_TYPE_NORMAL))
        impassableFlag = MAPGRID_COLLISION_MASK;
      else
        impassableFlag = 0;

      // Choose the metatile that has the wall background instead of the floor
      // if overlapping a wall.
      if (gDecorations[decoration].permission != DECORPERM_NA_WALL &&
          MetatileBehavior_IsSecretBaseNorthWall(
              MapGridGetMetatileBehaviorAt(x, y)) == TRUE)
        overlapsWall = 1;
      else
        overlapsWall = 0;

      elevation =
          GetDecorationElevation(gDecorations[decoration].id, j * decWidth + i);
      if (elevation != 0xFFFF)
...
void SetDecoration(void) {
  ...
  for (i = 0; i < NUM_DECORATION_FLAGS; i++) {
    if (FlagGet(FLAG_DECORATION_1 + i) == TRUE) {
      ...
      VarSet(VAR_OBJ_GFX_ID_0 + (gMapHeader.events->objectEvents[j].graphicsId -
                                 OBJ_EVENT_GFX_VAR_0),
      ...
      TrySpawnObjectEvent(gSpecialVar_0x8005, gSaveBlock1Ptr->location.mapNum,
                          gSaveBlock1Ptr->location.mapGroup);
      TryMoveObjectEventToMapCoords(gSpecialVar_0x8005,
                                    gSaveBlock1Ptr->location.mapNum,
                                    gSaveBlock1Ptr->location.mapGroup,
                                    gSpecialVar_0x8006, gSpecialVar_0x8007);
...
static void DecorationItemsMenuAction_AttemptPlace(u8 taskId) {
  if (sDecorationContext.isPlayerRoom == TRUE &&
    ...
    if (HasDecorationSpace() == TRUE) {
      ...
      ConvertIntToDecimalStringN(gStringVar1, sDecorationContext.size,
                                 STR_CONV_MODE_RIGHT_ALIGN, 2);
      if (sDecorationContext.isPlayerRoom == FALSE) {
...
static void Task_PlaceDecoration(u8 taskId) {
  ...
  case 1:
    ...
    SetUpPlacingDecorationPlayerAvatar(taskId,
                                       &sPlaceDecorationGraphicsDataBuffer);
    FadeInFromBlack();
...
static void SetUpPlacingDecorationPlayerAvatar(
  ...
  if (gSaveBlock2Ptr->playerGender == MALE)
    sDecor_CameraSpriteObjectIdx2 = CreateObjectGraphicsSprite(
        OBJ_EVENT_GFX_BRENDAN_DECORATING, SpriteCallbackDummy, x, 72, 0);
  else
    sDecor_CameraSpriteObjectIdx2 = CreateObjectGraphicsSprite(
        OBJ_EVENT_GFX_MAY_DECORATING, SpriteCallbackDummy, x, 72, 0);
...
// Can't place decoration where the player was standing when they interacted
// with the PC
...
static bool8 IsFloorOrBoardAndHole(u16 behaviorAt,
                                   const struct Decoration *decoration) {
  if (MetatileBehavior_IsSecretBaseTrainerSpot(behaviorAt) != TRUE) {
...
static bool8 CanPlaceDecoration(u8 taskId,
                                const struct Decoration *decoration) {
  u8 i;
  u8 j;
  u8 behaviorAt;
    ...
    for (i = 0; i < mapY; i++) {
      ...
      for (j = 0; j < mapX; j++) {
        ...
        layerType = GetMetatileAttributesById(
                        NUM_TILES_IN_PRIMARY +
                        decoration->tiles[(mapY - 1 - i) * mapX + j]) &
  ...
  case DECORPERM_BEHIND_FLOOR:
    ...
    for (j = 0; j < mapX; j++) {
      curX = gTasks[taskId].tCursorX + j;
      behaviorAt = MapGridGetMetatileBehaviorAt(curX, curY);
      layerType = GetMetatileAttributesById(NUM_TILES_IN_PRIMARY +
                                            decoration->tiles[j]) &
                  METATILE_ATTR_LAYER_MASK;
      if (!MetatileBehavior_IsNormal(behaviorAt) &&
...
static void AttemptPlaceDecoration_(u8 taskId) {
  if (CanPlaceDecoration(
          taskId, &gDecorations[gCurDecorationItems[gCurDecorationIndex]]) ==
      TRUE) {
...
static void PlaceDecoration(u8 taskId) {
  ...
  if (gDecorations[gCurDecorationItems[gCurDecorationIndex]].permission !=
      DECORPERM_SPRITE) {
    ShowDecorationOnMap(gTasks[taskId].tCursorX, gTasks[taskId].tCursorY,
                        gCurDecorationItems[gCurDecorationIndex]);
  } else {
  ...
  for (i = 0; i < sDecorationContext.size; i++) {
    if (sDecorationContext.items[i] == DECOR_NONE) {
      ...
      sDecorationContext.pos[i] =
          ((gTasks[taskId].tCursorX - MAP_OFFSET) << 4) +
          (gTasks[taskId].tCursorY - MAP_OFFSET);
      break;
    }
  }
...
static bool8 ApplyCursorMovement_IsInvalid(u8 taskId) {
  ...
  if (sDecorationLastDirectionMoved == DIR_SOUTH &&
      tCursorY - tDecorHeight - 6 < 0) {
    tCursorY++;
    return FALSE;
  }

  if (sDecorationLastDirectionMoved == DIR_NORTH &&
      tCursorY - 7 >= gMapHeader.mapLayout->height) {
    tCursorY--;
    return FALSE;
  }
...
static void ClearPlaceDecorationGraphicsDataBuffer(
    struct PlaceDecorationGraphicsDataBuffer *data) {
  CpuFill16(0, data, sizeof(*data));
}

static void CopyPalette(u16 *dest, u16 pal) {
  CpuFastCopy(&((u16 *)gTilesetPointer_SecretBase->palettes)[pal * 16], dest,
              sizeof(u16) * 16);
}
...
static void CopyTile(u8 *dest, u16 tile) {
  ...
  CpuFastCopy(&((u8 *)gTilesetPointer_SecretBase->tiles)[tile * TILE_SIZE_4BPP],
              buffer, TILE_SIZE_4BPP);
  switch (mode) {
  case 0:
  ...
  case BG_TILE_H_FLIP(0) >> 10:
    for (i = 0; i < 8; i++) {
      dest[4 * i + 0] = (buffer[4 * (i + 1) - 1] >> 4) +
                        ((buffer[4 * (i + 1) - 1] & 0x0F) << 4);
      dest[4 * i + 1] = (buffer[4 * (i + 1) - 2] >> 4) +
                        ((buffer[4 * (i + 1) - 2] & 0x0F) << 4);
      dest[4 * i + 2] = (buffer[4 * (i + 1) - 3] >> 4) +
                        ((buffer[4 * (i + 1) - 3] & 0x0F) << 4);
      dest[4 * i + 3] = (buffer[4 * (i + 1) - 4] >> 4) +
                        ((buffer[4 * (i + 1) - 4] & 0x0F) << 4);
    }
    break;
...
static void
SetDecorSelectionBoxTiles(struct PlaceDecorationGraphicsDataBuffer *data) {
  u16 i;
  for (i = 0; i < 64; i++)
...
static void
SetDecorSelectionMetatiles(struct PlaceDecorationGraphicsDataBuffer *data) {
  u8 i;
  u8 shape;

  shape = data->decoration->shape;
  for (i = 0; i < sDecorTilemaps[shape].size; i++) {
    data->tiles[sDecorTilemaps[shape].tiles[i]] =
        GetMetatile(data->decoration->tiles[sDecorTilemaps[shape].y[i]] *
                        NUM_TILES_PER_METATILE +
                    sDecorTilemaps[shape].x[i]);
  }
}
...
static u8 gpu_pal_decompress_alloc_tag_and_upload(
  ...
  if (data->decoration->permission == DECORPERM_SPRITE)
    return CreateObjectGraphicsSprite(data->decoration->tiles[0],
                                      SpriteCallbackDummy, 0, 0, 1);
  ...
  CopyPalette(
      ...
      ((u16 *)gTilesetPointer_SecretBaseRedCave
           ->metatiles)[(data->decoration->tiles[0] * NUM_TILES_PER_METATILE) +
                        7] >>
          12);
...
static u8 AddDecorationIconObjectFromIconTable(u16 tilesTag, u16 paletteTag,
  ...
  LZDecompressWram(GetDecorationIconPicOrPalette(decor, 0),
...
static u8 AddDecorationIconObjectFromObjectEvent(u16 tilesTag, u16 paletteTag,
                                                 u8 decor) {
  u8 spriteId;
  struct SpriteSheet sheet;
  ...
  if (sPlaceDecorationGraphicsDataBuffer.decoration->permission !=
      DECORPERM_SPRITE) {
    SetDecorSelectionMetatiles(&sPlaceDecorationGraphicsDataBuffer);
    SetDecorSelectionBoxOamAttributes(
        sPlaceDecorationGraphicsDataBuffer.decoration->shape);
    SetDecorSelectionBoxTiles(&sPlaceDecorationGraphicsDataBuffer);
    CopyPalette(
        sPlaceDecorationGraphicsDataBuffer.palette,
        ((u16 *)gTilesetPointer_SecretBaseRedCave->metatiles)
                [(sPlaceDecorationGraphicsDataBuffer.decoration->tiles[0] *
                  NUM_TILES_PER_METATILE) +
                 7] >>
            12);
    ...
    sheet.size =
        sDecorShapeSizes[sPlaceDecorationGraphicsDataBuffer.decoration->shape] *
        TILE_SIZE_4BPP;
    sheet.tag = tilesTag;
    LoadSpriteSheet(&sheet);
...
u8 AddDecorationIconObject(u8 decor, s16 x, s16 y, u8 priority, u16 tilesTag,
                           u16 paletteTag) {
  u8 spriteId;

  if (decor > NUM_DECORATIONS) {
    spriteId =
        AddDecorationIconObjectFromIconTable(tilesTag, paletteTag, DECOR_NONE);
    ...
    spriteId =
        AddDecorationIconObjectFromObjectEvent(tilesTag, paletteTag, decor);
    if (spriteId == MAX_SPRITES)
      return MAX_SPRITES;
    ...
    spriteId =
        AddDecorationIconObjectFromIconTable(tilesTag, paletteTag, decor);
    if (spriteId == MAX_SPRITES)
      return MAX_SPRITES;
  ...
  if (gSpecialVar_0x8004 == sCurDecorSelectedInRearrangement) {
    gSpecialVar_Result = TRUE;
                 ...
                 [sDecorationContext.items
                      [sDecorRearrangementDataBuffer[gSpecialVar_0x8004].idx]]
                     .permission == DECORPERM_SPRITE) {
    gSpecialVar_0x8005 =
        sDecorRearrangementDataBuffer[gSpecialVar_0x8004].flagId;
...
static void ClearRearrangementNonSprites(void) {
  ...
  for (i = 0; i < sCurDecorSelectedInRearrangement; i++) {
    perm = gDecorations[sDecorationContext
                            .items[sDecorRearrangementDataBuffer[i].idx]]
               .permission;
    ...
    if (perm != DECORPERM_SPRITE) {
      for (y = 0; y < sDecorRearrangementDataBuffer[i].height; y++) {
        for (x = 0; x < sDecorRearrangementDataBuffer[i].width; x++) {
          MapGridSetMetatileEntryAt(
              posX + MAP_OFFSET + x, posY + MAP_OFFSET - y,
              gMapHeader.mapLayout
                      ->map[posX + x +
                            gMapHeader.mapLayout->width * (posY - y)] |
                  0x3000);
        }
      }
...
static void SetUpPuttingAwayDecorationPlayerAvatar(void) {
  ...
  gFieldCamera.spriteId =
      CreateSprite(&sPuttingAwayCursorSpriteTemplate, 120, 80, 0);
  if (gSaveBlock2Ptr->playerGender == MALE)
    sDecor_CameraSpriteObjectIdx2 = CreateObjectGraphicsSprite(
        OBJ_EVENT_GFX_BRENDAN_DECORATING, SpriteCallbackDummy, 136, 72, 0);
  else
    sDecor_CameraSpriteObjectIdx2 = CreateObjectGraphicsSprite(
        OBJ_EVENT_GFX_MAY_DECORATING, SpriteCallbackDummy, 136, 72, 0);
...
static void
SetDecorRearrangementShape(u8 decor,
...
static bool8
DecorationIsUnderCursor(u8 taskId, u8 idx,
                        struct DecorRearrangementDataBuffer *data) {
  u8 x;
  u8 y;
  u8 xOff;
  ...
  if (sDecorationContext.items[idx] == DECOR_SAND_ORNAMENT &&
      MapGridGetMetatileIdAt(xOff + MAP_OFFSET, yOff + MAP_OFFSET) ==
          METATILE_SecretBase_SandOrnament_BrokenBase)
    ht--;
...
static void SetDecorRearrangementFlagIdIfFlagUnset(void) {
  ...
  xOff =
      sDecorationContext
          .pos[sDecorRearrangementDataBuffer[sCurDecorSelectedInRearrangement]
                   .idx] >>
      4;
  yOff =
      sDecorationContext
...
static bool8 AttemptMarkSpriteDecorUnderCursorForRemoval(u8 taskId) {
  ...
  for (i = 0; i < sDecorationContext.size; i++) {
    if (sDecorationContext.items[i] != DECOR_NONE) {
      if (gDecorations[sDecorationContext.items[i]].permission ==
          DECORPERM_SPRITE) {
        SetDecorRearrangementShape(sDecorationContext.items[i],
...
static void MarkSpriteDecorsInBoundsForRemoval(u8 left, u8 top, u8 right,
                                               u8 bottom) {
  u8 i;
  u8 xOff;
  u8 yOff;
  ...
  for (i = 0; i < sDecorationContext.size; i++) {
    ...
    if (decor != DECOR_NONE &&
        gDecorations[decor].permission == DECORPERM_SPRITE && left <= xOff &&
        top <= yOff && right >= xOff && bottom >= yOff) {
...
static void AttemptMarkDecorUnderCursorForRemoval(u8 taskId) {
  ...
  if (AttemptMarkSpriteDecorUnderCursorForRemoval(taskId) != TRUE) {
    ...
    for (i = 0; i < sDecorationContext.size; i++) {
      ...
      if (var1 != DECOR_NONE) {
        ...
        if (DecorationIsUnderCursor(
                taskId, i, &sDecorRearrangementDataBuffer[0]) == TRUE) {
    ...
    if (sCurDecorSelectedInRearrangement != 0) {
      ...
      yOff =
          sDecorationContext.pos[sDecorRearrangementDataBuffer[0].idx] & 0x0F;
...
static void DecorationItemsMenuAction_AttemptToss(u8 taskId) {
  if (IsSelectedDecorInThePC() == TRUE) {
    StringCopy(gStringVar1,
               gDecorations[gCurDecorationItems[gCurDecorationIndex]].name);
