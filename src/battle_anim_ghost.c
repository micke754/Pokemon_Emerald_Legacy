...
static const union AnimCmd sAnim_GrudgeFlame[] = {
    ...
    ANIMCMD_FRAME(24, 4), ANIMCMD_JUMP(0),
};
...
static void UpdateConfuseRayBallBlend(struct Sprite *sprite) {
  ...
  if ((sprite->data[7]++ & 0xFF) == 0) {
    ...
    SetGpuReg(REG_OFFSET_BLDALPHA,
              BLDALPHA_BLEND(sprite->data[6], 16 - sprite->data[6]));
...
// Creates a large transparent clone of the attacker centered on their position
// which shrinks to original size
...
static void AnimTask_NightShadeClone_Step1(u8 taskId) {
  ...
  if (gTasks[taskId].data[10] == 3) {
    ...
    SetGpuReg(REG_OFFSET_BLDALPHA,
              BLDALPHA_BLEND(gTasks[taskId].data[2], gTasks[taskId].data[3]));
...
static void AnimTask_NightShadeClone_Step2(u8 taskId) {
  ...
  if (gTasks[taskId].data[0] <= 0xFF) {
    SetSpriteRotScale(spriteId, gTasks[taskId].data[0], gTasks[taskId].data[0],
                      0);
  } else {
...
// Creates a transparent clone of the target which drifts up and away to the
// side
  ...
  case 0:
    ...
    if (task->data[5] == 3)
      if (task->data[3] <= 15)
        task->data[3] += 1;
    SetGpuReg(REG_OFFSET_BLDALPHA,
              BLDALPHA_BLEND(task->data[2], task->data[3]));
...
static void AnimTask_SpiteTargetShadow_Step1(u8 taskId) {
  ...
  case 1:
    ...
    CpuCopy32(&gPlttBufferUnfaded[task->data[4]],
              &gPlttBufferFaded[task->data[14]], PLTT_SIZE_4BPP);
  ...
  case 2:
    ...
    if (position == 1)
      task->data[10] =
          ScanlineEffect_InitWave(startLine, startLine + 64, 2, 6, 0,
                                  SCANLINE_EFFECT_REG_BG1HOFS, TRUE);
    else
      task->data[10] =
          ScanlineEffect_InitWave(startLine, startLine + 64, 2, 6, 0,
                                  SCANLINE_EFFECT_REG_BG2HOFS, TRUE);
  ...
  case 3:
    if (position == 1)
      SetGpuReg(REG_OFFSET_BLDCNT,
                (BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_ALL | BLDCNT_TGT1_BG1));
    else
      SetGpuReg(REG_OFFSET_BLDCNT,
                (BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_ALL | BLDCNT_TGT1_BG2));
...
static void AnimDestinyBondWhiteShadow(struct Sprite *sprite) {
  ...
  if (gBattleAnimArgs[0] == 0) {
    ...
    battler1Y =
        GetBattlerSpriteCoord(gBattleAnimAttacker, BATTLER_COORD_Y) + 28;
...
void AnimTask_DestinyBondWhiteShadow(u8 taskId) {
  ...
  baseY =
      GetBattlerSpriteCoordAttr(gBattleAnimAttacker, BATTLER_COORD_ATTR_BOTTOM);
  if (!IsContest()) {
    for (battler = 0; battler < MAX_BATTLERS_COUNT; battler++) {
      if (battler != gBattleAnimAttacker &&
          battler != BATTLE_PARTNER(gBattleAnimAttacker) &&
          IsBattlerSpriteVisible(battler)) {
        spriteId = CreateSprite(&gDestinyBondWhiteShadowSpriteTemplate, baseX,
                                baseY, 55);
    ...
    spriteId =
        CreateSprite(&gDestinyBondWhiteShadowSpriteTemplate, baseX, baseY, 55);
    if (spriteId != MAX_SPRITES) {
      x = 48;
      y = 40;
      gSprites[spriteId].data[0] = baseX << 4;
  ...
  case 0:
    if (task->data[6] == 0) {
      if (++task->data[5] > 1) {
        ...
        SetGpuReg(REG_OFFSET_BLDALPHA,
                  BLDALPHA_BLEND(task->data[8], task->data[9]));
        if (task->data[7] >= 24) {
          task->data[7] = 0;
          task->data[6] = 1;
        }
...
static void AnimCurseNail_Step2(struct Sprite *sprite) {
  if (sprite->data[0] == 0) {
    ...
    sprite->data[2]++;
    SetGpuReg(REG_OFFSET_BLDALPHA,
              (16 - sprite->data[2]) | (sprite->data[2] << 8));
    if (sprite->data[2] == 16) {
      sprite->invisible = TRUE;
...
static void AnimTask_GrudgeFlames_Step(u8 taskId) {
  ...
  case 0:
    for (i = 0; i < 6; i++) {
      spriteId = CreateSprite(&gGrudgeFlameSpriteTemplate, task->data[9],
                              task->data[10], task->data[6]);
      if (spriteId != MAX_SPRITES) {
