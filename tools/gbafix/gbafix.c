/*
    ...
    "$Header:
   /lvm/shared/ds/ds/cvs/devkitpro-cvsbackup/tools/gba/gbatools/gbafix.c,v 1.2
   2008-07-30 17:12:51 wntrmute Exp $"
...
/*
   ...
   Sierraffinity) Validates header of GBA roms.
...
//---------------------------------------------------------------------------------
{
  ...
  // show syntax
  if (argc <= 1) {
    printf("GBA ROM fixer v" VER
           " by Dark Fader / BlackThunder / WinterMute / Sierraffinity \n");
    printf("Syntax: gbafix <rom.gba> [-p] [-t[title]] [-c<game_code>] "
           "[-m<maker_code>] [-r<version>] [-d<debug>] [--silent]\n");
    printf("\n");
    printf("parameters:\n");
    printf(
    ...
    printf("    -d<debug>       Enable debugging handler and set debug entry "
           "point (0 or 1)\n");
  ...
  if (memcmp(&header, ELFMAG, 4) == 0) {
    ...
    for (i = 0; i < elfHeader->e_shnum; i++) {
      ...
      if (secHeader.sh_type == SHT_PROGBITS &&
          secHeader.sh_addr == elfHeader->e_entry)
        break;
    }
  ...
  memcpy(&header.device_type, &good_header.device_type,
         sizeof(header.device_type));
  ...
  for (arg = 1; arg < argc; arg++) {
    if (ARGV[0] == '-') {
      ...
      case 'd': // debug
      {
        ...
        header.device_type =
            (unsigned char)((NUMBER & 1) << 7); // debug handler entry point
        break;
      }
      case '-': // long arguments
      {
