...
void ReadJascPalette(char *path, struct Palette *palette) {
  ...
  if (palette->numColors < 1 || palette->numColors > 256)
    FATAL_ERROR("%d is an invalid number of colors. The number of colors must "
  ...
  for (int i = 0; i < palette->numColors; i++) {
    ...
    if (*s < '0' || *s > '9')
      FATAL_ERROR(
          "Expected only a space between red and green color components.\n");
    ...
    if (*s < '0' || *s > '9')
      FATAL_ERROR(
          "Expected only a space between green and blue color components.\n");
