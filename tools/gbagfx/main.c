...
void ConvertGbaToPng(char *inputPath, char *outputPath,
  ...
  if (options->paletteFilePath != NULL) {
    char *paletteFileExtension =
        GetFileExtensionAfterDot(options->paletteFilePath);
  ...
  if (options->isTiled) {
    if (options->tilemapFilePath != NULL) {
      int fileSize;
      image.tilemap.data.affine =
          ReadWholeFile(options->tilemapFilePath, &fileSize);
    ...
    ReadTileImage(inputPath, options->width, options->metatileWidth,
                  options->metatileHeight, &image, !image.hasPalette);
  } else {
...
void ConvertPngToGba(char *inputPath, char *outputPath,
                     struct PngToGbaOptions *options) {
  struct Image image;

  image.bitDepth = options->bitDepth;
  image.tilemap.data.affine =
      NULL; // initialize to NULL to avoid issues in FreeImage

  ReadPng(inputPath, &image);

  if (options->isTiled)
    WriteTileImage(outputPath, options->numTilesMode, options->numTiles,
                   options->metatileWidth, options->metatileHeight, &image,
                   !image.hasPalette);
  else
...
void HandleGbaToPngCommand(char *inputPath, char *outputPath, int argc,
                           char **argv) {
...
void HandlePngToGbaCommand(char *inputPath, char *outputPath, int argc,
                           char **argv) {
...
void HandlePngToJascPaletteCommand(char *inputPath, char *outputPath,
...
void HandlePngToGbaPaletteCommand(char *inputPath, char *outputPath,
                                  int argc UNUSED, char **argv UNUSED) {
  struct Palette palette = {};
...
void HandleGbaToJascPaletteCommand(char *inputPath, char *outputPath,
                                   int argc UNUSED, char **argv UNUSED) {
  struct Palette palette = {};
...
void HandleJascToGbaPaletteCommand(char *inputPath, char *outputPath, int argc,
                                   char **argv) {
  int numColors = 0;
...
void HandleLatinFontToPngCommand(char *inputPath, char *outputPath,
                                 int argc UNUSED, char **argv UNUSED) {
  struct Image image;
  image.tilemap.data.affine =
      NULL; // initialize to NULL to avoid issues in FreeImage
...
void HandlePngToLatinFontCommand(char *inputPath, char *outputPath,
                                 int argc UNUSED, char **argv UNUSED) {
  struct Image image;
  image.tilemap.data.affine =
      NULL; // initialize to NULL to avoid issues in FreeImage
...
void HandleHalfwidthJapaneseFontToPngCommand(char *inputPath, char *outputPath,
                                             int argc UNUSED,
                                             char **argv UNUSED) {
  struct Image image;
  image.tilemap.data.affine =
...
void HandlePngToHalfwidthJapaneseFontCommand(char *inputPath, char *outputPath,
                                             int argc UNUSED,
                                             char **argv UNUSED) {
  struct Image image;
  image.tilemap.data.affine =
...
void HandleFullwidthJapaneseFontToPngCommand(char *inputPath, char *outputPath,
                                             int argc UNUSED,
                                             char **argv UNUSED) {
  struct Image image;
  image.tilemap.data.affine =
...
void HandlePngToFullwidthJapaneseFontCommand(char *inputPath, char *outputPath,
                                             int argc UNUSED,
                                             char **argv UNUSED) {
  struct Image image;
  image.tilemap.data.affine =
...
void HandleLZCompressCommand(char *inputPath, char *outputPath, int argc,
                             char **argv) {
  int overflowSize = 0;
  ...
  unsigned char *buffer =
      ReadWholeFileZeroPadded(inputPath, &fileSize, overflowSize);

  int compressedSize;
  unsigned char *compressedData =
      LZCompress(buffer, fileSize + overflowSize, &compressedSize, minDistance);
...
void HandleLZDecompressCommand(char *inputPath, char *outputPath,
                               int argc UNUSED, char **argv UNUSED) {
  int fileSize;
  ...
  unsigned char *uncompressedData =
      LZDecompress(buffer, fileSize, &uncompressedSize);
...
void HandleRLCompressCommand(char *inputPath, char *outputPath, int argc UNUSED,
                             char **argv UNUSED) {
  int fileSize;
  unsigned char *buffer = ReadWholeFile(inputPath, &fileSize);
...
void HandleRLDecompressCommand(char *inputPath, char *outputPath,
                               int argc UNUSED, char **argv UNUSED) {
  int fileSize;
  unsigned char *buffer = ReadWholeFile(inputPath, &fileSize);
  ...
  int uncompressedSize;
  unsigned char *uncompressedData =
      RLDecompress(buffer, fileSize, &uncompressedSize);
...
void HandleHuffCompressCommand(char *inputPath, char *outputPath, int argc,
                               char **argv) {
  int fileSize;
  int bitDepth = 4;
  ...
  int compressedSize;
  unsigned char *compressedData =
      HuffCompress(buffer, fileSize, &compressedSize, bitDepth);
...
void HandleHuffDecompressCommand(char *inputPath, char *outputPath,
                                 int argc UNUSED, char **argv UNUSED) {
  int fileSize;
  unsigned char *buffer = ReadWholeFile(inputPath, &fileSize);
  ...
  int uncompressedSize;
  unsigned char *uncompressedData =
      HuffDecompress(buffer, fileSize, &uncompressedSize);
...
int main(int argc, char **argv) {
  ...
  if (outputFileExtension == NULL) {
    ...
    size_t newOutputPathSize = strlen(inputPath) - strlen(inputFileExtension) +
                               strlen(outputFileExtension);
