    ...
    for (j = 0; i < 2 * nitems - 1 && j < 1 << depth; j++) {
      ...
      if (k == depth) {
        ...
        if (traversal + i - parent > 128)
          FATAL_ERROR("Fatal error while compressing Huff file: unable to "
                      "encode binary tree.\n");
...
static inline void write_32_le(unsigned char *dest, int *destPos,
                               uint32_t *buff, int *buffPos) {
  dest[*destPos] = *buff;
...
static void write_bits(unsigned char *dest, int *destPos,
                       struct BitEncoding *encoding, int value, uint32_t *buff,
                       int *buffBits) {
...
unsigned char *HuffCompress(unsigned char *src, int srcSize,
                            int *compressedSize_p, int bitDepth) {
  if (srcSize <= 0)
    goto fail;
  ...
  for (int i = 0; i < nitems - 1; i++) {
    ...
    endptr->header.value =
        tree[i * 2].header.value + tree[i * 2 + 1].header.value;
    endptr->branch.left = tree + i * 2;
    endptr->branch.right = tree + i * 2 + 1;
    endptr--;
    if (i < nitems - 2 &&
        !msort(freqs, nitems - i - 1, sizeof(HuffNode_t), cmp_tree))
      goto fail;
  }
  ...
  for (int srcPos = 0; srcPos < srcSize;) {
    ...
    for (int i = 0; i < 32 / bitDepth; i++) {
      write_bits(dest, &destPos, encoding, srcBuf & (0xFF >> (8 - bitDepth)),
                 &destBuf, &destBitPos);
      srcBuf >>= bitDepth;
    }
  }
...
unsigned char *HuffDecompress(unsigned char *src, int srcSize,
                              int *uncompressedSize_p) {
  if (srcSize < 4)
    goto fail;
