...
static FILE *PngReadOpen(char *path, png_structp *pngStruct,
                         png_infop *pngInfo) {
...
static unsigned char *ConvertBitDepth(unsigned char *src, int srcBitDepth,
                                      int destBitDepth, int numPixels) {
  // Round the number of bits up to the next 8 and divide by 8 to get the number
  // of bytes.
  ...
  for (i = 0; i < srcSize; i++) {
    ...
    for (j = 8 - srcBitDepth; j >= 0; j -= srcBitDepth) {
      unsigned char pixel =
          ((srcByte >> j) % (1 << srcBitDepth)) % (1 << destBitDepth);
      *dest |= pixel << destBit;
...
void ReadPng(char *path, struct Image *image) {
  ...
  // Check if the image has a palette so that we can tell if the colors need to
  // be inverted later. Don't read the palette because it's not needed for now.
  ...
  if (bit_depth != image->bitDepth && image->tilemap.data.affine == NULL) {
    ...
    image->pixels = ConvertBitDepth(image->pixels, bit_depth, image->bitDepth,
...
void SetPngPalette(png_structp png_ptr, png_infop info_ptr,
...
void WritePng(char *path, struct Image *image) {
  ...
  int color_type =
      image->hasPalette ? PNG_COLOR_TYPE_PALETTE : PNG_COLOR_TYPE_GRAY;

  png_set_IHDR(png_ptr, info_ptr, image->width, image->height, image->bitDepth,
