...
static void AdvanceMetatilePosition(int *subTileX, int *subTileY,
                                    int *metatileX, int *metatileY,
                                    int metatilesWide, int metatileWidth,
                                    int metatileHeight) {
  (*subTileX)++;
...
static void ConvertFromTiles1Bpp(unsigned char *src, unsigned char *dest,
                                 int numTiles, int metatilesWide,
  ...
  for (int i = 0; i < numTiles; i++) {
    ...
    AdvanceMetatilePosition(&subTileX, &subTileY, &metatileX, &metatileY,
                            metatilesWide, metatileWidth, metatileHeight);
  }
}

static void ConvertFromTiles4Bpp(unsigned char *src, unsigned char *dest,
                                 int numTiles, int metatilesWide,
                                 int metatileWidth, int metatileHeight,
                                 bool invertColors) {
  int subTileX = 0;
...
static void ConvertFromTiles8Bpp(unsigned char *src, unsigned char *dest,
                                 int numTiles, int metatilesWide,
                                 int metatileWidth, int metatileHeight,
                                 bool invertColors) {
  int subTileX = 0;
...
static void ConvertToTiles1Bpp(unsigned char *src, unsigned char *dest,
                               int numTiles, int metatilesWide,
                               int metatileWidth, int metatileHeight,
                               bool invertColors) {
  int subTileX = 0;
...
static void ConvertToTiles4Bpp(unsigned char *src, unsigned char *dest,
                               int numTiles, int metatilesWide,
                               int metatileWidth, int metatileHeight,
                               bool invertColors) {
  int subTileX = 0;
...
static void ConvertToTiles8Bpp(unsigned char *src, unsigned char *dest,
                               int numTiles, int metatilesWide,
                               int metatileWidth, int metatileHeight,
                               bool invertColors) {
  int subTileX = 0;
...
static void CopyPlainPixels(unsigned char *src, unsigned char *dest, int size,
                            int dataWidth, bool invertColors) {
  if (dataWidth == 0)
    return;
...
static void DecodeAffineTilemap(unsigned char *input, unsigned char *output,
                                unsigned char *tilemap, int tileSize,
                                int numTiles) {
  for (int i = 0; i < numTiles; i++) {
...
static void DecodeNonAffineTilemap(unsigned char *input, unsigned char *output,
                                   struct NonAffineTile *tilemap, int tileSize,
                                   int outTileSize, int bitDepth,
                                   int numTiles) {
...
static unsigned char *DecodeTilemap(unsigned char *tiles,
                                    struct Tilemap *tilemap, int *numTiles_p,
                                    bool isAffine, int tileSize,
                                    int outTileSize, int bitDepth) {
  int mapTileSize = isAffine ? 1 : 2;
  int numTiles = tilemap->size / mapTileSize;
  unsigned char *decoded = calloc(numTiles, outTileSize);
  if (isAffine)
    DecodeAffineTilemap(tiles, decoded, tilemap->data.affine, tileSize,
                        numTiles);
  else
    DecodeNonAffineTilemap(tiles, decoded, tilemap->data.non_affine, tileSize,
                           outTileSize, bitDepth, numTiles);
  free(tiles);
  *numTiles_p = numTiles;
  return decoded;
}

void ReadTileImage(char *path, int tilesWidth, int metatileWidth,
                   int metatileHeight, struct Image *image, bool invertColors) {
  ...
  if (image->tilemap.data.affine != NULL) {
    int outTileSize =
        (image->bitDepth == 4 && image->palette.numColors > 16) ? 64 : tileSize;
    buffer = DecodeTilemap(buffer, &image->tilemap, &numTiles, image->isAffine,
                           tileSize, outTileSize, image->bitDepth);
    if (outTileSize == 64) {
      tileSize = 64;
      image->bitDepth = 8;
    }
  ...
  if (tilesWidth % metatileWidth != 0)
    FATAL_ERROR("The width in tiles (%d) isn't a multiple of the specified "
                "metatile width (%d)\n",
                tilesWidth, metatileWidth);

  if (tilesHeight % metatileHeight != 0)
    FATAL_ERROR("The height in tiles (%d) isn't a multiple of the specified "
                "metatile height (%d)\n",
  ...
  case 1:
    ConvertFromTiles1Bpp(buffer, image->pixels, numTiles, metatilesWide,
                         metatileWidth, metatileHeight, invertColors);
    break;
  case 4:
    ConvertFromTiles4Bpp(buffer, image->pixels, numTiles, metatilesWide,
                         metatileWidth, metatileHeight, invertColors);
    break;
  case 8:
    ConvertFromTiles8Bpp(buffer, image->pixels, numTiles, metatilesWide,
                         metatileWidth, metatileHeight, invertColors);
    break;
  }
...
void WriteTileImage(char *path, enum NumTilesMode numTilesMode, int numTiles,
                    int metatileWidth, int metatileHeight, struct Image *image,
                    bool invertColors) {
  int tileSize = image->bitDepth * 8;
  ...
  if (numTiles == 0)
    ...
    FATAL_ERROR("The specified number of tiles (%d) is greater than the "
  ...
  switch (image->bitDepth) {
  case 1:
    ConvertToTiles1Bpp(image->pixels, buffer, maxNumTiles, metatilesWide,
                       metatileWidth, metatileHeight, invertColors);
    break;
  case 4:
    ConvertToTiles4Bpp(image->pixels, buffer, maxNumTiles, metatilesWide,
                       metatileWidth, metatileHeight, invertColors);
    break;
  case 8:
    ConvertToTiles8Bpp(image->pixels, buffer, maxNumTiles, metatilesWide,
                       metatileWidth, metatileHeight, invertColors);
    break;
  }
  ...
  for (int i = bufferSize; i < maxBufferSize && zeroPadded; i++) {
    if (buffer[i] != 0) {
      ...
      case NUM_TILES_WARN:
        fprintf(stderr,
                "Ignoring -num_tiles %d because tile %d contains "
...
void ReadPlainImage(char *path, int dataWidth, struct Image *image,
                    bool invertColors) {
  int fileSize;
  unsigned char *buffer = ReadWholeFile(path, &fileSize);

  if (fileSize % dataWidth != 0)
    FATAL_ERROR("The image data size (%d) isn't a multiple of the specified "
                "data width %d.\n",
                fileSize, dataWidth);
...
void WritePlainImage(char *path, int dataWidth, struct Image *image,
                     bool invertColors) {
...
void ReadGbaPalette(char *path, struct Palette *palette) {
  ...
  for (int i = 0; i < palette->numColors; i++) {
    ...
    palette->colors[i].green =
        UPCONVERT_BIT_DEPTH(GET_GBA_PAL_GREEN(paletteEntry));
    palette->colors[i].blue =
        UPCONVERT_BIT_DEPTH(GET_GBA_PAL_BLUE(paletteEntry));
  }
  // png can only accept 16 or 256 colors, so fill the remainder with black
  if (palette->numColors > 16) {
    memset(&palette->colors[palette->numColors], 0,
           (256 - palette->numColors) * sizeof(struct Color));
