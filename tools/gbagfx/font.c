...
unsigned char gFontPalette[][3] = {
    {0x90, 0xC8,
     0xFF}, // bg (saturated blue that contrasts well with the shadow color)
    {0x38, 0x38, 0x38}, // fg (dark grey)
    {0xD8, 0xD8, 0xD8}, // shadow (light grey)
    {0xFF, 0xFF, 0xFF}  // box (white)
};

static void ConvertFromLatinFont(unsigned char *src, unsigned char *dest,
...
static void ConvertToLatinFont(unsigned char *src, unsigned char *dest,
                               unsigned int numRows) {
...
static void ConvertFromHalfwidthJapaneseFont(unsigned char *src,
                                             unsigned char *dest,
                                             unsigned int numRows) {
  for (unsigned int row = 0; row < numRows; row++) {
    for (unsigned int column = 0; column < 16; column++) {
      ...
      for (unsigned int glyphTile = 0; glyphTile < 2; glyphTile++) {
        ...
        unsigned int srcPixelsOffset =
            512 * (glyphIndex >> 4) + 16 * (glyphIndex & 0xF) + 256 * glyphTile;
...
static void ConvertToHalfwidthJapaneseFont(unsigned char *src,
                                           unsigned char *dest,
                                           unsigned int numRows) {
  for (unsigned int row = 0; row < numRows; row++) {
    for (unsigned int column = 0; column < 16; column++) {
...
static void ConvertFromFullwidthJapaneseFont(unsigned char *src,
                                             unsigned char *dest,
                                             unsigned int numRows) {
  for (unsigned int row = 0; row < numRows; row++) {
    for (unsigned int column = 0; column < 16; column++) {
      ...
      for (unsigned int glyphTile = 0; glyphTile < 4; glyphTile++) {
        unsigned int pixelsX = (column * 16) + ((glyphTile & 1) * 8);
        unsigned int srcPixelsOffset =
            512 * (glyphIndex >> 3) + 32 * (glyphIndex & 7) +
            256 * (glyphTile >> 1) + 16 * (glyphTile & 1);
...
static void ConvertToFullwidthJapaneseFont(unsigned char *src,
                                           unsigned char *dest,
                                           unsigned int numRows) {
  for (unsigned int row = 0; row < numRows; row++) {
    for (unsigned int column = 0; column < 16; column++) {
