...
struct ProtectStruct {
  ...
  u32 flag2Unknown : 1; // Only set to 0 once. Checked in 'WasUnableToUseMove'
                        // function.
...
struct BattleStruct {
  ...
  u16 assistPossibleMoves[PARTY_SIZE *
                          MAX_MON_MOVES]; // Each of mons can know max 4 moves.
  ...
  u8 palaceFlags; // First 4 bits are "is <= 50% HP and not asleep" for each
                  // battler, last 4 bits are selected moves to pass to AI
  ...
  u8 lastTakenMove[MAX_BATTLERS_COUNT * 2 *
                   2]; // Last move that a battler was hit with. This field
                       // seems to erroneously take 16 bytes instead of 8.
  ...
  u8 turnSideTracker;
  u8 unused_6[3];
  u8 givenExpMons; // Bits for enemy party's Pokémon that gave exp to player's
                   // party.
  ...
  u16 arenaStartHp[2];
  u8 arenaLostPlayerMons; // Bits for party member, lost as in referee's
                          // decision, not by fainting.
  u8 arenaLostOpponentMons;
  u8 alreadyStatusedMoveAttempt; // As bits for battlers; For example when using
                                 // Thunder Wave on an already paralyzed
                                 // Pokémon.
};

// The palaceFlags member of struct BattleStruct contains 1 flag per move to
// indicate which moves the AI should consider, and 1 flag per battler to
// indicate whether the battler is awake and at <= 50% HP (which affects move
// choice). The assert below is to ensure palaceFlags is large enough to store
// these flags without overlap.
STATIC_ASSERT(sizeof(((struct BattleStruct *)0)->palaceFlags) * 8 >=
                  MAX_BATTLERS_COUNT + MAX_MON_MOVES,
              PalaceFlagsTooSmall)

#define DYNAMIC_TYPE_MASK ((1 << 6) - 1)
#define F_DYNAMIC_TYPE_IGNORE_PHYSICALITY                                      \
  (1 << 6) // If set, the dynamic type's physicality won't be used for certain
           // move effects.
#define F_DYNAMIC_TYPE_SET                                                     \
  (1 << 7) // Set for all dynamic types to distinguish a dynamic type of Normal
           // (0) from no dynamic type.
...
#define IS_TYPE_PHYSICAL(moveType)                                             \
  ((moveType < TYPE_MYSTERY && moveType != TYPE_GHOST) ||                      \
   moveType == TYPE_DARK) // Doing dark/ghost switch this way to preserve IDs
                          // for pkhex/trade functionality
#define IS_TYPE_SPECIAL(moveType)                                              \
  ((moveType > TYPE_MYSTERY && moveType != TYPE_DARK) || moveType == TYPE_GHOST)
...
struct MonSpritesGfx {
  void
      *firstDecompressed; // ptr to the decompressed sprite of the first Pokémon
  union {
