...
enum {
  ...
  TAG_CONDITION_MARKINGS_MENU_2, // Used implicitly by
...
#define GET_NUM_CONDITION_SPARKLES(sheen)                                      \
  ((sheen) != MAX_SHEEN)                                                       \
      ? (sheen) / ((u32)MAX_SHEEN / (MAX_CONDITION_SPARKLES - 1) + 1)          \
      : MAX_CONDITION_SPARKLES - 1;

#define CONDITION_GRAPH_TOP_Y 56
#define CONDITION_GRAPH_BOTTOM_Y 121
#define CONDITION_GRAPH_HEIGHT                                                 \
  (CONDITION_GRAPH_BOTTOM_Y - CONDITION_GRAPH_TOP_Y + 1)
#define CONDITION_GRAPH_CENTER_X 155
#define CONDITION_GRAPH_CENTER_Y                                               \
  ((CONDITION_GRAPH_BOTTOM_Y + CONDITION_GRAPH_TOP_Y) / 2 + 3)
...
// Yet another order. This one is the same (by coincidence) as the contest
// categories
enum {
  GRAPH_COOL,
  GRAPH_BEAUTY,
...
struct ConditionGraph {
  ...
  /*0x014*/ struct UCoords16 savedPositions[CONDITION_GRAPH_LOAD_MAX]
                                           [CONDITION_COUNT];
  /*0x064*/ struct UCoords16 newPositions[CONDITION_GRAPH_UPDATE_STEPS]
                                         [CONDITION_COUNT];
...
void GetConditionMenuMonConditions(struct ConditionGraph *graph, u8 *sheen,
                                   u16 boxId, u16 monId, u16 partyId, u16 id,
                                   u16 numMons, bool8 excludesCancel);
