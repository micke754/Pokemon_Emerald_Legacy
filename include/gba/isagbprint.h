...
#define DebugAssert(pFile, nLine, pExpression, nStopProgram)                   \
  AGBAssert(pFile, nLine, pExpression, nStopProgram)
...
#define AGB_ASSERT(exp)                                                        \
  (exp) ? ((void *)0) : DebugAssert(__FILE__, __LINE__, #exp, TRUE)
#define AGB_WARNING(exp)                                                       \
  (exp) ? ((void *)0) : DebugAssert(__FILE__, __LINE__, #exp, FALSE)

#define AGB_WARNING_EX(exp, file, line)                                        \
  (exp) ? ((void *)0) : DebugAssert(file, line, #exp, FALSE);
#define AGB_ASSERT_EX(exp, file, line)                                         \
  (exp) ? ((void *)0) : DebugAssert(file, line, #exp, TRUE);
#endif
