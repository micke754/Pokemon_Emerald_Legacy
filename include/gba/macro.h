...
#define CPU_FILL(value, dest, size, bit)                                       \
  do {                                                                         \
    _Static_assert(_Alignof(dest) >= (bit / 8),                                \
...
#define CPU_COPY(src, dest, size, bit)                                         \
  do {                                                                         \
    _Static_assert(_Alignof(src) >= (bit / 8),                                 \
...
#define DmaSet(dmaNum, src, dest, control)                                     \
  do {                                                                         \
    _Static_assert(                                                            \
        _Alignof(src) >=                                                       \
            __builtin_choose_expr(__builtin_constant_p(control),               \
                                  ((control) & (DMA_32BIT << 16)) ? 4 : 2, 2), \
        "source potentially unaligned");                                       \
    _Static_assert(                                                            \
        _Alignof(dest) >=                                                      \
            __builtin_choose_expr(__builtin_constant_p(control),               \
