...
// The below macro does a%n, but (to match) will switch to a&(n-1) if n is a
// power of 2. There are cases where GF does a&(n-1) where we would really like
// to have a%n, because if n is changed to a value that isn't a power of 2 then
// a&(n-1) is unlikely to work as intended, and a%n for powers of 2 isn't always
// optimized to use &.
...
// There are many quirks in the source code which have overarching behavioral
// differences from a number of other files. For example, diploma.c seems to
// declare rodata before each use while other files declare out of order and
// must be at the beginning. There are also a number of macros which differ from
// one file to the next due to the method of obtaining the result, such as these
// below. Because of this, there is a theory (Two Team Theory) that states that
// these programming projects had more than 1 "programming team" which utilized
// different macros for each of the files that were worked on.
...
// NUM_DEX_FLAG_BYTES allocates more flags than it needs to, as NUM_SPECIES
// includes the "old unown" values that don't appear in the Pokédex.
// NATIONAL_DEX_COUNT does not include these values.
...
struct BerryCrush {
  u16 pressingSpeeds[4]; // For the record with each possible group size, 2-5
                         // players
...
struct BattleFrontier {
  ...
  /*0x738*/ struct EmeraldBattleTowerRecord
      towerRecords[BATTLE_TOWER_RECORD_COUNT]; // From record mixing.
  ...
  /*0xCB2*/ u16 curChallengeBattleNum; // Battle number / room number (Pike) /
                                       // floor number (Pyramid)
  ...
  /*0xD04*/ u16
      towerNumWins; // Increments to MAX_STREAK but never read otherwise
  ...
  /*0xD24*/ struct BattleDomeTrainer
      domeTrainers[DOME_TOURNAMENT_TRAINERS_COUNT];
  ...
  /*0xE2A*/ u8
      pyramidTrainerFlags; // 1 bit for each trainer (MAX_PYRAMID_TRAINERS)
  /*0xE2B*/                // u8 padding3;
...
struct ApprenticeQuestion {
  ...
  u8 monId : 2;
  u8 moveSlot : 2;
  u8 suggestedChange : 2; // TRUE if told to use held item or second move, FALSE
                          // if told to use no item or first move
  // u8 padding;
...
struct SaveBlock2 {
  ...
  u16 optionsWindowFrameType : 5; // Specifies one of the 20 decorative borders
                                  // for text boxes
  ...
  /*0x21C*/ struct RankingHall1P
      hallRecords1P[HALL_FACILITIES_COUNT][FRONTIER_LVL_MODE_COUNT]
                   [HALL_RECORDS_COUNT]; // From record mixing.
  /*0x57C*/ struct RankingHall2P
      hallRecords2P[FRONTIER_LVL_MODE_COUNT]
                   [HALL_RECORDS_COUNT]; // From record mixing.
...
struct SecretBase {
  ...
  /*0x1AA5*/ u8 trainerId[TRAINER_ID_LENGTH]; // byte 0 is used for determining
                                              // trainer class
  /*0x1AA9*/ u8 language;
...
struct WonderCardMetadata {
  ...
  u16 numTrades;
  u16 iconSpecies;
  u16 stampData[2][MAX_STAMP_CARD_STAMPS]; // First element is STAMP_SPECIES,
                                           // second is STAMP_ID
};
...
struct MysteryGiftSave {
  ...
  u32 trainerIds[2]
                [5]; // Saved ids for 10 trainers, 5 each for battles and trades
}; // 0x36C 0x3598

// For external event data storage. The majority of these may have never been
// used. In Emerald, the only known used fields are the PokeCoupon and BoxRS
// ones, but hacking the distribution discs allows Emerald to receive events and
// set the others
struct ExternalEventData {
  u8 unknownExternalDataFields1[7]; // if actually used, may be broken up into
                                    // different fields.
  u32 unknownExternalDataFields2 : 8;
  u32 currentPokeCoupons : 24; // PokéCoupons stored by Pokémon Colosseum and XD
                               // from Mt. Battle runs. Earned PokéCoupons are
                               // also added to totalEarnedPokeCoupons.
                               // Colosseum/XD caps this at 9,999,999, but will
                               // read up to 16,777,215.
  u32 gotGoldPokeCouponTitleReward : 1;   // Master Ball from JP Colosseum Bonus
                                          // Disc; for reaching 30,000
                                          // totalEarnedPokeCoupons
  u32 gotSilverPokeCouponTitleReward : 1; // Light Ball Pikachu from JP
                                          // Colosseum Bonus Disc; for reaching
                                          // 5000 totalEarnedPokeCoupons
  u32 gotBronzePokeCouponTitleReward : 1; // PP Max from JP Colosseum Bonus
                                          // Disc; for reaching 2500
                                          // totalEarnedPokeCoupons
  u32 receivedAgetoCelebi : 1;            // from JP Colosseum Bonus Disc
  u32 unknownExternalDataFields3 : 4;
  u32 totalEarnedPokeCoupons : 24;  // Used by the JP Colosseum bonus disc.
                                    // Determines PokéCoupon rank to distribute
                                    // rewards. Unread in International games.
                                    // Colosseum/XD caps this at 9,999,999.
  u8 unknownExternalDataFields4[5]; // if actually used, may be broken up into
                                    // different fields.
} __attribute__((packed));          /*size = 0x14*/

// For external event flags. The majority of these may have never been used.
// In Emerald, Jirachi cannot normally be received, but hacking the distribution
// discs allows Emerald to receive Jirachi and set the flag
struct ExternalEventFlags {
  u8 usedBoxRS : 1; // Set by Pokémon Box: Ruby & Sapphire; denotes whether this
                    // save has connected to it and triggered the free False
                    // Swipe Swablu Egg giveaway.
  u8 boxRSEggsUnlocked : 2; // Set by Pokémon Box: Ruby & Sapphire; denotes the
                            // number of Eggs unlocked from deposits; 1 for
                            // ExtremeSpeed Zigzagoon (at 100 deposited), 2 for
                            // Pay Day Skitty (at 500 deposited), 3 for Surf
                            // Pichu (at 1499 deposited)
  // u8 padding:5;
  u8 unknownFlag1;
  u8 receivedGCNJirachi; // Both the US Colosseum Bonus Disc and PAL/AUS Pokémon
                         // Channel use this field. One cannot receive a WISHMKR
                         // Jirachi and CHANNEL Jirachi with the same savefile.
  u8 unknownFlag3;
  u8 unknownFlag4;
  u8 unknownFlag5;
  u8 unknownFlag6;
...
struct SaveBlock1 {
  ...
  /*0xC70*/ struct ObjectEventTemplate
      objectEventTemplates[OBJECT_EVENT_TEMPLATES_COUNT];
  ...
  /*0x2E20*/ u8
      unlockedTrendySayings[NUM_TRENDY_SAYING_BYTES]; // Bitfield for unlockable
                                                      // Easy Chat words in
                                                      // EC_GROUP_TRENDY_SAYING
  /*0x2E25*/                                          // u8 padding5[3];
  /*0x2E28*/ OldMan oldMan;
  /*0x2e64*/ struct DewfordTrend dewfordTrends[SAVED_TRENDS_COUNT];
  /*0x2e90*/ struct ContestWinner
      contestWinners[NUM_CONTEST_WINNERS]; // see CONTEST_WINNER_*
