...
#define EC_WORD_ELLIPSIS_ELLIPSIS_ELLIPSIS                                     \
  ((EC_GROUP_VOICES << EC_MASK_BITS) | 6)
...
// If the Pokémon or Move group tables have an ID higher than 511, the word will
// be seen as invalid due to the index looping back to the beginning and not
// matching the proper value. To allow for higher values on those tables,
// increase the value of EC_MASK_BITS and the max value will increase to (2 ^
// EC_MASK_BITS). Keep in mind that existing saved words will be shifted.
...
#define EC_POKEMON_NATIONAL(mon)                                               \
  ((EC_GROUP_POKEMON_NATIONAL << EC_MASK_BITS) | SPECIES_##mon)
...
#define EC_WORD(group, index)                                                  \
  ((((group) & EC_MASK_GROUP) << EC_MASK_BITS) | ((index) & EC_MASK_INDEX))
