...
// temporary vars
// The first 0x10 vars are are temporary--they are cleared every time a map is
// loaded.
...
#define VAR_TEMP_3                                                             \
  (TEMP_VARS_START + 0x3) // Note: Used when the player checks a TV
...
// For example, the rival's sprite id is dynamically set based on the player's
// gender. See VarGetObjectEventGraphicsId().
...
#define VAR_DISTRIBUTE_EON_TICKET                                              \
  0x403F // This var is read and written, but is always zero. The only way to
         // obtain the Eon Ticket in Emerald is via Record Mixing
...
#define VAR_PETALBURG_GYM_STATE                                                \
  0x4085 // 0-1: Wally tutorial, 2-6: 0-4 badges, 7: Defeated Norman, 8: Rematch
         // Norman
...
#define VAR_SS_TIDAL_SCOTT_STATE                                               \
  0x40D4                          // Always equal to FLAG_MET_SCOTT_ON_SS_TIDAL
...
#define VAR_SECRET_BASE_IS_NOT_LOCAL                                           \
  0x40F0 // Set to TRUE while in another player's secret base.
...
// special vars
// They are commonly used as parameters to commands, or return values from
// commands.
...
#define VAR_TRAINER_BATTLE_OPPONENT_A                                          \
