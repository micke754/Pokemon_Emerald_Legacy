...
// Note: If moving ball IDs around, updating FIRST_BALL/LAST_BALL is not
// sufficient
//       Several places expect the ball IDs to be first and contiguous (e.g.
//       gBattlescriptsForBallThrow and MON_DATA_POKEBALL) If adding new balls,
//       it's easiest to insert them after the last ball and increment the below
//       IDs (and removing ITEM_034 for example)
...
// A special item id associated with "Cancel"/"Exit" etc. in a list of items or
// decorations Its icon is defined at ITEMS_COUNT as the "return to field" arrow
...
#define NUM_BERRY_MASTER_BERRIES                                               \
  (LAST_BERRY_MASTER_BERRY - FIRST_BERRY_MASTER_BERRY + 1)
#define NUM_BERRY_MASTER_BERRIES_SKIPPED                                       \
  (FIRST_BERRY_MASTER_BERRY - FIRST_BERRY_INDEX)
...
#define NUM_ROUTE_114_MAN_BERRIES                                              \
  (LAST_ROUTE_114_MAN_BERRY - FIRST_ROUTE_114_MAN_BERRY + 1)
#define NUM_ROUTE_114_MAN_BERRIES_SKIPPED                                      \
  (FIRST_ROUTE_114_MAN_BERRY - FIRST_BERRY_INDEX)
...
#define ITEM_HAS_EFFECT(item)                                                  \
  (((item) >= ITEM_POTION && (item) <= MAX_BERRY_INDEX) ||                     \
   (item) == ITEM_DRAGON_SCALE || (item) == ITEM_KINGS_ROCK ||                 \
   (item) == ITEM_DEEP_SEA_SCALE || (item) == ITEM_DEEP_SEA_TOOTH ||           \
   (item) == ITEM_METAL_COAT || (item) == ITEM_BRICK_PIECE ||                  \
   (item) == ITEM_UP_GRADE)
