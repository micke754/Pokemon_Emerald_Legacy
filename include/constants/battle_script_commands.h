...
// The following correspond to the struct members of BattleScripting by adding
// their offset
...
#define sPURSUIT_DOUBLES_ATTACKER                                              \
  (gBattleScripting + 0x20)                            // pursuitDoublesAttacker
...
#define sSPECIAL_TRAINER_BATTLE_TYPE                                           \
  (gBattleScripting + 0x26) // specialTrainerBattleType
...
// Cmd_statbuffchange
#define STAT_CHANGE_ALLOW_PTR                                                  \
  (1 << 0) // If set, allow use of jumpptr. Set in every use of statbuffchange
...
#define PARTY_SCREEN_OPTIONAL                                                  \
  (1 << 7) // Flag for first argument to openpartyscreen
