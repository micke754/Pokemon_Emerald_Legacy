...
// Sprites start at 10000 and thus must be subtracted of 10000 to account for
// the true index.
...
#define F_PAL_ANIM_1                                                           \
  (1 << 5) // Palette set for GetBattleAnimBg1Data/GetBgDataForTransform. Only
           // used (ineffectually?) by Aromatherapy.
#define F_PAL_ANIM_2                                                           \
  (1 << 6) // Palette set for GetBattleAnimBgData/GetBgDataForTransform. Unused.
...
// The below are only used by AnimTask_BlendBattleAnimPal to get battler sprite
// palettes by position rather than by role. It's redundant with F_PAL_BATTLERS,
// because they're only ever used together to refer to all the battlers at once.
