...
// In Gens 6 and 7, invalid versions instead show "a distant land" in the
// summary screen. In Gen 4 only, migrated Pokémon with Diamond, Pearl, or
// Platinum's ID show as "----------". Gen 5 and up read Diamond, Pearl, or
// Platinum's ID as "Sinnoh". In Gen 4 and up, migrated Pokémon with HeartGold
// or SoulSilver's ID show the otherwise unused "Johto" string.
...
#define LANGUAGE_KOREAN                                                        \
  6 // 6 goes unused but the theory is it was meant to be Korean
...
#define MAX_FRONTIER_PARTY_SIZE                                                \
  (max(FRONTIER_PARTY_SIZE,                                                    \
       max(FRONTIER_DOUBLES_PARTY_SIZE, FRONTIER_MULTI_PARTY_SIZE)))
...
// Excludes link modes. See RANKING_HALL_* in
...
#define FRONTIER_LVL_TENT                                                      \
  FRONTIER_LVL_MODE_COUNT // Special usage for indicating Battle Tent
...
#define POKEMON_NAME_BUFFER_SIZE                                               \
  max(20, POKEMON_NAME_LENGTH +                                                \
              1) // Frequently used buffer size. Larger than necessary
