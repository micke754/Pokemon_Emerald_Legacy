...
#define BATTLE_TYPE_LINK_IN_BATTLE                                             \
  (1 << 5) // Set on battle entry, cleared on exit. Checked rarely
#define BATTLE_TYPE_MULTI (1 << 6)
#define BATTLE_TYPE_SAFARI (1 << 7)
#define BATTLE_TYPE_BATTLE_TOWER (1 << 8)
#define BATTLE_TYPE_WALLY_TUTORIAL                                             \
  (1 << 9) // Used in pokefirered as BATTLE_TYPE_OLD_MAN_TUTORIAL.
#define BATTLE_TYPE_ROAMER (1 << 10)
#define BATTLE_TYPE_EREADER_TRAINER (1 << 11)
#define BATTLE_TYPE_KYOGRE_GROUDON (1 << 12)
#define BATTLE_TYPE_LEGENDARY (1 << 13)
#define BATTLE_TYPE_REGI (1 << 14)
#define BATTLE_TYPE_TWO_OPPONENTS                                              \
  (1 << 15) // Used in pokefirered as BATTLE_TYPE_GHOST.
#define BATTLE_TYPE_DOME                                                       \
  (1 << 16) // Used in pokefirered as BATTLE_TYPE_POKEDUDE.
#define BATTLE_TYPE_PALACE                                                     \
  (1 << 17) // Used in pokefirered as BATTLE_TYPE_WILD_SCRIPTED.
#define BATTLE_TYPE_ARENA                                                      \
  (1 << 18) // Used in pokefirered as BATTLE_TYPE_LEGENDARY_FRLG.
#define BATTLE_TYPE_FACTORY                                                    \
  (1 << 19) // Used in pokefirered as BATTLE_TYPE_TRAINER_TOWER.
...
#define BATTLE_TYPE_FRONTIER_NO_PYRAMID                                        \
  (BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_DOME | BATTLE_TYPE_PALACE |          \
   BATTLE_TYPE_ARENA | BATTLE_TYPE_FACTORY | BATTLE_TYPE_PIKE)
#define BATTLE_TYPE_RECORDED_INVALID                                           \
  ((BATTLE_TYPE_LINK | BATTLE_TYPE_SAFARI | BATTLE_TYPE_FIRST_BATTLE |         \
    BATTLE_TYPE_WALLY_TUTORIAL | BATTLE_TYPE_ROAMER |                          \
    BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_KYOGRE_GROUDON |                 \
    BATTLE_TYPE_LEGENDARY | BATTLE_TYPE_REGI | BATTLE_TYPE_RECORDED |          \
    BATTLE_TYPE_TRAINER_HILL | BATTLE_TYPE_SECRET_BASE | BATTLE_TYPE_GROUDON | \
    BATTLE_TYPE_KYOGRE | BATTLE_TYPE_RAYQUAZA))
...
#define STATUS1_SLEEP                                                          \
  (1 << 0 | 1 << 1 | 1 << 2) // First 3 bits (Number of turns to sleep)
#define STATUS1_SLEEP_TURN(num)                                                \
  ((num) << 0) // Just for readability (or if rearranging statuses)
...
#define STATUS1_ANY                                                            \
  (STATUS1_SLEEP | STATUS1_POISON | STATUS1_BURN | STATUS1_FREEZE |            \
   STATUS1_PARALYSIS | STATUS1_TOXIC_POISON)
...
#define STATUS2_INFATUATION                                                    \
  (1 << 16 | 1 << 17 | 1 << 18 | 1 << 19) // 4 bits, one for every battler
...
#define STATUS3_LEECHSEED_BATTLER                                              \
  (1 << 0 | 1 << 1) // The battler to receive HP from Leech Seed
#define STATUS3_LEECHSEED (1 << 2)
#define STATUS3_ALWAYS_HITS (1 << 3 | 1 << 4)
#define STATUS3_ALWAYS_HITS_TURN(num)                                          \
  (((num) << 3) &                                                              \
   STATUS3_ALWAYS_HITS) // "Always Hits" is set as a 2 turn timer, i.e. next
                        // turn is the last turn when it's active
...
#define STATUS3_SEMI_INVULNERABLE                                              \
  (STATUS3_UNDERGROUND | STATUS3_ON_AIR | STATUS3_UNDERWATER)

// Not really sure what a "hitmarker" is.
#define HITMARKER_WAKE_UP_CLEAR                                                \
  (1 << 4) // Cleared when waking up. Never set or checked.
...
#define HITMARKER_NEVER_SET                                                    \
  (1 << 26) // Cleared as part of a large group. Never set or checked
...
#define B_WEATHER_RAIN                                                         \
  (B_WEATHER_RAIN_TEMPORARY | B_WEATHER_RAIN_DOWNPOUR |                        \
   B_WEATHER_RAIN_PERMANENT)
#define B_WEATHER_SANDSTORM_TEMPORARY (1 << 3)
#define B_WEATHER_SANDSTORM_PERMANENT (1 << 4)
#define B_WEATHER_SANDSTORM                                                    \
  (B_WEATHER_SANDSTORM_TEMPORARY | B_WEATHER_SANDSTORM_PERMANENT)
...
#define B_WEATHER_ANY                                                          \
  (B_WEATHER_RAIN | B_WEATHER_SANDSTORM | B_WEATHER_SUN | B_WEATHER_HAIL)
...
#define PRIMARY_STATUS_MOVE_EFFECT                                             \
  MOVE_EFFECT_TOXIC // All above move effects apply primary status
...
// The following are duplicate id values for windows that Battle Arena uses
// differently.
