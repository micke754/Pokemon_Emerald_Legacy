...
void PutMemBlockHeader(void *block, struct MemBlock *prev,
                       struct MemBlock *next, u32 size) {
...
void PutFirstMemBlockHeader(void *block, u32 size) {
  PutMemBlockHeader(block, (struct MemBlock *)block, (struct MemBlock *)block,
...
void FreeInternal(void *heapStart, void *pointer) {
  if (pointer) {
    ...
    struct MemBlock *block =
        (struct MemBlock *)((u8 *)pointer - sizeof(struct MemBlock));
    block->flag = FALSE;
...
bool32 CheckMemBlockInternal(void *heapStart, void *pointer) {
  ...
  if (block->next != head &&
      block->next != (struct MemBlock *)(block->data + block->size))
    return FALSE;
...
void *Alloc(u32 size) { return AllocInternal(sHeapStart, size); }

void *AllocZeroed(u32 size) { return AllocZeroedInternal(sHeapStart, size); }

void Free(void *pointer) { FreeInternal(sHeapStart, pointer); }
