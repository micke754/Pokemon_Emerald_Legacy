...
#define SPRITE_TILE_IS_ALLOCATED(n)                                            \
  ((sSpriteTileAllocBitmap[(n) / 8] >> ((n) % 8)) & 1)
...
static u8 CreateSpriteAt(u8 index, const struct SpriteTemplate *template, s16 x,
                         s16 y, u8 subpriority);
static void ResetOamMatrices(void);
static void ResetSprite(struct Sprite *sprite);
static s16 AllocSpriteTiles(u16 tileCount);
static void RequestSpriteFrameImageCopy(u16 index, u16 tileNum,
...
static void ApplyAffineAnimFrameAbsolute(u8 matrixNum,
                                         struct AffineAnimFrameCmd *frameCmd);
static void DecrementAnimDelayCounter(struct Sprite *sprite);
static bool8 DecrementAffineAnimDelayCounter(struct Sprite *sprite,
                                             u8 matrixNum);
...
static void GetAffineAnimFrame(u8 matrixNum, struct Sprite *sprite,
                               struct AffineAnimFrameCmd *frameCmd);
static void ApplyAffineAnimFrame(u8 matrixNum,
                                 struct AffineAnimFrameCmd *frameCmd);
...
void AddSpritesToOamBuffer(void) {
  ...
  while (i < MAX_SPRITES) {
    ...
    if (sprite->inUse && !sprite->invisible &&
        AddSpriteToOamBuffer(sprite, &oamIndex))
      return;
    i++;
  }
...
u8 CreateSprite(const struct SpriteTemplate *template, s16 x, s16 y,
                u8 subpriority) {
  u8 i;
...
u8 CreateSpriteAtEnd(const struct SpriteTemplate *template, s16 x, s16 y,
                     u8 subpriority) {
  s16 i;
...
u8 CreateSpriteAt(u8 index, const struct SpriteTemplate *template, s16 x, s16 y,
                  u8 subpriority) {
...
u8 CreateSpriteAndAnimate(const struct SpriteTemplate *template, s16 x, s16 y,
                          u8 subpriority) {
  u8 i;
...
void DestroySprite(struct Sprite *sprite) {
  if (sprite->inUse) {
    if (!sprite->usingSheet) {
      u16 i;
      u16 tileEnd =
          (sprite->images->size / TILE_SIZE_4BPP) + sprite->oam.tileNum;
...
void ResetSprite(struct Sprite *sprite) { *sprite = sDummySprite; }

void CalcCenterToCornerVec(struct Sprite *sprite, u8 shape, u8 size,
                           u8 affineMode) {
...
void ProcessSpriteCopyRequests(void) {
  if (sShouldProcessSpriteCopyRequests) {
    ...
    while (sSpriteCopyRequestCount > 0) {
      CpuCopy16(sSpriteCopyRequests[i].src, sSpriteCopyRequests[i].dest,
...
void RequestSpriteFrameImageCopy(u16 index, u16 tileNum,
                                 const struct SpriteFrameImage *images) {
...
void BeginAnim(struct Sprite *sprite) {
  ...
  if (imageValue != -1) {
    ...
    if (sprite->usingSheet)
      ...
      RequestSpriteFrameImageCopy(imageValue, sprite->oam.tileNum,
                                  sprite->images);
  }
}
...
void AnimCmd_jump(struct Sprite *sprite) {
  ...
  sprite->animCmdIndex =
      sprite->anims[sprite->animNum][sprite->animCmdIndex].jump.target;
...
void BeginAnimLoop(struct Sprite *sprite) {
  sprite->animLoopCounter =
      sprite->anims[sprite->animNum][sprite->animCmdIndex].loop.count;
...
void JumpToTopOfAnimLoop(struct Sprite *sprite) {
  if (sprite->animLoopCounter) {
    ...
    while (sprite->anims[sprite->animNum][sprite->animCmdIndex - 1].type !=
           -3) {
...
void BeginAffineAnim(struct Sprite *sprite) {
  if ((sprite->oam.affineMode & ST_OAM_AFFINE_ON_MASK) &&
      sprite->affineAnims[0][0].type != 32767) {
    struct AffineAnimFrameCmd frameCmd;
...
// Only used by the minigame countdown, so that for instance the numbers don't
// slide up as they squish down before jumping.
...
void ApplyAffineAnimFrameAbsolute(u8 matrixNum,
                                  struct AffineAnimFrameCmd *frameCmd) {
...
void ApplyAffineAnimFrameRelativeAndUpdateMatrix(
    u8 matrixNum, struct AffineAnimFrameCmd *frameCmd) {
...
void GetAffineAnimFrame(u8 matrixNum, struct Sprite *sprite,
                        struct AffineAnimFrameCmd *frameCmd) {
  frameCmd->xScale =
      sprite
          ->affineAnims[sAffineAnimStates[matrixNum].animNum]
...
void SetOamMatrixRotationScaling(u8 matrixNum, s16 xScale, s16 yScale,
                                 u16 rotation) {
  struct ObjAffineSrcData srcData;
  struct OamMatrix matrix;
...
u16 LoadSpriteSheet(const struct SpriteSheet *sheet) {
  ...
  if (tileStart < 0) {
    ...
    AllocSpriteTileRange(sheet->tag, (u16)tileStart,
                         sheet->size / TILE_SIZE_4BPP);
    CpuCopy16(sheet->data, (u8 *)OBJ_VRAM0 + TILE_SIZE_4BPP * tileStart,
              sheet->size);
...
void SetSubspriteTables(struct Sprite *sprite,
...
bool8 AddSpriteToOamBuffer(struct Sprite *sprite, u8 *oamIndex) {
  ...
  if (!sprite->subspriteTables || sprite->subspriteMode == SUBSPRITES_OFF) {
    ...
    return AddSubspritesToOamBuffer(sprite, &gMain.oamBuffer[*oamIndex],
                                    oamIndex);
  }
}

bool8 AddSubspritesToOamBuffer(struct Sprite *sprite, struct OamData *destOam,
                               u8 *oamIndex) {
