...
u8 GetBgMode(void) { return sGpuBgConfigs.bgVisibilityAndMode & 0x7; }
...
static void SetBgControlAttributes(u8 bg, u8 charBaseIndex, u8 mapBaseIndex,
                                   u8 screenSize, u8 paletteMode, u8 priority,
...
static void SetBgAffineInternal(u8 bg, s32 srcCenterX, s32 srcCenterY,
                                s16 dispCenterX, s16 dispCenterY, s16 scaleX,
...
int BgTileAllocOp(int bg, int offset, int count, int mode) { return 0; }
...
void InitBgsFromTemplates(u8 bgMode, const struct BgTemplate *templates,
                          u8 numTemplates) {
  int i;
  u8 bg;
  ...
  for (i = 0; i < numTemplates; i++) {
    bg = templates[i].bg;
    if (bg < NUM_BACKGROUNDS) {
      SetBgControlAttributes(bg, templates[i].charBaseIndex,
                             templates[i].mapBaseIndex, templates[i].screenSize,
                             templates[i].paletteMode, templates[i].priority, 0,
                             0);
...
void InitBgFromTemplate(const struct BgTemplate *template) {
  ...
  if (bg < NUM_BACKGROUNDS) {
    SetBgControlAttributes(bg, template->charBaseIndex, template->mapBaseIndex,
                           template->screenSize, template->paletteMode,
...
void SetBgMode(u8 bgMode) { SetBgModeInternal(bgMode); }
...
u16 Unused_LoadBgPalette(u8 bg, const void *src, u16 size, u16 destOffset) {
  ...
  if (!IsInvalidBg32(bg)) {
    u16 paletteOffset =
        PLTT_OFFSET_4BPP(sGpuBgConfigs2[bg].basePalette) + (destOffset * 2);
...
void SetBgAffine(u8 bg, s32 srcCenterX, s32 srcCenterY, s16 dispCenterX,
                 s16 dispCenterY, s16 scaleX, s16 scaleY, u16 rotationAngle) {
  SetBgAffineInternal(bg, srcCenterX, srcCenterY, dispCenterX, dispCenterY,
...
void CopyToBgTilemapBuffer(u8 bg, const void *src, u16 mode, u16 destOffset) {
  if (!IsInvalidBg32(bg) && !IsTileMapOutsideWram(bg)) {
    if (mode != 0)
      CpuCopy16(src, (void *)(sGpuBgConfigs2[bg].tilemap + (destOffset * 2)),
                mode);
    else
      LZ77UnCompWram(src,
                     (void *)(sGpuBgConfigs2[bg].tilemap + (destOffset * 2)));
  }
}
...
void CopyToBgTilemapBufferRect(u8 bg, const void *src, u8 destX, u8 destY,
                               u8 width, u8 height) {
  u16 destX16;
  u16 destY16;
  u16 mode;
...
void CopyToBgTilemapBufferRect_ChangePalette(u8 bg, const void *src, u8 destX,
                                             u8 destY, u8 rectWidth,
                                             u8 rectHeight, u8 palette) {
  CopyRectToBgTilemapBufferRect(bg, src, 0, 0, rectWidth, rectHeight, destX,
                                destY, rectWidth, rectHeight, palette, 0, 0);
}

void CopyRectToBgTilemapBufferRect(u8 bg, const void *src, u8 srcX, u8 srcY,
                                   u8 srcWidth, u8 srcHeight, u8 destX,
                                   u8 destY, u8 rectWidth, u8 rectHeight,
                                   u8 palette1, s16 tileOffset, s16 palette2) {
    ...
    case BG_TYPE_NORMAL:
      ...
      for (i = destY; i < (destY + rectHeight); i++) {
        for (j = destX; j < (destX + rectWidth); j++) {
          u16 index = GetTileMapIndexFromCoords(j, i, screenSize, screenWidth,
                                                screenHeight);
          CopyTileMapEntry(srcPtr, sGpuBgConfigs2[bg].tilemap + (index * 2),
                           palette1, tileOffset, palette2);
          srcPtr += 2;
        }
...
void FillBgTilemapBufferRect_Palette0(u8 bg, u16 tileNum, u8 x, u8 y, u8 width,
                                      u8 height) {
  u16 x16;
  u16 y16;
  u16 mode;
...
void FillBgTilemapBufferRect(u8 bg, u16 tileNum, u8 x, u8 y, u8 width,
                             u8 height, u8 palette) {
  WriteSequenceToBgTilemapBuffer(bg, tileNum, x, y, width, height, palette, 0);
}

void WriteSequenceToBgTilemapBuffer(u8 bg, u16 firstTileNum, u8 x, u8 y,
      ...
      for (y16 = y; y16 < (y + height); y16++) {
        for (x16 = x; x16 < (x + width); x16++) {
          CopyTileMapEntry(&firstTileNum,
                           &((u16 *)sGpuBgConfigs2[bg]
                                 .tilemap)[(u16)GetTileMapIndexFromCoords(
                               x16, y16, attribute, mode, mode2)],
                           paletteSlot, 0, 0);
          firstTileNum =
              (firstTileNum & 0xFC00) + ((firstTileNum + tileNumDelta) & 0x3FF);
        }
      }
      break;
    case BG_TYPE_AFFINE:
      ...
      for (y16 = y; y16 < (y + height); y16++) {
        for (x16 = x; x16 < (x + width); x16++) {
          ((u8 *)sGpuBgConfigs2[bg].tilemap)[(y16 * mode3) + x16] =
              firstTileNum;
          firstTileNum =
              (firstTileNum & 0xFC00) + ((firstTileNum + tileNumDelta) & 0x3FF);
        }
...
u32 GetTileMapIndexFromCoords(s32 x, s32 y, s32 screenSize, u32 screenWidth,
                              u32 screenHeight) {
  x = x & (screenWidth - 1);
  y = y & (screenHeight - 1);
...
void CopyTileMapEntry(const u16 *src, u16 *dest, s32 palette1, s32 tileOffset,
                      s32 palette2) {
  u16 var;
