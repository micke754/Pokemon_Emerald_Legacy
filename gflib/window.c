...
bool16 InitWindows(const struct WindowTemplate *templates) {
  ...
  for (i = 0, allocatedBaseBlock = 0, bgLayer = templates[i].bg;
       bgLayer != 0xFF && i < WINDOWS_MAX; ++i, bgLayer = templates[i].bg) {
    if (gWindowTileAutoAllocEnabled == TRUE) {
      allocatedBaseBlock = BgTileAllocOp(
    ...
    allocatedTilemapBuffer =
        AllocZeroed((u16)(32 * (templates[i].width * templates[i].height)));

    if (allocatedTilemapBuffer == NULL) {
      if ((GetNumActiveWindowsOnBg(bgLayer) == 0) &&
          (gWindowBgTilemapBuffers[bgLayer] != DummyWindowBgTilemap)) {
    ...
    if (gWindowTileAutoAllocEnabled == TRUE) {
      ...
      BgTileAllocOp(bgLayer, allocatedBaseBlock,
...
u16 AddWindow(const struct WindowTemplate *template) {
  ...
  if (gWindowTileAutoAllocEnabled == TRUE) {
    allocatedBaseBlock =
        BgTileAllocOp(bgLayer, 0, template->width * template->height, 0);
  ...
  allocatedTilemapBuffer =
      AllocZeroed((u16)(32 * (template->width * template->height)));
...
void RemoveWindow(u8 windowId) {
  ...
  if (gWindowTileAutoAllocEnabled == TRUE)
    BgTileAllocOp(
        bgLayer, gWindows[windowId].window.baseBlock,
...
void FreeAllWindowBuffers(void) {
  ...
  for (i = 0; i < NUM_BACKGROUNDS; ++i) {
    if (gWindowBgTilemapBuffers[i] != NULL &&
        gWindowBgTilemapBuffers[i] != DummyWindowBgTilemap) {
...
void CopyWindowToVram(u8 windowId, u8 mode) {
  ...
  case COPYWIN_MAP:
    ...
    break;
  case COPYWIN_GFX:
    LoadBgTiles(windowLocal.window.bg, windowLocal.tileData, windowSize,
...
void CopyWindowRectToVram(u32 windowId, u32 mode, u32 x, u32 y, u32 w, u32 h) {
  ...
  if (w != 0 && h != 0) {
    ...
    case COPYWIN_MAP:
      ...
      break;
    case COPYWIN_GFX:
      LoadBgTiles(windowLocal.window.bg, windowLocal.tileData + (rectPos * 32),
                  rectSize, windowLocal.window.baseBlock + rectPos);
      break;
    case COPYWIN_FULL:
      LoadBgTiles(windowLocal.window.bg, windowLocal.tileData + (rectPos * 32),
                  rectSize, windowLocal.window.baseBlock + rectPos);
...
void PutWindowTilemap(u8 windowId) {
  ...
  WriteSequenceToBgTilemapBuffer(
      ...
      GetBgAttribute(windowLocal.window.bg, BG_ATTR_BASETILE) +
          windowLocal.window.baseBlock,
      windowLocal.window.tilemapLeft, windowLocal.window.tilemapTop,
      windowLocal.window.width, windowLocal.window.height,
      windowLocal.window.paletteNum, 1);
}

void PutWindowRectTilemapOverridePalette(u8 windowId, u8 x, u8 y, u8 width,
                                         u8 height, u8 palette) {
  struct Window windowLocal = gWindows[windowId];
  ...
  for (i = 0; i < height; ++i) {
    WriteSequenceToBgTilemapBuffer(
        windowLocal.window.bg, currentRow, windowLocal.window.tilemapLeft + x,
...
void ClearWindowTilemap(u8 windowId) {
  ...
  FillBgTilemapBufferRect(windowLocal.window.bg, gTransparentTileNumber,
...
void PutWindowRectTilemap(u8 windowId, u8 x, u8 y, u8 width, u8 height) {
  ...
  for (i = 0; i < height; ++i) {
    WriteSequenceToBgTilemapBuffer(windowLocal.window.bg, currentRow,
                                   windowLocal.window.tilemapLeft + x,
                                   windowLocal.window.tilemapTop + y + i, width,
                                   1, windowLocal.window.paletteNum, 1);

    currentRow += windowLocal.window.width;
  }
}

void BlitBitmapToWindow(u8 windowId, const u8 *pixels, u16 x, u16 y, u16 width,
                        u16 height) {
  BlitBitmapRectToWindow(windowId, pixels, 0, 0, width, height, x, y, width,
                         height);
}

void BlitBitmapRectToWindow(u8 windowId, const u8 *pixels, u16 srcX, u16 srcY,
                            u16 srcWidth, int srcHeight, u16 destX, u16 destY,
  ...
  BlitBitmapRect4Bit(&sourceRect, &destRect, srcX, srcY, destX, destY,
                     rectWidth, rectHeight, 0);
}

static void UNUSED BlitBitmapRectToWindowWithColorKey(
    u8 windowId, const u8 *pixels, u16 srcX, u16 srcY, u16 srcWidth,
  ...
  BlitBitmapRect4Bit(&sourceRect, &destRect, srcX, srcY, destX, destY,
                     rectWidth, rectHeight, colorKey);
}

void FillWindowPixelRect(u8 windowId, u8 fillValue, u16 x, u16 y, u16 width,
                         u16 height) {
...
void CopyToWindowPixelBuffer(u8 windowId, const void *src, u16 size,
                             u16 tileOffset) {
  if (size != 0)
...
#define MOVE_TILES_DOWN(a)                                                     \
  {                                                                            \
    destOffset = i + (a);                                                      \
    srcOffset =                                                                \
        i + (((width * (distanceLoop & ~7)) | (distanceLoop & 7)) * 4);        \
    if (srcOffset < size)                                                      \
...
void ScrollWindow(u8 windowId, u8 direction, u8 distance, u8 fillValue) {
  ...
  u32 fillValue32 =
      (fillValue << 24) | (fillValue << 16) | (fillValue << 8) | fillValue;
...
void CallWindowFunction(u8 windowId, void (*func)(u8, u8, u8, u8, u8, u8)) {
  struct WindowTemplate window = gWindows[windowId].window;
  func(window.bg, window.tilemapLeft, window.tilemapTop, window.width,
       window.height, window.paletteNum);
}
...
u16 AddWindow8Bit(const struct WindowTemplate *template) {
  ...
  if (gWindowBgTilemapBuffers[bgLayer] == NULL) {
    ...
    if (attribute != 0xFFFF) {
      ...
      if (memAddress == NULL)
        return WINDOW_NONE;
      for (i = 0; i < attribute; i++) // if we're going to zero out the memory
                                      // anyway, why not call AllocZeroed?
        memAddress[i] = 0;
  ...
  if (memAddress == NULL) {
    if (GetNumActiveWindowsOnBg8Bit(bgLayer) == 0 &&
        gWindowBgTilemapBuffers[bgLayer] != DummyWindowBgTilemap8Bit) {
      Free(gWindowBgTilemapBuffers[bgLayer]);
      gWindowBgTilemapBuffers[bgLayer] = NULL;
    }
    return WINDOW_NONE;
...
void FillWindowPixelBuffer8Bit(u8 windowId, u8 fillValue) {
  ...
  size = (u16)(64 * (gWindows[windowId].window.width *
                     gWindows[windowId].window.height));
  for (i = 0; i < size; i++)
    gWindows[windowId].tileData[i] = fillValue;
}

void FillWindowPixelRect8Bit(u8 windowId, u8 fillValue, u16 x, u16 y, u16 width,
                             u16 height) {
  struct Bitmap pixelRect;
...
void BlitBitmapRectToWindow4BitTo8Bit(u8 windowId, const u8 *pixels, u16 srcX,
                                      u16 srcY, u16 srcWidth, int srcHeight,
                                      u16 destX, u16 destY, u16 rectWidth,
                                      u16 rectHeight, u8 paletteNum) {
  struct Bitmap sourceRect;
  struct Bitmap destRect;
  ...
  BlitBitmapRect4BitTo8Bit(&sourceRect, &destRect, srcX, srcY, destX, destY,
                           rectWidth, rectHeight, 0, paletteNum);
}
...
void CopyWindowToVram8Bit(u8 windowId, u8 mode) {
  ...
  case COPYWIN_MAP:
    ...
    break;
  case COPYWIN_GFX:
    LoadBgTiles(sWindowPtr->window.bg, sWindowPtr->tileData, sWindowSize,
